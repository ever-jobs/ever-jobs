...
import { BuildOpsModule, BuildOpsService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'buildops-jobs.json'), 'utf8'),
);
...
/**
 * Spec 819 / T04 — `BuildOpsService` unit tests (≥ 9 cases).
 */
describe('BuildOpsService — Spec 819 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through BuildOpsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [BuildOpsModule],
      }).compile();
      const service = moduleRef.get(BuildOpsService);
      expect(service).toBeInstanceOf(BuildOpsService);
      await moduleRef.close();
    });

    it('exports the Site.BUILDOPS = "buildops" enum value', () => {
      expect(Site.BUILDOPS).toBe('buildops');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new BuildOpsService();
      const result = await service.scrape({
        siteType: [Site.BUILDOPS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'buildops-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.BUILDOPS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('BuildOps');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/buildops/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/buildops/jobs/');
      ...
      expect(job0?.description).toContain('BuildOps');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/buildops/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('BuildOps');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'buildops-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
