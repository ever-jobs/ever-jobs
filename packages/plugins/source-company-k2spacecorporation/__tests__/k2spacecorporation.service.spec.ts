...
import { K2SpaceModule, K2SpaceService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'k2spacecorporation-jobs.json'), 'utf8'),
);
...
/**
 * Spec 874 / T04 — `K2SpaceService` unit tests (≥ 9 cases).
 */
describe('K2SpaceService — Spec 874 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through K2SpaceModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [K2SpaceModule],
      }).compile();
      const service = moduleRef.get(K2SpaceService);
      expect(service).toBeInstanceOf(K2SpaceService);
      await moduleRef.close();
    });

    it('exports the Site.K2_SPACE = "k2spacecorporation" enum value', () => {
      expect(Site.K2_SPACE).toBe('k2spacecorporation');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new K2SpaceService();
      const result = await service.scrape({
        siteType: [Site.K2_SPACE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'k2spacecorporation-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.K2_SPACE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('K2 Space');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/k2spacecorporation/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/k2spacecorporation/jobs/');
      ...
      expect(job0?.description).toContain('K2 Space');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/k2spacecorporation/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('K2 Space');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'k2spacecorporation-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
