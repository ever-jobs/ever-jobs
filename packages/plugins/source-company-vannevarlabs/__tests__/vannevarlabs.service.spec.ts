...
import { VannevarlabsModule, VannevarlabsService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'vannevarlabs-jobs.json'), 'utf8'),
);
...
/**
 * Spec 557 / T04 — `VannevarlabsService` unit tests (≥ 9 cases).
 */
describe('VannevarlabsService — Spec 557 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through VannevarlabsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [VannevarlabsModule],
      }).compile();
      const service = moduleRef.get(VannevarlabsService);
      expect(service).toBeInstanceOf(VannevarlabsService);
      await moduleRef.close();
    });

    it('exports the Site.VANNEVAR_LABS = "vannevarlabs" enum value', () => {
      expect(Site.VANNEVAR_LABS).toBe('vannevarlabs');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new VannevarlabsService();
      const result = await service.scrape({
        siteType: [Site.VANNEVAR_LABS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'vannevarlabs-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.VANNEVAR_LABS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Vannevar Labs');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/vannevarlabs/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/vannevarlabs/jobs/');
      ...
      expect(job0?.description).toContain('Vannevar Labs');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/vannevarlabs/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Vannevar Labs');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'vannevarlabs-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
