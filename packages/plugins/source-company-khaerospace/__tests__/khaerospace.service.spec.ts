...
import { KHAerospaceModule, KHAerospaceService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'khaerospace-jobs.json'), 'utf8'),
);
...
/**
 * Spec 876 / T04 — `KHAerospaceService` unit tests (≥ 9 cases).
 */
describe('KHAerospaceService — Spec 876 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through KHAerospaceModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [KHAerospaceModule],
      }).compile();
      const service = moduleRef.get(KHAerospaceService);
      expect(service).toBeInstanceOf(KHAerospaceService);
      await moduleRef.close();
    });

    it('exports the Site.KH_AEROSPACE = "khaerospace" enum value', () => {
      expect(Site.KH_AEROSPACE).toBe('khaerospace');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new KHAerospaceService();
      const result = await service.scrape({
        siteType: [Site.KH_AEROSPACE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'khaerospace-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.KH_AEROSPACE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('KH Aerospace');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/khaerospace/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/khaerospace/jobs/');
      ...
      expect(job0?.description).toContain('KH Aerospace');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/khaerospace/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('KH Aerospace');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'khaerospace-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
