...
import { SprinterHealthModule, SprinterHealthService } from '../src';

const COMPANY_NAME_EXPECT = 'Sprinter Health';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'sprinterhealth-jobs.json'), 'utf8'),
);
...
describe('SprinterHealthService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SprinterHealthModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SprinterHealthModule],
      }).compile();
      const service = moduleRef.get(SprinterHealthService);
      expect(service).toBeInstanceOf(SprinterHealthService);
      await moduleRef.close();
    });

    it('exports the Site.SPRINTER_HEALTH = "sprinterhealth" enum value', () => {
      expect(Site.SPRINTER_HEALTH).toBe('sprinterhealth');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new SprinterHealthService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.SPRINTER_HEALTH],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'sprinterhealth-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.SPRINTER_HEALTH);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('sprinterhealth-' + first.id);
      ...
      expect(calledUrls[0]).toContain('sprinter-health');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SPRINTER_HEALTH);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('sprinterhealth-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new SprinterHealthService(registry);
      const result = await service.scrape({
        siteType: [Site.SPRINTER_HEALTH],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('sprinter-health');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('sprinterhealth-x1');
      expect(result.jobs[0].site).toBe(Site.SPRINTER_HEALTH);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('sprinterhealth-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new SprinterHealthService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.SPRINTER_HEALTH],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new SprinterHealthService();
      const result = await service.scrape({
        siteType: [Site.SPRINTER_HEALTH],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
