...
import { DutchieModule, DutchieService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'thedutchie-jobs.json'), 'utf8'),
);
...
/**
 * Spec 953 / T04 — `DutchieService` unit tests (≥ 9 cases).
 */
describe('DutchieService — Spec 953 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through DutchieModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [DutchieModule],
      }).compile();
      const service = moduleRef.get(DutchieService);
      expect(service).toBeInstanceOf(DutchieService);
      await moduleRef.close();
    });

    it('exports the Site.DUTCHIE = "thedutchie" enum value', () => {
      expect(Site.DUTCHIE).toBe('thedutchie');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new DutchieService();
      const result = await service.scrape({
        siteType: [Site.DUTCHIE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'thedutchie-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.DUTCHIE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Dutchie');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/thedutchie/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/thedutchie/jobs/');
      ...
      expect(job0?.description).toContain('Dutchie');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/thedutchie/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Dutchie');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'thedutchie-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
