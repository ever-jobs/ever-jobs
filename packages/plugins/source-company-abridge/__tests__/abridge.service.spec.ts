...
import { AbridgeModule, AbridgeService } from '../src';

const COMPANY_NAME_EXPECT = 'Abridge';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'abridge-jobs.json'), 'utf8'),
);
...
describe('AbridgeService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AbridgeModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AbridgeModule],
      }).compile();
      const service = moduleRef.get(AbridgeService);
      expect(service).toBeInstanceOf(AbridgeService);
      await moduleRef.close();
    });

    it('exports the Site.ABRIDGE = "abridge" enum value', () => {
      expect(Site.ABRIDGE).toBe('abridge');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new AbridgeService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.ABRIDGE],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('abridge');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ABRIDGE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('abridge-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new AbridgeService(registry);
      const result = await service.scrape({
        siteType: [Site.ABRIDGE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('abridge');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('abridge-x1');
      expect(result.jobs[0].site).toBe(Site.ABRIDGE);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('abridge-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new AbridgeService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.ABRIDGE],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new AbridgeService();
      const result = await service.scrape({
        siteType: [Site.ABRIDGE],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
