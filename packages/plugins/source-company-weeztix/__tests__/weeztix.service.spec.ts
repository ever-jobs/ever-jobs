...
import { WeeztixModule, WeeztixService } from '../src';

const COMPANY_NAME_EXPECT = 'Weeztix';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'weeztix-jobs.json'), 'utf8'),
);
...
describe('WeeztixService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through WeeztixModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [WeeztixModule],
      }).compile();
      const service = moduleRef.get(WeeztixService);
      expect(service).toBeInstanceOf(WeeztixService);
      await moduleRef.close();
    });

    it('exports the Site.WEEZTIX = "weeztix" enum value', () => {
      expect(Site.WEEZTIX).toBe('weeztix');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new WeeztixService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.WEEZTIX],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('weeztix.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.WEEZTIX);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('weeztix-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new WeeztixService(registry);
      const result = await service.scrape({
        siteType: [Site.WEEZTIX],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('weeztix');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('weeztix-x1');
      expect(result.jobs[0].site).toBe(Site.WEEZTIX);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('weeztix-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new WeeztixService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.WEEZTIX],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new WeeztixService();
      const result = await service.scrape({
        siteType: [Site.WEEZTIX],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
