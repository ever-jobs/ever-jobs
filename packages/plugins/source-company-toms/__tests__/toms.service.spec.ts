...
import { TOMSModule, TOMSService } from '../src';

const COMPANY_NAME_EXPECT = 'TOMS';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'toms-jobs.json'), 'utf8'),
);
...
describe('TOMSService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through TOMSModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [TOMSModule],
      }).compile();
      const service = moduleRef.get(TOMSService);
      expect(service).toBeInstanceOf(TOMSService);
      await moduleRef.close();
    });

    it('exports the Site.TOMS = "toms" enum value', () => {
      expect(Site.TOMS).toBe('toms');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new TOMSService(registryWithAshby());
      ...
      expect(calledUrls[0]).toContain('toms');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.TOMS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('toms-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new TOMSService(registry);
      const result = await service.scrape({
        siteType: [Site.TOMS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('toms');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('toms-x1');
      expect(result.jobs[0].site).toBe(Site.TOMS);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('toms-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new TOMSService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.TOMS],
      } as ScraperInputDto);
