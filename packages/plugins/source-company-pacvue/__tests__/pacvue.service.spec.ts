...
import { PacvueModule, PacvueService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'pacvue-jobs.json'), 'utf8'),
);
...
/**
 * Spec 906 / T04 — `PacvueService` unit tests (≥ 9 cases).
 */
describe('PacvueService — Spec 906 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through PacvueModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [PacvueModule],
      }).compile();
      const service = moduleRef.get(PacvueService);
      expect(service).toBeInstanceOf(PacvueService);
      await moduleRef.close();
    });

    it('exports the Site.PACVUE = "pacvue" enum value', () => {
      expect(Site.PACVUE).toBe('pacvue');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new PacvueService();
      const result = await service.scrape({
        siteType: [Site.PACVUE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'pacvue-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.PACVUE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Pacvue');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/pacvue/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/pacvue/jobs/');
      ...
      expect(job0?.description).toContain('Pacvue');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/pacvue/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Pacvue');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'pacvue-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
