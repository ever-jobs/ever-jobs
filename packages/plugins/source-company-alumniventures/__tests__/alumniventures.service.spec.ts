...
import { AlumniventuresModule, AlumniventuresService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'alumniventures-jobs.json'), 'utf8'),
);
...
/**
 * Spec 252 / T04 — `AlumniventuresService` unit tests (≥ 9 cases).
 */
describe('AlumniventuresService — Spec 252 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AlumniventuresModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AlumniventuresModule],
      }).compile();
      const service = moduleRef.get(AlumniventuresService);
      expect(service).toBeInstanceOf(AlumniventuresService);
      await moduleRef.close();
    });

    it('exports the Site.ALUMNIVENTURES = "alumniventures" enum value', () => {
      expect(Site.ALUMNIVENTURES).toBe('alumniventures');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AlumniventuresService();
      const result = await service.scrape({
        siteType: [Site.ALUMNIVENTURES],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'alumniventures-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ALUMNIVENTURES);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Alumni Ventures');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/alumniventures/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/alumniventures/jobs/');
      ...
      expect(job0?.description).toContain('Alumni Ventures');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/alumniventures/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Alumni Ventures');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'alumniventures-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
