...
import { DelineaModule, DelineaService } from '../src';

const COMPANY_NAME_EXPECT = 'Delinea';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'delinea-jobs.json'), 'utf8'),
);
...
describe('DelineaService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through DelineaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [DelineaModule],
      }).compile();
      const service = moduleRef.get(DelineaService);
      expect(service).toBeInstanceOf(DelineaService);
      await moduleRef.close();
    });

    it('exports the Site.DELINEA = "delinea" enum value', () => {
      expect(Site.DELINEA).toBe('delinea');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new DelineaService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.DELINEA],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('delinea');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.DELINEA);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('delinea-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new DelineaService(registry);
      const result = await service.scrape({
        siteType: [Site.DELINEA],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('delinea');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('delinea-x1');
      expect(result.jobs[0].site).toBe(Site.DELINEA);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('delinea-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new DelineaService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.DELINEA],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new DelineaService();
      const result = await service.scrape({
        siteType: [Site.DELINEA],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
