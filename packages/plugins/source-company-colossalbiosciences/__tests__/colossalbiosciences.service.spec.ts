...
import { ColossalBiosciencesModule, ColossalBiosciencesService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'colossalbiosciences-jobs.json'), 'utf8'),
);
...
/**
 * Spec 829 / T04 — `ColossalBiosciencesService` unit tests (≥ 9 cases).
 */
describe('ColossalBiosciencesService — Spec 829 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ColossalBiosciencesModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ColossalBiosciencesModule],
      }).compile();
      const service = moduleRef.get(ColossalBiosciencesService);
      expect(service).toBeInstanceOf(ColossalBiosciencesService);
      await moduleRef.close();
    });

    it('exports the Site.COLOSSAL_BIOSCIENCES = "colossalbiosciences" enum value', () => {
      expect(Site.COLOSSAL_BIOSCIENCES).toBe('colossalbiosciences');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ColossalBiosciencesService();
      const result = await service.scrape({
        siteType: [Site.COLOSSAL_BIOSCIENCES],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'colossalbiosciences-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.COLOSSAL_BIOSCIENCES);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Colossal Biosciences');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/colossalbiosciences/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/colossalbiosciences/jobs/');
      ...
      expect(job0?.description).toContain('Colossal Biosciences');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/colossalbiosciences/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Colossal Biosciences');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'colossalbiosciences-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
