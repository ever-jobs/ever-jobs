...
import { RIOTModule, RIOTService } from '../src';

const COMPANY_NAME_EXPECT = 'RIOT';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'tryriot-jobs.json'), 'utf8'),
);
...
describe('RIOTService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through RIOTModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [RIOTModule],
      }).compile();
      const service = moduleRef.get(RIOTService);
      expect(service).toBeInstanceOf(RIOTService);
      await moduleRef.close();
    });

    it('exports the Site.RIOT = "tryriot" enum value', () => {
      expect(Site.RIOT).toBe('tryriot');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new RIOTService(registryWithLever());
      ...
      expect(calledUrls[0]).toContain('tryriot');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.RIOT);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('tryriot-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new RIOTService(registry);
      const result = await service.scrape({
        siteType: [Site.RIOT],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('tryriot');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('tryriot-x1');
      expect(result.jobs[0].site).toBe(Site.RIOT);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('tryriot-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new RIOTService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.RIOT],
      } as ScraperInputDto);
