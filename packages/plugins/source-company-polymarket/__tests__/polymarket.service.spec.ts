...
import { PolymarketModule, PolymarketService } from '../src';

const COMPANY_NAME_EXPECT = 'Polymarket';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'polymarket-jobs.json'), 'utf8'),
);
...
describe('PolymarketService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through PolymarketModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [PolymarketModule],
      }).compile();
      const service = moduleRef.get(PolymarketService);
      expect(service).toBeInstanceOf(PolymarketService);
      await moduleRef.close();
    });

    it('exports the Site.POLYMARKET = "polymarket" enum value', () => {
      expect(Site.POLYMARKET).toBe('polymarket');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new PolymarketService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.POLYMARKET],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('polymarket');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.POLYMARKET);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('polymarket-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new PolymarketService(registry);
      const result = await service.scrape({
        siteType: [Site.POLYMARKET],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('polymarket');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('polymarket-x1');
      expect(result.jobs[0].site).toBe(Site.POLYMARKET);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('polymarket-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new PolymarketService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.POLYMARKET],
      } as ScraperInputDto);
