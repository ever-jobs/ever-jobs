...
import { AloYogaModule, AloYogaService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'aloyoga-jobs.json'), 'utf8'),
);
...
/**
 * Spec 606 / T04 — `AloYogaService` unit tests (≥ 9 cases).
 */
describe('AloYogaService — Spec 606 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AloYogaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AloYogaModule],
      }).compile();
      const service = moduleRef.get(AloYogaService);
      expect(service).toBeInstanceOf(AloYogaService);
      await moduleRef.close();
    });

    it('exports the Site.ALO_YOGA = "aloyoga" enum value', () => {
      expect(Site.ALO_YOGA).toBe('aloyoga');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AloYogaService();
      const result = await service.scrape({
        siteType: [Site.ALO_YOGA],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'aloyoga-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ALO_YOGA);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Alo Yoga');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/aloyoga/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/aloyoga/jobs/');
      ...
      expect(job0?.description).toContain('Alo Yoga');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/aloyoga/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Alo Yoga');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'aloyoga-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
