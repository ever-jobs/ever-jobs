...
import { EncordModule, EncordService } from '../src';

const COMPANY_NAME_EXPECT = 'Encord';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'encord-jobs.json'), 'utf8'),
);
...
describe('EncordService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through EncordModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [EncordModule],
      }).compile();
      const service = moduleRef.get(EncordService);
      expect(service).toBeInstanceOf(EncordService);
      await moduleRef.close();
    });

    it('exports the Site.ENCORD = "encord" enum value', () => {
      expect(Site.ENCORD).toBe('encord');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new EncordService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.ENCORD],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('encord');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ENCORD);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('encord-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new EncordService(registry);
      const result = await service.scrape({
        siteType: [Site.ENCORD],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('encord');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('encord-x1');
      expect(result.jobs[0].site).toBe(Site.ENCORD);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('encord-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new EncordService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.ENCORD],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new EncordService();
      const result = await service.scrape({
        siteType: [Site.ENCORD],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
