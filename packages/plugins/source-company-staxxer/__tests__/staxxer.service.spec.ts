...
import { StaxxerModule, StaxxerService } from '../src';

const COMPANY_NAME_EXPECT = 'Staxxer';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'staxxer-jobs.json'), 'utf8'),
);
...
describe('StaxxerService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through StaxxerModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [StaxxerModule],
      }).compile();
      const service = moduleRef.get(StaxxerService);
      expect(service).toBeInstanceOf(StaxxerService);
      await moduleRef.close();
    });

    it('exports the Site.STAXXER = "staxxer" enum value', () => {
      expect(Site.STAXXER).toBe('staxxer');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new StaxxerService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.STAXXER],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('staxxer.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.STAXXER);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('staxxer-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new StaxxerService(registry);
      const result = await service.scrape({
        siteType: [Site.STAXXER],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('staxxer');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('staxxer-x1');
      expect(result.jobs[0].site).toBe(Site.STAXXER);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('staxxer-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new StaxxerService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.STAXXER],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new StaxxerService();
      const result = await service.scrape({
        siteType: [Site.STAXXER],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
