...
import { AisquaredModule, AisquaredService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'aisquared-jobs.json'), 'utf8'),
);
...
/**
 * Spec 220 / T04 — `AisquaredService` unit tests (≥ 9 cases).
 */
describe('AisquaredService — Spec 220 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AisquaredModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AisquaredModule],
      }).compile();
      const service = moduleRef.get(AisquaredService);
      expect(service).toBeInstanceOf(AisquaredService);
      await moduleRef.close();
    });

    it('exports the Site.AISQUARED = "aisquared" enum value', () => {
      expect(Site.AISQUARED).toBe('aisquared');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AisquaredService();
      const result = await service.scrape({
        siteType: [Site.AISQUARED],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'aisquared-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.AISQUARED);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('AI Squared');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/aisquared/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/aisquared/jobs/');
      ...
      expect(job0?.description).toContain('AI Squared');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/aisquared/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('AI Squared');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'aisquared-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
