...
import { CarbonModule, CarbonService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'carbon-jobs.json'), 'utf8'),
);
...
/**
 * Spec 522 / T04 — `CarbonService` unit tests (≥ 9 cases).
 */
describe('CarbonService — Spec 522 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CarbonModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CarbonModule],
      }).compile();
      const service = moduleRef.get(CarbonService);
      expect(service).toBeInstanceOf(CarbonService);
      await moduleRef.close();
    });

    it('exports the Site.CARBON = "carbon" enum value', () => {
      expect(Site.CARBON).toBe('carbon');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const job0 = dto.jobs.find((j) => j.id === 'carbon-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.CARBON);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Carbon');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/carbon/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/carbon/jobs/');
      ...
      expect(job0?.description).toContain('Carbon');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/carbon/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Carbon');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'carbon-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
