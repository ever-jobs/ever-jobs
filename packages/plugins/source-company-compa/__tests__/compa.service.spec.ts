...
import { CompaModule, CompaService } from '../src';

const COMPANY_NAME_EXPECT = 'Compa';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'compa-jobs.json'), 'utf8'),
);
...
describe('CompaService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CompaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CompaModule],
      }).compile();
      const service = moduleRef.get(CompaService);
      expect(service).toBeInstanceOf(CompaService);
      await moduleRef.close();
    });

    it('exports the Site.COMPA = "compa" enum value', () => {
      expect(Site.COMPA).toBe('compa');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new CompaService(registryWithAshby());
      ...
      expect(calledUrls[0]).toContain('compa');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.COMPA);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('compa-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new CompaService(registry);
      const result = await service.scrape({
        siteType: [Site.COMPA],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('compa');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('compa-x1');
      expect(result.jobs[0].site).toBe(Site.COMPA);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('compa-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new CompaService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.COMPA],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new CompaService();
      const result = await service.scrape({
        siteType: [Site.COMPA],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
