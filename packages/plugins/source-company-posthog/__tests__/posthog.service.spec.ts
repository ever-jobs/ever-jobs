...
import { PostHogModule, PostHogService } from '../src';

const COMPANY_NAME_EXPECT = 'PostHog';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'posthog-jobs.json'), 'utf8'),
);
...
describe('PostHogService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through PostHogModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [PostHogModule],
      }).compile();
      const service = moduleRef.get(PostHogService);
      expect(service).toBeInstanceOf(PostHogService);
      await moduleRef.close();
    });

    it('exports the Site.POSTHOG = "posthog" enum value', () => {
      expect(Site.POSTHOG).toBe('posthog');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new PostHogService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.POSTHOG],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('posthog');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.POSTHOG);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('posthog-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new PostHogService(registry);
      const result = await service.scrape({
        siteType: [Site.POSTHOG],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('posthog');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('posthog-x1');
      expect(result.jobs[0].site).toBe(Site.POSTHOG);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('posthog-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new PostHogService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.POSTHOG],
      } as ScraperInputDto);
