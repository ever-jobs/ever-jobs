...
import { ChowbusModule, ChowbusService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'chowbus-jobs.json'), 'utf8'),
);
...
/**
 * Spec 546 / T04 — `ChowbusService` unit tests (≥ 9 cases).
 */
describe('ChowbusService — Spec 546 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ChowbusModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ChowbusModule],
      }).compile();
      const service = moduleRef.get(ChowbusService);
      expect(service).toBeInstanceOf(ChowbusService);
      await moduleRef.close();
    });

    it('exports the Site.CHOWBUS = "chowbus" enum value', () => {
      expect(Site.CHOWBUS).toBe('chowbus');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ChowbusService();
      const result = await service.scrape({
        siteType: [Site.CHOWBUS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'chowbus-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.CHOWBUS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Chowbus');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/chowbus/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/chowbus/jobs/');
      ...
      expect(job0?.description).toContain('Chowbus');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/chowbus/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Chowbus');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'chowbus-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
