...
import { FiddlerAIModule, FiddlerAIService } from '../src';

const COMPANY_NAME_EXPECT = 'Fiddler AI';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'fiddlerai-jobs.json'), 'utf8'),
);
...
describe('FiddlerAIService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through FiddlerAIModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [FiddlerAIModule],
      }).compile();
      const service = moduleRef.get(FiddlerAIService);
      expect(service).toBeInstanceOf(FiddlerAIService);
      await moduleRef.close();
    });

    it('exports the Site.FIDDLER_AI = "fiddlerai" enum value', () => {
      expect(Site.FIDDLER_AI).toBe('fiddlerai');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new FiddlerAIService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.FIDDLER_AI],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('fiddler-ai');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.FIDDLER_AI);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('fiddlerai-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new FiddlerAIService(registry);
      const result = await service.scrape({
        siteType: [Site.FIDDLER_AI],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('fiddler-ai');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('fiddlerai-x1');
      expect(result.jobs[0].site).toBe(Site.FIDDLER_AI);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('fiddlerai-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new FiddlerAIService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.FIDDLER_AI],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new FiddlerAIService();
      const result = await service.scrape({
        siteType: [Site.FIDDLER_AI],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
