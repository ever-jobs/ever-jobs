...
import { ReliefInternationalModule, ReliefInternationalService } from '../src';

const COMPANY_NAME_EXPECT = 'Relief International';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'reliefinternational-jobs.json'), 'utf8'),
);
...
describe('ReliefInternationalService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ReliefInternationalModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ReliefInternationalModule],
      }).compile();
      const service = moduleRef.get(ReliefInternationalService);
      expect(service).toBeInstanceOf(ReliefInternationalService);
      await moduleRef.close();
    });

    it('exports the Site.RELIEF_INTERNATIONAL = "reliefinternational" enum value', () => {
      expect(Site.RELIEF_INTERNATIONAL).toBe('reliefinternational');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new ReliefInternationalService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.RELIEF_INTERNATIONAL],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'reliefinternational-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.RELIEF_INTERNATIONAL);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('reliefinternational-' + first.id);
      ...
      expect(calledUrls[0]).toContain('ReliefInternational');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.RELIEF_INTERNATIONAL);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('reliefinternational-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new ReliefInternationalService(registry);
      const result = await service.scrape({
        siteType: [Site.RELIEF_INTERNATIONAL],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('ReliefInternational');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('reliefinternational-x1');
      expect(result.jobs[0].site).toBe(Site.RELIEF_INTERNATIONAL);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('reliefinternational-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new ReliefInternationalService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.RELIEF_INTERNATIONAL],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new ReliefInternationalService();
      const result = await service.scrape({
        siteType: [Site.RELIEF_INTERNATIONAL],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
