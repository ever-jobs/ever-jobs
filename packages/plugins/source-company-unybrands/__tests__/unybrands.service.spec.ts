...
import { UnybrandsModule, UnybrandsService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'unybrands-jobs.json'), 'utf8'),
);
...
/**
 * Spec 533 / T04 — `UnybrandsService` unit tests (≥ 9 cases).
 */
describe('UnybrandsService — Spec 533 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through UnybrandsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [UnybrandsModule],
      }).compile();
      const service = moduleRef.get(UnybrandsService);
      expect(service).toBeInstanceOf(UnybrandsService);
      await moduleRef.close();
    });

    it('exports the Site.UNYBRANDS = "unybrands" enum value', () => {
      expect(Site.UNYBRANDS).toBe('unybrands');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new UnybrandsService();
      const result = await service.scrape({
        siteType: [Site.UNYBRANDS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'unybrands-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.UNYBRANDS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('unybrands');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/unybrands/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/unybrands/jobs/');
      ...
      expect(job0?.description).toContain('unybrands');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/unybrands/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('unybrands');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'unybrands-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
