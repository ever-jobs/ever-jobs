...
import { KrakenModule, KrakenService } from '../src';

const COMPANY_NAME_EXPECT = 'Kraken';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'kraken123-jobs.json'), 'utf8'),
);
...
describe('KrakenService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through KrakenModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [KrakenModule],
      }).compile();
      const service = moduleRef.get(KrakenService);
      expect(service).toBeInstanceOf(KrakenService);
      await moduleRef.close();
    });

    it('exports the Site.KRAKEN = "kraken123" enum value', () => {
      expect(Site.KRAKEN).toBe('kraken123');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new KrakenService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.KRAKEN],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('kraken123');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.KRAKEN);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('kraken123-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new KrakenService(registry);
      const result = await service.scrape({
        siteType: [Site.KRAKEN],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('kraken123');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('kraken123-x1');
      expect(result.jobs[0].site).toBe(Site.KRAKEN);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('kraken123-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new KrakenService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.KRAKEN],
      } as ScraperInputDto);
