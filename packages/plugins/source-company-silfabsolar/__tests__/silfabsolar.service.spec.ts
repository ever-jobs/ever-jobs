...
import { SilfabSolarModule, SilfabSolarService } from '../src';

const COMPANY_NAME_EXPECT = 'Silfab Solar';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'silfabsolar-jobs.json'), 'utf8'),
);
...
describe('SilfabSolarService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SilfabSolarModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SilfabSolarModule],
      }).compile();
      const service = moduleRef.get(SilfabSolarService);
      expect(service).toBeInstanceOf(SilfabSolarService);
      await moduleRef.close();
    });

    it('exports the Site.SILFAB_SOLAR = "silfabsolar" enum value', () => {
      expect(Site.SILFAB_SOLAR).toBe('silfabsolar');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new SilfabSolarService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.SILFAB_SOLAR],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('SilfabSolar');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SILFAB_SOLAR);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('silfabsolar-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new SilfabSolarService(registry);
      const result = await service.scrape({
        siteType: [Site.SILFAB_SOLAR],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('SilfabSolar');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('silfabsolar-x1');
      expect(result.jobs[0].site).toBe(Site.SILFAB_SOLAR);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('silfabsolar-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new SilfabSolarService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.SILFAB_SOLAR],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new SilfabSolarService();
      const result = await service.scrape({
        siteType: [Site.SILFAB_SOLAR],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
