...
/**
 * Motorola Solutions — Communications technology (HQ: Chicago, IL, USA).
 *
 * Source (Spec 1736): Workday board, scraped in this order:
 *   - `motorolasolutions:5:Careers` — https://motorolasolutions.wd5.myworkdayjobs.com/Careers
 *     verified live 2026-09-24: 888 open postings.
 *
 ...
 * party board.
 *
 * Tags: segment=workday-enterprise; industry=communications-technology.
 */
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'motorolasolutions:5:Careers', atsIdPrefix: 'wd-motorolasolutions-' },
];
...
@SourcePlugin({
  site: Site.MOTOROLA_SOLUTIONS,
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['motorolasolutions.com'],
  description: 'Motorola Solutions careers via Workday. Tags: segment=workday-enterprise; industry=communications-technology.',
})
@Injectable()
export class MotorolaSolutionsService implements IScraper {
  private readonly logger = new Logger(MotorolaSolutionsService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Workday source plugin is not registered; cannot scrape Motorola Solutions');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`Motorola Solutions: delegating to Workday (${board.companySlug})`);
      ...
      for (const job of result.jobs ?? []) {
        job.site = Site.MOTOROLA_SOLUTIONS;
    ...
    this.logger.log(`Motorola Solutions: scraped ${jobs.length} jobs`);
