...
import { MotorolaSolutionsModule, MotorolaSolutionsService } from '../src';
...
const FIXTURE: Fixture = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'fixtures', 'motorolasolutions-boards.json'), 'utf8'),
);
const COMPANY_NAME = 'Motorola Solutions';
const ID_PREFIX = 'motorolasolutions-';
...
describe('MotorolaSolutionsService — Workday delegation (Spec 1736)', () => {
  beforeEach(() => {
    mockGet.mockReset();
    mockPost.mockReset();
  ...
  describe('registration', () => {
    it('resolves through MotorolaSolutionsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({ imports: [MotorolaSolutionsModule] }).compile();
      expect(moduleRef.get(MotorolaSolutionsService)).toBeInstanceOf(MotorolaSolutionsService);
      await moduleRef.close();
    });

    it('exports Site.MOTOROLA_SOLUTIONS = "motorolasolutions"', () => {
      expect(Site.MOTOROLA_SOLUTIONS).toBe('motorolasolutions');
    });

    it('declares a tagged company plugin with its domains', () => {
      const meta = Reflect.getMetadata(SOURCE_PLUGIN_METADATA, MotorolaSolutionsService);
      expect(meta.site).toBe(Site.MOTOROLA_SOLUTIONS);
      expect(meta.category).toBe('company');
      expect(meta.isAts).toBeFalsy();
      expect(meta.companyDomains).toEqual(['motorolasolutions.com']);
  ...
  describe('recorded board (real Workday adapter, mocked HTTP)', () => {
    it('maps every recorded posting and re-stamps the company identity', async () => {
      const service = new MotorolaSolutionsService(registryWith());
      const result = await service.scrape({ siteType: [Site.MOTOROLA_SOLUTIONS], resultsWanted: 100 } as ScraperInputDto);
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.MOTOROLA_SOLUTIONS);
    ...
    it('only requests the recorded board URLs', async () => {
      const service = new MotorolaSolutionsService(registryWith());
      await service.scrape({ siteType: [Site.MOTOROLA_SOLUTIONS] } as ScraperInputDto);
    ...
    it('honours resultsWanted=1', async () => {
      const service = new MotorolaSolutionsService(registryWith());
      const result = await service.scrape({ siteType: [Site.MOTOROLA_SOLUTIONS], resultsWanted: 1 } as ScraperInputDto);
    ...
    it('resolves with an empty result when the board is gone (HTTP 404)', async () => {
      ...
      const service = new MotorolaSolutionsService(registryWith());
      const result = await service.scrape({ siteType: [Site.MOTOROLA_SOLUTIONS] } as ScraperInputDto);
  ...
  describe('delegation contract', () => {
    it('forwards the board and the caller input untouched', async () => {
      ...
      const service = new MotorolaSolutionsService(
        registryWith(
          fakeBackend(
      ...
      const result = await service.scrape({
        siteType: [Site.MOTOROLA_SOLUTIONS],
      ...
      expect(result.jobs[0].site).toBe(Site.MOTOROLA_SOLUTIONS);
    ...
    it('never forwards the caller\'s credentials to the board (Spec 1735 §4.5)', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new MotorolaSolutionsService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
    ...
    it('makes no request when resultsWanted is 0', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new MotorolaSolutionsService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      const result = await service.scrape({ siteType: [Site.MOTOROLA_SOLUTIONS], resultsWanted: 0 } as ScraperInputDto);
  ...
  describe('resilience', () => {
    it('reports not_registered when no registry is injected', async () => {
      const result = await new MotorolaSolutionsService().scrape({ siteType: [Site.MOTOROLA_SOLUTIONS] } as ScraperInputDto);
      expect(result.jobs).toEqual([]);
    ...
    it('reports not_registered when Workday is missing from the registry', async () => {
      const result = await new MotorolaSolutionsService(new PluginRegistry()).scrape({
        siteType: [Site.MOTOROLA_SOLUTIONS],
      } as ScraperInputDto);
      expect(result.diagnostics?.reason).toBe('not_registered');
    });
