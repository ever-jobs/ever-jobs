...
import { StateStreetModule, StateStreetService } from '../src';
...
const FIXTURE: Fixture = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'fixtures', 'statestreet-boards.json'), 'utf8'),
);
const COMPANY_NAME = 'State Street';
const ID_PREFIX = 'statestreet-';
...
describe('StateStreetService — Workday delegation (Spec 1736)', () => {
  beforeEach(() => {
    mockGet.mockReset();
    mockPost.mockReset();
  ...
  describe('registration', () => {
    it('resolves through StateStreetModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({ imports: [StateStreetModule] }).compile();
      expect(moduleRef.get(StateStreetService)).toBeInstanceOf(StateStreetService);
      await moduleRef.close();
    });

    it('exports Site.STATE_STREET = "statestreet"', () => {
      expect(Site.STATE_STREET).toBe('statestreet');
    });

    it('declares a tagged company plugin with its domains', () => {
      const meta = Reflect.getMetadata(SOURCE_PLUGIN_METADATA, StateStreetService);
      expect(meta.site).toBe(Site.STATE_STREET);
      expect(meta.category).toBe('company');
      expect(meta.isAts).toBeFalsy();
      expect(meta.companyDomains).toEqual(['statestreet.com']);
  ...
  describe('recorded board (real Workday adapter, mocked HTTP)', () => {
    it('maps every recorded posting and re-stamps the company identity', async () => {
      const service = new StateStreetService(registryWith());
      const result = await service.scrape({ siteType: [Site.STATE_STREET], resultsWanted: 100 } as ScraperInputDto);
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.STATE_STREET);
    ...
    it('only requests the recorded board URLs', async () => {
      const service = new StateStreetService(registryWith());
      await service.scrape({ siteType: [Site.STATE_STREET] } as ScraperInputDto);
    ...
    it('honours resultsWanted=1', async () => {
      const service = new StateStreetService(registryWith());
      const result = await service.scrape({ siteType: [Site.STATE_STREET], resultsWanted: 1 } as ScraperInputDto);
    ...
    it('resolves with an empty result when the board is gone (HTTP 404)', async () => {
      ...
      const service = new StateStreetService(registryWith());
      const result = await service.scrape({ siteType: [Site.STATE_STREET] } as ScraperInputDto);
  ...
  describe('delegation contract', () => {
    it('forwards the board and the caller input untouched', async () => {
      ...
      const service = new StateStreetService(
        registryWith(
          fakeBackend(
      ...
      expect(result.jobs[0].site).toBe(Site.STATE_STREET);
      expect(result.jobs[0].companyName).toBe(COMPANY_NAME);
    });

    it('passes an absent resultsWanted through as absent', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new StateStreetService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      await service.scrape({ siteType: [Site.STATE_STREET] } as ScraperInputDto);
    ...
    it('makes no request when resultsWanted is 0', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new StateStreetService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      const result = await service.scrape({ siteType: [Site.STATE_STREET], resultsWanted: 0 } as ScraperInputDto);
  ...
  describe('resilience', () => {
    it('reports not_registered when no registry is injected', async () => {
      const result = await new StateStreetService().scrape({ siteType: [Site.STATE_STREET] } as ScraperInputDto);
      expect(result.jobs).toEqual([]);
    ...
    it('reports not_registered when Workday is missing from the registry', async () => {
      const result = await new StateStreetService(new PluginRegistry()).scrape({
        siteType: [Site.STATE_STREET],
      } as ScraperInputDto);
      expect(result.diagnostics?.reason).toBe('not_registered');
    });
