...
/**
 * State Street — Financial services (HQ: Boston, MA, USA).
 *
 * Source (Spec 1736): Workday board, scraped in this order:
 *   - `statestreet:1:Global` — https://statestreet.wd1.myworkdayjobs.com/Global
 *     verified live 2026-09-24: 1,360 open postings.
 *
 ...
 * party board.
 *
 * Tags: segment=workday-enterprise; industry=financial-services.
 */
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'statestreet:1:Global', atsIdPrefix: 'wd-statestreet-' },
];

@SourcePlugin({
  site: Site.STATE_STREET,
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['statestreet.com'],
  description: 'State Street careers via Workday. Tags: segment=workday-enterprise; industry=financial-services.',
})
@Injectable()
export class StateStreetService implements IScraper {
  private readonly logger = new Logger(StateStreetService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Workday source plugin is not registered; cannot scrape State Street');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`State Street: delegating to Workday (${board.companySlug})`);
    ...
    this.logger.log(`State Street: scraped ${jobs.length} jobs`);
