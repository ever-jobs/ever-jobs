...
import { SemperisModule, SemperisService } from '../src';

const COMPANY_NAME_EXPECT = 'Semperis';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'semperis-jobs.json'), 'utf8'),
);
...
describe('SemperisService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SemperisModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SemperisModule],
      }).compile();
      const service = moduleRef.get(SemperisService);
      expect(service).toBeInstanceOf(SemperisService);
      await moduleRef.close();
    });

    it('exports the Site.SEMPERIS = "semperis" enum value', () => {
      expect(Site.SEMPERIS).toBe('semperis');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new SemperisService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.SEMPERIS],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('semperis');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SEMPERIS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('semperis-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new SemperisService(registry);
      const result = await service.scrape({
        siteType: [Site.SEMPERIS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('semperis');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('semperis-x1');
      expect(result.jobs[0].site).toBe(Site.SEMPERIS);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('semperis-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new SemperisService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.SEMPERIS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new SemperisService();
      const result = await service.scrape({
        siteType: [Site.SEMPERIS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
