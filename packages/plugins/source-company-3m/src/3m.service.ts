...
/**
 * 3M — Industrial conglomerate (HQ: Saint Paul, MN, USA).
 *
 * Source (Spec 1736): Workday board, scraped in this order:
 *   - `3m:1:Search` — https://3m.wd1.myworkdayjobs.com/Search
 *     verified live 2026-09-24: 702 open postings.
 *
 ...
 * party board.
 *
 * Tags: segment=workday-enterprise; industry=industrial-conglomerate.
 */
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: '3m:1:Search', atsIdPrefix: 'wd-3m-' },
];
...
@SourcePlugin({
  ...
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['3m.com'],
  description: '3M careers via Workday. Tags: segment=workday-enterprise; industry=industrial-conglomerate.',
})
@Injectable()
export class ThreeMService implements IScraper {
  private readonly logger = new Logger(ThreeMService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Workday source plugin is not registered; cannot scrape 3M');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`3M: delegating to Workday (${board.companySlug})`);
    ...
    this.logger.log(`3M: scraped ${jobs.length} jobs`);
