...
import { ThreeMModule, ThreeMService } from '../src';
...
const FIXTURE: Fixture = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'fixtures', '3m-boards.json'), 'utf8'),
);
const COMPANY_NAME = '3M';
const ID_PREFIX = '3m-';
...
describe('ThreeMService — Workday delegation (Spec 1736)', () => {
  beforeEach(() => {
    mockGet.mockReset();
    mockPost.mockReset();
  ...
  describe('registration', () => {
    it('resolves through ThreeMModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({ imports: [ThreeMModule] }).compile();
      expect(moduleRef.get(ThreeMService)).toBeInstanceOf(ThreeMService);
      await moduleRef.close();
    });

    it('exports Site.THREE_M = "3m"', () => {
      expect(Site.THREE_M).toBe('3m');
    });

    it('declares a tagged company plugin with its domains', () => {
      const meta = Reflect.getMetadata(SOURCE_PLUGIN_METADATA, ThreeMService);
      expect(meta.site).toBe(Site.THREE_M);
      expect(meta.category).toBe('company');
      expect(meta.isAts).toBeFalsy();
      expect(meta.companyDomains).toEqual(['3m.com']);
  ...
  describe('recorded board (real Workday adapter, mocked HTTP)', () => {
    it('maps every recorded posting and re-stamps the company identity', async () => {
      const service = new ThreeMService(registryWith());
      const result = await service.scrape({ siteType: [Site.THREE_M], resultsWanted: 100 } as ScraperInputDto);
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.THREE_M);
    ...
    it('only requests the recorded board URLs', async () => {
      const service = new ThreeMService(registryWith());
      await service.scrape({ siteType: [Site.THREE_M] } as ScraperInputDto);
    ...
    it('honours resultsWanted=1', async () => {
      const service = new ThreeMService(registryWith());
      const result = await service.scrape({ siteType: [Site.THREE_M], resultsWanted: 1 } as ScraperInputDto);
    ...
    it('resolves with an empty result when the board is gone (HTTP 404)', async () => {
      ...
      const service = new ThreeMService(registryWith());
      const result = await service.scrape({ siteType: [Site.THREE_M] } as ScraperInputDto);
  ...
  describe('delegation contract', () => {
    it('forwards the board and the caller input untouched', async () => {
      ...
      const service = new ThreeMService(
        registryWith(
          fakeBackend(
      ...
      expect(result.jobs[0].site).toBe(Site.THREE_M);
      expect(result.jobs[0].companyName).toBe(COMPANY_NAME);
    });

    it('passes an absent resultsWanted through as absent', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new ThreeMService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      await service.scrape({ siteType: [Site.THREE_M] } as ScraperInputDto);
    ...
    it('makes no request when resultsWanted is 0', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new ThreeMService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      const result = await service.scrape({ siteType: [Site.THREE_M], resultsWanted: 0 } as ScraperInputDto);
  ...
  describe('resilience', () => {
    it('reports not_registered when no registry is injected', async () => {
      const result = await new ThreeMService().scrape({ siteType: [Site.THREE_M] } as ScraperInputDto);
      expect(result.jobs).toEqual([]);
    ...
    it('reports not_registered when Workday is missing from the registry', async () => {
      const result = await new ThreeMService(new PluginRegistry()).scrape({
        siteType: [Site.THREE_M],
      } as ScraperInputDto);
      expect(result.diagnostics?.reason).toBe('not_registered');
    });
