...
import { InstantSystemModule, InstantSystemService } from '../src';

const COMPANY_NAME_EXPECT = 'Instant System';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'instantsystem-jobs.json'), 'utf8'),
);
...
describe('InstantSystemService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through InstantSystemModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [InstantSystemModule],
      }).compile();
      const service = moduleRef.get(InstantSystemService);
      expect(service).toBeInstanceOf(InstantSystemService);
      await moduleRef.close();
    });

    it('exports the Site.INSTANT_SYSTEM = "instantsystem" enum value', () => {
      expect(Site.INSTANT_SYSTEM).toBe('instantsystem');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new InstantSystemService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.INSTANT_SYSTEM],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'instantsystem-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Recruitee's defaults
      expect(job0?.site).toBe(Site.INSTANT_SYSTEM);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('instantsystem-' + first.id);
      ...
      expect(calledUrls[0]).toContain('instantsystem.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.INSTANT_SYSTEM);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('instantsystem-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new InstantSystemService(registry);
      const result = await service.scrape({
        siteType: [Site.INSTANT_SYSTEM],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('instantsystem');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('instantsystem-x1');
      expect(result.jobs[0].site).toBe(Site.INSTANT_SYSTEM);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('instantsystem-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new InstantSystemService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.INSTANT_SYSTEM],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new InstantSystemService();
      const result = await service.scrape({
        siteType: [Site.INSTANT_SYSTEM],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
