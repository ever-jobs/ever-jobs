...
import { CabifyModule, CabifyService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'cabify-jobs.json'), 'utf8'),
);
...
/**
 * Spec 821 / T04 — `CabifyService` unit tests (≥ 9 cases).
 */
describe('CabifyService — Spec 821 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CabifyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CabifyModule],
      }).compile();
      const service = moduleRef.get(CabifyService);
      expect(service).toBeInstanceOf(CabifyService);
      await moduleRef.close();
    });

    it('exports the Site.CABIFY = "cabify" enum value', () => {
      expect(Site.CABIFY).toBe('cabify');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new CabifyService();
      const result = await service.scrape({
        siteType: [Site.CABIFY],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'cabify-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.CABIFY);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Cabify');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/cabify/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/cabify/jobs/');
      ...
      expect(job0?.description).toContain('Cabify');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/cabify/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Cabify');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'cabify-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
