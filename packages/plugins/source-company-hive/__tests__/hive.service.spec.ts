...
import { HiveModule, HiveService } from '../src';

const COMPANY_NAME_EXPECT = 'Hive';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'hive-jobs.json'), 'utf8'),
);
...
describe('HiveService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through HiveModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [HiveModule],
      }).compile();
      const service = moduleRef.get(HiveService);
      expect(service).toBeInstanceOf(HiveService);
      await moduleRef.close();
    });

    it('exports the Site.HIVE = "hive" enum value', () => {
      expect(Site.HIVE).toBe('hive');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new HiveService(registryWithLever());
      ...
      expect(calledUrls[0]).toContain('hive');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.HIVE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('hive-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new HiveService(registry);
      const result = await service.scrape({
        siteType: [Site.HIVE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('hive');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('hive-x1');
      expect(result.jobs[0].site).toBe(Site.HIVE);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('hive-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new HiveService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.HIVE],
      } as ScraperInputDto);
