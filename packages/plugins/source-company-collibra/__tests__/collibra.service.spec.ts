...
import { CollibraModule, CollibraService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'collibra-jobs.json'), 'utf8'),
);
...
/**
 * Spec 828 / T04 — `CollibraService` unit tests (≥ 9 cases).
 */
describe('CollibraService — Spec 828 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CollibraModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CollibraModule],
      }).compile();
      const service = moduleRef.get(CollibraService);
      expect(service).toBeInstanceOf(CollibraService);
      await moduleRef.close();
    });

    it('exports the Site.COLLIBRA = "collibra" enum value', () => {
      expect(Site.COLLIBRA).toBe('collibra');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new CollibraService();
      const result = await service.scrape({
        siteType: [Site.COLLIBRA],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'collibra-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.COLLIBRA);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Collibra');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/collibra/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/collibra/jobs/');
      ...
      expect(job0?.description).toContain('Collibra');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/collibra/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Collibra');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'collibra-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
