...
import { EverlaneModule, EverlaneService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'everlane-jobs.json'), 'utf8'),
);
...
/**
 * Spec 597 / T04 — `EverlaneService` unit tests (≥ 9 cases).
 */
describe('EverlaneService — Spec 597 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through EverlaneModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [EverlaneModule],
      }).compile();
      const service = moduleRef.get(EverlaneService);
      expect(service).toBeInstanceOf(EverlaneService);
      await moduleRef.close();
    });

    it('exports the Site.EVERLANE = "everlane" enum value', () => {
      expect(Site.EVERLANE).toBe('everlane');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new EverlaneService();
      const result = await service.scrape({
        siteType: [Site.EVERLANE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'everlane-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.EVERLANE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Everlane');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/everlane/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/everlane/jobs/');
      ...
      expect(job0?.description).toContain('Everlane');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/everlane/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Everlane');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'everlane-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
