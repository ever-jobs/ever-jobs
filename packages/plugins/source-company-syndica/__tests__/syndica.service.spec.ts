...
import { SyndicaModule, SyndicaService } from '../src';

const COMPANY_NAME_EXPECT = 'Syndica';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'syndica-jobs.json'), 'utf8'),
);
...
describe('SyndicaService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SyndicaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SyndicaModule],
      }).compile();
      const service = moduleRef.get(SyndicaService);
      expect(service).toBeInstanceOf(SyndicaService);
      await moduleRef.close();
    });

    it('exports the Site.SYNDICA = "syndica" enum value', () => {
      expect(Site.SYNDICA).toBe('syndica');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new SyndicaService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.SYNDICA],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('syndica');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SYNDICA);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('syndica-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new SyndicaService(registry);
      const result = await service.scrape({
        siteType: [Site.SYNDICA],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('syndica');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('syndica-x1');
      expect(result.jobs[0].site).toBe(Site.SYNDICA);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('syndica-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new SyndicaService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.SYNDICA],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new SyndicaService();
      const result = await service.scrape({
        siteType: [Site.SYNDICA],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
