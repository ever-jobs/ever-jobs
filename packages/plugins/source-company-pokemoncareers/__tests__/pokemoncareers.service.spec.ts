...
import { ThePokMonCompanyInternationalModule, ThePokMonCompanyInternationalService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'pokemoncareers-jobs.json'), 'utf8'),
);
...
/**
 * Spec 913 / T04 — `ThePokMonCompanyInternationalService` unit tests (≥ 9 cases).
 */
describe('ThePokMonCompanyInternationalService — Spec 913 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ThePokMonCompanyInternationalModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ThePokMonCompanyInternationalModule],
      }).compile();
      const service = moduleRef.get(ThePokMonCompanyInternationalService);
      expect(service).toBeInstanceOf(ThePokMonCompanyInternationalService);
      await moduleRef.close();
    });

    it('exports the Site.THE_POK_MON_COMPANY_INTERNATIONAL = "pokemoncareers" enum value', () => {
      expect(Site.THE_POK_MON_COMPANY_INTERNATIONAL).toBe('pokemoncareers');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ThePokMonCompanyInternationalService();
      const result = await service.scrape({
        siteType: [Site.THE_POK_MON_COMPANY_INTERNATIONAL],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'pokemoncareers-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.THE_POK_MON_COMPANY_INTERNATIONAL);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('The Pokémon Company International');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/pokemoncareers/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/pokemoncareers/jobs/');
      ...
      expect(job0?.description).toContain('The Pokémon Company International');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/pokemoncareers/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('The Pokémon Company International');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'pokemoncareers-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
