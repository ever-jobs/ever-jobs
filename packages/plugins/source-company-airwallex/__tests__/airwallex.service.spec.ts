...
import { AirwallexModule, AirwallexService } from '../src';

const COMPANY_NAME_EXPECT = 'Airwallex';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'airwallex-jobs.json'), 'utf8'),
);
...
describe('AirwallexService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AirwallexModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AirwallexModule],
      }).compile();
      const service = moduleRef.get(AirwallexService);
      expect(service).toBeInstanceOf(AirwallexService);
      await moduleRef.close();
    });

    it('exports the Site.AIRWALLEX = "airwallex" enum value', () => {
      expect(Site.AIRWALLEX).toBe('airwallex');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new AirwallexService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.AIRWALLEX],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('airwallex');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.AIRWALLEX);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('airwallex-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new AirwallexService(registry);
      const result = await service.scrape({
        siteType: [Site.AIRWALLEX],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('airwallex');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('airwallex-x1');
      expect(result.jobs[0].site).toBe(Site.AIRWALLEX);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('airwallex-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new AirwallexService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.AIRWALLEX],
      } as ScraperInputDto);
