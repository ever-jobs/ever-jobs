...
import { EtihadAirwaysModule, EtihadAirwaysService } from '../src';

const COMPANY_NAME_EXPECT = 'Etihad Airways';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'etihadairways-jobs.json'), 'utf8'),
);
...
describe('EtihadAirwaysService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through EtihadAirwaysModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [EtihadAirwaysModule],
      }).compile();
      const service = moduleRef.get(EtihadAirwaysService);
      expect(service).toBeInstanceOf(EtihadAirwaysService);
      await moduleRef.close();
    });

    it('exports the Site.ETIHAD_AIRWAYS = "etihadairways" enum value', () => {
      expect(Site.ETIHAD_AIRWAYS).toBe('etihadairways');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new EtihadAirwaysService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.ETIHAD_AIRWAYS],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'etihadairways-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.ETIHAD_AIRWAYS);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('etihadairways-' + first.id);
      ...
      expect(calledUrls[0]).toContain('EtihadAirways5');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ETIHAD_AIRWAYS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('etihadairways-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new EtihadAirwaysService(registry);
      const result = await service.scrape({
        siteType: [Site.ETIHAD_AIRWAYS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('EtihadAirways5');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('etihadairways-x1');
      expect(result.jobs[0].site).toBe(Site.ETIHAD_AIRWAYS);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('etihadairways-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new EtihadAirwaysService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.ETIHAD_AIRWAYS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new EtihadAirwaysService();
      const result = await service.scrape({
        siteType: [Site.ETIHAD_AIRWAYS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
