...
import { HighspotModule, HighspotService } from '../src';

const COMPANY_NAME_EXPECT = 'Highspot';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'highspot-jobs.json'), 'utf8'),
);
...
describe('HighspotService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through HighspotModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [HighspotModule],
      }).compile();
      const service = moduleRef.get(HighspotService);
      expect(service).toBeInstanceOf(HighspotService);
      await moduleRef.close();
    });

    it('exports the Site.HIGHSPOT = "highspot" enum value', () => {
      expect(Site.HIGHSPOT).toBe('highspot');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new HighspotService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.HIGHSPOT],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('highspot');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.HIGHSPOT);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('highspot-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new HighspotService(registry);
      const result = await service.scrape({
        siteType: [Site.HIGHSPOT],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('highspot');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('highspot-x1');
      expect(result.jobs[0].site).toBe(Site.HIGHSPOT);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('highspot-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new HighspotService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.HIGHSPOT],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new HighspotService();
      const result = await service.scrape({
        siteType: [Site.HIGHSPOT],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
