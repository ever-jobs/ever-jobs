...
import { TheNielsenCompanyModule, TheNielsenCompanyService } from '../src';

const COMPANY_NAME_EXPECT = 'The Nielsen Company';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'thenielsencompany-jobs.json'), 'utf8'),
);
...
describe('TheNielsenCompanyService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through TheNielsenCompanyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [TheNielsenCompanyModule],
      }).compile();
      const service = moduleRef.get(TheNielsenCompanyService);
      expect(service).toBeInstanceOf(TheNielsenCompanyService);
      await moduleRef.close();
    });

    it('exports the Site.THE_NIELSEN_COMPANY = "thenielsencompany" enum value', () => {
      expect(Site.THE_NIELSEN_COMPANY).toBe('thenielsencompany');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new TheNielsenCompanyService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.THE_NIELSEN_COMPANY],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'thenielsencompany-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.THE_NIELSEN_COMPANY);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('thenielsencompany-' + first.id);
      ...
      expect(calledUrls[0]).toContain('TheNielsenCompany');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.THE_NIELSEN_COMPANY);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('thenielsencompany-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new TheNielsenCompanyService(registry);
      const result = await service.scrape({
        siteType: [Site.THE_NIELSEN_COMPANY],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('TheNielsenCompany');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('thenielsencompany-x1');
      expect(result.jobs[0].site).toBe(Site.THE_NIELSEN_COMPANY);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('thenielsencompany-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new TheNielsenCompanyService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.THE_NIELSEN_COMPANY],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new TheNielsenCompanyService();
      const result = await service.scrape({
        siteType: [Site.THE_NIELSEN_COMPANY],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
