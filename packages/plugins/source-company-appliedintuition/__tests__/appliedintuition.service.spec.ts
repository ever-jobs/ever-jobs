...
import { AppliedIntuitionModule, AppliedIntuitionService } from '../src';

const COMPANY_NAME_EXPECT = 'Applied Intuition';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'appliedintuition-jobs.json'), 'utf8'),
);
...
describe('AppliedIntuitionService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AppliedIntuitionModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AppliedIntuitionModule],
      }).compile();
      const service = moduleRef.get(AppliedIntuitionService);
      expect(service).toBeInstanceOf(AppliedIntuitionService);
      await moduleRef.close();
    });

    it('exports the Site.APPLIED_INTUITION = "appliedintuition" enum value', () => {
      expect(Site.APPLIED_INTUITION).toBe('appliedintuition');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new AppliedIntuitionService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.APPLIED_INTUITION],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'appliedintuition-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.APPLIED_INTUITION);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('appliedintuition-' + first.id);
      ...
      expect(calledUrls[0]).toContain('applied');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.APPLIED_INTUITION);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('appliedintuition-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new AppliedIntuitionService(registry);
      const result = await service.scrape({
        siteType: [Site.APPLIED_INTUITION],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('applied');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('appliedintuition-x1');
      expect(result.jobs[0].site).toBe(Site.APPLIED_INTUITION);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('appliedintuition-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new AppliedIntuitionService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.APPLIED_INTUITION],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new AppliedIntuitionService();
      const result = await service.scrape({
        siteType: [Site.APPLIED_INTUITION],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
