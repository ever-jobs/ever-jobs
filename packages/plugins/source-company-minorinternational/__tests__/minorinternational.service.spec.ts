...
import { MinorInternationalModule, MinorInternationalService } from '../src';

const COMPANY_NAME_EXPECT = 'Minor International';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'minorinternational-jobs.json'), 'utf8'),
);
...
describe('MinorInternationalService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through MinorInternationalModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [MinorInternationalModule],
      }).compile();
      const service = moduleRef.get(MinorInternationalService);
      expect(service).toBeInstanceOf(MinorInternationalService);
      await moduleRef.close();
    });

    it('exports the Site.MINOR_INTERNATIONAL = "minorinternational" enum value', () => {
      expect(Site.MINOR_INTERNATIONAL).toBe('minorinternational');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new MinorInternationalService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.MINOR_INTERNATIONAL],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'minorinternational-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.MINOR_INTERNATIONAL);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('minorinternational-' + first.id);
      ...
      expect(calledUrls[0]).toContain('MinorInternational');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.MINOR_INTERNATIONAL);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('minorinternational-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new MinorInternationalService(registry);
      const result = await service.scrape({
        siteType: [Site.MINOR_INTERNATIONAL],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('MinorInternational');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('minorinternational-x1');
      expect(result.jobs[0].site).toBe(Site.MINOR_INTERNATIONAL);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('minorinternational-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new MinorInternationalService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.MINOR_INTERNATIONAL],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new MinorInternationalService();
      const result = await service.scrape({
        siteType: [Site.MINOR_INTERNATIONAL],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
