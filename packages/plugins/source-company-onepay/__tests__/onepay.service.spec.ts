...
import { OnePayModule, OnePayService } from '../src';

const COMPANY_NAME_EXPECT = 'OnePay';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'onepay-jobs.json'), 'utf8'),
);
...
describe('OnePayService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through OnePayModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [OnePayModule],
      }).compile();
      const service = moduleRef.get(OnePayService);
      expect(service).toBeInstanceOf(OnePayService);
      await moduleRef.close();
    });

    it('exports the Site.ONEPAY = "onepay" enum value', () => {
      expect(Site.ONEPAY).toBe('onepay');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new OnePayService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.ONEPAY],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('oneapp');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ONEPAY);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('onepay-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new OnePayService(registry);
      const result = await service.scrape({
        siteType: [Site.ONEPAY],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('oneapp');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('onepay-x1');
      expect(result.jobs[0].site).toBe(Site.ONEPAY);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('onepay-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new OnePayService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.ONEPAY],
      } as ScraperInputDto);
