...
import { ProtonModule, ProtonService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'proton-jobs.json'), 'utf8'),
);
...
/**
 * Spec 699 / T04 — `ProtonService` unit tests (≥ 9 cases).
 */
describe('ProtonService — Spec 699 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ProtonModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ProtonModule],
      }).compile();
      const service = moduleRef.get(ProtonService);
      expect(service).toBeInstanceOf(ProtonService);
      await moduleRef.close();
    });

    it('exports the Site.PROTON = "proton" enum value', () => {
      expect(Site.PROTON).toBe('proton');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ProtonService();
      const result = await service.scrape({
        siteType: [Site.PROTON],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'proton-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.PROTON);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Proton');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/proton/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/proton/jobs/');
      ...
      expect(job0?.description).toContain('Proton');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/proton/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Proton');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'proton-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
