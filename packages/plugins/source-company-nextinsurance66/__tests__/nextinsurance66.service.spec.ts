...
import { NextInsuranceModule, NextInsuranceService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'nextinsurance66-jobs.json'), 'utf8'),
);
...
/**
 * Spec 894 / T04 — `NextInsuranceService` unit tests (≥ 9 cases).
 */
describe('NextInsuranceService — Spec 894 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through NextInsuranceModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [NextInsuranceModule],
      }).compile();
      const service = moduleRef.get(NextInsuranceService);
      expect(service).toBeInstanceOf(NextInsuranceService);
      await moduleRef.close();
    });

    it('exports the Site.NEXT_INSURANCE = "nextinsurance66" enum value', () => {
      expect(Site.NEXT_INSURANCE).toBe('nextinsurance66');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new NextInsuranceService();
      const result = await service.scrape({
        siteType: [Site.NEXT_INSURANCE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'nextinsurance66-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.NEXT_INSURANCE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Next Insurance');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/nextinsurance66/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/nextinsurance66/jobs/');
      ...
      expect(job0?.description).toContain('Next Insurance');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/nextinsurance66/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Next Insurance');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'nextinsurance66-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
