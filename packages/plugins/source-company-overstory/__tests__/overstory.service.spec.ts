...
import { OverstoryModule, OverstoryService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'overstory-jobs.json'), 'utf8'),
);
...
/**
 * Spec 615 / T04 — `OverstoryService` unit tests (≥ 9 cases).
 */
describe('OverstoryService — Spec 615 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through OverstoryModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [OverstoryModule],
      }).compile();
      const service = moduleRef.get(OverstoryService);
      expect(service).toBeInstanceOf(OverstoryService);
      await moduleRef.close();
    });

    it('exports the Site.OVERSTORY = "overstory" enum value', () => {
      expect(Site.OVERSTORY).toBe('overstory');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new OverstoryService();
      const result = await service.scrape({
        siteType: [Site.OVERSTORY],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'overstory-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.OVERSTORY);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Overstory');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/overstory/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/overstory/jobs/');
      ...
      expect(job0?.description).toContain('Overstory');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/overstory/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Overstory');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'overstory-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
