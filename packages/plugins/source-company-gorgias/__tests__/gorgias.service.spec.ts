...
import { GorgiasModule, GorgiasService } from '../src';

const COMPANY_NAME_EXPECT = 'Gorgias';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'gorgias-jobs.json'), 'utf8'),
);
...
describe('GorgiasService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through GorgiasModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [GorgiasModule],
      }).compile();
      const service = moduleRef.get(GorgiasService);
      expect(service).toBeInstanceOf(GorgiasService);
      await moduleRef.close();
    });

    it('exports the Site.GORGIAS = "gorgias" enum value', () => {
      expect(Site.GORGIAS).toBe('gorgias');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new GorgiasService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.GORGIAS],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('gorgias');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.GORGIAS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('gorgias-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new GorgiasService(registry);
      const result = await service.scrape({
        siteType: [Site.GORGIAS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('gorgias');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('gorgias-x1');
      expect(result.jobs[0].site).toBe(Site.GORGIAS);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('gorgias-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new GorgiasService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.GORGIAS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new GorgiasService();
      const result = await service.scrape({
        siteType: [Site.GORGIAS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
