...
import { FastSpringModule, FastSpringService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'fastspring-jobs.json'), 'utf8'),
);
...
/**
 * Spec 793 / T04 — `FastSpringService` unit tests (≥ 9 cases).
 */
describe('FastSpringService — Spec 793 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through FastSpringModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [FastSpringModule],
      }).compile();
      const service = moduleRef.get(FastSpringService);
      expect(service).toBeInstanceOf(FastSpringService);
      await moduleRef.close();
    });

    it('exports the Site.FASTSPRING = "fastspring" enum value', () => {
      expect(Site.FASTSPRING).toBe('fastspring');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new FastSpringService();
      const result = await service.scrape({
        siteType: [Site.FASTSPRING],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'fastspring-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.FASTSPRING);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('FastSpring');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/fastspring/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/fastspring/jobs/');
      ...
      expect(job0?.description).toContain('FastSpring');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/fastspring/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('FastSpring');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'fastspring-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
