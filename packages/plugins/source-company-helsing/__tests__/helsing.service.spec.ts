...
import { HelsingModule, HelsingService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'helsing-jobs.json'), 'utf8'),
);
...
/**
 * Spec 781 / T04 — `HelsingService` unit tests (≥ 9 cases).
 */
describe('HelsingService — Spec 781 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through HelsingModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [HelsingModule],
      }).compile();
      const service = moduleRef.get(HelsingService);
      expect(service).toBeInstanceOf(HelsingService);
      await moduleRef.close();
    });

    it('exports the Site.HELSING = "helsing" enum value', () => {
      expect(Site.HELSING).toBe('helsing');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new HelsingService();
      const result = await service.scrape({
        siteType: [Site.HELSING],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'helsing-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.HELSING);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Helsing');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/helsing/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/helsing/jobs/');
      ...
      expect(job0?.description).toContain('Helsing');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/helsing/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Helsing');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'helsing-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
