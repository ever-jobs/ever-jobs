...
import { AlarmcomModule, AlarmcomService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'alarmcom-jobs.json'), 'utf8'),
);
...
/**
 * Spec 226 / T04 — `AlarmcomService` unit tests (≥ 9 cases).
 */
describe('AlarmcomService — Spec 226 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AlarmcomModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AlarmcomModule],
      }).compile();
      const service = moduleRef.get(AlarmcomService);
      expect(service).toBeInstanceOf(AlarmcomService);
      await moduleRef.close();
    });

    it('exports the Site.ALARMCOM = "alarmcom" enum value', () => {
      expect(Site.ALARMCOM).toBe('alarmcom');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AlarmcomService();
      const result = await service.scrape({
        siteType: [Site.ALARMCOM],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'alarmcom-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ALARMCOM);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Alarm.com');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/alarmcom/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/alarmcom/jobs/');
      ...
      expect(job0?.description).toContain('Alarm.com');
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Alarm.com');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'alarmcom-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
