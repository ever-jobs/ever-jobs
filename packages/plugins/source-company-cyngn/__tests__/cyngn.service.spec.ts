...
import { CyngnModule, CyngnService } from '../src';

const COMPANY_NAME_EXPECT = 'Cyngn';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'cyngn-jobs.json'), 'utf8'),
);
...
describe('CyngnService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CyngnModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CyngnModule],
      }).compile();
      const service = moduleRef.get(CyngnService);
      expect(service).toBeInstanceOf(CyngnService);
      await moduleRef.close();
    });

    it('exports the Site.CYNGN = "cyngn" enum value', () => {
      expect(Site.CYNGN).toBe('cyngn');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new CyngnService(registryWithLever());
      ...
      expect(calledUrls[0]).toContain('cyngn');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.CYNGN);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('cyngn-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new CyngnService(registry);
      const result = await service.scrape({
        siteType: [Site.CYNGN],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('cyngn');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('cyngn-x1');
      expect(result.jobs[0].site).toBe(Site.CYNGN);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('cyngn-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new CyngnService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.CYNGN],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new CyngnService();
      const result = await service.scrape({
        siteType: [Site.CYNGN],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
