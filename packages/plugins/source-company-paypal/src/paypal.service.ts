...
/**
 * PayPal — Payments (HQ: San Jose, CA, USA).
 *
 * Source (Spec 1736): Workday board, scraped in this order:
 *   - `paypal:1:jobs` — https://paypal.wd1.myworkdayjobs.com/jobs
 *     verified live 2026-09-24: 280 open postings.
 *
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'paypal:1:jobs', atsIdPrefix: 'wd-paypal-' },
];
...
@SourcePlugin({
  ...
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['paypal.com'],
  description: 'PayPal careers via Workday. Tags: segment=workday-enterprise; industry=payments.',
})
@Injectable()
export class PayPalService implements IScraper {
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Workday source plugin is not registered; cannot scrape PayPal');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`PayPal: delegating to Workday (${board.companySlug})`);
    ...
    this.logger.log(`PayPal: scraped ${jobs.length} jobs`);
