...
import { PayPalModule, PayPalService } from '../src';
...
const FIXTURE: Fixture = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'fixtures', 'paypal-boards.json'), 'utf8'),
);
const COMPANY_NAME = 'PayPal';
const ID_PREFIX = 'paypal-';
...
describe('PayPalService — Workday delegation (Spec 1736)', () => {
  beforeEach(() => {
    mockGet.mockReset();
    mockPost.mockReset();
  ...
  describe('registration', () => {
    it('resolves through PayPalModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({ imports: [PayPalModule] }).compile();
      expect(moduleRef.get(PayPalService)).toBeInstanceOf(PayPalService);
      await moduleRef.close();
    });

    it('exports Site.PAYPAL = "paypal"', () => {
      expect(Site.PAYPAL).toBe('paypal');
    });

    it('declares a tagged company plugin with its domains', () => {
      const meta = Reflect.getMetadata(SOURCE_PLUGIN_METADATA, PayPalService);
      expect(meta.site).toBe(Site.PAYPAL);
      expect(meta.category).toBe('company');
      expect(meta.isAts).toBeFalsy();
      expect(meta.companyDomains).toEqual(['paypal.com']);
  ...
  describe('recorded board (real Workday adapter, mocked HTTP)', () => {
    it('maps every recorded posting and re-stamps the company identity', async () => {
      const service = new PayPalService(registryWith());
      const result = await service.scrape({ siteType: [Site.PAYPAL], resultsWanted: 100 } as ScraperInputDto);
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.PAYPAL);
    ...
    it('only requests the recorded board URLs', async () => {
      const service = new PayPalService(registryWith());
      await service.scrape({ siteType: [Site.PAYPAL] } as ScraperInputDto);
    ...
    it('honours resultsWanted=1', async () => {
      const service = new PayPalService(registryWith());
      const result = await service.scrape({ siteType: [Site.PAYPAL], resultsWanted: 1 } as ScraperInputDto);
    ...
    it('resolves with an empty result when the board is gone (HTTP 404)', async () => {
      ...
      const service = new PayPalService(registryWith());
      const result = await service.scrape({ siteType: [Site.PAYPAL] } as ScraperInputDto);
  ...
  describe('delegation contract', () => {
    it('forwards the board and the caller input untouched', async () => {
      ...
      expect(result.jobs[0].site).toBe(Site.PAYPAL);
      expect(result.jobs[0].companyName).toBe(COMPANY_NAME);
    });

    it('passes an absent resultsWanted through as absent', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new PayPalService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      await service.scrape({ siteType: [Site.PAYPAL] } as ScraperInputDto);
    ...
    it('makes no request when resultsWanted is 0', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new PayPalService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      const result = await service.scrape({ siteType: [Site.PAYPAL], resultsWanted: 0 } as ScraperInputDto);
  ...
  describe('resilience', () => {
    it('reports not_registered when no registry is injected', async () => {
      const result = await new PayPalService().scrape({ siteType: [Site.PAYPAL] } as ScraperInputDto);
      expect(result.jobs).toEqual([]);
    ...
    it('reports not_registered when Workday is missing from the registry', async () => {
      const result = await new PayPalService(new PluginRegistry()).scrape({
        siteType: [Site.PAYPAL],
      } as ScraperInputDto);
      expect(result.diagnostics?.reason).toBe('not_registered');
    });
