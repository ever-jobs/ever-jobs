...
/**
 * Geneva Trading — Proprietary trading (HQ: Chicago, IL, USA).
 *
 * Source (Spec 1737): Greenhouse board, scraped in this order:
 *   - `genevatrading` — https://job-boards.greenhouse.io/genevatrading
 *     verified live 2026-09-24: 15 open postings.
 *
 ...
 * party board.
 *
 * Tags: segment=quant-trading; industry=proprietary-trading.
 */
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'genevatrading', atsIdPrefix: 'gh-' },
];

@SourcePlugin({
  site: Site.GENEVA_TRADING,
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['genevatrading.com'],
  description: 'Geneva Trading careers via Greenhouse. Tags: segment=quant-trading; industry=proprietary-trading.',
})
@Injectable()
export class GenevaTradingService implements IScraper {
  private readonly logger = new Logger(GenevaTradingService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Greenhouse source plugin is not registered; cannot scrape Geneva Trading');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`Geneva Trading: delegating to Greenhouse (${board.companySlug})`);
    ...
    this.logger.log(`Geneva Trading: scraped ${jobs.length} jobs`);
