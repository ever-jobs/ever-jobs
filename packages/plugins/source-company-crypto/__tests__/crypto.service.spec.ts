...
import { CryptoComModule, CryptoComService } from '../src';

const COMPANY_NAME_EXPECT = 'Crypto.com';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'crypto-jobs.json'), 'utf8'),
);
...
describe('CryptoComService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CryptoComModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CryptoComModule],
      }).compile();
      const service = moduleRef.get(CryptoComService);
      expect(service).toBeInstanceOf(CryptoComService);
      await moduleRef.close();
    });

    it('exports the Site.CRYPTO_COM = "crypto" enum value', () => {
      expect(Site.CRYPTO_COM).toBe('crypto');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new CryptoComService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.CRYPTO_COM],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('crypto');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.CRYPTO_COM);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('crypto-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new CryptoComService(registry);
      const result = await service.scrape({
        siteType: [Site.CRYPTO_COM],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('crypto');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('crypto-x1');
      expect(result.jobs[0].site).toBe(Site.CRYPTO_COM);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('crypto-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new CryptoComService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.CRYPTO_COM],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new CryptoComService();
      const result = await service.scrape({
        siteType: [Site.CRYPTO_COM],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
