...
import { NerosTechnologiesModule, NerosTechnologiesService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'nerostechnologies-jobs.json'), 'utf8'),
);
...
/**
 * Spec 891 / T04 — `NerosTechnologiesService` unit tests (≥ 9 cases).
 */
describe('NerosTechnologiesService — Spec 891 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through NerosTechnologiesModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [NerosTechnologiesModule],
      }).compile();
      const service = moduleRef.get(NerosTechnologiesService);
      expect(service).toBeInstanceOf(NerosTechnologiesService);
      await moduleRef.close();
    });

    it('exports the Site.NEROS_TECHNOLOGIES = "nerostechnologies" enum value', () => {
      expect(Site.NEROS_TECHNOLOGIES).toBe('nerostechnologies');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new NerosTechnologiesService();
      const result = await service.scrape({
        siteType: [Site.NEROS_TECHNOLOGIES],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'nerostechnologies-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.NEROS_TECHNOLOGIES);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Neros Technologies');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/nerostechnologies/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/nerostechnologies/jobs/');
      ...
      expect(job0?.description).toContain('Neros Technologies');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/nerostechnologies/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Neros Technologies');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'nerostechnologies-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
