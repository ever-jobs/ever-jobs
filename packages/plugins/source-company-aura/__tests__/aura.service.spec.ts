...
import { AuraModule, AuraService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'aura-jobs.json'), 'utf8'),
);
...
/**
 * Spec 812 / T04 — `AuraService` unit tests (≥ 9 cases).
 */
describe('AuraService — Spec 812 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AuraModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AuraModule],
      }).compile();
      const service = moduleRef.get(AuraService);
      expect(service).toBeInstanceOf(AuraService);
      await moduleRef.close();
    });

    it('exports the Site.AURA = "aura" enum value', () => {
      expect(Site.AURA).toBe('aura');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const job0 = dto.jobs.find((j) => j.id === 'aura-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.AURA);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Aura');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/aura/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/aura/jobs/');
      ...
      expect(job0?.description).toContain('Aura');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/aura/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Aura');
      }
    });
  });
