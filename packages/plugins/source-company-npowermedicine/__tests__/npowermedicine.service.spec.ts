...
import { NPowerMedicineModule, NPowerMedicineService } from '../src';

const COMPANY_NAME_EXPECT = 'N-Power Medicine';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'npowermedicine-jobs.json'), 'utf8'),
);
...
describe('NPowerMedicineService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through NPowerMedicineModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [NPowerMedicineModule],
      }).compile();
      const service = moduleRef.get(NPowerMedicineService);
      expect(service).toBeInstanceOf(NPowerMedicineService);
      await moduleRef.close();
    });

    it('exports the Site.N_POWER_MEDICINE = "npowermedicine" enum value', () => {
      expect(Site.N_POWER_MEDICINE).toBe('npowermedicine');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new NPowerMedicineService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.N_POWER_MEDICINE],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'npowermedicine-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.N_POWER_MEDICINE);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('npowermedicine-' + first.id);
      ...
      expect(calledUrls[0]).toContain('npowermedicine');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.N_POWER_MEDICINE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('npowermedicine-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new NPowerMedicineService(registry);
      const result = await service.scrape({
        siteType: [Site.N_POWER_MEDICINE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('npowermedicine');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('npowermedicine-x1');
      expect(result.jobs[0].site).toBe(Site.N_POWER_MEDICINE);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('npowermedicine-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new NPowerMedicineService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.N_POWER_MEDICINE],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new NPowerMedicineService();
      const result = await service.scrape({
        siteType: [Site.N_POWER_MEDICINE],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
