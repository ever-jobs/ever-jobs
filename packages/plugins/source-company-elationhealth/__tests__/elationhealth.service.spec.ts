...
import { ElationHealthModule, ElationHealthService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'elationhealth-jobs.json'), 'utf8'),
);
...
/**
 * Spec 760 / T04 — `ElationHealthService` unit tests (≥ 9 cases).
 */
describe('ElationHealthService — Spec 760 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ElationHealthModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ElationHealthModule],
      }).compile();
      const service = moduleRef.get(ElationHealthService);
      expect(service).toBeInstanceOf(ElationHealthService);
      await moduleRef.close();
    });

    it('exports the Site.ELATION_HEALTH = "elationhealth" enum value', () => {
      expect(Site.ELATION_HEALTH).toBe('elationhealth');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ElationHealthService();
      const result = await service.scrape({
        siteType: [Site.ELATION_HEALTH],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'elationhealth-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ELATION_HEALTH);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Elation Health');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/elationhealth/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/elationhealth/jobs/');
      ...
      expect(job0?.description).toContain('Elation Health');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/elationhealth/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Elation Health');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'elationhealth-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
