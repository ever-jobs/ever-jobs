...
import { ConstellrModule, ConstellrService } from '../src';

const COMPANY_NAME_EXPECT = 'constellr';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'constellr-jobs.json'), 'utf8'),
);
...
describe('ConstellrService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ConstellrModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ConstellrModule],
      }).compile();
      const service = moduleRef.get(ConstellrService);
      expect(service).toBeInstanceOf(ConstellrService);
      await moduleRef.close();
    });

    it('exports the Site.CONSTELLR = "constellr" enum value', () => {
      expect(Site.CONSTELLR).toBe('constellr');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new ConstellrService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.CONSTELLR],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('constellr.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.CONSTELLR);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('constellr-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new ConstellrService(registry);
      const result = await service.scrape({
        siteType: [Site.CONSTELLR],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('constellr');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('constellr-x1');
      expect(result.jobs[0].site).toBe(Site.CONSTELLR);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('constellr-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new ConstellrService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.CONSTELLR],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new ConstellrService();
      const result = await service.scrape({
        siteType: [Site.CONSTELLR],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
