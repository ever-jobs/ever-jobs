...
import { KittitasValleyHealthcareModule, KittitasValleyHealthcareService } from '../src';

const COMPANY_NAME_EXPECT = 'Kittitas Valley Healthcare';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'kittitasvalleyhealthcare-jobs.json'), 'utf8'),
);
...
describe('KittitasValleyHealthcareService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through KittitasValleyHealthcareModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [KittitasValleyHealthcareModule],
      }).compile();
      const service = moduleRef.get(KittitasValleyHealthcareService);
      expect(service).toBeInstanceOf(KittitasValleyHealthcareService);
      await moduleRef.close();
    });

    it('exports the Site.KITTITAS_VALLEY_HEALTHCARE = "kittitasvalleyhealthcare" enum value', () => {
      expect(Site.KITTITAS_VALLEY_HEALTHCARE).toBe('kittitasvalleyhealthcare');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new KittitasValleyHealthcareService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.KITTITAS_VALLEY_HEALTHCARE],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'kittitasvalleyhealthcare-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.KITTITAS_VALLEY_HEALTHCARE);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('kittitasvalleyhealthcare-' + first.id);
      ...
      expect(calledUrls[0]).toContain('KittitasValleyHealthcare');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.KITTITAS_VALLEY_HEALTHCARE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('kittitasvalleyhealthcare-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new KittitasValleyHealthcareService(registry);
      const result = await service.scrape({
        siteType: [Site.KITTITAS_VALLEY_HEALTHCARE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('KittitasValleyHealthcare');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('kittitasvalleyhealthcare-x1');
      expect(result.jobs[0].site).toBe(Site.KITTITAS_VALLEY_HEALTHCARE);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('kittitasvalleyhealthcare-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new KittitasValleyHealthcareService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.KITTITAS_VALLEY_HEALTHCARE],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new KittitasValleyHealthcareService();
      const result = await service.scrape({
        siteType: [Site.KITTITAS_VALLEY_HEALTHCARE],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
