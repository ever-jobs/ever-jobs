...
import { NetspendModule, NetspendService } from '../src';

const COMPANY_NAME_EXPECT = 'Netspend';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'netspend-jobs.json'), 'utf8'),
);
...
describe('NetspendService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through NetspendModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [NetspendModule],
      }).compile();
      const service = moduleRef.get(NetspendService);
      expect(service).toBeInstanceOf(NetspendService);
      await moduleRef.close();
    });

    it('exports the Site.NETSPEND = "netspend" enum value', () => {
      expect(Site.NETSPEND).toBe('netspend');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new NetspendService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.NETSPEND],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('netspend-careers-page');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.NETSPEND);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('netspend-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new NetspendService(registry);
      const result = await service.scrape({
        siteType: [Site.NETSPEND],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('netspend-careers-page');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('netspend-x1');
      expect(result.jobs[0].site).toBe(Site.NETSPEND);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('netspend-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new NetspendService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.NETSPEND],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new NetspendService();
      const result = await service.scrape({
        siteType: [Site.NETSPEND],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
