...
import { ShipBobModule, ShipBobService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'shipbobinc-jobs.json'), 'utf8'),
);
...
/**
 * Spec 931 / T04 — `ShipBobService` unit tests (≥ 9 cases).
 */
describe('ShipBobService — Spec 931 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ShipBobModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ShipBobModule],
      }).compile();
      const service = moduleRef.get(ShipBobService);
      expect(service).toBeInstanceOf(ShipBobService);
      await moduleRef.close();
    });

    it('exports the Site.SHIPBOB = "shipbobinc" enum value', () => {
      expect(Site.SHIPBOB).toBe('shipbobinc');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ShipBobService();
      const result = await service.scrape({
        siteType: [Site.SHIPBOB],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'shipbobinc-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.SHIPBOB);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('ShipBob');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/shipbobinc/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/shipbobinc/jobs/');
      ...
      expect(job0?.description).toContain('ShipBob');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/shipbobinc/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('ShipBob');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'shipbobinc-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
