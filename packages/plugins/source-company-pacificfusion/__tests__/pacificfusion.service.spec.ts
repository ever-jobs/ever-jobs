...
import { PacificfusionModule, PacificfusionService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'pacificfusion-jobs.json'), 'utf8'),
);
...
/**
 * Spec 494 / T04 — `PacificfusionService` unit tests (≥ 9 cases).
 */
describe('PacificfusionService — Spec 494 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through PacificfusionModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [PacificfusionModule],
      }).compile();
      const service = moduleRef.get(PacificfusionService);
      expect(service).toBeInstanceOf(PacificfusionService);
      await moduleRef.close();
    });

    it('exports the Site.PACIFICFUSION = "pacificfusion" enum value', () => {
      expect(Site.PACIFICFUSION).toBe('pacificfusion');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new PacificfusionService();
      const result = await service.scrape({
        siteType: [Site.PACIFICFUSION],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'pacificfusion-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.PACIFICFUSION);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Pacific Fusion');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/pacificfusion/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/pacificfusion/jobs/');
      ...
      expect(job0?.description).toContain('Pacific Fusion');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/pacificfusion/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Pacific Fusion');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'pacificfusion-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
