...
import { DiligentroboticsModule, DiligentroboticsService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'diligentrobotics-jobs.json'), 'utf8'),
);
...
/**
 * Spec 558 / T04 — `DiligentroboticsService` unit tests (≥ 9 cases).
 */
describe('DiligentroboticsService — Spec 558 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through DiligentroboticsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [DiligentroboticsModule],
      }).compile();
      const service = moduleRef.get(DiligentroboticsService);
      expect(service).toBeInstanceOf(DiligentroboticsService);
      await moduleRef.close();
    });

    it('exports the Site.DILIGENT_ROBOTICS = "diligentrobotics" enum value', () => {
      expect(Site.DILIGENT_ROBOTICS).toBe('diligentrobotics');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new DiligentroboticsService();
      const result = await service.scrape({
        siteType: [Site.DILIGENT_ROBOTICS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'diligentrobotics-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.DILIGENT_ROBOTICS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Diligent Robotics');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/diligentrobotics/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/diligentrobotics/jobs/');
      ...
      expect(job0?.description).toContain('Diligent Robotics');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/diligentrobotics/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Diligent Robotics');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'diligentrobotics-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
