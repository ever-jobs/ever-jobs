...
import { AegisventuresModule, AegisventuresService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'aegisventures-jobs.json'), 'utf8'),
);
...
/**
 * Spec 197 / T04 — `AegisventuresService` unit tests (≥ 9 cases).
 */
describe('AegisventuresService — Spec 197 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AegisventuresModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AegisventuresModule],
      }).compile();
      const service = moduleRef.get(AegisventuresService);
      expect(service).toBeInstanceOf(AegisventuresService);
      await moduleRef.close();
    });

    it('exports the Site.AEGISVENTURES = "aegisventures" enum value', () => {
      expect(Site.AEGISVENTURES).toBe('aegisventures');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AegisventuresService();
      const result = await service.scrape({
        siteType: [Site.AEGISVENTURES],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'aegisventures-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.AEGISVENTURES);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Aegis Ventures');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/aegisventures/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/aegisventures/jobs/');
      ...
      expect(job0?.description).toContain('Aegis Ventures');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/aegisventures/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Aegis Ventures');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'aegisventures-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
