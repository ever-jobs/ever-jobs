...
import { GeneratebiomedicinesModule, GeneratebiomedicinesService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'generatebiomedicines-jobs.json'), 'utf8'),
);
...
/**
 * Spec 530 / T04 — `GeneratebiomedicinesService` unit tests (≥ 9 cases).
 */
describe('GeneratebiomedicinesService — Spec 530 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through GeneratebiomedicinesModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [GeneratebiomedicinesModule],
      }).compile();
      const service = moduleRef.get(GeneratebiomedicinesService);
      expect(service).toBeInstanceOf(GeneratebiomedicinesService);
      await moduleRef.close();
    });

    it('exports the Site.GENERATE_BIOMEDICINES = "generatebiomedicines" enum value', () => {
      expect(Site.GENERATE_BIOMEDICINES).toBe('generatebiomedicines');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new GeneratebiomedicinesService();
      const result = await service.scrape({
        siteType: [Site.GENERATE_BIOMEDICINES],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'generatebiomedicines-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.GENERATE_BIOMEDICINES);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Generate Biomedicines');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/generatebiomedicines/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/generatebiomedicines/jobs/');
      ...
      expect(job0?.description).toContain('Generate Biomedicines');
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Generate Biomedicines');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'generatebiomedicines-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
