...
/**
 * G-Research — Quantitative research (HQ: London, United Kingdom).
 *
 * Source (Spec 1737): Workday board, scraped in this order:
 *   - `gresearch:103:G-Research` — https://gresearch.wd103.myworkdayjobs.com/G-Research
 *     verified live 2026-09-24: 64 open postings.
 *
 ...
 * party board.
 *
 * Tags: segment=quant-trading; industry=quantitative-research.
 */
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'gresearch:103:G-Research', atsIdPrefix: 'wd-gresearch-' },
];

@SourcePlugin({
  site: Site.G_RESEARCH,
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['gresearch.com'],
  description: 'G-Research careers via Workday. Tags: segment=quant-trading; industry=quantitative-research.',
})
@Injectable()
export class GResearchService implements IScraper {
  private readonly logger = new Logger(GResearchService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Workday source plugin is not registered; cannot scrape G-Research');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`G-Research: delegating to Workday (${board.companySlug})`);
    ...
    this.logger.log(`G-Research: scraped ${jobs.length} jobs`);
