...
import { GResearchModule, GResearchService } from '../src';
...
const FIXTURE: Fixture = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'fixtures', 'gresearch-boards.json'), 'utf8'),
);
const COMPANY_NAME = 'G-Research';
const ID_PREFIX = 'gresearch-';
...
describe('GResearchService — Workday delegation (Spec 1737)', () => {
  beforeEach(() => {
    mockGet.mockReset();
    mockPost.mockReset();
  ...
  describe('registration', () => {
    it('resolves through GResearchModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({ imports: [GResearchModule] }).compile();
      expect(moduleRef.get(GResearchService)).toBeInstanceOf(GResearchService);
      await moduleRef.close();
    });

    it('exports Site.G_RESEARCH = "gresearch"', () => {
      expect(Site.G_RESEARCH).toBe('gresearch');
    });

    it('declares a tagged company plugin with its domains', () => {
      const meta = Reflect.getMetadata(SOURCE_PLUGIN_METADATA, GResearchService);
      expect(meta.site).toBe(Site.G_RESEARCH);
      expect(meta.category).toBe('company');
      expect(meta.isAts).toBeFalsy();
      expect(meta.companyDomains).toEqual(['gresearch.com']);
  ...
  describe('recorded board (real Workday adapter, mocked HTTP)', () => {
    it('maps every recorded posting and re-stamps the company identity', async () => {
      const service = new GResearchService(registryWith());
      const result = await service.scrape({ siteType: [Site.G_RESEARCH], resultsWanted: 100 } as ScraperInputDto);
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.G_RESEARCH);
    ...
    it('only requests the recorded board URLs', async () => {
      const service = new GResearchService(registryWith());
      await service.scrape({ siteType: [Site.G_RESEARCH] } as ScraperInputDto);
    ...
    it('honours resultsWanted=1', async () => {
      const service = new GResearchService(registryWith());
      const result = await service.scrape({ siteType: [Site.G_RESEARCH], resultsWanted: 1 } as ScraperInputDto);
    ...
    it('resolves with an empty result when the board is gone (HTTP 404)', async () => {
      ...
      const service = new GResearchService(registryWith());
      const result = await service.scrape({ siteType: [Site.G_RESEARCH] } as ScraperInputDto);
  ...
  describe('delegation contract', () => {
    it('forwards the board and the caller input untouched', async () => {
      ...
      const service = new GResearchService(
        registryWith(
          fakeBackend(
      ...
      expect(result.jobs[0].site).toBe(Site.G_RESEARCH);
      expect(result.jobs[0].companyName).toBe(COMPANY_NAME);
    });

    it('passes an absent resultsWanted through as absent', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new GResearchService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      await service.scrape({ siteType: [Site.G_RESEARCH] } as ScraperInputDto);
    ...
    it('makes no request when resultsWanted is 0', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new GResearchService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      const result = await service.scrape({ siteType: [Site.G_RESEARCH], resultsWanted: 0 } as ScraperInputDto);
  ...
  describe('resilience', () => {
    it('reports not_registered when no registry is injected', async () => {
      const result = await new GResearchService().scrape({ siteType: [Site.G_RESEARCH] } as ScraperInputDto);
      expect(result.jobs).toEqual([]);
    ...
    it('reports not_registered when Workday is missing from the registry', async () => {
      const result = await new GResearchService(new PluginRegistry()).scrape({
        siteType: [Site.G_RESEARCH],
      } as ScraperInputDto);
      expect(result.diagnostics?.reason).toBe('not_registered');
    });
