...
import { DeepgramModule, DeepgramService } from '../src';

const COMPANY_NAME_EXPECT = 'Deepgram';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'deepgram-jobs.json'), 'utf8'),
);
...
describe('DeepgramService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through DeepgramModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [DeepgramModule],
      }).compile();
      const service = moduleRef.get(DeepgramService);
      expect(service).toBeInstanceOf(DeepgramService);
      await moduleRef.close();
    });

    it('exports the Site.DEEPGRAM = "deepgram" enum value', () => {
      expect(Site.DEEPGRAM).toBe('deepgram');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new DeepgramService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.DEEPGRAM],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('deepgram');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.DEEPGRAM);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('deepgram-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new DeepgramService(registry);
      const result = await service.scrape({
        siteType: [Site.DEEPGRAM],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('deepgram');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('deepgram-x1');
      expect(result.jobs[0].site).toBe(Site.DEEPGRAM);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('deepgram-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new DeepgramService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.DEEPGRAM],
      } as ScraperInputDto);
