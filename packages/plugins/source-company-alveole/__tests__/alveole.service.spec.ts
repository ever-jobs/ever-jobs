...
import { AlveoleModule, AlveoleService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'alveole-jobs.json'), 'utf8'),
);
...
/**
 * Spec 253 / T04 — `AlveoleService` unit tests (≥ 9 cases).
 */
describe('AlveoleService — Spec 253 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AlveoleModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AlveoleModule],
      }).compile();
      const service = moduleRef.get(AlveoleService);
      expect(service).toBeInstanceOf(AlveoleService);
      await moduleRef.close();
    });

    it('exports the Site.ALVEOLE = "alveole" enum value', () => {
      expect(Site.ALVEOLE).toBe('alveole');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AlveoleService();
      const result = await service.scrape({
        siteType: [Site.ALVEOLE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'alveole-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ALVEOLE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Alveole');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/alveole/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/alveole/jobs/');
      ...
      expect(job0?.description).toContain('Alveole');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/alveole/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Alveole');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'alveole-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
