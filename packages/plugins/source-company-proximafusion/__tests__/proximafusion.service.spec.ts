...
import { ProximaFusionModule, ProximaFusionService } from '../src';

const COMPANY_NAME_EXPECT = 'Proxima Fusion';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'proximafusion-jobs.json'), 'utf8'),
);
...
describe('ProximaFusionService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ProximaFusionModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ProximaFusionModule],
      }).compile();
      const service = moduleRef.get(ProximaFusionService);
      expect(service).toBeInstanceOf(ProximaFusionService);
      await moduleRef.close();
    });

    it('exports the Site.PROXIMA_FUSION = "proximafusion" enum value', () => {
      expect(Site.PROXIMA_FUSION).toBe('proximafusion');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new ProximaFusionService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.PROXIMA_FUSION],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'proximafusion-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.PROXIMA_FUSION);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('proximafusion-' + first.id);
      ...
      expect(calledUrls[0]).toContain('proxima-fusion');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.PROXIMA_FUSION);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('proximafusion-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new ProximaFusionService(registry);
      const result = await service.scrape({
        siteType: [Site.PROXIMA_FUSION],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('proxima-fusion');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('proximafusion-x1');
      expect(result.jobs[0].site).toBe(Site.PROXIMA_FUSION);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('proximafusion-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new ProximaFusionService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.PROXIMA_FUSION],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new ProximaFusionService();
      const result = await service.scrape({
        siteType: [Site.PROXIMA_FUSION],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
