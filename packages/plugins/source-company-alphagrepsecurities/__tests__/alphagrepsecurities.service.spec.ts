...
import { AlphagrepsecuritiesModule, AlphagrepsecuritiesService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'alphagrepsecurities-jobs.json'), 'utf8'),
);
...
/**
 * Spec 242 / T04 — `AlphagrepsecuritiesService` unit tests (≥ 9 cases).
 */
describe('AlphagrepsecuritiesService — Spec 242 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AlphagrepsecuritiesModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AlphagrepsecuritiesModule],
      }).compile();
      const service = moduleRef.get(AlphagrepsecuritiesService);
      expect(service).toBeInstanceOf(AlphagrepsecuritiesService);
      await moduleRef.close();
    });

    it('exports the Site.ALPHAGREPSECURITIES = "alphagrepsecurities" enum value', () => {
      expect(Site.ALPHAGREPSECURITIES).toBe('alphagrepsecurities');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AlphagrepsecuritiesService();
      const result = await service.scrape({
        siteType: [Site.ALPHAGREPSECURITIES],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'alphagrepsecurities-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ALPHAGREPSECURITIES);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('AlphaGrep Securities');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/alphagrepsecurities/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/alphagrepsecurities/jobs/');
      ...
      expect(job0?.description).toContain('AlphaGrep Securities');
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('AlphaGrep Securities');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'alphagrepsecurities-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
