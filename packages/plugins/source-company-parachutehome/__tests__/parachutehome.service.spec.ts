...
import { ParachuteHomeModule, ParachuteHomeService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'parachutehome-jobs.json'), 'utf8'),
);
...
/**
 * Spec 581 / T04 — `ParachuteHomeService` unit tests (≥ 9 cases).
 */
describe('ParachuteHomeService — Spec 581 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ParachuteHomeModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ParachuteHomeModule],
      }).compile();
      const service = moduleRef.get(ParachuteHomeService);
      expect(service).toBeInstanceOf(ParachuteHomeService);
      await moduleRef.close();
    });

    it('exports the Site.PARACHUTE_HOME = "parachutehome" enum value', () => {
      expect(Site.PARACHUTE_HOME).toBe('parachutehome');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ParachuteHomeService();
      const result = await service.scrape({
        siteType: [Site.PARACHUTE_HOME],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'parachutehome-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.PARACHUTE_HOME);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Parachute Home');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/parachutehome/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/parachutehome/jobs/');
      ...
      expect(job0?.description).toContain('Parachute Home');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/parachutehome/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Parachute Home');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'parachutehome-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
