...
import { PayoneerModule, PayoneerService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'payoneer-jobs.json'), 'utf8'),
);
...
/**
 * Spec 910 / T04 — `PayoneerService` unit tests (≥ 9 cases).
 */
describe('PayoneerService — Spec 910 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through PayoneerModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [PayoneerModule],
      }).compile();
      const service = moduleRef.get(PayoneerService);
      expect(service).toBeInstanceOf(PayoneerService);
      await moduleRef.close();
    });

    it('exports the Site.PAYONEER = "payoneer" enum value', () => {
      expect(Site.PAYONEER).toBe('payoneer');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new PayoneerService();
      const result = await service.scrape({
        siteType: [Site.PAYONEER],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'payoneer-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.PAYONEER);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Payoneer');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/payoneer/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/payoneer/jobs/');
      ...
      expect(job0?.description).toContain('Payoneer');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/payoneer/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Payoneer');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'payoneer-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
