...
import { HopperModule, HopperService } from '../src';

const COMPANY_NAME_EXPECT = 'Hopper';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'hopper-jobs.json'), 'utf8'),
);
...
describe('HopperService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through HopperModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [HopperModule],
      }).compile();
      const service = moduleRef.get(HopperService);
      expect(service).toBeInstanceOf(HopperService);
      await moduleRef.close();
    });

    it('exports the Site.HOPPER = "hopper" enum value', () => {
      expect(Site.HOPPER).toBe('hopper');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new HopperService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.HOPPER],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('hopper');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.HOPPER);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('hopper-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new HopperService(registry);
      const result = await service.scrape({
        siteType: [Site.HOPPER],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('hopper');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('hopper-x1');
      expect(result.jobs[0].site).toBe(Site.HOPPER);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('hopper-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new HopperService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.HOPPER],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new HopperService();
      const result = await service.scrape({
        siteType: [Site.HOPPER],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
