...
import { BoydGamingModule, BoydGamingService } from '../src';

const COMPANY_NAME_EXPECT = 'Boyd Gaming';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'boydgaming-jobs.json'), 'utf8'),
);
...
describe('BoydGamingService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through BoydGamingModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [BoydGamingModule],
      }).compile();
      const service = moduleRef.get(BoydGamingService);
      expect(service).toBeInstanceOf(BoydGamingService);
      await moduleRef.close();
    });

    it('exports the Site.BOYD_GAMING = "boydgaming" enum value', () => {
      expect(Site.BOYD_GAMING).toBe('boydgaming');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new BoydGamingService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.BOYD_GAMING],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('BoydGaming');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.BOYD_GAMING);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('boydgaming-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new BoydGamingService(registry);
      const result = await service.scrape({
        siteType: [Site.BOYD_GAMING],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('BoydGaming');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('boydgaming-x1');
      expect(result.jobs[0].site).toBe(Site.BOYD_GAMING);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('boydgaming-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new BoydGamingService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.BOYD_GAMING],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new BoydGamingService();
      const result = await service.scrape({
        siteType: [Site.BOYD_GAMING],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
