...
import { KasaModule, KasaService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'kasa-jobs.json'), 'utf8'),
);
...
/**
 * Spec 875 / T04 — `KasaService` unit tests (≥ 9 cases).
 */
describe('KasaService — Spec 875 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through KasaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [KasaModule],
      }).compile();
      const service = moduleRef.get(KasaService);
      expect(service).toBeInstanceOf(KasaService);
      await moduleRef.close();
    });

    it('exports the Site.KASA = "kasa" enum value', () => {
      expect(Site.KASA).toBe('kasa');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const job0 = dto.jobs.find((j) => j.id === 'kasa-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.KASA);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Kasa');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/kasa/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/kasa/jobs/');
      ...
      expect(job0?.description).toContain('Kasa');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/kasa/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Kasa');
      }
    });
  });
