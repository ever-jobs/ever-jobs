...
import { MitsubishiTanabePharmaAmericaModule, MitsubishiTanabePharmaAmericaService } from '../src';

const COMPANY_NAME_EXPECT = 'Mitsubishi Tanabe Pharma America';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'mitsubishitanabepharmaamerica-jobs.json'), 'utf8'),
);
...
describe('MitsubishiTanabePharmaAmericaService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through MitsubishiTanabePharmaAmericaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [MitsubishiTanabePharmaAmericaModule],
      }).compile();
      const service = moduleRef.get(MitsubishiTanabePharmaAmericaService);
      expect(service).toBeInstanceOf(MitsubishiTanabePharmaAmericaService);
      await moduleRef.close();
    });

    it('exports the Site.MITSUBISHI_TANABE_PHARMA_AMERICA = "mitsubishitanabepharmaamerica" enum value', () => {
      expect(Site.MITSUBISHI_TANABE_PHARMA_AMERICA).toBe('mitsubishitanabepharmaamerica');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new MitsubishiTanabePharmaAmericaService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.MITSUBISHI_TANABE_PHARMA_AMERICA],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'mitsubishitanabepharmaamerica-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.MITSUBISHI_TANABE_PHARMA_AMERICA);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('mitsubishitanabepharmaamerica-' + first.id);
      ...
      expect(calledUrls[0]).toContain('MitsubishiTanabePharmaAmerica');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.MITSUBISHI_TANABE_PHARMA_AMERICA);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('mitsubishitanabepharmaamerica-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new MitsubishiTanabePharmaAmericaService(registry);
      const result = await service.scrape({
        siteType: [Site.MITSUBISHI_TANABE_PHARMA_AMERICA],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('MitsubishiTanabePharmaAmerica');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('mitsubishitanabepharmaamerica-x1');
      expect(result.jobs[0].site).toBe(Site.MITSUBISHI_TANABE_PHARMA_AMERICA);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('mitsubishitanabepharmaamerica-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new MitsubishiTanabePharmaAmericaService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.MITSUBISHI_TANABE_PHARMA_AMERICA],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new MitsubishiTanabePharmaAmericaService();
      const result = await service.scrape({
        siteType: [Site.MITSUBISHI_TANABE_PHARMA_AMERICA],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
