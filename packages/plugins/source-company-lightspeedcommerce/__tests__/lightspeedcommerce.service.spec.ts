...
import { LightspeedCommerceModule, LightspeedCommerceService } from '../src';

const COMPANY_NAME_EXPECT = 'Lightspeed Commerce';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'lightspeedcommerce-jobs.json'), 'utf8'),
);
...
describe('LightspeedCommerceService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through LightspeedCommerceModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [LightspeedCommerceModule],
      }).compile();
      const service = moduleRef.get(LightspeedCommerceService);
      expect(service).toBeInstanceOf(LightspeedCommerceService);
      await moduleRef.close();
    });

    it('exports the Site.LIGHTSPEED_COMMERCE = "lightspeedcommerce" enum value', () => {
      expect(Site.LIGHTSPEED_COMMERCE).toBe('lightspeedcommerce');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new LightspeedCommerceService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.LIGHTSPEED_COMMERCE],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'lightspeedcommerce-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.LIGHTSPEED_COMMERCE);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('lightspeedcommerce-' + first.id);
      ...
      expect(calledUrls[0]).toContain('lightspeedhq');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.LIGHTSPEED_COMMERCE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('lightspeedcommerce-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new LightspeedCommerceService(registry);
      const result = await service.scrape({
        siteType: [Site.LIGHTSPEED_COMMERCE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('lightspeedhq');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('lightspeedcommerce-x1');
      expect(result.jobs[0].site).toBe(Site.LIGHTSPEED_COMMERCE);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('lightspeedcommerce-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new LightspeedCommerceService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.LIGHTSPEED_COMMERCE],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new LightspeedCommerceService();
      const result = await service.scrape({
        siteType: [Site.LIGHTSPEED_COMMERCE],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
