...
import { LotusHealthModule, LotusHealthService } from '../src';

const COMPANY_NAME_EXPECT = 'Lotus Health';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'lotushealth-jobs.json'), 'utf8'),
);
...
describe('LotusHealthService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through LotusHealthModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [LotusHealthModule],
      }).compile();
      const service = moduleRef.get(LotusHealthService);
      expect(service).toBeInstanceOf(LotusHealthService);
      await moduleRef.close();
    });

    it('exports the Site.LOTUS_HEALTH = "lotushealth" enum value', () => {
      expect(Site.LOTUS_HEALTH).toBe('lotushealth');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new LotusHealthService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.LOTUS_HEALTH],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'lotushealth-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.LOTUS_HEALTH);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('lotushealth-' + first.id);
      ...
      expect(calledUrls[0]).toContain('lotushealth');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.LOTUS_HEALTH);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('lotushealth-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new LotusHealthService(registry);
      const result = await service.scrape({
        siteType: [Site.LOTUS_HEALTH],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('lotushealth');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('lotushealth-x1');
      expect(result.jobs[0].site).toBe(Site.LOTUS_HEALTH);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('lotushealth-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new LotusHealthService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.LOTUS_HEALTH],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new LotusHealthService();
      const result = await service.scrape({
        siteType: [Site.LOTUS_HEALTH],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
