...
import { SimplisolarModule, SimplisolarService } from '../src';

const COMPANY_NAME_EXPECT = 'Simplisolar';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'simplisolar-jobs.json'), 'utf8'),
);
...
describe('SimplisolarService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SimplisolarModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SimplisolarModule],
      }).compile();
      const service = moduleRef.get(SimplisolarService);
      expect(service).toBeInstanceOf(SimplisolarService);
      await moduleRef.close();
    });

    it('exports the Site.SIMPLISOLAR = "simplisolar" enum value', () => {
      expect(Site.SIMPLISOLAR).toBe('simplisolar');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new SimplisolarService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.SIMPLISOLAR],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('Simplisolar');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SIMPLISOLAR);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('simplisolar-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new SimplisolarService(registry);
      const result = await service.scrape({
        siteType: [Site.SIMPLISOLAR],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('Simplisolar');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('simplisolar-x1');
      expect(result.jobs[0].site).toBe(Site.SIMPLISOLAR);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('simplisolar-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new SimplisolarService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.SIMPLISOLAR],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new SimplisolarService();
      const result = await service.scrape({
        siteType: [Site.SIMPLISOLAR],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
