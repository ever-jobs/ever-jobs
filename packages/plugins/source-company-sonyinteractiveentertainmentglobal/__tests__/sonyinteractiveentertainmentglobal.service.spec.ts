...
import { PlayStationSonyInteractiveEntertainmentModule, PlayStationSonyInteractiveEntertainmentService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'sonyinteractiveentertainmentglobal-jobs.json'), 'utf8'),
);
...
/**
 * Spec 937 / T04 — `PlayStationSonyInteractiveEntertainmentService` unit tests (≥ 9 cases).
 */
describe('PlayStationSonyInteractiveEntertainmentService — Spec 937 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through PlayStationSonyInteractiveEntertainmentModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [PlayStationSonyInteractiveEntertainmentModule],
      }).compile();
      const service = moduleRef.get(PlayStationSonyInteractiveEntertainmentService);
      expect(service).toBeInstanceOf(PlayStationSonyInteractiveEntertainmentService);
      await moduleRef.close();
    });

    it('exports the Site.PLAYSTATION_SONY_INTERACTIVE_ENTERTAINMENT = "sonyinteractiveentertainmentglobal" enum value', () => {
      expect(Site.PLAYSTATION_SONY_INTERACTIVE_ENTERTAINMENT).toBe('sonyinteractiveentertainmentglobal');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new PlayStationSonyInteractiveEntertainmentService();
      const result = await service.scrape({
        siteType: [Site.PLAYSTATION_SONY_INTERACTIVE_ENTERTAINMENT],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'sonyinteractiveentertainmentglobal-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.PLAYSTATION_SONY_INTERACTIVE_ENTERTAINMENT);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('PlayStation (Sony Interactive Entertainment)');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/sonyinteractiveentertainmentglobal/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/sonyinteractiveentertainmentglobal/jobs/');
      ...
      expect(job0?.description).toContain('PlayStation (Sony Interactive Entertainment)');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/sonyinteractiveentertainmentglobal/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('PlayStation (Sony Interactive Entertainment)');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'sonyinteractiveentertainmentglobal-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
