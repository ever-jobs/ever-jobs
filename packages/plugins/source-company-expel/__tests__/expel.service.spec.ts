...
import { ExpelModule, ExpelService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'expel-jobs.json'), 'utf8'),
);
...
/**
 * Spec 543 / T04 — `ExpelService` unit tests (≥ 9 cases).
 */
describe('ExpelService — Spec 543 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ExpelModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ExpelModule],
      }).compile();
      const service = moduleRef.get(ExpelService);
      expect(service).toBeInstanceOf(ExpelService);
      await moduleRef.close();
    });

    it('exports the Site.EXPEL = "expel" enum value', () => {
      expect(Site.EXPEL).toBe('expel');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ExpelService();
      ...
      const job0 = dto.jobs.find((j) => j.id === 'expel-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.EXPEL);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Expel');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/expel/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/expel/jobs/');
      ...
      expect(job0?.description).toContain('Expel');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/expel/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Expel');
      }
    });
  });
