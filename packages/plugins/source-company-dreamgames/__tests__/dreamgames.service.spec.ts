...
import { DreamGamesModule, DreamGamesService } from '../src';

const COMPANY_NAME_EXPECT = 'Dream Games';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'dreamgames-jobs.json'), 'utf8'),
);
...
describe('DreamGamesService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through DreamGamesModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [DreamGamesModule],
      }).compile();
      const service = moduleRef.get(DreamGamesService);
      expect(service).toBeInstanceOf(DreamGamesService);
      await moduleRef.close();
    });

    it('exports the Site.DREAM_GAMES = "dreamgames" enum value', () => {
      expect(Site.DREAM_GAMES).toBe('dreamgames');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new DreamGamesService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.DREAM_GAMES],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('dreamgames');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.DREAM_GAMES);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('dreamgames-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new DreamGamesService(registry);
      const result = await service.scrape({
        siteType: [Site.DREAM_GAMES],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('dreamgames');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('dreamgames-x1');
      expect(result.jobs[0].site).toBe(Site.DREAM_GAMES);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('dreamgames-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new DreamGamesService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.DREAM_GAMES],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new DreamGamesService();
      const result = await service.scrape({
        siteType: [Site.DREAM_GAMES],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
