...
import { SalesloftModule, SalesloftService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'salesloft-jobs.json'), 'utf8'),
);
...
/**
 * Spec 453 / T04 — `SalesloftService` unit tests (≥ 9 cases).
 */
describe('SalesloftService — Spec 453 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SalesloftModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SalesloftModule],
      }).compile();
      const service = moduleRef.get(SalesloftService);
      expect(service).toBeInstanceOf(SalesloftService);
      await moduleRef.close();
    });

    it('exports the Site.SALESLOFT = "salesloft" enum value', () => {
      expect(Site.SALESLOFT).toBe('salesloft');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new SalesloftService();
      const result = await service.scrape({
        siteType: [Site.SALESLOFT],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'salesloft-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.SALESLOFT);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Salesloft');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/salesloft/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/salesloft/jobs/');
      ...
      expect(job0?.description).toContain('Salesloft');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/salesloft/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Salesloft');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'salesloft-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
