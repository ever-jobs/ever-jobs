...
import { BlockchainComModule, BlockchainComService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'blockchain-jobs.json'), 'utf8'),
);
...
/**
 * Spec 817 / T04 — `BlockchainComService` unit tests (≥ 9 cases).
 */
describe('BlockchainComService — Spec 817 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through BlockchainComModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [BlockchainComModule],
      }).compile();
      const service = moduleRef.get(BlockchainComService);
      expect(service).toBeInstanceOf(BlockchainComService);
      await moduleRef.close();
    });

    it('exports the Site.BLOCKCHAIN_COM = "blockchain" enum value', () => {
      expect(Site.BLOCKCHAIN_COM).toBe('blockchain');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new BlockchainComService();
      const result = await service.scrape({
        siteType: [Site.BLOCKCHAIN_COM],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'blockchain-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.BLOCKCHAIN_COM);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Blockchain.com');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/blockchain/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/blockchain/jobs/');
      ...
      expect(job0?.description).toContain('Blockchain.com');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/blockchain/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Blockchain.com');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'blockchain-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
