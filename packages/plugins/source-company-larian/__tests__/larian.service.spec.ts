...
import { LarianStudiosModule, LarianStudiosService } from '../src';

const COMPANY_NAME_EXPECT = 'Larian Studios';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'larian-jobs.json'), 'utf8'),
);
...
describe('LarianStudiosService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through LarianStudiosModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [LarianStudiosModule],
      }).compile();
      const service = moduleRef.get(LarianStudiosService);
      expect(service).toBeInstanceOf(LarianStudiosService);
      await moduleRef.close();
    });

    it('exports the Site.LARIAN_STUDIOS = "larian" enum value', () => {
      expect(Site.LARIAN_STUDIOS).toBe('larian');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new LarianStudiosService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.LARIAN_STUDIOS],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('larian');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.LARIAN_STUDIOS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('larian-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new LarianStudiosService(registry);
      const result = await service.scrape({
        siteType: [Site.LARIAN_STUDIOS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('larian');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('larian-x1');
      expect(result.jobs[0].site).toBe(Site.LARIAN_STUDIOS);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('larian-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new LarianStudiosService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.LARIAN_STUDIOS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new LarianStudiosService();
      const result = await service.scrape({
        siteType: [Site.LARIAN_STUDIOS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
