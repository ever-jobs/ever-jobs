...
import { CertiKModule, CertiKService } from '../src';

const COMPANY_NAME_EXPECT = 'CertiK';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'certik-jobs.json'), 'utf8'),
);
...
describe('CertiKService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CertiKModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CertiKModule],
      }).compile();
      const service = moduleRef.get(CertiKService);
      expect(service).toBeInstanceOf(CertiKService);
      await moduleRef.close();
    });

    it('exports the Site.CERTIK = "certik" enum value', () => {
      expect(Site.CERTIK).toBe('certik');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new CertiKService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.CERTIK],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('certik');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.CERTIK);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('certik-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new CertiKService(registry);
      const result = await service.scrape({
        siteType: [Site.CERTIK],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('certik');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('certik-x1');
      expect(result.jobs[0].site).toBe(Site.CERTIK);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('certik-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new CertiKService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.CERTIK],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new CertiKService();
      const result = await service.scrape({
        siteType: [Site.CERTIK],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
