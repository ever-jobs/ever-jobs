...
import { VattenfallModule, VattenfallService } from '../src';

const COMPANY_NAME_EXPECT = 'Vattenfall';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'vattenfall-jobs.json'), 'utf8'),
);
...
describe('VattenfallService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through VattenfallModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [VattenfallModule],
      }).compile();
      const service = moduleRef.get(VattenfallService);
      expect(service).toBeInstanceOf(VattenfallService);
      await moduleRef.close();
    });

    it('exports the Site.VATTENFALL = "vattenfall" enum value', () => {
      expect(Site.VATTENFALL).toBe('vattenfall');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new VattenfallService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.VATTENFALL],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('Vattenfall');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.VATTENFALL);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('vattenfall-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new VattenfallService(registry);
      const result = await service.scrape({
        siteType: [Site.VATTENFALL],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('Vattenfall');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('vattenfall-x1');
      expect(result.jobs[0].site).toBe(Site.VATTENFALL);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('vattenfall-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new VattenfallService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.VATTENFALL],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new VattenfallService();
      const result = await service.scrape({
        siteType: [Site.VATTENFALL],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
