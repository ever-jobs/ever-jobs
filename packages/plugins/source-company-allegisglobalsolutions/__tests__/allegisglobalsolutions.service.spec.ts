...
import { AllegisGlobalSolutionsModule, AllegisGlobalSolutionsService } from '../src';

const COMPANY_NAME_EXPECT = 'Allegis Global Solutions';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'allegisglobalsolutions-jobs.json'), 'utf8'),
);
...
describe('AllegisGlobalSolutionsService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AllegisGlobalSolutionsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AllegisGlobalSolutionsModule],
      }).compile();
      const service = moduleRef.get(AllegisGlobalSolutionsService);
      expect(service).toBeInstanceOf(AllegisGlobalSolutionsService);
      await moduleRef.close();
    });

    it('exports the Site.ALLEGIS_GLOBAL_SOLUTIONS = "allegisglobalsolutions" enum value', () => {
      expect(Site.ALLEGIS_GLOBAL_SOLUTIONS).toBe('allegisglobalsolutions');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new AllegisGlobalSolutionsService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.ALLEGIS_GLOBAL_SOLUTIONS],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'allegisglobalsolutions-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.ALLEGIS_GLOBAL_SOLUTIONS);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('allegisglobalsolutions-' + first.id);
      ...
      expect(calledUrls[0]).toContain('AllegisGlobalSolutions');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ALLEGIS_GLOBAL_SOLUTIONS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('allegisglobalsolutions-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new AllegisGlobalSolutionsService(registry);
      const result = await service.scrape({
        siteType: [Site.ALLEGIS_GLOBAL_SOLUTIONS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('AllegisGlobalSolutions');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('allegisglobalsolutions-x1');
      expect(result.jobs[0].site).toBe(Site.ALLEGIS_GLOBAL_SOLUTIONS);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('allegisglobalsolutions-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new AllegisGlobalSolutionsService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.ALLEGIS_GLOBAL_SOLUTIONS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new AllegisGlobalSolutionsService();
      const result = await service.scrape({
        siteType: [Site.ALLEGIS_GLOBAL_SOLUTIONS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
