...
import { AstraSecurityModule, AstraSecurityService } from '../src';

const COMPANY_NAME_EXPECT = 'Astra Security';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'astrasecurity-jobs.json'), 'utf8'),
);
...
describe('AstraSecurityService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AstraSecurityModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AstraSecurityModule],
      }).compile();
      const service = moduleRef.get(AstraSecurityService);
      expect(service).toBeInstanceOf(AstraSecurityService);
      await moduleRef.close();
    });

    it('exports the Site.ASTRA_SECURITY = "astrasecurity" enum value', () => {
      expect(Site.ASTRA_SECURITY).toBe('astrasecurity');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new AstraSecurityService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.ASTRA_SECURITY],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'astrasecurity-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.ASTRA_SECURITY);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('astrasecurity-' + first.id);
      ...
      expect(calledUrls[0]).toContain('astra');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ASTRA_SECURITY);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('astrasecurity-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new AstraSecurityService(registry);
      const result = await service.scrape({
        siteType: [Site.ASTRA_SECURITY],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('astra');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('astrasecurity-x1');
      expect(result.jobs[0].site).toBe(Site.ASTRA_SECURITY);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('astrasecurity-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new AstraSecurityService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.ASTRA_SECURITY],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new AstraSecurityService();
      const result = await service.scrape({
        siteType: [Site.ASTRA_SECURITY],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
