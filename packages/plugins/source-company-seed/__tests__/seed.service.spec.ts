...
import { SeedHealthModule, SeedHealthService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'seed-jobs.json'), 'utf8'),
);
...
/**
 * Spec 679 / T04 — `SeedHealthService` unit tests (≥ 9 cases).
 */
describe('SeedHealthService — Spec 679 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SeedHealthModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SeedHealthModule],
      }).compile();
      const service = moduleRef.get(SeedHealthService);
      expect(service).toBeInstanceOf(SeedHealthService);
      await moduleRef.close();
    });

    it('exports the Site.SEED_HEALTH = "seed" enum value', () => {
      expect(Site.SEED_HEALTH).toBe('seed');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new SeedHealthService();
      const result = await service.scrape({
        siteType: [Site.SEED_HEALTH],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'seed-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.SEED_HEALTH);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Seed Health');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/seed/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/seed/jobs/');
      ...
      expect(job0?.description).toContain('Seed Health');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/seed/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Seed Health');
      }
    });
  });
