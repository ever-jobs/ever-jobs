...
import { LendbuzzModule, LendbuzzService } from '../src';

const COMPANY_NAME_EXPECT = 'Lendbuzz';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'lendbuzz-jobs.json'), 'utf8'),
);
...
describe('LendbuzzService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through LendbuzzModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [LendbuzzModule],
      }).compile();
      const service = moduleRef.get(LendbuzzService);
      expect(service).toBeInstanceOf(LendbuzzService);
      await moduleRef.close();
    });

    it('exports the Site.LENDBUZZ = "lendbuzz" enum value', () => {
      expect(Site.LENDBUZZ).toBe('lendbuzz');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new LendbuzzService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.LENDBUZZ],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('lendbuzz');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.LENDBUZZ);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('lendbuzz-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new LendbuzzService(registry);
      const result = await service.scrape({
        siteType: [Site.LENDBUZZ],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('lendbuzz');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('lendbuzz-x1');
      expect(result.jobs[0].site).toBe(Site.LENDBUZZ);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('lendbuzz-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new LendbuzzService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.LENDBUZZ],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new LendbuzzService();
      const result = await service.scrape({
        siteType: [Site.LENDBUZZ],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
