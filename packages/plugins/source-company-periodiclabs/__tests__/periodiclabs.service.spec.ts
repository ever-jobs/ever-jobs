...
import { PeriodicLabsModule, PeriodicLabsService } from '../src';

const COMPANY_NAME_EXPECT = 'Periodic Labs';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'periodiclabs-jobs.json'), 'utf8'),
);
...
describe('PeriodicLabsService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through PeriodicLabsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [PeriodicLabsModule],
      }).compile();
      const service = moduleRef.get(PeriodicLabsService);
      expect(service).toBeInstanceOf(PeriodicLabsService);
      await moduleRef.close();
    });

    it('exports the Site.PERIODIC_LABS = "periodiclabs" enum value', () => {
      expect(Site.PERIODIC_LABS).toBe('periodiclabs');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new PeriodicLabsService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.PERIODIC_LABS],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'periodiclabs-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.PERIODIC_LABS);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('periodiclabs-' + first.id);
      ...
      expect(calledUrls[0]).toContain('periodic-labs');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.PERIODIC_LABS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('periodiclabs-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new PeriodicLabsService(registry);
      const result = await service.scrape({
        siteType: [Site.PERIODIC_LABS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('periodic-labs');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('periodiclabs-x1');
      expect(result.jobs[0].site).toBe(Site.PERIODIC_LABS);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('periodiclabs-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new PeriodicLabsService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.PERIODIC_LABS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new PeriodicLabsService();
      const result = await service.scrape({
        siteType: [Site.PERIODIC_LABS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
