...
import { FlatironHealthModule, FlatironHealthService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'flatironhealth-jobs.json'), 'utf8'),
);
...
/**
 * Spec 761 / T04 — `FlatironHealthService` unit tests (≥ 9 cases).
 */
describe('FlatironHealthService — Spec 761 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through FlatironHealthModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [FlatironHealthModule],
      }).compile();
      const service = moduleRef.get(FlatironHealthService);
      expect(service).toBeInstanceOf(FlatironHealthService);
      await moduleRef.close();
    });

    it('exports the Site.FLATIRON_HEALTH = "flatironhealth" enum value', () => {
      expect(Site.FLATIRON_HEALTH).toBe('flatironhealth');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new FlatironHealthService();
      const result = await service.scrape({
        siteType: [Site.FLATIRON_HEALTH],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'flatironhealth-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.FLATIRON_HEALTH);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Flatiron Health');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/flatironhealth/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/flatironhealth/jobs/');
      ...
      expect(job0?.description).toContain('Flatiron Health');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/flatironhealth/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Flatiron Health');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'flatironhealth-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
