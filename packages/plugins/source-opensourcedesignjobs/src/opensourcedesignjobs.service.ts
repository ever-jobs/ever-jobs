...
@SourcePlugin({
  site: Site.OPENSOURCEDESIGNJOBS,
  name: 'OpenSourceDesignJobs',
  category: 'niche',
})
@Injectable()
export class OpensourcedesignjobsService implements IScraper {
  private readonly logger = new Logger(OpensourcedesignjobsService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    client.setHeaders(OPENSOURCEDESIGNJOBS_HEADERS);

    this.logger.log(
      `Fetching Open Source Design Jobs RSS feed (resultsWanted=${resultsWanted})`,
    );

    try {
      const response = await client.get(OPENSOURCEDESIGNJOBS_RSS_URL);
      const xml = response.data;

      if (!xml || typeof xml !== 'string') {
        this.logger.warn('Empty or invalid RSS response from Open Source Design Jobs');
        return new JobResponseDto([]);
      }

      const items = this.parseRssItems(xml);
      this.logger.log(`Parsed ${items.length} items from Open Source Design Jobs RSS feed`);
      ...
      for (const item of items) {
        ...
        try {
          ...
          this.logger.warn(
            `Error mapping Open Source Design Jobs job ${item.link}: ${err.message}`,
          );
        }
      }

      this.logger.log(`Open Source Design Jobs returned ${jobs.length} jobs`);
      return new JobResponseDto(jobs);
    } catch (err: any) {
      this.logger.error(`Open Source Design Jobs scrape error: ${err.message}`);
      return new JobResponseDto([], classifyScrapeError(err));
    }
  }

  /**
   * Parse RSS XML into an array of OpensourcedesignjobsRssItem using regex (no XML library).
   */
  private parseRssItems(xml: string): OpensourcedesignjobsRssItem[] {
    const items: OpensourcedesignjobsRssItem[] = [];
   ...
   */
  private matchesSearch(item: OpensourcedesignjobsRssItem, searchTerm: string): boolean {
   ...
   */
  private mapJob(
    item: OpensourcedesignjobsRssItem,
    descriptionFormat?: DescriptionFormat,
  ): JobPostDto | null {
    if (!item.title || !item.link) return null;

    // Process description (Open Source Design Jobs RSS may return HTML)
