...
/**
 * Headlands Technologies — Quantitative trading (HQ: Chicago, IL, USA).
 *
 * Source (Spec 1737): Greenhouse board, scraped in this order:
 *   - `headlandstechnologiesllc` — https://job-boards.greenhouse.io/headlandstechnologiesllc
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'headlandstechnologiesllc', atsIdPrefix: 'gh-' },
];

@SourcePlugin({
  site: Site.HEADLANDS_TECH,
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['headlandstech.com'],
  description: 'Headlands Technologies careers via Greenhouse. Tags: segment=quant-trading; industry=quantitative-trading.',
})
@Injectable()
export class HeadlandsTechService implements IScraper {
  private readonly logger = new Logger(HeadlandsTechService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Greenhouse source plugin is not registered; cannot scrape Headlands Technologies');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`Headlands Technologies: delegating to Greenhouse (${board.companySlug})`);
    ...
    this.logger.log(`Headlands Technologies: scraped ${jobs.length} jobs`);
