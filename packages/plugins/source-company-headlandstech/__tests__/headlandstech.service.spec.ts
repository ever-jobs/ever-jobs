...
import { HeadlandsTechModule, HeadlandsTechService } from '../src';
...
const FIXTURE: Fixture = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'fixtures', 'headlandstech-boards.json'), 'utf8'),
);
const COMPANY_NAME = 'Headlands Technologies';
const ID_PREFIX = 'headlandstech-';
...
describe('HeadlandsTechService — Greenhouse delegation (Spec 1737)', () => {
  beforeEach(() => {
    mockGet.mockReset();
    mockPost.mockReset();
  ...
  describe('registration', () => {
    it('resolves through HeadlandsTechModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({ imports: [HeadlandsTechModule] }).compile();
      expect(moduleRef.get(HeadlandsTechService)).toBeInstanceOf(HeadlandsTechService);
      await moduleRef.close();
    });

    it('exports Site.HEADLANDS_TECH = "headlandstech"', () => {
      expect(Site.HEADLANDS_TECH).toBe('headlandstech');
    });

    it('declares a tagged company plugin with its domains', () => {
      const meta = Reflect.getMetadata(SOURCE_PLUGIN_METADATA, HeadlandsTechService);
      expect(meta.site).toBe(Site.HEADLANDS_TECH);
      expect(meta.category).toBe('company');
      expect(meta.isAts).toBeFalsy();
      expect(meta.companyDomains).toEqual(['headlandstech.com']);
  ...
  describe('recorded board (real Greenhouse adapter, mocked HTTP)', () => {
    it('maps every recorded posting and re-stamps the company identity', async () => {
      const service = new HeadlandsTechService(registryWith());
      const result = await service.scrape({ siteType: [Site.HEADLANDS_TECH], resultsWanted: 100 } as ScraperInputDto);
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.HEADLANDS_TECH);
    ...
    it('only requests the recorded board URLs', async () => {
      const service = new HeadlandsTechService(registryWith());
      await service.scrape({ siteType: [Site.HEADLANDS_TECH] } as ScraperInputDto);
    ...
    it('honours resultsWanted=1', async () => {
      const service = new HeadlandsTechService(registryWith());
      const result = await service.scrape({ siteType: [Site.HEADLANDS_TECH], resultsWanted: 1 } as ScraperInputDto);
    ...
    it('resolves with an empty result when the board is gone (HTTP 404)', async () => {
      ...
      const service = new HeadlandsTechService(registryWith());
      const result = await service.scrape({ siteType: [Site.HEADLANDS_TECH] } as ScraperInputDto);
  ...
  describe('delegation contract', () => {
    it('forwards the board and the caller input untouched', async () => {
      ...
      const service = new HeadlandsTechService(
        registryWith(
          fakeBackend(
      ...
      expect(result.jobs[0].site).toBe(Site.HEADLANDS_TECH);
    ...
    it('never forwards the caller\'s credentials to the board (Spec 1735 §4.5)', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new HeadlandsTechService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
    ...
    it('makes no request when resultsWanted is 0', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new HeadlandsTechService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      const result = await service.scrape({ siteType: [Site.HEADLANDS_TECH], resultsWanted: 0 } as ScraperInputDto);
  ...
  describe('resilience', () => {
    it('reports not_registered when no registry is injected', async () => {
      const result = await new HeadlandsTechService().scrape({ siteType: [Site.HEADLANDS_TECH] } as ScraperInputDto);
      expect(result.jobs).toEqual([]);
    ...
    it('reports not_registered when Greenhouse is missing from the registry', async () => {
      const result = await new HeadlandsTechService(new PluginRegistry()).scrape({
        siteType: [Site.HEADLANDS_TECH],
      } as ScraperInputDto);
      expect(result.diagnostics?.reason).toBe('not_registered');
    });
