...
import { AotiModule, AotiService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'aoti-jobs.json'), 'utf8'),
);
...
/**
 * Spec 276 / T04 — `AotiService` unit tests (≥ 9 cases).
 */
describe('AotiService — Spec 276 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AotiModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AotiModule],
      }).compile();
      const service = moduleRef.get(AotiService);
      expect(service).toBeInstanceOf(AotiService);
      await moduleRef.close();
    });

    it('exports the Site.AOTI = "aoti" enum value', () => {
      expect(Site.AOTI).toBe('aoti');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const job0 = dto.jobs.find((j) => j.id === 'aoti-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.AOTI);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('AOTI');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/aoti/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/aoti/jobs/');
      ...
      expect(job0?.description).toContain('AOTI');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/aoti/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('AOTI');
      }
    });
  });
