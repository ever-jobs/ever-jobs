...
import { HeadwayModule, HeadwayService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'headway-jobs.json'), 'utf8'),
);
...
/**
 * Spec 525 / T04 — `HeadwayService` unit tests (≥ 9 cases).
 */
describe('HeadwayService — Spec 525 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through HeadwayModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [HeadwayModule],
      }).compile();
      const service = moduleRef.get(HeadwayService);
      expect(service).toBeInstanceOf(HeadwayService);
      await moduleRef.close();
    });

    it('exports the Site.HEADWAY = "headway" enum value', () => {
      expect(Site.HEADWAY).toBe('headway');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new HeadwayService();
      const result = await service.scrape({
        siteType: [Site.HEADWAY],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'headway-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.HEADWAY);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Headway');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/headway/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/headway/jobs/');
      ...
      expect(job0?.description).toContain('Headway');
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Headway');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'headway-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
