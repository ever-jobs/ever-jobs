...
import { RegalModule, RegalService } from '../src';

const COMPANY_NAME_EXPECT = 'Regal';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'regalai-jobs.json'), 'utf8'),
);
...
describe('RegalService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through RegalModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [RegalModule],
      }).compile();
      const service = moduleRef.get(RegalService);
      expect(service).toBeInstanceOf(RegalService);
      await moduleRef.close();
    });

    it('exports the Site.REGAL = "regalai" enum value', () => {
      expect(Site.REGAL).toBe('regalai');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new RegalService(registryWithLever());
      ...
      expect(calledUrls[0]).toContain('regal.ai');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.REGAL);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('regalai-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new RegalService(registry);
      const result = await service.scrape({
        siteType: [Site.REGAL],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('regal.ai');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('regalai-x1');
      expect(result.jobs[0].site).toBe(Site.REGAL);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('regalai-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new RegalService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.REGAL],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new RegalService();
      const result = await service.scrape({
        siteType: [Site.REGAL],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
