...
import { WorkOSModule, WorkOSService } from '../src';

const COMPANY_NAME_EXPECT = 'WorkOS';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'workos-jobs.json'), 'utf8'),
);
...
describe('WorkOSService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through WorkOSModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [WorkOSModule],
      }).compile();
      const service = moduleRef.get(WorkOSService);
      expect(service).toBeInstanceOf(WorkOSService);
      await moduleRef.close();
    });

    it('exports the Site.WORKOS = "workos" enum value', () => {
      expect(Site.WORKOS).toBe('workos');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new WorkOSService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.WORKOS],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('workos');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.WORKOS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('workos-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new WorkOSService(registry);
      const result = await service.scrape({
        siteType: [Site.WORKOS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('workos');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('workos-x1');
      expect(result.jobs[0].site).toBe(Site.WORKOS);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('workos-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new WorkOSService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.WORKOS],
      } as ScraperInputDto);
