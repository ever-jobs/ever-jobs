...
import { VaticLabsModule, VaticLabsService } from '../src';
...
const FIXTURE: Fixture = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'fixtures', 'vaticlabs-boards.json'), 'utf8'),
);
const COMPANY_NAME = 'Vatic Labs';
const ID_PREFIX = 'vaticlabs-';
...
describe('VaticLabsService — Greenhouse delegation (Spec 1737)', () => {
  beforeEach(() => {
    mockGet.mockReset();
    mockPost.mockReset();
  ...
  describe('registration', () => {
    it('resolves through VaticLabsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({ imports: [VaticLabsModule] }).compile();
      expect(moduleRef.get(VaticLabsService)).toBeInstanceOf(VaticLabsService);
      await moduleRef.close();
    });

    it('exports Site.VATIC_LABS = "vaticlabs"', () => {
      expect(Site.VATIC_LABS).toBe('vaticlabs');
    });

    it('declares a tagged company plugin with its domains', () => {
      const meta = Reflect.getMetadata(SOURCE_PLUGIN_METADATA, VaticLabsService);
      expect(meta.site).toBe(Site.VATIC_LABS);
      expect(meta.category).toBe('company');
      expect(meta.isAts).toBeFalsy();
      expect(meta.companyDomains).toEqual(['vaticlabs.ai']);
  ...
  describe('recorded board (real Greenhouse adapter, mocked HTTP)', () => {
    it('maps every recorded posting and re-stamps the company identity', async () => {
      const service = new VaticLabsService(registryWith());
      const result = await service.scrape({ siteType: [Site.VATIC_LABS], resultsWanted: 100 } as ScraperInputDto);
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.VATIC_LABS);
    ...
    it('only requests the recorded board URLs', async () => {
      const service = new VaticLabsService(registryWith());
      await service.scrape({ siteType: [Site.VATIC_LABS] } as ScraperInputDto);
    ...
    it('honours resultsWanted=1', async () => {
      const service = new VaticLabsService(registryWith());
      const result = await service.scrape({ siteType: [Site.VATIC_LABS], resultsWanted: 1 } as ScraperInputDto);
    ...
    it('resolves with an empty result when the board is gone (HTTP 404)', async () => {
      ...
      const service = new VaticLabsService(registryWith());
      const result = await service.scrape({ siteType: [Site.VATIC_LABS] } as ScraperInputDto);
  ...
  describe('delegation contract', () => {
    it('forwards the board and the caller input untouched', async () => {
      ...
      const service = new VaticLabsService(
        registryWith(
          fakeBackend(
      ...
      expect(result.jobs[0].site).toBe(Site.VATIC_LABS);
    ...
    it('never forwards the caller\'s credentials to the board (Spec 1735 §4.5)', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new VaticLabsService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
    ...
    it('makes no request when resultsWanted is 0', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new VaticLabsService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      const result = await service.scrape({ siteType: [Site.VATIC_LABS], resultsWanted: 0 } as ScraperInputDto);
  ...
  describe('resilience', () => {
    it('reports not_registered when no registry is injected', async () => {
      const result = await new VaticLabsService().scrape({ siteType: [Site.VATIC_LABS] } as ScraperInputDto);
      expect(result.jobs).toEqual([]);
    ...
    it('reports not_registered when Greenhouse is missing from the registry', async () => {
      const result = await new VaticLabsService(new PluginRegistry()).scrape({
        siteType: [Site.VATIC_LABS],
      } as ScraperInputDto);
      expect(result.diagnostics?.reason).toBe('not_registered');
    });
