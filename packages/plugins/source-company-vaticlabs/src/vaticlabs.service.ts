...
/**
 * Vatic Labs — Quantitative trading (HQ: New York, NY, USA).
 *
 * Source (Spec 1737): Greenhouse board, scraped in this order:
 *   - `vaticlabs` — https://job-boards.greenhouse.io/vaticlabs
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'vaticlabs', atsIdPrefix: 'gh-' },
];

@SourcePlugin({
  site: Site.VATIC_LABS,
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['vaticlabs.ai'],
  description: 'Vatic Labs careers via Greenhouse. Tags: segment=quant-trading; industry=quantitative-trading.',
})
@Injectable()
export class VaticLabsService implements IScraper {
  private readonly logger = new Logger(VaticLabsService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Greenhouse source plugin is not registered; cannot scrape Vatic Labs');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`Vatic Labs: delegating to Greenhouse (${board.companySlug})`);
    ...
    this.logger.log(`Vatic Labs: scraped ${jobs.length} jobs`);
