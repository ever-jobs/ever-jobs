...
import { NextdoorModule, NextdoorService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'nextdoor-jobs.json'), 'utf8'),
);
...
/**
 * Spec 434 / T04 — `NextdoorService` unit tests (≥ 9 cases).
 */
describe('NextdoorService — Spec 434 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through NextdoorModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [NextdoorModule],
      }).compile();
      const service = moduleRef.get(NextdoorService);
      expect(service).toBeInstanceOf(NextdoorService);
      await moduleRef.close();
    });

    it('exports the Site.NEXTDOOR = "nextdoor" enum value', () => {
      expect(Site.NEXTDOOR).toBe('nextdoor');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new NextdoorService();
      const result = await service.scrape({
        siteType: [Site.NEXTDOOR],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'nextdoor-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.NEXTDOOR);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Nextdoor');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/nextdoor/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/nextdoor/jobs/');
      ...
      expect(job0?.description).toContain('Nextdoor');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/nextdoor/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Nextdoor');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'nextdoor-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
