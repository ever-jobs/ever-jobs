...
import { ScalableCapitalModule, ScalableCapitalService } from '../src';

const COMPANY_NAME_EXPECT = 'Scalable Capital';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'scalablecapital-jobs.json'), 'utf8'),
);
...
describe('ScalableCapitalService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ScalableCapitalModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ScalableCapitalModule],
      }).compile();
      const service = moduleRef.get(ScalableCapitalService);
      expect(service).toBeInstanceOf(ScalableCapitalService);
      await moduleRef.close();
    });

    it('exports the Site.SCALABLE_CAPITAL = "scalablecapital" enum value', () => {
      expect(Site.SCALABLE_CAPITAL).toBe('scalablecapital');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new ScalableCapitalService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.SCALABLE_CAPITAL],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'scalablecapital-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.SCALABLE_CAPITAL);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('scalablecapital-' + first.id);
      ...
      expect(calledUrls[0]).toContain('scalablegmbh');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SCALABLE_CAPITAL);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('scalablecapital-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new ScalableCapitalService(registry);
      const result = await service.scrape({
        siteType: [Site.SCALABLE_CAPITAL],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('scalablegmbh');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('scalablecapital-x1');
      expect(result.jobs[0].site).toBe(Site.SCALABLE_CAPITAL);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('scalablecapital-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new ScalableCapitalService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.SCALABLE_CAPITAL],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new ScalableCapitalService();
      const result = await service.scrape({
        siteType: [Site.SCALABLE_CAPITAL],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
