...
import { RepRallyModule, RepRallyService } from '../src';

const COMPANY_NAME_EXPECT = 'RepRally';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'reprally-jobs.json'), 'utf8'),
);
...
describe('RepRallyService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through RepRallyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [RepRallyModule],
      }).compile();
      const service = moduleRef.get(RepRallyService);
      expect(service).toBeInstanceOf(RepRallyService);
      await moduleRef.close();
    });

    it('exports the Site.REPRALLY = "reprally" enum value', () => {
      expect(Site.REPRALLY).toBe('reprally');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new RepRallyService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.REPRALLY],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('reprally');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.REPRALLY);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('reprally-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new RepRallyService(registry);
      const result = await service.scrape({
        siteType: [Site.REPRALLY],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('reprally');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('reprally-x1');
      expect(result.jobs[0].site).toBe(Site.REPRALLY);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('reprally-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new RepRallyService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.REPRALLY],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new RepRallyService();
      const result = await service.scrape({
        siteType: [Site.REPRALLY],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
