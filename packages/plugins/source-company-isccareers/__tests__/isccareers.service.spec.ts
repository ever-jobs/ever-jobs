...
import { IntegratedSpecialtyCoveragesModule, IntegratedSpecialtyCoveragesService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'isccareers-jobs.json'), 'utf8'),
);
...
/**
 * Spec 872 / T04 — `IntegratedSpecialtyCoveragesService` unit tests (≥ 9 cases).
 */
describe('IntegratedSpecialtyCoveragesService — Spec 872 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through IntegratedSpecialtyCoveragesModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [IntegratedSpecialtyCoveragesModule],
      }).compile();
      const service = moduleRef.get(IntegratedSpecialtyCoveragesService);
      expect(service).toBeInstanceOf(IntegratedSpecialtyCoveragesService);
      await moduleRef.close();
    });

    it('exports the Site.INTEGRATED_SPECIALTY_COVERAGES = "isccareers" enum value', () => {
      expect(Site.INTEGRATED_SPECIALTY_COVERAGES).toBe('isccareers');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new IntegratedSpecialtyCoveragesService();
      const result = await service.scrape({
        siteType: [Site.INTEGRATED_SPECIALTY_COVERAGES],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'isccareers-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.INTEGRATED_SPECIALTY_COVERAGES);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Integrated Specialty Coverages');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/isccareers/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/isccareers/jobs/');
      ...
      expect(job0?.description).toContain('Integrated Specialty Coverages');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/isccareers/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Integrated Specialty Coverages');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'isccareers-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
