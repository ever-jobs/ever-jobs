...
import { AreteTechnologiesModule, AreteTechnologiesService } from '../src';

const COMPANY_NAME_EXPECT = 'Arete Technologies';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'aretetechnologies-jobs.json'), 'utf8'),
);
...
describe('AreteTechnologiesService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AreteTechnologiesModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AreteTechnologiesModule],
      }).compile();
      const service = moduleRef.get(AreteTechnologiesService);
      expect(service).toBeInstanceOf(AreteTechnologiesService);
      await moduleRef.close();
    });

    it('exports the Site.ARETE_TECHNOLOGIES = "aretetechnologies" enum value', () => {
      expect(Site.ARETE_TECHNOLOGIES).toBe('aretetechnologies');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new AreteTechnologiesService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.ARETE_TECHNOLOGIES],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'aretetechnologies-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.ARETE_TECHNOLOGIES);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('aretetechnologies-' + first.id);
      ...
      expect(calledUrls[0]).toContain('AreteTechnologiesInc');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ARETE_TECHNOLOGIES);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('aretetechnologies-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new AreteTechnologiesService(registry);
      const result = await service.scrape({
        siteType: [Site.ARETE_TECHNOLOGIES],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('AreteTechnologiesInc');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('aretetechnologies-x1');
      expect(result.jobs[0].site).toBe(Site.ARETE_TECHNOLOGIES);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('aretetechnologies-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new AreteTechnologiesService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.ARETE_TECHNOLOGIES],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new AreteTechnologiesService();
      const result = await service.scrape({
        siteType: [Site.ARETE_TECHNOLOGIES],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
