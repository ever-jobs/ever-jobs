...
import { SuperellipseModule, SuperellipseService } from '../src';

const COMPANY_NAME_EXPECT = 'Superellipse';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'superellipse-jobs.json'), 'utf8'),
);
...
describe('SuperellipseService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SuperellipseModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SuperellipseModule],
      }).compile();
      const service = moduleRef.get(SuperellipseService);
      expect(service).toBeInstanceOf(SuperellipseService);
      await moduleRef.close();
    });

    it('exports the Site.SUPERELLIPSE = "superellipse" enum value', () => {
      expect(Site.SUPERELLIPSE).toBe('superellipse');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new SuperellipseService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.SUPERELLIPSE],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'superellipse-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Recruitee's defaults
      expect(job0?.site).toBe(Site.SUPERELLIPSE);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('superellipse-' + first.id);
      ...
      expect(calledUrls[0]).toContain('superellipse.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SUPERELLIPSE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('superellipse-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new SuperellipseService(registry);
      const result = await service.scrape({
        siteType: [Site.SUPERELLIPSE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('superellipse');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('superellipse-x1');
      expect(result.jobs[0].site).toBe(Site.SUPERELLIPSE);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('superellipse-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new SuperellipseService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.SUPERELLIPSE],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new SuperellipseService();
      const result = await service.scrape({
        siteType: [Site.SUPERELLIPSE],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
