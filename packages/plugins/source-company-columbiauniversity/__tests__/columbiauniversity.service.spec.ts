...
import { ColumbiaUniversityModule, ColumbiaUniversityService } from '../src';

const COMPANY_NAME_EXPECT = 'Columbia University';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'columbiauniversity-jobs.json'), 'utf8'),
);
...
describe('ColumbiaUniversityService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ColumbiaUniversityModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ColumbiaUniversityModule],
      }).compile();
      const service = moduleRef.get(ColumbiaUniversityService);
      expect(service).toBeInstanceOf(ColumbiaUniversityService);
      await moduleRef.close();
    });

    it('exports the Site.COLUMBIA_UNIVERSITY = "columbiauniversity" enum value', () => {
      expect(Site.COLUMBIA_UNIVERSITY).toBe('columbiauniversity');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new ColumbiaUniversityService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.COLUMBIA_UNIVERSITY],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'columbiauniversity-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.COLUMBIA_UNIVERSITY);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('columbiauniversity-' + first.id);
      ...
      expect(calledUrls[0]).toContain('ColumbiaUniversity1');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.COLUMBIA_UNIVERSITY);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('columbiauniversity-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new ColumbiaUniversityService(registry);
      const result = await service.scrape({
        siteType: [Site.COLUMBIA_UNIVERSITY],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('ColumbiaUniversity1');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('columbiauniversity-x1');
      expect(result.jobs[0].site).toBe(Site.COLUMBIA_UNIVERSITY);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('columbiauniversity-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new ColumbiaUniversityService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.COLUMBIA_UNIVERSITY],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new ColumbiaUniversityService();
      const result = await service.scrape({
        siteType: [Site.COLUMBIA_UNIVERSITY],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
