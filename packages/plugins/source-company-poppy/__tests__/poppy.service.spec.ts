...
import { PoppyModule, PoppyService } from '../src';

const COMPANY_NAME_EXPECT = 'Poppy';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'poppy-jobs.json'), 'utf8'),
);
...
describe('PoppyService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through PoppyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [PoppyModule],
      }).compile();
      const service = moduleRef.get(PoppyService);
      expect(service).toBeInstanceOf(PoppyService);
      await moduleRef.close();
    });

    it('exports the Site.POPPY = "poppy" enum value', () => {
      expect(Site.POPPY).toBe('poppy');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new PoppyService(registryWithRecruitee());
      ...
      expect(calledUrls[0]).toContain('poppy.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.POPPY);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('poppy-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new PoppyService(registry);
      const result = await service.scrape({
        siteType: [Site.POPPY],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('poppy');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('poppy-x1');
      expect(result.jobs[0].site).toBe(Site.POPPY);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('poppy-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new PoppyService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.POPPY],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new PoppyService();
      const result = await service.scrape({
        siteType: [Site.POPPY],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
