...
import { LegionTechnologiesModule, LegionTechnologiesService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'legion-jobs.json'), 'utf8'),
);
...
/**
 * Spec 878 / T04 — `LegionTechnologiesService` unit tests (≥ 9 cases).
 */
describe('LegionTechnologiesService — Spec 878 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through LegionTechnologiesModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [LegionTechnologiesModule],
      }).compile();
      const service = moduleRef.get(LegionTechnologiesService);
      expect(service).toBeInstanceOf(LegionTechnologiesService);
      await moduleRef.close();
    });

    it('exports the Site.LEGION_TECHNOLOGIES = "legion" enum value', () => {
      expect(Site.LEGION_TECHNOLOGIES).toBe('legion');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new LegionTechnologiesService();
      const result = await service.scrape({
        siteType: [Site.LEGION_TECHNOLOGIES],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'legion-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.LEGION_TECHNOLOGIES);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Legion Technologies');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/legion/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/legion/jobs/');
      ...
      expect(job0?.description).toContain('Legion Technologies');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/legion/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Legion Technologies');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'legion-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
