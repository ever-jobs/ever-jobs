...
import { AmtechsoftwareModule, AmtechsoftwareService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'amtechsoftware-jobs.json'), 'utf8'),
);
...
/**
 * Spec 264 / T04 — `AmtechsoftwareService` unit tests (≥ 9 cases).
 */
describe('AmtechsoftwareService — Spec 264 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AmtechsoftwareModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AmtechsoftwareModule],
      }).compile();
      const service = moduleRef.get(AmtechsoftwareService);
      expect(service).toBeInstanceOf(AmtechsoftwareService);
      await moduleRef.close();
    });

    it('exports the Site.AMTECHSOFTWARE = "amtechsoftware" enum value', () => {
      expect(Site.AMTECHSOFTWARE).toBe('amtechsoftware');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AmtechsoftwareService();
      const result = await service.scrape({
        siteType: [Site.AMTECHSOFTWARE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'amtechsoftware-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.AMTECHSOFTWARE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Amtech Software');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/amtechsoftware/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/amtechsoftware/jobs/');
      ...
      expect(job0?.description).toContain('Amtech Software');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/amtechsoftware/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Amtech Software');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'amtechsoftware-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
