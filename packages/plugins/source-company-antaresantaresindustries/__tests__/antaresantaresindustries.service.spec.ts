...
import { AntaresAntaresIndustriesModule, AntaresAntaresIndustriesService } from '../src';

const COMPANY_NAME_EXPECT = 'Antares (Antares Industries)';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'antaresantaresindustries-jobs.json'), 'utf8'),
);
...
describe('AntaresAntaresIndustriesService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AntaresAntaresIndustriesModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AntaresAntaresIndustriesModule],
      }).compile();
      const service = moduleRef.get(AntaresAntaresIndustriesService);
      expect(service).toBeInstanceOf(AntaresAntaresIndustriesService);
      await moduleRef.close();
    });

    it('exports the Site.ANTARES_ANTARES_INDUSTRIES = "antaresantaresindustries" enum value', () => {
      expect(Site.ANTARES_ANTARES_INDUSTRIES).toBe('antaresantaresindustries');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new AntaresAntaresIndustriesService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.ANTARES_ANTARES_INDUSTRIES],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'antaresantaresindustries-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.ANTARES_ANTARES_INDUSTRIES);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('antaresantaresindustries-' + first.id);
      ...
      expect(calledUrls[0]).toContain('antares');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ANTARES_ANTARES_INDUSTRIES);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('antaresantaresindustries-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new AntaresAntaresIndustriesService(registry);
      const result = await service.scrape({
        siteType: [Site.ANTARES_ANTARES_INDUSTRIES],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('antares');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('antaresantaresindustries-x1');
      expect(result.jobs[0].site).toBe(Site.ANTARES_ANTARES_INDUSTRIES);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('antaresantaresindustries-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new AntaresAntaresIndustriesService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.ANTARES_ANTARES_INDUSTRIES],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new AntaresAntaresIndustriesService();
      const result = await service.scrape({
        siteType: [Site.ANTARES_ANTARES_INDUSTRIES],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
