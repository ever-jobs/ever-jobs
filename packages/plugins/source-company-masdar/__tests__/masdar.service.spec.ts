...
import { MasdarModule, MasdarService } from '../src';

const COMPANY_NAME_EXPECT = 'Masdar';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'masdar-jobs.json'), 'utf8'),
);
...
describe('MasdarService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through MasdarModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [MasdarModule],
      }).compile();
      const service = moduleRef.get(MasdarService);
      expect(service).toBeInstanceOf(MasdarService);
      await moduleRef.close();
    });

    it('exports the Site.MASDAR = "masdar" enum value', () => {
      expect(Site.MASDAR).toBe('masdar');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new MasdarService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.MASDAR],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('Masdar');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.MASDAR);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('masdar-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new MasdarService(registry);
      const result = await service.scrape({
        siteType: [Site.MASDAR],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('Masdar');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('masdar-x1');
      expect(result.jobs[0].site).toBe(Site.MASDAR);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('masdar-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new MasdarService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.MASDAR],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new MasdarService();
      const result = await service.scrape({
        siteType: [Site.MASDAR],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
