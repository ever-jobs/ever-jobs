...
import { BelieveModule, BelieveService } from '../src';

const COMPANY_NAME_EXPECT = 'Believe';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'believe-jobs.json'), 'utf8'),
);
...
describe('BelieveService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through BelieveModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [BelieveModule],
      }).compile();
      const service = moduleRef.get(BelieveService);
      expect(service).toBeInstanceOf(BelieveService);
      await moduleRef.close();
    });

    it('exports the Site.BELIEVE = "believe" enum value', () => {
      expect(Site.BELIEVE).toBe('believe');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new BelieveService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.BELIEVE],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('believe');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.BELIEVE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('believe-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new BelieveService(registry);
      const result = await service.scrape({
        siteType: [Site.BELIEVE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('believe');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('believe-x1');
      expect(result.jobs[0].site).toBe(Site.BELIEVE);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('believe-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new BelieveService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.BELIEVE],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new BelieveService();
      const result = await service.scrape({
        siteType: [Site.BELIEVE],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
