...
import { TruewerkModule, TruewerkService } from '../src';

const COMPANY_NAME_EXPECT = 'Truewerk';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'truewerk-jobs.json'), 'utf8'),
);
...
describe('TruewerkService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through TruewerkModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [TruewerkModule],
      }).compile();
      const service = moduleRef.get(TruewerkService);
      expect(service).toBeInstanceOf(TruewerkService);
      await moduleRef.close();
    });

    it('exports the Site.TRUEWERK = "truewerk" enum value', () => {
      expect(Site.TRUEWERK).toBe('truewerk');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new TruewerkService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.TRUEWERK],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('Truewerk');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.TRUEWERK);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('truewerk-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new TruewerkService(registry);
      const result = await service.scrape({
        siteType: [Site.TRUEWERK],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('Truewerk');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('truewerk-x1');
      expect(result.jobs[0].site).toBe(Site.TRUEWERK);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('truewerk-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new TruewerkService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.TRUEWERK],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new TruewerkService();
      const result = await service.scrape({
        siteType: [Site.TRUEWERK],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
