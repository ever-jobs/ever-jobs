...
import { MindpeakModule, MindpeakService } from '../src';

const COMPANY_NAME_EXPECT = 'Mindpeak';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'mindpeak-jobs.json'), 'utf8'),
);
...
describe('MindpeakService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through MindpeakModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [MindpeakModule],
      }).compile();
      const service = moduleRef.get(MindpeakService);
      expect(service).toBeInstanceOf(MindpeakService);
      await moduleRef.close();
    });

    it('exports the Site.MINDPEAK = "mindpeak" enum value', () => {
      expect(Site.MINDPEAK).toBe('mindpeak');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new MindpeakService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.MINDPEAK],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('mindpeak');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.MINDPEAK);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('mindpeak-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new MindpeakService(registry);
      const result = await service.scrape({
        siteType: [Site.MINDPEAK],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('mindpeak');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('mindpeak-x1');
      expect(result.jobs[0].site).toBe(Site.MINDPEAK);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('mindpeak-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new MindpeakService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.MINDPEAK],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new MindpeakService();
      const result = await service.scrape({
        siteType: [Site.MINDPEAK],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
