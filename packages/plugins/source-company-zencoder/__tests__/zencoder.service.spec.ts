...
import { ZencoderModule, ZencoderService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'zencoder-jobs.json'), 'utf8'),
);
...
/**
 * Spec 731 / T04 — `ZencoderService` unit tests (≥ 9 cases).
 */
describe('ZencoderService — Spec 731 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ZencoderModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ZencoderModule],
      }).compile();
      const service = moduleRef.get(ZencoderService);
      expect(service).toBeInstanceOf(ZencoderService);
      await moduleRef.close();
    });

    it('exports the Site.ZENCODER = "zencoder" enum value', () => {
      expect(Site.ZENCODER).toBe('zencoder');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ZencoderService();
      const result = await service.scrape({
        siteType: [Site.ZENCODER],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'zencoder-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ZENCODER);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Zencoder');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/zencoder/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/zencoder/jobs/');
      ...
      expect(job0?.description).toContain('Zencoder');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/zencoder/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Zencoder');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'zencoder-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
