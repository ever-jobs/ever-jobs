...
import { UnderdogModule, UnderdogService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'underdogfantasy-jobs.json'), 'utf8'),
);
...
/**
 * Spec 588 / T04 — `UnderdogService` unit tests (≥ 9 cases).
 */
describe('UnderdogService — Spec 588 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through UnderdogModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [UnderdogModule],
      }).compile();
      const service = moduleRef.get(UnderdogService);
      expect(service).toBeInstanceOf(UnderdogService);
      await moduleRef.close();
    });

    it('exports the Site.UNDERDOG = "underdogfantasy" enum value', () => {
      expect(Site.UNDERDOG).toBe('underdogfantasy');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new UnderdogService();
      const result = await service.scrape({
        siteType: [Site.UNDERDOG],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'underdogfantasy-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.UNDERDOG);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Underdog');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/underdogfantasy/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/underdogfantasy/jobs/');
      ...
      expect(job0?.description).toContain('Underdog');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/underdogfantasy/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Underdog');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'underdogfantasy-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
