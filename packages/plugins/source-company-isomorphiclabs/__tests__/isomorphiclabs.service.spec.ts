...
import { IsomorphicLabsModule, IsomorphicLabsService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'isomorphiclabs-jobs.json'), 'utf8'),
);
...
/**
 * Spec 627 / T04 — `IsomorphicLabsService` unit tests (≥ 9 cases).
 */
describe('IsomorphicLabsService — Spec 627 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through IsomorphicLabsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [IsomorphicLabsModule],
      }).compile();
      const service = moduleRef.get(IsomorphicLabsService);
      expect(service).toBeInstanceOf(IsomorphicLabsService);
      await moduleRef.close();
    });

    it('exports the Site.ISOMORPHIC_LABS = "isomorphiclabs" enum value', () => {
      expect(Site.ISOMORPHIC_LABS).toBe('isomorphiclabs');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new IsomorphicLabsService();
      ...
      const job0 = dto.jobs.find((j) => j.id === 'isomorphiclabs-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ISOMORPHIC_LABS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Isomorphic Labs');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/isomorphiclabs/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/isomorphiclabs/jobs/');
      ...
      expect(job0?.description).toContain('Isomorphic Labs');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/isomorphiclabs/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Isomorphic Labs');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'isomorphiclabs-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
