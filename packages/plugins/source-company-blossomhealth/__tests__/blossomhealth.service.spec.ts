...
import { BlossomHealthModule, BlossomHealthService } from '../src';

const COMPANY_NAME_EXPECT = 'Blossom Health';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'blossomhealth-jobs.json'), 'utf8'),
);
...
describe('BlossomHealthService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through BlossomHealthModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [BlossomHealthModule],
      }).compile();
      const service = moduleRef.get(BlossomHealthService);
      expect(service).toBeInstanceOf(BlossomHealthService);
      await moduleRef.close();
    });

    it('exports the Site.BLOSSOM_HEALTH = "blossomhealth" enum value', () => {
      expect(Site.BLOSSOM_HEALTH).toBe('blossomhealth');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new BlossomHealthService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.BLOSSOM_HEALTH],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'blossomhealth-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.BLOSSOM_HEALTH);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('blossomhealth-' + first.id);
      ...
      expect(calledUrls[0]).toContain('blossom-health');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.BLOSSOM_HEALTH);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('blossomhealth-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new BlossomHealthService(registry);
      const result = await service.scrape({
        siteType: [Site.BLOSSOM_HEALTH],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('blossom-health');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('blossomhealth-x1');
      expect(result.jobs[0].site).toBe(Site.BLOSSOM_HEALTH);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('blossomhealth-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new BlossomHealthService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.BLOSSOM_HEALTH],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new BlossomHealthService();
      const result = await service.scrape({
        siteType: [Site.BLOSSOM_HEALTH],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
