...
import { SpeechmaticsModule, SpeechmaticsService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'speechmatics-jobs.json'), 'utf8'),
);
...
/**
 * Spec 474 / T04 — `SpeechmaticsService` unit tests (≥ 9 cases).
 */
describe('SpeechmaticsService — Spec 474 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SpeechmaticsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SpeechmaticsModule],
      }).compile();
      const service = moduleRef.get(SpeechmaticsService);
      expect(service).toBeInstanceOf(SpeechmaticsService);
      await moduleRef.close();
    });

    it('exports the Site.SPEECHMATICS = "speechmatics" enum value', () => {
      expect(Site.SPEECHMATICS).toBe('speechmatics');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new SpeechmaticsService();
      const result = await service.scrape({
        siteType: [Site.SPEECHMATICS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'speechmatics-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.SPEECHMATICS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Speechmatics');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/speechmatics/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/speechmatics/jobs/');
      ...
      expect(job0?.description).toContain('Speechmatics');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/speechmatics/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Speechmatics');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'speechmatics-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
