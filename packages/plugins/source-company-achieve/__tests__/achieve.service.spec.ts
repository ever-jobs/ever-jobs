...
import { AchieveModule, AchieveService } from '../src';

const COMPANY_NAME_EXPECT = 'Achieve';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'achieve-jobs.json'), 'utf8'),
);
...
describe('AchieveService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AchieveModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AchieveModule],
      }).compile();
      const service = moduleRef.get(AchieveService);
      expect(service).toBeInstanceOf(AchieveService);
      await moduleRef.close();
    });

    it('exports the Site.ACHIEVE = "achieve" enum value', () => {
      expect(Site.ACHIEVE).toBe('achieve');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new AchieveService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.ACHIEVE],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('Achieve1');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ACHIEVE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('achieve-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new AchieveService(registry);
      const result = await service.scrape({
        siteType: [Site.ACHIEVE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('Achieve1');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('achieve-x1');
      expect(result.jobs[0].site).toBe(Site.ACHIEVE);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('achieve-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new AchieveService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.ACHIEVE],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new AchieveService();
      const result = await service.scrape({
        siteType: [Site.ACHIEVE],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
