...
import { PathaiModule, PathaiService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'pathai-jobs.json'), 'utf8'),
);
...
/**
 * Spec 548 / T04 — `PathaiService` unit tests (≥ 9 cases).
 */
describe('PathaiService — Spec 548 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through PathaiModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [PathaiModule],
      }).compile();
      const service = moduleRef.get(PathaiService);
      expect(service).toBeInstanceOf(PathaiService);
      await moduleRef.close();
    });

    it('exports the Site.PATHAI = "pathai" enum value', () => {
      expect(Site.PATHAI).toBe('pathai');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new PathaiService();
      const result = await service.scrape({
        siteType: [Site.PATHAI],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'pathai-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.PATHAI);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('PathAI');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/pathai/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/pathai/jobs/');
      ...
      expect(job0?.description).toContain('PathAI');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/pathai/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('PathAI');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'pathai-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
