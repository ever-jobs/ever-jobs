...
import { ElectricAirModule, ElectricAirService } from '../src';

const COMPANY_NAME_EXPECT = 'Electric Air';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'electricair-jobs.json'), 'utf8'),
);
...
describe('ElectricAirService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ElectricAirModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ElectricAirModule],
      }).compile();
      const service = moduleRef.get(ElectricAirService);
      expect(service).toBeInstanceOf(ElectricAirService);
      await moduleRef.close();
    });

    it('exports the Site.ELECTRIC_AIR = "electricair" enum value', () => {
      expect(Site.ELECTRIC_AIR).toBe('electricair');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new ElectricAirService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.ELECTRIC_AIR],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'electricair-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.ELECTRIC_AIR);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('electricair-' + first.id);
      ...
      expect(calledUrls[0]).toContain('electricair');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ELECTRIC_AIR);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('electricair-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new ElectricAirService(registry);
      const result = await service.scrape({
        siteType: [Site.ELECTRIC_AIR],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('electricair');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('electricair-x1');
      expect(result.jobs[0].site).toBe(Site.ELECTRIC_AIR);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('electricair-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new ElectricAirService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.ELECTRIC_AIR],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new ElectricAirService();
      const result = await service.scrape({
        siteType: [Site.ELECTRIC_AIR],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
