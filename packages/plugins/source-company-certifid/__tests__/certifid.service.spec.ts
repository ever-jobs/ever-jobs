...
import { CertifIDModule, CertifIDService } from '../src';

const COMPANY_NAME_EXPECT = 'CertifID';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'certifid-jobs.json'), 'utf8'),
);
...
describe('CertifIDService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CertifIDModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CertifIDModule],
      }).compile();
      const service = moduleRef.get(CertifIDService);
      expect(service).toBeInstanceOf(CertifIDService);
      await moduleRef.close();
    });

    it('exports the Site.CERTIFID = "certifid" enum value', () => {
      expect(Site.CERTIFID).toBe('certifid');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new CertifIDService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.CERTIFID],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('certifid');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.CERTIFID);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('certifid-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new CertifIDService(registry);
      const result = await service.scrape({
        siteType: [Site.CERTIFID],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('certifid');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('certifid-x1');
      expect(result.jobs[0].site).toBe(Site.CERTIFID);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('certifid-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new CertifIDService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.CERTIFID],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new CertifIDService();
      const result = await service.scrape({
        siteType: [Site.CERTIFID],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
