...
import { MobiappsModule, MobiappsService } from '../src';

const COMPANY_NAME_EXPECT = 'Mobiapps';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'mobiapps-jobs.json'), 'utf8'),
);
...
describe('MobiappsService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through MobiappsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [MobiappsModule],
      }).compile();
      const service = moduleRef.get(MobiappsService);
      expect(service).toBeInstanceOf(MobiappsService);
      await moduleRef.close();
    });

    it('exports the Site.MOBIAPPS = "mobiapps" enum value', () => {
      expect(Site.MOBIAPPS).toBe('mobiapps');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new MobiappsService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.MOBIAPPS],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('mobiapps.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.MOBIAPPS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('mobiapps-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new MobiappsService(registry);
      const result = await service.scrape({
        siteType: [Site.MOBIAPPS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('mobiapps');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('mobiapps-x1');
      expect(result.jobs[0].site).toBe(Site.MOBIAPPS);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('mobiapps-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new MobiappsService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.MOBIAPPS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new MobiappsService();
      const result = await service.scrape({
        siteType: [Site.MOBIAPPS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
