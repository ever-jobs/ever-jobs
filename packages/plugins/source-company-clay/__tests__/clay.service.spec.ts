...
import { ClayModule, ClayService } from '../src';

const COMPANY_NAME_EXPECT = 'Clay';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'clay-jobs.json'), 'utf8'),
);
...
describe('ClayService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ClayModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ClayModule],
      }).compile();
      const service = moduleRef.get(ClayService);
      expect(service).toBeInstanceOf(ClayService);
      await moduleRef.close();
    });

    it('exports the Site.CLAY = "clay" enum value', () => {
      expect(Site.CLAY).toBe('clay');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new ClayService(registryWithAshby());
      ...
      expect(calledUrls[0]).toContain('claylabs');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.CLAY);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('clay-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new ClayService(registry);
      const result = await service.scrape({
        siteType: [Site.CLAY],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('claylabs');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('clay-x1');
      expect(result.jobs[0].site).toBe(Site.CLAY);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('clay-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new ClayService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.CLAY],
      } as ScraperInputDto);
