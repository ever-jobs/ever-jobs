...
import { AdaptiveSecurityModule, AdaptiveSecurityService } from '../src';

const COMPANY_NAME_EXPECT = 'Adaptive Security';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'adaptivesecurity-jobs.json'), 'utf8'),
);
...
describe('AdaptiveSecurityService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AdaptiveSecurityModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AdaptiveSecurityModule],
      }).compile();
      const service = moduleRef.get(AdaptiveSecurityService);
      expect(service).toBeInstanceOf(AdaptiveSecurityService);
      await moduleRef.close();
    });

    it('exports the Site.ADAPTIVE_SECURITY = "adaptivesecurity" enum value', () => {
      expect(Site.ADAPTIVE_SECURITY).toBe('adaptivesecurity');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new AdaptiveSecurityService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.ADAPTIVE_SECURITY],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'adaptivesecurity-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.ADAPTIVE_SECURITY);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('adaptivesecurity-' + first.id);
      ...
      expect(calledUrls[0]).toContain('adaptivesecurity');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ADAPTIVE_SECURITY);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('adaptivesecurity-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new AdaptiveSecurityService(registry);
      const result = await service.scrape({
        siteType: [Site.ADAPTIVE_SECURITY],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('adaptivesecurity');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('adaptivesecurity-x1');
      expect(result.jobs[0].site).toBe(Site.ADAPTIVE_SECURITY);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('adaptivesecurity-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new AdaptiveSecurityService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.ADAPTIVE_SECURITY],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new AdaptiveSecurityService();
      const result = await service.scrape({
        siteType: [Site.ADAPTIVE_SECURITY],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
