...
import { OffchainLabsModule, OffchainLabsService } from '../src';

const COMPANY_NAME_EXPECT = 'Offchain Labs';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'offchainlabs-jobs.json'), 'utf8'),
);
...
describe('OffchainLabsService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through OffchainLabsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [OffchainLabsModule],
      }).compile();
      const service = moduleRef.get(OffchainLabsService);
      expect(service).toBeInstanceOf(OffchainLabsService);
      await moduleRef.close();
    });

    it('exports the Site.OFFCHAIN_LABS = "offchainlabs" enum value', () => {
      expect(Site.OFFCHAIN_LABS).toBe('offchainlabs');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new OffchainLabsService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.OFFCHAIN_LABS],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'offchainlabs-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.OFFCHAIN_LABS);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('offchainlabs-' + first.id);
      ...
      expect(calledUrls[0]).toContain('offchainlabs');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.OFFCHAIN_LABS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('offchainlabs-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new OffchainLabsService(registry);
      const result = await service.scrape({
        siteType: [Site.OFFCHAIN_LABS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('offchainlabs');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('offchainlabs-x1');
      expect(result.jobs[0].site).toBe(Site.OFFCHAIN_LABS);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('offchainlabs-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new OffchainLabsService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.OFFCHAIN_LABS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new OffchainLabsService();
      const result = await service.scrape({
        siteType: [Site.OFFCHAIN_LABS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
