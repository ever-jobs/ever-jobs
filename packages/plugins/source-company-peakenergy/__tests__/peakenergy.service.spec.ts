...
import { PeakEnergyModule, PeakEnergyService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'peakenergy-jobs.json'), 'utf8'),
);
...
/**
 * Spec 767 / T04 — `PeakEnergyService` unit tests (≥ 9 cases).
 */
describe('PeakEnergyService — Spec 767 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through PeakEnergyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [PeakEnergyModule],
      }).compile();
      const service = moduleRef.get(PeakEnergyService);
      expect(service).toBeInstanceOf(PeakEnergyService);
      await moduleRef.close();
    });

    it('exports the Site.PEAK_ENERGY = "peakenergy" enum value', () => {
      expect(Site.PEAK_ENERGY).toBe('peakenergy');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new PeakEnergyService();
      const result = await service.scrape({
        siteType: [Site.PEAK_ENERGY],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'peakenergy-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.PEAK_ENERGY);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Peak Energy');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/peakenergy/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/peakenergy/jobs/');
      ...
      expect(job0?.description).toContain('Peak Energy');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/peakenergy/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Peak Energy');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'peakenergy-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
