...
import { PeripassModule, PeripassService } from '../src';

const COMPANY_NAME_EXPECT = 'Peripass';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'peripass-jobs.json'), 'utf8'),
);
...
describe('PeripassService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through PeripassModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [PeripassModule],
      }).compile();
      const service = moduleRef.get(PeripassService);
      expect(service).toBeInstanceOf(PeripassService);
      await moduleRef.close();
    });

    it('exports the Site.PERIPASS = "peripass" enum value', () => {
      expect(Site.PERIPASS).toBe('peripass');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new PeripassService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.PERIPASS],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('peripass.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.PERIPASS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('peripass-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new PeripassService(registry);
      const result = await service.scrape({
        siteType: [Site.PERIPASS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('peripass');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('peripass-x1');
      expect(result.jobs[0].site).toBe(Site.PERIPASS);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('peripass-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new PeripassService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.PERIPASS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new PeripassService();
      const result = await service.scrape({
        siteType: [Site.PERIPASS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
