...
import { TheWonderfulCompanyModule, TheWonderfulCompanyService } from '../src';

const COMPANY_NAME_EXPECT = 'The Wonderful Company';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'thewonderfulcompany-jobs.json'), 'utf8'),
);
...
describe('TheWonderfulCompanyService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through TheWonderfulCompanyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [TheWonderfulCompanyModule],
      }).compile();
      const service = moduleRef.get(TheWonderfulCompanyService);
      expect(service).toBeInstanceOf(TheWonderfulCompanyService);
      await moduleRef.close();
    });

    it('exports the Site.THE_WONDERFUL_COMPANY = "thewonderfulcompany" enum value', () => {
      expect(Site.THE_WONDERFUL_COMPANY).toBe('thewonderfulcompany');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new TheWonderfulCompanyService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.THE_WONDERFUL_COMPANY],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'thewonderfulcompany-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.THE_WONDERFUL_COMPANY);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('thewonderfulcompany-' + first.id);
      ...
      expect(calledUrls[0]).toContain('TheWonderfulCompany');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.THE_WONDERFUL_COMPANY);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('thewonderfulcompany-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new TheWonderfulCompanyService(registry);
      const result = await service.scrape({
        siteType: [Site.THE_WONDERFUL_COMPANY],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('TheWonderfulCompany');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('thewonderfulcompany-x1');
      expect(result.jobs[0].site).toBe(Site.THE_WONDERFUL_COMPANY);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('thewonderfulcompany-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new TheWonderfulCompanyService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.THE_WONDERFUL_COMPANY],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new TheWonderfulCompanyService();
      const result = await service.scrape({
        siteType: [Site.THE_WONDERFUL_COMPANY],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
