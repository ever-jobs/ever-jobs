...
import { OwnwellModule, OwnwellService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'ownwell-jobs.json'), 'utf8'),
);
...
/**
 * Spec 673 / T04 — `OwnwellService` unit tests (≥ 9 cases).
 */
describe('OwnwellService — Spec 673 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through OwnwellModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [OwnwellModule],
      }).compile();
      const service = moduleRef.get(OwnwellService);
      expect(service).toBeInstanceOf(OwnwellService);
      await moduleRef.close();
    });

    it('exports the Site.OWNWELL = "ownwell" enum value', () => {
      expect(Site.OWNWELL).toBe('ownwell');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new OwnwellService();
      const result = await service.scrape({
        siteType: [Site.OWNWELL],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'ownwell-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.OWNWELL);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Ownwell');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/ownwell/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/ownwell/jobs/');
      ...
      expect(job0?.description).toContain('Ownwell');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/ownwell/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Ownwell');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'ownwell-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
