...
import { PivotBioModule, PivotBioService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'pivotbio-jobs.json'), 'utf8'),
);
...
/**
 * Spec 783 / T04 — `PivotBioService` unit tests (≥ 9 cases).
 */
describe('PivotBioService — Spec 783 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through PivotBioModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [PivotBioModule],
      }).compile();
      const service = moduleRef.get(PivotBioService);
      expect(service).toBeInstanceOf(PivotBioService);
      await moduleRef.close();
    });

    it('exports the Site.PIVOT_BIO = "pivotbio" enum value', () => {
      expect(Site.PIVOT_BIO).toBe('pivotbio');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new PivotBioService();
      const result = await service.scrape({
        siteType: [Site.PIVOT_BIO],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'pivotbio-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.PIVOT_BIO);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Pivot Bio');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/pivotbio/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/pivotbio/jobs/');
      ...
      expect(job0?.description).toContain('Pivot Bio');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/pivotbio/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Pivot Bio');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'pivotbio-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
