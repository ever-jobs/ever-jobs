...
import { NotableModule, NotableService } from '../src';

const COMPANY_NAME_EXPECT = 'Notable';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'notable-jobs.json'), 'utf8'),
);
...
describe('NotableService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through NotableModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [NotableModule],
      }).compile();
      const service = moduleRef.get(NotableService);
      expect(service).toBeInstanceOf(NotableService);
      await moduleRef.close();
    });

    it('exports the Site.NOTABLE = "notable" enum value', () => {
      expect(Site.NOTABLE).toBe('notable');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new NotableService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.NOTABLE],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('notable');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.NOTABLE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('notable-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new NotableService(registry);
      const result = await service.scrape({
        siteType: [Site.NOTABLE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('notable');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('notable-x1');
      expect(result.jobs[0].site).toBe(Site.NOTABLE);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('notable-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new NotableService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.NOTABLE],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new NotableService();
      const result = await service.scrape({
        siteType: [Site.NOTABLE],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
