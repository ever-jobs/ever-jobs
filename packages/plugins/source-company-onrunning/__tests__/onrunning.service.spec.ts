...
import { OnRunningModule, OnRunningService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'onrunning-jobs.json'), 'utf8'),
);
...
/**
 * Spec 663 / T04 — `OnRunningService` unit tests (≥ 9 cases).
 */
describe('OnRunningService — Spec 663 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through OnRunningModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [OnRunningModule],
      }).compile();
      const service = moduleRef.get(OnRunningService);
      expect(service).toBeInstanceOf(OnRunningService);
      await moduleRef.close();
    });

    it('exports the Site.ON_RUNNING = "onrunning" enum value', () => {
      expect(Site.ON_RUNNING).toBe('onrunning');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new OnRunningService();
      const result = await service.scrape({
        siteType: [Site.ON_RUNNING],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'onrunning-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ON_RUNNING);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('On Running');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/onrunning/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/onrunning/jobs/');
      ...
      expect(job0?.description).toContain('On Running');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/onrunning/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('On Running');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'onrunning-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
