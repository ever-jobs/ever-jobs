...
/**
 * Leidos — Defense and government IT (HQ: Reston, VA, USA).
 *
 * Source (Spec 1736): Workday board, scraped in this order:
 *   - `leidos:5:External` — https://leidos.wd5.myworkdayjobs.com/External
 ...
 * party board.
 *
 * Tags: segment=workday-enterprise; industry=defense-and-government-it.
 */
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'leidos:5:External', atsIdPrefix: 'wd-leidos-' },
];
...
@SourcePlugin({
  ...
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['leidos.com'],
  description: 'Leidos careers via Workday. Tags: segment=workday-enterprise; industry=defense-and-government-it.',
})
@Injectable()
export class LeidosService implements IScraper {
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Workday source plugin is not registered; cannot scrape Leidos');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`Leidos: delegating to Workday (${board.companySlug})`);
    ...
    this.logger.log(`Leidos: scraped ${jobs.length} jobs`);
