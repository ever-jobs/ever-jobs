...
import { LangChainModule, LangChainService } from '../src';

const COMPANY_NAME_EXPECT = 'LangChain';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'langchain-jobs.json'), 'utf8'),
);
...
describe('LangChainService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through LangChainModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [LangChainModule],
      }).compile();
      const service = moduleRef.get(LangChainService);
      expect(service).toBeInstanceOf(LangChainService);
      await moduleRef.close();
    });

    it('exports the Site.LANGCHAIN = "langchain" enum value', () => {
      expect(Site.LANGCHAIN).toBe('langchain');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new LangChainService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.LANGCHAIN],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('langchain');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.LANGCHAIN);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('langchain-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new LangChainService(registry);
      const result = await service.scrape({
        siteType: [Site.LANGCHAIN],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('langchain');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('langchain-x1');
      expect(result.jobs[0].site).toBe(Site.LANGCHAIN);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('langchain-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new LangChainService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.LANGCHAIN],
      } as ScraperInputDto);
