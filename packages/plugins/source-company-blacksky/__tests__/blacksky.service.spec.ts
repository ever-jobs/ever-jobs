...
import { BlackSkyTechnologyModule, BlackSkyTechnologyService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'blacksky-jobs.json'), 'utf8'),
);
...
/**
 * Spec 759 / T04 — `BlackSkyTechnologyService` unit tests (≥ 9 cases).
 */
describe('BlackSkyTechnologyService — Spec 759 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through BlackSkyTechnologyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [BlackSkyTechnologyModule],
      }).compile();
      const service = moduleRef.get(BlackSkyTechnologyService);
      expect(service).toBeInstanceOf(BlackSkyTechnologyService);
      await moduleRef.close();
    });

    it('exports the Site.BLACKSKY_TECHNOLOGY = "blacksky" enum value', () => {
      expect(Site.BLACKSKY_TECHNOLOGY).toBe('blacksky');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new BlackSkyTechnologyService();
      const result = await service.scrape({
        siteType: [Site.BLACKSKY_TECHNOLOGY],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'blacksky-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.BLACKSKY_TECHNOLOGY);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('BlackSky Technology');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/blacksky/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/blacksky/jobs/');
      ...
      expect(job0?.description).toContain('BlackSky Technology');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/blacksky/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('BlackSky Technology');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'blacksky-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
