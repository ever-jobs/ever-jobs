...
import { AppfireModule, AppfireService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'appfire-jobs.json'), 'utf8'),
);
...
/**
 * Spec 291 / T04 — `AppfireService` unit tests (≥ 9 cases).
 */
describe('AppfireService — Spec 291 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AppfireModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AppfireModule],
      }).compile();
      const service = moduleRef.get(AppfireService);
      expect(service).toBeInstanceOf(AppfireService);
      await moduleRef.close();
    });

    it('exports the Site.APPFIRE = "appfire" enum value', () => {
      expect(Site.APPFIRE).toBe('appfire');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AppfireService();
      const result = await service.scrape({
        siteType: [Site.APPFIRE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'appfire-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.APPFIRE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Appfire');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/appfire/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/appfire/jobs/');
      ...
      expect(job0?.description).toContain('Appfire');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/appfire/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Appfire');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'appfire-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
