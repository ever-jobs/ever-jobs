...
import { AnteriadModule, AnteriadService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'anteriad-jobs.json'), 'utf8'),
);
...
/**
 * Spec 273 / T04 — `AnteriadService` unit tests (≥ 9 cases).
 */
describe('AnteriadService — Spec 273 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AnteriadModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AnteriadModule],
      }).compile();
      const service = moduleRef.get(AnteriadService);
      expect(service).toBeInstanceOf(AnteriadService);
      await moduleRef.close();
    });

    it('exports the Site.ANTERIAD = "anteriad" enum value', () => {
      expect(Site.ANTERIAD).toBe('anteriad');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AnteriadService();
      const result = await service.scrape({
        siteType: [Site.ANTERIAD],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'anteriad-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ANTERIAD);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Anteriad ');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/anteriad/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/anteriad/jobs/');
      ...
      expect(job0?.description).toContain('Anteriad ');
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Anteriad ');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'anteriad-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
