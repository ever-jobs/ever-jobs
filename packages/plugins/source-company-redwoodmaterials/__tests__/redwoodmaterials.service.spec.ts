...
import { RedwoodmaterialsModule, RedwoodmaterialsService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'redwoodmaterials-jobs.json'), 'utf8'),
);
...
/**
 * Spec 520 / T04 — `RedwoodmaterialsService` unit tests (≥ 9 cases).
 */
describe('RedwoodmaterialsService — Spec 520 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through RedwoodmaterialsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [RedwoodmaterialsModule],
      }).compile();
      const service = moduleRef.get(RedwoodmaterialsService);
      expect(service).toBeInstanceOf(RedwoodmaterialsService);
      await moduleRef.close();
    });

    it('exports the Site.REDWOOD_MATERIALS = "redwoodmaterials" enum value', () => {
      expect(Site.REDWOOD_MATERIALS).toBe('redwoodmaterials');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new RedwoodmaterialsService();
      const result = await service.scrape({
        siteType: [Site.REDWOOD_MATERIALS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'redwoodmaterials-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.REDWOOD_MATERIALS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Redwood Materials');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/redwoodmaterials/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/redwoodmaterials/jobs/');
      ...
      expect(job0?.description).toContain('Redwood Materials');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/redwoodmaterials/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Redwood Materials');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'redwoodmaterials-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
