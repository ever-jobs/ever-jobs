...
import { DegreedModule, DegreedService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'degreed-jobs.json'), 'utf8'),
);
...
/**
 * Spec 576 / T04 — `DegreedService` unit tests (≥ 9 cases).
 */
describe('DegreedService — Spec 576 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through DegreedModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [DegreedModule],
      }).compile();
      const service = moduleRef.get(DegreedService);
      expect(service).toBeInstanceOf(DegreedService);
      await moduleRef.close();
    });

    it('exports the Site.DEGREED = "degreed" enum value', () => {
      expect(Site.DEGREED).toBe('degreed');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new DegreedService();
      const result = await service.scrape({
        siteType: [Site.DEGREED],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'degreed-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.DEGREED);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Degreed');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/degreed/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/degreed/jobs/');
      ...
      expect(job0?.description).toContain('Degreed');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/degreed/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Degreed');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'degreed-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
