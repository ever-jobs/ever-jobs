...
import { BRINCModule, BRINCService } from '../src';

const COMPANY_NAME_EXPECT = 'BRINC';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'brinc-jobs.json'), 'utf8'),
);
...
describe('BRINCService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through BRINCModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [BRINCModule],
      }).compile();
      const service = moduleRef.get(BRINCService);
      expect(service).toBeInstanceOf(BRINCService);
      await moduleRef.close();
    });

    it('exports the Site.BRINC = "brinc" enum value', () => {
      expect(Site.BRINC).toBe('brinc');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new BRINCService(registryWithAshby());
      ...
      expect(calledUrls[0]).toContain('brinc');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.BRINC);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('brinc-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new BRINCService(registry);
      const result = await service.scrape({
        siteType: [Site.BRINC],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('brinc');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('brinc-x1');
      expect(result.jobs[0].site).toBe(Site.BRINC);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('brinc-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new BRINCService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.BRINC],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new BRINCService();
      const result = await service.scrape({
        siteType: [Site.BRINC],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
