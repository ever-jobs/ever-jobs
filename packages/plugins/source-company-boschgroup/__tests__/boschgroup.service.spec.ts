...
import { BoschGroupModule, BoschGroupService } from '../src';

const COMPANY_NAME_EXPECT = 'Bosch Group';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'boschgroup-jobs.json'), 'utf8'),
);
...
describe('BoschGroupService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through BoschGroupModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [BoschGroupModule],
      }).compile();
      const service = moduleRef.get(BoschGroupService);
      expect(service).toBeInstanceOf(BoschGroupService);
      await moduleRef.close();
    });

    it('exports the Site.BOSCH_GROUP = "boschgroup" enum value', () => {
      expect(Site.BOSCH_GROUP).toBe('boschgroup');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new BoschGroupService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.BOSCH_GROUP],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('BoschGroup');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.BOSCH_GROUP);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('boschgroup-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new BoschGroupService(registry);
      const result = await service.scrape({
        siteType: [Site.BOSCH_GROUP],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('BoschGroup');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('boschgroup-x1');
      expect(result.jobs[0].site).toBe(Site.BOSCH_GROUP);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('boschgroup-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new BoschGroupService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.BOSCH_GROUP],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new BoschGroupService();
      const result = await service.scrape({
        siteType: [Site.BOSCH_GROUP],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
