...
import { WilliamOslerHealthSystemModule, WilliamOslerHealthSystemService } from '../src';

const COMPANY_NAME_EXPECT = 'William Osler Health System';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'williamoslerhealthsystem-jobs.json'), 'utf8'),
);
...
describe('WilliamOslerHealthSystemService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through WilliamOslerHealthSystemModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [WilliamOslerHealthSystemModule],
      }).compile();
      const service = moduleRef.get(WilliamOslerHealthSystemService);
      expect(service).toBeInstanceOf(WilliamOslerHealthSystemService);
      await moduleRef.close();
    });

    it('exports the Site.WILLIAM_OSLER_HEALTH_SYSTEM = "williamoslerhealthsystem" enum value', () => {
      expect(Site.WILLIAM_OSLER_HEALTH_SYSTEM).toBe('williamoslerhealthsystem');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new WilliamOslerHealthSystemService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.WILLIAM_OSLER_HEALTH_SYSTEM],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'williamoslerhealthsystem-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.WILLIAM_OSLER_HEALTH_SYSTEM);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('williamoslerhealthsystem-' + first.id);
      ...
      expect(calledUrls[0]).toContain('williamoslerhealthsystem1');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.WILLIAM_OSLER_HEALTH_SYSTEM);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('williamoslerhealthsystem-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new WilliamOslerHealthSystemService(registry);
      const result = await service.scrape({
        siteType: [Site.WILLIAM_OSLER_HEALTH_SYSTEM],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('williamoslerhealthsystem1');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('williamoslerhealthsystem-x1');
      expect(result.jobs[0].site).toBe(Site.WILLIAM_OSLER_HEALTH_SYSTEM);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('williamoslerhealthsystem-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new WilliamOslerHealthSystemService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.WILLIAM_OSLER_HEALTH_SYSTEM],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new WilliamOslerHealthSystemService();
      const result = await service.scrape({
        siteType: [Site.WILLIAM_OSLER_HEALTH_SYSTEM],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
