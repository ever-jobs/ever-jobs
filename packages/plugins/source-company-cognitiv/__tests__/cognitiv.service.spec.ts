...
import { CognitivModule, CognitivService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'cognitiv-jobs.json'), 'utf8'),
);
...
/**
 * Spec 827 / T04 — `CognitivService` unit tests (≥ 9 cases).
 */
describe('CognitivService — Spec 827 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CognitivModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CognitivModule],
      }).compile();
      const service = moduleRef.get(CognitivService);
      expect(service).toBeInstanceOf(CognitivService);
      await moduleRef.close();
    });

    it('exports the Site.COGNITIV = "cognitiv" enum value', () => {
      expect(Site.COGNITIV).toBe('cognitiv');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new CognitivService();
      const result = await service.scrape({
        siteType: [Site.COGNITIV],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'cognitiv-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.COGNITIV);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Cognitiv');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/cognitiv/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/cognitiv/jobs/');
      ...
      expect(job0?.description).toContain('Cognitiv');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/cognitiv/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Cognitiv');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'cognitiv-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
