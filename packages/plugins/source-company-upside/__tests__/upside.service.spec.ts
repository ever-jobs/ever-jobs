...
import { UpsideModule, UpsideService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'upside-jobs.json'), 'utf8'),
);
...
/**
 * Spec 503 / T04 — `UpsideService` unit tests (≥ 9 cases).
 */
describe('UpsideService — Spec 503 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through UpsideModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [UpsideModule],
      }).compile();
      const service = moduleRef.get(UpsideService);
      expect(service).toBeInstanceOf(UpsideService);
      await moduleRef.close();
    });

    it('exports the Site.UPSIDE = "upside" enum value', () => {
      expect(Site.UPSIDE).toBe('upside');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new UpsideService();
      const result = await service.scrape({
        siteType: [Site.UPSIDE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'upside-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.UPSIDE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Upside');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/upside/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/upside/jobs/');
      ...
      expect(job0?.description).toContain('Upside');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/upside/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Upside');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'upside-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
