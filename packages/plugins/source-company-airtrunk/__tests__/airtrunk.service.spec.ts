...
import { AirtrunkModule, AirtrunkService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'airtrunk-jobs.json'), 'utf8'),
);
...
/**
 * Spec 218 / T04 — `AirtrunkService` unit tests (≥ 9 cases).
 */
describe('AirtrunkService — Spec 218 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AirtrunkModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AirtrunkModule],
      }).compile();
      const service = moduleRef.get(AirtrunkService);
      expect(service).toBeInstanceOf(AirtrunkService);
      await moduleRef.close();
    });

    it('exports the Site.AIRTRUNK = "airtrunk" enum value', () => {
      expect(Site.AIRTRUNK).toBe('airtrunk');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AirtrunkService();
      const result = await service.scrape({
        siteType: [Site.AIRTRUNK],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'airtrunk-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.AIRTRUNK);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('AirTrunk');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/airtrunk/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/airtrunk/jobs/');
      ...
      expect(job0?.description).toContain('AirTrunk');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/airtrunk/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('AirTrunk');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'airtrunk-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
