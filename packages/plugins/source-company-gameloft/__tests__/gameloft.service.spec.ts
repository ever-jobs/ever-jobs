...
import { GameloftModule, GameloftService } from '../src';

const COMPANY_NAME_EXPECT = 'Gameloft';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'gameloft-jobs.json'), 'utf8'),
);
...
describe('GameloftService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through GameloftModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [GameloftModule],
      }).compile();
      const service = moduleRef.get(GameloftService);
      expect(service).toBeInstanceOf(GameloftService);
      await moduleRef.close();
    });

    it('exports the Site.GAMELOFT = "gameloft" enum value', () => {
      expect(Site.GAMELOFT).toBe('gameloft');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new GameloftService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.GAMELOFT],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('Gameloft');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.GAMELOFT);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('gameloft-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new GameloftService(registry);
      const result = await service.scrape({
        siteType: [Site.GAMELOFT],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('Gameloft');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('gameloft-x1');
      expect(result.jobs[0].site).toBe(Site.GAMELOFT);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('gameloft-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new GameloftService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.GAMELOFT],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new GameloftService();
      const result = await service.scrape({
        siteType: [Site.GAMELOFT],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
