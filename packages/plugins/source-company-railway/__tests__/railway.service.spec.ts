...
import { RailwayModule, RailwayService } from '../src';

const COMPANY_NAME_EXPECT = 'Railway';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'railway-jobs.json'), 'utf8'),
);
...
describe('RailwayService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through RailwayModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [RailwayModule],
      }).compile();
      const service = moduleRef.get(RailwayService);
      expect(service).toBeInstanceOf(RailwayService);
      await moduleRef.close();
    });

    it('exports the Site.RAILWAY = "railway" enum value', () => {
      expect(Site.RAILWAY).toBe('railway');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new RailwayService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.RAILWAY],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('railway');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.RAILWAY);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('railway-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new RailwayService(registry);
      const result = await service.scrape({
        siteType: [Site.RAILWAY],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('railway');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('railway-x1');
      expect(result.jobs[0].site).toBe(Site.RAILWAY);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('railway-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new RailwayService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.RAILWAY],
      } as ScraperInputDto);
