...
import { AppletreeprepModule, AppletreeprepService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'appletreeprep-jobs.json'), 'utf8'),
);
...
/**
 * Spec 294 / T04 — `AppletreeprepService` unit tests (≥ 9 cases).
 */
describe('AppletreeprepService — Spec 294 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AppletreeprepModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AppletreeprepModule],
      }).compile();
      const service = moduleRef.get(AppletreeprepService);
      expect(service).toBeInstanceOf(AppletreeprepService);
      await moduleRef.close();
    });

    it('exports the Site.APPLETREEPREP = "appletreeprep" enum value', () => {
      expect(Site.APPLETREEPREP).toBe('appletreeprep');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AppletreeprepService();
      const result = await service.scrape({
        siteType: [Site.APPLETREEPREP],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'appletreeprep-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.APPLETREEPREP);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('AppleTree Prep');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/appletreeprep/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/appletreeprep/jobs/');
      ...
      expect(job0?.description).toContain('AppleTree Prep');
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('AppleTree Prep');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'appletreeprep-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
