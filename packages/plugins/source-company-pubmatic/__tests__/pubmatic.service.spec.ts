...
import { PubMaticModule, PubMaticService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'pubmatic-jobs.json'), 'utf8'),
);
...
/**
 * Spec 915 / T04 — `PubMaticService` unit tests (≥ 9 cases).
 */
describe('PubMaticService — Spec 915 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through PubMaticModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [PubMaticModule],
      }).compile();
      const service = moduleRef.get(PubMaticService);
      expect(service).toBeInstanceOf(PubMaticService);
      await moduleRef.close();
    });

    it('exports the Site.PUBMATIC = "pubmatic" enum value', () => {
      expect(Site.PUBMATIC).toBe('pubmatic');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new PubMaticService();
      const result = await service.scrape({
        siteType: [Site.PUBMATIC],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'pubmatic-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.PUBMATIC);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('PubMatic');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/pubmatic/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/pubmatic/jobs/');
      ...
      expect(job0?.description).toContain('PubMatic');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/pubmatic/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('PubMatic');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'pubmatic-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
