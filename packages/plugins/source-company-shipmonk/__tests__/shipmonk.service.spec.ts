...
import { ShipMonkModule, ShipMonkService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'shipmonk-jobs.json'), 'utf8'),
);
...
/**
 * Spec 932 / T04 — `ShipMonkService` unit tests (≥ 9 cases).
 */
describe('ShipMonkService — Spec 932 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ShipMonkModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ShipMonkModule],
      }).compile();
      const service = moduleRef.get(ShipMonkService);
      expect(service).toBeInstanceOf(ShipMonkService);
      await moduleRef.close();
    });

    it('exports the Site.SHIPMONK = "shipmonk" enum value', () => {
      expect(Site.SHIPMONK).toBe('shipmonk');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ShipMonkService();
      const result = await service.scrape({
        siteType: [Site.SHIPMONK],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'shipmonk-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.SHIPMONK);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('ShipMonk');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/shipmonk/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/shipmonk/jobs/');
      ...
      expect(job0?.description).toContain('ShipMonk');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/shipmonk/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('ShipMonk');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'shipmonk-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
