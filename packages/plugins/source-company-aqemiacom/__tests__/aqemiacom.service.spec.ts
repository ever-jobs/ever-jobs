...
import { AqemiaModule, AqemiaService } from '../src';

const COMPANY_NAME_EXPECT = 'Aqemia';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'aqemiacom-jobs.json'), 'utf8'),
);
...
describe('AqemiaService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AqemiaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AqemiaModule],
      }).compile();
      const service = moduleRef.get(AqemiaService);
      expect(service).toBeInstanceOf(AqemiaService);
      await moduleRef.close();
    });

    it('exports the Site.AQEMIA = "aqemiacom" enum value', () => {
      expect(Site.AQEMIA).toBe('aqemiacom');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new AqemiaService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.AQEMIA],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('aqemia.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.AQEMIA);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('aqemiacom-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new AqemiaService(registry);
      const result = await service.scrape({
        siteType: [Site.AQEMIA],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('aqemia.com');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('aqemiacom-x1');
      expect(result.jobs[0].site).toBe(Site.AQEMIA);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('aqemiacom-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new AqemiaService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.AQEMIA],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new AqemiaService();
      const result = await service.scrape({
        siteType: [Site.AQEMIA],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
