...
import { FlorynModule, FlorynService } from '../src';

const COMPANY_NAME_EXPECT = 'Floryn';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'floryn-jobs.json'), 'utf8'),
);
...
describe('FlorynService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through FlorynModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [FlorynModule],
      }).compile();
      const service = moduleRef.get(FlorynService);
      expect(service).toBeInstanceOf(FlorynService);
      await moduleRef.close();
    });

    it('exports the Site.FLORYN = "floryn" enum value', () => {
      expect(Site.FLORYN).toBe('floryn');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new FlorynService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.FLORYN],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('floryn.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.FLORYN);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('floryn-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new FlorynService(registry);
      const result = await service.scrape({
        siteType: [Site.FLORYN],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('floryn');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('floryn-x1');
      expect(result.jobs[0].site).toBe(Site.FLORYN);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('floryn-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new FlorynService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.FLORYN],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new FlorynService();
      const result = await service.scrape({
        siteType: [Site.FLORYN],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
