...
import { ArcTeryxModule, ArcTeryxService } from '../src';

const COMPANY_NAME_EXPECT = 'Arc\'teryx';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'arcteryxcom-jobs.json'), 'utf8'),
);
...
describe('ArcTeryxService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ArcTeryxModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ArcTeryxModule],
      }).compile();
      const service = moduleRef.get(ArcTeryxService);
      expect(service).toBeInstanceOf(ArcTeryxService);
      await moduleRef.close();
    });

    it('exports the Site.ARC_TERYX = "arcteryxcom" enum value', () => {
      expect(Site.ARC_TERYX).toBe('arcteryxcom');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new ArcTeryxService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.ARC_TERYX],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('arcteryx.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ARC_TERYX);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('arcteryxcom-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new ArcTeryxService(registry);
      const result = await service.scrape({
        siteType: [Site.ARC_TERYX],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('arcteryx.com');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('arcteryxcom-x1');
      expect(result.jobs[0].site).toBe(Site.ARC_TERYX);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('arcteryxcom-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new ArcTeryxService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.ARC_TERYX],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new ArcTeryxService();
      const result = await service.scrape({
        siteType: [Site.ARC_TERYX],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
