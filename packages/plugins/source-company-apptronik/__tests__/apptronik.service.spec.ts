...
import { ApptronikModule, ApptronikService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'apptronik-jobs.json'), 'utf8'),
);
...
/**
 * Spec 553 / T04 — `ApptronikService` unit tests (≥ 9 cases).
 */
describe('ApptronikService — Spec 553 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ApptronikModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ApptronikModule],
      }).compile();
      const service = moduleRef.get(ApptronikService);
      expect(service).toBeInstanceOf(ApptronikService);
      await moduleRef.close();
    });

    it('exports the Site.APPTRONIK = "apptronik" enum value', () => {
      expect(Site.APPTRONIK).toBe('apptronik');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ApptronikService();
      const result = await service.scrape({
        siteType: [Site.APPTRONIK],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'apptronik-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.APPTRONIK);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Apptronik');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/apptronik/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/apptronik/jobs/');
      ...
      expect(job0?.description).toContain('Apptronik');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/apptronik/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Apptronik');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'apptronik-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
