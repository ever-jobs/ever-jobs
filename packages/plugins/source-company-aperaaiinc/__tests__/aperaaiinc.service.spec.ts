...
import { AperaaiincModule, AperaaiincService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'aperaaiinc-jobs.json'), 'utf8'),
);
...
/**
 * Spec 279 / T04 — `AperaaiincService` unit tests (≥ 9 cases).
 */
describe('AperaaiincService — Spec 279 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AperaaiincModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AperaaiincModule],
      }).compile();
      const service = moduleRef.get(AperaaiincService);
      expect(service).toBeInstanceOf(AperaaiincService);
      await moduleRef.close();
    });

    it('exports the Site.APERAAIINC = "aperaaiinc" enum value', () => {
      expect(Site.APERAAIINC).toBe('aperaaiinc');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AperaaiincService();
      const result = await service.scrape({
        siteType: [Site.APERAAIINC],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'aperaaiinc-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.APERAAIINC);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Apera AI Inc');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/aperaaiinc/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/aperaaiinc/jobs/');
      ...
      expect(job0?.description).toContain('Apera AI Inc');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/aperaaiinc/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Apera AI Inc');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'aperaaiinc-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
