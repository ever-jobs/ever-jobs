...
import { GravityClimateModule, GravityClimateService } from '../src';

const COMPANY_NAME_EXPECT = 'Gravity Climate';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'gravityclimate-jobs.json'), 'utf8'),
);
...
describe('GravityClimateService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through GravityClimateModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [GravityClimateModule],
      }).compile();
      const service = moduleRef.get(GravityClimateService);
      expect(service).toBeInstanceOf(GravityClimateService);
      await moduleRef.close();
    });

    it('exports the Site.GRAVITY_CLIMATE = "gravityclimate" enum value', () => {
      expect(Site.GRAVITY_CLIMATE).toBe('gravityclimate');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new GravityClimateService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.GRAVITY_CLIMATE],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'gravityclimate-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.GRAVITY_CLIMATE);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('gravityclimate-' + first.id);
      ...
      expect(calledUrls[0]).toContain('gravityclimate');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.GRAVITY_CLIMATE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('gravityclimate-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new GravityClimateService(registry);
      const result = await service.scrape({
        siteType: [Site.GRAVITY_CLIMATE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('gravityclimate');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('gravityclimate-x1');
      expect(result.jobs[0].site).toBe(Site.GRAVITY_CLIMATE);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('gravityclimate-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new GravityClimateService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.GRAVITY_CLIMATE],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new GravityClimateService();
      const result = await service.scrape({
        siteType: [Site.GRAVITY_CLIMATE],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
