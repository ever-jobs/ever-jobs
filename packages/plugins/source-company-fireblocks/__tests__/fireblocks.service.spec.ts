...
import { FireblocksModule, FireblocksService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'fireblocks-jobs.json'), 'utf8'),
);
...
/**
 * Spec 450 / T04 — `FireblocksService` unit tests (≥ 9 cases).
 */
describe('FireblocksService — Spec 450 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through FireblocksModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [FireblocksModule],
      }).compile();
      const service = moduleRef.get(FireblocksService);
      expect(service).toBeInstanceOf(FireblocksService);
      await moduleRef.close();
    });

    it('exports the Site.FIREBLOCKS = "fireblocks" enum value', () => {
      expect(Site.FIREBLOCKS).toBe('fireblocks');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const job0 = dto.jobs.find((j) => j.id === 'fireblocks-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.FIREBLOCKS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Fireblocks');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/fireblocks/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/fireblocks/jobs/');
      ...
      expect(job0?.description).toContain('Fireblocks');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/fireblocks/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Fireblocks');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'fireblocks-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
