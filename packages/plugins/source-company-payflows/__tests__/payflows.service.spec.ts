...
import { PayflowsModule, PayflowsService } from '../src';

const COMPANY_NAME_EXPECT = 'Payflows';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'payflows-jobs.json'), 'utf8'),
);
...
describe('PayflowsService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through PayflowsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [PayflowsModule],
      }).compile();
      const service = moduleRef.get(PayflowsService);
      expect(service).toBeInstanceOf(PayflowsService);
      await moduleRef.close();
    });

    it('exports the Site.PAYFLOWS = "payflows" enum value', () => {
      expect(Site.PAYFLOWS).toBe('payflows');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new PayflowsService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.PAYFLOWS],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('payflows.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.PAYFLOWS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('payflows-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new PayflowsService(registry);
      const result = await service.scrape({
        siteType: [Site.PAYFLOWS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('payflows');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('payflows-x1');
      expect(result.jobs[0].site).toBe(Site.PAYFLOWS);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('payflows-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new PayflowsService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.PAYFLOWS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new PayflowsService();
      const result = await service.scrape({
        siteType: [Site.PAYFLOWS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
