...
import { RobCoModule, RobCoService } from '../src';

const COMPANY_NAME_EXPECT = 'RobCo';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'robco-jobs.json'), 'utf8'),
);
...
describe('RobCoService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through RobCoModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [RobCoModule],
      }).compile();
      const service = moduleRef.get(RobCoService);
      expect(service).toBeInstanceOf(RobCoService);
      await moduleRef.close();
    });

    it('exports the Site.ROBCO = "robco" enum value', () => {
      expect(Site.ROBCO).toBe('robco');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new RobCoService(registryWithAshby());
      ...
      expect(calledUrls[0]).toContain('robco');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ROBCO);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('robco-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new RobCoService(registry);
      const result = await service.scrape({
        siteType: [Site.ROBCO],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('robco');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('robco-x1');
      expect(result.jobs[0].site).toBe(Site.ROBCO);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('robco-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new RobCoService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.ROBCO],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new RobCoService();
      const result = await service.scrape({
        siteType: [Site.ROBCO],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
