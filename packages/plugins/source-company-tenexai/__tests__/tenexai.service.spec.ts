...
import { TENEXAIModule, TENEXAIService } from '../src';

const COMPANY_NAME_EXPECT = 'TENEX.AI';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'tenexai-jobs.json'), 'utf8'),
);
...
describe('TENEXAIService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through TENEXAIModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [TENEXAIModule],
      }).compile();
      const service = moduleRef.get(TENEXAIService);
      expect(service).toBeInstanceOf(TENEXAIService);
      await moduleRef.close();
    });

    it('exports the Site.TENEX_AI = "tenexai" enum value', () => {
      expect(Site.TENEX_AI).toBe('tenexai');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new TENEXAIService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.TENEX_AI],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('tenex');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.TENEX_AI);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('tenexai-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new TENEXAIService(registry);
      const result = await service.scrape({
        siteType: [Site.TENEX_AI],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('tenex');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('tenexai-x1');
      expect(result.jobs[0].site).toBe(Site.TENEX_AI);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('tenexai-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new TENEXAIService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.TENEX_AI],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new TENEXAIService();
      const result = await service.scrape({
        siteType: [Site.TENEX_AI],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
