...
import { DbtLabsModule, DbtLabsService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'dbtlabsinc-jobs.json'), 'utf8'),
);
...
/**
 * Spec 833 / T04 — `DbtLabsService` unit tests (≥ 9 cases).
 */
describe('DbtLabsService — Spec 833 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through DbtLabsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [DbtLabsModule],
      }).compile();
      const service = moduleRef.get(DbtLabsService);
      expect(service).toBeInstanceOf(DbtLabsService);
      await moduleRef.close();
    });

    it('exports the Site.DBT_LABS = "dbtlabsinc" enum value', () => {
      expect(Site.DBT_LABS).toBe('dbtlabsinc');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new DbtLabsService();
      const result = await service.scrape({
        siteType: [Site.DBT_LABS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'dbtlabsinc-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.DBT_LABS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('dbt Labs');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/dbtlabsinc/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/dbtlabsinc/jobs/');
      ...
      expect(job0?.description).toContain('dbt Labs');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/dbtlabsinc/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('dbt Labs');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'dbtlabsinc-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
