...
import { ReplitModule, ReplitService } from '../src';

const COMPANY_NAME_EXPECT = 'Replit';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'replit-jobs.json'), 'utf8'),
);
...
describe('ReplitService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ReplitModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ReplitModule],
      }).compile();
      const service = moduleRef.get(ReplitService);
      expect(service).toBeInstanceOf(ReplitService);
      await moduleRef.close();
    });

    it('exports the Site.REPLIT = "replit" enum value', () => {
      expect(Site.REPLIT).toBe('replit');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new ReplitService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.REPLIT],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('replit');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.REPLIT);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('replit-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new ReplitService(registry);
      const result = await service.scrape({
        siteType: [Site.REPLIT],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('replit');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('replit-x1');
      expect(result.jobs[0].site).toBe(Site.REPLIT);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('replit-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new ReplitService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.REPLIT],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new ReplitService();
      const result = await service.scrape({
        siteType: [Site.REPLIT],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
