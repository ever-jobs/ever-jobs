...
import { CommonwealthFusionSystemsModule, CommonwealthFusionSystemsService } from '../src';

const COMPANY_NAME_EXPECT = 'Commonwealth Fusion Systems';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'cfsenergy-jobs.json'), 'utf8'),
);
...
describe('CommonwealthFusionSystemsService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CommonwealthFusionSystemsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CommonwealthFusionSystemsModule],
      }).compile();
      const service = moduleRef.get(CommonwealthFusionSystemsService);
      expect(service).toBeInstanceOf(CommonwealthFusionSystemsService);
      await moduleRef.close();
    });

    it('exports the Site.COMMONWEALTH_FUSION_SYSTEMS = "cfsenergy" enum value', () => {
      expect(Site.COMMONWEALTH_FUSION_SYSTEMS).toBe('cfsenergy');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new CommonwealthFusionSystemsService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.COMMONWEALTH_FUSION_SYSTEMS],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'cfsenergy-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.COMMONWEALTH_FUSION_SYSTEMS);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('cfsenergy-' + first.id);
      ...
      expect(calledUrls[0]).toContain('cfsenergy');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.COMMONWEALTH_FUSION_SYSTEMS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('cfsenergy-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new CommonwealthFusionSystemsService(registry);
      const result = await service.scrape({
        siteType: [Site.COMMONWEALTH_FUSION_SYSTEMS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('cfsenergy');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('cfsenergy-x1');
      expect(result.jobs[0].site).toBe(Site.COMMONWEALTH_FUSION_SYSTEMS);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('cfsenergy-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new CommonwealthFusionSystemsService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.COMMONWEALTH_FUSION_SYSTEMS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new CommonwealthFusionSystemsService();
      const result = await service.scrape({
        siteType: [Site.COMMONWEALTH_FUSION_SYSTEMS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
