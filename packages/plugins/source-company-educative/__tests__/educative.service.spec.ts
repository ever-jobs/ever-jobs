...
import { EducativeModule, EducativeService } from '../src';

const COMPANY_NAME_EXPECT = 'Educative';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'educative-jobs.json'), 'utf8'),
);
...
describe('EducativeService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through EducativeModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [EducativeModule],
      }).compile();
      const service = moduleRef.get(EducativeService);
      expect(service).toBeInstanceOf(EducativeService);
      await moduleRef.close();
    });

    it('exports the Site.EDUCATIVE = "educative" enum value', () => {
      expect(Site.EDUCATIVE).toBe('educative');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new EducativeService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.EDUCATIVE],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('educative');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.EDUCATIVE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('educative-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new EducativeService(registry);
      const result = await service.scrape({
        siteType: [Site.EDUCATIVE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('educative');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('educative-x1');
      expect(result.jobs[0].site).toBe(Site.EDUCATIVE);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('educative-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new EducativeService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.EDUCATIVE],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new EducativeService();
      const result = await service.scrape({
        siteType: [Site.EDUCATIVE],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
