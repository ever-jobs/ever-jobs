...
import { ViaModule, ViaService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'via-jobs.json'), 'utf8'),
);
...
/**
 * Spec 802 / T04 — `ViaService` unit tests (≥ 9 cases).
 */
describe('ViaService — Spec 802 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ViaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ViaModule],
      }).compile();
      const service = moduleRef.get(ViaService);
      expect(service).toBeInstanceOf(ViaService);
      await moduleRef.close();
    });

    it('exports the Site.VIA = "via" enum value', () => {
      expect(Site.VIA).toBe('via');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const job0 = dto.jobs.find((j) => j.id === 'via-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.VIA);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Via');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/via/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/via/jobs/');
      ...
      expect(job0?.description).toContain('Via');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/via/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Via');
      }
    });
  });
