...
import { SvixModule, SvixService } from '../src';

const COMPANY_NAME_EXPECT = 'Svix';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'svix-jobs.json'), 'utf8'),
);
...
describe('SvixService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SvixModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SvixModule],
      }).compile();
      const service = moduleRef.get(SvixService);
      expect(service).toBeInstanceOf(SvixService);
      await moduleRef.close();
    });

    it('exports the Site.SVIX = "svix" enum value', () => {
      expect(Site.SVIX).toBe('svix');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new SvixService(registryWithAshby());
      ...
      expect(calledUrls[0]).toContain('svix');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SVIX);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('svix-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new SvixService(registry);
      const result = await service.scrape({
        siteType: [Site.SVIX],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('svix');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('svix-x1');
      expect(result.jobs[0].site).toBe(Site.SVIX);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('svix-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new SvixService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.SVIX],
      } as ScraperInputDto);
