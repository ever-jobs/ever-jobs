...
import { OrchardModule, OrchardService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'orchard-jobs.json'), 'utf8'),
);
...
/**
 * Spec 592 / T04 — `OrchardService` unit tests (≥ 9 cases).
 */
describe('OrchardService — Spec 592 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through OrchardModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [OrchardModule],
      }).compile();
      const service = moduleRef.get(OrchardService);
      expect(service).toBeInstanceOf(OrchardService);
      await moduleRef.close();
    });

    it('exports the Site.ORCHARD = "orchard" enum value', () => {
      expect(Site.ORCHARD).toBe('orchard');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new OrchardService();
      const result = await service.scrape({
        siteType: [Site.ORCHARD],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'orchard-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ORCHARD);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Orchard');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/orchard/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/orchard/jobs/');
      ...
      expect(job0?.description).toContain('Orchard');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/orchard/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Orchard');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'orchard-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
