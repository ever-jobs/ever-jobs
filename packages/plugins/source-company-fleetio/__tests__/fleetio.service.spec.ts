...
import { FleetioModule, FleetioService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'fleetio-jobs.json'), 'utf8'),
);
...
/**
 * Spec 850 / T04 — `FleetioService` unit tests (≥ 9 cases).
 */
describe('FleetioService — Spec 850 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through FleetioModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [FleetioModule],
      }).compile();
      const service = moduleRef.get(FleetioService);
      expect(service).toBeInstanceOf(FleetioService);
      await moduleRef.close();
    });

    it('exports the Site.FLEETIO = "fleetio" enum value', () => {
      expect(Site.FLEETIO).toBe('fleetio');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new FleetioService();
      const result = await service.scrape({
        siteType: [Site.FLEETIO],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'fleetio-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.FLEETIO);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Fleetio');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/fleetio/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/fleetio/jobs/');
      ...
      expect(job0?.description).toContain('Fleetio');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/fleetio/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Fleetio');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'fleetio-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
