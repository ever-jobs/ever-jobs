...
import { PrenuvoModule, PrenuvoService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'prenuvo-jobs.json'), 'utf8'),
);
...
/**
 * Spec 645 / T04 — `PrenuvoService` unit tests (≥ 9 cases).
 */
describe('PrenuvoService — Spec 645 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through PrenuvoModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [PrenuvoModule],
      }).compile();
      const service = moduleRef.get(PrenuvoService);
      expect(service).toBeInstanceOf(PrenuvoService);
      await moduleRef.close();
    });

    it('exports the Site.PRENUVO = "prenuvo" enum value', () => {
      expect(Site.PRENUVO).toBe('prenuvo');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new PrenuvoService();
      const result = await service.scrape({
        siteType: [Site.PRENUVO],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'prenuvo-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.PRENUVO);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Prenuvo');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/prenuvo/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/prenuvo/jobs/');
      ...
      expect(job0?.description).toContain('Prenuvo');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/prenuvo/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Prenuvo');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'prenuvo-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
