...
import { GrupoMariposaModule, GrupoMariposaService } from '../src';

const COMPANY_NAME_EXPECT = 'Grupo Mariposa';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'grupomariposa-jobs.json'), 'utf8'),
);
...
describe('GrupoMariposaService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through GrupoMariposaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [GrupoMariposaModule],
      }).compile();
      const service = moduleRef.get(GrupoMariposaService);
      expect(service).toBeInstanceOf(GrupoMariposaService);
      await moduleRef.close();
    });

    it('exports the Site.GRUPO_MARIPOSA = "grupomariposa" enum value', () => {
      expect(Site.GRUPO_MARIPOSA).toBe('grupomariposa');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new GrupoMariposaService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.GRUPO_MARIPOSA],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'grupomariposa-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.GRUPO_MARIPOSA);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('grupomariposa-' + first.id);
      ...
      expect(calledUrls[0]).toContain('GrupoMariposa1');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.GRUPO_MARIPOSA);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('grupomariposa-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new GrupoMariposaService(registry);
      const result = await service.scrape({
        siteType: [Site.GRUPO_MARIPOSA],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('GrupoMariposa1');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('grupomariposa-x1');
      expect(result.jobs[0].site).toBe(Site.GRUPO_MARIPOSA);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('grupomariposa-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new GrupoMariposaService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.GRUPO_MARIPOSA],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new GrupoMariposaService();
      const result = await service.scrape({
        siteType: [Site.GRUPO_MARIPOSA],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
