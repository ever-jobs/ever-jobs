...
/**
 * IMC Trading — Market making (HQ: Amsterdam, Netherlands).
 *
 * Source (Spec 1737): Greenhouse board, scraped in this order:
 *   - `imc` — https://job-boards.greenhouse.io/imc
 *     verified live 2026-09-24: 172 open postings.
 *
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'imc', atsIdPrefix: 'gh-' },
];
...
@SourcePlugin({
  ...
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['imc.com'],
  description: 'IMC Trading careers via Greenhouse. Tags: segment=quant-trading; industry=market-making.',
})
@Injectable()
export class ImcService implements IScraper {
  private readonly logger = new Logger(ImcService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Greenhouse source plugin is not registered; cannot scrape IMC Trading');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`IMC Trading: delegating to Greenhouse (${board.companySlug})`);
    ...
    this.logger.log(`IMC Trading: scraped ${jobs.length} jobs`);
