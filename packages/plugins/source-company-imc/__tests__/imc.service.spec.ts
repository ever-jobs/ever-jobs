...
import { ImcModule, ImcService } from '../src';
...
const FIXTURE: Fixture = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'fixtures', 'imc-boards.json'), 'utf8'),
);
const COMPANY_NAME = 'IMC Trading';
const ID_PREFIX = 'imc-';
...
describe('ImcService — Greenhouse delegation (Spec 1737)', () => {
  beforeEach(() => {
    mockGet.mockReset();
    mockPost.mockReset();
  ...
  describe('registration', () => {
    it('resolves through ImcModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({ imports: [ImcModule] }).compile();
      expect(moduleRef.get(ImcService)).toBeInstanceOf(ImcService);
      await moduleRef.close();
    });

    it('exports Site.IMC = "imc"', () => {
      expect(Site.IMC).toBe('imc');
    });

    it('declares a tagged company plugin with its domains', () => {
      const meta = Reflect.getMetadata(SOURCE_PLUGIN_METADATA, ImcService);
      expect(meta.site).toBe(Site.IMC);
      expect(meta.category).toBe('company');
      expect(meta.isAts).toBeFalsy();
      expect(meta.companyDomains).toEqual(['imc.com']);
  ...
  describe('recorded board (real Greenhouse adapter, mocked HTTP)', () => {
    it('maps every recorded posting and re-stamps the company identity', async () => {
      const service = new ImcService(registryWith());
      const result = await service.scrape({ siteType: [Site.IMC], resultsWanted: 100 } as ScraperInputDto);
    ...
    it('only requests the recorded board URLs', async () => {
      const service = new ImcService(registryWith());
      await service.scrape({ siteType: [Site.IMC] } as ScraperInputDto);
    ...
    it('honours resultsWanted=1', async () => {
      const service = new ImcService(registryWith());
      const result = await service.scrape({ siteType: [Site.IMC], resultsWanted: 1 } as ScraperInputDto);
    ...
    it('resolves with an empty result when the board is gone (HTTP 404)', async () => {
      ...
      const service = new ImcService(registryWith());
      const result = await service.scrape({ siteType: [Site.IMC] } as ScraperInputDto);
  ...
  describe('delegation contract', () => {
    it('forwards the board and the caller input untouched', async () => {
      ...
      expect(result.jobs[0].site).toBe(Site.IMC);
    ...
    it('never forwards the caller\'s credentials to the board (Spec 1735 §4.5)', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new ImcService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
    ...
    it('makes no request when resultsWanted is 0', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new ImcService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      const result = await service.scrape({ siteType: [Site.IMC], resultsWanted: 0 } as ScraperInputDto);
  ...
  describe('resilience', () => {
    it('reports not_registered when no registry is injected', async () => {
      const result = await new ImcService().scrape({ siteType: [Site.IMC] } as ScraperInputDto);
      expect(result.jobs).toEqual([]);
    ...
    it('reports not_registered when Greenhouse is missing from the registry', async () => {
      const result = await new ImcService(new PluginRegistry()).scrape({
        siteType: [Site.IMC],
      } as ScraperInputDto);
      expect(result.diagnostics?.reason).toBe('not_registered');
    });
