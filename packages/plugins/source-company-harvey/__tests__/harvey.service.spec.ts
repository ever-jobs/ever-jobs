...
import { HarveyModule, HarveyService } from '../src';

const COMPANY_NAME_EXPECT = 'Harvey';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'harvey-jobs.json'), 'utf8'),
);
...
describe('HarveyService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through HarveyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [HarveyModule],
      }).compile();
      const service = moduleRef.get(HarveyService);
      expect(service).toBeInstanceOf(HarveyService);
      await moduleRef.close();
    });

    it('exports the Site.HARVEY = "harvey" enum value', () => {
      expect(Site.HARVEY).toBe('harvey');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new HarveyService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.HARVEY],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('harvey');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.HARVEY);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('harvey-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new HarveyService(registry);
      const result = await service.scrape({
        siteType: [Site.HARVEY],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('harvey');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('harvey-x1');
      expect(result.jobs[0].site).toBe(Site.HARVEY);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('harvey-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new HarveyService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.HARVEY],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new HarveyService();
      const result = await service.scrape({
        siteType: [Site.HARVEY],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
