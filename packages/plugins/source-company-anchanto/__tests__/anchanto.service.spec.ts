...
import { AnchantoModule, AnchantoService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'anchanto-jobs.json'), 'utf8'),
);
...
/**
 * Spec 268 / T04 — `AnchantoService` unit tests (≥ 9 cases).
 */
describe('AnchantoService — Spec 268 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AnchantoModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AnchantoModule],
      }).compile();
      const service = moduleRef.get(AnchantoService);
      expect(service).toBeInstanceOf(AnchantoService);
      await moduleRef.close();
    });

    it('exports the Site.ANCHANTO = "anchanto" enum value', () => {
      expect(Site.ANCHANTO).toBe('anchanto');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AnchantoService();
      const result = await service.scrape({
        siteType: [Site.ANCHANTO],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'anchanto-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ANCHANTO);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Anchanto');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/anchanto/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/anchanto/jobs/');
      ...
      expect(job0?.description).toContain('Anchanto');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/anchanto/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Anchanto');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'anchanto-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
