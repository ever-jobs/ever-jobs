...
import { VerkadaModule, VerkadaService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'verkada-jobs.json'), 'utf8'),
);
...
/**
 * Spec 433 / T04 — `VerkadaService` unit tests (≥ 9 cases).
 */
describe('VerkadaService — Spec 433 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through VerkadaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [VerkadaModule],
      }).compile();
      const service = moduleRef.get(VerkadaService);
      expect(service).toBeInstanceOf(VerkadaService);
      await moduleRef.close();
    });

    it('exports the Site.VERKADA = "verkada" enum value', () => {
      expect(Site.VERKADA).toBe('verkada');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new VerkadaService();
      const result = await service.scrape({
        siteType: [Site.VERKADA],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'verkada-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.VERKADA);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Verkada');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/verkada/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/verkada/jobs/');
      ...
      expect(job0?.description).toContain('Verkada');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/verkada/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Verkada');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'verkada-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
