...
import { EurofinsScientificModule, EurofinsScientificService } from '../src';

const COMPANY_NAME_EXPECT = 'Eurofins Scientific';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'eurofinsscientific-jobs.json'), 'utf8'),
);
...
describe('EurofinsScientificService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through EurofinsScientificModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [EurofinsScientificModule],
      }).compile();
      const service = moduleRef.get(EurofinsScientificService);
      expect(service).toBeInstanceOf(EurofinsScientificService);
      await moduleRef.close();
    });

    it('exports the Site.EUROFINS_SCIENTIFIC = "eurofinsscientific" enum value', () => {
      expect(Site.EUROFINS_SCIENTIFIC).toBe('eurofinsscientific');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new EurofinsScientificService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.EUROFINS_SCIENTIFIC],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'eurofinsscientific-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.EUROFINS_SCIENTIFIC);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('eurofinsscientific-' + first.id);
      ...
      expect(calledUrls[0]).toContain('Eurofins');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.EUROFINS_SCIENTIFIC);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('eurofinsscientific-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new EurofinsScientificService(registry);
      const result = await service.scrape({
        siteType: [Site.EUROFINS_SCIENTIFIC],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('Eurofins');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('eurofinsscientific-x1');
      expect(result.jobs[0].site).toBe(Site.EUROFINS_SCIENTIFIC);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('eurofinsscientific-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new EurofinsScientificService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.EUROFINS_SCIENTIFIC],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new EurofinsScientificService();
      const result = await service.scrape({
        siteType: [Site.EUROFINS_SCIENTIFIC],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
