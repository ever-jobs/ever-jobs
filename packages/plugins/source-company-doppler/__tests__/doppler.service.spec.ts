...
import { DopplerModule, DopplerService } from '../src';

const COMPANY_NAME_EXPECT = 'Doppler';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'doppler-jobs.json'), 'utf8'),
);
...
describe('DopplerService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through DopplerModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [DopplerModule],
      }).compile();
      const service = moduleRef.get(DopplerService);
      expect(service).toBeInstanceOf(DopplerService);
      await moduleRef.close();
    });

    it('exports the Site.DOPPLER = "doppler" enum value', () => {
      expect(Site.DOPPLER).toBe('doppler');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new DopplerService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.DOPPLER],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('doppler');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.DOPPLER);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('doppler-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new DopplerService(registry);
      const result = await service.scrape({
        siteType: [Site.DOPPLER],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('doppler');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('doppler-x1');
      expect(result.jobs[0].site).toBe(Site.DOPPLER);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('doppler-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new DopplerService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.DOPPLER],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new DopplerService();
      const result = await service.scrape({
        siteType: [Site.DOPPLER],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
