...
import { SwordHealthModule, SwordHealthService } from '../src';

const COMPANY_NAME_EXPECT = 'Sword Health';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'swordhealth-jobs.json'), 'utf8'),
);
...
describe('SwordHealthService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SwordHealthModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SwordHealthModule],
      }).compile();
      const service = moduleRef.get(SwordHealthService);
      expect(service).toBeInstanceOf(SwordHealthService);
      await moduleRef.close();
    });

    it('exports the Site.SWORD_HEALTH = "swordhealth" enum value', () => {
      expect(Site.SWORD_HEALTH).toBe('swordhealth');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new SwordHealthService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.SWORD_HEALTH],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'swordhealth-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.SWORD_HEALTH);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('swordhealth-' + first.id);
      ...
      expect(calledUrls[0]).toContain('swordhealth');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SWORD_HEALTH);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('swordhealth-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new SwordHealthService(registry);
      const result = await service.scrape({
        siteType: [Site.SWORD_HEALTH],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('swordhealth');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('swordhealth-x1');
      expect(result.jobs[0].site).toBe(Site.SWORD_HEALTH);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('swordhealth-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new SwordHealthService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.SWORD_HEALTH],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new SwordHealthService();
      const result = await service.scrape({
        siteType: [Site.SWORD_HEALTH],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
