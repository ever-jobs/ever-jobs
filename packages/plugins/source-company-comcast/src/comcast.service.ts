...
/**
 * Comcast — Media and telecommunications (HQ: Philadelphia, PA, USA).
 *
 * Source (Spec 1736): Workday board, scraped in this order:
 *   - `comcast:115:Comcast_Careers` — https://comcast.wd115.myworkdayjobs.com/Comcast_Careers
 *     verified live 2026-09-24: 673 open postings.
 *
 ...
 * through untouched.
 *
 * Tags: segment=workday-enterprise; industry=media-and-telecommunications.
 */
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'comcast:115:Comcast_Careers', atsIdPrefix: 'wd-comcast-' },
];
...
@SourcePlugin({
  ...
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['comcast.com'],
  description: 'Comcast careers via Workday. Tags: segment=workday-enterprise; industry=media-and-telecommunications.',
})
@Injectable()
export class ComcastService implements IScraper {
  private readonly logger = new Logger(ComcastService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Workday source plugin is not registered; cannot scrape Comcast');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`Comcast: delegating to Workday (${board.companySlug})`);
    ...
    this.logger.log(`Comcast: scraped ${jobs.length} jobs`);
