...
import { HumbleModule, HumbleService } from '../src';

const COMPANY_NAME_EXPECT = 'Humble';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'humblerobotics-jobs.json'), 'utf8'),
);
...
describe('HumbleService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through HumbleModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [HumbleModule],
      }).compile();
      const service = moduleRef.get(HumbleService);
      expect(service).toBeInstanceOf(HumbleService);
      await moduleRef.close();
    });

    it('exports the Site.HUMBLE = "humblerobotics" enum value', () => {
      expect(Site.HUMBLE).toBe('humblerobotics');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new HumbleService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.HUMBLE],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'humblerobotics-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.HUMBLE);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('humblerobotics-' + first.id);
      ...
      expect(calledUrls[0]).toContain('humble-robotics');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.HUMBLE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('humblerobotics-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new HumbleService(registry);
      const result = await service.scrape({
        siteType: [Site.HUMBLE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('humble-robotics');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('humblerobotics-x1');
      expect(result.jobs[0].site).toBe(Site.HUMBLE);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('humblerobotics-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new HumbleService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.HUMBLE],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new HumbleService();
      const result = await service.scrape({
        siteType: [Site.HUMBLE],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
