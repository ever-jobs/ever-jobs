...
import { KnockModule, KnockService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'knock-jobs.json'), 'utf8'),
);
...
/**
 * Spec 484 / T04 — `KnockService` unit tests (≥ 9 cases).
 */
describe('KnockService — Spec 484 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through KnockModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [KnockModule],
      }).compile();
      const service = moduleRef.get(KnockService);
      expect(service).toBeInstanceOf(KnockService);
      await moduleRef.close();
    });

    it('exports the Site.KNOCK = "knock" enum value', () => {
      expect(Site.KNOCK).toBe('knock');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new KnockService();
      ...
      const job0 = dto.jobs.find((j) => j.id === 'knock-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.KNOCK);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Knock');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/knock/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/knock/jobs/');
      ...
      expect(job0?.description).toContain('Knock');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/knock/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Knock');
      }
    });
  });
