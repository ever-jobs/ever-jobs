...
import { ReflectOrbitalModule, ReflectOrbitalService } from '../src';

const COMPANY_NAME_EXPECT = 'Reflect Orbital';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'reflectorbital-jobs.json'), 'utf8'),
);
...
describe('ReflectOrbitalService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ReflectOrbitalModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ReflectOrbitalModule],
      }).compile();
      const service = moduleRef.get(ReflectOrbitalService);
      expect(service).toBeInstanceOf(ReflectOrbitalService);
      await moduleRef.close();
    });

    it('exports the Site.REFLECT_ORBITAL = "reflectorbital" enum value', () => {
      expect(Site.REFLECT_ORBITAL).toBe('reflectorbital');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new ReflectOrbitalService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.REFLECT_ORBITAL],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'reflectorbital-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.REFLECT_ORBITAL);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('reflectorbital-' + first.id);
      ...
      expect(calledUrls[0]).toContain('reflect-orbital');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.REFLECT_ORBITAL);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('reflectorbital-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new ReflectOrbitalService(registry);
      const result = await service.scrape({
        siteType: [Site.REFLECT_ORBITAL],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('reflect-orbital');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('reflectorbital-x1');
      expect(result.jobs[0].site).toBe(Site.REFLECT_ORBITAL);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('reflectorbital-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new ReflectOrbitalService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.REFLECT_ORBITAL],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new ReflectOrbitalService();
      const result = await service.scrape({
        siteType: [Site.REFLECT_ORBITAL],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
