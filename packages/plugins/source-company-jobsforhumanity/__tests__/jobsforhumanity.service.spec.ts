...
import { JobsForHumanityModule, JobsForHumanityService } from '../src';

const COMPANY_NAME_EXPECT = 'Jobs for Humanity';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'jobsforhumanity-jobs.json'), 'utf8'),
);
...
describe('JobsForHumanityService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through JobsForHumanityModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [JobsForHumanityModule],
      }).compile();
      const service = moduleRef.get(JobsForHumanityService);
      expect(service).toBeInstanceOf(JobsForHumanityService);
      await moduleRef.close();
    });

    it('exports the Site.JOBS_FOR_HUMANITY = "jobsforhumanity" enum value', () => {
      expect(Site.JOBS_FOR_HUMANITY).toBe('jobsforhumanity');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new JobsForHumanityService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.JOBS_FOR_HUMANITY],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'jobsforhumanity-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.JOBS_FOR_HUMANITY);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('jobsforhumanity-' + first.id);
      ...
      expect(calledUrls[0]).toContain('jobsforhumanity');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.JOBS_FOR_HUMANITY);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('jobsforhumanity-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new JobsForHumanityService(registry);
      const result = await service.scrape({
        siteType: [Site.JOBS_FOR_HUMANITY],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('jobsforhumanity');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('jobsforhumanity-x1');
      expect(result.jobs[0].site).toBe(Site.JOBS_FOR_HUMANITY);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('jobsforhumanity-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new JobsForHumanityService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.JOBS_FOR_HUMANITY],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new JobsForHumanityService();
      const result = await service.scrape({
        siteType: [Site.JOBS_FOR_HUMANITY],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
