...
import { CerebrasSystemsModule, CerebrasSystemsService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'cerebrassystems-jobs.json'), 'utf8'),
);
...
/**
 * Spec 728 / T04 — `CerebrasSystemsService` unit tests (≥ 9 cases).
 */
describe('CerebrasSystemsService — Spec 728 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CerebrasSystemsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CerebrasSystemsModule],
      }).compile();
      const service = moduleRef.get(CerebrasSystemsService);
      expect(service).toBeInstanceOf(CerebrasSystemsService);
      await moduleRef.close();
    });

    it('exports the Site.CEREBRAS_SYSTEMS = "cerebrassystems" enum value', () => {
      expect(Site.CEREBRAS_SYSTEMS).toBe('cerebrassystems');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new CerebrasSystemsService();
      const result = await service.scrape({
        siteType: [Site.CEREBRAS_SYSTEMS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'cerebrassystems-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.CEREBRAS_SYSTEMS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Cerebras Systems');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/cerebrassystems/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/cerebrassystems/jobs/');
      ...
      expect(job0?.description).toContain('Cerebras Systems');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/cerebrassystems/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Cerebras Systems');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'cerebrassystems-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
