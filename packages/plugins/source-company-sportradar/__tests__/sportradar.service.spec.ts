...
import { SportradarModule, SportradarService } from '../src';

const COMPANY_NAME_EXPECT = 'Sportradar';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'sportradar-jobs.json'), 'utf8'),
);
...
describe('SportradarService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SportradarModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SportradarModule],
      }).compile();
      const service = moduleRef.get(SportradarService);
      expect(service).toBeInstanceOf(SportradarService);
      await moduleRef.close();
    });

    it('exports the Site.SPORTRADAR = "sportradar" enum value', () => {
      expect(Site.SPORTRADAR).toBe('sportradar');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new SportradarService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.SPORTRADAR],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('Sportradar');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SPORTRADAR);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('sportradar-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new SportradarService(registry);
      const result = await service.scrape({
        siteType: [Site.SPORTRADAR],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('Sportradar');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('sportradar-x1');
      expect(result.jobs[0].site).toBe(Site.SPORTRADAR);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('sportradar-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new SportradarService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.SPORTRADAR],
      } as ScraperInputDto);
