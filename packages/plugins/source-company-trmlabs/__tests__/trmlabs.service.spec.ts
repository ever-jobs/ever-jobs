...
import { TRMLabsModule, TRMLabsService } from '../src';

const COMPANY_NAME_EXPECT = 'TRM Labs';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'trmlabs-jobs.json'), 'utf8'),
);
...
describe('TRMLabsService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through TRMLabsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [TRMLabsModule],
      }).compile();
      const service = moduleRef.get(TRMLabsService);
      expect(service).toBeInstanceOf(TRMLabsService);
      await moduleRef.close();
    });

    it('exports the Site.TRM_LABS = "trmlabs" enum value', () => {
      expect(Site.TRM_LABS).toBe('trmlabs');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new TRMLabsService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.TRM_LABS],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('trm-labs');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.TRM_LABS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('trmlabs-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new TRMLabsService(registry);
      const result = await service.scrape({
        siteType: [Site.TRM_LABS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('trm-labs');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('trmlabs-x1');
      expect(result.jobs[0].site).toBe(Site.TRM_LABS);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('trmlabs-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new TRMLabsService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.TRM_LABS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new TRMLabsService();
      const result = await service.scrape({
        siteType: [Site.TRM_LABS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
