...
import { SayariModule, SayariService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'sayari-jobs.json'), 'utf8'),
);
...
/**
 * Spec 925 / T04 — `SayariService` unit tests (≥ 9 cases).
 */
describe('SayariService — Spec 925 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SayariModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SayariModule],
      }).compile();
      const service = moduleRef.get(SayariService);
      expect(service).toBeInstanceOf(SayariService);
      await moduleRef.close();
    });

    it('exports the Site.SAYARI = "sayari" enum value', () => {
      expect(Site.SAYARI).toBe('sayari');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new SayariService();
      const result = await service.scrape({
        siteType: [Site.SAYARI],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'sayari-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.SAYARI);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Sayari');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/sayari/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/sayari/jobs/');
      ...
      expect(job0?.description).toContain('Sayari');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/sayari/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Sayari');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'sayari-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
