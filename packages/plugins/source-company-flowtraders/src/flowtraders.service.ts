...
/**
 * Flow Traders — Market making (HQ: Amsterdam, Netherlands).
 *
 * Source (Spec 1737): Greenhouse board, scraped in this order:
 *   - `flowtraders` — https://job-boards.greenhouse.io/flowtraders
 *     verified live 2026-09-24: 48 open postings.
 *
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'flowtraders', atsIdPrefix: 'gh-' },
];

@SourcePlugin({
  site: Site.FLOW_TRADERS,
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['flowtraders.com'],
  description: 'Flow Traders careers via Greenhouse. Tags: segment=quant-trading; industry=market-making.',
})
@Injectable()
export class FlowTradersService implements IScraper {
  private readonly logger = new Logger(FlowTradersService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Greenhouse source plugin is not registered; cannot scrape Flow Traders');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`Flow Traders: delegating to Greenhouse (${board.companySlug})`);
    ...
    this.logger.log(`Flow Traders: scraped ${jobs.length} jobs`);
