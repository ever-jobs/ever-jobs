...
import { ProofModule, ProofService } from '../src';

const COMPANY_NAME_EXPECT = 'Proof';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'proof-jobs.json'), 'utf8'),
);
...
describe('ProofService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ProofModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ProofModule],
      }).compile();
      const service = moduleRef.get(ProofService);
      expect(service).toBeInstanceOf(ProofService);
      await moduleRef.close();
    });

    it('exports the Site.PROOF = "proof" enum value', () => {
      expect(Site.PROOF).toBe('proof');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new ProofService(registryWithLever());
      ...
      expect(calledUrls[0]).toContain('proof');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.PROOF);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('proof-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new ProofService(registry);
      const result = await service.scrape({
        siteType: [Site.PROOF],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('proof');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('proof-x1');
      expect(result.jobs[0].site).toBe(Site.PROOF);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('proof-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new ProofService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.PROOF],
      } as ScraperInputDto);
