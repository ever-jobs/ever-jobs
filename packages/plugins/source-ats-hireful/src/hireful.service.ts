...
import {
  ...
  HIREFUL_ROOT_DOMAIN,
  HIREFUL_ALT_DOMAINS,
  HIREFUL_SITEMAP_PATH,
  HIREFUL_SITEMAP_LOC_REGEX,
  HIREFUL_LASTMOD_REGEX,
  HIREFUL_VACANCY_URL_REGEX,
  HIREFUL_JSONLD_REGEX,
  HIREFUL_OG_TITLE_REGEX,
  HIREFUL_OG_URL_REGEX,
  HIREFUL_OG_DESCRIPTION_REGEX,
  HIREFUL_TITLE_TAG_REGEX,
  HIREFUL_REMOTE_REGEX,
  HIREFUL_DEFAULT_RESULTS,
  HIREFUL_HEADERS,
} from './hireful.constants';
import {
  HirefulJob,
  HirefulJobPosting,
  HirefulJobLocation,
  HirefulPostalAddress,
  HirefulSitemapEntry,
} from './hireful.types';

/**
 * Hireful (LiveVacancies) ATS careers scraper — generic, multi-tenant.
 *
 * Hireful (hireful.com / hireful.co.uk, UK) powers each customer's candidate
 * portal on the "LiveVacancies" platform at `https://{tenant}.livevacancies.co.uk/`
 * (some tenants also front it under a custom careers host such as
 * `agency.hireful.com`). The jobs index is a client-rendered SPA, so the adapter
 * instead enumerates the tenant's roles from the public XML sitemap
 * (`/sitemap.xml`, which lists every open `/vacancy/{vacancyId}/{slug}` page) and
 * parses each server-rendered detail page's schema.org `JobPosting` JSON-LD block
 * (with `og:` meta tags as defensive fallbacks).
 *
 * The caller addresses a tenant by `companySlug` (the sub-domain label, e.g.
 * `thebigissue`) or by `companyUrl` (a portal URL whose first sub-domain label is
 * the tenant, or a custom careers host used verbatim). The sitemap lists every
 * open role in one document — there is no server-side pagination of the job set —
 * so we fetch once and slice client-side to honour `resultsWanted`. A single
 * fetch error, an unknown tenant (HTTP 4xx), or a malformed page degrades to an
...
@Injectable()
export class HirefulService implements IScraper {
  private readonly logger = new Logger(HirefulService.name);

  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    const companySlug = input.companySlug;
    if (!companySlug && !input.companyUrl) {
      this.logger.warn('No companySlug or companyUrl provided for Hireful scraper');
      return new JobResponseDto([]);
    }

    const host = this.resolveHost(companySlug, input.companyUrl);
    if (!host) {
      this.logger.warn('Could not resolve a Hireful careers host from input');
      return new JobResponseDto([]);
    }
    ...
    client.setHeaders(HIREFUL_HEADERS);

    const resultsWanted = input.resultsWanted ?? HIREFUL_DEFAULT_RESULTS;
    const tenant = this.deriveTenant(companySlug, host);
    const seen = new Set<string>();
    const jobPosts: JobPostDto[] = [];

    try {
      this.logger.log(`Fetching Hireful sitemap from: ${host}`);

      // The sitemap enumerates every open role for the tenant in one document.
      const entries = await this.fetchSitemap(client, host);
      if (entries.length === 0) {
        this.logger.log(`Hireful tenant "${tenant}" has no open roles`);
        return new JobResponseDto([]);
      }

      // Only fetch as many detail pages as the caller asked for (deduped first).
      const wanted = entries
        .filter((e) => !seen.has(e.vacancyId) && seen.add(e.vacancyId))
      ...
      for (const entry of wanted) {
        try {
          ...
          this.logger.warn(`Error processing Hireful vacancy ${entry.vacancyId}: ${err.message}`);
        }
      }

      this.logger.log(`Hireful total: ${jobPosts.length} jobs for ${tenant}`);
      return new JobResponseDto(jobPosts);
    } catch (err: any) {
      this.logger.error(`Hireful scrape error for ${host}: ${err.message}`);
  ...
  private async fetchSitemap(
    ...
    host: string,
  ): Promise<HirefulSitemapEntry[]> {
    const url = `${host}${HIREFUL_SITEMAP_PATH}`;
    try {
      ...
      if (status && status >= 400 && status < 500) {
        this.logger.warn(`Hireful sitemap not found (HTTP ${status}) at ${url}`);
        return [];
      }
      throw err;
    }
  ...
  /**
   * Extract `/vacancy/{vacancyId}[/{slug}]` entries (with their `<lastmod>`) from
   * the sitemap XML. The bare `/vacancy/` index and other site pages (about,
   * preferences, departments) carry no id and are skipped by the vacancy-URL
   * regex.
   */
  private parseSitemap(xml: string): HirefulSitemapEntry[] {
    const entries: HirefulSitemapEntry[] = [];
    const seen = new Set<string>();

    // Walk each <loc> so we can also grab the sibling <lastmod> from its block.
    const locRegex = new RegExp(HIREFUL_SITEMAP_LOC_REGEX.source, 'gi');
    let match: RegExpExecArray | null;
    while ((match = locRegex.exec(xml)) !== null) {
      const loc = match[1];
      const vacancyMatch = HIREFUL_VACANCY_URL_REGEX.exec(loc);
      if (!vacancyMatch) continue;
      ...
      const lastmodMatch = HIREFUL_LASTMOD_REGEX.exec(window);
      entries.push({
        vacancyId,
        url: loc,
  ...
  private async processEntry(
    ...
    try {
      ...
      if (status && status >= 400 && status < 500) {
        ...
        this.logger.warn(`Hireful vacancy ${entry.vacancyId} not found (HTTP ${status})`);
        return null;
      }
      throw err;
    }
  ...
  /** Parse a detail page's HTML (JSON-LD JobPosting + og: fallbacks) into a HirefulJob. */
  private parseDetail(html: string, entry: HirefulSitemapEntry): HirefulJob {
    const posting = this.findJobPosting(html);

    const ogTitle = this.firstGroup(html, HIREFUL_OG_TITLE_REGEX);
    const titleTag = this.firstGroup(html, HIREFUL_TITLE_TAG_REGEX);
    const ogDescription = this.firstGroup(html, HIREFUL_OG_DESCRIPTION_REGEX);
    const ogUrl = this.firstGroup(html, HIREFUL_OG_URL_REGEX);
   ...
   */
  private findJobPosting(html: string): HirefulJobPosting | null {
    const re = new RegExp(HIREFUL_JSONLD_REGEX.source, 'gi');
    let match: RegExpExecArray | null;
  ...
  private extractPosting(value: unknown): HirefulJobPosting | null {
    ...
    if (value && typeof value === 'object') {
      ...
      if (this.isJobPostingType(obj['@type'])) return obj as HirefulJobPosting;
  ...
  /** Map a normalised HirefulJob → JobPostDto. */
  private processJob(
    job: HirefulJob,
    tenant: string,
    ...
    const atsId = String(job.vacancyId ?? '');
   ...
   * the canonical `{tenant}.livevacancies.co.uk` host; a `companyUrl` on the
   * `livevacancies.co.uk` domain (or on a known custom careers host) has its
   * origin used verbatim. Returns an empty string when neither yields a host.
   */
  private resolveHost(companySlug: string | undefined, companyUrl: string | undefined): string {
    if (companyUrl) {
      try {
        ...
        if (
          hostname.endsWith(HIREFUL_ROOT_DOMAIN) ||
          HIREFUL_ALT_DOMAINS.some((d) => hostname === d || hostname.endsWith(`.${d}`))
        ) {
    ...
    if (companySlug && companySlug.trim()) {
      ...
      // A caller may also pass a bare host (e.g. "thebigissue.livevacancies.co.uk").
      if (slug.includes(HIREFUL_ROOT_DOMAIN) || HIREFUL_ALT_DOMAINS.some((d) => slug.includes(d))) {
        const host = slug.replace(/^https?:\/\//, '').replace(/\/.*$/, '');
        return `https://${host}`;
      }
      return HIREFUL_HOST_TEMPLATE.replace('{tenant}', encodeURIComponent(slug));
    }
    return '';
  }
   ...
   */
  private extractLocation(job: HirefulJob): LocationDto | null {
    const city = job.city;
  ...
  private detectRemote(
    posting: HirefulJobPosting | null,
    title: string | null,
    ...
    for (const field of haystacks) {
      ...
      if (HIREFUL_REMOTE_REGEX.test(field)) return true;
    }
    return false;
  }
  ...
  private firstAddress(
    jobLocation: HirefulJobLocation | HirefulJobLocation[] | null | undefined,
  ): HirefulPostalAddress | null {
  ...
  private organizationName(
    org: HirefulJobPosting['hiringOrganization'],
  ): string | null {
    if (!org) return null;
