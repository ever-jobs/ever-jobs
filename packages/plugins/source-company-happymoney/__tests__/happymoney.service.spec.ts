...
import { HappyMoneyModule, HappyMoneyService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'happymoney-jobs.json'), 'utf8'),
);
...
/**
 * Spec 619 / T04 — `HappyMoneyService` unit tests (≥ 9 cases).
 */
describe('HappyMoneyService — Spec 619 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through HappyMoneyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [HappyMoneyModule],
      }).compile();
      const service = moduleRef.get(HappyMoneyService);
      expect(service).toBeInstanceOf(HappyMoneyService);
      await moduleRef.close();
    });

    it('exports the Site.HAPPY_MONEY = "happymoney" enum value', () => {
      expect(Site.HAPPY_MONEY).toBe('happymoney');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new HappyMoneyService();
      const result = await service.scrape({
        siteType: [Site.HAPPY_MONEY],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'happymoney-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.HAPPY_MONEY);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Happy Money');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/happymoney/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/happymoney/jobs/');
      ...
      expect(job0?.description).toContain('Happy Money');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/happymoney/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Happy Money');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'happymoney-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
