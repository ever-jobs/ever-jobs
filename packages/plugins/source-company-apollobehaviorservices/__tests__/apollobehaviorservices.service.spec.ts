...
import { ApollobehaviorservicesModule, ApollobehaviorservicesService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'apollobehaviorservices-jobs.json'), 'utf8'),
);
...
/**
 * Spec 287 / T04 — `ApollobehaviorservicesService` unit tests (≥ 9 cases).
 */
describe('ApollobehaviorservicesService — Spec 287 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ApollobehaviorservicesModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ApollobehaviorservicesModule],
      }).compile();
      const service = moduleRef.get(ApollobehaviorservicesService);
      expect(service).toBeInstanceOf(ApollobehaviorservicesService);
      await moduleRef.close();
    });

    it('exports the Site.APOLLOBEHAVIORSERVICES = "apollobehaviorservices" enum value', () => {
      expect(Site.APOLLOBEHAVIORSERVICES).toBe('apollobehaviorservices');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ApollobehaviorservicesService();
      const result = await service.scrape({
        siteType: [Site.APOLLOBEHAVIORSERVICES],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'apollobehaviorservices-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.APOLLOBEHAVIORSERVICES);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Apollo Behavior ');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/apollobehaviorservices/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/apollobehaviorservices/jobs/');
      ...
      expect(job0?.description).toContain('Apollo Behavior ');
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Apollo Behavior ');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'apollobehaviorservices-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
