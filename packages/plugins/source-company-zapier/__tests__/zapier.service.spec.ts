...
import { ZapierModule, ZapierService } from '../src';

const COMPANY_NAME_EXPECT = 'Zapier';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'zapier-jobs.json'), 'utf8'),
);
...
describe('ZapierService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ZapierModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ZapierModule],
      }).compile();
      const service = moduleRef.get(ZapierService);
      expect(service).toBeInstanceOf(ZapierService);
      await moduleRef.close();
    });

    it('exports the Site.ZAPIER = "zapier" enum value', () => {
      expect(Site.ZAPIER).toBe('zapier');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new ZapierService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.ZAPIER],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('zapier');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ZAPIER);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('zapier-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new ZapierService(registry);
      const result = await service.scrape({
        siteType: [Site.ZAPIER],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('zapier');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('zapier-x1');
      expect(result.jobs[0].site).toBe(Site.ZAPIER);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('zapier-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new ZapierService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.ZAPIER],
      } as ScraperInputDto);
