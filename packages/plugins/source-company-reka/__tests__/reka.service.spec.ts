...
import { RekaModule, RekaService } from '../src';

const COMPANY_NAME_EXPECT = 'Reka';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'reka-jobs.json'), 'utf8'),
);
...
describe('RekaService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through RekaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [RekaModule],
      }).compile();
      const service = moduleRef.get(RekaService);
      expect(service).toBeInstanceOf(RekaService);
      await moduleRef.close();
    });

    it('exports the Site.REKA = "reka" enum value', () => {
      expect(Site.REKA).toBe('reka');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new RekaService(registryWithAshby());
      ...
      expect(calledUrls[0]).toContain('reka');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.REKA);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('reka-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new RekaService(registry);
      const result = await service.scrape({
        siteType: [Site.REKA],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('reka');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('reka-x1');
      expect(result.jobs[0].site).toBe(Site.REKA);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('reka-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new RekaService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.REKA],
      } as ScraperInputDto);
