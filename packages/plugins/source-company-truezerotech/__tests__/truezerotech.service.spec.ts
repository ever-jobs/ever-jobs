...
import { TrueZeroTechnologiesModule, TrueZeroTechnologiesService } from '../src';

const COMPANY_NAME_EXPECT = 'True Zero Technologies';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'truezerotech-jobs.json'), 'utf8'),
);
...
describe('TrueZeroTechnologiesService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through TrueZeroTechnologiesModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [TrueZeroTechnologiesModule],
      }).compile();
      const service = moduleRef.get(TrueZeroTechnologiesService);
      expect(service).toBeInstanceOf(TrueZeroTechnologiesService);
      await moduleRef.close();
    });

    it('exports the Site.TRUE_ZERO_TECHNOLOGIES = "truezerotech" enum value', () => {
      expect(Site.TRUE_ZERO_TECHNOLOGIES).toBe('truezerotech');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new TrueZeroTechnologiesService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.TRUE_ZERO_TECHNOLOGIES],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'truezerotech-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.TRUE_ZERO_TECHNOLOGIES);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('truezerotech-' + first.id);
      ...
      expect(calledUrls[0]).toContain('truezerotech');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.TRUE_ZERO_TECHNOLOGIES);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('truezerotech-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new TrueZeroTechnologiesService(registry);
      const result = await service.scrape({
        siteType: [Site.TRUE_ZERO_TECHNOLOGIES],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('truezerotech');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('truezerotech-x1');
      expect(result.jobs[0].site).toBe(Site.TRUE_ZERO_TECHNOLOGIES);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('truezerotech-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new TrueZeroTechnologiesService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.TRUE_ZERO_TECHNOLOGIES],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new TrueZeroTechnologiesService();
      const result = await service.scrape({
        siteType: [Site.TRUE_ZERO_TECHNOLOGIES],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
