...
import { TalanModule, TalanService } from '../src';

const COMPANY_NAME_EXPECT = 'Talan';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'talan-jobs.json'), 'utf8'),
);
...
describe('TalanService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through TalanModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [TalanModule],
      }).compile();
      const service = moduleRef.get(TalanService);
      expect(service).toBeInstanceOf(TalanService);
      await moduleRef.close();
    });

    it('exports the Site.TALAN = "talan" enum value', () => {
      expect(Site.TALAN).toBe('talan');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new TalanService(registryWithSmartRecruiters());
      ...
      expect(calledUrls[0]).toContain('Talan');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.TALAN);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('talan-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new TalanService(registry);
      const result = await service.scrape({
        siteType: [Site.TALAN],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('Talan');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('talan-x1');
      expect(result.jobs[0].site).toBe(Site.TALAN);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('talan-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new TalanService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.TALAN],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new TalanService();
      const result = await service.scrape({
        siteType: [Site.TALAN],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
