...
import { PrimeLendingModule, PrimeLendingService } from '../src';

const COMPANY_NAME_EXPECT = 'PrimeLending';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'primelending-jobs.json'), 'utf8'),
);
...
describe('PrimeLendingService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through PrimeLendingModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [PrimeLendingModule],
      }).compile();
      const service = moduleRef.get(PrimeLendingService);
      expect(service).toBeInstanceOf(PrimeLendingService);
      await moduleRef.close();
    });

    it('exports the Site.PRIMELENDING = "primelending" enum value', () => {
      expect(Site.PRIMELENDING).toBe('primelending');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new PrimeLendingService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.PRIMELENDING],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('PrimeLending');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.PRIMELENDING);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('primelending-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new PrimeLendingService(registry);
      const result = await service.scrape({
        siteType: [Site.PRIMELENDING],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('PrimeLending');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('primelending-x1');
      expect(result.jobs[0].site).toBe(Site.PRIMELENDING);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('primelending-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new PrimeLendingService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.PRIMELENDING],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new PrimeLendingService();
      const result = await service.scrape({
        siteType: [Site.PRIMELENDING],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
