...
import { TwistBioscienceModule, TwistBioscienceService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'twistbioscience-jobs.json'), 'utf8'),
);
...
/**
 * Spec 771 / T04 — `TwistBioscienceService` unit tests (≥ 9 cases).
 */
describe('TwistBioscienceService — Spec 771 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through TwistBioscienceModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [TwistBioscienceModule],
      }).compile();
      const service = moduleRef.get(TwistBioscienceService);
      expect(service).toBeInstanceOf(TwistBioscienceService);
      await moduleRef.close();
    });

    it('exports the Site.TWIST_BIOSCIENCE = "twistbioscience" enum value', () => {
      expect(Site.TWIST_BIOSCIENCE).toBe('twistbioscience');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new TwistBioscienceService();
      const result = await service.scrape({
        siteType: [Site.TWIST_BIOSCIENCE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'twistbioscience-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.TWIST_BIOSCIENCE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Twist Bioscience');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/twistbioscience/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/twistbioscience/jobs/');
      ...
      expect(job0?.description).toContain('Twist Bioscience');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/twistbioscience/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Twist Bioscience');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'twistbioscience-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
