...
import { SpliceModule, SpliceService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'splice-jobs.json'), 'utf8'),
);
...
/**
 * Spec 940 / T04 — `SpliceService` unit tests (≥ 9 cases).
 */
describe('SpliceService — Spec 940 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SpliceModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SpliceModule],
      }).compile();
      const service = moduleRef.get(SpliceService);
      expect(service).toBeInstanceOf(SpliceService);
      await moduleRef.close();
    });

    it('exports the Site.SPLICE = "splice" enum value', () => {
      expect(Site.SPLICE).toBe('splice');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new SpliceService();
      const result = await service.scrape({
        siteType: [Site.SPLICE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'splice-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.SPLICE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Splice');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/splice/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/splice/jobs/');
      ...
      expect(job0?.description).toContain('Splice');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/splice/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Splice');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'splice-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
