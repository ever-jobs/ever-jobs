...
import { LedgyModule, LedgyService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'ledgy-jobs.json'), 'utf8'),
);
...
/**
 * Spec 655 / T04 — `LedgyService` unit tests (≥ 9 cases).
 */
describe('LedgyService — Spec 655 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through LedgyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [LedgyModule],
      }).compile();
      const service = moduleRef.get(LedgyService);
      expect(service).toBeInstanceOf(LedgyService);
      await moduleRef.close();
    });

    it('exports the Site.LEDGY = "ledgy" enum value', () => {
      expect(Site.LEDGY).toBe('ledgy');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new LedgyService();
      ...
      const job0 = dto.jobs.find((j) => j.id === 'ledgy-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.LEDGY);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Ledgy');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/ledgy/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/ledgy/jobs/');
      ...
      expect(job0?.description).toContain('Ledgy');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/ledgy/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Ledgy');
      }
    });
  });
