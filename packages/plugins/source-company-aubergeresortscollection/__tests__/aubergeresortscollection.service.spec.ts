...
import { AubergeResortsCollectionModule, AubergeResortsCollectionService } from '../src';

const COMPANY_NAME_EXPECT = 'Auberge Resorts Collection';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'aubergeresortscollection-jobs.json'), 'utf8'),
);
...
describe('AubergeResortsCollectionService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AubergeResortsCollectionModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AubergeResortsCollectionModule],
      }).compile();
      const service = moduleRef.get(AubergeResortsCollectionService);
      expect(service).toBeInstanceOf(AubergeResortsCollectionService);
      await moduleRef.close();
    });

    it('exports the Site.AUBERGE_RESORTS_COLLECTION = "aubergeresortscollection" enum value', () => {
      expect(Site.AUBERGE_RESORTS_COLLECTION).toBe('aubergeresortscollection');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new AubergeResortsCollectionService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.AUBERGE_RESORTS_COLLECTION],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'aubergeresortscollection-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.AUBERGE_RESORTS_COLLECTION);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('aubergeresortscollection-' + first.id);
      ...
      expect(calledUrls[0]).toContain('AubergeCollection');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.AUBERGE_RESORTS_COLLECTION);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('aubergeresortscollection-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new AubergeResortsCollectionService(registry);
      const result = await service.scrape({
        siteType: [Site.AUBERGE_RESORTS_COLLECTION],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('AubergeCollection');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('aubergeresortscollection-x1');
      expect(result.jobs[0].site).toBe(Site.AUBERGE_RESORTS_COLLECTION);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('aubergeresortscollection-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new AubergeResortsCollectionService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.AUBERGE_RESORTS_COLLECTION],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new AubergeResortsCollectionService();
      const result = await service.scrape({
        siteType: [Site.AUBERGE_RESORTS_COLLECTION],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
