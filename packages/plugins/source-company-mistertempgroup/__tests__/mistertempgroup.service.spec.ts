...
import { MistertempGroupModule, MistertempGroupService } from '../src';

const COMPANY_NAME_EXPECT = 'Mistertemp\' Group';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'mistertempgroup-jobs.json'), 'utf8'),
);
...
describe('MistertempGroupService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through MistertempGroupModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [MistertempGroupModule],
      }).compile();
      const service = moduleRef.get(MistertempGroupService);
      expect(service).toBeInstanceOf(MistertempGroupService);
      await moduleRef.close();
    });

    it('exports the Site.MISTERTEMP_GROUP = "mistertempgroup" enum value', () => {
      expect(Site.MISTERTEMP_GROUP).toBe('mistertempgroup');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new MistertempGroupService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.MISTERTEMP_GROUP],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'mistertempgroup-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Recruitee's defaults
      expect(job0?.site).toBe(Site.MISTERTEMP_GROUP);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('mistertempgroup-' + first.id);
      ...
      expect(calledUrls[0]).toContain('jobsmistertemp.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.MISTERTEMP_GROUP);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('mistertempgroup-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new MistertempGroupService(registry);
      const result = await service.scrape({
        siteType: [Site.MISTERTEMP_GROUP],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('jobsmistertemp');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('mistertempgroup-x1');
      expect(result.jobs[0].site).toBe(Site.MISTERTEMP_GROUP);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('mistertempgroup-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new MistertempGroupService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.MISTERTEMP_GROUP],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new MistertempGroupService();
      const result = await service.scrape({
        siteType: [Site.MISTERTEMP_GROUP],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
