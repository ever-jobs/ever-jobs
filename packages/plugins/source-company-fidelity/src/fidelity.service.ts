...
/**
 * Fidelity Investments — Financial services (HQ: Boston, MA, USA).
 *
 * Source (Spec 1736): Workday board, scraped in this order:
 *   - `fmr:1:FidelityCareers` — https://fmr.wd1.myworkdayjobs.com/FidelityCareers
 *     verified live 2026-09-24: 850 open postings.
 *
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'fmr:1:FidelityCareers', atsIdPrefix: 'wd-fmr-' },
];
...
@SourcePlugin({
  ...
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['fidelity.com'],
  description: 'Fidelity Investments careers via Workday. Tags: segment=workday-enterprise; industry=financial-services.',
})
@Injectable()
export class FidelityService implements IScraper {
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Workday source plugin is not registered; cannot scrape Fidelity Investments');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`Fidelity Investments: delegating to Workday (${board.companySlug})`);
    ...
    this.logger.log(`Fidelity Investments: scraped ${jobs.length} jobs`);
