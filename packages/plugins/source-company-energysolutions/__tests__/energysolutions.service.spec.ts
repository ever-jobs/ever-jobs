...
import { EnergySolutionsModule, EnergySolutionsService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'energysolutions-jobs.json'), 'utf8'),
);
...
/**
 * Spec 842 / T04 — `EnergySolutionsService` unit tests (≥ 9 cases).
 */
describe('EnergySolutionsService — Spec 842 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through EnergySolutionsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [EnergySolutionsModule],
      }).compile();
      const service = moduleRef.get(EnergySolutionsService);
      expect(service).toBeInstanceOf(EnergySolutionsService);
      await moduleRef.close();
    });

    it('exports the Site.ENERGY_SOLUTIONS = "energysolutions" enum value', () => {
      expect(Site.ENERGY_SOLUTIONS).toBe('energysolutions');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new EnergySolutionsService();
      const result = await service.scrape({
        siteType: [Site.ENERGY_SOLUTIONS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'energysolutions-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ENERGY_SOLUTIONS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Energy Solutions');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/energysolutions/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/energysolutions/jobs/');
      ...
      expect(job0?.description).toContain('Energy Solutions');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/energysolutions/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Energy Solutions');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'energysolutions-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
