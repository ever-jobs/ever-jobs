...
import { ABInBevModule, ABInBevService } from '../src';

const COMPANY_NAME_EXPECT = 'AB InBev';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'abinbev-jobs.json'), 'utf8'),
);
...
describe('ABInBevService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ABInBevModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ABInBevModule],
      }).compile();
      const service = moduleRef.get(ABInBevService);
      expect(service).toBeInstanceOf(ABInBevService);
      await moduleRef.close();
    });

    it('exports the Site.AB_INBEV = "abinbev" enum value', () => {
      expect(Site.AB_INBEV).toBe('abinbev');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new ABInBevService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.AB_INBEV],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('ABInBev1');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.AB_INBEV);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('abinbev-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new ABInBevService(registry);
      const result = await service.scrape({
        siteType: [Site.AB_INBEV],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('ABInBev1');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('abinbev-x1');
      expect(result.jobs[0].site).toBe(Site.AB_INBEV);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('abinbev-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new ABInBevService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.AB_INBEV],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new ABInBevService();
      const result = await service.scrape({
        siteType: [Site.AB_INBEV],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
