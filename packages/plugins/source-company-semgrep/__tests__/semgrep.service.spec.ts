...
import { SemgrepModule, SemgrepService } from '../src';

const COMPANY_NAME_EXPECT = 'Semgrep';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'semgrep-jobs.json'), 'utf8'),
);
...
describe('SemgrepService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SemgrepModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SemgrepModule],
      }).compile();
      const service = moduleRef.get(SemgrepService);
      expect(service).toBeInstanceOf(SemgrepService);
      await moduleRef.close();
    });

    it('exports the Site.SEMGREP = "semgrep" enum value', () => {
      expect(Site.SEMGREP).toBe('semgrep');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new SemgrepService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.SEMGREP],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('semgrep');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SEMGREP);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('semgrep-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new SemgrepService(registry);
      const result = await service.scrape({
        siteType: [Site.SEMGREP],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('semgrep');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('semgrep-x1');
      expect(result.jobs[0].site).toBe(Site.SEMGREP);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('semgrep-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new SemgrepService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.SEMGREP],
      } as ScraperInputDto);
