...
import { HungryrootModule, HungryrootService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'hungryroot-jobs.json'), 'utf8'),
);
...
/**
 * Spec 504 / T04 — `HungryrootService` unit tests (≥ 9 cases).
 */
describe('HungryrootService — Spec 504 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through HungryrootModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [HungryrootModule],
      }).compile();
      const service = moduleRef.get(HungryrootService);
      expect(service).toBeInstanceOf(HungryrootService);
      await moduleRef.close();
    });

    it('exports the Site.HUNGRYROOT = "hungryroot" enum value', () => {
      expect(Site.HUNGRYROOT).toBe('hungryroot');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new HungryrootService();
      const result = await service.scrape({
        siteType: [Site.HUNGRYROOT],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'hungryroot-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.HUNGRYROOT);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Hungryroot');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/hungryroot/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/hungryroot/jobs/');
      ...
      expect(job0?.description).toContain('Hungryroot');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/hungryroot/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Hungryroot');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'hungryroot-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
