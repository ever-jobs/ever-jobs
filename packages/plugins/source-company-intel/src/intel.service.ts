...
/**
 * Intel — Semiconductors (HQ: Santa Clara, CA, USA).
 *
 * Source (Spec 1736): Workday board, scraped in this order:
 *   - `intel:1:External` — https://intel.wd1.myworkdayjobs.com/External
 *     verified live 2026-09-24: 614 open postings.
 *
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'intel:1:External', atsIdPrefix: 'wd-intel-' },
];
...
@SourcePlugin({
  ...
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['intel.com'],
  description: 'Intel careers via Workday. Tags: segment=workday-enterprise; industry=semiconductors.',
})
@Injectable()
export class IntelService implements IScraper {
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Workday source plugin is not registered; cannot scrape Intel');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`Intel: delegating to Workday (${board.companySlug})`);
    ...
    this.logger.log(`Intel: scraped ${jobs.length} jobs`);
