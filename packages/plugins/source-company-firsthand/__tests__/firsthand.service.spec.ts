...
import { FirsthandHealthModule, FirsthandHealthService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'firsthand-jobs.json'), 'utf8'),
);
...
/**
 * Spec 667 / T04 — `FirsthandHealthService` unit tests (≥ 9 cases).
 */
describe('FirsthandHealthService — Spec 667 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through FirsthandHealthModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [FirsthandHealthModule],
      }).compile();
      const service = moduleRef.get(FirsthandHealthService);
      expect(service).toBeInstanceOf(FirsthandHealthService);
      await moduleRef.close();
    });

    it('exports the Site.FIRSTHAND_HEALTH = "firsthand" enum value', () => {
      expect(Site.FIRSTHAND_HEALTH).toBe('firsthand');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new FirsthandHealthService();
      const result = await service.scrape({
        siteType: [Site.FIRSTHAND_HEALTH],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'firsthand-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.FIRSTHAND_HEALTH);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('firsthand Health');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/firsthand/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/firsthand/jobs/');
      ...
      expect(job0?.description).toContain('firsthand Health');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/firsthand/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('firsthand Health');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'firsthand-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
