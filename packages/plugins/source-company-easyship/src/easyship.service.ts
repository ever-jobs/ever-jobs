...
/**
 * Easyship — Easyship is an employer in the E sector, headquartered in New York, New York, USA..
 *
 * Easyship is a company operating in E-commerce enablement
 * (shipping/fulfillment SaaS), headquartered in New York, New York,
 * USA. This source plugin ingests live open roles published on
 * Easyship's official Greenhouse-hosted careers board via the public
 ...
 *
 * Sector: E-commerce enablement (shipping/fulfillment SaaS). HQ: New York, New York, USA.
 *
 * Highlights:
 *   - Sector: E-commerce enablement (shipping/fulfillment SaaS)
 ...
 *     Jobs schema
 *
 * Source profile (Spec 838):
 *   - D-04 — Greenhouse canonical hosted-board host (variant 2):
 *     `https://job-boards.greenhouse.io/easyship/jobs/<id>`.
 *   - D-08 — entity-decode-then-tag-strip description pipeline.
 *   - D-09 — wire `company_name` pass-through (`'Easyship'`).
 ...
 */
const API_URL = 'https://api.greenhouse.io/v1/boards/easyship/jobs';
...
@Injectable()
export class EasyshipService implements IScraper {
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    try {
      ...
      this.logger.log(`Easyship: fetching ${url}`);
      ...
      for (const listing of listings) {
        ...
        jobs.push(
          new JobPostDto({
            ...
            companyName: listing.company_name ?? 'Easyship',
            // D-04: wire `absolute_url` flows through (variant 2).
            jobUrl:
              listing.absolute_url ??
              `https://job-boards.greenhouse.io/easyship/jobs/${listing.id}`,
            location,
      ...
      this.logger.log(`Easyship: scraped ${jobs.length} jobs`);
    } catch (err: any) {
      this.logger.error(`Easyship scrape failed: ${err.message}`);
