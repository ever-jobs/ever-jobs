...
import { EasyshipModule, EasyshipService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'easyship-jobs.json'), 'utf8'),
);
...
/**
 * Spec 838 / T04 — `EasyshipService` unit tests (≥ 9 cases).
 */
describe('EasyshipService — Spec 838 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through EasyshipModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [EasyshipModule],
      }).compile();
      const service = moduleRef.get(EasyshipService);
      expect(service).toBeInstanceOf(EasyshipService);
      await moduleRef.close();
    });

    it('exports the Site.EASYSHIP = "easyship" enum value', () => {
      expect(Site.EASYSHIP).toBe('easyship');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new EasyshipService();
      const result = await service.scrape({
        siteType: [Site.EASYSHIP],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'easyship-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.EASYSHIP);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Easyship');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/easyship/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/easyship/jobs/');
      ...
      expect(job0?.description).toContain('Easyship');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/easyship/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Easyship');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'easyship-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
