...
import { UpstartModule, UpstartService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'upstart-jobs.json'), 'utf8'),
);
...
/**
 * Spec 509 / T04 — `UpstartService` unit tests (≥ 9 cases).
 */
describe('UpstartService — Spec 509 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through UpstartModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [UpstartModule],
      }).compile();
      const service = moduleRef.get(UpstartService);
      expect(service).toBeInstanceOf(UpstartService);
      await moduleRef.close();
    });

    it('exports the Site.UPSTART = "upstart" enum value', () => {
      expect(Site.UPSTART).toBe('upstart');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new UpstartService();
      const result = await service.scrape({
        siteType: [Site.UPSTART],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'upstart-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.UPSTART);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Upstart');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/upstart/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/upstart/jobs/');
      ...
      expect(job0?.description).toContain('Upstart');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/upstart/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Upstart');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'upstart-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
