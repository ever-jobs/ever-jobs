...
/**
 * Schonfeld — Hedge fund (HQ: New York, NY, USA).
 *
 * Source (Spec 1737): Greenhouse board, scraped in this order:
 *   - `schonfeld` — https://job-boards.greenhouse.io/schonfeld
 *     verified live 2026-09-24: 79 open postings.
 *
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'schonfeld', atsIdPrefix: 'gh-' },
];
...
@SourcePlugin({
  ...
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['schonfeld.com'],
  description: 'Schonfeld careers via Greenhouse. Tags: segment=quant-trading; industry=hedge-fund.',
})
@Injectable()
export class SchonfeldService implements IScraper {
  private readonly logger = new Logger(SchonfeldService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Greenhouse source plugin is not registered; cannot scrape Schonfeld');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`Schonfeld: delegating to Greenhouse (${board.companySlug})`);
    ...
    this.logger.log(`Schonfeld: scraped ${jobs.length} jobs`);
