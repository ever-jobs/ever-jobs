...
import { SchonfeldModule, SchonfeldService } from '../src';
...
const FIXTURE: Fixture = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'fixtures', 'schonfeld-boards.json'), 'utf8'),
);
const COMPANY_NAME = 'Schonfeld';
const ID_PREFIX = 'schonfeld-';
...
describe('SchonfeldService — Greenhouse delegation (Spec 1737)', () => {
  beforeEach(() => {
    mockGet.mockReset();
    mockPost.mockReset();
  ...
  describe('registration', () => {
    it('resolves through SchonfeldModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({ imports: [SchonfeldModule] }).compile();
      expect(moduleRef.get(SchonfeldService)).toBeInstanceOf(SchonfeldService);
      await moduleRef.close();
    });

    it('exports Site.SCHONFELD = "schonfeld"', () => {
      expect(Site.SCHONFELD).toBe('schonfeld');
    });

    it('declares a tagged company plugin with its domains', () => {
      const meta = Reflect.getMetadata(SOURCE_PLUGIN_METADATA, SchonfeldService);
      expect(meta.site).toBe(Site.SCHONFELD);
      expect(meta.category).toBe('company');
      expect(meta.isAts).toBeFalsy();
      expect(meta.companyDomains).toEqual(['schonfeld.com']);
  ...
  describe('recorded board (real Greenhouse adapter, mocked HTTP)', () => {
    it('maps every recorded posting and re-stamps the company identity', async () => {
      const service = new SchonfeldService(registryWith());
      const result = await service.scrape({ siteType: [Site.SCHONFELD], resultsWanted: 100 } as ScraperInputDto);
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SCHONFELD);
    ...
    it('only requests the recorded board URLs', async () => {
      const service = new SchonfeldService(registryWith());
      await service.scrape({ siteType: [Site.SCHONFELD] } as ScraperInputDto);
    ...
    it('honours resultsWanted=1', async () => {
      const service = new SchonfeldService(registryWith());
      const result = await service.scrape({ siteType: [Site.SCHONFELD], resultsWanted: 1 } as ScraperInputDto);
    ...
    it('resolves with an empty result when the board is gone (HTTP 404)', async () => {
      ...
      const service = new SchonfeldService(registryWith());
      const result = await service.scrape({ siteType: [Site.SCHONFELD] } as ScraperInputDto);
  ...
  describe('delegation contract', () => {
    it('forwards the board and the caller input untouched', async () => {
      ...
      const service = new SchonfeldService(
        registryWith(
          fakeBackend(
      ...
      expect(result.jobs[0].site).toBe(Site.SCHONFELD);
    ...
    it('never forwards the caller\'s credentials to the board (Spec 1735 §4.5)', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new SchonfeldService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
    ...
    it('makes no request when resultsWanted is 0', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new SchonfeldService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      const result = await service.scrape({ siteType: [Site.SCHONFELD], resultsWanted: 0 } as ScraperInputDto);
  ...
  describe('resilience', () => {
    it('reports not_registered when no registry is injected', async () => {
      const result = await new SchonfeldService().scrape({ siteType: [Site.SCHONFELD] } as ScraperInputDto);
      expect(result.jobs).toEqual([]);
    ...
    it('reports not_registered when Greenhouse is missing from the registry', async () => {
      const result = await new SchonfeldService(new PluginRegistry()).scrape({
        siteType: [Site.SCHONFELD],
      } as ScraperInputDto);
      expect(result.diagnostics?.reason).toBe('not_registered');
    });
