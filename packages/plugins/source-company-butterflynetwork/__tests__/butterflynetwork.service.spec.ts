...
import { ButterflyNetworkModule, ButterflyNetworkService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'butterflynetwork-jobs.json'), 'utf8'),
);
...
/**
 * Spec 742 / T04 — `ButterflyNetworkService` unit tests (≥ 9 cases).
 */
describe('ButterflyNetworkService — Spec 742 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ButterflyNetworkModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ButterflyNetworkModule],
      }).compile();
      const service = moduleRef.get(ButterflyNetworkService);
      expect(service).toBeInstanceOf(ButterflyNetworkService);
      await moduleRef.close();
    });

    it('exports the Site.BUTTERFLY_NETWORK = "butterflynetwork" enum value', () => {
      expect(Site.BUTTERFLY_NETWORK).toBe('butterflynetwork');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ButterflyNetworkService();
      const result = await service.scrape({
        siteType: [Site.BUTTERFLY_NETWORK],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'butterflynetwork-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.BUTTERFLY_NETWORK);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Butterfly Network');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/butterflynetwork/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/butterflynetwork/jobs/');
      ...
      expect(job0?.description).toContain('Butterfly Network');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/butterflynetwork/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Butterfly Network');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'butterflynetwork-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
