...
/**
 * Pfizer — Pharmaceuticals (HQ: New York, NY, USA).
 *
 * Source (Spec 1736): Workday board, scraped in this order:
 *   - `pfizer:1:PfizerCareers` — https://pfizer.wd1.myworkdayjobs.com/PfizerCareers
 *     verified live 2026-09-24: 564 open postings.
 *
 ...
 * through untouched.
 *
 * Tags: segment=workday-enterprise; industry=pharmaceuticals.
 */
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'pfizer:1:PfizerCareers', atsIdPrefix: 'wd-pfizer-' },
];
...
@SourcePlugin({
  ...
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['pfizer.com'],
  description: 'Pfizer careers via Workday. Tags: segment=workday-enterprise; industry=pharmaceuticals.',
})
@Injectable()
export class PfizerService implements IScraper {
  private readonly logger = new Logger(PfizerService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Workday source plugin is not registered; cannot scrape Pfizer');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`Pfizer: delegating to Workday (${board.companySlug})`);
    ...
    this.logger.log(`Pfizer: scraped ${jobs.length} jobs`);
