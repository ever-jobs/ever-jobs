...
import { TempoModule, TempoService } from '../src';

const COMPANY_NAME_EXPECT = 'Tempo';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'tempo-jobs.json'), 'utf8'),
);
...
describe('TempoService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through TempoModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [TempoModule],
      }).compile();
      const service = moduleRef.get(TempoService);
      expect(service).toBeInstanceOf(TempoService);
      await moduleRef.close();
    });

    it('exports the Site.TEMPO = "tempo" enum value', () => {
      expect(Site.TEMPO).toBe('tempo');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new TempoService(registryWithAshby());
      ...
      expect(calledUrls[0]).toContain('tempo-xyz');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.TEMPO);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('tempo-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new TempoService(registry);
      const result = await service.scrape({
        siteType: [Site.TEMPO],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('tempo-xyz');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('tempo-x1');
      expect(result.jobs[0].site).toBe(Site.TEMPO);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('tempo-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new TempoService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.TEMPO],
      } as ScraperInputDto);
