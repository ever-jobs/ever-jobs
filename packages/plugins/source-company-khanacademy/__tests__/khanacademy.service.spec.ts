...
import { KhanAcademyModule, KhanAcademyService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'khanacademy-jobs.json'), 'utf8'),
);
...
/**
 * Spec 746 / T04 — `KhanAcademyService` unit tests (≥ 9 cases).
 */
describe('KhanAcademyService — Spec 746 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through KhanAcademyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [KhanAcademyModule],
      }).compile();
      const service = moduleRef.get(KhanAcademyService);
      expect(service).toBeInstanceOf(KhanAcademyService);
      await moduleRef.close();
    });

    it('exports the Site.KHAN_ACADEMY = "khanacademy" enum value', () => {
      expect(Site.KHAN_ACADEMY).toBe('khanacademy');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new KhanAcademyService();
      const result = await service.scrape({
        siteType: [Site.KHAN_ACADEMY],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'khanacademy-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.KHAN_ACADEMY);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Khan Academy');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/khanacademy/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/khanacademy/jobs/');
      ...
      expect(job0?.description).toContain('Khan Academy');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/khanacademy/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Khan Academy');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'khanacademy-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
