...
import { VerveGroupModule, VerveGroupService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'verve-jobs.json'), 'utf8'),
);
...
/**
 * Spec 695 / T04 — `VerveGroupService` unit tests (≥ 9 cases).
 */
describe('VerveGroupService — Spec 695 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through VerveGroupModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [VerveGroupModule],
      }).compile();
      const service = moduleRef.get(VerveGroupService);
      expect(service).toBeInstanceOf(VerveGroupService);
      await moduleRef.close();
    });

    it('exports the Site.VERVE_GROUP = "verve" enum value', () => {
      expect(Site.VERVE_GROUP).toBe('verve');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new VerveGroupService();
      const result = await service.scrape({
        siteType: [Site.VERVE_GROUP],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'verve-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.VERVE_GROUP);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Verve Group');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/verve/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/verve/jobs/');
      ...
      expect(job0?.description).toContain('Verve Group');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/verve/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Verve Group');
      }
    });
  });
