...
import { ArcInstituteModule, ArcInstituteService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'arcinstitute-jobs.json'), 'utf8'),
);
...
/**
 * Spec 724 / T04 — `ArcInstituteService` unit tests (≥ 9 cases).
 */
describe('ArcInstituteService — Spec 724 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ArcInstituteModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ArcInstituteModule],
      }).compile();
      const service = moduleRef.get(ArcInstituteService);
      expect(service).toBeInstanceOf(ArcInstituteService);
      await moduleRef.close();
    });

    it('exports the Site.ARC_INSTITUTE = "arcinstitute" enum value', () => {
      expect(Site.ARC_INSTITUTE).toBe('arcinstitute');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ArcInstituteService();
      const result = await service.scrape({
        siteType: [Site.ARC_INSTITUTE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'arcinstitute-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ARC_INSTITUTE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Arc Institute');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/arcinstitute/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/arcinstitute/jobs/');
      ...
      expect(job0?.description).toContain('Arc Institute');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/arcinstitute/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Arc Institute');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'arcinstitute-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
