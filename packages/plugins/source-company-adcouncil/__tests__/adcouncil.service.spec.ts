...
import { AdcouncilModule, AdcouncilService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'adcouncil-jobs.json'), 'utf8'),
);
...
/**
 * Spec 189 / T04 — `AdcouncilService` unit tests (≥ 9 cases).
 */
describe('AdcouncilService — Spec 189 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AdcouncilModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AdcouncilModule],
      }).compile();
      const service = moduleRef.get(AdcouncilService);
      expect(service).toBeInstanceOf(AdcouncilService);
      await moduleRef.close();
    });

    it('exports the Site.ADCOUNCIL = "adcouncil" enum value', () => {
      expect(Site.ADCOUNCIL).toBe('adcouncil');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AdcouncilService();
      const result = await service.scrape({
        siteType: [Site.ADCOUNCIL],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'adcouncil-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ADCOUNCIL);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('the Ad Council');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/adcouncil/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/adcouncil/jobs/');
      ...
      expect(job0?.description).toContain('the Ad Council');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/adcouncil/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('the Ad Council');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'adcouncil-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
