...
import { GoodAmericanModule, GoodAmericanService } from '../src';

const COMPANY_NAME_EXPECT = 'Good American';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'goodamerican-jobs.json'), 'utf8'),
);
...
describe('GoodAmericanService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through GoodAmericanModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [GoodAmericanModule],
      }).compile();
      const service = moduleRef.get(GoodAmericanService);
      expect(service).toBeInstanceOf(GoodAmericanService);
      await moduleRef.close();
    });

    it('exports the Site.GOOD_AMERICAN = "goodamerican" enum value', () => {
      expect(Site.GOOD_AMERICAN).toBe('goodamerican');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new GoodAmericanService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.GOOD_AMERICAN],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'goodamerican-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.GOOD_AMERICAN);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('goodamerican-' + first.id);
      ...
      expect(calledUrls[0]).toContain('goodamerican');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.GOOD_AMERICAN);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('goodamerican-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new GoodAmericanService(registry);
      const result = await service.scrape({
        siteType: [Site.GOOD_AMERICAN],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('goodamerican');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('goodamerican-x1');
      expect(result.jobs[0].site).toBe(Site.GOOD_AMERICAN);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('goodamerican-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new GoodAmericanService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.GOOD_AMERICAN],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new GoodAmericanService();
      const result = await service.scrape({
        siteType: [Site.GOOD_AMERICAN],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
