...
import { RenderModule, RenderService } from '../src';

const COMPANY_NAME_EXPECT = 'Render';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'render-jobs.json'), 'utf8'),
);
...
describe('RenderService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through RenderModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [RenderModule],
      }).compile();
      const service = moduleRef.get(RenderService);
      expect(service).toBeInstanceOf(RenderService);
      await moduleRef.close();
    });

    it('exports the Site.RENDER = "render" enum value', () => {
      expect(Site.RENDER).toBe('render');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new RenderService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.RENDER],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('render');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.RENDER);
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new RenderService(registry);
      const result = await service.scrape({
        siteType: [Site.RENDER],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('render');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('render-x1');
      expect(result.jobs[0].site).toBe(Site.RENDER);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('render-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new RenderService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.RENDER],
      } as ScraperInputDto);
