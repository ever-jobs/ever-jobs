...
import { HighnoteModule, HighnoteService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'highnote-jobs.json'), 'utf8'),
);
...
/**
 * Spec 475 / T04 — `HighnoteService` unit tests (≥ 9 cases).
 */
describe('HighnoteService — Spec 475 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through HighnoteModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [HighnoteModule],
      }).compile();
      const service = moduleRef.get(HighnoteService);
      expect(service).toBeInstanceOf(HighnoteService);
      await moduleRef.close();
    });

    it('exports the Site.HIGHNOTE = "highnote" enum value', () => {
      expect(Site.HIGHNOTE).toBe('highnote');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new HighnoteService();
      const result = await service.scrape({
        siteType: [Site.HIGHNOTE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'highnote-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.HIGHNOTE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Highnote');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/highnote/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/highnote/jobs/');
      ...
      expect(job0?.description).toContain('Highnote');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/highnote/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Highnote');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'highnote-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
