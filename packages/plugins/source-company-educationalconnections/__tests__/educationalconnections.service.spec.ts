...
import { EducationalConnectionsModule, EducationalConnectionsService } from '../src';

const COMPANY_NAME_EXPECT = 'Educational Connections';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'educationalconnections-jobs.json'), 'utf8'),
);
...
describe('EducationalConnectionsService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through EducationalConnectionsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [EducationalConnectionsModule],
      }).compile();
      const service = moduleRef.get(EducationalConnectionsService);
      expect(service).toBeInstanceOf(EducationalConnectionsService);
      await moduleRef.close();
    });

    it('exports the Site.EDUCATIONAL_CONNECTIONS = "educationalconnections" enum value', () => {
      expect(Site.EDUCATIONAL_CONNECTIONS).toBe('educationalconnections');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new EducationalConnectionsService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.EDUCATIONAL_CONNECTIONS],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'educationalconnections-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.EDUCATIONAL_CONNECTIONS);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('educationalconnections-' + first.id);
      ...
      expect(calledUrls[0]).toContain('EducationalConnections');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.EDUCATIONAL_CONNECTIONS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('educationalconnections-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new EducationalConnectionsService(registry);
      const result = await service.scrape({
        siteType: [Site.EDUCATIONAL_CONNECTIONS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('EducationalConnections');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('educationalconnections-x1');
      expect(result.jobs[0].site).toBe(Site.EDUCATIONAL_CONNECTIONS);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('educationalconnections-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new EducationalConnectionsService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.EDUCATIONAL_CONNECTIONS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new EducationalConnectionsService();
      const result = await service.scrape({
        siteType: [Site.EDUCATIONAL_CONNECTIONS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
