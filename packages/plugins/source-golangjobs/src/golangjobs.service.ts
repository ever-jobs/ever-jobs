...
@SourcePlugin({
  site: Site.GOLANGJOBS,
  name: 'GolangJobs',
  category: 'niche',
})
@Injectable()
export class GolangJobsService implements IScraper {
  private readonly logger = new Logger(GolangJobsService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    client.setHeaders(GOLANGJOBS_HEADERS);

    this.logger.log(`Fetching Golang Jobs RSS feed (resultsWanted=${resultsWanted})`);

    try {
      const response = await client.get(GOLANGJOBS_RSS_URL);
      const xml = response.data;

      if (!xml || typeof xml !== 'string') {
        this.logger.warn('Empty or invalid RSS response from Golang Jobs');
        return new JobResponseDto([]);
      }

      const items = this.parseRssItems(xml);
      this.logger.log(`Parsed ${items.length} items from Golang Jobs RSS feed`);
      ...
      for (const item of items) {
        ...
        try {
          ...
          this.logger.warn(`Error mapping Golang Jobs job ${item.link}: ${err.message}`);
        }
      }

      this.logger.log(`Golang Jobs returned ${jobs.length} jobs`);
      return new JobResponseDto(jobs);
    } catch (err: any) {
      this.logger.error(`Golang Jobs scrape error: ${err.message}`);
      return new JobResponseDto([], classifyScrapeError(err));
    }
  }

  /**
   * Parse RSS XML into an array of GolangJobsRssItem using regex (no XML library).
   */
  private parseRssItems(xml: string): GolangJobsRssItem[] {
    const items: GolangJobsRssItem[] = [];
   ...
   */
  private matchesSearch(item: GolangJobsRssItem, searchTerm: string): boolean {
