...
import { WaymarkHealthModule, WaymarkHealthService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'waymark-jobs.json'), 'utf8'),
);
...
/**
 * Spec 678 / T04 — `WaymarkHealthService` unit tests (≥ 9 cases).
 */
describe('WaymarkHealthService — Spec 678 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through WaymarkHealthModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [WaymarkHealthModule],
      }).compile();
      const service = moduleRef.get(WaymarkHealthService);
      expect(service).toBeInstanceOf(WaymarkHealthService);
      await moduleRef.close();
    });

    it('exports the Site.WAYMARK_HEALTH = "waymark" enum value', () => {
      expect(Site.WAYMARK_HEALTH).toBe('waymark');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new WaymarkHealthService();
      const result = await service.scrape({
        siteType: [Site.WAYMARK_HEALTH],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'waymark-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.WAYMARK_HEALTH);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Waymark Health');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/waymark/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/waymark/jobs/');
      ...
      expect(job0?.description).toContain('Waymark Health');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/waymark/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Waymark Health');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'waymark-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
