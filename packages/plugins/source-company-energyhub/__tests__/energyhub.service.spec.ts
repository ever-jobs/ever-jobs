...
import { EnergyHubModule, EnergyHubService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'energyhub-jobs.json'), 'utf8'),
);
...
/**
 * Spec 622 / T04 — `EnergyHubService` unit tests (≥ 9 cases).
 */
describe('EnergyHubService — Spec 622 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through EnergyHubModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [EnergyHubModule],
      }).compile();
      const service = moduleRef.get(EnergyHubService);
      expect(service).toBeInstanceOf(EnergyHubService);
      await moduleRef.close();
    });

    it('exports the Site.ENERGYHUB = "energyhub" enum value', () => {
      expect(Site.ENERGYHUB).toBe('energyhub');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new EnergyHubService();
      const result = await service.scrape({
        siteType: [Site.ENERGYHUB],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'energyhub-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ENERGYHUB);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('EnergyHub');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/energyhub/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/energyhub/jobs/');
      ...
      expect(job0?.description).toContain('EnergyHub');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/energyhub/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('EnergyHub');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'energyhub-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
