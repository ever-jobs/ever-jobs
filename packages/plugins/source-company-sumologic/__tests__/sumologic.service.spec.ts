...
import { SumoLogicModule, SumoLogicService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'sumologic-jobs.json'), 'utf8'),
);
...
/**
 * Spec 946 / T04 — `SumoLogicService` unit tests (≥ 9 cases).
 */
describe('SumoLogicService — Spec 946 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SumoLogicModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SumoLogicModule],
      }).compile();
      const service = moduleRef.get(SumoLogicService);
      expect(service).toBeInstanceOf(SumoLogicService);
      await moduleRef.close();
    });

    it('exports the Site.SUMO_LOGIC = "sumologic" enum value', () => {
      expect(Site.SUMO_LOGIC).toBe('sumologic');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new SumoLogicService();
      const result = await service.scrape({
        siteType: [Site.SUMO_LOGIC],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'sumologic-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.SUMO_LOGIC);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Sumo Logic');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/sumologic/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/sumologic/jobs/');
      ...
      expect(job0?.description).toContain('Sumo Logic');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/sumologic/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Sumo Logic');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'sumologic-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
