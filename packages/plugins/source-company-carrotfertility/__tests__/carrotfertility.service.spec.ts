...
import { CarrotFertilityModule, CarrotFertilityService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'carrotfertility-jobs.json'), 'utf8'),
);
...
/**
 * Spec 648 / T04 — `CarrotFertilityService` unit tests (≥ 9 cases).
 */
describe('CarrotFertilityService — Spec 648 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CarrotFertilityModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CarrotFertilityModule],
      }).compile();
      const service = moduleRef.get(CarrotFertilityService);
      expect(service).toBeInstanceOf(CarrotFertilityService);
      await moduleRef.close();
    });

    it('exports the Site.CARROT_FERTILITY = "carrotfertility" enum value', () => {
      expect(Site.CARROT_FERTILITY).toBe('carrotfertility');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new CarrotFertilityService();
      const result = await service.scrape({
        siteType: [Site.CARROT_FERTILITY],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'carrotfertility-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.CARROT_FERTILITY);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Carrot Fertility');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/carrotfertility/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/carrotfertility/jobs/');
      ...
      expect(job0?.description).toContain('Carrot Fertility');
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Carrot Fertility');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'carrotfertility-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
