...
import { MotiveModule, MotiveService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'motive-jobs.json'), 'utf8'),
);
...
/**
 * Spec 457 / T04 — `MotiveService` unit tests (≥ 9 cases).
 */
describe('MotiveService — Spec 457 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through MotiveModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [MotiveModule],
      }).compile();
      const service = moduleRef.get(MotiveService);
      expect(service).toBeInstanceOf(MotiveService);
      await moduleRef.close();
    });

    it('exports the Site.MOTIVE = "motive" enum value', () => {
      expect(Site.MOTIVE).toBe('motive');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new MotiveService();
      const result = await service.scrape({
        siteType: [Site.MOTIVE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'motive-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.MOTIVE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Motive');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/motive/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/motive/jobs/');
      ...
      expect(job0?.description).toContain('Motive');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/motive/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Motive');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'motive-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
