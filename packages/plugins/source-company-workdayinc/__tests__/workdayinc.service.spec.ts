...
import { WorkdayIncModule, WorkdayIncService } from '../src';
...
const FIXTURE: Fixture = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'fixtures', 'workdayinc-boards.json'), 'utf8'),
);
const COMPANY_NAME = 'Workday';
const ID_PREFIX = 'workdayinc-';
...
describe('WorkdayIncService — Workday delegation (Spec 1736)', () => {
  beforeEach(() => {
    mockGet.mockReset();
    mockPost.mockReset();
  ...
  describe('registration', () => {
    it('resolves through WorkdayIncModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({ imports: [WorkdayIncModule] }).compile();
      expect(moduleRef.get(WorkdayIncService)).toBeInstanceOf(WorkdayIncService);
      await moduleRef.close();
    });

    it('exports Site.WORKDAY_INC = "workdayinc"', () => {
      expect(Site.WORKDAY_INC).toBe('workdayinc');
    });

    it('declares a tagged company plugin with its domains', () => {
      const meta = Reflect.getMetadata(SOURCE_PLUGIN_METADATA, WorkdayIncService);
      expect(meta.site).toBe(Site.WORKDAY_INC);
      expect(meta.category).toBe('company');
      expect(meta.isAts).toBeFalsy();
      expect(meta.companyDomains).toEqual(['workday.com']);
  ...
  describe('recorded board (real Workday adapter, mocked HTTP)', () => {
    it('maps every recorded posting and re-stamps the company identity', async () => {
      const service = new WorkdayIncService(registryWith());
      const result = await service.scrape({ siteType: [Site.WORKDAY_INC], resultsWanted: 100 } as ScraperInputDto);
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.WORKDAY_INC);
    ...
    it('only requests the recorded board URLs', async () => {
      const service = new WorkdayIncService(registryWith());
      await service.scrape({ siteType: [Site.WORKDAY_INC] } as ScraperInputDto);
    ...
    it('honours resultsWanted=1', async () => {
      const service = new WorkdayIncService(registryWith());
      const result = await service.scrape({ siteType: [Site.WORKDAY_INC], resultsWanted: 1 } as ScraperInputDto);
    ...
    it('resolves with an empty result when the board is gone (HTTP 404)', async () => {
      ...
      const service = new WorkdayIncService(registryWith());
      const result = await service.scrape({ siteType: [Site.WORKDAY_INC] } as ScraperInputDto);
  ...
  describe('delegation contract', () => {
    it('forwards the board and the caller input untouched', async () => {
      ...
      const service = new WorkdayIncService(
        registryWith(
          fakeBackend(
      ...
      expect(result.jobs[0].site).toBe(Site.WORKDAY_INC);
      expect(result.jobs[0].companyName).toBe(COMPANY_NAME);
    });

    it('passes an absent resultsWanted through as absent', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new WorkdayIncService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      await service.scrape({ siteType: [Site.WORKDAY_INC] } as ScraperInputDto);
    ...
    it('makes no request when resultsWanted is 0', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new WorkdayIncService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      const result = await service.scrape({ siteType: [Site.WORKDAY_INC], resultsWanted: 0 } as ScraperInputDto);
  ...
  describe('resilience', () => {
    it('reports not_registered when no registry is injected', async () => {
      const result = await new WorkdayIncService().scrape({ siteType: [Site.WORKDAY_INC] } as ScraperInputDto);
      expect(result.jobs).toEqual([]);
    ...
    it('reports not_registered when Workday is missing from the registry', async () => {
      const result = await new WorkdayIncService(new PluginRegistry()).scrape({
        siteType: [Site.WORKDAY_INC],
      } as ScraperInputDto);
      expect(result.diagnostics?.reason).toBe('not_registered');
    });
