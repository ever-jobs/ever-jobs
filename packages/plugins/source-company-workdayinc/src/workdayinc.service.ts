...
/**
 * Workday — Enterprise software (HQ: Pleasanton, CA, USA).
 *
 * Source (Spec 1736): Workday board, scraped in this order:
 *   - `workday:5:Workday` — https://workday.wd5.myworkdayjobs.com/Workday
 *     verified live 2026-09-24: 382 open postings.
 *
 ...
 * party board.
 *
 * Tags: segment=workday-enterprise; industry=enterprise-software.
 */
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'workday:5:Workday', atsIdPrefix: 'wd-workday-' },
];

@SourcePlugin({
  site: Site.WORKDAY_INC,
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['workday.com'],
  description: 'Workday careers via Workday. Tags: segment=workday-enterprise; industry=enterprise-software.',
})
@Injectable()
export class WorkdayIncService implements IScraper {
  private readonly logger = new Logger(WorkdayIncService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Workday source plugin is not registered; cannot scrape Workday');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`Workday: delegating to Workday (${board.companySlug})`);
    ...
    this.logger.log(`Workday: scraped ${jobs.length} jobs`);
