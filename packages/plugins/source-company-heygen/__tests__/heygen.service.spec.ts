...
import { HeygenModule, HeygenService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'heygen-jobs.json'), 'utf8'),
);
...
/**
 * Spec 469 / T04 — `HeygenService` unit tests (≥ 9 cases).
 */
describe('HeygenService — Spec 469 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through HeygenModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [HeygenModule],
      }).compile();
      const service = moduleRef.get(HeygenService);
      expect(service).toBeInstanceOf(HeygenService);
      await moduleRef.close();
    });

    it('exports the Site.HEYGEN = "heygen" enum value', () => {
      expect(Site.HEYGEN).toBe('heygen');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new HeygenService();
      const result = await service.scrape({
        siteType: [Site.HEYGEN],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'heygen-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.HEYGEN);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('HeyGen');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/heygen/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/heygen/jobs/');
      ...
      expect(job0?.description).toContain('HeyGen');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/heygen/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('HeyGen');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'heygen-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
