...
import { BlueEnergyModule, BlueEnergyService } from '../src';

const COMPANY_NAME_EXPECT = 'Blue Energy';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'blueenergy-jobs.json'), 'utf8'),
);
...
describe('BlueEnergyService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through BlueEnergyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [BlueEnergyModule],
      }).compile();
      const service = moduleRef.get(BlueEnergyService);
      expect(service).toBeInstanceOf(BlueEnergyService);
      await moduleRef.close();
    });

    it('exports the Site.BLUE_ENERGY = "blueenergy" enum value', () => {
      expect(Site.BLUE_ENERGY).toBe('blueenergy');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new BlueEnergyService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.BLUE_ENERGY],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('blue-energy');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.BLUE_ENERGY);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('blueenergy-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new BlueEnergyService(registry);
      const result = await service.scrape({
        siteType: [Site.BLUE_ENERGY],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('blue-energy');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('blueenergy-x1');
      expect(result.jobs[0].site).toBe(Site.BLUE_ENERGY);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('blueenergy-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new BlueEnergyService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.BLUE_ENERGY],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new BlueEnergyService();
      const result = await service.scrape({
        siteType: [Site.BLUE_ENERGY],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
