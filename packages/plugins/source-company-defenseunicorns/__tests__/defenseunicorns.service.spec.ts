...
import { DefenseUnicornsModule, DefenseUnicornsService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'defenseunicorns-jobs.json'), 'utf8'),
);
...
/**
 * Spec 835 / T04 — `DefenseUnicornsService` unit tests (≥ 9 cases).
 */
describe('DefenseUnicornsService — Spec 835 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through DefenseUnicornsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [DefenseUnicornsModule],
      }).compile();
      const service = moduleRef.get(DefenseUnicornsService);
      expect(service).toBeInstanceOf(DefenseUnicornsService);
      await moduleRef.close();
    });

    it('exports the Site.DEFENSE_UNICORNS = "defenseunicorns" enum value', () => {
      expect(Site.DEFENSE_UNICORNS).toBe('defenseunicorns');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new DefenseUnicornsService();
      const result = await service.scrape({
        siteType: [Site.DEFENSE_UNICORNS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'defenseunicorns-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.DEFENSE_UNICORNS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Defense Unicorns');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/defenseunicorns/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/defenseunicorns/jobs/');
      ...
      expect(job0?.description).toContain('Defense Unicorns');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/defenseunicorns/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Defense Unicorns');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'defenseunicorns-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
