...
import { ThrivemarketModule, ThrivemarketService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'thrivemarket-jobs.json'), 'utf8'),
);
...
/**
 * Spec 515 / T04 — `ThrivemarketService` unit tests (≥ 9 cases).
 */
describe('ThrivemarketService — Spec 515 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ThrivemarketModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ThrivemarketModule],
      }).compile();
      const service = moduleRef.get(ThrivemarketService);
      expect(service).toBeInstanceOf(ThrivemarketService);
      await moduleRef.close();
    });

    it('exports the Site.THRIVE_MARKET = "thrivemarket" enum value', () => {
      expect(Site.THRIVE_MARKET).toBe('thrivemarket');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ThrivemarketService();
      const result = await service.scrape({
        siteType: [Site.THRIVE_MARKET],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'thrivemarket-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.THRIVE_MARKET);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Thrive Market');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/thrivemarket/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/thrivemarket/jobs/');
      ...
      expect(job0?.description).toContain('Thrive Market');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/thrivemarket/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Thrive Market');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'thrivemarket-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
