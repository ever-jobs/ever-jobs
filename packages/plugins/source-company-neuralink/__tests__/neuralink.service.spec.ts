...
import { NeuralinkModule, NeuralinkService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'neuralink-jobs.json'), 'utf8'),
);
...
/**
 * Spec 551 / T04 — `NeuralinkService` unit tests (≥ 9 cases).
 */
describe('NeuralinkService — Spec 551 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through NeuralinkModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [NeuralinkModule],
      }).compile();
      const service = moduleRef.get(NeuralinkService);
      expect(service).toBeInstanceOf(NeuralinkService);
      await moduleRef.close();
    });

    it('exports the Site.NEURALINK = "neuralink" enum value', () => {
      expect(Site.NEURALINK).toBe('neuralink');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new NeuralinkService();
      const result = await service.scrape({
        siteType: [Site.NEURALINK],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'neuralink-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.NEURALINK);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Neuralink');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/neuralink/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/neuralink/jobs/');
      ...
      expect(job0?.description).toContain('Neuralink');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/neuralink/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Neuralink');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'neuralink-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
