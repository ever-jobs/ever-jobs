...
import { DEMVSystemsModule, DEMVSystemsService } from '../src';

const COMPANY_NAME_EXPECT = 'DEMV Systems';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'demvsystems-jobs.json'), 'utf8'),
);
...
describe('DEMVSystemsService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through DEMVSystemsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [DEMVSystemsModule],
      }).compile();
      const service = moduleRef.get(DEMVSystemsService);
      expect(service).toBeInstanceOf(DEMVSystemsService);
      await moduleRef.close();
    });

    it('exports the Site.DEMV_SYSTEMS = "demvsystems" enum value', () => {
      expect(Site.DEMV_SYSTEMS).toBe('demvsystems');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new DEMVSystemsService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.DEMV_SYSTEMS],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('demvsystems.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.DEMV_SYSTEMS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('demvsystems-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new DEMVSystemsService(registry);
      const result = await service.scrape({
        siteType: [Site.DEMV_SYSTEMS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('demvsystems');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('demvsystems-x1');
      expect(result.jobs[0].site).toBe(Site.DEMV_SYSTEMS);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('demvsystems-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new DEMVSystemsService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.DEMV_SYSTEMS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new DEMVSystemsService();
      const result = await service.scrape({
        siteType: [Site.DEMV_SYSTEMS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
