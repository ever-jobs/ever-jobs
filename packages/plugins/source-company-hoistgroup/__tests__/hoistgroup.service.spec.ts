...
import { HoistGroupModule, HoistGroupService } from '../src';

const COMPANY_NAME_EXPECT = 'Hoist Group';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'hoistgroup-jobs.json'), 'utf8'),
);
...
describe('HoistGroupService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through HoistGroupModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [HoistGroupModule],
      }).compile();
      const service = moduleRef.get(HoistGroupService);
      expect(service).toBeInstanceOf(HoistGroupService);
      await moduleRef.close();
    });

    it('exports the Site.HOIST_GROUP = "hoistgroup" enum value', () => {
      expect(Site.HOIST_GROUP).toBe('hoistgroup');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new HoistGroupService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.HOIST_GROUP],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('HoistGroup');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.HOIST_GROUP);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('hoistgroup-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new HoistGroupService(registry);
      const result = await service.scrape({
        siteType: [Site.HOIST_GROUP],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('HoistGroup');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('hoistgroup-x1');
      expect(result.jobs[0].site).toBe(Site.HOIST_GROUP);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('hoistgroup-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new HoistGroupService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.HOIST_GROUP],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new HoistGroupService();
      const result = await service.scrape({
        siteType: [Site.HOIST_GROUP],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
