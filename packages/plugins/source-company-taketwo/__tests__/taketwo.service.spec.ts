...
import { TakeTwoInteractiveModule, TakeTwoInteractiveService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'taketwo-jobs.json'), 'utf8'),
);
...
/**
 * Spec 947 / T04 — `TakeTwoInteractiveService` unit tests (≥ 9 cases).
 */
describe('TakeTwoInteractiveService — Spec 947 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through TakeTwoInteractiveModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [TakeTwoInteractiveModule],
      }).compile();
      const service = moduleRef.get(TakeTwoInteractiveService);
      expect(service).toBeInstanceOf(TakeTwoInteractiveService);
      await moduleRef.close();
    });

    it('exports the Site.TAKE_TWO_INTERACTIVE = "taketwo" enum value', () => {
      expect(Site.TAKE_TWO_INTERACTIVE).toBe('taketwo');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new TakeTwoInteractiveService();
      const result = await service.scrape({
        siteType: [Site.TAKE_TWO_INTERACTIVE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'taketwo-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.TAKE_TWO_INTERACTIVE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Take-Two Interactive');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/taketwo/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/taketwo/jobs/');
      ...
      expect(job0?.description).toContain('Take-Two Interactive');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/taketwo/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Take-Two Interactive');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'taketwo-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
