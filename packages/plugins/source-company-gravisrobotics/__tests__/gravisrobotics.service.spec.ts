...
import { GravisRoboticsModule, GravisRoboticsService } from '../src';

const COMPANY_NAME_EXPECT = 'Gravis Robotics';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'gravisrobotics-jobs.json'), 'utf8'),
);
...
describe('GravisRoboticsService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through GravisRoboticsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [GravisRoboticsModule],
      }).compile();
      const service = moduleRef.get(GravisRoboticsService);
      expect(service).toBeInstanceOf(GravisRoboticsService);
      await moduleRef.close();
    });

    it('exports the Site.GRAVIS_ROBOTICS = "gravisrobotics" enum value', () => {
      expect(Site.GRAVIS_ROBOTICS).toBe('gravisrobotics');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new GravisRoboticsService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.GRAVIS_ROBOTICS],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'gravisrobotics-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.GRAVIS_ROBOTICS);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('gravisrobotics-' + first.id);
      ...
      expect(calledUrls[0]).toContain('gravisrobotics');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.GRAVIS_ROBOTICS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('gravisrobotics-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new GravisRoboticsService(registry);
      const result = await service.scrape({
        siteType: [Site.GRAVIS_ROBOTICS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('gravisrobotics');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('gravisrobotics-x1');
      expect(result.jobs[0].site).toBe(Site.GRAVIS_ROBOTICS);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('gravisrobotics-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new GravisRoboticsService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.GRAVIS_ROBOTICS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new GravisRoboticsService();
      const result = await service.scrape({
        siteType: [Site.GRAVIS_ROBOTICS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
