...
import { RevefiModule, RevefiService } from '../src';

const COMPANY_NAME_EXPECT = 'Revefi';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'revefi-jobs.json'), 'utf8'),
);
...
describe('RevefiService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through RevefiModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [RevefiModule],
      }).compile();
      const service = moduleRef.get(RevefiService);
      expect(service).toBeInstanceOf(RevefiService);
      await moduleRef.close();
    });

    it('exports the Site.REVEFI = "revefi" enum value', () => {
      expect(Site.REVEFI).toBe('revefi');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new RevefiService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.REVEFI],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('revefi');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.REVEFI);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('revefi-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new RevefiService(registry);
      const result = await service.scrape({
        siteType: [Site.REVEFI],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('revefi');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('revefi-x1');
      expect(result.jobs[0].site).toBe(Site.REVEFI);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('revefi-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new RevefiService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.REVEFI],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new RevefiService();
      const result = await service.scrape({
        siteType: [Site.REVEFI],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
