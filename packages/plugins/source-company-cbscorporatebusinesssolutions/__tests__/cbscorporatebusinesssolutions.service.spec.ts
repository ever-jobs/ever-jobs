...
import { CbsCorporateBusinessSolutionsModule, CbsCorporateBusinessSolutionsService } from '../src';

const COMPANY_NAME_EXPECT = 'cbs Corporate Business Solutions';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'cbscorporatebusinesssolutions-jobs.json'), 'utf8'),
);
...
describe('CbsCorporateBusinessSolutionsService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CbsCorporateBusinessSolutionsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CbsCorporateBusinessSolutionsModule],
      }).compile();
      const service = moduleRef.get(CbsCorporateBusinessSolutionsService);
      expect(service).toBeInstanceOf(CbsCorporateBusinessSolutionsService);
      await moduleRef.close();
    });

    it('exports the Site.CBS_CORPORATE_BUSINESS_SOLUTIONS = "cbscorporatebusinesssolutions" enum value', () => {
      expect(Site.CBS_CORPORATE_BUSINESS_SOLUTIONS).toBe('cbscorporatebusinesssolutions');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new CbsCorporateBusinessSolutionsService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.CBS_CORPORATE_BUSINESS_SOLUTIONS],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'cbscorporatebusinesssolutions-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Recruitee's defaults
      expect(job0?.site).toBe(Site.CBS_CORPORATE_BUSINESS_SOLUTIONS);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('cbscorporatebusinesssolutions-' + first.id);
      ...
      expect(calledUrls[0]).toContain('cbsconsulting.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.CBS_CORPORATE_BUSINESS_SOLUTIONS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('cbscorporatebusinesssolutions-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new CbsCorporateBusinessSolutionsService(registry);
      const result = await service.scrape({
        siteType: [Site.CBS_CORPORATE_BUSINESS_SOLUTIONS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('cbsconsulting');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('cbscorporatebusinesssolutions-x1');
      expect(result.jobs[0].site).toBe(Site.CBS_CORPORATE_BUSINESS_SOLUTIONS);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('cbscorporatebusinesssolutions-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new CbsCorporateBusinessSolutionsService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.CBS_CORPORATE_BUSINESS_SOLUTIONS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new CbsCorporateBusinessSolutionsService();
      const result = await service.scrape({
        siteType: [Site.CBS_CORPORATE_BUSINESS_SOLUTIONS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
