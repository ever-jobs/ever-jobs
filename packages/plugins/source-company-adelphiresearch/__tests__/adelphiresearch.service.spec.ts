...
import { AdelphiresearchModule, AdelphiresearchService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'adelphiresearch-jobs.json'), 'utf8'),
);
...
/**
 * Spec 191 / T04 — `AdelphiresearchService` unit tests (≥ 9 cases).
 */
describe('AdelphiresearchService — Spec 191 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AdelphiresearchModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AdelphiresearchModule],
      }).compile();
      const service = moduleRef.get(AdelphiresearchService);
      expect(service).toBeInstanceOf(AdelphiresearchService);
      await moduleRef.close();
    });

    it('exports the Site.ADELPHIRESEARCH = "adelphiresearch" enum value', () => {
      expect(Site.ADELPHIRESEARCH).toBe('adelphiresearch');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AdelphiresearchService();
      const result = await service.scrape({
        siteType: [Site.ADELPHIRESEARCH],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'adelphiresearch-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ADELPHIRESEARCH);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Adelphi Research');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/adelphiresearch/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/adelphiresearch/jobs/');
      ...
      expect(job0?.description).toContain('Adelphi Research');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/adelphiresearch/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Adelphi Research');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'adelphiresearch-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
