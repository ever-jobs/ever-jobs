...
import { FuseEnergyModule, FuseEnergyService } from '../src';

const COMPANY_NAME_EXPECT = 'Fuse Energy';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'fuseenergy-jobs.json'), 'utf8'),
);
...
describe('FuseEnergyService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through FuseEnergyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [FuseEnergyModule],
      }).compile();
      const service = moduleRef.get(FuseEnergyService);
      expect(service).toBeInstanceOf(FuseEnergyService);
      await moduleRef.close();
    });

    it('exports the Site.FUSE_ENERGY = "fuseenergy" enum value', () => {
      expect(Site.FUSE_ENERGY).toBe('fuseenergy');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new FuseEnergyService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.FUSE_ENERGY],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('fuse');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.FUSE_ENERGY);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('fuseenergy-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new FuseEnergyService(registry);
      const result = await service.scrape({
        siteType: [Site.FUSE_ENERGY],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('fuse');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('fuseenergy-x1');
      expect(result.jobs[0].site).toBe(Site.FUSE_ENERGY);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('fuseenergy-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new FuseEnergyService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.FUSE_ENERGY],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new FuseEnergyService();
      const result = await service.scrape({
        siteType: [Site.FUSE_ENERGY],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
