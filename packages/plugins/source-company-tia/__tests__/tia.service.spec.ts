...
import { TiaModule, TiaService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'tia-jobs.json'), 'utf8'),
);
...
/**
 * Spec 524 / T04 — `TiaService` unit tests (≥ 9 cases).
 */
describe('TiaService — Spec 524 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through TiaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [TiaModule],
      }).compile();
      const service = moduleRef.get(TiaService);
      expect(service).toBeInstanceOf(TiaService);
      await moduleRef.close();
    });

    it('exports the Site.TIA_HEALTH = "tia" enum value', () => {
      expect(Site.TIA_HEALTH).toBe('tia');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new TiaService();
      const result = await service.scrape({
        siteType: [Site.TIA_HEALTH],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'tia-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.TIA_HEALTH);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Tia');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/tia/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/tia/jobs/');
      ...
      expect(job0?.description).toContain('Tia');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/tia/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Tia');
      }
    });
  });
