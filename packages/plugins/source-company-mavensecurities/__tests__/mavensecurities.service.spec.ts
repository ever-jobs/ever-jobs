...
import { MavenSecuritiesModule, MavenSecuritiesService } from '../src';
...
const FIXTURE: Fixture = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'fixtures', 'mavensecurities-boards.json'), 'utf8'),
);
const COMPANY_NAME = 'Maven Securities';
const ID_PREFIX = 'mavensecurities-';
...
describe('MavenSecuritiesService — Greenhouse delegation (Spec 1737)', () => {
  beforeEach(() => {
    mockGet.mockReset();
    mockPost.mockReset();
  ...
  describe('registration', () => {
    it('resolves through MavenSecuritiesModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({ imports: [MavenSecuritiesModule] }).compile();
      expect(moduleRef.get(MavenSecuritiesService)).toBeInstanceOf(MavenSecuritiesService);
      await moduleRef.close();
    });

    it('exports Site.MAVEN_SECURITIES = "mavensecurities"', () => {
      expect(Site.MAVEN_SECURITIES).toBe('mavensecurities');
    });

    it('declares a tagged company plugin with its domains', () => {
      const meta = Reflect.getMetadata(SOURCE_PLUGIN_METADATA, MavenSecuritiesService);
      expect(meta.site).toBe(Site.MAVEN_SECURITIES);
      expect(meta.category).toBe('company');
      expect(meta.isAts).toBeFalsy();
      expect(meta.companyDomains).toEqual(['mavensecurities.com']);
  ...
  describe('recorded board (real Greenhouse adapter, mocked HTTP)', () => {
    it('maps every recorded posting and re-stamps the company identity', async () => {
      const service = new MavenSecuritiesService(registryWith());
      const result = await service.scrape({ siteType: [Site.MAVEN_SECURITIES], resultsWanted: 100 } as ScraperInputDto);
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.MAVEN_SECURITIES);
    ...
    it('only requests the recorded board URLs', async () => {
      const service = new MavenSecuritiesService(registryWith());
      await service.scrape({ siteType: [Site.MAVEN_SECURITIES] } as ScraperInputDto);
    ...
    it('honours resultsWanted=1', async () => {
      const service = new MavenSecuritiesService(registryWith());
      const result = await service.scrape({ siteType: [Site.MAVEN_SECURITIES], resultsWanted: 1 } as ScraperInputDto);
    ...
    it('resolves with an empty result when the board is gone (HTTP 404)', async () => {
      ...
      const service = new MavenSecuritiesService(registryWith());
      const result = await service.scrape({ siteType: [Site.MAVEN_SECURITIES] } as ScraperInputDto);
  ...
  describe('delegation contract', () => {
    it('forwards the board and the caller input untouched', async () => {
      ...
      const service = new MavenSecuritiesService(
        registryWith(
          fakeBackend(
      ...
      const result = await service.scrape({
        siteType: [Site.MAVEN_SECURITIES],
      ...
      expect(result.jobs[0].site).toBe(Site.MAVEN_SECURITIES);
      expect(result.jobs[0].companyName).toBe(COMPANY_NAME);
    });

    it('passes an absent resultsWanted through as absent', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new MavenSecuritiesService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      await service.scrape({ siteType: [Site.MAVEN_SECURITIES] } as ScraperInputDto);
    ...
    it('makes no request when resultsWanted is 0', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new MavenSecuritiesService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      const result = await service.scrape({ siteType: [Site.MAVEN_SECURITIES], resultsWanted: 0 } as ScraperInputDto);
  ...
  describe('resilience', () => {
    it('reports not_registered when no registry is injected', async () => {
      const result = await new MavenSecuritiesService().scrape({ siteType: [Site.MAVEN_SECURITIES] } as ScraperInputDto);
      expect(result.jobs).toEqual([]);
    ...
    it('reports not_registered when Greenhouse is missing from the registry', async () => {
      const result = await new MavenSecuritiesService(new PluginRegistry()).scrape({
        siteType: [Site.MAVEN_SECURITIES],
      } as ScraperInputDto);
      expect(result.diagnostics?.reason).toBe('not_registered');
    });
