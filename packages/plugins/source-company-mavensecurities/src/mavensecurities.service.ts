...
/**
 * Maven Securities — Market making (HQ: London, United Kingdom).
 *
 * Source (Spec 1737): Greenhouse board, scraped in this order:
 *   - `mavensecuritiesholdingltd` — https://job-boards.greenhouse.io/mavensecuritiesholdingltd
 *     verified live 2026-09-24: 45 open postings.
 *
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'mavensecuritiesholdingltd', atsIdPrefix: 'gh-' },
];

@SourcePlugin({
  site: Site.MAVEN_SECURITIES,
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['mavensecurities.com'],
  description: 'Maven Securities careers via Greenhouse. Tags: segment=quant-trading; industry=market-making.',
})
@Injectable()
export class MavenSecuritiesService implements IScraper {
  private readonly logger = new Logger(MavenSecuritiesService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Greenhouse source plugin is not registered; cannot scrape Maven Securities');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`Maven Securities: delegating to Greenhouse (${board.companySlug})`);
      ...
      for (const job of result.jobs ?? []) {
        job.site = Site.MAVEN_SECURITIES;
    ...
    this.logger.log(`Maven Securities: scraped ${jobs.length} jobs`);
