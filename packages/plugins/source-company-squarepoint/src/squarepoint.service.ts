...
/**
 * Squarepoint Capital — Quantitative investment management (HQ: London, United Kingdom).
 *
 * Source (Spec 1737): Greenhouse board, scraped in this order:
 *   - `squarepointcapital` — https://job-boards.greenhouse.io/squarepointcapital
 *     verified live 2026-09-24: 89 open postings.
 *
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'squarepointcapital', atsIdPrefix: 'gh-' },
];

@SourcePlugin({
  site: Site.SQUAREPOINT,
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['squarepoint-capital.com'],
  description: 'Squarepoint Capital careers via Greenhouse. Tags: segment=quant-trading; industry=quantitative-investment-management.',
})
@Injectable()
export class SquarepointService implements IScraper {
  private readonly logger = new Logger(SquarepointService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Greenhouse source plugin is not registered; cannot scrape Squarepoint Capital');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`Squarepoint Capital: delegating to Greenhouse (${board.companySlug})`);
    ...
    this.logger.log(`Squarepoint Capital: scraped ${jobs.length} jobs`);
