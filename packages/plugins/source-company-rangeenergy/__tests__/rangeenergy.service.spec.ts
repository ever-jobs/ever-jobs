...
import { RangeEnergyModule, RangeEnergyService } from '../src';

const COMPANY_NAME_EXPECT = 'Range Energy';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'rangeenergy-jobs.json'), 'utf8'),
);
...
describe('RangeEnergyService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through RangeEnergyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [RangeEnergyModule],
      }).compile();
      const service = moduleRef.get(RangeEnergyService);
      expect(service).toBeInstanceOf(RangeEnergyService);
      await moduleRef.close();
    });

    it('exports the Site.RANGE_ENERGY = "rangeenergy" enum value', () => {
      expect(Site.RANGE_ENERGY).toBe('rangeenergy');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new RangeEnergyService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.RANGE_ENERGY],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'rangeenergy-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.RANGE_ENERGY);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('rangeenergy-' + first.id);
      ...
      expect(calledUrls[0]).toContain('rangeenergy');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.RANGE_ENERGY);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('rangeenergy-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new RangeEnergyService(registry);
      const result = await service.scrape({
        siteType: [Site.RANGE_ENERGY],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('rangeenergy');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('rangeenergy-x1');
      expect(result.jobs[0].site).toBe(Site.RANGE_ENERGY);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('rangeenergy-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new RangeEnergyService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.RANGE_ENERGY],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new RangeEnergyService();
      const result = await service.scrape({
        siteType: [Site.RANGE_ENERGY],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
