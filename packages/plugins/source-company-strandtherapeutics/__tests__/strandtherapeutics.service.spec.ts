...
import { StrandTherapeuticsModule, StrandTherapeuticsService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'strandtherapeutics-jobs.json'), 'utf8'),
);
...
/**
 * Spec 945 / T04 — `StrandTherapeuticsService` unit tests (≥ 9 cases).
 */
describe('StrandTherapeuticsService — Spec 945 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through StrandTherapeuticsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [StrandTherapeuticsModule],
      }).compile();
      const service = moduleRef.get(StrandTherapeuticsService);
      expect(service).toBeInstanceOf(StrandTherapeuticsService);
      await moduleRef.close();
    });

    it('exports the Site.STRAND_THERAPEUTICS = "strandtherapeutics" enum value', () => {
      expect(Site.STRAND_THERAPEUTICS).toBe('strandtherapeutics');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new StrandTherapeuticsService();
      const result = await service.scrape({
        siteType: [Site.STRAND_THERAPEUTICS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'strandtherapeutics-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.STRAND_THERAPEUTICS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Strand Therapeutics');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/strandtherapeutics/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/strandtherapeutics/jobs/');
      ...
      expect(job0?.description).toContain('Strand Therapeutics');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/strandtherapeutics/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Strand Therapeutics');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'strandtherapeutics-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
