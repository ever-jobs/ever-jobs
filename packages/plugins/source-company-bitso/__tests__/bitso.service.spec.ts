...
import { BitsoModule, BitsoService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'bitso-jobs.json'), 'utf8'),
);
...
/**
 * Spec 638 / T04 — `BitsoService` unit tests (≥ 9 cases).
 */
describe('BitsoService — Spec 638 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through BitsoModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [BitsoModule],
      }).compile();
      const service = moduleRef.get(BitsoService);
      expect(service).toBeInstanceOf(BitsoService);
      await moduleRef.close();
    });

    it('exports the Site.BITSO = "bitso" enum value', () => {
      expect(Site.BITSO).toBe('bitso');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new BitsoService();
      ...
      const job0 = dto.jobs.find((j) => j.id === 'bitso-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.BITSO);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Bitso');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/bitso/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/bitso/jobs/');
      ...
      expect(job0?.description).toContain('Bitso');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/bitso/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Bitso');
      }
    });
  });
