...
import { CheckbookModule, CheckbookService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'checkbook-jobs.json'), 'utf8'),
);
...
/**
 * Spec 825 / T04 — `CheckbookService` unit tests (≥ 9 cases).
 */
describe('CheckbookService — Spec 825 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CheckbookModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CheckbookModule],
      }).compile();
      const service = moduleRef.get(CheckbookService);
      expect(service).toBeInstanceOf(CheckbookService);
      await moduleRef.close();
    });

    it('exports the Site.CHECKBOOK = "checkbook" enum value', () => {
      expect(Site.CHECKBOOK).toBe('checkbook');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new CheckbookService();
      const result = await service.scrape({
        siteType: [Site.CHECKBOOK],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'checkbook-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.CHECKBOOK);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Checkbook');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/checkbook/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/checkbook/jobs/');
      ...
      expect(job0?.description).toContain('Checkbook');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/checkbook/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Checkbook');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'checkbook-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
