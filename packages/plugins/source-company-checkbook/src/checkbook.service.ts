...
/**
 * Checkbook — Checkbook is an employer in the Fintech sector, headquartered in San Mateo, California, USA..
 *
 * Checkbook is a company operating in Fintech — digital payments /
 * disbursements, headquartered in San Mateo, California, USA. This
 * source plugin ingests live open roles published on Checkbook's
 ...
 *
 * Sector: Fintech — digital payments / disbursements. HQ: San Mateo, California, USA.
 *
 * Highlights:
 *   - Sector: Fintech — digital payments / disbursements
 ...
 *     Jobs schema
 *
 * Source profile (Spec 825):
 *   - D-04 — Greenhouse canonical hosted-board host (variant 2):
 *     `https://job-boards.greenhouse.io/checkbook/jobs/<id>`.
 *   - D-08 — entity-decode-then-tag-strip description pipeline.
 *   - D-09 — wire `company_name` pass-through (`'Checkbook'`).
 ...
 */
const API_URL = 'https://api.greenhouse.io/v1/boards/checkbook/jobs';
...
@Injectable()
export class CheckbookService implements IScraper {
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    try {
      ...
      this.logger.log(`Checkbook: fetching ${url}`);
      ...
      for (const listing of listings) {
        ...
        jobs.push(
          new JobPostDto({
            ...
            companyName: listing.company_name ?? 'Checkbook',
            // D-04: wire `absolute_url` flows through (variant 2).
            jobUrl:
              listing.absolute_url ??
              `https://job-boards.greenhouse.io/checkbook/jobs/${listing.id}`,
            location,
      ...
      this.logger.log(`Checkbook: scraped ${jobs.length} jobs`);
    } catch (err: any) {
      this.logger.error(`Checkbook scrape failed: ${err.message}`);
