...
import { AlamarbiosciencesModule, AlamarbiosciencesService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'alamarbiosciences-jobs.json'), 'utf8'),
);
...
/**
 * Spec 225 / T04 — `AlamarbiosciencesService` unit tests (≥ 9 cases).
 */
describe('AlamarbiosciencesService — Spec 225 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AlamarbiosciencesModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AlamarbiosciencesModule],
      }).compile();
      const service = moduleRef.get(AlamarbiosciencesService);
      expect(service).toBeInstanceOf(AlamarbiosciencesService);
      await moduleRef.close();
    });

    it('exports the Site.ALAMARBIOSCIENCES = "alamarbiosciences" enum value', () => {
      expect(Site.ALAMARBIOSCIENCES).toBe('alamarbiosciences');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AlamarbiosciencesService();
      const result = await service.scrape({
        siteType: [Site.ALAMARBIOSCIENCES],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'alamarbiosciences-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ALAMARBIOSCIENCES);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Alamar Biosciences');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/alamarbiosciences/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/alamarbiosciences/jobs/');
      ...
      expect(job0?.description).toContain('Alamar Biosciences');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/alamarbiosciences/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Alamar Biosciences');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'alamarbiosciences-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
