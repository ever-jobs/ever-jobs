...
import { SeoulRoboticsModule, SeoulRoboticsService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'seoulrobotics-jobs.json'), 'utf8'),
);
...
/**
 * Spec 929 / T04 — `SeoulRoboticsService` unit tests (≥ 9 cases).
 */
describe('SeoulRoboticsService — Spec 929 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SeoulRoboticsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SeoulRoboticsModule],
      }).compile();
      const service = moduleRef.get(SeoulRoboticsService);
      expect(service).toBeInstanceOf(SeoulRoboticsService);
      await moduleRef.close();
    });

    it('exports the Site.SEOUL_ROBOTICS = "seoulrobotics" enum value', () => {
      expect(Site.SEOUL_ROBOTICS).toBe('seoulrobotics');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new SeoulRoboticsService();
      const result = await service.scrape({
        siteType: [Site.SEOUL_ROBOTICS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'seoulrobotics-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.SEOUL_ROBOTICS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Seoul Robotics');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/seoulrobotics/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/seoulrobotics/jobs/');
      ...
      expect(job0?.description).toContain('Seoul Robotics');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/seoulrobotics/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Seoul Robotics');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'seoulrobotics-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
