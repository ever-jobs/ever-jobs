...
import { UdacityModule, UdacityService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'udacity-jobs.json'), 'utf8'),
);
...
/**
 * Spec 755 / T04 — `UdacityService` unit tests (≥ 9 cases).
 */
describe('UdacityService — Spec 755 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through UdacityModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [UdacityModule],
      }).compile();
      const service = moduleRef.get(UdacityService);
      expect(service).toBeInstanceOf(UdacityService);
      await moduleRef.close();
    });

    it('exports the Site.UDACITY = "udacity" enum value', () => {
      expect(Site.UDACITY).toBe('udacity');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new UdacityService();
      const result = await service.scrape({
        siteType: [Site.UDACITY],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'udacity-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.UDACITY);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Udacity');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/udacity/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/udacity/jobs/');
      ...
      expect(job0?.description).toContain('Udacity');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/udacity/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Udacity');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'udacity-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
