...
import { AgecareersModule, AgecareersService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'agecareers-jobs.json'), 'utf8'),
);
...
/**
 * Spec 206 / T04 — `AgecareersService` unit tests (≥ 9 cases).
 */
describe('AgecareersService — Spec 206 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AgecareersModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AgecareersModule],
      }).compile();
      const service = moduleRef.get(AgecareersService);
      expect(service).toBeInstanceOf(AgecareersService);
      await moduleRef.close();
    });

    it('exports the Site.AGECAREERS = "agecareers" enum value', () => {
      expect(Site.AGECAREERS).toBe('agecareers');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AgecareersService();
      const result = await service.scrape({
        siteType: [Site.AGECAREERS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'agecareers-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.AGECAREERS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('AGE Solutions');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/agecareers/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/agecareers/jobs/');
      ...
      expect(job0?.description).toContain('AGE Solutions');
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('AGE Solutions');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'agecareers-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
