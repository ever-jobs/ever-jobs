...
import { NothingModule, NothingService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'nothing-jobs.json'), 'utf8'),
);
...
/**
 * Spec 898 / T04 — `NothingService` unit tests (≥ 9 cases).
 */
describe('NothingService — Spec 898 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through NothingModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [NothingModule],
      }).compile();
      const service = moduleRef.get(NothingService);
      expect(service).toBeInstanceOf(NothingService);
      await moduleRef.close();
    });

    it('exports the Site.NOTHING = "nothing" enum value', () => {
      expect(Site.NOTHING).toBe('nothing');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new NothingService();
      const result = await service.scrape({
        siteType: [Site.NOTHING],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'nothing-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.NOTHING);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Nothing');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/nothing/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/nothing/jobs/');
      ...
      expect(job0?.description).toContain('Nothing');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/nothing/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Nothing');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'nothing-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
