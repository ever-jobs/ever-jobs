...
import { ConfluentModule, ConfluentService } from '../src';

const COMPANY_NAME_EXPECT = 'Confluent';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'confluent-jobs.json'), 'utf8'),
);
...
describe('ConfluentService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ConfluentModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ConfluentModule],
      }).compile();
      const service = moduleRef.get(ConfluentService);
      expect(service).toBeInstanceOf(ConfluentService);
      await moduleRef.close();
    });

    it('exports the Site.CONFLUENT = "confluent" enum value', () => {
      expect(Site.CONFLUENT).toBe('confluent');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new ConfluentService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.CONFLUENT],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('confluent');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.CONFLUENT);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('confluent-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new ConfluentService(registry);
      const result = await service.scrape({
        siteType: [Site.CONFLUENT],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('confluent');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('confluent-x1');
      expect(result.jobs[0].site).toBe(Site.CONFLUENT);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('confluent-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new ConfluentService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.CONFLUENT],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new ConfluentService();
      const result = await service.scrape({
        siteType: [Site.CONFLUENT],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
