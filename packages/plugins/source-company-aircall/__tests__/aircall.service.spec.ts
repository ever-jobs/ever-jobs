...
import { AircallModule, AircallService } from '../src';

const COMPANY_NAME_EXPECT = 'Aircall';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'aircall-jobs.json'), 'utf8'),
);
...
describe('AircallService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AircallModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AircallModule],
      }).compile();
      const service = moduleRef.get(AircallService);
      expect(service).toBeInstanceOf(AircallService);
      await moduleRef.close();
    });

    it('exports the Site.AIRCALL = "aircall" enum value', () => {
      expect(Site.AIRCALL).toBe('aircall');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new AircallService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.AIRCALL],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('aircall');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.AIRCALL);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('aircall-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new AircallService(registry);
      const result = await service.scrape({
        siteType: [Site.AIRCALL],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('aircall');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('aircall-x1');
      expect(result.jobs[0].site).toBe(Site.AIRCALL);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('aircall-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new AircallService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.AIRCALL],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new AircallService();
      const result = await service.scrape({
        siteType: [Site.AIRCALL],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
