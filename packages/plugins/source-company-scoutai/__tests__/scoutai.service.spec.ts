...
import { ScoutAIModule, ScoutAIService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'scoutai-jobs.json'), 'utf8'),
);
...
/**
 * Spec 926 / T04 — `ScoutAIService` unit tests (≥ 9 cases).
 */
describe('ScoutAIService — Spec 926 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ScoutAIModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ScoutAIModule],
      }).compile();
      const service = moduleRef.get(ScoutAIService);
      expect(service).toBeInstanceOf(ScoutAIService);
      await moduleRef.close();
    });

    it('exports the Site.SCOUT_AI = "scoutai" enum value', () => {
      expect(Site.SCOUT_AI).toBe('scoutai');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ScoutAIService();
      const result = await service.scrape({
        siteType: [Site.SCOUT_AI],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'scoutai-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.SCOUT_AI);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Scout AI');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/scoutai/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/scoutai/jobs/');
      ...
      expect(job0?.description).toContain('Scout AI');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/scoutai/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Scout AI');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'scoutai-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
