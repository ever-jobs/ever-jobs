...
import { PapaModule, PapaService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'papa-jobs.json'), 'utf8'),
);
...
/**
 * Spec 508 / T04 — `PapaService` unit tests (≥ 9 cases).
 */
describe('PapaService — Spec 508 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through PapaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [PapaModule],
      }).compile();
      const service = moduleRef.get(PapaService);
      expect(service).toBeInstanceOf(PapaService);
      await moduleRef.close();
    });

    it('exports the Site.PAPA = "papa" enum value', () => {
      expect(Site.PAPA).toBe('papa');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const job0 = dto.jobs.find((j) => j.id === 'papa-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.PAPA);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Papa');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/papa/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/papa/jobs/');
      ...
      expect(job0?.description).toContain('Papa');
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Papa');
      }
    });
  });
