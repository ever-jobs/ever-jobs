...
import { IbVogtModule, IbVogtService } from '../src';

const COMPANY_NAME_EXPECT = 'ib vogt';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'ibvogt-jobs.json'), 'utf8'),
);
...
describe('IbVogtService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through IbVogtModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [IbVogtModule],
      }).compile();
      const service = moduleRef.get(IbVogtService);
      expect(service).toBeInstanceOf(IbVogtService);
      await moduleRef.close();
    });

    it('exports the Site.IB_VOGT = "ibvogt" enum value', () => {
      expect(Site.IB_VOGT).toBe('ibvogt');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new IbVogtService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.IB_VOGT],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('ib-vogt-GmbH');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.IB_VOGT);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('ibvogt-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new IbVogtService(registry);
      const result = await service.scrape({
        siteType: [Site.IB_VOGT],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('ib-vogt-GmbH');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('ibvogt-x1');
      expect(result.jobs[0].site).toBe(Site.IB_VOGT);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('ibvogt-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new IbVogtService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.IB_VOGT],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new IbVogtService();
      const result = await service.scrape({
        siteType: [Site.IB_VOGT],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
