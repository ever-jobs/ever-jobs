...
/**
 * Expedia Group — Travel technology (HQ: Seattle, WA, USA).
 *
 * Source (Spec 1736): Workday board, scraped in this order:
 *   - `expedia:108:search` — https://expedia.wd108.myworkdayjobs.com/search
 *     verified live 2026-09-24: 169 open postings.
 *
 ...
 * through untouched.
 *
 * Tags: segment=workday-enterprise; industry=travel-technology.
 */
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'expedia:108:search', atsIdPrefix: 'wd-expedia-' },
];

@SourcePlugin({
  site: Site.EXPEDIA_GROUP,
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['expediagroup.com', 'expedia.com'],
  description: 'Expedia Group careers via Workday. Tags: segment=workday-enterprise; industry=travel-technology.',
})
@Injectable()
export class ExpediaGroupService implements IScraper {
  private readonly logger = new Logger(ExpediaGroupService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Workday source plugin is not registered; cannot scrape Expedia Group');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`Expedia Group: delegating to Workday (${board.companySlug})`);
    ...
    this.logger.log(`Expedia Group: scraped ${jobs.length} jobs`);
