...
import { ExpediaGroupModule, ExpediaGroupService } from '../src';
...
const FIXTURE: Fixture = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'fixtures', 'expediagroup-boards.json'), 'utf8'),
);
const COMPANY_NAME = 'Expedia Group';
const ID_PREFIX = 'expediagroup-';
...
describe('ExpediaGroupService — Workday delegation (Spec 1736)', () => {
  beforeEach(() => {
    mockGet.mockReset();
    mockPost.mockReset();
  ...
  describe('registration', () => {
    it('resolves through ExpediaGroupModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({ imports: [ExpediaGroupModule] }).compile();
      expect(moduleRef.get(ExpediaGroupService)).toBeInstanceOf(ExpediaGroupService);
      await moduleRef.close();
    });

    it('exports Site.EXPEDIA_GROUP = "expediagroup"', () => {
      expect(Site.EXPEDIA_GROUP).toBe('expediagroup');
    });

    it('declares a tagged company plugin with its domains', () => {
      const meta = Reflect.getMetadata(SOURCE_PLUGIN_METADATA, ExpediaGroupService);
      expect(meta.site).toBe(Site.EXPEDIA_GROUP);
      expect(meta.category).toBe('company');
      expect(meta.isAts).toBeFalsy();
      expect(meta.companyDomains).toEqual(['expediagroup.com','expedia.com']);
  ...
  describe('recorded board (real Workday adapter, mocked HTTP)', () => {
    it('maps every recorded posting and re-stamps the company identity', async () => {
      const service = new ExpediaGroupService(registryWith());
      const result = await service.scrape({ siteType: [Site.EXPEDIA_GROUP], resultsWanted: 100 } as ScraperInputDto);
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.EXPEDIA_GROUP);
    ...
    it('only requests the recorded board URLs', async () => {
      const service = new ExpediaGroupService(registryWith());
      await service.scrape({ siteType: [Site.EXPEDIA_GROUP] } as ScraperInputDto);
    ...
    it('honours resultsWanted=1', async () => {
      const service = new ExpediaGroupService(registryWith());
      const result = await service.scrape({ siteType: [Site.EXPEDIA_GROUP], resultsWanted: 1 } as ScraperInputDto);
    ...
    it('resolves with an empty result when the board is gone (HTTP 404)', async () => {
      ...
      const service = new ExpediaGroupService(registryWith());
      const result = await service.scrape({ siteType: [Site.EXPEDIA_GROUP] } as ScraperInputDto);
  ...
  describe('delegation contract', () => {
    it('forwards the board and the caller input untouched', async () => {
      ...
      const service = new ExpediaGroupService(
        registryWith(
          fakeBackend(
      ...
      expect(result.jobs[0].site).toBe(Site.EXPEDIA_GROUP);
    ...
    it('never forwards the caller\'s credentials to the board (Spec 1735 §4.5)', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new ExpediaGroupService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
    ...
    it('makes no request when resultsWanted is 0', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new ExpediaGroupService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      const result = await service.scrape({ siteType: [Site.EXPEDIA_GROUP], resultsWanted: 0 } as ScraperInputDto);
  ...
  describe('resilience', () => {
    it('reports not_registered when no registry is injected', async () => {
      const result = await new ExpediaGroupService().scrape({ siteType: [Site.EXPEDIA_GROUP] } as ScraperInputDto);
      expect(result.jobs).toEqual([]);
    ...
    it('reports not_registered when Workday is missing from the registry', async () => {
      const result = await new ExpediaGroupService(new PluginRegistry()).scrape({
        siteType: [Site.EXPEDIA_GROUP],
      } as ScraperInputDto);
      expect(result.diagnostics?.reason).toBe('not_registered');
    });
