...
import { AmylyxModule, AmylyxService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'amylyx-jobs.json'), 'utf8'),
);
...
/**
 * Spec 266 / T04 — `AmylyxService` unit tests (≥ 9 cases).
 */
describe('AmylyxService — Spec 266 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AmylyxModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AmylyxModule],
      }).compile();
      const service = moduleRef.get(AmylyxService);
      expect(service).toBeInstanceOf(AmylyxService);
      await moduleRef.close();
    });

    it('exports the Site.AMYLYX = "amylyx" enum value', () => {
      expect(Site.AMYLYX).toBe('amylyx');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AmylyxService();
      const result = await service.scrape({
        siteType: [Site.AMYLYX],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'amylyx-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.AMYLYX);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Amylyx Pharmaceuticals');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/amylyx/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/amylyx/jobs/');
      ...
      expect(job0?.description).toContain('Amylyx Pharmaceuticals');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/amylyx/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Amylyx Pharmaceuticals');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'amylyx-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
