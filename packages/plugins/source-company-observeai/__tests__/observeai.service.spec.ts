...
import { ObserveaiModule, ObserveaiService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'observeai-jobs.json'), 'utf8'),
);
...
/**
 * Spec 496 / T04 — `ObserveaiService` unit tests (≥ 9 cases).
 */
describe('ObserveaiService — Spec 496 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ObserveaiModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ObserveaiModule],
      }).compile();
      const service = moduleRef.get(ObserveaiService);
      expect(service).toBeInstanceOf(ObserveaiService);
      await moduleRef.close();
    });

    it('exports the Site.OBSERVEAI = "observeai" enum value', () => {
      expect(Site.OBSERVEAI).toBe('observeai');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ObserveaiService();
      const result = await service.scrape({
        siteType: [Site.OBSERVEAI],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'observeai-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.OBSERVEAI);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Observe.AI');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/observeai/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/observeai/jobs/');
      ...
      expect(job0?.description).toContain('Observe.AI');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/observeai/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Observe.AI');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'observeai-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
