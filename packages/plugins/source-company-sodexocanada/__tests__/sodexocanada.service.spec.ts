...
import { SodexoCanadaModule, SodexoCanadaService } from '../src';

const COMPANY_NAME_EXPECT = 'Sodexo Canada';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'sodexocanada-jobs.json'), 'utf8'),
);
...
describe('SodexoCanadaService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SodexoCanadaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SodexoCanadaModule],
      }).compile();
      const service = moduleRef.get(SodexoCanadaService);
      expect(service).toBeInstanceOf(SodexoCanadaService);
      await moduleRef.close();
    });

    it('exports the Site.SODEXO_CANADA = "sodexocanada" enum value', () => {
      expect(Site.SODEXO_CANADA).toBe('sodexocanada');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new SodexoCanadaService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.SODEXO_CANADA],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('SodexoCanadaLtd');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SODEXO_CANADA);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('sodexocanada-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new SodexoCanadaService(registry);
      const result = await service.scrape({
        siteType: [Site.SODEXO_CANADA],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('SodexoCanadaLtd');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('sodexocanada-x1');
      expect(result.jobs[0].site).toBe(Site.SODEXO_CANADA);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('sodexocanada-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new SodexoCanadaService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.SODEXO_CANADA],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new SodexoCanadaService();
      const result = await service.scrape({
        siteType: [Site.SODEXO_CANADA],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
