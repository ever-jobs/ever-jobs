...
import { FunXyzModule, FunXyzService } from '../src';

const COMPANY_NAME_EXPECT = 'Fun.xyz';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'funxyz-jobs.json'), 'utf8'),
);
...
describe('FunXyzService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through FunXyzModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [FunXyzModule],
      }).compile();
      const service = moduleRef.get(FunXyzService);
      expect(service).toBeInstanceOf(FunXyzService);
      await moduleRef.close();
    });

    it('exports the Site.FUN_XYZ = "funxyz" enum value', () => {
      expect(Site.FUN_XYZ).toBe('funxyz');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new FunXyzService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.FUN_XYZ],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('funxyz');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.FUN_XYZ);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('funxyz-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new FunXyzService(registry);
      const result = await service.scrape({
        siteType: [Site.FUN_XYZ],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('funxyz');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('funxyz-x1');
      expect(result.jobs[0].site).toBe(Site.FUN_XYZ);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('funxyz-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new FunXyzService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.FUN_XYZ],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new FunXyzService();
      const result = await service.scrape({
        siteType: [Site.FUN_XYZ],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
