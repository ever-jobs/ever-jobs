...
import { LogicGateModule, LogicGateService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'logicgate-jobs.json'), 'utf8'),
);
...
/**
 * Spec 879 / T04 — `LogicGateService` unit tests (≥ 9 cases).
 */
describe('LogicGateService — Spec 879 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through LogicGateModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [LogicGateModule],
      }).compile();
      const service = moduleRef.get(LogicGateService);
      expect(service).toBeInstanceOf(LogicGateService);
      await moduleRef.close();
    });

    it('exports the Site.LOGICGATE = "logicgate" enum value', () => {
      expect(Site.LOGICGATE).toBe('logicgate');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new LogicGateService();
      const result = await service.scrape({
        siteType: [Site.LOGICGATE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'logicgate-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.LOGICGATE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('LogicGate');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/logicgate/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/logicgate/jobs/');
      ...
      expect(job0?.description).toContain('LogicGate');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/logicgate/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('LogicGate');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'logicgate-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
