...
import { SolarLandscapeModule, SolarLandscapeService } from '../src';

const COMPANY_NAME_EXPECT = 'Solar Landscape';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'solarlandscape-jobs.json'), 'utf8'),
);
...
describe('SolarLandscapeService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SolarLandscapeModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SolarLandscapeModule],
      }).compile();
      const service = moduleRef.get(SolarLandscapeService);
      expect(service).toBeInstanceOf(SolarLandscapeService);
      await moduleRef.close();
    });

    it('exports the Site.SOLAR_LANDSCAPE = "solarlandscape" enum value', () => {
      expect(Site.SOLAR_LANDSCAPE).toBe('solarlandscape');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new SolarLandscapeService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.SOLAR_LANDSCAPE],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'solarlandscape-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.SOLAR_LANDSCAPE);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('solarlandscape-' + first.id);
      ...
      expect(calledUrls[0]).toContain('solarlandscape');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SOLAR_LANDSCAPE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('solarlandscape-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new SolarLandscapeService(registry);
      const result = await service.scrape({
        siteType: [Site.SOLAR_LANDSCAPE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('solarlandscape');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('solarlandscape-x1');
      expect(result.jobs[0].site).toBe(Site.SOLAR_LANDSCAPE);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('solarlandscape-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new SolarLandscapeService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.SOLAR_LANDSCAPE],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new SolarLandscapeService();
      const result = await service.scrape({
        siteType: [Site.SOLAR_LANDSCAPE],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
