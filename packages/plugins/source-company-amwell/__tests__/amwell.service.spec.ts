...
import { AmwellModule, AmwellService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'amwell-jobs.json'), 'utf8'),
);
...
/**
 * Spec 265 / T04 — `AmwellService` unit tests (≥ 9 cases).
 */
describe('AmwellService — Spec 265 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AmwellModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AmwellModule],
      }).compile();
      const service = moduleRef.get(AmwellService);
      expect(service).toBeInstanceOf(AmwellService);
      await moduleRef.close();
    });

    it('exports the Site.AMWELL = "amwell" enum value', () => {
      expect(Site.AMWELL).toBe('amwell');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AmwellService();
      const result = await service.scrape({
        siteType: [Site.AMWELL],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'amwell-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.AMWELL);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Amwell');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/amwell/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/amwell/jobs/');
      ...
      expect(job0?.description).toContain('Amwell');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/amwell/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Amwell');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'amwell-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
