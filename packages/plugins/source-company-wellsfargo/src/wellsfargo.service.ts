...
/**
 * Wells Fargo — Banking (HQ: San Francisco, CA, USA).
 *
 * Source (Spec 1736): Workday board, scraped in this order:
 *   - `wf:1:WellsFargoJobs` — https://wf.wd1.myworkdayjobs.com/WellsFargoJobs
 *     verified live 2026-09-24: 1,893 open postings.
 *
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'wf:1:WellsFargoJobs', atsIdPrefix: 'wd-wf-' },
];

@SourcePlugin({
  site: Site.WELLS_FARGO,
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['wellsfargo.com'],
  description: 'Wells Fargo careers via Workday. Tags: segment=workday-enterprise; industry=banking.',
})
@Injectable()
export class WellsFargoService implements IScraper {
  private readonly logger = new Logger(WellsFargoService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Workday source plugin is not registered; cannot scrape Wells Fargo');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`Wells Fargo: delegating to Workday (${board.companySlug})`);
    ...
    this.logger.log(`Wells Fargo: scraped ${jobs.length} jobs`);
