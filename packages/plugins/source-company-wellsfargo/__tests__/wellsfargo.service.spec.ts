...
import { WellsFargoModule, WellsFargoService } from '../src';
...
const FIXTURE: Fixture = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'fixtures', 'wellsfargo-boards.json'), 'utf8'),
);
const COMPANY_NAME = 'Wells Fargo';
const ID_PREFIX = 'wellsfargo-';
...
describe('WellsFargoService — Workday delegation (Spec 1736)', () => {
  beforeEach(() => {
    mockGet.mockReset();
    mockPost.mockReset();
  ...
  describe('registration', () => {
    it('resolves through WellsFargoModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({ imports: [WellsFargoModule] }).compile();
      expect(moduleRef.get(WellsFargoService)).toBeInstanceOf(WellsFargoService);
      await moduleRef.close();
    });

    it('exports Site.WELLS_FARGO = "wellsfargo"', () => {
      expect(Site.WELLS_FARGO).toBe('wellsfargo');
    });

    it('declares a tagged company plugin with its domains', () => {
      const meta = Reflect.getMetadata(SOURCE_PLUGIN_METADATA, WellsFargoService);
      expect(meta.site).toBe(Site.WELLS_FARGO);
      expect(meta.category).toBe('company');
      expect(meta.isAts).toBeFalsy();
      expect(meta.companyDomains).toEqual(['wellsfargo.com']);
  ...
  describe('recorded board (real Workday adapter, mocked HTTP)', () => {
    it('maps every recorded posting and re-stamps the company identity', async () => {
      const service = new WellsFargoService(registryWith());
      const result = await service.scrape({ siteType: [Site.WELLS_FARGO], resultsWanted: 100 } as ScraperInputDto);
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.WELLS_FARGO);
    ...
    it('only requests the recorded board URLs', async () => {
      const service = new WellsFargoService(registryWith());
      await service.scrape({ siteType: [Site.WELLS_FARGO] } as ScraperInputDto);
    ...
    it('honours resultsWanted=1', async () => {
      const service = new WellsFargoService(registryWith());
      const result = await service.scrape({ siteType: [Site.WELLS_FARGO], resultsWanted: 1 } as ScraperInputDto);
    ...
    it('resolves with an empty result when the board is gone (HTTP 404)', async () => {
      ...
      const service = new WellsFargoService(registryWith());
      const result = await service.scrape({ siteType: [Site.WELLS_FARGO] } as ScraperInputDto);
  ...
  describe('delegation contract', () => {
    it('forwards the board and the caller input untouched', async () => {
      ...
      const service = new WellsFargoService(
        registryWith(
          fakeBackend(
      ...
      expect(result.jobs[0].site).toBe(Site.WELLS_FARGO);
    ...
    it('never forwards the caller\'s credentials to the board (Spec 1735 §4.5)', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new WellsFargoService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
    ...
    it('makes no request when resultsWanted is 0', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new WellsFargoService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      const result = await service.scrape({ siteType: [Site.WELLS_FARGO], resultsWanted: 0 } as ScraperInputDto);
  ...
  describe('resilience', () => {
    it('reports not_registered when no registry is injected', async () => {
      const result = await new WellsFargoService().scrape({ siteType: [Site.WELLS_FARGO] } as ScraperInputDto);
      expect(result.jobs).toEqual([]);
    ...
    it('reports not_registered when Workday is missing from the registry', async () => {
      const result = await new WellsFargoService(new PluginRegistry()).scrape({
        siteType: [Site.WELLS_FARGO],
      } as ScraperInputDto);
      expect(result.diagnostics?.reason).toBe('not_registered');
    });
