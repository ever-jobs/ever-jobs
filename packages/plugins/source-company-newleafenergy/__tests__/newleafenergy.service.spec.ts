...
import { NewLeafEnergyModule, NewLeafEnergyService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'newleafenergy-jobs.json'), 'utf8'),
);
...
/**
 * Spec 892 / T04 — `NewLeafEnergyService` unit tests (≥ 9 cases).
 */
describe('NewLeafEnergyService — Spec 892 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through NewLeafEnergyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [NewLeafEnergyModule],
      }).compile();
      const service = moduleRef.get(NewLeafEnergyService);
      expect(service).toBeInstanceOf(NewLeafEnergyService);
      await moduleRef.close();
    });

    it('exports the Site.NEW_LEAF_ENERGY = "newleafenergy" enum value', () => {
      expect(Site.NEW_LEAF_ENERGY).toBe('newleafenergy');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new NewLeafEnergyService();
      const result = await service.scrape({
        siteType: [Site.NEW_LEAF_ENERGY],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'newleafenergy-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.NEW_LEAF_ENERGY);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('New Leaf Energy');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/newleafenergy/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/newleafenergy/jobs/');
      ...
      expect(job0?.description).toContain('New Leaf Energy');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/newleafenergy/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('New Leaf Energy');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'newleafenergy-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
