...
import { AEVEXAerospaceModule, AEVEXAerospaceService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'aevexaerospace-jobs.json'), 'utf8'),
);
...
/**
 * Spec 805 / T04 — `AEVEXAerospaceService` unit tests (≥ 9 cases).
 */
describe('AEVEXAerospaceService — Spec 805 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AEVEXAerospaceModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AEVEXAerospaceModule],
      }).compile();
      const service = moduleRef.get(AEVEXAerospaceService);
      expect(service).toBeInstanceOf(AEVEXAerospaceService);
      await moduleRef.close();
    });

    it('exports the Site.AEVEX_AEROSPACE = "aevexaerospace" enum value', () => {
      expect(Site.AEVEX_AEROSPACE).toBe('aevexaerospace');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AEVEXAerospaceService();
      const result = await service.scrape({
        siteType: [Site.AEVEX_AEROSPACE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'aevexaerospace-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.AEVEX_AEROSPACE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('AEVEX Aerospace');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/aevexaerospace/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/aevexaerospace/jobs/');
      ...
      expect(job0?.description).toContain('AEVEX Aerospace');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/aevexaerospace/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('AEVEX Aerospace');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'aevexaerospace-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
