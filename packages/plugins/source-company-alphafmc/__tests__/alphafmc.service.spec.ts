...
import { AlphafmcModule, AlphafmcService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'alphafmc-jobs.json'), 'utf8'),
);
...
/**
 * Spec 240 / T04 — `AlphafmcService` unit tests (≥ 9 cases).
 */
describe('AlphafmcService — Spec 240 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AlphafmcModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AlphafmcModule],
      }).compile();
      const service = moduleRef.get(AlphafmcService);
      expect(service).toBeInstanceOf(AlphafmcService);
      await moduleRef.close();
    });

    it('exports the Site.ALPHAFMC = "alphafmc" enum value', () => {
      expect(Site.ALPHAFMC).toBe('alphafmc');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AlphafmcService();
      const result = await service.scrape({
        siteType: [Site.ALPHAFMC],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'alphafmc-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ALPHAFMC);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Alpha FMC - UK ');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/alphafmc/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/alphafmc/jobs/');
      ...
      expect(job0?.description).toContain('Alpha FMC - UK ');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/alphafmc/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Alpha FMC - UK ');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'alphafmc-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
