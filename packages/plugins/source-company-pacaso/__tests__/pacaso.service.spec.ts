...
import { PacasoModule, PacasoService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'pacaso-jobs.json'), 'utf8'),
);
...
/**
 * Spec 591 / T04 — `PacasoService` unit tests (≥ 9 cases).
 */
describe('PacasoService — Spec 591 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through PacasoModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [PacasoModule],
      }).compile();
      const service = moduleRef.get(PacasoService);
      expect(service).toBeInstanceOf(PacasoService);
      await moduleRef.close();
    });

    it('exports the Site.PACASO = "pacaso" enum value', () => {
      expect(Site.PACASO).toBe('pacaso');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new PacasoService();
      const result = await service.scrape({
        siteType: [Site.PACASO],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'pacaso-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.PACASO);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Pacaso');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/pacaso/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/pacaso/jobs/');
      ...
      expect(job0?.description).toContain('Pacaso');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/pacaso/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Pacaso');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'pacaso-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
