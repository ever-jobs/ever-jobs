...
import { DriveWealthModule, DriveWealthService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'drivewealth-jobs.json'), 'utf8'),
);
...
/**
 * Spec 603 / T04 — `DriveWealthService` unit tests (≥ 9 cases).
 */
describe('DriveWealthService — Spec 603 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through DriveWealthModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [DriveWealthModule],
      }).compile();
      const service = moduleRef.get(DriveWealthService);
      expect(service).toBeInstanceOf(DriveWealthService);
      await moduleRef.close();
    });

    it('exports the Site.DRIVEWEALTH = "drivewealth" enum value', () => {
      expect(Site.DRIVEWEALTH).toBe('drivewealth');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new DriveWealthService();
      const result = await service.scrape({
        siteType: [Site.DRIVEWEALTH],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'drivewealth-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.DRIVEWEALTH);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('DriveWealth');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/drivewealth/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/drivewealth/jobs/');
      ...
      expect(job0?.description).toContain('DriveWealth');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/drivewealth/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('DriveWealth');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'drivewealth-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
