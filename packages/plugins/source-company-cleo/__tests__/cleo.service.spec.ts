...
import { CleoAIModule, CleoAIService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'cleo-jobs.json'), 'utf8'),
);
...
/**
 * Spec 686 / T04 — `CleoAIService` unit tests (≥ 9 cases).
 */
describe('CleoAIService — Spec 686 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CleoAIModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CleoAIModule],
      }).compile();
      const service = moduleRef.get(CleoAIService);
      expect(service).toBeInstanceOf(CleoAIService);
      await moduleRef.close();
    });

    it('exports the Site.CLEO_AI = "cleo" enum value', () => {
      expect(Site.CLEO_AI).toBe('cleo');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new CleoAIService();
      const result = await service.scrape({
        siteType: [Site.CLEO_AI],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'cleo-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.CLEO_AI);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Cleo AI');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/cleo/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/cleo/jobs/');
      ...
      expect(job0?.description).toContain('Cleo AI');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/cleo/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Cleo AI');
      }
    });
  });
