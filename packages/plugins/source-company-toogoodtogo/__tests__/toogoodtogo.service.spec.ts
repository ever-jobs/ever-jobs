...
import { TooGoodToGoModule, TooGoodToGoService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'toogoodtogo-jobs.json'), 'utf8'),
);
...
/**
 * Spec 957 / T04 — `TooGoodToGoService` unit tests (≥ 9 cases).
 */
describe('TooGoodToGoService — Spec 957 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through TooGoodToGoModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [TooGoodToGoModule],
      }).compile();
      const service = moduleRef.get(TooGoodToGoService);
      expect(service).toBeInstanceOf(TooGoodToGoService);
      await moduleRef.close();
    });

    it('exports the Site.TOO_GOOD_TO_GO = "toogoodtogo" enum value', () => {
      expect(Site.TOO_GOOD_TO_GO).toBe('toogoodtogo');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new TooGoodToGoService();
      const result = await service.scrape({
        siteType: [Site.TOO_GOOD_TO_GO],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'toogoodtogo-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.TOO_GOOD_TO_GO);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Too Good To Go');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/toogoodtogo/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/toogoodtogo/jobs/');
      ...
      expect(job0?.description).toContain('Too Good To Go');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/toogoodtogo/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Too Good To Go');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'toogoodtogo-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
