...
import { CarbonRoboticsModule, CarbonRoboticsService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'carbonrobotics-jobs.json'), 'utf8'),
);
...
/**
 * Spec 772 / T04 — `CarbonRoboticsService` unit tests (≥ 9 cases).
 */
describe('CarbonRoboticsService — Spec 772 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CarbonRoboticsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CarbonRoboticsModule],
      }).compile();
      const service = moduleRef.get(CarbonRoboticsService);
      expect(service).toBeInstanceOf(CarbonRoboticsService);
      await moduleRef.close();
    });

    it('exports the Site.CARBON_ROBOTICS = "carbonrobotics" enum value', () => {
      expect(Site.CARBON_ROBOTICS).toBe('carbonrobotics');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new CarbonRoboticsService();
      const result = await service.scrape({
        siteType: [Site.CARBON_ROBOTICS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'carbonrobotics-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.CARBON_ROBOTICS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Carbon Robotics');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/carbonrobotics/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/carbonrobotics/jobs/');
      ...
      expect(job0?.description).toContain('Carbon Robotics');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/carbonrobotics/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Carbon Robotics');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'carbonrobotics-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
