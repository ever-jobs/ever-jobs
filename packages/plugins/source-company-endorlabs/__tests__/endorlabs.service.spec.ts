...
import { EndorlabsModule, EndorlabsService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'endorlabs-jobs.json'), 'utf8'),
);
...
/**
 * Spec 540 / T04 — `EndorlabsService` unit tests (≥ 9 cases).
 */
describe('EndorlabsService — Spec 540 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through EndorlabsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [EndorlabsModule],
      }).compile();
      const service = moduleRef.get(EndorlabsService);
      expect(service).toBeInstanceOf(EndorlabsService);
      await moduleRef.close();
    });

    it('exports the Site.ENDOR_LABS = "endorlabs" enum value', () => {
      expect(Site.ENDOR_LABS).toBe('endorlabs');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new EndorlabsService();
      const result = await service.scrape({
        siteType: [Site.ENDOR_LABS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'endorlabs-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ENDOR_LABS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Endor Labs');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/endorlabs/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/endorlabs/jobs/');
      ...
      expect(job0?.description).toContain('Endor Labs');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/endorlabs/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Endor Labs');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'endorlabs-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
