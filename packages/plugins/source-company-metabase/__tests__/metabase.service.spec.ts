...
import { MetabaseModule, MetabaseService } from '../src';

const COMPANY_NAME_EXPECT = 'Metabase';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'metabase-jobs.json'), 'utf8'),
);
...
describe('MetabaseService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through MetabaseModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [MetabaseModule],
      }).compile();
      const service = moduleRef.get(MetabaseService);
      expect(service).toBeInstanceOf(MetabaseService);
      await moduleRef.close();
    });

    it('exports the Site.METABASE = "metabase" enum value', () => {
      expect(Site.METABASE).toBe('metabase');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new MetabaseService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.METABASE],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('metabase');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.METABASE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('metabase-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new MetabaseService(registry);
      const result = await service.scrape({
        siteType: [Site.METABASE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('metabase');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('metabase-x1');
      expect(result.jobs[0].site).toBe(Site.METABASE);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('metabase-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new MetabaseService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.METABASE],
      } as ScraperInputDto);
