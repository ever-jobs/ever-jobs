...
import { EcoVadisModule, EcoVadisService } from '../src';

const COMPANY_NAME_EXPECT = 'EcoVadis';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'ecovadis-jobs.json'), 'utf8'),
);
...
describe('EcoVadisService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through EcoVadisModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [EcoVadisModule],
      }).compile();
      const service = moduleRef.get(EcoVadisService);
      expect(service).toBeInstanceOf(EcoVadisService);
      await moduleRef.close();
    });

    it('exports the Site.ECOVADIS = "ecovadis" enum value', () => {
      expect(Site.ECOVADIS).toBe('ecovadis');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new EcoVadisService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.ECOVADIS],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('ecovadis');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ECOVADIS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('ecovadis-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new EcoVadisService(registry);
      const result = await service.scrape({
        siteType: [Site.ECOVADIS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('ecovadis');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('ecovadis-x1');
      expect(result.jobs[0].site).toBe(Site.ECOVADIS);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('ecovadis-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new EcoVadisService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.ECOVADIS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new EcoVadisService();
      const result = await service.scrape({
        siteType: [Site.ECOVADIS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
