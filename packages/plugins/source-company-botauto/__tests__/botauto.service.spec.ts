...
import { BotAutoModule, BotAutoService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'botauto-jobs.json'), 'utf8'),
);
...
/**
 * Spec 818 / T04 — `BotAutoService` unit tests (≥ 9 cases).
 */
describe('BotAutoService — Spec 818 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through BotAutoModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [BotAutoModule],
      }).compile();
      const service = moduleRef.get(BotAutoService);
      expect(service).toBeInstanceOf(BotAutoService);
      await moduleRef.close();
    });

    it('exports the Site.BOT_AUTO = "botauto" enum value', () => {
      expect(Site.BOT_AUTO).toBe('botauto');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new BotAutoService();
      const result = await service.scrape({
        siteType: [Site.BOT_AUTO],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'botauto-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.BOT_AUTO);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Bot Auto');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/botauto/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/botauto/jobs/');
      ...
      expect(job0?.description).toContain('Bot Auto');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/botauto/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Bot Auto');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'botauto-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
