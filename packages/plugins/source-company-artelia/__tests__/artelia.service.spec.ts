...
import { ArteliaModule, ArteliaService } from '../src';

const COMPANY_NAME_EXPECT = 'Artelia';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'artelia-jobs.json'), 'utf8'),
);
...
describe('ArteliaService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ArteliaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ArteliaModule],
      }).compile();
      const service = moduleRef.get(ArteliaService);
      expect(service).toBeInstanceOf(ArteliaService);
      await moduleRef.close();
    });

    it('exports the Site.ARTELIA = "artelia" enum value', () => {
      expect(Site.ARTELIA).toBe('artelia');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new ArteliaService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.ARTELIA],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('Artelia');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ARTELIA);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('artelia-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new ArteliaService(registry);
      const result = await service.scrape({
        siteType: [Site.ARTELIA],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('Artelia');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('artelia-x1');
      expect(result.jobs[0].site).toBe(Site.ARTELIA);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('artelia-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new ArteliaService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.ARTELIA],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new ArteliaService();
      const result = await service.scrape({
        siteType: [Site.ARTELIA],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
