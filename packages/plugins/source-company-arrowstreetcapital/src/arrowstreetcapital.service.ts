...
/**
 * Arrowstreet Capital — Quantitative investment management (HQ: Boston, MA, USA).
 *
 * Source (Spec 1737): Workday board, scraped in this order:
 *   - `arrowstreetcapital:5:Arrowstreet` — https://arrowstreetcapital.wd5.myworkdayjobs.com/Arrowstreet
 *     verified live 2026-09-24: 23 open postings.
 *
 ...
 * party board.
 *
 * Tags: segment=quant-trading; industry=quantitative-investment-management.
 */
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'arrowstreetcapital:5:Arrowstreet', atsIdPrefix: 'wd-arrowstreetcapital-' },
];
...
@SourcePlugin({
  site: Site.ARROWSTREET_CAPITAL,
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['arrowstreetcapital.com'],
  description: 'Arrowstreet Capital careers via Workday. Tags: segment=quant-trading; industry=quantitative-investment-management.',
})
@Injectable()
export class ArrowstreetCapitalService implements IScraper {
  private readonly logger = new Logger(ArrowstreetCapitalService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Workday source plugin is not registered; cannot scrape Arrowstreet Capital');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`Arrowstreet Capital: delegating to Workday (${board.companySlug})`);
      ...
      for (const job of result.jobs ?? []) {
        job.site = Site.ARROWSTREET_CAPITAL;
    ...
    this.logger.log(`Arrowstreet Capital: scraped ${jobs.length} jobs`);
