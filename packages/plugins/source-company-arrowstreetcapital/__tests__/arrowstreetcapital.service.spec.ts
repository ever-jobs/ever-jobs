...
import { ArrowstreetCapitalModule, ArrowstreetCapitalService } from '../src';
...
const FIXTURE: Fixture = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'fixtures', 'arrowstreetcapital-boards.json'), 'utf8'),
);
const COMPANY_NAME = 'Arrowstreet Capital';
const ID_PREFIX = 'arrowstreetcapital-';
...
describe('ArrowstreetCapitalService — Workday delegation (Spec 1737)', () => {
  beforeEach(() => {
    mockGet.mockReset();
    mockPost.mockReset();
  ...
  describe('registration', () => {
    it('resolves through ArrowstreetCapitalModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({ imports: [ArrowstreetCapitalModule] }).compile();
      expect(moduleRef.get(ArrowstreetCapitalService)).toBeInstanceOf(ArrowstreetCapitalService);
      await moduleRef.close();
    });

    it('exports Site.ARROWSTREET_CAPITAL = "arrowstreetcapital"', () => {
      expect(Site.ARROWSTREET_CAPITAL).toBe('arrowstreetcapital');
    });

    it('declares a tagged company plugin with its domains', () => {
      const meta = Reflect.getMetadata(SOURCE_PLUGIN_METADATA, ArrowstreetCapitalService);
      expect(meta.site).toBe(Site.ARROWSTREET_CAPITAL);
      expect(meta.category).toBe('company');
      expect(meta.isAts).toBeFalsy();
      expect(meta.companyDomains).toEqual(['arrowstreetcapital.com']);
  ...
  describe('recorded board (real Workday adapter, mocked HTTP)', () => {
    it('maps every recorded posting and re-stamps the company identity', async () => {
      const service = new ArrowstreetCapitalService(registryWith());
      const result = await service.scrape({ siteType: [Site.ARROWSTREET_CAPITAL], resultsWanted: 100 } as ScraperInputDto);
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ARROWSTREET_CAPITAL);
    ...
    it('only requests the recorded board URLs', async () => {
      const service = new ArrowstreetCapitalService(registryWith());
      await service.scrape({ siteType: [Site.ARROWSTREET_CAPITAL] } as ScraperInputDto);
    ...
    it('honours resultsWanted=1', async () => {
      const service = new ArrowstreetCapitalService(registryWith());
      const result = await service.scrape({ siteType: [Site.ARROWSTREET_CAPITAL], resultsWanted: 1 } as ScraperInputDto);
    ...
    it('resolves with an empty result when the board is gone (HTTP 404)', async () => {
      ...
      const service = new ArrowstreetCapitalService(registryWith());
      const result = await service.scrape({ siteType: [Site.ARROWSTREET_CAPITAL] } as ScraperInputDto);
  ...
  describe('delegation contract', () => {
    it('forwards the board and the caller input untouched', async () => {
      ...
      const service = new ArrowstreetCapitalService(
        registryWith(
          fakeBackend(
      ...
      const result = await service.scrape({
        siteType: [Site.ARROWSTREET_CAPITAL],
      ...
      expect(result.jobs[0].site).toBe(Site.ARROWSTREET_CAPITAL);
      expect(result.jobs[0].companyName).toBe(COMPANY_NAME);
    });

    it('passes an absent resultsWanted through as absent', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new ArrowstreetCapitalService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      await service.scrape({ siteType: [Site.ARROWSTREET_CAPITAL] } as ScraperInputDto);
    ...
    it('makes no request when resultsWanted is 0', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new ArrowstreetCapitalService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      const result = await service.scrape({ siteType: [Site.ARROWSTREET_CAPITAL], resultsWanted: 0 } as ScraperInputDto);
  ...
  describe('resilience', () => {
    it('reports not_registered when no registry is injected', async () => {
      const result = await new ArrowstreetCapitalService().scrape({ siteType: [Site.ARROWSTREET_CAPITAL] } as ScraperInputDto);
      expect(result.jobs).toEqual([]);
    ...
    it('reports not_registered when Workday is missing from the registry', async () => {
      const result = await new ArrowstreetCapitalService(new PluginRegistry()).scrape({
        siteType: [Site.ARROWSTREET_CAPITAL],
      } as ScraperInputDto);
      expect(result.diagnostics?.reason).toBe('not_registered');
    });
