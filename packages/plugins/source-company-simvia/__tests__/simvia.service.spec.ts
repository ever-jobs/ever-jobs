...
import { SimviaModule, SimviaService } from '../src';

const COMPANY_NAME_EXPECT = 'Simvia';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'simvia-jobs.json'), 'utf8'),
);
...
describe('SimviaService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SimviaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SimviaModule],
      }).compile();
      const service = moduleRef.get(SimviaService);
      expect(service).toBeInstanceOf(SimviaService);
      await moduleRef.close();
    });

    it('exports the Site.SIMVIA = "simvia" enum value', () => {
      expect(Site.SIMVIA).toBe('simvia');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new SimviaService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.SIMVIA],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('simvia.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SIMVIA);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('simvia-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new SimviaService(registry);
      const result = await service.scrape({
        siteType: [Site.SIMVIA],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('simvia');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('simvia-x1');
      expect(result.jobs[0].site).toBe(Site.SIMVIA);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('simvia-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new SimviaService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.SIMVIA],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new SimviaService();
      const result = await service.scrape({
        siteType: [Site.SIMVIA],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
