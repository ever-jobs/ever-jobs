...
import { NarvarModule, NarvarService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'narvar-jobs.json'), 'utf8'),
);
...
/**
 * Spec 499 / T04 — `NarvarService` unit tests (≥ 9 cases).
 */
describe('NarvarService — Spec 499 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through NarvarModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [NarvarModule],
      }).compile();
      const service = moduleRef.get(NarvarService);
      expect(service).toBeInstanceOf(NarvarService);
      await moduleRef.close();
    });

    it('exports the Site.NARVAR = "narvar" enum value', () => {
      expect(Site.NARVAR).toBe('narvar');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new NarvarService();
      const result = await service.scrape({
        siteType: [Site.NARVAR],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'narvar-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.NARVAR);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Narvar');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/narvar/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/narvar/jobs/');
      ...
      expect(job0?.description).toContain('Narvar');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/narvar/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Narvar');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'narvar-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
