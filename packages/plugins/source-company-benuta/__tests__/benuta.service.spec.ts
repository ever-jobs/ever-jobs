...
import { BenutaModule, BenutaService } from '../src';

const COMPANY_NAME_EXPECT = 'benuta';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'benuta-jobs.json'), 'utf8'),
);
...
describe('BenutaService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through BenutaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [BenutaModule],
      }).compile();
      const service = moduleRef.get(BenutaService);
      expect(service).toBeInstanceOf(BenutaService);
      await moduleRef.close();
    });

    it('exports the Site.BENUTA = "benuta" enum value', () => {
      expect(Site.BENUTA).toBe('benuta');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new BenutaService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.BENUTA],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('benutagmbh.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.BENUTA);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('benuta-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new BenutaService(registry);
      const result = await service.scrape({
        siteType: [Site.BENUTA],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('benutagmbh');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('benuta-x1');
      expect(result.jobs[0].site).toBe(Site.BENUTA);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('benuta-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new BenutaService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.BENUTA],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new BenutaService();
      const result = await service.scrape({
        siteType: [Site.BENUTA],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
