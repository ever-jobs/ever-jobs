...
import { OfferUpModule, OfferUpService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'offerup-jobs.json'), 'utf8'),
);
...
/**
 * Spec 752 / T04 — `OfferUpService` unit tests (≥ 9 cases).
 */
describe('OfferUpService — Spec 752 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through OfferUpModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [OfferUpModule],
      }).compile();
      const service = moduleRef.get(OfferUpService);
      expect(service).toBeInstanceOf(OfferUpService);
      await moduleRef.close();
    });

    it('exports the Site.OFFERUP = "offerup" enum value', () => {
      expect(Site.OFFERUP).toBe('offerup');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new OfferUpService();
      const result = await service.scrape({
        siteType: [Site.OFFERUP],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'offerup-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.OFFERUP);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('OfferUp');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/offerup/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/offerup/jobs/');
      ...
      expect(job0?.description).toContain('OfferUp');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/offerup/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('OfferUp');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'offerup-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
