...
/**
 * Old Mission — Proprietary trading (HQ: Chicago, IL, USA).
 *
 * Source (Spec 1737): Greenhouse board, scraped in this order:
 *   - `oldmissioncapital` — https://job-boards.greenhouse.io/oldmissioncapital
 *     verified live 2026-09-24: 37 open postings.
 *
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'oldmissioncapital', atsIdPrefix: 'gh-' },
];

@SourcePlugin({
  site: Site.OLD_MISSION,
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['oldmissioncapital.com'],
  description: 'Old Mission careers via Greenhouse. Tags: segment=quant-trading; industry=proprietary-trading.',
})
@Injectable()
export class OldMissionService implements IScraper {
  private readonly logger = new Logger(OldMissionService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Greenhouse source plugin is not registered; cannot scrape Old Mission');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`Old Mission: delegating to Greenhouse (${board.companySlug})`);
    ...
    this.logger.log(`Old Mission: scraped ${jobs.length} jobs`);
