...
import { CustomizedEnergySolutionsModule, CustomizedEnergySolutionsService } from '../src';

const COMPANY_NAME_EXPECT = 'Customized Energy Solutions';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'customizedenergysolutions-jobs.json'), 'utf8'),
);
...
describe('CustomizedEnergySolutionsService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CustomizedEnergySolutionsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CustomizedEnergySolutionsModule],
      }).compile();
      const service = moduleRef.get(CustomizedEnergySolutionsService);
      expect(service).toBeInstanceOf(CustomizedEnergySolutionsService);
      await moduleRef.close();
    });

    it('exports the Site.CUSTOMIZED_ENERGY_SOLUTIONS = "customizedenergysolutions" enum value', () => {
      expect(Site.CUSTOMIZED_ENERGY_SOLUTIONS).toBe('customizedenergysolutions');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new CustomizedEnergySolutionsService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.CUSTOMIZED_ENERGY_SOLUTIONS],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'customizedenergysolutions-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.CUSTOMIZED_ENERGY_SOLUTIONS);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('customizedenergysolutions-' + first.id);
      ...
      expect(calledUrls[0]).toContain('CustomizedEnergySolutions');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.CUSTOMIZED_ENERGY_SOLUTIONS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('customizedenergysolutions-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new CustomizedEnergySolutionsService(registry);
      const result = await service.scrape({
        siteType: [Site.CUSTOMIZED_ENERGY_SOLUTIONS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('CustomizedEnergySolutions');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('customizedenergysolutions-x1');
      expect(result.jobs[0].site).toBe(Site.CUSTOMIZED_ENERGY_SOLUTIONS);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('customizedenergysolutions-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new CustomizedEnergySolutionsService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.CUSTOMIZED_ENERGY_SOLUTIONS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new CustomizedEnergySolutionsService();
      const result = await service.scrape({
        siteType: [Site.CUSTOMIZED_ENERGY_SOLUTIONS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
