...
import { ECLModule, ECLService } from '../src';

const COMPANY_NAME_EXPECT = 'ECL';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'ecldc-jobs.json'), 'utf8'),
);
...
describe('ECLService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ECLModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ECLModule],
      }).compile();
      const service = moduleRef.get(ECLService);
      expect(service).toBeInstanceOf(ECLService);
      await moduleRef.close();
    });

    it('exports the Site.ECL = "ecldc" enum value', () => {
      expect(Site.ECL).toBe('ecldc');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new ECLService(registryWithLever());
      ...
      expect(calledUrls[0]).toContain('ecldc');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ECL);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('ecldc-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new ECLService(registry);
      const result = await service.scrape({
        siteType: [Site.ECL],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('ecldc');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('ecldc-x1');
      expect(result.jobs[0].site).toBe(Site.ECL);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('ecldc-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new ECLService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.ECL],
      } as ScraperInputDto);
