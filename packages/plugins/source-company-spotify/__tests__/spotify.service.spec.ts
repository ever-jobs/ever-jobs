...
import { SpotifyModule, SpotifyService } from '../src';

const COMPANY_NAME_EXPECT = 'Spotify';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'spotify-jobs.json'), 'utf8'),
);
...
describe('SpotifyService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SpotifyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SpotifyModule],
      }).compile();
      const service = moduleRef.get(SpotifyService);
      expect(service).toBeInstanceOf(SpotifyService);
      await moduleRef.close();
    });

    it('exports the Site.SPOTIFY = "spotify" enum value', () => {
      expect(Site.SPOTIFY).toBe('spotify');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new SpotifyService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.SPOTIFY],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('spotify');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SPOTIFY);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('spotify-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new SpotifyService(registry);
      const result = await service.scrape({
        siteType: [Site.SPOTIFY],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('spotify');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('spotify-x1');
      expect(result.jobs[0].site).toBe(Site.SPOTIFY);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('spotify-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new SpotifyService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.SPOTIFY],
      } as ScraperInputDto);
