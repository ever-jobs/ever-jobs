...
import { IntersectPowerModule, IntersectPowerService } from '../src';

const COMPANY_NAME_EXPECT = 'Intersect Power';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'intersect-jobs.json'), 'utf8'),
);
...
describe('IntersectPowerService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through IntersectPowerModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [IntersectPowerModule],
      }).compile();
      const service = moduleRef.get(IntersectPowerService);
      expect(service).toBeInstanceOf(IntersectPowerService);
      await moduleRef.close();
    });

    it('exports the Site.INTERSECT_POWER = "intersect" enum value', () => {
      expect(Site.INTERSECT_POWER).toBe('intersect');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new IntersectPowerService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.INTERSECT_POWER],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'intersect-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.INTERSECT_POWER);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('intersect-' + first.id);
      ...
      expect(calledUrls[0]).toContain('intersect');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.INTERSECT_POWER);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('intersect-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new IntersectPowerService(registry);
      const result = await service.scrape({
        siteType: [Site.INTERSECT_POWER],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('intersect');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('intersect-x1');
      expect(result.jobs[0].site).toBe(Site.INTERSECT_POWER);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('intersect-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new IntersectPowerService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.INTERSECT_POWER],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new IntersectPowerService();
      const result = await service.scrape({
        siteType: [Site.INTERSECT_POWER],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
