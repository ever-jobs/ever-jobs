...
import { UniversityOfTheWestOfEnglandModule, UniversityOfTheWestOfEnglandService } from '../src';

const COMPANY_NAME_EXPECT = 'University of the West of England';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'universityofthewestofengland-jobs.json'), 'utf8'),
);
...
describe('UniversityOfTheWestOfEnglandService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through UniversityOfTheWestOfEnglandModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [UniversityOfTheWestOfEnglandModule],
      }).compile();
      const service = moduleRef.get(UniversityOfTheWestOfEnglandService);
      expect(service).toBeInstanceOf(UniversityOfTheWestOfEnglandService);
      await moduleRef.close();
    });

    it('exports the Site.UNIVERSITY_OF_THE_WEST_OF_ENGLAND = "universityofthewestofengland" enum value', () => {
      expect(Site.UNIVERSITY_OF_THE_WEST_OF_ENGLAND).toBe('universityofthewestofengland');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new UniversityOfTheWestOfEnglandService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.UNIVERSITY_OF_THE_WEST_OF_ENGLAND],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'universityofthewestofengland-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.UNIVERSITY_OF_THE_WEST_OF_ENGLAND);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('universityofthewestofengland-' + first.id);
      ...
      expect(calledUrls[0]).toContain('UniversityOfTheWestOfEngland');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.UNIVERSITY_OF_THE_WEST_OF_ENGLAND);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('universityofthewestofengland-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new UniversityOfTheWestOfEnglandService(registry);
      const result = await service.scrape({
        siteType: [Site.UNIVERSITY_OF_THE_WEST_OF_ENGLAND],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('UniversityOfTheWestOfEngland');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('universityofthewestofengland-x1');
      expect(result.jobs[0].site).toBe(Site.UNIVERSITY_OF_THE_WEST_OF_ENGLAND);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('universityofthewestofengland-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new UniversityOfTheWestOfEnglandService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.UNIVERSITY_OF_THE_WEST_OF_ENGLAND],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new UniversityOfTheWestOfEnglandService();
      const result = await service.scrape({
        siteType: [Site.UNIVERSITY_OF_THE_WEST_OF_ENGLAND],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
