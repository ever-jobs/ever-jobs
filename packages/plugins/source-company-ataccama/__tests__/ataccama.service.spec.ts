...
import { AtaccamaModule, AtaccamaService } from '../src';

const COMPANY_NAME_EXPECT = 'Ataccama';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'ataccama-jobs.json'), 'utf8'),
);
...
describe('AtaccamaService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AtaccamaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AtaccamaModule],
      }).compile();
      const service = moduleRef.get(AtaccamaService);
      expect(service).toBeInstanceOf(AtaccamaService);
      await moduleRef.close();
    });

    it('exports the Site.ATACCAMA = "ataccama" enum value', () => {
      expect(Site.ATACCAMA).toBe('ataccama');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new AtaccamaService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.ATACCAMA],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('ataccama');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ATACCAMA);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('ataccama-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new AtaccamaService(registry);
      const result = await service.scrape({
        siteType: [Site.ATACCAMA],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('ataccama');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('ataccama-x1');
      expect(result.jobs[0].site).toBe(Site.ATACCAMA);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('ataccama-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new AtaccamaService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.ATACCAMA],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new AtaccamaService();
      const result = await service.scrape({
        siteType: [Site.ATACCAMA],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
