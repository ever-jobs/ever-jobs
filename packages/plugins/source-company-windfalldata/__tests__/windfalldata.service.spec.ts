...
import { WindfallModule, WindfallService } from '../src';

const COMPANY_NAME_EXPECT = 'Windfall';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'windfalldata-jobs.json'), 'utf8'),
);
...
describe('WindfallService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through WindfallModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [WindfallModule],
      }).compile();
      const service = moduleRef.get(WindfallService);
      expect(service).toBeInstanceOf(WindfallService);
      await moduleRef.close();
    });

    it('exports the Site.WINDFALL = "windfalldata" enum value', () => {
      expect(Site.WINDFALL).toBe('windfalldata');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new WindfallService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.WINDFALL],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'windfalldata-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.WINDFALL);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('windfalldata-' + first.id);
      ...
      expect(calledUrls[0]).toContain('windfalldata');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.WINDFALL);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('windfalldata-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new WindfallService(registry);
      const result = await service.scrape({
        siteType: [Site.WINDFALL],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('windfalldata');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('windfalldata-x1');
      expect(result.jobs[0].site).toBe(Site.WINDFALL);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('windfalldata-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new WindfallService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.WINDFALL],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new WindfallService();
      const result = await service.scrape({
        siteType: [Site.WINDFALL],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
