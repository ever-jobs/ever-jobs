...
import { GovTechSingaporeGovernmentTechnologyAgencyModule, GovTechSingaporeGovernmentTechnologyAgencyService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'govtech-jobs.json'), 'utf8'),
);
...
/**
 * Spec 860 / T04 — `GovTechSingaporeGovernmentTechnologyAgencyService` unit tests (≥ 9 cases).
 */
describe('GovTechSingaporeGovernmentTechnologyAgencyService — Spec 860 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through GovTechSingaporeGovernmentTechnologyAgencyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [GovTechSingaporeGovernmentTechnologyAgencyModule],
      }).compile();
      const service = moduleRef.get(GovTechSingaporeGovernmentTechnologyAgencyService);
      expect(service).toBeInstanceOf(GovTechSingaporeGovernmentTechnologyAgencyService);
      await moduleRef.close();
    });

    it('exports the Site.GOVTECH_SINGAPORE_GOVERNMENT_TECHNOLOGY_AGENCY = "govtech" enum value', () => {
      expect(Site.GOVTECH_SINGAPORE_GOVERNMENT_TECHNOLOGY_AGENCY).toBe('govtech');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new GovTechSingaporeGovernmentTechnologyAgencyService();
      const result = await service.scrape({
        siteType: [Site.GOVTECH_SINGAPORE_GOVERNMENT_TECHNOLOGY_AGENCY],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'govtech-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.GOVTECH_SINGAPORE_GOVERNMENT_TECHNOLOGY_AGENCY);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('GovTech Singapore (Government Technology Agency)');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/govtech/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/govtech/jobs/');
      ...
      expect(job0?.description).toContain('GovTech Singapore (Government Technology Agency)');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/govtech/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('GovTech Singapore (Government Technology Agency)');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'govtech-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
