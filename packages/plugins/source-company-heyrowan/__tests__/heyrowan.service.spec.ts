...
import { RowanModule, RowanService } from '../src';

const COMPANY_NAME_EXPECT = 'Rowan';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'heyrowan-jobs.json'), 'utf8'),
);
...
describe('RowanService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through RowanModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [RowanModule],
      }).compile();
      const service = moduleRef.get(RowanService);
      expect(service).toBeInstanceOf(RowanService);
      await moduleRef.close();
    });

    it('exports the Site.ROWAN = "heyrowan" enum value', () => {
      expect(Site.ROWAN).toBe('heyrowan');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new RowanService(registryWithLever());
      ...
      expect(calledUrls[0]).toContain('heyrowan');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ROWAN);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('heyrowan-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new RowanService(registry);
      const result = await service.scrape({
        siteType: [Site.ROWAN],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('heyrowan');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('heyrowan-x1');
      expect(result.jobs[0].site).toBe(Site.ROWAN);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('heyrowan-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new RowanService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.ROWAN],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new RowanService();
      const result = await service.scrape({
        siteType: [Site.ROWAN],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
