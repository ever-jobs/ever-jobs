...
import { HadrianModule, HadrianService } from '../src';

const COMPANY_NAME_EXPECT = 'Hadrian';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'hadrian-jobs.json'), 'utf8'),
);
...
describe('HadrianService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through HadrianModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [HadrianModule],
      }).compile();
      const service = moduleRef.get(HadrianService);
      expect(service).toBeInstanceOf(HadrianService);
      await moduleRef.close();
    });

    it('exports the Site.HADRIAN = "hadrian" enum value', () => {
      expect(Site.HADRIAN).toBe('hadrian');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new HadrianService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.HADRIAN],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('hadrian-automation');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.HADRIAN);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('hadrian-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new HadrianService(registry);
      const result = await service.scrape({
        siteType: [Site.HADRIAN],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('hadrian-automation');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('hadrian-x1');
      expect(result.jobs[0].site).toBe(Site.HADRIAN);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('hadrian-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new HadrianService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.HADRIAN],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new HadrianService();
      const result = await service.scrape({
        siteType: [Site.HADRIAN],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
