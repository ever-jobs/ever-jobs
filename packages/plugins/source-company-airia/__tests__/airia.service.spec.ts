...
import { AiriaModule, AiriaService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'airia-jobs.json'), 'utf8'),
);
...
/**
 * Spec 214 / T04 — `AiriaService` unit tests (≥ 9 cases).
 */
describe('AiriaService — Spec 214 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AiriaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AiriaModule],
      }).compile();
      const service = moduleRef.get(AiriaService);
      expect(service).toBeInstanceOf(AiriaService);
      await moduleRef.close();
    });

    it('exports the Site.AIRIA = "airia" enum value', () => {
      expect(Site.AIRIA).toBe('airia');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AiriaService();
      ...
      const job0 = dto.jobs.find((j) => j.id === 'airia-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.AIRIA);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Airia');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/airia/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/airia/jobs/');
      ...
      expect(job0?.description).toContain('Airia');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/airia/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Airia');
      }
    });
  });
