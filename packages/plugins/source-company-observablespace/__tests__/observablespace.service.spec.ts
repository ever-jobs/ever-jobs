...
import { ObservableSpaceModule, ObservableSpaceService } from '../src';

const COMPANY_NAME_EXPECT = 'Observable Space';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'observablespace-jobs.json'), 'utf8'),
);
...
describe('ObservableSpaceService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ObservableSpaceModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ObservableSpaceModule],
      }).compile();
      const service = moduleRef.get(ObservableSpaceService);
      expect(service).toBeInstanceOf(ObservableSpaceService);
      await moduleRef.close();
    });

    it('exports the Site.OBSERVABLE_SPACE = "observablespace" enum value', () => {
      expect(Site.OBSERVABLE_SPACE).toBe('observablespace');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new ObservableSpaceService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.OBSERVABLE_SPACE],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'observablespace-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.OBSERVABLE_SPACE);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('observablespace-' + first.id);
      ...
      expect(calledUrls[0]).toContain('observable-space');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.OBSERVABLE_SPACE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('observablespace-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new ObservableSpaceService(registry);
      const result = await service.scrape({
        siteType: [Site.OBSERVABLE_SPACE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('observable-space');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('observablespace-x1');
      expect(result.jobs[0].site).toBe(Site.OBSERVABLE_SPACE);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('observablespace-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new ObservableSpaceService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.OBSERVABLE_SPACE],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new ObservableSpaceService();
      const result = await service.scrape({
        siteType: [Site.OBSERVABLE_SPACE],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
