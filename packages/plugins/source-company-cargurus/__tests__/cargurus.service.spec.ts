...
import { CarGurusModule, CarGurusService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'cargurus-jobs.json'), 'utf8'),
);
...
/**
 * Spec 594 / T04 — `CarGurusService` unit tests (≥ 9 cases).
 */
describe('CarGurusService — Spec 594 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CarGurusModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CarGurusModule],
      }).compile();
      const service = moduleRef.get(CarGurusService);
      expect(service).toBeInstanceOf(CarGurusService);
      await moduleRef.close();
    });

    it('exports the Site.CARGURUS = "cargurus" enum value', () => {
      expect(Site.CARGURUS).toBe('cargurus');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new CarGurusService();
      const result = await service.scrape({
        siteType: [Site.CARGURUS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'cargurus-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.CARGURUS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('CarGurus');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/cargurus/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/cargurus/jobs/');
      ...
      expect(job0?.description).toContain('CarGurus');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/cargurus/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('CarGurus');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'cargurus-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
