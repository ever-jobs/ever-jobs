...
import { LightmatterModule, LightmatterService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'lightmatter-jobs.json'), 'utf8'),
);
...
/**
 * Spec 702 / T04 — `LightmatterService` unit tests (≥ 9 cases).
 */
describe('LightmatterService — Spec 702 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through LightmatterModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [LightmatterModule],
      }).compile();
      const service = moduleRef.get(LightmatterService);
      expect(service).toBeInstanceOf(LightmatterService);
      await moduleRef.close();
    });

    it('exports the Site.LIGHTMATTER = "lightmatter" enum value', () => {
      expect(Site.LIGHTMATTER).toBe('lightmatter');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new LightmatterService();
      const result = await service.scrape({
        siteType: [Site.LIGHTMATTER],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'lightmatter-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.LIGHTMATTER);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Lightmatter');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/lightmatter/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/lightmatter/jobs/');
      ...
      expect(job0?.description).toContain('Lightmatter');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/lightmatter/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Lightmatter');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'lightmatter-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
