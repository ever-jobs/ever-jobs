...
import { InflectionaiModule, InflectionaiService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'inflectionai-jobs.json'), 'utf8'),
);
...
/**
 * Spec 480 / T04 — `InflectionaiService` unit tests (≥ 9 cases).
 */
describe('InflectionaiService — Spec 480 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through InflectionaiModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [InflectionaiModule],
      }).compile();
      const service = moduleRef.get(InflectionaiService);
      expect(service).toBeInstanceOf(InflectionaiService);
      await moduleRef.close();
    });

    it('exports the Site.INFLECTIONAI = "inflectionai" enum value', () => {
      expect(Site.INFLECTIONAI).toBe('inflectionai');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new InflectionaiService();
      const result = await service.scrape({
        siteType: [Site.INFLECTIONAI],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'inflectionai-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.INFLECTIONAI);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Inflection AI');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/inflectionai/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/inflectionai/jobs/');
      ...
      expect(job0?.description).toContain('Inflection AI');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/inflectionai/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Inflection AI');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'inflectionai-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
