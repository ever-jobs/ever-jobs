...
import { RenuityModule, RenuityService } from '../src';

const COMPANY_NAME_EXPECT = 'Renuity';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'renuity-jobs.json'), 'utf8'),
);
...
describe('RenuityService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through RenuityModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [RenuityModule],
      }).compile();
      const service = moduleRef.get(RenuityService);
      expect(service).toBeInstanceOf(RenuityService);
      await moduleRef.close();
    });

    it('exports the Site.RENUITY = "renuity" enum value', () => {
      expect(Site.RENUITY).toBe('renuity');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new RenuityService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.RENUITY],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('renuity');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.RENUITY);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('renuity-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new RenuityService(registry);
      const result = await service.scrape({
        siteType: [Site.RENUITY],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('renuity');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('renuity-x1');
      expect(result.jobs[0].site).toBe(Site.RENUITY);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('renuity-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new RenuityService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.RENUITY],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new RenuityService();
      const result = await service.scrape({
        siteType: [Site.RENUITY],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
