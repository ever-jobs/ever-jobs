...
import { OlipopModule, OlipopService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'olipop-jobs.json'), 'utf8'),
);
...
/**
 * Spec 556 / T04 — `OlipopService` unit tests (≥ 9 cases).
 */
describe('OlipopService — Spec 556 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through OlipopModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [OlipopModule],
      }).compile();
      const service = moduleRef.get(OlipopService);
      expect(service).toBeInstanceOf(OlipopService);
      await moduleRef.close();
    });

    it('exports the Site.OLIPOP = "olipop" enum value', () => {
      expect(Site.OLIPOP).toBe('olipop');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new OlipopService();
      const result = await service.scrape({
        siteType: [Site.OLIPOP],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'olipop-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.OLIPOP);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('OLIPOP');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/olipop/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/olipop/jobs/');
      ...
      expect(job0?.description).toContain('OLIPOP');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/olipop/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('OLIPOP');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'olipop-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
