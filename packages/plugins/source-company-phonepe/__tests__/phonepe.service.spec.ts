...
import { PhonepeModule, PhonepeService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'phonepe-jobs.json'), 'utf8'),
);
...
/**
 * Spec 577 / T04 — `PhonepeService` unit tests (≥ 9 cases).
 */
describe('PhonepeService — Spec 577 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through PhonepeModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [PhonepeModule],
      }).compile();
      const service = moduleRef.get(PhonepeService);
      expect(service).toBeInstanceOf(PhonepeService);
      await moduleRef.close();
    });

    it('exports the Site.PHONEPE = "phonepe" enum value', () => {
      expect(Site.PHONEPE).toBe('phonepe');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new PhonepeService();
      const result = await service.scrape({
        siteType: [Site.PHONEPE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'phonepe-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.PHONEPE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('PhonePe');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/phonepe/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/phonepe/jobs/');
      ...
      expect(job0?.description).toContain('PhonePe');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/phonepe/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('PhonePe');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'phonepe-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
