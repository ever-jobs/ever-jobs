...
import { Align46Module, Align46Service } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'align46-jobs.json'), 'utf8'),
);
...
/**
 * Spec 231 / T04 — `Align46Service` unit tests (≥ 9 cases).
 */
describe('Align46Service — Spec 231 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through Align46Module via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [Align46Module],
      }).compile();
      const service = moduleRef.get(Align46Service);
      expect(service).toBeInstanceOf(Align46Service);
      await moduleRef.close();
    });

    it('exports the Site.ALIGN46 = "align46" enum value', () => {
      expect(Site.ALIGN46).toBe('align46');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new Align46Service();
      const result = await service.scrape({
        siteType: [Site.ALIGN46],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'align46-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ALIGN46);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Align Communications');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/align46/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/align46/jobs/');
      ...
      expect(job0?.description).toContain('Align Communications');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/align46/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Align Communications');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'align46-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
