...
import { LeadBankModule, LeadBankService } from '../src';

const COMPANY_NAME_EXPECT = 'Lead Bank';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'leadbank-jobs.json'), 'utf8'),
);
...
describe('LeadBankService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through LeadBankModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [LeadBankModule],
      }).compile();
      const service = moduleRef.get(LeadBankService);
      expect(service).toBeInstanceOf(LeadBankService);
      await moduleRef.close();
    });

    it('exports the Site.LEAD_BANK = "leadbank" enum value', () => {
      expect(Site.LEAD_BANK).toBe('leadbank');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new LeadBankService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.LEAD_BANK],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('leadbank');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.LEAD_BANK);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('leadbank-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new LeadBankService(registry);
      const result = await service.scrape({
        siteType: [Site.LEAD_BANK],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('leadbank');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('leadbank-x1');
      expect(result.jobs[0].site).toBe(Site.LEAD_BANK);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('leadbank-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new LeadBankService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.LEAD_BANK],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new LeadBankService();
      const result = await service.scrape({
        siteType: [Site.LEAD_BANK],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
