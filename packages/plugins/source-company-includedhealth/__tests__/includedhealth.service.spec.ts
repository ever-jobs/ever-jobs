...
import { IncludedHealthModule, IncludedHealthService } from '../src';

const COMPANY_NAME_EXPECT = 'Included Health';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'includedhealth-jobs.json'), 'utf8'),
);
...
describe('IncludedHealthService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through IncludedHealthModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [IncludedHealthModule],
      }).compile();
      const service = moduleRef.get(IncludedHealthService);
      expect(service).toBeInstanceOf(IncludedHealthService);
      await moduleRef.close();
    });

    it('exports the Site.INCLUDED_HEALTH = "includedhealth" enum value', () => {
      expect(Site.INCLUDED_HEALTH).toBe('includedhealth');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new IncludedHealthService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.INCLUDED_HEALTH],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'includedhealth-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.INCLUDED_HEALTH);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('includedhealth-' + first.id);
      ...
      expect(calledUrls[0]).toContain('includedhealth');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.INCLUDED_HEALTH);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('includedhealth-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new IncludedHealthService(registry);
      const result = await service.scrape({
        siteType: [Site.INCLUDED_HEALTH],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('includedhealth');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('includedhealth-x1');
      expect(result.jobs[0].site).toBe(Site.INCLUDED_HEALTH);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('includedhealth-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new IncludedHealthService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.INCLUDED_HEALTH],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new IncludedHealthService();
      const result = await service.scrape({
        siteType: [Site.INCLUDED_HEALTH],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
