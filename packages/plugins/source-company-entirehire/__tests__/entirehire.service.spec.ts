...
import { EntireHireModule, EntireHireService } from '../src';

const COMPANY_NAME_EXPECT = 'Entire Hire';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'entirehire-jobs.json'), 'utf8'),
);
...
describe('EntireHireService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through EntireHireModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [EntireHireModule],
      }).compile();
      const service = moduleRef.get(EntireHireService);
      expect(service).toBeInstanceOf(EntireHireService);
      await moduleRef.close();
    });

    it('exports the Site.ENTIRE_HIRE = "entirehire" enum value', () => {
      expect(Site.ENTIRE_HIRE).toBe('entirehire');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new EntireHireService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.ENTIRE_HIRE],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('entirehire');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ENTIRE_HIRE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('entirehire-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new EntireHireService(registry);
      const result = await service.scrape({
        siteType: [Site.ENTIRE_HIRE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('entirehire');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('entirehire-x1');
      expect(result.jobs[0].site).toBe(Site.ENTIRE_HIRE);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('entirehire-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new EntireHireService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.ENTIRE_HIRE],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new EntireHireService();
      const result = await service.scrape({
        siteType: [Site.ENTIRE_HIRE],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
