...
import { CowboySpaceCorpModule, CowboySpaceCorpService } from '../src';

const COMPANY_NAME_EXPECT = 'Cowboy Space Corp.';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'cowboyspacecorp-jobs.json'), 'utf8'),
);
...
describe('CowboySpaceCorpService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CowboySpaceCorpModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CowboySpaceCorpModule],
      }).compile();
      const service = moduleRef.get(CowboySpaceCorpService);
      expect(service).toBeInstanceOf(CowboySpaceCorpService);
      await moduleRef.close();
    });

    it('exports the Site.COWBOY_SPACE_CORP = "cowboyspacecorp" enum value', () => {
      expect(Site.COWBOY_SPACE_CORP).toBe('cowboyspacecorp');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new CowboySpaceCorpService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.COWBOY_SPACE_CORP],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'cowboyspacecorp-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.COWBOY_SPACE_CORP);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('cowboyspacecorp-' + first.id);
      ...
      expect(calledUrls[0]).toContain('cowboyspace');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.COWBOY_SPACE_CORP);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('cowboyspacecorp-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new CowboySpaceCorpService(registry);
      const result = await service.scrape({
        siteType: [Site.COWBOY_SPACE_CORP],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('cowboyspace');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('cowboyspacecorp-x1');
      expect(result.jobs[0].site).toBe(Site.COWBOY_SPACE_CORP);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('cowboyspacecorp-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new CowboySpaceCorpService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.COWBOY_SPACE_CORP],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new CowboySpaceCorpService();
      const result = await service.scrape({
        siteType: [Site.COWBOY_SPACE_CORP],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
