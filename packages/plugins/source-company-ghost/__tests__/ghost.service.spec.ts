...
import { GhostModule, GhostService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'ghost-jobs.json'), 'utf8'),
);
...
/**
 * Spec 625 / T04 — `GhostService` unit tests (≥ 9 cases).
 */
describe('GhostService — Spec 625 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through GhostModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [GhostModule],
      }).compile();
      const service = moduleRef.get(GhostService);
      expect(service).toBeInstanceOf(GhostService);
      await moduleRef.close();
    });

    it('exports the Site.GHOST = "ghost" enum value', () => {
      expect(Site.GHOST).toBe('ghost');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const job0 = dto.jobs.find((j) => j.id === 'ghost-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.GHOST);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Ghost');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/ghost/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/ghost/jobs/');
      ...
      expect(job0?.description).toContain('Ghost');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/ghost/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Ghost');
      }
    });
  });
