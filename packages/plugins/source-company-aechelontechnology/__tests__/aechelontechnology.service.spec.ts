...
import { AechelontechnologyModule, AechelontechnologyService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'aechelontechnology-jobs.json'), 'utf8'),
);
...
/**
 * Spec 196 / T04 — `AechelontechnologyService` unit tests (≥ 9 cases).
 */
describe('AechelontechnologyService — Spec 196 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AechelontechnologyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AechelontechnologyModule],
      }).compile();
      const service = moduleRef.get(AechelontechnologyService);
      expect(service).toBeInstanceOf(AechelontechnologyService);
      await moduleRef.close();
    });

    it('exports the Site.AECHELONTECHNOLOGY = "aechelontechnology" enum value', () => {
      expect(Site.AECHELONTECHNOLOGY).toBe('aechelontechnology');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AechelontechnologyService();
      const result = await service.scrape({
        siteType: [Site.AECHELONTECHNOLOGY],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'aechelontechnology-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.AECHELONTECHNOLOGY);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Aechelon Technology');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/aechelontechnology/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/aechelontechnology/jobs/');
      ...
      expect(job0?.description).toContain('Aechelon Technology');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/aechelontechnology/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Aechelon Technology');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'aechelontechnology-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
