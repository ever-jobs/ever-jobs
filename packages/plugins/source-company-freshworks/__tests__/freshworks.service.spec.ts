...
import { FreshworksModule, FreshworksService } from '../src';

const COMPANY_NAME_EXPECT = 'Freshworks';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'freshworks-jobs.json'), 'utf8'),
);
...
describe('FreshworksService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through FreshworksModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [FreshworksModule],
      }).compile();
      const service = moduleRef.get(FreshworksService);
      expect(service).toBeInstanceOf(FreshworksService);
      await moduleRef.close();
    });

    it('exports the Site.FRESHWORKS = "freshworks" enum value', () => {
      expect(Site.FRESHWORKS).toBe('freshworks');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new FreshworksService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.FRESHWORKS],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('Freshworks');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.FRESHWORKS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('freshworks-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new FreshworksService(registry);
      const result = await service.scrape({
        siteType: [Site.FRESHWORKS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('Freshworks');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('freshworks-x1');
      expect(result.jobs[0].site).toBe(Site.FRESHWORKS);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('freshworks-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new FreshworksService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.FRESHWORKS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new FreshworksService();
      const result = await service.scrape({
        siteType: [Site.FRESHWORKS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
