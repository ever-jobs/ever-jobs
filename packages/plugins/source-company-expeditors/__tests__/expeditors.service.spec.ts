...
import { ExpeditorsModule, ExpeditorsService } from '../src';

const COMPANY_NAME_EXPECT = 'Expeditors';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'expeditors-jobs.json'), 'utf8'),
);
...
describe('ExpeditorsService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ExpeditorsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ExpeditorsModule],
      }).compile();
      const service = moduleRef.get(ExpeditorsService);
      expect(service).toBeInstanceOf(ExpeditorsService);
      await moduleRef.close();
    });

    it('exports the Site.EXPEDITORS = "expeditors" enum value', () => {
      expect(Site.EXPEDITORS).toBe('expeditors');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new ExpeditorsService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.EXPEDITORS],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('Expeditors');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.EXPEDITORS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('expeditors-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new ExpeditorsService(registry);
      const result = await service.scrape({
        siteType: [Site.EXPEDITORS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('Expeditors');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('expeditors-x1');
      expect(result.jobs[0].site).toBe(Site.EXPEDITORS);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('expeditors-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new ExpeditorsService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.EXPEDITORS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new ExpeditorsService();
      const result = await service.scrape({
        siteType: [Site.EXPEDITORS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
