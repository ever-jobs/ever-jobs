...
import { OrbAerospaceModule, OrbAerospaceService } from '../src';

const COMPANY_NAME_EXPECT = 'Orb Aerospace';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'orbaerospace-jobs.json'), 'utf8'),
);
...
describe('OrbAerospaceService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through OrbAerospaceModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [OrbAerospaceModule],
      }).compile();
      const service = moduleRef.get(OrbAerospaceService);
      expect(service).toBeInstanceOf(OrbAerospaceService);
      await moduleRef.close();
    });

    it('exports the Site.ORB_AEROSPACE = "orbaerospace" enum value', () => {
      expect(Site.ORB_AEROSPACE).toBe('orbaerospace');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new OrbAerospaceService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.ORB_AEROSPACE],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'orbaerospace-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.ORB_AEROSPACE);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('orbaerospace-' + first.id);
      ...
      expect(calledUrls[0]).toContain('orbaerospace');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ORB_AEROSPACE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('orbaerospace-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new OrbAerospaceService(registry);
      const result = await service.scrape({
        siteType: [Site.ORB_AEROSPACE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('orbaerospace');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('orbaerospace-x1');
      expect(result.jobs[0].site).toBe(Site.ORB_AEROSPACE);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('orbaerospace-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new OrbAerospaceService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.ORB_AEROSPACE],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new OrbAerospaceService();
      const result = await service.scrape({
        siteType: [Site.ORB_AEROSPACE],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
