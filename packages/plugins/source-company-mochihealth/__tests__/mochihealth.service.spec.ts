...
import { MochiHealthModule, MochiHealthService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'mochihealth-jobs.json'), 'utf8'),
);
...
/**
 * Spec 657 / T04 — `MochiHealthService` unit tests (≥ 9 cases).
 */
describe('MochiHealthService — Spec 657 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through MochiHealthModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [MochiHealthModule],
      }).compile();
      const service = moduleRef.get(MochiHealthService);
      expect(service).toBeInstanceOf(MochiHealthService);
      await moduleRef.close();
    });

    it('exports the Site.MOCHI_HEALTH = "mochihealth" enum value', () => {
      expect(Site.MOCHI_HEALTH).toBe('mochihealth');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new MochiHealthService();
      const result = await service.scrape({
        siteType: [Site.MOCHI_HEALTH],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'mochihealth-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.MOCHI_HEALTH);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Mochi Health');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/mochihealth/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/mochihealth/jobs/');
      ...
      expect(job0?.description).toContain('Mochi Health');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/mochihealth/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Mochi Health');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'mochihealth-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
