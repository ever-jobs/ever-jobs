...
/**
 * Target — Retail (HQ: Minneapolis, MN, USA).
 *
 * Source (Spec 1736): Workday board, scraped in this order:
 *   - `target:5:targetcareers` — https://target.wd5.myworkdayjobs.com/targetcareers
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'target:5:targetcareers', atsIdPrefix: 'wd-target-' },
];
...
@SourcePlugin({
  ...
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['target.com'],
  description: 'Target careers via Workday. Tags: segment=workday-enterprise; industry=retail.',
})
@Injectable()
export class TargetService implements IScraper {
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Workday source plugin is not registered; cannot scrape Target');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`Target: delegating to Workday (${board.companySlug})`);
    ...
    this.logger.log(`Target: scraped ${jobs.length} jobs`);
