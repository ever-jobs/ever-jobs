...
import { AssentModule, AssentService } from '../src';

const COMPANY_NAME_EXPECT = 'Assent';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'assent-jobs.json'), 'utf8'),
);
...
describe('AssentService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AssentModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AssentModule],
      }).compile();
      const service = moduleRef.get(AssentService);
      expect(service).toBeInstanceOf(AssentService);
      await moduleRef.close();
    });

    it('exports the Site.ASSENT = "assent" enum value', () => {
      expect(Site.ASSENT).toBe('assent');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new AssentService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.ASSENT],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('Assent');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ASSENT);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('assent-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new AssentService(registry);
      const result = await service.scrape({
        siteType: [Site.ASSENT],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('Assent');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('assent-x1');
      expect(result.jobs[0].site).toBe(Site.ASSENT);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('assent-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new AssentService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.ASSENT],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new AssentService();
      const result = await service.scrape({
        siteType: [Site.ASSENT],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
