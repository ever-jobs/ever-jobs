...
import { MrBeastBeastIndustriesModule, MrBeastBeastIndustriesService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'mrbeastyoutube-jobs.json'), 'utf8'),
);
...
/**
 * Spec 887 / T04 — `MrBeastBeastIndustriesService` unit tests (≥ 9 cases).
 */
describe('MrBeastBeastIndustriesService — Spec 887 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through MrBeastBeastIndustriesModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [MrBeastBeastIndustriesModule],
      }).compile();
      const service = moduleRef.get(MrBeastBeastIndustriesService);
      expect(service).toBeInstanceOf(MrBeastBeastIndustriesService);
      await moduleRef.close();
    });

    it('exports the Site.MRBEAST_BEAST_INDUSTRIES = "mrbeastyoutube" enum value', () => {
      expect(Site.MRBEAST_BEAST_INDUSTRIES).toBe('mrbeastyoutube');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new MrBeastBeastIndustriesService();
      const result = await service.scrape({
        siteType: [Site.MRBEAST_BEAST_INDUSTRIES],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'mrbeastyoutube-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.MRBEAST_BEAST_INDUSTRIES);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('MrBeast (Beast Industries)');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/mrbeastyoutube/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/mrbeastyoutube/jobs/');
      ...
      expect(job0?.description).toContain('MrBeast (Beast Industries)');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/mrbeastyoutube/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('MrBeast (Beast Industries)');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'mrbeastyoutube-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
