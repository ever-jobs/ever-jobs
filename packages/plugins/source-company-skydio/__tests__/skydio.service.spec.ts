...
import { SkydioModule, SkydioService } from '../src';

const COMPANY_NAME_EXPECT = 'Skydio';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'skydio-jobs.json'), 'utf8'),
);
...
describe('SkydioService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SkydioModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SkydioModule],
      }).compile();
      const service = moduleRef.get(SkydioService);
      expect(service).toBeInstanceOf(SkydioService);
      await moduleRef.close();
    });

    it('exports the Site.SKYDIO = "skydio" enum value', () => {
      expect(Site.SKYDIO).toBe('skydio');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new SkydioService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.SKYDIO],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('skydio');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SKYDIO);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('skydio-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new SkydioService(registry);
      const result = await service.scrape({
        siteType: [Site.SKYDIO],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('skydio');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('skydio-x1');
      expect(result.jobs[0].site).toBe(Site.SKYDIO);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('skydio-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new SkydioService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.SKYDIO],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new SkydioService();
      const result = await service.scrape({
        siteType: [Site.SKYDIO],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
