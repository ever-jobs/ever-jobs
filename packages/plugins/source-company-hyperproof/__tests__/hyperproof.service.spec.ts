...
import { HyperproofModule, HyperproofService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'hyperproof-jobs.json'), 'utf8'),
);
...
/**
 * Spec 866 / T04 — `HyperproofService` unit tests (≥ 9 cases).
 */
describe('HyperproofService — Spec 866 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through HyperproofModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [HyperproofModule],
      }).compile();
      const service = moduleRef.get(HyperproofService);
      expect(service).toBeInstanceOf(HyperproofService);
      await moduleRef.close();
    });

    it('exports the Site.HYPERPROOF = "hyperproof" enum value', () => {
      expect(Site.HYPERPROOF).toBe('hyperproof');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new HyperproofService();
      const result = await service.scrape({
        siteType: [Site.HYPERPROOF],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'hyperproof-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.HYPERPROOF);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Hyperproof');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/hyperproof/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/hyperproof/jobs/');
      ...
      expect(job0?.description).toContain('Hyperproof');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/hyperproof/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Hyperproof');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'hyperproof-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
