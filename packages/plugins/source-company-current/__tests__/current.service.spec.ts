...
import { CurrentModule, CurrentService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'current-jobs.json'), 'utf8'),
);
...
/**
 * Spec 483 / T04 — `CurrentService` unit tests (≥ 9 cases).
 */
describe('CurrentService — Spec 483 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CurrentModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CurrentModule],
      }).compile();
      const service = moduleRef.get(CurrentService);
      expect(service).toBeInstanceOf(CurrentService);
      await moduleRef.close();
    });

    it('exports the Site.CURRENT = "current" enum value', () => {
      expect(Site.CURRENT).toBe('current');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const job0 = dto.jobs.find((j) => j.id === 'current-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.CURRENT);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Current');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/current/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/current/jobs/');
      ...
      expect(job0?.description).toContain('Current');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/current/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Current');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'current-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
