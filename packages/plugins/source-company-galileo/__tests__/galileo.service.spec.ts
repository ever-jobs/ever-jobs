...
import { GalileoModule, GalileoService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'galileo-jobs.json'), 'utf8'),
);
...
/**
 * Spec 479 / T04 — `GalileoService` unit tests (≥ 9 cases).
 */
describe('GalileoService — Spec 479 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through GalileoModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [GalileoModule],
      }).compile();
      const service = moduleRef.get(GalileoService);
      expect(service).toBeInstanceOf(GalileoService);
      await moduleRef.close();
    });

    it('exports the Site.GALILEO = "galileo" enum value', () => {
      expect(Site.GALILEO).toBe('galileo');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new GalileoService();
      const result = await service.scrape({
        siteType: [Site.GALILEO],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'galileo-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.GALILEO);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Galileo');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/galileo/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/galileo/jobs/');
      ...
      expect(job0?.description).toContain('Galileo');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/galileo/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Galileo');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'galileo-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
