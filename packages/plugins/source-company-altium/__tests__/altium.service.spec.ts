...
import { AltiumModule, AltiumService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'altium-jobs.json'), 'utf8'),
);
...
/**
 * Spec 248 / T04 — `AltiumService` unit tests (≥ 9 cases).
 */
describe('AltiumService — Spec 248 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AltiumModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AltiumModule],
      }).compile();
      const service = moduleRef.get(AltiumService);
      expect(service).toBeInstanceOf(AltiumService);
      await moduleRef.close();
    });

    it('exports the Site.ALTIUM = "altium" enum value', () => {
      expect(Site.ALTIUM).toBe('altium');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AltiumService();
      const result = await service.scrape({
        siteType: [Site.ALTIUM],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'altium-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ALTIUM);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Altium');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/altium/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/altium/jobs/');
      ...
      expect(job0?.description).toContain('Altium');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/altium/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Altium');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'altium-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
