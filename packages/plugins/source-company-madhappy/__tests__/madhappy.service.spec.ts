...
import { MadhappyModule, MadhappyService } from '../src';

const COMPANY_NAME_EXPECT = 'Madhappy';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'madhappy-jobs.json'), 'utf8'),
);
...
describe('MadhappyService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through MadhappyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [MadhappyModule],
      }).compile();
      const service = moduleRef.get(MadhappyService);
      expect(service).toBeInstanceOf(MadhappyService);
      await moduleRef.close();
    });

    it('exports the Site.MADHAPPY = "madhappy" enum value', () => {
      expect(Site.MADHAPPY).toBe('madhappy');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new MadhappyService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.MADHAPPY],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('madhappy');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.MADHAPPY);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('madhappy-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new MadhappyService(registry);
      const result = await service.scrape({
        siteType: [Site.MADHAPPY],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('madhappy');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('madhappy-x1');
      expect(result.jobs[0].site).toBe(Site.MADHAPPY);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('madhappy-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new MadhappyService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.MADHAPPY],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new MadhappyService();
      const result = await service.scrape({
        siteType: [Site.MADHAPPY],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
