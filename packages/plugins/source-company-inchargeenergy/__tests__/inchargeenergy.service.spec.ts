...
import { InChargeEnergyModule, InChargeEnergyService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'inchargeenergy-jobs.json'), 'utf8'),
);
...
/**
 * Spec 868 / T04 — `InChargeEnergyService` unit tests (≥ 9 cases).
 */
describe('InChargeEnergyService — Spec 868 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through InChargeEnergyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [InChargeEnergyModule],
      }).compile();
      const service = moduleRef.get(InChargeEnergyService);
      expect(service).toBeInstanceOf(InChargeEnergyService);
      await moduleRef.close();
    });

    it('exports the Site.INCHARGE_ENERGY = "inchargeenergy" enum value', () => {
      expect(Site.INCHARGE_ENERGY).toBe('inchargeenergy');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new InChargeEnergyService();
      const result = await service.scrape({
        siteType: [Site.INCHARGE_ENERGY],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'inchargeenergy-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.INCHARGE_ENERGY);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('InCharge Energy');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/inchargeenergy/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/inchargeenergy/jobs/');
      ...
      expect(job0?.description).toContain('InCharge Energy');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/inchargeenergy/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('InCharge Energy');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'inchargeenergy-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
