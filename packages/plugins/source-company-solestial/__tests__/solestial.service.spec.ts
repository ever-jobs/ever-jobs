...
import { SolestialModule, SolestialService } from '../src';

const COMPANY_NAME_EXPECT = 'Solestial';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'solestial-jobs.json'), 'utf8'),
);
...
describe('SolestialService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SolestialModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SolestialModule],
      }).compile();
      const service = moduleRef.get(SolestialService);
      expect(service).toBeInstanceOf(SolestialService);
      await moduleRef.close();
    });

    it('exports the Site.SOLESTIAL = "solestial" enum value', () => {
      expect(Site.SOLESTIAL).toBe('solestial');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new SolestialService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.SOLESTIAL],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('solestial');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SOLESTIAL);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('solestial-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new SolestialService(registry);
      const result = await service.scrape({
        siteType: [Site.SOLESTIAL],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('solestial');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('solestial-x1');
      expect(result.jobs[0].site).toBe(Site.SOLESTIAL);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('solestial-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new SolestialService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.SOLESTIAL],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new SolestialService();
      const result = await service.scrape({
        siteType: [Site.SOLESTIAL],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
