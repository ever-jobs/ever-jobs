...
import { EatalyModule, EatalyService } from '../src';

const COMPANY_NAME_EXPECT = 'Eataly';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'eataly-jobs.json'), 'utf8'),
);
...
describe('EatalyService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through EatalyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [EatalyModule],
      }).compile();
      const service = moduleRef.get(EatalyService);
      expect(service).toBeInstanceOf(EatalyService);
      await moduleRef.close();
    });

    it('exports the Site.EATALY = "eataly" enum value', () => {
      expect(Site.EATALY).toBe('eataly');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new EatalyService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.EATALY],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('Eataly');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.EATALY);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('eataly-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new EatalyService(registry);
      const result = await service.scrape({
        siteType: [Site.EATALY],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('Eataly');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('eataly-x1');
      expect(result.jobs[0].site).toBe(Site.EATALY);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('eataly-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new EatalyService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.EATALY],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new EatalyService();
      const result = await service.scrape({
        siteType: [Site.EATALY],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
