...
import { BluecruxModule, BluecruxService } from '../src';

const COMPANY_NAME_EXPECT = 'Bluecrux';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'bluecrux-jobs.json'), 'utf8'),
);
...
describe('BluecruxService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through BluecruxModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [BluecruxModule],
      }).compile();
      const service = moduleRef.get(BluecruxService);
      expect(service).toBeInstanceOf(BluecruxService);
      await moduleRef.close();
    });

    it('exports the Site.BLUECRUX = "bluecrux" enum value', () => {
      expect(Site.BLUECRUX).toBe('bluecrux');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new BluecruxService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.BLUECRUX],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('bluecrux.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.BLUECRUX);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('bluecrux-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new BluecruxService(registry);
      const result = await service.scrape({
        siteType: [Site.BLUECRUX],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('bluecrux');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('bluecrux-x1');
      expect(result.jobs[0].site).toBe(Site.BLUECRUX);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('bluecrux-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new BluecruxService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.BLUECRUX],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new BluecruxService();
      const result = await service.scrape({
        siteType: [Site.BLUECRUX],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
