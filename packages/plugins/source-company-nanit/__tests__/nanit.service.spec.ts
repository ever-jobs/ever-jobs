...
import { NanitModule, NanitService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'nanit-jobs.json'), 'utf8'),
);
...
/**
 * Spec 676 / T04 — `NanitService` unit tests (≥ 9 cases).
 */
describe('NanitService — Spec 676 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through NanitModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [NanitModule],
      }).compile();
      const service = moduleRef.get(NanitService);
      expect(service).toBeInstanceOf(NanitService);
      await moduleRef.close();
    });

    it('exports the Site.NANIT = "nanit" enum value', () => {
      expect(Site.NANIT).toBe('nanit');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new NanitService();
      ...
      const job0 = dto.jobs.find((j) => j.id === 'nanit-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.NANIT);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Nanit');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/nanit/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/nanit/jobs/');
      ...
      expect(job0?.description).toContain('Nanit');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/nanit/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Nanit');
      }
    });
  });
