...
import { NautoModule, NautoService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'nauto-jobs.json'), 'utf8'),
);
...
/**
 * Spec 749 / T04 — `NautoService` unit tests (≥ 9 cases).
 */
describe('NautoService — Spec 749 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through NautoModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [NautoModule],
      }).compile();
      const service = moduleRef.get(NautoService);
      expect(service).toBeInstanceOf(NautoService);
      await moduleRef.close();
    });

    it('exports the Site.NAUTO = "nauto" enum value', () => {
      expect(Site.NAUTO).toBe('nauto');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new NautoService();
      ...
      const job0 = dto.jobs.find((j) => j.id === 'nauto-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.NAUTO);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Nauto');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/nauto/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/nauto/jobs/');
      ...
      expect(job0?.description).toContain('Nauto');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/nauto/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Nauto');
      }
    });
  });
