...
import { ModernizeHomeServicesModule, ModernizeHomeServicesService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'modernize-jobs.json'), 'utf8'),
);
...
/**
 * Spec 886 / T04 — `ModernizeHomeServicesService` unit tests (≥ 9 cases).
 */
describe('ModernizeHomeServicesService — Spec 886 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ModernizeHomeServicesModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ModernizeHomeServicesModule],
      }).compile();
      const service = moduleRef.get(ModernizeHomeServicesService);
      expect(service).toBeInstanceOf(ModernizeHomeServicesService);
      await moduleRef.close();
    });

    it('exports the Site.MODERNIZE_HOME_SERVICES = "modernize" enum value', () => {
      expect(Site.MODERNIZE_HOME_SERVICES).toBe('modernize');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ModernizeHomeServicesService();
      const result = await service.scrape({
        siteType: [Site.MODERNIZE_HOME_SERVICES],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'modernize-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.MODERNIZE_HOME_SERVICES);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Modernize Home Services');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/modernize/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/modernize/jobs/');
      ...
      expect(job0?.description).toContain('Modernize Home Services');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/modernize/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Modernize Home Services');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'modernize-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
