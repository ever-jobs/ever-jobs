...
import { BeamTherapeuticsModule, BeamTherapeuticsService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'beamtherapeutics-jobs.json'), 'utf8'),
);
...
/**
 * Spec 816 / T04 — `BeamTherapeuticsService` unit tests (≥ 9 cases).
 */
describe('BeamTherapeuticsService — Spec 816 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through BeamTherapeuticsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [BeamTherapeuticsModule],
      }).compile();
      const service = moduleRef.get(BeamTherapeuticsService);
      expect(service).toBeInstanceOf(BeamTherapeuticsService);
      await moduleRef.close();
    });

    it('exports the Site.BEAM_THERAPEUTICS = "beamtherapeutics" enum value', () => {
      expect(Site.BEAM_THERAPEUTICS).toBe('beamtherapeutics');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new BeamTherapeuticsService();
      const result = await service.scrape({
        siteType: [Site.BEAM_THERAPEUTICS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'beamtherapeutics-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.BEAM_THERAPEUTICS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Beam Therapeutics');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/beamtherapeutics/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/beamtherapeutics/jobs/');
      ...
      expect(job0?.description).toContain('Beam Therapeutics');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/beamtherapeutics/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Beam Therapeutics');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'beamtherapeutics-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
