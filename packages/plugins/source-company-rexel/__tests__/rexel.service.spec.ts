...
import { RexelModule, RexelService } from '../src';

const COMPANY_NAME_EXPECT = 'Rexel';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'rexel-jobs.json'), 'utf8'),
);
...
describe('RexelService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through RexelModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [RexelModule],
      }).compile();
      const service = moduleRef.get(RexelService);
      expect(service).toBeInstanceOf(RexelService);
      await moduleRef.close();
    });

    it('exports the Site.REXEL = "rexel" enum value', () => {
      expect(Site.REXEL).toBe('rexel');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new RexelService(registryWithSmartRecruiters());
      ...
      expect(calledUrls[0]).toContain('REXEL1');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.REXEL);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('rexel-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new RexelService(registry);
      const result = await service.scrape({
        siteType: [Site.REXEL],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('REXEL1');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('rexel-x1');
      expect(result.jobs[0].site).toBe(Site.REXEL);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('rexel-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new RexelService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.REXEL],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new RexelService();
      const result = await service.scrape({
        siteType: [Site.REXEL],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
