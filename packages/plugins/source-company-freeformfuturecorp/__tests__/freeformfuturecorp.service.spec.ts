...
import { FreeformModule, FreeformService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'freeformfuturecorp-jobs.json'), 'utf8'),
);
...
/**
 * Spec 853 / T04 — `FreeformService` unit tests (≥ 9 cases).
 */
describe('FreeformService — Spec 853 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through FreeformModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [FreeformModule],
      }).compile();
      const service = moduleRef.get(FreeformService);
      expect(service).toBeInstanceOf(FreeformService);
      await moduleRef.close();
    });

    it('exports the Site.FREEFORM = "freeformfuturecorp" enum value', () => {
      expect(Site.FREEFORM).toBe('freeformfuturecorp');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new FreeformService();
      const result = await service.scrape({
        siteType: [Site.FREEFORM],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'freeformfuturecorp-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.FREEFORM);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Freeform');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/freeformfuturecorp/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/freeformfuturecorp/jobs/');
      ...
      expect(job0?.description).toContain('Freeform');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/freeformfuturecorp/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Freeform');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'freeformfuturecorp-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
