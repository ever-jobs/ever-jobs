...
import { DeBeersGroupModule, DeBeersGroupService } from '../src';

const COMPANY_NAME_EXPECT = 'De Beers Group';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'debeersgroup-jobs.json'), 'utf8'),
);
...
describe('DeBeersGroupService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through DeBeersGroupModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [DeBeersGroupModule],
      }).compile();
      const service = moduleRef.get(DeBeersGroupService);
      expect(service).toBeInstanceOf(DeBeersGroupService);
      await moduleRef.close();
    });

    it('exports the Site.DE_BEERS_GROUP = "debeersgroup" enum value', () => {
      expect(Site.DE_BEERS_GROUP).toBe('debeersgroup');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new DeBeersGroupService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.DE_BEERS_GROUP],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('AngloAmericanDeBeersGroup');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.DE_BEERS_GROUP);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('debeersgroup-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new DeBeersGroupService(registry);
      const result = await service.scrape({
        siteType: [Site.DE_BEERS_GROUP],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('AngloAmericanDeBeersGroup');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('debeersgroup-x1');
      expect(result.jobs[0].site).toBe(Site.DE_BEERS_GROUP);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('debeersgroup-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new DeBeersGroupService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.DE_BEERS_GROUP],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new DeBeersGroupService();
      const result = await service.scrape({
        siteType: [Site.DE_BEERS_GROUP],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
