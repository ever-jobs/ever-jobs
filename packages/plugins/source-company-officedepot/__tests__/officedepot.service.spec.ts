...
import { OfficeDepotModule, OfficeDepotService } from '../src';

const COMPANY_NAME_EXPECT = 'Office Depot';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'officedepot-jobs.json'), 'utf8'),
);
...
describe('OfficeDepotService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through OfficeDepotModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [OfficeDepotModule],
      }).compile();
      const service = moduleRef.get(OfficeDepotService);
      expect(service).toBeInstanceOf(OfficeDepotService);
      await moduleRef.close();
    });

    it('exports the Site.OFFICE_DEPOT = "officedepot" enum value', () => {
      expect(Site.OFFICE_DEPOT).toBe('officedepot');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new OfficeDepotService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.OFFICE_DEPOT],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('OfficeDepot');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.OFFICE_DEPOT);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('officedepot-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new OfficeDepotService(registry);
      const result = await service.scrape({
        siteType: [Site.OFFICE_DEPOT],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('OfficeDepot');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('officedepot-x1');
      expect(result.jobs[0].site).toBe(Site.OFFICE_DEPOT);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('officedepot-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new OfficeDepotService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.OFFICE_DEPOT],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new OfficeDepotService();
      const result = await service.scrape({
        siteType: [Site.OFFICE_DEPOT],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
