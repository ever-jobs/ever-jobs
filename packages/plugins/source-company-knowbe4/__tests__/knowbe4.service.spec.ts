...
import { KnowBe4Module, KnowBe4Service } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'knowbe4-jobs.json'), 'utf8'),
);
...
/**
 * Spec 877 / T04 — `KnowBe4Service` unit tests (≥ 9 cases).
 */
describe('KnowBe4Service — Spec 877 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through KnowBe4Module via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [KnowBe4Module],
      }).compile();
      const service = moduleRef.get(KnowBe4Service);
      expect(service).toBeInstanceOf(KnowBe4Service);
      await moduleRef.close();
    });

    it('exports the Site.KNOWBE4 = "knowbe4" enum value', () => {
      expect(Site.KNOWBE4).toBe('knowbe4');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new KnowBe4Service();
      const result = await service.scrape({
        siteType: [Site.KNOWBE4],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'knowbe4-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.KNOWBE4);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('KnowBe4');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/knowbe4/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/knowbe4/jobs/');
      ...
      expect(job0?.description).toContain('KnowBe4');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/knowbe4/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('KnowBe4');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'knowbe4-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
