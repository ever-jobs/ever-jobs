...
import { EDFUKModule, EDFUKService } from '../src';

const COMPANY_NAME_EXPECT = 'EDF UK';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'edfuk-jobs.json'), 'utf8'),
);
...
describe('EDFUKService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through EDFUKModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [EDFUKModule],
      }).compile();
      const service = moduleRef.get(EDFUKService);
      expect(service).toBeInstanceOf(EDFUKService);
      await moduleRef.close();
    });

    it('exports the Site.EDF_UK = "edfuk" enum value', () => {
      expect(Site.EDF_UK).toBe('edfuk');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new EDFUKService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.EDF_UK],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('EDF-UK');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.EDF_UK);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('edfuk-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new EDFUKService(registry);
      const result = await service.scrape({
        siteType: [Site.EDF_UK],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('EDF-UK');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('edfuk-x1');
      expect(result.jobs[0].site).toBe(Site.EDF_UK);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('edfuk-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new EDFUKService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.EDF_UK],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new EDFUKService();
      const result = await service.scrape({
        siteType: [Site.EDF_UK],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
