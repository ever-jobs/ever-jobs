...
import { UnitModule, UnitService } from '../src';

const COMPANY_NAME_EXPECT = 'Unit';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'unit-jobs.json'), 'utf8'),
);
...
describe('UnitService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through UnitModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [UnitModule],
      }).compile();
      const service = moduleRef.get(UnitService);
      expect(service).toBeInstanceOf(UnitService);
      await moduleRef.close();
    });

    it('exports the Site.UNIT = "unit" enum value', () => {
      expect(Site.UNIT).toBe('unit');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new UnitService(registryWithAshby());
      ...
      expect(calledUrls[0]).toContain('unit');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.UNIT);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('unit-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new UnitService(registry);
      const result = await service.scrape({
        siteType: [Site.UNIT],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('unit');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('unit-x1');
      expect(result.jobs[0].site).toBe(Site.UNIT);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('unit-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new UnitService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.UNIT],
      } as ScraperInputDto);
