...
import { CockroachlabsModule, CockroachlabsService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'cockroachlabs-jobs.json'), 'utf8'),
);
...
/**
 * Spec 432 / T04 — `CockroachlabsService` unit tests (≥ 9 cases).
 */
describe('CockroachlabsService — Spec 432 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CockroachlabsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CockroachlabsModule],
      }).compile();
      const service = moduleRef.get(CockroachlabsService);
      expect(service).toBeInstanceOf(CockroachlabsService);
      await moduleRef.close();
    });

    it('exports the Site.COCKROACHLABS = "cockroachlabs" enum value', () => {
      expect(Site.COCKROACHLABS).toBe('cockroachlabs');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new CockroachlabsService();
      const result = await service.scrape({
        siteType: [Site.COCKROACHLABS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'cockroachlabs-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.COCKROACHLABS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Cockroach Labs');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/cockroachlabs/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/cockroachlabs/jobs/');
      ...
      expect(job0?.description).toContain('Cockroach Labs');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/cockroachlabs/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Cockroach Labs');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'cockroachlabs-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
