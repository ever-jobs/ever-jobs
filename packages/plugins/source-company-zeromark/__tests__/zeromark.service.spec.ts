...
import { ZeroMarkModule, ZeroMarkService } from '../src';

const COMPANY_NAME_EXPECT = 'ZeroMark';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'zeromark-jobs.json'), 'utf8'),
);
...
describe('ZeroMarkService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ZeroMarkModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ZeroMarkModule],
      }).compile();
      const service = moduleRef.get(ZeroMarkService);
      expect(service).toBeInstanceOf(ZeroMarkService);
      await moduleRef.close();
    });

    it('exports the Site.ZEROMARK = "zeromark" enum value', () => {
      expect(Site.ZEROMARK).toBe('zeromark');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new ZeroMarkService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.ZEROMARK],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('zeromark');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ZEROMARK);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('zeromark-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new ZeroMarkService(registry);
      const result = await service.scrape({
        siteType: [Site.ZEROMARK],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('zeromark');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('zeromark-x1');
      expect(result.jobs[0].site).toBe(Site.ZEROMARK);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('zeromark-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new ZeroMarkService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.ZEROMARK],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new ZeroMarkService();
      const result = await service.scrape({
        siteType: [Site.ZEROMARK],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
