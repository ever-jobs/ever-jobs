...
import { OverviewEnergyModule, OverviewEnergyService } from '../src';

const COMPANY_NAME_EXPECT = 'Overview Energy';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'overviewenergy-jobs.json'), 'utf8'),
);
...
describe('OverviewEnergyService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through OverviewEnergyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [OverviewEnergyModule],
      }).compile();
      const service = moduleRef.get(OverviewEnergyService);
      expect(service).toBeInstanceOf(OverviewEnergyService);
      await moduleRef.close();
    });

    it('exports the Site.OVERVIEW_ENERGY = "overviewenergy" enum value', () => {
      expect(Site.OVERVIEW_ENERGY).toBe('overviewenergy');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new OverviewEnergyService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.OVERVIEW_ENERGY],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'overviewenergy-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.OVERVIEW_ENERGY);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('overviewenergy-' + first.id);
      ...
      expect(calledUrls[0]).toContain('overviewenergy');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.OVERVIEW_ENERGY);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('overviewenergy-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new OverviewEnergyService(registry);
      const result = await service.scrape({
        siteType: [Site.OVERVIEW_ENERGY],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('overviewenergy');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('overviewenergy-x1');
      expect(result.jobs[0].site).toBe(Site.OVERVIEW_ENERGY);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('overviewenergy-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new OverviewEnergyService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.OVERVIEW_ENERGY],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new OverviewEnergyService();
      const result = await service.scrape({
        siteType: [Site.OVERVIEW_ENERGY],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
