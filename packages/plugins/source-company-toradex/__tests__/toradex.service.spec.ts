...
import { ToradexModule, ToradexService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'toradex-jobs.json'), 'utf8'),
);
...
/**
 * Spec 958 / T04 — `ToradexService` unit tests (≥ 9 cases).
 */
describe('ToradexService — Spec 958 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ToradexModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ToradexModule],
      }).compile();
      const service = moduleRef.get(ToradexService);
      expect(service).toBeInstanceOf(ToradexService);
      await moduleRef.close();
    });

    it('exports the Site.TORADEX = "toradex" enum value', () => {
      expect(Site.TORADEX).toBe('toradex');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ToradexService();
      const result = await service.scrape({
        siteType: [Site.TORADEX],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'toradex-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.TORADEX);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Toradex');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/toradex/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/toradex/jobs/');
      ...
      expect(job0?.description).toContain('Toradex');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/toradex/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Toradex');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'toradex-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
