...
import { AltruistModule, AltruistService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'altruist-jobs.json'), 'utf8'),
);
...
/**
 * Spec 608 / T04 — `AltruistService` unit tests (≥ 9 cases).
 */
describe('AltruistService — Spec 608 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AltruistModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AltruistModule],
      }).compile();
      const service = moduleRef.get(AltruistService);
      expect(service).toBeInstanceOf(AltruistService);
      await moduleRef.close();
    });

    it('exports the Site.ALTRUIST = "altruist" enum value', () => {
      expect(Site.ALTRUIST).toBe('altruist');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AltruistService();
      const result = await service.scrape({
        siteType: [Site.ALTRUIST],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'altruist-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ALTRUIST);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Altruist');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/altruist/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/altruist/jobs/');
      ...
      expect(job0?.description).toContain('Altruist');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/altruist/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Altruist');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'altruist-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
