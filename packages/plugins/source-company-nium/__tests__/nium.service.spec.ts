...
import { NiumModule, NiumService } from '../src';

const COMPANY_NAME_EXPECT = 'Nium';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'nium-jobs.json'), 'utf8'),
);
...
describe('NiumService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through NiumModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [NiumModule],
      }).compile();
      const service = moduleRef.get(NiumService);
      expect(service).toBeInstanceOf(NiumService);
      await moduleRef.close();
    });

    it('exports the Site.NIUM = "nium" enum value', () => {
      expect(Site.NIUM).toBe('nium');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new NiumService(registryWithLever());
      ...
      expect(calledUrls[0]).toContain('nium');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.NIUM);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('nium-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new NiumService(registry);
      const result = await service.scrape({
        siteType: [Site.NIUM],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('nium');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('nium-x1');
      expect(result.jobs[0].site).toBe(Site.NIUM);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('nium-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new NiumService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.NIUM],
      } as ScraperInputDto);
