...
import { MintlifyModule, MintlifyService } from '../src';

const COMPANY_NAME_EXPECT = 'Mintlify';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'mintlify-jobs.json'), 'utf8'),
);
...
describe('MintlifyService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through MintlifyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [MintlifyModule],
      }).compile();
      const service = moduleRef.get(MintlifyService);
      expect(service).toBeInstanceOf(MintlifyService);
      await moduleRef.close();
    });

    it('exports the Site.MINTLIFY = "mintlify" enum value', () => {
      expect(Site.MINTLIFY).toBe('mintlify');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new MintlifyService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.MINTLIFY],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('mintlify');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.MINTLIFY);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('mintlify-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new MintlifyService(registry);
      const result = await service.scrape({
        siteType: [Site.MINTLIFY],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('mintlify');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('mintlify-x1');
      expect(result.jobs[0].site).toBe(Site.MINTLIFY);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('mintlify-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new MintlifyService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.MINTLIFY],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new MintlifyService();
      const result = await service.scrape({
        siteType: [Site.MINTLIFY],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
