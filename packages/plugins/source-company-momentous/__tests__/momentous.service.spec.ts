...
import { MomentousModule, MomentousService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'momentous-jobs.json'), 'utf8'),
);
...
/**
 * Spec 688 / T04 — `MomentousService` unit tests (≥ 9 cases).
 */
describe('MomentousService — Spec 688 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through MomentousModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [MomentousModule],
      }).compile();
      const service = moduleRef.get(MomentousService);
      expect(service).toBeInstanceOf(MomentousService);
      await moduleRef.close();
    });

    it('exports the Site.MOMENTOUS = "momentous" enum value', () => {
      expect(Site.MOMENTOUS).toBe('momentous');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new MomentousService();
      const result = await service.scrape({
        siteType: [Site.MOMENTOUS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'momentous-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.MOMENTOUS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Momentous');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/momentous/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/momentous/jobs/');
      ...
      expect(job0?.description).toContain('Momentous');
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Momentous');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'momentous-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
