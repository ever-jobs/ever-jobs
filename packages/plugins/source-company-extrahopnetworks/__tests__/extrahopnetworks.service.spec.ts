...
import { ExtraHopModule, ExtraHopService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'extrahopnetworks-jobs.json'), 'utf8'),
);
...
/**
 * Spec 845 / T04 — `ExtraHopService` unit tests (≥ 9 cases).
 */
describe('ExtraHopService — Spec 845 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ExtraHopModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ExtraHopModule],
      }).compile();
      const service = moduleRef.get(ExtraHopService);
      expect(service).toBeInstanceOf(ExtraHopService);
      await moduleRef.close();
    });

    it('exports the Site.EXTRAHOP = "extrahopnetworks" enum value', () => {
      expect(Site.EXTRAHOP).toBe('extrahopnetworks');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ExtraHopService();
      const result = await service.scrape({
        siteType: [Site.EXTRAHOP],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'extrahopnetworks-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.EXTRAHOP);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('ExtraHop');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/extrahopnetworks/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/extrahopnetworks/jobs/');
      ...
      expect(job0?.description).toContain('ExtraHop');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/extrahopnetworks/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('ExtraHop');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'extrahopnetworks-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
