...
import { GreenlightFinancialTechnologyModule, GreenlightFinancialTechnologyService } from '../src';

const COMPANY_NAME_EXPECT = 'Greenlight Financial Technology';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'greenlight-jobs.json'), 'utf8'),
);
...
describe('GreenlightFinancialTechnologyService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through GreenlightFinancialTechnologyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [GreenlightFinancialTechnologyModule],
      }).compile();
      const service = moduleRef.get(GreenlightFinancialTechnologyService);
      expect(service).toBeInstanceOf(GreenlightFinancialTechnologyService);
      await moduleRef.close();
    });

    it('exports the Site.GREENLIGHT_FINANCIAL_TECHNOLOGY = "greenlight" enum value', () => {
      expect(Site.GREENLIGHT_FINANCIAL_TECHNOLOGY).toBe('greenlight');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new GreenlightFinancialTechnologyService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.GREENLIGHT_FINANCIAL_TECHNOLOGY],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'greenlight-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.GREENLIGHT_FINANCIAL_TECHNOLOGY);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('greenlight-' + first.id);
      ...
      expect(calledUrls[0]).toContain('greenlight');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.GREENLIGHT_FINANCIAL_TECHNOLOGY);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('greenlight-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new GreenlightFinancialTechnologyService(registry);
      const result = await service.scrape({
        siteType: [Site.GREENLIGHT_FINANCIAL_TECHNOLOGY],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('greenlight');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('greenlight-x1');
      expect(result.jobs[0].site).toBe(Site.GREENLIGHT_FINANCIAL_TECHNOLOGY);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('greenlight-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new GreenlightFinancialTechnologyService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.GREENLIGHT_FINANCIAL_TECHNOLOGY],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new GreenlightFinancialTechnologyService();
      const result = await service.scrape({
        siteType: [Site.GREENLIGHT_FINANCIAL_TECHNOLOGY],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
