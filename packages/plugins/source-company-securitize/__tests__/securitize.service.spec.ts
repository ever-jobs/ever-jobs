...
import { SecuritizeModule, SecuritizeService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'securitize-jobs.json'), 'utf8'),
);
...
/**
 * Spec 713 / T04 — `SecuritizeService` unit tests (≥ 9 cases).
 */
describe('SecuritizeService — Spec 713 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SecuritizeModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SecuritizeModule],
      }).compile();
      const service = moduleRef.get(SecuritizeService);
      expect(service).toBeInstanceOf(SecuritizeService);
      await moduleRef.close();
    });

    it('exports the Site.SECURITIZE = "securitize" enum value', () => {
      expect(Site.SECURITIZE).toBe('securitize');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new SecuritizeService();
      const result = await service.scrape({
        siteType: [Site.SECURITIZE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'securitize-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.SECURITIZE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Securitize');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/securitize/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/securitize/jobs/');
      ...
      expect(job0?.description).toContain('Securitize');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/securitize/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Securitize');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'securitize-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
