...
import { LoftOrbitalSolutionsModule, LoftOrbitalSolutionsService } from '../src';

const COMPANY_NAME_EXPECT = 'Loft Orbital Solutions';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'loftorbital-jobs.json'), 'utf8'),
);
...
describe('LoftOrbitalSolutionsService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through LoftOrbitalSolutionsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [LoftOrbitalSolutionsModule],
      }).compile();
      const service = moduleRef.get(LoftOrbitalSolutionsService);
      expect(service).toBeInstanceOf(LoftOrbitalSolutionsService);
      await moduleRef.close();
    });

    it('exports the Site.LOFT_ORBITAL_SOLUTIONS = "loftorbital" enum value', () => {
      expect(Site.LOFT_ORBITAL_SOLUTIONS).toBe('loftorbital');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new LoftOrbitalSolutionsService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.LOFT_ORBITAL_SOLUTIONS],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'loftorbital-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.LOFT_ORBITAL_SOLUTIONS);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('loftorbital-' + first.id);
      ...
      expect(calledUrls[0]).toContain('loftorbital');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.LOFT_ORBITAL_SOLUTIONS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('loftorbital-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new LoftOrbitalSolutionsService(registry);
      const result = await service.scrape({
        siteType: [Site.LOFT_ORBITAL_SOLUTIONS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('loftorbital');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('loftorbital-x1');
      expect(result.jobs[0].site).toBe(Site.LOFT_ORBITAL_SOLUTIONS);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('loftorbital-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new LoftOrbitalSolutionsService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.LOFT_ORBITAL_SOLUTIONS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new LoftOrbitalSolutionsService();
      const result = await service.scrape({
        siteType: [Site.LOFT_ORBITAL_SOLUTIONS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
