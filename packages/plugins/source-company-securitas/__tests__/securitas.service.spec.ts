...
import { SecuritasModule, SecuritasService } from '../src';

const COMPANY_NAME_EXPECT = 'Securitas';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'securitas-jobs.json'), 'utf8'),
);
...
describe('SecuritasService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SecuritasModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SecuritasModule],
      }).compile();
      const service = moduleRef.get(SecuritasService);
      expect(service).toBeInstanceOf(SecuritasService);
      await moduleRef.close();
    });

    it('exports the Site.SECURITAS = "securitas" enum value', () => {
      expect(Site.SECURITAS).toBe('securitas');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new SecuritasService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.SECURITAS],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('Securitas');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SECURITAS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('securitas-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new SecuritasService(registry);
      const result = await service.scrape({
        siteType: [Site.SECURITAS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('Securitas');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('securitas-x1');
      expect(result.jobs[0].site).toBe(Site.SECURITAS);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('securitas-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new SecuritasService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.SECURITAS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new SecuritasService();
      const result = await service.scrape({
        siteType: [Site.SECURITAS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
