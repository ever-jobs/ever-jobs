...
import { AuroraSolarModule, AuroraSolarService } from '../src';

const COMPANY_NAME_EXPECT = 'Aurora Solar';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'aurorasolar-jobs.json'), 'utf8'),
);
...
describe('AuroraSolarService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AuroraSolarModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AuroraSolarModule],
      }).compile();
      const service = moduleRef.get(AuroraSolarService);
      expect(service).toBeInstanceOf(AuroraSolarService);
      await moduleRef.close();
    });

    it('exports the Site.AURORA_SOLAR = "aurorasolar" enum value', () => {
      expect(Site.AURORA_SOLAR).toBe('aurorasolar');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new AuroraSolarService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.AURORA_SOLAR],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'aurorasolar-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.AURORA_SOLAR);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('aurorasolar-' + first.id);
      ...
      expect(calledUrls[0]).toContain('aurorasolar');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.AURORA_SOLAR);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('aurorasolar-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new AuroraSolarService(registry);
      const result = await service.scrape({
        siteType: [Site.AURORA_SOLAR],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('aurorasolar');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('aurorasolar-x1');
      expect(result.jobs[0].site).toBe(Site.AURORA_SOLAR);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('aurorasolar-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new AuroraSolarService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.AURORA_SOLAR],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new AuroraSolarService();
      const result = await service.scrape({
        siteType: [Site.AURORA_SOLAR],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
