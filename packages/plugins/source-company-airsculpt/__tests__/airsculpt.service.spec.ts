...
import { AirsculptModule, AirsculptService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'airsculpt-jobs.json'), 'utf8'),
);
...
/**
 * Spec 216 / T04 — `AirsculptService` unit tests (≥ 9 cases).
 */
describe('AirsculptService — Spec 216 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AirsculptModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AirsculptModule],
      }).compile();
      const service = moduleRef.get(AirsculptService);
      expect(service).toBeInstanceOf(AirsculptService);
      await moduleRef.close();
    });

    it('exports the Site.AIRSCULPT = "airsculpt" enum value', () => {
      expect(Site.AIRSCULPT).toBe('airsculpt');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AirsculptService();
      const result = await service.scrape({
        siteType: [Site.AIRSCULPT],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'airsculpt-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.AIRSCULPT);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('AirSculpt');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/airsculpt/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/airsculpt/jobs/');
      ...
      expect(job0?.description).toContain('AirSculpt');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/airsculpt/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('AirSculpt');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'airsculpt-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
