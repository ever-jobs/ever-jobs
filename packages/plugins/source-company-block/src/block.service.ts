 ...
 * parent of Square / Cash App / Tidal / Spiral / TBD / AfterPay) publishes
 * its consolidated careers board through Greenhouse at the bare `block`
 * slug (no asymmetry; see Spec 042 § 10 D-05).
 */
const API_URL = 'https://api.greenhouse.io/v1/boards/block/jobs';
...
export class BlockService implements IScraper {
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    try {
      ...
      this.logger.log(`Block: fetching ${url}`);
      ...
      this.logger.log(`Block: scraped ${jobs.length} jobs`);
    } catch (err: any) {
      this.logger.error(`Block scrape failed: ${err.message}`);
