...
import { StatkraftModule, StatkraftService } from '../src';

const COMPANY_NAME_EXPECT = 'Statkraft';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'statkraft-jobs.json'), 'utf8'),
);
...
describe('StatkraftService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through StatkraftModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [StatkraftModule],
      }).compile();
      const service = moduleRef.get(StatkraftService);
      expect(service).toBeInstanceOf(StatkraftService);
      await moduleRef.close();
    });

    it('exports the Site.STATKRAFT = "statkraft" enum value', () => {
      expect(Site.STATKRAFT).toBe('statkraft');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new StatkraftService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.STATKRAFT],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('statkraft1');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.STATKRAFT);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('statkraft-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new StatkraftService(registry);
      const result = await service.scrape({
        siteType: [Site.STATKRAFT],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('statkraft1');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('statkraft-x1');
      expect(result.jobs[0].site).toBe(Site.STATKRAFT);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('statkraft-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new StatkraftService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.STATKRAFT],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new StatkraftService();
      const result = await service.scrape({
        siteType: [Site.STATKRAFT],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
