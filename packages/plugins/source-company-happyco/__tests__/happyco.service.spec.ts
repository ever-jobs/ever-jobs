...
import { HappyCoModule, HappyCoService } from '../src';

const COMPANY_NAME_EXPECT = 'HappyCo';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'happyco-jobs.json'), 'utf8'),
);
...
describe('HappyCoService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through HappyCoModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [HappyCoModule],
      }).compile();
      const service = moduleRef.get(HappyCoService);
      expect(service).toBeInstanceOf(HappyCoService);
      await moduleRef.close();
    });

    it('exports the Site.HAPPYCO = "happyco" enum value', () => {
      expect(Site.HAPPYCO).toBe('happyco');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new HappyCoService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.HAPPYCO],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('happyco');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.HAPPYCO);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('happyco-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new HappyCoService(registry);
      const result = await service.scrape({
        siteType: [Site.HAPPYCO],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('happyco');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('happyco-x1');
      expect(result.jobs[0].site).toBe(Site.HAPPYCO);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('happyco-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new HappyCoService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.HAPPYCO],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new HappyCoService();
      const result = await service.scrape({
        siteType: [Site.HAPPYCO],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
