...
import { NmbrsModule, NmbrsService } from '../src';

const COMPANY_NAME_EXPECT = 'Nmbrs';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'nmbrs-jobs.json'), 'utf8'),
);
...
describe('NmbrsService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through NmbrsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [NmbrsModule],
      }).compile();
      const service = moduleRef.get(NmbrsService);
      expect(service).toBeInstanceOf(NmbrsService);
      await moduleRef.close();
    });

    it('exports the Site.NMBRS = "nmbrs" enum value', () => {
      expect(Site.NMBRS).toBe('nmbrs');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new NmbrsService(registryWithRecruitee());
      ...
      expect(calledUrls[0]).toContain('nmbrs.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.NMBRS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('nmbrs-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new NmbrsService(registry);
      const result = await service.scrape({
        siteType: [Site.NMBRS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('nmbrs');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('nmbrs-x1');
      expect(result.jobs[0].site).toBe(Site.NMBRS);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('nmbrs-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new NmbrsService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.NMBRS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new NmbrsService();
      const result = await service.scrape({
        siteType: [Site.NMBRS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
