...
import { ModernanimalModule, ModernanimalService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'modernanimal-jobs.json'), 'utf8'),
);
...
/**
 * Spec 560 / T04 — `ModernanimalService` unit tests (≥ 9 cases).
 */
describe('ModernanimalService — Spec 560 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ModernanimalModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ModernanimalModule],
      }).compile();
      const service = moduleRef.get(ModernanimalService);
      expect(service).toBeInstanceOf(ModernanimalService);
      await moduleRef.close();
    });

    it('exports the Site.MODERN_ANIMAL = "modernanimal" enum value', () => {
      expect(Site.MODERN_ANIMAL).toBe('modernanimal');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ModernanimalService();
      const result = await service.scrape({
        siteType: [Site.MODERN_ANIMAL],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'modernanimal-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.MODERN_ANIMAL);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Modern Animal');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/modernanimal/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/modernanimal/jobs/');
      ...
      expect(job0?.description).toContain('Modern Animal');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/modernanimal/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Modern Animal');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'modernanimal-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
