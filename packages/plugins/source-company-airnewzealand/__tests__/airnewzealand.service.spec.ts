...
import { AirNewZealandModule, AirNewZealandService } from '../src';

const COMPANY_NAME_EXPECT = 'Air New Zealand';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'airnewzealand-jobs.json'), 'utf8'),
);
...
describe('AirNewZealandService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AirNewZealandModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AirNewZealandModule],
      }).compile();
      const service = moduleRef.get(AirNewZealandService);
      expect(service).toBeInstanceOf(AirNewZealandService);
      await moduleRef.close();
    });

    it('exports the Site.AIR_NEW_ZEALAND = "airnewzealand" enum value', () => {
      expect(Site.AIR_NEW_ZEALAND).toBe('airnewzealand');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new AirNewZealandService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.AIR_NEW_ZEALAND],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'airnewzealand-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.AIR_NEW_ZEALAND);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('airnewzealand-' + first.id);
      ...
      expect(calledUrls[0]).toContain('airnewzealand');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.AIR_NEW_ZEALAND);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('airnewzealand-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new AirNewZealandService(registry);
      const result = await service.scrape({
        siteType: [Site.AIR_NEW_ZEALAND],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('airnewzealand');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('airnewzealand-x1');
      expect(result.jobs[0].site).toBe(Site.AIR_NEW_ZEALAND);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('airnewzealand-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new AirNewZealandService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.AIR_NEW_ZEALAND],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new AirNewZealandService();
      const result = await service.scrape({
        siteType: [Site.AIR_NEW_ZEALAND],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
