...
import { WeaviateModule, WeaviateService } from '../src';

const COMPANY_NAME_EXPECT = 'Weaviate';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'weaviate-jobs.json'), 'utf8'),
);
...
describe('WeaviateService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through WeaviateModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [WeaviateModule],
      }).compile();
      const service = moduleRef.get(WeaviateService);
      expect(service).toBeInstanceOf(WeaviateService);
      await moduleRef.close();
    });

    it('exports the Site.WEAVIATE = "weaviate" enum value', () => {
      expect(Site.WEAVIATE).toBe('weaviate');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new WeaviateService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.WEAVIATE],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('weaviate');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.WEAVIATE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('weaviate-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new WeaviateService(registry);
      const result = await service.scrape({
        siteType: [Site.WEAVIATE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('weaviate');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('weaviate-x1');
      expect(result.jobs[0].site).toBe(Site.WEAVIATE);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('weaviate-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new WeaviateService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.WEAVIATE],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new WeaviateService();
      const result = await service.scrape({
        siteType: [Site.WEAVIATE],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
