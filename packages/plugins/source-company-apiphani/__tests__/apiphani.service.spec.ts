...
import { ApiphaniModule, ApiphaniService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'apiphani-jobs.json'), 'utf8'),
);
...
/**
 * Spec 284 / T04 — `ApiphaniService` unit tests (≥ 9 cases).
 */
describe('ApiphaniService — Spec 284 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ApiphaniModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ApiphaniModule],
      }).compile();
      const service = moduleRef.get(ApiphaniService);
      expect(service).toBeInstanceOf(ApiphaniService);
      await moduleRef.close();
    });

    it('exports the Site.APIPHANI = "apiphani" enum value', () => {
      expect(Site.APIPHANI).toBe('apiphani');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ApiphaniService();
      const result = await service.scrape({
        siteType: [Site.APIPHANI],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'apiphani-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.APIPHANI);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('apiphani');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/apiphani/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/apiphani/jobs/');
      ...
      expect(job0?.description).toContain('apiphani');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/apiphani/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('apiphani');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'apiphani-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
