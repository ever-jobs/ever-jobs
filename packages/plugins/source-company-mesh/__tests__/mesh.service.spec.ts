...
import { MeshPaymentsModule, MeshPaymentsService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'mesh-jobs.json'), 'utf8'),
);
...
/**
 * Spec 725 / T04 — `MeshPaymentsService` unit tests (≥ 9 cases).
 */
describe('MeshPaymentsService — Spec 725 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through MeshPaymentsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [MeshPaymentsModule],
      }).compile();
      const service = moduleRef.get(MeshPaymentsService);
      expect(service).toBeInstanceOf(MeshPaymentsService);
      await moduleRef.close();
    });

    it('exports the Site.MESH_PAYMENTS = "mesh" enum value', () => {
      expect(Site.MESH_PAYMENTS).toBe('mesh');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new MeshPaymentsService();
      const result = await service.scrape({
        siteType: [Site.MESH_PAYMENTS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'mesh-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.MESH_PAYMENTS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Mesh Payments');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/mesh/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/mesh/jobs/');
      ...
      expect(job0?.description).toContain('Mesh Payments');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/mesh/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Mesh Payments');
      }
    });
  });
