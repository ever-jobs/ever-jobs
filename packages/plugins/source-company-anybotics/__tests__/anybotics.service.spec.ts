...
import { ANYboticsModule, ANYboticsService } from '../src';

const COMPANY_NAME_EXPECT = 'ANYbotics';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'anybotics-jobs.json'), 'utf8'),
);
...
describe('ANYboticsService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ANYboticsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ANYboticsModule],
      }).compile();
      const service = moduleRef.get(ANYboticsService);
      expect(service).toBeInstanceOf(ANYboticsService);
      await moduleRef.close();
    });

    it('exports the Site.ANYBOTICS = "anybotics" enum value', () => {
      expect(Site.ANYBOTICS).toBe('anybotics');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new ANYboticsService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.ANYBOTICS],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('anybotics');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ANYBOTICS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('anybotics-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new ANYboticsService(registry);
      const result = await service.scrape({
        siteType: [Site.ANYBOTICS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('anybotics');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('anybotics-x1');
      expect(result.jobs[0].site).toBe(Site.ANYBOTICS);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('anybotics-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new ANYboticsService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.ANYBOTICS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new ANYboticsService();
      const result = await service.scrape({
        siteType: [Site.ANYBOTICS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
