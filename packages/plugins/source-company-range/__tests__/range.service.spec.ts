...
import { RangeModule, RangeService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'range-jobs.json'), 'utf8'),
);
...
/**
 * Spec 612 / T04 — `RangeService` unit tests (≥ 9 cases).
 */
describe('RangeService — Spec 612 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through RangeModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [RangeModule],
      }).compile();
      const service = moduleRef.get(RangeService);
      expect(service).toBeInstanceOf(RangeService);
      await moduleRef.close();
    });

    it('exports the Site.RANGE_TRAVEL = "range" enum value', () => {
      expect(Site.RANGE_TRAVEL).toBe('range');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const job0 = dto.jobs.find((j) => j.id === 'range-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.RANGE_TRAVEL);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Range');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/range/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/range/jobs/');
      ...
      expect(job0?.description).toContain('Range');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/range/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Range');
      }
    });
  });
