...
import { HealthcareSupportStaffingModule, HealthcareSupportStaffingService } from '../src';

const COMPANY_NAME_EXPECT = 'Healthcare Support Staffing';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'healthcaresupportstaffing-jobs.json'), 'utf8'),
);
...
describe('HealthcareSupportStaffingService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through HealthcareSupportStaffingModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [HealthcareSupportStaffingModule],
      }).compile();
      const service = moduleRef.get(HealthcareSupportStaffingService);
      expect(service).toBeInstanceOf(HealthcareSupportStaffingService);
      await moduleRef.close();
    });

    it('exports the Site.HEALTHCARE_SUPPORT_STAFFING = "healthcaresupportstaffing" enum value', () => {
      expect(Site.HEALTHCARE_SUPPORT_STAFFING).toBe('healthcaresupportstaffing');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new HealthcareSupportStaffingService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.HEALTHCARE_SUPPORT_STAFFING],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'healthcaresupportstaffing-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.HEALTHCARE_SUPPORT_STAFFING);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('healthcaresupportstaffing-' + first.id);
      ...
      expect(calledUrls[0]).toContain('HealthcareSupportStaffing1');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.HEALTHCARE_SUPPORT_STAFFING);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('healthcaresupportstaffing-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new HealthcareSupportStaffingService(registry);
      const result = await service.scrape({
        siteType: [Site.HEALTHCARE_SUPPORT_STAFFING],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('HealthcareSupportStaffing1');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('healthcaresupportstaffing-x1');
      expect(result.jobs[0].site).toBe(Site.HEALTHCARE_SUPPORT_STAFFING);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('healthcaresupportstaffing-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new HealthcareSupportStaffingService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.HEALTHCARE_SUPPORT_STAFFING],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new HealthcareSupportStaffingService();
      const result = await service.scrape({
        siteType: [Site.HEALTHCARE_SUPPORT_STAFFING],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
