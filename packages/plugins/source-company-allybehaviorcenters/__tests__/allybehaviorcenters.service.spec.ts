...
import { AllybehaviorcentersModule, AllybehaviorcentersService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'allybehaviorcenters-jobs.json'), 'utf8'),
);
...
/**
 * Spec 238 / T04 — `AllybehaviorcentersService` unit tests (≥ 9 cases).
 */
describe('AllybehaviorcentersService — Spec 238 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AllybehaviorcentersModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AllybehaviorcentersModule],
      }).compile();
      const service = moduleRef.get(AllybehaviorcentersService);
      expect(service).toBeInstanceOf(AllybehaviorcentersService);
      await moduleRef.close();
    });

    it('exports the Site.ALLYBEHAVIORCENTERS = "allybehaviorcenters" enum value', () => {
      expect(Site.ALLYBEHAVIORCENTERS).toBe('allybehaviorcenters');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AllybehaviorcentersService();
      const result = await service.scrape({
        siteType: [Site.ALLYBEHAVIORCENTERS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'allybehaviorcenters-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ALLYBEHAVIORCENTERS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Ally Behavior Centers');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/allybehaviorcenters/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/allybehaviorcenters/jobs/');
      ...
      expect(job0?.description).toContain('Ally Behavior Centers');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/allybehaviorcenters/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Ally Behavior Centers');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'allybehaviorcenters-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
