...
import { SoundAgricultureModule, SoundAgricultureService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'soundagriculture-jobs.json'), 'utf8'),
);
...
/**
 * Spec 786 / T04 — `SoundAgricultureService` unit tests (≥ 9 cases).
 */
describe('SoundAgricultureService — Spec 786 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SoundAgricultureModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SoundAgricultureModule],
      }).compile();
      const service = moduleRef.get(SoundAgricultureService);
      expect(service).toBeInstanceOf(SoundAgricultureService);
      await moduleRef.close();
    });

    it('exports the Site.SOUND_AGRICULTURE = "soundagriculture" enum value', () => {
      expect(Site.SOUND_AGRICULTURE).toBe('soundagriculture');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new SoundAgricultureService();
      const result = await service.scrape({
        siteType: [Site.SOUND_AGRICULTURE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'soundagriculture-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.SOUND_AGRICULTURE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Sound Agriculture');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/soundagriculture/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/soundagriculture/jobs/');
      ...
      expect(job0?.description).toContain('Sound Agriculture');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/soundagriculture/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Sound Agriculture');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'soundagriculture-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
