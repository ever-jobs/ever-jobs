...
import { UberFreightModule, UberFreightService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'uberfreight-jobs.json'), 'utf8'),
);
...
/**
 * Spec 962 / T04 — `UberFreightService` unit tests (≥ 9 cases).
 */
describe('UberFreightService — Spec 962 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through UberFreightModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [UberFreightModule],
      }).compile();
      const service = moduleRef.get(UberFreightService);
      expect(service).toBeInstanceOf(UberFreightService);
      await moduleRef.close();
    });

    it('exports the Site.UBER_FREIGHT = "uberfreight" enum value', () => {
      expect(Site.UBER_FREIGHT).toBe('uberfreight');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new UberFreightService();
      const result = await service.scrape({
        siteType: [Site.UBER_FREIGHT],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'uberfreight-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.UBER_FREIGHT);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Uber Freight');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/uberfreight/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/uberfreight/jobs/');
      ...
      expect(job0?.description).toContain('Uber Freight');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/uberfreight/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Uber Freight');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'uberfreight-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
