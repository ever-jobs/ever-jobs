...
import { StanfordMedicineChildrenSHealthModule, StanfordMedicineChildrenSHealthService } from '../src';

const COMPANY_NAME_EXPECT = 'Stanford Medicine Children\'s Health';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'stanfordmedicinechildrenshealth-jobs.json'), 'utf8'),
);
...
describe('StanfordMedicineChildrenSHealthService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through StanfordMedicineChildrenSHealthModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [StanfordMedicineChildrenSHealthModule],
      }).compile();
      const service = moduleRef.get(StanfordMedicineChildrenSHealthService);
      expect(service).toBeInstanceOf(StanfordMedicineChildrenSHealthService);
      await moduleRef.close();
    });

    it('exports the Site.STANFORD_MEDICINE_CHILDREN_S_HEALTH = "stanfordmedicinechildrenshealth" enum value', () => {
      expect(Site.STANFORD_MEDICINE_CHILDREN_S_HEALTH).toBe('stanfordmedicinechildrenshealth');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new StanfordMedicineChildrenSHealthService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.STANFORD_MEDICINE_CHILDREN_S_HEALTH],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'stanfordmedicinechildrenshealth-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.STANFORD_MEDICINE_CHILDREN_S_HEALTH);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('stanfordmedicinechildrenshealth-' + first.id);
      ...
      expect(calledUrls[0]).toContain('StanfordMedicineChildrensHealth');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.STANFORD_MEDICINE_CHILDREN_S_HEALTH);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('stanfordmedicinechildrenshealth-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new StanfordMedicineChildrenSHealthService(registry);
      const result = await service.scrape({
        siteType: [Site.STANFORD_MEDICINE_CHILDREN_S_HEALTH],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('StanfordMedicineChildrensHealth');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('stanfordmedicinechildrenshealth-x1');
      expect(result.jobs[0].site).toBe(Site.STANFORD_MEDICINE_CHILDREN_S_HEALTH);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('stanfordmedicinechildrenshealth-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new StanfordMedicineChildrenSHealthService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.STANFORD_MEDICINE_CHILDREN_S_HEALTH],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new StanfordMedicineChildrenSHealthService();
      const result = await service.scrape({
        siteType: [Site.STANFORD_MEDICINE_CHILDREN_S_HEALTH],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
