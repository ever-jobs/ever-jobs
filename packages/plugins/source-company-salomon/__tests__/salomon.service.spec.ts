...
import { SalomonModule, SalomonService } from '../src';

const COMPANY_NAME_EXPECT = 'Salomon';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'salomon-jobs.json'), 'utf8'),
);
...
describe('SalomonService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SalomonModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SalomonModule],
      }).compile();
      const service = moduleRef.get(SalomonService);
      expect(service).toBeInstanceOf(SalomonService);
      await moduleRef.close();
    });

    it('exports the Site.SALOMON = "salomon" enum value', () => {
      expect(Site.SALOMON).toBe('salomon');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new SalomonService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.SALOMON],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('Salomon');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SALOMON);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('salomon-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new SalomonService(registry);
      const result = await service.scrape({
        siteType: [Site.SALOMON],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('Salomon');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('salomon-x1');
      expect(result.jobs[0].site).toBe(Site.SALOMON);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('salomon-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new SalomonService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.SALOMON],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new SalomonService();
      const result = await service.scrape({
        siteType: [Site.SALOMON],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
