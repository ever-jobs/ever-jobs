...
import { SanaBiotechnologyModule, SanaBiotechnologyService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'sanabiotech-jobs.json'), 'utf8'),
);
...
/**
 * Spec 924 / T04 — `SanaBiotechnologyService` unit tests (≥ 9 cases).
 */
describe('SanaBiotechnologyService — Spec 924 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SanaBiotechnologyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SanaBiotechnologyModule],
      }).compile();
      const service = moduleRef.get(SanaBiotechnologyService);
      expect(service).toBeInstanceOf(SanaBiotechnologyService);
      await moduleRef.close();
    });

    it('exports the Site.SANA_BIOTECHNOLOGY = "sanabiotech" enum value', () => {
      expect(Site.SANA_BIOTECHNOLOGY).toBe('sanabiotech');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new SanaBiotechnologyService();
      const result = await service.scrape({
        siteType: [Site.SANA_BIOTECHNOLOGY],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'sanabiotech-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.SANA_BIOTECHNOLOGY);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Sana Biotechnology');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/sanabiotech/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/sanabiotech/jobs/');
      ...
      expect(job0?.description).toContain('Sana Biotechnology');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/sanabiotech/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Sana Biotechnology');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'sanabiotech-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
