...
import { CounterpartModule, CounterpartService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'counterpart-jobs.json'), 'utf8'),
);
...
/**
 * Spec 621 / T04 — `CounterpartService` unit tests (≥ 9 cases).
 */
describe('CounterpartService — Spec 621 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CounterpartModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CounterpartModule],
      }).compile();
      const service = moduleRef.get(CounterpartService);
      expect(service).toBeInstanceOf(CounterpartService);
      await moduleRef.close();
    });

    it('exports the Site.COUNTERPART = "counterpart" enum value', () => {
      expect(Site.COUNTERPART).toBe('counterpart');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new CounterpartService();
      const result = await service.scrape({
        siteType: [Site.COUNTERPART],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'counterpart-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.COUNTERPART);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Counterpart');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/counterpart/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/counterpart/jobs/');
      ...
      expect(job0?.description).toContain('Counterpart');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/counterpart/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Counterpart');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'counterpart-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
