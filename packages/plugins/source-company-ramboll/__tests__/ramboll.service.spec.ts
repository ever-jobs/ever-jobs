...
import { RambollModule, RambollService } from '../src';

const COMPANY_NAME_EXPECT = 'Ramboll';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'ramboll-jobs.json'), 'utf8'),
);
...
describe('RambollService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through RambollModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [RambollModule],
      }).compile();
      const service = moduleRef.get(RambollService);
      expect(service).toBeInstanceOf(RambollService);
      await moduleRef.close();
    });

    it('exports the Site.RAMBOLL = "ramboll" enum value', () => {
      expect(Site.RAMBOLL).toBe('ramboll');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new RambollService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.RAMBOLL],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('Ramboll3');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.RAMBOLL);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('ramboll-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new RambollService(registry);
      const result = await service.scrape({
        siteType: [Site.RAMBOLL],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('Ramboll3');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('ramboll-x1');
      expect(result.jobs[0].site).toBe(Site.RAMBOLL);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('ramboll-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new RambollService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.RAMBOLL],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new RambollService();
      const result = await service.scrape({
        siteType: [Site.RAMBOLL],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
