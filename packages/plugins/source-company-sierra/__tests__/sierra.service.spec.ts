...
import { SierraModule, SierraService } from '../src';

const COMPANY_NAME_EXPECT = 'Sierra';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'sierra-jobs.json'), 'utf8'),
);
...
describe('SierraService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SierraModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SierraModule],
      }).compile();
      const service = moduleRef.get(SierraService);
      expect(service).toBeInstanceOf(SierraService);
      await moduleRef.close();
    });

    it('exports the Site.SIERRA = "sierra" enum value', () => {
      expect(Site.SIERRA).toBe('sierra');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new SierraService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.SIERRA],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('sierra');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SIERRA);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('sierra-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new SierraService(registry);
      const result = await service.scrape({
        siteType: [Site.SIERRA],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('sierra');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('sierra-x1');
      expect(result.jobs[0].site).toBe(Site.SIERRA);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('sierra-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new SierraService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.SIERRA],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new SierraService();
      const result = await service.scrape({
        siteType: [Site.SIERRA],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
