...
import { AlloyModule, AlloyService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'alloy-jobs.json'), 'utf8'),
);
...
/**
 * Spec 472 / T04 — `AlloyService` unit tests (≥ 9 cases).
 */
describe('AlloyService — Spec 472 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AlloyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AlloyModule],
      }).compile();
      const service = moduleRef.get(AlloyService);
      expect(service).toBeInstanceOf(AlloyService);
      await moduleRef.close();
    });

    it('exports the Site.ALLOY = "alloy" enum value', () => {
      expect(Site.ALLOY).toBe('alloy');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AlloyService();
      ...
      const job0 = dto.jobs.find((j) => j.id === 'alloy-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ALLOY);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Alloy');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/alloy/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/alloy/jobs/');
      ...
      expect(job0?.description).toContain('Alloy');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/alloy/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Alloy');
      }
    });
  });
