...
import { CultureampModule, CultureampService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'cultureamp-jobs.json'), 'utf8'),
);
...
/**
 * Spec 536 / T04 — `CultureampService` unit tests (≥ 9 cases).
 */
describe('CultureampService — Spec 536 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CultureampModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CultureampModule],
      }).compile();
      const service = moduleRef.get(CultureampService);
      expect(service).toBeInstanceOf(CultureampService);
      await moduleRef.close();
    });

    it('exports the Site.CULTURE_AMP = "cultureamp" enum value', () => {
      expect(Site.CULTURE_AMP).toBe('cultureamp');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new CultureampService();
      const result = await service.scrape({
        siteType: [Site.CULTURE_AMP],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'cultureamp-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.CULTURE_AMP);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Culture Amp');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/cultureamp/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/cultureamp/jobs/');
      ...
      expect(job0?.description).toContain('Culture Amp');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/cultureamp/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Culture Amp');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'cultureamp-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
