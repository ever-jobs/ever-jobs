...
import { MannarinoSystemsSoftwareModule, MannarinoSystemsSoftwareService } from '../src';

const COMPANY_NAME_EXPECT = 'Mannarino Systems & Software';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'mannarinosystemssoftware-jobs.json'), 'utf8'),
);
...
describe('MannarinoSystemsSoftwareService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through MannarinoSystemsSoftwareModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [MannarinoSystemsSoftwareModule],
      }).compile();
      const service = moduleRef.get(MannarinoSystemsSoftwareService);
      expect(service).toBeInstanceOf(MannarinoSystemsSoftwareService);
      await moduleRef.close();
    });

    it('exports the Site.MANNARINO_SYSTEMS_SOFTWARE = "mannarinosystemssoftware" enum value', () => {
      expect(Site.MANNARINO_SYSTEMS_SOFTWARE).toBe('mannarinosystemssoftware');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new MannarinoSystemsSoftwareService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.MANNARINO_SYSTEMS_SOFTWARE],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'mannarinosystemssoftware-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.MANNARINO_SYSTEMS_SOFTWARE);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('mannarinosystemssoftware-' + first.id);
      ...
      expect(calledUrls[0]).toContain('MannarinoSystemsSoftwareInc');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.MANNARINO_SYSTEMS_SOFTWARE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('mannarinosystemssoftware-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new MannarinoSystemsSoftwareService(registry);
      const result = await service.scrape({
        siteType: [Site.MANNARINO_SYSTEMS_SOFTWARE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('MannarinoSystemsSoftwareInc');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('mannarinosystemssoftware-x1');
      expect(result.jobs[0].site).toBe(Site.MANNARINO_SYSTEMS_SOFTWARE);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('mannarinosystemssoftware-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new MannarinoSystemsSoftwareService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.MANNARINO_SYSTEMS_SOFTWARE],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new MannarinoSystemsSoftwareService();
      const result = await service.scrape({
        siteType: [Site.MANNARINO_SYSTEMS_SOFTWARE],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
