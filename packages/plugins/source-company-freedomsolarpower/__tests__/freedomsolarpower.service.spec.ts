...
import { FreedomSolarPowerModule, FreedomSolarPowerService } from '../src';

const COMPANY_NAME_EXPECT = 'Freedom Solar Power';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'freedomsolarpower-jobs.json'), 'utf8'),
);
...
describe('FreedomSolarPowerService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through FreedomSolarPowerModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [FreedomSolarPowerModule],
      }).compile();
      const service = moduleRef.get(FreedomSolarPowerService);
      expect(service).toBeInstanceOf(FreedomSolarPowerService);
      await moduleRef.close();
    });

    it('exports the Site.FREEDOM_SOLAR_POWER = "freedomsolarpower" enum value', () => {
      expect(Site.FREEDOM_SOLAR_POWER).toBe('freedomsolarpower');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new FreedomSolarPowerService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.FREEDOM_SOLAR_POWER],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'freedomsolarpower-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.FREEDOM_SOLAR_POWER);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('freedomsolarpower-' + first.id);
      ...
      expect(calledUrls[0]).toContain('freedomsolarpower');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.FREEDOM_SOLAR_POWER);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('freedomsolarpower-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new FreedomSolarPowerService(registry);
      const result = await service.scrape({
        siteType: [Site.FREEDOM_SOLAR_POWER],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('freedomsolarpower');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('freedomsolarpower-x1');
      expect(result.jobs[0].site).toBe(Site.FREEDOM_SOLAR_POWER);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('freedomsolarpower-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new FreedomSolarPowerService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.FREEDOM_SOLAR_POWER],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new FreedomSolarPowerService();
      const result = await service.scrape({
        siteType: [Site.FREEDOM_SOLAR_POWER],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
