...
import { PickleRobotCompanyModule, PickleRobotCompanyService } from '../src';

const COMPANY_NAME_EXPECT = 'Pickle Robot Company';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'picklerobot-jobs.json'), 'utf8'),
);
...
describe('PickleRobotCompanyService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through PickleRobotCompanyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [PickleRobotCompanyModule],
      }).compile();
      const service = moduleRef.get(PickleRobotCompanyService);
      expect(service).toBeInstanceOf(PickleRobotCompanyService);
      await moduleRef.close();
    });

    it('exports the Site.PICKLE_ROBOT_COMPANY = "picklerobot" enum value', () => {
      expect(Site.PICKLE_ROBOT_COMPANY).toBe('picklerobot');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new PickleRobotCompanyService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.PICKLE_ROBOT_COMPANY],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'picklerobot-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.PICKLE_ROBOT_COMPANY);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('picklerobot-' + first.id);
      ...
      expect(calledUrls[0]).toContain('picklerobot');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.PICKLE_ROBOT_COMPANY);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('picklerobot-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new PickleRobotCompanyService(registry);
      const result = await service.scrape({
        siteType: [Site.PICKLE_ROBOT_COMPANY],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('picklerobot');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('picklerobot-x1');
      expect(result.jobs[0].site).toBe(Site.PICKLE_ROBOT_COMPANY);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('picklerobot-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new PickleRobotCompanyService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.PICKLE_ROBOT_COMPANY],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new PickleRobotCompanyService();
      const result = await service.scrape({
        siteType: [Site.PICKLE_ROBOT_COMPANY],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
