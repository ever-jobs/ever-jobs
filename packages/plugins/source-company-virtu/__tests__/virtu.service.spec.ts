...
import { VirtuModule, VirtuService } from '../src';
...
const FIXTURE: Fixture = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'fixtures', 'virtu-boards.json'), 'utf8'),
);
const COMPANY_NAME = 'Virtu Financial';
const ID_PREFIX = 'virtu-';
...
describe('VirtuService — Greenhouse delegation (Spec 1737)', () => {
  beforeEach(() => {
    mockGet.mockReset();
    mockPost.mockReset();
  ...
  describe('registration', () => {
    it('resolves through VirtuModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({ imports: [VirtuModule] }).compile();
      expect(moduleRef.get(VirtuService)).toBeInstanceOf(VirtuService);
      await moduleRef.close();
    });

    it('exports Site.VIRTU = "virtu"', () => {
      expect(Site.VIRTU).toBe('virtu');
    });

    it('declares a tagged company plugin with its domains', () => {
      const meta = Reflect.getMetadata(SOURCE_PLUGIN_METADATA, VirtuService);
      expect(meta.site).toBe(Site.VIRTU);
      expect(meta.category).toBe('company');
      expect(meta.isAts).toBeFalsy();
      expect(meta.companyDomains).toEqual(['virtu.com']);
  ...
  describe('recorded board (real Greenhouse adapter, mocked HTTP)', () => {
    it('maps every recorded posting and re-stamps the company identity', async () => {
      const service = new VirtuService(registryWith());
      const result = await service.scrape({ siteType: [Site.VIRTU], resultsWanted: 100 } as ScraperInputDto);
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.VIRTU);
    ...
    it('only requests the recorded board URLs', async () => {
      const service = new VirtuService(registryWith());
      await service.scrape({ siteType: [Site.VIRTU] } as ScraperInputDto);
    ...
    it('honours resultsWanted=1', async () => {
      const service = new VirtuService(registryWith());
      const result = await service.scrape({ siteType: [Site.VIRTU], resultsWanted: 1 } as ScraperInputDto);
    ...
    it('resolves with an empty result when the board is gone (HTTP 404)', async () => {
      ...
      const service = new VirtuService(registryWith());
      const result = await service.scrape({ siteType: [Site.VIRTU] } as ScraperInputDto);
  ...
  describe('delegation contract', () => {
    it('forwards the board and the caller input untouched', async () => {
      ...
      const service = new VirtuService(
        registryWith(
          fakeBackend(
      ...
      expect(result.jobs[0].site).toBe(Site.VIRTU);
    ...
    it('never forwards the caller\'s credentials to the board (Spec 1735 §4.5)', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new VirtuService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
    ...
    it('makes no request when resultsWanted is 0', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new VirtuService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      const result = await service.scrape({ siteType: [Site.VIRTU], resultsWanted: 0 } as ScraperInputDto);
  ...
  describe('resilience', () => {
    it('reports not_registered when no registry is injected', async () => {
      const result = await new VirtuService().scrape({ siteType: [Site.VIRTU] } as ScraperInputDto);
      expect(result.jobs).toEqual([]);
    ...
    it('reports not_registered when Greenhouse is missing from the registry', async () => {
      const result = await new VirtuService(new PluginRegistry()).scrape({
        siteType: [Site.VIRTU],
      } as ScraperInputDto);
      expect(result.diagnostics?.reason).toBe('not_registered');
    });
