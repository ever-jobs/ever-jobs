...
/**
 * Virtu Financial — Electronic market making (HQ: New York, NY, USA).
 *
 * Source (Spec 1737): Greenhouse board, scraped in this order:
 *   - `virtu` — https://job-boards.greenhouse.io/virtu
 *     verified live 2026-09-24: 49 open postings.
 *
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'virtu', atsIdPrefix: 'gh-' },
];
...
@SourcePlugin({
  ...
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['virtu.com'],
  description: 'Virtu Financial careers via Greenhouse. Tags: segment=quant-trading; industry=electronic-market-making.',
})
@Injectable()
export class VirtuService implements IScraper {
  private readonly logger = new Logger(VirtuService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Greenhouse source plugin is not registered; cannot scrape Virtu Financial');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`Virtu Financial: delegating to Greenhouse (${board.companySlug})`);
    ...
    this.logger.log(`Virtu Financial: scraped ${jobs.length} jobs`);
