...
import { BitpandaModule, BitpandaService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'bitpanda-jobs.json'), 'utf8'),
);
...
/**
 * Spec 683 / T04 — `BitpandaService` unit tests (≥ 9 cases).
 */
describe('BitpandaService — Spec 683 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through BitpandaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [BitpandaModule],
      }).compile();
      const service = moduleRef.get(BitpandaService);
      expect(service).toBeInstanceOf(BitpandaService);
      await moduleRef.close();
    });

    it('exports the Site.BITPANDA = "bitpanda" enum value', () => {
      expect(Site.BITPANDA).toBe('bitpanda');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new BitpandaService();
      const result = await service.scrape({
        siteType: [Site.BITPANDA],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'bitpanda-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.BITPANDA);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Bitpanda');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/bitpanda/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/bitpanda/jobs/');
      ...
      expect(job0?.description).toContain('Bitpanda');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/bitpanda/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Bitpanda');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'bitpanda-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
