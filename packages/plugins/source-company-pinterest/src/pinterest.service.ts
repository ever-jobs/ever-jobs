...
/**
 * Pinterest publishes its careers board through Greenhouse at the bare
 * `pinterest` slug (no asymmetry; see Spec 028 § 10 D-05).
 */
const API_URL = 'https://api.greenhouse.io/v1/boards/pinterest/jobs';
...
export class PinterestService implements IScraper {
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    try {
      ...
      this.logger.log(`Pinterest: fetching ${url}`);
        ...
        jobs.push(
          new JobPostDto({
            ...
            jobUrl:
              ...
              `https://www.pinterestcareers.com/jobs/${listing.id}/`,
            location,
      ...
      this.logger.log(`Pinterest: scraped ${jobs.length} jobs`);
    } catch (err: any) {
      this.logger.error(`Pinterest scrape failed: ${err.message}`);
