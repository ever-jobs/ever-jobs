...
import { BicaraTherapeuticsModule, BicaraTherapeuticsService } from '../src';

const COMPANY_NAME_EXPECT = 'Bicara Therapeutics';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'bicaratherapeutics-jobs.json'), 'utf8'),
);
...
describe('BicaraTherapeuticsService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through BicaraTherapeuticsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [BicaraTherapeuticsModule],
      }).compile();
      const service = moduleRef.get(BicaraTherapeuticsService);
      expect(service).toBeInstanceOf(BicaraTherapeuticsService);
      await moduleRef.close();
    });

    it('exports the Site.BICARA_THERAPEUTICS = "bicaratherapeutics" enum value', () => {
      expect(Site.BICARA_THERAPEUTICS).toBe('bicaratherapeutics');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new BicaraTherapeuticsService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.BICARA_THERAPEUTICS],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'bicaratherapeutics-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.BICARA_THERAPEUTICS);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('bicaratherapeutics-' + first.id);
      ...
      expect(calledUrls[0]).toContain('bicara-therapeutics');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.BICARA_THERAPEUTICS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('bicaratherapeutics-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new BicaraTherapeuticsService(registry);
      const result = await service.scrape({
        siteType: [Site.BICARA_THERAPEUTICS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('bicara-therapeutics');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('bicaratherapeutics-x1');
      expect(result.jobs[0].site).toBe(Site.BICARA_THERAPEUTICS);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('bicaratherapeutics-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new BicaraTherapeuticsService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.BICARA_THERAPEUTICS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new BicaraTherapeuticsService();
      const result = await service.scrape({
        siteType: [Site.BICARA_THERAPEUTICS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
