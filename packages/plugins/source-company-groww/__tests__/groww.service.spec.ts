...
import { GrowwModule, GrowwService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'groww-jobs.json'), 'utf8'),
);
...
/**
 * Spec 578 / T04 — `GrowwService` unit tests (≥ 9 cases).
 */
describe('GrowwService — Spec 578 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through GrowwModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [GrowwModule],
      }).compile();
      const service = moduleRef.get(GrowwService);
      expect(service).toBeInstanceOf(GrowwService);
      await moduleRef.close();
    });

    it('exports the Site.GROWW = "groww" enum value', () => {
      expect(Site.GROWW).toBe('groww');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const job0 = dto.jobs.find((j) => j.id === 'groww-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.GROWW);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Groww');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/groww/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/groww/jobs/');
      ...
      expect(job0?.description).toContain('Groww');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/groww/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Groww');
      }
    });
  });
