...
import { SukiAIModule, SukiAIService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'suki-jobs.json'), 'utf8'),
);
...
/**
 * Spec 770 / T04 — `SukiAIService` unit tests (≥ 9 cases).
 */
describe('SukiAIService — Spec 770 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SukiAIModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SukiAIModule],
      }).compile();
      const service = moduleRef.get(SukiAIService);
      expect(service).toBeInstanceOf(SukiAIService);
      await moduleRef.close();
    });

    it('exports the Site.SUKI_AI = "suki" enum value', () => {
      expect(Site.SUKI_AI).toBe('suki');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new SukiAIService();
      const result = await service.scrape({
        siteType: [Site.SUKI_AI],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'suki-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.SUKI_AI);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Suki AI');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/suki/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/suki/jobs/');
      ...
      expect(job0?.description).toContain('Suki AI');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/suki/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Suki AI');
      }
    });
  });
