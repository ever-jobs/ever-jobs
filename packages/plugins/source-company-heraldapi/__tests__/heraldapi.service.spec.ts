...
import { HeraldModule, HeraldService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'heraldapi-jobs.json'), 'utf8'),
);
...
/**
 * Spec 863 / T04 — `HeraldService` unit tests (≥ 9 cases).
 */
describe('HeraldService — Spec 863 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through HeraldModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [HeraldModule],
      }).compile();
      const service = moduleRef.get(HeraldService);
      expect(service).toBeInstanceOf(HeraldService);
      await moduleRef.close();
    });

    it('exports the Site.HERALD = "heraldapi" enum value', () => {
      expect(Site.HERALD).toBe('heraldapi');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new HeraldService();
      const result = await service.scrape({
        siteType: [Site.HERALD],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'heraldapi-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.HERALD);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Herald');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/heraldapi/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/heraldapi/jobs/');
      ...
      expect(job0?.description).toContain('Herald');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/heraldapi/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Herald');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'heraldapi-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
