...
/**
 * Herald — Herald is an employer in the Insurtech and insurance technology sector, headquartered in New York, New York, USA..
 *
 * Herald is a company operating in Insurtech and insurance technology,
 * headquartered in New York, New York, USA. This source plugin ingests
 * live open roles published on Herald's official Greenhouse-hosted
 ...
 *     Jobs schema
 *
 * Source profile (Spec 863):
 *   - D-04 — Greenhouse canonical hosted-board host (variant 2):
 *     `https://job-boards.greenhouse.io/heraldapi/jobs/<id>`.
 *   - D-08 — entity-decode-then-tag-strip description pipeline.
 *   - D-09 — wire `company_name` pass-through (`'Herald'`).
 ...
 */
const API_URL = 'https://api.greenhouse.io/v1/boards/heraldapi/jobs';
...
@Injectable()
export class HeraldService implements IScraper {
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    try {
      ...
      this.logger.log(`Herald: fetching ${url}`);
      ...
      for (const listing of listings) {
        ...
        jobs.push(
          new JobPostDto({
            ...
            companyName: listing.company_name ?? 'Herald',
            // D-04: wire `absolute_url` flows through (variant 2).
            jobUrl:
              listing.absolute_url ??
              `https://job-boards.greenhouse.io/heraldapi/jobs/${listing.id}`,
            location,
      ...
      this.logger.log(`Herald: scraped ${jobs.length} jobs`);
    } catch (err: any) {
      this.logger.error(`Herald scrape failed: ${err.message}`);
