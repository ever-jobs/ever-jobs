...
import { AbbyCareModule, AbbyCareService } from '../src';

const COMPANY_NAME_EXPECT = 'Abby Care';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'abbycare-jobs.json'), 'utf8'),
);
...
describe('AbbyCareService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AbbyCareModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AbbyCareModule],
      }).compile();
      const service = moduleRef.get(AbbyCareService);
      expect(service).toBeInstanceOf(AbbyCareService);
      await moduleRef.close();
    });

    it('exports the Site.ABBY_CARE = "abbycare" enum value', () => {
      expect(Site.ABBY_CARE).toBe('abbycare');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new AbbyCareService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.ABBY_CARE],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('abby-care');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ABBY_CARE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('abbycare-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new AbbyCareService(registry);
      const result = await service.scrape({
        siteType: [Site.ABBY_CARE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('abby-care');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('abbycare-x1');
      expect(result.jobs[0].site).toBe(Site.ABBY_CARE);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('abbycare-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new AbbyCareService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.ABBY_CARE],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new AbbyCareService();
      const result = await service.scrape({
        siteType: [Site.ABBY_CARE],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
