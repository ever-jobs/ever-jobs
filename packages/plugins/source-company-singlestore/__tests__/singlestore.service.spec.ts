...
import { SinglestoreModule, SinglestoreService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'singlestore-jobs.json'), 'utf8'),
);
...
/**
 * Spec 439 / T04 — `SinglestoreService` unit tests (≥ 9 cases).
 */
describe('SinglestoreService — Spec 439 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SinglestoreModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SinglestoreModule],
      }).compile();
      const service = moduleRef.get(SinglestoreService);
      expect(service).toBeInstanceOf(SinglestoreService);
      await moduleRef.close();
    });

    it('exports the Site.SINGLESTORE = "singlestore" enum value', () => {
      expect(Site.SINGLESTORE).toBe('singlestore');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new SinglestoreService();
      const result = await service.scrape({
        siteType: [Site.SINGLESTORE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'singlestore-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.SINGLESTORE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('SingleStore');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/singlestore/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/singlestore/jobs/');
      ...
      expect(job0?.description).toContain('SingleStore');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/singlestore/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('SingleStore');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'singlestore-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
