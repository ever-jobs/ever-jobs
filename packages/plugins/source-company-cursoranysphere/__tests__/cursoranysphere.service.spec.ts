...
import { CursorAnysphereModule, CursorAnysphereService } from '../src';

const COMPANY_NAME_EXPECT = 'Cursor (Anysphere)';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'cursoranysphere-jobs.json'), 'utf8'),
);
...
describe('CursorAnysphereService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CursorAnysphereModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CursorAnysphereModule],
      }).compile();
      const service = moduleRef.get(CursorAnysphereService);
      expect(service).toBeInstanceOf(CursorAnysphereService);
      await moduleRef.close();
    });

    it('exports the Site.CURSOR_ANYSPHERE = "cursoranysphere" enum value', () => {
      expect(Site.CURSOR_ANYSPHERE).toBe('cursoranysphere');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new CursorAnysphereService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.CURSOR_ANYSPHERE],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'cursoranysphere-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.CURSOR_ANYSPHERE);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('cursoranysphere-' + first.id);
      ...
      expect(calledUrls[0]).toContain('cursor');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.CURSOR_ANYSPHERE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('cursoranysphere-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new CursorAnysphereService(registry);
      const result = await service.scrape({
        siteType: [Site.CURSOR_ANYSPHERE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('cursor');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('cursoranysphere-x1');
      expect(result.jobs[0].site).toBe(Site.CURSOR_ANYSPHERE);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('cursoranysphere-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new CursorAnysphereService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.CURSOR_ANYSPHERE],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new CursorAnysphereService();
      const result = await service.scrape({
        siteType: [Site.CURSOR_ANYSPHERE],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
