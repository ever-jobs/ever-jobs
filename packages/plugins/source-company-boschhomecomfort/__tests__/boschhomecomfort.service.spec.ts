...
import { BoschHomeComfortModule, BoschHomeComfortService } from '../src';

const COMPANY_NAME_EXPECT = 'Bosch Home Comfort';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'boschhomecomfort-jobs.json'), 'utf8'),
);
...
describe('BoschHomeComfortService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through BoschHomeComfortModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [BoschHomeComfortModule],
      }).compile();
      const service = moduleRef.get(BoschHomeComfortService);
      expect(service).toBeInstanceOf(BoschHomeComfortService);
      await moduleRef.close();
    });

    it('exports the Site.BOSCH_HOME_COMFORT = "boschhomecomfort" enum value', () => {
      expect(Site.BOSCH_HOME_COMFORT).toBe('boschhomecomfort');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new BoschHomeComfortService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.BOSCH_HOME_COMFORT],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'boschhomecomfort-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.BOSCH_HOME_COMFORT);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('boschhomecomfort-' + first.id);
      ...
      expect(calledUrls[0]).toContain('Bosch-HomeComfort');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.BOSCH_HOME_COMFORT);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('boschhomecomfort-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new BoschHomeComfortService(registry);
      const result = await service.scrape({
        siteType: [Site.BOSCH_HOME_COMFORT],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('Bosch-HomeComfort');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('boschhomecomfort-x1');
      expect(result.jobs[0].site).toBe(Site.BOSCH_HOME_COMFORT);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('boschhomecomfort-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new BoschHomeComfortService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.BOSCH_HOME_COMFORT],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new BoschHomeComfortService();
      const result = await service.scrape({
        siteType: [Site.BOSCH_HOME_COMFORT],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
