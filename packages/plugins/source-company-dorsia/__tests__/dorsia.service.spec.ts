...
import { DorsiaModule, DorsiaService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'dorsia-jobs.json'), 'utf8'),
);
...
/**
 * Spec 837 / T04 — `DorsiaService` unit tests (≥ 9 cases).
 */
describe('DorsiaService — Spec 837 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through DorsiaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [DorsiaModule],
      }).compile();
      const service = moduleRef.get(DorsiaService);
      expect(service).toBeInstanceOf(DorsiaService);
      await moduleRef.close();
    });

    it('exports the Site.DORSIA = "dorsia" enum value', () => {
      expect(Site.DORSIA).toBe('dorsia');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new DorsiaService();
      const result = await service.scrape({
        siteType: [Site.DORSIA],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'dorsia-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.DORSIA);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Dorsia');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/dorsia/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/dorsia/jobs/');
      ...
      expect(job0?.description).toContain('Dorsia');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/dorsia/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Dorsia');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'dorsia-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
