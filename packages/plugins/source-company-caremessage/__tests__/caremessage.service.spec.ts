...
import { CareMessageModule, CareMessageService } from '../src';

const COMPANY_NAME_EXPECT = 'CareMessage';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'caremessage-jobs.json'), 'utf8'),
);
...
describe('CareMessageService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CareMessageModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CareMessageModule],
      }).compile();
      const service = moduleRef.get(CareMessageService);
      expect(service).toBeInstanceOf(CareMessageService);
      await moduleRef.close();
    });

    it('exports the Site.CAREMESSAGE = "caremessage" enum value', () => {
      expect(Site.CAREMESSAGE).toBe('caremessage');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new CareMessageService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.CAREMESSAGE],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'caremessage-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.CAREMESSAGE);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('caremessage-' + first.id);
      ...
      expect(calledUrls[0]).toContain('caremessage');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.CAREMESSAGE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('caremessage-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new CareMessageService(registry);
      const result = await service.scrape({
        siteType: [Site.CAREMESSAGE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('caremessage');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('caremessage-x1');
      expect(result.jobs[0].site).toBe(Site.CAREMESSAGE);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('caremessage-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new CareMessageService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.CAREMESSAGE],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new CareMessageService();
      const result = await service.scrape({
        siteType: [Site.CAREMESSAGE],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
