...
import { BuckMasonModule, BuckMasonService } from '../src';

const COMPANY_NAME_EXPECT = 'Buck Mason';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'buckmason-jobs.json'), 'utf8'),
);
...
describe('BuckMasonService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through BuckMasonModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [BuckMasonModule],
      }).compile();
      const service = moduleRef.get(BuckMasonService);
      expect(service).toBeInstanceOf(BuckMasonService);
      await moduleRef.close();
    });

    it('exports the Site.BUCK_MASON = "buckmason" enum value', () => {
      expect(Site.BUCK_MASON).toBe('buckmason');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new BuckMasonService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.BUCK_MASON],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('buckmason');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.BUCK_MASON);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('buckmason-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new BuckMasonService(registry);
      const result = await service.scrape({
        siteType: [Site.BUCK_MASON],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('buckmason');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('buckmason-x1');
      expect(result.jobs[0].site).toBe(Site.BUCK_MASON);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('buckmason-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new BuckMasonService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.BUCK_MASON],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new BuckMasonService();
      const result = await service.scrape({
        siteType: [Site.BUCK_MASON],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
