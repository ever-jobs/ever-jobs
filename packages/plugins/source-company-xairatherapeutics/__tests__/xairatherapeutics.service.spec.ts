...
import { XairaTherapeuticsModule, XairaTherapeuticsService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'xairatherapeutics-jobs.json'), 'utf8'),
);
...
/**
 * Spec 726 / T04 — `XairaTherapeuticsService` unit tests (≥ 9 cases).
 */
describe('XairaTherapeuticsService — Spec 726 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through XairaTherapeuticsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [XairaTherapeuticsModule],
      }).compile();
      const service = moduleRef.get(XairaTherapeuticsService);
      expect(service).toBeInstanceOf(XairaTherapeuticsService);
      await moduleRef.close();
    });

    it('exports the Site.XAIRA_THERAPEUTICS = "xairatherapeutics" enum value', () => {
      expect(Site.XAIRA_THERAPEUTICS).toBe('xairatherapeutics');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new XairaTherapeuticsService();
      const result = await service.scrape({
        siteType: [Site.XAIRA_THERAPEUTICS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'xairatherapeutics-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.XAIRA_THERAPEUTICS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Xaira Therapeutics');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/xairatherapeutics/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/xairatherapeutics/jobs/');
      ...
      expect(job0?.description).toContain('Xaira Therapeutics');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/xairatherapeutics/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Xaira Therapeutics');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'xairatherapeutics-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
