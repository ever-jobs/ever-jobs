...
import { EVERSANAModule, EVERSANAService } from '../src';

const COMPANY_NAME_EXPECT = 'EVERSANA';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'eversana-jobs.json'), 'utf8'),
);
...
describe('EVERSANAService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through EVERSANAModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [EVERSANAModule],
      }).compile();
      const service = moduleRef.get(EVERSANAService);
      expect(service).toBeInstanceOf(EVERSANAService);
      await moduleRef.close();
    });

    it('exports the Site.EVERSANA = "eversana" enum value', () => {
      expect(Site.EVERSANA).toBe('eversana');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new EVERSANAService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.EVERSANA],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('EVERSANA1');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.EVERSANA);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('eversana-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new EVERSANAService(registry);
      const result = await service.scrape({
        siteType: [Site.EVERSANA],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('EVERSANA1');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('eversana-x1');
      expect(result.jobs[0].site).toBe(Site.EVERSANA);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('eversana-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new EVERSANAService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.EVERSANA],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new EVERSANAService();
      const result = await service.scrape({
        siteType: [Site.EVERSANA],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
