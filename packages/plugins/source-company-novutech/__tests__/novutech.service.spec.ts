...
import { NovutechModule, NovutechService } from '../src';

const COMPANY_NAME_EXPECT = 'Novutech';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'novutech-jobs.json'), 'utf8'),
);
...
describe('NovutechService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through NovutechModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [NovutechModule],
      }).compile();
      const service = moduleRef.get(NovutechService);
      expect(service).toBeInstanceOf(NovutechService);
      await moduleRef.close();
    });

    it('exports the Site.NOVUTECH = "novutech" enum value', () => {
      expect(Site.NOVUTECH).toBe('novutech');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new NovutechService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.NOVUTECH],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('novutech.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.NOVUTECH);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('novutech-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new NovutechService(registry);
      const result = await service.scrape({
        siteType: [Site.NOVUTECH],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('novutech');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('novutech-x1');
      expect(result.jobs[0].site).toBe(Site.NOVUTECH);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('novutech-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new NovutechService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.NOVUTECH],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new NovutechService();
      const result = await service.scrape({
        siteType: [Site.NOVUTECH],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
