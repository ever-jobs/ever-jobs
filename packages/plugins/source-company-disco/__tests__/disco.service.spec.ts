...
import { DiscoModule, DiscoService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'disco-jobs.json'), 'utf8'),
);
...
/**
 * Spec 461 / T04 — `DiscoService` unit tests (≥ 9 cases).
 */
describe('DiscoService — Spec 461 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through DiscoModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [DiscoModule],
      }).compile();
      const service = moduleRef.get(DiscoService);
      expect(service).toBeInstanceOf(DiscoService);
      await moduleRef.close();
    });

    it('exports the Site.DISCO = "disco" enum value', () => {
      expect(Site.DISCO).toBe('disco');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new DiscoService();
      ...
      const job0 = dto.jobs.find((j) => j.id === 'disco-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.DISCO);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('DISCO');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/disco/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/disco/jobs/');
      ...
      expect(job0?.description).toContain('DISCO');
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('DISCO');
      }
    });
  });
