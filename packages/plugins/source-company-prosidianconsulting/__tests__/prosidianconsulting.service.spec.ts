...
import { ProSidianConsultingModule, ProSidianConsultingService } from '../src';

const COMPANY_NAME_EXPECT = 'ProSidian Consulting';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'prosidianconsulting-jobs.json'), 'utf8'),
);
...
describe('ProSidianConsultingService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ProSidianConsultingModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ProSidianConsultingModule],
      }).compile();
      const service = moduleRef.get(ProSidianConsultingService);
      expect(service).toBeInstanceOf(ProSidianConsultingService);
      await moduleRef.close();
    });

    it('exports the Site.PROSIDIAN_CONSULTING = "prosidianconsulting" enum value', () => {
      expect(Site.PROSIDIAN_CONSULTING).toBe('prosidianconsulting');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new ProSidianConsultingService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.PROSIDIAN_CONSULTING],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'prosidianconsulting-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.PROSIDIAN_CONSULTING);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('prosidianconsulting-' + first.id);
      ...
      expect(calledUrls[0]).toContain('prosidianconsulting');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.PROSIDIAN_CONSULTING);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('prosidianconsulting-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new ProSidianConsultingService(registry);
      const result = await service.scrape({
        siteType: [Site.PROSIDIAN_CONSULTING],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('prosidianconsulting');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('prosidianconsulting-x1');
      expect(result.jobs[0].site).toBe(Site.PROSIDIAN_CONSULTING);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('prosidianconsulting-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new ProSidianConsultingService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.PROSIDIAN_CONSULTING],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new ProSidianConsultingService();
      const result = await service.scrape({
        siteType: [Site.PROSIDIAN_CONSULTING],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
