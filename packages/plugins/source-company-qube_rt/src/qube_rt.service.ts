...
/**
 * Qube Research & Technologies — Quantitative investment management (HQ: London, United Kingdom).
 *
 * Source (Spec 1737): Greenhouse board, scraped in this order:
 *   - `quberesearchandtechnologies` — https://job-boards.greenhouse.io/quberesearchandtechnologies
 *     verified live 2026-09-24: 201 open postings.
 *
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'quberesearchandtechnologies', atsIdPrefix: 'gh-' },
];
...
@SourcePlugin({
  ...
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['qube-rt.com'],
  description: 'Qube Research & Technologies careers via Greenhouse. Tags: segment=quant-trading; industry=quantitative-investment-management.',
})
@Injectable()
export class QubeRtService implements IScraper {
  private readonly logger = new Logger(QubeRtService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Greenhouse source plugin is not registered; cannot scrape Qube Research & Technologies');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`Qube Research & Technologies: delegating to Greenhouse (${board.companySlug})`);
    ...
    this.logger.log(`Qube Research & Technologies: scraped ${jobs.length} jobs`);
