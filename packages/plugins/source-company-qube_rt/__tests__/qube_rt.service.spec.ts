...
import { QubeRtModule, QubeRtService } from '../src';
...
const FIXTURE: Fixture = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'fixtures', 'qube_rt-boards.json'), 'utf8'),
);
const COMPANY_NAME = 'Qube Research & Technologies';
const ID_PREFIX = 'qube_rt-';
...
describe('QubeRtService — Greenhouse delegation (Spec 1737)', () => {
  beforeEach(() => {
    mockGet.mockReset();
    mockPost.mockReset();
  ...
  describe('registration', () => {
    it('resolves through QubeRtModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({ imports: [QubeRtModule] }).compile();
      expect(moduleRef.get(QubeRtService)).toBeInstanceOf(QubeRtService);
      await moduleRef.close();
    });

    it('exports Site.QUBE_RT = "qube_rt"', () => {
      expect(Site.QUBE_RT).toBe('qube_rt');
    });

    it('declares a tagged company plugin with its domains', () => {
      const meta = Reflect.getMetadata(SOURCE_PLUGIN_METADATA, QubeRtService);
      expect(meta.site).toBe(Site.QUBE_RT);
      expect(meta.category).toBe('company');
      expect(meta.isAts).toBeFalsy();
      expect(meta.companyDomains).toEqual(['qube-rt.com']);
  ...
  describe('recorded board (real Greenhouse adapter, mocked HTTP)', () => {
    it('maps every recorded posting and re-stamps the company identity', async () => {
      const service = new QubeRtService(registryWith());
      const result = await service.scrape({ siteType: [Site.QUBE_RT], resultsWanted: 100 } as ScraperInputDto);
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.QUBE_RT);
    ...
    it('only requests the recorded board URLs', async () => {
      const service = new QubeRtService(registryWith());
      await service.scrape({ siteType: [Site.QUBE_RT] } as ScraperInputDto);
    ...
    it('honours resultsWanted=1', async () => {
      const service = new QubeRtService(registryWith());
      const result = await service.scrape({ siteType: [Site.QUBE_RT], resultsWanted: 1 } as ScraperInputDto);
    ...
    it('resolves with an empty result when the board is gone (HTTP 404)', async () => {
      ...
      const service = new QubeRtService(registryWith());
      const result = await service.scrape({ siteType: [Site.QUBE_RT] } as ScraperInputDto);
  ...
  describe('delegation contract', () => {
    it('forwards the board and the caller input untouched', async () => {
      ...
      const service = new QubeRtService(
        registryWith(
          fakeBackend(
      ...
      expect(result.jobs[0].site).toBe(Site.QUBE_RT);
      expect(result.jobs[0].companyName).toBe(COMPANY_NAME);
    });

    it('passes an absent resultsWanted through as absent', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new QubeRtService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      await service.scrape({ siteType: [Site.QUBE_RT] } as ScraperInputDto);
    ...
    it('makes no request when resultsWanted is 0', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new QubeRtService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      const result = await service.scrape({ siteType: [Site.QUBE_RT], resultsWanted: 0 } as ScraperInputDto);
  ...
  describe('resilience', () => {
    it('reports not_registered when no registry is injected', async () => {
      const result = await new QubeRtService().scrape({ siteType: [Site.QUBE_RT] } as ScraperInputDto);
      expect(result.jobs).toEqual([]);
    ...
    it('reports not_registered when Greenhouse is missing from the registry', async () => {
      const result = await new QubeRtService(new PluginRegistry()).scrape({
        siteType: [Site.QUBE_RT],
      } as ScraperInputDto);
      expect(result.diagnostics?.reason).toBe('not_registered');
    });
