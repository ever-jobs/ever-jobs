...
import { VerticalAerospaceModule, VerticalAerospaceService } from '../src';

const COMPANY_NAME_EXPECT = 'Vertical Aerospace';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'verticalaerospace-jobs.json'), 'utf8'),
);
...
describe('VerticalAerospaceService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through VerticalAerospaceModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [VerticalAerospaceModule],
      }).compile();
      const service = moduleRef.get(VerticalAerospaceService);
      expect(service).toBeInstanceOf(VerticalAerospaceService);
      await moduleRef.close();
    });

    it('exports the Site.VERTICAL_AEROSPACE = "verticalaerospace" enum value', () => {
      expect(Site.VERTICAL_AEROSPACE).toBe('verticalaerospace');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new VerticalAerospaceService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.VERTICAL_AEROSPACE],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'verticalaerospace-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.VERTICAL_AEROSPACE);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('verticalaerospace-' + first.id);
      ...
      expect(calledUrls[0]).toContain('vertical-aerospace');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.VERTICAL_AEROSPACE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('verticalaerospace-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new VerticalAerospaceService(registry);
      const result = await service.scrape({
        siteType: [Site.VERTICAL_AEROSPACE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('vertical-aerospace');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('verticalaerospace-x1');
      expect(result.jobs[0].site).toBe(Site.VERTICAL_AEROSPACE);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('verticalaerospace-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new VerticalAerospaceService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.VERTICAL_AEROSPACE],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new VerticalAerospaceService();
      const result = await service.scrape({
        siteType: [Site.VERTICAL_AEROSPACE],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
