...
import { GammaModule, GammaService } from '../src';

const COMPANY_NAME_EXPECT = 'Gamma';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'gamma-jobs.json'), 'utf8'),
);
...
describe('GammaService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through GammaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [GammaModule],
      }).compile();
      const service = moduleRef.get(GammaService);
      expect(service).toBeInstanceOf(GammaService);
      await moduleRef.close();
    });

    it('exports the Site.GAMMA = "gamma" enum value', () => {
      expect(Site.GAMMA).toBe('gamma');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new GammaService(registryWithAshby());
      ...
      expect(calledUrls[0]).toContain('gamma');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.GAMMA);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('gamma-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new GammaService(registry);
      const result = await service.scrape({
        siteType: [Site.GAMMA],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('gamma');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('gamma-x1');
      expect(result.jobs[0].site).toBe(Site.GAMMA);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('gamma-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new GammaService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.GAMMA],
      } as ScraperInputDto);
