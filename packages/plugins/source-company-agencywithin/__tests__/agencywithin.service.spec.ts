...
import { AgencywithinModule, AgencywithinService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'agencywithin-jobs.json'), 'utf8'),
);
...
/**
 * Spec 207 / T04 — `AgencywithinService` unit tests (≥ 9 cases).
 */
describe('AgencywithinService — Spec 207 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AgencywithinModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AgencywithinModule],
      }).compile();
      const service = moduleRef.get(AgencywithinService);
      expect(service).toBeInstanceOf(AgencywithinService);
      await moduleRef.close();
    });

    it('exports the Site.AGENCYWITHIN = "agencywithin" enum value', () => {
      expect(Site.AGENCYWITHIN).toBe('agencywithin');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AgencywithinService();
      const result = await service.scrape({
        siteType: [Site.AGENCYWITHIN],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'agencywithin-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.AGENCYWITHIN);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('WITHIN');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/agencywithin/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/agencywithin/jobs/');
      ...
      expect(job0?.description).toContain('WITHIN');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/agencywithin/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('WITHIN');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'agencywithin-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
