...
import { BelvedereTradingModule, BelvedereTradingService } from '../src';
...
const FIXTURE: Fixture = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'fixtures', 'belvederetrading-boards.json'), 'utf8'),
);
...
function registryWith(scraper: IScraper = new LeverService()): PluginRegistry {
  const registry = new PluginRegistry();
  registry.register({ site: Site.LEVER, name: 'Lever', category: 'ats', isAts: true }, scraper);
  return registry;
}
...
describe('BelvedereTradingService — Lever delegation (Spec 1737)', () => {
  beforeEach(() => {
    mockGet.mockReset();
    mockPost.mockReset();
  ...
  describe('registration', () => {
    it('resolves through BelvedereTradingModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({ imports: [BelvedereTradingModule] }).compile();
      expect(moduleRef.get(BelvedereTradingService)).toBeInstanceOf(BelvedereTradingService);
      await moduleRef.close();
    });

    it('exports Site.BELVEDERE_TRADING = "belvederetrading"', () => {
      expect(Site.BELVEDERE_TRADING).toBe('belvederetrading');
    });

    it('declares a tagged company plugin with its domains', () => {
      const meta = Reflect.getMetadata(SOURCE_PLUGIN_METADATA, BelvedereTradingService);
      expect(meta.site).toBe(Site.BELVEDERE_TRADING);
      expect(meta.category).toBe('company');
      expect(meta.isAts).toBeFalsy();
      expect(meta.companyDomains).toEqual(['belvederetrading.com']);
      expect(meta.description).toContain('segment=quant-trading');
    });
  });

  describe('recorded board (real Lever adapter, mocked HTTP)', () => {
    it('maps every recorded posting and re-stamps the company identity', async () => {
      const service = new BelvedereTradingService(registryWith());
      const result = await service.scrape({ siteType: [Site.BELVEDERE_TRADING], resultsWanted: 100 } as ScraperInputDto);
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.BELVEDERE_TRADING);
    ...
    it('only requests the recorded board URLs', async () => {
      const service = new BelvedereTradingService(registryWith());
      await service.scrape({ siteType: [Site.BELVEDERE_TRADING] } as ScraperInputDto);
    ...
    it('honours resultsWanted=1', async () => {
      const service = new BelvedereTradingService(registryWith());
      const result = await service.scrape({ siteType: [Site.BELVEDERE_TRADING], resultsWanted: 1 } as ScraperInputDto);
    ...
    it('resolves with an empty result when the board is gone (HTTP 404)', async () => {
      ...
      const service = new BelvedereTradingService(registryWith());
      const result = await service.scrape({ siteType: [Site.BELVEDERE_TRADING] } as ScraperInputDto);
  ...
  describe('delegation contract', () => {
    it('forwards the board and the caller input untouched', async () => {
      ...
      const service = new BelvedereTradingService(
        registryWith(
          fakeBackend(
      ...
      const result = await service.scrape({
        siteType: [Site.BELVEDERE_TRADING],
      ...
      expect(result.jobs[0].site).toBe(Site.BELVEDERE_TRADING);
    ...
    it('never forwards the caller\'s credentials to the board (Spec 1735 §4.5)', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new BelvedereTradingService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      await service.scrape({
        siteType: [Site.BELVEDERE_TRADING],
        auth: { lever: { apiKey: 'caller-key' } },
    ...
    it('makes no request when resultsWanted is 0', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new BelvedereTradingService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      const result = await service.scrape({ siteType: [Site.BELVEDERE_TRADING], resultsWanted: 0 } as ScraperInputDto);
  ...
  describe('resilience', () => {
    it('reports not_registered when no registry is injected', async () => {
      const result = await new BelvedereTradingService().scrape({ siteType: [Site.BELVEDERE_TRADING] } as ScraperInputDto);
      expect(result.jobs).toEqual([]);
      expect(result.diagnostics?.reason).toBe('not_registered');
      expect(result.diagnostics?.detail).toContain('Lever');
    });

    it('reports not_registered when Lever is missing from the registry', async () => {
      const result = await new BelvedereTradingService(new PluginRegistry()).scrape({
        siteType: [Site.BELVEDERE_TRADING],
      } as ScraperInputDto);
      expect(result.diagnostics?.reason).toBe('not_registered');
    });
