...
import { CharlieHealthModule, CharlieHealthService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'charliehealth-jobs.json'), 'utf8'),
);
...
/**
 * Spec 664 / T04 — `CharlieHealthService` unit tests (≥ 9 cases).
 */
describe('CharlieHealthService — Spec 664 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CharlieHealthModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CharlieHealthModule],
      }).compile();
      const service = moduleRef.get(CharlieHealthService);
      expect(service).toBeInstanceOf(CharlieHealthService);
      await moduleRef.close();
    });

    it('exports the Site.CHARLIE_HEALTH = "charliehealth" enum value', () => {
      expect(Site.CHARLIE_HEALTH).toBe('charliehealth');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new CharlieHealthService();
      const result = await service.scrape({
        siteType: [Site.CHARLIE_HEALTH],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'charliehealth-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.CHARLIE_HEALTH);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Charlie Health');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/charliehealth/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/charliehealth/jobs/');
      ...
      expect(job0?.description).toContain('Charlie Health');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/charliehealth/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Charlie Health');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'charliehealth-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
