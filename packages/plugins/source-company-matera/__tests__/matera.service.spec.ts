...
import { MateraModule, MateraService } from '../src';

const COMPANY_NAME_EXPECT = 'Matera';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'matera-jobs.json'), 'utf8'),
);
...
describe('MateraService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through MateraModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [MateraModule],
      }).compile();
      const service = moduleRef.get(MateraService);
      expect(service).toBeInstanceOf(MateraService);
      await moduleRef.close();
    });

    it('exports the Site.MATERA = "matera" enum value', () => {
      expect(Site.MATERA).toBe('matera');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new MateraService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.MATERA],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('matera.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.MATERA);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('matera-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new MateraService(registry);
      const result = await service.scrape({
        siteType: [Site.MATERA],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('matera');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('matera-x1');
      expect(result.jobs[0].site).toBe(Site.MATERA);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('matera-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new MateraService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.MATERA],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new MateraService();
      const result = await service.scrape({
        siteType: [Site.MATERA],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
