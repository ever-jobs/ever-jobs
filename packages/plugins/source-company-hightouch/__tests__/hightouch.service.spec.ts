...
import { HightouchModule, HightouchService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'hightouch-jobs.json'), 'utf8'),
);
...
/**
 * Spec 430 / T04 — `HightouchService` unit tests (≥ 9 cases).
 */
describe('HightouchService — Spec 430 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through HightouchModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [HightouchModule],
      }).compile();
      const service = moduleRef.get(HightouchService);
      expect(service).toBeInstanceOf(HightouchService);
      await moduleRef.close();
    });

    it('exports the Site.HIGHTOUCH = "hightouch" enum value', () => {
      expect(Site.HIGHTOUCH).toBe('hightouch');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new HightouchService();
      const result = await service.scrape({
        siteType: [Site.HIGHTOUCH],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'hightouch-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.HIGHTOUCH);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Hightouch');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/hightouch/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/hightouch/jobs/');
      ...
      expect(job0?.description).toContain('Hightouch');
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Hightouch');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'hightouch-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
