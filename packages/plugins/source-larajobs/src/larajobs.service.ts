...
@Injectable()
export class LaraJobsService implements IScraper {
  private readonly logger = new Logger(LaraJobsService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    client.setHeaders(LARAJOBS_HEADERS);

    this.logger.log(
      `Fetching LaraJobs RSS feed (resultsWanted=${resultsWanted})`,
    );

    try {
      const response = await client.get(LARAJOBS_RSS_URL);
      const xml = response.data;

      if (!xml || typeof xml !== 'string') {
        this.logger.warn('Empty or invalid RSS response from LaraJobs');
        return new JobResponseDto([]);
      }

      const items = this.parseRssItems(xml);
      this.logger.log(`Parsed ${items.length} items from LaraJobs RSS feed`);
      ...
      for (const item of items) {
        ...
        try {
          ...
          this.logger.warn(
            `Error mapping LaraJobs job ${item.link}: ${err.message}`,
          );
        }
      }

      this.logger.log(`LaraJobs returned ${jobs.length} jobs`);
      return new JobResponseDto(jobs);
    } catch (err: any) {
      this.logger.error(`LaraJobs scrape error: ${err.message}`);
      return new JobResponseDto([], classifyScrapeError(err));
    }
  }

  /**
   * Parse RSS XML into an array of LaraJobsRssItem using regex (no XML library).
   */
  private parseRssItems(xml: string): LaraJobsRssItem[] {
    const items: LaraJobsRssItem[] = [];
   ...
   */
  private matchesSearch(item: LaraJobsRssItem, searchTerm: string): boolean {
   ...
   */
  private mapJob(
    item: LaraJobsRssItem,
    descriptionFormat?: DescriptionFormat,
  ): JobPostDto | null {
    if (!item.title || !item.link) return null;

    // Process description (LaraJobs RSS may return HTML)
