...
import { UP42Module, UP42Service } from '../src';

const COMPANY_NAME_EXPECT = 'UP42';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'up42-jobs.json'), 'utf8'),
);
...
describe('UP42Service — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through UP42Module via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [UP42Module],
      }).compile();
      const service = moduleRef.get(UP42Service);
      expect(service).toBeInstanceOf(UP42Service);
      await moduleRef.close();
    });

    it('exports the Site.UP42 = "up42" enum value', () => {
      expect(Site.UP42).toBe('up42');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new UP42Service(registryWithRecruitee());
      ...
      expect(calledUrls[0]).toContain('up42gmbh.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.UP42);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('up42-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new UP42Service(registry);
      const result = await service.scrape({
        siteType: [Site.UP42],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('up42gmbh');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('up42-x1');
      expect(result.jobs[0].site).toBe(Site.UP42);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('up42-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new UP42Service(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.UP42],
      } as ScraperInputDto);
