...
import { IDMeModule, IDMeService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'idme-jobs.json'), 'utf8'),
);
...
/**
 * Spec 867 / T04 — `IDMeService` unit tests (≥ 9 cases).
 */
describe('IDMeService — Spec 867 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through IDMeModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [IDMeModule],
      }).compile();
      const service = moduleRef.get(IDMeService);
      expect(service).toBeInstanceOf(IDMeService);
      await moduleRef.close();
    });

    it('exports the Site.ID_ME = "idme" enum value', () => {
      expect(Site.ID_ME).toBe('idme');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const job0 = dto.jobs.find((j) => j.id === 'idme-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ID_ME);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('ID.me');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/idme/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/idme/jobs/');
      ...
      expect(job0?.description).toContain('ID.me');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/idme/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('ID.me');
      }
    });
  });
