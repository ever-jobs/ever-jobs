...
import { RecordedFutureModule, RecordedFutureService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'recordedfuture-jobs.json'), 'utf8'),
);
...
/**
 * Spec 918 / T04 — `RecordedFutureService` unit tests (≥ 9 cases).
 */
describe('RecordedFutureService — Spec 918 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through RecordedFutureModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [RecordedFutureModule],
      }).compile();
      const service = moduleRef.get(RecordedFutureService);
      expect(service).toBeInstanceOf(RecordedFutureService);
      await moduleRef.close();
    });

    it('exports the Site.RECORDED_FUTURE = "recordedfuture" enum value', () => {
      expect(Site.RECORDED_FUTURE).toBe('recordedfuture');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new RecordedFutureService();
      const result = await service.scrape({
        siteType: [Site.RECORDED_FUTURE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'recordedfuture-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.RECORDED_FUTURE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Recorded Future');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/recordedfuture/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/recordedfuture/jobs/');
      ...
      expect(job0?.description).toContain('Recorded Future');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/recordedfuture/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Recorded Future');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'recordedfuture-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
