...
/**
 * General Motors — Automotive (HQ: Detroit, MI, USA).
 *
 * Source (Spec 1736): Workday board, scraped in this order:
 *   - `generalmotors:5:Careers_GM` — https://generalmotors.wd5.myworkdayjobs.com/Careers_GM
 *     verified live 2026-09-24: 865 open postings.
 *
 ...
 * party board.
 *
 * Tags: segment=workday-enterprise; industry=automotive.
 */
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'generalmotors:5:Careers_GM', atsIdPrefix: 'wd-generalmotors-' },
];
...
@SourcePlugin({
  site: Site.GENERAL_MOTORS,
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['gm.com'],
  description: 'General Motors careers via Workday. Tags: segment=workday-enterprise; industry=automotive.',
})
@Injectable()
export class GeneralMotorsService implements IScraper {
  private readonly logger = new Logger(GeneralMotorsService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Workday source plugin is not registered; cannot scrape General Motors');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`General Motors: delegating to Workday (${board.companySlug})`);
    ...
    this.logger.log(`General Motors: scraped ${jobs.length} jobs`);
