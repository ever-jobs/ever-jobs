...
import { GeneralMotorsModule, GeneralMotorsService } from '../src';
...
const FIXTURE: Fixture = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'fixtures', 'generalmotors-boards.json'), 'utf8'),
);
const COMPANY_NAME = 'General Motors';
const ID_PREFIX = 'generalmotors-';
...
describe('GeneralMotorsService — Workday delegation (Spec 1736)', () => {
  beforeEach(() => {
    mockGet.mockReset();
    mockPost.mockReset();
  ...
  describe('registration', () => {
    it('resolves through GeneralMotorsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({ imports: [GeneralMotorsModule] }).compile();
      expect(moduleRef.get(GeneralMotorsService)).toBeInstanceOf(GeneralMotorsService);
      await moduleRef.close();
    });

    it('exports Site.GENERAL_MOTORS = "generalmotors"', () => {
      expect(Site.GENERAL_MOTORS).toBe('generalmotors');
    });

    it('declares a tagged company plugin with its domains', () => {
      const meta = Reflect.getMetadata(SOURCE_PLUGIN_METADATA, GeneralMotorsService);
      expect(meta.site).toBe(Site.GENERAL_MOTORS);
      expect(meta.category).toBe('company');
      expect(meta.isAts).toBeFalsy();
      expect(meta.companyDomains).toEqual(['gm.com']);
  ...
  describe('recorded board (real Workday adapter, mocked HTTP)', () => {
    it('maps every recorded posting and re-stamps the company identity', async () => {
      const service = new GeneralMotorsService(registryWith());
      const result = await service.scrape({ siteType: [Site.GENERAL_MOTORS], resultsWanted: 100 } as ScraperInputDto);
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.GENERAL_MOTORS);
    ...
    it('only requests the recorded board URLs', async () => {
      const service = new GeneralMotorsService(registryWith());
      await service.scrape({ siteType: [Site.GENERAL_MOTORS] } as ScraperInputDto);
    ...
    it('honours resultsWanted=1', async () => {
      const service = new GeneralMotorsService(registryWith());
      const result = await service.scrape({ siteType: [Site.GENERAL_MOTORS], resultsWanted: 1 } as ScraperInputDto);
    ...
    it('resolves with an empty result when the board is gone (HTTP 404)', async () => {
      ...
      const service = new GeneralMotorsService(registryWith());
      const result = await service.scrape({ siteType: [Site.GENERAL_MOTORS] } as ScraperInputDto);
  ...
  describe('delegation contract', () => {
    it('forwards the board and the caller input untouched', async () => {
      ...
      const service = new GeneralMotorsService(
        registryWith(
          fakeBackend(
      ...
      expect(result.jobs[0].site).toBe(Site.GENERAL_MOTORS);
    ...
    it('never forwards the caller\'s credentials to the board (Spec 1735 §4.5)', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new GeneralMotorsService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
    ...
    it('makes no request when resultsWanted is 0', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new GeneralMotorsService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      const result = await service.scrape({ siteType: [Site.GENERAL_MOTORS], resultsWanted: 0 } as ScraperInputDto);
  ...
  describe('resilience', () => {
    it('reports not_registered when no registry is injected', async () => {
      const result = await new GeneralMotorsService().scrape({ siteType: [Site.GENERAL_MOTORS] } as ScraperInputDto);
      expect(result.jobs).toEqual([]);
    ...
    it('reports not_registered when Workday is missing from the registry', async () => {
      const result = await new GeneralMotorsService(new PluginRegistry()).scrape({
        siteType: [Site.GENERAL_MOTORS],
      } as ScraperInputDto);
      expect(result.diagnostics?.reason).toBe('not_registered');
    });
