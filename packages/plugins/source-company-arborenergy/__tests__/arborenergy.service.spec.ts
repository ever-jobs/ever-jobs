...
import { ArborEnergyModule, ArborEnergyService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'arborenergy-jobs.json'), 'utf8'),
);
...
/**
 * Spec 789 / T04 — `ArborEnergyService` unit tests (≥ 9 cases).
 */
describe('ArborEnergyService — Spec 789 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ArborEnergyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ArborEnergyModule],
      }).compile();
      const service = moduleRef.get(ArborEnergyService);
      expect(service).toBeInstanceOf(ArborEnergyService);
      await moduleRef.close();
    });

    it('exports the Site.ARBOR_ENERGY = "arborenergy" enum value', () => {
      expect(Site.ARBOR_ENERGY).toBe('arborenergy');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ArborEnergyService();
      const result = await service.scrape({
        siteType: [Site.ARBOR_ENERGY],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'arborenergy-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ARBOR_ENERGY);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Arbor Energy');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/arborenergy/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/arborenergy/jobs/');
      ...
      expect(job0?.description).toContain('Arbor Energy');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/arborenergy/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Arbor Energy');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'arborenergy-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
