...
import { RiverlaneModule, RiverlaneService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'riverlane-jobs.json'), 'utf8'),
);
...
/**
 * Spec 705 / T04 — `RiverlaneService` unit tests (≥ 9 cases).
 */
describe('RiverlaneService — Spec 705 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through RiverlaneModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [RiverlaneModule],
      }).compile();
      const service = moduleRef.get(RiverlaneService);
      expect(service).toBeInstanceOf(RiverlaneService);
      await moduleRef.close();
    });

    it('exports the Site.RIVERLANE = "riverlane" enum value', () => {
      expect(Site.RIVERLANE).toBe('riverlane');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new RiverlaneService();
      const result = await service.scrape({
        siteType: [Site.RIVERLANE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'riverlane-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.RIVERLANE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Riverlane');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/riverlane/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/riverlane/jobs/');
      ...
      expect(job0?.description).toContain('Riverlane');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/riverlane/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Riverlane');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'riverlane-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
