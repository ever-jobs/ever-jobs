...
import { ElephantTechnologiesModule, ElephantTechnologiesService } from '../src';

const COMPANY_NAME_EXPECT = 'Elephant Technologies';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'elephanttechnologies-jobs.json'), 'utf8'),
);
...
describe('ElephantTechnologiesService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ElephantTechnologiesModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ElephantTechnologiesModule],
      }).compile();
      const service = moduleRef.get(ElephantTechnologiesService);
      expect(service).toBeInstanceOf(ElephantTechnologiesService);
      await moduleRef.close();
    });

    it('exports the Site.ELEPHANT_TECHNOLOGIES = "elephanttechnologies" enum value', () => {
      expect(Site.ELEPHANT_TECHNOLOGIES).toBe('elephanttechnologies');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new ElephantTechnologiesService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.ELEPHANT_TECHNOLOGIES],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'elephanttechnologies-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Recruitee's defaults
      expect(job0?.site).toBe(Site.ELEPHANT_TECHNOLOGIES);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('elephanttechnologies-' + first.id);
      ...
      expect(calledUrls[0]).toContain('elephanttechnologies.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ELEPHANT_TECHNOLOGIES);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('elephanttechnologies-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new ElephantTechnologiesService(registry);
      const result = await service.scrape({
        siteType: [Site.ELEPHANT_TECHNOLOGIES],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('elephanttechnologies');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('elephanttechnologies-x1');
      expect(result.jobs[0].site).toBe(Site.ELEPHANT_TECHNOLOGIES);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('elephanttechnologies-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new ElephantTechnologiesService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.ELEPHANT_TECHNOLOGIES],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new ElephantTechnologiesService();
      const result = await service.scrape({
        siteType: [Site.ELEPHANT_TECHNOLOGIES],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
