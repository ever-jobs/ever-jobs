...
import { NatuvionModule, NatuvionService } from '../src';

const COMPANY_NAME_EXPECT = 'Natuvion';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'natuvion-jobs.json'), 'utf8'),
);
...
describe('NatuvionService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through NatuvionModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [NatuvionModule],
      }).compile();
      const service = moduleRef.get(NatuvionService);
      expect(service).toBeInstanceOf(NatuvionService);
      await moduleRef.close();
    });

    it('exports the Site.NATUVION = "natuvion" enum value', () => {
      expect(Site.NATUVION).toBe('natuvion');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new NatuvionService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.NATUVION],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('natuvion.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.NATUVION);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('natuvion-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new NatuvionService(registry);
      const result = await service.scrape({
        siteType: [Site.NATUVION],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('natuvion');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('natuvion-x1');
      expect(result.jobs[0].site).toBe(Site.NATUVION);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('natuvion-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new NatuvionService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.NATUVION],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new NatuvionService();
      const result = await service.scrape({
        siteType: [Site.NATUVION],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
