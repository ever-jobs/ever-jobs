...
import { CircadiaHealthModule, CircadiaHealthService } from '../src';

const COMPANY_NAME_EXPECT = 'Circadia Health';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'circadiahealth-jobs.json'), 'utf8'),
);
...
describe('CircadiaHealthService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CircadiaHealthModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CircadiaHealthModule],
      }).compile();
      const service = moduleRef.get(CircadiaHealthService);
      expect(service).toBeInstanceOf(CircadiaHealthService);
      await moduleRef.close();
    });

    it('exports the Site.CIRCADIA_HEALTH = "circadiahealth" enum value', () => {
      expect(Site.CIRCADIA_HEALTH).toBe('circadiahealth');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new CircadiaHealthService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.CIRCADIA_HEALTH],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'circadiahealth-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.CIRCADIA_HEALTH);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('circadiahealth-' + first.id);
      ...
      expect(calledUrls[0]).toContain('circadiahealth');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.CIRCADIA_HEALTH);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('circadiahealth-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new CircadiaHealthService(registry);
      const result = await service.scrape({
        siteType: [Site.CIRCADIA_HEALTH],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('circadiahealth');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('circadiahealth-x1');
      expect(result.jobs[0].site).toBe(Site.CIRCADIA_HEALTH);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('circadiahealth-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new CircadiaHealthService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.CIRCADIA_HEALTH],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new CircadiaHealthService();
      const result = await service.scrape({
        siteType: [Site.CIRCADIA_HEALTH],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
