...
import { SuntivaModule, SuntivaService } from '../src';

const COMPANY_NAME_EXPECT = 'Suntiva';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'suntiva-jobs.json'), 'utf8'),
);
...
describe('SuntivaService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SuntivaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SuntivaModule],
      }).compile();
      const service = moduleRef.get(SuntivaService);
      expect(service).toBeInstanceOf(SuntivaService);
      await moduleRef.close();
    });

    it('exports the Site.SUNTIVA = "suntiva" enum value', () => {
      expect(Site.SUNTIVA).toBe('suntiva');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new SuntivaService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.SUNTIVA],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('SuntivaLLC');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SUNTIVA);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('suntiva-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new SuntivaService(registry);
      const result = await service.scrape({
        siteType: [Site.SUNTIVA],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('SuntivaLLC');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('suntiva-x1');
      expect(result.jobs[0].site).toBe(Site.SUNTIVA);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('suntiva-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new SuntivaService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.SUNTIVA],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new SuntivaService();
      const result = await service.scrape({
        siteType: [Site.SUNTIVA],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
