...
import { GotionModule, GotionService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'gotion-jobs.json'), 'utf8'),
);
...
/**
 * Spec 859 / T04 — `GotionService` unit tests (≥ 9 cases).
 */
describe('GotionService — Spec 859 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through GotionModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [GotionModule],
      }).compile();
      const service = moduleRef.get(GotionService);
      expect(service).toBeInstanceOf(GotionService);
      await moduleRef.close();
    });

    it('exports the Site.GOTION = "gotion" enum value', () => {
      expect(Site.GOTION).toBe('gotion');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new GotionService();
      const result = await service.scrape({
        siteType: [Site.GOTION],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'gotion-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.GOTION);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Gotion');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/gotion/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/gotion/jobs/');
      ...
      expect(job0?.description).toContain('Gotion');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/gotion/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Gotion');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'gotion-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
