...
import { AnchorageDigitalModule, AnchorageDigitalService } from '../src';

const COMPANY_NAME_EXPECT = 'Anchorage Digital';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'anchorage-jobs.json'), 'utf8'),
);
...
describe('AnchorageDigitalService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AnchorageDigitalModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AnchorageDigitalModule],
      }).compile();
      const service = moduleRef.get(AnchorageDigitalService);
      expect(service).toBeInstanceOf(AnchorageDigitalService);
      await moduleRef.close();
    });

    it('exports the Site.ANCHORAGE_DIGITAL = "anchorage" enum value', () => {
      expect(Site.ANCHORAGE_DIGITAL).toBe('anchorage');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new AnchorageDigitalService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.ANCHORAGE_DIGITAL],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'anchorage-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.ANCHORAGE_DIGITAL);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('anchorage-' + first.id);
      ...
      expect(calledUrls[0]).toContain('anchorage');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ANCHORAGE_DIGITAL);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('anchorage-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new AnchorageDigitalService(registry);
      const result = await service.scrape({
        siteType: [Site.ANCHORAGE_DIGITAL],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('anchorage');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('anchorage-x1');
      expect(result.jobs[0].site).toBe(Site.ANCHORAGE_DIGITAL);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('anchorage-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new AnchorageDigitalService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.ANCHORAGE_DIGITAL],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new AnchorageDigitalService();
      const result = await service.scrape({
        siteType: [Site.ANCHORAGE_DIGITAL],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
