...
import { AdvancedtechnologyservicesModule, AdvancedtechnologyservicesService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'advancedtechnologyservices-jobs.json'), 'utf8'),
);
...
/**
 * Spec 193 / T04 — `AdvancedtechnologyservicesService` unit tests (≥ 9 cases).
 */
describe('AdvancedtechnologyservicesService — Spec 193 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AdvancedtechnologyservicesModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AdvancedtechnologyservicesModule],
      }).compile();
      const service = moduleRef.get(AdvancedtechnologyservicesService);
      expect(service).toBeInstanceOf(AdvancedtechnologyservicesService);
      await moduleRef.close();
    });

    it('exports the Site.ADVANCEDTECHNOLOGYSERVICES = "advancedtechnologyservices" enum value', () => {
      expect(Site.ADVANCEDTECHNOLOGYSERVICES).toBe('advancedtechnologyservices');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AdvancedtechnologyservicesService();
      const result = await service.scrape({
        siteType: [Site.ADVANCEDTECHNOLOGYSERVICES],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'advancedtechnologyservices-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ADVANCEDTECHNOLOGYSERVICES);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Advanced Technology Services');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/advancedtechnologyservices/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/advancedtechnologyservices/jobs/');
      ...
      expect(job0?.description).toContain('Advanced Technology Services');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/advancedtechnologyservices/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Advanced Technology Services');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'advancedtechnologyservices-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
