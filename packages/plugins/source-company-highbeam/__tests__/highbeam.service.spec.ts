...
import { HighbeamModule, HighbeamService } from '../src';

const COMPANY_NAME_EXPECT = 'Highbeam';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'highbeam-jobs.json'), 'utf8'),
);
...
describe('HighbeamService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through HighbeamModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [HighbeamModule],
      }).compile();
      const service = moduleRef.get(HighbeamService);
      expect(service).toBeInstanceOf(HighbeamService);
      await moduleRef.close();
    });

    it('exports the Site.HIGHBEAM = "highbeam" enum value', () => {
      expect(Site.HIGHBEAM).toBe('highbeam');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new HighbeamService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.HIGHBEAM],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('highbeam');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.HIGHBEAM);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('highbeam-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new HighbeamService(registry);
      const result = await service.scrape({
        siteType: [Site.HIGHBEAM],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('highbeam');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('highbeam-x1');
      expect(result.jobs[0].site).toBe(Site.HIGHBEAM);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('highbeam-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new HighbeamService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.HIGHBEAM],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new HighbeamService();
      const result = await service.scrape({
        siteType: [Site.HIGHBEAM],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
