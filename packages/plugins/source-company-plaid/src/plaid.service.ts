...
/**
 * Plaid publishes its careers board through Greenhouse at the bare
 * `plaid` slug (no asymmetry; see Spec 030 § 10 D-05).
 */
const API_URL = 'https://api.greenhouse.io/v1/boards/plaid/jobs';
...
export class PlaidService implements IScraper {
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    try {
      ...
      this.logger.log(`Plaid: fetching ${url}`);
        ...
        jobs.push(
          new JobPostDto({
            ...
            jobUrl:
              ...
              `https://plaid.com/careers/openings/${listing.id}/`,
            location,
      ...
      this.logger.log(`Plaid: scraped ${jobs.length} jobs`);
    } catch (err: any) {
      this.logger.error(`Plaid scrape failed: ${err.message}`);
