...
import { AgilysysModule, AgilysysService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'agilysys-jobs.json'), 'utf8'),
);
...
/**
 * Spec 209 / T04 — `AgilysysService` unit tests (≥ 9 cases).
 */
describe('AgilysysService — Spec 209 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AgilysysModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AgilysysModule],
      }).compile();
      const service = moduleRef.get(AgilysysService);
      expect(service).toBeInstanceOf(AgilysysService);
      await moduleRef.close();
    });

    it('exports the Site.AGILYSYS = "agilysys" enum value', () => {
      expect(Site.AGILYSYS).toBe('agilysys');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AgilysysService();
      const result = await service.scrape({
        siteType: [Site.AGILYSYS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'agilysys-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.AGILYSYS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Agilysys');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/agilysys/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/agilysys/jobs/');
      ...
      expect(job0?.description).toContain('Agilysys');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/agilysys/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Agilysys');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'agilysys-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
