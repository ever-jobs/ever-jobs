...
import { OpenSpaceModule, OpenSpaceService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'openspace-jobs.json'), 'utf8'),
);
...
/**
 * Spec 900 / T04 — `OpenSpaceService` unit tests (≥ 9 cases).
 */
describe('OpenSpaceService — Spec 900 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through OpenSpaceModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [OpenSpaceModule],
      }).compile();
      const service = moduleRef.get(OpenSpaceService);
      expect(service).toBeInstanceOf(OpenSpaceService);
      await moduleRef.close();
    });

    it('exports the Site.OPENSPACE = "openspace" enum value', () => {
      expect(Site.OPENSPACE).toBe('openspace');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new OpenSpaceService();
      const result = await service.scrape({
        siteType: [Site.OPENSPACE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'openspace-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.OPENSPACE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('OpenSpace');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/openspace/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/openspace/jobs/');
      ...
      expect(job0?.description).toContain('OpenSpace');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/openspace/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('OpenSpace');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'openspace-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
