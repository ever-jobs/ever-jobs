...
import { LegalFlyModule, LegalFlyService } from '../src';

const COMPANY_NAME_EXPECT = 'LegalFly';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'legalfly-jobs.json'), 'utf8'),
);
...
describe('LegalFlyService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through LegalFlyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [LegalFlyModule],
      }).compile();
      const service = moduleRef.get(LegalFlyService);
      expect(service).toBeInstanceOf(LegalFlyService);
      await moduleRef.close();
    });

    it('exports the Site.LEGALFLY = "legalfly" enum value', () => {
      expect(Site.LEGALFLY).toBe('legalfly');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new LegalFlyService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.LEGALFLY],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('legalfly.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.LEGALFLY);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('legalfly-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new LegalFlyService(registry);
      const result = await service.scrape({
        siteType: [Site.LEGALFLY],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('legalfly');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('legalfly-x1');
      expect(result.jobs[0].site).toBe(Site.LEGALFLY);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('legalfly-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new LegalFlyService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.LEGALFLY],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new LegalFlyService();
      const result = await service.scrape({
        siteType: [Site.LEGALFLY],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
