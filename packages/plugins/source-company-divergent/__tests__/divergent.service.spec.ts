...
import { DivergentModule, DivergentService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'divergent-jobs.json'), 'utf8'),
);
...
/**
 * Spec 610 / T04 — `DivergentService` unit tests (≥ 9 cases).
 */
describe('DivergentService — Spec 610 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through DivergentModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [DivergentModule],
      }).compile();
      const service = moduleRef.get(DivergentService);
      expect(service).toBeInstanceOf(DivergentService);
      await moduleRef.close();
    });

    it('exports the Site.DIVERGENT = "divergent" enum value', () => {
      expect(Site.DIVERGENT).toBe('divergent');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new DivergentService();
      const result = await service.scrape({
        siteType: [Site.DIVERGENT],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'divergent-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.DIVERGENT);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Divergent');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/divergent/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/divergent/jobs/');
      ...
      expect(job0?.description).toContain('Divergent');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/divergent/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Divergent');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'divergent-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
