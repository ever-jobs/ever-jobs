...
import { CityOfPhiladelphiaModule, CityOfPhiladelphiaService } from '../src';

const COMPANY_NAME_EXPECT = 'City of Philadelphia';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'cityofphiladelphia-jobs.json'), 'utf8'),
);
...
describe('CityOfPhiladelphiaService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CityOfPhiladelphiaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CityOfPhiladelphiaModule],
      }).compile();
      const service = moduleRef.get(CityOfPhiladelphiaService);
      expect(service).toBeInstanceOf(CityOfPhiladelphiaService);
      await moduleRef.close();
    });

    it('exports the Site.CITY_OF_PHILADELPHIA = "cityofphiladelphia" enum value', () => {
      expect(Site.CITY_OF_PHILADELPHIA).toBe('cityofphiladelphia');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new CityOfPhiladelphiaService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.CITY_OF_PHILADELPHIA],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'cityofphiladelphia-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.CITY_OF_PHILADELPHIA);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('cityofphiladelphia-' + first.id);
      ...
      expect(calledUrls[0]).toContain('CityofPhiladelphia');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.CITY_OF_PHILADELPHIA);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('cityofphiladelphia-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new CityOfPhiladelphiaService(registry);
      const result = await service.scrape({
        siteType: [Site.CITY_OF_PHILADELPHIA],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('CityofPhiladelphia');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('cityofphiladelphia-x1');
      expect(result.jobs[0].site).toBe(Site.CITY_OF_PHILADELPHIA);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('cityofphiladelphia-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new CityOfPhiladelphiaService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.CITY_OF_PHILADELPHIA],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new CityOfPhiladelphiaService();
      const result = await service.scrape({
        siteType: [Site.CITY_OF_PHILADELPHIA],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
