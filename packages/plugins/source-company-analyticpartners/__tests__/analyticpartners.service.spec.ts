...
import { AnalyticPartnersModule, AnalyticPartnersService } from '../src';

const COMPANY_NAME_EXPECT = 'Analytic Partners';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'analyticpartners-jobs.json'), 'utf8'),
);
...
describe('AnalyticPartnersService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AnalyticPartnersModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AnalyticPartnersModule],
      }).compile();
      const service = moduleRef.get(AnalyticPartnersService);
      expect(service).toBeInstanceOf(AnalyticPartnersService);
      await moduleRef.close();
    });

    it('exports the Site.ANALYTIC_PARTNERS = "analyticpartners" enum value', () => {
      expect(Site.ANALYTIC_PARTNERS).toBe('analyticpartners');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new AnalyticPartnersService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.ANALYTIC_PARTNERS],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'analyticpartners-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.ANALYTIC_PARTNERS);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('analyticpartners-' + first.id);
      ...
      expect(calledUrls[0]).toContain('analyticpartners');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ANALYTIC_PARTNERS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('analyticpartners-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new AnalyticPartnersService(registry);
      const result = await service.scrape({
        siteType: [Site.ANALYTIC_PARTNERS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('analyticpartners');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('analyticpartners-x1');
      expect(result.jobs[0].site).toBe(Site.ANALYTIC_PARTNERS);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('analyticpartners-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new AnalyticPartnersService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.ANALYTIC_PARTNERS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new AnalyticPartnersService();
      const result = await service.scrape({
        siteType: [Site.ANALYTIC_PARTNERS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
