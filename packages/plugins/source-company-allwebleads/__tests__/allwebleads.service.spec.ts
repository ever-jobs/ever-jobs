...
import { AllwebleadsModule, AllwebleadsService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'allwebleads-jobs.json'), 'utf8'),
);
...
/**
 * Spec 237 / T04 — `AllwebleadsService` unit tests (≥ 9 cases).
 */
describe('AllwebleadsService — Spec 237 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AllwebleadsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AllwebleadsModule],
      }).compile();
      const service = moduleRef.get(AllwebleadsService);
      expect(service).toBeInstanceOf(AllwebleadsService);
      await moduleRef.close();
    });

    it('exports the Site.ALLWEBLEADS = "allwebleads" enum value', () => {
      expect(Site.ALLWEBLEADS).toBe('allwebleads');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AllwebleadsService();
      const result = await service.scrape({
        siteType: [Site.ALLWEBLEADS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'allwebleads-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ALLWEBLEADS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('AWL');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/allwebleads/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/allwebleads/jobs/');
      ...
      expect(job0?.description).toContain('AWL');
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('AWL');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'allwebleads-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
