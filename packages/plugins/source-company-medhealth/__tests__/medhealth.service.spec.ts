...
import { MedHealthModule, MedHealthService } from '../src';

const COMPANY_NAME_EXPECT = 'MedHealth';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'medhealth-jobs.json'), 'utf8'),
);
...
describe('MedHealthService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through MedHealthModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [MedHealthModule],
      }).compile();
      const service = moduleRef.get(MedHealthService);
      expect(service).toBeInstanceOf(MedHealthService);
      await moduleRef.close();
    });

    it('exports the Site.MEDHEALTH = "medhealth" enum value', () => {
      expect(Site.MEDHEALTH).toBe('medhealth');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new MedHealthService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.MEDHEALTH],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('medhealth3');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.MEDHEALTH);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('medhealth-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new MedHealthService(registry);
      const result = await service.scrape({
        siteType: [Site.MEDHEALTH],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('medhealth3');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('medhealth-x1');
      expect(result.jobs[0].site).toBe(Site.MEDHEALTH);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('medhealth-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new MedHealthService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.MEDHEALTH],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new MedHealthService();
      const result = await service.scrape({
        siteType: [Site.MEDHEALTH],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
