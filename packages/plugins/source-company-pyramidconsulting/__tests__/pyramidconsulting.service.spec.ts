...
import { PyramidConsultingModule, PyramidConsultingService } from '../src';

const COMPANY_NAME_EXPECT = 'Pyramid Consulting';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'pyramidconsulting-jobs.json'), 'utf8'),
);
...
describe('PyramidConsultingService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through PyramidConsultingModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [PyramidConsultingModule],
      }).compile();
      const service = moduleRef.get(PyramidConsultingService);
      expect(service).toBeInstanceOf(PyramidConsultingService);
      await moduleRef.close();
    });

    it('exports the Site.PYRAMID_CONSULTING = "pyramidconsulting" enum value', () => {
      expect(Site.PYRAMID_CONSULTING).toBe('pyramidconsulting');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new PyramidConsultingService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.PYRAMID_CONSULTING],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'pyramidconsulting-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.PYRAMID_CONSULTING);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('pyramidconsulting-' + first.id);
      ...
      expect(calledUrls[0]).toContain('PyramidIT1');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.PYRAMID_CONSULTING);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('pyramidconsulting-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new PyramidConsultingService(registry);
      const result = await service.scrape({
        siteType: [Site.PYRAMID_CONSULTING],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('PyramidIT1');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('pyramidconsulting-x1');
      expect(result.jobs[0].site).toBe(Site.PYRAMID_CONSULTING);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('pyramidconsulting-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new PyramidConsultingService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.PYRAMID_CONSULTING],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new PyramidConsultingService();
      const result = await service.scrape({
        siteType: [Site.PYRAMID_CONSULTING],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
