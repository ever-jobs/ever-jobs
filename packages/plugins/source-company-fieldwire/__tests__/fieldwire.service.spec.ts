...
import { FieldwireModule, FieldwireService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'fieldwire-jobs.json'), 'utf8'),
);
...
/**
 * Spec 848 / T04 — `FieldwireService` unit tests (≥ 9 cases).
 */
describe('FieldwireService — Spec 848 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through FieldwireModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [FieldwireModule],
      }).compile();
      const service = moduleRef.get(FieldwireService);
      expect(service).toBeInstanceOf(FieldwireService);
      await moduleRef.close();
    });

    it('exports the Site.FIELDWIRE = "fieldwire" enum value', () => {
      expect(Site.FIELDWIRE).toBe('fieldwire');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new FieldwireService();
      const result = await service.scrape({
        siteType: [Site.FIELDWIRE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'fieldwire-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.FIELDWIRE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Fieldwire');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/fieldwire/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/fieldwire/jobs/');
      ...
      expect(job0?.description).toContain('Fieldwire');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/fieldwire/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Fieldwire');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'fieldwire-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
