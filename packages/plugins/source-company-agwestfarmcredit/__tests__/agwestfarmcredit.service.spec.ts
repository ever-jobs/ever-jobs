...
import { AgwestfarmcreditModule, AgwestfarmcreditService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'agwestfarmcredit-jobs.json'), 'utf8'),
);
...
/**
 * Spec 211 / T04 — `AgwestfarmcreditService` unit tests (≥ 9 cases).
 */
describe('AgwestfarmcreditService — Spec 211 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AgwestfarmcreditModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AgwestfarmcreditModule],
      }).compile();
      const service = moduleRef.get(AgwestfarmcreditService);
      expect(service).toBeInstanceOf(AgwestfarmcreditService);
      await moduleRef.close();
    });

    it('exports the Site.AGWESTFARMCREDIT = "agwestfarmcredit" enum value', () => {
      expect(Site.AGWESTFARMCREDIT).toBe('agwestfarmcredit');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AgwestfarmcreditService();
      const result = await service.scrape({
        siteType: [Site.AGWESTFARMCREDIT],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'agwestfarmcredit-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.AGWESTFARMCREDIT);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('AgWest Farm Credit');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/agwestfarmcredit/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/agwestfarmcredit/jobs/');
      ...
      expect(job0?.description).toContain('AgWest Farm Credit');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/agwestfarmcredit/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('AgWest Farm Credit');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'agwestfarmcredit-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
