...
import { CypressCreekRenewablesModule, CypressCreekRenewablesService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'cypresscreekrenewables-jobs.json'), 'utf8'),
);
...
/**
 * Spec 831 / T04 — `CypressCreekRenewablesService` unit tests (≥ 9 cases).
 */
describe('CypressCreekRenewablesService — Spec 831 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CypressCreekRenewablesModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CypressCreekRenewablesModule],
      }).compile();
      const service = moduleRef.get(CypressCreekRenewablesService);
      expect(service).toBeInstanceOf(CypressCreekRenewablesService);
      await moduleRef.close();
    });

    it('exports the Site.CYPRESS_CREEK_RENEWABLES = "cypresscreekrenewables" enum value', () => {
      expect(Site.CYPRESS_CREEK_RENEWABLES).toBe('cypresscreekrenewables');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new CypressCreekRenewablesService();
      const result = await service.scrape({
        siteType: [Site.CYPRESS_CREEK_RENEWABLES],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'cypresscreekrenewables-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.CYPRESS_CREEK_RENEWABLES);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Cypress Creek Renewables');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/cypresscreekrenewables/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/cypresscreekrenewables/jobs/');
      ...
      expect(job0?.description).toContain('Cypress Creek Renewables');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/cypresscreekrenewables/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Cypress Creek Renewables');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'cypresscreekrenewables-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
