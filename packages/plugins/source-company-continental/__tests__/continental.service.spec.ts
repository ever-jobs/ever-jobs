...
import { ContinentalModule, ContinentalService } from '../src';

const COMPANY_NAME_EXPECT = 'Continental';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'continental-jobs.json'), 'utf8'),
);
...
describe('ContinentalService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ContinentalModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ContinentalModule],
      }).compile();
      const service = moduleRef.get(ContinentalService);
      expect(service).toBeInstanceOf(ContinentalService);
      await moduleRef.close();
    });

    it('exports the Site.CONTINENTAL = "continental" enum value', () => {
      expect(Site.CONTINENTAL).toBe('continental');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new ContinentalService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.CONTINENTAL],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('Continental');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.CONTINENTAL);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('continental-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new ContinentalService(registry);
      const result = await service.scrape({
        siteType: [Site.CONTINENTAL],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('Continental');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('continental-x1');
      expect(result.jobs[0].site).toBe(Site.CONTINENTAL);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('continental-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new ContinentalService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.CONTINENTAL],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new ContinentalService();
      const result = await service.scrape({
        siteType: [Site.CONTINENTAL],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
