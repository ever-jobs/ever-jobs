...
import { ASTSpaceMobileModule, ASTSpaceMobileService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'astspacemobile-jobs.json'), 'utf8'),
);
...
/**
 * Spec 758 / T04 — `ASTSpaceMobileService` unit tests (≥ 9 cases).
 */
describe('ASTSpaceMobileService — Spec 758 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ASTSpaceMobileModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ASTSpaceMobileModule],
      }).compile();
      const service = moduleRef.get(ASTSpaceMobileService);
      expect(service).toBeInstanceOf(ASTSpaceMobileService);
      await moduleRef.close();
    });

    it('exports the Site.AST_SPACEMOBILE = "astspacemobile" enum value', () => {
      expect(Site.AST_SPACEMOBILE).toBe('astspacemobile');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ASTSpaceMobileService();
      const result = await service.scrape({
        siteType: [Site.AST_SPACEMOBILE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'astspacemobile-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.AST_SPACEMOBILE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('AST SpaceMobile');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/astspacemobile/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/astspacemobile/jobs/');
      ...
      expect(job0?.description).toContain('AST SpaceMobile');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/astspacemobile/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('AST SpaceMobile');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'astspacemobile-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
