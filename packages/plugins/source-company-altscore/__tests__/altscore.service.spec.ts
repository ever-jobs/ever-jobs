...
import { AltscoreModule, AltscoreService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'altscore-jobs.json'), 'utf8'),
);
...
/**
 * Spec 250 / T04 — `AltscoreService` unit tests (≥ 9 cases).
 */
describe('AltscoreService — Spec 250 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AltscoreModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AltscoreModule],
      }).compile();
      const service = moduleRef.get(AltscoreService);
      expect(service).toBeInstanceOf(AltscoreService);
      await moduleRef.close();
    });

    it('exports the Site.ALTSCORE = "altscore" enum value', () => {
      expect(Site.ALTSCORE).toBe('altscore');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AltscoreService();
      const result = await service.scrape({
        siteType: [Site.ALTSCORE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'altscore-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ALTSCORE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('AltScore');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/altscore/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/altscore/jobs/');
      ...
      expect(job0?.description).toContain('AltScore');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/altscore/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('AltScore');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'altscore-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
