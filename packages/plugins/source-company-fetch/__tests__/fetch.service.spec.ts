...
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'fetch-jobs.json'), 'utf8'),
);
...
/**
 * Spec 489 / T04 — `FetchService` unit tests (≥ 9 cases).
 */
describe('FetchService — Spec 489 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through FetchModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [FetchModule],
      }).compile();
      const service = moduleRef.get(FetchService);
      expect(service).toBeInstanceOf(FetchService);
      await moduleRef.close();
    });

    it('exports the Site.FETCH = "fetch" enum value', () => {
      expect(Site.FETCH).toBe('fetch');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const job0 = dto.jobs.find((j) => j.id === 'fetch-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.FETCH);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Fetch');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/fetch/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/fetch/jobs/');
      ...
      expect(job0?.description).toContain('Fetch');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/fetch/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Fetch');
      }
    });
  });
