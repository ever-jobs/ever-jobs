...
import { AnimalmedicalcenterModule, AnimalmedicalcenterService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'animalmedicalcenter-jobs.json'), 'utf8'),
);
...
/**
 * Spec 269 / T04 — `AnimalmedicalcenterService` unit tests (≥ 9 cases).
 */
describe('AnimalmedicalcenterService — Spec 269 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AnimalmedicalcenterModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AnimalmedicalcenterModule],
      }).compile();
      const service = moduleRef.get(AnimalmedicalcenterService);
      expect(service).toBeInstanceOf(AnimalmedicalcenterService);
      await moduleRef.close();
    });

    it('exports the Site.ANIMALMEDICALCENTER = "animalmedicalcenter" enum value', () => {
      expect(Site.ANIMALMEDICALCENTER).toBe('animalmedicalcenter');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AnimalmedicalcenterService();
      const result = await service.scrape({
        siteType: [Site.ANIMALMEDICALCENTER],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'animalmedicalcenter-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ANIMALMEDICALCENTER);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Schwarzman Animal Medical Center');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/animalmedicalcenter/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/animalmedicalcenter/jobs/');
      ...
      expect(job0?.description).toContain('Schwarzman Animal Medical Center');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/animalmedicalcenter/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Schwarzman Animal Medical Center');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'animalmedicalcenter-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
