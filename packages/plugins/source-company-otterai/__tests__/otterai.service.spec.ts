...
import { OtteraiModule, OtteraiService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'otterai-jobs.json'), 'utf8'),
);
...
/**
 * Spec 495 / T04 — `OtteraiService` unit tests (≥ 9 cases).
 */
describe('OtteraiService — Spec 495 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through OtteraiModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [OtteraiModule],
      }).compile();
      const service = moduleRef.get(OtteraiService);
      expect(service).toBeInstanceOf(OtteraiService);
      await moduleRef.close();
    });

    it('exports the Site.OTTERAI = "otterai" enum value', () => {
      expect(Site.OTTERAI).toBe('otterai');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new OtteraiService();
      const result = await service.scrape({
        siteType: [Site.OTTERAI],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'otterai-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.OTTERAI);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Otter.ai');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/otterai/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/otterai/jobs/');
      ...
      expect(job0?.description).toContain('Otter.ai');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/otterai/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Otter.ai');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'otterai-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
