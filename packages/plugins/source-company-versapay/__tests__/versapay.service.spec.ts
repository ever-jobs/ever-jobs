...
import { VersapayModule, VersapayService } from '../src';

const COMPANY_NAME_EXPECT = 'Versapay';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'versapay-jobs.json'), 'utf8'),
);
...
describe('VersapayService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through VersapayModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [VersapayModule],
      }).compile();
      const service = moduleRef.get(VersapayService);
      expect(service).toBeInstanceOf(VersapayService);
      await moduleRef.close();
    });

    it('exports the Site.VERSAPAY = "versapay" enum value', () => {
      expect(Site.VERSAPAY).toBe('versapay');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new VersapayService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.VERSAPAY],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('versapay');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.VERSAPAY);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('versapay-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new VersapayService(registry);
      const result = await service.scrape({
        siteType: [Site.VERSAPAY],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('versapay');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('versapay-x1');
      expect(result.jobs[0].site).toBe(Site.VERSAPAY);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('versapay-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new VersapayService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.VERSAPAY],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new VersapayService();
      const result = await service.scrape({
        siteType: [Site.VERSAPAY],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
