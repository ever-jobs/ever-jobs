...
import { ZwiftModule, ZwiftService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'zwift-jobs.json'), 'utf8'),
);
...
/**
 * Spec 590 / T04 — `ZwiftService` unit tests (≥ 9 cases).
 */
describe('ZwiftService — Spec 590 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ZwiftModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ZwiftModule],
      }).compile();
      const service = moduleRef.get(ZwiftService);
      expect(service).toBeInstanceOf(ZwiftService);
      await moduleRef.close();
    });

    it('exports the Site.ZWIFT = "zwift" enum value', () => {
      expect(Site.ZWIFT).toBe('zwift');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ZwiftService();
      ...
      const job0 = dto.jobs.find((j) => j.id === 'zwift-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ZWIFT);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Zwift');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/zwift/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/zwift/jobs/');
      ...
      expect(job0?.description).toContain('Zwift');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/zwift/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Zwift');
      }
    });
  });
