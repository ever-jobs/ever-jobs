...
import { QuinceModule, QuinceService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'quince-jobs.json'), 'utf8'),
);
...
/**
 * Spec 596 / T04 — `QuinceService` unit tests (≥ 9 cases).
 */
describe('QuinceService — Spec 596 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through QuinceModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [QuinceModule],
      }).compile();
      const service = moduleRef.get(QuinceService);
      expect(service).toBeInstanceOf(QuinceService);
      await moduleRef.close();
    });

    it('exports the Site.QUINCE = "quince" enum value', () => {
      expect(Site.QUINCE).toBe('quince');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new QuinceService();
      const result = await service.scrape({
        siteType: [Site.QUINCE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'quince-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.QUINCE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Quince');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/quince/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/quince/jobs/');
      ...
      expect(job0?.description).toContain('Quince');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/quince/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Quince');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'quince-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
