...
import { IntrinsicModule, IntrinsicService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'intrinsicrobotics-jobs.json'), 'utf8'),
);
...
/**
 * Spec 871 / T04 — `IntrinsicService` unit tests (≥ 9 cases).
 */
describe('IntrinsicService — Spec 871 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through IntrinsicModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [IntrinsicModule],
      }).compile();
      const service = moduleRef.get(IntrinsicService);
      expect(service).toBeInstanceOf(IntrinsicService);
      await moduleRef.close();
    });

    it('exports the Site.INTRINSIC = "intrinsicrobotics" enum value', () => {
      expect(Site.INTRINSIC).toBe('intrinsicrobotics');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new IntrinsicService();
      const result = await service.scrape({
        siteType: [Site.INTRINSIC],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'intrinsicrobotics-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.INTRINSIC);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Intrinsic');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/intrinsicrobotics/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/intrinsicrobotics/jobs/');
      ...
      expect(job0?.description).toContain('Intrinsic');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/intrinsicrobotics/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Intrinsic');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'intrinsicrobotics-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
