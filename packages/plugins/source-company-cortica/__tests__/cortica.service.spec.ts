...
import { CorticaModule, CorticaService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'cortica-jobs.json'), 'utf8'),
);
...
/**
 * Spec 775 / T04 — `CorticaService` unit tests (≥ 9 cases).
 */
describe('CorticaService — Spec 775 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CorticaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CorticaModule],
      }).compile();
      const service = moduleRef.get(CorticaService);
      expect(service).toBeInstanceOf(CorticaService);
      await moduleRef.close();
    });

    it('exports the Site.CORTICA = "cortica" enum value', () => {
      expect(Site.CORTICA).toBe('cortica');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new CorticaService();
      const result = await service.scrape({
        siteType: [Site.CORTICA],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'cortica-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.CORTICA);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Cortica');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/cortica/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/cortica/jobs/');
      ...
      expect(job0?.description).toContain('Cortica');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/cortica/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Cortica');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'cortica-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
