...
import { LVMHModule, LVMHService } from '../src';

const COMPANY_NAME_EXPECT = 'LVMH';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'lvmh-jobs.json'), 'utf8'),
);
...
describe('LVMHService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through LVMHModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [LVMHModule],
      }).compile();
      const service = moduleRef.get(LVMHService);
      expect(service).toBeInstanceOf(LVMHService);
      await moduleRef.close();
    });

    it('exports the Site.LVMH = "lvmh" enum value', () => {
      expect(Site.LVMH).toBe('lvmh');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new LVMHService(registryWithSmartRecruiters());
      ...
      expect(calledUrls[0]).toContain('LVMH2');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.LVMH);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('lvmh-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new LVMHService(registry);
      const result = await service.scrape({
        siteType: [Site.LVMH],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('LVMH2');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('lvmh-x1');
      expect(result.jobs[0].site).toBe(Site.LVMH);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('lvmh-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new LVMHService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.LVMH],
      } as ScraperInputDto);
