...
import { ClickUpModule, ClickUpService } from '../src';

const COMPANY_NAME_EXPECT = 'ClickUp';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'clickup-jobs.json'), 'utf8'),
);
...
describe('ClickUpService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ClickUpModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ClickUpModule],
      }).compile();
      const service = moduleRef.get(ClickUpService);
      expect(service).toBeInstanceOf(ClickUpService);
      await moduleRef.close();
    });

    it('exports the Site.CLICKUP = "clickup" enum value', () => {
      expect(Site.CLICKUP).toBe('clickup');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new ClickUpService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.CLICKUP],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('clickup');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.CLICKUP);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('clickup-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new ClickUpService(registry);
      const result = await service.scrape({
        siteType: [Site.CLICKUP],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('clickup');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('clickup-x1');
      expect(result.jobs[0].site).toBe(Site.CLICKUP);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('clickup-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new ClickUpService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.CLICKUP],
      } as ScraperInputDto);
