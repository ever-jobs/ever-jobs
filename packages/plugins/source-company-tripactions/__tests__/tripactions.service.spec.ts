...
import { TripactionsModule, TripactionsService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'tripactions-jobs.json'), 'utf8'),
);
...
/**
 * Spec 459 / T04 — `TripactionsService` unit tests (≥ 9 cases).
 */
describe('TripactionsService — Spec 459 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through TripactionsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [TripactionsModule],
      }).compile();
      const service = moduleRef.get(TripactionsService);
      expect(service).toBeInstanceOf(TripactionsService);
      await moduleRef.close();
    });

    it('exports the Site.TRIPACTIONS = "tripactions" enum value', () => {
      expect(Site.TRIPACTIONS).toBe('tripactions');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new TripactionsService();
      const result = await service.scrape({
        siteType: [Site.TRIPACTIONS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'tripactions-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.TRIPACTIONS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Navan');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/tripactions/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/tripactions/jobs/');
      ...
      expect(job0?.description).toContain('Navan');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/tripactions/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Navan');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'tripactions-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
