...
import { EthosModule, EthosService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'ethos-jobs.json'), 'utf8'),
);
...
/**
 * Spec 623 / T04 — `EthosService` unit tests (≥ 9 cases).
 */
describe('EthosService — Spec 623 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through EthosModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [EthosModule],
      }).compile();
      const service = moduleRef.get(EthosService);
      expect(service).toBeInstanceOf(EthosService);
      await moduleRef.close();
    });

    it('exports the Site.ETHOS = "ethos" enum value', () => {
      expect(Site.ETHOS).toBe('ethos');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new EthosService();
      ...
      const job0 = dto.jobs.find((j) => j.id === 'ethos-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ETHOS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Ethos');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/ethos/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/ethos/jobs/');
      ...
      expect(job0?.description).toContain('Ethos');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/ethos/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Ethos');
      }
    });
  });
