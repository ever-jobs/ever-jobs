...
import { AmarokModule, AmarokService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'amarok-jobs.json'), 'utf8'),
);
...
/**
 * Spec 256 / T04 — `AmarokService` unit tests (≥ 9 cases).
 */
describe('AmarokService — Spec 256 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AmarokModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AmarokModule],
      }).compile();
      const service = moduleRef.get(AmarokService);
      expect(service).toBeInstanceOf(AmarokService);
      await moduleRef.close();
    });

    it('exports the Site.AMAROK = "amarok" enum value', () => {
      expect(Site.AMAROK).toBe('amarok');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AmarokService();
      const result = await service.scrape({
        siteType: [Site.AMAROK],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'amarok-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.AMAROK);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('AMAROK');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/amarok/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/amarok/jobs/');
      ...
      expect(job0?.description).toContain('AMAROK');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/amarok/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('AMAROK');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'amarok-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
