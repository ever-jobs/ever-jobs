...
import { AlpadiaLanguageSchoolsModule, AlpadiaLanguageSchoolsService } from '../src';

const COMPANY_NAME_EXPECT = 'Alpadia Language Schools';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'alpadialanguageschools-jobs.json'), 'utf8'),
);
...
describe('AlpadiaLanguageSchoolsService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AlpadiaLanguageSchoolsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AlpadiaLanguageSchoolsModule],
      }).compile();
      const service = moduleRef.get(AlpadiaLanguageSchoolsService);
      expect(service).toBeInstanceOf(AlpadiaLanguageSchoolsService);
      await moduleRef.close();
    });

    it('exports the Site.ALPADIA_LANGUAGE_SCHOOLS = "alpadialanguageschools" enum value', () => {
      expect(Site.ALPADIA_LANGUAGE_SCHOOLS).toBe('alpadialanguageschools');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new AlpadiaLanguageSchoolsService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.ALPADIA_LANGUAGE_SCHOOLS],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'alpadialanguageschools-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.ALPADIA_LANGUAGE_SCHOOLS);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('alpadialanguageschools-' + first.id);
      ...
      expect(calledUrls[0]).toContain('AlpadiaLanguageSchoolsSA');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ALPADIA_LANGUAGE_SCHOOLS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('alpadialanguageschools-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new AlpadiaLanguageSchoolsService(registry);
      const result = await service.scrape({
        siteType: [Site.ALPADIA_LANGUAGE_SCHOOLS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('AlpadiaLanguageSchoolsSA');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('alpadialanguageschools-x1');
      expect(result.jobs[0].site).toBe(Site.ALPADIA_LANGUAGE_SCHOOLS);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('alpadialanguageschools-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new AlpadiaLanguageSchoolsService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.ALPADIA_LANGUAGE_SCHOOLS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new AlpadiaLanguageSchoolsService();
      const result = await service.scrape({
        siteType: [Site.ALPADIA_LANGUAGE_SCHOOLS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
