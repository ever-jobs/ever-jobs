...
import { AltentechnologyusaModule, AltentechnologyusaService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'altentechnologyusa-jobs.json'), 'utf8'),
);
...
/**
 * Spec 247 / T04 — `AltentechnologyusaService` unit tests (≥ 9 cases).
 */
describe('AltentechnologyusaService — Spec 247 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AltentechnologyusaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AltentechnologyusaModule],
      }).compile();
      const service = moduleRef.get(AltentechnologyusaService);
      expect(service).toBeInstanceOf(AltentechnologyusaService);
      await moduleRef.close();
    });

    it('exports the Site.ALTENTECHNOLOGYUSA = "altentechnologyusa" enum value', () => {
      expect(Site.ALTENTECHNOLOGYUSA).toBe('altentechnologyusa');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AltentechnologyusaService();
      const result = await service.scrape({
        siteType: [Site.ALTENTECHNOLOGYUSA],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'altentechnologyusa-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ALTENTECHNOLOGYUSA);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('ALTEN Technology USA');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/altentechnologyusa/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/altentechnologyusa/jobs/');
      ...
      expect(job0?.description).toContain('ALTEN Technology USA');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/altentechnologyusa/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('ALTEN Technology USA');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'altentechnologyusa-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
