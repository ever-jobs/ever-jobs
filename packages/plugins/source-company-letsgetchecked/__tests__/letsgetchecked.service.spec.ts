...
import { LetsGetCheckedModule, LetsGetCheckedService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'letsgetchecked-jobs.json'), 'utf8'),
);
...
/**
 * Spec 618 / T04 — `LetsGetCheckedService` unit tests (≥ 9 cases).
 */
describe('LetsGetCheckedService — Spec 618 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through LetsGetCheckedModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [LetsGetCheckedModule],
      }).compile();
      const service = moduleRef.get(LetsGetCheckedService);
      expect(service).toBeInstanceOf(LetsGetCheckedService);
      await moduleRef.close();
    });

    it('exports the Site.LETSGETCHECKED = "letsgetchecked" enum value', () => {
      expect(Site.LETSGETCHECKED).toBe('letsgetchecked');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new LetsGetCheckedService();
      const result = await service.scrape({
        siteType: [Site.LETSGETCHECKED],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'letsgetchecked-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.LETSGETCHECKED);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('LetsGetChecked');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/letsgetchecked/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/letsgetchecked/jobs/');
      ...
      expect(job0?.description).toContain('LetsGetChecked');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/letsgetchecked/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('LetsGetChecked');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'letsgetchecked-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
