...
import { AmericancapitalgroupModule, AmericancapitalgroupService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'americancapitalgroup-jobs.json'), 'utf8'),
);
...
/**
 * Spec 260 / T04 — `AmericancapitalgroupService` unit tests (≥ 9 cases).
 */
describe('AmericancapitalgroupService — Spec 260 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AmericancapitalgroupModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AmericancapitalgroupModule],
      }).compile();
      const service = moduleRef.get(AmericancapitalgroupService);
      expect(service).toBeInstanceOf(AmericancapitalgroupService);
      await moduleRef.close();
    });

    it('exports the Site.AMERICANCAPITALGROUP = "americancapitalgroup" enum value', () => {
      expect(Site.AMERICANCAPITALGROUP).toBe('americancapitalgroup');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AmericancapitalgroupService();
      const result = await service.scrape({
        siteType: [Site.AMERICANCAPITALGROUP],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'americancapitalgroup-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.AMERICANCAPITALGROUP);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('American Capital Group');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/americancapitalgroup/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/americancapitalgroup/jobs/');
      ...
      expect(job0?.description).toContain('American Capital Group');
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('American Capital Group');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'americancapitalgroup-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
