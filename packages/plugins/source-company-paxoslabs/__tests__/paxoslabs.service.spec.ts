...
import { PaxosLabsModule, PaxosLabsService } from '../src';

const COMPANY_NAME_EXPECT = 'Paxos Labs';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'paxoslabs-jobs.json'), 'utf8'),
);
...
describe('PaxosLabsService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through PaxosLabsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [PaxosLabsModule],
      }).compile();
      const service = moduleRef.get(PaxosLabsService);
      expect(service).toBeInstanceOf(PaxosLabsService);
      await moduleRef.close();
    });

    it('exports the Site.PAXOS_LABS = "paxoslabs" enum value', () => {
      expect(Site.PAXOS_LABS).toBe('paxoslabs');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new PaxosLabsService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.PAXOS_LABS],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('paxoslabs');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.PAXOS_LABS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('paxoslabs-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new PaxosLabsService(registry);
      const result = await service.scrape({
        siteType: [Site.PAXOS_LABS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('paxoslabs');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('paxoslabs-x1');
      expect(result.jobs[0].site).toBe(Site.PAXOS_LABS);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('paxoslabs-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new PaxosLabsService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.PAXOS_LABS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new PaxosLabsService();
      const result = await service.scrape({
        siteType: [Site.PAXOS_LABS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
