...
import { KikoffModule, KikoffService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'kikoff-jobs.json'), 'utf8'),
);
...
/**
 * Spec 602 / T04 — `KikoffService` unit tests (≥ 9 cases).
 */
describe('KikoffService — Spec 602 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through KikoffModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [KikoffModule],
      }).compile();
      const service = moduleRef.get(KikoffService);
      expect(service).toBeInstanceOf(KikoffService);
      await moduleRef.close();
    });

    it('exports the Site.KIKOFF = "kikoff" enum value', () => {
      expect(Site.KIKOFF).toBe('kikoff');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new KikoffService();
      const result = await service.scrape({
        siteType: [Site.KIKOFF],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'kikoff-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.KIKOFF);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Kikoff');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/kikoff/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/kikoff/jobs/');
      ...
      expect(job0?.description).toContain('Kikoff');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/kikoff/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Kikoff');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'kikoff-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
