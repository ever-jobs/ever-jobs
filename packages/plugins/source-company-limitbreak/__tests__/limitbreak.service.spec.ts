...
import { LimitBreakModule, LimitBreakService } from '../src';

const COMPANY_NAME_EXPECT = 'Limit Break';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'limitbreak-jobs.json'), 'utf8'),
);
...
describe('LimitBreakService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through LimitBreakModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [LimitBreakModule],
      }).compile();
      const service = moduleRef.get(LimitBreakService);
      expect(service).toBeInstanceOf(LimitBreakService);
      await moduleRef.close();
    });

    it('exports the Site.LIMIT_BREAK = "limitbreak" enum value', () => {
      expect(Site.LIMIT_BREAK).toBe('limitbreak');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new LimitBreakService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.LIMIT_BREAK],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('limitbreak');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.LIMIT_BREAK);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('limitbreak-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new LimitBreakService(registry);
      const result = await service.scrape({
        siteType: [Site.LIMIT_BREAK],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('limitbreak');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('limitbreak-x1');
      expect(result.jobs[0].site).toBe(Site.LIMIT_BREAK);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('limitbreak-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new LimitBreakService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.LIMIT_BREAK],
      } as ScraperInputDto);
