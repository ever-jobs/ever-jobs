...
import { AIMIntelligentMachinesModule, AIMIntelligentMachinesService } from '../src';

const COMPANY_NAME_EXPECT = 'AIM Intelligent Machines';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'aimintelligentmachines-jobs.json'), 'utf8'),
);
...
describe('AIMIntelligentMachinesService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AIMIntelligentMachinesModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AIMIntelligentMachinesModule],
      }).compile();
      const service = moduleRef.get(AIMIntelligentMachinesService);
      expect(service).toBeInstanceOf(AIMIntelligentMachinesService);
      await moduleRef.close();
    });

    it('exports the Site.AIM_INTELLIGENT_MACHINES = "aimintelligentmachines" enum value', () => {
      expect(Site.AIM_INTELLIGENT_MACHINES).toBe('aimintelligentmachines');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new AIMIntelligentMachinesService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.AIM_INTELLIGENT_MACHINES],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'aimintelligentmachines-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.AIM_INTELLIGENT_MACHINES);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('aimintelligentmachines-' + first.id);
      ...
      expect(calledUrls[0]).toContain('aim');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.AIM_INTELLIGENT_MACHINES);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('aimintelligentmachines-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new AIMIntelligentMachinesService(registry);
      const result = await service.scrape({
        siteType: [Site.AIM_INTELLIGENT_MACHINES],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('aim');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('aimintelligentmachines-x1');
      expect(result.jobs[0].site).toBe(Site.AIM_INTELLIGENT_MACHINES);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('aimintelligentmachines-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new AIMIntelligentMachinesService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.AIM_INTELLIGENT_MACHINES],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new AIMIntelligentMachinesService();
      const result = await service.scrape({
        siteType: [Site.AIM_INTELLIGENT_MACHINES],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
