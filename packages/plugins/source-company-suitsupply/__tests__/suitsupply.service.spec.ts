...
import { SuitsupplyModule, SuitsupplyService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'suitsupply-jobs.json'), 'utf8'),
);
...
/**
 * Spec 605 / T04 — `SuitsupplyService` unit tests (≥ 9 cases).
 */
describe('SuitsupplyService — Spec 605 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SuitsupplyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SuitsupplyModule],
      }).compile();
      const service = moduleRef.get(SuitsupplyService);
      expect(service).toBeInstanceOf(SuitsupplyService);
      await moduleRef.close();
    });

    it('exports the Site.SUITSUPPLY = "suitsupply" enum value', () => {
      expect(Site.SUITSUPPLY).toBe('suitsupply');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new SuitsupplyService();
      const result = await service.scrape({
        siteType: [Site.SUITSUPPLY],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'suitsupply-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.SUITSUPPLY);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Suitsupply');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/suitsupply/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/suitsupply/jobs/');
      ...
      expect(job0?.description).toContain('Suitsupply');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/suitsupply/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Suitsupply');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'suitsupply-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
