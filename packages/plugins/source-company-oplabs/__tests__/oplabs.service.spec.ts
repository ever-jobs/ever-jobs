...
import { OPLabsModule, OPLabsService } from '../src';

const COMPANY_NAME_EXPECT = 'OP Labs';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'oplabs-jobs.json'), 'utf8'),
);
...
describe('OPLabsService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through OPLabsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [OPLabsModule],
      }).compile();
      const service = moduleRef.get(OPLabsService);
      expect(service).toBeInstanceOf(OPLabsService);
      await moduleRef.close();
    });

    it('exports the Site.OP_LABS = "oplabs" enum value', () => {
      expect(Site.OP_LABS).toBe('oplabs');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new OPLabsService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.OP_LABS],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('oplabs');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.OP_LABS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('oplabs-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new OPLabsService(registry);
      const result = await service.scrape({
        siteType: [Site.OP_LABS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('oplabs');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('oplabs-x1');
      expect(result.jobs[0].site).toBe(Site.OP_LABS);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('oplabs-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new OPLabsService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.OP_LABS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new OPLabsService();
      const result = await service.scrape({
        siteType: [Site.OP_LABS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
