...
import { MatiaModule, MatiaService } from '../src';

const COMPANY_NAME_EXPECT = 'Matia';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'matia-jobs.json'), 'utf8'),
);
...
describe('MatiaService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through MatiaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [MatiaModule],
      }).compile();
      const service = moduleRef.get(MatiaService);
      expect(service).toBeInstanceOf(MatiaService);
      await moduleRef.close();
    });

    it('exports the Site.MATIA = "matia" enum value', () => {
      expect(Site.MATIA).toBe('matia');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new MatiaService(registryWithAshby());
      ...
      expect(calledUrls[0]).toContain('matia');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.MATIA);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('matia-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new MatiaService(registry);
      const result = await service.scrape({
        siteType: [Site.MATIA],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('matia');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('matia-x1');
      expect(result.jobs[0].site).toBe(Site.MATIA);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('matia-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new MatiaService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.MATIA],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new MatiaService();
      const result = await service.scrape({
        siteType: [Site.MATIA],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
