...
import { ZillowModule, ZillowService } from '../src';
...
const FIXTURE: Fixture = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'fixtures', 'zillow-boards.json'), 'utf8'),
);
const COMPANY_NAME = 'Zillow';
const ID_PREFIX = 'zillow-';
...
describe('ZillowService — Workday delegation (Spec 1736)', () => {
  beforeEach(() => {
    mockGet.mockReset();
    mockPost.mockReset();
  ...
  describe('registration', () => {
    it('resolves through ZillowModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({ imports: [ZillowModule] }).compile();
      expect(moduleRef.get(ZillowService)).toBeInstanceOf(ZillowService);
      await moduleRef.close();
    });

    it('exports Site.ZILLOW = "zillow"', () => {
      expect(Site.ZILLOW).toBe('zillow');
    });

    it('declares a tagged company plugin with its domains', () => {
      const meta = Reflect.getMetadata(SOURCE_PLUGIN_METADATA, ZillowService);
      expect(meta.site).toBe(Site.ZILLOW);
      expect(meta.category).toBe('company');
      expect(meta.isAts).toBeFalsy();
      expect(meta.companyDomains).toEqual(['zillow.com']);
  ...
  describe('recorded board (real Workday adapter, mocked HTTP)', () => {
    it('maps every recorded posting and re-stamps the company identity', async () => {
      const service = new ZillowService(registryWith());
      const result = await service.scrape({ siteType: [Site.ZILLOW], resultsWanted: 100 } as ScraperInputDto);
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ZILLOW);
    ...
    it('only requests the recorded board URLs', async () => {
      const service = new ZillowService(registryWith());
      await service.scrape({ siteType: [Site.ZILLOW] } as ScraperInputDto);
    ...
    it('honours resultsWanted=1', async () => {
      const service = new ZillowService(registryWith());
      const result = await service.scrape({ siteType: [Site.ZILLOW], resultsWanted: 1 } as ScraperInputDto);
    ...
    it('resolves with an empty result when the board is gone (HTTP 404)', async () => {
      ...
      const service = new ZillowService(registryWith());
      const result = await service.scrape({ siteType: [Site.ZILLOW] } as ScraperInputDto);
  ...
  describe('delegation contract', () => {
    it('forwards the board and the caller input untouched', async () => {
      ...
      const service = new ZillowService(
        registryWith(
          fakeBackend(
      ...
      expect(result.jobs[0].site).toBe(Site.ZILLOW);
      expect(result.jobs[0].companyName).toBe(COMPANY_NAME);
    });

    it('passes an absent resultsWanted through as absent', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new ZillowService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      await service.scrape({ siteType: [Site.ZILLOW] } as ScraperInputDto);
    ...
    it('makes no request when resultsWanted is 0', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new ZillowService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      const result = await service.scrape({ siteType: [Site.ZILLOW], resultsWanted: 0 } as ScraperInputDto);
  ...
  describe('resilience', () => {
    it('reports not_registered when no registry is injected', async () => {
      const result = await new ZillowService().scrape({ siteType: [Site.ZILLOW] } as ScraperInputDto);
      expect(result.jobs).toEqual([]);
    ...
    it('reports not_registered when Workday is missing from the registry', async () => {
      const result = await new ZillowService(new PluginRegistry()).scrape({
        siteType: [Site.ZILLOW],
      } as ScraperInputDto);
      expect(result.diagnostics?.reason).toBe('not_registered');
    });
