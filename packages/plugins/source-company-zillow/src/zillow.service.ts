...
/**
 * Zillow — Real-estate technology (HQ: Seattle, WA, USA).
 *
 * Source (Spec 1736): Workday board, scraped in this order:
 *   - `zillow:5:Zillow_Group_External` — https://zillow.wd5.myworkdayjobs.com/Zillow_Group_External
 *     verified live 2026-09-24: 120 open postings.
 *
 ...
 * party board.
 *
 * Tags: segment=workday-enterprise; industry=real-estate-technology.
 */
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'zillow:5:Zillow_Group_External', atsIdPrefix: 'wd-zillow-' },
];
...
@SourcePlugin({
  ...
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['zillow.com'],
  description: 'Zillow careers via Workday. Tags: segment=workday-enterprise; industry=real-estate-technology.',
})
@Injectable()
export class ZillowService implements IScraper {
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Workday source plugin is not registered; cannot scrape Zillow');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`Zillow: delegating to Workday (${board.companySlug})`);
    ...
    this.logger.log(`Zillow: scraped ${jobs.length} jobs`);
