...
import { MorganStanleyModule, MorganStanleyService } from '../src';
...
const FIXTURE: Fixture = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'fixtures', 'morganstanley-boards.json'), 'utf8'),
);
const COMPANY_NAME = 'Morgan Stanley';
const ID_PREFIX = 'morganstanley-';
...
describe('MorganStanleyService — Workday delegation (Spec 1736)', () => {
  beforeEach(() => {
    mockGet.mockReset();
    mockPost.mockReset();
  ...
  describe('registration', () => {
    it('resolves through MorganStanleyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({ imports: [MorganStanleyModule] }).compile();
      expect(moduleRef.get(MorganStanleyService)).toBeInstanceOf(MorganStanleyService);
      await moduleRef.close();
    });

    it('exports Site.MORGAN_STANLEY = "morganstanley"', () => {
      expect(Site.MORGAN_STANLEY).toBe('morganstanley');
    });

    it('declares a tagged company plugin with its domains', () => {
      const meta = Reflect.getMetadata(SOURCE_PLUGIN_METADATA, MorganStanleyService);
      expect(meta.site).toBe(Site.MORGAN_STANLEY);
      expect(meta.category).toBe('company');
      expect(meta.isAts).toBeFalsy();
      expect(meta.companyDomains).toEqual(['morganstanley.com']);
  ...
  describe('recorded board (real Workday adapter, mocked HTTP)', () => {
    it('maps every recorded posting and re-stamps the company identity', async () => {
      const service = new MorganStanleyService(registryWith());
      const result = await service.scrape({ siteType: [Site.MORGAN_STANLEY], resultsWanted: 100 } as ScraperInputDto);
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.MORGAN_STANLEY);
    ...
    it('only requests the recorded board URLs', async () => {
      const service = new MorganStanleyService(registryWith());
      await service.scrape({ siteType: [Site.MORGAN_STANLEY] } as ScraperInputDto);
    ...
    it('honours resultsWanted=1', async () => {
      const service = new MorganStanleyService(registryWith());
      const result = await service.scrape({ siteType: [Site.MORGAN_STANLEY], resultsWanted: 1 } as ScraperInputDto);
    ...
    it('resolves with an empty result when the board is gone (HTTP 404)', async () => {
      ...
      const service = new MorganStanleyService(registryWith());
      const result = await service.scrape({ siteType: [Site.MORGAN_STANLEY] } as ScraperInputDto);
  ...
  describe('delegation contract', () => {
    it('forwards the board and the caller input untouched', async () => {
      ...
      const service = new MorganStanleyService(
        registryWith(
          fakeBackend(
      ...
      expect(result.jobs[0].site).toBe(Site.MORGAN_STANLEY);
      expect(result.jobs[0].companyName).toBe(COMPANY_NAME);
    });

    it('passes an absent resultsWanted through as absent', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new MorganStanleyService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      await service.scrape({ siteType: [Site.MORGAN_STANLEY] } as ScraperInputDto);
    ...
    it('makes no request when resultsWanted is 0', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new MorganStanleyService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      const result = await service.scrape({ siteType: [Site.MORGAN_STANLEY], resultsWanted: 0 } as ScraperInputDto);
  ...
  describe('resilience', () => {
    it('reports not_registered when no registry is injected', async () => {
      const result = await new MorganStanleyService().scrape({ siteType: [Site.MORGAN_STANLEY] } as ScraperInputDto);
      expect(result.jobs).toEqual([]);
    ...
    it('reports not_registered when Workday is missing from the registry', async () => {
      const result = await new MorganStanleyService(new PluginRegistry()).scrape({
        siteType: [Site.MORGAN_STANLEY],
      } as ScraperInputDto);
      expect(result.diagnostics?.reason).toBe('not_registered');
    });
