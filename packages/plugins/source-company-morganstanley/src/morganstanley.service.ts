...
/**
 * Morgan Stanley — Investment banking (HQ: New York, NY, USA).
 *
 * Source (Spec 1736): Workday board, scraped in this order:
 *   - `ms:5:External` — https://ms.wd5.myworkdayjobs.com/External
 *     verified live 2026-09-24: 1,325 open postings.
 *
 ...
 * party board.
 *
 * Tags: segment=workday-enterprise; industry=investment-banking.
 */
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'ms:5:External', atsIdPrefix: 'wd-ms-' },
];
...
@SourcePlugin({
  site: Site.MORGAN_STANLEY,
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['morganstanley.com'],
  description: 'Morgan Stanley careers via Workday. Tags: segment=workday-enterprise; industry=investment-banking.',
})
@Injectable()
export class MorganStanleyService implements IScraper {
  private readonly logger = new Logger(MorganStanleyService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Workday source plugin is not registered; cannot scrape Morgan Stanley');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`Morgan Stanley: delegating to Workday (${board.companySlug})`);
    ...
    this.logger.log(`Morgan Stanley: scraped ${jobs.length} jobs`);
