...
import { AppierModule, AppierService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'appier-jobs.json'), 'utf8'),
);
...
/**
 * Spec 293 / T04 — `AppierService` unit tests (≥ 9 cases).
 */
describe('AppierService — Spec 293 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AppierModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AppierModule],
      }).compile();
      const service = moduleRef.get(AppierService);
      expect(service).toBeInstanceOf(AppierService);
      await moduleRef.close();
    });

    it('exports the Site.APPIER = "appier" enum value', () => {
      expect(Site.APPIER).toBe('appier');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AppierService();
      const result = await service.scrape({
        siteType: [Site.APPIER],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'appier-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.APPIER);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Appier');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/appier/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/appier/jobs/');
      ...
      expect(job0?.description).toContain('Appier');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/appier/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Appier');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'appier-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
