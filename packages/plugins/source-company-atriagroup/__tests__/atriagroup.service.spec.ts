...
import { AtriaGroupModule, AtriaGroupService } from '../src';

const COMPANY_NAME_EXPECT = 'Atria Group';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'atriagroup-jobs.json'), 'utf8'),
);
...
describe('AtriaGroupService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AtriaGroupModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AtriaGroupModule],
      }).compile();
      const service = moduleRef.get(AtriaGroupService);
      expect(service).toBeInstanceOf(AtriaGroupService);
      await moduleRef.close();
    });

    it('exports the Site.ATRIA_GROUP = "atriagroup" enum value', () => {
      expect(Site.ATRIA_GROUP).toBe('atriagroup');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new AtriaGroupService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.ATRIA_GROUP],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('AtriaGroupLLC');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ATRIA_GROUP);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('atriagroup-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new AtriaGroupService(registry);
      const result = await service.scrape({
        siteType: [Site.ATRIA_GROUP],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('AtriaGroupLLC');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('atriagroup-x1');
      expect(result.jobs[0].site).toBe(Site.ATRIA_GROUP);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('atriagroup-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new AtriaGroupService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.ATRIA_GROUP],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new AtriaGroupService();
      const result = await service.scrape({
        siteType: [Site.ATRIA_GROUP],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
