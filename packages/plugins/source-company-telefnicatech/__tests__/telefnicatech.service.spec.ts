...
import { TelefNicaTechModule, TelefNicaTechService } from '../src';

const COMPANY_NAME_EXPECT = 'Telefónica Tech';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'telefnicatech-jobs.json'), 'utf8'),
);
...
describe('TelefNicaTechService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through TelefNicaTechModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [TelefNicaTechModule],
      }).compile();
      const service = moduleRef.get(TelefNicaTechService);
      expect(service).toBeInstanceOf(TelefNicaTechService);
      await moduleRef.close();
    });

    it('exports the Site.TELEF_NICA_TECH = "telefnicatech" enum value', () => {
      expect(Site.TELEF_NICA_TECH).toBe('telefnicatech');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new TelefNicaTechService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.TELEF_NICA_TECH],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'telefnicatech-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.TELEF_NICA_TECH);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('telefnicatech-' + first.id);
      ...
      expect(calledUrls[0]).toContain('telefonicatech');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.TELEF_NICA_TECH);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('telefnicatech-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new TelefNicaTechService(registry);
      const result = await service.scrape({
        siteType: [Site.TELEF_NICA_TECH],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('telefonicatech');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('telefnicatech-x1');
      expect(result.jobs[0].site).toBe(Site.TELEF_NICA_TECH);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('telefnicatech-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new TelefNicaTechService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.TELEF_NICA_TECH],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new TelefNicaTechService();
      const result = await service.scrape({
        siteType: [Site.TELEF_NICA_TECH],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
