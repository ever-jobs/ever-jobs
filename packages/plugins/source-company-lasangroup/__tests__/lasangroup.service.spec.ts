...
import { LasanGroupModule, LasanGroupService } from '../src';

const COMPANY_NAME_EXPECT = 'Lasan Group';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'lasangroup-jobs.json'), 'utf8'),
);
...
describe('LasanGroupService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through LasanGroupModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [LasanGroupModule],
      }).compile();
      const service = moduleRef.get(LasanGroupService);
      expect(service).toBeInstanceOf(LasanGroupService);
      await moduleRef.close();
    });

    it('exports the Site.LASAN_GROUP = "lasangroup" enum value', () => {
      expect(Site.LASAN_GROUP).toBe('lasangroup');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new LasanGroupService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.LASAN_GROUP],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('LasanGroup');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.LASAN_GROUP);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('lasangroup-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new LasanGroupService(registry);
      const result = await service.scrape({
        siteType: [Site.LASAN_GROUP],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('LasanGroup');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('lasangroup-x1');
      expect(result.jobs[0].site).toBe(Site.LASAN_GROUP);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('lasangroup-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new LasanGroupService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.LASAN_GROUP],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new LasanGroupService();
      const result = await service.scrape({
        siteType: [Site.LASAN_GROUP],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
