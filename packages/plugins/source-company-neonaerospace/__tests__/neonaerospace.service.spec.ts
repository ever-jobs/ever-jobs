...
import { NeonAerospaceModule, NeonAerospaceService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'neonaerospace-jobs.json'), 'utf8'),
);
...
/**
 * Spec 890 / T04 — `NeonAerospaceService` unit tests (≥ 9 cases).
 */
describe('NeonAerospaceService — Spec 890 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through NeonAerospaceModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [NeonAerospaceModule],
      }).compile();
      const service = moduleRef.get(NeonAerospaceService);
      expect(service).toBeInstanceOf(NeonAerospaceService);
      await moduleRef.close();
    });

    it('exports the Site.NEON_AEROSPACE = "neonaerospace" enum value', () => {
      expect(Site.NEON_AEROSPACE).toBe('neonaerospace');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new NeonAerospaceService();
      const result = await service.scrape({
        siteType: [Site.NEON_AEROSPACE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'neonaerospace-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.NEON_AEROSPACE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Neon Aerospace');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/neonaerospace/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/neonaerospace/jobs/');
      ...
      expect(job0?.description).toContain('Neon Aerospace');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/neonaerospace/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Neon Aerospace');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'neonaerospace-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
