...
import { TrendyolModule, TrendyolService } from '../src';

const COMPANY_NAME_EXPECT = 'Trendyol';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'trendyol-jobs.json'), 'utf8'),
);
...
describe('TrendyolService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through TrendyolModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [TrendyolModule],
      }).compile();
      const service = moduleRef.get(TrendyolService);
      expect(service).toBeInstanceOf(TrendyolService);
      await moduleRef.close();
    });

    it('exports the Site.TRENDYOL = "trendyol" enum value', () => {
      expect(Site.TRENDYOL).toBe('trendyol');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new TrendyolService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.TRENDYOL],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('trendyol');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.TRENDYOL);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('trendyol-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new TrendyolService(registry);
      const result = await service.scrape({
        siteType: [Site.TRENDYOL],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('trendyol');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('trendyol-x1');
      expect(result.jobs[0].site).toBe(Site.TRENDYOL);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('trendyol-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new TrendyolService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.TRENDYOL],
      } as ScraperInputDto);
