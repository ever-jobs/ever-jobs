...
import { VardaSpaceIndustriesModule, VardaSpaceIndustriesService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'vardaspace-jobs.json'), 'utf8'),
);
...
/**
 * Spec 964 / T04 — `VardaSpaceIndustriesService` unit tests (≥ 9 cases).
 */
describe('VardaSpaceIndustriesService — Spec 964 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through VardaSpaceIndustriesModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [VardaSpaceIndustriesModule],
      }).compile();
      const service = moduleRef.get(VardaSpaceIndustriesService);
      expect(service).toBeInstanceOf(VardaSpaceIndustriesService);
      await moduleRef.close();
    });

    it('exports the Site.VARDA_SPACE_INDUSTRIES = "vardaspace" enum value', () => {
      expect(Site.VARDA_SPACE_INDUSTRIES).toBe('vardaspace');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new VardaSpaceIndustriesService();
      const result = await service.scrape({
        siteType: [Site.VARDA_SPACE_INDUSTRIES],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'vardaspace-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.VARDA_SPACE_INDUSTRIES);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Varda Space Industries');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/vardaspace/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/vardaspace/jobs/');
      ...
      expect(job0?.description).toContain('Varda Space Industries');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/vardaspace/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Varda Space Industries');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'vardaspace-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
