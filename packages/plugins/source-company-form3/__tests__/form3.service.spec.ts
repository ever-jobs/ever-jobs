...
import { Form3Module, Form3Service } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'form3-jobs.json'), 'utf8'),
);
...
/**
 * Spec 516 / T04 — `Form3Service` unit tests (≥ 9 cases).
 */
describe('Form3Service — Spec 516 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through Form3Module via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [Form3Module],
      }).compile();
      const service = moduleRef.get(Form3Service);
      expect(service).toBeInstanceOf(Form3Service);
      await moduleRef.close();
    });

    it('exports the Site.FORM3 = "form3" enum value', () => {
      expect(Site.FORM3).toBe('form3');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new Form3Service();
      ...
      const job0 = dto.jobs.find((j) => j.id === 'form3-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.FORM3);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Form3');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/form3/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/form3/jobs/');
      ...
      expect(job0?.description).toContain('Form3');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/form3/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Form3');
      }
    });
  });
