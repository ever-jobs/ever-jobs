...
import { AmbiententerprisesModule, AmbiententerprisesService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'ambiententerprises-jobs.json'), 'utf8'),
);
...
/**
 * Spec 257 / T04 — `AmbiententerprisesService` unit tests (≥ 9 cases).
 */
describe('AmbiententerprisesService — Spec 257 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AmbiententerprisesModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AmbiententerprisesModule],
      }).compile();
      const service = moduleRef.get(AmbiententerprisesService);
      expect(service).toBeInstanceOf(AmbiententerprisesService);
      await moduleRef.close();
    });

    it('exports the Site.AMBIENTENTERPRISES = "ambiententerprises" enum value', () => {
      expect(Site.AMBIENTENTERPRISES).toBe('ambiententerprises');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AmbiententerprisesService();
      const result = await service.scrape({
        siteType: [Site.AMBIENTENTERPRISES],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'ambiententerprises-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.AMBIENTENTERPRISES);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Ambient Enterprises');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/ambiententerprises/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/ambiententerprises/jobs/');
      ...
      expect(job0?.description).toContain('Ambient Enterprises');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/ambiententerprises/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Ambient Enterprises');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'ambiententerprises-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
