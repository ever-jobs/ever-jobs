...
import { TrustlyModule, TrustlyService } from '../src';

const COMPANY_NAME_EXPECT = 'Trustly';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'trustly-jobs.json'), 'utf8'),
);
...
describe('TrustlyService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through TrustlyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [TrustlyModule],
      }).compile();
      const service = moduleRef.get(TrustlyService);
      expect(service).toBeInstanceOf(TrustlyService);
      await moduleRef.close();
    });

    it('exports the Site.TRUSTLY = "trustly" enum value', () => {
      expect(Site.TRUSTLY).toBe('trustly');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new TrustlyService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.TRUSTLY],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('trustly');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.TRUSTLY);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('trustly-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new TrustlyService(registry);
      const result = await service.scrape({
        siteType: [Site.TRUSTLY],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('trustly');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('trustly-x1');
      expect(result.jobs[0].site).toBe(Site.TRUSTLY);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('trustly-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new TrustlyService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.TRUSTLY],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new TrustlyService();
      const result = await service.scrape({
        siteType: [Site.TRUSTLY],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
