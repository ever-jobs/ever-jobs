...
import { YotpoModule, YotpoService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'yotpo-jobs.json'), 'utf8'),
);
...
/**
 * Spec 534 / T04 — `YotpoService` unit tests (≥ 9 cases).
 */
describe('YotpoService — Spec 534 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through YotpoModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [YotpoModule],
      }).compile();
      const service = moduleRef.get(YotpoService);
      expect(service).toBeInstanceOf(YotpoService);
      await moduleRef.close();
    });

    it('exports the Site.YOTPO = "yotpo" enum value', () => {
      expect(Site.YOTPO).toBe('yotpo');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new YotpoService();
      ...
      const job0 = dto.jobs.find((j) => j.id === 'yotpo-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.YOTPO);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Yotpo');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/yotpo/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/yotpo/jobs/');
      ...
      expect(job0?.description).toContain('Yotpo');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/yotpo/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Yotpo');
      }
    });
  });
