...
import { AmericaninstituteModule, AmericaninstituteService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'americaninstitute-jobs.json'), 'utf8'),
);
...
/**
 * Spec 261 / T04 — `AmericaninstituteService` unit tests (≥ 9 cases).
 */
describe('AmericaninstituteService — Spec 261 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AmericaninstituteModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AmericaninstituteModule],
      }).compile();
      const service = moduleRef.get(AmericaninstituteService);
      expect(service).toBeInstanceOf(AmericaninstituteService);
      await moduleRef.close();
    });

    it('exports the Site.AMERICANINSTITUTE = "americaninstitute" enum value', () => {
      expect(Site.AMERICANINSTITUTE).toBe('americaninstitute');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AmericaninstituteService();
      const result = await service.scrape({
        siteType: [Site.AMERICANINSTITUTE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'americaninstitute-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.AMERICANINSTITUTE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('American Institute');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/americaninstitute/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/americaninstitute/jobs/');
      ...
      expect(job0?.description).toContain('American Institute');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/americaninstitute/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('American Institute');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'americaninstitute-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
