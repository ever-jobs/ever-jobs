...
import { NBCUniversalModule, NBCUniversalService } from '../src';

const COMPANY_NAME_EXPECT = 'NBCUniversal';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'nbcuniversal-jobs.json'), 'utf8'),
);
...
describe('NBCUniversalService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through NBCUniversalModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [NBCUniversalModule],
      }).compile();
      const service = moduleRef.get(NBCUniversalService);
      expect(service).toBeInstanceOf(NBCUniversalService);
      await moduleRef.close();
    });

    it('exports the Site.NBCUNIVERSAL = "nbcuniversal" enum value', () => {
      expect(Site.NBCUNIVERSAL).toBe('nbcuniversal');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new NBCUniversalService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.NBCUNIVERSAL],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('NBCUniversal3');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.NBCUNIVERSAL);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('nbcuniversal-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new NBCUniversalService(registry);
      const result = await service.scrape({
        siteType: [Site.NBCUNIVERSAL],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('NBCUniversal3');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('nbcuniversal-x1');
      expect(result.jobs[0].site).toBe(Site.NBCUNIVERSAL);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('nbcuniversal-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new NBCUniversalService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.NBCUNIVERSAL],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new NBCUniversalService();
      const result = await service.scrape({
        siteType: [Site.NBCUNIVERSAL],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
