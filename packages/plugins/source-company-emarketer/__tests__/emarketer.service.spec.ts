...
import { EMARKETERModule, EMARKETERService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'emarketer-jobs.json'), 'utf8'),
);
...
/**
 * Spec 840 / T04 — `EMARKETERService` unit tests (≥ 9 cases).
 */
describe('EMARKETERService — Spec 840 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through EMARKETERModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [EMARKETERModule],
      }).compile();
      const service = moduleRef.get(EMARKETERService);
      expect(service).toBeInstanceOf(EMARKETERService);
      await moduleRef.close();
    });

    it('exports the Site.EMARKETER = "emarketer" enum value', () => {
      expect(Site.EMARKETER).toBe('emarketer');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new EMARKETERService();
      const result = await service.scrape({
        siteType: [Site.EMARKETER],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'emarketer-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.EMARKETER);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('EMARKETER');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/emarketer/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/emarketer/jobs/');
      ...
      expect(job0?.description).toContain('EMARKETER');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/emarketer/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('EMARKETER');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'emarketer-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
