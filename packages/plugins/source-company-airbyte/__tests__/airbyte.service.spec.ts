...
import { AirbyteModule, AirbyteService } from '../src';

const COMPANY_NAME_EXPECT = 'Airbyte';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'airbyte-jobs.json'), 'utf8'),
);
...
describe('AirbyteService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AirbyteModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AirbyteModule],
      }).compile();
      const service = moduleRef.get(AirbyteService);
      expect(service).toBeInstanceOf(AirbyteService);
      await moduleRef.close();
    });

    it('exports the Site.AIRBYTE = "airbyte" enum value', () => {
      expect(Site.AIRBYTE).toBe('airbyte');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new AirbyteService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.AIRBYTE],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('airbyte');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.AIRBYTE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('airbyte-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new AirbyteService(registry);
      const result = await service.scrape({
        siteType: [Site.AIRBYTE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('airbyte');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('airbyte-x1');
      expect(result.jobs[0].site).toBe(Site.AIRBYTE);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('airbyte-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new AirbyteService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.AIRBYTE],
      } as ScraperInputDto);
