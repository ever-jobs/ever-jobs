...
import { HONKModule, HONKService } from '../src';

const COMPANY_NAME_EXPECT = 'HONK';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'honkforhelp-jobs.json'), 'utf8'),
);
...
describe('HONKService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through HONKModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [HONKModule],
      }).compile();
      const service = moduleRef.get(HONKService);
      expect(service).toBeInstanceOf(HONKService);
      await moduleRef.close();
    });

    it('exports the Site.HONK = "honkforhelp" enum value', () => {
      expect(Site.HONK).toBe('honkforhelp');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new HONKService(registryWithLever());
      ...
      expect(calledUrls[0]).toContain('honkforhelp');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.HONK);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('honkforhelp-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new HONKService(registry);
      const result = await service.scrape({
        siteType: [Site.HONK],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('honkforhelp');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('honkforhelp-x1');
      expect(result.jobs[0].site).toBe(Site.HONK);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('honkforhelp-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new HONKService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.HONK],
      } as ScraperInputDto);
