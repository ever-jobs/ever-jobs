...
import { SambaTVModule, SambaTVService } from '../src';

const COMPANY_NAME_EXPECT = 'Samba TV';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'sambatv-jobs.json'), 'utf8'),
);
...
describe('SambaTVService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SambaTVModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SambaTVModule],
      }).compile();
      const service = moduleRef.get(SambaTVService);
      expect(service).toBeInstanceOf(SambaTVService);
      await moduleRef.close();
    });

    it('exports the Site.SAMBA_TV = "sambatv" enum value', () => {
      expect(Site.SAMBA_TV).toBe('sambatv');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new SambaTVService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.SAMBA_TV],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('sambatv');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SAMBA_TV);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('sambatv-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new SambaTVService(registry);
      const result = await service.scrape({
        siteType: [Site.SAMBA_TV],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('sambatv');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('sambatv-x1');
      expect(result.jobs[0].site).toBe(Site.SAMBA_TV);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('sambatv-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new SambaTVService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.SAMBA_TV],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new SambaTVService();
      const result = await service.scrape({
        siteType: [Site.SAMBA_TV],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
