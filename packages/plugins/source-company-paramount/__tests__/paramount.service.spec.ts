...
import { ParamountModule, ParamountService } from '../src';

const COMPANY_NAME_EXPECT = 'Paramount';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'paramount-jobs.json'), 'utf8'),
);
...
describe('ParamountService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ParamountModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ParamountModule],
      }).compile();
      const service = moduleRef.get(ParamountService);
      expect(service).toBeInstanceOf(ParamountService);
      await moduleRef.close();
    });

    it('exports the Site.PARAMOUNT = "paramount" enum value', () => {
      expect(Site.PARAMOUNT).toBe('paramount');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new ParamountService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.PARAMOUNT],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('ParamountATL');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.PARAMOUNT);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('paramount-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new ParamountService(registry);
      const result = await service.scrape({
        siteType: [Site.PARAMOUNT],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('ParamountATL');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('paramount-x1');
      expect(result.jobs[0].site).toBe(Site.PARAMOUNT);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('paramount-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new ParamountService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.PARAMOUNT],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new ParamountService();
      const result = await service.scrape({
        siteType: [Site.PARAMOUNT],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
