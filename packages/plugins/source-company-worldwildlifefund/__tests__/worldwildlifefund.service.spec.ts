...
import { WorldWildlifeFundModule, WorldWildlifeFundService } from '../src';

const COMPANY_NAME_EXPECT = 'World Wildlife Fund';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'worldwildlifefund-jobs.json'), 'utf8'),
);
...
describe('WorldWildlifeFundService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through WorldWildlifeFundModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [WorldWildlifeFundModule],
      }).compile();
      const service = moduleRef.get(WorldWildlifeFundService);
      expect(service).toBeInstanceOf(WorldWildlifeFundService);
      await moduleRef.close();
    });

    it('exports the Site.WORLD_WILDLIFE_FUND = "worldwildlifefund" enum value', () => {
      expect(Site.WORLD_WILDLIFE_FUND).toBe('worldwildlifefund');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new WorldWildlifeFundService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.WORLD_WILDLIFE_FUND],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'worldwildlifefund-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.WORLD_WILDLIFE_FUND);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('worldwildlifefund-' + first.id);
      ...
      expect(calledUrls[0]).toContain('WorldWildlifeFundInc1');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.WORLD_WILDLIFE_FUND);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('worldwildlifefund-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new WorldWildlifeFundService(registry);
      const result = await service.scrape({
        siteType: [Site.WORLD_WILDLIFE_FUND],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('WorldWildlifeFundInc1');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('worldwildlifefund-x1');
      expect(result.jobs[0].site).toBe(Site.WORLD_WILDLIFE_FUND);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('worldwildlifefund-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new WorldWildlifeFundService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.WORLD_WILDLIFE_FUND],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new WorldWildlifeFundService();
      const result = await service.scrape({
        siteType: [Site.WORLD_WILDLIFE_FUND],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
