...
import { AboveLendingModule, AboveLendingService } from '../src';

const COMPANY_NAME_EXPECT = 'Above Lending';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'abovelending-jobs.json'), 'utf8'),
);
...
describe('AboveLendingService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AboveLendingModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AboveLendingModule],
      }).compile();
      const service = moduleRef.get(AboveLendingService);
      expect(service).toBeInstanceOf(AboveLendingService);
      await moduleRef.close();
    });

    it('exports the Site.ABOVE_LENDING = "abovelending" enum value', () => {
      expect(Site.ABOVE_LENDING).toBe('abovelending');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new AboveLendingService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.ABOVE_LENDING],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'abovelending-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.ABOVE_LENDING);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('abovelending-' + first.id);
      ...
      expect(calledUrls[0]).toContain('abovelending');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ABOVE_LENDING);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('abovelending-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new AboveLendingService(registry);
      const result = await service.scrape({
        siteType: [Site.ABOVE_LENDING],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('abovelending');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('abovelending-x1');
      expect(result.jobs[0].site).toBe(Site.ABOVE_LENDING);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('abovelending-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new AboveLendingService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.ABOVE_LENDING],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new AboveLendingService();
      const result = await service.scrape({
        siteType: [Site.ABOVE_LENDING],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
