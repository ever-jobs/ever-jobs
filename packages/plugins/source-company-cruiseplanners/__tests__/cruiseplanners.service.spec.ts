...
import { CruisePlannersModule, CruisePlannersService } from '../src';

const COMPANY_NAME_EXPECT = 'Cruise Planners';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'cruiseplanners-jobs.json'), 'utf8'),
);
...
describe('CruisePlannersService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CruisePlannersModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CruisePlannersModule],
      }).compile();
      const service = moduleRef.get(CruisePlannersService);
      expect(service).toBeInstanceOf(CruisePlannersService);
      await moduleRef.close();
    });

    it('exports the Site.CRUISE_PLANNERS = "cruiseplanners" enum value', () => {
      expect(Site.CRUISE_PLANNERS).toBe('cruiseplanners');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new CruisePlannersService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.CRUISE_PLANNERS],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'cruiseplanners-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.CRUISE_PLANNERS);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('cruiseplanners-' + first.id);
      ...
      expect(calledUrls[0]).toContain('CruisePlanners');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.CRUISE_PLANNERS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('cruiseplanners-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new CruisePlannersService(registry);
      const result = await service.scrape({
        siteType: [Site.CRUISE_PLANNERS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('CruisePlanners');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('cruiseplanners-x1');
      expect(result.jobs[0].site).toBe(Site.CRUISE_PLANNERS);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('cruiseplanners-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new CruisePlannersService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.CRUISE_PLANNERS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new CruisePlannersService();
      const result = await service.scrape({
        siteType: [Site.CRUISE_PLANNERS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
