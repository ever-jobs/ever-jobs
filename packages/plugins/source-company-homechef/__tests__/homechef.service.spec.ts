...
import { HomechefModule, HomechefService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'homechef-jobs.json'), 'utf8'),
);
...
/**
 * Spec 493 / T04 — `HomechefService` unit tests (≥ 9 cases).
 */
describe('HomechefService — Spec 493 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through HomechefModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [HomechefModule],
      }).compile();
      const service = moduleRef.get(HomechefService);
      expect(service).toBeInstanceOf(HomechefService);
      await moduleRef.close();
    });

    it('exports the Site.HOMECHEF = "homechef" enum value', () => {
      expect(Site.HOMECHEF).toBe('homechef');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new HomechefService();
      const result = await service.scrape({
        siteType: [Site.HOMECHEF],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'homechef-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.HOMECHEF);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Home Chef');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/homechef/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/homechef/jobs/');
      ...
      expect(job0?.description).toContain('Home Chef');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/homechef/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Home Chef');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'homechef-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
