...
import { OportunModule, OportunService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'oportun-jobs.json'), 'utf8'),
);
...
/**
 * Spec 754 / T04 — `OportunService` unit tests (≥ 9 cases).
 */
describe('OportunService — Spec 754 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through OportunModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [OportunModule],
      }).compile();
      const service = moduleRef.get(OportunService);
      expect(service).toBeInstanceOf(OportunService);
      await moduleRef.close();
    });

    it('exports the Site.OPORTUN = "oportun" enum value', () => {
      expect(Site.OPORTUN).toBe('oportun');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new OportunService();
      const result = await service.scrape({
        siteType: [Site.OPORTUN],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'oportun-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.OPORTUN);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Oportun');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/oportun/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/oportun/jobs/');
      ...
      expect(job0?.description).toContain('Oportun');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/oportun/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Oportun');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'oportun-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
