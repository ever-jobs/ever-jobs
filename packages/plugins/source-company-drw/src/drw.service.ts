...
/**
 * DRW — Proprietary trading (HQ: Chicago, IL, USA).
 *
 * Source (Spec 1737): Greenhouse board, scraped in this order:
 *   - `drweng` — https://job-boards.greenhouse.io/drweng
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'drweng', atsIdPrefix: 'gh-' },
];
...
@SourcePlugin({
  ...
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['drw.com'],
  description: 'DRW careers via Greenhouse. Tags: segment=quant-trading; industry=proprietary-trading.',
})
@Injectable()
export class DrwService implements IScraper {
  private readonly logger = new Logger(DrwService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Greenhouse source plugin is not registered; cannot scrape DRW');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`DRW: delegating to Greenhouse (${board.companySlug})`);
    ...
    this.logger.log(`DRW: scraped ${jobs.length} jobs`);
