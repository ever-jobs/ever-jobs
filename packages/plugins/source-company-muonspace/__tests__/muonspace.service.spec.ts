...
import { MuonSpaceModule, MuonSpaceService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'muonspace-jobs.json'), 'utf8'),
);
...
/**
 * Spec 586 / T04 — `MuonSpaceService` unit tests (≥ 9 cases).
 */
describe('MuonSpaceService — Spec 586 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through MuonSpaceModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [MuonSpaceModule],
      }).compile();
      const service = moduleRef.get(MuonSpaceService);
      expect(service).toBeInstanceOf(MuonSpaceService);
      await moduleRef.close();
    });

    it('exports the Site.MUON_SPACE = "muonspace" enum value', () => {
      expect(Site.MUON_SPACE).toBe('muonspace');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new MuonSpaceService();
      const result = await service.scrape({
        siteType: [Site.MUON_SPACE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'muonspace-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.MUON_SPACE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Muon Space');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/muonspace/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/muonspace/jobs/');
      ...
      expect(job0?.description).toContain('Muon Space');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/muonspace/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Muon Space');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'muonspace-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
