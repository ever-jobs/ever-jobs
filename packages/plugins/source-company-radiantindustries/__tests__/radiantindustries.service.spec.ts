...
import { RadiantIndustriesModule, RadiantIndustriesService } from '../src';

const COMPANY_NAME_EXPECT = 'Radiant Industries';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'radiantindustries-jobs.json'), 'utf8'),
);
...
describe('RadiantIndustriesService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through RadiantIndustriesModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [RadiantIndustriesModule],
      }).compile();
      const service = moduleRef.get(RadiantIndustriesService);
      expect(service).toBeInstanceOf(RadiantIndustriesService);
      await moduleRef.close();
    });

    it('exports the Site.RADIANT_INDUSTRIES = "radiantindustries" enum value', () => {
      expect(Site.RADIANT_INDUSTRIES).toBe('radiantindustries');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new RadiantIndustriesService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.RADIANT_INDUSTRIES],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'radiantindustries-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.RADIANT_INDUSTRIES);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('radiantindustries-' + first.id);
      ...
      expect(calledUrls[0]).toContain('radiant-industries');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.RADIANT_INDUSTRIES);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('radiantindustries-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new RadiantIndustriesService(registry);
      const result = await service.scrape({
        siteType: [Site.RADIANT_INDUSTRIES],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('radiant-industries');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('radiantindustries-x1');
      expect(result.jobs[0].site).toBe(Site.RADIANT_INDUSTRIES);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('radiantindustries-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new RadiantIndustriesService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.RADIANT_INDUSTRIES],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new RadiantIndustriesService();
      const result = await service.scrape({
        siteType: [Site.RADIANT_INDUSTRIES],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
