...
import { ApexcompaniesModule, ApexcompaniesService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'apexcompanies-jobs.json'), 'utf8'),
);
...
/**
 * Spec 281 / T04 — `ApexcompaniesService` unit tests (≥ 9 cases).
 */
describe('ApexcompaniesService — Spec 281 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ApexcompaniesModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ApexcompaniesModule],
      }).compile();
      const service = moduleRef.get(ApexcompaniesService);
      expect(service).toBeInstanceOf(ApexcompaniesService);
      await moduleRef.close();
    });

    it('exports the Site.APEXCOMPANIES = "apexcompanies" enum value', () => {
      expect(Site.APEXCOMPANIES).toBe('apexcompanies');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ApexcompaniesService();
      const result = await service.scrape({
        siteType: [Site.APEXCOMPANIES],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'apexcompanies-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.APEXCOMPANIES);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Apex Companies');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/apexcompanies/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/apexcompanies/jobs/');
      ...
      expect(job0?.description).toContain('Apex Companies');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/apexcompanies/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Apex Companies');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'apexcompanies-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
