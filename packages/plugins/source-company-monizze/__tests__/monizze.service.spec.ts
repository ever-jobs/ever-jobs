...
import { MonizzeModule, MonizzeService } from '../src';

const COMPANY_NAME_EXPECT = 'Monizze';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'monizze-jobs.json'), 'utf8'),
);
...
describe('MonizzeService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through MonizzeModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [MonizzeModule],
      }).compile();
      const service = moduleRef.get(MonizzeService);
      expect(service).toBeInstanceOf(MonizzeService);
      await moduleRef.close();
    });

    it('exports the Site.MONIZZE = "monizze" enum value', () => {
      expect(Site.MONIZZE).toBe('monizze');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new MonizzeService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.MONIZZE],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('monizze.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.MONIZZE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('monizze-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new MonizzeService(registry);
      const result = await service.scrape({
        siteType: [Site.MONIZZE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('monizze');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('monizze-x1');
      expect(result.jobs[0].site).toBe(Site.MONIZZE);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('monizze-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new MonizzeService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.MONIZZE],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new MonizzeService();
      const result = await service.scrape({
        siteType: [Site.MONIZZE],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
