...
import { HealthjoyModule, HealthjoyService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'healthjoy-jobs.json'), 'utf8'),
);
...
/**
 * Spec 507 / T04 — `HealthjoyService` unit tests (≥ 9 cases).
 */
describe('HealthjoyService — Spec 507 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through HealthjoyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [HealthjoyModule],
      }).compile();
      const service = moduleRef.get(HealthjoyService);
      expect(service).toBeInstanceOf(HealthjoyService);
      await moduleRef.close();
    });

    it('exports the Site.HEALTHJOY = "healthjoy" enum value', () => {
      expect(Site.HEALTHJOY).toBe('healthjoy');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new HealthjoyService();
      const result = await service.scrape({
        siteType: [Site.HEALTHJOY],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'healthjoy-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.HEALTHJOY);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('HealthJoy');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/healthjoy/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/healthjoy/jobs/');
      ...
      expect(job0?.description).toContain('HealthJoy');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/healthjoy/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('HealthJoy');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'healthjoy-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
