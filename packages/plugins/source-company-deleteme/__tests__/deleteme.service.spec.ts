...
import { DeleteMeModule, DeleteMeService } from '../src';

const COMPANY_NAME_EXPECT = 'DeleteMe';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'deleteme-jobs.json'), 'utf8'),
);
...
describe('DeleteMeService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through DeleteMeModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [DeleteMeModule],
      }).compile();
      const service = moduleRef.get(DeleteMeService);
      expect(service).toBeInstanceOf(DeleteMeService);
      await moduleRef.close();
    });

    it('exports the Site.DELETEME = "deleteme" enum value', () => {
      expect(Site.DELETEME).toBe('deleteme');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new DeleteMeService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.DELETEME],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('deleteme');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.DELETEME);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('deleteme-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new DeleteMeService(registry);
      const result = await service.scrape({
        siteType: [Site.DELETEME],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('deleteme');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('deleteme-x1');
      expect(result.jobs[0].site).toBe(Site.DELETEME);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('deleteme-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new DeleteMeService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.DELETEME],
      } as ScraperInputDto);
