...
import { EpilotModule, EpilotService } from '../src';

const COMPANY_NAME_EXPECT = 'epilot';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'epilot-jobs.json'), 'utf8'),
);
...
describe('EpilotService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through EpilotModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [EpilotModule],
      }).compile();
      const service = moduleRef.get(EpilotService);
      expect(service).toBeInstanceOf(EpilotService);
      await moduleRef.close();
    });

    it('exports the Site.EPILOT = "epilot" enum value', () => {
      expect(Site.EPILOT).toBe('epilot');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new EpilotService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.EPILOT],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('epilot.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.EPILOT);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('epilot-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new EpilotService(registry);
      const result = await service.scrape({
        siteType: [Site.EPILOT],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('epilot');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('epilot-x1');
      expect(result.jobs[0].site).toBe(Site.EPILOT);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('epilot-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new EpilotService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.EPILOT],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new EpilotService();
      const result = await service.scrape({
        siteType: [Site.EPILOT],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
