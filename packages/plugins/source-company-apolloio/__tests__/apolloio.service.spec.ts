...
import { ApolloioModule, ApolloioService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'apolloio-jobs.json'), 'utf8'),
);
...
/**
 * Spec 288 / T04 — `ApolloioService` unit tests (≥ 9 cases).
 */
describe('ApolloioService — Spec 288 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ApolloioModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ApolloioModule],
      }).compile();
      const service = moduleRef.get(ApolloioService);
      expect(service).toBeInstanceOf(ApolloioService);
      await moduleRef.close();
    });

    it('exports the Site.APOLLOIO = "apolloio" enum value', () => {
      expect(Site.APOLLOIO).toBe('apolloio');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ApolloioService();
      const result = await service.scrape({
        siteType: [Site.APOLLOIO],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'apolloio-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.APOLLOIO);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Apollo.io');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/apolloio/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/apolloio/jobs/');
      ...
      expect(job0?.description).toContain('Apollo.io');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/apolloio/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Apollo.io');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'apolloio-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
