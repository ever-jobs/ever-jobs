...
/**
 * Moderna — Biotechnology (HQ: Cambridge, MA, USA).
 *
 * Source (Spec 1736): Workday board, scraped in this order:
 *   - `modernatx:1:M_tx` — https://modernatx.wd1.myworkdayjobs.com/M_tx
 *     verified live 2026-09-24: 205 open postings.
 *
 ...
 * through untouched.
 *
 * Tags: segment=workday-enterprise; industry=biotechnology.
 */
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'modernatx:1:M_tx', atsIdPrefix: 'wd-modernatx-' },
];
...
@SourcePlugin({
  ...
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['modernatx.com'],
  description: 'Moderna careers via Workday. Tags: segment=workday-enterprise; industry=biotechnology.',
})
@Injectable()
export class ModernaService implements IScraper {
  private readonly logger = new Logger(ModernaService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Workday source plugin is not registered; cannot scrape Moderna');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`Moderna: delegating to Workday (${board.companySlug})`);
    ...
    this.logger.log(`Moderna: scraped ${jobs.length} jobs`);
