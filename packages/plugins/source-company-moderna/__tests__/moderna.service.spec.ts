...
import { ModernaModule, ModernaService } from '../src';
...
const FIXTURE: Fixture = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'fixtures', 'moderna-boards.json'), 'utf8'),
);
const COMPANY_NAME = 'Moderna';
const ID_PREFIX = 'moderna-';
...
describe('ModernaService — Workday delegation (Spec 1736)', () => {
  beforeEach(() => {
    mockGet.mockReset();
    mockPost.mockReset();
  ...
  describe('registration', () => {
    it('resolves through ModernaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({ imports: [ModernaModule] }).compile();
      expect(moduleRef.get(ModernaService)).toBeInstanceOf(ModernaService);
      await moduleRef.close();
    });

    it('exports Site.MODERNA = "moderna"', () => {
      expect(Site.MODERNA).toBe('moderna');
    });

    it('declares a tagged company plugin with its domains', () => {
      const meta = Reflect.getMetadata(SOURCE_PLUGIN_METADATA, ModernaService);
      expect(meta.site).toBe(Site.MODERNA);
      expect(meta.category).toBe('company');
      expect(meta.isAts).toBeFalsy();
      expect(meta.companyDomains).toEqual(['modernatx.com']);
  ...
  describe('recorded board (real Workday adapter, mocked HTTP)', () => {
    it('maps every recorded posting and re-stamps the company identity', async () => {
      const service = new ModernaService(registryWith());
      const result = await service.scrape({ siteType: [Site.MODERNA], resultsWanted: 100 } as ScraperInputDto);
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.MODERNA);
    ...
    it('only requests the recorded board URLs', async () => {
      const service = new ModernaService(registryWith());
      await service.scrape({ siteType: [Site.MODERNA] } as ScraperInputDto);
    ...
    it('honours resultsWanted=1', async () => {
      const service = new ModernaService(registryWith());
      const result = await service.scrape({ siteType: [Site.MODERNA], resultsWanted: 1 } as ScraperInputDto);
    ...
    it('resolves with an empty result when the board is gone (HTTP 404)', async () => {
      ...
      const service = new ModernaService(registryWith());
      const result = await service.scrape({ siteType: [Site.MODERNA] } as ScraperInputDto);
  ...
  describe('delegation contract', () => {
    it('forwards the board and the caller input untouched', async () => {
      ...
      const service = new ModernaService(
        registryWith(
          fakeBackend(
      ...
      expect(result.jobs[0].site).toBe(Site.MODERNA);
      expect(result.jobs[0].companyName).toBe(COMPANY_NAME);
    });

    it('passes an absent resultsWanted through as absent', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new ModernaService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      await service.scrape({ siteType: [Site.MODERNA] } as ScraperInputDto);
    ...
    it('makes no request when resultsWanted is 0', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new ModernaService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      const result = await service.scrape({ siteType: [Site.MODERNA], resultsWanted: 0 } as ScraperInputDto);
  ...
  describe('resilience', () => {
    it('reports not_registered when no registry is injected', async () => {
      const result = await new ModernaService().scrape({ siteType: [Site.MODERNA] } as ScraperInputDto);
      expect(result.jobs).toEqual([]);
    ...
    it('reports not_registered when Workday is missing from the registry', async () => {
      const result = await new ModernaService(new PluginRegistry()).scrape({
        siteType: [Site.MODERNA],
      } as ScraperInputDto);
      expect(result.diagnostics?.reason).toBe('not_registered');
    });
