...
import { ZenniOpticalModule, ZenniOpticalService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'zennioptical-jobs.json'), 'utf8'),
);
...
/**
 * Spec 598 / T04 — `ZenniOpticalService` unit tests (≥ 9 cases).
 */
describe('ZenniOpticalService — Spec 598 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ZenniOpticalModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ZenniOpticalModule],
      }).compile();
      const service = moduleRef.get(ZenniOpticalService);
      expect(service).toBeInstanceOf(ZenniOpticalService);
      await moduleRef.close();
    });

    it('exports the Site.ZENNI_OPTICAL = "zennioptical" enum value', () => {
      expect(Site.ZENNI_OPTICAL).toBe('zennioptical');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ZenniOpticalService();
      const result = await service.scrape({
        siteType: [Site.ZENNI_OPTICAL],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'zennioptical-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ZENNI_OPTICAL);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Zenni Optical');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/zennioptical/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/zennioptical/jobs/');
      ...
      expect(job0?.description).toContain('Zenni Optical');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/zennioptical/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Zenni Optical');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'zennioptical-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
