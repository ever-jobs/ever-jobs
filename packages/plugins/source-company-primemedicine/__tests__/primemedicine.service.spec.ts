...
import { PrimeMedicineModule, PrimeMedicineService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'primemedicine-jobs.json'), 'utf8'),
);
...
/**
 * Spec 914 / T04 — `PrimeMedicineService` unit tests (≥ 9 cases).
 */
describe('PrimeMedicineService — Spec 914 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through PrimeMedicineModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [PrimeMedicineModule],
      }).compile();
      const service = moduleRef.get(PrimeMedicineService);
      expect(service).toBeInstanceOf(PrimeMedicineService);
      await moduleRef.close();
    });

    it('exports the Site.PRIME_MEDICINE = "primemedicine" enum value', () => {
      expect(Site.PRIME_MEDICINE).toBe('primemedicine');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new PrimeMedicineService();
      const result = await service.scrape({
        siteType: [Site.PRIME_MEDICINE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'primemedicine-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.PRIME_MEDICINE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Prime Medicine');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/primemedicine/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/primemedicine/jobs/');
      ...
      expect(job0?.description).toContain('Prime Medicine');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/primemedicine/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Prime Medicine');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'primemedicine-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
