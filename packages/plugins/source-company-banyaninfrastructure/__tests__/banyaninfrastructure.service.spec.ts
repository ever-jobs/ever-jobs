...
import { BanyanInfrastructureModule, BanyanInfrastructureService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'banyaninfrastructure-jobs.json'), 'utf8'),
);
...
/**
 * Spec 681 / T04 — `BanyanInfrastructureService` unit tests (≥ 9 cases).
 */
describe('BanyanInfrastructureService — Spec 681 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through BanyanInfrastructureModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [BanyanInfrastructureModule],
      }).compile();
      const service = moduleRef.get(BanyanInfrastructureService);
      expect(service).toBeInstanceOf(BanyanInfrastructureService);
      await moduleRef.close();
    });

    it('exports the Site.BANYAN_INFRASTRUCTURE = "banyaninfrastructure" enum value', () => {
      expect(Site.BANYAN_INFRASTRUCTURE).toBe('banyaninfrastructure');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new BanyanInfrastructureService();
      const result = await service.scrape({
        siteType: [Site.BANYAN_INFRASTRUCTURE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'banyaninfrastructure-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.BANYAN_INFRASTRUCTURE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Banyan Infrastructure');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/banyaninfrastructure/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/banyaninfrastructure/jobs/');
      ...
      expect(job0?.description).toContain('Banyan Infrastructure');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/banyaninfrastructure/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Banyan Infrastructure');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'banyaninfrastructure-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
