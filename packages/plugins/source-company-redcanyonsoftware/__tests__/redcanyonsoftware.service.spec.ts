...
import { RedCanyonEngineeringSoftwareModule, RedCanyonEngineeringSoftwareService } from '../src';

const COMPANY_NAME_EXPECT = 'Red Canyon Engineering & Software';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'redcanyonsoftware-jobs.json'), 'utf8'),
);
...
describe('RedCanyonEngineeringSoftwareService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through RedCanyonEngineeringSoftwareModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [RedCanyonEngineeringSoftwareModule],
      }).compile();
      const service = moduleRef.get(RedCanyonEngineeringSoftwareService);
      expect(service).toBeInstanceOf(RedCanyonEngineeringSoftwareService);
      await moduleRef.close();
    });

    it('exports the Site.RED_CANYON_ENGINEERING_SOFTWARE = "redcanyonsoftware" enum value', () => {
      expect(Site.RED_CANYON_ENGINEERING_SOFTWARE).toBe('redcanyonsoftware');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new RedCanyonEngineeringSoftwareService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.RED_CANYON_ENGINEERING_SOFTWARE],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'redcanyonsoftware-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.RED_CANYON_ENGINEERING_SOFTWARE);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('redcanyonsoftware-' + first.id);
      ...
      expect(calledUrls[0]).toContain('redcanyonsoftware');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.RED_CANYON_ENGINEERING_SOFTWARE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('redcanyonsoftware-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new RedCanyonEngineeringSoftwareService(registry);
      const result = await service.scrape({
        siteType: [Site.RED_CANYON_ENGINEERING_SOFTWARE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('redcanyonsoftware');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('redcanyonsoftware-x1');
      expect(result.jobs[0].site).toBe(Site.RED_CANYON_ENGINEERING_SOFTWARE);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('redcanyonsoftware-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new RedCanyonEngineeringSoftwareService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.RED_CANYON_ENGINEERING_SOFTWARE],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new RedCanyonEngineeringSoftwareService();
      const result = await service.scrape({
        siteType: [Site.RED_CANYON_ENGINEERING_SOFTWARE],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
