...
import { XtxMarketsModule, XtxMarketsService } from '../src';
...
const FIXTURE: Fixture = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'fixtures', 'xtxmarkets-boards.json'), 'utf8'),
);
const COMPANY_NAME = 'XTX Markets';
const ID_PREFIX = 'xtxmarkets-';
...
describe('XtxMarketsService — Greenhouse delegation (Spec 1737)', () => {
  beforeEach(() => {
    mockGet.mockReset();
    mockPost.mockReset();
  ...
  describe('registration', () => {
    it('resolves through XtxMarketsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({ imports: [XtxMarketsModule] }).compile();
      expect(moduleRef.get(XtxMarketsService)).toBeInstanceOf(XtxMarketsService);
      await moduleRef.close();
    });

    it('exports Site.XTX_MARKETS = "xtxmarkets"', () => {
      expect(Site.XTX_MARKETS).toBe('xtxmarkets');
    });

    it('declares a tagged company plugin with its domains', () => {
      const meta = Reflect.getMetadata(SOURCE_PLUGIN_METADATA, XtxMarketsService);
      expect(meta.site).toBe(Site.XTX_MARKETS);
      expect(meta.category).toBe('company');
      expect(meta.isAts).toBeFalsy();
      expect(meta.companyDomains).toEqual(['xtxmarkets.com']);
  ...
  describe('recorded board (real Greenhouse adapter, mocked HTTP)', () => {
    it('maps every recorded posting and re-stamps the company identity', async () => {
      const service = new XtxMarketsService(registryWith());
      const result = await service.scrape({ siteType: [Site.XTX_MARKETS], resultsWanted: 100 } as ScraperInputDto);
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.XTX_MARKETS);
    ...
    it('only requests the recorded board URLs', async () => {
      const service = new XtxMarketsService(registryWith());
      await service.scrape({ siteType: [Site.XTX_MARKETS] } as ScraperInputDto);
    ...
    it('honours resultsWanted=1', async () => {
      const service = new XtxMarketsService(registryWith());
      const result = await service.scrape({ siteType: [Site.XTX_MARKETS], resultsWanted: 1 } as ScraperInputDto);
    ...
    it('resolves with an empty result when the board is gone (HTTP 404)', async () => {
      ...
      const service = new XtxMarketsService(registryWith());
      const result = await service.scrape({ siteType: [Site.XTX_MARKETS] } as ScraperInputDto);
  ...
  describe('delegation contract', () => {
    it('forwards the board and the caller input untouched', async () => {
      ...
      const service = new XtxMarketsService(
        registryWith(
          fakeBackend(
      ...
      expect(result.jobs[0].site).toBe(Site.XTX_MARKETS);
      expect(result.jobs[0].companyName).toBe(COMPANY_NAME);
    });

    it('passes an absent resultsWanted through as absent', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new XtxMarketsService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      await service.scrape({ siteType: [Site.XTX_MARKETS] } as ScraperInputDto);
    ...
    it('makes no request when resultsWanted is 0', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new XtxMarketsService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      const result = await service.scrape({ siteType: [Site.XTX_MARKETS], resultsWanted: 0 } as ScraperInputDto);
  ...
  describe('resilience', () => {
    it('reports not_registered when no registry is injected', async () => {
      const result = await new XtxMarketsService().scrape({ siteType: [Site.XTX_MARKETS] } as ScraperInputDto);
      expect(result.jobs).toEqual([]);
    ...
    it('reports not_registered when Greenhouse is missing from the registry', async () => {
      const result = await new XtxMarketsService(new PluginRegistry()).scrape({
        siteType: [Site.XTX_MARKETS],
      } as ScraperInputDto);
      expect(result.diagnostics?.reason).toBe('not_registered');
    });
