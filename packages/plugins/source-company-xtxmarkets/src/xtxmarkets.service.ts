...
/**
 * XTX Markets — Electronic market making (HQ: London, United Kingdom).
 *
 * Source (Spec 1737): Greenhouse board, scraped in this order:
 *   - `xtxmarketstechnologies` — https://job-boards.greenhouse.io/xtxmarketstechnologies
 *     verified live 2026-09-24: 9 open postings.
 *
 ...
 * through untouched.
 *
 * Tags: segment=quant-trading; industry=electronic-market-making.
 */
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'xtxmarketstechnologies', atsIdPrefix: 'gh-' },
];

@SourcePlugin({
  site: Site.XTX_MARKETS,
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['xtxmarkets.com'],
  description: 'XTX Markets careers via Greenhouse. Tags: segment=quant-trading; industry=electronic-market-making.',
})
@Injectable()
export class XtxMarketsService implements IScraper {
  private readonly logger = new Logger(XtxMarketsService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Greenhouse source plugin is not registered; cannot scrape XTX Markets');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`XTX Markets: delegating to Greenhouse (${board.companySlug})`);
    ...
    this.logger.log(`XTX Markets: scraped ${jobs.length} jobs`);
