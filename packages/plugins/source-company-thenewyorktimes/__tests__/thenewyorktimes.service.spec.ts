...
import { TheNewYorkTimesModule, TheNewYorkTimesService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'thenewyorktimes-jobs.json'), 'utf8'),
);
...
/**
 * Spec 954 / T04 — `TheNewYorkTimesService` unit tests (≥ 9 cases).
 */
describe('TheNewYorkTimesService — Spec 954 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through TheNewYorkTimesModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [TheNewYorkTimesModule],
      }).compile();
      const service = moduleRef.get(TheNewYorkTimesService);
      expect(service).toBeInstanceOf(TheNewYorkTimesService);
      await moduleRef.close();
    });

    it('exports the Site.THE_NEW_YORK_TIMES = "thenewyorktimes" enum value', () => {
      expect(Site.THE_NEW_YORK_TIMES).toBe('thenewyorktimes');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new TheNewYorkTimesService();
      ...
      const job0 = dto.jobs.find((j) => j.id === 'thenewyorktimes-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.THE_NEW_YORK_TIMES);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('The New York Times');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/thenewyorktimes/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/thenewyorktimes/jobs/');
      ...
      expect(job0?.description).toContain('The New York Times');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/thenewyorktimes/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('The New York Times');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'thenewyorktimes-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
