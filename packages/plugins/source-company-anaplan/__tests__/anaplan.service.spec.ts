...
import { AnaplanModule, AnaplanService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'anaplan-jobs.json'), 'utf8'),
);
...
/**
 * Spec 267 / T04 — `AnaplanService` unit tests (≥ 9 cases).
 */
describe('AnaplanService — Spec 267 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AnaplanModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AnaplanModule],
      }).compile();
      const service = moduleRef.get(AnaplanService);
      expect(service).toBeInstanceOf(AnaplanService);
      await moduleRef.close();
    });

    it('exports the Site.ANAPLAN = "anaplan" enum value', () => {
      expect(Site.ANAPLAN).toBe('anaplan');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AnaplanService();
      const result = await service.scrape({
        siteType: [Site.ANAPLAN],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'anaplan-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ANAPLAN);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Anaplan');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/anaplan/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/anaplan/jobs/');
      ...
      expect(job0?.description).toContain('Anaplan');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/anaplan/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Anaplan');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'anaplan-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
