...
import { GroveCollaborativeModule, GroveCollaborativeService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'grovecollaborative-jobs.json'), 'utf8'),
);
...
/**
 * Spec 674 / T04 — `GroveCollaborativeService` unit tests (≥ 9 cases).
 */
describe('GroveCollaborativeService — Spec 674 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through GroveCollaborativeModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [GroveCollaborativeModule],
      }).compile();
      const service = moduleRef.get(GroveCollaborativeService);
      expect(service).toBeInstanceOf(GroveCollaborativeService);
      await moduleRef.close();
    });

    it('exports the Site.GROVE_COLLABORATIVE = "grovecollaborative" enum value', () => {
      expect(Site.GROVE_COLLABORATIVE).toBe('grovecollaborative');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new GroveCollaborativeService();
      const result = await service.scrape({
        siteType: [Site.GROVE_COLLABORATIVE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'grovecollaborative-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.GROVE_COLLABORATIVE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Grove Collaborative');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/grovecollaborative/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/grovecollaborative/jobs/');
      ...
      expect(job0?.description).toContain('Grove Collaborative');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/grovecollaborative/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Grove Collaborative');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'grovecollaborative-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
