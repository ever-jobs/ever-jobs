...
import { SpireGlobalModule, SpireGlobalService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'spire-jobs.json'), 'utf8'),
);
...
/**
 * Spec 585 / T04 — `SpireGlobalService` unit tests (≥ 9 cases).
 */
describe('SpireGlobalService — Spec 585 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SpireGlobalModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SpireGlobalModule],
      }).compile();
      const service = moduleRef.get(SpireGlobalService);
      expect(service).toBeInstanceOf(SpireGlobalService);
      await moduleRef.close();
    });

    it('exports the Site.SPIRE_GLOBAL = "spire" enum value', () => {
      expect(Site.SPIRE_GLOBAL).toBe('spire');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new SpireGlobalService();
      const result = await service.scrape({
        siteType: [Site.SPIRE_GLOBAL],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'spire-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.SPIRE_GLOBAL);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Spire Global');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/spire/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/spire/jobs/');
      ...
      expect(job0?.description).toContain('Spire Global');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/spire/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Spire Global');
      }
    });
  });
