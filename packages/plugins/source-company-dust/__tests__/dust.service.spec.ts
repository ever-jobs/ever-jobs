...
import { DustModule, DustService } from '../src';

const COMPANY_NAME_EXPECT = 'Dust';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'dust-jobs.json'), 'utf8'),
);
...
describe('DustService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through DustModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [DustModule],
      }).compile();
      const service = moduleRef.get(DustService);
      expect(service).toBeInstanceOf(DustService);
      await moduleRef.close();
    });

    it('exports the Site.DUST = "dust" enum value', () => {
      expect(Site.DUST).toBe('dust');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new DustService(registryWithAshby());
      ...
      expect(calledUrls[0]).toContain('dust');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.DUST);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('dust-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new DustService(registry);
      const result = await service.scrape({
        siteType: [Site.DUST],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('dust');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('dust-x1');
      expect(result.jobs[0].site).toBe(Site.DUST);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('dust-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new DustService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.DUST],
      } as ScraperInputDto);
