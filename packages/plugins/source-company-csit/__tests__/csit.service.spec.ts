...
import { CentreForStrategicInfocommTechnologiesCSITModule, CentreForStrategicInfocommTechnologiesCSITService } from '../src';

const COMPANY_NAME_EXPECT = 'Centre for Strategic Infocomm Technologies (CSIT)';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'csit-jobs.json'), 'utf8'),
);
...
describe('CentreForStrategicInfocommTechnologiesCSITService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CentreForStrategicInfocommTechnologiesCSITModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CentreForStrategicInfocommTechnologiesCSITModule],
      }).compile();
      const service = moduleRef.get(CentreForStrategicInfocommTechnologiesCSITService);
      expect(service).toBeInstanceOf(CentreForStrategicInfocommTechnologiesCSITService);
      await moduleRef.close();
    });

    it('exports the Site.CENTRE_FOR_STRATEGIC_INFOCOMM_TECHNOLOGIES_CSIT = "csit" enum value', () => {
      expect(Site.CENTRE_FOR_STRATEGIC_INFOCOMM_TECHNOLOGIES_CSIT).toBe('csit');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new CentreForStrategicInfocommTechnologiesCSITService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.CENTRE_FOR_STRATEGIC_INFOCOMM_TECHNOLOGIES_CSIT],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'csit-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.CENTRE_FOR_STRATEGIC_INFOCOMM_TECHNOLOGIES_CSIT);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('csit-' + first.id);
      ...
      expect(calledUrls[0]).toContain('csit');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.CENTRE_FOR_STRATEGIC_INFOCOMM_TECHNOLOGIES_CSIT);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('csit-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new CentreForStrategicInfocommTechnologiesCSITService(registry);
      const result = await service.scrape({
        siteType: [Site.CENTRE_FOR_STRATEGIC_INFOCOMM_TECHNOLOGIES_CSIT],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('csit');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('csit-x1');
      expect(result.jobs[0].site).toBe(Site.CENTRE_FOR_STRATEGIC_INFOCOMM_TECHNOLOGIES_CSIT);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('csit-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new CentreForStrategicInfocommTechnologiesCSITService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.CENTRE_FOR_STRATEGIC_INFOCOMM_TECHNOLOGIES_CSIT],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new CentreForStrategicInfocommTechnologiesCSITService();
      const result = await service.scrape({
        siteType: [Site.CENTRE_FOR_STRATEGIC_INFOCOMM_TECHNOLOGIES_CSIT],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
