...
/**
 * Vercel Inc. — frontend cloud / Next.js framework / edge-network developer
 * platform — publishes its consolidated careers board through Greenhouse at
 * the bare `vercel` slug (no asymmetry; see Spec 043 § 10 D-05).
 */
const API_URL = 'https://api.greenhouse.io/v1/boards/vercel/jobs';
...
export class VercelService implements IScraper {
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    try {
      ...
      this.logger.log(`Vercel: fetching ${url}`);
        ...
        jobs.push(
          new JobPostDto({
            ...
            jobUrl:
              ...
              `https://job-boards.greenhouse.io/vercel/jobs/${listing.id}`,
            location,
      ...
      this.logger.log(`Vercel: scraped ${jobs.length} jobs`);
    } catch (err: any) {
      this.logger.error(`Vercel scrape failed: ${err.message}`);
