...
/**
 * Chevron — Energy (HQ: Houston, TX, USA).
 *
 * Source (Spec 1736): Workday board, scraped in this order:
 *   - `chevron:5:jobs` — https://chevron.wd5.myworkdayjobs.com/jobs
 *     verified live 2026-09-24: 132 open postings.
 *
 ...
 * through untouched.
 *
 * Tags: segment=workday-enterprise; industry=energy.
 */
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'chevron:5:jobs', atsIdPrefix: 'wd-chevron-' },
];
...
@SourcePlugin({
  ...
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['chevron.com'],
  description: 'Chevron careers via Workday. Tags: segment=workday-enterprise; industry=energy.',
})
@Injectable()
export class ChevronService implements IScraper {
  private readonly logger = new Logger(ChevronService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Workday source plugin is not registered; cannot scrape Chevron');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`Chevron: delegating to Workday (${board.companySlug})`);
    ...
    this.logger.log(`Chevron: scraped ${jobs.length} jobs`);
