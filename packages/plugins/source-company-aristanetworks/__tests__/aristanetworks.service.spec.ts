...
import { AristaNetworksModule, AristaNetworksService } from '../src';

const COMPANY_NAME_EXPECT = 'Arista Networks';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'aristanetworks-jobs.json'), 'utf8'),
);
...
describe('AristaNetworksService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AristaNetworksModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AristaNetworksModule],
      }).compile();
      const service = moduleRef.get(AristaNetworksService);
      expect(service).toBeInstanceOf(AristaNetworksService);
      await moduleRef.close();
    });

    it('exports the Site.ARISTA_NETWORKS = "aristanetworks" enum value', () => {
      expect(Site.ARISTA_NETWORKS).toBe('aristanetworks');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new AristaNetworksService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.ARISTA_NETWORKS],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'aristanetworks-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.ARISTA_NETWORKS);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('aristanetworks-' + first.id);
      ...
      expect(calledUrls[0]).toContain('aristanetworks');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ARISTA_NETWORKS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('aristanetworks-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new AristaNetworksService(registry);
      const result = await service.scrape({
        siteType: [Site.ARISTA_NETWORKS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('aristanetworks');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('aristanetworks-x1');
      expect(result.jobs[0].site).toBe(Site.ARISTA_NETWORKS);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('aristanetworks-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new AristaNetworksService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.ARISTA_NETWORKS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new AristaNetworksService();
      const result = await service.scrape({
        siteType: [Site.ARISTA_NETWORKS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
