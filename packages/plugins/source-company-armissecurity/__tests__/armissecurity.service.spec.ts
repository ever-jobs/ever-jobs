...
import { ArmisModule, ArmisService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'armissecurity-jobs.json'), 'utf8'),
);
...
/**
 * Spec 808 / T04 — `ArmisService` unit tests (≥ 9 cases).
 */
describe('ArmisService — Spec 808 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ArmisModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ArmisModule],
      }).compile();
      const service = moduleRef.get(ArmisService);
      expect(service).toBeInstanceOf(ArmisService);
      await moduleRef.close();
    });

    it('exports the Site.ARMIS = "armissecurity" enum value', () => {
      expect(Site.ARMIS).toBe('armissecurity');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ArmisService();
      ...
      const job0 = dto.jobs.find((j) => j.id === 'armissecurity-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ARMIS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Armis');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/armissecurity/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/armissecurity/jobs/');
      ...
      expect(job0?.description).toContain('Armis');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/armissecurity/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Armis');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'armissecurity-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
