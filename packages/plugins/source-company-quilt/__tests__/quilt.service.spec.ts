...
import { QuiltModule, QuiltService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'quilt-jobs.json'), 'utf8'),
);
...
/**
 * Spec 704 / T04 — `QuiltService` unit tests (≥ 9 cases).
 */
describe('QuiltService — Spec 704 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through QuiltModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [QuiltModule],
      }).compile();
      const service = moduleRef.get(QuiltService);
      expect(service).toBeInstanceOf(QuiltService);
      await moduleRef.close();
    });

    it('exports the Site.QUILT = "quilt" enum value', () => {
      expect(Site.QUILT).toBe('quilt');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new QuiltService();
      ...
      const job0 = dto.jobs.find((j) => j.id === 'quilt-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.QUILT);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Quilt');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/quilt/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/quilt/jobs/');
      ...
      expect(job0?.description).toContain('Quilt');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/quilt/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Quilt');
      }
    });
  });
