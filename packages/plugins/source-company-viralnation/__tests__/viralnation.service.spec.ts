...
import { ViralNationModule, ViralNationService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'viralnation-jobs.json'), 'utf8'),
);
...
/**
 * Spec 967 / T04 — `ViralNationService` unit tests (≥ 9 cases).
 */
describe('ViralNationService — Spec 967 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ViralNationModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ViralNationModule],
      }).compile();
      const service = moduleRef.get(ViralNationService);
      expect(service).toBeInstanceOf(ViralNationService);
      await moduleRef.close();
    });

    it('exports the Site.VIRAL_NATION = "viralnation" enum value', () => {
      expect(Site.VIRAL_NATION).toBe('viralnation');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ViralNationService();
      const result = await service.scrape({
        siteType: [Site.VIRAL_NATION],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'viralnation-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.VIRAL_NATION);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Viral Nation');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/viralnation/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/viralnation/jobs/');
      ...
      expect(job0?.description).toContain('Viral Nation');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/viralnation/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Viral Nation');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'viralnation-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
