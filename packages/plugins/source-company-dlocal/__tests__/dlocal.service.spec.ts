...
import { DLocalModule, DLocalService } from '../src';

const COMPANY_NAME_EXPECT = 'dLocal';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'dlocal-jobs.json'), 'utf8'),
);
...
describe('DLocalService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through DLocalModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [DLocalModule],
      }).compile();
      const service = moduleRef.get(DLocalService);
      expect(service).toBeInstanceOf(DLocalService);
      await moduleRef.close();
    });

    it('exports the Site.DLOCAL = "dlocal" enum value', () => {
      expect(Site.DLOCAL).toBe('dlocal');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new DLocalService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.DLOCAL],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('dlocal');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.DLOCAL);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('dlocal-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new DLocalService(registry);
      const result = await service.scrape({
        siteType: [Site.DLOCAL],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('dlocal');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('dlocal-x1');
      expect(result.jobs[0].site).toBe(Site.DLOCAL);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('dlocal-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new DLocalService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.DLOCAL],
      } as ScraperInputDto);
