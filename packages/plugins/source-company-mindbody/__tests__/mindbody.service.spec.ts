...
import { MindbodyModule, MindbodyService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'mindbody-jobs.json'), 'utf8'),
);
...
/**
 * Spec 435 / T04 — `MindbodyService` unit tests (≥ 9 cases).
 */
describe('MindbodyService — Spec 435 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through MindbodyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [MindbodyModule],
      }).compile();
      const service = moduleRef.get(MindbodyService);
      expect(service).toBeInstanceOf(MindbodyService);
      await moduleRef.close();
    });

    it('exports the Site.MINDBODY = "mindbody" enum value', () => {
      expect(Site.MINDBODY).toBe('mindbody');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new MindbodyService();
      const result = await service.scrape({
        siteType: [Site.MINDBODY],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'mindbody-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.MINDBODY);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Mindbody');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/mindbody/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/mindbody/jobs/');
      ...
      expect(job0?.description).toContain('Mindbody');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/mindbody/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Mindbody');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'mindbody-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
