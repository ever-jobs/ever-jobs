...
import { MonteCarloModule, MonteCarloService } from '../src';

const COMPANY_NAME_EXPECT = 'Monte Carlo';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'montecarlo-jobs.json'), 'utf8'),
);
...
describe('MonteCarloService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through MonteCarloModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [MonteCarloModule],
      }).compile();
      const service = moduleRef.get(MonteCarloService);
      expect(service).toBeInstanceOf(MonteCarloService);
      await moduleRef.close();
    });

    it('exports the Site.MONTE_CARLO = "montecarlo" enum value', () => {
      expect(Site.MONTE_CARLO).toBe('montecarlo');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new MonteCarloService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.MONTE_CARLO],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('montecarlodata');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.MONTE_CARLO);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('montecarlo-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new MonteCarloService(registry);
      const result = await service.scrape({
        siteType: [Site.MONTE_CARLO],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('montecarlodata');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('montecarlo-x1');
      expect(result.jobs[0].site).toBe(Site.MONTE_CARLO);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('montecarlo-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new MonteCarloService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.MONTE_CARLO],
      } as ScraperInputDto);
