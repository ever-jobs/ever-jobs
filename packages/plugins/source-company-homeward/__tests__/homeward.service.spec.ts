...
import { HomewardModule, HomewardService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'homeward-jobs.json'), 'utf8'),
);
...
/**
 * Spec 864 / T04 — `HomewardService` unit tests (≥ 9 cases).
 */
describe('HomewardService — Spec 864 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through HomewardModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [HomewardModule],
      }).compile();
      const service = moduleRef.get(HomewardService);
      expect(service).toBeInstanceOf(HomewardService);
      await moduleRef.close();
    });

    it('exports the Site.HOMEWARD = "homeward" enum value', () => {
      expect(Site.HOMEWARD).toBe('homeward');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new HomewardService();
      const result = await service.scrape({
        siteType: [Site.HOMEWARD],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'homeward-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.HOMEWARD);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Homeward');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/homeward/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/homeward/jobs/');
      ...
      expect(job0?.description).toContain('Homeward');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/homeward/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Homeward');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'homeward-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
