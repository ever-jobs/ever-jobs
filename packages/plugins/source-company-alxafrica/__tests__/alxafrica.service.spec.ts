...
import { AlxafricaModule, AlxafricaService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'alxafrica-jobs.json'), 'utf8'),
);
...
/**
 * Spec 254 / T04 — `AlxafricaService` unit tests (≥ 9 cases).
 */
describe('AlxafricaService — Spec 254 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AlxafricaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AlxafricaModule],
      }).compile();
      const service = moduleRef.get(AlxafricaService);
      expect(service).toBeInstanceOf(AlxafricaService);
      await moduleRef.close();
    });

    it('exports the Site.ALXAFRICA = "alxafrica" enum value', () => {
      expect(Site.ALXAFRICA).toBe('alxafrica');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AlxafricaService();
      const result = await service.scrape({
        siteType: [Site.ALXAFRICA],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'alxafrica-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ALXAFRICA);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('ALX Africa');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/alxafrica/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/alxafrica/jobs/');
      ...
      expect(job0?.description).toContain('ALX Africa');
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('ALX Africa');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'alxafrica-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
