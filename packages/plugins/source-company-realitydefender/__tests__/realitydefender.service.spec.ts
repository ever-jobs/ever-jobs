...
import { RealityDefenderModule, RealityDefenderService } from '../src';

const COMPANY_NAME_EXPECT = 'Reality Defender';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'realitydefender-jobs.json'), 'utf8'),
);
...
describe('RealityDefenderService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through RealityDefenderModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [RealityDefenderModule],
      }).compile();
      const service = moduleRef.get(RealityDefenderService);
      expect(service).toBeInstanceOf(RealityDefenderService);
      await moduleRef.close();
    });

    it('exports the Site.REALITY_DEFENDER = "realitydefender" enum value', () => {
      expect(Site.REALITY_DEFENDER).toBe('realitydefender');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new RealityDefenderService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.REALITY_DEFENDER],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'realitydefender-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.REALITY_DEFENDER);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('realitydefender-' + first.id);
      ...
      expect(calledUrls[0]).toContain('realitydefender');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.REALITY_DEFENDER);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('realitydefender-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new RealityDefenderService(registry);
      const result = await service.scrape({
        siteType: [Site.REALITY_DEFENDER],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('realitydefender');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('realitydefender-x1');
      expect(result.jobs[0].site).toBe(Site.REALITY_DEFENDER);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('realitydefender-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new RealityDefenderService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.REALITY_DEFENDER],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new RealityDefenderService();
      const result = await service.scrape({
        siteType: [Site.REALITY_DEFENDER],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
