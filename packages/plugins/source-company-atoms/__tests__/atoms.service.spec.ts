...
import { AtomsTechModule, AtomsTechService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'atoms-jobs.json'), 'utf8'),
);
...
/**
 * Spec 668 / T04 — `AtomsTechService` unit tests (≥ 9 cases).
 */
describe('AtomsTechService — Spec 668 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AtomsTechModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AtomsTechModule],
      }).compile();
      const service = moduleRef.get(AtomsTechService);
      expect(service).toBeInstanceOf(AtomsTechService);
      await moduleRef.close();
    });

    it('exports the Site.ATOMS_TECH = "atoms" enum value', () => {
      expect(Site.ATOMS_TECH).toBe('atoms');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AtomsTechService();
      const result = await service.scrape({
        siteType: [Site.ATOMS_TECH],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'atoms-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ATOMS_TECH);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Atoms Tech');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/atoms/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/atoms/jobs/');
      ...
      expect(job0?.description).toContain('Atoms Tech');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/atoms/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Atoms Tech');
      }
    });
  });
