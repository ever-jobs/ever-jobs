...
import { NorthspyreModule, NorthspyreService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'northspyre-jobs.json'), 'utf8'),
);
...
/**
 * Spec 897 / T04 — `NorthspyreService` unit tests (≥ 9 cases).
 */
describe('NorthspyreService — Spec 897 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through NorthspyreModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [NorthspyreModule],
      }).compile();
      const service = moduleRef.get(NorthspyreService);
      expect(service).toBeInstanceOf(NorthspyreService);
      await moduleRef.close();
    });

    it('exports the Site.NORTHSPYRE = "northspyre" enum value', () => {
      expect(Site.NORTHSPYRE).toBe('northspyre');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new NorthspyreService();
      const result = await service.scrape({
        siteType: [Site.NORTHSPYRE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'northspyre-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.NORTHSPYRE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Northspyre');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/northspyre/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/northspyre/jobs/');
      ...
      expect(job0?.description).toContain('Northspyre');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/northspyre/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Northspyre');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'northspyre-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
