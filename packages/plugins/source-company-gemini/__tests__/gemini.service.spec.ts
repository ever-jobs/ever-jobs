...
import { GeminiModule, GeminiService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'gemini-jobs.json'), 'utf8'),
);
...
/**
 * Spec 427 / T04 — `GeminiService` unit tests (≥ 9 cases).
 */
describe('GeminiService — Spec 427 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through GeminiModule via NestJS DI', async () => {
    ...
    it('exports the Site.GEMINI = "gemini" enum value', () => {
      expect(Site.GEMINI).toBe('gemini');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const job0 = dto.jobs.find((j) => j.id === 'gemini-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.GEMINI);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Gemini');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/gemini/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/gemini/jobs/');
      ...
      expect(job0?.description).toContain('Gemini');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/gemini/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Gemini');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'gemini-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
