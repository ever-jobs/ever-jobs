...
/**
 * Chicago Trading Company — Options market making (HQ: Chicago, IL, USA).
 *
 * Source (Spec 1737): Greenhouse boards, scraped in this order:
 *   - `chicagotradingcampus` (campus) — https://job-boards.greenhouse.io/chicagotradingcampus
 *     verified live 2026-09-24: 8 open postings.
 *   - `chicagotrading` (lateral) — https://job-boards.greenhouse.io/chicagotrading
 *     verified live 2026-09-24: 25 open postings.
 *
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'chicagotradingcampus', atsIdPrefix: 'gh-' },
  { companySlug: 'chicagotrading', atsIdPrefix: 'gh-' },
];

@SourcePlugin({
  site: Site.CHICAGO_TRADING,
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['chicagotrading.com'],
  description: 'Chicago Trading Company careers via Greenhouse. Tags: segment=quant-trading; industry=options-market-making.',
})
@Injectable()
export class ChicagoTradingService implements IScraper {
  private readonly logger = new Logger(ChicagoTradingService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Greenhouse source plugin is not registered; cannot scrape Chicago Trading Company');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`Chicago Trading Company: delegating to Greenhouse (${board.companySlug})`);
    ...
    this.logger.log(`Chicago Trading Company: scraped ${jobs.length} jobs`);
