...
import { ChicagoTradingModule, ChicagoTradingService } from '../src';
...
const FIXTURE: Fixture = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'fixtures', 'chicagotrading-boards.json'), 'utf8'),
);
const COMPANY_NAME = 'Chicago Trading Company';
const ID_PREFIX = 'chicagotrading-';
...
describe('ChicagoTradingService — Greenhouse delegation (Spec 1737)', () => {
  beforeEach(() => {
    mockGet.mockReset();
    mockPost.mockReset();
  ...
  describe('registration', () => {
    it('resolves through ChicagoTradingModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({ imports: [ChicagoTradingModule] }).compile();
      expect(moduleRef.get(ChicagoTradingService)).toBeInstanceOf(ChicagoTradingService);
      await moduleRef.close();
    });

    it('exports Site.CHICAGO_TRADING = "chicagotrading"', () => {
      expect(Site.CHICAGO_TRADING).toBe('chicagotrading');
    });

    it('declares a tagged company plugin with its domains', () => {
      const meta = Reflect.getMetadata(SOURCE_PLUGIN_METADATA, ChicagoTradingService);
      expect(meta.site).toBe(Site.CHICAGO_TRADING);
      expect(meta.category).toBe('company');
      expect(meta.isAts).toBeFalsy();
      expect(meta.companyDomains).toEqual(['chicagotrading.com']);
      expect(meta.description).toContain('segment=quant-trading');
    });
  });

  describe('recorded boards (real Greenhouse adapter, mocked HTTP)', () => {
    it('maps every recorded posting and re-stamps the company identity', async () => {
      const service = new ChicagoTradingService(registryWith());
      const result = await service.scrape({ siteType: [Site.CHICAGO_TRADING], resultsWanted: 100 } as ScraperInputDto);
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.CHICAGO_TRADING);
    ...
    it('only requests the recorded board URLs', async () => {
      const service = new ChicagoTradingService(registryWith());
      await service.scrape({ siteType: [Site.CHICAGO_TRADING] } as ScraperInputDto);
    ...
    it('honours resultsWanted=1', async () => {
      const service = new ChicagoTradingService(registryWith());
      const result = await service.scrape({ siteType: [Site.CHICAGO_TRADING], resultsWanted: 1 } as ScraperInputDto);
    ...
    it('resolves with an empty result when the board is gone (HTTP 404)', async () => {
      ...
      const service = new ChicagoTradingService(registryWith());
      const result = await service.scrape({ siteType: [Site.CHICAGO_TRADING] } as ScraperInputDto);
  ...
  describe('delegation contract', () => {
    it('forwards the board and the caller input untouched', async () => {
      ...
      const service = new ChicagoTradingService(
        registryWith(
          fakeBackend(
      ...
      expect(result.jobs[0].site).toBe(Site.CHICAGO_TRADING);
      expect(result.jobs[0].companyName).toBe(COMPANY_NAME);
    });

    it('passes an absent resultsWanted through as absent', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new ChicagoTradingService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      await service.scrape({ siteType: [Site.CHICAGO_TRADING] } as ScraperInputDto);
    ...
    it('makes no request when resultsWanted is 0', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new ChicagoTradingService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      const result = await service.scrape({ siteType: [Site.CHICAGO_TRADING], resultsWanted: 0 } as ScraperInputDto);
  ...
  describe('resilience', () => {
    it('reports not_registered when no registry is injected', async () => {
      const result = await new ChicagoTradingService().scrape({ siteType: [Site.CHICAGO_TRADING] } as ScraperInputDto);
      expect(result.jobs).toEqual([]);
    ...
    it('reports not_registered when Greenhouse is missing from the registry', async () => {
      const result = await new ChicagoTradingService(new PluginRegistry()).scrape({
        siteType: [Site.CHICAGO_TRADING],
      } as ScraperInputDto);
      expect(result.diagnostics?.reason).toBe('not_registered');
    });
  ...
  describe('multiple boards', () => {
    it('scrapes the boards in order and spends only the remaining budget', async () => {
      ...
      const result = await service.scrape({ siteType: [Site.CHICAGO_TRADING], resultsWanted: 7 } as ScraperInputDto);
