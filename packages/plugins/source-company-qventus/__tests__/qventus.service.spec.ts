...
import { QventusModule, QventusService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'qventus-jobs.json'), 'utf8'),
);
...
/**
 * Spec 768 / T04 — `QventusService` unit tests (≥ 9 cases).
 */
describe('QventusService — Spec 768 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through QventusModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [QventusModule],
      }).compile();
      const service = moduleRef.get(QventusService);
      expect(service).toBeInstanceOf(QventusService);
      await moduleRef.close();
    });

    it('exports the Site.QVENTUS = "qventus" enum value', () => {
      expect(Site.QVENTUS).toBe('qventus');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new QventusService();
      const result = await service.scrape({
        siteType: [Site.QVENTUS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'qventus-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.QVENTUS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Qventus');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/qventus/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/qventus/jobs/');
      ...
      expect(job0?.description).toContain('Qventus');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/qventus/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Qventus');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'qventus-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
