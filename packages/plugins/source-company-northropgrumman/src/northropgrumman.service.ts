...
/**
 * Northrop Grumman — Aerospace and defense (HQ: Falls Church, VA, USA).
 *
 * Source (Spec 1736): Workday board, scraped in this order:
 *   - `ngc:1:Northrop_Grumman_External_Site` — https://ngc.wd1.myworkdayjobs.com/Northrop_Grumman_External_Site
 *     verified live 2026-09-24: 3,854 open postings.
 *
 ...
 * party board.
 *
 * Tags: segment=workday-enterprise; industry=aerospace-and-defense.
 */
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'ngc:1:Northrop_Grumman_External_Site', atsIdPrefix: 'wd-ngc-' },
];

@SourcePlugin({
  site: Site.NORTHROP_GRUMMAN,
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['northropgrumman.com'],
  description: 'Northrop Grumman careers via Workday. Tags: segment=workday-enterprise; industry=aerospace-and-defense.',
})
@Injectable()
export class NorthropGrummanService implements IScraper {
  private readonly logger = new Logger(NorthropGrummanService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Workday source plugin is not registered; cannot scrape Northrop Grumman');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`Northrop Grumman: delegating to Workday (${board.companySlug})`);
      ...
      for (const job of result.jobs ?? []) {
        job.site = Site.NORTHROP_GRUMMAN;
    ...
    this.logger.log(`Northrop Grumman: scraped ${jobs.length} jobs`);
