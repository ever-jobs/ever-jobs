...
import { NorthropGrummanModule, NorthropGrummanService } from '../src';
...
const FIXTURE: Fixture = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'fixtures', 'northropgrumman-boards.json'), 'utf8'),
);
const COMPANY_NAME = 'Northrop Grumman';
const ID_PREFIX = 'northropgrumman-';
...
describe('NorthropGrummanService — Workday delegation (Spec 1736)', () => {
  beforeEach(() => {
    mockGet.mockReset();
    mockPost.mockReset();
  ...
  describe('registration', () => {
    it('resolves through NorthropGrummanModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({ imports: [NorthropGrummanModule] }).compile();
      expect(moduleRef.get(NorthropGrummanService)).toBeInstanceOf(NorthropGrummanService);
      await moduleRef.close();
    });

    it('exports Site.NORTHROP_GRUMMAN = "northropgrumman"', () => {
      expect(Site.NORTHROP_GRUMMAN).toBe('northropgrumman');
    });

    it('declares a tagged company plugin with its domains', () => {
      const meta = Reflect.getMetadata(SOURCE_PLUGIN_METADATA, NorthropGrummanService);
      expect(meta.site).toBe(Site.NORTHROP_GRUMMAN);
      expect(meta.category).toBe('company');
      expect(meta.isAts).toBeFalsy();
      expect(meta.companyDomains).toEqual(['northropgrumman.com']);
  ...
  describe('recorded board (real Workday adapter, mocked HTTP)', () => {
    it('maps every recorded posting and re-stamps the company identity', async () => {
      const service = new NorthropGrummanService(registryWith());
      const result = await service.scrape({ siteType: [Site.NORTHROP_GRUMMAN], resultsWanted: 100 } as ScraperInputDto);
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.NORTHROP_GRUMMAN);
    ...
    it('only requests the recorded board URLs', async () => {
      const service = new NorthropGrummanService(registryWith());
      await service.scrape({ siteType: [Site.NORTHROP_GRUMMAN] } as ScraperInputDto);
    ...
    it('honours resultsWanted=1', async () => {
      const service = new NorthropGrummanService(registryWith());
      const result = await service.scrape({ siteType: [Site.NORTHROP_GRUMMAN], resultsWanted: 1 } as ScraperInputDto);
    ...
    it('resolves with an empty result when the board is gone (HTTP 404)', async () => {
      ...
      const service = new NorthropGrummanService(registryWith());
      const result = await service.scrape({ siteType: [Site.NORTHROP_GRUMMAN] } as ScraperInputDto);
  ...
  describe('delegation contract', () => {
    it('forwards the board and the caller input untouched', async () => {
      ...
      const service = new NorthropGrummanService(
        registryWith(
          fakeBackend(
      ...
      const result = await service.scrape({
        siteType: [Site.NORTHROP_GRUMMAN],
      ...
      expect(result.jobs[0].site).toBe(Site.NORTHROP_GRUMMAN);
      expect(result.jobs[0].companyName).toBe(COMPANY_NAME);
    });

    it('passes an absent resultsWanted through as absent', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new NorthropGrummanService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      await service.scrape({ siteType: [Site.NORTHROP_GRUMMAN] } as ScraperInputDto);
    ...
    it('makes no request when resultsWanted is 0', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new NorthropGrummanService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      const result = await service.scrape({ siteType: [Site.NORTHROP_GRUMMAN], resultsWanted: 0 } as ScraperInputDto);
  ...
  describe('resilience', () => {
    it('reports not_registered when no registry is injected', async () => {
      const result = await new NorthropGrummanService().scrape({ siteType: [Site.NORTHROP_GRUMMAN] } as ScraperInputDto);
      expect(result.jobs).toEqual([]);
    ...
    it('reports not_registered when Workday is missing from the registry', async () => {
      const result = await new NorthropGrummanService(new PluginRegistry()).scrape({
        siteType: [Site.NORTHROP_GRUMMAN],
      } as ScraperInputDto);
      expect(result.diagnostics?.reason).toBe('not_registered');
    });
