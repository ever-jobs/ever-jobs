...
import { LunarenergyModule, LunarenergyService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'lunarenergy-jobs.json'), 'utf8'),
);
...
/**
 * Spec 562 / T04 — `LunarenergyService` unit tests (≥ 9 cases).
 */
describe('LunarenergyService — Spec 562 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through LunarenergyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [LunarenergyModule],
      }).compile();
      const service = moduleRef.get(LunarenergyService);
      expect(service).toBeInstanceOf(LunarenergyService);
      await moduleRef.close();
    });

    it('exports the Site.LUNAR_ENERGY = "lunarenergy" enum value', () => {
      expect(Site.LUNAR_ENERGY).toBe('lunarenergy');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new LunarenergyService();
      const result = await service.scrape({
        siteType: [Site.LUNAR_ENERGY],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'lunarenergy-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.LUNAR_ENERGY);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Lunar Energy');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/lunarenergy/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/lunarenergy/jobs/');
      ...
      expect(job0?.description).toContain('Lunar Energy');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/lunarenergy/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Lunar Energy');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'lunarenergy-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
