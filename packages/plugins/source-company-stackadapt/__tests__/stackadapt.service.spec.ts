...
import { StackAdaptModule, StackAdaptService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'stackadapt-jobs.json'), 'utf8'),
);
...
/**
 * Spec 942 / T04 — `StackAdaptService` unit tests (≥ 9 cases).
 */
describe('StackAdaptService — Spec 942 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through StackAdaptModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [StackAdaptModule],
      }).compile();
      const service = moduleRef.get(StackAdaptService);
      expect(service).toBeInstanceOf(StackAdaptService);
      await moduleRef.close();
    });

    it('exports the Site.STACKADAPT = "stackadapt" enum value', () => {
      expect(Site.STACKADAPT).toBe('stackadapt');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new StackAdaptService();
      const result = await service.scrape({
        siteType: [Site.STACKADAPT],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'stackadapt-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.STACKADAPT);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('StackAdapt');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/stackadapt/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/stackadapt/jobs/');
      ...
      expect(job0?.description).toContain('StackAdapt');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/stackadapt/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('StackAdapt');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'stackadapt-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
