...
import { RithumModule, RithumService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'rithum-jobs.json'), 'utf8'),
);
...
/**
 * Spec 921 / T04 — `RithumService` unit tests (≥ 9 cases).
 */
describe('RithumService — Spec 921 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through RithumModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [RithumModule],
      }).compile();
      const service = moduleRef.get(RithumService);
      expect(service).toBeInstanceOf(RithumService);
      await moduleRef.close();
    });

    it('exports the Site.RITHUM = "rithum" enum value', () => {
      expect(Site.RITHUM).toBe('rithum');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new RithumService();
      const result = await service.scrape({
        siteType: [Site.RITHUM],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'rithum-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.RITHUM);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Rithum');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/rithum/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/rithum/jobs/');
      ...
      expect(job0?.description).toContain('Rithum');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/rithum/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Rithum');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'rithum-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
