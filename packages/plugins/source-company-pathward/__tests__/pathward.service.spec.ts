...
import { PathwardModule, PathwardService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'pathward-jobs.json'), 'utf8'),
);
...
/**
 * Spec 908 / T04 — `PathwardService` unit tests (≥ 9 cases).
 */
describe('PathwardService — Spec 908 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through PathwardModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [PathwardModule],
      }).compile();
      const service = moduleRef.get(PathwardService);
      expect(service).toBeInstanceOf(PathwardService);
      await moduleRef.close();
    });

    it('exports the Site.PATHWARD = "pathward" enum value', () => {
      expect(Site.PATHWARD).toBe('pathward');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new PathwardService();
      const result = await service.scrape({
        siteType: [Site.PATHWARD],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'pathward-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.PATHWARD);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Pathward');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/pathward/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/pathward/jobs/');
      ...
      expect(job0?.description).toContain('Pathward');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/pathward/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Pathward');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'pathward-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
