...
import { CanvasMedicalModule, CanvasMedicalService } from '../src';

const COMPANY_NAME_EXPECT = 'Canvas Medical';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'canvasmedical-jobs.json'), 'utf8'),
);
...
describe('CanvasMedicalService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CanvasMedicalModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CanvasMedicalModule],
      }).compile();
      const service = moduleRef.get(CanvasMedicalService);
      expect(service).toBeInstanceOf(CanvasMedicalService);
      await moduleRef.close();
    });

    it('exports the Site.CANVAS_MEDICAL = "canvasmedical" enum value', () => {
      expect(Site.CANVAS_MEDICAL).toBe('canvasmedical');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new CanvasMedicalService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.CANVAS_MEDICAL],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'canvasmedical-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.CANVAS_MEDICAL);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('canvasmedical-' + first.id);
      ...
      expect(calledUrls[0]).toContain('canvasmedical');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.CANVAS_MEDICAL);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('canvasmedical-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new CanvasMedicalService(registry);
      const result = await service.scrape({
        siteType: [Site.CANVAS_MEDICAL],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('canvasmedical');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('canvasmedical-x1');
      expect(result.jobs[0].site).toBe(Site.CANVAS_MEDICAL);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('canvasmedical-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new CanvasMedicalService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.CANVAS_MEDICAL],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new CanvasMedicalService();
      const result = await service.scrape({
        siteType: [Site.CANVAS_MEDICAL],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
