...
import { AdvocateconstructionModule, AdvocateconstructionService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'advocateconstruction-jobs.json'), 'utf8'),
);
...
/**
 * Spec 194 / T04 — `AdvocateconstructionService` unit tests (≥ 9 cases).
 */
describe('AdvocateconstructionService — Spec 194 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AdvocateconstructionModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AdvocateconstructionModule],
      }).compile();
      const service = moduleRef.get(AdvocateconstructionService);
      expect(service).toBeInstanceOf(AdvocateconstructionService);
      await moduleRef.close();
    });

    it('exports the Site.ADVOCATECONSTRUCTION = "advocateconstruction" enum value', () => {
      expect(Site.ADVOCATECONSTRUCTION).toBe('advocateconstruction');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AdvocateconstructionService();
      const result = await service.scrape({
        siteType: [Site.ADVOCATECONSTRUCTION],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'advocateconstruction-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ADVOCATECONSTRUCTION);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Advocate Construction');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/advocateconstruction/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/advocateconstruction/jobs/');
      ...
      expect(job0?.description).toContain('Advocate Construction');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/advocateconstruction/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Advocate Construction');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'advocateconstruction-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
