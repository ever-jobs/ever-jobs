...
import { AuguryModule, AuguryService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'augury-jobs.json'), 'utf8'),
);
...
/**
 * Spec 811 / T04 — `AuguryService` unit tests (≥ 9 cases).
 */
describe('AuguryService — Spec 811 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AuguryModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AuguryModule],
      }).compile();
      const service = moduleRef.get(AuguryService);
      expect(service).toBeInstanceOf(AuguryService);
      await moduleRef.close();
    });

    it('exports the Site.AUGURY = "augury" enum value', () => {
      expect(Site.AUGURY).toBe('augury');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AuguryService();
      const result = await service.scrape({
        siteType: [Site.AUGURY],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'augury-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.AUGURY);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Augury');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/augury/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/augury/jobs/');
      ...
      expect(job0?.description).toContain('Augury');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/augury/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Augury');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'augury-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
