...
import { MoonPayModule, MoonPayService } from '../src';

const COMPANY_NAME_EXPECT = 'MoonPay';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'moonpay-jobs.json'), 'utf8'),
);
...
describe('MoonPayService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through MoonPayModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [MoonPayModule],
      }).compile();
      const service = moduleRef.get(MoonPayService);
      expect(service).toBeInstanceOf(MoonPayService);
      await moduleRef.close();
    });

    it('exports the Site.MOONPAY = "moonpay" enum value', () => {
      expect(Site.MOONPAY).toBe('moonpay');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new MoonPayService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.MOONPAY],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('moonpay');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.MOONPAY);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('moonpay-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new MoonPayService(registry);
      const result = await service.scrape({
        siteType: [Site.MOONPAY],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('moonpay');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('moonpay-x1');
      expect(result.jobs[0].site).toBe(Site.MOONPAY);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('moonpay-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new MoonPayService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.MOONPAY],
      } as ScraperInputDto);
