...
import { AppdirectModule, AppdirectService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'appdirect-jobs.json'), 'utf8'),
);
...
/**
 * Spec 290 / T04 — `AppdirectService` unit tests (≥ 9 cases).
 */
describe('AppdirectService — Spec 290 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AppdirectModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AppdirectModule],
      }).compile();
      const service = moduleRef.get(AppdirectService);
      expect(service).toBeInstanceOf(AppdirectService);
      await moduleRef.close();
    });

    it('exports the Site.APPDIRECT = "appdirect" enum value', () => {
      expect(Site.APPDIRECT).toBe('appdirect');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AppdirectService();
      const result = await service.scrape({
        siteType: [Site.APPDIRECT],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'appdirect-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.APPDIRECT);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('AppDirect');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/appdirect/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/appdirect/jobs/');
      ...
      expect(job0?.description).toContain('AppDirect');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/appdirect/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('AppDirect');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'appdirect-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
