...
import { ScalianModule, ScalianService } from '../src';

const COMPANY_NAME_EXPECT = 'Scalian';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'scalian-jobs.json'), 'utf8'),
);
...
describe('ScalianService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ScalianModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ScalianModule],
      }).compile();
      const service = moduleRef.get(ScalianService);
      expect(service).toBeInstanceOf(ScalianService);
      await moduleRef.close();
    });

    it('exports the Site.SCALIAN = "scalian" enum value', () => {
      expect(Site.SCALIAN).toBe('scalian');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new ScalianService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.SCALIAN],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('Scalian');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SCALIAN);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('scalian-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new ScalianService(registry);
      const result = await service.scrape({
        siteType: [Site.SCALIAN],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('Scalian');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('scalian-x1');
      expect(result.jobs[0].site).toBe(Site.SCALIAN);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('scalian-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new ScalianService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.SCALIAN],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new ScalianService();
      const result = await service.scrape({
        siteType: [Site.SCALIAN],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
