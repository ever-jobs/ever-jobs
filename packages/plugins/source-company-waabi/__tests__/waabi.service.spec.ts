...
import { WaabiModule, WaabiService } from '../src';

const COMPANY_NAME_EXPECT = 'Waabi';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'waabi-jobs.json'), 'utf8'),
);
...
describe('WaabiService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through WaabiModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [WaabiModule],
      }).compile();
      const service = moduleRef.get(WaabiService);
      expect(service).toBeInstanceOf(WaabiService);
      await moduleRef.close();
    });

    it('exports the Site.WAABI = "waabi" enum value', () => {
      expect(Site.WAABI).toBe('waabi');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new WaabiService(registryWithLever());
      ...
      expect(calledUrls[0]).toContain('waabi');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.WAABI);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('waabi-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new WaabiService(registry);
      const result = await service.scrape({
        siteType: [Site.WAABI],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('waabi');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('waabi-x1');
      expect(result.jobs[0].site).toBe(Site.WAABI);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('waabi-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new WaabiService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.WAABI],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new WaabiService();
      const result = await service.scrape({
        siteType: [Site.WAABI],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
