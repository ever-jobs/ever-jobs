...
import { AssystemModule, AssystemService } from '../src';

const COMPANY_NAME_EXPECT = 'Assystem';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'assystem-jobs.json'), 'utf8'),
);
...
describe('AssystemService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AssystemModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AssystemModule],
      }).compile();
      const service = moduleRef.get(AssystemService);
      expect(service).toBeInstanceOf(AssystemService);
      await moduleRef.close();
    });

    it('exports the Site.ASSYSTEM = "assystem" enum value', () => {
      expect(Site.ASSYSTEM).toBe('assystem');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new AssystemService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.ASSYSTEM],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('ASSYSTEM');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ASSYSTEM);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('assystem-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new AssystemService(registry);
      const result = await service.scrape({
        siteType: [Site.ASSYSTEM],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('ASSYSTEM');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('assystem-x1');
      expect(result.jobs[0].site).toBe(Site.ASSYSTEM);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('assystem-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new AssystemService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.ASSYSTEM],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new AssystemService();
      const result = await service.scrape({
        siteType: [Site.ASSYSTEM],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
