...
import { CuyanaModule, CuyanaService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'cuyana-jobs.json'), 'utf8'),
);
...
/**
 * Spec 601 / T04 — `CuyanaService` unit tests (≥ 9 cases).
 */
describe('CuyanaService — Spec 601 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CuyanaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CuyanaModule],
      }).compile();
      const service = moduleRef.get(CuyanaService);
      expect(service).toBeInstanceOf(CuyanaService);
      await moduleRef.close();
    });

    it('exports the Site.CUYANA = "cuyana" enum value', () => {
      expect(Site.CUYANA).toBe('cuyana');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new CuyanaService();
      const result = await service.scrape({
        siteType: [Site.CUYANA],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'cuyana-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.CUYANA);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Cuyana');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/cuyana/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/cuyana/jobs/');
      ...
      expect(job0?.description).toContain('Cuyana');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/cuyana/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Cuyana');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'cuyana-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
