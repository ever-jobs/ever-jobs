...
import { DaybreakGameCompanyModule, DaybreakGameCompanyService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'daybreakgames-jobs.json'), 'utf8'),
);
...
/**
 * Spec 832 / T04 — `DaybreakGameCompanyService` unit tests (≥ 9 cases).
 */
describe('DaybreakGameCompanyService — Spec 832 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through DaybreakGameCompanyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [DaybreakGameCompanyModule],
      }).compile();
      const service = moduleRef.get(DaybreakGameCompanyService);
      expect(service).toBeInstanceOf(DaybreakGameCompanyService);
      await moduleRef.close();
    });

    it('exports the Site.DAYBREAK_GAME_COMPANY = "daybreakgames" enum value', () => {
      expect(Site.DAYBREAK_GAME_COMPANY).toBe('daybreakgames');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new DaybreakGameCompanyService();
      const result = await service.scrape({
        siteType: [Site.DAYBREAK_GAME_COMPANY],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'daybreakgames-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.DAYBREAK_GAME_COMPANY);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Daybreak Game Company');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/daybreakgames/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/daybreakgames/jobs/');
      ...
      expect(job0?.description).toContain('Daybreak Game Company');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/daybreakgames/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Daybreak Game Company');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'daybreakgames-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
