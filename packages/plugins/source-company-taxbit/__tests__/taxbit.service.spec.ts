...
import { TaxbitModule, TaxbitService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'taxbit-jobs.json'), 'utf8'),
);
...
/**
 * Spec 535 / T04 — `TaxbitService` unit tests (≥ 9 cases).
 */
describe('TaxbitService — Spec 535 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through TaxbitModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [TaxbitModule],
      }).compile();
      const service = moduleRef.get(TaxbitService);
      expect(service).toBeInstanceOf(TaxbitService);
      await moduleRef.close();
    });

    it('exports the Site.TAXBIT = "taxbit" enum value', () => {
      expect(Site.TAXBIT).toBe('taxbit');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new TaxbitService();
      const result = await service.scrape({
        siteType: [Site.TAXBIT],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'taxbit-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.TAXBIT);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('TaxBit');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/taxbit/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/taxbit/jobs/');
      ...
      expect(job0?.description).toContain('TaxBit');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/taxbit/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('TaxBit');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'taxbit-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
