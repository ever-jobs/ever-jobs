...
import { JeevesModule, JeevesService } from '../src';

const COMPANY_NAME_EXPECT = 'Jeeves';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'tryjeeves-jobs.json'), 'utf8'),
);
...
describe('JeevesService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through JeevesModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [JeevesModule],
      }).compile();
      const service = moduleRef.get(JeevesService);
      expect(service).toBeInstanceOf(JeevesService);
      await moduleRef.close();
    });

    it('exports the Site.JEEVES = "tryjeeves" enum value', () => {
      expect(Site.JEEVES).toBe('tryjeeves');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new JeevesService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.JEEVES],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('tryjeeves');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.JEEVES);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('tryjeeves-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new JeevesService(registry);
      const result = await service.scrape({
        siteType: [Site.JEEVES],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('tryjeeves');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('tryjeeves-x1');
      expect(result.jobs[0].site).toBe(Site.JEEVES);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('tryjeeves-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new JeevesService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.JEEVES],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new JeevesService();
      const result = await service.scrape({
        siteType: [Site.JEEVES],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
