...
import { GuildModule, GuildService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'guild-jobs.json'), 'utf8'),
);
...
/**
 * Spec 575 / T04 — `GuildService` unit tests (≥ 9 cases).
 */
describe('GuildService — Spec 575 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through GuildModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [GuildModule],
      }).compile();
      const service = moduleRef.get(GuildService);
      expect(service).toBeInstanceOf(GuildService);
      await moduleRef.close();
    });

    it('exports the Site.GUILD = "guild" enum value', () => {
      expect(Site.GUILD).toBe('guild');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const job0 = dto.jobs.find((j) => j.id === 'guild-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.GUILD);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Guild');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/guild/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/guild/jobs/');
      ...
      expect(job0?.description).toContain('Guild');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/guild/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Guild');
      }
    });
  });
