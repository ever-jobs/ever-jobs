...
import { RippleModule, RippleService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'ripple-jobs.json'), 'utf8'),
);
...
/**
 * Spec 428 / T04 — `RippleService` unit tests (≥ 9 cases).
 */
describe('RippleService — Spec 428 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through RippleModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [RippleModule],
      }).compile();
      const service = moduleRef.get(RippleService);
      expect(service).toBeInstanceOf(RippleService);
      await moduleRef.close();
    });

    it('exports the Site.RIPPLE = "ripple" enum value', () => {
      expect(Site.RIPPLE).toBe('ripple');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new RippleService();
      const result = await service.scrape({
        siteType: [Site.RIPPLE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'ripple-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.RIPPLE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Ripple');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/ripple/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/ripple/jobs/');
      ...
      expect(job0?.description).toContain('Ripple');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/ripple/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Ripple');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'ripple-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
