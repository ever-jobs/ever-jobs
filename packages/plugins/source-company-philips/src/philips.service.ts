...
/**
 * Philips — Health technology (HQ: Amsterdam, Netherlands).
 *
 * Source (Spec 1736): Workday board, scraped in this order:
 *   - `philips:3:jobs-and-careers` — https://philips.wd3.myworkdayjobs.com/jobs-and-careers
 *     verified live 2026-09-24: 870 open postings.
 *
 ...
 * party board.
 *
 * Tags: segment=workday-enterprise; industry=health-technology.
 */
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'philips:3:jobs-and-careers', atsIdPrefix: 'wd-philips-' },
];
...
@SourcePlugin({
  ...
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['philips.com'],
  description: 'Philips careers via Workday. Tags: segment=workday-enterprise; industry=health-technology.',
})
@Injectable()
export class PhilipsService implements IScraper {
  private readonly logger = new Logger(PhilipsService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Workday source plugin is not registered; cannot scrape Philips');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`Philips: delegating to Workday (${board.companySlug})`);
    ...
    this.logger.log(`Philips: scraped ${jobs.length} jobs`);
