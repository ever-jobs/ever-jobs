...
import { ResendModule, ResendService } from '../src';

const COMPANY_NAME_EXPECT = 'Resend';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'resend-jobs.json'), 'utf8'),
);
...
describe('ResendService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ResendModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ResendModule],
      }).compile();
      const service = moduleRef.get(ResendService);
      expect(service).toBeInstanceOf(ResendService);
      await moduleRef.close();
    });

    it('exports the Site.RESEND = "resend" enum value', () => {
      expect(Site.RESEND).toBe('resend');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new ResendService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.RESEND],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('resend');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.RESEND);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('resend-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new ResendService(registry);
      const result = await service.scrape({
        siteType: [Site.RESEND],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('resend');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('resend-x1');
      expect(result.jobs[0].site).toBe(Site.RESEND);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('resend-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new ResendService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.RESEND],
      } as ScraperInputDto);
