...
import { CohereHealthModule, CohereHealthService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'coherehealth-jobs.json'), 'utf8'),
);
...
/**
 * Spec 774 / T04 — `CohereHealthService` unit tests (≥ 9 cases).
 */
describe('CohereHealthService — Spec 774 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CohereHealthModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CohereHealthModule],
      }).compile();
      const service = moduleRef.get(CohereHealthService);
      expect(service).toBeInstanceOf(CohereHealthService);
      await moduleRef.close();
    });

    it('exports the Site.COHERE_HEALTH = "coherehealth" enum value', () => {
      expect(Site.COHERE_HEALTH).toBe('coherehealth');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new CohereHealthService();
      const result = await service.scrape({
        siteType: [Site.COHERE_HEALTH],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'coherehealth-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.COHERE_HEALTH);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Cohere Health');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/coherehealth/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/coherehealth/jobs/');
      ...
      expect(job0?.description).toContain('Cohere Health');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/coherehealth/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Cohere Health');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'coherehealth-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
