...
import { ContoroRoboticsModule, ContoroRoboticsService } from '../src';

const COMPANY_NAME_EXPECT = 'Contoro Robotics';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'contororobotics-jobs.json'), 'utf8'),
);
...
describe('ContoroRoboticsService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ContoroRoboticsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ContoroRoboticsModule],
      }).compile();
      const service = moduleRef.get(ContoroRoboticsService);
      expect(service).toBeInstanceOf(ContoroRoboticsService);
      await moduleRef.close();
    });

    it('exports the Site.CONTORO_ROBOTICS = "contororobotics" enum value', () => {
      expect(Site.CONTORO_ROBOTICS).toBe('contororobotics');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new ContoroRoboticsService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.CONTORO_ROBOTICS],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'contororobotics-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.CONTORO_ROBOTICS);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('contororobotics-' + first.id);
      ...
      expect(calledUrls[0]).toContain('contoro');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.CONTORO_ROBOTICS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('contororobotics-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new ContoroRoboticsService(registry);
      const result = await service.scrape({
        siteType: [Site.CONTORO_ROBOTICS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('contoro');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('contororobotics-x1');
      expect(result.jobs[0].site).toBe(Site.CONTORO_ROBOTICS);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('contororobotics-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new ContoroRoboticsService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.CONTORO_ROBOTICS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new ContoroRoboticsService();
      const result = await service.scrape({
        siteType: [Site.CONTORO_ROBOTICS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
