...
import { VenusAerospaceModule, VenusAerospaceService } from '../src';

const COMPANY_NAME_EXPECT = 'Venus Aerospace';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'venusaero-jobs.json'), 'utf8'),
);
...
describe('VenusAerospaceService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through VenusAerospaceModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [VenusAerospaceModule],
      }).compile();
      const service = moduleRef.get(VenusAerospaceService);
      expect(service).toBeInstanceOf(VenusAerospaceService);
      await moduleRef.close();
    });

    it('exports the Site.VENUS_AEROSPACE = "venusaero" enum value', () => {
      expect(Site.VENUS_AEROSPACE).toBe('venusaero');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new VenusAerospaceService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.VENUS_AEROSPACE],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'venusaero-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.VENUS_AEROSPACE);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('venusaero-' + first.id);
      ...
      expect(calledUrls[0]).toContain('venusaero');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.VENUS_AEROSPACE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('venusaero-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new VenusAerospaceService(registry);
      const result = await service.scrape({
        siteType: [Site.VENUS_AEROSPACE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('venusaero');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('venusaero-x1');
      expect(result.jobs[0].site).toBe(Site.VENUS_AEROSPACE);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('venusaero-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new VenusAerospaceService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.VENUS_AEROSPACE],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new VenusAerospaceService();
      const result = await service.scrape({
        siteType: [Site.VENUS_AEROSPACE],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
