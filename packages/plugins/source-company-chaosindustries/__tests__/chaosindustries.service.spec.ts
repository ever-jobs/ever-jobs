...
import { CHAOSIndustriesModule, CHAOSIndustriesService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'chaosindustries-jobs.json'), 'utf8'),
);
...
/**
 * Spec 773 / T04 — `CHAOSIndustriesService` unit tests (≥ 9 cases).
 */
describe('CHAOSIndustriesService — Spec 773 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CHAOSIndustriesModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CHAOSIndustriesModule],
      }).compile();
      const service = moduleRef.get(CHAOSIndustriesService);
      expect(service).toBeInstanceOf(CHAOSIndustriesService);
      await moduleRef.close();
    });

    it('exports the Site.CHAOS_INDUSTRIES = "chaosindustries" enum value', () => {
      expect(Site.CHAOS_INDUSTRIES).toBe('chaosindustries');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new CHAOSIndustriesService();
      const result = await service.scrape({
        siteType: [Site.CHAOS_INDUSTRIES],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'chaosindustries-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.CHAOS_INDUSTRIES);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('CHAOS Industries');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/chaosindustries/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/chaosindustries/jobs/');
      ...
      expect(job0?.description).toContain('CHAOS Industries');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/chaosindustries/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('CHAOS Industries');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'chaosindustries-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
