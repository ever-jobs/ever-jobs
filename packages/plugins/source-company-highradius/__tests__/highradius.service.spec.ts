...
import { HighRadiusModule, HighRadiusService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'highradius-jobs.json'), 'utf8'),
);
...
/**
 * Spec 745 / T04 — `HighRadiusService` unit tests (≥ 9 cases).
 */
describe('HighRadiusService — Spec 745 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through HighRadiusModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [HighRadiusModule],
      }).compile();
      const service = moduleRef.get(HighRadiusService);
      expect(service).toBeInstanceOf(HighRadiusService);
      await moduleRef.close();
    });

    it('exports the Site.HIGHRADIUS = "highradius" enum value', () => {
      expect(Site.HIGHRADIUS).toBe('highradius');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new HighRadiusService();
      const result = await service.scrape({
        siteType: [Site.HIGHRADIUS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'highradius-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.HIGHRADIUS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('HighRadius');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/highradius/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/highradius/jobs/');
      ...
      expect(job0?.description).toContain('HighRadius');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/highradius/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('HighRadius');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'highradius-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
