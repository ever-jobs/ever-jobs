...
import { WiseModule, WiseService } from '../src';

const COMPANY_NAME_EXPECT = 'Wise';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'wise-jobs.json'), 'utf8'),
);
...
describe('WiseService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through WiseModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [WiseModule],
      }).compile();
      const service = moduleRef.get(WiseService);
      expect(service).toBeInstanceOf(WiseService);
      await moduleRef.close();
    });

    it('exports the Site.WISE = "wise" enum value', () => {
      expect(Site.WISE).toBe('wise');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new WiseService(registryWithSmartRecruiters());
      ...
      expect(calledUrls[0]).toContain('Wise');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.WISE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('wise-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new WiseService(registry);
      const result = await service.scrape({
        siteType: [Site.WISE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('Wise');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('wise-x1');
      expect(result.jobs[0].site).toBe(Site.WISE);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('wise-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new WiseService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.WISE],
      } as ScraperInputDto);
