...
import { FourkitesModule, FourkitesService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'fourkites-jobs.json'), 'utf8'),
);
...
/**
 * Spec 477 / T04 — `FourkitesService` unit tests (≥ 9 cases).
 */
describe('FourkitesService — Spec 477 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through FourkitesModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [FourkitesModule],
      }).compile();
      const service = moduleRef.get(FourkitesService);
      expect(service).toBeInstanceOf(FourkitesService);
      await moduleRef.close();
    });

    it('exports the Site.FOURKITES = "fourkites" enum value', () => {
      expect(Site.FOURKITES).toBe('fourkites');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new FourkitesService();
      const result = await service.scrape({
        siteType: [Site.FOURKITES],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'fourkites-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.FOURKITES);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('FourKites');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/fourkites/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/fourkites/jobs/');
      ...
      expect(job0?.description).toContain('FourKites');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/fourkites/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('FourKites');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'fourkites-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
