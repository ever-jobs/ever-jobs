...
import { ReadMeModule, ReadMeService } from '../src';

const COMPANY_NAME_EXPECT = 'ReadMe';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'readme-jobs.json'), 'utf8'),
);
...
describe('ReadMeService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ReadMeModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ReadMeModule],
      }).compile();
      const service = moduleRef.get(ReadMeService);
      expect(service).toBeInstanceOf(ReadMeService);
      await moduleRef.close();
    });

    it('exports the Site.README = "readme" enum value', () => {
      expect(Site.README).toBe('readme');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new ReadMeService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.README],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('readme');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.README);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('readme-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new ReadMeService(registry);
      const result = await service.scrape({
        siteType: [Site.README],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('readme');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('readme-x1');
      expect(result.jobs[0].site).toBe(Site.README);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('readme-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new ReadMeService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.README],
      } as ScraperInputDto);
