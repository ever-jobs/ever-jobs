...
import { AmountModule, AmountService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'amount-jobs.json'), 'utf8'),
);
...
/**
 * Spec 263 / T04 — `AmountService` unit tests (≥ 9 cases).
 */
describe('AmountService — Spec 263 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AmountModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AmountModule],
      }).compile();
      const service = moduleRef.get(AmountService);
      expect(service).toBeInstanceOf(AmountService);
      await moduleRef.close();
    });

    it('exports the Site.AMOUNT = "amount" enum value', () => {
      expect(Site.AMOUNT).toBe('amount');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const job0 = dto.jobs.find((j) => j.id === 'amount-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.AMOUNT);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('FIS® Amount™');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/amount/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/amount/jobs/');
      ...
      expect(job0?.description).toContain('FIS® Amount™');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/amount/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('FIS® Amount™');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'amount-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
