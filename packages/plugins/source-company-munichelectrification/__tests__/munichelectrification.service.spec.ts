...
import { MunichElectrificationModule, MunichElectrificationService } from '../src';

const COMPANY_NAME_EXPECT = 'Munich Electrification';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'munichelectrification-jobs.json'), 'utf8'),
);
...
describe('MunichElectrificationService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through MunichElectrificationModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [MunichElectrificationModule],
      }).compile();
      const service = moduleRef.get(MunichElectrificationService);
      expect(service).toBeInstanceOf(MunichElectrificationService);
      await moduleRef.close();
    });

    it('exports the Site.MUNICH_ELECTRIFICATION = "munichelectrification" enum value', () => {
      expect(Site.MUNICH_ELECTRIFICATION).toBe('munichelectrification');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new MunichElectrificationService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.MUNICH_ELECTRIFICATION],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'munichelectrification-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.MUNICH_ELECTRIFICATION);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('munichelectrification-' + first.id);
      ...
      expect(calledUrls[0]).toContain('munich-electrification');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.MUNICH_ELECTRIFICATION);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('munichelectrification-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new MunichElectrificationService(registry);
      const result = await service.scrape({
        siteType: [Site.MUNICH_ELECTRIFICATION],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('munich-electrification');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('munichelectrification-x1');
      expect(result.jobs[0].site).toBe(Site.MUNICH_ELECTRIFICATION);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('munichelectrification-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new MunichElectrificationService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.MUNICH_ELECTRIFICATION],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new MunichElectrificationService();
      const result = await service.scrape({
        siteType: [Site.MUNICH_ELECTRIFICATION],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
