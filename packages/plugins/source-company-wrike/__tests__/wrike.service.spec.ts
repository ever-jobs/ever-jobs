...
import { WrikeModule, WrikeService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'wrike-jobs.json'), 'utf8'),
);
...
/**
 * Spec 441 / T04 — `WrikeService` unit tests (≥ 9 cases).
 */
describe('WrikeService — Spec 441 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through WrikeModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [WrikeModule],
      }).compile();
      const service = moduleRef.get(WrikeService);
      expect(service).toBeInstanceOf(WrikeService);
      await moduleRef.close();
    });

    it('exports the Site.WRIKE = "wrike" enum value', () => {
      expect(Site.WRIKE).toBe('wrike');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new WrikeService();
      ...
      const job0 = dto.jobs.find((j) => j.id === 'wrike-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.WRIKE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Wrike');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/wrike/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/wrike/jobs/');
      ...
      expect(job0?.description).toContain('Wrike');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/wrike/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Wrike');
      }
    });
  });
