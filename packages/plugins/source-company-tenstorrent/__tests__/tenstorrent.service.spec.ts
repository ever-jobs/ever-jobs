...
import { TenstorrentModule, TenstorrentService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'tenstorrent-jobs.json'), 'utf8'),
);
...
/**
 * Spec 460 / T04 — `TenstorrentService` unit tests (≥ 9 cases).
 */
describe('TenstorrentService — Spec 460 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through TenstorrentModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [TenstorrentModule],
      }).compile();
      const service = moduleRef.get(TenstorrentService);
      expect(service).toBeInstanceOf(TenstorrentService);
      await moduleRef.close();
    });

    it('exports the Site.TENSTORRENT = "tenstorrent" enum value', () => {
      expect(Site.TENSTORRENT).toBe('tenstorrent');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new TenstorrentService();
      const result = await service.scrape({
        siteType: [Site.TENSTORRENT],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'tenstorrent-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.TENSTORRENT);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Tenstorrent');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/tenstorrent/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/tenstorrent/jobs/');
      ...
      expect(job0?.description).toContain('Tenstorrent');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/tenstorrent/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Tenstorrent');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'tenstorrent-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
