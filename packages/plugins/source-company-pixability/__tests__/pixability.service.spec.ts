...
import { PixabilityModule, PixabilityService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'pixability-jobs.json'), 'utf8'),
);
...
/**
 * Spec 911 / T04 — `PixabilityService` unit tests (≥ 9 cases).
 */
describe('PixabilityService — Spec 911 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through PixabilityModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [PixabilityModule],
      }).compile();
      const service = moduleRef.get(PixabilityService);
      expect(service).toBeInstanceOf(PixabilityService);
      await moduleRef.close();
    });

    it('exports the Site.PIXABILITY = "pixability" enum value', () => {
      expect(Site.PIXABILITY).toBe('pixability');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new PixabilityService();
      const result = await service.scrape({
        siteType: [Site.PIXABILITY],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'pixability-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.PIXABILITY);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Pixability');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/pixability/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/pixability/jobs/');
      ...
      expect(job0?.description).toContain('Pixability');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/pixability/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Pixability');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'pixability-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
