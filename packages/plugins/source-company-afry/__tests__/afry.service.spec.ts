...
import { AFRYModule, AFRYService } from '../src';

const COMPANY_NAME_EXPECT = 'AFRY';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'afry-jobs.json'), 'utf8'),
);
...
describe('AFRYService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AFRYModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AFRYModule],
      }).compile();
      const service = moduleRef.get(AFRYService);
      expect(service).toBeInstanceOf(AFRYService);
      await moduleRef.close();
    });

    it('exports the Site.AFRY = "afry" enum value', () => {
      expect(Site.AFRY).toBe('afry');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new AFRYService(registryWithSmartRecruiters());
      ...
      expect(calledUrls[0]).toContain('AFRY');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.AFRY);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('afry-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new AFRYService(registry);
      const result = await service.scrape({
        siteType: [Site.AFRY],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('AFRY');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('afry-x1');
      expect(result.jobs[0].site).toBe(Site.AFRY);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('afry-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new AFRYService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.AFRY],
      } as ScraperInputDto);
