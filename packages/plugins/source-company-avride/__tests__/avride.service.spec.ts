...
import { AvrideModule, AvrideService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'avride-jobs.json'), 'utf8'),
);
...
/**
 * Spec 814 / T04 — `AvrideService` unit tests (≥ 9 cases).
 */
describe('AvrideService — Spec 814 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AvrideModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AvrideModule],
      }).compile();
      const service = moduleRef.get(AvrideService);
      expect(service).toBeInstanceOf(AvrideService);
      await moduleRef.close();
    });

    it('exports the Site.AVRIDE = "avride" enum value', () => {
      expect(Site.AVRIDE).toBe('avride');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AvrideService();
      const result = await service.scrape({
        siteType: [Site.AVRIDE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'avride-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.AVRIDE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Avride');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/avride/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/avride/jobs/');
      ...
      expect(job0?.description).toContain('Avride');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/avride/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Avride');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'avride-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
