...
import { AECOMModule, AECOMService } from '../src';

const COMPANY_NAME_EXPECT = 'AECOM';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'aecom-jobs.json'), 'utf8'),
);
...
describe('AECOMService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AECOMModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AECOMModule],
      }).compile();
      const service = moduleRef.get(AECOMService);
      expect(service).toBeInstanceOf(AECOMService);
      await moduleRef.close();
    });

    it('exports the Site.AECOM = "aecom" enum value', () => {
      expect(Site.AECOM).toBe('aecom');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new AECOMService(registryWithSmartRecruiters());
      ...
      expect(calledUrls[0]).toContain('AECOM2');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.AECOM);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('aecom-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new AECOMService(registry);
      const result = await service.scrape({
        siteType: [Site.AECOM],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('AECOM2');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('aecom-x1');
      expect(result.jobs[0].site).toBe(Site.AECOM);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('aecom-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new AECOMService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.AECOM],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new AECOMService();
      const result = await service.scrape({
        siteType: [Site.AECOM],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
