...
import { QantasGroupModule, QantasGroupService } from '../src';

const COMPANY_NAME_EXPECT = 'Qantas Group';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'qantasgroup-jobs.json'), 'utf8'),
);
...
describe('QantasGroupService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through QantasGroupModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [QantasGroupModule],
      }).compile();
      const service = moduleRef.get(QantasGroupService);
      expect(service).toBeInstanceOf(QantasGroupService);
      await moduleRef.close();
    });

    it('exports the Site.QANTAS_GROUP = "qantasgroup" enum value', () => {
      expect(Site.QANTAS_GROUP).toBe('qantasgroup');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new QantasGroupService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.QANTAS_GROUP],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('QantasGroup');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.QANTAS_GROUP);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('qantasgroup-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new QantasGroupService(registry);
      const result = await service.scrape({
        siteType: [Site.QANTAS_GROUP],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('QantasGroup');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('qantasgroup-x1');
      expect(result.jobs[0].site).toBe(Site.QANTAS_GROUP);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('qantasgroup-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new QantasGroupService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.QANTAS_GROUP],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new QantasGroupService();
      const result = await service.scrape({
        siteType: [Site.QANTAS_GROUP],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
