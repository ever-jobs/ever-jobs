...
import { SilaNanotechnologiesModule, SilaNanotechnologiesService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'silananotechnologies-jobs.json'), 'utf8'),
);
...
/**
 * Spec 769 / T04 — `SilaNanotechnologiesService` unit tests (≥ 9 cases).
 */
describe('SilaNanotechnologiesService — Spec 769 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SilaNanotechnologiesModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SilaNanotechnologiesModule],
      }).compile();
      const service = moduleRef.get(SilaNanotechnologiesService);
      expect(service).toBeInstanceOf(SilaNanotechnologiesService);
      await moduleRef.close();
    });

    it('exports the Site.SILA_NANOTECHNOLOGIES = "silananotechnologies" enum value', () => {
      expect(Site.SILA_NANOTECHNOLOGIES).toBe('silananotechnologies');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new SilaNanotechnologiesService();
      const result = await service.scrape({
        siteType: [Site.SILA_NANOTECHNOLOGIES],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'silananotechnologies-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.SILA_NANOTECHNOLOGIES);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Sila Nanotechnologies');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/silananotechnologies/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/silananotechnologies/jobs/');
      ...
      expect(job0?.description).toContain('Sila Nanotechnologies');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/silananotechnologies/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Sila Nanotechnologies');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'silananotechnologies-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
