...
import { LVMHPerfumesCosmeticsModule, LVMHPerfumesCosmeticsService } from '../src';

const COMPANY_NAME_EXPECT = 'LVMH Perfumes & Cosmetics';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'lvmhperfumescosmetics-jobs.json'), 'utf8'),
);
...
describe('LVMHPerfumesCosmeticsService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through LVMHPerfumesCosmeticsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [LVMHPerfumesCosmeticsModule],
      }).compile();
      const service = moduleRef.get(LVMHPerfumesCosmeticsService);
      expect(service).toBeInstanceOf(LVMHPerfumesCosmeticsService);
      await moduleRef.close();
    });

    it('exports the Site.LVMH_PERFUMES_COSMETICS = "lvmhperfumescosmetics" enum value', () => {
      expect(Site.LVMH_PERFUMES_COSMETICS).toBe('lvmhperfumescosmetics');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new LVMHPerfumesCosmeticsService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.LVMH_PERFUMES_COSMETICS],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'lvmhperfumescosmetics-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.LVMH_PERFUMES_COSMETICS);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('lvmhperfumescosmetics-' + first.id);
      ...
      expect(calledUrls[0]).toContain('lvmhperfumescosmetics');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.LVMH_PERFUMES_COSMETICS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('lvmhperfumescosmetics-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new LVMHPerfumesCosmeticsService(registry);
      const result = await service.scrape({
        siteType: [Site.LVMH_PERFUMES_COSMETICS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('lvmhperfumescosmetics');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('lvmhperfumescosmetics-x1');
      expect(result.jobs[0].site).toBe(Site.LVMH_PERFUMES_COSMETICS);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('lvmhperfumescosmetics-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new LVMHPerfumesCosmeticsService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.LVMH_PERFUMES_COSMETICS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new LVMHPerfumesCosmeticsService();
      const result = await service.scrape({
        siteType: [Site.LVMH_PERFUMES_COSMETICS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
