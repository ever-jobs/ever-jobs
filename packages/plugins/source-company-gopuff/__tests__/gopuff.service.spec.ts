...
import { GopuffModule, GopuffService } from '../src';

const COMPANY_NAME_EXPECT = 'Gopuff';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'gopuff-jobs.json'), 'utf8'),
);
...
describe('GopuffService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through GopuffModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [GopuffModule],
      }).compile();
      const service = moduleRef.get(GopuffService);
      expect(service).toBeInstanceOf(GopuffService);
      await moduleRef.close();
    });

    it('exports the Site.GOPUFF = "gopuff" enum value', () => {
      expect(Site.GOPUFF).toBe('gopuff');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new GopuffService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.GOPUFF],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('gopuff');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.GOPUFF);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('gopuff-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new GopuffService(registry);
      const result = await service.scrape({
        siteType: [Site.GOPUFF],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('gopuff');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('gopuff-x1');
      expect(result.jobs[0].site).toBe(Site.GOPUFF);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('gopuff-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new GopuffService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.GOPUFF],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new GopuffService();
      const result = await service.scrape({
        siteType: [Site.GOPUFF],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
