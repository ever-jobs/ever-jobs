...
import { TalkspaceModule, TalkspaceService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'talkspace-jobs.json'), 'utf8'),
);
...
/**
 * Spec 526 / T04 — `TalkspaceService` unit tests (≥ 9 cases).
 */
describe('TalkspaceService — Spec 526 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through TalkspaceModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [TalkspaceModule],
      }).compile();
      const service = moduleRef.get(TalkspaceService);
      expect(service).toBeInstanceOf(TalkspaceService);
      await moduleRef.close();
    });

    it('exports the Site.TALKSPACE = "talkspace" enum value', () => {
      expect(Site.TALKSPACE).toBe('talkspace');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new TalkspaceService();
      const result = await service.scrape({
        siteType: [Site.TALKSPACE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'talkspace-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.TALKSPACE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Talkspace');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/talkspace/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/talkspace/jobs/');
      ...
      expect(job0?.description).toContain('Talkspace');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/talkspace/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Talkspace');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'talkspace-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
