...
import { NagarroModule, NagarroService } from '../src';

const COMPANY_NAME_EXPECT = 'Nagarro';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'nagarro-jobs.json'), 'utf8'),
);
...
describe('NagarroService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through NagarroModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [NagarroModule],
      }).compile();
      const service = moduleRef.get(NagarroService);
      expect(service).toBeInstanceOf(NagarroService);
      await moduleRef.close();
    });

    it('exports the Site.NAGARRO = "nagarro" enum value', () => {
      expect(Site.NAGARRO).toBe('nagarro');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new NagarroService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.NAGARRO],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('Nagarro1');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.NAGARRO);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('nagarro-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new NagarroService(registry);
      const result = await service.scrape({
        siteType: [Site.NAGARRO],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('Nagarro1');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('nagarro-x1');
      expect(result.jobs[0].site).toBe(Site.NAGARRO);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('nagarro-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new NagarroService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.NAGARRO],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new NagarroService();
      const result = await service.scrape({
        siteType: [Site.NAGARRO],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
