...
import { QphoXModule, QphoXService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'qphox-jobs.json'), 'utf8'),
);
...
/**
 * Spec 730 / T04 — `QphoXService` unit tests (≥ 9 cases).
 */
describe('QphoXService — Spec 730 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through QphoXModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [QphoXModule],
      }).compile();
      const service = moduleRef.get(QphoXService);
      expect(service).toBeInstanceOf(QphoXService);
      await moduleRef.close();
    });

    it('exports the Site.QPHOX = "qphox" enum value', () => {
      expect(Site.QPHOX).toBe('qphox');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new QphoXService();
      ...
      const job0 = dto.jobs.find((j) => j.id === 'qphox-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.QPHOX);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('QphoX');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/qphox/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/qphox/jobs/');
      ...
      expect(job0?.description).toContain('QphoX');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/qphox/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('QphoX');
      }
    });
  });
