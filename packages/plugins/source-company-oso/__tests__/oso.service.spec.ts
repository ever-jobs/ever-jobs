...
import { OsoModule, OsoService } from '../src';

const COMPANY_NAME_EXPECT = 'Oso';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'oso-jobs.json'), 'utf8'),
);
...
describe('OsoService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through OsoModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [OsoModule],
      }).compile();
      const service = moduleRef.get(OsoService);
      expect(service).toBeInstanceOf(OsoService);
      await moduleRef.close();
    });

    it('exports the Site.OSO = "oso" enum value', () => {
      expect(Site.OSO).toBe('oso');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new OsoService(registryWithAshby());
      ...
      expect(job0?.site).toBe(Site.OSO);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('oso-' + first.id);
      ...
      expect(calledUrls[0]).toContain('oso');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.OSO);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('oso-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new OsoService(registry);
      const result = await service.scrape({
        siteType: [Site.OSO],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('oso');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('oso-x1');
      expect(result.jobs[0].site).toBe(Site.OSO);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('oso-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new OsoService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.OSO],
      } as ScraperInputDto);
