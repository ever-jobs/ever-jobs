...
import { MobilityPlusModule, MobilityPlusService } from '../src';

const COMPANY_NAME_EXPECT = 'MobilityPlus';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'mobilityplus-jobs.json'), 'utf8'),
);
...
describe('MobilityPlusService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through MobilityPlusModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [MobilityPlusModule],
      }).compile();
      const service = moduleRef.get(MobilityPlusService);
      expect(service).toBeInstanceOf(MobilityPlusService);
      await moduleRef.close();
    });

    it('exports the Site.MOBILITYPLUS = "mobilityplus" enum value', () => {
      expect(Site.MOBILITYPLUS).toBe('mobilityplus');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new MobilityPlusService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.MOBILITYPLUS],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'mobilityplus-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Recruitee's defaults
      expect(job0?.site).toBe(Site.MOBILITYPLUS);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('mobilityplus-' + first.id);
      ...
      expect(calledUrls[0]).toContain('mobilityplus.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.MOBILITYPLUS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('mobilityplus-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new MobilityPlusService(registry);
      const result = await service.scrape({
        siteType: [Site.MOBILITYPLUS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('mobilityplus');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('mobilityplus-x1');
      expect(result.jobs[0].site).toBe(Site.MOBILITYPLUS);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('mobilityplus-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new MobilityPlusService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.MOBILITYPLUS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new MobilityPlusService();
      const result = await service.scrape({
        siteType: [Site.MOBILITYPLUS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
