...
import { GatikAIModule, GatikAIService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'gatikaiinc-jobs.json'), 'utf8'),
);
...
/**
 * Spec 855 / T04 — `GatikAIService` unit tests (≥ 9 cases).
 */
describe('GatikAIService — Spec 855 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through GatikAIModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [GatikAIModule],
      }).compile();
      const service = moduleRef.get(GatikAIService);
      expect(service).toBeInstanceOf(GatikAIService);
      await moduleRef.close();
    });

    it('exports the Site.GATIK_AI = "gatikaiinc" enum value', () => {
      expect(Site.GATIK_AI).toBe('gatikaiinc');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new GatikAIService();
      const result = await service.scrape({
        siteType: [Site.GATIK_AI],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'gatikaiinc-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.GATIK_AI);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Gatik AI');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/gatikaiinc/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/gatikaiinc/jobs/');
      ...
      expect(job0?.description).toContain('Gatik AI');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/gatikaiinc/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Gatik AI');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'gatikaiinc-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
