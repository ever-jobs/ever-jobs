...
import { ImbueModule, ImbueService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'imbue-jobs.json'), 'utf8'),
);
...
/**
 * Spec 565 / T04 — `ImbueService` unit tests (≥ 9 cases).
 */
describe('ImbueService — Spec 565 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ImbueModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ImbueModule],
      }).compile();
      const service = moduleRef.get(ImbueService);
      expect(service).toBeInstanceOf(ImbueService);
      await moduleRef.close();
    });

    it('exports the Site.IMBUE = "imbue" enum value', () => {
      expect(Site.IMBUE).toBe('imbue');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ImbueService();
      ...
      const job0 = dto.jobs.find((j) => j.id === 'imbue-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.IMBUE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Imbue');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/imbue/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/imbue/jobs/');
      ...
      expect(job0?.description).toContain('Imbue');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/imbue/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Imbue');
      }
    });
  });
