...
import { SmartRentModule, SmartRentService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'smartrent-jobs.json'), 'utf8'),
);
...
/**
 * Spec 934 / T04 — `SmartRentService` unit tests (≥ 9 cases).
 */
describe('SmartRentService — Spec 934 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SmartRentModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SmartRentModule],
      }).compile();
      const service = moduleRef.get(SmartRentService);
      expect(service).toBeInstanceOf(SmartRentService);
      await moduleRef.close();
    });

    it('exports the Site.SMARTRENT = "smartrent" enum value', () => {
      expect(Site.SMARTRENT).toBe('smartrent');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new SmartRentService();
      const result = await service.scrape({
        siteType: [Site.SMARTRENT],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'smartrent-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.SMARTRENT);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('SmartRent');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/smartrent/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/smartrent/jobs/');
      ...
      expect(job0?.description).toContain('SmartRent');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/smartrent/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('SmartRent');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'smartrent-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
