...
import { AmperityModule, AmperityService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'amperity-jobs.json'), 'utf8'),
);
...
/**
 * Spec 696 / T04 — `AmperityService` unit tests (≥ 9 cases).
 */
describe('AmperityService — Spec 696 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AmperityModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AmperityModule],
      }).compile();
      const service = moduleRef.get(AmperityService);
      expect(service).toBeInstanceOf(AmperityService);
      await moduleRef.close();
    });

    it('exports the Site.AMPERITY = "amperity" enum value', () => {
      expect(Site.AMPERITY).toBe('amperity');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AmperityService();
      const result = await service.scrape({
        siteType: [Site.AMPERITY],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'amperity-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.AMPERITY);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Amperity');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/amperity/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/amperity/jobs/');
      ...
      expect(job0?.description).toContain('Amperity');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/amperity/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Amperity');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'amperity-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
