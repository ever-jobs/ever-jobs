...
/**
 * The Voleon Group — Machine-learning investment management (HQ: Berkeley, CA, USA).
 *
 * Source (Spec 1737): Ashby board, scraped in this order:
 *   - `voleon` — https://jobs.ashbyhq.com/voleon
 *     verified live 2026-09-24: 55 open postings.
 *
 * The plugin re-implements no parsing. It resolves the registered Ashby
 ...
 * companyName, id prefix) so every Ashby field fix is inherited and no
 * plugin imports a peer. The search term and every other caller input pass
 * through untouched.
 *
 * Tags: segment=quant-trading; industry=machine-learning-investment-management.
 */
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'voleon', atsIdPrefix: 'ashby-' },
];
...
@SourcePlugin({
  ...
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['voleon.com'],
  description: 'The Voleon Group careers via Ashby. Tags: segment=quant-trading; industry=machine-learning-investment-management.',
})
@Injectable()
export class VoleonService implements IScraper {
  private readonly logger = new Logger(VoleonService.name);

  constructor(@Optional() private readonly registry?: PluginRegistry) {}

  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    const backend = this.registry?.getScraper(Site.ASHBY);
    if (!backend) {
      this.logger.error('Ashby source plugin is not registered; cannot scrape The Voleon Group');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`The Voleon Group: delegating to Ashby (${board.companySlug})`);
    ...
    this.logger.log(`The Voleon Group: scraped ${jobs.length} jobs`);
