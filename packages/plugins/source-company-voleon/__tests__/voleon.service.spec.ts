...
import { VoleonModule, VoleonService } from '../src';
...
const FIXTURE: Fixture = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'fixtures', 'voleon-boards.json'), 'utf8'),
);
const COMPANY_NAME = 'The Voleon Group';
const ID_PREFIX = 'voleon-';
...
function registryWith(scraper: IScraper = new AshbyService()): PluginRegistry {
  const registry = new PluginRegistry();
  registry.register({ site: Site.ASHBY, name: 'Ashby', category: 'ats', isAts: true }, scraper);
  return registry;
}
...
describe('VoleonService — Ashby delegation (Spec 1737)', () => {
  beforeEach(() => {
    mockGet.mockReset();
    mockPost.mockReset();
  ...
  describe('registration', () => {
    it('resolves through VoleonModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({ imports: [VoleonModule] }).compile();
      expect(moduleRef.get(VoleonService)).toBeInstanceOf(VoleonService);
      await moduleRef.close();
    });

    it('exports Site.VOLEON = "voleon"', () => {
      expect(Site.VOLEON).toBe('voleon');
    });

    it('declares a tagged company plugin with its domains', () => {
      const meta = Reflect.getMetadata(SOURCE_PLUGIN_METADATA, VoleonService);
      expect(meta.site).toBe(Site.VOLEON);
      expect(meta.category).toBe('company');
      expect(meta.isAts).toBeFalsy();
      expect(meta.companyDomains).toEqual(['voleon.com']);
      expect(meta.description).toContain('segment=quant-trading');
    });
  });

  describe('recorded board (real Ashby adapter, mocked HTTP)', () => {
    it('maps every recorded posting and re-stamps the company identity', async () => {
      const service = new VoleonService(registryWith());
      const result = await service.scrape({ siteType: [Site.VOLEON], resultsWanted: 100 } as ScraperInputDto);
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.VOLEON);
    ...
    it('only requests the recorded board URLs', async () => {
      const service = new VoleonService(registryWith());
      await service.scrape({ siteType: [Site.VOLEON] } as ScraperInputDto);
    ...
    it('honours resultsWanted=1', async () => {
      const service = new VoleonService(registryWith());
      const result = await service.scrape({ siteType: [Site.VOLEON], resultsWanted: 1 } as ScraperInputDto);
    ...
    it('resolves with an empty result when the board is gone (HTTP 404)', async () => {
      ...
      const service = new VoleonService(registryWith());
      const result = await service.scrape({ siteType: [Site.VOLEON] } as ScraperInputDto);
  ...
  describe('delegation contract', () => {
    it('forwards the board and the caller input untouched', async () => {
      ...
      const service = new VoleonService(
        registryWith(
          fakeBackend(
      ...
      expect(result.jobs[0].site).toBe(Site.VOLEON);
    ...
    it('makes no request when resultsWanted is 0', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new VoleonService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      const result = await service.scrape({ siteType: [Site.VOLEON], resultsWanted: 0 } as ScraperInputDto);
  ...
  describe('resilience', () => {
    it('reports not_registered when no registry is injected', async () => {
      const result = await new VoleonService().scrape({ siteType: [Site.VOLEON] } as ScraperInputDto);
      expect(result.jobs).toEqual([]);
      expect(result.diagnostics?.reason).toBe('not_registered');
      expect(result.diagnostics?.detail).toContain('Ashby');
    });

    it('reports not_registered when Ashby is missing from the registry', async () => {
      const result = await new VoleonService(new PluginRegistry()).scrape({
        siteType: [Site.VOLEON],
      } as ScraperInputDto);
      expect(result.diagnostics?.reason).toBe('not_registered');
    });
