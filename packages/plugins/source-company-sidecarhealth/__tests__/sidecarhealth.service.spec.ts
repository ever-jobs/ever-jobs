...
import { SidecarHealthModule, SidecarHealthService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'sidecarhealth-jobs.json'), 'utf8'),
);
...
/**
 * Spec 672 / T04 — `SidecarHealthService` unit tests (≥ 9 cases).
 */
describe('SidecarHealthService — Spec 672 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SidecarHealthModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SidecarHealthModule],
      }).compile();
      const service = moduleRef.get(SidecarHealthService);
      expect(service).toBeInstanceOf(SidecarHealthService);
      await moduleRef.close();
    });

    it('exports the Site.SIDECAR_HEALTH = "sidecarhealth" enum value', () => {
      expect(Site.SIDECAR_HEALTH).toBe('sidecarhealth');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new SidecarHealthService();
      const result = await service.scrape({
        siteType: [Site.SIDECAR_HEALTH],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'sidecarhealth-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.SIDECAR_HEALTH);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Sidecar Health');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/sidecarhealth/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/sidecarhealth/jobs/');
      ...
      expect(job0?.description).toContain('Sidecar Health');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/sidecarhealth/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Sidecar Health');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'sidecarhealth-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
