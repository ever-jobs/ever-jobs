...
import { SailorHealthModule, SailorHealthService } from '../src';

const COMPANY_NAME_EXPECT = 'Sailor Health';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'sailorhealth-jobs.json'), 'utf8'),
);
...
describe('SailorHealthService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SailorHealthModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SailorHealthModule],
      }).compile();
      const service = moduleRef.get(SailorHealthService);
      expect(service).toBeInstanceOf(SailorHealthService);
      await moduleRef.close();
    });

    it('exports the Site.SAILOR_HEALTH = "sailorhealth" enum value', () => {
      expect(Site.SAILOR_HEALTH).toBe('sailorhealth');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new SailorHealthService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.SAILOR_HEALTH],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'sailorhealth-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.SAILOR_HEALTH);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('sailorhealth-' + first.id);
      ...
      expect(calledUrls[0]).toContain('sailorhealth');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SAILOR_HEALTH);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('sailorhealth-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new SailorHealthService(registry);
      const result = await service.scrape({
        siteType: [Site.SAILOR_HEALTH],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('sailorhealth');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('sailorhealth-x1');
      expect(result.jobs[0].site).toBe(Site.SAILOR_HEALTH);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('sailorhealth-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new SailorHealthService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.SAILOR_HEALTH],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new SailorHealthService();
      const result = await service.scrape({
        siteType: [Site.SAILOR_HEALTH],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
