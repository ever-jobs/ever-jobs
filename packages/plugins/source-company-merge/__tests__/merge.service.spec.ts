...
import { MergeModule, MergeService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'merge-jobs.json'), 'utf8'),
);
...
/**
 * Spec 471 / T04 — `MergeService` unit tests (≥ 9 cases).
 */
describe('MergeService — Spec 471 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through MergeModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [MergeModule],
      }).compile();
      const service = moduleRef.get(MergeService);
      expect(service).toBeInstanceOf(MergeService);
      await moduleRef.close();
    });

    it('exports the Site.MERGE = "merge" enum value', () => {
      expect(Site.MERGE).toBe('merge');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const job0 = dto.jobs.find((j) => j.id === 'merge-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.MERGE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Merge');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/merge/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/merge/jobs/');
      ...
      expect(job0?.description).toContain('Merge');
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Merge');
      }
    });
  });
