...
/**
 * Merge — Single unified API for HR, payroll, accounting and CRM integrations..
 *
 ...
 *
 * Highlights:
 *   - One unified API for integrations
 *   - Hundreds of supported platforms
 *   - Normalized data models
 *
 * Source profile (Spec 471):
 *   - D-04 — Greenhouse canonical hosted-board host (variant 2):
 *     `https://job-boards.greenhouse.io/merge/jobs/<id>`.
 *   - D-08 — entity-decode-then-tag-strip description pipeline.
 *   - D-09 — wire `company_name` pass-through (`'Merge'`).
 ...
 */
const API_URL = 'https://api.greenhouse.io/v1/boards/merge/jobs';
...
export class MergeService implements IScraper {
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    try {
      ...
      this.logger.log(`Merge: fetching ${url}`);
      ...
      for (const listing of listings) {
        ...
        jobs.push(
          new JobPostDto({
            ...
            companyName: listing.company_name ?? 'Merge',
            // D-04: wire `absolute_url` flows through (variant 2).
            jobUrl:
              listing.absolute_url ??
              `https://job-boards.greenhouse.io/merge/jobs/${listing.id}`,
            location,
      ...
      this.logger.log(`Merge: scraped ${jobs.length} jobs`);
    } catch (err: any) {
      this.logger.error(`Merge scrape failed: ${err.message}`);
