...
import { InstituteOfFoundationModelsModule, InstituteOfFoundationModelsService } from '../src';

const COMPANY_NAME_EXPECT = 'Institute of Foundation Models';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'ifmus-jobs.json'), 'utf8'),
);
...
describe('InstituteOfFoundationModelsService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through InstituteOfFoundationModelsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [InstituteOfFoundationModelsModule],
      }).compile();
      const service = moduleRef.get(InstituteOfFoundationModelsService);
      expect(service).toBeInstanceOf(InstituteOfFoundationModelsService);
      await moduleRef.close();
    });

    it('exports the Site.INSTITUTE_OF_FOUNDATION_MODELS = "ifmus" enum value', () => {
      expect(Site.INSTITUTE_OF_FOUNDATION_MODELS).toBe('ifmus');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new InstituteOfFoundationModelsService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.INSTITUTE_OF_FOUNDATION_MODELS],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'ifmus-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.INSTITUTE_OF_FOUNDATION_MODELS);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('ifmus-' + first.id);
      ...
      expect(calledUrls[0]).toContain('ifm-us');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.INSTITUTE_OF_FOUNDATION_MODELS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('ifmus-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new InstituteOfFoundationModelsService(registry);
      const result = await service.scrape({
        siteType: [Site.INSTITUTE_OF_FOUNDATION_MODELS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('ifm-us');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('ifmus-x1');
      expect(result.jobs[0].site).toBe(Site.INSTITUTE_OF_FOUNDATION_MODELS);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('ifmus-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new InstituteOfFoundationModelsService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.INSTITUTE_OF_FOUNDATION_MODELS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new InstituteOfFoundationModelsService();
      const result = await service.scrape({
        siteType: [Site.INSTITUTE_OF_FOUNDATION_MODELS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
