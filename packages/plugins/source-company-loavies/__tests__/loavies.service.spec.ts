...
import { LOAVIESModule, LOAVIESService } from '../src';

const COMPANY_NAME_EXPECT = 'LOAVIES';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'loavies-jobs.json'), 'utf8'),
);
...
describe('LOAVIESService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through LOAVIESModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [LOAVIESModule],
      }).compile();
      const service = moduleRef.get(LOAVIESService);
      expect(service).toBeInstanceOf(LOAVIESService);
      await moduleRef.close();
    });

    it('exports the Site.LOAVIES = "loavies" enum value', () => {
      expect(Site.LOAVIES).toBe('loavies');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new LOAVIESService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.LOAVIES],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('loavies.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.LOAVIES);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('loavies-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new LOAVIESService(registry);
      const result = await service.scrape({
        siteType: [Site.LOAVIES],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('loavies');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('loavies-x1');
      expect(result.jobs[0].site).toBe(Site.LOAVIES);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('loavies-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new LOAVIESService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.LOAVIES],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new LOAVIESService();
      const result = await service.scrape({
        siteType: [Site.LOAVIES],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
