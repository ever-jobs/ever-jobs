...
import { NielsenIQModule, NielsenIQService } from '../src';

const COMPANY_NAME_EXPECT = 'NielsenIQ';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'nielseniq-jobs.json'), 'utf8'),
);
...
describe('NielsenIQService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through NielsenIQModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [NielsenIQModule],
      }).compile();
      const service = moduleRef.get(NielsenIQService);
      expect(service).toBeInstanceOf(NielsenIQService);
      await moduleRef.close();
    });

    it('exports the Site.NIELSENIQ = "nielseniq" enum value', () => {
      expect(Site.NIELSENIQ).toBe('nielseniq');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new NielsenIQService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.NIELSENIQ],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('NielsenIQ');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.NIELSENIQ);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('nielseniq-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new NielsenIQService(registry);
      const result = await service.scrape({
        siteType: [Site.NIELSENIQ],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('NielsenIQ');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('nielseniq-x1');
      expect(result.jobs[0].site).toBe(Site.NIELSENIQ);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('nielseniq-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new NielsenIQService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.NIELSENIQ],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new NielsenIQService();
      const result = await service.scrape({
        siteType: [Site.NIELSENIQ],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
