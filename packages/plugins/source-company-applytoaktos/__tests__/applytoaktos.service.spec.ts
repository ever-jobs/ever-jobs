...
import { ApplytoaktosModule, ApplytoaktosService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'applytoaktos-jobs.json'), 'utf8'),
);
...
/**
 * Spec 295 / T04 — `ApplytoaktosService` unit tests (≥ 9 cases).
 */
describe('ApplytoaktosService — Spec 295 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ApplytoaktosModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ApplytoaktosModule],
      }).compile();
      const service = moduleRef.get(ApplytoaktosService);
      expect(service).toBeInstanceOf(ApplytoaktosService);
      await moduleRef.close();
    });

    it('exports the Site.APPLYTOAKTOS = "applytoaktos" enum value', () => {
      expect(Site.APPLYTOAKTOS).toBe('applytoaktos');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ApplytoaktosService();
      const result = await service.scrape({
        siteType: [Site.APPLYTOAKTOS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'applytoaktos-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.APPLYTOAKTOS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Aktos');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/applytoaktos/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/applytoaktos/jobs/');
      ...
      expect(job0?.description).toContain('Aktos');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/applytoaktos/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Aktos');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'applytoaktos-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
