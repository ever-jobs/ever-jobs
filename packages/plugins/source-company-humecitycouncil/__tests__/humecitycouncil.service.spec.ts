...
import { HumeCityCouncilModule, HumeCityCouncilService } from '../src';

const COMPANY_NAME_EXPECT = 'Hume City Council';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'humecitycouncil-jobs.json'), 'utf8'),
);
...
describe('HumeCityCouncilService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through HumeCityCouncilModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [HumeCityCouncilModule],
      }).compile();
      const service = moduleRef.get(HumeCityCouncilService);
      expect(service).toBeInstanceOf(HumeCityCouncilService);
      await moduleRef.close();
    });

    it('exports the Site.HUME_CITY_COUNCIL = "humecitycouncil" enum value', () => {
      expect(Site.HUME_CITY_COUNCIL).toBe('humecitycouncil');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new HumeCityCouncilService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.HUME_CITY_COUNCIL],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'humecitycouncil-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.HUME_CITY_COUNCIL);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('humecitycouncil-' + first.id);
      ...
      expect(calledUrls[0]).toContain('HumeCityCouncil');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.HUME_CITY_COUNCIL);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('humecitycouncil-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new HumeCityCouncilService(registry);
      const result = await service.scrape({
        siteType: [Site.HUME_CITY_COUNCIL],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('HumeCityCouncil');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('humecitycouncil-x1');
      expect(result.jobs[0].site).toBe(Site.HUME_CITY_COUNCIL);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('humecitycouncil-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new HumeCityCouncilService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.HUME_CITY_COUNCIL],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new HumeCityCouncilService();
      const result = await service.scrape({
        siteType: [Site.HUME_CITY_COUNCIL],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
