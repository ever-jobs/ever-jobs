...
import { MistralAIModule, MistralAIService } from '../src';

const COMPANY_NAME_EXPECT = 'Mistral AI';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'mistral-jobs.json'), 'utf8'),
);
...
describe('MistralAIService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through MistralAIModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [MistralAIModule],
      }).compile();
      const service = moduleRef.get(MistralAIService);
      expect(service).toBeInstanceOf(MistralAIService);
      await moduleRef.close();
    });

    it('exports the Site.MISTRAL_AI = "mistral" enum value', () => {
      expect(Site.MISTRAL_AI).toBe('mistral');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new MistralAIService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.MISTRAL_AI],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('mistral');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.MISTRAL_AI);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('mistral-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new MistralAIService(registry);
      const result = await service.scrape({
        siteType: [Site.MISTRAL_AI],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('mistral');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('mistral-x1');
      expect(result.jobs[0].site).toBe(Site.MISTRAL_AI);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('mistral-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new MistralAIService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.MISTRAL_AI],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new MistralAIService();
      const result = await service.scrape({
        siteType: [Site.MISTRAL_AI],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
