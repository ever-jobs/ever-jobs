...
import { AkayshaEnergyModule, AkayshaEnergyService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'akayshaenergy-jobs.json'), 'utf8'),
);
...
/**
 * Spec 806 / T04 — `AkayshaEnergyService` unit tests (≥ 9 cases).
 */
describe('AkayshaEnergyService — Spec 806 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AkayshaEnergyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AkayshaEnergyModule],
      }).compile();
      const service = moduleRef.get(AkayshaEnergyService);
      expect(service).toBeInstanceOf(AkayshaEnergyService);
      await moduleRef.close();
    });

    it('exports the Site.AKAYSHA_ENERGY = "akayshaenergy" enum value', () => {
      expect(Site.AKAYSHA_ENERGY).toBe('akayshaenergy');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AkayshaEnergyService();
      const result = await service.scrape({
        siteType: [Site.AKAYSHA_ENERGY],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'akayshaenergy-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.AKAYSHA_ENERGY);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Akaysha Energy');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/akayshaenergy/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/akayshaenergy/jobs/');
      ...
      expect(job0?.description).toContain('Akaysha Energy');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/akayshaenergy/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Akaysha Energy');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'akayshaenergy-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
