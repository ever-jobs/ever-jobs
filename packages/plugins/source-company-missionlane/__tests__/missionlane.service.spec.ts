...
import { MissionLaneModule, MissionLaneService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'missionlane-jobs.json'), 'utf8'),
);
...
/**
 * Spec 656 / T04 — `MissionLaneService` unit tests (≥ 9 cases).
 */
describe('MissionLaneService — Spec 656 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through MissionLaneModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [MissionLaneModule],
      }).compile();
      const service = moduleRef.get(MissionLaneService);
      expect(service).toBeInstanceOf(MissionLaneService);
      await moduleRef.close();
    });

    it('exports the Site.MISSION_LANE = "missionlane" enum value', () => {
      expect(Site.MISSION_LANE).toBe('missionlane');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new MissionLaneService();
      const result = await service.scrape({
        siteType: [Site.MISSION_LANE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'missionlane-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.MISSION_LANE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Mission Lane');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/missionlane/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/missionlane/jobs/');
      ...
      expect(job0?.description).toContain('Mission Lane');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/missionlane/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Mission Lane');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'missionlane-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
