...
import { StratasFoodsModule, StratasFoodsService } from '../src';

const COMPANY_NAME_EXPECT = 'Stratas Foods';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'stratasfoods-jobs.json'), 'utf8'),
);
...
describe('StratasFoodsService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through StratasFoodsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [StratasFoodsModule],
      }).compile();
      const service = moduleRef.get(StratasFoodsService);
      expect(service).toBeInstanceOf(StratasFoodsService);
      await moduleRef.close();
    });

    it('exports the Site.STRATAS_FOODS = "stratasfoods" enum value', () => {
      expect(Site.STRATAS_FOODS).toBe('stratasfoods');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new StratasFoodsService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.STRATAS_FOODS],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('StratasFoods');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.STRATAS_FOODS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('stratasfoods-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new StratasFoodsService(registry);
      const result = await service.scrape({
        siteType: [Site.STRATAS_FOODS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('StratasFoods');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('stratasfoods-x1');
      expect(result.jobs[0].site).toBe(Site.STRATAS_FOODS);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('stratasfoods-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new StratasFoodsService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.STRATAS_FOODS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new StratasFoodsService();
      const result = await service.scrape({
        siteType: [Site.STRATAS_FOODS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
