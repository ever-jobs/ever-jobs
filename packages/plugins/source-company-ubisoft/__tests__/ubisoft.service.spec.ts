...
import { UbisoftModule, UbisoftService } from '../src';

const COMPANY_NAME_EXPECT = 'Ubisoft';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'ubisoft-jobs.json'), 'utf8'),
);
...
describe('UbisoftService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through UbisoftModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [UbisoftModule],
      }).compile();
      const service = moduleRef.get(UbisoftService);
      expect(service).toBeInstanceOf(UbisoftService);
      await moduleRef.close();
    });

    it('exports the Site.UBISOFT = "ubisoft" enum value', () => {
      expect(Site.UBISOFT).toBe('ubisoft');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new UbisoftService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.UBISOFT],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('ubisoft2');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.UBISOFT);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('ubisoft-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new UbisoftService(registry);
      const result = await service.scrape({
        siteType: [Site.UBISOFT],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('ubisoft2');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('ubisoft-x1');
      expect(result.jobs[0].site).toBe(Site.UBISOFT);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('ubisoft-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new UbisoftService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.UBISOFT],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new UbisoftService();
      const result = await service.scrape({
        siteType: [Site.UBISOFT],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
