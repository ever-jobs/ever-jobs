...
import { SikaModule, SikaService } from '../src';

const COMPANY_NAME_EXPECT = 'Sika';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'sika-jobs.json'), 'utf8'),
);
...
describe('SikaService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SikaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SikaModule],
      }).compile();
      const service = moduleRef.get(SikaService);
      expect(service).toBeInstanceOf(SikaService);
      await moduleRef.close();
    });

    it('exports the Site.SIKA = "sika" enum value', () => {
      expect(Site.SIKA).toBe('sika');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new SikaService(registryWithSmartRecruiters());
      ...
      expect(calledUrls[0]).toContain('SikaAG');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SIKA);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('sika-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new SikaService(registry);
      const result = await service.scrape({
        siteType: [Site.SIKA],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('SikaAG');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('sika-x1');
      expect(result.jobs[0].site).toBe(Site.SIKA);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('sika-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new SikaService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.SIKA],
      } as ScraperInputDto);
