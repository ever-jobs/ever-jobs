...
import { NutrafolModule, NutrafolService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'nutrafol-jobs.json'), 'utf8'),
);
...
/**
 * Spec 677 / T04 — `NutrafolService` unit tests (≥ 9 cases).
 */
describe('NutrafolService — Spec 677 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through NutrafolModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [NutrafolModule],
      }).compile();
      const service = moduleRef.get(NutrafolService);
      expect(service).toBeInstanceOf(NutrafolService);
      await moduleRef.close();
    });

    it('exports the Site.NUTRAFOL = "nutrafol" enum value', () => {
      expect(Site.NUTRAFOL).toBe('nutrafol');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new NutrafolService();
      const result = await service.scrape({
        siteType: [Site.NUTRAFOL],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'nutrafol-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.NUTRAFOL);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Nutrafol');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/nutrafol/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/nutrafol/jobs/');
      ...
      expect(job0?.description).toContain('Nutrafol');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/nutrafol/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Nutrafol');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'nutrafol-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
