...
import { TailscaleModule, TailscaleService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'tailscale-jobs.json'), 'utf8'),
);
...
/**
 * Spec 451 / T04 — `TailscaleService` unit tests (≥ 9 cases).
 */
describe('TailscaleService — Spec 451 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through TailscaleModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [TailscaleModule],
      }).compile();
      const service = moduleRef.get(TailscaleService);
      expect(service).toBeInstanceOf(TailscaleService);
      await moduleRef.close();
    });

    it('exports the Site.TAILSCALE = "tailscale" enum value', () => {
      expect(Site.TAILSCALE).toBe('tailscale');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const job0 = dto.jobs.find((j) => j.id === 'tailscale-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.TAILSCALE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Tailscale');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/tailscale/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/tailscale/jobs/');
      ...
      expect(job0?.description).toContain('Tailscale');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/tailscale/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Tailscale');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'tailscale-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
