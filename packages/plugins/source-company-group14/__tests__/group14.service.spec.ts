...
import { Group14Module, Group14Service } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'group14-jobs.json'), 'utf8'),
);
...
/**
 * Spec 521 / T04 — `Group14Service` unit tests (≥ 9 cases).
 */
describe('Group14Service — Spec 521 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through Group14Module via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [Group14Module],
      }).compile();
      const service = moduleRef.get(Group14Service);
      expect(service).toBeInstanceOf(Group14Service);
      await moduleRef.close();
    });

    it('exports the Site.GROUP14 = "group14" enum value', () => {
      expect(Site.GROUP14).toBe('group14');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new Group14Service();
      const result = await service.scrape({
        siteType: [Site.GROUP14],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'group14-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.GROUP14);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Group14 Technologies');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/group14/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/group14/jobs/');
      ...
      expect(job0?.description).toContain('Group14 Technologies');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/group14/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Group14 Technologies');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'group14-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
