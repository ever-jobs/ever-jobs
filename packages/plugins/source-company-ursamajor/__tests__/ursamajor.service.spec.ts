...
import { UrsaMajorModule, UrsaMajorService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'ursamajor-jobs.json'), 'utf8'),
);
...
/**
 * Spec 801 / T04 — `UrsaMajorService` unit tests (≥ 9 cases).
 */
describe('UrsaMajorService — Spec 801 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through UrsaMajorModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [UrsaMajorModule],
      }).compile();
      const service = moduleRef.get(UrsaMajorService);
      expect(service).toBeInstanceOf(UrsaMajorService);
      await moduleRef.close();
    });

    it('exports the Site.URSA_MAJOR = "ursamajor" enum value', () => {
      expect(Site.URSA_MAJOR).toBe('ursamajor');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new UrsaMajorService();
      const result = await service.scrape({
        siteType: [Site.URSA_MAJOR],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'ursamajor-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.URSA_MAJOR);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Ursa Major');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/ursamajor/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/ursamajor/jobs/');
      ...
      expect(job0?.description).toContain('Ursa Major');
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Ursa Major');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'ursamajor-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
