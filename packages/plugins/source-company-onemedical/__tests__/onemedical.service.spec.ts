...
import { OnemedicalModule, OnemedicalService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'onemedical-jobs.json'), 'utf8'),
);
...
/**
 * Spec 566 / T04 — `OnemedicalService` unit tests (≥ 9 cases).
 */
describe('OnemedicalService — Spec 566 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through OnemedicalModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [OnemedicalModule],
      }).compile();
      const service = moduleRef.get(OnemedicalService);
      expect(service).toBeInstanceOf(OnemedicalService);
      await moduleRef.close();
    });

    it('exports the Site.ONE_MEDICAL = "onemedical" enum value', () => {
      expect(Site.ONE_MEDICAL).toBe('onemedical');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new OnemedicalService();
      const result = await service.scrape({
        siteType: [Site.ONE_MEDICAL],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'onemedical-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ONE_MEDICAL);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('One Medical');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/onemedical/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/onemedical/jobs/');
      ...
      expect(job0?.description).toContain('One Medical');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/onemedical/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('One Medical');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'onemedical-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
