...
import { AVIVGroupModule, AVIVGroupService } from '../src';

const COMPANY_NAME_EXPECT = 'AVIV Group';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'avivgroup-jobs.json'), 'utf8'),
);
...
describe('AVIVGroupService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AVIVGroupModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AVIVGroupModule],
      }).compile();
      const service = moduleRef.get(AVIVGroupService);
      expect(service).toBeInstanceOf(AVIVGroupService);
      await moduleRef.close();
    });

    it('exports the Site.AVIV_GROUP = "avivgroup" enum value', () => {
      expect(Site.AVIV_GROUP).toBe('avivgroup');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new AVIVGroupService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.AVIV_GROUP],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('avivgroup');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.AVIV_GROUP);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('avivgroup-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new AVIVGroupService(registry);
      const result = await service.scrape({
        siteType: [Site.AVIV_GROUP],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('avivgroup');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('avivgroup-x1');
      expect(result.jobs[0].site).toBe(Site.AVIV_GROUP);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('avivgroup-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new AVIVGroupService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.AVIV_GROUP],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new AVIVGroupService();
      const result = await service.scrape({
        siteType: [Site.AVIV_GROUP],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
