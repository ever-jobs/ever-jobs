...
import { OphelosModule, OphelosService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'ophelos-jobs.json'), 'utf8'),
);
...
/**
 * Spec 753 / T04 — `OphelosService` unit tests (≥ 9 cases).
 */
describe('OphelosService — Spec 753 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through OphelosModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [OphelosModule],
      }).compile();
      const service = moduleRef.get(OphelosService);
      expect(service).toBeInstanceOf(OphelosService);
      await moduleRef.close();
    });

    it('exports the Site.OPHELOS = "ophelos" enum value', () => {
      expect(Site.OPHELOS).toBe('ophelos');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new OphelosService();
      const result = await service.scrape({
        siteType: [Site.OPHELOS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'ophelos-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.OPHELOS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Ophelos');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/ophelos/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/ophelos/jobs/');
      ...
      expect(job0?.description).toContain('Ophelos');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/ophelos/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Ophelos');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'ophelos-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
