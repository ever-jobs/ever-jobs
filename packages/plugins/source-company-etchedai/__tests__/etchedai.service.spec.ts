...
import { EtchedModule, EtchedService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'etchedai-jobs.json'), 'utf8'),
);
...
/**
 * Spec 778 / T04 — `EtchedService` unit tests (≥ 9 cases).
 */
describe('EtchedService — Spec 778 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through EtchedModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [EtchedModule],
      }).compile();
      const service = moduleRef.get(EtchedService);
      expect(service).toBeInstanceOf(EtchedService);
      await moduleRef.close();
    });

    it('exports the Site.ETCHED = "etchedai" enum value', () => {
      expect(Site.ETCHED).toBe('etchedai');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new EtchedService();
      const result = await service.scrape({
        siteType: [Site.ETCHED],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'etchedai-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ETCHED);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Etched');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/etchedai/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/etchedai/jobs/');
      ...
      expect(job0?.description).toContain('Etched');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/etchedai/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Etched');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'etchedai-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
