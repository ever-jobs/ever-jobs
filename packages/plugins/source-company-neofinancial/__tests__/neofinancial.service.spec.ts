...
import { NeoFinancialModule, NeoFinancialService } from '../src';

const COMPANY_NAME_EXPECT = 'Neo Financial';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'neofinancial-jobs.json'), 'utf8'),
);
...
describe('NeoFinancialService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through NeoFinancialModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [NeoFinancialModule],
      }).compile();
      const service = moduleRef.get(NeoFinancialService);
      expect(service).toBeInstanceOf(NeoFinancialService);
      await moduleRef.close();
    });

    it('exports the Site.NEO_FINANCIAL = "neofinancial" enum value', () => {
      expect(Site.NEO_FINANCIAL).toBe('neofinancial');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new NeoFinancialService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.NEO_FINANCIAL],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'neofinancial-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.NEO_FINANCIAL);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('neofinancial-' + first.id);
      ...
      expect(calledUrls[0]).toContain('neofinancial');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.NEO_FINANCIAL);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('neofinancial-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new NeoFinancialService(registry);
      const result = await service.scrape({
        siteType: [Site.NEO_FINANCIAL],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('neofinancial');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('neofinancial-x1');
      expect(result.jobs[0].site).toBe(Site.NEO_FINANCIAL);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('neofinancial-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new NeoFinancialService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.NEO_FINANCIAL],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new NeoFinancialService();
      const result = await service.scrape({
        siteType: [Site.NEO_FINANCIAL],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
