...
import { GarnerHealthModule, GarnerHealthService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'garnerhealth-jobs.json'), 'utf8'),
);
...
/**
 * Spec 780 / T04 — `GarnerHealthService` unit tests (≥ 9 cases).
 */
describe('GarnerHealthService — Spec 780 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through GarnerHealthModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [GarnerHealthModule],
      }).compile();
      const service = moduleRef.get(GarnerHealthService);
      expect(service).toBeInstanceOf(GarnerHealthService);
      await moduleRef.close();
    });

    it('exports the Site.GARNER_HEALTH = "garnerhealth" enum value', () => {
      expect(Site.GARNER_HEALTH).toBe('garnerhealth');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new GarnerHealthService();
      const result = await service.scrape({
        siteType: [Site.GARNER_HEALTH],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'garnerhealth-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.GARNER_HEALTH);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Garner Health');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/garnerhealth/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/garnerhealth/jobs/');
      ...
      expect(job0?.description).toContain('Garner Health');
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Garner Health');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'garnerhealth-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
