...
import { OzowModule, OzowService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'ozow-jobs.json'), 'utf8'),
);
...
/**
 * Spec 690 / T04 — `OzowService` unit tests (≥ 9 cases).
 */
describe('OzowService — Spec 690 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through OzowModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [OzowModule],
      }).compile();
      const service = moduleRef.get(OzowService);
      expect(service).toBeInstanceOf(OzowService);
      await moduleRef.close();
    });

    it('exports the Site.OZOW = "ozow" enum value', () => {
      expect(Site.OZOW).toBe('ozow');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const job0 = dto.jobs.find((j) => j.id === 'ozow-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.OZOW);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Ozow');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/ozow/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/ozow/jobs/');
      ...
      expect(job0?.description).toContain('Ozow');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/ozow/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Ozow');
      }
    });
  });
