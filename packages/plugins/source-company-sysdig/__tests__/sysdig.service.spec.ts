...
import { SysdigModule, SysdigService } from '../src';

const COMPANY_NAME_EXPECT = 'Sysdig';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'sysdig-jobs.json'), 'utf8'),
);
...
describe('SysdigService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SysdigModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SysdigModule],
      }).compile();
      const service = moduleRef.get(SysdigService);
      expect(service).toBeInstanceOf(SysdigService);
      await moduleRef.close();
    });

    it('exports the Site.SYSDIG = "sysdig" enum value', () => {
      expect(Site.SYSDIG).toBe('sysdig');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new SysdigService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.SYSDIG],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('sysdig');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SYSDIG);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('sysdig-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new SysdigService(registry);
      const result = await service.scrape({
        siteType: [Site.SYSDIG],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('sysdig');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('sysdig-x1');
      expect(result.jobs[0].site).toBe(Site.SYSDIG);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('sysdig-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new SysdigService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.SYSDIG],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new SysdigService();
      const result = await service.scrape({
        siteType: [Site.SYSDIG],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
