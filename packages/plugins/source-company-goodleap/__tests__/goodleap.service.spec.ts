...
import { GoodLeapModule, GoodLeapService } from '../src';

const COMPANY_NAME_EXPECT = 'GoodLeap';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'goodleap-jobs.json'), 'utf8'),
);
...
describe('GoodLeapService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through GoodLeapModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [GoodLeapModule],
      }).compile();
      const service = moduleRef.get(GoodLeapService);
      expect(service).toBeInstanceOf(GoodLeapService);
      await moduleRef.close();
    });

    it('exports the Site.GOODLEAP = "goodleap" enum value', () => {
      expect(Site.GOODLEAP).toBe('goodleap');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new GoodLeapService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.GOODLEAP],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('goodleap');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.GOODLEAP);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('goodleap-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new GoodLeapService(registry);
      const result = await service.scrape({
        siteType: [Site.GOODLEAP],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('goodleap');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('goodleap-x1');
      expect(result.jobs[0].site).toBe(Site.GOODLEAP);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('goodleap-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new GoodLeapService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.GOODLEAP],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new GoodLeapService();
      const result = await service.scrape({
        siteType: [Site.GOODLEAP],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
