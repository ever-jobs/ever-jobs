...
import { StackAVModule, StackAVService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'stackav-jobs.json'), 'utf8'),
);
...
/**
 * Spec 798 / T04 — `StackAVService` unit tests (≥ 9 cases).
 */
describe('StackAVService — Spec 798 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through StackAVModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [StackAVModule],
      }).compile();
      const service = moduleRef.get(StackAVService);
      expect(service).toBeInstanceOf(StackAVService);
      await moduleRef.close();
    });

    it('exports the Site.STACK_AV = "stackav" enum value', () => {
      expect(Site.STACK_AV).toBe('stackav');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new StackAVService();
      const result = await service.scrape({
        siteType: [Site.STACK_AV],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'stackav-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.STACK_AV);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Stack AV');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/stackav/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/stackav/jobs/');
      ...
      expect(job0?.description).toContain('Stack AV');
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Stack AV');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'stackav-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
