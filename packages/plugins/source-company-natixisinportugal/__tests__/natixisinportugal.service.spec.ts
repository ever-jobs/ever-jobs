...
import { NatixisInPortugalModule, NatixisInPortugalService } from '../src';

const COMPANY_NAME_EXPECT = 'Natixis in Portugal';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'natixisinportugal-jobs.json'), 'utf8'),
);
...
describe('NatixisInPortugalService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through NatixisInPortugalModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [NatixisInPortugalModule],
      }).compile();
      const service = moduleRef.get(NatixisInPortugalService);
      expect(service).toBeInstanceOf(NatixisInPortugalService);
      await moduleRef.close();
    });

    it('exports the Site.NATIXIS_IN_PORTUGAL = "natixisinportugal" enum value', () => {
      expect(Site.NATIXIS_IN_PORTUGAL).toBe('natixisinportugal');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new NatixisInPortugalService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.NATIXIS_IN_PORTUGAL],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'natixisinportugal-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.NATIXIS_IN_PORTUGAL);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('natixisinportugal-' + first.id);
      ...
      expect(calledUrls[0]).toContain('natixisinportugal');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.NATIXIS_IN_PORTUGAL);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('natixisinportugal-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new NatixisInPortugalService(registry);
      const result = await service.scrape({
        siteType: [Site.NATIXIS_IN_PORTUGAL],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('natixisinportugal');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('natixisinportugal-x1');
      expect(result.jobs[0].site).toBe(Site.NATIXIS_IN_PORTUGAL);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('natixisinportugal-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new NatixisInPortugalService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.NATIXIS_IN_PORTUGAL],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new NatixisInPortugalService();
      const result = await service.scrape({
        siteType: [Site.NATIXIS_IN_PORTUGAL],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
