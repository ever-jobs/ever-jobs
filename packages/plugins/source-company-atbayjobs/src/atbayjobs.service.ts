...
/**
 * At-Bay — At-Bay is an employer in the Cyber insurance / security sector, headquartered in San Francisco, California, USA..
 *
 * At-Bay is a company operating in Cyber insurance / security,
 * headquartered in San Francisco, California, USA. This source plugin
 * ingests live open roles published on At-Bay's official
 ...
 *
 * Sector: Cyber insurance / security. HQ: San Francisco, California, USA.
 *
 * Highlights:
 *   - Sector: Cyber insurance / security
 ...
 *     Jobs schema
 *
 * Source profile (Spec 809):
 *   - D-04 — Greenhouse canonical hosted-board host (variant 2):
 *     `https://job-boards.greenhouse.io/atbayjobs/jobs/<id>`.
 *   - D-08 — entity-decode-then-tag-strip description pipeline.
 *   - D-09 — wire `company_name` pass-through (`'At-Bay'`).
 ...
 */
const API_URL = 'https://api.greenhouse.io/v1/boards/atbayjobs/jobs';
...
@Injectable()
export class AtBayService implements IScraper {
  private readonly logger = new Logger(AtBayService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    try {
      ...
      this.logger.log(`At-Bay: fetching ${url}`);
      ...
      for (const listing of listings) {
        ...
        jobs.push(
          new JobPostDto({
            ...
            companyName: listing.company_name ?? 'At-Bay',
            // D-04: wire `absolute_url` flows through (variant 2).
            jobUrl:
              listing.absolute_url ??
              `https://job-boards.greenhouse.io/atbayjobs/jobs/${listing.id}`,
            location,
      ...
      this.logger.log(`At-Bay: scraped ${jobs.length} jobs`);
    } catch (err: any) {
      this.logger.error(`At-Bay scrape failed: ${err.message}`);
