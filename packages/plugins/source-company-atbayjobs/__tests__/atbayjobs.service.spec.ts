...
import { AtBayModule, AtBayService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'atbayjobs-jobs.json'), 'utf8'),
);
...
/**
 * Spec 809 / T04 — `AtBayService` unit tests (≥ 9 cases).
 */
describe('AtBayService — Spec 809 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AtBayModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AtBayModule],
      }).compile();
      const service = moduleRef.get(AtBayService);
      expect(service).toBeInstanceOf(AtBayService);
      await moduleRef.close();
    });

    it('exports the Site.AT_BAY = "atbayjobs" enum value', () => {
      expect(Site.AT_BAY).toBe('atbayjobs');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AtBayService();
      const result = await service.scrape({
        siteType: [Site.AT_BAY],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'atbayjobs-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.AT_BAY);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('At-Bay');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/atbayjobs/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/atbayjobs/jobs/');
      ...
      expect(job0?.description).toContain('At-Bay');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/atbayjobs/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('At-Bay');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'atbayjobs-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
