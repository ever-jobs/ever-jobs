...
import { LindusHealthModule, LindusHealthService } from '../src';

const COMPANY_NAME_EXPECT = 'Lindus Health';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'lindushealth-jobs.json'), 'utf8'),
);
...
describe('LindusHealthService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through LindusHealthModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [LindusHealthModule],
      }).compile();
      const service = moduleRef.get(LindusHealthService);
      expect(service).toBeInstanceOf(LindusHealthService);
      await moduleRef.close();
    });

    it('exports the Site.LINDUS_HEALTH = "lindushealth" enum value', () => {
      expect(Site.LINDUS_HEALTH).toBe('lindushealth');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new LindusHealthService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.LINDUS_HEALTH],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'lindushealth-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.LINDUS_HEALTH);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('lindushealth-' + first.id);
      ...
      expect(calledUrls[0]).toContain('lindus');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.LINDUS_HEALTH);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('lindushealth-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new LindusHealthService(registry);
      const result = await service.scrape({
        siteType: [Site.LINDUS_HEALTH],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('lindus');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('lindushealth-x1');
      expect(result.jobs[0].site).toBe(Site.LINDUS_HEALTH);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('lindushealth-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new LindusHealthService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.LINDUS_HEALTH],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new LindusHealthService();
      const result = await service.scrape({
        siteType: [Site.LINDUS_HEALTH],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
