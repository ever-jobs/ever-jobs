...
import { GoustoModule, GoustoService } from '../src';

const COMPANY_NAME_EXPECT = 'Gousto';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'gousto-jobs.json'), 'utf8'),
);
...
describe('GoustoService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through GoustoModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [GoustoModule],
      }).compile();
      const service = moduleRef.get(GoustoService);
      expect(service).toBeInstanceOf(GoustoService);
      await moduleRef.close();
    });

    it('exports the Site.GOUSTO = "gousto" enum value', () => {
      expect(Site.GOUSTO).toBe('gousto');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new GoustoService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.GOUSTO],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('Gousto');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.GOUSTO);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('gousto-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new GoustoService(registry);
      const result = await service.scrape({
        siteType: [Site.GOUSTO],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('Gousto');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('gousto-x1');
      expect(result.jobs[0].site).toBe(Site.GOUSTO);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('gousto-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new GoustoService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.GOUSTO],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new GoustoService();
      const result = await service.scrape({
        siteType: [Site.GOUSTO],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
