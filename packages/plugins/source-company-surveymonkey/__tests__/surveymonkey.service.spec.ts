...
import { SurveymonkeyModule, SurveymonkeyService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'surveymonkey-jobs.json'), 'utf8'),
);
...
/**
 * Spec 465 / T04 — `SurveymonkeyService` unit tests (≥ 9 cases).
 */
describe('SurveymonkeyService — Spec 465 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SurveymonkeyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SurveymonkeyModule],
      }).compile();
      const service = moduleRef.get(SurveymonkeyService);
      expect(service).toBeInstanceOf(SurveymonkeyService);
      await moduleRef.close();
    });

    it('exports the Site.SURVEYMONKEY = "surveymonkey" enum value', () => {
      expect(Site.SURVEYMONKEY).toBe('surveymonkey');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new SurveymonkeyService();
      const result = await service.scrape({
        siteType: [Site.SURVEYMONKEY],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'surveymonkey-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.SURVEYMONKEY);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('SurveyMonkey');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/surveymonkey/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/surveymonkey/jobs/');
      ...
      expect(job0?.description).toContain('SurveyMonkey');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/surveymonkey/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('SurveyMonkey');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'surveymonkey-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
