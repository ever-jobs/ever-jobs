...
import { LakeshoreLearningMaterialsModule, LakeshoreLearningMaterialsService } from '../src';

const COMPANY_NAME_EXPECT = 'Lakeshore Learning Materials';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'lakeshorelearningmaterials-jobs.json'), 'utf8'),
);
...
describe('LakeshoreLearningMaterialsService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through LakeshoreLearningMaterialsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [LakeshoreLearningMaterialsModule],
      }).compile();
      const service = moduleRef.get(LakeshoreLearningMaterialsService);
      expect(service).toBeInstanceOf(LakeshoreLearningMaterialsService);
      await moduleRef.close();
    });

    it('exports the Site.LAKESHORE_LEARNING_MATERIALS = "lakeshorelearningmaterials" enum value', () => {
      expect(Site.LAKESHORE_LEARNING_MATERIALS).toBe('lakeshorelearningmaterials');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new LakeshoreLearningMaterialsService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.LAKESHORE_LEARNING_MATERIALS],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'lakeshorelearningmaterials-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.LAKESHORE_LEARNING_MATERIALS);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('lakeshorelearningmaterials-' + first.id);
      ...
      expect(calledUrls[0]).toContain('Lakeshore');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.LAKESHORE_LEARNING_MATERIALS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('lakeshorelearningmaterials-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new LakeshoreLearningMaterialsService(registry);
      const result = await service.scrape({
        siteType: [Site.LAKESHORE_LEARNING_MATERIALS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('Lakeshore');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('lakeshorelearningmaterials-x1');
      expect(result.jobs[0].site).toBe(Site.LAKESHORE_LEARNING_MATERIALS);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('lakeshorelearningmaterials-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new LakeshoreLearningMaterialsService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.LAKESHORE_LEARNING_MATERIALS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new LakeshoreLearningMaterialsService();
      const result = await service.scrape({
        siteType: [Site.LAKESHORE_LEARNING_MATERIALS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
