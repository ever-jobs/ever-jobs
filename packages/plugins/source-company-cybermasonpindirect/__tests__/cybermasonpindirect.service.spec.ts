...
import { CyberMasonPinDirectModule, CyberMasonPinDirectService } from '../src';

const COMPANY_NAME_EXPECT = 'Cyber & Mason (PinDirect)';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'cybermasonpindirect-jobs.json'), 'utf8'),
);
...
describe('CyberMasonPinDirectService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CyberMasonPinDirectModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CyberMasonPinDirectModule],
      }).compile();
      const service = moduleRef.get(CyberMasonPinDirectService);
      expect(service).toBeInstanceOf(CyberMasonPinDirectService);
      await moduleRef.close();
    });

    it('exports the Site.CYBER_MASON_PINDIRECT = "cybermasonpindirect" enum value', () => {
      expect(Site.CYBER_MASON_PINDIRECT).toBe('cybermasonpindirect');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new CyberMasonPinDirectService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.CYBER_MASON_PINDIRECT],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'cybermasonpindirect-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Recruitee's defaults
      expect(job0?.site).toBe(Site.CYBER_MASON_PINDIRECT);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('cybermasonpindirect-' + first.id);
      ...
      expect(calledUrls[0]).toContain('cybermason.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.CYBER_MASON_PINDIRECT);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('cybermasonpindirect-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new CyberMasonPinDirectService(registry);
      const result = await service.scrape({
        siteType: [Site.CYBER_MASON_PINDIRECT],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('cybermason');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('cybermasonpindirect-x1');
      expect(result.jobs[0].site).toBe(Site.CYBER_MASON_PINDIRECT);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('cybermasonpindirect-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new CyberMasonPinDirectService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.CYBER_MASON_PINDIRECT],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new CyberMasonPinDirectService();
      const result = await service.scrape({
        siteType: [Site.CYBER_MASON_PINDIRECT],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
