...
import { CapitalOneModule, CapitalOneService } from '../src';
...
const FIXTURE: Fixture = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'fixtures', 'capitalone-boards.json'), 'utf8'),
);
const COMPANY_NAME = 'Capital One';
const ID_PREFIX = 'capitalone-';
...
describe('CapitalOneService — Workday delegation (Spec 1736)', () => {
  beforeEach(() => {
    mockGet.mockReset();
    mockPost.mockReset();
  ...
  describe('registration', () => {
    it('resolves through CapitalOneModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({ imports: [CapitalOneModule] }).compile();
      expect(moduleRef.get(CapitalOneService)).toBeInstanceOf(CapitalOneService);
      await moduleRef.close();
    });

    it('exports Site.CAPITAL_ONE = "capitalone"', () => {
      expect(Site.CAPITAL_ONE).toBe('capitalone');
    });

    it('declares a tagged company plugin with its domains', () => {
      const meta = Reflect.getMetadata(SOURCE_PLUGIN_METADATA, CapitalOneService);
      expect(meta.site).toBe(Site.CAPITAL_ONE);
      expect(meta.category).toBe('company');
      expect(meta.isAts).toBeFalsy();
      expect(meta.companyDomains).toEqual(['capitalone.com']);
  ...
  describe('recorded board (real Workday adapter, mocked HTTP)', () => {
    it('maps every recorded posting and re-stamps the company identity', async () => {
      const service = new CapitalOneService(registryWith());
      const result = await service.scrape({ siteType: [Site.CAPITAL_ONE], resultsWanted: 100 } as ScraperInputDto);
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.CAPITAL_ONE);
    ...
    it('only requests the recorded board URLs', async () => {
      const service = new CapitalOneService(registryWith());
      await service.scrape({ siteType: [Site.CAPITAL_ONE] } as ScraperInputDto);
    ...
    it('honours resultsWanted=1', async () => {
      const service = new CapitalOneService(registryWith());
      const result = await service.scrape({ siteType: [Site.CAPITAL_ONE], resultsWanted: 1 } as ScraperInputDto);
    ...
    it('resolves with an empty result when the board is gone (HTTP 404)', async () => {
      ...
      const service = new CapitalOneService(registryWith());
      const result = await service.scrape({ siteType: [Site.CAPITAL_ONE] } as ScraperInputDto);
  ...
  describe('delegation contract', () => {
    it('forwards the board and the caller input untouched', async () => {
      ...
      const service = new CapitalOneService(
        registryWith(
          fakeBackend(
      ...
      expect(result.jobs[0].site).toBe(Site.CAPITAL_ONE);
    ...
    it('never forwards the caller\'s credentials to the board (Spec 1735 §4.5)', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new CapitalOneService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
    ...
    it('makes no request when resultsWanted is 0', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new CapitalOneService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      const result = await service.scrape({ siteType: [Site.CAPITAL_ONE], resultsWanted: 0 } as ScraperInputDto);
  ...
  describe('resilience', () => {
    it('reports not_registered when no registry is injected', async () => {
      const result = await new CapitalOneService().scrape({ siteType: [Site.CAPITAL_ONE] } as ScraperInputDto);
      expect(result.jobs).toEqual([]);
    ...
    it('reports not_registered when Workday is missing from the registry', async () => {
      const result = await new CapitalOneService(new PluginRegistry()).scrape({
        siteType: [Site.CAPITAL_ONE],
      } as ScraperInputDto);
      expect(result.diagnostics?.reason).toBe('not_registered');
    });
