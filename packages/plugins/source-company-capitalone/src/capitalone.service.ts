...
/**
 * Capital One — Banking (HQ: McLean, VA, USA).
 *
 * Source (Spec 1736): Workday board, scraped in this order:
 *   - `capitalone:12:Capital_One` — https://capitalone.wd12.myworkdayjobs.com/Capital_One
 *     verified live 2026-09-24: 1,843 open postings.
 *
 ...
 * party board.
 *
 * Tags: segment=workday-enterprise; industry=banking.
 */
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'capitalone:12:Capital_One', atsIdPrefix: 'wd-capitalone-' },
];

@SourcePlugin({
  site: Site.CAPITAL_ONE,
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['capitalone.com'],
  description: 'Capital One careers via Workday. Tags: segment=workday-enterprise; industry=banking.',
})
@Injectable()
export class CapitalOneService implements IScraper {
  private readonly logger = new Logger(CapitalOneService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Workday source plugin is not registered; cannot scrape Capital One');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`Capital One: delegating to Workday (${board.companySlug})`);
    ...
    this.logger.log(`Capital One: scraped ${jobs.length} jobs`);
