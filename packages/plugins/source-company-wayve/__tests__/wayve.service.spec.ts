...
import { WayveModule, WayveService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'wayve-jobs.json'), 'utf8'),
);
...
/**
 * Spec 559 / T04 — `WayveService` unit tests (≥ 9 cases).
 */
describe('WayveService — Spec 559 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through WayveModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [WayveModule],
      }).compile();
      const service = moduleRef.get(WayveService);
      expect(service).toBeInstanceOf(WayveService);
      await moduleRef.close();
    });

    it('exports the Site.WAYVE = "wayve" enum value', () => {
      expect(Site.WAYVE).toBe('wayve');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new WayveService();
      ...
      const job0 = dto.jobs.find((j) => j.id === 'wayve-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.WAYVE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Wayve');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/wayve/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/wayve/jobs/');
      ...
      expect(job0?.description).toContain('Wayve');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/wayve/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Wayve');
      }
    });
  });
