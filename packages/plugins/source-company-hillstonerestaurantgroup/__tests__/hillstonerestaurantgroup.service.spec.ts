...
import { HillstoneRestaurantGroupModule, HillstoneRestaurantGroupService } from '../src';

const COMPANY_NAME_EXPECT = 'Hillstone Restaurant Group';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'hillstonerestaurantgroup-jobs.json'), 'utf8'),
);
...
describe('HillstoneRestaurantGroupService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through HillstoneRestaurantGroupModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [HillstoneRestaurantGroupModule],
      }).compile();
      const service = moduleRef.get(HillstoneRestaurantGroupService);
      expect(service).toBeInstanceOf(HillstoneRestaurantGroupService);
      await moduleRef.close();
    });

    it('exports the Site.HILLSTONE_RESTAURANT_GROUP = "hillstonerestaurantgroup" enum value', () => {
      expect(Site.HILLSTONE_RESTAURANT_GROUP).toBe('hillstonerestaurantgroup');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new HillstoneRestaurantGroupService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.HILLSTONE_RESTAURANT_GROUP],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'hillstonerestaurantgroup-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.HILLSTONE_RESTAURANT_GROUP);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('hillstonerestaurantgroup-' + first.id);
      ...
      expect(calledUrls[0]).toContain('HillstoneRestaurantGroup');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.HILLSTONE_RESTAURANT_GROUP);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('hillstonerestaurantgroup-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new HillstoneRestaurantGroupService(registry);
      const result = await service.scrape({
        siteType: [Site.HILLSTONE_RESTAURANT_GROUP],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('HillstoneRestaurantGroup');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('hillstonerestaurantgroup-x1');
      expect(result.jobs[0].site).toBe(Site.HILLSTONE_RESTAURANT_GROUP);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('hillstonerestaurantgroup-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new HillstoneRestaurantGroupService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.HILLSTONE_RESTAURANT_GROUP],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new HillstoneRestaurantGroupService();
      const result = await service.scrape({
        siteType: [Site.HILLSTONE_RESTAURANT_GROUP],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
