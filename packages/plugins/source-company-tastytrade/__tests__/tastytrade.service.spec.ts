...
import { TastytradeModule, TastytradeService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'tastytrade-jobs.json'), 'utf8'),
);
...
/**
 * Spec 799 / T04 — `TastytradeService` unit tests (≥ 9 cases).
 */
describe('TastytradeService — Spec 799 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through TastytradeModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [TastytradeModule],
      }).compile();
      const service = moduleRef.get(TastytradeService);
      expect(service).toBeInstanceOf(TastytradeService);
      await moduleRef.close();
    });

    it('exports the Site.TASTYTRADE = "tastytrade" enum value', () => {
      expect(Site.TASTYTRADE).toBe('tastytrade');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new TastytradeService();
      const result = await service.scrape({
        siteType: [Site.TASTYTRADE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'tastytrade-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.TASTYTRADE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('tastytrade');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/tastytrade/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/tastytrade/jobs/');
      ...
      expect(job0?.description).toContain('tastytrade');
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('tastytrade');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'tastytrade-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
