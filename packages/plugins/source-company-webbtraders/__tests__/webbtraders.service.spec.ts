...
import { WEBBTradersModule, WEBBTradersService } from '../src';

const COMPANY_NAME_EXPECT = 'WEBB Traders';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'webbtraders-jobs.json'), 'utf8'),
);
...
describe('WEBBTradersService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through WEBBTradersModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [WEBBTradersModule],
      }).compile();
      const service = moduleRef.get(WEBBTradersService);
      expect(service).toBeInstanceOf(WEBBTradersService);
      await moduleRef.close();
    });

    it('exports the Site.WEBB_TRADERS = "webbtraders" enum value', () => {
      expect(Site.WEBB_TRADERS).toBe('webbtraders');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new WEBBTradersService(registryWithRecruitee());
      ...
      expect(calledUrls[0]).toContain('webbtraders.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.WEBB_TRADERS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('webbtraders-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new WEBBTradersService(registry);
      const result = await service.scrape({
        siteType: [Site.WEBB_TRADERS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('webbtraders');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('webbtraders-x1');
      expect(result.jobs[0].site).toBe(Site.WEBB_TRADERS);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('webbtraders-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new WEBBTradersService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.WEBB_TRADERS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new WEBBTradersService();
      const result = await service.scrape({
        siteType: [Site.WEBB_TRADERS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
