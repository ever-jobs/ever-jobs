...
import { OsanoModule, OsanoService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'osano-jobs.json'), 'utf8'),
);
...
/**
 * Spec 905 / T04 — `OsanoService` unit tests (≥ 9 cases).
 */
describe('OsanoService — Spec 905 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through OsanoModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [OsanoModule],
      }).compile();
      const service = moduleRef.get(OsanoService);
      expect(service).toBeInstanceOf(OsanoService);
      await moduleRef.close();
    });

    it('exports the Site.OSANO = "osano" enum value', () => {
      expect(Site.OSANO).toBe('osano');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new OsanoService();
      ...
      const job0 = dto.jobs.find((j) => j.id === 'osano-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.OSANO);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Osano');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/osano/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/osano/jobs/');
      ...
      expect(job0?.description).toContain('Osano');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/osano/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Osano');
      }
    });
  });
