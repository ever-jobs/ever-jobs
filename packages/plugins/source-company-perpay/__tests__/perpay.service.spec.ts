...
import { PerpayModule, PerpayService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'perpay-jobs.json'), 'utf8'),
);
...
/**
 * Spec 644 / T04 — `PerpayService` unit tests (≥ 9 cases).
 */
describe('PerpayService — Spec 644 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through PerpayModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [PerpayModule],
      }).compile();
      const service = moduleRef.get(PerpayService);
      expect(service).toBeInstanceOf(PerpayService);
      await moduleRef.close();
    });

    it('exports the Site.PERPAY = "perpay" enum value', () => {
      expect(Site.PERPAY).toBe('perpay');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new PerpayService();
      const result = await service.scrape({
        siteType: [Site.PERPAY],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'perpay-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.PERPAY);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Perpay');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/perpay/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/perpay/jobs/');
      ...
      expect(job0?.description).toContain('Perpay');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/perpay/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Perpay');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'perpay-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
