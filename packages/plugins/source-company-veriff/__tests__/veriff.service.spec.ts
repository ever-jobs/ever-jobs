...
import { VeriffModule, VeriffService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'veriff-jobs.json'), 'utf8'),
);
...
/**
 * Spec 538 / T04 — `VeriffService` unit tests (≥ 9 cases).
 */
describe('VeriffService — Spec 538 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through VeriffModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [VeriffModule],
      }).compile();
      const service = moduleRef.get(VeriffService);
      expect(service).toBeInstanceOf(VeriffService);
      await moduleRef.close();
    });

    it('exports the Site.VERIFF = "veriff" enum value', () => {
      expect(Site.VERIFF).toBe('veriff');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new VeriffService();
      const result = await service.scrape({
        siteType: [Site.VERIFF],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'veriff-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.VERIFF);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Veriff');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/veriff/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/veriff/jobs/');
      ...
      expect(job0?.description).toContain('Veriff');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/veriff/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Veriff');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'veriff-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
