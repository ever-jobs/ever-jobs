...
import { SafeSecurityModule, SafeSecurityService } from '../src';

const COMPANY_NAME_EXPECT = 'Safe Security';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'safe-jobs.json'), 'utf8'),
);
...
describe('SafeSecurityService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SafeSecurityModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SafeSecurityModule],
      }).compile();
      const service = moduleRef.get(SafeSecurityService);
      expect(service).toBeInstanceOf(SafeSecurityService);
      await moduleRef.close();
    });

    it('exports the Site.SAFE_SECURITY = "safe" enum value', () => {
      expect(Site.SAFE_SECURITY).toBe('safe');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new SafeSecurityService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.SAFE_SECURITY],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('safe');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SAFE_SECURITY);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('safe-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new SafeSecurityService(registry);
      const result = await service.scrape({
        siteType: [Site.SAFE_SECURITY],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('safe');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('safe-x1');
      expect(result.jobs[0].site).toBe(Site.SAFE_SECURITY);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('safe-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new SafeSecurityService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.SAFE_SECURITY],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new SafeSecurityService();
      const result = await service.scrape({
        siteType: [Site.SAFE_SECURITY],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
