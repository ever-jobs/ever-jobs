...
import { ChainalysisModule, ChainalysisService } from '../src';

const COMPANY_NAME_EXPECT = 'Chainalysis';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'chainalysis-jobs.json'), 'utf8'),
);
...
describe('ChainalysisService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ChainalysisModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ChainalysisModule],
      }).compile();
      const service = moduleRef.get(ChainalysisService);
      expect(service).toBeInstanceOf(ChainalysisService);
      await moduleRef.close();
    });

    it('exports the Site.CHAINALYSIS = "chainalysis" enum value', () => {
      expect(Site.CHAINALYSIS).toBe('chainalysis');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new ChainalysisService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.CHAINALYSIS],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'chainalysis-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.CHAINALYSIS);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('chainalysis-' + first.id);
      ...
      expect(calledUrls[0]).toContain('chainalysis-careers');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.CHAINALYSIS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('chainalysis-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new ChainalysisService(registry);
      const result = await service.scrape({
        siteType: [Site.CHAINALYSIS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('chainalysis-careers');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('chainalysis-x1');
      expect(result.jobs[0].site).toBe(Site.CHAINALYSIS);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('chainalysis-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new ChainalysisService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.CHAINALYSIS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new ChainalysisService();
      const result = await service.scrape({
        siteType: [Site.CHAINALYSIS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
