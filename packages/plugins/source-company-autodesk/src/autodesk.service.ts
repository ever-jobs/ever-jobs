...
/**
 * Autodesk — Design software (HQ: San Francisco, CA, USA).
 *
 * Source (Spec 1736): Workday board, scraped in this order:
 *   - `autodesk:1:Ext` — https://autodesk.wd1.myworkdayjobs.com/Ext
 *     verified live 2026-09-24: 378 open postings.
 *
 ...
 * through untouched.
 *
 * Tags: segment=workday-enterprise; industry=design-software.
 */
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'autodesk:1:Ext', atsIdPrefix: 'wd-autodesk-' },
];
...
@SourcePlugin({
  ...
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['autodesk.com'],
  description: 'Autodesk careers via Workday. Tags: segment=workday-enterprise; industry=design-software.',
})
@Injectable()
export class AutodeskService implements IScraper {
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Workday source plugin is not registered; cannot scrape Autodesk');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`Autodesk: delegating to Workday (${board.companySlug})`);
    ...
    this.logger.log(`Autodesk: scraped ${jobs.length} jobs`);
