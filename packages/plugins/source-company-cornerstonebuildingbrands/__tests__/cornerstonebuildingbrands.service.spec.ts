...
import { CornerstoneBuildingBrandsModule, CornerstoneBuildingBrandsService } from '../src';

const COMPANY_NAME_EXPECT = 'Cornerstone Building Brands';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'cornerstonebuildingbrands-jobs.json'), 'utf8'),
);
...
describe('CornerstoneBuildingBrandsService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CornerstoneBuildingBrandsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CornerstoneBuildingBrandsModule],
      }).compile();
      const service = moduleRef.get(CornerstoneBuildingBrandsService);
      expect(service).toBeInstanceOf(CornerstoneBuildingBrandsService);
      await moduleRef.close();
    });

    it('exports the Site.CORNERSTONE_BUILDING_BRANDS = "cornerstonebuildingbrands" enum value', () => {
      expect(Site.CORNERSTONE_BUILDING_BRANDS).toBe('cornerstonebuildingbrands');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new CornerstoneBuildingBrandsService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.CORNERSTONE_BUILDING_BRANDS],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'cornerstonebuildingbrands-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.CORNERSTONE_BUILDING_BRANDS);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('cornerstonebuildingbrands-' + first.id);
      ...
      expect(calledUrls[0]).toContain('cornerstonebuildingbrandscareers');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.CORNERSTONE_BUILDING_BRANDS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('cornerstonebuildingbrands-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new CornerstoneBuildingBrandsService(registry);
      const result = await service.scrape({
        siteType: [Site.CORNERSTONE_BUILDING_BRANDS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('cornerstonebuildingbrandscareers');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('cornerstonebuildingbrands-x1');
      expect(result.jobs[0].site).toBe(Site.CORNERSTONE_BUILDING_BRANDS);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('cornerstonebuildingbrands-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new CornerstoneBuildingBrandsService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.CORNERSTONE_BUILDING_BRANDS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new CornerstoneBuildingBrandsService();
      const result = await service.scrape({
        siteType: [Site.CORNERSTONE_BUILDING_BRANDS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
