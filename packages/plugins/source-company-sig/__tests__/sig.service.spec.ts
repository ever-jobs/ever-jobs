...
import { SigModule, SigService } from '../src';
...
const FIXTURE: Fixture = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'fixtures', 'sig-boards.json'), 'utf8'),
);
...
function registryWith(scraper: IScraper = new IcimsService()): PluginRegistry {
  const registry = new PluginRegistry();
  registry.register({ site: Site.ICIMS, name: 'iCIMS', category: 'ats', isAts: true }, scraper);
  return registry;
}
...
describe('SigService — iCIMS delegation (Spec 1737)', () => {
  beforeEach(() => {
    mockGet.mockReset();
    mockPost.mockReset();
  ...
  describe('registration', () => {
    it('resolves through SigModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({ imports: [SigModule] }).compile();
      expect(moduleRef.get(SigService)).toBeInstanceOf(SigService);
      await moduleRef.close();
    });

    it('exports Site.SIG = "sig"', () => {
      expect(Site.SIG).toBe('sig');
    });

    it('declares a tagged company plugin with its domains', () => {
      const meta = Reflect.getMetadata(SOURCE_PLUGIN_METADATA, SigService);
      expect(meta.site).toBe(Site.SIG);
      expect(meta.category).toBe('company');
      expect(meta.isAts).toBeFalsy();
      expect(meta.companyDomains).toEqual(['sig.com']);
      expect(meta.description).toContain('segment=quant-trading');
    });
  });

  describe('recorded board (real iCIMS adapter, mocked HTTP)', () => {
    it('maps every recorded posting and re-stamps the company identity', async () => {
      const service = new SigService(registryWith());
      const result = await service.scrape({ siteType: [Site.SIG], resultsWanted: 100 } as ScraperInputDto);
    ...
    it('only requests the recorded board URLs', async () => {
      const service = new SigService(registryWith());
      await service.scrape({ siteType: [Site.SIG] } as ScraperInputDto);
    ...
    it('honours resultsWanted=1', async () => {
      const service = new SigService(registryWith());
      const result = await service.scrape({ siteType: [Site.SIG], resultsWanted: 1 } as ScraperInputDto);
    ...
    it('resolves with an empty result when the board is gone (HTTP 404)', async () => {
      ...
      const service = new SigService(registryWith());
      const result = await service.scrape({ siteType: [Site.SIG] } as ScraperInputDto);
      expect(result.jobs).toEqual([]);
      // iCIMS treats a 4xx board as an unknown tenant: a bare empty result.
    });
  });
  ...
  describe('delegation contract', () => {
    it('forwards the board and the caller input untouched', async () => {
      ...
      expect(result.jobs[0].site).toBe(Site.SIG);
    ...
    it('makes no request when resultsWanted is 0', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new SigService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      const result = await service.scrape({ siteType: [Site.SIG], resultsWanted: 0 } as ScraperInputDto);
  ...
  describe('resilience', () => {
    it('reports not_registered when no registry is injected', async () => {
      const result = await new SigService().scrape({ siteType: [Site.SIG] } as ScraperInputDto);
      expect(result.jobs).toEqual([]);
      expect(result.diagnostics?.reason).toBe('not_registered');
      expect(result.diagnostics?.detail).toContain('iCIMS');
    });

    it('reports not_registered when iCIMS is missing from the registry', async () => {
      const result = await new SigService(new PluginRegistry()).scrape({
        siteType: [Site.SIG],
      } as ScraperInputDto);
      expect(result.diagnostics?.reason).toBe('not_registered');
    });
