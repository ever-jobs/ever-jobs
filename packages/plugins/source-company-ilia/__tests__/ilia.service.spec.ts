...
import { IliaDigitalModule, IliaDigitalService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'ilia-jobs.json'), 'utf8'),
);
...
/**
 * Spec 670 / T04 — `IliaDigitalService` unit tests (≥ 9 cases).
 */
describe('IliaDigitalService — Spec 670 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through IliaDigitalModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [IliaDigitalModule],
      }).compile();
      const service = moduleRef.get(IliaDigitalService);
      expect(service).toBeInstanceOf(IliaDigitalService);
      await moduleRef.close();
    });

    it('exports the Site.ILIA_DIGITAL = "ilia" enum value', () => {
      expect(Site.ILIA_DIGITAL).toBe('ilia');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new IliaDigitalService();
      const result = await service.scrape({
        siteType: [Site.ILIA_DIGITAL],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'ilia-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ILIA_DIGITAL);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Ilia Digital');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/ilia/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/ilia/jobs/');
      ...
      expect(job0?.description).toContain('Ilia Digital');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/ilia/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Ilia Digital');
      }
    });
  });
