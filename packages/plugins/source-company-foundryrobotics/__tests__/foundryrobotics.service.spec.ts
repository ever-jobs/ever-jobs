...
import { FoundryRoboticsModule, FoundryRoboticsService } from '../src';

const COMPANY_NAME_EXPECT = 'Foundry Robotics';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'foundryrobotics-jobs.json'), 'utf8'),
);
...
describe('FoundryRoboticsService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through FoundryRoboticsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [FoundryRoboticsModule],
      }).compile();
      const service = moduleRef.get(FoundryRoboticsService);
      expect(service).toBeInstanceOf(FoundryRoboticsService);
      await moduleRef.close();
    });

    it('exports the Site.FOUNDRY_ROBOTICS = "foundryrobotics" enum value', () => {
      expect(Site.FOUNDRY_ROBOTICS).toBe('foundryrobotics');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new FoundryRoboticsService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.FOUNDRY_ROBOTICS],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'foundryrobotics-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.FOUNDRY_ROBOTICS);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('foundryrobotics-' + first.id);
      ...
      expect(calledUrls[0]).toContain('foundry-robotics');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.FOUNDRY_ROBOTICS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('foundryrobotics-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new FoundryRoboticsService(registry);
      const result = await service.scrape({
        siteType: [Site.FOUNDRY_ROBOTICS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('foundry-robotics');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('foundryrobotics-x1');
      expect(result.jobs[0].site).toBe(Site.FOUNDRY_ROBOTICS);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('foundryrobotics-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new FoundryRoboticsService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.FOUNDRY_ROBOTICS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new FoundryRoboticsService();
      const result = await service.scrape({
        siteType: [Site.FOUNDRY_ROBOTICS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
