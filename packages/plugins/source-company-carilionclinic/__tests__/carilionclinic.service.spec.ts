...
import { CarilionClinicModule, CarilionClinicService } from '../src';

const COMPANY_NAME_EXPECT = 'Carilion Clinic';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'carilionclinic-jobs.json'), 'utf8'),
);
...
describe('CarilionClinicService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CarilionClinicModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CarilionClinicModule],
      }).compile();
      const service = moduleRef.get(CarilionClinicService);
      expect(service).toBeInstanceOf(CarilionClinicService);
      await moduleRef.close();
    });

    it('exports the Site.CARILION_CLINIC = "carilionclinic" enum value', () => {
      expect(Site.CARILION_CLINIC).toBe('carilionclinic');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new CarilionClinicService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.CARILION_CLINIC],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'carilionclinic-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.CARILION_CLINIC);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('carilionclinic-' + first.id);
      ...
      expect(calledUrls[0]).toContain('CarilionClinic');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.CARILION_CLINIC);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('carilionclinic-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new CarilionClinicService(registry);
      const result = await service.scrape({
        siteType: [Site.CARILION_CLINIC],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('CarilionClinic');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('carilionclinic-x1');
      expect(result.jobs[0].site).toBe(Site.CARILION_CLINIC);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('carilionclinic-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new CarilionClinicService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.CARILION_CLINIC],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new CarilionClinicService();
      const result = await service.scrape({
        siteType: [Site.CARILION_CLINIC],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
