...
import { OutreachModule, OutreachService } from '../src';

const COMPANY_NAME_EXPECT = 'Outreach';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'outreach-jobs.json'), 'utf8'),
);
...
describe('OutreachService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through OutreachModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [OutreachModule],
      }).compile();
      const service = moduleRef.get(OutreachService);
      expect(service).toBeInstanceOf(OutreachService);
      await moduleRef.close();
    });

    it('exports the Site.OUTREACH = "outreach" enum value', () => {
      expect(Site.OUTREACH).toBe('outreach');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new OutreachService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.OUTREACH],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('outreach');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.OUTREACH);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('outreach-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new OutreachService(registry);
      const result = await service.scrape({
        siteType: [Site.OUTREACH],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('outreach');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('outreach-x1');
      expect(result.jobs[0].site).toBe(Site.OUTREACH);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('outreach-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new OutreachService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.OUTREACH],
      } as ScraperInputDto);
