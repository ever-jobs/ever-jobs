...
import { PikaModule, PikaService } from '../src';

const COMPANY_NAME_EXPECT = 'Pika';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'pika-jobs.json'), 'utf8'),
);
...
describe('PikaService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through PikaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [PikaModule],
      }).compile();
      const service = moduleRef.get(PikaService);
      expect(service).toBeInstanceOf(PikaService);
      await moduleRef.close();
    });

    it('exports the Site.PIKA = "pika" enum value', () => {
      expect(Site.PIKA).toBe('pika');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new PikaService(registryWithAshby());
      ...
      expect(calledUrls[0]).toContain('pika');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.PIKA);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('pika-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new PikaService(registry);
      const result = await service.scrape({
        siteType: [Site.PIKA],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('pika');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('pika-x1');
      expect(result.jobs[0].site).toBe(Site.PIKA);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('pika-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new PikaService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.PIKA],
      } as ScraperInputDto);
