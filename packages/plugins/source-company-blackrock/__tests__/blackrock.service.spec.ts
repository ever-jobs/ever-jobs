...
import { BlackRockModule, BlackRockService } from '../src';
...
const FIXTURE: Fixture = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'fixtures', 'blackrock-boards.json'), 'utf8'),
);
const COMPANY_NAME = 'BlackRock';
const ID_PREFIX = 'blackrock-';
...
describe('BlackRockService — Workday delegation (Spec 1736)', () => {
  beforeEach(() => {
    mockGet.mockReset();
    mockPost.mockReset();
  ...
  describe('registration', () => {
    it('resolves through BlackRockModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({ imports: [BlackRockModule] }).compile();
      expect(moduleRef.get(BlackRockService)).toBeInstanceOf(BlackRockService);
      await moduleRef.close();
    });

    it('exports Site.BLACKROCK = "blackrock"', () => {
      expect(Site.BLACKROCK).toBe('blackrock');
    });

    it('declares a tagged company plugin with its domains', () => {
      const meta = Reflect.getMetadata(SOURCE_PLUGIN_METADATA, BlackRockService);
      expect(meta.site).toBe(Site.BLACKROCK);
      expect(meta.category).toBe('company');
      expect(meta.isAts).toBeFalsy();
      expect(meta.companyDomains).toEqual(['blackrock.com']);
  ...
  describe('recorded board (real Workday adapter, mocked HTTP)', () => {
    it('maps every recorded posting and re-stamps the company identity', async () => {
      const service = new BlackRockService(registryWith());
      const result = await service.scrape({ siteType: [Site.BLACKROCK], resultsWanted: 100 } as ScraperInputDto);
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.BLACKROCK);
    ...
    it('only requests the recorded board URLs', async () => {
      const service = new BlackRockService(registryWith());
      await service.scrape({ siteType: [Site.BLACKROCK] } as ScraperInputDto);
    ...
    it('honours resultsWanted=1', async () => {
      const service = new BlackRockService(registryWith());
      const result = await service.scrape({ siteType: [Site.BLACKROCK], resultsWanted: 1 } as ScraperInputDto);
    ...
    it('resolves with an empty result when the board is gone (HTTP 404)', async () => {
      ...
      const service = new BlackRockService(registryWith());
      const result = await service.scrape({ siteType: [Site.BLACKROCK] } as ScraperInputDto);
  ...
  describe('delegation contract', () => {
    it('forwards the board and the caller input untouched', async () => {
      ...
      const service = new BlackRockService(
        registryWith(
          fakeBackend(
      ...
      expect(result.jobs[0].site).toBe(Site.BLACKROCK);
    ...
    it('never forwards the caller\'s credentials to the board (Spec 1735 §4.5)', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new BlackRockService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
    ...
    it('makes no request when resultsWanted is 0', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new BlackRockService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      const result = await service.scrape({ siteType: [Site.BLACKROCK], resultsWanted: 0 } as ScraperInputDto);
  ...
  describe('resilience', () => {
    it('reports not_registered when no registry is injected', async () => {
      const result = await new BlackRockService().scrape({ siteType: [Site.BLACKROCK] } as ScraperInputDto);
      expect(result.jobs).toEqual([]);
    ...
    it('reports not_registered when Workday is missing from the registry', async () => {
      const result = await new BlackRockService(new PluginRegistry()).scrape({
        siteType: [Site.BLACKROCK],
      } as ScraperInputDto);
      expect(result.diagnostics?.reason).toBe('not_registered');
    });
