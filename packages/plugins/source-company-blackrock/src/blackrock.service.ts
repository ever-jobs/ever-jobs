...
/**
 * BlackRock — Asset management (HQ: New York, NY, USA).
 *
 * Source (Spec 1736): Workday board, scraped in this order:
 *   - `blackrock:1:BlackRock_Professional` — https://blackrock.wd1.myworkdayjobs.com/BlackRock_Professional
 *     verified live 2026-09-24: 352 open postings.
 *
 ...
 * through untouched.
 *
 * Tags: segment=workday-enterprise; industry=asset-management.
 */
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'blackrock:1:BlackRock_Professional', atsIdPrefix: 'wd-blackrock-' },
];
...
@SourcePlugin({
  ...
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['blackrock.com'],
  description: 'BlackRock careers via Workday. Tags: segment=workday-enterprise; industry=asset-management.',
})
@Injectable()
export class BlackRockService implements IScraper {
  private readonly logger = new Logger(BlackRockService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Workday source plugin is not registered; cannot scrape BlackRock');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`BlackRock: delegating to Workday (${board.companySlug})`);
    ...
    this.logger.log(`BlackRock: scraped ${jobs.length} jobs`);
