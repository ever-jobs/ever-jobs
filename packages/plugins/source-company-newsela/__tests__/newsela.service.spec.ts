...
import { NewselaModule, NewselaService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'newsela-jobs.json'), 'utf8'),
);
...
/**
 * Spec 751 / T04 — `NewselaService` unit tests (≥ 9 cases).
 */
describe('NewselaService — Spec 751 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through NewselaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [NewselaModule],
      }).compile();
      const service = moduleRef.get(NewselaService);
      expect(service).toBeInstanceOf(NewselaService);
      await moduleRef.close();
    });

    it('exports the Site.NEWSELA = "newsela" enum value', () => {
      expect(Site.NEWSELA).toBe('newsela');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new NewselaService();
      const result = await service.scrape({
        siteType: [Site.NEWSELA],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'newsela-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.NEWSELA);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Newsela');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/newsela/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/newsela/jobs/');
      ...
      expect(job0?.description).toContain('Newsela');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/newsela/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Newsela');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'newsela-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
