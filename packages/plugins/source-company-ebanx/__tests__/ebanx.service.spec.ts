...
import { EBANXModule, EBANXService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'ebanx-jobs.json'), 'utf8'),
);
...
/**
 * Spec 650 / T04 — `EBANXService` unit tests (≥ 9 cases).
 */
describe('EBANXService — Spec 650 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through EBANXModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [EBANXModule],
      }).compile();
      const service = moduleRef.get(EBANXService);
      expect(service).toBeInstanceOf(EBANXService);
      await moduleRef.close();
    });

    it('exports the Site.EBANX = "ebanx" enum value', () => {
      expect(Site.EBANX).toBe('ebanx');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new EBANXService();
      ...
      const job0 = dto.jobs.find((j) => j.id === 'ebanx-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.EBANX);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('EBANX');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/ebanx/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/ebanx/jobs/');
      ...
      expect(job0?.description).toContain('EBANX');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/ebanx/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('EBANX');
      }
    });
  });
