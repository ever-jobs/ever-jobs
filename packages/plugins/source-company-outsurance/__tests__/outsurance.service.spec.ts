...
import { OUTsuranceModule, OUTsuranceService } from '../src';

const COMPANY_NAME_EXPECT = 'OUTsurance';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'outsurance-jobs.json'), 'utf8'),
);
...
describe('OUTsuranceService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through OUTsuranceModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [OUTsuranceModule],
      }).compile();
      const service = moduleRef.get(OUTsuranceService);
      expect(service).toBeInstanceOf(OUTsuranceService);
      await moduleRef.close();
    });

    it('exports the Site.OUTSURANCE = "outsurance" enum value', () => {
      expect(Site.OUTSURANCE).toBe('outsurance');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new OUTsuranceService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.OUTSURANCE],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('OUTsurance');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.OUTSURANCE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('outsurance-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new OUTsuranceService(registry);
      const result = await service.scrape({
        siteType: [Site.OUTSURANCE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('OUTsurance');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('outsurance-x1');
      expect(result.jobs[0].site).toBe(Site.OUTSURANCE);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('outsurance-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new OUTsuranceService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.OUTSURANCE],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new OUTsuranceService();
      const result = await service.scrape({
        siteType: [Site.OUTSURANCE],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
