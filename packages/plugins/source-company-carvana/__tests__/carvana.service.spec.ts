...
import { CarvanaModule, CarvanaService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'carvana-jobs.json'), 'utf8'),
);
...
/**
 * Spec 532 / T04 — `CarvanaService` unit tests (≥ 9 cases).
 */
describe('CarvanaService — Spec 532 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CarvanaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CarvanaModule],
      }).compile();
      const service = moduleRef.get(CarvanaService);
      expect(service).toBeInstanceOf(CarvanaService);
      await moduleRef.close();
    });

    it('exports the Site.CARVANA = "carvana" enum value', () => {
      expect(Site.CARVANA).toBe('carvana');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new CarvanaService();
      const result = await service.scrape({
        siteType: [Site.CARVANA],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'carvana-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.CARVANA);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Carvana');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/carvana/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/carvana/jobs/');
      ...
      expect(job0?.description).toContain('Carvana');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/carvana/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Carvana');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'carvana-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
