...
import { BybitModule, BybitService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'bybit-jobs.json'), 'utf8'),
);
...
/**
 * Spec 709 / T04 — `BybitService` unit tests (≥ 9 cases).
 */
describe('BybitService — Spec 709 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through BybitModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [BybitModule],
      }).compile();
      const service = moduleRef.get(BybitService);
      expect(service).toBeInstanceOf(BybitService);
      await moduleRef.close();
    });

    it('exports the Site.BYBIT = "bybit" enum value', () => {
      expect(Site.BYBIT).toBe('bybit');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const job0 = dto.jobs.find((j) => j.id === 'bybit-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.BYBIT);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Bybit');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/bybit/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/bybit/jobs/');
      ...
      expect(job0?.description).toContain('Bybit');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/bybit/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Bybit');
      }
    });
  });
