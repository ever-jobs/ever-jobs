...
import { SmileDigitalHealthModule, SmileDigitalHealthService } from '../src';

const COMPANY_NAME_EXPECT = 'Smile Digital Health';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'smiledigitalhealth-jobs.json'), 'utf8'),
);
...
describe('SmileDigitalHealthService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SmileDigitalHealthModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SmileDigitalHealthModule],
      }).compile();
      const service = moduleRef.get(SmileDigitalHealthService);
      expect(service).toBeInstanceOf(SmileDigitalHealthService);
      await moduleRef.close();
    });

    it('exports the Site.SMILE_DIGITAL_HEALTH = "smiledigitalhealth" enum value', () => {
      expect(Site.SMILE_DIGITAL_HEALTH).toBe('smiledigitalhealth');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new SmileDigitalHealthService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.SMILE_DIGITAL_HEALTH],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'smiledigitalhealth-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.SMILE_DIGITAL_HEALTH);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('smiledigitalhealth-' + first.id);
      ...
      expect(calledUrls[0]).toContain('smiledigitalhealth');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SMILE_DIGITAL_HEALTH);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('smiledigitalhealth-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new SmileDigitalHealthService(registry);
      const result = await service.scrape({
        siteType: [Site.SMILE_DIGITAL_HEALTH],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('smiledigitalhealth');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('smiledigitalhealth-x1');
      expect(result.jobs[0].site).toBe(Site.SMILE_DIGITAL_HEALTH);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('smiledigitalhealth-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new SmileDigitalHealthService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.SMILE_DIGITAL_HEALTH],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new SmileDigitalHealthService();
      const result = await service.scrape({
        siteType: [Site.SMILE_DIGITAL_HEALTH],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
