...
import { TigerDataTimescaleModule, TigerDataTimescaleService } from '../src';

const COMPANY_NAME_EXPECT = 'TigerData (Timescale)';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'tigerdatatimescale-jobs.json'), 'utf8'),
);
...
describe('TigerDataTimescaleService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through TigerDataTimescaleModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [TigerDataTimescaleModule],
      }).compile();
      const service = moduleRef.get(TigerDataTimescaleService);
      expect(service).toBeInstanceOf(TigerDataTimescaleService);
      await moduleRef.close();
    });

    it('exports the Site.TIGERDATA_TIMESCALE = "tigerdatatimescale" enum value', () => {
      expect(Site.TIGERDATA_TIMESCALE).toBe('tigerdatatimescale');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new TigerDataTimescaleService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.TIGERDATA_TIMESCALE],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'tigerdatatimescale-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.TIGERDATA_TIMESCALE);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('tigerdatatimescale-' + first.id);
      ...
      expect(calledUrls[0]).toContain('tigerdata');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.TIGERDATA_TIMESCALE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('tigerdatatimescale-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new TigerDataTimescaleService(registry);
      const result = await service.scrape({
        siteType: [Site.TIGERDATA_TIMESCALE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('tigerdata');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('tigerdatatimescale-x1');
      expect(result.jobs[0].site).toBe(Site.TIGERDATA_TIMESCALE);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('tigerdatatimescale-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new TigerDataTimescaleService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.TIGERDATA_TIMESCALE],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new TigerDataTimescaleService();
      const result = await service.scrape({
        siteType: [Site.TIGERDATA_TIMESCALE],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
