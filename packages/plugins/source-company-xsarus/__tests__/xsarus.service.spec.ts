...
import { XSARUSModule, XSARUSService } from '../src';

const COMPANY_NAME_EXPECT = 'XSARUS';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'xsarus-jobs.json'), 'utf8'),
);
...
describe('XSARUSService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through XSARUSModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [XSARUSModule],
      }).compile();
      const service = moduleRef.get(XSARUSService);
      expect(service).toBeInstanceOf(XSARUSService);
      await moduleRef.close();
    });

    it('exports the Site.XSARUS = "xsarus" enum value', () => {
      expect(Site.XSARUS).toBe('xsarus');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new XSARUSService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.XSARUS],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('xsarus.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.XSARUS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('xsarus-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new XSARUSService(registry);
      const result = await service.scrape({
        siteType: [Site.XSARUS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('xsarus');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('xsarus-x1');
      expect(result.jobs[0].site).toBe(Site.XSARUS);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('xsarus-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new XSARUSService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.XSARUS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new XSARUSService();
      const result = await service.scrape({
        siteType: [Site.XSARUS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
