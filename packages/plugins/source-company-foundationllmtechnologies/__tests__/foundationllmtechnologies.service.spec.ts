...
import { FoundationEGIModule, FoundationEGIService } from '../src';

const COMPANY_NAME_EXPECT = 'Foundation EGI';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'foundationllmtechnologies-jobs.json'), 'utf8'),
);
...
describe('FoundationEGIService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through FoundationEGIModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [FoundationEGIModule],
      }).compile();
      const service = moduleRef.get(FoundationEGIService);
      expect(service).toBeInstanceOf(FoundationEGIService);
      await moduleRef.close();
    });

    it('exports the Site.FOUNDATION_EGI = "foundationllmtechnologies" enum value', () => {
      expect(Site.FOUNDATION_EGI).toBe('foundationllmtechnologies');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new FoundationEGIService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.FOUNDATION_EGI],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'foundationllmtechnologies-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.FOUNDATION_EGI);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('foundationllmtechnologies-' + first.id);
      ...
      expect(calledUrls[0]).toContain('foundation-llm-technologies');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.FOUNDATION_EGI);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('foundationllmtechnologies-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new FoundationEGIService(registry);
      const result = await service.scrape({
        siteType: [Site.FOUNDATION_EGI],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('foundation-llm-technologies');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('foundationllmtechnologies-x1');
      expect(result.jobs[0].site).toBe(Site.FOUNDATION_EGI);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('foundationllmtechnologies-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new FoundationEGIService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.FOUNDATION_EGI],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new FoundationEGIService();
      const result = await service.scrape({
        siteType: [Site.FOUNDATION_EGI],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
