...
import { MATHoldingsModule, MATHoldingsService } from '../src';

const COMPANY_NAME_EXPECT = 'MAT Holdings';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'matholdings-jobs.json'), 'utf8'),
);
...
describe('MATHoldingsService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through MATHoldingsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [MATHoldingsModule],
      }).compile();
      const service = moduleRef.get(MATHoldingsService);
      expect(service).toBeInstanceOf(MATHoldingsService);
      await moduleRef.close();
    });

    it('exports the Site.MAT_HOLDINGS = "matholdings" enum value', () => {
      expect(Site.MAT_HOLDINGS).toBe('matholdings');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new MATHoldingsService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.MAT_HOLDINGS],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('MATHoldings');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.MAT_HOLDINGS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('matholdings-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new MATHoldingsService(registry);
      const result = await service.scrape({
        siteType: [Site.MAT_HOLDINGS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('MATHoldings');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('matholdings-x1');
      expect(result.jobs[0].site).toBe(Site.MAT_HOLDINGS);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('matholdings-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new MATHoldingsService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.MAT_HOLDINGS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new MATHoldingsService();
      const result = await service.scrape({
        siteType: [Site.MAT_HOLDINGS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
