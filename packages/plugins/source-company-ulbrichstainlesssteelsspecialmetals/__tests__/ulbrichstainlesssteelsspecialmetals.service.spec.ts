...
import { UlbrichStainlessSteelsSpecialMetalsModule, UlbrichStainlessSteelsSpecialMetalsService } from '../src';

const COMPANY_NAME_EXPECT = 'Ulbrich Stainless Steels & Special Metals';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'ulbrichstainlesssteelsspecialmetals-jobs.json'), 'utf8'),
);
...
describe('UlbrichStainlessSteelsSpecialMetalsService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through UlbrichStainlessSteelsSpecialMetalsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [UlbrichStainlessSteelsSpecialMetalsModule],
      }).compile();
      const service = moduleRef.get(UlbrichStainlessSteelsSpecialMetalsService);
      expect(service).toBeInstanceOf(UlbrichStainlessSteelsSpecialMetalsService);
      await moduleRef.close();
    });

    it('exports the Site.ULBRICH_STAINLESS_STEELS_SPECIAL_METALS = "ulbrichstainlesssteelsspecialmetals" enum value', () => {
      expect(Site.ULBRICH_STAINLESS_STEELS_SPECIAL_METALS).toBe('ulbrichstainlesssteelsspecialmetals');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new UlbrichStainlessSteelsSpecialMetalsService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.ULBRICH_STAINLESS_STEELS_SPECIAL_METALS],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'ulbrichstainlesssteelsspecialmetals-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.ULBRICH_STAINLESS_STEELS_SPECIAL_METALS);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('ulbrichstainlesssteelsspecialmetals-' + first.id);
      ...
      expect(calledUrls[0]).toContain('UlbrichSteel');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ULBRICH_STAINLESS_STEELS_SPECIAL_METALS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('ulbrichstainlesssteelsspecialmetals-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new UlbrichStainlessSteelsSpecialMetalsService(registry);
      const result = await service.scrape({
        siteType: [Site.ULBRICH_STAINLESS_STEELS_SPECIAL_METALS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('UlbrichSteel');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('ulbrichstainlesssteelsspecialmetals-x1');
      expect(result.jobs[0].site).toBe(Site.ULBRICH_STAINLESS_STEELS_SPECIAL_METALS);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('ulbrichstainlesssteelsspecialmetals-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new UlbrichStainlessSteelsSpecialMetalsService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.ULBRICH_STAINLESS_STEELS_SPECIAL_METALS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new UlbrichStainlessSteelsSpecialMetalsService();
      const result = await service.scrape({
        siteType: [Site.ULBRICH_STAINLESS_STEELS_SPECIAL_METALS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
