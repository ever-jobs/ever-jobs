...
import { ApexcompaniescswModule, ApexcompaniescswService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'apexcompaniescsw-jobs.json'), 'utf8'),
);
...
/**
 * Spec 282 / T04 — `ApexcompaniescswService` unit tests (≥ 9 cases).
 */
describe('ApexcompaniescswService — Spec 282 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ApexcompaniescswModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ApexcompaniescswModule],
      }).compile();
      const service = moduleRef.get(ApexcompaniescswService);
      expect(service).toBeInstanceOf(ApexcompaniescswService);
      await moduleRef.close();
    });

    it('exports the Site.APEXCOMPANIESCSW = "apexcompaniescsw" enum value', () => {
      expect(Site.APEXCOMPANIESCSW).toBe('apexcompaniescsw');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ApexcompaniescswService();
      const result = await service.scrape({
        siteType: [Site.APEXCOMPANIESCSW],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'apexcompaniescsw-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.APEXCOMPANIESCSW);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Apex Companies - CSW');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/apexcompaniescsw/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/apexcompaniescsw/jobs/');
      ...
      expect(job0?.description).toContain('Apex Companies - CSW');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/apexcompaniescsw/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Apex Companies - CSW');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'apexcompaniescsw-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
