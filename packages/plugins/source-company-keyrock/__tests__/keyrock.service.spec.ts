...
import { KeyrockModule, KeyrockService } from '../src';

const COMPANY_NAME_EXPECT = 'Keyrock';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'keyrock-jobs.json'), 'utf8'),
);
...
describe('KeyrockService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through KeyrockModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [KeyrockModule],
      }).compile();
      const service = moduleRef.get(KeyrockService);
      expect(service).toBeInstanceOf(KeyrockService);
      await moduleRef.close();
    });

    it('exports the Site.KEYROCK = "keyrock" enum value', () => {
      expect(Site.KEYROCK).toBe('keyrock');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new KeyrockService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.KEYROCK],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('keyrock');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.KEYROCK);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('keyrock-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new KeyrockService(registry);
      const result = await service.scrape({
        siteType: [Site.KEYROCK],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('keyrock');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('keyrock-x1');
      expect(result.jobs[0].site).toBe(Site.KEYROCK);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('keyrock-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new KeyrockService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.KEYROCK],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new KeyrockService();
      const result = await service.scrape({
        siteType: [Site.KEYROCK],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
