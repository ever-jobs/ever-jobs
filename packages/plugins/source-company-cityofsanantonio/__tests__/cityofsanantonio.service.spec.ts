...
import { CityOfSanAntonioModule, CityOfSanAntonioService } from '../src';

const COMPANY_NAME_EXPECT = 'City of San Antonio';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'cityofsanantonio-jobs.json'), 'utf8'),
);
...
describe('CityOfSanAntonioService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CityOfSanAntonioModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CityOfSanAntonioModule],
      }).compile();
      const service = moduleRef.get(CityOfSanAntonioService);
      expect(service).toBeInstanceOf(CityOfSanAntonioService);
      await moduleRef.close();
    });

    it('exports the Site.CITY_OF_SAN_ANTONIO = "cityofsanantonio" enum value', () => {
      expect(Site.CITY_OF_SAN_ANTONIO).toBe('cityofsanantonio');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new CityOfSanAntonioService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.CITY_OF_SAN_ANTONIO],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'cityofsanantonio-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.CITY_OF_SAN_ANTONIO);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('cityofsanantonio-' + first.id);
      ...
      expect(calledUrls[0]).toContain('CityOfSanAntonio');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.CITY_OF_SAN_ANTONIO);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('cityofsanantonio-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new CityOfSanAntonioService(registry);
      const result = await service.scrape({
        siteType: [Site.CITY_OF_SAN_ANTONIO],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('CityOfSanAntonio');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('cityofsanantonio-x1');
      expect(result.jobs[0].site).toBe(Site.CITY_OF_SAN_ANTONIO);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('cityofsanantonio-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new CityOfSanAntonioService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.CITY_OF_SAN_ANTONIO],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new CityOfSanAntonioService();
      const result = await service.scrape({
        siteType: [Site.CITY_OF_SAN_ANTONIO],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
