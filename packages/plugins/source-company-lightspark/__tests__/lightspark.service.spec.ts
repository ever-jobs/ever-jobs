...
import { LightsparkModule, LightsparkService } from '../src';

const COMPANY_NAME_EXPECT = 'Lightspark';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'lightspark-jobs.json'), 'utf8'),
);
...
describe('LightsparkService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through LightsparkModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [LightsparkModule],
      }).compile();
      const service = moduleRef.get(LightsparkService);
      expect(service).toBeInstanceOf(LightsparkService);
      await moduleRef.close();
    });

    it('exports the Site.LIGHTSPARK = "lightspark" enum value', () => {
      expect(Site.LIGHTSPARK).toBe('lightspark');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new LightsparkService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.LIGHTSPARK],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('lightspark');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.LIGHTSPARK);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('lightspark-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new LightsparkService(registry);
      const result = await service.scrape({
        siteType: [Site.LIGHTSPARK],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('lightspark');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('lightspark-x1');
      expect(result.jobs[0].site).toBe(Site.LIGHTSPARK);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('lightspark-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new LightsparkService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.LIGHTSPARK],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new LightsparkService();
      const result = await service.scrape({
        siteType: [Site.LIGHTSPARK],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
