...
/**
 * The Walt Disney Company — Media and entertainment (HQ: Burbank, CA, USA).
 *
 * Source (Spec 1736): Workday board, scraped in this order:
 *   - `disney:5:disneycareer` — https://disney.wd5.myworkdayjobs.com/disneycareer
 *     verified live 2026-09-24: 679 open postings.
 *
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'disney:5:disneycareer', atsIdPrefix: 'wd-disney-' },
];
...
@SourcePlugin({
  ...
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['disney.com', 'thewaltdisneycompany.com'],
  description: 'The Walt Disney Company careers via Workday. Tags: segment=workday-enterprise; industry=media-and-entertainment.',
})
@Injectable()
export class DisneyService implements IScraper {
  private readonly logger = new Logger(DisneyService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Workday source plugin is not registered; cannot scrape The Walt Disney Company');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`The Walt Disney Company: delegating to Workday (${board.companySlug})`);
    ...
    this.logger.log(`The Walt Disney Company: scraped ${jobs.length} jobs`);
