...
import { CityFurnitureModule, CityFurnitureService } from '../src';

const COMPANY_NAME_EXPECT = 'City Furniture';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'cityfurniture-jobs.json'), 'utf8'),
);
...
describe('CityFurnitureService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CityFurnitureModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CityFurnitureModule],
      }).compile();
      const service = moduleRef.get(CityFurnitureService);
      expect(service).toBeInstanceOf(CityFurnitureService);
      await moduleRef.close();
    });

    it('exports the Site.CITY_FURNITURE = "cityfurniture" enum value', () => {
      expect(Site.CITY_FURNITURE).toBe('cityfurniture');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new CityFurnitureService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.CITY_FURNITURE],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'cityfurniture-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.CITY_FURNITURE);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('cityfurniture-' + first.id);
      ...
      expect(calledUrls[0]).toContain('CityFurniture1');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.CITY_FURNITURE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('cityfurniture-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new CityFurnitureService(registry);
      const result = await service.scrape({
        siteType: [Site.CITY_FURNITURE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('CityFurniture1');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('cityfurniture-x1');
      expect(result.jobs[0].site).toBe(Site.CITY_FURNITURE);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('cityfurniture-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new CityFurnitureService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.CITY_FURNITURE],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new CityFurnitureService();
      const result = await service.scrape({
        siteType: [Site.CITY_FURNITURE],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
