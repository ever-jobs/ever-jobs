...
import { RoofstockModule, RoofstockService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'roofstock-jobs.json'), 'utf8'),
);
...
/**
 * Spec 593 / T04 — `RoofstockService` unit tests (≥ 9 cases).
 */
describe('RoofstockService — Spec 593 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through RoofstockModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [RoofstockModule],
      }).compile();
      const service = moduleRef.get(RoofstockService);
      expect(service).toBeInstanceOf(RoofstockService);
      await moduleRef.close();
    });

    it('exports the Site.ROOFSTOCK = "roofstock" enum value', () => {
      expect(Site.ROOFSTOCK).toBe('roofstock');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new RoofstockService();
      const result = await service.scrape({
        siteType: [Site.ROOFSTOCK],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'roofstock-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ROOFSTOCK);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Roofstock');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/roofstock/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/roofstock/jobs/');
      ...
      expect(job0?.description).toContain('Roofstock');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/roofstock/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Roofstock');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'roofstock-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
