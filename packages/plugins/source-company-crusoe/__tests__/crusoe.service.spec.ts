...
import { CrusoeModule, CrusoeService } from '../src';

const COMPANY_NAME_EXPECT = 'Crusoe';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'crusoe-jobs.json'), 'utf8'),
);
...
describe('CrusoeService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CrusoeModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CrusoeModule],
      }).compile();
      const service = moduleRef.get(CrusoeService);
      expect(service).toBeInstanceOf(CrusoeService);
      await moduleRef.close();
    });

    it('exports the Site.CRUSOE = "crusoe" enum value', () => {
      expect(Site.CRUSOE).toBe('crusoe');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new CrusoeService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.CRUSOE],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('crusoe');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.CRUSOE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('crusoe-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new CrusoeService(registry);
      const result = await service.scrape({
        siteType: [Site.CRUSOE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('crusoe');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('crusoe-x1');
      expect(result.jobs[0].site).toBe(Site.CRUSOE);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('crusoe-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new CrusoeService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.CRUSOE],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new CrusoeService();
      const result = await service.scrape({
        siteType: [Site.CRUSOE],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
