...
import { UltraVioletCyberModule, UltraVioletCyberService } from '../src';

const COMPANY_NAME_EXPECT = 'UltraViolet Cyber';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'uvcyber-jobs.json'), 'utf8'),
);
...
describe('UltraVioletCyberService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through UltraVioletCyberModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [UltraVioletCyberModule],
      }).compile();
      const service = moduleRef.get(UltraVioletCyberService);
      expect(service).toBeInstanceOf(UltraVioletCyberService);
      await moduleRef.close();
    });

    it('exports the Site.ULTRAVIOLET_CYBER = "uvcyber" enum value', () => {
      expect(Site.ULTRAVIOLET_CYBER).toBe('uvcyber');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new UltraVioletCyberService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.ULTRAVIOLET_CYBER],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('uvcyber');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ULTRAVIOLET_CYBER);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('uvcyber-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new UltraVioletCyberService(registry);
      const result = await service.scrape({
        siteType: [Site.ULTRAVIOLET_CYBER],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('uvcyber');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('uvcyber-x1');
      expect(result.jobs[0].site).toBe(Site.ULTRAVIOLET_CYBER);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('uvcyber-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new UltraVioletCyberService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.ULTRAVIOLET_CYBER],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new UltraVioletCyberService();
      const result = await service.scrape({
        siteType: [Site.ULTRAVIOLET_CYBER],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
