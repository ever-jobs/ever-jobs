...
import { SambaNovaSystemsModule, SambaNovaSystemsService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'sambanovasystems-jobs.json'), 'utf8'),
);
...
/**
 * Spec 784 / T04 — `SambaNovaSystemsService` unit tests (≥ 9 cases).
 */
describe('SambaNovaSystemsService — Spec 784 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SambaNovaSystemsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SambaNovaSystemsModule],
      }).compile();
      const service = moduleRef.get(SambaNovaSystemsService);
      expect(service).toBeInstanceOf(SambaNovaSystemsService);
      await moduleRef.close();
    });

    it('exports the Site.SAMBANOVA_SYSTEMS = "sambanovasystems" enum value', () => {
      expect(Site.SAMBANOVA_SYSTEMS).toBe('sambanovasystems');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new SambaNovaSystemsService();
      const result = await service.scrape({
        siteType: [Site.SAMBANOVA_SYSTEMS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'sambanovasystems-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.SAMBANOVA_SYSTEMS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('SambaNova Systems');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/sambanovasystems/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/sambanovasystems/jobs/');
      ...
      expect(job0?.description).toContain('SambaNova Systems');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/sambanovasystems/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('SambaNova Systems');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'sambanovasystems-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
