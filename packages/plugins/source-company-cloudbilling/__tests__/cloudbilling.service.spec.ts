...
import { CloudBillingModule, CloudBillingService } from '../src';

const COMPANY_NAME_EXPECT = 'CloudBilling';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'cloudbilling-jobs.json'), 'utf8'),
);
...
describe('CloudBillingService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CloudBillingModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CloudBillingModule],
      }).compile();
      const service = moduleRef.get(CloudBillingService);
      expect(service).toBeInstanceOf(CloudBillingService);
      await moduleRef.close();
    });

    it('exports the Site.CLOUDBILLING = "cloudbilling" enum value', () => {
      expect(Site.CLOUDBILLING).toBe('cloudbilling');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new CloudBillingService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.CLOUDBILLING],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'cloudbilling-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Recruitee's defaults
      expect(job0?.site).toBe(Site.CLOUDBILLING);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('cloudbilling-' + first.id);
      ...
      expect(calledUrls[0]).toContain('cloudbilling.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.CLOUDBILLING);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('cloudbilling-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new CloudBillingService(registry);
      const result = await service.scrape({
        siteType: [Site.CLOUDBILLING],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('cloudbilling');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('cloudbilling-x1');
      expect(result.jobs[0].site).toBe(Site.CLOUDBILLING);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('cloudbilling-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new CloudBillingService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.CLOUDBILLING],
      } as ScraperInputDto);
