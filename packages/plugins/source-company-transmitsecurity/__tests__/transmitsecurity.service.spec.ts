...
import { TransmitSecurityModule, TransmitSecurityService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'transmitsecurity-jobs.json'), 'utf8'),
);
...
/**
 * Spec 959 / T04 — `TransmitSecurityService` unit tests (≥ 9 cases).
 */
describe('TransmitSecurityService — Spec 959 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through TransmitSecurityModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [TransmitSecurityModule],
      }).compile();
      const service = moduleRef.get(TransmitSecurityService);
      expect(service).toBeInstanceOf(TransmitSecurityService);
      await moduleRef.close();
    });

    it('exports the Site.TRANSMIT_SECURITY = "transmitsecurity" enum value', () => {
      expect(Site.TRANSMIT_SECURITY).toBe('transmitsecurity');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new TransmitSecurityService();
      const result = await service.scrape({
        siteType: [Site.TRANSMIT_SECURITY],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'transmitsecurity-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.TRANSMIT_SECURITY);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Transmit Security');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/transmitsecurity/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/transmitsecurity/jobs/');
      ...
      expect(job0?.description).toContain('Transmit Security');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/transmitsecurity/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Transmit Security');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'transmitsecurity-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
