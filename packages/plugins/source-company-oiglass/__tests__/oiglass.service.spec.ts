...
import { OIGlassModule, OIGlassService } from '../src';

const COMPANY_NAME_EXPECT = 'O-I Glass';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'oiglass-jobs.json'), 'utf8'),
);
...
describe('OIGlassService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through OIGlassModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [OIGlassModule],
      }).compile();
      const service = moduleRef.get(OIGlassService);
      expect(service).toBeInstanceOf(OIGlassService);
      await moduleRef.close();
    });

    it('exports the Site.O_I_GLASS = "oiglass" enum value', () => {
      expect(Site.O_I_GLASS).toBe('oiglass');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new OIGlassService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.O_I_GLASS],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('O-I');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.O_I_GLASS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('oiglass-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new OIGlassService(registry);
      const result = await service.scrape({
        siteType: [Site.O_I_GLASS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('O-I');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('oiglass-x1');
      expect(result.jobs[0].site).toBe(Site.O_I_GLASS);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('oiglass-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new OIGlassService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.O_I_GLASS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new OIGlassService();
      const result = await service.scrape({
        siteType: [Site.O_I_GLASS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
