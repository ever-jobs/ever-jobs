...
/**
 * Broadcom — Semiconductors and infrastructure software (HQ: Palo Alto, CA, USA).
 *
 * Source (Spec 1736): Workday board, scraped in this order:
 *   - `broadcom:1:External_Career` — https://broadcom.wd1.myworkdayjobs.com/External_Career
 *     verified live 2026-09-24: 375 open postings.
 *
 ...
 * through untouched.
 *
 * Tags: segment=workday-enterprise; industry=semiconductors-and-infrastructure-software.
 */
...
@SourcePlugin({
  ...
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['broadcom.com'],
  description: 'Broadcom careers via Workday. Tags: segment=workday-enterprise; industry=semiconductors-and-infrastructure-software.',
})
@Injectable()
export class BroadcomService implements IScraper {
  private readonly logger = new Logger(BroadcomService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Workday source plugin is not registered; cannot scrape Broadcom');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`Broadcom: delegating to Workday (${board.companySlug})`);
    ...
    this.logger.log(`Broadcom: scraped ${jobs.length} jobs`);
