...
import { PayJoyModule, PayJoyService } from '../src';

const COMPANY_NAME_EXPECT = 'PayJoy';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'payjoy-jobs.json'), 'utf8'),
);
...
describe('PayJoyService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through PayJoyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [PayJoyModule],
      }).compile();
      const service = moduleRef.get(PayJoyService);
      expect(service).toBeInstanceOf(PayJoyService);
      await moduleRef.close();
    });

    it('exports the Site.PAYJOY = "payjoy" enum value', () => {
      expect(Site.PAYJOY).toBe('payjoy');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new PayJoyService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.PAYJOY],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('payjoy');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.PAYJOY);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('payjoy-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new PayJoyService(registry);
      const result = await service.scrape({
        siteType: [Site.PAYJOY],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('payjoy');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('payjoy-x1');
      expect(result.jobs[0].site).toBe(Site.PAYJOY);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('payjoy-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new PayJoyService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.PAYJOY],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new PayJoyService();
      const result = await service.scrape({
        siteType: [Site.PAYJOY],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
