...
import { NikeModule, NikeService } from '../src';
...
const FIXTURE: Fixture = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'fixtures', 'nike-boards.json'), 'utf8'),
);
const COMPANY_NAME = 'Nike';
const ID_PREFIX = 'nike-';
...
describe('NikeService — Workday delegation (Spec 1736)', () => {
  beforeEach(() => {
    mockGet.mockReset();
    mockPost.mockReset();
  ...
  describe('registration', () => {
    it('resolves through NikeModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({ imports: [NikeModule] }).compile();
      expect(moduleRef.get(NikeService)).toBeInstanceOf(NikeService);
      await moduleRef.close();
    });

    it('exports Site.NIKE = "nike"', () => {
      expect(Site.NIKE).toBe('nike');
    });

    it('declares a tagged company plugin with its domains', () => {
      const meta = Reflect.getMetadata(SOURCE_PLUGIN_METADATA, NikeService);
      expect(meta.site).toBe(Site.NIKE);
      expect(meta.category).toBe('company');
      expect(meta.isAts).toBeFalsy();
      expect(meta.companyDomains).toEqual(['nike.com']);
  ...
  describe('recorded board (real Workday adapter, mocked HTTP)', () => {
    it('maps every recorded posting and re-stamps the company identity', async () => {
      const service = new NikeService(registryWith());
      const result = await service.scrape({ siteType: [Site.NIKE], resultsWanted: 100 } as ScraperInputDto);
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.NIKE);
    ...
    it('only requests the recorded board URLs', async () => {
      const service = new NikeService(registryWith());
      await service.scrape({ siteType: [Site.NIKE] } as ScraperInputDto);
    ...
    it('honours resultsWanted=1', async () => {
      const service = new NikeService(registryWith());
      const result = await service.scrape({ siteType: [Site.NIKE], resultsWanted: 1 } as ScraperInputDto);
    ...
    it('resolves with an empty result when the board is gone (HTTP 404)', async () => {
      ...
      const service = new NikeService(registryWith());
      const result = await service.scrape({ siteType: [Site.NIKE] } as ScraperInputDto);
  ...
  describe('delegation contract', () => {
    it('forwards the board and the caller input untouched', async () => {
      ...
      expect(result.jobs[0].site).toBe(Site.NIKE);
    ...
    it('never forwards the caller\'s credentials to the board (Spec 1735 §4.5)', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new NikeService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
    ...
    it('makes no request when resultsWanted is 0', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new NikeService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      const result = await service.scrape({ siteType: [Site.NIKE], resultsWanted: 0 } as ScraperInputDto);
  ...
  describe('resilience', () => {
    it('reports not_registered when no registry is injected', async () => {
      const result = await new NikeService().scrape({ siteType: [Site.NIKE] } as ScraperInputDto);
      expect(result.jobs).toEqual([]);
    ...
    it('reports not_registered when Workday is missing from the registry', async () => {
      const result = await new NikeService(new PluginRegistry()).scrape({
        siteType: [Site.NIKE],
      } as ScraperInputDto);
      expect(result.diagnostics?.reason).toBe('not_registered');
    });
