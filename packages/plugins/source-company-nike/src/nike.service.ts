...
/**
 * Nike — Apparel and footwear (HQ: Beaverton, OR, USA).
 *
 * Source (Spec 1736): Workday board, scraped in this order:
 *   - `nike:1:nke` — https://nike.wd1.myworkdayjobs.com/nke
 *     verified live 2026-09-24: 837 open postings.
 *
 ...
 * through untouched.
 *
 * Tags: segment=workday-enterprise; industry=apparel-and-footwear.
 */
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'nike:1:nke', atsIdPrefix: 'wd-nike-' },
];
...
@SourcePlugin({
  ...
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['nike.com'],
  description: 'Nike careers via Workday. Tags: segment=workday-enterprise; industry=apparel-and-footwear.',
})
@Injectable()
export class NikeService implements IScraper {
  private readonly logger = new Logger(NikeService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Workday source plugin is not registered; cannot scrape Nike');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`Nike: delegating to Workday (${board.companySlug})`);
    ...
    this.logger.log(`Nike: scraped ${jobs.length} jobs`);
