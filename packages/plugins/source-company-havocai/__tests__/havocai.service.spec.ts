...
import { HavocAIModule, HavocAIService } from '../src';

const COMPANY_NAME_EXPECT = 'Havoc AI';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'havocai-jobs.json'), 'utf8'),
);
...
describe('HavocAIService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through HavocAIModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [HavocAIModule],
      }).compile();
      const service = moduleRef.get(HavocAIService);
      expect(service).toBeInstanceOf(HavocAIService);
      await moduleRef.close();
    });

    it('exports the Site.HAVOC_AI = "havocai" enum value', () => {
      expect(Site.HAVOC_AI).toBe('havocai');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new HavocAIService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.HAVOC_AI],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('havocai');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.HAVOC_AI);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('havocai-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new HavocAIService(registry);
      const result = await service.scrape({
        siteType: [Site.HAVOC_AI],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('havocai');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('havocai-x1');
      expect(result.jobs[0].site).toBe(Site.HAVOC_AI);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('havocai-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new HavocAIService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.HAVOC_AI],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new HavocAIService();
      const result = await service.scrape({
        siteType: [Site.HAVOC_AI],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
