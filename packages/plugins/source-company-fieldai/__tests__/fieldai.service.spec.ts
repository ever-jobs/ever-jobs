...
import { FieldAIModule, FieldAIService } from '../src';

const COMPANY_NAME_EXPECT = 'Field AI';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'fieldai-jobs.json'), 'utf8'),
);
...
describe('FieldAIService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through FieldAIModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [FieldAIModule],
      }).compile();
      const service = moduleRef.get(FieldAIService);
      expect(service).toBeInstanceOf(FieldAIService);
      await moduleRef.close();
    });

    it('exports the Site.FIELD_AI = "fieldai" enum value', () => {
      expect(Site.FIELD_AI).toBe('fieldai');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new FieldAIService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.FIELD_AI],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('field-ai');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.FIELD_AI);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('fieldai-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new FieldAIService(registry);
      const result = await service.scrape({
        siteType: [Site.FIELD_AI],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('field-ai');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('fieldai-x1');
      expect(result.jobs[0].site).toBe(Site.FIELD_AI);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('fieldai-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new FieldAIService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.FIELD_AI],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new FieldAIService();
      const result = await service.scrape({
        siteType: [Site.FIELD_AI],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
