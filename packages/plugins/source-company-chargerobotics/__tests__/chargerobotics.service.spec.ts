...
import { ChargeRoboticsModule, ChargeRoboticsService } from '../src';

const COMPANY_NAME_EXPECT = 'Charge Robotics';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'chargerobotics-jobs.json'), 'utf8'),
);
...
describe('ChargeRoboticsService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ChargeRoboticsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ChargeRoboticsModule],
      }).compile();
      const service = moduleRef.get(ChargeRoboticsService);
      expect(service).toBeInstanceOf(ChargeRoboticsService);
      await moduleRef.close();
    });

    it('exports the Site.CHARGE_ROBOTICS = "chargerobotics" enum value', () => {
      expect(Site.CHARGE_ROBOTICS).toBe('chargerobotics');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new ChargeRoboticsService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.CHARGE_ROBOTICS],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'chargerobotics-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.CHARGE_ROBOTICS);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('chargerobotics-' + first.id);
      ...
      expect(calledUrls[0]).toContain('charge-robotics');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.CHARGE_ROBOTICS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('chargerobotics-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new ChargeRoboticsService(registry);
      const result = await service.scrape({
        siteType: [Site.CHARGE_ROBOTICS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('charge-robotics');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('chargerobotics-x1');
      expect(result.jobs[0].site).toBe(Site.CHARGE_ROBOTICS);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('chargerobotics-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new ChargeRoboticsService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.CHARGE_ROBOTICS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new ChargeRoboticsService();
      const result = await service.scrape({
        siteType: [Site.CHARGE_ROBOTICS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
