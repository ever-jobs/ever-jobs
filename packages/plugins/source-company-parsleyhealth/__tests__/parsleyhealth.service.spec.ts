...
import { ParsleyhealthModule, ParsleyhealthService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'parsleyhealth-jobs.json'), 'utf8'),
);
...
/**
 * Spec 550 / T04 — `ParsleyhealthService` unit tests (≥ 9 cases).
 */
describe('ParsleyhealthService — Spec 550 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ParsleyhealthModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ParsleyhealthModule],
      }).compile();
      const service = moduleRef.get(ParsleyhealthService);
      expect(service).toBeInstanceOf(ParsleyhealthService);
      await moduleRef.close();
    });

    it('exports the Site.PARSLEY_HEALTH = "parsleyhealth" enum value', () => {
      expect(Site.PARSLEY_HEALTH).toBe('parsleyhealth');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ParsleyhealthService();
      const result = await service.scrape({
        siteType: [Site.PARSLEY_HEALTH],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'parsleyhealth-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.PARSLEY_HEALTH);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Parsley Health');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/parsleyhealth/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/parsleyhealth/jobs/');
      ...
      expect(job0?.description).toContain('Parsley Health');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/parsleyhealth/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Parsley Health');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'parsleyhealth-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
