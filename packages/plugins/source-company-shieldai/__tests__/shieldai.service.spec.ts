...
import { ShieldAIModule, ShieldAIService } from '../src';

const COMPANY_NAME_EXPECT = 'Shield AI';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'shieldai-jobs.json'), 'utf8'),
);
...
describe('ShieldAIService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ShieldAIModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ShieldAIModule],
      }).compile();
      const service = moduleRef.get(ShieldAIService);
      expect(service).toBeInstanceOf(ShieldAIService);
      await moduleRef.close();
    });

    it('exports the Site.SHIELD_AI = "shieldai" enum value', () => {
      expect(Site.SHIELD_AI).toBe('shieldai');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new ShieldAIService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.SHIELD_AI],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('shieldai');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SHIELD_AI);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('shieldai-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new ShieldAIService(registry);
      const result = await service.scrape({
        siteType: [Site.SHIELD_AI],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('shieldai');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('shieldai-x1');
      expect(result.jobs[0].site).toBe(Site.SHIELD_AI);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('shieldai-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new ShieldAIService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.SHIELD_AI],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new ShieldAIService();
      const result = await service.scrape({
        siteType: [Site.SHIELD_AI],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
