...
import { GleanModule, GleanService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'gleanwork-jobs.json'), 'utf8'),
);
...
/**
 * Spec 856 / T04 — `GleanService` unit tests (≥ 9 cases).
 */
describe('GleanService — Spec 856 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through GleanModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [GleanModule],
      }).compile();
      const service = moduleRef.get(GleanService);
      expect(service).toBeInstanceOf(GleanService);
      await moduleRef.close();
    });

    it('exports the Site.GLEAN = "gleanwork" enum value', () => {
      expect(Site.GLEAN).toBe('gleanwork');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new GleanService();
      ...
      const job0 = dto.jobs.find((j) => j.id === 'gleanwork-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.GLEAN);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Glean');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/gleanwork/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/gleanwork/jobs/');
      ...
      expect(job0?.description).toContain('Glean');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/gleanwork/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Glean');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'gleanwork-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
