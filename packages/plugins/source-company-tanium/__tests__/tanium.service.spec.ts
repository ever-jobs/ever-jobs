...
import { TaniumModule, TaniumService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'tanium-jobs.json'), 'utf8'),
);
...
/**
 * Spec 542 / T04 — `TaniumService` unit tests (≥ 9 cases).
 */
describe('TaniumService — Spec 542 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through TaniumModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [TaniumModule],
      }).compile();
      const service = moduleRef.get(TaniumService);
      expect(service).toBeInstanceOf(TaniumService);
      await moduleRef.close();
    });

    it('exports the Site.TANIUM = "tanium" enum value', () => {
      expect(Site.TANIUM).toBe('tanium');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new TaniumService();
      const result = await service.scrape({
        siteType: [Site.TANIUM],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'tanium-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.TANIUM);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Tanium');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/tanium/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/tanium/jobs/');
      ...
      expect(job0?.description).toContain('Tanium');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/tanium/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Tanium');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'tanium-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
