...
import { GlobalLendingServicesModule, GlobalLendingServicesService } from '../src';

const COMPANY_NAME_EXPECT = 'Global Lending Services';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'glsllc-jobs.json'), 'utf8'),
);
...
describe('GlobalLendingServicesService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through GlobalLendingServicesModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [GlobalLendingServicesModule],
      }).compile();
      const service = moduleRef.get(GlobalLendingServicesService);
      expect(service).toBeInstanceOf(GlobalLendingServicesService);
      await moduleRef.close();
    });

    it('exports the Site.GLOBAL_LENDING_SERVICES = "glsllc" enum value', () => {
      expect(Site.GLOBAL_LENDING_SERVICES).toBe('glsllc');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new GlobalLendingServicesService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.GLOBAL_LENDING_SERVICES],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'glsllc-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.GLOBAL_LENDING_SERVICES);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('glsllc-' + first.id);
      ...
      expect(calledUrls[0]).toContain('glsllc');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.GLOBAL_LENDING_SERVICES);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('glsllc-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new GlobalLendingServicesService(registry);
      const result = await service.scrape({
        siteType: [Site.GLOBAL_LENDING_SERVICES],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('glsllc');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('glsllc-x1');
      expect(result.jobs[0].site).toBe(Site.GLOBAL_LENDING_SERVICES);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('glsllc-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new GlobalLendingServicesService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.GLOBAL_LENDING_SERVICES],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new GlobalLendingServicesService();
      const result = await service.scrape({
        siteType: [Site.GLOBAL_LENDING_SERVICES],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
