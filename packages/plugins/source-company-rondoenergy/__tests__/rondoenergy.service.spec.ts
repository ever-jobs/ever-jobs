...
import { RondoEnergyModule, RondoEnergyService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'rondoenergy-jobs.json'), 'utf8'),
);
...
/**
 * Spec 661 / T04 — `RondoEnergyService` unit tests (≥ 9 cases).
 */
describe('RondoEnergyService — Spec 661 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through RondoEnergyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [RondoEnergyModule],
      }).compile();
      const service = moduleRef.get(RondoEnergyService);
      expect(service).toBeInstanceOf(RondoEnergyService);
      await moduleRef.close();
    });

    it('exports the Site.RONDO_ENERGY = "rondoenergy" enum value', () => {
      expect(Site.RONDO_ENERGY).toBe('rondoenergy');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new RondoEnergyService();
      const result = await service.scrape({
        siteType: [Site.RONDO_ENERGY],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'rondoenergy-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.RONDO_ENERGY);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Rondo Energy');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/rondoenergy/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/rondoenergy/jobs/');
      ...
      expect(job0?.description).toContain('Rondo Energy');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/rondoenergy/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Rondo Energy');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'rondoenergy-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
