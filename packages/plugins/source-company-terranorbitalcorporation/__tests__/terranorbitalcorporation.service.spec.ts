...
import { TerranOrbitalModule, TerranOrbitalService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'terranorbitalcorporation-jobs.json'), 'utf8'),
);
...
/**
 * Spec 951 / T04 — `TerranOrbitalService` unit tests (≥ 9 cases).
 */
describe('TerranOrbitalService — Spec 951 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through TerranOrbitalModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [TerranOrbitalModule],
      }).compile();
      const service = moduleRef.get(TerranOrbitalService);
      expect(service).toBeInstanceOf(TerranOrbitalService);
      await moduleRef.close();
    });

    it('exports the Site.TERRAN_ORBITAL = "terranorbitalcorporation" enum value', () => {
      expect(Site.TERRAN_ORBITAL).toBe('terranorbitalcorporation');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new TerranOrbitalService();
      const result = await service.scrape({
        siteType: [Site.TERRAN_ORBITAL],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'terranorbitalcorporation-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.TERRAN_ORBITAL);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Terran Orbital');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/terranorbitalcorporation/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/terranorbitalcorporation/jobs/');
      ...
      expect(job0?.description).toContain('Terran Orbital');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/terranorbitalcorporation/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Terran Orbital');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'terranorbitalcorporation-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
