...
import { Project44Module, Project44Service } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'project44-jobs.json'), 'utf8'),
);
...
/**
 * Spec 452 / T04 — `Project44Service` unit tests (≥ 9 cases).
 */
describe('Project44Service — Spec 452 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through Project44Module via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [Project44Module],
      }).compile();
      const service = moduleRef.get(Project44Service);
      expect(service).toBeInstanceOf(Project44Service);
      await moduleRef.close();
    });

    it('exports the Site.PROJECT44 = "project44" enum value', () => {
      expect(Site.PROJECT44).toBe('project44');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new Project44Service();
      const result = await service.scrape({
        siteType: [Site.PROJECT44],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'project44-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.PROJECT44);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('project44');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/project44/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/project44/jobs/');
      ...
      expect(job0?.description).toContain('project44');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/project44/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('project44');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'project44-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
