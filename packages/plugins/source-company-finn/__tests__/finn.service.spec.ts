...
import { FINNModule, FINNService } from '../src';

const COMPANY_NAME_EXPECT = 'FINN';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'finn-jobs.json'), 'utf8'),
);
...
describe('FINNService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through FINNModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [FINNModule],
      }).compile();
      const service = moduleRef.get(FINNService);
      expect(service).toBeInstanceOf(FINNService);
      await moduleRef.close();
    });

    it('exports the Site.FINN = "finn" enum value', () => {
      expect(Site.FINN).toBe('finn');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new FINNService(registryWithLever());
      ...
      expect(calledUrls[0]).toContain('finn');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.FINN);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('finn-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new FINNService(registry);
      const result = await service.scrape({
        siteType: [Site.FINN],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('finn');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('finn-x1');
      expect(result.jobs[0].site).toBe(Site.FINN);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('finn-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new FINNService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.FINN],
      } as ScraperInputDto);
