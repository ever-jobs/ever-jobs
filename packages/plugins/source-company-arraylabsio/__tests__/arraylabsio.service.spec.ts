...
import { ArrayLabsModule, ArrayLabsService } from '../src';

const COMPANY_NAME_EXPECT = 'Array Labs';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'arraylabsio-jobs.json'), 'utf8'),
);
...
describe('ArrayLabsService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ArrayLabsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ArrayLabsModule],
      }).compile();
      const service = moduleRef.get(ArrayLabsService);
      expect(service).toBeInstanceOf(ArrayLabsService);
      await moduleRef.close();
    });

    it('exports the Site.ARRAY_LABS = "arraylabsio" enum value', () => {
      expect(Site.ARRAY_LABS).toBe('arraylabsio');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new ArrayLabsService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.ARRAY_LABS],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'arraylabsio-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.ARRAY_LABS);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('arraylabsio-' + first.id);
      ...
      expect(calledUrls[0]).toContain('arraylabs.io');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ARRAY_LABS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('arraylabsio-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new ArrayLabsService(registry);
      const result = await service.scrape({
        siteType: [Site.ARRAY_LABS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('arraylabs.io');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('arraylabsio-x1');
      expect(result.jobs[0].site).toBe(Site.ARRAY_LABS);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('arraylabsio-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new ArrayLabsService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.ARRAY_LABS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new ArrayLabsService();
      const result = await service.scrape({
        siteType: [Site.ARRAY_LABS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
