...
import { WriterModule, WriterService } from '../src';

const COMPANY_NAME_EXPECT = 'Writer';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'writer-jobs.json'), 'utf8'),
);
...
describe('WriterService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through WriterModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [WriterModule],
      }).compile();
      const service = moduleRef.get(WriterService);
      expect(service).toBeInstanceOf(WriterService);
      await moduleRef.close();
    });

    it('exports the Site.WRITER = "writer" enum value', () => {
      expect(Site.WRITER).toBe('writer');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new WriterService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.WRITER],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('writer');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.WRITER);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('writer-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new WriterService(registry);
      const result = await service.scrape({
        siteType: [Site.WRITER],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('writer');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('writer-x1');
      expect(result.jobs[0].site).toBe(Site.WRITER);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('writer-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new WriterService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.WRITER],
      } as ScraperInputDto);
