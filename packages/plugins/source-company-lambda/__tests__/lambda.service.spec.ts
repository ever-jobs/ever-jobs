...
import { LambdaModule, LambdaService } from '../src';

const COMPANY_NAME_EXPECT = 'Lambda';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'lambda-jobs.json'), 'utf8'),
);
...
describe('LambdaService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through LambdaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [LambdaModule],
      }).compile();
      const service = moduleRef.get(LambdaService);
      expect(service).toBeInstanceOf(LambdaService);
      await moduleRef.close();
    });

    it('exports the Site.LAMBDA = "lambda" enum value', () => {
      expect(Site.LAMBDA).toBe('lambda');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new LambdaService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.LAMBDA],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('lambda');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.LAMBDA);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('lambda-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new LambdaService(registry);
      const result = await service.scrape({
        siteType: [Site.LAMBDA],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('lambda');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('lambda-x1');
      expect(result.jobs[0].site).toBe(Site.LAMBDA);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('lambda-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new LambdaService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.LAMBDA],
      } as ScraperInputDto);
