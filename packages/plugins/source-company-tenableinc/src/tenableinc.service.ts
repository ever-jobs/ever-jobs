 ...
 *
 * Tenable is a company operating in Vulnerability & exposure
 * management, headquartered in Columbia, Maryland, USA. This source
 * plugin ingests live open roles published on Tenable's official
 ...
 *
 * Sector: Vulnerability & exposure management. HQ: Columbia, Maryland, USA.
 *
 * Highlights:
 *   - Sector: Vulnerability & exposure management
 ...
 *     Jobs schema
 *
 * Source profile (Spec 950):
 *   - D-04 — Greenhouse canonical hosted-board host (variant 2):
 *     `https://job-boards.greenhouse.io/tenableinc/jobs/<id>`.
 *   - D-08 — entity-decode-then-tag-strip description pipeline.
 *   - D-09 — wire `company_name` pass-through (`'Tenable'`).
 ...
 */
const API_URL = 'https://api.greenhouse.io/v1/boards/tenableinc/jobs';
...
@Injectable()
export class TenableService implements IScraper {
  private readonly logger = new Logger(TenableService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    try {
      ...
      this.logger.log(`Tenable: fetching ${url}`);
      ...
      for (const listing of listings) {
        ...
        jobs.push(
          new JobPostDto({
            ...
            companyName: listing.company_name ?? 'Tenable',
      ...
      this.logger.log(`Tenable: scraped ${jobs.length} jobs`);
    } catch (err: any) {
      this.logger.error(`Tenable scrape failed: ${err.message}`);
