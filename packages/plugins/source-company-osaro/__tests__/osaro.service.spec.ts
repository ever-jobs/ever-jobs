...
import { OSAROModule, OSAROService } from '../src';

const COMPANY_NAME_EXPECT = 'OSARO';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'osaro-jobs.json'), 'utf8'),
);
...
describe('OSAROService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through OSAROModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [OSAROModule],
      }).compile();
      const service = moduleRef.get(OSAROService);
      expect(service).toBeInstanceOf(OSAROService);
      await moduleRef.close();
    });

    it('exports the Site.OSARO = "osaro" enum value', () => {
      expect(Site.OSARO).toBe('osaro');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new OSAROService(registryWithLever());
      ...
      expect(calledUrls[0]).toContain('osaro');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.OSARO);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('osaro-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new OSAROService(registry);
      const result = await service.scrape({
        siteType: [Site.OSARO],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('osaro');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('osaro-x1');
      expect(result.jobs[0].site).toBe(Site.OSARO);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('osaro-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new OSAROService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.OSARO],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new OSAROService();
      const result = await service.scrape({
        siteType: [Site.OSARO],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
