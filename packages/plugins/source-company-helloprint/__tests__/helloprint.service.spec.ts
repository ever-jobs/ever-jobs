...
import { HelloprintModule, HelloprintService } from '../src';

const COMPANY_NAME_EXPECT = 'Helloprint';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'helloprint-jobs.json'), 'utf8'),
);
...
describe('HelloprintService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through HelloprintModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [HelloprintModule],
      }).compile();
      const service = moduleRef.get(HelloprintService);
      expect(service).toBeInstanceOf(HelloprintService);
      await moduleRef.close();
    });

    it('exports the Site.HELLOPRINT = "helloprint" enum value', () => {
      expect(Site.HELLOPRINT).toBe('helloprint');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new HelloprintService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.HELLOPRINT],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('helloprint.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.HELLOPRINT);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('helloprint-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new HelloprintService(registry);
      const result = await service.scrape({
        siteType: [Site.HELLOPRINT],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('helloprint');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('helloprint-x1');
      expect(result.jobs[0].site).toBe(Site.HELLOPRINT);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('helloprint-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new HelloprintService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.HELLOPRINT],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new HelloprintService();
      const result = await service.scrape({
        siteType: [Site.HELLOPRINT],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
