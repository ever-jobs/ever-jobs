...
import { StandardBotsModule, StandardBotsService } from '../src';

const COMPANY_NAME_EXPECT = 'Standard Bots';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'standardbots-jobs.json'), 'utf8'),
);
...
describe('StandardBotsService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through StandardBotsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [StandardBotsModule],
      }).compile();
      const service = moduleRef.get(StandardBotsService);
      expect(service).toBeInstanceOf(StandardBotsService);
      await moduleRef.close();
    });

    it('exports the Site.STANDARD_BOTS = "standardbots" enum value', () => {
      expect(Site.STANDARD_BOTS).toBe('standardbots');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new StandardBotsService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.STANDARD_BOTS],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'standardbots-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.STANDARD_BOTS);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('standardbots-' + first.id);
      ...
      expect(calledUrls[0]).toContain('standardbots');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.STANDARD_BOTS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('standardbots-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new StandardBotsService(registry);
      const result = await service.scrape({
        siteType: [Site.STANDARD_BOTS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('standardbots');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('standardbots-x1');
      expect(result.jobs[0].site).toBe(Site.STANDARD_BOTS);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('standardbots-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new StandardBotsService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.STANDARD_BOTS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new StandardBotsService();
      const result = await service.scrape({
        siteType: [Site.STANDARD_BOTS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
