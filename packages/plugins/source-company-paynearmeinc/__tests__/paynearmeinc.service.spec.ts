...
import { PayNearMeModule, PayNearMeService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'paynearmeinc-jobs.json'), 'utf8'),
);
...
/**
 * Spec 909 / T04 — `PayNearMeService` unit tests (≥ 9 cases).
 */
describe('PayNearMeService — Spec 909 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through PayNearMeModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [PayNearMeModule],
      }).compile();
      const service = moduleRef.get(PayNearMeService);
      expect(service).toBeInstanceOf(PayNearMeService);
      await moduleRef.close();
    });

    it('exports the Site.PAYNEARME = "paynearmeinc" enum value', () => {
      expect(Site.PAYNEARME).toBe('paynearmeinc');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new PayNearMeService();
      const result = await service.scrape({
        siteType: [Site.PAYNEARME],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'paynearmeinc-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.PAYNEARME);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('PayNearMe');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/paynearmeinc/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/paynearmeinc/jobs/');
      ...
      expect(job0?.description).toContain('PayNearMe');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/paynearmeinc/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('PayNearMe');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'paynearmeinc-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
