...
import { UpgradeModule, UpgradeService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'upgrade-jobs.json'), 'utf8'),
);
...
/**
 * Spec 613 / T04 — `UpgradeService` unit tests (≥ 9 cases).
 */
describe('UpgradeService — Spec 613 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through UpgradeModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [UpgradeModule],
      }).compile();
      const service = moduleRef.get(UpgradeService);
      expect(service).toBeInstanceOf(UpgradeService);
      await moduleRef.close();
    });

    it('exports the Site.UPGRADE = "upgrade" enum value', () => {
      expect(Site.UPGRADE).toBe('upgrade');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const job0 = dto.jobs.find((j) => j.id === 'upgrade-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.UPGRADE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Upgrade');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/upgrade/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/upgrade/jobs/');
      ...
      expect(job0?.description).toContain('Upgrade');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/upgrade/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Upgrade');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'upgrade-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
