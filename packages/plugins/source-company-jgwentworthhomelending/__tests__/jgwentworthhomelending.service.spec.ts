...
import { JGWentworthHomeLendingModule, JGWentworthHomeLendingService } from '../src';

const COMPANY_NAME_EXPECT = 'JG Wentworth Home Lending';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'jgwentworthhomelending-jobs.json'), 'utf8'),
);
...
describe('JGWentworthHomeLendingService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through JGWentworthHomeLendingModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [JGWentworthHomeLendingModule],
      }).compile();
      const service = moduleRef.get(JGWentworthHomeLendingService);
      expect(service).toBeInstanceOf(JGWentworthHomeLendingService);
      await moduleRef.close();
    });

    it('exports the Site.JG_WENTWORTH_HOME_LENDING = "jgwentworthhomelending" enum value', () => {
      expect(Site.JG_WENTWORTH_HOME_LENDING).toBe('jgwentworthhomelending');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new JGWentworthHomeLendingService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.JG_WENTWORTH_HOME_LENDING],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'jgwentworthhomelending-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.JG_WENTWORTH_HOME_LENDING);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('jgwentworthhomelending-' + first.id);
      ...
      expect(calledUrls[0]).toContain('JGWentworthHomeLendingLLC');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.JG_WENTWORTH_HOME_LENDING);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('jgwentworthhomelending-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new JGWentworthHomeLendingService(registry);
      const result = await service.scrape({
        siteType: [Site.JG_WENTWORTH_HOME_LENDING],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('JGWentworthHomeLendingLLC');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('jgwentworthhomelending-x1');
      expect(result.jobs[0].site).toBe(Site.JG_WENTWORTH_HOME_LENDING);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('jgwentworthhomelending-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new JGWentworthHomeLendingService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.JG_WENTWORTH_HOME_LENDING],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new JGWentworthHomeLendingService();
      const result = await service.scrape({
        siteType: [Site.JG_WENTWORTH_HOME_LENDING],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
