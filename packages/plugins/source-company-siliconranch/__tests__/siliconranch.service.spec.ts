...
import { SiliconRanchModule, SiliconRanchService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'siliconranch-jobs.json'), 'utf8'),
);
...
/**
 * Spec 662 / T04 — `SiliconRanchService` unit tests (≥ 9 cases).
 */
describe('SiliconRanchService — Spec 662 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SiliconRanchModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SiliconRanchModule],
      }).compile();
      const service = moduleRef.get(SiliconRanchService);
      expect(service).toBeInstanceOf(SiliconRanchService);
      await moduleRef.close();
    });

    it('exports the Site.SILICON_RANCH = "siliconranch" enum value', () => {
      expect(Site.SILICON_RANCH).toBe('siliconranch');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new SiliconRanchService();
      const result = await service.scrape({
        siteType: [Site.SILICON_RANCH],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'siliconranch-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.SILICON_RANCH);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Silicon Ranch');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/siliconranch/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/siliconranch/jobs/');
      ...
      expect(job0?.description).toContain('Silicon Ranch');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/siliconranch/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Silicon Ranch');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'siliconranch-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
