...
import { AfreshModule, AfreshService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'afresh-jobs.json'), 'utf8'),
);
...
/**
 * Spec 202 / T04 — `AfreshService` unit tests (≥ 9 cases).
 */
describe('AfreshService — Spec 202 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AfreshModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AfreshModule],
      }).compile();
      const service = moduleRef.get(AfreshService);
      expect(service).toBeInstanceOf(AfreshService);
      await moduleRef.close();
    });

    it('exports the Site.AFRESH = "afresh" enum value', () => {
      expect(Site.AFRESH).toBe('afresh');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AfreshService();
      const result = await service.scrape({
        siteType: [Site.AFRESH],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'afresh-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.AFRESH);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Afresh');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/afresh/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/afresh/jobs/');
      ...
      expect(job0?.description).toContain('Afresh');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/afresh/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Afresh');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'afresh-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
