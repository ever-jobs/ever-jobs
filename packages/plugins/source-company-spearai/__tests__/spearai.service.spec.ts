...
import { SpearAIModule, SpearAIService } from '../src';

const COMPANY_NAME_EXPECT = 'Spear AI';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'spearai-jobs.json'), 'utf8'),
);
...
describe('SpearAIService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SpearAIModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SpearAIModule],
      }).compile();
      const service = moduleRef.get(SpearAIService);
      expect(service).toBeInstanceOf(SpearAIService);
      await moduleRef.close();
    });

    it('exports the Site.SPEAR_AI = "spearai" enum value', () => {
      expect(Site.SPEAR_AI).toBe('spearai');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new SpearAIService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.SPEAR_AI],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('spear-ai');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SPEAR_AI);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('spearai-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new SpearAIService(registry);
      const result = await service.scrape({
        siteType: [Site.SPEAR_AI],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('spear-ai');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('spearai-x1');
      expect(result.jobs[0].site).toBe(Site.SPEAR_AI);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('spearai-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new SpearAIService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.SPEAR_AI],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new SpearAIService();
      const result = await service.scrape({
        siteType: [Site.SPEAR_AI],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
