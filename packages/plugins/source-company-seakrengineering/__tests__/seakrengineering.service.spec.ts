...
import { SEAKREngineeringModule, SEAKREngineeringService } from '../src';

const COMPANY_NAME_EXPECT = 'SEAKR Engineering';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'seakrengineering-jobs.json'), 'utf8'),
);
...
describe('SEAKREngineeringService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SEAKREngineeringModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SEAKREngineeringModule],
      }).compile();
      const service = moduleRef.get(SEAKREngineeringService);
      expect(service).toBeInstanceOf(SEAKREngineeringService);
      await moduleRef.close();
    });

    it('exports the Site.SEAKR_ENGINEERING = "seakrengineering" enum value', () => {
      expect(Site.SEAKR_ENGINEERING).toBe('seakrengineering');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new SEAKREngineeringService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.SEAKR_ENGINEERING],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'seakrengineering-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.SEAKR_ENGINEERING);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('seakrengineering-' + first.id);
      ...
      expect(calledUrls[0]).toContain('SEAKREngineering');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SEAKR_ENGINEERING);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('seakrengineering-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new SEAKREngineeringService(registry);
      const result = await service.scrape({
        siteType: [Site.SEAKR_ENGINEERING],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('SEAKREngineering');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('seakrengineering-x1');
      expect(result.jobs[0].site).toBe(Site.SEAKR_ENGINEERING);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('seakrengineering-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new SEAKREngineeringService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.SEAKR_ENGINEERING],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new SEAKREngineeringService();
      const result = await service.scrape({
        siteType: [Site.SEAKR_ENGINEERING],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
