...
import { ShawSModule, ShawSService } from '../src';

const COMPANY_NAME_EXPECT = 'Shaw\'s';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'shaws-jobs.json'), 'utf8'),
);
...
describe('ShawSService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ShawSModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ShawSModule],
      }).compile();
      const service = moduleRef.get(ShawSService);
      expect(service).toBeInstanceOf(ShawSService);
      await moduleRef.close();
    });

    it('exports the Site.SHAW_S = "shaws" enum value', () => {
      expect(Site.SHAW_S).toBe('shaws');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new ShawSService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.SHAW_S],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('Shaws');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SHAW_S);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('shaws-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new ShawSService(registry);
      const result = await service.scrape({
        siteType: [Site.SHAW_S],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('Shaws');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('shaws-x1');
      expect(result.jobs[0].site).toBe(Site.SHAW_S);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('shaws-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new ShawSService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.SHAW_S],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new ShawSService();
      const result = await service.scrape({
        siteType: [Site.SHAW_S],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
