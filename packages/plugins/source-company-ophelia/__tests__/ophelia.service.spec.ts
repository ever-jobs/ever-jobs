...
import { OpheliaHealthModule, OpheliaHealthService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'ophelia-jobs.json'), 'utf8'),
);
...
/**
 * Spec 766 / T04 — `OpheliaHealthService` unit tests (≥ 9 cases).
 */
describe('OpheliaHealthService — Spec 766 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through OpheliaHealthModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [OpheliaHealthModule],
      }).compile();
      const service = moduleRef.get(OpheliaHealthService);
      expect(service).toBeInstanceOf(OpheliaHealthService);
      await moduleRef.close();
    });

    it('exports the Site.OPHELIA_HEALTH = "ophelia" enum value', () => {
      expect(Site.OPHELIA_HEALTH).toBe('ophelia');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new OpheliaHealthService();
      const result = await service.scrape({
        siteType: [Site.OPHELIA_HEALTH],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'ophelia-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.OPHELIA_HEALTH);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Ophelia Health');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/ophelia/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/ophelia/jobs/');
      ...
      expect(job0?.description).toContain('Ophelia Health');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/ophelia/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Ophelia Health');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'ophelia-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
