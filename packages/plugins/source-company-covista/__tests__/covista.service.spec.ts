...
import { CovistaModule, CovistaService } from '../src';

const COMPANY_NAME_EXPECT = 'Covista';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'covista-jobs.json'), 'utf8'),
);
...
describe('CovistaService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CovistaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CovistaModule],
      }).compile();
      const service = moduleRef.get(CovistaService);
      expect(service).toBeInstanceOf(CovistaService);
      await moduleRef.close();
    });

    it('exports the Site.COVISTA = "covista" enum value', () => {
      expect(Site.COVISTA).toBe('covista');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new CovistaService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.COVISTA],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('Covista');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.COVISTA);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('covista-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new CovistaService(registry);
      const result = await service.scrape({
        siteType: [Site.COVISTA],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('Covista');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('covista-x1');
      expect(result.jobs[0].site).toBe(Site.COVISTA);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('covista-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new CovistaService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.COVISTA],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new CovistaService();
      const result = await service.scrape({
        siteType: [Site.COVISTA],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
