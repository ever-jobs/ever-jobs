...
import { YugabyteModule, YugabyteService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'yugabyte-jobs.json'), 'utf8'),
);
...
/**
 * Spec 440 / T04 — `YugabyteService` unit tests (≥ 9 cases).
 */
describe('YugabyteService — Spec 440 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through YugabyteModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [YugabyteModule],
      }).compile();
      const service = moduleRef.get(YugabyteService);
      expect(service).toBeInstanceOf(YugabyteService);
      await moduleRef.close();
    });

    it('exports the Site.YUGABYTE = "yugabyte" enum value', () => {
      expect(Site.YUGABYTE).toBe('yugabyte');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new YugabyteService();
      const result = await service.scrape({
        siteType: [Site.YUGABYTE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'yugabyte-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.YUGABYTE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('YugabyteDB');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/yugabyte/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/yugabyte/jobs/');
      ...
      expect(job0?.description).toContain('YugabyteDB');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/yugabyte/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('YugabyteDB');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'yugabyte-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
