...
import { ParloaModule, ParloaService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'parloa-jobs.json'), 'utf8'),
);
...
/**
 * Spec 729 / T04 — `ParloaService` unit tests (≥ 9 cases).
 */
describe('ParloaService — Spec 729 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ParloaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ParloaModule],
      }).compile();
      const service = moduleRef.get(ParloaService);
      expect(service).toBeInstanceOf(ParloaService);
      await moduleRef.close();
    });

    it('exports the Site.PARLOA = "parloa" enum value', () => {
      expect(Site.PARLOA).toBe('parloa');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ParloaService();
      const result = await service.scrape({
        siteType: [Site.PARLOA],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'parloa-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.PARLOA);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Parloa');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/parloa/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/parloa/jobs/');
      ...
      expect(job0?.description).toContain('Parloa');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/parloa/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Parloa');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'parloa-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
