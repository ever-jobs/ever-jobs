...
import { ProveModule, ProveService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'prove-jobs.json'), 'utf8'),
);
...
/**
 * Spec 712 / T04 — `ProveService` unit tests (≥ 9 cases).
 */
describe('ProveService — Spec 712 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ProveModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ProveModule],
      }).compile();
      const service = moduleRef.get(ProveService);
      expect(service).toBeInstanceOf(ProveService);
      await moduleRef.close();
    });

    it('exports the Site.PROVE = "prove" enum value', () => {
      expect(Site.PROVE).toBe('prove');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ProveService();
      ...
      const job0 = dto.jobs.find((j) => j.id === 'prove-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.PROVE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Prove');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/prove/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/prove/jobs/');
      ...
      expect(job0?.description).toContain('Prove');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/prove/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Prove');
      }
    });
  });
