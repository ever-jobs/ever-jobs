...
import { NewLimitModule, NewLimitService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'newlimit-jobs.json'), 'utf8'),
);
...
/**
 * Spec 689 / T04 — `NewLimitService` unit tests (≥ 9 cases).
 */
describe('NewLimitService — Spec 689 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through NewLimitModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [NewLimitModule],
      }).compile();
      const service = moduleRef.get(NewLimitService);
      expect(service).toBeInstanceOf(NewLimitService);
      await moduleRef.close();
    });

    it('exports the Site.NEWLIMIT = "newlimit" enum value', () => {
      expect(Site.NEWLIMIT).toBe('newlimit');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new NewLimitService();
      ...
      const job0 = dto.jobs.find((j) => j.id === 'newlimit-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.NEWLIMIT);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('NewLimit');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/newlimit/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/newlimit/jobs/');
      ...
      expect(job0?.description).toContain('NewLimit');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/newlimit/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('NewLimit');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'newlimit-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
