...
import { VentiTechnologiesModule, VentiTechnologiesService } from '../src';

const COMPANY_NAME_EXPECT = 'Venti Technologies';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'ventitechnologies-jobs.json'), 'utf8'),
);
...
describe('VentiTechnologiesService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through VentiTechnologiesModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [VentiTechnologiesModule],
      }).compile();
      const service = moduleRef.get(VentiTechnologiesService);
      expect(service).toBeInstanceOf(VentiTechnologiesService);
      await moduleRef.close();
    });

    it('exports the Site.VENTI_TECHNOLOGIES = "ventitechnologies" enum value', () => {
      expect(Site.VENTI_TECHNOLOGIES).toBe('ventitechnologies');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new VentiTechnologiesService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.VENTI_TECHNOLOGIES],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'ventitechnologies-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.VENTI_TECHNOLOGIES);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('ventitechnologies-' + first.id);
      ...
      expect(calledUrls[0]).toContain('goventi');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.VENTI_TECHNOLOGIES);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('ventitechnologies-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new VentiTechnologiesService(registry);
      const result = await service.scrape({
        siteType: [Site.VENTI_TECHNOLOGIES],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('goventi');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('ventitechnologies-x1');
      expect(result.jobs[0].site).toBe(Site.VENTI_TECHNOLOGIES);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('ventitechnologies-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new VentiTechnologiesService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.VENTI_TECHNOLOGIES],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new VentiTechnologiesService();
      const result = await service.scrape({
        siteType: [Site.VENTI_TECHNOLOGIES],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
