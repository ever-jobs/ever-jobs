...
import { SynapticureModule, SynapticureService } from '../src';

const COMPANY_NAME_EXPECT = 'Synapticure';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'synapticure-jobs.json'), 'utf8'),
);
...
describe('SynapticureService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SynapticureModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SynapticureModule],
      }).compile();
      const service = moduleRef.get(SynapticureService);
      expect(service).toBeInstanceOf(SynapticureService);
      await moduleRef.close();
    });

    it('exports the Site.SYNAPTICURE = "synapticure" enum value', () => {
      expect(Site.SYNAPTICURE).toBe('synapticure');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new SynapticureService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.SYNAPTICURE],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'synapticure-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.SYNAPTICURE);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('synapticure-' + first.id);
      ...
      expect(calledUrls[0]).toContain('synapticure');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SYNAPTICURE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('synapticure-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new SynapticureService(registry);
      const result = await service.scrape({
        siteType: [Site.SYNAPTICURE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('synapticure');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('synapticure-x1');
      expect(result.jobs[0].site).toBe(Site.SYNAPTICURE);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('synapticure-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new SynapticureService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.SYNAPTICURE],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new SynapticureService();
      const result = await service.scrape({
        siteType: [Site.SYNAPTICURE],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
