...
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'truelayer-jobs.json'), 'utf8'),
);
...
/**
 * Spec 511 / T04 — `TruelayerService` unit tests (≥ 9 cases).
 */
describe('TruelayerService — Spec 511 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through TruelayerModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [TruelayerModule],
      }).compile();
      const service = moduleRef.get(TruelayerService);
      expect(service).toBeInstanceOf(TruelayerService);
      await moduleRef.close();
    });

    it('exports the Site.TRUELAYER = "truelayer" enum value', () => {
      expect(Site.TRUELAYER).toBe('truelayer');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new TruelayerService();
      const result = await service.scrape({
        siteType: [Site.TRUELAYER],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'truelayer-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.TRUELAYER);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('TrueLayer');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/truelayer/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/truelayer/jobs/');
      ...
      expect(job0?.description).toContain('TrueLayer');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/truelayer/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('TrueLayer');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'truelayer-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
