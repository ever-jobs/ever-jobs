...
import { XenditModule, XenditService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'xendit-jobs.json'), 'utf8'),
);
...
/**
 * Spec 707 / T04 — `XenditService` unit tests (≥ 9 cases).
 */
describe('XenditService — Spec 707 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through XenditModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [XenditModule],
      }).compile();
      const service = moduleRef.get(XenditService);
      expect(service).toBeInstanceOf(XenditService);
      await moduleRef.close();
    });

    it('exports the Site.XENDIT = "xendit" enum value', () => {
      expect(Site.XENDIT).toBe('xendit');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new XenditService();
      const result = await service.scrape({
        siteType: [Site.XENDIT],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'xendit-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.XENDIT);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Xendit');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/xendit/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/xendit/jobs/');
      ...
      expect(job0?.description).toContain('Xendit');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/xendit/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Xendit');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'xendit-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
