...
import { GEHealthCareModule, GEHealthCareService } from '../src';

const COMPANY_NAME_EXPECT = 'GE HealthCare';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'gehealthcare-jobs.json'), 'utf8'),
);
...
describe('GEHealthCareService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through GEHealthCareModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [GEHealthCareModule],
      }).compile();
      const service = moduleRef.get(GEHealthCareService);
      expect(service).toBeInstanceOf(GEHealthCareService);
      await moduleRef.close();
    });

    it('exports the Site.GE_HEALTHCARE = "gehealthcare" enum value', () => {
      expect(Site.GE_HEALTHCARE).toBe('gehealthcare');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new GEHealthCareService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.GE_HEALTHCARE],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('GEHealthcare2');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.GE_HEALTHCARE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('gehealthcare-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new GEHealthCareService(registry);
      const result = await service.scrape({
        siteType: [Site.GE_HEALTHCARE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('GEHealthcare2');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('gehealthcare-x1');
      expect(result.jobs[0].site).toBe(Site.GE_HEALTHCARE);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('gehealthcare-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new GEHealthCareService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.GE_HEALTHCARE],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new GEHealthCareService();
      const result = await service.scrape({
        siteType: [Site.GE_HEALTHCARE],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
