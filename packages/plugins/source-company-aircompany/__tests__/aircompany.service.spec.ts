...
import { AIRCOMPANYModule, AIRCOMPANYService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'aircompany-jobs.json'), 'utf8'),
);
...
/**
 * Spec 788 / T04 — `AIRCOMPANYService` unit tests (≥ 9 cases).
 */
describe('AIRCOMPANYService — Spec 788 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AIRCOMPANYModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AIRCOMPANYModule],
      }).compile();
      const service = moduleRef.get(AIRCOMPANYService);
      expect(service).toBeInstanceOf(AIRCOMPANYService);
      await moduleRef.close();
    });

    it('exports the Site.AIR_COMPANY = "aircompany" enum value', () => {
      expect(Site.AIR_COMPANY).toBe('aircompany');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AIRCOMPANYService();
      const result = await service.scrape({
        siteType: [Site.AIR_COMPANY],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'aircompany-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.AIR_COMPANY);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('AIR COMPANY');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/aircompany/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/aircompany/jobs/');
      ...
      expect(job0?.description).toContain('AIR COMPANY');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/aircompany/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('AIR COMPANY');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'aircompany-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
