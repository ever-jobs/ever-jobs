...
import { HEMERIAModule, HEMERIAService } from '../src';

const COMPANY_NAME_EXPECT = 'HEMERIA';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'hemeria-jobs.json'), 'utf8'),
);
...
describe('HEMERIAService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through HEMERIAModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [HEMERIAModule],
      }).compile();
      const service = moduleRef.get(HEMERIAService);
      expect(service).toBeInstanceOf(HEMERIAService);
      await moduleRef.close();
    });

    it('exports the Site.HEMERIA = "hemeria" enum value', () => {
      expect(Site.HEMERIA).toBe('hemeria');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new HEMERIAService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.HEMERIA],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('hemeriagroup.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.HEMERIA);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('hemeria-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new HEMERIAService(registry);
      const result = await service.scrape({
        siteType: [Site.HEMERIA],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('hemeriagroup');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('hemeria-x1');
      expect(result.jobs[0].site).toBe(Site.HEMERIA);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('hemeria-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new HEMERIAService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.HEMERIA],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new HEMERIAService();
      const result = await service.scrape({
        siteType: [Site.HEMERIA],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
