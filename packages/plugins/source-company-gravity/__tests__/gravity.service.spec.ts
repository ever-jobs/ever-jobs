...
import { GravityRDModule, GravityRDService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'gravity-jobs.json'), 'utf8'),
);
...
/**
 * Spec 794 / T04 — `GravityRDService` unit tests (≥ 9 cases).
 */
describe('GravityRDService — Spec 794 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through GravityRDModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [GravityRDModule],
      }).compile();
      const service = moduleRef.get(GravityRDService);
      expect(service).toBeInstanceOf(GravityRDService);
      await moduleRef.close();
    });

    it('exports the Site.GRAVITY_R_D = "gravity" enum value', () => {
      expect(Site.GRAVITY_R_D).toBe('gravity');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new GravityRDService();
      const result = await service.scrape({
        siteType: [Site.GRAVITY_R_D],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'gravity-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.GRAVITY_R_D);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Gravity R&D');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/gravity/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/gravity/jobs/');
      ...
      expect(job0?.description).toContain('Gravity R&D');
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Gravity R&D');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'gravity-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
