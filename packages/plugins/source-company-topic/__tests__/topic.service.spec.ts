...
import { TOPICModule, TOPICService } from '../src';

const COMPANY_NAME_EXPECT = 'TOPIC';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'topic-jobs.json'), 'utf8'),
);
...
describe('TOPICService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through TOPICModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [TOPICModule],
      }).compile();
      const service = moduleRef.get(TOPICService);
      expect(service).toBeInstanceOf(TOPICService);
      await moduleRef.close();
    });

    it('exports the Site.TOPIC = "topic" enum value', () => {
      expect(Site.TOPIC).toBe('topic');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new TOPICService(registryWithRecruitee());
      ...
      expect(calledUrls[0]).toContain('topicsoftwaredevelopment.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.TOPIC);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('topic-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new TOPICService(registry);
      const result = await service.scrape({
        siteType: [Site.TOPIC],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('topicsoftwaredevelopment');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('topic-x1');
      expect(result.jobs[0].site).toBe(Site.TOPIC);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('topic-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new TOPICService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.TOPIC],
      } as ScraperInputDto);
