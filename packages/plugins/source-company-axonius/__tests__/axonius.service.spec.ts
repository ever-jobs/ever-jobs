...
import { AxoniusModule, AxoniusService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'axonius-jobs.json'), 'utf8'),
);
...
/**
 * Spec 815 / T04 — `AxoniusService` unit tests (≥ 9 cases).
 */
describe('AxoniusService — Spec 815 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AxoniusModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AxoniusModule],
      }).compile();
      const service = moduleRef.get(AxoniusService);
      expect(service).toBeInstanceOf(AxoniusService);
      await moduleRef.close();
    });

    it('exports the Site.AXONIUS = "axonius" enum value', () => {
      expect(Site.AXONIUS).toBe('axonius');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AxoniusService();
      const result = await service.scrape({
        siteType: [Site.AXONIUS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'axonius-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.AXONIUS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Axonius');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/axonius/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/axonius/jobs/');
      ...
      expect(job0?.description).toContain('Axonius');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/axonius/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Axonius');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'axonius-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
