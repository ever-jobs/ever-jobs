...
import { CensysModule, CensysService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'censys-jobs.json'), 'utf8'),
);
...
/**
 * Spec 823 / T04 — `CensysService` unit tests (≥ 9 cases).
 */
describe('CensysService — Spec 823 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CensysModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CensysModule],
      }).compile();
      const service = moduleRef.get(CensysService);
      expect(service).toBeInstanceOf(CensysService);
      await moduleRef.close();
    });

    it('exports the Site.CENSYS = "censys" enum value', () => {
      expect(Site.CENSYS).toBe('censys');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new CensysService();
      const result = await service.scrape({
        siteType: [Site.CENSYS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'censys-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.CENSYS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Censys');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/censys/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/censys/jobs/');
      ...
      expect(job0?.description).toContain('Censys');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/censys/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Censys');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'censys-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
