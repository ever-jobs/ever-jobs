...
import { ContentsquareModule, ContentsquareService } from '../src';

const COMPANY_NAME_EXPECT = 'Contentsquare';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'contentsquare-jobs.json'), 'utf8'),
);
...
describe('ContentsquareService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ContentsquareModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ContentsquareModule],
      }).compile();
      const service = moduleRef.get(ContentsquareService);
      expect(service).toBeInstanceOf(ContentsquareService);
      await moduleRef.close();
    });

    it('exports the Site.CONTENTSQUARE = "contentsquare" enum value', () => {
      expect(Site.CONTENTSQUARE).toBe('contentsquare');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new ContentsquareService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.CONTENTSQUARE],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'contentsquare-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.CONTENTSQUARE);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('contentsquare-' + first.id);
      ...
      expect(calledUrls[0]).toContain('contentsquare');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.CONTENTSQUARE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('contentsquare-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new ContentsquareService(registry);
      const result = await service.scrape({
        siteType: [Site.CONTENTSQUARE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('contentsquare');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('contentsquare-x1');
      expect(result.jobs[0].site).toBe(Site.CONTENTSQUARE);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('contentsquare-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new ContentsquareService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.CONTENTSQUARE],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new ContentsquareService();
      const result = await service.scrape({
        siteType: [Site.CONTENTSQUARE],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
