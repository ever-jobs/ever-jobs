...
import { XsollaModule, XsollaService } from '../src';

const COMPANY_NAME_EXPECT = 'Xsolla';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'xsolla-jobs.json'), 'utf8'),
);
...
describe('XsollaService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through XsollaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [XsollaModule],
      }).compile();
      const service = moduleRef.get(XsollaService);
      expect(service).toBeInstanceOf(XsollaService);
      await moduleRef.close();
    });

    it('exports the Site.XSOLLA = "xsolla" enum value', () => {
      expect(Site.XSOLLA).toBe('xsolla');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new XsollaService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.XSOLLA],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('xsolla');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.XSOLLA);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('xsolla-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new XsollaService(registry);
      const result = await service.scrape({
        siteType: [Site.XSOLLA],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('xsolla');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('xsolla-x1');
      expect(result.jobs[0].site).toBe(Site.XSOLLA);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('xsolla-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new XsollaService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.XSOLLA],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new XsollaService();
      const result = await service.scrape({
        siteType: [Site.XSOLLA],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
