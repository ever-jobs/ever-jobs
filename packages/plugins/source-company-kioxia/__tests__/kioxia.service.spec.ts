...
import { KioxiaModule, KioxiaService } from '../src';

const COMPANY_NAME_EXPECT = 'Kioxia';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'kioxia-jobs.json'), 'utf8'),
);
...
describe('KioxiaService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through KioxiaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [KioxiaModule],
      }).compile();
      const service = moduleRef.get(KioxiaService);
      expect(service).toBeInstanceOf(KioxiaService);
      await moduleRef.close();
    });

    it('exports the Site.KIOXIA = "kioxia" enum value', () => {
      expect(Site.KIOXIA).toBe('kioxia');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new KioxiaService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.KIOXIA],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('Kioxia');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.KIOXIA);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('kioxia-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new KioxiaService(registry);
      const result = await service.scrape({
        siteType: [Site.KIOXIA],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('Kioxia');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('kioxia-x1');
      expect(result.jobs[0].site).toBe(Site.KIOXIA);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('kioxia-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new KioxiaService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.KIOXIA],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new KioxiaService();
      const result = await service.scrape({
        siteType: [Site.KIOXIA],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
