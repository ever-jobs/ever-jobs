...
import { AiserajobsModule, AiserajobsService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'aiserajobs-jobs.json'), 'utf8'),
);
...
/**
 * Spec 219 / T04 — `AiserajobsService` unit tests (≥ 9 cases).
 */
describe('AiserajobsService — Spec 219 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AiserajobsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AiserajobsModule],
      }).compile();
      const service = moduleRef.get(AiserajobsService);
      expect(service).toBeInstanceOf(AiserajobsService);
      await moduleRef.close();
    });

    it('exports the Site.AISERAJOBS = "aiserajobs" enum value', () => {
      expect(Site.AISERAJOBS).toBe('aiserajobs');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AiserajobsService();
      const result = await service.scrape({
        siteType: [Site.AISERAJOBS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'aiserajobs-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.AISERAJOBS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Aisera');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/aiserajobs/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/aiserajobs/jobs/');
      ...
      expect(job0?.description).toContain('Aisera');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/aiserajobs/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Aisera');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'aiserajobs-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
