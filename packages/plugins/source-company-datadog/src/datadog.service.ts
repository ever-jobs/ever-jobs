...
/**
 * Datadog publishes its careers board through Greenhouse at the bare
 * `datadog` slug (no asymmetry; see Spec 038 § 10 D-05).
 */
const API_URL = 'https://api.greenhouse.io/v1/boards/datadog/jobs';
...
export class DatadogService implements IScraper {
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    try {
      ...
      this.logger.log(`Datadog: fetching ${url}`);
        ...
        jobs.push(
          new JobPostDto({
            ...
            jobUrl:
              ...
              `https://careers.datadoghq.com/detail/${listing.id}`,
            location,
      ...
      this.logger.log(`Datadog: scraped ${jobs.length} jobs`);
    } catch (err: any) {
      this.logger.error(`Datadog scrape failed: ${err.message}`);
