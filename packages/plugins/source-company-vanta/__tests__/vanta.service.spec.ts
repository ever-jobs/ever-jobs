...
import { VantaModule, VantaService } from '../src';

const COMPANY_NAME_EXPECT = 'Vanta';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'vanta-jobs.json'), 'utf8'),
);
...
describe('VantaService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through VantaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [VantaModule],
      }).compile();
      const service = moduleRef.get(VantaService);
      expect(service).toBeInstanceOf(VantaService);
      await moduleRef.close();
    });

    it('exports the Site.VANTA = "vanta" enum value', () => {
      expect(Site.VANTA).toBe('vanta');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new VantaService(registryWithAshby());
      ...
      expect(calledUrls[0]).toContain('vanta');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.VANTA);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('vanta-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new VantaService(registry);
      const result = await service.scrape({
        siteType: [Site.VANTA],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('vanta');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('vanta-x1');
      expect(result.jobs[0].site).toBe(Site.VANTA);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('vanta-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new VantaService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.VANTA],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new VantaService();
      const result = await service.scrape({
        siteType: [Site.VANTA],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
