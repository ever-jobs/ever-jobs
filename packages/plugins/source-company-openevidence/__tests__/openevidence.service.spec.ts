...
import { OpenEvidenceModule, OpenEvidenceService } from '../src';

const COMPANY_NAME_EXPECT = 'OpenEvidence';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'openevidence-jobs.json'), 'utf8'),
);
...
describe('OpenEvidenceService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through OpenEvidenceModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [OpenEvidenceModule],
      }).compile();
      const service = moduleRef.get(OpenEvidenceService);
      expect(service).toBeInstanceOf(OpenEvidenceService);
      await moduleRef.close();
    });

    it('exports the Site.OPENEVIDENCE = "openevidence" enum value', () => {
      expect(Site.OPENEVIDENCE).toBe('openevidence');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new OpenEvidenceService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.OPENEVIDENCE],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'openevidence-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.OPENEVIDENCE);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('openevidence-' + first.id);
      ...
      expect(calledUrls[0]).toContain('openevidence');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.OPENEVIDENCE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('openevidence-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new OpenEvidenceService(registry);
      const result = await service.scrape({
        siteType: [Site.OPENEVIDENCE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('openevidence');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('openevidence-x1');
      expect(result.jobs[0].site).toBe(Site.OPENEVIDENCE);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('openevidence-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new OpenEvidenceService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.OPENEVIDENCE],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new OpenEvidenceService();
      const result = await service.scrape({
        siteType: [Site.OPENEVIDENCE],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
