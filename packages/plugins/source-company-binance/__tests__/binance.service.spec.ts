...
import { BinanceModule, BinanceService } from '../src';

const COMPANY_NAME_EXPECT = 'Binance';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'binance-jobs.json'), 'utf8'),
);
...
describe('BinanceService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through BinanceModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [BinanceModule],
      }).compile();
      const service = moduleRef.get(BinanceService);
      expect(service).toBeInstanceOf(BinanceService);
      await moduleRef.close();
    });

    it('exports the Site.BINANCE = "binance" enum value', () => {
      expect(Site.BINANCE).toBe('binance');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new BinanceService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.BINANCE],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('binance');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.BINANCE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('binance-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new BinanceService(registry);
      const result = await service.scrape({
        siteType: [Site.BINANCE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('binance');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('binance-x1');
      expect(result.jobs[0].site).toBe(Site.BINANCE);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('binance-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new BinanceService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.BINANCE],
      } as ScraperInputDto);
