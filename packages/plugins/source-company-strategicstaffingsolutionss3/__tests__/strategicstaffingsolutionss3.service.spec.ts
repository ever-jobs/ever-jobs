...
import { StrategicStaffingSolutionsS3Module, StrategicStaffingSolutionsS3Service } from '../src';

const COMPANY_NAME_EXPECT = 'Strategic Staffing Solutions (S3)';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'strategicstaffingsolutionss3-jobs.json'), 'utf8'),
);
...
describe('StrategicStaffingSolutionsS3Service — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through StrategicStaffingSolutionsS3Module via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [StrategicStaffingSolutionsS3Module],
      }).compile();
      const service = moduleRef.get(StrategicStaffingSolutionsS3Service);
      expect(service).toBeInstanceOf(StrategicStaffingSolutionsS3Service);
      await moduleRef.close();
    });

    it('exports the Site.STRATEGIC_STAFFING_SOLUTIONS_S3 = "strategicstaffingsolutionss3" enum value', () => {
      expect(Site.STRATEGIC_STAFFING_SOLUTIONS_S3).toBe('strategicstaffingsolutionss3');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new StrategicStaffingSolutionsS3Service(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.STRATEGIC_STAFFING_SOLUTIONS_S3],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'strategicstaffingsolutionss3-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.STRATEGIC_STAFFING_SOLUTIONS_S3);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('strategicstaffingsolutionss3-' + first.id);
      ...
      expect(calledUrls[0]).toContain('StrategicStaffingSolutionsS3');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.STRATEGIC_STAFFING_SOLUTIONS_S3);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('strategicstaffingsolutionss3-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new StrategicStaffingSolutionsS3Service(registry);
      const result = await service.scrape({
        siteType: [Site.STRATEGIC_STAFFING_SOLUTIONS_S3],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('StrategicStaffingSolutionsS3');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('strategicstaffingsolutionss3-x1');
      expect(result.jobs[0].site).toBe(Site.STRATEGIC_STAFFING_SOLUTIONS_S3);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('strategicstaffingsolutionss3-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new StrategicStaffingSolutionsS3Service(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.STRATEGIC_STAFFING_SOLUTIONS_S3],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new StrategicStaffingSolutionsS3Service();
      const result = await service.scrape({
        siteType: [Site.STRATEGIC_STAFFING_SOLUTIONS_S3],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
