...
import { QuaiseModule, QuaiseService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'quaise-jobs.json'), 'utf8'),
);
...
/**
 * Spec 502 / T04 — `QuaiseService` unit tests (≥ 9 cases).
 */
describe('QuaiseService — Spec 502 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through QuaiseModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [QuaiseModule],
      }).compile();
      const service = moduleRef.get(QuaiseService);
      expect(service).toBeInstanceOf(QuaiseService);
      await moduleRef.close();
    });

    it('exports the Site.QUAISE = "quaise" enum value', () => {
      expect(Site.QUAISE).toBe('quaise');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new QuaiseService();
      const result = await service.scrape({
        siteType: [Site.QUAISE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'quaise-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.QUAISE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Quaise Energy');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/quaise/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/quaise/jobs/');
      ...
      expect(job0?.description).toContain('Quaise Energy');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/quaise/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Quaise Energy');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'quaise-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
