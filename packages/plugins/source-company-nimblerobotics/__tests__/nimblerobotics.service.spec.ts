...
import { NimbleRoboticsModule, NimbleRoboticsService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'nimblerobotics-jobs.json'), 'utf8'),
);
...
/**
 * Spec 895 / T04 — `NimbleRoboticsService` unit tests (≥ 9 cases).
 */
describe('NimbleRoboticsService — Spec 895 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through NimbleRoboticsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [NimbleRoboticsModule],
      }).compile();
      const service = moduleRef.get(NimbleRoboticsService);
      expect(service).toBeInstanceOf(NimbleRoboticsService);
      await moduleRef.close();
    });

    it('exports the Site.NIMBLE_ROBOTICS = "nimblerobotics" enum value', () => {
      expect(Site.NIMBLE_ROBOTICS).toBe('nimblerobotics');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new NimbleRoboticsService();
      const result = await service.scrape({
        siteType: [Site.NIMBLE_ROBOTICS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'nimblerobotics-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.NIMBLE_ROBOTICS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Nimble Robotics');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/nimblerobotics/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/nimblerobotics/jobs/');
      ...
      expect(job0?.description).toContain('Nimble Robotics');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/nimblerobotics/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Nimble Robotics');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'nimblerobotics-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
