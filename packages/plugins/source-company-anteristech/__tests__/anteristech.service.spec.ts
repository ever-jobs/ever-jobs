...
import { AnteristechModule, AnteristechService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'anteristech-jobs.json'), 'utf8'),
);
...
/**
 * Spec 274 / T04 — `AnteristechService` unit tests (≥ 9 cases).
 */
describe('AnteristechService — Spec 274 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AnteristechModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AnteristechModule],
      }).compile();
      const service = moduleRef.get(AnteristechService);
      expect(service).toBeInstanceOf(AnteristechService);
      await moduleRef.close();
    });

    it('exports the Site.ANTERISTECH = "anteristech" enum value', () => {
      expect(Site.ANTERISTECH).toBe('anteristech');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AnteristechService();
      const result = await service.scrape({
        siteType: [Site.ANTERISTECH],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'anteristech-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ANTERISTECH);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Anteris Technologies');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/anteristech/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/anteristech/jobs/');
      ...
      expect(job0?.description).toContain('Anteris Technologies');
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Anteris Technologies');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'anteristech-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
