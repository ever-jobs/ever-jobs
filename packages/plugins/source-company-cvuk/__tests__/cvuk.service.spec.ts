...
import { CVUKModule, CVUKService } from '../src';

const COMPANY_NAME_EXPECT = 'CVUK';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'cvuk-jobs.json'), 'utf8'),
);
...
describe('CVUKService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CVUKModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CVUKModule],
      }).compile();
      const service = moduleRef.get(CVUKService);
      expect(service).toBeInstanceOf(CVUKService);
      await moduleRef.close();
    });

    it('exports the Site.CVUK = "cvuk" enum value', () => {
      expect(Site.CVUK).toBe('cvuk');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new CVUKService(registryWithSmartRecruiters());
      ...
      expect(calledUrls[0]).toContain('CVUK');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.CVUK);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('cvuk-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new CVUKService(registry);
      const result = await service.scrape({
        siteType: [Site.CVUK],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('CVUK');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('cvuk-x1');
      expect(result.jobs[0].site).toBe(Site.CVUK);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('cvuk-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new CVUKService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.CVUK],
      } as ScraperInputDto);
