...
import { AlliancedefendingfreedomModule, AlliancedefendingfreedomService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'alliancedefendingfreedom-jobs.json'), 'utf8'),
);
...
/**
 * Spec 235 / T04 — `AlliancedefendingfreedomService` unit tests (≥ 9 cases).
 */
describe('AlliancedefendingfreedomService — Spec 235 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AlliancedefendingfreedomModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AlliancedefendingfreedomModule],
      }).compile();
      const service = moduleRef.get(AlliancedefendingfreedomService);
      expect(service).toBeInstanceOf(AlliancedefendingfreedomService);
      await moduleRef.close();
    });

    it('exports the Site.ALLIANCEDEFENDINGFREEDOM = "alliancedefendingfreedom" enum value', () => {
      expect(Site.ALLIANCEDEFENDINGFREEDOM).toBe('alliancedefendingfreedom');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AlliancedefendingfreedomService();
      const result = await service.scrape({
        siteType: [Site.ALLIANCEDEFENDINGFREEDOM],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'alliancedefendingfreedom-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ALLIANCEDEFENDINGFREEDOM);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Alliance Defending Freedom');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/alliancedefendingfreedom/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/alliancedefendingfreedom/jobs/');
      ...
      expect(job0?.description).toContain('Alliance Defending Freedom');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/alliancedefendingfreedom/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Alliance Defending Freedom');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'alliancedefendingfreedom-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
