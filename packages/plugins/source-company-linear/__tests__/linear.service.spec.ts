...
import { LinearModule, LinearService } from '../src';

const COMPANY_NAME_EXPECT = 'Linear';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'linear-jobs.json'), 'utf8'),
);
...
describe('LinearService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through LinearModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [LinearModule],
      }).compile();
      const service = moduleRef.get(LinearService);
      expect(service).toBeInstanceOf(LinearService);
      await moduleRef.close();
    });

    it('exports the Site.LINEAR = "linear" enum value', () => {
      expect(Site.LINEAR).toBe('linear');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new LinearService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.LINEAR],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('linear');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.LINEAR);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('linear-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new LinearService(registry);
      const result = await service.scrape({
        siteType: [Site.LINEAR],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('linear');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('linear-x1');
      expect(result.jobs[0].site).toBe(Site.LINEAR);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('linear-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new LinearService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.LINEAR],
      } as ScraperInputDto);
