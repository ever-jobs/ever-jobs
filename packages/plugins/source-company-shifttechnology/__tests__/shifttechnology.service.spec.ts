...
import { ShiftTechnologyModule, ShiftTechnologyService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'shifttechnology-jobs.json'), 'utf8'),
);
...
/**
 * Spec 930 / T04 — `ShiftTechnologyService` unit tests (≥ 9 cases).
 */
describe('ShiftTechnologyService — Spec 930 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ShiftTechnologyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ShiftTechnologyModule],
      }).compile();
      const service = moduleRef.get(ShiftTechnologyService);
      expect(service).toBeInstanceOf(ShiftTechnologyService);
      await moduleRef.close();
    });

    it('exports the Site.SHIFT_TECHNOLOGY = "shifttechnology" enum value', () => {
      expect(Site.SHIFT_TECHNOLOGY).toBe('shifttechnology');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ShiftTechnologyService();
      const result = await service.scrape({
        siteType: [Site.SHIFT_TECHNOLOGY],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'shifttechnology-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.SHIFT_TECHNOLOGY);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Shift Technology');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/shifttechnology/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/shifttechnology/jobs/');
      ...
      expect(job0?.description).toContain('Shift Technology');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/shifttechnology/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Shift Technology');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'shifttechnology-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
