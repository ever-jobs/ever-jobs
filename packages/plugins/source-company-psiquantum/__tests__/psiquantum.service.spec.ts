...
import { PsiQuantumModule, PsiQuantumService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'psiquantum-jobs.json'), 'utf8'),
);
...
/**
 * Spec 703 / T04 — `PsiQuantumService` unit tests (≥ 9 cases).
 */
describe('PsiQuantumService — Spec 703 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through PsiQuantumModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [PsiQuantumModule],
      }).compile();
      const service = moduleRef.get(PsiQuantumService);
      expect(service).toBeInstanceOf(PsiQuantumService);
      await moduleRef.close();
    });

    it('exports the Site.PSIQUANTUM = "psiquantum" enum value', () => {
      expect(Site.PSIQUANTUM).toBe('psiquantum');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new PsiQuantumService();
      const result = await service.scrape({
        siteType: [Site.PSIQUANTUM],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'psiquantum-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.PSIQUANTUM);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('PsiQuantum');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/psiquantum/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/psiquantum/jobs/');
      ...
      expect(job0?.description).toContain('PsiQuantum');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/psiquantum/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('PsiQuantum');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'psiquantum-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
