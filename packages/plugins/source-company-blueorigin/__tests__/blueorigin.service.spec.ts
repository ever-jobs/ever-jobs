...
import { BlueOriginModule, BlueOriginService } from '../src';
...
const FIXTURE: Fixture = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'fixtures', 'blueorigin-boards.json'), 'utf8'),
);
const COMPANY_NAME = 'Blue Origin';
const ID_PREFIX = 'blueorigin-';
...
describe('BlueOriginService — Workday delegation (Spec 1736)', () => {
  beforeEach(() => {
    mockGet.mockReset();
    mockPost.mockReset();
  ...
  describe('registration', () => {
    it('resolves through BlueOriginModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({ imports: [BlueOriginModule] }).compile();
      expect(moduleRef.get(BlueOriginService)).toBeInstanceOf(BlueOriginService);
      await moduleRef.close();
    });

    it('exports Site.BLUE_ORIGIN = "blueorigin"', () => {
      expect(Site.BLUE_ORIGIN).toBe('blueorigin');
    });

    it('declares a tagged company plugin with its domains', () => {
      const meta = Reflect.getMetadata(SOURCE_PLUGIN_METADATA, BlueOriginService);
      expect(meta.site).toBe(Site.BLUE_ORIGIN);
      expect(meta.category).toBe('company');
      expect(meta.isAts).toBeFalsy();
      expect(meta.companyDomains).toEqual(['blueorigin.com']);
  ...
  describe('recorded board (real Workday adapter, mocked HTTP)', () => {
    it('maps every recorded posting and re-stamps the company identity', async () => {
      const service = new BlueOriginService(registryWith());
      const result = await service.scrape({ siteType: [Site.BLUE_ORIGIN], resultsWanted: 100 } as ScraperInputDto);
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.BLUE_ORIGIN);
    ...
    it('only requests the recorded board URLs', async () => {
      const service = new BlueOriginService(registryWith());
      await service.scrape({ siteType: [Site.BLUE_ORIGIN] } as ScraperInputDto);
    ...
    it('honours resultsWanted=1', async () => {
      const service = new BlueOriginService(registryWith());
      const result = await service.scrape({ siteType: [Site.BLUE_ORIGIN], resultsWanted: 1 } as ScraperInputDto);
    ...
    it('resolves with an empty result when the board is gone (HTTP 404)', async () => {
      ...
      const service = new BlueOriginService(registryWith());
      const result = await service.scrape({ siteType: [Site.BLUE_ORIGIN] } as ScraperInputDto);
  ...
  describe('delegation contract', () => {
    it('forwards the board and the caller input untouched', async () => {
      ...
      const service = new BlueOriginService(
        registryWith(
          fakeBackend(
      ...
      expect(result.jobs[0].site).toBe(Site.BLUE_ORIGIN);
      expect(result.jobs[0].companyName).toBe(COMPANY_NAME);
    });

    it('passes an absent resultsWanted through as absent', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new BlueOriginService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      await service.scrape({ siteType: [Site.BLUE_ORIGIN] } as ScraperInputDto);
    ...
    it('makes no request when resultsWanted is 0', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new BlueOriginService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      const result = await service.scrape({ siteType: [Site.BLUE_ORIGIN], resultsWanted: 0 } as ScraperInputDto);
  ...
  describe('resilience', () => {
    it('reports not_registered when no registry is injected', async () => {
      const result = await new BlueOriginService().scrape({ siteType: [Site.BLUE_ORIGIN] } as ScraperInputDto);
      expect(result.jobs).toEqual([]);
    ...
    it('reports not_registered when Workday is missing from the registry', async () => {
      const result = await new BlueOriginService(new PluginRegistry()).scrape({
        siteType: [Site.BLUE_ORIGIN],
      } as ScraperInputDto);
      expect(result.diagnostics?.reason).toBe('not_registered');
    });
