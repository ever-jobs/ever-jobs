...
/**
 * Blue Origin — Aerospace (HQ: Kent, WA, USA).
 *
 * Source (Spec 1736): Workday board, scraped in this order:
 *   - `blueorigin:5:BlueOrigin` — https://blueorigin.wd5.myworkdayjobs.com/BlueOrigin
 *     verified live 2026-09-24: 1,745 open postings.
 *
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'blueorigin:5:BlueOrigin', atsIdPrefix: 'wd-blueorigin-' },
];
...
@SourcePlugin({
  site: Site.BLUE_ORIGIN,
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['blueorigin.com'],
  description: 'Blue Origin careers via Workday. Tags: segment=workday-enterprise; industry=aerospace.',
})
@Injectable()
export class BlueOriginService implements IScraper {
  private readonly logger = new Logger(BlueOriginService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Workday source plugin is not registered; cannot scrape Blue Origin');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`Blue Origin: delegating to Workday (${board.companySlug})`);
    ...
    this.logger.log(`Blue Origin: scraped ${jobs.length} jobs`);
