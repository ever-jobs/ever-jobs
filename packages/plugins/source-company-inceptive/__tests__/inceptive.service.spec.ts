...
import { InceptiveModule, InceptiveService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'inceptive-jobs.json'), 'utf8'),
);
...
/**
 * Spec 687 / T04 — `InceptiveService` unit tests (≥ 9 cases).
 */
describe('InceptiveService — Spec 687 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through InceptiveModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [InceptiveModule],
      }).compile();
      const service = moduleRef.get(InceptiveService);
      expect(service).toBeInstanceOf(InceptiveService);
      await moduleRef.close();
    });

    it('exports the Site.INCEPTIVE = "inceptive" enum value', () => {
      expect(Site.INCEPTIVE).toBe('inceptive');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new InceptiveService();
      const result = await service.scrape({
        siteType: [Site.INCEPTIVE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'inceptive-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.INCEPTIVE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Inceptive');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/inceptive/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/inceptive/jobs/');
      ...
      expect(job0?.description).toContain('Inceptive');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/inceptive/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Inceptive');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'inceptive-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
