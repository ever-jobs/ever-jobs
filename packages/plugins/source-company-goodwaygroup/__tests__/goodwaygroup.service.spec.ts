...
import { GoodwayGroupModule, GoodwayGroupService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'goodwaygroup-jobs.json'), 'utf8'),
);
...
/**
 * Spec 858 / T04 — `GoodwayGroupService` unit tests (≥ 9 cases).
 */
describe('GoodwayGroupService — Spec 858 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through GoodwayGroupModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [GoodwayGroupModule],
      }).compile();
      const service = moduleRef.get(GoodwayGroupService);
      expect(service).toBeInstanceOf(GoodwayGroupService);
      await moduleRef.close();
    });

    it('exports the Site.GOODWAY_GROUP = "goodwaygroup" enum value', () => {
      expect(Site.GOODWAY_GROUP).toBe('goodwaygroup');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new GoodwayGroupService();
      const result = await service.scrape({
        siteType: [Site.GOODWAY_GROUP],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'goodwaygroup-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.GOODWAY_GROUP);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Goodway Group');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/goodwaygroup/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/goodwaygroup/jobs/');
      ...
      expect(job0?.description).toContain('Goodway Group');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/goodwaygroup/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Goodway Group');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'goodwaygroup-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
