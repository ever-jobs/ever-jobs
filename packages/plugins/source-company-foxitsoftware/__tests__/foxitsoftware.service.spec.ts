...
import { FoxitModule, FoxitService } from '../src';

const COMPANY_NAME_EXPECT = 'Foxit';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'foxitsoftware-jobs.json'), 'utf8'),
);
...
describe('FoxitService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through FoxitModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [FoxitModule],
      }).compile();
      const service = moduleRef.get(FoxitService);
      expect(service).toBeInstanceOf(FoxitService);
      await moduleRef.close();
    });

    it('exports the Site.FOXIT = "foxitsoftware" enum value', () => {
      expect(Site.FOXIT).toBe('foxitsoftware');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new FoxitService(registryWithLever());
      ...
      expect(calledUrls[0]).toContain('foxitsoftware');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.FOXIT);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('foxitsoftware-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new FoxitService(registry);
      const result = await service.scrape({
        siteType: [Site.FOXIT],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('foxitsoftware');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('foxitsoftware-x1');
      expect(result.jobs[0].site).toBe(Site.FOXIT);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('foxitsoftware-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new FoxitService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.FOXIT],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new FoxitService();
      const result = await service.scrape({
        siteType: [Site.FOXIT],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
