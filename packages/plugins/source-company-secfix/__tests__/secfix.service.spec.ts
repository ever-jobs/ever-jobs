...
import { SecfixModule, SecfixService } from '../src';

const COMPANY_NAME_EXPECT = 'Secfix';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'secfix-jobs.json'), 'utf8'),
);
...
describe('SecfixService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SecfixModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SecfixModule],
      }).compile();
      const service = moduleRef.get(SecfixService);
      expect(service).toBeInstanceOf(SecfixService);
      await moduleRef.close();
    });

    it('exports the Site.SECFIX = "secfix" enum value', () => {
      expect(Site.SECFIX).toBe('secfix');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new SecfixService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.SECFIX],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('secfix');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SECFIX);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('secfix-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new SecfixService(registry);
      const result = await service.scrape({
        siteType: [Site.SECFIX],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('secfix');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('secfix-x1');
      expect(result.jobs[0].site).toBe(Site.SECFIX);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('secfix-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new SecfixService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.SECFIX],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new SecfixService();
      const result = await service.scrape({
        siteType: [Site.SECFIX],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
