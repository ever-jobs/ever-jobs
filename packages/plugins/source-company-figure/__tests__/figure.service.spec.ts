...
import { FigureLendingModule, FigureLendingService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'figure-jobs.json'), 'utf8'),
);
...
/**
 * Spec 743 / T04 — `FigureLendingService` unit tests (≥ 9 cases).
 */
describe('FigureLendingService — Spec 743 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through FigureLendingModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [FigureLendingModule],
      }).compile();
      const service = moduleRef.get(FigureLendingService);
      expect(service).toBeInstanceOf(FigureLendingService);
      await moduleRef.close();
    });

    it('exports the Site.FIGURE_LENDING = "figure" enum value', () => {
      expect(Site.FIGURE_LENDING).toBe('figure');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new FigureLendingService();
      ...
      const job0 = dto.jobs.find((j) => j.id === 'figure-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.FIGURE_LENDING);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Figure Lending');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/figure/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/figure/jobs/');
      ...
      expect(job0?.description).toContain('Figure Lending');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/figure/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Figure Lending');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'figure-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
