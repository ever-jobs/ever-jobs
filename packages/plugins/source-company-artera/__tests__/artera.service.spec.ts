...
import { ArteraModule, ArteraService } from '../src';

const COMPANY_NAME_EXPECT = 'Artera';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'artera-jobs.json'), 'utf8'),
);
...
describe('ArteraService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ArteraModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ArteraModule],
      }).compile();
      const service = moduleRef.get(ArteraService);
      expect(service).toBeInstanceOf(ArteraService);
      await moduleRef.close();
    });

    it('exports the Site.ARTERA = "artera" enum value', () => {
      expect(Site.ARTERA).toBe('artera');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new ArteraService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.ARTERA],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('artera');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ARTERA);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('artera-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new ArteraService(registry);
      const result = await service.scrape({
        siteType: [Site.ARTERA],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('artera');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('artera-x1');
      expect(result.jobs[0].site).toBe(Site.ARTERA);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('artera-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new ArteraService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.ARTERA],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new ArteraService();
      const result = await service.scrape({
        siteType: [Site.ARTERA],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
