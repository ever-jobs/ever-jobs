...
import { ExtendModule, ExtendService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'extend-jobs.json'), 'utf8'),
);
...
/**
 * Spec 624 / T04 — `ExtendService` unit tests (≥ 9 cases).
 */
describe('ExtendService — Spec 624 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ExtendModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ExtendModule],
      }).compile();
      const service = moduleRef.get(ExtendService);
      expect(service).toBeInstanceOf(ExtendService);
      await moduleRef.close();
    });

    it('exports the Site.EXTEND = "extend" enum value', () => {
      expect(Site.EXTEND).toBe('extend');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ExtendService();
      const result = await service.scrape({
        siteType: [Site.EXTEND],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'extend-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.EXTEND);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Extend');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/extend/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/extend/jobs/');
      ...
      expect(job0?.description).toContain('Extend');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/extend/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Extend');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'extend-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
