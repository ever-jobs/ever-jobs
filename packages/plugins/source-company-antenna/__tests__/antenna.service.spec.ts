...
import { AntennaModule, AntennaService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'antenna-jobs.json'), 'utf8'),
);
...
/**
 * Spec 272 / T04 — `AntennaService` unit tests (≥ 9 cases).
 */
describe('AntennaService — Spec 272 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AntennaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AntennaModule],
      }).compile();
      const service = moduleRef.get(AntennaService);
      expect(service).toBeInstanceOf(AntennaService);
      await moduleRef.close();
    });

    it('exports the Site.ANTENNA = "antenna" enum value', () => {
      expect(Site.ANTENNA).toBe('antenna');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const job0 = dto.jobs.find((j) => j.id === 'antenna-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ANTENNA);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Antenna');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/antenna/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/antenna/jobs/');
      ...
      expect(job0?.description).toContain('Antenna');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/antenna/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Antenna');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'antenna-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
