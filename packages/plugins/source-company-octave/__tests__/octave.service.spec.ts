...
import { OctaveModule, OctaveService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'octave-jobs.json'), 'utf8'),
);
...
/**
 * Spec 527 / T04 — `OctaveService` unit tests (≥ 9 cases).
 */
describe('OctaveService — Spec 527 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through OctaveModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [OctaveModule],
      }).compile();
      const service = moduleRef.get(OctaveService);
      expect(service).toBeInstanceOf(OctaveService);
      await moduleRef.close();
    });

    it('exports the Site.OCTAVE_HEALTH = "octave" enum value', () => {
      expect(Site.OCTAVE_HEALTH).toBe('octave');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new OctaveService();
      const result = await service.scrape({
        siteType: [Site.OCTAVE_HEALTH],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'octave-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.OCTAVE_HEALTH);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Octave');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/octave/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/octave/jobs/');
      ...
      expect(job0?.description).toContain('Octave');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/octave/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Octave');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'octave-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
