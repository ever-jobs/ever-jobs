...
import { StandardSubseaModule, StandardSubseaService } from '../src';

const COMPANY_NAME_EXPECT = 'Standard Subsea';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'standardsubsea-jobs.json'), 'utf8'),
);
...
describe('StandardSubseaService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through StandardSubseaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [StandardSubseaModule],
      }).compile();
      const service = moduleRef.get(StandardSubseaService);
      expect(service).toBeInstanceOf(StandardSubseaService);
      await moduleRef.close();
    });

    it('exports the Site.STANDARD_SUBSEA = "standardsubsea" enum value', () => {
      expect(Site.STANDARD_SUBSEA).toBe('standardsubsea');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new StandardSubseaService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.STANDARD_SUBSEA],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'standardsubsea-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.STANDARD_SUBSEA);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('standardsubsea-' + first.id);
      ...
      expect(calledUrls[0]).toContain('standardsubsea');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.STANDARD_SUBSEA);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('standardsubsea-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new StandardSubseaService(registry);
      const result = await service.scrape({
        siteType: [Site.STANDARD_SUBSEA],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('standardsubsea');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('standardsubsea-x1');
      expect(result.jobs[0].site).toBe(Site.STANDARD_SUBSEA);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('standardsubsea-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new StandardSubseaService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.STANDARD_SUBSEA],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new StandardSubseaService();
      const result = await service.scrape({
        siteType: [Site.STANDARD_SUBSEA],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
