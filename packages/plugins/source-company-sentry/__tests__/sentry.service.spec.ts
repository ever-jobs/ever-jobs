...
const COMPANY_NAME_EXPECT = 'Sentry';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'sentry-jobs.json'), 'utf8'),
);
...
describe('SentryService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SentryModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SentryModule],
      }).compile();
      const service = moduleRef.get(SentryService);
      expect(service).toBeInstanceOf(SentryService);
      await moduleRef.close();
    });

    it('exports the Site.SENTRY = "sentry" enum value', () => {
      expect(Site.SENTRY).toBe('sentry');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new SentryService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.SENTRY],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('sentry');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SENTRY);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('sentry-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new SentryService(registry);
      const result = await service.scrape({
        siteType: [Site.SENTRY],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('sentry');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('sentry-x1');
      expect(result.jobs[0].site).toBe(Site.SENTRY);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('sentry-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new SentryService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.SENTRY],
      } as ScraperInputDto);
