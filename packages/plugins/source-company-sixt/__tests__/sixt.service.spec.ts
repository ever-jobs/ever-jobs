...
import { SIXTModule, SIXTService } from '../src';

const COMPANY_NAME_EXPECT = 'SIXT';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'sixt-jobs.json'), 'utf8'),
);
...
describe('SIXTService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SIXTModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SIXTModule],
      }).compile();
      const service = moduleRef.get(SIXTService);
      expect(service).toBeInstanceOf(SIXTService);
      await moduleRef.close();
    });

    it('exports the Site.SIXT = "sixt" enum value', () => {
      expect(Site.SIXT).toBe('sixt');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new SIXTService(registryWithSmartRecruiters());
      ...
      expect(calledUrls[0]).toContain('Sixt');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SIXT);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('sixt-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new SIXTService(registry);
      const result = await service.scrape({
        siteType: [Site.SIXT],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('Sixt');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('sixt-x1');
      expect(result.jobs[0].site).toBe(Site.SIXT);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('sixt-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new SIXTService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.SIXT],
      } as ScraperInputDto);
