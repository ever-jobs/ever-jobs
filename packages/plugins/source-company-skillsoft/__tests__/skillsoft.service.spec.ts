...
import { SkillsoftModule, SkillsoftService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'skillsoft-jobs.json'), 'utf8'),
);
...
/**
 * Spec 933 / T04 — `SkillsoftService` unit tests (≥ 9 cases).
 */
describe('SkillsoftService — Spec 933 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SkillsoftModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SkillsoftModule],
      }).compile();
      const service = moduleRef.get(SkillsoftService);
      expect(service).toBeInstanceOf(SkillsoftService);
      await moduleRef.close();
    });

    it('exports the Site.SKILLSOFT = "skillsoft" enum value', () => {
      expect(Site.SKILLSOFT).toBe('skillsoft');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new SkillsoftService();
      const result = await service.scrape({
        siteType: [Site.SKILLSOFT],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'skillsoft-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.SKILLSOFT);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Skillsoft');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/skillsoft/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/skillsoft/jobs/');
      ...
      expect(job0?.description).toContain('Skillsoft');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/skillsoft/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Skillsoft');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'skillsoft-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
