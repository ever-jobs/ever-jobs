...
import { GustiLederModule, GustiLederService } from '../src';

const COMPANY_NAME_EXPECT = 'Gusti Leder';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'gustileder-jobs.json'), 'utf8'),
);
...
describe('GustiLederService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through GustiLederModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [GustiLederModule],
      }).compile();
      const service = moduleRef.get(GustiLederService);
      expect(service).toBeInstanceOf(GustiLederService);
      await moduleRef.close();
    });

    it('exports the Site.GUSTI_LEDER = "gustileder" enum value', () => {
      expect(Site.GUSTI_LEDER).toBe('gustileder');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new GustiLederService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.GUSTI_LEDER],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('gustileder.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.GUSTI_LEDER);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('gustileder-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new GustiLederService(registry);
      const result = await service.scrape({
        siteType: [Site.GUSTI_LEDER],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('gustileder');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('gustileder-x1');
      expect(result.jobs[0].site).toBe(Site.GUSTI_LEDER);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('gustileder-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new GustiLederService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.GUSTI_LEDER],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new GustiLederService();
      const result = await service.scrape({
        siteType: [Site.GUSTI_LEDER],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
