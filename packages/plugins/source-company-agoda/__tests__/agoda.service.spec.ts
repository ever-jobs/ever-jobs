...
import { AgodaModule, AgodaService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'agoda-jobs.json'), 'utf8'),
);
...
/**
 * Spec 210 / T04 — `AgodaService` unit tests (≥ 9 cases).
 */
describe('AgodaService — Spec 210 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AgodaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AgodaModule],
      }).compile();
      const service = moduleRef.get(AgodaService);
      expect(service).toBeInstanceOf(AgodaService);
      await moduleRef.close();
    });

    it('exports the Site.AGODA = "agoda" enum value', () => {
      expect(Site.AGODA).toBe('agoda');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AgodaService();
      ...
      const job0 = dto.jobs.find((j) => j.id === 'agoda-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.AGODA);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Agoda');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/agoda/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/agoda/jobs/');
      ...
      expect(job0?.description).toContain('Agoda');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/agoda/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Agoda');
      }
    });
  });
