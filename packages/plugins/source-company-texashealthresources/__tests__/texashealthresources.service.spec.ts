...
import { TexasHealthResourcesModule, TexasHealthResourcesService } from '../src';

const COMPANY_NAME_EXPECT = 'Texas Health Resources';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'texashealthresources-jobs.json'), 'utf8'),
);
...
describe('TexasHealthResourcesService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through TexasHealthResourcesModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [TexasHealthResourcesModule],
      }).compile();
      const service = moduleRef.get(TexasHealthResourcesService);
      expect(service).toBeInstanceOf(TexasHealthResourcesService);
      await moduleRef.close();
    });

    it('exports the Site.TEXAS_HEALTH_RESOURCES = "texashealthresources" enum value', () => {
      expect(Site.TEXAS_HEALTH_RESOURCES).toBe('texashealthresources');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new TexasHealthResourcesService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.TEXAS_HEALTH_RESOURCES],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'texashealthresources-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.TEXAS_HEALTH_RESOURCES);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('texashealthresources-' + first.id);
      ...
      expect(calledUrls[0]).toContain('TexasHealthResources');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.TEXAS_HEALTH_RESOURCES);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('texashealthresources-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new TexasHealthResourcesService(registry);
      const result = await service.scrape({
        siteType: [Site.TEXAS_HEALTH_RESOURCES],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('TexasHealthResources');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('texashealthresources-x1');
      expect(result.jobs[0].site).toBe(Site.TEXAS_HEALTH_RESOURCES);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('texashealthresources-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new TexasHealthResourcesService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.TEXAS_HEALTH_RESOURCES],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new TexasHealthResourcesService();
      const result = await service.scrape({
        siteType: [Site.TEXAS_HEALTH_RESOURCES],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
