...
/**
 * Walmart — Retail (HQ: Bentonville, AR, USA).
 *
 * Source (Spec 1736): Workday board, scraped in this order:
 *   - `walmart:504:WalmartExternal` — https://walmart.wd504.myworkdayjobs.com/WalmartExternal
 ...
 * party board.
 *
 * Tags: segment=workday-enterprise; industry=retail.
 */
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'walmart:504:WalmartExternal', atsIdPrefix: 'wd-walmart-' },
];
...
@SourcePlugin({
  ...
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['walmart.com'],
  description: 'Walmart careers via Workday. Tags: segment=workday-enterprise; industry=retail.',
})
@Injectable()
export class WalmartService implements IScraper {
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Workday source plugin is not registered; cannot scrape Walmart');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`Walmart: delegating to Workday (${board.companySlug})`);
    ...
    this.logger.log(`Walmart: scraped ${jobs.length} jobs`);
