...
import { ImplyModule, ImplyService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'imply-jobs.json'), 'utf8'),
);
...
/**
 * Spec 456 / T04 — `ImplyService` unit tests (≥ 9 cases).
 */
describe('ImplyService — Spec 456 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ImplyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ImplyModule],
      }).compile();
      const service = moduleRef.get(ImplyService);
      expect(service).toBeInstanceOf(ImplyService);
      await moduleRef.close();
    });

    it('exports the Site.IMPLY = "imply" enum value', () => {
      expect(Site.IMPLY).toBe('imply');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ImplyService();
      ...
      const job0 = dto.jobs.find((j) => j.id === 'imply-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.IMPLY);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Imply');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/imply/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/imply/jobs/');
      ...
      expect(job0?.description).toContain('Imply');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/imply/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Imply');
      }
    });
  });
