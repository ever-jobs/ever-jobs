...
/**
 * Marvell Technology — Semiconductors (HQ: Wilmington, DE, USA).
 *
 * Source (Spec 1736): Workday board, scraped in this order:
 *   - `marvell:1:MarvellCareers` — https://marvell.wd1.myworkdayjobs.com/MarvellCareers
 *     verified live 2026-09-24: 246 open postings.
 *
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'marvell:1:MarvellCareers', atsIdPrefix: 'wd-marvell-' },
];
...
@SourcePlugin({
  ...
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['marvell.com'],
  description: 'Marvell Technology careers via Workday. Tags: segment=workday-enterprise; industry=semiconductors.',
})
@Injectable()
export class MarvellService implements IScraper {
  private readonly logger = new Logger(MarvellService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Workday source plugin is not registered; cannot scrape Marvell Technology');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`Marvell Technology: delegating to Workday (${board.companySlug})`);
    ...
    this.logger.log(`Marvell Technology: scraped ${jobs.length} jobs`);
