...
import { MintedModule, MintedService } from '../src';

const COMPANY_NAME_EXPECT = 'Minted';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'minted-jobs.json'), 'utf8'),
);
...
describe('MintedService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through MintedModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [MintedModule],
      }).compile();
      const service = moduleRef.get(MintedService);
      expect(service).toBeInstanceOf(MintedService);
      await moduleRef.close();
    });

    it('exports the Site.MINTED = "minted" enum value', () => {
      expect(Site.MINTED).toBe('minted');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new MintedService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.MINTED],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('minted');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.MINTED);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('minted-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new MintedService(registry);
      const result = await service.scrape({
        siteType: [Site.MINTED],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('minted');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('minted-x1');
      expect(result.jobs[0].site).toBe(Site.MINTED);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('minted-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new MintedService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.MINTED],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new MintedService();
      const result = await service.scrape({
        siteType: [Site.MINTED],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
