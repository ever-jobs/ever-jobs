...
import { EverlywellModule, EverlywellService } from '../src';

const COMPANY_NAME_EXPECT = 'Everlywell';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'everlywell-jobs.json'), 'utf8'),
);
...
describe('EverlywellService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through EverlywellModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [EverlywellModule],
      }).compile();
      const service = moduleRef.get(EverlywellService);
      expect(service).toBeInstanceOf(EverlywellService);
      await moduleRef.close();
    });

    it('exports the Site.EVERLYWELL = "everlywell" enum value', () => {
      expect(Site.EVERLYWELL).toBe('everlywell');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new EverlywellService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.EVERLYWELL],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('everlywell');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.EVERLYWELL);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('everlywell-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new EverlywellService(registry);
      const result = await service.scrape({
        siteType: [Site.EVERLYWELL],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('everlywell');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('everlywell-x1');
      expect(result.jobs[0].site).toBe(Site.EVERLYWELL);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('everlywell-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new EverlywellService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.EVERLYWELL],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new EverlywellService();
      const result = await service.scrape({
        siteType: [Site.EVERLYWELL],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
