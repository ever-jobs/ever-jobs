...
import { ApogeetherapeuticsModule, ApogeetherapeuticsService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'apogeetherapeutics-jobs.json'), 'utf8'),
);
...
/**
 * Spec 286 / T04 — `ApogeetherapeuticsService` unit tests (≥ 9 cases).
 */
describe('ApogeetherapeuticsService — Spec 286 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ApogeetherapeuticsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ApogeetherapeuticsModule],
      }).compile();
      const service = moduleRef.get(ApogeetherapeuticsService);
      expect(service).toBeInstanceOf(ApogeetherapeuticsService);
      await moduleRef.close();
    });

    it('exports the Site.APOGEETHERAPEUTICS = "apogeetherapeutics" enum value', () => {
      expect(Site.APOGEETHERAPEUTICS).toBe('apogeetherapeutics');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ApogeetherapeuticsService();
      const result = await service.scrape({
        siteType: [Site.APOGEETHERAPEUTICS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'apogeetherapeutics-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.APOGEETHERAPEUTICS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Apogee Therapeutics');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/apogeetherapeutics/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/apogeetherapeutics/jobs/');
      ...
      expect(job0?.description).toContain('Apogee Therapeutics');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/apogeetherapeutics/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Apogee Therapeutics');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'apogeetherapeutics-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
