...
import { DeepGenomicsModule, DeepGenomicsService } from '../src';

const COMPANY_NAME_EXPECT = 'Deep Genomics';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'deepgenomics-jobs.json'), 'utf8'),
);
...
describe('DeepGenomicsService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through DeepGenomicsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [DeepGenomicsModule],
      }).compile();
      const service = moduleRef.get(DeepGenomicsService);
      expect(service).toBeInstanceOf(DeepGenomicsService);
      await moduleRef.close();
    });

    it('exports the Site.DEEP_GENOMICS = "deepgenomics" enum value', () => {
      expect(Site.DEEP_GENOMICS).toBe('deepgenomics');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new DeepGenomicsService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.DEEP_GENOMICS],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'deepgenomics-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.DEEP_GENOMICS);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('deepgenomics-' + first.id);
      ...
      expect(calledUrls[0]).toContain('deepgenomics');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.DEEP_GENOMICS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('deepgenomics-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new DeepGenomicsService(registry);
      const result = await service.scrape({
        siteType: [Site.DEEP_GENOMICS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('deepgenomics');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('deepgenomics-x1');
      expect(result.jobs[0].site).toBe(Site.DEEP_GENOMICS);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('deepgenomics-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new DeepGenomicsService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.DEEP_GENOMICS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new DeepGenomicsService();
      const result = await service.scrape({
        siteType: [Site.DEEP_GENOMICS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
