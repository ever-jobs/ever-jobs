...
import { SolarisModule, SolarisService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'solarisbank-jobs.json'), 'utf8'),
);
...
/**
 * Spec 797 / T04 — `SolarisService` unit tests (≥ 9 cases).
 */
describe('SolarisService — Spec 797 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SolarisModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SolarisModule],
      }).compile();
      const service = moduleRef.get(SolarisService);
      expect(service).toBeInstanceOf(SolarisService);
      await moduleRef.close();
    });

    it('exports the Site.SOLARIS = "solarisbank" enum value', () => {
      expect(Site.SOLARIS).toBe('solarisbank');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new SolarisService();
      const result = await service.scrape({
        siteType: [Site.SOLARIS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'solarisbank-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.SOLARIS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Solaris');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/solarisbank/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/solarisbank/jobs/');
      ...
      expect(job0?.description).toContain('Solaris');
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Solaris');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'solarisbank-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
