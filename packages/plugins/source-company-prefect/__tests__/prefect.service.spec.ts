...
import { PrefectModule, PrefectService } from '../src';

const COMPANY_NAME_EXPECT = 'Prefect';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'prefect-jobs.json'), 'utf8'),
);
...
describe('PrefectService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through PrefectModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [PrefectModule],
      }).compile();
      const service = moduleRef.get(PrefectService);
      expect(service).toBeInstanceOf(PrefectService);
      await moduleRef.close();
    });

    it('exports the Site.PREFECT = "prefect" enum value', () => {
      expect(Site.PREFECT).toBe('prefect');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new PrefectService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.PREFECT],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('prefect');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.PREFECT);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('prefect-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new PrefectService(registry);
      const result = await service.scrape({
        siteType: [Site.PREFECT],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('prefect');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('prefect-x1');
      expect(result.jobs[0].site).toBe(Site.PREFECT);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('prefect-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new PrefectService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.PREFECT],
      } as ScraperInputDto);
