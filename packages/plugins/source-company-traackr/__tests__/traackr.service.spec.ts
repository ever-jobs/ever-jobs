...
import { TraackrModule, TraackrService } from '../src';

const COMPANY_NAME_EXPECT = 'Traackr';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'traackr-jobs.json'), 'utf8'),
);
...
describe('TraackrService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through TraackrModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [TraackrModule],
      }).compile();
      const service = moduleRef.get(TraackrService);
      expect(service).toBeInstanceOf(TraackrService);
      await moduleRef.close();
    });

    it('exports the Site.TRAACKR = "traackr" enum value', () => {
      expect(Site.TRAACKR).toBe('traackr');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new TraackrService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.TRAACKR],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('traackr');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.TRAACKR);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('traackr-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new TraackrService(registry);
      const result = await service.scrape({
        siteType: [Site.TRAACKR],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('traackr');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('traackr-x1');
      expect(result.jobs[0].site).toBe(Site.TRAACKR);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('traackr-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new TraackrService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.TRAACKR],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new TraackrService();
      const result = await service.scrape({
        siteType: [Site.TRAACKR],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
