...
import { ApaleoModule, ApaleoService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'apaleo-jobs.json'), 'utf8'),
);
...
/**
 * Spec 277 / T04 — `ApaleoService` unit tests (≥ 9 cases).
 */
describe('ApaleoService — Spec 277 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ApaleoModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ApaleoModule],
      }).compile();
      const service = moduleRef.get(ApaleoService);
      expect(service).toBeInstanceOf(ApaleoService);
      await moduleRef.close();
    });

    it('exports the Site.APALEO = "apaleo" enum value', () => {
      expect(Site.APALEO).toBe('apaleo');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ApaleoService();
      const result = await service.scrape({
        siteType: [Site.APALEO],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'apaleo-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.APALEO);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Apaleo');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/apaleo/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/apaleo/jobs/');
      ...
      expect(job0?.description).toContain('Apaleo');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/apaleo/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Apaleo');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'apaleo-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
