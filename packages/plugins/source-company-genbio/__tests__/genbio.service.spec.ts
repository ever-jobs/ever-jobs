...
import { GenBioAIModule, GenBioAIService } from '../src';

const COMPANY_NAME_EXPECT = 'GenBio AI';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'genbio-jobs.json'), 'utf8'),
);
...
describe('GenBioAIService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through GenBioAIModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [GenBioAIModule],
      }).compile();
      const service = moduleRef.get(GenBioAIService);
      expect(service).toBeInstanceOf(GenBioAIService);
      await moduleRef.close();
    });

    it('exports the Site.GENBIO_AI = "genbio" enum value', () => {
      expect(Site.GENBIO_AI).toBe('genbio');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new GenBioAIService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.GENBIO_AI],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('genbio');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.GENBIO_AI);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('genbio-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new GenBioAIService(registry);
      const result = await service.scrape({
        siteType: [Site.GENBIO_AI],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('genbio');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('genbio-x1');
      expect(result.jobs[0].site).toBe(Site.GENBIO_AI);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('genbio-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new GenBioAIService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.GENBIO_AI],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new GenBioAIService();
      const result = await service.scrape({
        siteType: [Site.GENBIO_AI],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
