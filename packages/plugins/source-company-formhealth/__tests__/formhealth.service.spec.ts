...
import { FormHealthModule, FormHealthService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'formhealth-jobs.json'), 'utf8'),
);
...
/**
 * Spec 669 / T04 — `FormHealthService` unit tests (≥ 9 cases).
 */
describe('FormHealthService — Spec 669 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through FormHealthModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [FormHealthModule],
      }).compile();
      const service = moduleRef.get(FormHealthService);
      expect(service).toBeInstanceOf(FormHealthService);
      await moduleRef.close();
    });

    it('exports the Site.FORM_HEALTH = "formhealth" enum value', () => {
      expect(Site.FORM_HEALTH).toBe('formhealth');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new FormHealthService();
      const result = await service.scrape({
        siteType: [Site.FORM_HEALTH],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'formhealth-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.FORM_HEALTH);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Form Health');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/formhealth/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/formhealth/jobs/');
      ...
      expect(job0?.description).toContain('Form Health');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/formhealth/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Form Health');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'formhealth-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
