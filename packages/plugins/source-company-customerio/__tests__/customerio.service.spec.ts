...
import { CustomerIoModule, CustomerIoService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'customerio-jobs.json'), 'utf8'),
);
...
/**
 * Spec 830 / T04 — `CustomerIoService` unit tests (≥ 9 cases).
 */
describe('CustomerIoService — Spec 830 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CustomerIoModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CustomerIoModule],
      }).compile();
      const service = moduleRef.get(CustomerIoService);
      expect(service).toBeInstanceOf(CustomerIoService);
      await moduleRef.close();
    });

    it('exports the Site.CUSTOMER_IO = "customerio" enum value', () => {
      expect(Site.CUSTOMER_IO).toBe('customerio');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new CustomerIoService();
      const result = await service.scrape({
        siteType: [Site.CUSTOMER_IO],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'customerio-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.CUSTOMER_IO);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Customer.io');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/customerio/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/customerio/jobs/');
      ...
      expect(job0?.description).toContain('Customer.io');
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Customer.io');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'customerio-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
