...
import { SonderMindModule, SonderMindService } from '../src';

const COMPANY_NAME_EXPECT = 'SonderMind';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'sondermind-jobs.json'), 'utf8'),
);
...
describe('SonderMindService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SonderMindModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SonderMindModule],
      }).compile();
      const service = moduleRef.get(SonderMindService);
      expect(service).toBeInstanceOf(SonderMindService);
      await moduleRef.close();
    });

    it('exports the Site.SONDERMIND = "sondermind" enum value', () => {
      expect(Site.SONDERMIND).toBe('sondermind');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new SonderMindService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.SONDERMIND],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('sondermind');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SONDERMIND);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('sondermind-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new SonderMindService(registry);
      const result = await service.scrape({
        siteType: [Site.SONDERMIND],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('sondermind');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('sondermind-x1');
      expect(result.jobs[0].site).toBe(Site.SONDERMIND);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('sondermind-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new SonderMindService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.SONDERMIND],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new SonderMindService();
      const result = await service.scrape({
        siteType: [Site.SONDERMIND],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
