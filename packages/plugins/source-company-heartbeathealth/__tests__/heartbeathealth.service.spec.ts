...
import { HeartbeatHealthModule, HeartbeatHealthService } from '../src';

const COMPANY_NAME_EXPECT = 'Heartbeat Health';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'heartbeathealth-jobs.json'), 'utf8'),
);
...
describe('HeartbeatHealthService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through HeartbeatHealthModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [HeartbeatHealthModule],
      }).compile();
      const service = moduleRef.get(HeartbeatHealthService);
      expect(service).toBeInstanceOf(HeartbeatHealthService);
      await moduleRef.close();
    });

    it('exports the Site.HEARTBEAT_HEALTH = "heartbeathealth" enum value', () => {
      expect(Site.HEARTBEAT_HEALTH).toBe('heartbeathealth');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new HeartbeatHealthService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.HEARTBEAT_HEALTH],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'heartbeathealth-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.HEARTBEAT_HEALTH);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('heartbeathealth-' + first.id);
      ...
      expect(calledUrls[0]).toContain('heartbeathealth');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.HEARTBEAT_HEALTH);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('heartbeathealth-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new HeartbeatHealthService(registry);
      const result = await service.scrape({
        siteType: [Site.HEARTBEAT_HEALTH],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('heartbeathealth');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('heartbeathealth-x1');
      expect(result.jobs[0].site).toBe(Site.HEARTBEAT_HEALTH);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('heartbeathealth-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new HeartbeatHealthService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.HEARTBEAT_HEALTH],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new HeartbeatHealthService();
      const result = await service.scrape({
        siteType: [Site.HEARTBEAT_HEALTH],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
