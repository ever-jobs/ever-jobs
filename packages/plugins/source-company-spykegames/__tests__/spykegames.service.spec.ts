...
import { SpykeGamesModule, SpykeGamesService } from '../src';

const COMPANY_NAME_EXPECT = 'Spyke Games';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'spykegames-jobs.json'), 'utf8'),
);
...
describe('SpykeGamesService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SpykeGamesModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SpykeGamesModule],
      }).compile();
      const service = moduleRef.get(SpykeGamesService);
      expect(service).toBeInstanceOf(SpykeGamesService);
      await moduleRef.close();
    });

    it('exports the Site.SPYKE_GAMES = "spykegames" enum value', () => {
      expect(Site.SPYKE_GAMES).toBe('spykegames');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new SpykeGamesService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.SPYKE_GAMES],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('spyke-games');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SPYKE_GAMES);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('spykegames-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new SpykeGamesService(registry);
      const result = await service.scrape({
        siteType: [Site.SPYKE_GAMES],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('spyke-games');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('spykegames-x1');
      expect(result.jobs[0].site).toBe(Site.SPYKE_GAMES);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('spykegames-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new SpykeGamesService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.SPYKE_GAMES],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new SpykeGamesService();
      const result = await service.scrape({
        siteType: [Site.SPYKE_GAMES],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
