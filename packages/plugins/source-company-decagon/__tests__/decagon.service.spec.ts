...
import { DecagonModule, DecagonService } from '../src';

const COMPANY_NAME_EXPECT = 'Decagon';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'decagon-jobs.json'), 'utf8'),
);
...
describe('DecagonService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through DecagonModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [DecagonModule],
      }).compile();
      const service = moduleRef.get(DecagonService);
      expect(service).toBeInstanceOf(DecagonService);
      await moduleRef.close();
    });

    it('exports the Site.DECAGON = "decagon" enum value', () => {
      expect(Site.DECAGON).toBe('decagon');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new DecagonService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.DECAGON],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('decagon');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.DECAGON);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('decagon-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new DecagonService(registry);
      const result = await service.scrape({
        siteType: [Site.DECAGON],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('decagon');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('decagon-x1');
      expect(result.jobs[0].site).toBe(Site.DECAGON);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('decagon-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new DecagonService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.DECAGON],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new DecagonService();
      const result = await service.scrape({
        siteType: [Site.DECAGON],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
