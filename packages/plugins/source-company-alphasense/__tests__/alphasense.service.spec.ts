...
import { AlphasenseModule, AlphasenseService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'alphasense-jobs.json'), 'utf8'),
);
...
/**
 * Spec 243 / T04 — `AlphasenseService` unit tests (≥ 9 cases).
 */
describe('AlphasenseService — Spec 243 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AlphasenseModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AlphasenseModule],
      }).compile();
      const service = moduleRef.get(AlphasenseService);
      expect(service).toBeInstanceOf(AlphasenseService);
      await moduleRef.close();
    });

    it('exports the Site.ALPHASENSE = "alphasense" enum value', () => {
      expect(Site.ALPHASENSE).toBe('alphasense');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AlphasenseService();
      const result = await service.scrape({
        siteType: [Site.ALPHASENSE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'alphasense-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ALPHASENSE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('AlphaSense');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/alphasense/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/alphasense/jobs/');
      ...
      expect(job0?.description).toContain('AlphaSense');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/alphasense/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('AlphaSense');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'alphasense-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
