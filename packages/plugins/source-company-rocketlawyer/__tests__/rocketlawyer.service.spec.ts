...
import { RocketLawyerModule, RocketLawyerService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'rocketlawyer-jobs.json'), 'utf8'),
);
...
/**
 * Spec 922 / T04 — `RocketLawyerService` unit tests (≥ 9 cases).
 */
describe('RocketLawyerService — Spec 922 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through RocketLawyerModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [RocketLawyerModule],
      }).compile();
      const service = moduleRef.get(RocketLawyerService);
      expect(service).toBeInstanceOf(RocketLawyerService);
      await moduleRef.close();
    });

    it('exports the Site.ROCKET_LAWYER = "rocketlawyer" enum value', () => {
      expect(Site.ROCKET_LAWYER).toBe('rocketlawyer');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new RocketLawyerService();
      const result = await service.scrape({
        siteType: [Site.ROCKET_LAWYER],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'rocketlawyer-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ROCKET_LAWYER);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Rocket Lawyer');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/rocketlawyer/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/rocketlawyer/jobs/');
      ...
      expect(job0?.description).toContain('Rocket Lawyer');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/rocketlawyer/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Rocket Lawyer');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'rocketlawyer-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
