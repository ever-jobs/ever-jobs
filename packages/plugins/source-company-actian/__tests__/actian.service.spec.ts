...
import { ActianModule, ActianService } from '../src';

const COMPANY_NAME_EXPECT = 'Actian';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'actian-jobs.json'), 'utf8'),
);
...
describe('ActianService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ActianModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ActianModule],
      }).compile();
      const service = moduleRef.get(ActianService);
      expect(service).toBeInstanceOf(ActianService);
      await moduleRef.close();
    });

    it('exports the Site.ACTIAN = "actian" enum value', () => {
      expect(Site.ACTIAN).toBe('actian');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new ActianService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.ACTIAN],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('actian');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ACTIAN);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('actian-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new ActianService(registry);
      const result = await service.scrape({
        siteType: [Site.ACTIAN],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('actian');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('actian-x1');
      expect(result.jobs[0].site).toBe(Site.ACTIAN);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('actian-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new ActianService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.ACTIAN],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new ActianService();
      const result = await service.scrape({
        siteType: [Site.ACTIAN],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
