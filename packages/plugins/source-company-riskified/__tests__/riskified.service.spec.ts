...
import { RiskifiedModule, RiskifiedService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'riskified-jobs.json'), 'utf8'),
);
...
/**
 * Spec 920 / T04 — `RiskifiedService` unit tests (≥ 9 cases).
 */
describe('RiskifiedService — Spec 920 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through RiskifiedModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [RiskifiedModule],
      }).compile();
      const service = moduleRef.get(RiskifiedService);
      expect(service).toBeInstanceOf(RiskifiedService);
      await moduleRef.close();
    });

    it('exports the Site.RISKIFIED = "riskified" enum value', () => {
      expect(Site.RISKIFIED).toBe('riskified');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new RiskifiedService();
      const result = await service.scrape({
        siteType: [Site.RISKIFIED],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'riskified-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.RISKIFIED);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Riskified');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/riskified/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/riskified/jobs/');
      ...
      expect(job0?.description).toContain('Riskified');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/riskified/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Riskified');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'riskified-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
