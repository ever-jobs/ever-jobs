...
import { FietsenwinkelNlModule, FietsenwinkelNlService } from '../src';

const COMPANY_NAME_EXPECT = 'Fietsenwinkel.nl';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'fietsenwinkelnl-jobs.json'), 'utf8'),
);
...
describe('FietsenwinkelNlService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through FietsenwinkelNlModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [FietsenwinkelNlModule],
      }).compile();
      const service = moduleRef.get(FietsenwinkelNlService);
      expect(service).toBeInstanceOf(FietsenwinkelNlService);
      await moduleRef.close();
    });

    it('exports the Site.FIETSENWINKEL_NL = "fietsenwinkelnl" enum value', () => {
      expect(Site.FIETSENWINKEL_NL).toBe('fietsenwinkelnl');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new FietsenwinkelNlService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.FIETSENWINKEL_NL],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'fietsenwinkelnl-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Recruitee's defaults
      expect(job0?.site).toBe(Site.FIETSENWINKEL_NL);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('fietsenwinkelnl-' + first.id);
      ...
      expect(calledUrls[0]).toContain('fietsenwinkelnl.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.FIETSENWINKEL_NL);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('fietsenwinkelnl-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new FietsenwinkelNlService(registry);
      const result = await service.scrape({
        siteType: [Site.FIETSENWINKEL_NL],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('fietsenwinkelnl');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('fietsenwinkelnl-x1');
      expect(result.jobs[0].site).toBe(Site.FIETSENWINKEL_NL);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('fietsenwinkelnl-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new FietsenwinkelNlService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.FIETSENWINKEL_NL],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new FietsenwinkelNlService();
      const result = await service.scrape({
        siteType: [Site.FIETSENWINKEL_NL],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
