...
import { DeloitteNordicModule, DeloitteNordicService } from '../src';

const COMPANY_NAME_EXPECT = 'Deloitte (Nordic)';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'deloittenordic-jobs.json'), 'utf8'),
);
...
describe('DeloitteNordicService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through DeloitteNordicModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [DeloitteNordicModule],
      }).compile();
      const service = moduleRef.get(DeloitteNordicService);
      expect(service).toBeInstanceOf(DeloitteNordicService);
      await moduleRef.close();
    });

    it('exports the Site.DELOITTE_NORDIC = "deloittenordic" enum value', () => {
      expect(Site.DELOITTE_NORDIC).toBe('deloittenordic');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new DeloitteNordicService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.DELOITTE_NORDIC],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'deloittenordic-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.DELOITTE_NORDIC);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('deloittenordic-' + first.id);
      ...
      expect(calledUrls[0]).toContain('DeloitteNordic');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.DELOITTE_NORDIC);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('deloittenordic-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new DeloitteNordicService(registry);
      const result = await service.scrape({
        siteType: [Site.DELOITTE_NORDIC],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('DeloitteNordic');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('deloittenordic-x1');
      expect(result.jobs[0].site).toBe(Site.DELOITTE_NORDIC);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('deloittenordic-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new DeloitteNordicService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.DELOITTE_NORDIC],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new DeloitteNordicService();
      const result = await service.scrape({
        siteType: [Site.DELOITTE_NORDIC],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
