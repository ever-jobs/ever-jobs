...
/**
 * Five Rings — Proprietary trading (HQ: New York, NY, USA).
 *
 * Source (Spec 1737): Greenhouse board, scraped in this order:
 *   - `fiveringsllc` — https://job-boards.greenhouse.io/fiveringsllc
 *     verified live 2026-09-24: 16 open postings.
 *
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'fiveringsllc', atsIdPrefix: 'gh-' },
];

@SourcePlugin({
  site: Site.FIVE_RINGS,
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['fiverings.com'],
  description: 'Five Rings careers via Greenhouse. Tags: segment=quant-trading; industry=proprietary-trading.',
})
@Injectable()
export class FiveRingsService implements IScraper {
  private readonly logger = new Logger(FiveRingsService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Greenhouse source plugin is not registered; cannot scrape Five Rings');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`Five Rings: delegating to Greenhouse (${board.companySlug})`);
    ...
    this.logger.log(`Five Rings: scraped ${jobs.length} jobs`);
