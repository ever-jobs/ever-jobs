...
import { EverlawModule, EverlawService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'everlaw-jobs.json'), 'utf8'),
);
...
/**
 * Spec 464 / T04 — `EverlawService` unit tests (≥ 9 cases).
 */
describe('EverlawService — Spec 464 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through EverlawModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [EverlawModule],
      }).compile();
      const service = moduleRef.get(EverlawService);
      expect(service).toBeInstanceOf(EverlawService);
      await moduleRef.close();
    });

    it('exports the Site.EVERLAW = "everlaw" enum value', () => {
      expect(Site.EVERLAW).toBe('everlaw');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new EverlawService();
      const result = await service.scrape({
        siteType: [Site.EVERLAW],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'everlaw-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.EVERLAW);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Everlaw');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/everlaw/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/everlaw/jobs/');
      ...
      expect(job0?.description).toContain('Everlaw');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/everlaw/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Everlaw');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'everlaw-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
