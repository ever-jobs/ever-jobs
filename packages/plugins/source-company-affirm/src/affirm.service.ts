...
/**
 * Affirm Holdings, Inc. — buy-now-pay-later / consumer-credit-fintech /
 * merchant-checkout-financing parent — publishes its consolidated careers
 * board through Greenhouse at the bare `affirm` slug (no asymmetry; see
 * Spec 044 § 10 D-05).
 */
const API_URL = 'https://api.greenhouse.io/v1/boards/affirm/jobs';
...
export class AffirmService implements IScraper {
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    try {
      ...
      this.logger.log(`Affirm: fetching ${url}`);
        ...
        jobs.push(
          new JobPostDto({
            ...
            jobUrl:
              ...
              `https://job-boards.greenhouse.io/affirm/jobs/${listing.id}`,
            location,
      ...
      this.logger.log(`Affirm: scraped ${jobs.length} jobs`);
    } catch (err: any) {
      this.logger.error(`Affirm scrape failed: ${err.message}`);
