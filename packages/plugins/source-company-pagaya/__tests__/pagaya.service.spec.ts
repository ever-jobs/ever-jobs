...
import { PagayaModule, PagayaService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'pagaya-jobs.json'), 'utf8'),
);
...
/**
 * Spec 617 / T04 — `PagayaService` unit tests (≥ 9 cases).
 */
describe('PagayaService — Spec 617 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through PagayaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [PagayaModule],
      }).compile();
      const service = moduleRef.get(PagayaService);
      expect(service).toBeInstanceOf(PagayaService);
      await moduleRef.close();
    });

    it('exports the Site.PAGAYA = "pagaya" enum value', () => {
      expect(Site.PAGAYA).toBe('pagaya');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new PagayaService();
      const result = await service.scrape({
        siteType: [Site.PAGAYA],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'pagaya-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.PAGAYA);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Pagaya');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/pagaya/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/pagaya/jobs/');
      ...
      expect(job0?.description).toContain('Pagaya');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/pagaya/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Pagaya');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'pagaya-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
