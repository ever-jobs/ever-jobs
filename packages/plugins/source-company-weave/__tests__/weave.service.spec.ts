...
import { WeaveModule, WeaveService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'weave-jobs.json'), 'utf8'),
);
...
/**
 * Spec 635 / T04 — `WeaveService` unit tests (≥ 9 cases).
 */
describe('WeaveService — Spec 635 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through WeaveModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [WeaveModule],
      }).compile();
      const service = moduleRef.get(WeaveService);
      expect(service).toBeInstanceOf(WeaveService);
      await moduleRef.close();
    });

    it('exports the Site.WEAVE = "weave" enum value', () => {
      expect(Site.WEAVE).toBe('weave');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new WeaveService();
      ...
      const job0 = dto.jobs.find((j) => j.id === 'weave-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.WEAVE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Weave');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/weave/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/weave/jobs/');
      ...
      expect(job0?.description).toContain('Weave');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/weave/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Weave');
      }
    });
  });
