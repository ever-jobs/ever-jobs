...
import { ForterModule, ForterService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'forter-jobs.json'), 'utf8'),
);
...
/**
 * Spec 852 / T04 — `ForterService` unit tests (≥ 9 cases).
 */
describe('ForterService — Spec 852 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ForterModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ForterModule],
      }).compile();
      const service = moduleRef.get(ForterService);
      expect(service).toBeInstanceOf(ForterService);
      await moduleRef.close();
    });

    it('exports the Site.FORTER = "forter" enum value', () => {
      expect(Site.FORTER).toBe('forter');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ForterService();
      const result = await service.scrape({
        siteType: [Site.FORTER],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'forter-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.FORTER);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Forter');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/forter/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/forter/jobs/');
      ...
      expect(job0?.description).toContain('Forter');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/forter/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Forter');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'forter-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
