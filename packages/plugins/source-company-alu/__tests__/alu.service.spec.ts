...
import { AluModule, AluService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'alu-jobs.json'), 'utf8'),
);
...
/**
 * Spec 251 / T04 — `AluService` unit tests (≥ 9 cases).
 */
describe('AluService — Spec 251 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AluModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AluModule],
      }).compile();
      const service = moduleRef.get(AluService);
      expect(service).toBeInstanceOf(AluService);
      await moduleRef.close();
    });

    it('exports the Site.ALU = "alu" enum value', () => {
      expect(Site.ALU).toBe('alu');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const job0 = dto.jobs.find((j) => j.id === 'alu-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ALU);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('ALU');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/alu/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/alu/jobs/');
      ...
      expect(job0?.description).toContain('ALU');
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('ALU');
      }
    });
  });
