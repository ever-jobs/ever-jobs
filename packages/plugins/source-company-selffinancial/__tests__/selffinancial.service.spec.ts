...
import { SelfFinancialModule, SelfFinancialService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'selffinancial-jobs.json'), 'utf8'),
);
...
/**
 * Spec 706 / T04 — `SelfFinancialService` unit tests (≥ 9 cases).
 */
describe('SelfFinancialService — Spec 706 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SelfFinancialModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SelfFinancialModule],
      }).compile();
      const service = moduleRef.get(SelfFinancialService);
      expect(service).toBeInstanceOf(SelfFinancialService);
      await moduleRef.close();
    });

    it('exports the Site.SELF_FINANCIAL = "selffinancial" enum value', () => {
      expect(Site.SELF_FINANCIAL).toBe('selffinancial');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new SelfFinancialService();
      const result = await service.scrape({
        siteType: [Site.SELF_FINANCIAL],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'selffinancial-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.SELF_FINANCIAL);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Self Financial');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/selffinancial/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/selffinancial/jobs/');
      ...
      expect(job0?.description).toContain('Self Financial');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/selffinancial/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Self Financial');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'selffinancial-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
