...
import { AdvancedspaceModule, AdvancedspaceService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'advancedspace-jobs.json'), 'utf8'),
);
...
/**
 * Spec 192 / T04 — `AdvancedspaceService` unit tests (≥ 9 cases).
 */
describe('AdvancedspaceService — Spec 192 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AdvancedspaceModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AdvancedspaceModule],
      }).compile();
      const service = moduleRef.get(AdvancedspaceService);
      expect(service).toBeInstanceOf(AdvancedspaceService);
      await moduleRef.close();
    });

    it('exports the Site.ADVANCEDSPACE = "advancedspace" enum value', () => {
      expect(Site.ADVANCEDSPACE).toBe('advancedspace');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AdvancedspaceService();
      const result = await service.scrape({
        siteType: [Site.ADVANCEDSPACE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'advancedspace-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ADVANCEDSPACE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Advanced Space');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/advancedspace/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/advancedspace/jobs/');
      ...
      expect(job0?.description).toContain('Advanced Space');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/advancedspace/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Advanced Space');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'advancedspace-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
