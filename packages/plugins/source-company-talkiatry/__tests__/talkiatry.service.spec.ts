...
import { TalkiatryModule, TalkiatryService } from '../src';

const COMPANY_NAME_EXPECT = 'Talkiatry';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'talkiatry-jobs.json'), 'utf8'),
);
...
describe('TalkiatryService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through TalkiatryModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [TalkiatryModule],
      }).compile();
      const service = moduleRef.get(TalkiatryService);
      expect(service).toBeInstanceOf(TalkiatryService);
      await moduleRef.close();
    });

    it('exports the Site.TALKIATRY = "talkiatry" enum value', () => {
      expect(Site.TALKIATRY).toBe('talkiatry');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new TalkiatryService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.TALKIATRY],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('talkiatry');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.TALKIATRY);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('talkiatry-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new TalkiatryService(registry);
      const result = await service.scrape({
        siteType: [Site.TALKIATRY],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('talkiatry');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('talkiatry-x1');
      expect(result.jobs[0].site).toBe(Site.TALKIATRY);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('talkiatry-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new TalkiatryService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.TALKIATRY],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new TalkiatryService();
      const result = await service.scrape({
        siteType: [Site.TALKIATRY],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
