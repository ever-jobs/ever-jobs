...
import { DeliveryHeroModule, DeliveryHeroService } from '../src';

const COMPANY_NAME_EXPECT = 'Delivery Hero';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'deliveryhero-jobs.json'), 'utf8'),
);
...
describe('DeliveryHeroService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through DeliveryHeroModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [DeliveryHeroModule],
      }).compile();
      const service = moduleRef.get(DeliveryHeroService);
      expect(service).toBeInstanceOf(DeliveryHeroService);
      await moduleRef.close();
    });

    it('exports the Site.DELIVERY_HERO = "deliveryhero" enum value', () => {
      expect(Site.DELIVERY_HERO).toBe('deliveryhero');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new DeliveryHeroService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.DELIVERY_HERO],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('DeliveryHero');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.DELIVERY_HERO);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('deliveryhero-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new DeliveryHeroService(registry);
      const result = await service.scrape({
        siteType: [Site.DELIVERY_HERO],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('DeliveryHero');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('deliveryhero-x1');
      expect(result.jobs[0].site).toBe(Site.DELIVERY_HERO);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('deliveryhero-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new DeliveryHeroService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.DELIVERY_HERO],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new DeliveryHeroService();
      const result = await service.scrape({
        siteType: [Site.DELIVERY_HERO],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
