...
import { InnovidModule, InnovidService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'innovid-jobs.json'), 'utf8'),
);
...
/**
 * Spec 869 / T04 — `InnovidService` unit tests (≥ 9 cases).
 */
describe('InnovidService — Spec 869 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through InnovidModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [InnovidModule],
      }).compile();
      const service = moduleRef.get(InnovidService);
      expect(service).toBeInstanceOf(InnovidService);
      await moduleRef.close();
    });

    it('exports the Site.INNOVID = "innovid" enum value', () => {
      expect(Site.INNOVID).toBe('innovid');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new InnovidService();
      const result = await service.scrape({
        siteType: [Site.INNOVID],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'innovid-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.INNOVID);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Innovid');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/innovid/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/innovid/jobs/');
      ...
      expect(job0?.description).toContain('Innovid');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/innovid/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Innovid');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'innovid-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
