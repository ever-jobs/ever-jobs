...
import { EgisGroupModule, EgisGroupService } from '../src';

const COMPANY_NAME_EXPECT = 'Egis Group';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'egisgroup-jobs.json'), 'utf8'),
);
...
describe('EgisGroupService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through EgisGroupModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [EgisGroupModule],
      }).compile();
      const service = moduleRef.get(EgisGroupService);
      expect(service).toBeInstanceOf(EgisGroupService);
      await moduleRef.close();
    });

    it('exports the Site.EGIS_GROUP = "egisgroup" enum value', () => {
      expect(Site.EGIS_GROUP).toBe('egisgroup');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new EgisGroupService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.EGIS_GROUP],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('EgisGroup');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.EGIS_GROUP);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('egisgroup-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new EgisGroupService(registry);
      const result = await service.scrape({
        siteType: [Site.EGIS_GROUP],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('EgisGroup');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('egisgroup-x1');
      expect(result.jobs[0].site).toBe(Site.EGIS_GROUP);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('egisgroup-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new EgisGroupService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.EGIS_GROUP],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new EgisGroupService();
      const result = await service.scrape({
        siteType: [Site.EGIS_GROUP],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
