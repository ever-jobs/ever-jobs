...
import { AltanaaiModule, AltanaaiService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'altanaai-jobs.json'), 'utf8'),
);
...
/**
 * Spec 246 / T04 — `AltanaaiService` unit tests (≥ 9 cases).
 */
describe('AltanaaiService — Spec 246 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AltanaaiModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AltanaaiModule],
      }).compile();
      const service = moduleRef.get(AltanaaiService);
      expect(service).toBeInstanceOf(AltanaaiService);
      await moduleRef.close();
    });

    it('exports the Site.ALTANAAI = "altanaai" enum value', () => {
      expect(Site.ALTANAAI).toBe('altanaai');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AltanaaiService();
      const result = await service.scrape({
        siteType: [Site.ALTANAAI],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'altanaai-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ALTANAAI);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Altana ');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/altanaai/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/altanaai/jobs/');
      ...
      expect(job0?.description).toContain('Altana ');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/altanaai/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Altana ');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'altanaai-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
