...
import { RedHatModule, RedHatService } from '../src';
...
const FIXTURE: Fixture = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'fixtures', 'redhat-boards.json'), 'utf8'),
);
const COMPANY_NAME = 'Red Hat';
const ID_PREFIX = 'redhat-';
...
describe('RedHatService — Workday delegation (Spec 1736)', () => {
  beforeEach(() => {
    mockGet.mockReset();
    mockPost.mockReset();
  ...
  describe('registration', () => {
    it('resolves through RedHatModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({ imports: [RedHatModule] }).compile();
      expect(moduleRef.get(RedHatService)).toBeInstanceOf(RedHatService);
      await moduleRef.close();
    });

    it('exports Site.RED_HAT = "redhat"', () => {
      expect(Site.RED_HAT).toBe('redhat');
    });

    it('declares a tagged company plugin with its domains', () => {
      const meta = Reflect.getMetadata(SOURCE_PLUGIN_METADATA, RedHatService);
      expect(meta.site).toBe(Site.RED_HAT);
      expect(meta.category).toBe('company');
      expect(meta.isAts).toBeFalsy();
      expect(meta.companyDomains).toEqual(['redhat.com']);
  ...
  describe('recorded board (real Workday adapter, mocked HTTP)', () => {
    it('maps every recorded posting and re-stamps the company identity', async () => {
      const service = new RedHatService(registryWith());
      const result = await service.scrape({ siteType: [Site.RED_HAT], resultsWanted: 100 } as ScraperInputDto);
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.RED_HAT);
    ...
    it('only requests the recorded board URLs', async () => {
      const service = new RedHatService(registryWith());
      await service.scrape({ siteType: [Site.RED_HAT] } as ScraperInputDto);
    ...
    it('honours resultsWanted=1', async () => {
      const service = new RedHatService(registryWith());
      const result = await service.scrape({ siteType: [Site.RED_HAT], resultsWanted: 1 } as ScraperInputDto);
    ...
    it('resolves with an empty result when the board is gone (HTTP 404)', async () => {
      ...
      const service = new RedHatService(registryWith());
      const result = await service.scrape({ siteType: [Site.RED_HAT] } as ScraperInputDto);
  ...
  describe('delegation contract', () => {
    it('forwards the board and the caller input untouched', async () => {
      ...
      const service = new RedHatService(
        registryWith(
          fakeBackend(
      ...
      expect(result.jobs[0].site).toBe(Site.RED_HAT);
      expect(result.jobs[0].companyName).toBe(COMPANY_NAME);
    });

    it('passes an absent resultsWanted through as absent', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new RedHatService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      await service.scrape({ siteType: [Site.RED_HAT] } as ScraperInputDto);
    ...
    it('makes no request when resultsWanted is 0', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new RedHatService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      const result = await service.scrape({ siteType: [Site.RED_HAT], resultsWanted: 0 } as ScraperInputDto);
  ...
  describe('resilience', () => {
    it('reports not_registered when no registry is injected', async () => {
      const result = await new RedHatService().scrape({ siteType: [Site.RED_HAT] } as ScraperInputDto);
      expect(result.jobs).toEqual([]);
    ...
    it('reports not_registered when Workday is missing from the registry', async () => {
      const result = await new RedHatService(new PluginRegistry()).scrape({
        siteType: [Site.RED_HAT],
      } as ScraperInputDto);
      expect(result.diagnostics?.reason).toBe('not_registered');
    });
