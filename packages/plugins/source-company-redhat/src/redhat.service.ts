...
/**
 * Red Hat — Open-source software (HQ: Raleigh, NC, USA).
 *
 * Source (Spec 1736): Workday board, scraped in this order:
 *   - `redhat:5:jobs` — https://redhat.wd5.myworkdayjobs.com/jobs
 *     verified live 2026-09-24: 161 open postings.
 *
 ...
 * party board.
 *
 * Tags: segment=workday-enterprise; industry=open-source-software.
 */
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'redhat:5:jobs', atsIdPrefix: 'wd-redhat-' },
];
...
@SourcePlugin({
  ...
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['redhat.com'],
  description: 'Red Hat careers via Workday. Tags: segment=workday-enterprise; industry=open-source-software.',
})
@Injectable()
export class RedHatService implements IScraper {
  private readonly logger = new Logger(RedHatService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Workday source plugin is not registered; cannot scrape Red Hat');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`Red Hat: delegating to Workday (${board.companySlug})`);
    ...
    this.logger.log(`Red Hat: scraped ${jobs.length} jobs`);
