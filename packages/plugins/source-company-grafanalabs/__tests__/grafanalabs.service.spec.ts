...
import { GrafanalabsModule, GrafanalabsService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'grafanalabs-jobs.json'), 'utf8'),
);
...
/**
 * Spec 431 / T04 — `GrafanalabsService` unit tests (≥ 9 cases).
 */
describe('GrafanalabsService — Spec 431 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through GrafanalabsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [GrafanalabsModule],
      }).compile();
      const service = moduleRef.get(GrafanalabsService);
      expect(service).toBeInstanceOf(GrafanalabsService);
      await moduleRef.close();
    });

    it('exports the Site.GRAFANALABS = "grafanalabs" enum value', () => {
      expect(Site.GRAFANALABS).toBe('grafanalabs');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new GrafanalabsService();
      const result = await service.scrape({
        siteType: [Site.GRAFANALABS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'grafanalabs-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.GRAFANALABS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Grafana Labs');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/grafanalabs/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/grafanalabs/jobs/');
      ...
      expect(job0?.description).toContain('Grafana Labs');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/grafanalabs/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Grafana Labs');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'grafanalabs-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
