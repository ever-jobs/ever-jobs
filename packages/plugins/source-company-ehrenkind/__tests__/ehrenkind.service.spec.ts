...
import { EhrenkindModule, EhrenkindService } from '../src';

const COMPANY_NAME_EXPECT = 'Ehrenkind';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'ehrenkind-jobs.json'), 'utf8'),
);
...
describe('EhrenkindService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through EhrenkindModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [EhrenkindModule],
      }).compile();
      const service = moduleRef.get(EhrenkindService);
      expect(service).toBeInstanceOf(EhrenkindService);
      await moduleRef.close();
    });

    it('exports the Site.EHRENKIND = "ehrenkind" enum value', () => {
      expect(Site.EHRENKIND).toBe('ehrenkind');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new EhrenkindService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.EHRENKIND],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('ehrenkindgmbh.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.EHRENKIND);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('ehrenkind-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new EhrenkindService(registry);
      const result = await service.scrape({
        siteType: [Site.EHRENKIND],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('ehrenkindgmbh');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('ehrenkind-x1');
      expect(result.jobs[0].site).toBe(Site.EHRENKIND);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('ehrenkind-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new EhrenkindService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.EHRENKIND],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new EhrenkindService();
      const result = await service.scrape({
        siteType: [Site.EHRENKIND],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
