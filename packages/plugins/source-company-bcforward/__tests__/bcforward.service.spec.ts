...
import { BCforwardModule, BCforwardService } from '../src';

const COMPANY_NAME_EXPECT = 'BCforward';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'bcforward-jobs.json'), 'utf8'),
);
...
describe('BCforwardService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through BCforwardModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [BCforwardModule],
      }).compile();
      const service = moduleRef.get(BCforwardService);
      expect(service).toBeInstanceOf(BCforwardService);
      await moduleRef.close();
    });

    it('exports the Site.BCFORWARD = "bcforward" enum value', () => {
      expect(Site.BCFORWARD).toBe('bcforward');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new BCforwardService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.BCFORWARD],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('BCforward3');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.BCFORWARD);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('bcforward-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new BCforwardService(registry);
      const result = await service.scrape({
        siteType: [Site.BCFORWARD],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('BCforward3');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('bcforward-x1');
      expect(result.jobs[0].site).toBe(Site.BCFORWARD);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('bcforward-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new BCforwardService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.BCFORWARD],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new BCforwardService();
      const result = await service.scrape({
        siteType: [Site.BCFORWARD],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
