...
import { AsporaModule, AsporaService } from '../src';

const COMPANY_NAME_EXPECT = 'Aspora';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'aspora-jobs.json'), 'utf8'),
);
...
describe('AsporaService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AsporaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AsporaModule],
      }).compile();
      const service = moduleRef.get(AsporaService);
      expect(service).toBeInstanceOf(AsporaService);
      await moduleRef.close();
    });

    it('exports the Site.ASPORA = "aspora" enum value', () => {
      expect(Site.ASPORA).toBe('aspora');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new AsporaService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.ASPORA],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('aspora');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ASPORA);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('aspora-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new AsporaService(registry);
      const result = await service.scrape({
        siteType: [Site.ASPORA],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('aspora');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('aspora-x1');
      expect(result.jobs[0].site).toBe(Site.ASPORA);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('aspora-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new AsporaService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.ASPORA],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new AsporaService();
      const result = await service.scrape({
        siteType: [Site.ASPORA],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
