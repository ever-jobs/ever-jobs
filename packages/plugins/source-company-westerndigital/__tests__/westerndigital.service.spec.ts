...
import { WesternDigitalModule, WesternDigitalService } from '../src';

const COMPANY_NAME_EXPECT = 'Western Digital';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'westerndigital-jobs.json'), 'utf8'),
);
...
describe('WesternDigitalService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through WesternDigitalModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [WesternDigitalModule],
      }).compile();
      const service = moduleRef.get(WesternDigitalService);
      expect(service).toBeInstanceOf(WesternDigitalService);
      await moduleRef.close();
    });

    it('exports the Site.WESTERN_DIGITAL = "westerndigital" enum value', () => {
      expect(Site.WESTERN_DIGITAL).toBe('westerndigital');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new WesternDigitalService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.WESTERN_DIGITAL],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'westerndigital-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.WESTERN_DIGITAL);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('westerndigital-' + first.id);
      ...
      expect(calledUrls[0]).toContain('WesternDigital');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.WESTERN_DIGITAL);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('westerndigital-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new WesternDigitalService(registry);
      const result = await service.scrape({
        siteType: [Site.WESTERN_DIGITAL],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('WesternDigital');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('westerndigital-x1');
      expect(result.jobs[0].site).toBe(Site.WESTERN_DIGITAL);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('westerndigital-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new WesternDigitalService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.WESTERN_DIGITAL],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new WesternDigitalService();
      const result = await service.scrape({
        siteType: [Site.WESTERN_DIGITAL],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
