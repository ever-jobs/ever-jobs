...
import { PatchCaregivingModule, PatchCaregivingService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'patch-jobs.json'), 'utf8'),
);
...
/**
 * Spec 675 / T04 — `PatchCaregivingService` unit tests (≥ 9 cases).
 */
describe('PatchCaregivingService — Spec 675 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through PatchCaregivingModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [PatchCaregivingModule],
      }).compile();
      const service = moduleRef.get(PatchCaregivingService);
      expect(service).toBeInstanceOf(PatchCaregivingService);
      await moduleRef.close();
    });

    it('exports the Site.PATCH_CAREGIVING = "patch" enum value', () => {
      expect(Site.PATCH_CAREGIVING).toBe('patch');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new PatchCaregivingService();
      const result = await service.scrape({
        siteType: [Site.PATCH_CAREGIVING],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'patch-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.PATCH_CAREGIVING);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Patch Caregiving');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/patch/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/patch/jobs/');
      ...
      expect(job0?.description).toContain('Patch Caregiving');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/patch/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Patch Caregiving');
      }
    });
  });
