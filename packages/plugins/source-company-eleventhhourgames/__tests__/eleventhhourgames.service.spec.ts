...
import { EleventhHourGamesModule, EleventhHourGamesService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'eleventhhourgames-jobs.json'), 'utf8'),
);
...
/**
 * Spec 839 / T04 — `EleventhHourGamesService` unit tests (≥ 9 cases).
 */
describe('EleventhHourGamesService — Spec 839 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through EleventhHourGamesModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [EleventhHourGamesModule],
      }).compile();
      const service = moduleRef.get(EleventhHourGamesService);
      expect(service).toBeInstanceOf(EleventhHourGamesService);
      await moduleRef.close();
    });

    it('exports the Site.ELEVENTH_HOUR_GAMES = "eleventhhourgames" enum value', () => {
      expect(Site.ELEVENTH_HOUR_GAMES).toBe('eleventhhourgames');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new EleventhHourGamesService();
      const result = await service.scrape({
        siteType: [Site.ELEVENTH_HOUR_GAMES],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'eleventhhourgames-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ELEVENTH_HOUR_GAMES);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Eleventh Hour Games');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/eleventhhourgames/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/eleventhhourgames/jobs/');
      ...
      expect(job0?.description).toContain('Eleventh Hour Games');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/eleventhhourgames/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Eleventh Hour Games');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'eleventhhourgames-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
