...
import { TrialLibraryModule, TrialLibraryService } from '../src';

const COMPANY_NAME_EXPECT = 'Trial Library';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'triallibrary-jobs.json'), 'utf8'),
);
...
describe('TrialLibraryService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through TrialLibraryModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [TrialLibraryModule],
      }).compile();
      const service = moduleRef.get(TrialLibraryService);
      expect(service).toBeInstanceOf(TrialLibraryService);
      await moduleRef.close();
    });

    it('exports the Site.TRIAL_LIBRARY = "triallibrary" enum value', () => {
      expect(Site.TRIAL_LIBRARY).toBe('triallibrary');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new TrialLibraryService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.TRIAL_LIBRARY],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'triallibrary-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.TRIAL_LIBRARY);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('triallibrary-' + first.id);
      ...
      expect(calledUrls[0]).toContain('triallibrary');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.TRIAL_LIBRARY);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('triallibrary-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new TrialLibraryService(registry);
      const result = await service.scrape({
        siteType: [Site.TRIAL_LIBRARY],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('triallibrary');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('triallibrary-x1');
      expect(result.jobs[0].site).toBe(Site.TRIAL_LIBRARY);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('triallibrary-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new TrialLibraryService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.TRIAL_LIBRARY],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new TrialLibraryService();
      const result = await service.scrape({
        siteType: [Site.TRIAL_LIBRARY],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
