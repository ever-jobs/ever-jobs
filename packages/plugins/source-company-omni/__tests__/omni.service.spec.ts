...
import { OmniModule, OmniService } from '../src';

const COMPANY_NAME_EXPECT = 'Omni';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'omni-jobs.json'), 'utf8'),
);
...
describe('OmniService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through OmniModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [OmniModule],
      }).compile();
      const service = moduleRef.get(OmniService);
      expect(service).toBeInstanceOf(OmniService);
      await moduleRef.close();
    });

    it('exports the Site.OMNI = "omni" enum value', () => {
      expect(Site.OMNI).toBe('omni');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new OmniService(registryWithAshby());
      ...
      expect(calledUrls[0]).toContain('omni');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.OMNI);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('omni-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new OmniService(registry);
      const result = await service.scrape({
        siteType: [Site.OMNI],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('omni');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('omni-x1');
      expect(result.jobs[0].site).toBe(Site.OMNI);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('omni-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new OmniService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.OMNI],
      } as ScraperInputDto);
