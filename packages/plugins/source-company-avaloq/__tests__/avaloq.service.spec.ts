...
import { AvaloqModule, AvaloqService } from '../src';

const COMPANY_NAME_EXPECT = 'Avaloq';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'avaloq-jobs.json'), 'utf8'),
);
...
describe('AvaloqService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AvaloqModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AvaloqModule],
      }).compile();
      const service = moduleRef.get(AvaloqService);
      expect(service).toBeInstanceOf(AvaloqService);
      await moduleRef.close();
    });

    it('exports the Site.AVALOQ = "avaloq" enum value', () => {
      expect(Site.AVALOQ).toBe('avaloq');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new AvaloqService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.AVALOQ],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('Avaloq1');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.AVALOQ);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('avaloq-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new AvaloqService(registry);
      const result = await service.scrape({
        siteType: [Site.AVALOQ],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('Avaloq1');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('avaloq-x1');
      expect(result.jobs[0].site).toBe(Site.AVALOQ);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('avaloq-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new AvaloqService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.AVALOQ],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new AvaloqService();
      const result = await service.scrape({
        siteType: [Site.AVALOQ],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
