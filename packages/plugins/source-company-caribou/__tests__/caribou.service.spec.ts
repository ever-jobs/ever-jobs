...
import { CaribouModule, CaribouService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'caribou-jobs.json'), 'utf8'),
);
...
/**
 * Spec 506 / T04 — `CaribouService` unit tests (≥ 9 cases).
 */
describe('CaribouService — Spec 506 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CaribouModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CaribouModule],
      }).compile();
      const service = moduleRef.get(CaribouService);
      expect(service).toBeInstanceOf(CaribouService);
      await moduleRef.close();
    });

    it('exports the Site.CARIBOU = "caribou" enum value', () => {
      expect(Site.CARIBOU).toBe('caribou');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new CaribouService();
      const result = await service.scrape({
        siteType: [Site.CARIBOU],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'caribou-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.CARIBOU);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Caribou Financial');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/caribou/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/caribou/jobs/');
      ...
      expect(job0?.description).toContain('Caribou Financial');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/caribou/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Caribou Financial');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'caribou-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
