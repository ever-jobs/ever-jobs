...
import { SendbirdModule, SendbirdService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'sendbird-jobs.json'), 'utf8'),
);
...
/**
 * Spec 437 / T04 — `SendbirdService` unit tests (≥ 9 cases).
 */
describe('SendbirdService — Spec 437 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SendbirdModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SendbirdModule],
      }).compile();
      const service = moduleRef.get(SendbirdService);
      expect(service).toBeInstanceOf(SendbirdService);
      await moduleRef.close();
    });

    it('exports the Site.SENDBIRD = "sendbird" enum value', () => {
      expect(Site.SENDBIRD).toBe('sendbird');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new SendbirdService();
      const result = await service.scrape({
        siteType: [Site.SENDBIRD],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'sendbird-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.SENDBIRD);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Sendbird');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/sendbird/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/sendbird/jobs/');
      ...
      expect(job0?.description).toContain('Sendbird');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/sendbird/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Sendbird');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'sendbird-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
