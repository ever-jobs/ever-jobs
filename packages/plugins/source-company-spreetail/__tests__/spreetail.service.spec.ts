...
import { SpreetailModule, SpreetailService } from '../src';

const COMPANY_NAME_EXPECT = 'Spreetail';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'spreetail-jobs.json'), 'utf8'),
);
...
describe('SpreetailService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SpreetailModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SpreetailModule],
      }).compile();
      const service = moduleRef.get(SpreetailService);
      expect(service).toBeInstanceOf(SpreetailService);
      await moduleRef.close();
    });

    it('exports the Site.SPREETAIL = "spreetail" enum value', () => {
      expect(Site.SPREETAIL).toBe('spreetail');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new SpreetailService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.SPREETAIL],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('spreetail');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SPREETAIL);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('spreetail-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new SpreetailService(registry);
      const result = await service.scrape({
        siteType: [Site.SPREETAIL],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('spreetail');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('spreetail-x1');
      expect(result.jobs[0].site).toBe(Site.SPREETAIL);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('spreetail-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new SpreetailService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.SPREETAIL],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new SpreetailService();
      const result = await service.scrape({
        siteType: [Site.SPREETAIL],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
