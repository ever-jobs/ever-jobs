...
import { FloHealthModule, FloHealthService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'flohealth-jobs.json'), 'utf8'),
);
...
/**
 * Spec 652 / T04 — `FloHealthService` unit tests (≥ 9 cases).
 */
describe('FloHealthService — Spec 652 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through FloHealthModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [FloHealthModule],
      }).compile();
      const service = moduleRef.get(FloHealthService);
      expect(service).toBeInstanceOf(FloHealthService);
      await moduleRef.close();
    });

    it('exports the Site.FLO_HEALTH = "flohealth" enum value', () => {
      expect(Site.FLO_HEALTH).toBe('flohealth');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new FloHealthService();
      const result = await service.scrape({
        siteType: [Site.FLO_HEALTH],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'flohealth-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.FLO_HEALTH);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Flo Health');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/flohealth/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/flohealth/jobs/');
      ...
      expect(job0?.description).toContain('Flo Health');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/flohealth/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Flo Health');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'flohealth-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
