...
import { AestudioModule, AestudioService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'aestudio-jobs.json'), 'utf8'),
);
...
/**
 * Spec 199 / T04 — `AestudioService` unit tests (≥ 9 cases).
 */
describe('AestudioService — Spec 199 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AestudioModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AestudioModule],
      }).compile();
      const service = moduleRef.get(AestudioService);
      expect(service).toBeInstanceOf(AestudioService);
      await moduleRef.close();
    });

    it('exports the Site.AESTUDIO = "aestudio" enum value', () => {
      expect(Site.AESTUDIO).toBe('aestudio');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AestudioService();
      const result = await service.scrape({
        siteType: [Site.AESTUDIO],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'aestudio-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.AESTUDIO);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('AE Studio');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/aestudio/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/aestudio/jobs/');
      ...
      expect(job0?.description).toContain('AE Studio');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/aestudio/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('AE Studio');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'aestudio-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
