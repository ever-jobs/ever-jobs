...
import { LocusRoboticsModule, LocusRoboticsService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'locusrobotics-jobs.json'), 'utf8'),
);
...
/**
 * Spec 747 / T04 — `LocusRoboticsService` unit tests (≥ 9 cases).
 */
describe('LocusRoboticsService — Spec 747 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through LocusRoboticsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [LocusRoboticsModule],
      }).compile();
      const service = moduleRef.get(LocusRoboticsService);
      expect(service).toBeInstanceOf(LocusRoboticsService);
      await moduleRef.close();
    });

    it('exports the Site.LOCUS_ROBOTICS = "locusrobotics" enum value', () => {
      expect(Site.LOCUS_ROBOTICS).toBe('locusrobotics');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new LocusRoboticsService();
      const result = await service.scrape({
        siteType: [Site.LOCUS_ROBOTICS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'locusrobotics-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.LOCUS_ROBOTICS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Locus Robotics');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/locusrobotics/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/locusrobotics/jobs/');
      ...
      expect(job0?.description).toContain('Locus Robotics');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/locusrobotics/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Locus Robotics');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'locusrobotics-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
