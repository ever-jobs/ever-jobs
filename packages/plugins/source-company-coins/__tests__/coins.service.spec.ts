...
import { CoinsPhModule, CoinsPhService } from '../src';

const COMPANY_NAME_EXPECT = 'Coins.ph';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'coins-jobs.json'), 'utf8'),
);
...
describe('CoinsPhService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CoinsPhModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CoinsPhModule],
      }).compile();
      const service = moduleRef.get(CoinsPhService);
      expect(service).toBeInstanceOf(CoinsPhService);
      await moduleRef.close();
    });

    it('exports the Site.COINS_PH = "coins" enum value', () => {
      expect(Site.COINS_PH).toBe('coins');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new CoinsPhService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.COINS_PH],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('coins');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.COINS_PH);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('coins-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new CoinsPhService(registry);
      const result = await service.scrape({
        siteType: [Site.COINS_PH],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('coins');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('coins-x1');
      expect(result.jobs[0].site).toBe(Site.COINS_PH);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('coins-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new CoinsPhService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.COINS_PH],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new CoinsPhService();
      const result = await service.scrape({
        siteType: [Site.COINS_PH],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
