...
import { FederatoModule, FederatoService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'federato-jobs.json'), 'utf8'),
);
...
/**
 * Spec 846 / T04 — `FederatoService` unit tests (≥ 9 cases).
 */
describe('FederatoService — Spec 846 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through FederatoModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [FederatoModule],
      }).compile();
      const service = moduleRef.get(FederatoService);
      expect(service).toBeInstanceOf(FederatoService);
      await moduleRef.close();
    });

    it('exports the Site.FEDERATO = "federato" enum value', () => {
      expect(Site.FEDERATO).toBe('federato');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new FederatoService();
      const result = await service.scrape({
        siteType: [Site.FEDERATO],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'federato-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.FEDERATO);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Federato');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/federato/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/federato/jobs/');
      ...
      expect(job0?.description).toContain('Federato');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/federato/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Federato');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'federato-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
