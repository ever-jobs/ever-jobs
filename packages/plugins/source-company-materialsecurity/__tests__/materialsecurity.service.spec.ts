...
import { MaterialSecurityModule, MaterialSecurityService } from '../src';

const COMPANY_NAME_EXPECT = 'Material Security';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'materialsecurity-jobs.json'), 'utf8'),
);
...
describe('MaterialSecurityService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through MaterialSecurityModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [MaterialSecurityModule],
      }).compile();
      const service = moduleRef.get(MaterialSecurityService);
      expect(service).toBeInstanceOf(MaterialSecurityService);
      await moduleRef.close();
    });

    it('exports the Site.MATERIAL_SECURITY = "materialsecurity" enum value', () => {
      expect(Site.MATERIAL_SECURITY).toBe('materialsecurity');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new MaterialSecurityService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.MATERIAL_SECURITY],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'materialsecurity-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.MATERIAL_SECURITY);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('materialsecurity-' + first.id);
      ...
      expect(calledUrls[0]).toContain('materialsecurity');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.MATERIAL_SECURITY);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('materialsecurity-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new MaterialSecurityService(registry);
      const result = await service.scrape({
        siteType: [Site.MATERIAL_SECURITY],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('materialsecurity');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('materialsecurity-x1');
      expect(result.jobs[0].site).toBe(Site.MATERIAL_SECURITY);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('materialsecurity-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new MaterialSecurityService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.MATERIAL_SECURITY],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new MaterialSecurityService();
      const result = await service.scrape({
        siteType: [Site.MATERIAL_SECURITY],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
