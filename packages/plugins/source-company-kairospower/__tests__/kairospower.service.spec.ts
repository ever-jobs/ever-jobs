...
import { KairospowerModule, KairospowerService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'kairospower-jobs.json'), 'utf8'),
);
...
/**
 * Spec 518 / T04 — `KairospowerService` unit tests (≥ 9 cases).
 */
describe('KairospowerService — Spec 518 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through KairospowerModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [KairospowerModule],
      }).compile();
      const service = moduleRef.get(KairospowerService);
      expect(service).toBeInstanceOf(KairospowerService);
      await moduleRef.close();
    });

    it('exports the Site.KAIROS_POWER = "kairospower" enum value', () => {
      expect(Site.KAIROS_POWER).toBe('kairospower');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new KairospowerService();
      const result = await service.scrape({
        siteType: [Site.KAIROS_POWER],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'kairospower-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.KAIROS_POWER);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Kairos Power');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/kairospower/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/kairospower/jobs/');
      ...
      expect(job0?.description).toContain('Kairos Power');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/kairospower/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Kairos Power');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'kairospower-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
