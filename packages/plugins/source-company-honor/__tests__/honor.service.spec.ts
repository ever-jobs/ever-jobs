...
import { HonorModule, HonorService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'honor-jobs.json'), 'utf8'),
);
...
/**
 * Spec 497 / T04 — `HonorService` unit tests (≥ 9 cases).
 */
describe('HonorService — Spec 497 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through HonorModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [HonorModule],
      }).compile();
      const service = moduleRef.get(HonorService);
      expect(service).toBeInstanceOf(HonorService);
      await moduleRef.close();
    });

    it('exports the Site.HONOR = "honor" enum value', () => {
      expect(Site.HONOR).toBe('honor');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new HonorService();
      ...
      const job0 = dto.jobs.find((j) => j.id === 'honor-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.HONOR);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Honor');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/honor/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/honor/jobs/');
      ...
      expect(job0?.description).toContain('Honor');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/honor/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Honor');
      }
    });
  });
