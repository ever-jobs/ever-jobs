...
import { SaatvaModule, SaatvaService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'saatva-jobs.json'), 'utf8'),
);
...
/**
 * Spec 692 / T04 — `SaatvaService` unit tests (≥ 9 cases).
 */
describe('SaatvaService — Spec 692 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SaatvaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SaatvaModule],
      }).compile();
      const service = moduleRef.get(SaatvaService);
      expect(service).toBeInstanceOf(SaatvaService);
      await moduleRef.close();
    });

    it('exports the Site.SAATVA = "saatva" enum value', () => {
      expect(Site.SAATVA).toBe('saatva');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new SaatvaService();
      const result = await service.scrape({
        siteType: [Site.SAATVA],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'saatva-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.SAATVA);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Saatva');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/saatva/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/saatva/jobs/');
      ...
      expect(job0?.description).toContain('Saatva');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/saatva/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Saatva');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'saatva-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
