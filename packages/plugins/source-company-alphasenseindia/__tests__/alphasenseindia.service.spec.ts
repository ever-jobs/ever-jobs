...
import { AlphasenseindiaModule, AlphasenseindiaService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'alphasenseindia-jobs.json'), 'utf8'),
);
...
/**
 * Spec 244 / T04 — `AlphasenseindiaService` unit tests (≥ 9 cases).
 */
describe('AlphasenseindiaService — Spec 244 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AlphasenseindiaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AlphasenseindiaModule],
      }).compile();
      const service = moduleRef.get(AlphasenseindiaService);
      expect(service).toBeInstanceOf(AlphasenseindiaService);
      await moduleRef.close();
    });

    it('exports the Site.ALPHASENSEINDIA = "alphasenseindia" enum value', () => {
      expect(Site.ALPHASENSEINDIA).toBe('alphasenseindia');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AlphasenseindiaService();
      const result = await service.scrape({
        siteType: [Site.ALPHASENSEINDIA],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'alphasenseindia-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ALPHASENSEINDIA);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('AlphaSense India');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/alphasenseindia/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/alphasenseindia/jobs/');
      ...
      expect(job0?.description).toContain('AlphaSense India');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/alphasenseindia/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('AlphaSense India');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'alphasenseindia-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
