...
import { ArtechModule, ArtechService } from '../src';

const COMPANY_NAME_EXPECT = 'Artech';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'artech-jobs.json'), 'utf8'),
);
...
describe('ArtechService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ArtechModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ArtechModule],
      }).compile();
      const service = moduleRef.get(ArtechService);
      expect(service).toBeInstanceOf(ArtechService);
      await moduleRef.close();
    });

    it('exports the Site.ARTECH = "artech" enum value', () => {
      expect(Site.ARTECH).toBe('artech');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new ArtechService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.ARTECH],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('ArtechInformationSystemLLC');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ARTECH);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('artech-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new ArtechService(registry);
      const result = await service.scrape({
        siteType: [Site.ARTECH],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('ArtechInformationSystemLLC');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('artech-x1');
      expect(result.jobs[0].site).toBe(Site.ARTECH);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('artech-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new ArtechService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.ARTECH],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new ArtechService();
      const result = await service.scrape({
        siteType: [Site.ARTECH],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
