...
/**
 * RTX — Aerospace and defense (HQ: Arlington, VA, USA).
 *
 * Source (Spec 1736): Workday board, scraped in this order:
 *   - `globalhr:5:REC_RTX_Ext_Gateway` — https://globalhr.wd5.myworkdayjobs.com/REC_RTX_Ext_Gateway
 *     verified live 2026-09-24: 4,859 open postings.
 *
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'globalhr:5:REC_RTX_Ext_Gateway', atsIdPrefix: 'wd-globalhr-' },
];
...
@SourcePlugin({
  ...
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['rtx.com'],
  description: 'RTX careers via Workday. Tags: segment=workday-enterprise; industry=aerospace-and-defense.',
})
@Injectable()
export class RtxService implements IScraper {
  private readonly logger = new Logger(RtxService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Workday source plugin is not registered; cannot scrape RTX');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`RTX: delegating to Workday (${board.companySlug})`);
    ...
    this.logger.log(`RTX: scraped ${jobs.length} jobs`);
