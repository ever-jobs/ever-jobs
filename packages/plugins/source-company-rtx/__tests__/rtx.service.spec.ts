...
import { RtxModule, RtxService } from '../src';
...
const FIXTURE: Fixture = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'fixtures', 'rtx-boards.json'), 'utf8'),
);
const COMPANY_NAME = 'RTX';
const ID_PREFIX = 'rtx-';
...
describe('RtxService — Workday delegation (Spec 1736)', () => {
  beforeEach(() => {
    mockGet.mockReset();
    mockPost.mockReset();
  ...
  describe('registration', () => {
    it('resolves through RtxModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({ imports: [RtxModule] }).compile();
      expect(moduleRef.get(RtxService)).toBeInstanceOf(RtxService);
      await moduleRef.close();
    });

    it('exports Site.RTX = "rtx"', () => {
      expect(Site.RTX).toBe('rtx');
    });

    it('declares a tagged company plugin with its domains', () => {
      const meta = Reflect.getMetadata(SOURCE_PLUGIN_METADATA, RtxService);
      expect(meta.site).toBe(Site.RTX);
      expect(meta.category).toBe('company');
      expect(meta.isAts).toBeFalsy();
      expect(meta.companyDomains).toEqual(['rtx.com']);
  ...
  describe('recorded board (real Workday adapter, mocked HTTP)', () => {
    it('maps every recorded posting and re-stamps the company identity', async () => {
      const service = new RtxService(registryWith());
      const result = await service.scrape({ siteType: [Site.RTX], resultsWanted: 100 } as ScraperInputDto);
    ...
    it('only requests the recorded board URLs', async () => {
      const service = new RtxService(registryWith());
      await service.scrape({ siteType: [Site.RTX] } as ScraperInputDto);
    ...
    it('honours resultsWanted=1', async () => {
      const service = new RtxService(registryWith());
      const result = await service.scrape({ siteType: [Site.RTX], resultsWanted: 1 } as ScraperInputDto);
    ...
    it('resolves with an empty result when the board is gone (HTTP 404)', async () => {
      ...
      const service = new RtxService(registryWith());
      const result = await service.scrape({ siteType: [Site.RTX] } as ScraperInputDto);
  ...
  describe('delegation contract', () => {
    it('forwards the board and the caller input untouched', async () => {
      ...
      expect(result.jobs[0].site).toBe(Site.RTX);
      expect(result.jobs[0].companyName).toBe(COMPANY_NAME);
    });

    it('passes an absent resultsWanted through as absent', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new RtxService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      await service.scrape({ siteType: [Site.RTX] } as ScraperInputDto);
    ...
    it('makes no request when resultsWanted is 0', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new RtxService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      const result = await service.scrape({ siteType: [Site.RTX], resultsWanted: 0 } as ScraperInputDto);
  ...
  describe('resilience', () => {
    it('reports not_registered when no registry is injected', async () => {
      const result = await new RtxService().scrape({ siteType: [Site.RTX] } as ScraperInputDto);
      expect(result.jobs).toEqual([]);
    ...
    it('reports not_registered when Workday is missing from the registry', async () => {
      const result = await new RtxService(new PluginRegistry()).scrape({
        siteType: [Site.RTX],
      } as ScraperInputDto);
      expect(result.diagnostics?.reason).toBe('not_registered');
    });
