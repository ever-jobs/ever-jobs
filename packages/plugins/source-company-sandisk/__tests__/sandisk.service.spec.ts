...
import { SanDiskModule, SanDiskService } from '../src';

const COMPANY_NAME_EXPECT = 'SanDisk';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'sandisk-jobs.json'), 'utf8'),
);
...
describe('SanDiskService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SanDiskModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SanDiskModule],
      }).compile();
      const service = moduleRef.get(SanDiskService);
      expect(service).toBeInstanceOf(SanDiskService);
      await moduleRef.close();
    });

    it('exports the Site.SANDISK = "sandisk" enum value', () => {
      expect(Site.SANDISK).toBe('sandisk');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new SanDiskService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.SANDISK],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('Sandisk');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SANDISK);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('sandisk-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new SanDiskService(registry);
      const result = await service.scrape({
        siteType: [Site.SANDISK],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('Sandisk');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('sandisk-x1');
      expect(result.jobs[0].site).toBe(Site.SANDISK);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('sandisk-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new SanDiskService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.SANDISK],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new SanDiskService();
      const result = await service.scrape({
        siteType: [Site.SANDISK],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
