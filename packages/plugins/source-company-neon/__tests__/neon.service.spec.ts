...
import { NeonModule, NeonService } from '../src';

const COMPANY_NAME_EXPECT = 'Neon';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'neon-jobs.json'), 'utf8'),
);
...
describe('NeonService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through NeonModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [NeonModule],
      }).compile();
      const service = moduleRef.get(NeonService);
      expect(service).toBeInstanceOf(NeonService);
      await moduleRef.close();
    });

    it('exports the Site.NEON = "neon" enum value', () => {
      expect(Site.NEON).toBe('neon');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new NeonService(registryWithAshby());
      ...
      expect(calledUrls[0]).toContain('neon');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.NEON);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('neon-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new NeonService(registry);
      const result = await service.scrape({
        siteType: [Site.NEON],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('neon');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('neon-x1');
      expect(result.jobs[0].site).toBe(Site.NEON);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('neon-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new NeonService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.NEON],
      } as ScraperInputDto);
