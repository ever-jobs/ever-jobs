...
import { Unit4Module, Unit4Service } from '../src';

const COMPANY_NAME_EXPECT = 'Unit4';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'unit4-jobs.json'), 'utf8'),
);
...
describe('Unit4Service — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through Unit4Module via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [Unit4Module],
      }).compile();
      const service = moduleRef.get(Unit4Service);
      expect(service).toBeInstanceOf(Unit4Service);
      await moduleRef.close();
    });

    it('exports the Site.UNIT4 = "unit4" enum value', () => {
      expect(Site.UNIT4).toBe('unit4');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new Unit4Service(registryWithSmartRecruiters());
      ...
      expect(calledUrls[0]).toContain('unit44');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.UNIT4);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('unit4-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new Unit4Service(registry);
      const result = await service.scrape({
        siteType: [Site.UNIT4],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('unit44');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('unit4-x1');
      expect(result.jobs[0].site).toBe(Site.UNIT4);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('unit4-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new Unit4Service(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.UNIT4],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new Unit4Service();
      const result = await service.scrape({
        siteType: [Site.UNIT4],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
