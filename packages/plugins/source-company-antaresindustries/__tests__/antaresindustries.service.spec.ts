...
import { AntaresIndustriesModule, AntaresIndustriesService } from '../src';

const COMPANY_NAME_EXPECT = 'Antares Industries';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'antaresindustries-jobs.json'), 'utf8'),
);
...
describe('AntaresIndustriesService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AntaresIndustriesModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AntaresIndustriesModule],
      }).compile();
      const service = moduleRef.get(AntaresIndustriesService);
      expect(service).toBeInstanceOf(AntaresIndustriesService);
      await moduleRef.close();
    });

    it('exports the Site.ANTARES_INDUSTRIES = "antaresindustries" enum value', () => {
      expect(Site.ANTARES_INDUSTRIES).toBe('antaresindustries');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new AntaresIndustriesService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.ANTARES_INDUSTRIES],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'antaresindustries-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.ANTARES_INDUSTRIES);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('antaresindustries-' + first.id);
      ...
      expect(calledUrls[0]).toContain('8fleet-inc');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ANTARES_INDUSTRIES);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('antaresindustries-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new AntaresIndustriesService(registry);
      const result = await service.scrape({
        siteType: [Site.ANTARES_INDUSTRIES],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('8fleet-inc');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('antaresindustries-x1');
      expect(result.jobs[0].site).toBe(Site.ANTARES_INDUSTRIES);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('antaresindustries-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new AntaresIndustriesService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.ANTARES_INDUSTRIES],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new AntaresIndustriesService();
      const result = await service.scrape({
        siteType: [Site.ANTARES_INDUSTRIES],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
