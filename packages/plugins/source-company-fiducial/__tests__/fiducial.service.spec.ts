...
import { FiducialModule, FiducialService } from '../src';

const COMPANY_NAME_EXPECT = 'Fiducial';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'fiducial-jobs.json'), 'utf8'),
);
...
describe('FiducialService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through FiducialModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [FiducialModule],
      }).compile();
      const service = moduleRef.get(FiducialService);
      expect(service).toBeInstanceOf(FiducialService);
      await moduleRef.close();
    });

    it('exports the Site.FIDUCIAL = "fiducial" enum value', () => {
      expect(Site.FIDUCIAL).toBe('fiducial');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new FiducialService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.FIDUCIAL],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('fiducial');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.FIDUCIAL);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('fiducial-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new FiducialService(registry);
      const result = await service.scrape({
        siteType: [Site.FIDUCIAL],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('fiducial');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('fiducial-x1');
      expect(result.jobs[0].site).toBe(Site.FIDUCIAL);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('fiducial-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new FiducialService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.FIDUCIAL],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new FiducialService();
      const result = await service.scrape({
        siteType: [Site.FIDUCIAL],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
