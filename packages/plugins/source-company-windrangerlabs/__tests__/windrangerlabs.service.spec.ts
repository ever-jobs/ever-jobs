...
import { WindRangerLabsModule, WindRangerLabsService } from '../src';

const COMPANY_NAME_EXPECT = 'WindRanger Labs';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'windrangerlabs-jobs.json'), 'utf8'),
);
...
describe('WindRangerLabsService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through WindRangerLabsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [WindRangerLabsModule],
      }).compile();
      const service = moduleRef.get(WindRangerLabsService);
      expect(service).toBeInstanceOf(WindRangerLabsService);
      await moduleRef.close();
    });

    it('exports the Site.WINDRANGER_LABS = "windrangerlabs" enum value', () => {
      expect(Site.WINDRANGER_LABS).toBe('windrangerlabs');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new WindRangerLabsService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.WINDRANGER_LABS],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'windrangerlabs-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.WINDRANGER_LABS);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('windrangerlabs-' + first.id);
      ...
      expect(calledUrls[0]).toContain('windranger');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.WINDRANGER_LABS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('windrangerlabs-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new WindRangerLabsService(registry);
      const result = await service.scrape({
        siteType: [Site.WINDRANGER_LABS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('windranger');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('windrangerlabs-x1');
      expect(result.jobs[0].site).toBe(Site.WINDRANGER_LABS);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('windrangerlabs-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new WindRangerLabsService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.WINDRANGER_LABS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new WindRangerLabsService();
      const result = await service.scrape({
        siteType: [Site.WINDRANGER_LABS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
