...
import { ClickhouseModule, ClickhouseService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'clickhouse-jobs.json'), 'utf8'),
);
...
/**
 * Spec 438 / T04 — `ClickhouseService` unit tests (≥ 9 cases).
 */
describe('ClickhouseService — Spec 438 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ClickhouseModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ClickhouseModule],
      }).compile();
      const service = moduleRef.get(ClickhouseService);
      expect(service).toBeInstanceOf(ClickhouseService);
      await moduleRef.close();
    });

    it('exports the Site.CLICKHOUSE = "clickhouse" enum value', () => {
      expect(Site.CLICKHOUSE).toBe('clickhouse');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const job0 = dto.jobs.find((j) => j.id === 'clickhouse-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.CLICKHOUSE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('ClickHouse');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/clickhouse/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/clickhouse/jobs/');
      ...
      expect(job0?.description).toContain('ClickHouse');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/clickhouse/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('ClickHouse');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'clickhouse-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
