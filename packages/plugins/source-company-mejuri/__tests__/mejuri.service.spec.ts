...
import { MejuriModule, MejuriService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'mejuri-jobs.json'), 'utf8'),
);
...
/**
 * Spec 580 / T04 — `MejuriService` unit tests (≥ 9 cases).
 */
describe('MejuriService — Spec 580 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through MejuriModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [MejuriModule],
      }).compile();
      const service = moduleRef.get(MejuriService);
      expect(service).toBeInstanceOf(MejuriService);
      await moduleRef.close();
    });

    it('exports the Site.MEJURI = "mejuri" enum value', () => {
      expect(Site.MEJURI).toBe('mejuri');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new MejuriService();
      const result = await service.scrape({
        siteType: [Site.MEJURI],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'mejuri-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.MEJURI);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Mejuri');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/mejuri/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/mejuri/jobs/');
      ...
      expect(job0?.description).toContain('Mejuri');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/mejuri/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Mejuri');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'mejuri-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
