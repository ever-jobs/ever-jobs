...
import { TransatATModule, TransatATService } from '../src';

const COMPANY_NAME_EXPECT = 'Transat AT';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'transatat-jobs.json'), 'utf8'),
);
...
describe('TransatATService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through TransatATModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [TransatATModule],
      }).compile();
      const service = moduleRef.get(TransatATService);
      expect(service).toBeInstanceOf(TransatATService);
      await moduleRef.close();
    });

    it('exports the Site.TRANSAT_AT = "transatat" enum value', () => {
      expect(Site.TRANSAT_AT).toBe('transatat');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new TransatATService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.TRANSAT_AT],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('TransatAT1');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.TRANSAT_AT);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('transatat-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new TransatATService(registry);
      const result = await service.scrape({
        siteType: [Site.TRANSAT_AT],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('TransatAT1');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('transatat-x1');
      expect(result.jobs[0].site).toBe(Site.TRANSAT_AT);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('transatat-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new TransatATService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.TRANSAT_AT],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new TransatATService();
      const result = await service.scrape({
        siteType: [Site.TRANSAT_AT],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
