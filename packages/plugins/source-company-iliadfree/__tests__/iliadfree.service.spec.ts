...
import { IliadFreeModule, IliadFreeService } from '../src';

const COMPANY_NAME_EXPECT = 'Iliad / Free';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'iliadfree-jobs.json'), 'utf8'),
);
...
describe('IliadFreeService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through IliadFreeModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [IliadFreeModule],
      }).compile();
      const service = moduleRef.get(IliadFreeService);
      expect(service).toBeInstanceOf(IliadFreeService);
      await moduleRef.close();
    });

    it('exports the Site.ILIAD_FREE = "iliadfree" enum value', () => {
      expect(Site.ILIAD_FREE).toBe('iliadfree');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new IliadFreeService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.ILIAD_FREE],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('Iliad-Free');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ILIAD_FREE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('iliadfree-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new IliadFreeService(registry);
      const result = await service.scrape({
        siteType: [Site.ILIAD_FREE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('Iliad-Free');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('iliadfree-x1');
      expect(result.jobs[0].site).toBe(Site.ILIAD_FREE);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('iliadfree-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new IliadFreeService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.ILIAD_FREE],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new IliadFreeService();
      const result = await service.scrape({
        siteType: [Site.ILIAD_FREE],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
