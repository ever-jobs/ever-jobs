...
import { AlignModule, AlignService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'align-jobs.json'), 'utf8'),
);
...
/**
 * Spec 230 / T04 — `AlignService` unit tests (≥ 9 cases).
 */
describe('AlignService — Spec 230 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AlignModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AlignModule],
      }).compile();
      const service = moduleRef.get(AlignService);
      expect(service).toBeInstanceOf(AlignService);
      await moduleRef.close();
    });

    it('exports the Site.ALIGN = "align" enum value', () => {
      expect(Site.ALIGN).toBe('align');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AlignService();
      ...
      const job0 = dto.jobs.find((j) => j.id === 'align-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ALIGN);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('A-LIGN External');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/align/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/align/jobs/');
      ...
      expect(job0?.description).toContain('A-LIGN External');
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('A-LIGN External');
      }
    });
  });
