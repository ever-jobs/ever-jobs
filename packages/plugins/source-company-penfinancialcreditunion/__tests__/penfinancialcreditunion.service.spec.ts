...
import { PenFinancialCreditUnionModule, PenFinancialCreditUnionService } from '../src';

const COMPANY_NAME_EXPECT = 'PenFinancial Credit Union';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'penfinancialcreditunion-jobs.json'), 'utf8'),
);
...
describe('PenFinancialCreditUnionService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through PenFinancialCreditUnionModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [PenFinancialCreditUnionModule],
      }).compile();
      const service = moduleRef.get(PenFinancialCreditUnionService);
      expect(service).toBeInstanceOf(PenFinancialCreditUnionService);
      await moduleRef.close();
    });

    it('exports the Site.PENFINANCIAL_CREDIT_UNION = "penfinancialcreditunion" enum value', () => {
      expect(Site.PENFINANCIAL_CREDIT_UNION).toBe('penfinancialcreditunion');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new PenFinancialCreditUnionService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.PENFINANCIAL_CREDIT_UNION],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'penfinancialcreditunion-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.PENFINANCIAL_CREDIT_UNION);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('penfinancialcreditunion-' + first.id);
      ...
      expect(calledUrls[0]).toContain('PenFinancialCreditUnion');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.PENFINANCIAL_CREDIT_UNION);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('penfinancialcreditunion-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new PenFinancialCreditUnionService(registry);
      const result = await service.scrape({
        siteType: [Site.PENFINANCIAL_CREDIT_UNION],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('PenFinancialCreditUnion');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('penfinancialcreditunion-x1');
      expect(result.jobs[0].site).toBe(Site.PENFINANCIAL_CREDIT_UNION);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('penfinancialcreditunion-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new PenFinancialCreditUnionService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.PENFINANCIAL_CREDIT_UNION],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new PenFinancialCreditUnionService();
      const result = await service.scrape({
        siteType: [Site.PENFINANCIAL_CREDIT_UNION],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
