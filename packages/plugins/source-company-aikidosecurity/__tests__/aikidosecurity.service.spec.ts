...
import { AikidoSecurityModule, AikidoSecurityService } from '../src';

const COMPANY_NAME_EXPECT = 'Aikido Security';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'aikidosecurity-jobs.json'), 'utf8'),
);
...
describe('AikidoSecurityService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AikidoSecurityModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AikidoSecurityModule],
      }).compile();
      const service = moduleRef.get(AikidoSecurityService);
      expect(service).toBeInstanceOf(AikidoSecurityService);
      await moduleRef.close();
    });

    it('exports the Site.AIKIDO_SECURITY = "aikidosecurity" enum value', () => {
      expect(Site.AIKIDO_SECURITY).toBe('aikidosecurity');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new AikidoSecurityService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.AIKIDO_SECURITY],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'aikidosecurity-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Recruitee's defaults
      expect(job0?.site).toBe(Site.AIKIDO_SECURITY);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('aikidosecurity-' + first.id);
      ...
      expect(calledUrls[0]).toContain('aikidosecurity.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.AIKIDO_SECURITY);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('aikidosecurity-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new AikidoSecurityService(registry);
      const result = await service.scrape({
        siteType: [Site.AIKIDO_SECURITY],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('aikidosecurity');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('aikidosecurity-x1');
      expect(result.jobs[0].site).toBe(Site.AIKIDO_SECURITY);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('aikidosecurity-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new AikidoSecurityService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.AIKIDO_SECURITY],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new AikidoSecurityService();
      const result = await service.scrape({
        siteType: [Site.AIKIDO_SECURITY],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
