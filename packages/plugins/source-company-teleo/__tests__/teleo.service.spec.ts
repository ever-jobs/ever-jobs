...
import { TeleoModule, TeleoService } from '../src';

const COMPANY_NAME_EXPECT = 'Teleo';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'teleo-jobs.json'), 'utf8'),
);
...
describe('TeleoService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through TeleoModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [TeleoModule],
      }).compile();
      const service = moduleRef.get(TeleoService);
      expect(service).toBeInstanceOf(TeleoService);
      await moduleRef.close();
    });

    it('exports the Site.TELEO = "teleo" enum value', () => {
      expect(Site.TELEO).toBe('teleo');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new TeleoService(registryWithLever());
      ...
      expect(calledUrls[0]).toContain('teleo');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.TELEO);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('teleo-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new TeleoService(registry);
      const result = await service.scrape({
        siteType: [Site.TELEO],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('teleo');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('teleo-x1');
      expect(result.jobs[0].site).toBe(Site.TELEO);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('teleo-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new TeleoService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.TELEO],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new TeleoService();
      const result = await service.scrape({
        siteType: [Site.TELEO],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
