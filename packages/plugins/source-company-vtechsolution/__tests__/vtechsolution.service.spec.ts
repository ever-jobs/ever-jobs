...
import { VTechSolutionModule, VTechSolutionService } from '../src';

const COMPANY_NAME_EXPECT = 'vTech Solution';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'vtechsolution-jobs.json'), 'utf8'),
);
...
describe('VTechSolutionService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through VTechSolutionModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [VTechSolutionModule],
      }).compile();
      const service = moduleRef.get(VTechSolutionService);
      expect(service).toBeInstanceOf(VTechSolutionService);
      await moduleRef.close();
    });

    it('exports the Site.VTECH_SOLUTION = "vtechsolution" enum value', () => {
      expect(Site.VTECH_SOLUTION).toBe('vtechsolution');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new VTechSolutionService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.VTECH_SOLUTION],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'vtechsolution-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.VTECH_SOLUTION);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('vtechsolution-' + first.id);
      ...
      expect(calledUrls[0]).toContain('VTechSolution1');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.VTECH_SOLUTION);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('vtechsolution-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new VTechSolutionService(registry);
      const result = await service.scrape({
        siteType: [Site.VTECH_SOLUTION],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('VTechSolution1');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('vtechsolution-x1');
      expect(result.jobs[0].site).toBe(Site.VTECH_SOLUTION);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('vtechsolution-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new VTechSolutionService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.VTECH_SOLUTION],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new VTechSolutionService();
      const result = await service.scrape({
        siteType: [Site.VTECH_SOLUTION],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
