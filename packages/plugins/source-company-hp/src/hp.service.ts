...
/**
 * HP Inc. — Computer hardware (HQ: Palo Alto, CA, USA).
 *
 * Source (Spec 1736): Workday board, scraped in this order:
 *   - `hp:5:ExternalCareerSite` — https://hp.wd5.myworkdayjobs.com/ExternalCareerSite
 *     verified live 2026-09-24: 926 open postings.
 *
 ...
 * party board.
 *
 * Tags: segment=workday-enterprise; industry=computer-hardware.
 */
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'hp:5:ExternalCareerSite', atsIdPrefix: 'wd-hp-' },
];
...
@SourcePlugin({
  ...
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['hp.com'],
  description: 'HP Inc. careers via Workday. Tags: segment=workday-enterprise; industry=computer-hardware.',
})
@Injectable()
export class HpService implements IScraper {
  private readonly logger = new Logger(HpService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Workday source plugin is not registered; cannot scrape HP Inc.');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`HP Inc.: delegating to Workday (${board.companySlug})`);
    ...
    this.logger.log(`HP Inc.: scraped ${jobs.length} jobs`);
