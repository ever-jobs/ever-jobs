...
import { HpModule, HpService } from '../src';
...
const FIXTURE: Fixture = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'fixtures', 'hp-boards.json'), 'utf8'),
);
const COMPANY_NAME = 'HP Inc.';
const ID_PREFIX = 'hp-';
...
describe('HpService — Workday delegation (Spec 1736)', () => {
  beforeEach(() => {
    mockGet.mockReset();
    mockPost.mockReset();
  ...
  describe('registration', () => {
    it('resolves through HpModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({ imports: [HpModule] }).compile();
      expect(moduleRef.get(HpService)).toBeInstanceOf(HpService);
      await moduleRef.close();
    });

    it('exports Site.HP = "hp"', () => {
      expect(Site.HP).toBe('hp');
    });

    it('declares a tagged company plugin with its domains', () => {
      const meta = Reflect.getMetadata(SOURCE_PLUGIN_METADATA, HpService);
      expect(meta.site).toBe(Site.HP);
      expect(meta.category).toBe('company');
      expect(meta.isAts).toBeFalsy();
      expect(meta.companyDomains).toEqual(['hp.com']);
  ...
  describe('recorded board (real Workday adapter, mocked HTTP)', () => {
    it('maps every recorded posting and re-stamps the company identity', async () => {
      const service = new HpService(registryWith());
      const result = await service.scrape({ siteType: [Site.HP], resultsWanted: 100 } as ScraperInputDto);
    ...
    it('only requests the recorded board URLs', async () => {
      const service = new HpService(registryWith());
      await service.scrape({ siteType: [Site.HP] } as ScraperInputDto);
    ...
    it('honours resultsWanted=1', async () => {
      const service = new HpService(registryWith());
      const result = await service.scrape({ siteType: [Site.HP], resultsWanted: 1 } as ScraperInputDto);
    ...
    it('resolves with an empty result when the board is gone (HTTP 404)', async () => {
      ...
      const service = new HpService(registryWith());
      const result = await service.scrape({ siteType: [Site.HP] } as ScraperInputDto);
  ...
  describe('delegation contract', () => {
    it('forwards the board and the caller input untouched', async () => {
      ...
      expect(result.jobs[0].site).toBe(Site.HP);
    ...
    it('never forwards the caller\'s credentials to the board (Spec 1735 §4.5)', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new HpService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
    ...
    it('makes no request when resultsWanted is 0', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new HpService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      const result = await service.scrape({ siteType: [Site.HP], resultsWanted: 0 } as ScraperInputDto);
  ...
  describe('resilience', () => {
    it('reports not_registered when no registry is injected', async () => {
      const result = await new HpService().scrape({ siteType: [Site.HP] } as ScraperInputDto);
      expect(result.jobs).toEqual([]);
    ...
    it('reports not_registered when Workday is missing from the registry', async () => {
      const result = await new HpService(new PluginRegistry()).scrape({
        siteType: [Site.HP],
      } as ScraperInputDto);
      expect(result.diagnostics?.reason).toBe('not_registered');
    });
