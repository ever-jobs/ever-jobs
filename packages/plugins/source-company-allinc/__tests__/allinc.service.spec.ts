...
import { AllincModule, AllincService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'allinc-jobs.json'), 'utf8'),
);
...
/**
 * Spec 236 / T04 — `AllincService` unit tests (≥ 9 cases).
 */
describe('AllincService — Spec 236 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AllincModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AllincModule],
      }).compile();
      const service = moduleRef.get(AllincService);
      expect(service).toBeInstanceOf(AllincService);
      await moduleRef.close();
    });

    it('exports the Site.ALLINC = "allinc" enum value', () => {
      expect(Site.ALLINC).toBe('allinc');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AllincService();
      const result = await service.scrape({
        siteType: [Site.ALLINC],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'allinc-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ALLINC);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Aspire Living & Learning ');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/allinc/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/allinc/jobs/');
      ...
      expect(job0?.description).toContain('Aspire Living & Learning ');
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Aspire Living & Learning ');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'allinc-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
