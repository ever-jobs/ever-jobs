...
import { AccelaModule, AccelaService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'accela-jobs.json'), 'utf8'),
);
...
/**
 * Spec 804 / T04 — `AccelaService` unit tests (≥ 9 cases).
 */
describe('AccelaService — Spec 804 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AccelaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AccelaModule],
      }).compile();
      const service = moduleRef.get(AccelaService);
      expect(service).toBeInstanceOf(AccelaService);
      await moduleRef.close();
    });

    it('exports the Site.ACCELA = "accela" enum value', () => {
      expect(Site.ACCELA).toBe('accela');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AccelaService();
      const result = await service.scrape({
        siteType: [Site.ACCELA],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'accela-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ACCELA);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Accela');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/accela/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/accela/jobs/');
      ...
      expect(job0?.description).toContain('Accela');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/accela/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Accela');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'accela-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
