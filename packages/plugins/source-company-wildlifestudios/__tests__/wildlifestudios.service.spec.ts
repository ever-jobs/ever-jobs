...
import { WildlifeStudiosModule, WildlifeStudiosService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'wildlifestudios-jobs.json'), 'utf8'),
);
...
/**
 * Spec 970 / T04 — `WildlifeStudiosService` unit tests (≥ 9 cases).
 */
describe('WildlifeStudiosService — Spec 970 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through WildlifeStudiosModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [WildlifeStudiosModule],
      }).compile();
      const service = moduleRef.get(WildlifeStudiosService);
      expect(service).toBeInstanceOf(WildlifeStudiosService);
      await moduleRef.close();
    });

    it('exports the Site.WILDLIFE_STUDIOS = "wildlifestudios" enum value', () => {
      expect(Site.WILDLIFE_STUDIOS).toBe('wildlifestudios');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new WildlifeStudiosService();
      const result = await service.scrape({
        siteType: [Site.WILDLIFE_STUDIOS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'wildlifestudios-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.WILDLIFE_STUDIOS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Wildlife Studios');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/wildlifestudios/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/wildlifestudios/jobs/');
      ...
      expect(job0?.description).toContain('Wildlife Studios');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/wildlifestudios/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Wildlife Studios');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'wildlifestudios-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
