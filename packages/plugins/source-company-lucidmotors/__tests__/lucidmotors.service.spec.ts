...
import { LucidmotorsModule, LucidmotorsService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'lucidmotors-jobs.json'), 'utf8'),
);
...
/**
 * Spec 447 / T04 — `LucidmotorsService` unit tests (≥ 9 cases).
 */
describe('LucidmotorsService — Spec 447 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through LucidmotorsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [LucidmotorsModule],
      }).compile();
      const service = moduleRef.get(LucidmotorsService);
      expect(service).toBeInstanceOf(LucidmotorsService);
      await moduleRef.close();
    });

    it('exports the Site.LUCIDMOTORS = "lucidmotors" enum value', () => {
      expect(Site.LUCIDMOTORS).toBe('lucidmotors');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new LucidmotorsService();
      const result = await service.scrape({
        siteType: [Site.LUCIDMOTORS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'lucidmotors-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.LUCIDMOTORS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Lucid Motors');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/lucidmotors/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/lucidmotors/jobs/');
      ...
      expect(job0?.description).toContain('Lucid Motors');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/lucidmotors/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Lucid Motors');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'lucidmotors-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
