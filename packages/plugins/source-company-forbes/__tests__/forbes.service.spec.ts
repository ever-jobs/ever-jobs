...
import { ForbesModule, ForbesService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'forbes-jobs.json'), 'utf8'),
);
...
/**
 * Spec 851 / T04 — `ForbesService` unit tests (≥ 9 cases).
 */
describe('ForbesService — Spec 851 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ForbesModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ForbesModule],
      }).compile();
      const service = moduleRef.get(ForbesService);
      expect(service).toBeInstanceOf(ForbesService);
      await moduleRef.close();
    });

    it('exports the Site.FORBES = "forbes" enum value', () => {
      expect(Site.FORBES).toBe('forbes');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ForbesService();
      const result = await service.scrape({
        siteType: [Site.FORBES],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'forbes-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.FORBES);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Forbes');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/forbes/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/forbes/jobs/');
      ...
      expect(job0?.description).toContain('Forbes');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/forbes/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Forbes');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'forbes-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
