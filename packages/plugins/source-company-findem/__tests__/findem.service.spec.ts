...
import { FindemModule, FindemService } from '../src';

const COMPANY_NAME_EXPECT = 'Findem';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'findem-jobs.json'), 'utf8'),
);
...
describe('FindemService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through FindemModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [FindemModule],
      }).compile();
      const service = moduleRef.get(FindemService);
      expect(service).toBeInstanceOf(FindemService);
      await moduleRef.close();
    });

    it('exports the Site.FINDEM = "findem" enum value', () => {
      expect(Site.FINDEM).toBe('findem');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new FindemService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.FINDEM],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('findem');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.FINDEM);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('findem-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new FindemService(registry);
      const result = await service.scrape({
        siteType: [Site.FINDEM],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('findem');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('findem-x1');
      expect(result.jobs[0].site).toBe(Site.FINDEM);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('findem-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new FindemService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.FINDEM],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new FindemService();
      const result = await service.scrape({
        siteType: [Site.FINDEM],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
