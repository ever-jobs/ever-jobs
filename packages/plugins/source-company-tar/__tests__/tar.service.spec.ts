...
import { TARModule, TARService } from '../src';

const COMPANY_NAME_EXPECT = 'TAR';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'tar-jobs.json'), 'utf8'),
);
...
describe('TARService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through TARModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [TARModule],
      }).compile();
      const service = moduleRef.get(TARService);
      expect(service).toBeInstanceOf(TARService);
      await moduleRef.close();
    });

    it('exports the Site.TAR = "tar" enum value', () => {
      expect(Site.TAR).toBe('tar');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new TARService(registryWithAshby());
      ...
      expect(job0?.site).toBe(Site.TAR);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('tar-' + first.id);
      ...
      expect(calledUrls[0]).toContain('tar');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.TAR);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('tar-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new TARService(registry);
      const result = await service.scrape({
        siteType: [Site.TAR],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('tar');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('tar-x1');
      expect(result.jobs[0].site).toBe(Site.TAR);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('tar-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new TARService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.TAR],
      } as ScraperInputDto);
