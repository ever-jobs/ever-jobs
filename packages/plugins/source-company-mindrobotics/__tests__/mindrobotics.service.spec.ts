...
import { MindRoboticsModule, MindRoboticsService } from '../src';

const COMPANY_NAME_EXPECT = 'Mind Robotics';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'mindrobotics-jobs.json'), 'utf8'),
);
...
describe('MindRoboticsService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through MindRoboticsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [MindRoboticsModule],
      }).compile();
      const service = moduleRef.get(MindRoboticsService);
      expect(service).toBeInstanceOf(MindRoboticsService);
      await moduleRef.close();
    });

    it('exports the Site.MIND_ROBOTICS = "mindrobotics" enum value', () => {
      expect(Site.MIND_ROBOTICS).toBe('mindrobotics');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new MindRoboticsService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.MIND_ROBOTICS],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'mindrobotics-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.MIND_ROBOTICS);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('mindrobotics-' + first.id);
      ...
      expect(calledUrls[0]).toContain('mindrobotics');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.MIND_ROBOTICS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('mindrobotics-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new MindRoboticsService(registry);
      const result = await service.scrape({
        siteType: [Site.MIND_ROBOTICS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('mindrobotics');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('mindrobotics-x1');
      expect(result.jobs[0].site).toBe(Site.MIND_ROBOTICS);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('mindrobotics-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new MindRoboticsService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.MIND_ROBOTICS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new MindRoboticsService();
      const result = await service.scrape({
        siteType: [Site.MIND_ROBOTICS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
