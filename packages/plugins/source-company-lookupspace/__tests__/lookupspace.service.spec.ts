...
import { LookUpSpaceModule, LookUpSpaceService } from '../src';

const COMPANY_NAME_EXPECT = 'Look Up Space';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'lookupspace-jobs.json'), 'utf8'),
);
...
describe('LookUpSpaceService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through LookUpSpaceModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [LookUpSpaceModule],
      }).compile();
      const service = moduleRef.get(LookUpSpaceService);
      expect(service).toBeInstanceOf(LookUpSpaceService);
      await moduleRef.close();
    });

    it('exports the Site.LOOK_UP_SPACE = "lookupspace" enum value', () => {
      expect(Site.LOOK_UP_SPACE).toBe('lookupspace');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new LookUpSpaceService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.LOOK_UP_SPACE],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('lookup.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.LOOK_UP_SPACE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('lookupspace-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new LookUpSpaceService(registry);
      const result = await service.scrape({
        siteType: [Site.LOOK_UP_SPACE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('lookup');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('lookupspace-x1');
      expect(result.jobs[0].site).toBe(Site.LOOK_UP_SPACE);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('lookupspace-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new LookUpSpaceService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.LOOK_UP_SPACE],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new LookUpSpaceService();
      const result = await service.scrape({
        siteType: [Site.LOOK_UP_SPACE],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
