...
/**
 * Mastercard — Payments (HQ: Purchase, NY, USA).
 *
 * Source (Spec 1736): Workday board, scraped in this order:
 *   - `mastercard:1:CorporateCareers` — https://mastercard.wd1.myworkdayjobs.com/CorporateCareers
 *     verified live 2026-09-24: 1,066 open postings.
 *
 ...
 * party board.
 *
 * Tags: segment=workday-enterprise; industry=payments.
 */
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'mastercard:1:CorporateCareers', atsIdPrefix: 'wd-mastercard-' },
];

@SourcePlugin({
  site: Site.MASTERCARD,
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['mastercard.com'],
  description: 'Mastercard careers via Workday. Tags: segment=workday-enterprise; industry=payments.',
})
@Injectable()
export class MastercardService implements IScraper {
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Workday source plugin is not registered; cannot scrape Mastercard');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`Mastercard: delegating to Workday (${board.companySlug})`);
    ...
    this.logger.log(`Mastercard: scraped ${jobs.length} jobs`);
