...
import { HuntressModule, HuntressService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'huntress-jobs.json'), 'utf8'),
);
...
/**
 * Spec 467 / T04 — `HuntressService` unit tests (≥ 9 cases).
 */
describe('HuntressService — Spec 467 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through HuntressModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [HuntressModule],
      }).compile();
      const service = moduleRef.get(HuntressService);
      expect(service).toBeInstanceOf(HuntressService);
      await moduleRef.close();
    });

    it('exports the Site.HUNTRESS = "huntress" enum value', () => {
      expect(Site.HUNTRESS).toBe('huntress');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new HuntressService();
      const result = await service.scrape({
        siteType: [Site.HUNTRESS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'huntress-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.HUNTRESS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Huntress');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/huntress/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/huntress/jobs/');
      ...
      expect(job0?.description).toContain('Huntress');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/huntress/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Huntress');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'huntress-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
