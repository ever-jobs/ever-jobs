...
import { ReformationModule, ReformationService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'reformation-jobs.json'), 'utf8'),
);
...
/**
 * Spec 571 / T04 — `ReformationService` unit tests (≥ 9 cases).
 */
describe('ReformationService — Spec 571 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ReformationModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ReformationModule],
      }).compile();
      const service = moduleRef.get(ReformationService);
      expect(service).toBeInstanceOf(ReformationService);
      await moduleRef.close();
    });

    it('exports the Site.REFORMATION = "reformation" enum value', () => {
      expect(Site.REFORMATION).toBe('reformation');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ReformationService();
      const result = await service.scrape({
        siteType: [Site.REFORMATION],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'reformation-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.REFORMATION);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Reformation');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/reformation/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/reformation/jobs/');
      ...
      expect(job0?.description).toContain('Reformation');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/reformation/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Reformation');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'reformation-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
