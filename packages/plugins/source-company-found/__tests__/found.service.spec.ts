...
import { FoundModule, FoundService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'found-jobs.json'), 'utf8'),
);
...
/**
 * Spec 549 / T04 — `FoundService` unit tests (≥ 9 cases).
 */
describe('FoundService — Spec 549 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through FoundModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [FoundModule],
      }).compile();
      const service = moduleRef.get(FoundService);
      expect(service).toBeInstanceOf(FoundService);
      await moduleRef.close();
    });

    it('exports the Site.FOUND = "found" enum value', () => {
      expect(Site.FOUND).toBe('found');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new FoundService();
      ...
      const job0 = dto.jobs.find((j) => j.id === 'found-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.FOUND);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Found');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/found/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/found/jobs/');
      ...
      expect(job0?.description).toContain('Found');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/found/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Found');
      }
    });
  });
