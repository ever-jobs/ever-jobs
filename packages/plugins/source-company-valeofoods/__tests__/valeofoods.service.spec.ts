...
import { ValeoFoodsModule, ValeoFoodsService } from '../src';

const COMPANY_NAME_EXPECT = 'Valeo Foods';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'valeofoods-jobs.json'), 'utf8'),
);
...
describe('ValeoFoodsService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ValeoFoodsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ValeoFoodsModule],
      }).compile();
      const service = moduleRef.get(ValeoFoodsService);
      expect(service).toBeInstanceOf(ValeoFoodsService);
      await moduleRef.close();
    });

    it('exports the Site.VALEO_FOODS = "valeofoods" enum value', () => {
      expect(Site.VALEO_FOODS).toBe('valeofoods');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new ValeoFoodsService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.VALEO_FOODS],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('valeofoods');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.VALEO_FOODS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('valeofoods-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new ValeoFoodsService(registry);
      const result = await service.scrape({
        siteType: [Site.VALEO_FOODS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('valeofoods');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('valeofoods-x1');
      expect(result.jobs[0].site).toBe(Site.VALEO_FOODS);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('valeofoods-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new ValeoFoodsService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.VALEO_FOODS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new ValeoFoodsService();
      const result = await service.scrape({
        siteType: [Site.VALEO_FOODS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
