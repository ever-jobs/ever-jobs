...
import { DatafoldModule, DatafoldService } from '../src';

const COMPANY_NAME_EXPECT = 'Datafold';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'datafold-jobs.json'), 'utf8'),
);
...
describe('DatafoldService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through DatafoldModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [DatafoldModule],
      }).compile();
      const service = moduleRef.get(DatafoldService);
      expect(service).toBeInstanceOf(DatafoldService);
      await moduleRef.close();
    });

    it('exports the Site.DATAFOLD = "datafold" enum value', () => {
      expect(Site.DATAFOLD).toBe('datafold');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new DatafoldService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.DATAFOLD],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('datafold');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.DATAFOLD);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('datafold-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new DatafoldService(registry);
      const result = await service.scrape({
        siteType: [Site.DATAFOLD],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('datafold');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('datafold-x1');
      expect(result.jobs[0].site).toBe(Site.DATAFOLD);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('datafold-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new DatafoldService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.DATAFOLD],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new DatafoldService();
      const result = await service.scrape({
        siteType: [Site.DATAFOLD],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
