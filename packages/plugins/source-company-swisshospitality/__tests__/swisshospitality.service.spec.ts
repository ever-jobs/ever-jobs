...
import { SwissHospitalityModule, SwissHospitalityService } from '../src';

const COMPANY_NAME_EXPECT = 'Swiss Hospitality';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'swisshospitality-jobs.json'), 'utf8'),
);
...
describe('SwissHospitalityService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SwissHospitalityModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SwissHospitalityModule],
      }).compile();
      const service = moduleRef.get(SwissHospitalityService);
      expect(service).toBeInstanceOf(SwissHospitalityService);
      await moduleRef.close();
    });

    it('exports the Site.SWISS_HOSPITALITY = "swisshospitality" enum value', () => {
      expect(Site.SWISS_HOSPITALITY).toBe('swisshospitality');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new SwissHospitalityService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.SWISS_HOSPITALITY],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'swisshospitality-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.SWISS_HOSPITALITY);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('swisshospitality-' + first.id);
      ...
      expect(calledUrls[0]).toContain('SwissHospitality');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SWISS_HOSPITALITY);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('swisshospitality-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new SwissHospitalityService(registry);
      const result = await service.scrape({
        siteType: [Site.SWISS_HOSPITALITY],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('SwissHospitality');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('swisshospitality-x1');
      expect(result.jobs[0].site).toBe(Site.SWISS_HOSPITALITY);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('swisshospitality-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new SwissHospitalityService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.SWISS_HOSPITALITY],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new SwissHospitalityService();
      const result = await service.scrape({
        siteType: [Site.SWISS_HOSPITALITY],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
