...
import { WHOOPModule, WHOOPService } from '../src';

const COMPANY_NAME_EXPECT = 'WHOOP';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'whoop-jobs.json'), 'utf8'),
);
...
describe('WHOOPService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through WHOOPModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [WHOOPModule],
      }).compile();
      const service = moduleRef.get(WHOOPService);
      expect(service).toBeInstanceOf(WHOOPService);
      await moduleRef.close();
    });

    it('exports the Site.WHOOP = "whoop" enum value', () => {
      expect(Site.WHOOP).toBe('whoop');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new WHOOPService(registryWithLever());
      ...
      expect(calledUrls[0]).toContain('whoop');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.WHOOP);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('whoop-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new WHOOPService(registry);
      const result = await service.scrape({
        siteType: [Site.WHOOP],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('whoop');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('whoop-x1');
      expect(result.jobs[0].site).toBe(Site.WHOOP);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('whoop-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new WHOOPService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.WHOOP],
      } as ScraperInputDto);
