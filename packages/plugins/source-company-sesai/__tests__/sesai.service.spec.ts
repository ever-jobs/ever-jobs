...
import { SESAIModule, SESAIService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'sesai-jobs.json'), 'utf8'),
);
...
/**
 * Spec 796 / T04 — `SESAIService` unit tests (≥ 9 cases).
 */
describe('SESAIService — Spec 796 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SESAIModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SESAIModule],
      }).compile();
      const service = moduleRef.get(SESAIService);
      expect(service).toBeInstanceOf(SESAIService);
      await moduleRef.close();
    });

    it('exports the Site.SES_AI = "sesai" enum value', () => {
      expect(Site.SES_AI).toBe('sesai');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new SESAIService();
      const result = await service.scrape({
        siteType: [Site.SES_AI],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'sesai-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.SES_AI);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('SES AI');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/sesai/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/sesai/jobs/');
      ...
      expect(job0?.description).toContain('SES AI');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/sesai/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('SES AI');
      }
    });
  });
