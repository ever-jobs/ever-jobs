...
import { ArloModule, ArloService } from '../src';

const COMPANY_NAME_EXPECT = 'Arlo';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'arlo-jobs.json'), 'utf8'),
);
...
describe('ArloService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ArloModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ArloModule],
      }).compile();
      const service = moduleRef.get(ArloService);
      expect(service).toBeInstanceOf(ArloService);
      await moduleRef.close();
    });

    it('exports the Site.ARLO = "arlo" enum value', () => {
      expect(Site.ARLO).toBe('arlo');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new ArloService(registryWithAshby());
      ...
      expect(calledUrls[0]).toContain('arlo');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ARLO);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('arlo-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new ArloService(registry);
      const result = await service.scrape({
        siteType: [Site.ARLO],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('arlo');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('arlo-x1');
      expect(result.jobs[0].site).toBe(Site.ARLO);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('arlo-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new ArloService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.ARLO],
      } as ScraperInputDto);
