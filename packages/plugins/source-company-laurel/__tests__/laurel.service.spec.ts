...
import { LaurelModule, LaurelService } from '../src';

const COMPANY_NAME_EXPECT = 'Laurel';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'laurel-jobs.json'), 'utf8'),
);
...
describe('LaurelService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through LaurelModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [LaurelModule],
      }).compile();
      const service = moduleRef.get(LaurelService);
      expect(service).toBeInstanceOf(LaurelService);
      await moduleRef.close();
    });

    it('exports the Site.LAUREL = "laurel" enum value', () => {
      expect(Site.LAUREL).toBe('laurel');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new LaurelService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.LAUREL],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('laurel');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.LAUREL);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('laurel-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new LaurelService(registry);
      const result = await service.scrape({
        siteType: [Site.LAUREL],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('laurel');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('laurel-x1');
      expect(result.jobs[0].site).toBe(Site.LAUREL);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('laurel-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new LaurelService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.LAUREL],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new LaurelService();
      const result = await service.scrape({
        siteType: [Site.LAUREL],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
