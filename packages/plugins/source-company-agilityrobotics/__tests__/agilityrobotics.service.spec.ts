...
import { AgilityRoboticsModule, AgilityRoboticsService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'agilityrobotics-jobs.json'), 'utf8'),
);
...
/**
 * Spec 723 / T04 — `AgilityRoboticsService` unit tests (≥ 9 cases).
 */
describe('AgilityRoboticsService — Spec 723 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AgilityRoboticsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AgilityRoboticsModule],
      }).compile();
      const service = moduleRef.get(AgilityRoboticsService);
      expect(service).toBeInstanceOf(AgilityRoboticsService);
      await moduleRef.close();
    });

    it('exports the Site.AGILITY_ROBOTICS = "agilityrobotics" enum value', () => {
      expect(Site.AGILITY_ROBOTICS).toBe('agilityrobotics');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AgilityRoboticsService();
      const result = await service.scrape({
        siteType: [Site.AGILITY_ROBOTICS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'agilityrobotics-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.AGILITY_ROBOTICS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Agility Robotics');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/agilityrobotics/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/agilityrobotics/jobs/');
      ...
      expect(job0?.description).toContain('Agility Robotics');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/agilityrobotics/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Agility Robotics');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'agilityrobotics-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
