...
import { DealpathModule, DealpathService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'dealpath-jobs.json'), 'utf8'),
);
...
/**
 * Spec 834 / T04 — `DealpathService` unit tests (≥ 9 cases).
 */
describe('DealpathService — Spec 834 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through DealpathModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [DealpathModule],
      }).compile();
      const service = moduleRef.get(DealpathService);
      expect(service).toBeInstanceOf(DealpathService);
      await moduleRef.close();
    });

    it('exports the Site.DEALPATH = "dealpath" enum value', () => {
      expect(Site.DEALPATH).toBe('dealpath');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new DealpathService();
      const result = await service.scrape({
        siteType: [Site.DEALPATH],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'dealpath-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.DEALPATH);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Dealpath');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/dealpath/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/dealpath/jobs/');
      ...
      expect(job0?.description).toContain('Dealpath');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/dealpath/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Dealpath');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'dealpath-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
