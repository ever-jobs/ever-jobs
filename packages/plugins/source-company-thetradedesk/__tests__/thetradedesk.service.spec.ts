...
import { TheTradeDeskModule, TheTradeDeskService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'thetradedesk-jobs.json'), 'utf8'),
);
...
/**
 * Spec 955 / T04 — `TheTradeDeskService` unit tests (≥ 9 cases).
 */
describe('TheTradeDeskService — Spec 955 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through TheTradeDeskModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [TheTradeDeskModule],
      }).compile();
      const service = moduleRef.get(TheTradeDeskService);
      expect(service).toBeInstanceOf(TheTradeDeskService);
      await moduleRef.close();
    });

    it('exports the Site.THE_TRADE_DESK = "thetradedesk" enum value', () => {
      expect(Site.THE_TRADE_DESK).toBe('thetradedesk');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new TheTradeDeskService();
      const result = await service.scrape({
        siteType: [Site.THE_TRADE_DESK],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'thetradedesk-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.THE_TRADE_DESK);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('The Trade Desk');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/thetradedesk/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/thetradedesk/jobs/');
      ...
      expect(job0?.description).toContain('The Trade Desk');
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('The Trade Desk');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'thetradedesk-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
