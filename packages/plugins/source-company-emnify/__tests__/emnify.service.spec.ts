...
import { EmnifyModule, EmnifyService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'emnify-jobs.json'), 'utf8'),
);
...
/**
 * Spec 841 / T04 — `EmnifyService` unit tests (≥ 9 cases).
 */
describe('EmnifyService — Spec 841 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through EmnifyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [EmnifyModule],
      }).compile();
      const service = moduleRef.get(EmnifyService);
      expect(service).toBeInstanceOf(EmnifyService);
      await moduleRef.close();
    });

    it('exports the Site.EMNIFY = "emnify" enum value', () => {
      expect(Site.EMNIFY).toBe('emnify');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new EmnifyService();
      const result = await service.scrape({
        siteType: [Site.EMNIFY],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'emnify-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.EMNIFY);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('emnify');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/emnify/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/emnify/jobs/');
      ...
      expect(job0?.description).toContain('emnify');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/emnify/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('emnify');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'emnify-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
