...
import { HealthPartnersModule, HealthPartnersService } from '../src';

const COMPANY_NAME_EXPECT = 'HealthPartners';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'healthpartners-jobs.json'), 'utf8'),
);
...
describe('HealthPartnersService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through HealthPartnersModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [HealthPartnersModule],
      }).compile();
      const service = moduleRef.get(HealthPartnersService);
      expect(service).toBeInstanceOf(HealthPartnersService);
      await moduleRef.close();
    });

    it('exports the Site.HEALTHPARTNERS = "healthpartners" enum value', () => {
      expect(Site.HEALTHPARTNERS).toBe('healthpartners');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new HealthPartnersService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.HEALTHPARTNERS],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'healthpartners-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.HEALTHPARTNERS);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('healthpartners-' + first.id);
      ...
      expect(calledUrls[0]).toContain('HealthPartners1');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.HEALTHPARTNERS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('healthpartners-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new HealthPartnersService(registry);
      const result = await service.scrape({
        siteType: [Site.HEALTHPARTNERS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('HealthPartners1');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('healthpartners-x1');
      expect(result.jobs[0].site).toBe(Site.HEALTHPARTNERS);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('healthpartners-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new HealthPartnersService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.HEALTHPARTNERS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new HealthPartnersService();
      const result = await service.scrape({
        siteType: [Site.HEALTHPARTNERS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
