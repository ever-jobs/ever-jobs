...
import { AlbedoModule, AlbedoService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'albedo-jobs.json'), 'utf8'),
);
...
/**
 * Spec 227 / T04 — `AlbedoService` unit tests (≥ 9 cases).
 */
describe('AlbedoService — Spec 227 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AlbedoModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AlbedoModule],
      }).compile();
      const service = moduleRef.get(AlbedoService);
      expect(service).toBeInstanceOf(AlbedoService);
      await moduleRef.close();
    });

    it('exports the Site.ALBEDO = "albedo" enum value', () => {
      expect(Site.ALBEDO).toBe('albedo');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AlbedoService();
      const result = await service.scrape({
        siteType: [Site.ALBEDO],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'albedo-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ALBEDO);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Albedo');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/albedo/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/albedo/jobs/');
      ...
      expect(job0?.description).toContain('Albedo');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/albedo/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Albedo');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'albedo-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
