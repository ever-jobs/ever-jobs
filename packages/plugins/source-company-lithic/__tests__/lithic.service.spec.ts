...
import { LithicModule, LithicService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'lithic-jobs.json'), 'utf8'),
);
...
/**
 * Spec 476 / T04 — `LithicService` unit tests (≥ 9 cases).
 */
describe('LithicService — Spec 476 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through LithicModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [LithicModule],
      }).compile();
      const service = moduleRef.get(LithicService);
      expect(service).toBeInstanceOf(LithicService);
      await moduleRef.close();
    });

    it('exports the Site.LITHIC = "lithic" enum value', () => {
      expect(Site.LITHIC).toBe('lithic');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new LithicService();
      const result = await service.scrape({
        siteType: [Site.LITHIC],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'lithic-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.LITHIC);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Lithic');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/lithic/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/lithic/jobs/');
      ...
      expect(job0?.description).toContain('Lithic');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/lithic/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Lithic');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'lithic-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
