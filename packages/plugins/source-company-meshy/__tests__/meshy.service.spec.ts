...
import { MeshyModule, MeshyService } from '../src';

const COMPANY_NAME_EXPECT = 'Meshy';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'meshy-jobs.json'), 'utf8'),
);
...
describe('MeshyService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through MeshyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [MeshyModule],
      }).compile();
      const service = moduleRef.get(MeshyService);
      expect(service).toBeInstanceOf(MeshyService);
      await moduleRef.close();
    });

    it('exports the Site.MESHY = "meshy" enum value', () => {
      expect(Site.MESHY).toBe('meshy');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new MeshyService(registryWithAshby());
      ...
      expect(calledUrls[0]).toContain('meshy');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.MESHY);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('meshy-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new MeshyService(registry);
      const result = await service.scrape({
        siteType: [Site.MESHY],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('meshy');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('meshy-x1');
      expect(result.jobs[0].site).toBe(Site.MESHY);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('meshy-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new MeshyService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.MESHY],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new MeshyService();
      const result = await service.scrape({
        siteType: [Site.MESHY],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
