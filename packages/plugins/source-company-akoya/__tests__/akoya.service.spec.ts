...
import { AkoyaModule, AkoyaService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'akoya-jobs.json'), 'utf8'),
);
...
/**
 * Spec 223 / T04 — `AkoyaService` unit tests (≥ 9 cases).
 */
describe('AkoyaService — Spec 223 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AkoyaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AkoyaModule],
      }).compile();
      const service = moduleRef.get(AkoyaService);
      expect(service).toBeInstanceOf(AkoyaService);
      await moduleRef.close();
    });

    it('exports the Site.AKOYA = "akoya" enum value', () => {
      expect(Site.AKOYA).toBe('akoya');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AkoyaService();
      ...
      const job0 = dto.jobs.find((j) => j.id === 'akoya-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.AKOYA);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Akoya');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/akoya/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/akoya/jobs/');
      ...
      expect(job0?.description).toContain('Akoya');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/akoya/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Akoya');
      }
    });
  });
