...
import { SeuratTechnologiesModule, SeuratTechnologiesService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'seurat-jobs.json'), 'utf8'),
);
...
/**
 * Spec 631 / T04 — `SeuratTechnologiesService` unit tests (≥ 9 cases).
 */
describe('SeuratTechnologiesService — Spec 631 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SeuratTechnologiesModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SeuratTechnologiesModule],
      }).compile();
      const service = moduleRef.get(SeuratTechnologiesService);
      expect(service).toBeInstanceOf(SeuratTechnologiesService);
      await moduleRef.close();
    });

    it('exports the Site.SEURAT_TECHNOLOGIES = "seurat" enum value', () => {
      expect(Site.SEURAT_TECHNOLOGIES).toBe('seurat');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new SeuratTechnologiesService();
      const result = await service.scrape({
        siteType: [Site.SEURAT_TECHNOLOGIES],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'seurat-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.SEURAT_TECHNOLOGIES);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Seurat Technologies');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/seurat/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/seurat/jobs/');
      ...
      expect(job0?.description).toContain('Seurat Technologies');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/seurat/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Seurat Technologies');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'seurat-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
