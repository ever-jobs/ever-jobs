...
import { BoxLunchHotTopicModule, BoxLunchHotTopicService } from '../src';

const COMPANY_NAME_EXPECT = 'BoxLunch & Hot Topic';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'boxlunch-jobs.json'), 'utf8'),
);
...
describe('BoxLunchHotTopicService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through BoxLunchHotTopicModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [BoxLunchHotTopicModule],
      }).compile();
      const service = moduleRef.get(BoxLunchHotTopicService);
      expect(service).toBeInstanceOf(BoxLunchHotTopicService);
      await moduleRef.close();
    });

    it('exports the Site.BOXLUNCH_HOT_TOPIC = "boxlunch" enum value', () => {
      expect(Site.BOXLUNCH_HOT_TOPIC).toBe('boxlunch');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new BoxLunchHotTopicService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.BOXLUNCH_HOT_TOPIC],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'boxlunch-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.BOXLUNCH_HOT_TOPIC);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('boxlunch-' + first.id);
      ...
      expect(calledUrls[0]).toContain('boxlunch');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.BOXLUNCH_HOT_TOPIC);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('boxlunch-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new BoxLunchHotTopicService(registry);
      const result = await service.scrape({
        siteType: [Site.BOXLUNCH_HOT_TOPIC],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('boxlunch');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('boxlunch-x1');
      expect(result.jobs[0].site).toBe(Site.BOXLUNCH_HOT_TOPIC);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('boxlunch-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new BoxLunchHotTopicService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.BOXLUNCH_HOT_TOPIC],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new BoxLunchHotTopicService();
      const result = await service.scrape({
        siteType: [Site.BOXLUNCH_HOT_TOPIC],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
