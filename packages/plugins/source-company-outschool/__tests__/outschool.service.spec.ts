...
import { OutschoolModule, OutschoolService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'outschool-jobs.json'), 'utf8'),
);
...
/**
 * Spec 574 / T04 — `OutschoolService` unit tests (≥ 9 cases).
 */
describe('OutschoolService — Spec 574 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through OutschoolModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [OutschoolModule],
      }).compile();
      const service = moduleRef.get(OutschoolService);
      expect(service).toBeInstanceOf(OutschoolService);
      await moduleRef.close();
    });

    it('exports the Site.OUTSCHOOL = "outschool" enum value', () => {
      expect(Site.OUTSCHOOL).toBe('outschool');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new OutschoolService();
      const result = await service.scrape({
        siteType: [Site.OUTSCHOOL],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'outschool-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.OUTSCHOOL);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Outschool');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/outschool/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/outschool/jobs/');
      ...
      expect(job0?.description).toContain('Outschool');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/outschool/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Outschool');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'outschool-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
