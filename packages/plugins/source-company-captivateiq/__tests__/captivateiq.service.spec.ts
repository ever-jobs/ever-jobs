...
import { CaptivateIQModule, CaptivateIQService } from '../src';

const COMPANY_NAME_EXPECT = 'CaptivateIQ';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'captivateiq-jobs.json'), 'utf8'),
);
...
describe('CaptivateIQService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CaptivateIQModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CaptivateIQModule],
      }).compile();
      const service = moduleRef.get(CaptivateIQService);
      expect(service).toBeInstanceOf(CaptivateIQService);
      await moduleRef.close();
    });

    it('exports the Site.CAPTIVATEIQ = "captivateiq" enum value', () => {
      expect(Site.CAPTIVATEIQ).toBe('captivateiq');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new CaptivateIQService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.CAPTIVATEIQ],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'captivateiq-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.CAPTIVATEIQ);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('captivateiq-' + first.id);
      ...
      expect(calledUrls[0]).toContain('captivateiq');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.CAPTIVATEIQ);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('captivateiq-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new CaptivateIQService(registry);
      const result = await service.scrape({
        siteType: [Site.CAPTIVATEIQ],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('captivateiq');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('captivateiq-x1');
      expect(result.jobs[0].site).toBe(Site.CAPTIVATEIQ);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('captivateiq-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new CaptivateIQService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.CAPTIVATEIQ],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new CaptivateIQService();
      const result = await service.scrape({
        siteType: [Site.CAPTIVATEIQ],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
