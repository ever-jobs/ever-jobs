...
import { AstranisModule, AstranisService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'astranis-jobs.json'), 'utf8'),
);
...
/**
 * Spec 757 / T04 — `AstranisService` unit tests (≥ 9 cases).
 */
describe('AstranisService — Spec 757 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AstranisModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AstranisModule],
      }).compile();
      const service = moduleRef.get(AstranisService);
      expect(service).toBeInstanceOf(AstranisService);
      await moduleRef.close();
    });

    it('exports the Site.ASTRANIS = "astranis" enum value', () => {
      expect(Site.ASTRANIS).toBe('astranis');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AstranisService();
      const result = await service.scrape({
        siteType: [Site.ASTRANIS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'astranis-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ASTRANIS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Astranis');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/astranis/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/astranis/jobs/');
      ...
      expect(job0?.description).toContain('Astranis');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/astranis/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Astranis');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'astranis-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
