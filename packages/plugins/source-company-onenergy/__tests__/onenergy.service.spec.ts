...
import { OnEnergyModule, OnEnergyService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'onenergy-jobs.json'), 'utf8'),
);
...
/**
 * Spec 609 / T04 — `OnEnergyService` unit tests (≥ 9 cases).
 */
describe('OnEnergyService — Spec 609 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through OnEnergyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [OnEnergyModule],
      }).compile();
      const service = moduleRef.get(OnEnergyService);
      expect(service).toBeInstanceOf(OnEnergyService);
      await moduleRef.close();
    });

    it('exports the Site.ON_ENERGY = "onenergy" enum value', () => {
      expect(Site.ON_ENERGY).toBe('onenergy');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new OnEnergyService();
      const result = await service.scrape({
        siteType: [Site.ON_ENERGY],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'onenergy-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ON_ENERGY);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('ON.energy');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/onenergy/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/onenergy/jobs/');
      ...
      expect(job0?.description).toContain('ON.energy');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/onenergy/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('ON.energy');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'onenergy-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
