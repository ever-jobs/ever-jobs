...
import { PrincessPollyModule, PrincessPollyService } from '../src';

const COMPANY_NAME_EXPECT = 'Princess Polly';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'princesspolly-jobs.json'), 'utf8'),
);
...
describe('PrincessPollyService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through PrincessPollyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [PrincessPollyModule],
      }).compile();
      const service = moduleRef.get(PrincessPollyService);
      expect(service).toBeInstanceOf(PrincessPollyService);
      await moduleRef.close();
    });

    it('exports the Site.PRINCESS_POLLY = "princesspolly" enum value', () => {
      expect(Site.PRINCESS_POLLY).toBe('princesspolly');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new PrincessPollyService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.PRINCESS_POLLY],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'princesspolly-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.PRINCESS_POLLY);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('princesspolly-' + first.id);
      ...
      expect(calledUrls[0]).toContain('princesspolly');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.PRINCESS_POLLY);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('princesspolly-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new PrincessPollyService(registry);
      const result = await service.scrape({
        siteType: [Site.PRINCESS_POLLY],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('princesspolly');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('princesspolly-x1');
      expect(result.jobs[0].site).toBe(Site.PRINCESS_POLLY);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('princesspolly-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new PrincessPollyService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.PRINCESS_POLLY],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new PrincessPollyService();
      const result = await service.scrape({
        siteType: [Site.PRINCESS_POLLY],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
