...
import { TrustWalletModule, TrustWalletService } from '../src';

const COMPANY_NAME_EXPECT = 'Trust Wallet';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'trustwallet-jobs.json'), 'utf8'),
);
...
describe('TrustWalletService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through TrustWalletModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [TrustWalletModule],
      }).compile();
      const service = moduleRef.get(TrustWalletService);
      expect(service).toBeInstanceOf(TrustWalletService);
      await moduleRef.close();
    });

    it('exports the Site.TRUST_WALLET = "trustwallet" enum value', () => {
      expect(Site.TRUST_WALLET).toBe('trustwallet');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new TrustWalletService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.TRUST_WALLET],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'trustwallet-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.TRUST_WALLET);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('trustwallet-' + first.id);
      ...
      expect(calledUrls[0]).toContain('trust-wallet');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.TRUST_WALLET);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('trustwallet-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new TrustWalletService(registry);
      const result = await service.scrape({
        siteType: [Site.TRUST_WALLET],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('trust-wallet');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('trustwallet-x1');
      expect(result.jobs[0].site).toBe(Site.TRUST_WALLET);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('trustwallet-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new TrustWalletService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.TRUST_WALLET],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new TrustWalletService();
      const result = await service.scrape({
        siteType: [Site.TRUST_WALLET],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
