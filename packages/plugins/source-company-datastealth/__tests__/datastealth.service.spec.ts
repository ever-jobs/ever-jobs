...
import { DataStealthModule, DataStealthService } from '../src';

const COMPANY_NAME_EXPECT = 'DataStealth';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'datastealth-jobs.json'), 'utf8'),
);
...
describe('DataStealthService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through DataStealthModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [DataStealthModule],
      }).compile();
      const service = moduleRef.get(DataStealthService);
      expect(service).toBeInstanceOf(DataStealthService);
      await moduleRef.close();
    });

    it('exports the Site.DATASTEALTH = "datastealth" enum value', () => {
      expect(Site.DATASTEALTH).toBe('datastealth');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new DataStealthService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.DATASTEALTH],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'datastealth-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.DATASTEALTH);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('datastealth-' + first.id);
      ...
      expect(calledUrls[0]).toContain('datastealth');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.DATASTEALTH);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('datastealth-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new DataStealthService(registry);
      const result = await service.scrape({
        siteType: [Site.DATASTEALTH],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('datastealth');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('datastealth-x1');
      expect(result.jobs[0].site).toBe(Site.DATASTEALTH);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('datastealth-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new DataStealthService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.DATASTEALTH],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new DataStealthService();
      const result = await service.scrape({
        siteType: [Site.DATASTEALTH],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
