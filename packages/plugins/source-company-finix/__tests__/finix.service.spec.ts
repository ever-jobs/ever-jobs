...
import { FinixModule, FinixService } from '../src';

const COMPANY_NAME_EXPECT = 'Finix';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'finix-jobs.json'), 'utf8'),
);
...
describe('FinixService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through FinixModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [FinixModule],
      }).compile();
      const service = moduleRef.get(FinixService);
      expect(service).toBeInstanceOf(FinixService);
      await moduleRef.close();
    });

    it('exports the Site.FINIX = "finix" enum value', () => {
      expect(Site.FINIX).toBe('finix');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new FinixService(registryWithLever());
      ...
      expect(calledUrls[0]).toContain('finix');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.FINIX);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('finix-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new FinixService(registry);
      const result = await service.scrape({
        siteType: [Site.FINIX],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('finix');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('finix-x1');
      expect(result.jobs[0].site).toBe(Site.FINIX);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('finix-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new FinixService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.FINIX],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new FinixService();
      const result = await service.scrape({
        siteType: [Site.FINIX],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
