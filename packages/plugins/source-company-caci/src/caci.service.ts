...
/**
 * CACI — Defense and intelligence IT (HQ: Reston, VA, USA).
 *
 * Source (Spec 1736): Workday board, scraped in this order:
 *   - `caci:1:External` — https://caci.wd1.myworkdayjobs.com/External
 *     verified live 2026-09-24: 1,873 open postings.
 *
 ...
 * through untouched.
 *
 * Tags: segment=workday-enterprise; industry=defense-and-intelligence-it.
 */
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'caci:1:External', atsIdPrefix: 'wd-caci-' },
];
...
@SourcePlugin({
  ...
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['caci.com'],
  description: 'CACI careers via Workday. Tags: segment=workday-enterprise; industry=defense-and-intelligence-it.',
})
@Injectable()
export class CaciService implements IScraper {
  private readonly logger = new Logger(CaciService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Workday source plugin is not registered; cannot scrape CACI');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`CACI: delegating to Workday (${board.companySlug})`);
    ...
    this.logger.log(`CACI: scraped ${jobs.length} jobs`);
