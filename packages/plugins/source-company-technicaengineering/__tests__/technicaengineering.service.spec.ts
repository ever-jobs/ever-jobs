...
import { TechnicaEngineeringModule, TechnicaEngineeringService } from '../src';

const COMPANY_NAME_EXPECT = 'Technica Engineering';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'technicaengineering-jobs.json'), 'utf8'),
);
...
describe('TechnicaEngineeringService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through TechnicaEngineeringModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [TechnicaEngineeringModule],
      }).compile();
      const service = moduleRef.get(TechnicaEngineeringService);
      expect(service).toBeInstanceOf(TechnicaEngineeringService);
      await moduleRef.close();
    });

    it('exports the Site.TECHNICA_ENGINEERING = "technicaengineering" enum value', () => {
      expect(Site.TECHNICA_ENGINEERING).toBe('technicaengineering');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new TechnicaEngineeringService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.TECHNICA_ENGINEERING],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'technicaengineering-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Recruitee's defaults
      expect(job0?.site).toBe(Site.TECHNICA_ENGINEERING);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('technicaengineering-' + first.id);
      ...
      expect(calledUrls[0]).toContain('technicaengineeringgmbh.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.TECHNICA_ENGINEERING);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('technicaengineering-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new TechnicaEngineeringService(registry);
      const result = await service.scrape({
        siteType: [Site.TECHNICA_ENGINEERING],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('technicaengineeringgmbh');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('technicaengineering-x1');
      expect(result.jobs[0].site).toBe(Site.TECHNICA_ENGINEERING);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('technicaengineering-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new TechnicaEngineeringService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.TECHNICA_ENGINEERING],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new TechnicaEngineeringService();
      const result = await service.scrape({
        siteType: [Site.TECHNICA_ENGINEERING],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
