...
import { MoniepointModule, MoniepointService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'moniepoint-jobs.json'), 'utf8'),
);
...
/**
 * Spec 514 / T04 — `MoniepointService` unit tests (≥ 9 cases).
 */
describe('MoniepointService — Spec 514 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through MoniepointModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [MoniepointModule],
      }).compile();
      const service = moduleRef.get(MoniepointService);
      expect(service).toBeInstanceOf(MoniepointService);
      await moduleRef.close();
    });

    it('exports the Site.MONIEPOINT = "moniepoint" enum value', () => {
      expect(Site.MONIEPOINT).toBe('moniepoint');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new MoniepointService();
      const result = await service.scrape({
        siteType: [Site.MONIEPOINT],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'moniepoint-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.MONIEPOINT);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Moniepoint');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/moniepoint/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/moniepoint/jobs/');
      ...
      expect(job0?.description).toContain('Moniepoint');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/moniepoint/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Moniepoint');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'moniepoint-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
