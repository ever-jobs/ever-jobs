...
import { OrigisEnergyModule, OrigisEnergyService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'origisenergy-jobs.json'), 'utf8'),
);
...
/**
 * Spec 904 / T04 — `OrigisEnergyService` unit tests (≥ 9 cases).
 */
describe('OrigisEnergyService — Spec 904 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through OrigisEnergyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [OrigisEnergyModule],
      }).compile();
      const service = moduleRef.get(OrigisEnergyService);
      expect(service).toBeInstanceOf(OrigisEnergyService);
      await moduleRef.close();
    });

    it('exports the Site.ORIGIS_ENERGY = "origisenergy" enum value', () => {
      expect(Site.ORIGIS_ENERGY).toBe('origisenergy');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new OrigisEnergyService();
      const result = await service.scrape({
        siteType: [Site.ORIGIS_ENERGY],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'origisenergy-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ORIGIS_ENERGY);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Origis Energy');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/origisenergy/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/origisenergy/jobs/');
      ...
      expect(job0?.description).toContain('Origis Energy');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/origisenergy/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Origis Energy');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'origisenergy-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
