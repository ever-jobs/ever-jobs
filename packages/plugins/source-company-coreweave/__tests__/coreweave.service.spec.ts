...
import { CoreWeaveModule, CoreWeaveService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'coreweave-jobs.json'), 'utf8'),
);
...
/**
 * Spec 715 / T04 — `CoreWeaveService` unit tests (≥ 9 cases).
 */
describe('CoreWeaveService — Spec 715 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CoreWeaveModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CoreWeaveModule],
      }).compile();
      const service = moduleRef.get(CoreWeaveService);
      expect(service).toBeInstanceOf(CoreWeaveService);
      await moduleRef.close();
    });

    it('exports the Site.COREWEAVE = "coreweave" enum value', () => {
      expect(Site.COREWEAVE).toBe('coreweave');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new CoreWeaveService();
      const result = await service.scrape({
        siteType: [Site.COREWEAVE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'coreweave-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.COREWEAVE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('CoreWeave');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/coreweave/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/coreweave/jobs/');
      ...
      expect(job0?.description).toContain('CoreWeave');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/coreweave/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('CoreWeave');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'coreweave-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
