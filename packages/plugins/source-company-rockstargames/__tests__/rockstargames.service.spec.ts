...
import { RockstargamesModule, RockstargamesService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'rockstargames-jobs.json'), 'utf8'),
);
...
/**
 * Spec 573 / T04 — `RockstargamesService` unit tests (≥ 9 cases).
 */
describe('RockstargamesService — Spec 573 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through RockstargamesModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [RockstargamesModule],
      }).compile();
      const service = moduleRef.get(RockstargamesService);
      expect(service).toBeInstanceOf(RockstargamesService);
      await moduleRef.close();
    });

    it('exports the Site.ROCKSTAR_GAMES = "rockstargames" enum value', () => {
      expect(Site.ROCKSTAR_GAMES).toBe('rockstargames');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new RockstargamesService();
      const result = await service.scrape({
        siteType: [Site.ROCKSTAR_GAMES],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'rockstargames-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ROCKSTAR_GAMES);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Rockstar Games');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/rockstargames/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/rockstargames/jobs/');
      ...
      expect(job0?.description).toContain('Rockstar Games');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/rockstargames/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Rockstar Games');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'rockstargames-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
