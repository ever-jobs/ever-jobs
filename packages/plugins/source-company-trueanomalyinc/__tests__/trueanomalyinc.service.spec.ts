...
import { TrueAnomalyModule, TrueAnomalyService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'trueanomalyinc-jobs.json'), 'utf8'),
);
...
/**
 * Spec 960 / T04 — `TrueAnomalyService` unit tests (≥ 9 cases).
 */
describe('TrueAnomalyService — Spec 960 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through TrueAnomalyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [TrueAnomalyModule],
      }).compile();
      const service = moduleRef.get(TrueAnomalyService);
      expect(service).toBeInstanceOf(TrueAnomalyService);
      await moduleRef.close();
    });

    it('exports the Site.TRUE_ANOMALY = "trueanomalyinc" enum value', () => {
      expect(Site.TRUE_ANOMALY).toBe('trueanomalyinc');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new TrueAnomalyService();
      const result = await service.scrape({
        siteType: [Site.TRUE_ANOMALY],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'trueanomalyinc-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.TRUE_ANOMALY);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('True Anomaly');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/trueanomalyinc/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/trueanomalyinc/jobs/');
      ...
      expect(job0?.description).toContain('True Anomaly');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/trueanomalyinc/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('True Anomaly');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'trueanomalyinc-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
