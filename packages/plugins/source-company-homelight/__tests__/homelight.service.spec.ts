...
import { HomeLightModule, HomeLightService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'homelight-jobs.json'), 'utf8'),
);
...
/**
 * Spec 626 / T04 — `HomeLightService` unit tests (≥ 9 cases).
 */
describe('HomeLightService — Spec 626 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through HomeLightModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [HomeLightModule],
      }).compile();
      const service = moduleRef.get(HomeLightService);
      expect(service).toBeInstanceOf(HomeLightService);
      await moduleRef.close();
    });

    it('exports the Site.HOMELIGHT = "homelight" enum value', () => {
      expect(Site.HOMELIGHT).toBe('homelight');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new HomeLightService();
      const result = await service.scrape({
        siteType: [Site.HOMELIGHT],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'homelight-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.HOMELIGHT);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('HomeLight');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/homelight/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/homelight/jobs/');
      ...
      expect(job0?.description).toContain('HomeLight');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/homelight/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('HomeLight');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'homelight-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
