...
import { NetradyneModule, NetradyneService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'netradyne-jobs.json'), 'utf8'),
);
...
/**
 * Spec 750 / T04 — `NetradyneService` unit tests (≥ 9 cases).
 */
describe('NetradyneService — Spec 750 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through NetradyneModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [NetradyneModule],
      }).compile();
      const service = moduleRef.get(NetradyneService);
      expect(service).toBeInstanceOf(NetradyneService);
      await moduleRef.close();
    });

    it('exports the Site.NETRADYNE = "netradyne" enum value', () => {
      expect(Site.NETRADYNE).toBe('netradyne');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new NetradyneService();
      const result = await service.scrape({
        siteType: [Site.NETRADYNE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'netradyne-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.NETRADYNE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Netradyne');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/netradyne/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/netradyne/jobs/');
      ...
      expect(job0?.description).toContain('Netradyne');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/netradyne/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Netradyne');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'netradyne-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
