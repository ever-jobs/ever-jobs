...
import { MarvelfusionModule, MarvelfusionService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'marvelfusion-jobs.json'), 'utf8'),
);
...
/**
 * Spec 517 / T04 — `MarvelfusionService` unit tests (≥ 9 cases).
 */
describe('MarvelfusionService — Spec 517 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through MarvelfusionModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [MarvelfusionModule],
      }).compile();
      const service = moduleRef.get(MarvelfusionService);
      expect(service).toBeInstanceOf(MarvelfusionService);
      await moduleRef.close();
    });

    it('exports the Site.MARVEL_FUSION = "marvelfusion" enum value', () => {
      expect(Site.MARVEL_FUSION).toBe('marvelfusion');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new MarvelfusionService();
      const result = await service.scrape({
        siteType: [Site.MARVEL_FUSION],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'marvelfusion-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.MARVEL_FUSION);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Marvel Fusion');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/marvelfusion/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/marvelfusion/jobs/');
      ...
      expect(job0?.description).toContain('Marvel Fusion');
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Marvel Fusion');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'marvelfusion-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
