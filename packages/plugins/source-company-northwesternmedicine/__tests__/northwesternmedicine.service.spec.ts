...
import { NorthwesternMedicineModule, NorthwesternMedicineService } from '../src';

const COMPANY_NAME_EXPECT = 'Northwestern Medicine';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'northwesternmedicine-jobs.json'), 'utf8'),
);
...
describe('NorthwesternMedicineService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through NorthwesternMedicineModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [NorthwesternMedicineModule],
      }).compile();
      const service = moduleRef.get(NorthwesternMedicineService);
      expect(service).toBeInstanceOf(NorthwesternMedicineService);
      await moduleRef.close();
    });

    it('exports the Site.NORTHWESTERN_MEDICINE = "northwesternmedicine" enum value', () => {
      expect(Site.NORTHWESTERN_MEDICINE).toBe('northwesternmedicine');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new NorthwesternMedicineService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.NORTHWESTERN_MEDICINE],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'northwesternmedicine-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.NORTHWESTERN_MEDICINE);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('northwesternmedicine-' + first.id);
      ...
      expect(calledUrls[0]).toContain('northwesternmedicine');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.NORTHWESTERN_MEDICINE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('northwesternmedicine-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new NorthwesternMedicineService(registry);
      const result = await service.scrape({
        siteType: [Site.NORTHWESTERN_MEDICINE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('northwesternmedicine');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('northwesternmedicine-x1');
      expect(result.jobs[0].site).toBe(Site.NORTHWESTERN_MEDICINE);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('northwesternmedicine-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new NorthwesternMedicineService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.NORTHWESTERN_MEDICINE],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new NorthwesternMedicineService();
      const result = await service.scrape({
        siteType: [Site.NORTHWESTERN_MEDICINE],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
