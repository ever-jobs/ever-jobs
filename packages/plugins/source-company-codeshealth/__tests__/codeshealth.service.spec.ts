...
import { CodesHealthModule, CodesHealthService } from '../src';

const COMPANY_NAME_EXPECT = 'Codes Health';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'codeshealth-jobs.json'), 'utf8'),
);
...
describe('CodesHealthService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CodesHealthModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CodesHealthModule],
      }).compile();
      const service = moduleRef.get(CodesHealthService);
      expect(service).toBeInstanceOf(CodesHealthService);
      await moduleRef.close();
    });

    it('exports the Site.CODES_HEALTH = "codeshealth" enum value', () => {
      expect(Site.CODES_HEALTH).toBe('codeshealth');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new CodesHealthService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.CODES_HEALTH],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'codeshealth-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.CODES_HEALTH);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('codeshealth-' + first.id);
      ...
      expect(calledUrls[0]).toContain('codes-health');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.CODES_HEALTH);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('codeshealth-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new CodesHealthService(registry);
      const result = await service.scrape({
        siteType: [Site.CODES_HEALTH],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('codes-health');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('codeshealth-x1');
      expect(result.jobs[0].site).toBe(Site.CODES_HEALTH);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('codeshealth-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new CodesHealthService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.CODES_HEALTH],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new CodesHealthService();
      const result = await service.scrape({
        siteType: [Site.CODES_HEALTH],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
