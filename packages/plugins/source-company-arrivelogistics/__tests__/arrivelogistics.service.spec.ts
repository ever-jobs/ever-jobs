...
import { ArriveLogisticsModule, ArriveLogisticsService } from '../src';

const COMPANY_NAME_EXPECT = 'Arrive Logistics';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'arrivelogistics-jobs.json'), 'utf8'),
);
...
describe('ArriveLogisticsService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ArriveLogisticsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ArriveLogisticsModule],
      }).compile();
      const service = moduleRef.get(ArriveLogisticsService);
      expect(service).toBeInstanceOf(ArriveLogisticsService);
      await moduleRef.close();
    });

    it('exports the Site.ARRIVE_LOGISTICS = "arrivelogistics" enum value', () => {
      expect(Site.ARRIVE_LOGISTICS).toBe('arrivelogistics');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new ArriveLogisticsService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.ARRIVE_LOGISTICS],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'arrivelogistics-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.ARRIVE_LOGISTICS);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('arrivelogistics-' + first.id);
      ...
      expect(calledUrls[0]).toContain('arrivelogistics');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ARRIVE_LOGISTICS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('arrivelogistics-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new ArriveLogisticsService(registry);
      const result = await service.scrape({
        siteType: [Site.ARRIVE_LOGISTICS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('arrivelogistics');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('arrivelogistics-x1');
      expect(result.jobs[0].site).toBe(Site.ARRIVE_LOGISTICS);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('arrivelogistics-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new ArriveLogisticsService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.ARRIVE_LOGISTICS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new ArriveLogisticsService();
      const result = await service.scrape({
        siteType: [Site.ARRIVE_LOGISTICS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
