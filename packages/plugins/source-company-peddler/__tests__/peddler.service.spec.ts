...
import { PeddlerModule, PeddlerService } from '../src';

const COMPANY_NAME_EXPECT = 'Peddler';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'peddler-jobs.json'), 'utf8'),
);
...
describe('PeddlerService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through PeddlerModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [PeddlerModule],
      }).compile();
      const service = moduleRef.get(PeddlerService);
      expect(service).toBeInstanceOf(PeddlerService);
      await moduleRef.close();
    });

    it('exports the Site.PEDDLER = "peddler" enum value', () => {
      expect(Site.PEDDLER).toBe('peddler');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new PeddlerService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.PEDDLER],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('peddler.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.PEDDLER);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('peddler-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new PeddlerService(registry);
      const result = await service.scrape({
        siteType: [Site.PEDDLER],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('peddler');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('peddler-x1');
      expect(result.jobs[0].site).toBe(Site.PEDDLER);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('peddler-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new PeddlerService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.PEDDLER],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new PeddlerService();
      const result = await service.scrape({
        siteType: [Site.PEDDLER],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
