...
import { AbbVieModule, AbbVieService } from '../src';

const COMPANY_NAME_EXPECT = 'AbbVie';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'abbvie-jobs.json'), 'utf8'),
);
...
describe('AbbVieService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AbbVieModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AbbVieModule],
      }).compile();
      const service = moduleRef.get(AbbVieService);
      expect(service).toBeInstanceOf(AbbVieService);
      await moduleRef.close();
    });

    it('exports the Site.ABBVIE = "abbvie" enum value', () => {
      expect(Site.ABBVIE).toBe('abbvie');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new AbbVieService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.ABBVIE],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('AbbVie');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ABBVIE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('abbvie-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new AbbVieService(registry);
      const result = await service.scrape({
        siteType: [Site.ABBVIE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('AbbVie');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('abbvie-x1');
      expect(result.jobs[0].site).toBe(Site.ABBVIE);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('abbvie-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new AbbVieService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.ABBVIE],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new AbbVieService();
      const result = await service.scrape({
        siteType: [Site.ABBVIE],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
