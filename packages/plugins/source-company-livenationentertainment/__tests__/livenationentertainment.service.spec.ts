...
import { LiveNationEntertainmentModule, LiveNationEntertainmentService } from '../src';

const COMPANY_NAME_EXPECT = 'Live Nation Entertainment';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'livenationentertainment-jobs.json'), 'utf8'),
);
...
describe('LiveNationEntertainmentService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through LiveNationEntertainmentModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [LiveNationEntertainmentModule],
      }).compile();
      const service = moduleRef.get(LiveNationEntertainmentService);
      expect(service).toBeInstanceOf(LiveNationEntertainmentService);
      await moduleRef.close();
    });

    it('exports the Site.LIVE_NATION_ENTERTAINMENT = "livenationentertainment" enum value', () => {
      expect(Site.LIVE_NATION_ENTERTAINMENT).toBe('livenationentertainment');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new LiveNationEntertainmentService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.LIVE_NATION_ENTERTAINMENT],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'livenationentertainment-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.LIVE_NATION_ENTERTAINMENT);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('livenationentertainment-' + first.id);
      ...
      expect(calledUrls[0]).toContain('LiveNationEntertainment');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.LIVE_NATION_ENTERTAINMENT);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('livenationentertainment-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new LiveNationEntertainmentService(registry);
      const result = await service.scrape({
        siteType: [Site.LIVE_NATION_ENTERTAINMENT],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('LiveNationEntertainment');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('livenationentertainment-x1');
      expect(result.jobs[0].site).toBe(Site.LIVE_NATION_ENTERTAINMENT);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('livenationentertainment-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new LiveNationEntertainmentService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.LIVE_NATION_ENTERTAINMENT],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new LiveNationEntertainmentService();
      const result = await service.scrape({
        siteType: [Site.LIVE_NATION_ENTERTAINMENT],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
