...
import { RelayRoboticsModule, RelayRoboticsService } from '../src';

const COMPANY_NAME_EXPECT = 'Relay Robotics';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'relayroboticscom-jobs.json'), 'utf8'),
);
...
describe('RelayRoboticsService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through RelayRoboticsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [RelayRoboticsModule],
      }).compile();
      const service = moduleRef.get(RelayRoboticsService);
      expect(service).toBeInstanceOf(RelayRoboticsService);
      await moduleRef.close();
    });

    it('exports the Site.RELAY_ROBOTICS = "relayroboticscom" enum value', () => {
      expect(Site.RELAY_ROBOTICS).toBe('relayroboticscom');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new RelayRoboticsService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.RELAY_ROBOTICS],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'relayroboticscom-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.RELAY_ROBOTICS);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('relayroboticscom-' + first.id);
      ...
      expect(calledUrls[0]).toContain('relayrobotics.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.RELAY_ROBOTICS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('relayroboticscom-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new RelayRoboticsService(registry);
      const result = await service.scrape({
        siteType: [Site.RELAY_ROBOTICS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('relayrobotics.com');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('relayroboticscom-x1');
      expect(result.jobs[0].site).toBe(Site.RELAY_ROBOTICS);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('relayroboticscom-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new RelayRoboticsService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.RELAY_ROBOTICS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new RelayRoboticsService();
      const result = await service.scrape({
        siteType: [Site.RELAY_ROBOTICS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
