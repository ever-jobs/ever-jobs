...
import { AllcareersModule, AllcareersService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'allcareers-jobs.json'), 'utf8'),
);
...
/**
 * Spec 232 / T04 — `AllcareersService` unit tests (≥ 9 cases).
 */
describe('AllcareersService — Spec 232 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AllcareersModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AllcareersModule],
      }).compile();
      const service = moduleRef.get(AllcareersService);
      expect(service).toBeInstanceOf(AllcareersService);
      await moduleRef.close();
    });

    it('exports the Site.ALLCAREERS = "allcareers" enum value', () => {
      expect(Site.ALLCAREERS).toBe('allcareers');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AllcareersService();
      ...
      const job0 = dto.jobs.find((j) => j.id === 'allcareers-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ALLCAREERS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Cortica - Neurodevelopmental');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/allcareers/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/allcareers/jobs/');
      ...
      expect(job0?.description).toContain('Cortica - Neurodevelopmental');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/allcareers/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Cortica - Neurodevelopmental');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'allcareers-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
