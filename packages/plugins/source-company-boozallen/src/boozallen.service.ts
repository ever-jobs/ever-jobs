...
/**
 * Booz Allen Hamilton — Consulting and defense technology (HQ: McLean, VA, USA).
 *
 * Source (Spec 1736): Workday board, scraped in this order:
 *   - `bah:1:BAH_Jobs` — https://bah.wd1.myworkdayjobs.com/BAH_Jobs
 ...
 * through untouched.
 *
 * Tags: segment=workday-enterprise; industry=consulting-and-defense-technology.
 */
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'bah:1:BAH_Jobs', atsIdPrefix: 'wd-bah-' },
];

@SourcePlugin({
  site: Site.BOOZ_ALLEN,
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['boozallen.com'],
  description: 'Booz Allen Hamilton careers via Workday. Tags: segment=workday-enterprise; industry=consulting-and-defense-technology.',
})
@Injectable()
export class BoozAllenService implements IScraper {
  private readonly logger = new Logger(BoozAllenService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Workday source plugin is not registered; cannot scrape Booz Allen Hamilton');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`Booz Allen Hamilton: delegating to Workday (${board.companySlug})`);
    ...
    this.logger.log(`Booz Allen Hamilton: scraped ${jobs.length} jobs`);
