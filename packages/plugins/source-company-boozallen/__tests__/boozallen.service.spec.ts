...
import { BoozAllenModule, BoozAllenService } from '../src';
...
const FIXTURE: Fixture = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'fixtures', 'boozallen-boards.json'), 'utf8'),
);
const COMPANY_NAME = 'Booz Allen Hamilton';
const ID_PREFIX = 'boozallen-';
...
describe('BoozAllenService — Workday delegation (Spec 1736)', () => {
  beforeEach(() => {
    mockGet.mockReset();
    mockPost.mockReset();
  ...
  describe('registration', () => {
    it('resolves through BoozAllenModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({ imports: [BoozAllenModule] }).compile();
      expect(moduleRef.get(BoozAllenService)).toBeInstanceOf(BoozAllenService);
      await moduleRef.close();
    });

    it('exports Site.BOOZ_ALLEN = "boozallen"', () => {
      expect(Site.BOOZ_ALLEN).toBe('boozallen');
    });

    it('declares a tagged company plugin with its domains', () => {
      const meta = Reflect.getMetadata(SOURCE_PLUGIN_METADATA, BoozAllenService);
      expect(meta.site).toBe(Site.BOOZ_ALLEN);
      expect(meta.category).toBe('company');
      expect(meta.isAts).toBeFalsy();
      expect(meta.companyDomains).toEqual(['boozallen.com']);
  ...
  describe('recorded board (real Workday adapter, mocked HTTP)', () => {
    it('maps every recorded posting and re-stamps the company identity', async () => {
      const service = new BoozAllenService(registryWith());
      const result = await service.scrape({ siteType: [Site.BOOZ_ALLEN], resultsWanted: 100 } as ScraperInputDto);
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.BOOZ_ALLEN);
    ...
    it('only requests the recorded board URLs', async () => {
      const service = new BoozAllenService(registryWith());
      await service.scrape({ siteType: [Site.BOOZ_ALLEN] } as ScraperInputDto);
    ...
    it('honours resultsWanted=1', async () => {
      const service = new BoozAllenService(registryWith());
      const result = await service.scrape({ siteType: [Site.BOOZ_ALLEN], resultsWanted: 1 } as ScraperInputDto);
    ...
    it('resolves with an empty result when the board is gone (HTTP 404)', async () => {
      ...
      const service = new BoozAllenService(registryWith());
      const result = await service.scrape({ siteType: [Site.BOOZ_ALLEN] } as ScraperInputDto);
  ...
  describe('delegation contract', () => {
    it('forwards the board and the caller input untouched', async () => {
      ...
      const service = new BoozAllenService(
        registryWith(
          fakeBackend(
      ...
      expect(result.jobs[0].site).toBe(Site.BOOZ_ALLEN);
    ...
    it('never forwards the caller\'s credentials to the board (Spec 1735 §4.5)', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new BoozAllenService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
    ...
    it('makes no request when resultsWanted is 0', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new BoozAllenService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      const result = await service.scrape({ siteType: [Site.BOOZ_ALLEN], resultsWanted: 0 } as ScraperInputDto);
  ...
  describe('resilience', () => {
    it('reports not_registered when no registry is injected', async () => {
      const result = await new BoozAllenService().scrape({ siteType: [Site.BOOZ_ALLEN] } as ScraperInputDto);
      expect(result.jobs).toEqual([]);
    ...
    it('reports not_registered when Workday is missing from the registry', async () => {
      const result = await new BoozAllenService(new PluginRegistry()).scrape({
        siteType: [Site.BOOZ_ALLEN],
      } as ScraperInputDto);
      expect(result.diagnostics?.reason).toBe('not_registered');
    });
