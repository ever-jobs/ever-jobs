...
import { VertiGISModule, VertiGISService } from '../src';

const COMPANY_NAME_EXPECT = 'VertiGIS';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'vertigis-jobs.json'), 'utf8'),
);
...
describe('VertiGISService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through VertiGISModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [VertiGISModule],
      }).compile();
      const service = moduleRef.get(VertiGISService);
      expect(service).toBeInstanceOf(VertiGISService);
      await moduleRef.close();
    });

    it('exports the Site.VERTIGIS = "vertigis" enum value', () => {
      expect(Site.VERTIGIS).toBe('vertigis');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new VertiGISService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.VERTIGIS],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('vertigis.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.VERTIGIS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('vertigis-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new VertiGISService(registry);
      const result = await service.scrape({
        siteType: [Site.VERTIGIS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('vertigis');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('vertigis-x1');
      expect(result.jobs[0].site).toBe(Site.VERTIGIS);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('vertigis-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new VertiGISService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.VERTIGIS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new VertiGISService();
      const result = await service.scrape({
        siteType: [Site.VERTIGIS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
