...
import { EthosLifeModule, EthosLifeService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'ethoslife-jobs.json'), 'utf8'),
);
...
/**
 * Spec 651 / T04 — `EthosLifeService` unit tests (≥ 9 cases).
 */
describe('EthosLifeService — Spec 651 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through EthosLifeModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [EthosLifeModule],
      }).compile();
      const service = moduleRef.get(EthosLifeService);
      expect(service).toBeInstanceOf(EthosLifeService);
      await moduleRef.close();
    });

    it('exports the Site.ETHOS_LIFE = "ethoslife" enum value', () => {
      expect(Site.ETHOS_LIFE).toBe('ethoslife');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new EthosLifeService();
      const result = await service.scrape({
        siteType: [Site.ETHOS_LIFE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'ethoslife-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ETHOS_LIFE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Ethos Life');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/ethoslife/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/ethoslife/jobs/');
      ...
      expect(job0?.description).toContain('Ethos Life');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/ethoslife/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Ethos Life');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'ethoslife-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
