...
import { ZuoraModule, ZuoraService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'zuora-jobs.json'), 'utf8'),
);
...
/**
 * Spec 803 / T04 — `ZuoraService` unit tests (≥ 9 cases).
 */
describe('ZuoraService — Spec 803 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ZuoraModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ZuoraModule],
      }).compile();
      const service = moduleRef.get(ZuoraService);
      expect(service).toBeInstanceOf(ZuoraService);
      await moduleRef.close();
    });

    it('exports the Site.ZUORA = "zuora" enum value', () => {
      expect(Site.ZUORA).toBe('zuora');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ZuoraService();
      ...
      const job0 = dto.jobs.find((j) => j.id === 'zuora-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ZUORA);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Zuora');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/zuora/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/zuora/jobs/');
      ...
      expect(job0?.description).toContain('Zuora');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/zuora/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Zuora');
      }
    });
  });
