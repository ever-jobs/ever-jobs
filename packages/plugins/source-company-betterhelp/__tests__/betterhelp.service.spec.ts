...
import { BetterHelpModule, BetterHelpService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'betterhelp-jobs.json'), 'utf8'),
);
...
/**
 * Spec 708 / T04 — `BetterHelpService` unit tests (≥ 9 cases).
 */
describe('BetterHelpService — Spec 708 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through BetterHelpModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [BetterHelpModule],
      }).compile();
      const service = moduleRef.get(BetterHelpService);
      expect(service).toBeInstanceOf(BetterHelpService);
      await moduleRef.close();
    });

    it('exports the Site.BETTERHELP = "betterhelp" enum value', () => {
      expect(Site.BETTERHELP).toBe('betterhelp');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new BetterHelpService();
      ...
      const job0 = dto.jobs.find((j) => j.id === 'betterhelp-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.BETTERHELP);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('BetterHelp');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/betterhelp/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/betterhelp/jobs/');
      ...
      expect(job0?.description).toContain('BetterHelp');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/betterhelp/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('BetterHelp');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'betterhelp-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
