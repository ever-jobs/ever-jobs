...
import { QuipModule, QuipService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'quip-jobs.json'), 'utf8'),
);
...
/**
 * Spec 660 / T04 — `QuipService` unit tests (≥ 9 cases).
 */
describe('QuipService — Spec 660 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through QuipModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [QuipModule],
      }).compile();
      const service = moduleRef.get(QuipService);
      expect(service).toBeInstanceOf(QuipService);
      await moduleRef.close();
    });

    it('exports the Site.QUIP = "quip" enum value', () => {
      expect(Site.QUIP).toBe('quip');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const job0 = dto.jobs.find((j) => j.id === 'quip-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.QUIP);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('quip');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/quip/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/quip/jobs/');
      ...
      expect(job0?.description).toContain('quip');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/quip/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('quip');
      }
    });
  });
