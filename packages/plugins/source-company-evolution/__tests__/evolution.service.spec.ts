...
import { EvolutionModule, EvolutionService } from '../src';

const COMPANY_NAME_EXPECT = 'Evolution';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'evolution-jobs.json'), 'utf8'),
);
...
describe('EvolutionService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through EvolutionModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [EvolutionModule],
      }).compile();
      const service = moduleRef.get(EvolutionService);
      expect(service).toBeInstanceOf(EvolutionService);
      await moduleRef.close();
    });

    it('exports the Site.EVOLUTION = "evolution" enum value', () => {
      expect(Site.EVOLUTION).toBe('evolution');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new EvolutionService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.EVOLUTION],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('Evolution');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.EVOLUTION);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('evolution-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new EvolutionService(registry);
      const result = await service.scrape({
        siteType: [Site.EVOLUTION],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('Evolution');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('evolution-x1');
      expect(result.jobs[0].site).toBe(Site.EVOLUTION);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('evolution-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new EvolutionService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.EVOLUTION],
      } as ScraperInputDto);
