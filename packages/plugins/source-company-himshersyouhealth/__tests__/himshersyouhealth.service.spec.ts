...
import { HimsHersYouHealthModule, HimsHersYouHealthService } from '../src';

const COMPANY_NAME_EXPECT = 'Hims & Hers (You Health)';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'himshersyouhealth-jobs.json'), 'utf8'),
);
...
describe('HimsHersYouHealthService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through HimsHersYouHealthModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [HimsHersYouHealthModule],
      }).compile();
      const service = moduleRef.get(HimsHersYouHealthService);
      expect(service).toBeInstanceOf(HimsHersYouHealthService);
      await moduleRef.close();
    });

    it('exports the Site.HIMS_HERS_YOU_HEALTH = "himshersyouhealth" enum value', () => {
      expect(Site.HIMS_HERS_YOU_HEALTH).toBe('himshersyouhealth');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new HimsHersYouHealthService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.HIMS_HERS_YOU_HEALTH],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'himshersyouhealth-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.HIMS_HERS_YOU_HEALTH);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('himshersyouhealth-' + first.id);
      ...
      expect(calledUrls[0]).toContain('you-health');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.HIMS_HERS_YOU_HEALTH);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('himshersyouhealth-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new HimsHersYouHealthService(registry);
      const result = await service.scrape({
        siteType: [Site.HIMS_HERS_YOU_HEALTH],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('you-health');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('himshersyouhealth-x1');
      expect(result.jobs[0].site).toBe(Site.HIMS_HERS_YOU_HEALTH);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('himshersyouhealth-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new HimsHersYouHealthService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.HIMS_HERS_YOU_HEALTH],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new HimsHersYouHealthService();
      const result = await service.scrape({
        siteType: [Site.HIMS_HERS_YOU_HEALTH],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
