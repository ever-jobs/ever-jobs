...
import { XcimerEnergyModule, XcimerEnergyService } from '../src';

const COMPANY_NAME_EXPECT = 'Xcimer Energy';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'xcimer-jobs.json'), 'utf8'),
);
...
describe('XcimerEnergyService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through XcimerEnergyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [XcimerEnergyModule],
      }).compile();
      const service = moduleRef.get(XcimerEnergyService);
      expect(service).toBeInstanceOf(XcimerEnergyService);
      await moduleRef.close();
    });

    it('exports the Site.XCIMER_ENERGY = "xcimer" enum value', () => {
      expect(Site.XCIMER_ENERGY).toBe('xcimer');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new XcimerEnergyService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.XCIMER_ENERGY],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('xcimer');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.XCIMER_ENERGY);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('xcimer-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new XcimerEnergyService(registry);
      const result = await service.scrape({
        siteType: [Site.XCIMER_ENERGY],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('xcimer');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('xcimer-x1');
      expect(result.jobs[0].site).toBe(Site.XCIMER_ENERGY);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('xcimer-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new XcimerEnergyService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.XCIMER_ENERGY],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new XcimerEnergyService();
      const result = await service.scrape({
        siteType: [Site.XCIMER_ENERGY],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
