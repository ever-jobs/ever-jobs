...
import { AninebingModule, AninebingService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'aninebing-jobs.json'), 'utf8'),
);
...
/**
 * Spec 270 / T04 — `AninebingService` unit tests (≥ 9 cases).
 */
describe('AninebingService — Spec 270 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AninebingModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AninebingModule],
      }).compile();
      const service = moduleRef.get(AninebingService);
      expect(service).toBeInstanceOf(AninebingService);
      await moduleRef.close();
    });

    it('exports the Site.ANINEBING = "aninebing" enum value', () => {
      expect(Site.ANINEBING).toBe('aninebing');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AninebingService();
      const result = await service.scrape({
        siteType: [Site.ANINEBING],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'aninebing-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ANINEBING);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('ANINE BING');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/aninebing/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/aninebing/jobs/');
      ...
      expect(job0?.description).toContain('ANINE BING');
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('ANINE BING');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'aninebing-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
