...
import { MicronModule, MicronService } from '../src';
...
const FIXTURE: Fixture = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'fixtures', 'micron-boards.json'), 'utf8'),
);
const COMPANY_NAME = 'Micron Technology';
const ID_PREFIX = 'micron-';
...
describe('MicronService — Workday delegation (Spec 1736)', () => {
  beforeEach(() => {
    mockGet.mockReset();
    mockPost.mockReset();
  ...
  describe('registration', () => {
    it('resolves through MicronModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({ imports: [MicronModule] }).compile();
      expect(moduleRef.get(MicronService)).toBeInstanceOf(MicronService);
      await moduleRef.close();
    });

    it('exports Site.MICRON = "micron"', () => {
      expect(Site.MICRON).toBe('micron');
    });

    it('declares a tagged company plugin with its domains', () => {
      const meta = Reflect.getMetadata(SOURCE_PLUGIN_METADATA, MicronService);
      expect(meta.site).toBe(Site.MICRON);
      expect(meta.category).toBe('company');
      expect(meta.isAts).toBeFalsy();
      expect(meta.companyDomains).toEqual(['micron.com']);
  ...
  describe('recorded board (real Workday adapter, mocked HTTP)', () => {
    it('maps every recorded posting and re-stamps the company identity', async () => {
      const service = new MicronService(registryWith());
      const result = await service.scrape({ siteType: [Site.MICRON], resultsWanted: 100 } as ScraperInputDto);
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.MICRON);
    ...
    it('only requests the recorded board URLs', async () => {
      const service = new MicronService(registryWith());
      await service.scrape({ siteType: [Site.MICRON] } as ScraperInputDto);
    ...
    it('honours resultsWanted=1', async () => {
      const service = new MicronService(registryWith());
      const result = await service.scrape({ siteType: [Site.MICRON], resultsWanted: 1 } as ScraperInputDto);
    ...
    it('resolves with an empty result when the board is gone (HTTP 404)', async () => {
      ...
      const service = new MicronService(registryWith());
      const result = await service.scrape({ siteType: [Site.MICRON] } as ScraperInputDto);
  ...
  describe('delegation contract', () => {
    it('forwards the board and the caller input untouched', async () => {
      ...
      const service = new MicronService(
        registryWith(
          fakeBackend(
      ...
      expect(result.jobs[0].site).toBe(Site.MICRON);
    ...
    it('never forwards the caller\'s credentials to the board (Spec 1735 §4.5)', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new MicronService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
    ...
    it('makes no request when resultsWanted is 0', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new MicronService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      const result = await service.scrape({ siteType: [Site.MICRON], resultsWanted: 0 } as ScraperInputDto);
  ...
  describe('resilience', () => {
    it('reports not_registered when no registry is injected', async () => {
      const result = await new MicronService().scrape({ siteType: [Site.MICRON] } as ScraperInputDto);
      expect(result.jobs).toEqual([]);
    ...
    it('reports not_registered when Workday is missing from the registry', async () => {
      const result = await new MicronService(new PluginRegistry()).scrape({
        siteType: [Site.MICRON],
      } as ScraperInputDto);
      expect(result.diagnostics?.reason).toBe('not_registered');
    });
