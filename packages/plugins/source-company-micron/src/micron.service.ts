...
/**
 * Micron Technology — Semiconductors (HQ: Boise, ID, USA).
 *
 * Source (Spec 1736): Workday board, scraped in this order:
 *   - `micron:1:External` — https://micron.wd1.myworkdayjobs.com/External
 *     verified live 2026-09-24: 3,072 open postings.
 *
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'micron:1:External', atsIdPrefix: 'wd-micron-' },
];
...
@SourcePlugin({
  ...
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['micron.com'],
  description: 'Micron Technology careers via Workday. Tags: segment=workday-enterprise; industry=semiconductors.',
})
@Injectable()
export class MicronService implements IScraper {
  private readonly logger = new Logger(MicronService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Workday source plugin is not registered; cannot scrape Micron Technology');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`Micron Technology: delegating to Workday (${board.companySlug})`);
    ...
    this.logger.log(`Micron Technology: scraped ${jobs.length} jobs`);
