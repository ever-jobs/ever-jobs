...
import { LoopModule, LoopService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'loop-jobs.json'), 'utf8'),
);
...
/**
 * Spec 628 / T04 — `LoopService` unit tests (≥ 9 cases).
 */
describe('LoopService — Spec 628 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through LoopModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [LoopModule],
      }).compile();
      const service = moduleRef.get(LoopService);
      expect(service).toBeInstanceOf(LoopService);
      await moduleRef.close();
    });

    it('exports the Site.LOOP = "loop" enum value', () => {
      expect(Site.LOOP).toBe('loop');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const job0 = dto.jobs.find((j) => j.id === 'loop-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.LOOP);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Loop');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/loop/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/loop/jobs/');
      ...
      expect(job0?.description).toContain('Loop');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/loop/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Loop');
      }
    });
  });
