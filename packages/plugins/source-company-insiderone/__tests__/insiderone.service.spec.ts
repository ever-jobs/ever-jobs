...
import { InsiderModule, InsiderService } from '../src';

const COMPANY_NAME_EXPECT = 'Insider';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'insiderone-jobs.json'), 'utf8'),
);
...
describe('InsiderService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through InsiderModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [InsiderModule],
      }).compile();
      const service = moduleRef.get(InsiderService);
      expect(service).toBeInstanceOf(InsiderService);
      await moduleRef.close();
    });

    it('exports the Site.INSIDER = "insiderone" enum value', () => {
      expect(Site.INSIDER).toBe('insiderone');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new InsiderService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.INSIDER],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('insiderone');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.INSIDER);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('insiderone-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new InsiderService(registry);
      const result = await service.scrape({
        siteType: [Site.INSIDER],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('insiderone');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('insiderone-x1');
      expect(result.jobs[0].site).toBe(Site.INSIDER);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('insiderone-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new InsiderService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.INSIDER],
      } as ScraperInputDto);
