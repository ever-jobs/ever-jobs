...
import { SigNozModule, SigNozService } from '../src';

const COMPANY_NAME_EXPECT = 'SigNoz';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'signoz-jobs.json'), 'utf8'),
);
...
describe('SigNozService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SigNozModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SigNozModule],
      }).compile();
      const service = moduleRef.get(SigNozService);
      expect(service).toBeInstanceOf(SigNozService);
      await moduleRef.close();
    });

    it('exports the Site.SIGNOZ = "signoz" enum value', () => {
      expect(Site.SIGNOZ).toBe('signoz');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new SigNozService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.SIGNOZ],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('signoz');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SIGNOZ);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('signoz-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new SigNozService(registry);
      const result = await service.scrape({
        siteType: [Site.SIGNOZ],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('signoz');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('signoz-x1');
      expect(result.jobs[0].site).toBe(Site.SIGNOZ);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('signoz-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new SigNozService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.SIGNOZ],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new SigNozService();
      const result = await service.scrape({
        siteType: [Site.SIGNOZ],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
