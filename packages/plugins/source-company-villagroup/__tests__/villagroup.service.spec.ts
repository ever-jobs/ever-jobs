...
import { VillaGroupModule, VillaGroupService } from '../src';

const COMPANY_NAME_EXPECT = 'Villa Group';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'villagroup-jobs.json'), 'utf8'),
);
...
describe('VillaGroupService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through VillaGroupModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [VillaGroupModule],
      }).compile();
      const service = moduleRef.get(VillaGroupService);
      expect(service).toBeInstanceOf(VillaGroupService);
      await moduleRef.close();
    });

    it('exports the Site.VILLA_GROUP = "villagroup" enum value', () => {
      expect(Site.VILLA_GROUP).toBe('villagroup');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new VillaGroupService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.VILLA_GROUP],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('VillaGroup');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.VILLA_GROUP);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('villagroup-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new VillaGroupService(registry);
      const result = await service.scrape({
        siteType: [Site.VILLA_GROUP],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('VillaGroup');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('villagroup-x1');
      expect(result.jobs[0].site).toBe(Site.VILLA_GROUP);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('villagroup-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new VillaGroupService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.VILLA_GROUP],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new VillaGroupService();
      const result = await service.scrape({
        siteType: [Site.VILLA_GROUP],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
