...
import { PlusPowerModule, PlusPowerService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'pluspower-jobs.json'), 'utf8'),
);
...
/**
 * Spec 912 / T04 — `PlusPowerService` unit tests (≥ 9 cases).
 */
describe('PlusPowerService — Spec 912 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through PlusPowerModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [PlusPowerModule],
      }).compile();
      const service = moduleRef.get(PlusPowerService);
      expect(service).toBeInstanceOf(PlusPowerService);
      await moduleRef.close();
    });

    it('exports the Site.PLUS_POWER = "pluspower" enum value', () => {
      expect(Site.PLUS_POWER).toBe('pluspower');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new PlusPowerService();
      const result = await service.scrape({
        siteType: [Site.PLUS_POWER],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'pluspower-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.PLUS_POWER);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Plus Power');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/pluspower/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/pluspower/jobs/');
      ...
      expect(job0?.description).toContain('Plus Power');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/pluspower/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Plus Power');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'pluspower-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
