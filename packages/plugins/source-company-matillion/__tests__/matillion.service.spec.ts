...
import { MatillionModule, MatillionService } from '../src';

const COMPANY_NAME_EXPECT = 'Matillion';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'matillion-jobs.json'), 'utf8'),
);
...
describe('MatillionService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through MatillionModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [MatillionModule],
      }).compile();
      const service = moduleRef.get(MatillionService);
      expect(service).toBeInstanceOf(MatillionService);
      await moduleRef.close();
    });

    it('exports the Site.MATILLION = "matillion" enum value', () => {
      expect(Site.MATILLION).toBe('matillion');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new MatillionService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.MATILLION],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('matillion');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.MATILLION);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('matillion-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new MatillionService(registry);
      const result = await service.scrape({
        siteType: [Site.MATILLION],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('matillion');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('matillion-x1');
      expect(result.jobs[0].site).toBe(Site.MATILLION);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('matillion-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new MatillionService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.MATILLION],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new MatillionService();
      const result = await service.scrape({
        siteType: [Site.MATILLION],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
