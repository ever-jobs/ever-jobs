...
import { AnsaModule, AnsaService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'ansa-jobs.json'), 'utf8'),
);
...
/**
 * Spec 271 / T04 — `AnsaService` unit tests (≥ 9 cases).
 */
describe('AnsaService — Spec 271 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AnsaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AnsaModule],
      }).compile();
      const service = moduleRef.get(AnsaService);
      expect(service).toBeInstanceOf(AnsaService);
      await moduleRef.close();
    });

    it('exports the Site.ANSA = "ansa" enum value', () => {
      expect(Site.ANSA).toBe('ansa');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const job0 = dto.jobs.find((j) => j.id === 'ansa-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ANSA);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Ansa');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/ansa/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/ansa/jobs/');
      ...
      expect(job0?.description).toContain('Ansa');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/ansa/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Ansa');
      }
    });
  });
