...
import { QuantumMetricModule, QuantumMetricService } from '../src';

const COMPANY_NAME_EXPECT = 'Quantum Metric';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'quantummetric-jobs.json'), 'utf8'),
);
...
describe('QuantumMetricService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through QuantumMetricModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [QuantumMetricModule],
      }).compile();
      const service = moduleRef.get(QuantumMetricService);
      expect(service).toBeInstanceOf(QuantumMetricService);
      await moduleRef.close();
    });

    it('exports the Site.QUANTUM_METRIC = "quantummetric" enum value', () => {
      expect(Site.QUANTUM_METRIC).toBe('quantummetric');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new QuantumMetricService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.QUANTUM_METRIC],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'quantummetric-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.QUANTUM_METRIC);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('quantummetric-' + first.id);
      ...
      expect(calledUrls[0]).toContain('quantummetric');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.QUANTUM_METRIC);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('quantummetric-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new QuantumMetricService(registry);
      const result = await service.scrape({
        siteType: [Site.QUANTUM_METRIC],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('quantummetric');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('quantummetric-x1');
      expect(result.jobs[0].site).toBe(Site.QUANTUM_METRIC);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('quantummetric-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new QuantumMetricService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.QUANTUM_METRIC],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new QuantumMetricService();
      const result = await service.scrape({
        siteType: [Site.QUANTUM_METRIC],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
