...
/**
 * CVS Health — Healthcare (HQ: Woonsocket, RI, USA).
 *
 * Source (Spec 1736): Workday board, scraped in this order:
 *   - `cvshealth:1:CVS_Health_Careers` — https://cvshealth.wd1.myworkdayjobs.com/CVS_Health_Careers
 *     verified live 2026-09-24: 19,970 open postings.
 *
 ...
 * party board.
 *
 * Tags: segment=workday-enterprise; industry=healthcare.
 */
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'cvshealth:1:CVS_Health_Careers', atsIdPrefix: 'wd-cvshealth-' },
];

@SourcePlugin({
  site: Site.CVS_HEALTH,
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['cvshealth.com'],
  description: 'CVS Health careers via Workday. Tags: segment=workday-enterprise; industry=healthcare.',
})
@Injectable()
export class CvsHealthService implements IScraper {
  private readonly logger = new Logger(CvsHealthService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Workday source plugin is not registered; cannot scrape CVS Health');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`CVS Health: delegating to Workday (${board.companySlug})`);
    ...
    this.logger.log(`CVS Health: scraped ${jobs.length} jobs`);
