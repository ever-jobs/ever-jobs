...
import { CvsHealthModule, CvsHealthService } from '../src';
...
const FIXTURE: Fixture = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'fixtures', 'cvshealth-boards.json'), 'utf8'),
);
const COMPANY_NAME = 'CVS Health';
const ID_PREFIX = 'cvshealth-';
...
describe('CvsHealthService — Workday delegation (Spec 1736)', () => {
  beforeEach(() => {
    mockGet.mockReset();
    mockPost.mockReset();
  ...
  describe('registration', () => {
    it('resolves through CvsHealthModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({ imports: [CvsHealthModule] }).compile();
      expect(moduleRef.get(CvsHealthService)).toBeInstanceOf(CvsHealthService);
      await moduleRef.close();
    });

    it('exports Site.CVS_HEALTH = "cvshealth"', () => {
      expect(Site.CVS_HEALTH).toBe('cvshealth');
    });

    it('declares a tagged company plugin with its domains', () => {
      const meta = Reflect.getMetadata(SOURCE_PLUGIN_METADATA, CvsHealthService);
      expect(meta.site).toBe(Site.CVS_HEALTH);
      expect(meta.category).toBe('company');
      expect(meta.isAts).toBeFalsy();
      expect(meta.companyDomains).toEqual(['cvshealth.com']);
  ...
  describe('recorded board (real Workday adapter, mocked HTTP)', () => {
    it('maps every recorded posting and re-stamps the company identity', async () => {
      const service = new CvsHealthService(registryWith());
      const result = await service.scrape({ siteType: [Site.CVS_HEALTH], resultsWanted: 100 } as ScraperInputDto);
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.CVS_HEALTH);
    ...
    it('only requests the recorded board URLs', async () => {
      const service = new CvsHealthService(registryWith());
      await service.scrape({ siteType: [Site.CVS_HEALTH] } as ScraperInputDto);
    ...
    it('honours resultsWanted=1', async () => {
      const service = new CvsHealthService(registryWith());
      const result = await service.scrape({ siteType: [Site.CVS_HEALTH], resultsWanted: 1 } as ScraperInputDto);
    ...
    it('resolves with an empty result when the board is gone (HTTP 404)', async () => {
      ...
      const service = new CvsHealthService(registryWith());
      const result = await service.scrape({ siteType: [Site.CVS_HEALTH] } as ScraperInputDto);
  ...
  describe('delegation contract', () => {
    it('forwards the board and the caller input untouched', async () => {
      ...
      const service = new CvsHealthService(
        registryWith(
          fakeBackend(
      ...
      expect(result.jobs[0].site).toBe(Site.CVS_HEALTH);
      expect(result.jobs[0].companyName).toBe(COMPANY_NAME);
    });

    it('passes an absent resultsWanted through as absent', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new CvsHealthService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      await service.scrape({ siteType: [Site.CVS_HEALTH] } as ScraperInputDto);
    ...
    it('makes no request when resultsWanted is 0', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new CvsHealthService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      const result = await service.scrape({ siteType: [Site.CVS_HEALTH], resultsWanted: 0 } as ScraperInputDto);
  ...
  describe('resilience', () => {
    it('reports not_registered when no registry is injected', async () => {
      const result = await new CvsHealthService().scrape({ siteType: [Site.CVS_HEALTH] } as ScraperInputDto);
      expect(result.jobs).toEqual([]);
    ...
    it('reports not_registered when Workday is missing from the registry', async () => {
      const result = await new CvsHealthService(new PluginRegistry()).scrape({
        siteType: [Site.CVS_HEALTH],
      } as ScraperInputDto);
      expect(result.diagnostics?.reason).toBe('not_registered');
    });
