...
import { PowerGenRenewableEnergyModule, PowerGenRenewableEnergyService } from '../src';

const COMPANY_NAME_EXPECT = 'PowerGen Renewable Energy';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'powergenrenewableenergy-jobs.json'), 'utf8'),
);
...
describe('PowerGenRenewableEnergyService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through PowerGenRenewableEnergyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [PowerGenRenewableEnergyModule],
      }).compile();
      const service = moduleRef.get(PowerGenRenewableEnergyService);
      expect(service).toBeInstanceOf(PowerGenRenewableEnergyService);
      await moduleRef.close();
    });

    it('exports the Site.POWERGEN_RENEWABLE_ENERGY = "powergenrenewableenergy" enum value', () => {
      expect(Site.POWERGEN_RENEWABLE_ENERGY).toBe('powergenrenewableenergy');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new PowerGenRenewableEnergyService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.POWERGEN_RENEWABLE_ENERGY],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'powergenrenewableenergy-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.POWERGEN_RENEWABLE_ENERGY);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('powergenrenewableenergy-' + first.id);
      ...
      expect(calledUrls[0]).toContain('PowerGenRenewableEnergy');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.POWERGEN_RENEWABLE_ENERGY);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('powergenrenewableenergy-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new PowerGenRenewableEnergyService(registry);
      const result = await service.scrape({
        siteType: [Site.POWERGEN_RENEWABLE_ENERGY],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('PowerGenRenewableEnergy');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('powergenrenewableenergy-x1');
      expect(result.jobs[0].site).toBe(Site.POWERGEN_RENEWABLE_ENERGY);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('powergenrenewableenergy-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new PowerGenRenewableEnergyService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.POWERGEN_RENEWABLE_ENERGY],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new PowerGenRenewableEnergyService();
      const result = await service.scrape({
        siteType: [Site.POWERGEN_RENEWABLE_ENERGY],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
