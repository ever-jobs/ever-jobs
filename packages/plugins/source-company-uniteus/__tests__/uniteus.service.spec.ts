...
import { UniteUsModule, UniteUsService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'uniteus-jobs.json'), 'utf8'),
);
...
/**
 * Spec 680 / T04 — `UniteUsService` unit tests (≥ 9 cases).
 */
describe('UniteUsService — Spec 680 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through UniteUsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [UniteUsModule],
      }).compile();
      const service = moduleRef.get(UniteUsService);
      expect(service).toBeInstanceOf(UniteUsService);
      await moduleRef.close();
    });

    it('exports the Site.UNITE_US = "uniteus" enum value', () => {
      expect(Site.UNITE_US).toBe('uniteus');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new UniteUsService();
      const result = await service.scrape({
        siteType: [Site.UNITE_US],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'uniteus-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.UNITE_US);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Unite Us');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/uniteus/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/uniteus/jobs/');
      ...
      expect(job0?.description).toContain('Unite Us');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/uniteus/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Unite Us');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'uniteus-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
