...
import { SmithsGroupModule, SmithsGroupService } from '../src';

const COMPANY_NAME_EXPECT = 'Smiths Group';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'smithsgroup-jobs.json'), 'utf8'),
);
...
describe('SmithsGroupService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SmithsGroupModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SmithsGroupModule],
      }).compile();
      const service = moduleRef.get(SmithsGroupService);
      expect(service).toBeInstanceOf(SmithsGroupService);
      await moduleRef.close();
    });

    it('exports the Site.SMITHS_GROUP = "smithsgroup" enum value', () => {
      expect(Site.SMITHS_GROUP).toBe('smithsgroup');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new SmithsGroupService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.SMITHS_GROUP],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('SmithsGroup2');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SMITHS_GROUP);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('smithsgroup-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new SmithsGroupService(registry);
      const result = await service.scrape({
        siteType: [Site.SMITHS_GROUP],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('SmithsGroup2');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('smithsgroup-x1');
      expect(result.jobs[0].site).toBe(Site.SMITHS_GROUP);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('smithsgroup-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new SmithsGroupService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.SMITHS_GROUP],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new SmithsGroupService();
      const result = await service.scrape({
        siteType: [Site.SMITHS_GROUP],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
