...
import { FreenomeModule, FreenomeService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'freenome-jobs.json'), 'utf8'),
);
...
/**
 * Spec 528 / T04 — `FreenomeService` unit tests (≥ 9 cases).
 */
describe('FreenomeService — Spec 528 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through FreenomeModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [FreenomeModule],
      }).compile();
      const service = moduleRef.get(FreenomeService);
      expect(service).toBeInstanceOf(FreenomeService);
      await moduleRef.close();
    });

    it('exports the Site.FREENOME = "freenome" enum value', () => {
      expect(Site.FREENOME).toBe('freenome');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new FreenomeService();
      const result = await service.scrape({
        siteType: [Site.FREENOME],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'freenome-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.FREENOME);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Freenome');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/freenome/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/freenome/jobs/');
      ...
      expect(job0?.description).toContain('Freenome');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/freenome/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Freenome');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'freenome-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
