...
import { SparkfundModule, SparkfundService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'sparkfund-jobs.json'), 'utf8'),
);
...
/**
 * Spec 633 / T04 — `SparkfundService` unit tests (≥ 9 cases).
 */
describe('SparkfundService — Spec 633 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SparkfundModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SparkfundModule],
      }).compile();
      const service = moduleRef.get(SparkfundService);
      expect(service).toBeInstanceOf(SparkfundService);
      await moduleRef.close();
    });

    it('exports the Site.SPARKFUND = "sparkfund" enum value', () => {
      expect(Site.SPARKFUND).toBe('sparkfund');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new SparkfundService();
      const result = await service.scrape({
        siteType: [Site.SPARKFUND],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'sparkfund-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.SPARKFUND);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Sparkfund');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/sparkfund/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/sparkfund/jobs/');
      ...
      expect(job0?.description).toContain('Sparkfund');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/sparkfund/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Sparkfund');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'sparkfund-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
