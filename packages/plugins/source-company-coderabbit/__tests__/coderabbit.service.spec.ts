...
import { CodeRabbitModule, CodeRabbitService } from '../src';

const COMPANY_NAME_EXPECT = 'CodeRabbit';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'coderabbit-jobs.json'), 'utf8'),
);
...
describe('CodeRabbitService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CodeRabbitModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CodeRabbitModule],
      }).compile();
      const service = moduleRef.get(CodeRabbitService);
      expect(service).toBeInstanceOf(CodeRabbitService);
      await moduleRef.close();
    });

    it('exports the Site.CODERABBIT = "coderabbit" enum value', () => {
      expect(Site.CODERABBIT).toBe('coderabbit');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new CodeRabbitService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.CODERABBIT],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('coderabbit');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.CODERABBIT);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('coderabbit-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new CodeRabbitService(registry);
      const result = await service.scrape({
        siteType: [Site.CODERABBIT],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('coderabbit');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('coderabbit-x1');
      expect(result.jobs[0].site).toBe(Site.CODERABBIT);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('coderabbit-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new CodeRabbitService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.CODERABBIT],
      } as ScraperInputDto);
