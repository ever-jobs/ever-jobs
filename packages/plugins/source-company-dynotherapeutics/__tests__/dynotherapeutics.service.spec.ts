...
import { DynoTherapeuticsModule, DynoTherapeuticsService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'dynotherapeutics-jobs.json'), 'utf8'),
);
...
/**
 * Spec 776 / T04 — `DynoTherapeuticsService` unit tests (≥ 9 cases).
 */
describe('DynoTherapeuticsService — Spec 776 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through DynoTherapeuticsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [DynoTherapeuticsModule],
      }).compile();
      const service = moduleRef.get(DynoTherapeuticsService);
      expect(service).toBeInstanceOf(DynoTherapeuticsService);
      await moduleRef.close();
    });

    it('exports the Site.DYNO_THERAPEUTICS = "dynotherapeutics" enum value', () => {
      expect(Site.DYNO_THERAPEUTICS).toBe('dynotherapeutics');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new DynoTherapeuticsService();
      const result = await service.scrape({
        siteType: [Site.DYNO_THERAPEUTICS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'dynotherapeutics-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.DYNO_THERAPEUTICS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Dyno Therapeutics');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/dynotherapeutics/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/dynotherapeutics/jobs/');
      ...
      expect(job0?.description).toContain('Dyno Therapeutics');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/dynotherapeutics/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Dyno Therapeutics');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'dynotherapeutics-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
