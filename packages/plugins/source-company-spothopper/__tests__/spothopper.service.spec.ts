...
import { SpotHopperModule, SpotHopperService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'spothopper-jobs.json'), 'utf8'),
);
...
/**
 * Spec 941 / T04 — `SpotHopperService` unit tests (≥ 9 cases).
 */
describe('SpotHopperService — Spec 941 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SpotHopperModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SpotHopperModule],
      }).compile();
      const service = moduleRef.get(SpotHopperService);
      expect(service).toBeInstanceOf(SpotHopperService);
      await moduleRef.close();
    });

    it('exports the Site.SPOTHOPPER = "spothopper" enum value', () => {
      expect(Site.SPOTHOPPER).toBe('spothopper');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new SpotHopperService();
      const result = await service.scrape({
        siteType: [Site.SPOTHOPPER],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'spothopper-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.SPOTHOPPER);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('SpotHopper');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/spothopper/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/spothopper/jobs/');
      ...
      expect(job0?.description).toContain('SpotHopper');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/spothopper/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('SpotHopper');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'spothopper-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
