...
import { GinkgoBioworksModule, GinkgoBioworksService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'ginkgobioworks-jobs.json'), 'utf8'),
);
...
/**
 * Spec 582 / T04 — `GinkgoBioworksService` unit tests (≥ 9 cases).
 */
describe('GinkgoBioworksService — Spec 582 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through GinkgoBioworksModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [GinkgoBioworksModule],
      }).compile();
      const service = moduleRef.get(GinkgoBioworksService);
      expect(service).toBeInstanceOf(GinkgoBioworksService);
      await moduleRef.close();
    });

    it('exports the Site.GINKGO_BIOWORKS = "ginkgobioworks" enum value', () => {
      expect(Site.GINKGO_BIOWORKS).toBe('ginkgobioworks');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new GinkgoBioworksService();
      const result = await service.scrape({
        siteType: [Site.GINKGO_BIOWORKS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'ginkgobioworks-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.GINKGO_BIOWORKS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Ginkgo Bioworks');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/ginkgobioworks/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/ginkgobioworks/jobs/');
      ...
      expect(job0?.description).toContain('Ginkgo Bioworks');
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Ginkgo Bioworks');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'ginkgobioworks-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
