...
import { HighTechHighModule, HighTechHighService } from '../src';

const COMPANY_NAME_EXPECT = 'High Tech High';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'hightechhigh-jobs.json'), 'utf8'),
);
...
describe('HighTechHighService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through HighTechHighModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [HighTechHighModule],
      }).compile();
      const service = moduleRef.get(HighTechHighService);
      expect(service).toBeInstanceOf(HighTechHighService);
      await moduleRef.close();
    });

    it('exports the Site.HIGH_TECH_HIGH = "hightechhigh" enum value', () => {
      expect(Site.HIGH_TECH_HIGH).toBe('hightechhigh');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new HighTechHighService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.HIGH_TECH_HIGH],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'hightechhigh-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.HIGH_TECH_HIGH);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('hightechhigh-' + first.id);
      ...
      expect(calledUrls[0]).toContain('hightechhigh');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.HIGH_TECH_HIGH);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('hightechhigh-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new HighTechHighService(registry);
      const result = await service.scrape({
        siteType: [Site.HIGH_TECH_HIGH],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('hightechhigh');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('hightechhigh-x1');
      expect(result.jobs[0].site).toBe(Site.HIGH_TECH_HIGH);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('hightechhigh-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new HighTechHighService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.HIGH_TECH_HIGH],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new HighTechHighService();
      const result = await service.scrape({
        siteType: [Site.HIGH_TECH_HIGH],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
