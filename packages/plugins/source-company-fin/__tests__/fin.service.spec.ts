...
import { FinModule, FinService } from '../src';

const COMPANY_NAME_EXPECT = 'Fin';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'fin-jobs.json'), 'utf8'),
);
...
describe('FinService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through FinModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [FinModule],
      }).compile();
      const service = moduleRef.get(FinService);
      expect(service).toBeInstanceOf(FinService);
      await moduleRef.close();
    });

    it('exports the Site.FIN = "fin" enum value', () => {
      expect(Site.FIN).toBe('fin');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new FinService(registryWithAshby());
      ...
      expect(job0?.site).toBe(Site.FIN);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('fin-' + first.id);
      ...
      expect(calledUrls[0]).toContain('fin');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.FIN);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('fin-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new FinService(registry);
      const result = await service.scrape({
        siteType: [Site.FIN],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('fin');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('fin-x1');
      expect(result.jobs[0].site).toBe(Site.FIN);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('fin-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new FinService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.FIN],
      } as ScraperInputDto);
