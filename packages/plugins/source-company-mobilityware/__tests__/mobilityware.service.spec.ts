...
import { MobilityWareModule, MobilityWareService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'mobilityware-jobs.json'), 'utf8'),
);
...
/**
 * Spec 885 / T04 — `MobilityWareService` unit tests (≥ 9 cases).
 */
describe('MobilityWareService — Spec 885 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through MobilityWareModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [MobilityWareModule],
      }).compile();
      const service = moduleRef.get(MobilityWareService);
      expect(service).toBeInstanceOf(MobilityWareService);
      await moduleRef.close();
    });

    it('exports the Site.MOBILITYWARE = "mobilityware" enum value', () => {
      expect(Site.MOBILITYWARE).toBe('mobilityware');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new MobilityWareService();
      const result = await service.scrape({
        siteType: [Site.MOBILITYWARE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'mobilityware-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.MOBILITYWARE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('MobilityWare');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/mobilityware/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/mobilityware/jobs/');
      ...
      expect(job0?.description).toContain('MobilityWare');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/mobilityware/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('MobilityWare');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'mobilityware-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
