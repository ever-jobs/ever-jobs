...
import { EsusuModule, EsusuService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'esusu-jobs.json'), 'utf8'),
);
...
/**
 * Spec 843 / T04 — `EsusuService` unit tests (≥ 9 cases).
 */
describe('EsusuService — Spec 843 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through EsusuModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [EsusuModule],
      }).compile();
      const service = moduleRef.get(EsusuService);
      expect(service).toBeInstanceOf(EsusuService);
      await moduleRef.close();
    });

    it('exports the Site.ESUSU = "esusu" enum value', () => {
      expect(Site.ESUSU).toBe('esusu');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new EsusuService();
      ...
      const job0 = dto.jobs.find((j) => j.id === 'esusu-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ESUSU);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Esusu');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/esusu/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/esusu/jobs/');
      ...
      expect(job0?.description).toContain('Esusu');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/esusu/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Esusu');
      }
    });
  });
