...
import { PalantirTechnologiesModule, PalantirTechnologiesService } from '../src';

const COMPANY_NAME_EXPECT = 'Palantir Technologies';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'palantir-jobs.json'), 'utf8'),
);
...
describe('PalantirTechnologiesService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through PalantirTechnologiesModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [PalantirTechnologiesModule],
      }).compile();
      const service = moduleRef.get(PalantirTechnologiesService);
      expect(service).toBeInstanceOf(PalantirTechnologiesService);
      await moduleRef.close();
    });

    it('exports the Site.PALANTIR_TECHNOLOGIES = "palantir" enum value', () => {
      expect(Site.PALANTIR_TECHNOLOGIES).toBe('palantir');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new PalantirTechnologiesService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.PALANTIR_TECHNOLOGIES],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'palantir-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.PALANTIR_TECHNOLOGIES);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('palantir-' + first.id);
      ...
      expect(calledUrls[0]).toContain('palantir');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.PALANTIR_TECHNOLOGIES);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('palantir-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new PalantirTechnologiesService(registry);
      const result = await service.scrape({
        siteType: [Site.PALANTIR_TECHNOLOGIES],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('palantir');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('palantir-x1');
      expect(result.jobs[0].site).toBe(Site.PALANTIR_TECHNOLOGIES);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('palantir-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new PalantirTechnologiesService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.PALANTIR_TECHNOLOGIES],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new PalantirTechnologiesService();
      const result = await service.scrape({
        siteType: [Site.PALANTIR_TECHNOLOGIES],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
