...
import { TMobileModule, TMobileService } from '../src';
...
const FIXTURE: Fixture = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'fixtures', 'tmobile-boards.json'), 'utf8'),
);
...
describe('TMobileService — Workday delegation (Spec 1736)', () => {
  beforeEach(() => {
    mockGet.mockReset();
    mockPost.mockReset();
  ...
  describe('registration', () => {
    it('resolves through TMobileModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({ imports: [TMobileModule] }).compile();
      expect(moduleRef.get(TMobileService)).toBeInstanceOf(TMobileService);
      await moduleRef.close();
    });

    it('exports Site.T_MOBILE = "tmobile"', () => {
      expect(Site.T_MOBILE).toBe('tmobile');
    });

    it('declares a tagged company plugin with its domains', () => {
      const meta = Reflect.getMetadata(SOURCE_PLUGIN_METADATA, TMobileService);
      expect(meta.site).toBe(Site.T_MOBILE);
      expect(meta.category).toBe('company');
      expect(meta.isAts).toBeFalsy();
      expect(meta.companyDomains).toEqual(['t-mobile.com']);
  ...
  describe('recorded board (real Workday adapter, mocked HTTP)', () => {
    it('maps every recorded posting and re-stamps the company identity', async () => {
      const service = new TMobileService(registryWith());
      const result = await service.scrape({ siteType: [Site.T_MOBILE], resultsWanted: 100 } as ScraperInputDto);
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.T_MOBILE);
    ...
    it('only requests the recorded board URLs', async () => {
      const service = new TMobileService(registryWith());
      await service.scrape({ siteType: [Site.T_MOBILE] } as ScraperInputDto);
    ...
    it('honours resultsWanted=1', async () => {
      const service = new TMobileService(registryWith());
      const result = await service.scrape({ siteType: [Site.T_MOBILE], resultsWanted: 1 } as ScraperInputDto);
    ...
    it('resolves with an empty result when the board is gone (HTTP 404)', async () => {
      ...
      const service = new TMobileService(registryWith());
      const result = await service.scrape({ siteType: [Site.T_MOBILE] } as ScraperInputDto);
  ...
  describe('delegation contract', () => {
    it('forwards the board and the caller input untouched', async () => {
      ...
      const service = new TMobileService(
        registryWith(
          fakeBackend(
      ...
      expect(result.jobs[0].site).toBe(Site.T_MOBILE);
    ...
    it('never forwards the caller\'s credentials to the board (Spec 1735 §4.5)', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new TMobileService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
    ...
    it('makes no request when resultsWanted is 0', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new TMobileService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      const result = await service.scrape({ siteType: [Site.T_MOBILE], resultsWanted: 0 } as ScraperInputDto);
  ...
  describe('resilience', () => {
    it('reports not_registered when no registry is injected', async () => {
      const result = await new TMobileService().scrape({ siteType: [Site.T_MOBILE] } as ScraperInputDto);
      expect(result.jobs).toEqual([]);
    ...
    it('reports not_registered when Workday is missing from the registry', async () => {
      const result = await new TMobileService(new PluginRegistry()).scrape({
        siteType: [Site.T_MOBILE],
      } as ScraperInputDto);
      expect(result.diagnostics?.reason).toBe('not_registered');
    });
