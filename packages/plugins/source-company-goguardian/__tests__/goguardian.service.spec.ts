...
import { GoGuardianModule, GoGuardianService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'goguardian-jobs.json'), 'utf8'),
);
...
/**
 * Spec 744 / T04 — `GoGuardianService` unit tests (≥ 9 cases).
 */
describe('GoGuardianService — Spec 744 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through GoGuardianModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [GoGuardianModule],
      }).compile();
      const service = moduleRef.get(GoGuardianService);
      expect(service).toBeInstanceOf(GoGuardianService);
      await moduleRef.close();
    });

    it('exports the Site.GOGUARDIAN = "goguardian" enum value', () => {
      expect(Site.GOGUARDIAN).toBe('goguardian');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new GoGuardianService();
      const result = await service.scrape({
        siteType: [Site.GOGUARDIAN],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'goguardian-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.GOGUARDIAN);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('GoGuardian');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/goguardian/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/goguardian/jobs/');
      ...
      expect(job0?.description).toContain('GoGuardian');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/goguardian/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('GoGuardian');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'goguardian-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
