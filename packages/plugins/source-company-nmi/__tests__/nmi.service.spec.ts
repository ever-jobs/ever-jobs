...
import { NMIModule, NMIService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'nmi-jobs.json'), 'utf8'),
);
...
/**
 * Spec 896 / T04 — `NMIService` unit tests (≥ 9 cases).
 */
describe('NMIService — Spec 896 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through NMIModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [NMIModule],
      }).compile();
      const service = moduleRef.get(NMIService);
      expect(service).toBeInstanceOf(NMIService);
      await moduleRef.close();
    });

    it('exports the Site.NMI = "nmi" enum value', () => {
      expect(Site.NMI).toBe('nmi');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const job0 = dto.jobs.find((j) => j.id === 'nmi-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.NMI);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('NMI');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/nmi/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/nmi/jobs/');
      ...
      expect(job0?.description).toContain('NMI');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/nmi/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('NMI');
      }
    });
  });
