...
/**
 * NMI — NMI is an employer in the Fintech sector, headquartered in Schaumburg, Illinois, USA..
 *
 * NMI is a company operating in Fintech — payments gateway / embedded
 * payments, headquartered in Schaumburg, Illinois, USA. This source
 * plugin ingests live open roles published on NMI's official
 ...
 *
 * Sector: Fintech — payments gateway / embedded payments. HQ: Schaumburg, Illinois, USA.
 *
 * Highlights:
 *   - Sector: Fintech — payments gateway / embedded payments
 ...
 *     Jobs schema
 *
 * Source profile (Spec 896):
 *   - D-04 — Greenhouse canonical hosted-board host (variant 2):
 *     `https://job-boards.greenhouse.io/nmi/jobs/<id>`.
 *   - D-08 — entity-decode-then-tag-strip description pipeline.
 *   - D-09 — wire `company_name` pass-through (`'NMI'`).
 ...
 */
const API_URL = 'https://api.greenhouse.io/v1/boards/nmi/jobs';
...
@Injectable()
export class NMIService implements IScraper {
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    try {
      ...
      this.logger.log(`NMI: fetching ${url}`);
      ...
      for (const listing of listings) {
        ...
        jobs.push(
          new JobPostDto({
            ...
            companyName: listing.company_name ?? 'NMI',
            // D-04: wire `absolute_url` flows through (variant 2).
            jobUrl:
              listing.absolute_url ??
              `https://job-boards.greenhouse.io/nmi/jobs/${listing.id}`,
            location,
      ...
      this.logger.log(`NMI: scraped ${jobs.length} jobs`);
    } catch (err: any) {
      this.logger.error(`NMI scrape failed: ${err.message}`);
