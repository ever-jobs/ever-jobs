...
import { DroneDeployModule, DroneDeployService } from '../src';

const COMPANY_NAME_EXPECT = 'DroneDeploy';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'dronedeploy-jobs.json'), 'utf8'),
);
...
describe('DroneDeployService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through DroneDeployModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [DroneDeployModule],
      }).compile();
      const service = moduleRef.get(DroneDeployService);
      expect(service).toBeInstanceOf(DroneDeployService);
      await moduleRef.close();
    });

    it('exports the Site.DRONEDEPLOY = "dronedeploy" enum value', () => {
      expect(Site.DRONEDEPLOY).toBe('dronedeploy');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new DroneDeployService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.DRONEDEPLOY],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'dronedeploy-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.DRONEDEPLOY);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('dronedeploy-' + first.id);
      ...
      expect(calledUrls[0]).toContain('dronedeploy');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.DRONEDEPLOY);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('dronedeploy-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new DroneDeployService(registry);
      const result = await service.scrape({
        siteType: [Site.DRONEDEPLOY],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('dronedeploy');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('dronedeploy-x1');
      expect(result.jobs[0].site).toBe(Site.DRONEDEPLOY);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('dronedeploy-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new DroneDeployService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.DRONEDEPLOY],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new DroneDeployService();
      const result = await service.scrape({
        siteType: [Site.DRONEDEPLOY],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
