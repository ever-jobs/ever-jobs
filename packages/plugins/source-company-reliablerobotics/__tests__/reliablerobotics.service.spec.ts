...
import { ReliableRoboticsModule, ReliableRoboticsService } from '../src';

const COMPANY_NAME_EXPECT = 'Reliable Robotics';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'reliablerobotics-jobs.json'), 'utf8'),
);
...
describe('ReliableRoboticsService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ReliableRoboticsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ReliableRoboticsModule],
      }).compile();
      const service = moduleRef.get(ReliableRoboticsService);
      expect(service).toBeInstanceOf(ReliableRoboticsService);
      await moduleRef.close();
    });

    it('exports the Site.RELIABLE_ROBOTICS = "reliablerobotics" enum value', () => {
      expect(Site.RELIABLE_ROBOTICS).toBe('reliablerobotics');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new ReliableRoboticsService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.RELIABLE_ROBOTICS],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'reliablerobotics-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.RELIABLE_ROBOTICS);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('reliablerobotics-' + first.id);
      ...
      expect(calledUrls[0]).toContain('reliable-robotics');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.RELIABLE_ROBOTICS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('reliablerobotics-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new ReliableRoboticsService(registry);
      const result = await service.scrape({
        siteType: [Site.RELIABLE_ROBOTICS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('reliable-robotics');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('reliablerobotics-x1');
      expect(result.jobs[0].site).toBe(Site.RELIABLE_ROBOTICS);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('reliablerobotics-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new ReliableRoboticsService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.RELIABLE_ROBOTICS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new ReliableRoboticsService();
      const result = await service.scrape({
        siteType: [Site.RELIABLE_ROBOTICS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
