...
import { PrijsvrijVakantiesModule, PrijsvrijVakantiesService } from '../src';

const COMPANY_NAME_EXPECT = 'Prijsvrij Vakanties';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'prijsvrijvakanties-jobs.json'), 'utf8'),
);
...
describe('PrijsvrijVakantiesService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through PrijsvrijVakantiesModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [PrijsvrijVakantiesModule],
      }).compile();
      const service = moduleRef.get(PrijsvrijVakantiesService);
      expect(service).toBeInstanceOf(PrijsvrijVakantiesService);
      await moduleRef.close();
    });

    it('exports the Site.PRIJSVRIJ_VAKANTIES = "prijsvrijvakanties" enum value', () => {
      expect(Site.PRIJSVRIJ_VAKANTIES).toBe('prijsvrijvakanties');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new PrijsvrijVakantiesService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.PRIJSVRIJ_VAKANTIES],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'prijsvrijvakanties-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Recruitee's defaults
      expect(job0?.site).toBe(Site.PRIJSVRIJ_VAKANTIES);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('prijsvrijvakanties-' + first.id);
      ...
      expect(calledUrls[0]).toContain('prijsvrijvakanties.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.PRIJSVRIJ_VAKANTIES);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('prijsvrijvakanties-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new PrijsvrijVakantiesService(registry);
      const result = await service.scrape({
        siteType: [Site.PRIJSVRIJ_VAKANTIES],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('prijsvrijvakanties');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('prijsvrijvakanties-x1');
      expect(result.jobs[0].site).toBe(Site.PRIJSVRIJ_VAKANTIES);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('prijsvrijvakanties-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new PrijsvrijVakantiesService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.PRIJSVRIJ_VAKANTIES],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new PrijsvrijVakantiesService();
      const result = await service.scrape({
        siteType: [Site.PRIJSVRIJ_VAKANTIES],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
