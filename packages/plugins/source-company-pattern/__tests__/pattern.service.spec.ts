...
import { PatternModule, PatternService } from '../src';

const COMPANY_NAME_EXPECT = 'Pattern';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'pattern-jobs.json'), 'utf8'),
);
...
describe('PatternService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through PatternModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [PatternModule],
      }).compile();
      const service = moduleRef.get(PatternService);
      expect(service).toBeInstanceOf(PatternService);
      await moduleRef.close();
    });

    it('exports the Site.PATTERN = "pattern" enum value', () => {
      expect(Site.PATTERN).toBe('pattern');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new PatternService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.PATTERN],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('pattern');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.PATTERN);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('pattern-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new PatternService(registry);
      const result = await service.scrape({
        siteType: [Site.PATTERN],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('pattern');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('pattern-x1');
      expect(result.jobs[0].site).toBe(Site.PATTERN);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('pattern-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new PatternService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.PATTERN],
      } as ScraperInputDto);
