...
import { OpenTableModule, OpenTableService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'opentable-jobs.json'), 'utf8'),
);
...
/**
 * Spec 901 / T04 — `OpenTableService` unit tests (≥ 9 cases).
 */
describe('OpenTableService — Spec 901 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through OpenTableModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [OpenTableModule],
      }).compile();
      const service = moduleRef.get(OpenTableService);
      expect(service).toBeInstanceOf(OpenTableService);
      await moduleRef.close();
    });

    it('exports the Site.OPENTABLE = "opentable" enum value', () => {
      expect(Site.OPENTABLE).toBe('opentable');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new OpenTableService();
      const result = await service.scrape({
        siteType: [Site.OPENTABLE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'opentable-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.OPENTABLE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('OpenTable');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/opentable/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/opentable/jobs/');
      ...
      expect(job0?.description).toContain('OpenTable');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/opentable/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('OpenTable');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'opentable-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
