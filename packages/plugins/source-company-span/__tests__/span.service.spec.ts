...
import { SPANModule, SPANService } from '../src';

const COMPANY_NAME_EXPECT = 'SPAN';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'span-jobs.json'), 'utf8'),
);
...
describe('SPANService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SPANModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SPANModule],
      }).compile();
      const service = moduleRef.get(SPANService);
      expect(service).toBeInstanceOf(SPANService);
      await moduleRef.close();
    });

    it('exports the Site.SPAN = "span" enum value', () => {
      expect(Site.SPAN).toBe('span');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new SPANService(registryWithAshby());
      ...
      expect(calledUrls[0]).toContain('span');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SPAN);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('span-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new SPANService(registry);
      const result = await service.scrape({
        siteType: [Site.SPAN],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('span');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('span-x1');
      expect(result.jobs[0].site).toBe(Site.SPAN);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('span-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new SPANService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.SPAN],
      } as ScraperInputDto);
