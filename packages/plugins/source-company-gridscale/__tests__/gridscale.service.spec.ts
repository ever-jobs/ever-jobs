...
import { GridscaleModule, GridscaleService } from '../src';

const COMPANY_NAME_EXPECT = 'gridscale';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'gridscale-jobs.json'), 'utf8'),
);
...
describe('GridscaleService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through GridscaleModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [GridscaleModule],
      }).compile();
      const service = moduleRef.get(GridscaleService);
      expect(service).toBeInstanceOf(GridscaleService);
      await moduleRef.close();
    });

    it('exports the Site.GRIDSCALE = "gridscale" enum value', () => {
      expect(Site.GRIDSCALE).toBe('gridscale');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new GridscaleService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.GRIDSCALE],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('gridscale.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.GRIDSCALE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('gridscale-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new GridscaleService(registry);
      const result = await service.scrape({
        siteType: [Site.GRIDSCALE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('gridscale');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('gridscale-x1');
      expect(result.jobs[0].site).toBe(Site.GRIDSCALE);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('gridscale-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new GridscaleService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.GRIDSCALE],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new GridscaleService();
      const result = await service.scrape({
        siteType: [Site.GRIDSCALE],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
