...
import { TypefaceModule, TypefaceService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'typeface-jobs.json'), 'utf8'),
);
...
/**
 * Spec 611 / T04 — `TypefaceService` unit tests (≥ 9 cases).
 */
describe('TypefaceService — Spec 611 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through TypefaceModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [TypefaceModule],
      }).compile();
      const service = moduleRef.get(TypefaceService);
      expect(service).toBeInstanceOf(TypefaceService);
      await moduleRef.close();
    });

    it('exports the Site.TYPEFACE = "typeface" enum value', () => {
      expect(Site.TYPEFACE).toBe('typeface');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new TypefaceService();
      const result = await service.scrape({
        siteType: [Site.TYPEFACE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'typeface-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.TYPEFACE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Typeface');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/typeface/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/typeface/jobs/');
      ...
      expect(job0?.description).toContain('Typeface');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/typeface/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Typeface');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'typeface-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
