...
/**
 * Jane Street — Quantitative trading (HQ: New York, NY, USA).
 *
 * Source (Spec 1737): Greenhouse board, scraped in this order:
 *   - `janestreet` — https://job-boards.greenhouse.io/janestreet
 *     verified live 2026-09-24: 228 open postings.
 *
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'janestreet', atsIdPrefix: 'gh-' },
];

@SourcePlugin({
  site: Site.JANE_STREET,
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['janestreet.com'],
  description: 'Jane Street careers via Greenhouse. Tags: segment=quant-trading; industry=quantitative-trading.',
})
@Injectable()
export class JaneStreetService implements IScraper {
  private readonly logger = new Logger(JaneStreetService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Greenhouse source plugin is not registered; cannot scrape Jane Street');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`Jane Street: delegating to Greenhouse (${board.companySlug})`);
    ...
    this.logger.log(`Jane Street: scraped ${jobs.length} jobs`);
