...
import { JaneStreetModule, JaneStreetService } from '../src';
...
const FIXTURE: Fixture = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'fixtures', 'janestreet-boards.json'), 'utf8'),
);
const COMPANY_NAME = 'Jane Street';
const ID_PREFIX = 'janestreet-';
...
describe('JaneStreetService — Greenhouse delegation (Spec 1737)', () => {
  beforeEach(() => {
    mockGet.mockReset();
    mockPost.mockReset();
  ...
  describe('registration', () => {
    it('resolves through JaneStreetModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({ imports: [JaneStreetModule] }).compile();
      expect(moduleRef.get(JaneStreetService)).toBeInstanceOf(JaneStreetService);
      await moduleRef.close();
    });

    it('exports Site.JANE_STREET = "janestreet"', () => {
      expect(Site.JANE_STREET).toBe('janestreet');
    });

    it('declares a tagged company plugin with its domains', () => {
      const meta = Reflect.getMetadata(SOURCE_PLUGIN_METADATA, JaneStreetService);
      expect(meta.site).toBe(Site.JANE_STREET);
      expect(meta.category).toBe('company');
      expect(meta.isAts).toBeFalsy();
      expect(meta.companyDomains).toEqual(['janestreet.com']);
  ...
  describe('recorded board (real Greenhouse adapter, mocked HTTP)', () => {
    it('maps every recorded posting and re-stamps the company identity', async () => {
      const service = new JaneStreetService(registryWith());
      const result = await service.scrape({ siteType: [Site.JANE_STREET], resultsWanted: 100 } as ScraperInputDto);
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.JANE_STREET);
    ...
    it('only requests the recorded board URLs', async () => {
      const service = new JaneStreetService(registryWith());
      await service.scrape({ siteType: [Site.JANE_STREET] } as ScraperInputDto);
    ...
    it('honours resultsWanted=1', async () => {
      const service = new JaneStreetService(registryWith());
      const result = await service.scrape({ siteType: [Site.JANE_STREET], resultsWanted: 1 } as ScraperInputDto);
    ...
    it('resolves with an empty result when the board is gone (HTTP 404)', async () => {
      ...
      const service = new JaneStreetService(registryWith());
      const result = await service.scrape({ siteType: [Site.JANE_STREET] } as ScraperInputDto);
  ...
  describe('delegation contract', () => {
    it('forwards the board and the caller input untouched', async () => {
      ...
      const service = new JaneStreetService(
        registryWith(
          fakeBackend(
      ...
      expect(result.jobs[0].site).toBe(Site.JANE_STREET);
      expect(result.jobs[0].companyName).toBe(COMPANY_NAME);
    });

    it('passes an absent resultsWanted through as absent', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new JaneStreetService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      await service.scrape({ siteType: [Site.JANE_STREET] } as ScraperInputDto);
    ...
    it('makes no request when resultsWanted is 0', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new JaneStreetService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      const result = await service.scrape({ siteType: [Site.JANE_STREET], resultsWanted: 0 } as ScraperInputDto);
  ...
  describe('resilience', () => {
    it('reports not_registered when no registry is injected', async () => {
      const result = await new JaneStreetService().scrape({ siteType: [Site.JANE_STREET] } as ScraperInputDto);
      expect(result.jobs).toEqual([]);
    ...
    it('reports not_registered when Greenhouse is missing from the registry', async () => {
      const result = await new JaneStreetService(new PluginRegistry()).scrape({
        siteType: [Site.JANE_STREET],
      } as ScraperInputDto);
      expect(result.diagnostics?.reason).toBe('not_registered');
    });
