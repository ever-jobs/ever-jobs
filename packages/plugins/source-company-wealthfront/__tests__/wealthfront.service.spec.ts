...
import { WealthfrontModule, WealthfrontService } from '../src';

const COMPANY_NAME_EXPECT = 'Wealthfront';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'wealthfront-jobs.json'), 'utf8'),
);
...
describe('WealthfrontService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through WealthfrontModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [WealthfrontModule],
      }).compile();
      const service = moduleRef.get(WealthfrontService);
      expect(service).toBeInstanceOf(WealthfrontService);
      await moduleRef.close();
    });

    it('exports the Site.WEALTHFRONT = "wealthfront" enum value', () => {
      expect(Site.WEALTHFRONT).toBe('wealthfront');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new WealthfrontService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.WEALTHFRONT],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'wealthfront-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.WEALTHFRONT);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('wealthfront-' + first.id);
      ...
      expect(calledUrls[0]).toContain('wealthfront');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.WEALTHFRONT);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('wealthfront-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new WealthfrontService(registry);
      const result = await service.scrape({
        siteType: [Site.WEALTHFRONT],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('wealthfront');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('wealthfront-x1');
      expect(result.jobs[0].site).toBe(Site.WEALTHFRONT);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('wealthfront-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new WealthfrontService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.WEALTHFRONT],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new WealthfrontService();
      const result = await service.scrape({
        siteType: [Site.WEALTHFRONT],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
