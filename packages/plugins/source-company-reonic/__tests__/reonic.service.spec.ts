...
import { ReonicModule, ReonicService } from '../src';

const COMPANY_NAME_EXPECT = 'Reonic';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'reonic-jobs.json'), 'utf8'),
);
...
describe('ReonicService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ReonicModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ReonicModule],
      }).compile();
      const service = moduleRef.get(ReonicService);
      expect(service).toBeInstanceOf(ReonicService);
      await moduleRef.close();
    });

    it('exports the Site.REONIC = "reonic" enum value', () => {
      expect(Site.REONIC).toBe('reonic');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new ReonicService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.REONIC],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('reonic');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.REONIC);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('reonic-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new ReonicService(registry);
      const result = await service.scrape({
        siteType: [Site.REONIC],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('reonic');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('reonic-x1');
      expect(result.jobs[0].site).toBe(Site.REONIC);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('reonic-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new ReonicService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.REONIC],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new ReonicService();
      const result = await service.scrape({
        siteType: [Site.REONIC],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
