...
import { EquipHealthModule, EquipHealthService } from '../src';

const COMPANY_NAME_EXPECT = 'Equip Health';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'equiphealth-jobs.json'), 'utf8'),
);
...
describe('EquipHealthService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through EquipHealthModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [EquipHealthModule],
      }).compile();
      const service = moduleRef.get(EquipHealthService);
      expect(service).toBeInstanceOf(EquipHealthService);
      await moduleRef.close();
    });

    it('exports the Site.EQUIP_HEALTH = "equiphealth" enum value', () => {
      expect(Site.EQUIP_HEALTH).toBe('equiphealth');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new EquipHealthService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.EQUIP_HEALTH],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'equiphealth-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.EQUIP_HEALTH);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('equiphealth-' + first.id);
      ...
      expect(calledUrls[0]).toContain('equip');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.EQUIP_HEALTH);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('equiphealth-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new EquipHealthService(registry);
      const result = await service.scrape({
        siteType: [Site.EQUIP_HEALTH],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('equip');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('equiphealth-x1');
      expect(result.jobs[0].site).toBe(Site.EQUIP_HEALTH);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('equiphealth-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new EquipHealthService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.EQUIP_HEALTH],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new EquipHealthService();
      const result = await service.scrape({
        siteType: [Site.EQUIP_HEALTH],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
