...
import { LuckyCartModule, LuckyCartService } from '../src';

const COMPANY_NAME_EXPECT = 'Lucky Cart';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'luckycart-jobs.json'), 'utf8'),
);
...
describe('LuckyCartService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through LuckyCartModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [LuckyCartModule],
      }).compile();
      const service = moduleRef.get(LuckyCartService);
      expect(service).toBeInstanceOf(LuckyCartService);
      await moduleRef.close();
    });

    it('exports the Site.LUCKY_CART = "luckycart" enum value', () => {
      expect(Site.LUCKY_CART).toBe('luckycart');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new LuckyCartService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.LUCKY_CART],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('luckycart.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.LUCKY_CART);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('luckycart-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new LuckyCartService(registry);
      const result = await service.scrape({
        siteType: [Site.LUCKY_CART],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('luckycart');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('luckycart-x1');
      expect(result.jobs[0].site).toBe(Site.LUCKY_CART);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('luckycart-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new LuckyCartService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.LUCKY_CART],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new LuckyCartService();
      const result = await service.scrape({
        siteType: [Site.LUCKY_CART],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
