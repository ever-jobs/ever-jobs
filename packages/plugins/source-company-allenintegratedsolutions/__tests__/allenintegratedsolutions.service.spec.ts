...
import { AllenintegratedsolutionsModule, AllenintegratedsolutionsService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'allenintegratedsolutions-jobs.json'), 'utf8'),
);
...
/**
 * Spec 234 / T04 — `AllenintegratedsolutionsService` unit tests (≥ 9 cases).
 */
describe('AllenintegratedsolutionsService — Spec 234 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AllenintegratedsolutionsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AllenintegratedsolutionsModule],
      }).compile();
      const service = moduleRef.get(AllenintegratedsolutionsService);
      expect(service).toBeInstanceOf(AllenintegratedsolutionsService);
      await moduleRef.close();
    });

    it('exports the Site.ALLENINTEGRATEDSOLUTIONS = "allenintegratedsolutions" enum value', () => {
      expect(Site.ALLENINTEGRATEDSOLUTIONS).toBe('allenintegratedsolutions');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AllenintegratedsolutionsService();
      const result = await service.scrape({
        siteType: [Site.ALLENINTEGRATEDSOLUTIONS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'allenintegratedsolutions-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ALLENINTEGRATEDSOLUTIONS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Allen Integrated Solutions');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/allenintegratedsolutions/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/allenintegratedsolutions/jobs/');
      ...
      expect(job0?.description).toContain('Allen Integrated Solutions');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/allenintegratedsolutions/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Allen Integrated Solutions');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'allenintegratedsolutions-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
