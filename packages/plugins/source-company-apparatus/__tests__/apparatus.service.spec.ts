...
import { ApparatusModule, ApparatusService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'apparatus-jobs.json'), 'utf8'),
);
...
/**
 * Spec 289 / T04 — `ApparatusService` unit tests (≥ 9 cases).
 */
describe('ApparatusService — Spec 289 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ApparatusModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ApparatusModule],
      }).compile();
      const service = moduleRef.get(ApparatusService);
      expect(service).toBeInstanceOf(ApparatusService);
      await moduleRef.close();
    });

    it('exports the Site.APPARATUS = "apparatus" enum value', () => {
      expect(Site.APPARATUS).toBe('apparatus');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ApparatusService();
      const result = await service.scrape({
        siteType: [Site.APPARATUS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'apparatus-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.APPARATUS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('APPARATUS');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/apparatus/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/apparatus/jobs/');
      ...
      expect(job0?.description).toContain('APPARATUS');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/apparatus/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('APPARATUS');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'apparatus-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
