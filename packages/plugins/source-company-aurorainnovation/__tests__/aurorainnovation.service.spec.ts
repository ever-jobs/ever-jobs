...
import { AuroraInnovationModule, AuroraInnovationService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'aurorainnovation-jobs.json'), 'utf8'),
);
...
/**
 * Spec 790 / T04 — `AuroraInnovationService` unit tests (≥ 9 cases).
 */
describe('AuroraInnovationService — Spec 790 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AuroraInnovationModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AuroraInnovationModule],
      }).compile();
      const service = moduleRef.get(AuroraInnovationService);
      expect(service).toBeInstanceOf(AuroraInnovationService);
      await moduleRef.close();
    });

    it('exports the Site.AURORA_INNOVATION = "aurorainnovation" enum value', () => {
      expect(Site.AURORA_INNOVATION).toBe('aurorainnovation');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AuroraInnovationService();
      const result = await service.scrape({
        siteType: [Site.AURORA_INNOVATION],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'aurorainnovation-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.AURORA_INNOVATION);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Aurora Innovation');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/aurorainnovation/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/aurorainnovation/jobs/');
      ...
      expect(job0?.description).toContain('Aurora Innovation');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/aurorainnovation/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Aurora Innovation');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'aurorainnovation-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
