...
import { QualiaModule, QualiaService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'qualia-jobs.json'), 'utf8'),
);
...
/**
 * Spec 916 / T04 — `QualiaService` unit tests (≥ 9 cases).
 */
describe('QualiaService — Spec 916 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through QualiaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [QualiaModule],
      }).compile();
      const service = moduleRef.get(QualiaService);
      expect(service).toBeInstanceOf(QualiaService);
      await moduleRef.close();
    });

    it('exports the Site.QUALIA = "qualia" enum value', () => {
      expect(Site.QUALIA).toBe('qualia');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const job0 = dto.jobs.find((j) => j.id === 'qualia-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.QUALIA);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Qualia');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/qualia/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/qualia/jobs/');
      ...
      expect(job0?.description).toContain('Qualia');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/qualia/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Qualia');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'qualia-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
