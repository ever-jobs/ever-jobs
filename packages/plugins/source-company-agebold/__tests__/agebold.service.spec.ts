...
import { AgeboldModule, AgeboldService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'agebold-jobs.json'), 'utf8'),
);
...
/**
 * Spec 205 / T04 — `AgeboldService` unit tests (≥ 9 cases).
 */
describe('AgeboldService — Spec 205 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AgeboldModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AgeboldModule],
      }).compile();
      const service = moduleRef.get(AgeboldService);
      expect(service).toBeInstanceOf(AgeboldService);
      await moduleRef.close();
    });

    it('exports the Site.AGEBOLD = "agebold" enum value', () => {
      expect(Site.AGEBOLD).toBe('agebold');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AgeboldService();
      const result = await service.scrape({
        siteType: [Site.AGEBOLD],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'agebold-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.AGEBOLD);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Age Bold');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/agebold/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/agebold/jobs/');
      ...
      expect(job0?.description).toContain('Age Bold');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/agebold/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Age Bold');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'agebold-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
