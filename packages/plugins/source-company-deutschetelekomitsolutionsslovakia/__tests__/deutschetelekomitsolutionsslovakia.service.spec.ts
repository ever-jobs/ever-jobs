...
import { DeutscheTelekomITSolutionsSlovakiaModule, DeutscheTelekomITSolutionsSlovakiaService } from '../src';

const COMPANY_NAME_EXPECT = 'Deutsche Telekom IT Solutions Slovakia';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'deutschetelekomitsolutionsslovakia-jobs.json'), 'utf8'),
);
...
describe('DeutscheTelekomITSolutionsSlovakiaService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through DeutscheTelekomITSolutionsSlovakiaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [DeutscheTelekomITSolutionsSlovakiaModule],
      }).compile();
      const service = moduleRef.get(DeutscheTelekomITSolutionsSlovakiaService);
      expect(service).toBeInstanceOf(DeutscheTelekomITSolutionsSlovakiaService);
      await moduleRef.close();
    });

    it('exports the Site.DEUTSCHE_TELEKOM_IT_SOLUTIONS_SLOVAKIA = "deutschetelekomitsolutionsslovakia" enum value', () => {
      expect(Site.DEUTSCHE_TELEKOM_IT_SOLUTIONS_SLOVAKIA).toBe('deutschetelekomitsolutionsslovakia');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new DeutscheTelekomITSolutionsSlovakiaService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.DEUTSCHE_TELEKOM_IT_SOLUTIONS_SLOVAKIA],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'deutschetelekomitsolutionsslovakia-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.DEUTSCHE_TELEKOM_IT_SOLUTIONS_SLOVAKIA);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('deutschetelekomitsolutionsslovakia-' + first.id);
      ...
      expect(calledUrls[0]).toContain('DeutscheTelekomITSolutionsSlovakia');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.DEUTSCHE_TELEKOM_IT_SOLUTIONS_SLOVAKIA);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('deutschetelekomitsolutionsslovakia-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new DeutscheTelekomITSolutionsSlovakiaService(registry);
      const result = await service.scrape({
        siteType: [Site.DEUTSCHE_TELEKOM_IT_SOLUTIONS_SLOVAKIA],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('DeutscheTelekomITSolutionsSlovakia');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('deutschetelekomitsolutionsslovakia-x1');
      expect(result.jobs[0].site).toBe(Site.DEUTSCHE_TELEKOM_IT_SOLUTIONS_SLOVAKIA);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('deutschetelekomitsolutionsslovakia-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new DeutscheTelekomITSolutionsSlovakiaService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.DEUTSCHE_TELEKOM_IT_SOLUTIONS_SLOVAKIA],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new DeutscheTelekomITSolutionsSlovakiaService();
      const result = await service.scrape({
        siteType: [Site.DEUTSCHE_TELEKOM_IT_SOLUTIONS_SLOVAKIA],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
      ...
      expect(result.diagnostics?.detail).toContain('SmartRecruiters');
    });
  });
