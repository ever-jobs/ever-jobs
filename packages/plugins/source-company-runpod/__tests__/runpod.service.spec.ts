...
import { RunpodModule, RunpodService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'runpod-jobs.json'), 'utf8'),
);
...
/**
 * Spec 470 / T04 — `RunpodService` unit tests (≥ 9 cases).
 */
describe('RunpodService — Spec 470 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through RunpodModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [RunpodModule],
      }).compile();
      const service = moduleRef.get(RunpodService);
      expect(service).toBeInstanceOf(RunpodService);
      await moduleRef.close();
    });

    it('exports the Site.RUNPOD = "runpod" enum value', () => {
      expect(Site.RUNPOD).toBe('runpod');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const job0 = dto.jobs.find((j) => j.id === 'runpod-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.RUNPOD);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Runpod');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/runpod/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/runpod/jobs/');
      ...
      expect(job0?.description).toContain('Runpod');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/runpod/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Runpod');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'runpod-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
