...
import { RaisingCaneSModule, RaisingCaneSService } from '../src';

const COMPANY_NAME_EXPECT = 'Raising Cane\'s';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'raisingcanes-jobs.json'), 'utf8'),
);
...
describe('RaisingCaneSService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through RaisingCaneSModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [RaisingCaneSModule],
      }).compile();
      const service = moduleRef.get(RaisingCaneSService);
      expect(service).toBeInstanceOf(RaisingCaneSService);
      await moduleRef.close();
    });

    it('exports the Site.RAISING_CANE_S = "raisingcanes" enum value', () => {
      expect(Site.RAISING_CANE_S).toBe('raisingcanes');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new RaisingCaneSService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.RAISING_CANE_S],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('RaisingCanes');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.RAISING_CANE_S);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('raisingcanes-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new RaisingCaneSService(registry);
      const result = await service.scrape({
        siteType: [Site.RAISING_CANE_S],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('RaisingCanes');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('raisingcanes-x1');
      expect(result.jobs[0].site).toBe(Site.RAISING_CANE_S);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('raisingcanes-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new RaisingCaneSService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.RAISING_CANE_S],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new RaisingCaneSService();
      const result = await service.scrape({
        siteType: [Site.RAISING_CANE_S],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
