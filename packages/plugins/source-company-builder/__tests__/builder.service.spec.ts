...
import { BuilderModule, BuilderService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'builder-jobs.json'), 'utf8'),
);
...
/**
 * Spec 454 / T04 — `BuilderService` unit tests (≥ 9 cases).
 */
describe('BuilderService — Spec 454 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through BuilderModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [BuilderModule],
      }).compile();
      const service = moduleRef.get(BuilderService);
      expect(service).toBeInstanceOf(BuilderService);
      await moduleRef.close();
    });

    it('exports the Site.BUILDER = "builder" enum value', () => {
      expect(Site.BUILDER).toBe('builder');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const job0 = dto.jobs.find((j) => j.id === 'builder-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.BUILDER);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Builder.io');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/builder/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/builder/jobs/');
      ...
      expect(job0?.description).toContain('Builder.io');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/builder/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Builder.io');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'builder-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
