...
import { RunwiseModule, RunwiseService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'runwise-jobs.json'), 'utf8'),
);
...
/**
 * Spec 795 / T04 — `RunwiseService` unit tests (≥ 9 cases).
 */
describe('RunwiseService — Spec 795 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through RunwiseModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [RunwiseModule],
      }).compile();
      const service = moduleRef.get(RunwiseService);
      expect(service).toBeInstanceOf(RunwiseService);
      await moduleRef.close();
    });

    it('exports the Site.RUNWISE = "runwise" enum value', () => {
      expect(Site.RUNWISE).toBe('runwise');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new RunwiseService();
      const result = await service.scrape({
        siteType: [Site.RUNWISE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'runwise-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.RUNWISE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Runwise');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/runwise/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/runwise/jobs/');
      ...
      expect(job0?.description).toContain('Runwise');
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Runwise');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'runwise-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
