...
import { RelaytherapeuticsModule, RelaytherapeuticsService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'relaytherapeutics-jobs.json'), 'utf8'),
);
...
/**
 * Spec 567 / T04 — `RelaytherapeuticsService` unit tests (≥ 9 cases).
 */
describe('RelaytherapeuticsService — Spec 567 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through RelaytherapeuticsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [RelaytherapeuticsModule],
      }).compile();
      const service = moduleRef.get(RelaytherapeuticsService);
      expect(service).toBeInstanceOf(RelaytherapeuticsService);
      await moduleRef.close();
    });

    it('exports the Site.RELAY_THERAPEUTICS = "relaytherapeutics" enum value', () => {
      expect(Site.RELAY_THERAPEUTICS).toBe('relaytherapeutics');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new RelaytherapeuticsService();
      const result = await service.scrape({
        siteType: [Site.RELAY_THERAPEUTICS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'relaytherapeutics-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.RELAY_THERAPEUTICS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Relay Therapeutics');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/relaytherapeutics/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/relaytherapeutics/jobs/');
      ...
      expect(job0?.description).toContain('Relay Therapeutics');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/relaytherapeutics/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Relay Therapeutics');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'relaytherapeutics-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
