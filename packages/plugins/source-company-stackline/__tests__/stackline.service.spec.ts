...
import { StacklineModule, StacklineService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'stackline-jobs.json'), 'utf8'),
);
...
/**
 * Spec 616 / T04 — `StacklineService` unit tests (≥ 9 cases).
 */
describe('StacklineService — Spec 616 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through StacklineModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [StacklineModule],
      }).compile();
      const service = moduleRef.get(StacklineService);
      expect(service).toBeInstanceOf(StacklineService);
      await moduleRef.close();
    });

    it('exports the Site.STACKLINE = "stackline" enum value', () => {
      expect(Site.STACKLINE).toBe('stackline');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new StacklineService();
      const result = await service.scrape({
        siteType: [Site.STACKLINE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'stackline-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.STACKLINE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Stackline');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/stackline/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/stackline/jobs/');
      ...
      expect(job0?.description).toContain('Stackline');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/stackline/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Stackline');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'stackline-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
