...
import { FigureaiModule, FigureaiService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'figureai-jobs.json'), 'utf8'),
);
...
/**
 * Spec 544 / T04 — `FigureaiService` unit tests (≥ 9 cases).
 */
describe('FigureaiService — Spec 544 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through FigureaiModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [FigureaiModule],
      }).compile();
      const service = moduleRef.get(FigureaiService);
      expect(service).toBeInstanceOf(FigureaiService);
      await moduleRef.close();
    });

    it('exports the Site.FIGURE = "figureai" enum value', () => {
      expect(Site.FIGURE).toBe('figureai');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new FigureaiService();
      const result = await service.scrape({
        siteType: [Site.FIGURE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'figureai-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.FIGURE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Figure');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/figureai/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/figureai/jobs/');
      ...
      expect(job0?.description).toContain('Figure');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/figureai/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Figure');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'figureai-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
