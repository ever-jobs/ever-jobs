...
import { FixicoModule, FixicoService } from '../src';

const COMPANY_NAME_EXPECT = 'Fixico';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'fixico-jobs.json'), 'utf8'),
);
...
describe('FixicoService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through FixicoModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [FixicoModule],
      }).compile();
      const service = moduleRef.get(FixicoService);
      expect(service).toBeInstanceOf(FixicoService);
      await moduleRef.close();
    });

    it('exports the Site.FIXICO = "fixico" enum value', () => {
      expect(Site.FIXICO).toBe('fixico');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new FixicoService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.FIXICO],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('fixico.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.FIXICO);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('fixico-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new FixicoService(registry);
      const result = await service.scrape({
        siteType: [Site.FIXICO],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('fixico');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('fixico-x1');
      expect(result.jobs[0].site).toBe(Site.FIXICO);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('fixico-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new FixicoService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.FIXICO],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new FixicoService();
      const result = await service.scrape({
        siteType: [Site.FIXICO],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
