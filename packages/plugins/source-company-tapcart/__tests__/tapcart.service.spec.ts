...
import { TapcartModule, TapcartService } from '../src';

const COMPANY_NAME_EXPECT = 'Tapcart';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'tapcart-jobs.json'), 'utf8'),
);
...
describe('TapcartService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through TapcartModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [TapcartModule],
      }).compile();
      const service = moduleRef.get(TapcartService);
      expect(service).toBeInstanceOf(TapcartService);
      await moduleRef.close();
    });

    it('exports the Site.TAPCART = "tapcart" enum value', () => {
      expect(Site.TAPCART).toBe('tapcart');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new TapcartService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.TAPCART],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('tapcart');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.TAPCART);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('tapcart-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new TapcartService(registry);
      const result = await service.scrape({
        siteType: [Site.TAPCART],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('tapcart');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('tapcart-x1');
      expect(result.jobs[0].site).toBe(Site.TAPCART);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('tapcart-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new TapcartService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.TAPCART],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new TapcartService();
      const result = await service.scrape({
        siteType: [Site.TAPCART],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
