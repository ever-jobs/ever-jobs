...
import { NexampModule, NexampService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'nexamp-jobs.json'), 'utf8'),
);
...
/**
 * Spec 666 / T04 — `NexampService` unit tests (≥ 9 cases).
 */
describe('NexampService — Spec 666 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through NexampModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [NexampModule],
      }).compile();
      const service = moduleRef.get(NexampService);
      expect(service).toBeInstanceOf(NexampService);
      await moduleRef.close();
    });

    it('exports the Site.NEXAMP = "nexamp" enum value', () => {
      expect(Site.NEXAMP).toBe('nexamp');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new NexampService();
      const result = await service.scrape({
        siteType: [Site.NEXAMP],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'nexamp-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.NEXAMP);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Nexamp');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/nexamp/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/nexamp/jobs/');
      ...
      expect(job0?.description).toContain('Nexamp');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/nexamp/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Nexamp');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'nexamp-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
