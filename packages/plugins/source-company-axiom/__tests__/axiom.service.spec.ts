...
import { AxiomModule, AxiomService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'axiom-jobs.json'), 'utf8'),
);
...
/**
 * Spec 637 / T04 — `AxiomService` unit tests (≥ 9 cases).
 */
describe('AxiomService — Spec 637 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AxiomModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AxiomModule],
      }).compile();
      const service = moduleRef.get(AxiomService);
      expect(service).toBeInstanceOf(AxiomService);
      await moduleRef.close();
    });

    it('exports the Site.AXIOM = "axiom" enum value', () => {
      expect(Site.AXIOM).toBe('axiom');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AxiomService();
      ...
      const job0 = dto.jobs.find((j) => j.id === 'axiom-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.AXIOM);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Axiom');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/axiom/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/axiom/jobs/');
      ...
      expect(job0?.description).toContain('Axiom');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/axiom/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Axiom');
      }
    });
  });
