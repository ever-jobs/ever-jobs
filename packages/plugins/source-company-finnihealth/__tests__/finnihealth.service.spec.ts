...
import { FinniHealthModule, FinniHealthService } from '../src';

const COMPANY_NAME_EXPECT = 'Finni Health';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'finnihealth-jobs.json'), 'utf8'),
);
...
describe('FinniHealthService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through FinniHealthModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [FinniHealthModule],
      }).compile();
      const service = moduleRef.get(FinniHealthService);
      expect(service).toBeInstanceOf(FinniHealthService);
      await moduleRef.close();
    });

    it('exports the Site.FINNI_HEALTH = "finnihealth" enum value', () => {
      expect(Site.FINNI_HEALTH).toBe('finnihealth');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new FinniHealthService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.FINNI_HEALTH],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'finnihealth-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.FINNI_HEALTH);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('finnihealth-' + first.id);
      ...
      expect(calledUrls[0]).toContain('finni-health');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.FINNI_HEALTH);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('finnihealth-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new FinniHealthService(registry);
      const result = await service.scrape({
        siteType: [Site.FINNI_HEALTH],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('finni-health');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('finnihealth-x1');
      expect(result.jobs[0].site).toBe(Site.FINNI_HEALTH);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('finnihealth-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new FinniHealthService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.FINNI_HEALTH],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new FinniHealthService();
      const result = await service.scrape({
        siteType: [Site.FINNI_HEALTH],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
