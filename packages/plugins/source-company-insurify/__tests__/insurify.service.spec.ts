...
import { InsurifyModule, InsurifyService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'insurify-jobs.json'), 'utf8'),
);
...
/**
 * Spec 654 / T04 — `InsurifyService` unit tests (≥ 9 cases).
 */
describe('InsurifyService — Spec 654 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through InsurifyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [InsurifyModule],
      }).compile();
      const service = moduleRef.get(InsurifyService);
      expect(service).toBeInstanceOf(InsurifyService);
      await moduleRef.close();
    });

    it('exports the Site.INSURIFY = "insurify" enum value', () => {
      expect(Site.INSURIFY).toBe('insurify');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new InsurifyService();
      const result = await service.scrape({
        siteType: [Site.INSURIFY],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'insurify-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.INSURIFY);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Insurify');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/insurify/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/insurify/jobs/');
      ...
      expect(job0?.description).toContain('Insurify');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/insurify/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Insurify');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'insurify-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
