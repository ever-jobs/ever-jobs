...
import { HarrySModule, HarrySService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'harrys-jobs.json'), 'utf8'),
);
...
/**
 * Spec 653 / T04 — `HarrySService` unit tests (≥ 9 cases).
 */
describe('HarrySService — Spec 653 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through HarrySModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [HarrySModule],
      }).compile();
      const service = moduleRef.get(HarrySService);
      expect(service).toBeInstanceOf(HarrySService);
      await moduleRef.close();
    });

    it('exports the Site.HARRY_S = "harrys" enum value', () => {
      expect(Site.HARRY_S).toBe('harrys');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new HarrySService();
      const result = await service.scrape({
        siteType: [Site.HARRY_S],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'harrys-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.HARRY_S);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Harry\'s');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/harrys/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/harrys/jobs/');
      ...
      expect(job0?.description).toContain('Harry\'s');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/harrys/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Harry\'s');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'harrys-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
