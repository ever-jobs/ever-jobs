...
/**
 * Spin — Spin is an employer in the Shared micromobility sector, headquartered in San Francisco, California, USA..
 *
 * Spin is a company operating in Shared micromobility (e-scooters),
 * headquartered in San Francisco, California, USA. This source plugin
 * ingests live open roles published on Spin's official
 ...
 *
 * Sector: Shared micromobility (e-scooters). HQ: San Francisco, California, USA.
 *
 * Highlights:
 *   - Sector: Shared micromobility (e-scooters)
 ...
 *     Jobs schema
 *
 * Source profile (Spec 939):
 *   - D-04 — Greenhouse canonical hosted-board host (variant 2):
 *     `https://job-boards.greenhouse.io/spin/jobs/<id>`.
 *   - D-08 — entity-decode-then-tag-strip description pipeline.
 *   - D-09 — wire `company_name` pass-through (`'Spin'`).
 ...
 */
const API_URL = 'https://api.greenhouse.io/v1/boards/spin/jobs';
...
@Injectable()
export class SpinService implements IScraper {
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    try {
      ...
      this.logger.log(`Spin: fetching ${url}`);
      ...
      for (const listing of listings) {
        ...
        jobs.push(
          new JobPostDto({
            ...
            companyName: listing.company_name ?? 'Spin',
            // D-04: wire `absolute_url` flows through (variant 2).
            jobUrl:
              listing.absolute_url ??
              `https://job-boards.greenhouse.io/spin/jobs/${listing.id}`,
            location,
      ...
      this.logger.log(`Spin: scraped ${jobs.length} jobs`);
    } catch (err: any) {
      this.logger.error(`Spin scrape failed: ${err.message}`);
