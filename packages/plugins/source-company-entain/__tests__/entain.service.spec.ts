...
import { EntainModule, EntainService } from '../src';

const COMPANY_NAME_EXPECT = 'Entain';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'entain-jobs.json'), 'utf8'),
);
...
describe('EntainService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through EntainModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [EntainModule],
      }).compile();
      const service = moduleRef.get(EntainService);
      expect(service).toBeInstanceOf(EntainService);
      await moduleRef.close();
    });

    it('exports the Site.ENTAIN = "entain" enum value', () => {
      expect(Site.ENTAIN).toBe('entain');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new EntainService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.ENTAIN],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('Entain');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ENTAIN);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('entain-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new EntainService(registry);
      const result = await service.scrape({
        siteType: [Site.ENTAIN],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('Entain');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('entain-x1');
      expect(result.jobs[0].site).toBe(Site.ENTAIN);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('entain-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new EntainService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.ENTAIN],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new EntainService();
      const result = await service.scrape({
        siteType: [Site.ENTAIN],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
