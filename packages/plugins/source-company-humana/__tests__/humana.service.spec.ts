...
import { HumanaModule, HumanaService } from '../src';
...
const FIXTURE: Fixture = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'fixtures', 'humana-boards.json'), 'utf8'),
);
const COMPANY_NAME = 'Humana';
const ID_PREFIX = 'humana-';
...
describe('HumanaService — Workday delegation (Spec 1736)', () => {
  beforeEach(() => {
    mockGet.mockReset();
    mockPost.mockReset();
  ...
  describe('registration', () => {
    it('resolves through HumanaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({ imports: [HumanaModule] }).compile();
      expect(moduleRef.get(HumanaService)).toBeInstanceOf(HumanaService);
      await moduleRef.close();
    });

    it('exports Site.HUMANA = "humana"', () => {
      expect(Site.HUMANA).toBe('humana');
    });

    it('declares a tagged company plugin with its domains', () => {
      const meta = Reflect.getMetadata(SOURCE_PLUGIN_METADATA, HumanaService);
      expect(meta.site).toBe(Site.HUMANA);
      expect(meta.category).toBe('company');
      expect(meta.isAts).toBeFalsy();
      expect(meta.companyDomains).toEqual(['humana.com']);
  ...
  describe('recorded board (real Workday adapter, mocked HTTP)', () => {
    it('maps every recorded posting and re-stamps the company identity', async () => {
      const service = new HumanaService(registryWith());
      const result = await service.scrape({ siteType: [Site.HUMANA], resultsWanted: 100 } as ScraperInputDto);
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.HUMANA);
    ...
    it('only requests the recorded board URLs', async () => {
      const service = new HumanaService(registryWith());
      await service.scrape({ siteType: [Site.HUMANA] } as ScraperInputDto);
    ...
    it('honours resultsWanted=1', async () => {
      const service = new HumanaService(registryWith());
      const result = await service.scrape({ siteType: [Site.HUMANA], resultsWanted: 1 } as ScraperInputDto);
    ...
    it('resolves with an empty result when the board is gone (HTTP 404)', async () => {
      ...
      const service = new HumanaService(registryWith());
      const result = await service.scrape({ siteType: [Site.HUMANA] } as ScraperInputDto);
  ...
  describe('delegation contract', () => {
    it('forwards the board and the caller input untouched', async () => {
      ...
      const service = new HumanaService(
        registryWith(
          fakeBackend(
      ...
      expect(result.jobs[0].site).toBe(Site.HUMANA);
    ...
    it('never forwards the caller\'s credentials to the board (Spec 1735 §4.5)', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new HumanaService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
    ...
    it('makes no request when resultsWanted is 0', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new HumanaService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      const result = await service.scrape({ siteType: [Site.HUMANA], resultsWanted: 0 } as ScraperInputDto);
  ...
  describe('resilience', () => {
    it('reports not_registered when no registry is injected', async () => {
      const result = await new HumanaService().scrape({ siteType: [Site.HUMANA] } as ScraperInputDto);
      expect(result.jobs).toEqual([]);
    ...
    it('reports not_registered when Workday is missing from the registry', async () => {
      const result = await new HumanaService(new PluginRegistry()).scrape({
        siteType: [Site.HUMANA],
      } as ScraperInputDto);
      expect(result.diagnostics?.reason).toBe('not_registered');
    });
