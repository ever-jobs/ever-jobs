...
/**
 * Humana — Health insurance (HQ: Louisville, KY, USA).
 *
 * Source (Spec 1736): Workday board, scraped in this order:
 *   - `humana:5:Humana_External_Career_Site` — https://humana.wd5.myworkdayjobs.com/Humana_External_Career_Site
 *     verified live 2026-09-24: 381 open postings.
 *
 ...
 * party board.
 *
 * Tags: segment=workday-enterprise; industry=health-insurance.
 */
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'humana:5:Humana_External_Career_Site', atsIdPrefix: 'wd-humana-' },
];
...
@SourcePlugin({
  ...
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['humana.com'],
  description: 'Humana careers via Workday. Tags: segment=workday-enterprise; industry=health-insurance.',
})
@Injectable()
export class HumanaService implements IScraper {
  private readonly logger = new Logger(HumanaService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Workday source plugin is not registered; cannot scrape Humana');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`Humana: delegating to Workday (${board.companySlug})`);
    ...
    this.logger.log(`Humana: scraped ${jobs.length} jobs`);
