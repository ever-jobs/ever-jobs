...
import { AmericaninstitutesforresearchModule, AmericaninstitutesforresearchService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'americaninstitutesforresearch-jobs.json'), 'utf8'),
);
...
/**
 * Spec 262 / T04 — `AmericaninstitutesforresearchService` unit tests (≥ 9 cases).
 */
describe('AmericaninstitutesforresearchService — Spec 262 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AmericaninstitutesforresearchModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AmericaninstitutesforresearchModule],
      }).compile();
      const service = moduleRef.get(AmericaninstitutesforresearchService);
      expect(service).toBeInstanceOf(AmericaninstitutesforresearchService);
      await moduleRef.close();
    });

    it('exports the Site.AMERICANINSTITUTESFORRESEARCH = "americaninstitutesforresearch" enum value', () => {
      expect(Site.AMERICANINSTITUTESFORRESEARCH).toBe('americaninstitutesforresearch');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AmericaninstitutesforresearchService();
      const result = await service.scrape({
        siteType: [Site.AMERICANINSTITUTESFORRESEARCH],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'americaninstitutesforresearch-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.AMERICANINSTITUTESFORRESEARCH);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('American Institutes for Research');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/americaninstitutesforresearch/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/americaninstitutesforresearch/jobs/');
      ...
      expect(job0?.description).toContain('American Institutes for Research');
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('American Institutes for Research');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'americaninstitutesforresearch-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
