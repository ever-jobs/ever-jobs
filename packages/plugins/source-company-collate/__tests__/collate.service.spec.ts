...
import { CollateModule, CollateService } from '../src';

const COMPANY_NAME_EXPECT = 'Collate';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'collate-jobs.json'), 'utf8'),
);
...
describe('CollateService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CollateModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CollateModule],
      }).compile();
      const service = moduleRef.get(CollateService);
      expect(service).toBeInstanceOf(CollateService);
      await moduleRef.close();
    });

    it('exports the Site.COLLATE = "collate" enum value', () => {
      expect(Site.COLLATE).toBe('collate');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new CollateService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.COLLATE],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('collate');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.COLLATE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('collate-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new CollateService(registry);
      const result = await service.scrape({
        siteType: [Site.COLLATE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('collate');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('collate-x1');
      expect(result.jobs[0].site).toBe(Site.COLLATE);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('collate-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new CollateService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.COLLATE],
      } as ScraperInputDto);
