...
import { AbsciModule, AbsciService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'absci-jobs.json'), 'utf8'),
);
...
/**
 * Spec 756 / T04 — `AbsciService` unit tests (≥ 9 cases).
 */
describe('AbsciService — Spec 756 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AbsciModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AbsciModule],
      }).compile();
      const service = moduleRef.get(AbsciService);
      expect(service).toBeInstanceOf(AbsciService);
      await moduleRef.close();
    });

    it('exports the Site.ABSCI = "absci" enum value', () => {
      expect(Site.ABSCI).toBe('absci');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AbsciService();
      ...
      const job0 = dto.jobs.find((j) => j.id === 'absci-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ABSCI);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Absci');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/absci/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/absci/jobs/');
      ...
      expect(job0?.description).toContain('Absci');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/absci/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Absci');
      }
    });
  });
