...
import { QLFBrandsLampenlichtNlModule, QLFBrandsLampenlichtNlService } from '../src';

const COMPANY_NAME_EXPECT = 'QLF Brands (lampenlicht.nl)';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'qlfbrandslampenlichtnl-jobs.json'), 'utf8'),
);
...
describe('QLFBrandsLampenlichtNlService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through QLFBrandsLampenlichtNlModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [QLFBrandsLampenlichtNlModule],
      }).compile();
      const service = moduleRef.get(QLFBrandsLampenlichtNlService);
      expect(service).toBeInstanceOf(QLFBrandsLampenlichtNlService);
      await moduleRef.close();
    });

    it('exports the Site.QLF_BRANDS_LAMPENLICHT_NL = "qlfbrandslampenlichtnl" enum value', () => {
      expect(Site.QLF_BRANDS_LAMPENLICHT_NL).toBe('qlfbrandslampenlichtnl');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new QLFBrandsLampenlichtNlService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.QLF_BRANDS_LAMPENLICHT_NL],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'qlfbrandslampenlichtnl-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Recruitee's defaults
      expect(job0?.site).toBe(Site.QLF_BRANDS_LAMPENLICHT_NL);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('qlfbrandslampenlichtnl-' + first.id);
      ...
      expect(calledUrls[0]).toContain('qlfbrands.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.QLF_BRANDS_LAMPENLICHT_NL);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('qlfbrandslampenlichtnl-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new QLFBrandsLampenlichtNlService(registry);
      const result = await service.scrape({
        siteType: [Site.QLF_BRANDS_LAMPENLICHT_NL],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('qlfbrands');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('qlfbrandslampenlichtnl-x1');
      expect(result.jobs[0].site).toBe(Site.QLF_BRANDS_LAMPENLICHT_NL);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('qlfbrandslampenlichtnl-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new QLFBrandsLampenlichtNlService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.QLF_BRANDS_LAMPENLICHT_NL],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new QLFBrandsLampenlichtNlService();
      const result = await service.scrape({
        siteType: [Site.QLF_BRANDS_LAMPENLICHT_NL],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
