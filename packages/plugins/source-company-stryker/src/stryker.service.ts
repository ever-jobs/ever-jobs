...
/**
 * Stryker — Medical technology (HQ: Portage, MI, USA).
 *
 * Source (Spec 1736): Workday board, scraped in this order:
 *   - `stryker:1:StrykerCareers` — https://stryker.wd1.myworkdayjobs.com/StrykerCareers
 *     verified live 2026-09-24: 1,480 open postings.
 *
 ...
 * through untouched.
 *
 * Tags: segment=workday-enterprise; industry=medical-technology.
 */
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'stryker:1:StrykerCareers', atsIdPrefix: 'wd-stryker-' },
];
...
@SourcePlugin({
  ...
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['stryker.com'],
  description: 'Stryker careers via Workday. Tags: segment=workday-enterprise; industry=medical-technology.',
})
@Injectable()
export class StrykerService implements IScraper {
  private readonly logger = new Logger(StrykerService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Workday source plugin is not registered; cannot scrape Stryker');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`Stryker: delegating to Workday (${board.companySlug})`);
    ...
    this.logger.log(`Stryker: scraped ${jobs.length} jobs`);
