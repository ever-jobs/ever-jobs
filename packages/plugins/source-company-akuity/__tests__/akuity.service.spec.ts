...
import { AkuityModule, AkuityService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'akuity-jobs.json'), 'utf8'),
);
...
/**
 * Spec 224 / T04 — `AkuityService` unit tests (≥ 9 cases).
 */
describe('AkuityService — Spec 224 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AkuityModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AkuityModule],
      }).compile();
      const service = moduleRef.get(AkuityService);
      expect(service).toBeInstanceOf(AkuityService);
      await moduleRef.close();
    });

    it('exports the Site.AKUITY = "akuity" enum value', () => {
      expect(Site.AKUITY).toBe('akuity');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AkuityService();
      const result = await service.scrape({
        siteType: [Site.AKUITY],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'akuity-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.AKUITY);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Akuity');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/akuity/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/akuity/jobs/');
      ...
      expect(job0?.description).toContain('Akuity');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/akuity/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Akuity');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'akuity-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
