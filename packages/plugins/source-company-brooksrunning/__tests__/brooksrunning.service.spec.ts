...
import { BrooksRunningModule, BrooksRunningService } from '../src';

const COMPANY_NAME_EXPECT = 'Brooks Running';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'brooksrunning-jobs.json'), 'utf8'),
);
...
describe('BrooksRunningService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through BrooksRunningModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [BrooksRunningModule],
      }).compile();
      const service = moduleRef.get(BrooksRunningService);
      expect(service).toBeInstanceOf(BrooksRunningService);
      await moduleRef.close();
    });

    it('exports the Site.BROOKS_RUNNING = "brooksrunning" enum value', () => {
      expect(Site.BROOKS_RUNNING).toBe('brooksrunning');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new BrooksRunningService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.BROOKS_RUNNING],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'brooksrunning-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.BROOKS_RUNNING);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('brooksrunning-' + first.id);
      ...
      expect(calledUrls[0]).toContain('brooksrunning');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.BROOKS_RUNNING);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('brooksrunning-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new BrooksRunningService(registry);
      const result = await service.scrape({
        siteType: [Site.BROOKS_RUNNING],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('brooksrunning');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('brooksrunning-x1');
      expect(result.jobs[0].site).toBe(Site.BROOKS_RUNNING);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('brooksrunning-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new BrooksRunningService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.BROOKS_RUNNING],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new BrooksRunningService();
      const result = await service.scrape({
        siteType: [Site.BROOKS_RUNNING],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
