...
import { RebuyModule, RebuyService } from '../src';

const COMPANY_NAME_EXPECT = 'Rebuy';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'rebuy-jobs.json'), 'utf8'),
);
...
describe('RebuyService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through RebuyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [RebuyModule],
      }).compile();
      const service = moduleRef.get(RebuyService);
      expect(service).toBeInstanceOf(RebuyService);
      await moduleRef.close();
    });

    it('exports the Site.REBUY = "rebuy" enum value', () => {
      expect(Site.REBUY).toBe('rebuy');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new RebuyService(registryWithAshby());
      ...
      expect(calledUrls[0]).toContain('rebuy');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.REBUY);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('rebuy-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new RebuyService(registry);
      const result = await service.scrape({
        siteType: [Site.REBUY],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('rebuy');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('rebuy-x1');
      expect(result.jobs[0].site).toBe(Site.REBUY);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('rebuy-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new RebuyService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.REBUY],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new RebuyService();
      const result = await service.scrape({
        siteType: [Site.REBUY],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
