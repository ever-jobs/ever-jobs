...
import { StructubeModule, StructubeService } from '../src';

const COMPANY_NAME_EXPECT = 'Structube';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'structube-jobs.json'), 'utf8'),
);
...
describe('StructubeService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through StructubeModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [StructubeModule],
      }).compile();
      const service = moduleRef.get(StructubeService);
      expect(service).toBeInstanceOf(StructubeService);
      await moduleRef.close();
    });

    it('exports the Site.STRUCTUBE = "structube" enum value', () => {
      expect(Site.STRUCTUBE).toBe('structube');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new StructubeService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.STRUCTUBE],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('Structube1');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.STRUCTUBE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('structube-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new StructubeService(registry);
      const result = await service.scrape({
        siteType: [Site.STRUCTUBE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('Structube1');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('structube-x1');
      expect(result.jobs[0].site).toBe(Site.STRUCTUBE);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('structube-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new StructubeService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.STRUCTUBE],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new StructubeService();
      const result = await service.scrape({
        siteType: [Site.STRUCTUBE],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
