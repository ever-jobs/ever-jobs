...
import { GridCAREModule, GridCAREService } from '../src';

const COMPANY_NAME_EXPECT = 'GridCARE';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'gridcare-jobs.json'), 'utf8'),
);
...
describe('GridCAREService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through GridCAREModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [GridCAREModule],
      }).compile();
      const service = moduleRef.get(GridCAREService);
      expect(service).toBeInstanceOf(GridCAREService);
      await moduleRef.close();
    });

    it('exports the Site.GRIDCARE = "gridcare" enum value', () => {
      expect(Site.GRIDCARE).toBe('gridcare');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new GridCAREService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.GRIDCARE],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('gridcare');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.GRIDCARE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('gridcare-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new GridCAREService(registry);
      const result = await service.scrape({
        siteType: [Site.GRIDCARE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('gridcare');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('gridcare-x1');
      expect(result.jobs[0].site).toBe(Site.GRIDCARE);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('gridcare-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new GridCAREService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.GRIDCARE],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new GridCAREService();
      const result = await service.scrape({
        siteType: [Site.GRIDCARE],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
