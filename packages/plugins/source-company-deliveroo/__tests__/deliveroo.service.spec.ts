...
import { DeliverooModule, DeliverooService } from '../src';

const COMPANY_NAME_EXPECT = 'Deliveroo';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'deliveroo-jobs.json'), 'utf8'),
);
...
describe('DeliverooService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through DeliverooModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [DeliverooModule],
      }).compile();
      const service = moduleRef.get(DeliverooService);
      expect(service).toBeInstanceOf(DeliverooService);
      await moduleRef.close();
    });

    it('exports the Site.DELIVEROO = "deliveroo" enum value', () => {
      expect(Site.DELIVEROO).toBe('deliveroo');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new DeliverooService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.DELIVEROO],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('deliveroo');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.DELIVEROO);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('deliveroo-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new DeliverooService(registry);
      const result = await service.scrape({
        siteType: [Site.DELIVEROO],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('deliveroo');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('deliveroo-x1');
      expect(result.jobs[0].site).toBe(Site.DELIVEROO);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('deliveroo-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new DeliverooService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.DELIVEROO],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new DeliverooService();
      const result = await service.scrape({
        siteType: [Site.DELIVEROO],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
