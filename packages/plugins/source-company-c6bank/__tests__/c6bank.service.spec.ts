...
import { C6BankModule, C6BankService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'c6bank-jobs.json'), 'utf8'),
);
...
/**
 * Spec 647 / T04 — `C6BankService` unit tests (≥ 9 cases).
 */
describe('C6BankService — Spec 647 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through C6BankModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [C6BankModule],
      }).compile();
      const service = moduleRef.get(C6BankService);
      expect(service).toBeInstanceOf(C6BankService);
      await moduleRef.close();
    });

    it('exports the Site.C6_BANK = "c6bank" enum value', () => {
      expect(Site.C6_BANK).toBe('c6bank');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new C6BankService();
      const result = await service.scrape({
        siteType: [Site.C6_BANK],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'c6bank-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.C6_BANK);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('C6 Bank');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/c6bank/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/c6bank/jobs/');
      ...
      expect(job0?.description).toContain('C6 Bank');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/c6bank/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('C6 Bank');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'c6bank-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
