...
import { SwapModule, SwapService } from '../src';

const COMPANY_NAME_EXPECT = 'Swap';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'swap-jobs.json'), 'utf8'),
);
...
describe('SwapService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SwapModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SwapModule],
      }).compile();
      const service = moduleRef.get(SwapService);
      expect(service).toBeInstanceOf(SwapService);
      await moduleRef.close();
    });

    it('exports the Site.SWAP = "swap" enum value', () => {
      expect(Site.SWAP).toBe('swap');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new SwapService(registryWithAshby());
      ...
      expect(calledUrls[0]).toContain('swap');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SWAP);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('swap-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new SwapService(registry);
      const result = await service.scrape({
        siteType: [Site.SWAP],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('swap');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('swap-x1');
      expect(result.jobs[0].site).toBe(Site.SWAP);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('swap-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new SwapService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.SWAP],
      } as ScraperInputDto);
