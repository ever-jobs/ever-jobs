...
import { CargomaticModule, CargomaticService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'cargomatic-jobs.json'), 'utf8'),
);
...
/**
 * Spec 822 / T04 — `CargomaticService` unit tests (≥ 9 cases).
 */
describe('CargomaticService — Spec 822 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CargomaticModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CargomaticModule],
      }).compile();
      const service = moduleRef.get(CargomaticService);
      expect(service).toBeInstanceOf(CargomaticService);
      await moduleRef.close();
    });

    it('exports the Site.CARGOMATIC = "cargomatic" enum value', () => {
      expect(Site.CARGOMATIC).toBe('cargomatic');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new CargomaticService();
      const result = await service.scrape({
        siteType: [Site.CARGOMATIC],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'cargomatic-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.CARGOMATIC);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Cargomatic');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/cargomatic/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/cargomatic/jobs/');
      ...
      expect(job0?.description).toContain('Cargomatic');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/cargomatic/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Cargomatic');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'cargomatic-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
