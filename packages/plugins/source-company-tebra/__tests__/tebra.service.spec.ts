...
import { TebraModule, TebraService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'tebra-jobs.json'), 'utf8'),
);
...
/**
 * Spec 948 / T04 — `TebraService` unit tests (≥ 9 cases).
 */
describe('TebraService — Spec 948 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through TebraModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [TebraModule],
      }).compile();
      const service = moduleRef.get(TebraService);
      expect(service).toBeInstanceOf(TebraService);
      await moduleRef.close();
    });

    it('exports the Site.TEBRA = "tebra" enum value', () => {
      expect(Site.TEBRA).toBe('tebra');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new TebraService();
      ...
      const job0 = dto.jobs.find((j) => j.id === 'tebra-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.TEBRA);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Tebra');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/tebra/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/tebra/jobs/');
      ...
      expect(job0?.description).toContain('Tebra');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/tebra/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Tebra');
      }
    });
  });
