...
import { CyderesModule, CyderesService } from '../src';

const COMPANY_NAME_EXPECT = 'Cyderes';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'cyderes-jobs.json'), 'utf8'),
);
...
describe('CyderesService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CyderesModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CyderesModule],
      }).compile();
      const service = moduleRef.get(CyderesService);
      expect(service).toBeInstanceOf(CyderesService);
      await moduleRef.close();
    });

    it('exports the Site.CYDERES = "cyderes" enum value', () => {
      expect(Site.CYDERES).toBe('cyderes');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new CyderesService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.CYDERES],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('cyderes');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.CYDERES);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('cyderes-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new CyderesService(registry);
      const result = await service.scrape({
        siteType: [Site.CYDERES],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('cyderes');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('cyderes-x1');
      expect(result.jobs[0].site).toBe(Site.CYDERES);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('cyderes-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new CyderesService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.CYDERES],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new CyderesService();
      const result = await service.scrape({
        siteType: [Site.CYDERES],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
