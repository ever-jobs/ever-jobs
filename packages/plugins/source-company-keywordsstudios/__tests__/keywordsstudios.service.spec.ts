...
import { KeywordsStudiosModule, KeywordsStudiosService } from '../src';

const COMPANY_NAME_EXPECT = 'Keywords Studios';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'keywordsstudios-jobs.json'), 'utf8'),
);
...
describe('KeywordsStudiosService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through KeywordsStudiosModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [KeywordsStudiosModule],
      }).compile();
      const service = moduleRef.get(KeywordsStudiosService);
      expect(service).toBeInstanceOf(KeywordsStudiosService);
      await moduleRef.close();
    });

    it('exports the Site.KEYWORDS_STUDIOS = "keywordsstudios" enum value', () => {
      expect(Site.KEYWORDS_STUDIOS).toBe('keywordsstudios');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new KeywordsStudiosService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.KEYWORDS_STUDIOS],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'keywordsstudios-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.KEYWORDS_STUDIOS);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('keywordsstudios-' + first.id);
      ...
      expect(calledUrls[0]).toContain('KeywordsStudios');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.KEYWORDS_STUDIOS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('keywordsstudios-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new KeywordsStudiosService(registry);
      const result = await service.scrape({
        siteType: [Site.KEYWORDS_STUDIOS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('KeywordsStudios');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('keywordsstudios-x1');
      expect(result.jobs[0].site).toBe(Site.KEYWORDS_STUDIOS);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('keywordsstudios-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new KeywordsStudiosService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.KEYWORDS_STUDIOS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new KeywordsStudiosService();
      const result = await service.scrape({
        siteType: [Site.KEYWORDS_STUDIOS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
