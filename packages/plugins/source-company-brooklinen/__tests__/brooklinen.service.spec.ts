...
import { BrooklinenModule, BrooklinenService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'brooklinen-jobs.json'), 'utf8'),
);
...
/**
 * Spec 570 / T04 — `BrooklinenService` unit tests (≥ 9 cases).
 */
describe('BrooklinenService — Spec 570 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through BrooklinenModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [BrooklinenModule],
      }).compile();
      const service = moduleRef.get(BrooklinenService);
      expect(service).toBeInstanceOf(BrooklinenService);
      await moduleRef.close();
    });

    it('exports the Site.BROOKLINEN = "brooklinen" enum value', () => {
      expect(Site.BROOKLINEN).toBe('brooklinen');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new BrooklinenService();
      const result = await service.scrape({
        siteType: [Site.BROOKLINEN],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'brooklinen-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.BROOKLINEN);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Brooklinen');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/brooklinen/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/brooklinen/jobs/');
      ...
      expect(job0?.description).toContain('Brooklinen');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/brooklinen/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Brooklinen');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'brooklinen-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
