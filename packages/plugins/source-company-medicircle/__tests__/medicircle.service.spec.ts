...
import { MediCircleModule, MediCircleService } from '../src';

const COMPANY_NAME_EXPECT = 'MediCircle';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'medicircle-jobs.json'), 'utf8'),
);
...
describe('MediCircleService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through MediCircleModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [MediCircleModule],
      }).compile();
      const service = moduleRef.get(MediCircleService);
      expect(service).toBeInstanceOf(MediCircleService);
      await moduleRef.close();
    });

    it('exports the Site.MEDICIRCLE = "medicircle" enum value', () => {
      expect(Site.MEDICIRCLE).toBe('medicircle');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new MediCircleService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.MEDICIRCLE],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('medicircle');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.MEDICIRCLE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('medicircle-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new MediCircleService(registry);
      const result = await service.scrape({
        siteType: [Site.MEDICIRCLE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('medicircle');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('medicircle-x1');
      expect(result.jobs[0].site).toBe(Site.MEDICIRCLE);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('medicircle-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new MediCircleService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.MEDICIRCLE],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new MediCircleService();
      const result = await service.scrape({
        siteType: [Site.MEDICIRCLE],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
