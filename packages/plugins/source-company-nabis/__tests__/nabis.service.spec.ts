...
import { NabisModule, NabisService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'nabis-jobs.json'), 'utf8'),
);
...
/**
 * Spec 888 / T04 — `NabisService` unit tests (≥ 9 cases).
 */
describe('NabisService — Spec 888 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through NabisModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [NabisModule],
      }).compile();
      const service = moduleRef.get(NabisService);
      expect(service).toBeInstanceOf(NabisService);
      await moduleRef.close();
    });

    it('exports the Site.NABIS = "nabis" enum value', () => {
      expect(Site.NABIS).toBe('nabis');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new NabisService();
      ...
      const job0 = dto.jobs.find((j) => j.id === 'nabis-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.NABIS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Nabis');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/nabis/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/nabis/jobs/');
      ...
      expect(job0?.description).toContain('Nabis');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/nabis/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Nabis');
      }
    });
  });
