...
import { FaradayFutureModule, FaradayFutureService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'faradayfuture-jobs.json'), 'utf8'),
);
...
/**
 * Spec 792 / T04 — `FaradayFutureService` unit tests (≥ 9 cases).
 */
describe('FaradayFutureService — Spec 792 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through FaradayFutureModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [FaradayFutureModule],
      }).compile();
      const service = moduleRef.get(FaradayFutureService);
      expect(service).toBeInstanceOf(FaradayFutureService);
      await moduleRef.close();
    });

    it('exports the Site.FARADAY_FUTURE = "faradayfuture" enum value', () => {
      expect(Site.FARADAY_FUTURE).toBe('faradayfuture');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new FaradayFutureService();
      const result = await service.scrape({
        siteType: [Site.FARADAY_FUTURE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'faradayfuture-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.FARADAY_FUTURE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Faraday Future');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/faradayfuture/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/faradayfuture/jobs/');
      ...
      expect(job0?.description).toContain('Faraday Future');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/faradayfuture/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Faraday Future');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'faradayfuture-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
