...
import { AkkoModule, AkkoService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'akko-jobs.json'), 'utf8'),
);
...
/**
 * Spec 222 / T04 — `AkkoService` unit tests (≥ 9 cases).
 */
describe('AkkoService — Spec 222 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AkkoModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AkkoModule],
      }).compile();
      const service = moduleRef.get(AkkoService);
      expect(service).toBeInstanceOf(AkkoService);
      await moduleRef.close();
    });

    it('exports the Site.AKKO = "akko" enum value', () => {
      expect(Site.AKKO).toBe('akko');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const job0 = dto.jobs.find((j) => j.id === 'akko-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.AKKO);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('AKKO');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/akko/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/akko/jobs/');
      ...
      expect(job0?.description).toContain('AKKO');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/akko/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('AKKO');
      }
    });
  });
