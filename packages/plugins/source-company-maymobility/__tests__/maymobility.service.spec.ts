...
import { MayMobilityModule, MayMobilityService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'maymobility-jobs.json'), 'utf8'),
);
...
/**
 * Spec 882 / T04 — `MayMobilityService` unit tests (≥ 9 cases).
 */
describe('MayMobilityService — Spec 882 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through MayMobilityModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [MayMobilityModule],
      }).compile();
      const service = moduleRef.get(MayMobilityService);
      expect(service).toBeInstanceOf(MayMobilityService);
      await moduleRef.close();
    });

    it('exports the Site.MAY_MOBILITY = "maymobility" enum value', () => {
      expect(Site.MAY_MOBILITY).toBe('maymobility');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new MayMobilityService();
      const result = await service.scrape({
        siteType: [Site.MAY_MOBILITY],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'maymobility-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.MAY_MOBILITY);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('May Mobility');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/maymobility/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/maymobility/jobs/');
      ...
      expect(job0?.description).toContain('May Mobility');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/maymobility/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('May Mobility');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'maymobility-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
