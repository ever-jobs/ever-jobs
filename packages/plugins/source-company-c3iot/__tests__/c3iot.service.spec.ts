...
import { C3AIModule, C3AIService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'c3iot-jobs.json'), 'utf8'),
);
...
/**
 * Spec 820 / T04 — `C3AIService` unit tests (≥ 9 cases).
 */
describe('C3AIService — Spec 820 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through C3AIModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [C3AIModule],
      }).compile();
      const service = moduleRef.get(C3AIService);
      expect(service).toBeInstanceOf(C3AIService);
      await moduleRef.close();
    });

    it('exports the Site.C3_AI = "c3iot" enum value', () => {
      expect(Site.C3_AI).toBe('c3iot');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const job0 = dto.jobs.find((j) => j.id === 'c3iot-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.C3_AI);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('C3 AI');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/c3iot/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/c3iot/jobs/');
      ...
      expect(job0?.description).toContain('C3 AI');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/c3iot/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('C3 AI');
      }
    });
  });
