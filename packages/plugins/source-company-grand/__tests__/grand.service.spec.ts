...
import { GrandGamesModule, GrandGamesService } from '../src';

const COMPANY_NAME_EXPECT = 'Grand Games';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'grand-jobs.json'), 'utf8'),
);
...
describe('GrandGamesService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through GrandGamesModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [GrandGamesModule],
      }).compile();
      const service = moduleRef.get(GrandGamesService);
      expect(service).toBeInstanceOf(GrandGamesService);
      await moduleRef.close();
    });

    it('exports the Site.GRAND_GAMES = "grand" enum value', () => {
      expect(Site.GRAND_GAMES).toBe('grand');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new GrandGamesService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.GRAND_GAMES],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('grand');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.GRAND_GAMES);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('grand-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new GrandGamesService(registry);
      const result = await service.scrape({
        siteType: [Site.GRAND_GAMES],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('grand');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('grand-x1');
      expect(result.jobs[0].site).toBe(Site.GRAND_GAMES);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('grand-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new GrandGamesService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.GRAND_GAMES],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new GrandGamesService();
      const result = await service.scrape({
        siteType: [Site.GRAND_GAMES],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
