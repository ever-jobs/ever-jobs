...
import { AzumutaModule, AzumutaService } from '../src';

const COMPANY_NAME_EXPECT = 'Azumuta';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'azumuta-jobs.json'), 'utf8'),
);
...
describe('AzumutaService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AzumutaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AzumutaModule],
      }).compile();
      const service = moduleRef.get(AzumutaService);
      expect(service).toBeInstanceOf(AzumutaService);
      await moduleRef.close();
    });

    it('exports the Site.AZUMUTA = "azumuta" enum value', () => {
      expect(Site.AZUMUTA).toBe('azumuta');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new AzumutaService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.AZUMUTA],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('azumuta.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.AZUMUTA);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('azumuta-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new AzumutaService(registry);
      const result = await service.scrape({
        siteType: [Site.AZUMUTA],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('azumuta');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('azumuta-x1');
      expect(result.jobs[0].site).toBe(Site.AZUMUTA);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('azumuta-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new AzumutaService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.AZUMUTA],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new AzumutaService();
      const result = await service.scrape({
        siteType: [Site.AZUMUTA],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
