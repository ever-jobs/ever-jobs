...
import { Healthtech1Module, Healthtech1Service } from '../src';

const COMPANY_NAME_EXPECT = 'Healthtech-1';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'healthtech1-jobs.json'), 'utf8'),
);
...
describe('Healthtech1Service — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through Healthtech1Module via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [Healthtech1Module],
      }).compile();
      const service = moduleRef.get(Healthtech1Service);
      expect(service).toBeInstanceOf(Healthtech1Service);
      await moduleRef.close();
    });

    it('exports the Site.HEALTHTECH_1 = "healthtech1" enum value', () => {
      expect(Site.HEALTHTECH_1).toBe('healthtech1');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new Healthtech1Service(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.HEALTHTECH_1],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'healthtech1-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.HEALTHTECH_1);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('healthtech1-' + first.id);
      ...
      expect(calledUrls[0]).toContain('healthtech-1');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.HEALTHTECH_1);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('healthtech1-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new Healthtech1Service(registry);
      const result = await service.scrape({
        siteType: [Site.HEALTHTECH_1],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('healthtech-1');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('healthtech1-x1');
      expect(result.jobs[0].site).toBe(Site.HEALTHTECH_1);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('healthtech1-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new Healthtech1Service(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.HEALTHTECH_1],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new Healthtech1Service();
      const result = await service.scrape({
        siteType: [Site.HEALTHTECH_1],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
