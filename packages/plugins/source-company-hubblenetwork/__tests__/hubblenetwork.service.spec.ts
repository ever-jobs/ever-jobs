...
import { HubbleNetworkModule, HubbleNetworkService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'hubblenetwork-jobs.json'), 'utf8'),
);
...
/**
 * Spec 764 / T04 — `HubbleNetworkService` unit tests (≥ 9 cases).
 */
describe('HubbleNetworkService — Spec 764 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through HubbleNetworkModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [HubbleNetworkModule],
      }).compile();
      const service = moduleRef.get(HubbleNetworkService);
      expect(service).toBeInstanceOf(HubbleNetworkService);
      await moduleRef.close();
    });

    it('exports the Site.HUBBLE_NETWORK = "hubblenetwork" enum value', () => {
      expect(Site.HUBBLE_NETWORK).toBe('hubblenetwork');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new HubbleNetworkService();
      ...
      const job0 = dto.jobs.find((j) => j.id === 'hubblenetwork-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.HUBBLE_NETWORK);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Hubble Network');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/hubblenetwork/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/hubblenetwork/jobs/');
      ...
      expect(job0?.description).toContain('Hubble Network');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/hubblenetwork/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Hubble Network');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'hubblenetwork-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
