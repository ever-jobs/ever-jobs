...
import { CharterUPModule, CharterUPService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'charterup-jobs.json'), 'utf8'),
);
...
/**
 * Spec 824 / T04 — `CharterUPService` unit tests (≥ 9 cases).
 */
describe('CharterUPService — Spec 824 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CharterUPModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CharterUPModule],
      }).compile();
      const service = moduleRef.get(CharterUPService);
      expect(service).toBeInstanceOf(CharterUPService);
      await moduleRef.close();
    });

    it('exports the Site.CHARTERUP = "charterup" enum value', () => {
      expect(Site.CHARTERUP).toBe('charterup');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new CharterUPService();
      const result = await service.scrape({
        siteType: [Site.CHARTERUP],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'charterup-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.CHARTERUP);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('CharterUP');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/charterup/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/charterup/jobs/');
      ...
      expect(job0?.description).toContain('CharterUP');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/charterup/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('CharterUP');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'charterup-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
