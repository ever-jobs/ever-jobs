...
import { CIMAModule, CIMAService } from '../src';

const COMPANY_NAME_EXPECT = 'CIMA+';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'cima-jobs.json'), 'utf8'),
);
...
describe('CIMAService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CIMAModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CIMAModule],
      }).compile();
      const service = moduleRef.get(CIMAService);
      expect(service).toBeInstanceOf(CIMAService);
      await moduleRef.close();
    });

    it('exports the Site.CIMA = "cima" enum value', () => {
      expect(Site.CIMA).toBe('cima');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new CIMAService(registryWithSmartRecruiters());
      ...
      expect(calledUrls[0]).toContain('CIMA2');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.CIMA);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('cima-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new CIMAService(registry);
      const result = await service.scrape({
        siteType: [Site.CIMA],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('CIMA2');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('cima-x1');
      expect(result.jobs[0].site).toBe(Site.CIMA);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('cima-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new CIMAService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.CIMA],
      } as ScraperInputDto);
