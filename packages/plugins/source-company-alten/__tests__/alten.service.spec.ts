...
import { ALTENModule, ALTENService } from '../src';

const COMPANY_NAME_EXPECT = 'ALTEN';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'alten-jobs.json'), 'utf8'),
);
...
describe('ALTENService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ALTENModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ALTENModule],
      }).compile();
      const service = moduleRef.get(ALTENService);
      expect(service).toBeInstanceOf(ALTENService);
      await moduleRef.close();
    });

    it('exports the Site.ALTEN = "alten" enum value', () => {
      expect(Site.ALTEN).toBe('alten');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new ALTENService(registryWithSmartRecruiters());
      ...
      expect(calledUrls[0]).toContain('ALTEN');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ALTEN);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('alten-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new ALTENService(registry);
      const result = await service.scrape({
        siteType: [Site.ALTEN],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('ALTEN');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('alten-x1');
      expect(result.jobs[0].site).toBe(Site.ALTEN);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('alten-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new ALTENService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.ALTEN],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new ALTENService();
      const result = await service.scrape({
        siteType: [Site.ALTEN],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
