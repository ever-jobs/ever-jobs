...
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'aperiasolutions-jobs.json'), 'utf8'),
);
...
/**
 * Spec 280 / T04 — `AperiasolutionsService` unit tests (≥ 9 cases).
 */
describe('AperiasolutionsService — Spec 280 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AperiasolutionsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AperiasolutionsModule],
      }).compile();
      const service = moduleRef.get(AperiasolutionsService);
      expect(service).toBeInstanceOf(AperiasolutionsService);
      await moduleRef.close();
    });

    it('exports the Site.APERIASOLUTIONS = "aperiasolutions" enum value', () => {
      expect(Site.APERIASOLUTIONS).toBe('aperiasolutions');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AperiasolutionsService();
      const result = await service.scrape({
        siteType: [Site.APERIASOLUTIONS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'aperiasolutions-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.APERIASOLUTIONS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Aperia');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/aperiasolutions/jobs/' + first.id,
      );
      ...
      expect(job0?.description).toContain('Aperia');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/aperiasolutions/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Aperia');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'aperiasolutions-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
