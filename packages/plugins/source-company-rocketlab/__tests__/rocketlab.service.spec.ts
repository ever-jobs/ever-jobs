...
import { RocketLabModule, RocketLabService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'rocketlab-jobs.json'), 'utf8'),
);
...
/**
 * Spec 630 / T04 — `RocketLabService` unit tests (≥ 9 cases).
 */
describe('RocketLabService — Spec 630 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through RocketLabModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [RocketLabModule],
      }).compile();
      const service = moduleRef.get(RocketLabService);
      expect(service).toBeInstanceOf(RocketLabService);
      await moduleRef.close();
    });

    it('exports the Site.ROCKET_LAB = "rocketlab" enum value', () => {
      expect(Site.ROCKET_LAB).toBe('rocketlab');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new RocketLabService();
      const result = await service.scrape({
        siteType: [Site.ROCKET_LAB],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'rocketlab-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ROCKET_LAB);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Rocket Lab');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/rocketlab/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/rocketlab/jobs/');
      ...
      expect(job0?.description).toContain('Rocket Lab');
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Rocket Lab');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'rocketlab-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
