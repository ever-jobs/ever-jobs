...
import { RedoxModule, RedoxService } from '../src';

const COMPANY_NAME_EXPECT = 'Redox';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'redoxengine-jobs.json'), 'utf8'),
);
...
describe('RedoxService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through RedoxModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [RedoxModule],
      }).compile();
      const service = moduleRef.get(RedoxService);
      expect(service).toBeInstanceOf(RedoxService);
      await moduleRef.close();
    });

    it('exports the Site.REDOX = "redoxengine" enum value', () => {
      expect(Site.REDOX).toBe('redoxengine');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new RedoxService(registryWithLever());
      ...
      expect(calledUrls[0]).toContain('redoxengine');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.REDOX);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('redoxengine-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new RedoxService(registry);
      const result = await service.scrape({
        siteType: [Site.REDOX],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('redoxengine');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('redoxengine-x1');
      expect(result.jobs[0].site).toBe(Site.REDOX);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('redoxengine-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new RedoxService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.REDOX],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new RedoxService();
      const result = await service.scrape({
        siteType: [Site.REDOX],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
