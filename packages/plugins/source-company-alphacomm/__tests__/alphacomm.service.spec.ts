...
import { AlphacommModule, AlphacommService } from '../src';

const COMPANY_NAME_EXPECT = 'Alphacomm';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'alphacomm-jobs.json'), 'utf8'),
);
...
describe('AlphacommService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AlphacommModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AlphacommModule],
      }).compile();
      const service = moduleRef.get(AlphacommService);
      expect(service).toBeInstanceOf(AlphacommService);
      await moduleRef.close();
    });

    it('exports the Site.ALPHACOMM = "alphacomm" enum value', () => {
      expect(Site.ALPHACOMM).toBe('alphacomm');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new AlphacommService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.ALPHACOMM],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('alphacomm.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ALPHACOMM);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('alphacomm-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new AlphacommService(registry);
      const result = await service.scrape({
        siteType: [Site.ALPHACOMM],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('alphacomm');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('alphacomm-x1');
      expect(result.jobs[0].site).toBe(Site.ALPHACOMM);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('alphacomm-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new AlphacommService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.ALPHACOMM],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new AlphacommService();
      const result = await service.scrape({
        siteType: [Site.ALPHACOMM],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
