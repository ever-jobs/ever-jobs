...
import { BasetenModule, BasetenService } from '../src';

const COMPANY_NAME_EXPECT = 'Baseten';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'baseten-jobs.json'), 'utf8'),
);
...
describe('BasetenService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through BasetenModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [BasetenModule],
      }).compile();
      const service = moduleRef.get(BasetenService);
      expect(service).toBeInstanceOf(BasetenService);
      await moduleRef.close();
    });

    it('exports the Site.BASETEN = "baseten" enum value', () => {
      expect(Site.BASETEN).toBe('baseten');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new BasetenService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.BASETEN],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('baseten');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.BASETEN);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('baseten-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new BasetenService(registry);
      const result = await service.scrape({
        siteType: [Site.BASETEN],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('baseten');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('baseten-x1');
      expect(result.jobs[0].site).toBe(Site.BASETEN);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('baseten-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new BasetenService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.BASETEN],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new BasetenService();
      const result = await service.scrape({
        siteType: [Site.BASETEN],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
