...
import { MaticInsuranceModule, MaticInsuranceService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'matic-jobs.json'), 'utf8'),
);
...
/**
 * Spec 881 / T04 — `MaticInsuranceService` unit tests (≥ 9 cases).
 */
describe('MaticInsuranceService — Spec 881 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through MaticInsuranceModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [MaticInsuranceModule],
      }).compile();
      const service = moduleRef.get(MaticInsuranceService);
      expect(service).toBeInstanceOf(MaticInsuranceService);
      await moduleRef.close();
    });

    it('exports the Site.MATIC_INSURANCE = "matic" enum value', () => {
      expect(Site.MATIC_INSURANCE).toBe('matic');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new MaticInsuranceService();
      const result = await service.scrape({
        siteType: [Site.MATIC_INSURANCE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'matic-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.MATIC_INSURANCE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Matic Insurance');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/matic/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/matic/jobs/');
      ...
      expect(job0?.description).toContain('Matic Insurance');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/matic/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Matic Insurance');
      }
    });
  });
