...
import { RelativityModule, RelativityService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'relativity-jobs.json'), 'utf8'),
);
...
/**
 * Spec 458 / T04 — `RelativityService` unit tests (≥ 9 cases).
 */
describe('RelativityService — Spec 458 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through RelativityModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [RelativityModule],
      }).compile();
      const service = moduleRef.get(RelativityService);
      expect(service).toBeInstanceOf(RelativityService);
      await moduleRef.close();
    });

    it('exports the Site.RELATIVITY = "relativity" enum value', () => {
      expect(Site.RELATIVITY).toBe('relativity');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new RelativityService();
      const result = await service.scrape({
        siteType: [Site.RELATIVITY],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'relativity-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.RELATIVITY);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Relativity Space');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/relativity/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/relativity/jobs/');
      ...
      expect(job0?.description).toContain('Relativity Space');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/relativity/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Relativity Space');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'relativity-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
