...
import { GlassHealthModule, GlassHealthService } from '../src';

const COMPANY_NAME_EXPECT = 'Glass Health';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'glasshealthinc-jobs.json'), 'utf8'),
);
...
describe('GlassHealthService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through GlassHealthModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [GlassHealthModule],
      }).compile();
      const service = moduleRef.get(GlassHealthService);
      expect(service).toBeInstanceOf(GlassHealthService);
      await moduleRef.close();
    });

    it('exports the Site.GLASS_HEALTH = "glasshealthinc" enum value', () => {
      expect(Site.GLASS_HEALTH).toBe('glasshealthinc');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new GlassHealthService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.GLASS_HEALTH],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'glasshealthinc-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.GLASS_HEALTH);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('glasshealthinc-' + first.id);
      ...
      expect(calledUrls[0]).toContain('glass-health-inc');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.GLASS_HEALTH);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('glasshealthinc-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new GlassHealthService(registry);
      const result = await service.scrape({
        siteType: [Site.GLASS_HEALTH],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('glass-health-inc');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('glasshealthinc-x1');
      expect(result.jobs[0].site).toBe(Site.GLASS_HEALTH);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('glasshealthinc-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new GlassHealthService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.GLASS_HEALTH],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new GlassHealthService();
      const result = await service.scrape({
        siteType: [Site.GLASS_HEALTH],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
