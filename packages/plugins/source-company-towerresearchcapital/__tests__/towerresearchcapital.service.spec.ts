...
import { TowerResearchCapitalModule, TowerResearchCapitalService } from '../src';
...
const FIXTURE: Fixture = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'fixtures', 'towerresearchcapital-boards.json'), 'utf8'),
);
const COMPANY_NAME = 'Tower Research Capital';
const ID_PREFIX = 'towerresearchcapital-';
...
describe('TowerResearchCapitalService — Greenhouse delegation (Spec 1737)', () => {
  beforeEach(() => {
    mockGet.mockReset();
    mockPost.mockReset();
  ...
  describe('registration', () => {
    it('resolves through TowerResearchCapitalModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({ imports: [TowerResearchCapitalModule] }).compile();
      expect(moduleRef.get(TowerResearchCapitalService)).toBeInstanceOf(TowerResearchCapitalService);
      await moduleRef.close();
    });

    it('exports Site.TOWER_RESEARCH = "towerresearchcapital"', () => {
      expect(Site.TOWER_RESEARCH).toBe('towerresearchcapital');
    });

    it('declares a tagged company plugin with its domains', () => {
      const meta = Reflect.getMetadata(SOURCE_PLUGIN_METADATA, TowerResearchCapitalService);
      expect(meta.site).toBe(Site.TOWER_RESEARCH);
      expect(meta.category).toBe('company');
      expect(meta.isAts).toBeFalsy();
      expect(meta.companyDomains).toEqual(['tower-research.com']);
  ...
  describe('recorded board (real Greenhouse adapter, mocked HTTP)', () => {
    it('maps every recorded posting and re-stamps the company identity', async () => {
      const service = new TowerResearchCapitalService(registryWith());
      const result = await service.scrape({ siteType: [Site.TOWER_RESEARCH], resultsWanted: 100 } as ScraperInputDto);
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.TOWER_RESEARCH);
    ...
    it('only requests the recorded board URLs', async () => {
      const service = new TowerResearchCapitalService(registryWith());
      await service.scrape({ siteType: [Site.TOWER_RESEARCH] } as ScraperInputDto);
    ...
    it('honours resultsWanted=1', async () => {
      const service = new TowerResearchCapitalService(registryWith());
      const result = await service.scrape({ siteType: [Site.TOWER_RESEARCH], resultsWanted: 1 } as ScraperInputDto);
    ...
    it('resolves with an empty result when the board is gone (HTTP 404)', async () => {
      ...
      const service = new TowerResearchCapitalService(registryWith());
      const result = await service.scrape({ siteType: [Site.TOWER_RESEARCH] } as ScraperInputDto);
  ...
  describe('delegation contract', () => {
    it('forwards the board and the caller input untouched', async () => {
      ...
      const service = new TowerResearchCapitalService(
        registryWith(
          fakeBackend(
      ...
      expect(result.jobs[0].site).toBe(Site.TOWER_RESEARCH);
      expect(result.jobs[0].companyName).toBe(COMPANY_NAME);
    });

    it('passes an absent resultsWanted through as absent', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new TowerResearchCapitalService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      await service.scrape({ siteType: [Site.TOWER_RESEARCH] } as ScraperInputDto);
    ...
    it('makes no request when resultsWanted is 0', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new TowerResearchCapitalService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      const result = await service.scrape({ siteType: [Site.TOWER_RESEARCH], resultsWanted: 0 } as ScraperInputDto);
  ...
  describe('resilience', () => {
    it('reports not_registered when no registry is injected', async () => {
      const result = await new TowerResearchCapitalService().scrape({ siteType: [Site.TOWER_RESEARCH] } as ScraperInputDto);
      expect(result.jobs).toEqual([]);
    ...
    it('reports not_registered when Greenhouse is missing from the registry', async () => {
      const result = await new TowerResearchCapitalService(new PluginRegistry()).scrape({
        siteType: [Site.TOWER_RESEARCH],
      } as ScraperInputDto);
      expect(result.diagnostics?.reason).toBe('not_registered');
    });
