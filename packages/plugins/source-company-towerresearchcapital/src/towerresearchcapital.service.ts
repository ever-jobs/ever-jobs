...
/**
 * Tower Research Capital — Quantitative trading (HQ: New York, NY, USA).
 *
 * Source (Spec 1737): Greenhouse board, scraped in this order:
 *   - `towerresearchcapital` — https://job-boards.greenhouse.io/towerresearchcapital
 *     verified live 2026-09-24: 91 open postings.
 *
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'towerresearchcapital', atsIdPrefix: 'gh-' },
];

@SourcePlugin({
  site: Site.TOWER_RESEARCH,
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['tower-research.com'],
  description: 'Tower Research Capital careers via Greenhouse. Tags: segment=quant-trading; industry=quantitative-trading.',
})
@Injectable()
export class TowerResearchCapitalService implements IScraper {
  private readonly logger = new Logger(TowerResearchCapitalService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Greenhouse source plugin is not registered; cannot scrape Tower Research Capital');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`Tower Research Capital: delegating to Greenhouse (${board.companySlug})`);
    ...
    this.logger.log(`Tower Research Capital: scraped ${jobs.length} jobs`);
