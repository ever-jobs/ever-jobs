...
import { AirnorthModule, AirnorthService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'airnorth-jobs.json'), 'utf8'),
);
...
/**
 * Spec 215 / T04 — `AirnorthService` unit tests (≥ 9 cases).
 */
describe('AirnorthService — Spec 215 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AirnorthModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AirnorthModule],
      }).compile();
      const service = moduleRef.get(AirnorthService);
      expect(service).toBeInstanceOf(AirnorthService);
      await moduleRef.close();
    });

    it('exports the Site.AIRNORTH = "airnorth" enum value', () => {
      expect(Site.AIRNORTH).toBe('airnorth');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AirnorthService();
      const result = await service.scrape({
        siteType: [Site.AIRNORTH],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'airnorth-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.AIRNORTH);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Air North');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/airnorth/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/airnorth/jobs/');
      ...
      expect(job0?.description).toContain('Air North');
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Air North');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'airnorth-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
