...
import { AplayersModule, AplayersService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'aplayers-jobs.json'), 'utf8'),
);
...
/**
 * Spec 285 / T04 — `AplayersService` unit tests (≥ 9 cases).
 */
describe('AplayersService — Spec 285 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AplayersModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AplayersModule],
      }).compile();
      const service = moduleRef.get(AplayersService);
      expect(service).toBeInstanceOf(AplayersService);
      await moduleRef.close();
    });

    it('exports the Site.APLAYERS = "aplayers" enum value', () => {
      expect(Site.APLAYERS).toBe('aplayers');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AplayersService();
      const result = await service.scrape({
        siteType: [Site.APLAYERS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'aplayers-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.APLAYERS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('MrBeast Contract Jobs');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/aplayers/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/aplayers/jobs/');
      ...
      expect(job0?.description).toContain('MrBeast Contract Jobs');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/aplayers/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('MrBeast Contract Jobs');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'aplayers-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
