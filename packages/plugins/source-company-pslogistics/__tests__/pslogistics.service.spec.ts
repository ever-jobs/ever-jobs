...
import { PSLogisticsModule, PSLogisticsService } from '../src';

const COMPANY_NAME_EXPECT = 'PS Logistics';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'pslogistics-jobs.json'), 'utf8'),
);
...
describe('PSLogisticsService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through PSLogisticsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [PSLogisticsModule],
      }).compile();
      const service = moduleRef.get(PSLogisticsService);
      expect(service).toBeInstanceOf(PSLogisticsService);
      await moduleRef.close();
    });

    it('exports the Site.PS_LOGISTICS = "pslogistics" enum value', () => {
      expect(Site.PS_LOGISTICS).toBe('pslogistics');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new PSLogisticsService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.PS_LOGISTICS],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('PSLogistics');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.PS_LOGISTICS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('pslogistics-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new PSLogisticsService(registry);
      const result = await service.scrape({
        siteType: [Site.PS_LOGISTICS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('PSLogistics');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('pslogistics-x1');
      expect(result.jobs[0].site).toBe(Site.PS_LOGISTICS);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('pslogistics-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new PSLogisticsService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.PS_LOGISTICS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new PSLogisticsService();
      const result = await service.scrape({
        siteType: [Site.PS_LOGISTICS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
