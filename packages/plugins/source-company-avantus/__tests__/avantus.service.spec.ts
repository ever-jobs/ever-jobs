...
import { AvantusModule, AvantusService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'avantus-jobs.json'), 'utf8'),
);
...
/**
 * Spec 813 / T04 — `AvantusService` unit tests (≥ 9 cases).
 */
describe('AvantusService — Spec 813 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AvantusModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AvantusModule],
      }).compile();
      const service = moduleRef.get(AvantusService);
      expect(service).toBeInstanceOf(AvantusService);
      await moduleRef.close();
    });

    it('exports the Site.AVANTUS = "avantus" enum value', () => {
      expect(Site.AVANTUS).toBe('avantus');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AvantusService();
      const result = await service.scrape({
        siteType: [Site.AVANTUS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'avantus-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.AVANTUS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Avantus');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/avantus/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/avantus/jobs/');
      ...
      expect(job0?.description).toContain('Avantus');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/avantus/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Avantus');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'avantus-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
