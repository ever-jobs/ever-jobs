...
import { TranscarentModule, TranscarentService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'transcarent-jobs.json'), 'utf8'),
);
...
/**
 * Spec 500 / T04 — `TranscarentService` unit tests (≥ 9 cases).
 */
describe('TranscarentService — Spec 500 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through TranscarentModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [TranscarentModule],
      }).compile();
      const service = moduleRef.get(TranscarentService);
      expect(service).toBeInstanceOf(TranscarentService);
      await moduleRef.close();
    });

    it('exports the Site.TRANSCARENT = "transcarent" enum value', () => {
      expect(Site.TRANSCARENT).toBe('transcarent');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new TranscarentService();
      const result = await service.scrape({
        siteType: [Site.TRANSCARENT],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'transcarent-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.TRANSCARENT);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Transcarent');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/transcarent/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/transcarent/jobs/');
      ...
      expect(job0?.description).toContain('Transcarent');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/transcarent/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Transcarent');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'transcarent-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
