...
import { ZocdocModule, ZocdocService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'zocdoc-jobs.json'), 'utf8'),
);
...
/**
 * Spec 490 / T04 — `ZocdocService` unit tests (≥ 9 cases).
 */
describe('ZocdocService — Spec 490 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ZocdocModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ZocdocModule],
      }).compile();
      const service = moduleRef.get(ZocdocService);
      expect(service).toBeInstanceOf(ZocdocService);
      await moduleRef.close();
    });

    it('exports the Site.ZOCDOC = "zocdoc" enum value', () => {
      expect(Site.ZOCDOC).toBe('zocdoc');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ZocdocService();
      const result = await service.scrape({
        siteType: [Site.ZOCDOC],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'zocdoc-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ZOCDOC);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Zocdoc');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/zocdoc/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/zocdoc/jobs/');
      ...
      expect(job0?.description).toContain('Zocdoc');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/zocdoc/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Zocdoc');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'zocdoc-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
