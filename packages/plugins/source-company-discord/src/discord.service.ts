...
/** Discord publishes its careers board through Greenhouse. */
const API_URL = 'https://api.greenhouse.io/v1/boards/discord/jobs';
...
export class DiscordService implements IScraper {
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    try {
      ...
      this.logger.log(`Discord: fetching ${url}`);
        ...
        jobs.push(
          new JobPostDto({
            ...
            jobUrl:
              ...
              `https://discord.com/careers/${listing.id}`,
            location,
      ...
      this.logger.log(`Discord: scraped ${jobs.length} jobs`);
    } catch (err: any) {
      this.logger.error(`Discord scrape failed: ${err.message}`);
