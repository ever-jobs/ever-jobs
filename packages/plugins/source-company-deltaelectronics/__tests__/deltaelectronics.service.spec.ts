...
import { DeltaElectronicsModule, DeltaElectronicsService } from '../src';

const COMPANY_NAME_EXPECT = 'Delta Electronics';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'deltaelectronics-jobs.json'), 'utf8'),
);
...
describe('DeltaElectronicsService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through DeltaElectronicsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [DeltaElectronicsModule],
      }).compile();
      const service = moduleRef.get(DeltaElectronicsService);
      expect(service).toBeInstanceOf(DeltaElectronicsService);
      await moduleRef.close();
    });

    it('exports the Site.DELTA_ELECTRONICS = "deltaelectronics" enum value', () => {
      expect(Site.DELTA_ELECTRONICS).toBe('deltaelectronics');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new DeltaElectronicsService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.DELTA_ELECTRONICS],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'deltaelectronics-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.DELTA_ELECTRONICS);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('deltaelectronics-' + first.id);
      ...
      expect(calledUrls[0]).toContain('DeltaElectronics');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.DELTA_ELECTRONICS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('deltaelectronics-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new DeltaElectronicsService(registry);
      const result = await service.scrape({
        siteType: [Site.DELTA_ELECTRONICS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('DeltaElectronics');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('deltaelectronics-x1');
      expect(result.jobs[0].site).toBe(Site.DELTA_ELECTRONICS);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('deltaelectronics-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new DeltaElectronicsService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.DELTA_ELECTRONICS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new DeltaElectronicsService();
      const result = await service.scrape({
        siteType: [Site.DELTA_ELECTRONICS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
