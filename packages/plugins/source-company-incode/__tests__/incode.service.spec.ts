...
import { IncodeTechnologiesModule, IncodeTechnologiesService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'incode-jobs.json'), 'utf8'),
);
...
/**
 * Spec 642 / T04 — `IncodeTechnologiesService` unit tests (≥ 9 cases).
 */
describe('IncodeTechnologiesService — Spec 642 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through IncodeTechnologiesModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [IncodeTechnologiesModule],
      }).compile();
      const service = moduleRef.get(IncodeTechnologiesService);
      expect(service).toBeInstanceOf(IncodeTechnologiesService);
      await moduleRef.close();
    });

    it('exports the Site.INCODE_TECHNOLOGIES = "incode" enum value', () => {
      expect(Site.INCODE_TECHNOLOGIES).toBe('incode');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new IncodeTechnologiesService();
      const result = await service.scrape({
        siteType: [Site.INCODE_TECHNOLOGIES],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'incode-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.INCODE_TECHNOLOGIES);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Incode Technologies');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/incode/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/incode/jobs/');
      ...
      expect(job0?.description).toContain('Incode Technologies');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/incode/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Incode Technologies');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'incode-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
