...
import { FalconXModule, FalconXService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'falconx-jobs.json'), 'utf8'),
);
...
/**
 * Spec 710 / T04 — `FalconXService` unit tests (≥ 9 cases).
 */
describe('FalconXService — Spec 710 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through FalconXModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [FalconXModule],
      }).compile();
      const service = moduleRef.get(FalconXService);
      expect(service).toBeInstanceOf(FalconXService);
      await moduleRef.close();
    });

    it('exports the Site.FALCONX = "falconx" enum value', () => {
      expect(Site.FALCONX).toBe('falconx');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new FalconXService();
      const result = await service.scrape({
        siteType: [Site.FALCONX],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'falconx-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.FALCONX);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('FalconX');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/falconx/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/falconx/jobs/');
      ...
      expect(job0?.description).toContain('FalconX');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/falconx/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('FalconX');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'falconx-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
