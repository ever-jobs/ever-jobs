...
import { FinQueryModule, FinQueryService } from '../src';

const COMPANY_NAME_EXPECT = 'FinQuery';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'finquery-jobs.json'), 'utf8'),
);
...
describe('FinQueryService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through FinQueryModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [FinQueryModule],
      }).compile();
      const service = moduleRef.get(FinQueryService);
      expect(service).toBeInstanceOf(FinQueryService);
      await moduleRef.close();
    });

    it('exports the Site.FINQUERY = "finquery" enum value', () => {
      expect(Site.FINQUERY).toBe('finquery');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new FinQueryService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.FINQUERY],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('finquery');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.FINQUERY);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('finquery-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new FinQueryService(registry);
      const result = await service.scrape({
        siteType: [Site.FINQUERY],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('finquery');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('finquery-x1');
      expect(result.jobs[0].site).toBe(Site.FINQUERY);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('finquery-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new FinQueryService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.FINQUERY],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new FinQueryService();
      const result = await service.scrape({
        siteType: [Site.FINQUERY],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
