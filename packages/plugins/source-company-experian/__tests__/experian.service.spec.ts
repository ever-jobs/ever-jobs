...
import { ExperianModule, ExperianService } from '../src';

const COMPANY_NAME_EXPECT = 'Experian';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'experian-jobs.json'), 'utf8'),
);
...
describe('ExperianService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ExperianModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ExperianModule],
      }).compile();
      const service = moduleRef.get(ExperianService);
      expect(service).toBeInstanceOf(ExperianService);
      await moduleRef.close();
    });

    it('exports the Site.EXPERIAN = "experian" enum value', () => {
      expect(Site.EXPERIAN).toBe('experian');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new ExperianService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.EXPERIAN],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('Experian');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.EXPERIAN);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('experian-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new ExperianService(registry);
      const result = await service.scrape({
        siteType: [Site.EXPERIAN],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('Experian');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('experian-x1');
      expect(result.jobs[0].site).toBe(Site.EXPERIAN);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('experian-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new ExperianService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.EXPERIAN],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new ExperianService();
      const result = await service.scrape({
        siteType: [Site.EXPERIAN],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
