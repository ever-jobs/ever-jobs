...
import { CognitionModule, CognitionService } from '../src';

const COMPANY_NAME_EXPECT = 'Cognition';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'cognition-jobs.json'), 'utf8'),
);
...
describe('CognitionService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CognitionModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CognitionModule],
      }).compile();
      const service = moduleRef.get(CognitionService);
      expect(service).toBeInstanceOf(CognitionService);
      await moduleRef.close();
    });

    it('exports the Site.COGNITION = "cognition" enum value', () => {
      expect(Site.COGNITION).toBe('cognition');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new CognitionService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.COGNITION],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('cognition');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.COGNITION);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('cognition-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new CognitionService(registry);
      const result = await service.scrape({
        siteType: [Site.COGNITION],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('cognition');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('cognition-x1');
      expect(result.jobs[0].site).toBe(Site.COGNITION);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('cognition-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new CognitionService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.COGNITION],
      } as ScraperInputDto);
