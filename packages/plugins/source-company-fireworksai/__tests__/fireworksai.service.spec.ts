...
import { FireworksaiModule, FireworksaiService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'fireworksai-jobs.json'), 'utf8'),
);
...
/**
 * Spec 468 / T04 — `FireworksaiService` unit tests (≥ 9 cases).
 */
describe('FireworksaiService — Spec 468 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through FireworksaiModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [FireworksaiModule],
      }).compile();
      const service = moduleRef.get(FireworksaiService);
      expect(service).toBeInstanceOf(FireworksaiService);
      await moduleRef.close();
    });

    it('exports the Site.FIREWORKSAI = "fireworksai" enum value', () => {
      expect(Site.FIREWORKSAI).toBe('fireworksai');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new FireworksaiService();
      const result = await service.scrape({
        siteType: [Site.FIREWORKSAI],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'fireworksai-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.FIREWORKSAI);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Fireworks AI');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/fireworksai/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/fireworksai/jobs/');
      ...
      expect(job0?.description).toContain('Fireworks AI');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/fireworksai/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Fireworks AI');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'fireworksai-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
