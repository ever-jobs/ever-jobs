...
import { FlashfoodModule, FlashfoodService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'flashfood-jobs.json'), 'utf8'),
);
...
/**
 * Spec 849 / T04 — `FlashfoodService` unit tests (≥ 9 cases).
 */
describe('FlashfoodService — Spec 849 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through FlashfoodModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [FlashfoodModule],
      }).compile();
      const service = moduleRef.get(FlashfoodService);
      expect(service).toBeInstanceOf(FlashfoodService);
      await moduleRef.close();
    });

    it('exports the Site.FLASHFOOD = "flashfood" enum value', () => {
      expect(Site.FLASHFOOD).toBe('flashfood');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new FlashfoodService();
      const result = await service.scrape({
        siteType: [Site.FLASHFOOD],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'flashfood-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.FLASHFOOD);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Flashfood');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/flashfood/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/flashfood/jobs/');
      ...
      expect(job0?.description).toContain('Flashfood');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/flashfood/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Flashfood');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'flashfood-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
