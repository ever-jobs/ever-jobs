...
import { EzraModule, EzraService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'ezra-jobs.json'), 'utf8'),
);
...
/**
 * Spec 639 / T04 — `EzraService` unit tests (≥ 9 cases).
 */
describe('EzraService — Spec 639 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through EzraModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [EzraModule],
      }).compile();
      const service = moduleRef.get(EzraService);
      expect(service).toBeInstanceOf(EzraService);
      await moduleRef.close();
    });

    it('exports the Site.EZRA = "ezra" enum value', () => {
      expect(Site.EZRA).toBe('ezra');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const job0 = dto.jobs.find((j) => j.id === 'ezra-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.EZRA);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Ezra');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/ezra/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/ezra/jobs/');
      ...
      expect(job0?.description).toContain('Ezra');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/ezra/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Ezra');
      }
    });
  });
