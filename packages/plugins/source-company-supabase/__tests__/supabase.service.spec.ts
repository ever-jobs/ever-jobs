...
import { SupabaseModule, SupabaseService } from '../src';

const COMPANY_NAME_EXPECT = 'Supabase';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'supabase-jobs.json'), 'utf8'),
);
...
describe('SupabaseService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SupabaseModule via NestJS DI', async () => {
    ...
    it('exports the Site.SUPABASE = "supabase" enum value', () => {
      expect(Site.SUPABASE).toBe('supabase');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new SupabaseService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.SUPABASE],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('supabase');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SUPABASE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('supabase-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new SupabaseService(registry);
      const result = await service.scrape({
        siteType: [Site.SUPABASE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('supabase');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('supabase-x1');
      expect(result.jobs[0].site).toBe(Site.SUPABASE);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('supabase-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new SupabaseService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.SUPABASE],
      } as ScraperInputDto);
