...
import { UnitekLearningModule, UnitekLearningService } from '../src';

const COMPANY_NAME_EXPECT = 'Unitek Learning';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'uniteklearning-jobs.json'), 'utf8'),
);
...
describe('UnitekLearningService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through UnitekLearningModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [UnitekLearningModule],
      }).compile();
      const service = moduleRef.get(UnitekLearningService);
      expect(service).toBeInstanceOf(UnitekLearningService);
      await moduleRef.close();
    });

    it('exports the Site.UNITEK_LEARNING = "uniteklearning" enum value', () => {
      expect(Site.UNITEK_LEARNING).toBe('uniteklearning');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new UnitekLearningService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.UNITEK_LEARNING],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'uniteklearning-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.UNITEK_LEARNING);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('uniteklearning-' + first.id);
      ...
      expect(calledUrls[0]).toContain('UnitekLearning');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.UNITEK_LEARNING);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('uniteklearning-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new UnitekLearningService(registry);
      const result = await service.scrape({
        siteType: [Site.UNITEK_LEARNING],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('UnitekLearning');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('uniteklearning-x1');
      expect(result.jobs[0].site).toBe(Site.UNITEK_LEARNING);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('uniteklearning-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new UnitekLearningService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.UNITEK_LEARNING],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new UnitekLearningService();
      const result = await service.scrape({
        siteType: [Site.UNITEK_LEARNING],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
