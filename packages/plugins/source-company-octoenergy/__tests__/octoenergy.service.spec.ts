...
import { OctopusEnergyGroupModule, OctopusEnergyGroupService } from '../src';

const COMPANY_NAME_EXPECT = 'Octopus Energy Group';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'octoenergy-jobs.json'), 'utf8'),
);
...
describe('OctopusEnergyGroupService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through OctopusEnergyGroupModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [OctopusEnergyGroupModule],
      }).compile();
      const service = moduleRef.get(OctopusEnergyGroupService);
      expect(service).toBeInstanceOf(OctopusEnergyGroupService);
      await moduleRef.close();
    });

    it('exports the Site.OCTOPUS_ENERGY_GROUP = "octoenergy" enum value', () => {
      expect(Site.OCTOPUS_ENERGY_GROUP).toBe('octoenergy');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new OctopusEnergyGroupService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.OCTOPUS_ENERGY_GROUP],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'octoenergy-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.OCTOPUS_ENERGY_GROUP);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('octoenergy-' + first.id);
      ...
      expect(calledUrls[0]).toContain('octoenergy');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.OCTOPUS_ENERGY_GROUP);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('octoenergy-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new OctopusEnergyGroupService(registry);
      const result = await service.scrape({
        siteType: [Site.OCTOPUS_ENERGY_GROUP],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('octoenergy');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('octoenergy-x1');
      expect(result.jobs[0].site).toBe(Site.OCTOPUS_ENERGY_GROUP);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('octoenergy-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new OctopusEnergyGroupService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.OCTOPUS_ENERGY_GROUP],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new OctopusEnergyGroupService();
      const result = await service.scrape({
        siteType: [Site.OCTOPUS_ENERGY_GROUP],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
