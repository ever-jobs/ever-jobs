...
import { GoodrModule, GoodrService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'goodr-jobs.json'), 'utf8'),
);
...
/**
 * Spec 599 / T04 — `GoodrService` unit tests (≥ 9 cases).
 */
describe('GoodrService — Spec 599 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through GoodrModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [GoodrModule],
      }).compile();
      const service = moduleRef.get(GoodrService);
      expect(service).toBeInstanceOf(GoodrService);
      await moduleRef.close();
    });

    it('exports the Site.GOODR = "goodr" enum value', () => {
      expect(Site.GOODR).toBe('goodr');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new GoodrService();
      ...
      const job0 = dto.jobs.find((j) => j.id === 'goodr-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.GOODR);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('goodr');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/goodr/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/goodr/jobs/');
      ...
      expect(job0?.description).toContain('goodr');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/goodr/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('goodr');
      }
    });
  });
