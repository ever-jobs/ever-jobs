...
import { SeekOutModule, SeekOutService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'seekout-jobs.json'), 'utf8'),
);
...
/**
 * Spec 928 / T04 — `SeekOutService` unit tests (≥ 9 cases).
 */
describe('SeekOutService — Spec 928 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SeekOutModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SeekOutModule],
      }).compile();
      const service = moduleRef.get(SeekOutService);
      expect(service).toBeInstanceOf(SeekOutService);
      await moduleRef.close();
    });

    it('exports the Site.SEEKOUT = "seekout" enum value', () => {
      expect(Site.SEEKOUT).toBe('seekout');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new SeekOutService();
      const result = await service.scrape({
        siteType: [Site.SEEKOUT],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'seekout-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.SEEKOUT);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('SeekOut');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/seekout/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/seekout/jobs/');
      ...
      expect(job0?.description).toContain('SeekOut');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/seekout/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('SeekOut');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'seekout-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
