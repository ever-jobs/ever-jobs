...
import { ETalentModule, ETalentService } from '../src';

const COMPANY_NAME_EXPECT = 'eTalent';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'etalent-jobs.json'), 'utf8'),
);
...
describe('ETalentService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ETalentModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ETalentModule],
      }).compile();
      const service = moduleRef.get(ETalentService);
      expect(service).toBeInstanceOf(ETalentService);
      await moduleRef.close();
    });

    it('exports the Site.ETALENT = "etalent" enum value', () => {
      expect(Site.ETALENT).toBe('etalent');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new ETalentService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.ETALENT],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('ETalent');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ETALENT);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('etalent-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new ETalentService(registry);
      const result = await service.scrape({
        siteType: [Site.ETALENT],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('ETalent');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('etalent-x1');
      expect(result.jobs[0].site).toBe(Site.ETALENT);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('etalent-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new ETalentService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.ETALENT],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new ETalentService();
      const result = await service.scrape({
        siteType: [Site.ETALENT],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
