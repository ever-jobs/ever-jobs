...
import { LendableModule, LendableService } from '../src';

const COMPANY_NAME_EXPECT = 'Lendable';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'lendable-jobs.json'), 'utf8'),
);
...
describe('LendableService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through LendableModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [LendableModule],
      }).compile();
      const service = moduleRef.get(LendableService);
      expect(service).toBeInstanceOf(LendableService);
      await moduleRef.close();
    });

    it('exports the Site.LENDABLE = "lendable" enum value', () => {
      expect(Site.LENDABLE).toBe('lendable');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new LendableService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.LENDABLE],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('lendable');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.LENDABLE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('lendable-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new LendableService(registry);
      const result = await service.scrape({
        siteType: [Site.LENDABLE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('lendable');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('lendable-x1');
      expect(result.jobs[0].site).toBe(Site.LENDABLE);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('lendable-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new LendableService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.LENDABLE],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new LendableService();
      const result = await service.scrape({
        siteType: [Site.LENDABLE],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
