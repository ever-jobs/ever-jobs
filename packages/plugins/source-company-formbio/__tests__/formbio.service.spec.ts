...
import { FormBioModule, FormBioService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'formbio-jobs.json'), 'utf8'),
);
...
/**
 * Spec 762 / T04 — `FormBioService` unit tests (≥ 9 cases).
 */
describe('FormBioService — Spec 762 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through FormBioModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [FormBioModule],
      }).compile();
      const service = moduleRef.get(FormBioService);
      expect(service).toBeInstanceOf(FormBioService);
      await moduleRef.close();
    });

    it('exports the Site.FORM_BIO = "formbio" enum value', () => {
      expect(Site.FORM_BIO).toBe('formbio');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new FormBioService();
      const result = await service.scrape({
        siteType: [Site.FORM_BIO],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'formbio-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.FORM_BIO);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Form Bio');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/formbio/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/formbio/jobs/');
      ...
      expect(job0?.description).toContain('Form Bio');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/formbio/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Form Bio');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'formbio-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
