...
import { NexthinkModule, NexthinkService } from '../src';

const COMPANY_NAME_EXPECT = 'Nexthink';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'nexthink-jobs.json'), 'utf8'),
);
...
describe('NexthinkService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through NexthinkModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [NexthinkModule],
      }).compile();
      const service = moduleRef.get(NexthinkService);
      expect(service).toBeInstanceOf(NexthinkService);
      await moduleRef.close();
    });

    it('exports the Site.NEXTHINK = "nexthink" enum value', () => {
      expect(Site.NEXTHINK).toBe('nexthink');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new NexthinkService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.NEXTHINK],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('Nexthink');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.NEXTHINK);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('nexthink-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new NexthinkService(registry);
      const result = await service.scrape({
        siteType: [Site.NEXTHINK],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('Nexthink');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('nexthink-x1');
      expect(result.jobs[0].site).toBe(Site.NEXTHINK);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('nexthink-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new NexthinkService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.NEXTHINK],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new NexthinkService();
      const result = await service.scrape({
        siteType: [Site.NEXTHINK],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
