...
import { ValohealthModule, ValohealthService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'valohealth-jobs.json'), 'utf8'),
);
...
/**
 * Spec 569 / T04 — `ValohealthService` unit tests (≥ 9 cases).
 */
describe('ValohealthService — Spec 569 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ValohealthModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ValohealthModule],
      }).compile();
      const service = moduleRef.get(ValohealthService);
      expect(service).toBeInstanceOf(ValohealthService);
      await moduleRef.close();
    });

    it('exports the Site.VALO_HEALTH = "valohealth" enum value', () => {
      expect(Site.VALO_HEALTH).toBe('valohealth');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ValohealthService();
      const result = await service.scrape({
        siteType: [Site.VALO_HEALTH],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'valohealth-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.VALO_HEALTH);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Valo Health');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/valohealth/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/valohealth/jobs/');
      ...
      expect(job0?.description).toContain('Valo Health');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/valohealth/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Valo Health');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'valohealth-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
