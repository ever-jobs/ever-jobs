...
import { AlpacaModule, AlpacaService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'alpaca-jobs.json'), 'utf8'),
);
...
/**
 * Spec 239 / T04 — `AlpacaService` unit tests (≥ 9 cases).
 */
describe('AlpacaService — Spec 239 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AlpacaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AlpacaModule],
      }).compile();
      const service = moduleRef.get(AlpacaService);
      expect(service).toBeInstanceOf(AlpacaService);
      await moduleRef.close();
    });

    it('exports the Site.ALPACA = "alpaca" enum value', () => {
      expect(Site.ALPACA).toBe('alpaca');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const job0 = dto.jobs.find((j) => j.id === 'alpaca-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ALPACA);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Alpaca ');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/alpaca/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/alpaca/jobs/');
      ...
      expect(job0?.description).toContain('Alpaca ');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/alpaca/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Alpaca ');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'alpaca-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
