...
/**
 * GE Aerospace — Aerospace (HQ: Evendale, OH, USA).
 *
 * Source (Spec 1736): Workday board, scraped in this order:
 *   - `geaerospace:5:GE_ExternalSite` — https://geaerospace.wd5.myworkdayjobs.com/GE_ExternalSite
 *     verified live 2026-09-24: 506 open postings.
 *
...
@SourcePlugin({
  ...
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['geaerospace.com'],
  description: 'GE Aerospace careers via Workday. Tags: segment=workday-enterprise; industry=aerospace.',
})
@Injectable()
export class GeAerospaceService implements IScraper {
  private readonly logger = new Logger(GeAerospaceService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Workday source plugin is not registered; cannot scrape GE Aerospace');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`GE Aerospace: delegating to Workday (${board.companySlug})`);
    ...
    this.logger.log(`GE Aerospace: scraped ${jobs.length} jobs`);
