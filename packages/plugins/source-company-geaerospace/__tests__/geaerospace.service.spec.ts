...
import { GeAerospaceModule, GeAerospaceService } from '../src';
...
const FIXTURE: Fixture = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'fixtures', 'geaerospace-boards.json'), 'utf8'),
);
const COMPANY_NAME = 'GE Aerospace';
const ID_PREFIX = 'geaerospace-';
...
describe('GeAerospaceService — Workday delegation (Spec 1736)', () => {
  beforeEach(() => {
    mockGet.mockReset();
    mockPost.mockReset();
  ...
  describe('registration', () => {
    it('resolves through GeAerospaceModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({ imports: [GeAerospaceModule] }).compile();
      expect(moduleRef.get(GeAerospaceService)).toBeInstanceOf(GeAerospaceService);
      await moduleRef.close();
    });

    it('exports Site.GE_AEROSPACE = "geaerospace"', () => {
      expect(Site.GE_AEROSPACE).toBe('geaerospace');
    });

    it('declares a tagged company plugin with its domains', () => {
      const meta = Reflect.getMetadata(SOURCE_PLUGIN_METADATA, GeAerospaceService);
      expect(meta.site).toBe(Site.GE_AEROSPACE);
      expect(meta.category).toBe('company');
      expect(meta.isAts).toBeFalsy();
      expect(meta.companyDomains).toEqual(['geaerospace.com']);
  ...
  describe('recorded board (real Workday adapter, mocked HTTP)', () => {
    it('maps every recorded posting and re-stamps the company identity', async () => {
      const service = new GeAerospaceService(registryWith());
      const result = await service.scrape({ siteType: [Site.GE_AEROSPACE], resultsWanted: 100 } as ScraperInputDto);
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.GE_AEROSPACE);
    ...
    it('only requests the recorded board URLs', async () => {
      const service = new GeAerospaceService(registryWith());
      await service.scrape({ siteType: [Site.GE_AEROSPACE] } as ScraperInputDto);
    ...
    it('honours resultsWanted=1', async () => {
      const service = new GeAerospaceService(registryWith());
      const result = await service.scrape({ siteType: [Site.GE_AEROSPACE], resultsWanted: 1 } as ScraperInputDto);
    ...
    it('resolves with an empty result when the board is gone (HTTP 404)', async () => {
      ...
      const service = new GeAerospaceService(registryWith());
      const result = await service.scrape({ siteType: [Site.GE_AEROSPACE] } as ScraperInputDto);
  ...
  describe('delegation contract', () => {
    it('forwards the board and the caller input untouched', async () => {
      ...
      const service = new GeAerospaceService(
        registryWith(
          fakeBackend(
      ...
      expect(result.jobs[0].site).toBe(Site.GE_AEROSPACE);
      expect(result.jobs[0].companyName).toBe(COMPANY_NAME);
    });

    it('passes an absent resultsWanted through as absent', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new GeAerospaceService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      await service.scrape({ siteType: [Site.GE_AEROSPACE] } as ScraperInputDto);
    ...
    it('makes no request when resultsWanted is 0', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new GeAerospaceService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      const result = await service.scrape({ siteType: [Site.GE_AEROSPACE], resultsWanted: 0 } as ScraperInputDto);
  ...
  describe('resilience', () => {
    it('reports not_registered when no registry is injected', async () => {
      const result = await new GeAerospaceService().scrape({ siteType: [Site.GE_AEROSPACE] } as ScraperInputDto);
      expect(result.jobs).toEqual([]);
    ...
    it('reports not_registered when Workday is missing from the registry', async () => {
      const result = await new GeAerospaceService(new PluginRegistry()).scrape({
        siteType: [Site.GE_AEROSPACE],
      } as ScraperInputDto);
      expect(result.diagnostics?.reason).toBe('not_registered');
    });
