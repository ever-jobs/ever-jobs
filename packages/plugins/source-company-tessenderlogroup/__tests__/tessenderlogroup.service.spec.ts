...
import { TessenderloGroupModule, TessenderloGroupService } from '../src';

const COMPANY_NAME_EXPECT = 'Tessenderlo Group';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'tessenderlogroup-jobs.json'), 'utf8'),
);
...
describe('TessenderloGroupService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through TessenderloGroupModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [TessenderloGroupModule],
      }).compile();
      const service = moduleRef.get(TessenderloGroupService);
      expect(service).toBeInstanceOf(TessenderloGroupService);
      await moduleRef.close();
    });

    it('exports the Site.TESSENDERLO_GROUP = "tessenderlogroup" enum value', () => {
      expect(Site.TESSENDERLO_GROUP).toBe('tessenderlogroup');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new TessenderloGroupService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.TESSENDERLO_GROUP],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'tessenderlogroup-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.TESSENDERLO_GROUP);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('tessenderlogroup-' + first.id);
      ...
      expect(calledUrls[0]).toContain('TessenderloGroup');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.TESSENDERLO_GROUP);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('tessenderlogroup-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new TessenderloGroupService(registry);
      const result = await service.scrape({
        siteType: [Site.TESSENDERLO_GROUP],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('TessenderloGroup');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('tessenderlogroup-x1');
      expect(result.jobs[0].site).toBe(Site.TESSENDERLO_GROUP);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('tessenderlogroup-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new TessenderloGroupService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.TESSENDERLO_GROUP],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new TessenderloGroupService();
      const result = await service.scrape({
        siteType: [Site.TESSENDERLO_GROUP],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
