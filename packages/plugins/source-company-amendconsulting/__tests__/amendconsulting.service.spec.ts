...
import { AmendconsultingModule, AmendconsultingService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'amendconsulting-jobs.json'), 'utf8'),
);
...
/**
 * Spec 259 / T04 — `AmendconsultingService` unit tests (≥ 9 cases).
 */
describe('AmendconsultingService — Spec 259 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AmendconsultingModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AmendconsultingModule],
      }).compile();
      const service = moduleRef.get(AmendconsultingService);
      expect(service).toBeInstanceOf(AmendconsultingService);
      await moduleRef.close();
    });

    it('exports the Site.AMENDCONSULTING = "amendconsulting" enum value', () => {
      expect(Site.AMENDCONSULTING).toBe('amendconsulting');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AmendconsultingService();
      const result = await service.scrape({
        siteType: [Site.AMENDCONSULTING],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'amendconsulting-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.AMENDCONSULTING);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('AMEND Consulting');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/amendconsulting/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/amendconsulting/jobs/');
      ...
      expect(job0?.description).toContain('AMEND Consulting');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/amendconsulting/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('AMEND Consulting');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'amendconsulting-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
