...
import { WorldLabsModule, WorldLabsService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'worldlabs-jobs.json'), 'utf8'),
);
...
/**
 * Spec 583 / T04 — `WorldLabsService` unit tests (≥ 9 cases).
 */
describe('WorldLabsService — Spec 583 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through WorldLabsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [WorldLabsModule],
      }).compile();
      const service = moduleRef.get(WorldLabsService);
      expect(service).toBeInstanceOf(WorldLabsService);
      await moduleRef.close();
    });

    it('exports the Site.WORLD_LABS = "worldlabs" enum value', () => {
      expect(Site.WORLD_LABS).toBe('worldlabs');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const job0 = dto.jobs.find((j) => j.id === 'worldlabs-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.WORLD_LABS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('World Labs');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/worldlabs/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/worldlabs/jobs/');
      ...
      expect(job0?.description).toContain('World Labs');
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('World Labs');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'worldlabs-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
