...
import { BridgewaterModule, BridgewaterService } from '../src';
...
const FIXTURE: Fixture = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'fixtures', 'bridgewater-boards.json'), 'utf8'),
);
...
describe('BridgewaterService — Greenhouse delegation (Spec 1737)', () => {
  beforeEach(() => {
    mockGet.mockReset();
    mockPost.mockReset();
  ...
  describe('registration', () => {
    it('resolves through BridgewaterModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({ imports: [BridgewaterModule] }).compile();
      expect(moduleRef.get(BridgewaterService)).toBeInstanceOf(BridgewaterService);
      await moduleRef.close();
    });

    it('exports Site.BRIDGEWATER = "bridgewater"', () => {
      expect(Site.BRIDGEWATER).toBe('bridgewater');
    });

    it('declares a tagged company plugin with its domains', () => {
      const meta = Reflect.getMetadata(SOURCE_PLUGIN_METADATA, BridgewaterService);
      expect(meta.site).toBe(Site.BRIDGEWATER);
      expect(meta.category).toBe('company');
      expect(meta.isAts).toBeFalsy();
      expect(meta.companyDomains).toEqual(['bridgewater.com']);
  ...
  describe('recorded board (real Greenhouse adapter, mocked HTTP)', () => {
    it('maps every recorded posting and re-stamps the company identity', async () => {
      ...
      const result = await service.scrape({ siteType: [Site.BRIDGEWATER], resultsWanted: 100 } as ScraperInputDto);
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.BRIDGEWATER);
    ...
    it('only requests the recorded board URLs', async () => {
      const service = new BridgewaterService(registryWith());
      await service.scrape({ siteType: [Site.BRIDGEWATER] } as ScraperInputDto);
    ...
    it('honours resultsWanted=1', async () => {
      const service = new BridgewaterService(registryWith());
      const result = await service.scrape({ siteType: [Site.BRIDGEWATER], resultsWanted: 1 } as ScraperInputDto);
    ...
    it('resolves with an empty result when the board is gone (HTTP 404)', async () => {
      ...
      const service = new BridgewaterService(registryWith());
      const result = await service.scrape({ siteType: [Site.BRIDGEWATER] } as ScraperInputDto);
  ...
  describe('delegation contract', () => {
    it('forwards the board and the caller input untouched', async () => {
      ...
      const service = new BridgewaterService(
        registryWith(
          fakeBackend(
      ...
      expect(result.jobs[0].site).toBe(Site.BRIDGEWATER);
    ...
    it('makes no request when resultsWanted is 0', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new BridgewaterService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      const result = await service.scrape({ siteType: [Site.BRIDGEWATER], resultsWanted: 0 } as ScraperInputDto);
  ...
  describe('resilience', () => {
    it('reports not_registered when no registry is injected', async () => {
      const result = await new BridgewaterService().scrape({ siteType: [Site.BRIDGEWATER] } as ScraperInputDto);
      expect(result.jobs).toEqual([]);
    ...
    it('reports not_registered when Greenhouse is missing from the registry', async () => {
      const result = await new BridgewaterService(new PluginRegistry()).scrape({
        siteType: [Site.BRIDGEWATER],
      } as ScraperInputDto);
      expect(result.diagnostics?.reason).toBe('not_registered');
    });
