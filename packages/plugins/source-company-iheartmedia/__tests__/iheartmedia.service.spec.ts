...
import { IHeartMediaModule, IHeartMediaService } from '../src';

const COMPANY_NAME_EXPECT = 'iHeartMedia';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'iheartmedia-jobs.json'), 'utf8'),
);
...
describe('IHeartMediaService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through IHeartMediaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [IHeartMediaModule],
      }).compile();
      const service = moduleRef.get(IHeartMediaService);
      expect(service).toBeInstanceOf(IHeartMediaService);
      await moduleRef.close();
    });

    it('exports the Site.IHEARTMEDIA = "iheartmedia" enum value', () => {
      expect(Site.IHEARTMEDIA).toBe('iheartmedia');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new IHeartMediaService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.IHEARTMEDIA],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('IHeartMedia');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.IHEARTMEDIA);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('iheartmedia-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new IHeartMediaService(registry);
      const result = await service.scrape({
        siteType: [Site.IHEARTMEDIA],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('IHeartMedia');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('iheartmedia-x1');
      expect(result.jobs[0].site).toBe(Site.IHEARTMEDIA);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('iheartmedia-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new IHeartMediaService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.IHEARTMEDIA],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new IHeartMediaService();
      const result = await service.scrape({
        siteType: [Site.IHEARTMEDIA],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
