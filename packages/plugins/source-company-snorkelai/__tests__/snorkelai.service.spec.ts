...
import { SnorkelAIModule, SnorkelAIService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'snorkelai-jobs.json'), 'utf8'),
);
...
/**
 * Spec 935 / T04 — `SnorkelAIService` unit tests (≥ 9 cases).
 */
describe('SnorkelAIService — Spec 935 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SnorkelAIModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SnorkelAIModule],
      }).compile();
      const service = moduleRef.get(SnorkelAIService);
      expect(service).toBeInstanceOf(SnorkelAIService);
      await moduleRef.close();
    });

    it('exports the Site.SNORKEL_AI = "snorkelai" enum value', () => {
      expect(Site.SNORKEL_AI).toBe('snorkelai');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new SnorkelAIService();
      const result = await service.scrape({
        siteType: [Site.SNORKEL_AI],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'snorkelai-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.SNORKEL_AI);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Snorkel AI');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/snorkelai/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/snorkelai/jobs/');
      ...
      expect(job0?.description).toContain('Snorkel AI');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/snorkelai/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Snorkel AI');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'snorkelai-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
