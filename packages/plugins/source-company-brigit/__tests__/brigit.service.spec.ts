...
import { BrigitModule, BrigitService } from '../src';

const COMPANY_NAME_EXPECT = 'Brigit';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'brigit-jobs.json'), 'utf8'),
);
...
describe('BrigitService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through BrigitModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [BrigitModule],
      }).compile();
      const service = moduleRef.get(BrigitService);
      expect(service).toBeInstanceOf(BrigitService);
      await moduleRef.close();
    });

    it('exports the Site.BRIGIT = "brigit" enum value', () => {
      expect(Site.BRIGIT).toBe('brigit');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new BrigitService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.BRIGIT],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('brigit');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.BRIGIT);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('brigit-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new BrigitService(registry);
      const result = await service.scrape({
        siteType: [Site.BRIGIT],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('brigit');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('brigit-x1');
      expect(result.jobs[0].site).toBe(Site.BRIGIT);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('brigit-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new BrigitService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.BRIGIT],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new BrigitService();
      const result = await service.scrape({
        siteType: [Site.BRIGIT],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
