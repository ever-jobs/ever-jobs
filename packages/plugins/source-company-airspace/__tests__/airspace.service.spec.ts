...
import { AirspaceModule, AirspaceService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'airspace-jobs.json'), 'utf8'),
);
...
/**
 * Spec 217 / T04 — `AirspaceService` unit tests (≥ 9 cases).
 */
describe('AirspaceService — Spec 217 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AirspaceModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AirspaceModule],
      }).compile();
      const service = moduleRef.get(AirspaceService);
      expect(service).toBeInstanceOf(AirspaceService);
      await moduleRef.close();
    });

    it('exports the Site.AIRSPACE = "airspace" enum value', () => {
      expect(Site.AIRSPACE).toBe('airspace');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AirspaceService();
      const result = await service.scrape({
        siteType: [Site.AIRSPACE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'airspace-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.AIRSPACE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Airspace ');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/airspace/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/airspace/jobs/');
      ...
      expect(job0?.description).toContain('Airspace ');
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Airspace ');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'airspace-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
