...
import { GymsharkModule, GymsharkService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'gymshark-jobs.json'), 'utf8'),
);
...
/**
 * Spec 572 / T04 — `GymsharkService` unit tests (≥ 9 cases).
 */
describe('GymsharkService — Spec 572 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through GymsharkModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [GymsharkModule],
      }).compile();
      const service = moduleRef.get(GymsharkService);
      expect(service).toBeInstanceOf(GymsharkService);
      await moduleRef.close();
    });

    it('exports the Site.GYMSHARK = "gymshark" enum value', () => {
      expect(Site.GYMSHARK).toBe('gymshark');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new GymsharkService();
      const result = await service.scrape({
        siteType: [Site.GYMSHARK],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'gymshark-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.GYMSHARK);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Gymshark');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/gymshark/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/gymshark/jobs/');
      ...
      expect(job0?.description).toContain('Gymshark');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/gymshark/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Gymshark');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'gymshark-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
