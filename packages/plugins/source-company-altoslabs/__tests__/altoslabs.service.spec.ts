...
import { AltoslabsModule, AltoslabsService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'altoslabs-jobs.json'), 'utf8'),
);
...
/**
 * Spec 249 / T04 — `AltoslabsService` unit tests (≥ 9 cases).
 */
describe('AltoslabsService — Spec 249 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AltoslabsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AltoslabsModule],
      }).compile();
      const service = moduleRef.get(AltoslabsService);
      expect(service).toBeInstanceOf(AltoslabsService);
      await moduleRef.close();
    });

    it('exports the Site.ALTOSLABS = "altoslabs" enum value', () => {
      expect(Site.ALTOSLABS).toBe('altoslabs');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AltoslabsService();
      const result = await service.scrape({
        siteType: [Site.ALTOSLABS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'altoslabs-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ALTOSLABS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Altos Labs');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/altoslabs/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/altoslabs/jobs/');
      ...
      expect(job0?.description).toContain('Altos Labs');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/altoslabs/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Altos Labs');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'altoslabs-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
