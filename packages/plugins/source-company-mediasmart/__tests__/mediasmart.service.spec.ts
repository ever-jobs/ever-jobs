...
import { MediasmartModule, MediasmartService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'mediasmart-jobs.json'), 'utf8'),
);
...
/**
 * Spec 883 / T04 — `MediasmartService` unit tests (≥ 9 cases).
 */
describe('MediasmartService — Spec 883 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through MediasmartModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [MediasmartModule],
      }).compile();
      const service = moduleRef.get(MediasmartService);
      expect(service).toBeInstanceOf(MediasmartService);
      await moduleRef.close();
    });

    it('exports the Site.MEDIASMART = "mediasmart" enum value', () => {
      expect(Site.MEDIASMART).toBe('mediasmart');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new MediasmartService();
      const result = await service.scrape({
        siteType: [Site.MEDIASMART],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'mediasmart-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.MEDIASMART);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('mediasmart');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/mediasmart/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/mediasmart/jobs/');
      ...
      expect(job0?.description).toContain('mediasmart');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/mediasmart/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('mediasmart');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'mediasmart-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
