...
import { UpSlideModule, UpSlideService } from '../src';

const COMPANY_NAME_EXPECT = 'UpSlide';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'upslide-jobs.json'), 'utf8'),
);
...
describe('UpSlideService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through UpSlideModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [UpSlideModule],
      }).compile();
      const service = moduleRef.get(UpSlideService);
      expect(service).toBeInstanceOf(UpSlideService);
      await moduleRef.close();
    });

    it('exports the Site.UPSLIDE = "upslide" enum value', () => {
      expect(Site.UPSLIDE).toBe('upslide');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new UpSlideService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.UPSLIDE],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('upslide.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.UPSLIDE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('upslide-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new UpSlideService(registry);
      const result = await service.scrape({
        siteType: [Site.UPSLIDE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('upslide');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('upslide-x1');
      expect(result.jobs[0].site).toBe(Site.UPSLIDE);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('upslide-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new UpSlideService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.UPSLIDE],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new UpSlideService();
      const result = await service.scrape({
        siteType: [Site.UPSLIDE],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
