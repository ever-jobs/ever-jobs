...
import { CreativeClicksModule, CreativeClicksService } from '../src';

const COMPANY_NAME_EXPECT = 'Creative Clicks';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'creativeclicks-jobs.json'), 'utf8'),
);
...
describe('CreativeClicksService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CreativeClicksModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CreativeClicksModule],
      }).compile();
      const service = moduleRef.get(CreativeClicksService);
      expect(service).toBeInstanceOf(CreativeClicksService);
      await moduleRef.close();
    });

    it('exports the Site.CREATIVE_CLICKS = "creativeclicks" enum value', () => {
      expect(Site.CREATIVE_CLICKS).toBe('creativeclicks');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new CreativeClicksService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.CREATIVE_CLICKS],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'creativeclicks-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Recruitee's defaults
      expect(job0?.site).toBe(Site.CREATIVE_CLICKS);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('creativeclicks-' + first.id);
      ...
      expect(calledUrls[0]).toContain('creativeclicks.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.CREATIVE_CLICKS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('creativeclicks-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new CreativeClicksService(registry);
      const result = await service.scrape({
        siteType: [Site.CREATIVE_CLICKS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('creativeclicks');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('creativeclicks-x1');
      expect(result.jobs[0].site).toBe(Site.CREATIVE_CLICKS);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('creativeclicks-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new CreativeClicksService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.CREATIVE_CLICKS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new CreativeClicksService();
      const result = await service.scrape({
        siteType: [Site.CREATIVE_CLICKS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
