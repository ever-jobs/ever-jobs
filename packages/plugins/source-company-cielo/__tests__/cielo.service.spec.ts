...
import { CieloModule, CieloService } from '../src';

const COMPANY_NAME_EXPECT = 'Cielo';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'cielo-jobs.json'), 'utf8'),
);
...
describe('CieloService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CieloModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CieloModule],
      }).compile();
      const service = moduleRef.get(CieloService);
      expect(service).toBeInstanceOf(CieloService);
      await moduleRef.close();
    });

    it('exports the Site.CIELO = "cielo" enum value', () => {
      expect(Site.CIELO).toBe('cielo');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new CieloService(registryWithSmartRecruiters());
      ...
      expect(calledUrls[0]).toContain('Cielo2');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.CIELO);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('cielo-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new CieloService(registry);
      const result = await service.scrape({
        siteType: [Site.CIELO],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('Cielo2');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('cielo-x1');
      expect(result.jobs[0].site).toBe(Site.CIELO);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('cielo-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new CieloService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.CIELO],
      } as ScraperInputDto);
