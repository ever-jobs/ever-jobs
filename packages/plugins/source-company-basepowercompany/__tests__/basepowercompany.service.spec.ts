...
import { BasePowerCompanyModule, BasePowerCompanyService } from '../src';

const COMPANY_NAME_EXPECT = 'Base Power Company';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'basepowercompany-jobs.json'), 'utf8'),
);
...
describe('BasePowerCompanyService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through BasePowerCompanyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [BasePowerCompanyModule],
      }).compile();
      const service = moduleRef.get(BasePowerCompanyService);
      expect(service).toBeInstanceOf(BasePowerCompanyService);
      await moduleRef.close();
    });

    it('exports the Site.BASE_POWER_COMPANY = "basepowercompany" enum value', () => {
      expect(Site.BASE_POWER_COMPANY).toBe('basepowercompany');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new BasePowerCompanyService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.BASE_POWER_COMPANY],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'basepowercompany-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.BASE_POWER_COMPANY);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('basepowercompany-' + first.id);
      ...
      expect(calledUrls[0]).toContain('base-power');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.BASE_POWER_COMPANY);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('basepowercompany-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new BasePowerCompanyService(registry);
      const result = await service.scrape({
        siteType: [Site.BASE_POWER_COMPANY],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('base-power');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('basepowercompany-x1');
      expect(result.jobs[0].site).toBe(Site.BASE_POWER_COMPANY);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('basepowercompany-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new BasePowerCompanyService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.BASE_POWER_COMPANY],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new BasePowerCompanyService();
      const result = await service.scrape({
        siteType: [Site.BASE_POWER_COMPANY],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
