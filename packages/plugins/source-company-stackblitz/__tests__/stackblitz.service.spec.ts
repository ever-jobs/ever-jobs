...
import { StackBlitzModule, StackBlitzService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'stackblitz-jobs.json'), 'utf8'),
);
...
/**
 * Spec 700 / T04 — `StackBlitzService` unit tests (≥ 9 cases).
 */
describe('StackBlitzService — Spec 700 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through StackBlitzModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [StackBlitzModule],
      }).compile();
      const service = moduleRef.get(StackBlitzService);
      expect(service).toBeInstanceOf(StackBlitzService);
      await moduleRef.close();
    });

    it('exports the Site.STACKBLITZ = "stackblitz" enum value', () => {
      expect(Site.STACKBLITZ).toBe('stackblitz');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const job0 = dto.jobs.find((j) => j.id === 'stackblitz-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.STACKBLITZ);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('StackBlitz');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/stackblitz/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/stackblitz/jobs/');
      ...
      expect(job0?.description).toContain('StackBlitz');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/stackblitz/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('StackBlitz');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'stackblitz-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
