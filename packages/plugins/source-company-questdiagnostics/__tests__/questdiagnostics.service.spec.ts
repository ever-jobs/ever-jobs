...
import { QuestDiagnosticsModule, QuestDiagnosticsService } from '../src';

const COMPANY_NAME_EXPECT = 'Quest Diagnostics';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'questdiagnostics-jobs.json'), 'utf8'),
);
...
describe('QuestDiagnosticsService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through QuestDiagnosticsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [QuestDiagnosticsModule],
      }).compile();
      const service = moduleRef.get(QuestDiagnosticsService);
      expect(service).toBeInstanceOf(QuestDiagnosticsService);
      await moduleRef.close();
    });

    it('exports the Site.QUEST_DIAGNOSTICS = "questdiagnostics" enum value', () => {
      expect(Site.QUEST_DIAGNOSTICS).toBe('questdiagnostics');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new QuestDiagnosticsService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.QUEST_DIAGNOSTICS],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'questdiagnostics-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.QUEST_DIAGNOSTICS);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('questdiagnostics-' + first.id);
      ...
      expect(calledUrls[0]).toContain('QuestDiagnostics7');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.QUEST_DIAGNOSTICS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('questdiagnostics-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new QuestDiagnosticsService(registry);
      const result = await service.scrape({
        siteType: [Site.QUEST_DIAGNOSTICS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('QuestDiagnostics7');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('questdiagnostics-x1');
      expect(result.jobs[0].site).toBe(Site.QUEST_DIAGNOSTICS);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('questdiagnostics-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new QuestDiagnosticsService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.QUEST_DIAGNOSTICS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new QuestDiagnosticsService();
      const result = await service.scrape({
        siteType: [Site.QUEST_DIAGNOSTICS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
