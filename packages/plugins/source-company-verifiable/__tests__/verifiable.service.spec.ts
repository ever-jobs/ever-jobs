...
import { VerifiableModule, VerifiableService } from '../src';

const COMPANY_NAME_EXPECT = 'Verifiable';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'verifiable-jobs.json'), 'utf8'),
);
...
describe('VerifiableService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through VerifiableModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [VerifiableModule],
      }).compile();
      const service = moduleRef.get(VerifiableService);
      expect(service).toBeInstanceOf(VerifiableService);
      await moduleRef.close();
    });

    it('exports the Site.VERIFIABLE = "verifiable" enum value', () => {
      expect(Site.VERIFIABLE).toBe('verifiable');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new VerifiableService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.VERIFIABLE],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('verifiable');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.VERIFIABLE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('verifiable-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new VerifiableService(registry);
      const result = await service.scrape({
        siteType: [Site.VERIFIABLE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('verifiable');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('verifiable-x1');
      expect(result.jobs[0].site).toBe(Site.VERIFIABLE);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('verifiable-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new VerifiableService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.VERIFIABLE],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new VerifiableService();
      const result = await service.scrape({
        siteType: [Site.VERIFIABLE],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
