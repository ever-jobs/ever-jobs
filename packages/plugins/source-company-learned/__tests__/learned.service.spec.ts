...
import { LearnedModule, LearnedService } from '../src';

const COMPANY_NAME_EXPECT = 'Learned';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'learned-jobs.json'), 'utf8'),
);
...
describe('LearnedService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through LearnedModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [LearnedModule],
      }).compile();
      const service = moduleRef.get(LearnedService);
      expect(service).toBeInstanceOf(LearnedService);
      await moduleRef.close();
    });

    it('exports the Site.LEARNED = "learned" enum value', () => {
      expect(Site.LEARNED).toBe('learned');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new LearnedService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.LEARNED],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('learned.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.LEARNED);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('learned-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new LearnedService(registry);
      const result = await service.scrape({
        siteType: [Site.LEARNED],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('learned');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('learned-x1');
      expect(result.jobs[0].site).toBe(Site.LEARNED);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('learned-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new LearnedService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.LEARNED],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new LearnedService();
      const result = await service.scrape({
        siteType: [Site.LEARNED],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
