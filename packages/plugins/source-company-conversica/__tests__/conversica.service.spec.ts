...
import { ConversicaModule, ConversicaService } from '../src';

const COMPANY_NAME_EXPECT = 'Conversica';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'conversica-jobs.json'), 'utf8'),
);
...
describe('ConversicaService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ConversicaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ConversicaModule],
      }).compile();
      const service = moduleRef.get(ConversicaService);
      expect(service).toBeInstanceOf(ConversicaService);
      await moduleRef.close();
    });

    it('exports the Site.CONVERSICA = "conversica" enum value', () => {
      expect(Site.CONVERSICA).toBe('conversica');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new ConversicaService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.CONVERSICA],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('conversica');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.CONVERSICA);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('conversica-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new ConversicaService(registry);
      const result = await service.scrape({
        siteType: [Site.CONVERSICA],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('conversica');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('conversica-x1');
      expect(result.jobs[0].site).toBe(Site.CONVERSICA);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('conversica-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new ConversicaService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.CONVERSICA],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new ConversicaService();
      const result = await service.scrape({
        siteType: [Site.CONVERSICA],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
