...
/**
 * Akuna Capital — Options market making (HQ: Chicago, IL, USA).
 *
 * Source (Spec 1737): Greenhouse board, scraped in this order:
 *   - `akunacapital` — https://job-boards.greenhouse.io/akunacapital
 *     verified live 2026-09-24: 42 open postings.
 *
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'akunacapital', atsIdPrefix: 'gh-' },
];

@SourcePlugin({
  site: Site.AKUNA_CAPITAL,
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['akunacapital.com'],
  description: 'Akuna Capital careers via Greenhouse. Tags: segment=quant-trading; industry=options-market-making.',
})
@Injectable()
export class AkunaCapitalService implements IScraper {
  private readonly logger = new Logger(AkunaCapitalService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Greenhouse source plugin is not registered; cannot scrape Akuna Capital');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`Akuna Capital: delegating to Greenhouse (${board.companySlug})`);
    ...
    this.logger.log(`Akuna Capital: scraped ${jobs.length} jobs`);
