...
import { TurnerTownsendModule, TurnerTownsendService } from '../src';

const COMPANY_NAME_EXPECT = 'Turner & Townsend';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'turnertownsend-jobs.json'), 'utf8'),
);
...
describe('TurnerTownsendService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through TurnerTownsendModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [TurnerTownsendModule],
      }).compile();
      const service = moduleRef.get(TurnerTownsendService);
      expect(service).toBeInstanceOf(TurnerTownsendService);
      await moduleRef.close();
    });

    it('exports the Site.TURNER_TOWNSEND = "turnertownsend" enum value', () => {
      expect(Site.TURNER_TOWNSEND).toBe('turnertownsend');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new TurnerTownsendService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.TURNER_TOWNSEND],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'turnertownsend-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.TURNER_TOWNSEND);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('turnertownsend-' + first.id);
      ...
      expect(calledUrls[0]).toContain('TurnerTownsend');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.TURNER_TOWNSEND);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('turnertownsend-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new TurnerTownsendService(registry);
      const result = await service.scrape({
        siteType: [Site.TURNER_TOWNSEND],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('TurnerTownsend');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('turnertownsend-x1');
      expect(result.jobs[0].site).toBe(Site.TURNER_TOWNSEND);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('turnertownsend-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new TurnerTownsendService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.TURNER_TOWNSEND],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new TurnerTownsendService();
      const result = await service.scrape({
        siteType: [Site.TURNER_TOWNSEND],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
