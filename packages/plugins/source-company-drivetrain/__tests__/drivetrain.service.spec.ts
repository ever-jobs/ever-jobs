...
import { DrivetrainModule, DrivetrainService } from '../src';

const COMPANY_NAME_EXPECT = 'Drivetrain';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'drivetrain-jobs.json'), 'utf8'),
);
...
describe('DrivetrainService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through DrivetrainModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [DrivetrainModule],
      }).compile();
      const service = moduleRef.get(DrivetrainService);
      expect(service).toBeInstanceOf(DrivetrainService);
      await moduleRef.close();
    });

    it('exports the Site.DRIVETRAIN = "drivetrain" enum value', () => {
      expect(Site.DRIVETRAIN).toBe('drivetrain');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new DrivetrainService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.DRIVETRAIN],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('drivetrain');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.DRIVETRAIN);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('drivetrain-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new DrivetrainService(registry);
      const result = await service.scrape({
        siteType: [Site.DRIVETRAIN],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('drivetrain');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('drivetrain-x1');
      expect(result.jobs[0].site).toBe(Site.DRIVETRAIN);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('drivetrain-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new DrivetrainService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.DRIVETRAIN],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new DrivetrainService();
      const result = await service.scrape({
        siteType: [Site.DRIVETRAIN],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
