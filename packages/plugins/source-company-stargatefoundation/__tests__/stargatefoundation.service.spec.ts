...
import { StargateFoundationModule, StargateFoundationService } from '../src';

const COMPANY_NAME_EXPECT = 'Stargate Foundation';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'stargatefoundation-jobs.json'), 'utf8'),
);
...
describe('StargateFoundationService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through StargateFoundationModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [StargateFoundationModule],
      }).compile();
      const service = moduleRef.get(StargateFoundationService);
      expect(service).toBeInstanceOf(StargateFoundationService);
      await moduleRef.close();
    });

    it('exports the Site.STARGATE_FOUNDATION = "stargatefoundation" enum value', () => {
      expect(Site.STARGATE_FOUNDATION).toBe('stargatefoundation');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new StargateFoundationService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.STARGATE_FOUNDATION],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'stargatefoundation-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.STARGATE_FOUNDATION);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('stargatefoundation-' + first.id);
      ...
      expect(calledUrls[0]).toContain('stargate-foundation');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.STARGATE_FOUNDATION);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('stargatefoundation-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new StargateFoundationService(registry);
      const result = await service.scrape({
        siteType: [Site.STARGATE_FOUNDATION],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('stargate-foundation');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('stargatefoundation-x1');
      expect(result.jobs[0].site).toBe(Site.STARGATE_FOUNDATION);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('stargatefoundation-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new StargateFoundationService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.STARGATE_FOUNDATION],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new StargateFoundationService();
      const result = await service.scrape({
        siteType: [Site.STARGATE_FOUNDATION],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
