...
/**
 * CodePath — CodePath is an employer in the Edtech / tech career upskilling sector, headquartered in San Francisco, California, USA..
 *
 * CodePath is a company operating in Edtech / tech career upskilling,
 * headquartered in San Francisco, California, USA. This source plugin
 * ingests live open roles published on CodePath's official
 ...
 *
 * Sector: Edtech / tech career upskilling. HQ: San Francisco, California, USA.
 *
 * Highlights:
 *   - Sector: Edtech / tech career upskilling
 ...
 *     Jobs schema
 *
 * Source profile (Spec 826):
 *   - D-04 — Greenhouse canonical hosted-board host (variant 2):
 *     `https://job-boards.greenhouse.io/codepath/jobs/<id>`.
 *   - D-08 — entity-decode-then-tag-strip description pipeline.
 *   - D-09 — wire `company_name` pass-through (`'CodePath'`).
 ...
 */
const API_URL = 'https://api.greenhouse.io/v1/boards/codepath/jobs';
...
@Injectable()
export class CodePathService implements IScraper {
  private readonly logger = new Logger(CodePathService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    try {
      ...
      this.logger.log(`CodePath: fetching ${url}`);
      ...
      for (const listing of listings) {
        ...
        jobs.push(
          new JobPostDto({
            ...
            companyName: listing.company_name ?? 'CodePath',
            // D-04: wire `absolute_url` flows through (variant 2).
            jobUrl:
              listing.absolute_url ??
              `https://job-boards.greenhouse.io/codepath/jobs/${listing.id}`,
            location,
      ...
      this.logger.log(`CodePath: scraped ${jobs.length} jobs`);
    } catch (err: any) {
      this.logger.error(`CodePath scrape failed: ${err.message}`);
