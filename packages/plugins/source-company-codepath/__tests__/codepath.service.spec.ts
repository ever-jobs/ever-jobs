...
import { CodePathModule, CodePathService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'codepath-jobs.json'), 'utf8'),
);
...
/**
 * Spec 826 / T04 — `CodePathService` unit tests (≥ 9 cases).
 */
describe('CodePathService — Spec 826 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CodePathModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CodePathModule],
      }).compile();
      const service = moduleRef.get(CodePathService);
      expect(service).toBeInstanceOf(CodePathService);
      await moduleRef.close();
    });

    it('exports the Site.CODEPATH = "codepath" enum value', () => {
      expect(Site.CODEPATH).toBe('codepath');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new CodePathService();
      const result = await service.scrape({
        siteType: [Site.CODEPATH],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'codepath-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.CODEPATH);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('CodePath');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/codepath/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/codepath/jobs/');
      ...
      expect(job0?.description).toContain('CodePath');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/codepath/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('CodePath');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'codepath-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
