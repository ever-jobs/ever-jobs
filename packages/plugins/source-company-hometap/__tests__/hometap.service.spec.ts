...
import { HometapModule, HometapService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'hometap-jobs.json'), 'utf8'),
);
...
/**
 * Spec 701 / T04 — `HometapService` unit tests (≥ 9 cases).
 */
describe('HometapService — Spec 701 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through HometapModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [HometapModule],
      }).compile();
      const service = moduleRef.get(HometapService);
      expect(service).toBeInstanceOf(HometapService);
      await moduleRef.close();
    });

    it('exports the Site.HOMETAP = "hometap" enum value', () => {
      expect(Site.HOMETAP).toBe('hometap');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new HometapService();
      const result = await service.scrape({
        siteType: [Site.HOMETAP],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'hometap-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.HOMETAP);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Hometap');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/hometap/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/hometap/jobs/');
      ...
      expect(job0?.description).toContain('Hometap');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/hometap/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Hometap');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'hometap-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
