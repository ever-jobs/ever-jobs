...
import { FrontierTechnologiesModule, FrontierTechnologiesService } from '../src';

const COMPANY_NAME_EXPECT = 'Frontier Technologies';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'frontiertechnologies-jobs.json'), 'utf8'),
);
...
describe('FrontierTechnologiesService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through FrontierTechnologiesModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [FrontierTechnologiesModule],
      }).compile();
      const service = moduleRef.get(FrontierTechnologiesService);
      expect(service).toBeInstanceOf(FrontierTechnologiesService);
      await moduleRef.close();
    });

    it('exports the Site.FRONTIER_TECHNOLOGIES = "frontiertechnologies" enum value', () => {
      expect(Site.FRONTIER_TECHNOLOGIES).toBe('frontiertechnologies');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new FrontierTechnologiesService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.FRONTIER_TECHNOLOGIES],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'frontiertechnologies-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.FRONTIER_TECHNOLOGIES);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('frontiertechnologies-' + first.id);
      ...
      expect(calledUrls[0]).toContain('FrontierTechnologiesLLC1');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.FRONTIER_TECHNOLOGIES);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('frontiertechnologies-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new FrontierTechnologiesService(registry);
      const result = await service.scrape({
        siteType: [Site.FRONTIER_TECHNOLOGIES],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('FrontierTechnologiesLLC1');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('frontiertechnologies-x1');
      expect(result.jobs[0].site).toBe(Site.FRONTIER_TECHNOLOGIES);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('frontiertechnologies-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new FrontierTechnologiesService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.FRONTIER_TECHNOLOGIES],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new FrontierTechnologiesService();
      const result = await service.scrape({
        siteType: [Site.FRONTIER_TECHNOLOGIES],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
