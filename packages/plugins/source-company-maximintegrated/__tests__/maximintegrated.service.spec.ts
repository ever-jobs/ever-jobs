...
import { MaximIntegratedModule, MaximIntegratedService } from '../src';

const COMPANY_NAME_EXPECT = 'Maxim Integrated';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'maximintegrated-jobs.json'), 'utf8'),
);
...
describe('MaximIntegratedService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through MaximIntegratedModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [MaximIntegratedModule],
      }).compile();
      const service = moduleRef.get(MaximIntegratedService);
      expect(service).toBeInstanceOf(MaximIntegratedService);
      await moduleRef.close();
    });

    it('exports the Site.MAXIM_INTEGRATED = "maximintegrated" enum value', () => {
      expect(Site.MAXIM_INTEGRATED).toBe('maximintegrated');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new MaximIntegratedService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.MAXIM_INTEGRATED],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'maximintegrated-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.MAXIM_INTEGRATED);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('maximintegrated-' + first.id);
      ...
      expect(calledUrls[0]).toContain('MaximIntegrated');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.MAXIM_INTEGRATED);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('maximintegrated-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new MaximIntegratedService(registry);
      const result = await service.scrape({
        siteType: [Site.MAXIM_INTEGRATED],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('MaximIntegrated');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('maximintegrated-x1');
      expect(result.jobs[0].site).toBe(Site.MAXIM_INTEGRATED);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('maximintegrated-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new MaximIntegratedService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.MAXIM_INTEGRATED],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new MaximIntegratedService();
      const result = await service.scrape({
        siteType: [Site.MAXIM_INTEGRATED],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
