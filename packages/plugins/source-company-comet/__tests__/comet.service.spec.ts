...
import { CometModule, CometService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'comet-jobs.json'), 'utf8'),
);
...
/**
 * Spec 478 / T04 — `CometService` unit tests (≥ 9 cases).
 */
describe('CometService — Spec 478 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CometModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CometModule],
      }).compile();
      const service = moduleRef.get(CometService);
      expect(service).toBeInstanceOf(CometService);
      await moduleRef.close();
    });

    it('exports the Site.COMET = "comet" enum value', () => {
      expect(Site.COMET).toBe('comet');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new CometService();
      ...
      const job0 = dto.jobs.find((j) => j.id === 'comet-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.COMET);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Comet');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/comet/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/comet/jobs/');
      ...
      expect(job0?.description).toContain('Comet');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/comet/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Comet');
      }
    });
  });
