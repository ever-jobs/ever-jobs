...
import { EarnInModule, EarnInService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'earnin-jobs.json'), 'utf8'),
);
...
/**
 * Spec 791 / T04 — `EarnInService` unit tests (≥ 9 cases).
 */
describe('EarnInService — Spec 791 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through EarnInModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [EarnInModule],
      }).compile();
      const service = moduleRef.get(EarnInService);
      expect(service).toBeInstanceOf(EarnInService);
      await moduleRef.close();
    });

    it('exports the Site.EARNIN = "earnin" enum value', () => {
      expect(Site.EARNIN).toBe('earnin');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new EarnInService();
      const result = await service.scrape({
        siteType: [Site.EARNIN],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'earnin-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.EARNIN);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('EarnIn');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/earnin/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/earnin/jobs/');
      ...
      expect(job0?.description).toContain('EarnIn');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/earnin/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('EarnIn');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'earnin-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
