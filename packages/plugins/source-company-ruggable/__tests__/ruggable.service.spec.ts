...
import { RuggableModule, RuggableService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'ruggable-jobs.json'), 'utf8'),
);
...
/**
 * Spec 595 / T04 — `RuggableService` unit tests (≥ 9 cases).
 */
describe('RuggableService — Spec 595 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through RuggableModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [RuggableModule],
      }).compile();
      const service = moduleRef.get(RuggableService);
      expect(service).toBeInstanceOf(RuggableService);
      await moduleRef.close();
    });

    it('exports the Site.RUGGABLE = "ruggable" enum value', () => {
      expect(Site.RUGGABLE).toBe('ruggable');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new RuggableService();
      const result = await service.scrape({
        siteType: [Site.RUGGABLE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'ruggable-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.RUGGABLE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Ruggable');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/ruggable/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/ruggable/jobs/');
      ...
      expect(job0?.description).toContain('Ruggable');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/ruggable/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Ruggable');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'ruggable-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
