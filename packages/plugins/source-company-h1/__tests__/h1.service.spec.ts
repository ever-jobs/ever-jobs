...
import { H1Module, H1Service } from '../src';

const COMPANY_NAME_EXPECT = 'H1';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'h1-jobs.json'), 'utf8'),
);
...
describe('H1Service — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through H1Module via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [H1Module],
      }).compile();
      const service = moduleRef.get(H1Service);
      expect(service).toBeInstanceOf(H1Service);
      await moduleRef.close();
    });

    it('exports the Site.H1 = "h1" enum value', () => {
      expect(Site.H1).toBe('h1');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new H1Service(registryWithLever());
      ...
      expect(job0?.site).toBe(Site.H1);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('h1-' + first.id);
      ...
      expect(calledUrls[0]).toContain('h1');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.H1);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('h1-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new H1Service(registry);
      const result = await service.scrape({
        siteType: [Site.H1],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('h1');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('h1-x1');
      expect(result.jobs[0].site).toBe(Site.H1);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('h1-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new H1Service(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.H1],
      } as ScraperInputDto);
