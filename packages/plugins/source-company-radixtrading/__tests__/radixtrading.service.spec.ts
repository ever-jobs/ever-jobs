...
import { RadixTradingModule, RadixTradingService } from '../src';
...
const FIXTURE: Fixture = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'fixtures', 'radixtrading-boards.json'), 'utf8'),
);
...
describe('RadixTradingService — Greenhouse delegation (Spec 1737)', () => {
  beforeEach(() => {
    mockGet.mockReset();
    mockPost.mockReset();
  ...
  describe('registration', () => {
    it('resolves through RadixTradingModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({ imports: [RadixTradingModule] }).compile();
      expect(moduleRef.get(RadixTradingService)).toBeInstanceOf(RadixTradingService);
      await moduleRef.close();
    });

    it('exports Site.RADIX_TRADING = "radixtrading"', () => {
      expect(Site.RADIX_TRADING).toBe('radixtrading');
    });

    it('declares a tagged company plugin with its domains', () => {
      const meta = Reflect.getMetadata(SOURCE_PLUGIN_METADATA, RadixTradingService);
      expect(meta.site).toBe(Site.RADIX_TRADING);
      expect(meta.category).toBe('company');
      expect(meta.isAts).toBeFalsy();
      expect(meta.companyDomains).toEqual(['radixtrading.com']);
  ...
  describe('recorded board (real Greenhouse adapter, mocked HTTP)', () => {
    it('maps every recorded posting and re-stamps the company identity', async () => {
      const service = new RadixTradingService(registryWith());
      const result = await service.scrape({ siteType: [Site.RADIX_TRADING], resultsWanted: 100 } as ScraperInputDto);
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.RADIX_TRADING);
    ...
    it('only requests the recorded board URLs', async () => {
      const service = new RadixTradingService(registryWith());
      await service.scrape({ siteType: [Site.RADIX_TRADING] } as ScraperInputDto);
    ...
    it('honours resultsWanted=1', async () => {
      const service = new RadixTradingService(registryWith());
      const result = await service.scrape({ siteType: [Site.RADIX_TRADING], resultsWanted: 1 } as ScraperInputDto);
    ...
    it('resolves with an empty result when the board is gone (HTTP 404)', async () => {
      ...
      const service = new RadixTradingService(registryWith());
      const result = await service.scrape({ siteType: [Site.RADIX_TRADING] } as ScraperInputDto);
  ...
  describe('delegation contract', () => {
    it('forwards the board and the caller input untouched', async () => {
      ...
      const service = new RadixTradingService(
        registryWith(
          fakeBackend(
      ...
      expect(result.jobs[0].site).toBe(Site.RADIX_TRADING);
    ...
    it('never forwards the caller\'s credentials to the board (Spec 1735 §4.5)', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new RadixTradingService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
    ...
    it('makes no request when resultsWanted is 0', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new RadixTradingService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      const result = await service.scrape({ siteType: [Site.RADIX_TRADING], resultsWanted: 0 } as ScraperInputDto);
  ...
  describe('resilience', () => {
    it('reports not_registered when no registry is injected', async () => {
      const result = await new RadixTradingService().scrape({ siteType: [Site.RADIX_TRADING] } as ScraperInputDto);
      expect(result.jobs).toEqual([]);
    ...
    it('reports not_registered when Greenhouse is missing from the registry', async () => {
      const result = await new RadixTradingService(new PluginRegistry()).scrape({
        siteType: [Site.RADIX_TRADING],
      } as ScraperInputDto);
      expect(result.diagnostics?.reason).toBe('not_registered');
    });
