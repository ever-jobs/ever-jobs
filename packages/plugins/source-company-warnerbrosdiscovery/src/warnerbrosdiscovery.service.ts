...
/**
 * Warner Bros. Discovery — Media and entertainment (HQ: New York, NY, USA).
 *
 * Source (Spec 1736): Workday board, scraped in this order:
 *   - `warnerbros:5:global` — https://warnerbros.wd5.myworkdayjobs.com/global
 *     verified live 2026-09-24: 317 open postings.
 *
 ...
 * party board.
 *
 * Tags: segment=workday-enterprise; industry=media-and-entertainment.
 */
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'warnerbros:5:global', atsIdPrefix: 'wd-warnerbros-' },
];
...
@SourcePlugin({
  site: Site.WARNER_BROS_DISCOVERY,
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['wbd.com'],
  description: 'Warner Bros. Discovery careers via Workday. Tags: segment=workday-enterprise; industry=media-and-entertainment.',
})
@Injectable()
export class WarnerBrosDiscoveryService implements IScraper {
  private readonly logger = new Logger(WarnerBrosDiscoveryService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Workday source plugin is not registered; cannot scrape Warner Bros. Discovery');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`Warner Bros. Discovery: delegating to Workday (${board.companySlug})`);
      ...
      for (const job of result.jobs ?? []) {
        job.site = Site.WARNER_BROS_DISCOVERY;
    ...
    this.logger.log(`Warner Bros. Discovery: scraped ${jobs.length} jobs`);
