...
import { WarnerBrosDiscoveryModule, WarnerBrosDiscoveryService } from '../src';
...
const FIXTURE: Fixture = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'fixtures', 'warnerbrosdiscovery-boards.json'), 'utf8'),
);
const COMPANY_NAME = 'Warner Bros. Discovery';
const ID_PREFIX = 'warnerbrosdiscovery-';
...
describe('WarnerBrosDiscoveryService — Workday delegation (Spec 1736)', () => {
  beforeEach(() => {
    mockGet.mockReset();
    mockPost.mockReset();
  ...
  describe('registration', () => {
    it('resolves through WarnerBrosDiscoveryModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({ imports: [WarnerBrosDiscoveryModule] }).compile();
      expect(moduleRef.get(WarnerBrosDiscoveryService)).toBeInstanceOf(WarnerBrosDiscoveryService);
      await moduleRef.close();
    });

    it('exports Site.WARNER_BROS_DISCOVERY = "warnerbrosdiscovery"', () => {
      expect(Site.WARNER_BROS_DISCOVERY).toBe('warnerbrosdiscovery');
    });

    it('declares a tagged company plugin with its domains', () => {
      const meta = Reflect.getMetadata(SOURCE_PLUGIN_METADATA, WarnerBrosDiscoveryService);
      expect(meta.site).toBe(Site.WARNER_BROS_DISCOVERY);
      expect(meta.category).toBe('company');
      expect(meta.isAts).toBeFalsy();
      expect(meta.companyDomains).toEqual(['wbd.com']);
  ...
  describe('recorded board (real Workday adapter, mocked HTTP)', () => {
    it('maps every recorded posting and re-stamps the company identity', async () => {
      const service = new WarnerBrosDiscoveryService(registryWith());
      const result = await service.scrape({ siteType: [Site.WARNER_BROS_DISCOVERY], resultsWanted: 100 } as ScraperInputDto);
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.WARNER_BROS_DISCOVERY);
    ...
    it('only requests the recorded board URLs', async () => {
      const service = new WarnerBrosDiscoveryService(registryWith());
      await service.scrape({ siteType: [Site.WARNER_BROS_DISCOVERY] } as ScraperInputDto);
    ...
    it('honours resultsWanted=1', async () => {
      const service = new WarnerBrosDiscoveryService(registryWith());
      const result = await service.scrape({ siteType: [Site.WARNER_BROS_DISCOVERY], resultsWanted: 1 } as ScraperInputDto);
    ...
    it('resolves with an empty result when the board is gone (HTTP 404)', async () => {
      ...
      const service = new WarnerBrosDiscoveryService(registryWith());
      const result = await service.scrape({ siteType: [Site.WARNER_BROS_DISCOVERY] } as ScraperInputDto);
  ...
  describe('delegation contract', () => {
    it('forwards the board and the caller input untouched', async () => {
      ...
      const service = new WarnerBrosDiscoveryService(
        registryWith(
          fakeBackend(
      ...
      const result = await service.scrape({
        siteType: [Site.WARNER_BROS_DISCOVERY],
      ...
      expect(result.jobs[0].site).toBe(Site.WARNER_BROS_DISCOVERY);
    ...
    it('never forwards the caller\'s credentials to the board (Spec 1735 §4.5)', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new WarnerBrosDiscoveryService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
    ...
    it('makes no request when resultsWanted is 0', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new WarnerBrosDiscoveryService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      const result = await service.scrape({ siteType: [Site.WARNER_BROS_DISCOVERY], resultsWanted: 0 } as ScraperInputDto);
  ...
  describe('resilience', () => {
    it('reports not_registered when no registry is injected', async () => {
      const result = await new WarnerBrosDiscoveryService().scrape({ siteType: [Site.WARNER_BROS_DISCOVERY] } as ScraperInputDto);
      expect(result.jobs).toEqual([]);
    ...
    it('reports not_registered when Workday is missing from the registry', async () => {
      const result = await new WarnerBrosDiscoveryService(new PluginRegistry()).scrape({
        siteType: [Site.WARNER_BROS_DISCOVERY],
      } as ScraperInputDto);
      expect(result.diagnostics?.reason).toBe('not_registered');
    });
