...
import { BedrockRoboticsModule, BedrockRoboticsService } from '../src';

const COMPANY_NAME_EXPECT = 'Bedrock Robotics';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'bedrockrobotics-jobs.json'), 'utf8'),
);
...
describe('BedrockRoboticsService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through BedrockRoboticsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [BedrockRoboticsModule],
      }).compile();
      const service = moduleRef.get(BedrockRoboticsService);
      expect(service).toBeInstanceOf(BedrockRoboticsService);
      await moduleRef.close();
    });

    it('exports the Site.BEDROCK_ROBOTICS = "bedrockrobotics" enum value', () => {
      expect(Site.BEDROCK_ROBOTICS).toBe('bedrockrobotics');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new BedrockRoboticsService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.BEDROCK_ROBOTICS],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'bedrockrobotics-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.BEDROCK_ROBOTICS);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('bedrockrobotics-' + first.id);
      ...
      expect(calledUrls[0]).toContain('bedrock-robotics');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.BEDROCK_ROBOTICS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('bedrockrobotics-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new BedrockRoboticsService(registry);
      const result = await service.scrape({
        siteType: [Site.BEDROCK_ROBOTICS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('bedrock-robotics');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('bedrockrobotics-x1');
      expect(result.jobs[0].site).toBe(Site.BEDROCK_ROBOTICS);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('bedrockrobotics-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new BedrockRoboticsService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.BEDROCK_ROBOTICS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new BedrockRoboticsService();
      const result = await service.scrape({
        siteType: [Site.BEDROCK_ROBOTICS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
