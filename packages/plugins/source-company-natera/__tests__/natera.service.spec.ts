...
import { NateraModule, NateraService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'natera-jobs.json'), 'utf8'),
);
...
/**
 * Spec 529 / T04 — `NateraService` unit tests (≥ 9 cases).
 */
describe('NateraService — Spec 529 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through NateraModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [NateraModule],
      }).compile();
      const service = moduleRef.get(NateraService);
      expect(service).toBeInstanceOf(NateraService);
      await moduleRef.close();
    });

    it('exports the Site.NATERA = "natera" enum value', () => {
      expect(Site.NATERA).toBe('natera');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new NateraService();
      const result = await service.scrape({
        siteType: [Site.NATERA],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'natera-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.NATERA);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Natera');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/natera/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/natera/jobs/');
      ...
      expect(job0?.description).toContain('Natera');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/natera/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Natera');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'natera-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
