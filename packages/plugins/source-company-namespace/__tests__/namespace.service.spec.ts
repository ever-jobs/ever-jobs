...
import { NamespaceModule, NamespaceService } from '../src';

const COMPANY_NAME_EXPECT = 'Namespace';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'namespace-jobs.json'), 'utf8'),
);
...
describe('NamespaceService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through NamespaceModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [NamespaceModule],
      }).compile();
      const service = moduleRef.get(NamespaceService);
      expect(service).toBeInstanceOf(NamespaceService);
      await moduleRef.close();
    });

    it('exports the Site.NAMESPACE = "namespace" enum value', () => {
      expect(Site.NAMESPACE).toBe('namespace');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new NamespaceService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.NAMESPACE],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('namespace');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.NAMESPACE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('namespace-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new NamespaceService(registry);
      const result = await service.scrape({
        siteType: [Site.NAMESPACE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('namespace');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('namespace-x1');
      expect(result.jobs[0].site).toBe(Site.NAMESPACE);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('namespace-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new NamespaceService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.NAMESPACE],
      } as ScraperInputDto);
