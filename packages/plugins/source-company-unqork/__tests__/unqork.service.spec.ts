...
import { UnqorkModule, UnqorkService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'unqork-jobs.json'), 'utf8'),
);
...
/**
 * Spec 963 / T04 — `UnqorkService` unit tests (≥ 9 cases).
 */
describe('UnqorkService — Spec 963 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through UnqorkModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [UnqorkModule],
      }).compile();
      const service = moduleRef.get(UnqorkService);
      expect(service).toBeInstanceOf(UnqorkService);
      await moduleRef.close();
    });

    it('exports the Site.UNQORK = "unqork" enum value', () => {
      expect(Site.UNQORK).toBe('unqork');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new UnqorkService();
      const result = await service.scrape({
        siteType: [Site.UNQORK],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'unqork-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.UNQORK);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Unqork');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/unqork/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/unqork/jobs/');
      ...
      expect(job0?.description).toContain('Unqork');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/unqork/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Unqork');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'unqork-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
