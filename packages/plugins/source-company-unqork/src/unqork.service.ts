...
/**
 * Unqork — Unqork is an employer in the Insurtech and insurance technology sector, headquartered in New York, New York, USA..
 *
 * Unqork is a company operating in Insurtech and insurance technology,
 * headquartered in New York, New York, USA. This source plugin ingests
 * live open roles published on Unqork's official Greenhouse-hosted
 ...
 * credentials.
 *
 * Sector: Insurtech and insurance technology. HQ: New York, New York, USA.
 *
 * Highlights:
 ...
 *     Jobs schema
 *
 * Source profile (Spec 963):
 *   - D-04 — Greenhouse canonical hosted-board host (variant 2):
 *     `https://job-boards.greenhouse.io/unqork/jobs/<id>`.
 *   - D-08 — entity-decode-then-tag-strip description pipeline.
 *   - D-09 — wire `company_name` pass-through (`'Unqork'`).
 ...
 */
const API_URL = 'https://api.greenhouse.io/v1/boards/unqork/jobs';
...
@Injectable()
export class UnqorkService implements IScraper {
  private readonly logger = new Logger(UnqorkService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    try {
      ...
      this.logger.log(`Unqork: fetching ${url}`);
      ...
      for (const listing of listings) {
        ...
        jobs.push(
          new JobPostDto({
            ...
            companyName: listing.company_name ?? 'Unqork',
            // D-04: wire `absolute_url` flows through (variant 2).
            jobUrl:
              listing.absolute_url ??
              `https://job-boards.greenhouse.io/unqork/jobs/${listing.id}`,
            location,
      ...
      this.logger.log(`Unqork: scraped ${jobs.length} jobs`);
    } catch (err: any) {
      this.logger.error(`Unqork scrape failed: ${err.message}`);
