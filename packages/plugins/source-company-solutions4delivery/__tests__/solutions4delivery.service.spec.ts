...
import { Solutions4DeliveryModule, Solutions4DeliveryService } from '../src';

const COMPANY_NAME_EXPECT = 'Solutions 4 Delivery';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'solutions4delivery-jobs.json'), 'utf8'),
);
...
describe('Solutions4DeliveryService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through Solutions4DeliveryModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [Solutions4DeliveryModule],
      }).compile();
      const service = moduleRef.get(Solutions4DeliveryService);
      expect(service).toBeInstanceOf(Solutions4DeliveryService);
      await moduleRef.close();
    });

    it('exports the Site.SOLUTIONS_4_DELIVERY = "solutions4delivery" enum value', () => {
      expect(Site.SOLUTIONS_4_DELIVERY).toBe('solutions4delivery');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new Solutions4DeliveryService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.SOLUTIONS_4_DELIVERY],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'solutions4delivery-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Recruitee's defaults
      expect(job0?.site).toBe(Site.SOLUTIONS_4_DELIVERY);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('solutions4delivery-' + first.id);
      ...
      expect(calledUrls[0]).toContain('s4d.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SOLUTIONS_4_DELIVERY);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('solutions4delivery-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new Solutions4DeliveryService(registry);
      const result = await service.scrape({
        siteType: [Site.SOLUTIONS_4_DELIVERY],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('s4d');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('solutions4delivery-x1');
      expect(result.jobs[0].site).toBe(Site.SOLUTIONS_4_DELIVERY);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('solutions4delivery-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new Solutions4DeliveryService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.SOLUTIONS_4_DELIVERY],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new Solutions4DeliveryService();
      const result = await service.scrape({
        siteType: [Site.SOLUTIONS_4_DELIVERY],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
