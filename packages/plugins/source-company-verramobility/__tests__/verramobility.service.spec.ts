...
import { VerraMobilityModule, VerraMobilityService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'verramobility-jobs.json'), 'utf8'),
);
...
/**
 * Spec 965 / T04 — `VerraMobilityService` unit tests (≥ 9 cases).
 */
describe('VerraMobilityService — Spec 965 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through VerraMobilityModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [VerraMobilityModule],
      }).compile();
      const service = moduleRef.get(VerraMobilityService);
      expect(service).toBeInstanceOf(VerraMobilityService);
      await moduleRef.close();
    });

    it('exports the Site.VERRA_MOBILITY = "verramobility" enum value', () => {
      expect(Site.VERRA_MOBILITY).toBe('verramobility');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new VerraMobilityService();
      const result = await service.scrape({
        siteType: [Site.VERRA_MOBILITY],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'verramobility-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.VERRA_MOBILITY);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Verra Mobility');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/verramobility/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/verramobility/jobs/');
      ...
      expect(job0?.description).toContain('Verra Mobility');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/verramobility/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Verra Mobility');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'verramobility-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
