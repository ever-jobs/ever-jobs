...
import { ThirdWaveAutomationModule, ThirdWaveAutomationService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'thirdwaveautomation-jobs.json'), 'utf8'),
);
...
/**
 * Spec 956 / T04 — `ThirdWaveAutomationService` unit tests (≥ 9 cases).
 */
describe('ThirdWaveAutomationService — Spec 956 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ThirdWaveAutomationModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ThirdWaveAutomationModule],
      }).compile();
      const service = moduleRef.get(ThirdWaveAutomationService);
      expect(service).toBeInstanceOf(ThirdWaveAutomationService);
      await moduleRef.close();
    });

    it('exports the Site.THIRD_WAVE_AUTOMATION = "thirdwaveautomation" enum value', () => {
      expect(Site.THIRD_WAVE_AUTOMATION).toBe('thirdwaveautomation');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ThirdWaveAutomationService();
      const result = await service.scrape({
        siteType: [Site.THIRD_WAVE_AUTOMATION],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'thirdwaveautomation-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.THIRD_WAVE_AUTOMATION);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Third Wave Automation');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/thirdwaveautomation/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/thirdwaveautomation/jobs/');
      ...
      expect(job0?.description).toContain('Third Wave Automation');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/thirdwaveautomation/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Third Wave Automation');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'thirdwaveautomation-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
