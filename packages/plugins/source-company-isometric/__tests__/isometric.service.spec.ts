...
import { IsometricModule, IsometricService } from '../src';

const COMPANY_NAME_EXPECT = 'Isometric';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'isometric-jobs.json'), 'utf8'),
);
...
describe('IsometricService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through IsometricModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [IsometricModule],
      }).compile();
      const service = moduleRef.get(IsometricService);
      expect(service).toBeInstanceOf(IsometricService);
      await moduleRef.close();
    });

    it('exports the Site.ISOMETRIC = "isometric" enum value', () => {
      expect(Site.ISOMETRIC).toBe('isometric');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new IsometricService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.ISOMETRIC],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('isometric');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ISOMETRIC);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('isometric-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new IsometricService(registry);
      const result = await service.scrape({
        siteType: [Site.ISOMETRIC],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('isometric');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('isometric-x1');
      expect(result.jobs[0].site).toBe(Site.ISOMETRIC);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('isometric-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new IsometricService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.ISOMETRIC],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new IsometricService();
      const result = await service.scrape({
        siteType: [Site.ISOMETRIC],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
