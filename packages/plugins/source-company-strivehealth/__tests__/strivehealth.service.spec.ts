...
import { StrivehealthModule, StrivehealthService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'strivehealth-jobs.json'), 'utf8'),
);
...
/**
 * Spec 492 / T04 — `StrivehealthService` unit tests (≥ 9 cases).
 */
describe('StrivehealthService — Spec 492 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through StrivehealthModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [StrivehealthModule],
      }).compile();
      const service = moduleRef.get(StrivehealthService);
      expect(service).toBeInstanceOf(StrivehealthService);
      await moduleRef.close();
    });

    it('exports the Site.STRIVEHEALTH = "strivehealth" enum value', () => {
      expect(Site.STRIVEHEALTH).toBe('strivehealth');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new StrivehealthService();
      const result = await service.scrape({
        siteType: [Site.STRIVEHEALTH],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'strivehealth-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.STRIVEHEALTH);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Strive Health');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/strivehealth/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/strivehealth/jobs/');
      ...
      expect(job0?.description).toContain('Strive Health');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/strivehealth/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Strive Health');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'strivehealth-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
