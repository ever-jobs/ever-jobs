...
import { SunStudioModule, SunStudioService } from '../src';

const COMPANY_NAME_EXPECT = 'Sun Studio';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'sunstudio-jobs.json'), 'utf8'),
);
...
describe('SunStudioService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SunStudioModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SunStudioModule],
      }).compile();
      const service = moduleRef.get(SunStudioService);
      expect(service).toBeInstanceOf(SunStudioService);
      await moduleRef.close();
    });

    it('exports the Site.SUN_STUDIO = "sunstudio" enum value', () => {
      expect(Site.SUN_STUDIO).toBe('sunstudio');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new SunStudioService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.SUN_STUDIO],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('sunstudio');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SUN_STUDIO);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('sunstudio-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new SunStudioService(registry);
      const result = await service.scrape({
        siteType: [Site.SUN_STUDIO],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('sunstudio');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('sunstudio-x1');
      expect(result.jobs[0].site).toBe(Site.SUN_STUDIO);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('sunstudio-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new SunStudioService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.SUN_STUDIO],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new SunStudioService();
      const result = await service.scrape({
        siteType: [Site.SUN_STUDIO],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
