...
import { TaptapSendModule, TaptapSendService } from '../src';

const COMPANY_NAME_EXPECT = 'Taptap Send';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'taptapsend-jobs.json'), 'utf8'),
);
...
describe('TaptapSendService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through TaptapSendModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [TaptapSendModule],
      }).compile();
      const service = moduleRef.get(TaptapSendService);
      expect(service).toBeInstanceOf(TaptapSendService);
      await moduleRef.close();
    });

    it('exports the Site.TAPTAP_SEND = "taptapsend" enum value', () => {
      expect(Site.TAPTAP_SEND).toBe('taptapsend');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new TaptapSendService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.TAPTAP_SEND],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('taptapsend');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.TAPTAP_SEND);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('taptapsend-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new TaptapSendService(registry);
      const result = await service.scrape({
        siteType: [Site.TAPTAP_SEND],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('taptapsend');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('taptapsend-x1');
      expect(result.jobs[0].site).toBe(Site.TAPTAP_SEND);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('taptapsend-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new TaptapSendService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.TAPTAP_SEND],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new TaptapSendService();
      const result = await service.scrape({
        siteType: [Site.TAPTAP_SEND],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
