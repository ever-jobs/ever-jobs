...
import { FormationbioModule, FormationbioService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'formationbio-jobs.json'), 'utf8'),
);
...
/**
 * Spec 568 / T04 — `FormationbioService` unit tests (≥ 9 cases).
 */
describe('FormationbioService — Spec 568 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through FormationbioModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [FormationbioModule],
      }).compile();
      const service = moduleRef.get(FormationbioService);
      expect(service).toBeInstanceOf(FormationbioService);
      await moduleRef.close();
    });

    it('exports the Site.FORMATION_BIO = "formationbio" enum value', () => {
      expect(Site.FORMATION_BIO).toBe('formationbio');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new FormationbioService();
      const result = await service.scrape({
        siteType: [Site.FORMATION_BIO],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'formationbio-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.FORMATION_BIO);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Formation Bio');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/formationbio/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/formationbio/jobs/');
      ...
      expect(job0?.description).toContain('Formation Bio');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/formationbio/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Formation Bio');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'formationbio-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
