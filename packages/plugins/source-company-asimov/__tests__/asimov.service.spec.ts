...
import { AsimovModule, AsimovService } from '../src';

const COMPANY_NAME_EXPECT = 'Asimov';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'asimov-jobs.json'), 'utf8'),
);
...
describe('AsimovService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AsimovModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AsimovModule],
      }).compile();
      const service = moduleRef.get(AsimovService);
      expect(service).toBeInstanceOf(AsimovService);
      await moduleRef.close();
    });

    it('exports the Site.ASIMOV = "asimov" enum value', () => {
      expect(Site.ASIMOV).toBe('asimov');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new AsimovService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.ASIMOV],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('asimov');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ASIMOV);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('asimov-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new AsimovService(registry);
      const result = await service.scrape({
        siteType: [Site.ASIMOV],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('asimov');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('asimov-x1');
      expect(result.jobs[0].site).toBe(Site.ASIMOV);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('asimov-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new AsimovService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.ASIMOV],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new AsimovService();
      const result = await service.scrape({
        siteType: [Site.ASIMOV],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
