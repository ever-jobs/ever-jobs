...
import { NubankModule, NubankService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'nubank-jobs.json'), 'utf8'),
);
...
/**
 * Spec 486 / T04 — `NubankService` unit tests (≥ 9 cases).
 */
describe('NubankService — Spec 486 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through NubankModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [NubankModule],
      }).compile();
      const service = moduleRef.get(NubankService);
      expect(service).toBeInstanceOf(NubankService);
      await moduleRef.close();
    });

    it('exports the Site.NUBANK = "nubank" enum value', () => {
      expect(Site.NUBANK).toBe('nubank');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new NubankService();
      const result = await service.scrape({
        siteType: [Site.NUBANK],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'nubank-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.NUBANK);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Nubank');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/nubank/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/nubank/jobs/');
      ...
      expect(job0?.description).toContain('Nubank');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/nubank/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Nubank');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'nubank-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
