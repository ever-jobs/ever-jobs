...
import { PericomSemiconductorModule, PericomSemiconductorService } from '../src';

const COMPANY_NAME_EXPECT = 'Pericom Semiconductor';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'pericomsemiconductor-jobs.json'), 'utf8'),
);
...
describe('PericomSemiconductorService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through PericomSemiconductorModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [PericomSemiconductorModule],
      }).compile();
      const service = moduleRef.get(PericomSemiconductorService);
      expect(service).toBeInstanceOf(PericomSemiconductorService);
      await moduleRef.close();
    });

    it('exports the Site.PERICOM_SEMICONDUCTOR = "pericomsemiconductor" enum value', () => {
      expect(Site.PERICOM_SEMICONDUCTOR).toBe('pericomsemiconductor');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new PericomSemiconductorService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.PERICOM_SEMICONDUCTOR],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'pericomsemiconductor-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.PERICOM_SEMICONDUCTOR);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('pericomsemiconductor-' + first.id);
      ...
      expect(calledUrls[0]).toContain('PericomSemiconductorCorporation');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.PERICOM_SEMICONDUCTOR);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('pericomsemiconductor-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new PericomSemiconductorService(registry);
      const result = await service.scrape({
        siteType: [Site.PERICOM_SEMICONDUCTOR],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('PericomSemiconductorCorporation');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('pericomsemiconductor-x1');
      expect(result.jobs[0].site).toBe(Site.PERICOM_SEMICONDUCTOR);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('pericomsemiconductor-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new PericomSemiconductorService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.PERICOM_SEMICONDUCTOR],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new PericomSemiconductorService();
      const result = await service.scrape({
        siteType: [Site.PERICOM_SEMICONDUCTOR],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
