...
import { OuraModule, OuraService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'oura-jobs.json'), 'utf8'),
);
...
/**
 * Spec 531 / T04 — `OuraService` unit tests (≥ 9 cases).
 */
describe('OuraService — Spec 531 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through OuraModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [OuraModule],
      }).compile();
      const service = moduleRef.get(OuraService);
      expect(service).toBeInstanceOf(OuraService);
      await moduleRef.close();
    });

    it('exports the Site.OURA = "oura" enum value', () => {
      expect(Site.OURA).toBe('oura');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const job0 = dto.jobs.find((j) => j.id === 'oura-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.OURA);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Oura');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/oura/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/oura/jobs/');
      ...
      expect(job0?.description).toContain('Oura');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/oura/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Oura');
      }
    });
  });
