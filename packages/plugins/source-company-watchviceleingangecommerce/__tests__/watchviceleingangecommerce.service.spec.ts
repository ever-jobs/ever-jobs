...
import { WATCHVICELeingangECommerceModule, WATCHVICELeingangECommerceService } from '../src';

const COMPANY_NAME_EXPECT = 'WATCHVICE (Leingang E-Commerce)';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'watchviceleingangecommerce-jobs.json'), 'utf8'),
);
...
describe('WATCHVICELeingangECommerceService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through WATCHVICELeingangECommerceModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [WATCHVICELeingangECommerceModule],
      }).compile();
      const service = moduleRef.get(WATCHVICELeingangECommerceService);
      expect(service).toBeInstanceOf(WATCHVICELeingangECommerceService);
      await moduleRef.close();
    });

    it('exports the Site.WATCHVICE_LEINGANG_E_COMMERCE = "watchviceleingangecommerce" enum value', () => {
      expect(Site.WATCHVICE_LEINGANG_E_COMMERCE).toBe('watchviceleingangecommerce');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new WATCHVICELeingangECommerceService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.WATCHVICE_LEINGANG_E_COMMERCE],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'watchviceleingangecommerce-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Recruitee's defaults
      expect(job0?.site).toBe(Site.WATCHVICE_LEINGANG_E_COMMERCE);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('watchviceleingangecommerce-' + first.id);
      ...
      expect(calledUrls[0]).toContain('watchvice.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.WATCHVICE_LEINGANG_E_COMMERCE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('watchviceleingangecommerce-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new WATCHVICELeingangECommerceService(registry);
      const result = await service.scrape({
        siteType: [Site.WATCHVICE_LEINGANG_E_COMMERCE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('watchvice');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('watchviceleingangecommerce-x1');
      expect(result.jobs[0].site).toBe(Site.WATCHVICE_LEINGANG_E_COMMERCE);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('watchviceleingangecommerce-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new WATCHVICELeingangECommerceService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.WATCHVICE_LEINGANG_E_COMMERCE],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new WATCHVICELeingangECommerceService();
      const result = await service.scrape({
        siteType: [Site.WATCHVICE_LEINGANG_E_COMMERCE],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
