...
import { PaytmModule, PaytmService } from '../src';

const COMPANY_NAME_EXPECT = 'Paytm';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'paytm-jobs.json'), 'utf8'),
);
...
describe('PaytmService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through PaytmModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [PaytmModule],
      }).compile();
      const service = moduleRef.get(PaytmService);
      expect(service).toBeInstanceOf(PaytmService);
      await moduleRef.close();
    });

    it('exports the Site.PAYTM = "paytm" enum value', () => {
      expect(Site.PAYTM).toBe('paytm');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new PaytmService(registryWithLever());
      ...
      expect(calledUrls[0]).toContain('paytm');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.PAYTM);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('paytm-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new PaytmService(registry);
      const result = await service.scrape({
        siteType: [Site.PAYTM],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('paytm');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('paytm-x1');
      expect(result.jobs[0].site).toBe(Site.PAYTM);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('paytm-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new PaytmService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.PAYTM],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new PaytmService();
      const result = await service.scrape({
        siteType: [Site.PAYTM],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
