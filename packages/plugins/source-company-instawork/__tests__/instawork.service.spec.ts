...
import { InstaworkModule, InstaworkService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'instawork-jobs.json'), 'utf8'),
);
...
/**
 * Spec 870 / T04 — `InstaworkService` unit tests (≥ 9 cases).
 */
describe('InstaworkService — Spec 870 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through InstaworkModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [InstaworkModule],
      }).compile();
      const service = moduleRef.get(InstaworkService);
      expect(service).toBeInstanceOf(InstaworkService);
      await moduleRef.close();
    });

    it('exports the Site.INSTAWORK = "instawork" enum value', () => {
      expect(Site.INSTAWORK).toBe('instawork');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new InstaworkService();
      const result = await service.scrape({
        siteType: [Site.INSTAWORK],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'instawork-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.INSTAWORK);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Instawork');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/instawork/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/instawork/jobs/');
      ...
      expect(job0?.description).toContain('Instawork');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/instawork/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Instawork');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'instawork-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
