...
import { KeplerCommunicationsModule, KeplerCommunicationsService } from '../src';

const COMPANY_NAME_EXPECT = 'Kepler Communications';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'kepler-jobs.json'), 'utf8'),
);
...
describe('KeplerCommunicationsService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through KeplerCommunicationsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [KeplerCommunicationsModule],
      }).compile();
      const service = moduleRef.get(KeplerCommunicationsService);
      expect(service).toBeInstanceOf(KeplerCommunicationsService);
      await moduleRef.close();
    });

    it('exports the Site.KEPLER_COMMUNICATIONS = "kepler" enum value', () => {
      expect(Site.KEPLER_COMMUNICATIONS).toBe('kepler');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new KeplerCommunicationsService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.KEPLER_COMMUNICATIONS],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'kepler-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.KEPLER_COMMUNICATIONS);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('kepler-' + first.id);
      ...
      expect(calledUrls[0]).toContain('kepler');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.KEPLER_COMMUNICATIONS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('kepler-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new KeplerCommunicationsService(registry);
      const result = await service.scrape({
        siteType: [Site.KEPLER_COMMUNICATIONS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('kepler');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('kepler-x1');
      expect(result.jobs[0].site).toBe(Site.KEPLER_COMMUNICATIONS);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('kepler-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new KeplerCommunicationsService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.KEPLER_COMMUNICATIONS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new KeplerCommunicationsService();
      const result = await service.scrape({
        siteType: [Site.KEPLER_COMMUNICATIONS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
