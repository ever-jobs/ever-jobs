...
import { GRAILModule, GRAILService } from '../src';

const COMPANY_NAME_EXPECT = 'GRAIL';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'grailbio-jobs.json'), 'utf8'),
);
...
describe('GRAILService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through GRAILModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [GRAILModule],
      }).compile();
      const service = moduleRef.get(GRAILService);
      expect(service).toBeInstanceOf(GRAILService);
      await moduleRef.close();
    });

    it('exports the Site.GRAIL = "grailbio" enum value', () => {
      expect(Site.GRAIL).toBe('grailbio');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new GRAILService(registryWithLever());
      ...
      expect(calledUrls[0]).toContain('grailbio');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.GRAIL);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('grailbio-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new GRAILService(registry);
      const result = await service.scrape({
        siteType: [Site.GRAIL],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('grailbio');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('grailbio-x1');
      expect(result.jobs[0].site).toBe(Site.GRAIL);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('grailbio-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new GRAILService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.GRAIL],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new GRAILService();
      const result = await service.scrape({
        siteType: [Site.GRAIL],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
