...
import { AkidolabsModule, AkidolabsService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'akidolabs-jobs.json'), 'utf8'),
);
...
/**
 * Spec 221 / T04 — `AkidolabsService` unit tests (≥ 9 cases).
 */
describe('AkidolabsService — Spec 221 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AkidolabsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AkidolabsModule],
      }).compile();
      const service = moduleRef.get(AkidolabsService);
      expect(service).toBeInstanceOf(AkidolabsService);
      await moduleRef.close();
    });

    it('exports the Site.AKIDOLABS = "akidolabs" enum value', () => {
      expect(Site.AKIDOLABS).toBe('akidolabs');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AkidolabsService();
      const result = await service.scrape({
        siteType: [Site.AKIDOLABS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'akidolabs-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.AKIDOLABS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Akido');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/akidolabs/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/akidolabs/jobs/');
      ...
      expect(job0?.description).toContain('Akido');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/akidolabs/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Akido');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'akidolabs-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
