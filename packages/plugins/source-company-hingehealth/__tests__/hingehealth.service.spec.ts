...
import { HingeHealthModule, HingeHealthService } from '../src';

const COMPANY_NAME_EXPECT = 'Hinge Health';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'hingehealth-jobs.json'), 'utf8'),
);
...
describe('HingeHealthService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through HingeHealthModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [HingeHealthModule],
      }).compile();
      const service = moduleRef.get(HingeHealthService);
      expect(service).toBeInstanceOf(HingeHealthService);
      await moduleRef.close();
    });

    it('exports the Site.HINGE_HEALTH = "hingehealth" enum value', () => {
      expect(Site.HINGE_HEALTH).toBe('hingehealth');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new HingeHealthService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.HINGE_HEALTH],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'hingehealth-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.HINGE_HEALTH);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('hingehealth-' + first.id);
      ...
      expect(calledUrls[0]).toContain('hinge-health');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.HINGE_HEALTH);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('hingehealth-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new HingeHealthService(registry);
      const result = await service.scrape({
        siteType: [Site.HINGE_HEALTH],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('hinge-health');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('hingehealth-x1');
      expect(result.jobs[0].site).toBe(Site.HINGE_HEALTH);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('hingehealth-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new HingeHealthService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.HINGE_HEALTH],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new HingeHealthService();
      const result = await service.scrape({
        siteType: [Site.HINGE_HEALTH],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
