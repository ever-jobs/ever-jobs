...
import { QuintoAndarModule, QuintoAndarService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'quintoandar-jobs.json'), 'utf8'),
);
...
/**
 * Spec 659 / T04 — `QuintoAndarService` unit tests (≥ 9 cases).
 */
describe('QuintoAndarService — Spec 659 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through QuintoAndarModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [QuintoAndarModule],
      }).compile();
      const service = moduleRef.get(QuintoAndarService);
      expect(service).toBeInstanceOf(QuintoAndarService);
      await moduleRef.close();
    });

    it('exports the Site.QUINTOANDAR = "quintoandar" enum value', () => {
      expect(Site.QUINTOANDAR).toBe('quintoandar');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new QuintoAndarService();
      const result = await service.scrape({
        siteType: [Site.QUINTOANDAR],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'quintoandar-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.QUINTOANDAR);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('QuintoAndar');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/quintoandar/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/quintoandar/jobs/');
      ...
      expect(job0?.description).toContain('QuintoAndar');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/quintoandar/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('QuintoAndar');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'quintoandar-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
