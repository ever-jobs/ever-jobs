...
import { HyliionModule, HyliionService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'hyliion-jobs.json'), 'utf8'),
);
...
/**
 * Spec 865 / T04 — `HyliionService` unit tests (≥ 9 cases).
 */
describe('HyliionService — Spec 865 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through HyliionModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [HyliionModule],
      }).compile();
      const service = moduleRef.get(HyliionService);
      expect(service).toBeInstanceOf(HyliionService);
      await moduleRef.close();
    });

    it('exports the Site.HYLIION = "hyliion" enum value', () => {
      expect(Site.HYLIION).toBe('hyliion');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new HyliionService();
      const result = await service.scrape({
        siteType: [Site.HYLIION],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'hyliion-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.HYLIION);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Hyliion');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/hyliion/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/hyliion/jobs/');
      ...
      expect(job0?.description).toContain('Hyliion');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/hyliion/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Hyliion');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'hyliion-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
