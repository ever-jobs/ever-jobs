...
import { MainspringEnergyModule, MainspringEnergyService } from '../src';

const COMPANY_NAME_EXPECT = 'Mainspring Energy';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'mainspringenergy-jobs.json'), 'utf8'),
);
...
describe('MainspringEnergyService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through MainspringEnergyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [MainspringEnergyModule],
      }).compile();
      const service = moduleRef.get(MainspringEnergyService);
      expect(service).toBeInstanceOf(MainspringEnergyService);
      await moduleRef.close();
    });

    it('exports the Site.MAINSPRING_ENERGY = "mainspringenergy" enum value', () => {
      expect(Site.MAINSPRING_ENERGY).toBe('mainspringenergy');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new MainspringEnergyService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.MAINSPRING_ENERGY],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'mainspringenergy-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.MAINSPRING_ENERGY);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('mainspringenergy-' + first.id);
      ...
      expect(calledUrls[0]).toContain('mainspringenergy');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.MAINSPRING_ENERGY);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('mainspringenergy-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new MainspringEnergyService(registry);
      const result = await service.scrape({
        siteType: [Site.MAINSPRING_ENERGY],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('mainspringenergy');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('mainspringenergy-x1');
      expect(result.jobs[0].site).toBe(Site.MAINSPRING_ENERGY);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('mainspringenergy-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new MainspringEnergyService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.MAINSPRING_ENERGY],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new MainspringEnergyService();
      const result = await service.scrape({
        siteType: [Site.MAINSPRING_ENERGY],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
