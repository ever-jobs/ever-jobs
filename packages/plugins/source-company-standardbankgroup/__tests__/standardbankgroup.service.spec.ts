...
import { StandardBankGroupModule, StandardBankGroupService } from '../src';

const COMPANY_NAME_EXPECT = 'Standard Bank Group';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'standardbankgroup-jobs.json'), 'utf8'),
);
...
describe('StandardBankGroupService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through StandardBankGroupModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [StandardBankGroupModule],
      }).compile();
      const service = moduleRef.get(StandardBankGroupService);
      expect(service).toBeInstanceOf(StandardBankGroupService);
      await moduleRef.close();
    });

    it('exports the Site.STANDARD_BANK_GROUP = "standardbankgroup" enum value', () => {
      expect(Site.STANDARD_BANK_GROUP).toBe('standardbankgroup');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new StandardBankGroupService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.STANDARD_BANK_GROUP],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'standardbankgroup-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.STANDARD_BANK_GROUP);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('standardbankgroup-' + first.id);
      ...
      expect(calledUrls[0]).toContain('StandardBankGroup');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.STANDARD_BANK_GROUP);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('standardbankgroup-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new StandardBankGroupService(registry);
      const result = await service.scrape({
        siteType: [Site.STANDARD_BANK_GROUP],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('StandardBankGroup');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('standardbankgroup-x1');
      expect(result.jobs[0].site).toBe(Site.STANDARD_BANK_GROUP);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('standardbankgroup-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new StandardBankGroupService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.STANDARD_BANK_GROUP],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new StandardBankGroupService();
      const result = await service.scrape({
        siteType: [Site.STANDARD_BANK_GROUP],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
