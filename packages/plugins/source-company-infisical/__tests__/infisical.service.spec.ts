...
import { InfisicalModule, InfisicalService } from '../src';

const COMPANY_NAME_EXPECT = 'Infisical';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'infisical-jobs.json'), 'utf8'),
);
...
describe('InfisicalService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through InfisicalModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [InfisicalModule],
      }).compile();
      const service = moduleRef.get(InfisicalService);
      expect(service).toBeInstanceOf(InfisicalService);
      await moduleRef.close();
    });

    it('exports the Site.INFISICAL = "infisical" enum value', () => {
      expect(Site.INFISICAL).toBe('infisical');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new InfisicalService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.INFISICAL],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('infisical');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.INFISICAL);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('infisical-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new InfisicalService(registry);
      const result = await service.scrape({
        siteType: [Site.INFISICAL],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('infisical');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('infisical-x1');
      expect(result.jobs[0].site).toBe(Site.INFISICAL);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('infisical-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new InfisicalService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.INFISICAL],
      } as ScraperInputDto);
