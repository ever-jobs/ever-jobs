...
import { QualifyzeModule, QualifyzeService } from '../src';

const COMPANY_NAME_EXPECT = 'Qualifyze';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'qualifyze-jobs.json'), 'utf8'),
);
...
describe('QualifyzeService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through QualifyzeModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [QualifyzeModule],
      }).compile();
      const service = moduleRef.get(QualifyzeService);
      expect(service).toBeInstanceOf(QualifyzeService);
      await moduleRef.close();
    });

    it('exports the Site.QUALIFYZE = "qualifyze" enum value', () => {
      expect(Site.QUALIFYZE).toBe('qualifyze');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new QualifyzeService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.QUALIFYZE],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('qualifyzegmbh.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.QUALIFYZE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('qualifyze-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new QualifyzeService(registry);
      const result = await service.scrape({
        siteType: [Site.QUALIFYZE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('qualifyzegmbh');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('qualifyze-x1');
      expect(result.jobs[0].site).toBe(Site.QUALIFYZE);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('qualifyze-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new QualifyzeService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.QUALIFYZE],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new QualifyzeService();
      const result = await service.scrape({
        siteType: [Site.QUALIFYZE],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
