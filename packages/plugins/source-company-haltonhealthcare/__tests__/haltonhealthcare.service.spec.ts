...
import { HaltonHealthcareModule, HaltonHealthcareService } from '../src';

const COMPANY_NAME_EXPECT = 'Halton Healthcare';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'haltonhealthcare-jobs.json'), 'utf8'),
);
...
describe('HaltonHealthcareService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through HaltonHealthcareModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [HaltonHealthcareModule],
      }).compile();
      const service = moduleRef.get(HaltonHealthcareService);
      expect(service).toBeInstanceOf(HaltonHealthcareService);
      await moduleRef.close();
    });

    it('exports the Site.HALTON_HEALTHCARE = "haltonhealthcare" enum value', () => {
      expect(Site.HALTON_HEALTHCARE).toBe('haltonhealthcare');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new HaltonHealthcareService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.HALTON_HEALTHCARE],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'haltonhealthcare-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.HALTON_HEALTHCARE);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('haltonhealthcare-' + first.id);
      ...
      expect(calledUrls[0]).toContain('HaltonHealthcare1');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.HALTON_HEALTHCARE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('haltonhealthcare-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new HaltonHealthcareService(registry);
      const result = await service.scrape({
        siteType: [Site.HALTON_HEALTHCARE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('HaltonHealthcare1');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('haltonhealthcare-x1');
      expect(result.jobs[0].site).toBe(Site.HALTON_HEALTHCARE);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('haltonhealthcare-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new HaltonHealthcareService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.HALTON_HEALTHCARE],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new HaltonHealthcareService();
      const result = await service.scrape({
        siteType: [Site.HALTON_HEALTHCARE],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
