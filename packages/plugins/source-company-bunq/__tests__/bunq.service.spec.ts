...
import { BunqModule, BunqService } from '../src';

const COMPANY_NAME_EXPECT = 'bunq';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'bunq-jobs.json'), 'utf8'),
);
...
describe('BunqService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through BunqModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [BunqModule],
      }).compile();
      const service = moduleRef.get(BunqService);
      expect(service).toBeInstanceOf(BunqService);
      await moduleRef.close();
    });

    it('exports the Site.BUNQ = "bunq" enum value', () => {
      expect(Site.BUNQ).toBe('bunq');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new BunqService(registryWithRecruitee());
      ...
      expect(calledUrls[0]).toContain('bunq.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.BUNQ);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('bunq-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new BunqService(registry);
      const result = await service.scrape({
        siteType: [Site.BUNQ],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('bunq');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('bunq-x1');
      expect(result.jobs[0].site).toBe(Site.BUNQ);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('bunq-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new BunqService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.BUNQ],
      } as ScraperInputDto);
