...
import { KIPPModule, KIPPService } from '../src';

const COMPANY_NAME_EXPECT = 'KIPP';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'kipp-jobs.json'), 'utf8'),
);
...
describe('KIPPService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through KIPPModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [KIPPModule],
      }).compile();
      const service = moduleRef.get(KIPPService);
      expect(service).toBeInstanceOf(KIPPService);
      await moduleRef.close();
    });

    it('exports the Site.KIPP = "kipp" enum value', () => {
      expect(Site.KIPP).toBe('kipp');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new KIPPService(registryWithSmartRecruiters());
      ...
      expect(calledUrls[0]).toContain('KIPP');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.KIPP);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('kipp-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new KIPPService(registry);
      const result = await service.scrape({
        siteType: [Site.KIPP],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('KIPP');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('kipp-x1');
      expect(result.jobs[0].site).toBe(Site.KIPP);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('kipp-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new KIPPService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.KIPP],
      } as ScraperInputDto);
