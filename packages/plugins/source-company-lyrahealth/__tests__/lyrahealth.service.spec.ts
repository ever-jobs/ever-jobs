...
import { LyraHealthModule, LyraHealthService } from '../src';

const COMPANY_NAME_EXPECT = 'Lyra Health';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'lyrahealth-jobs.json'), 'utf8'),
);
...
describe('LyraHealthService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through LyraHealthModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [LyraHealthModule],
      }).compile();
      const service = moduleRef.get(LyraHealthService);
      expect(service).toBeInstanceOf(LyraHealthService);
      await moduleRef.close();
    });

    it('exports the Site.LYRA_HEALTH = "lyrahealth" enum value', () => {
      expect(Site.LYRA_HEALTH).toBe('lyrahealth');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new LyraHealthService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.LYRA_HEALTH],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('lyrahealth');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.LYRA_HEALTH);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('lyrahealth-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new LyraHealthService(registry);
      const result = await service.scrape({
        siteType: [Site.LYRA_HEALTH],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('lyrahealth');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('lyrahealth-x1');
      expect(result.jobs[0].site).toBe(Site.LYRA_HEALTH);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('lyrahealth-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new LyraHealthService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.LYRA_HEALTH],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new LyraHealthService();
      const result = await service.scrape({
        siteType: [Site.LYRA_HEALTH],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
