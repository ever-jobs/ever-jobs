...
import { HanwhaRenewablesModule, HanwhaRenewablesService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'hanwharenewables-jobs.json'), 'utf8'),
);
...
/**
 * Spec 862 / T04 — `HanwhaRenewablesService` unit tests (≥ 9 cases).
 */
describe('HanwhaRenewablesService — Spec 862 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through HanwhaRenewablesModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [HanwhaRenewablesModule],
      }).compile();
      const service = moduleRef.get(HanwhaRenewablesService);
      expect(service).toBeInstanceOf(HanwhaRenewablesService);
      await moduleRef.close();
    });

    it('exports the Site.HANWHA_RENEWABLES = "hanwharenewables" enum value', () => {
      expect(Site.HANWHA_RENEWABLES).toBe('hanwharenewables');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new HanwhaRenewablesService();
      const result = await service.scrape({
        siteType: [Site.HANWHA_RENEWABLES],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'hanwharenewables-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.HANWHA_RENEWABLES);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Hanwha Renewables');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/hanwharenewables/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/hanwharenewables/jobs/');
      ...
      expect(job0?.description).toContain('Hanwha Renewables');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/hanwharenewables/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Hanwha Renewables');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'hanwharenewables-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
