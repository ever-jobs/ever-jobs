...
import { EpirusModule, EpirusService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'epirus-jobs.json'), 'utf8'),
);
...
/**
 * Spec 463 / T04 — `EpirusService` unit tests (≥ 9 cases).
 */
describe('EpirusService — Spec 463 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through EpirusModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [EpirusModule],
      }).compile();
      const service = moduleRef.get(EpirusService);
      expect(service).toBeInstanceOf(EpirusService);
      await moduleRef.close();
    });

    it('exports the Site.EPIRUS = "epirus" enum value', () => {
      expect(Site.EPIRUS).toBe('epirus');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new EpirusService();
      const result = await service.scrape({
        siteType: [Site.EPIRUS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'epirus-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.EPIRUS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Epirus');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/epirus/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/epirus/jobs/');
      ...
      expect(job0?.description).toContain('Epirus');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/epirus/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Epirus');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'epirus-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
