...
import { OneTrustModule, OneTrustService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'onetrust-jobs.json'), 'utf8'),
);
...
/**
 * Spec 899 / T04 — `OneTrustService` unit tests (≥ 9 cases).
 */
describe('OneTrustService — Spec 899 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through OneTrustModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [OneTrustModule],
      }).compile();
      const service = moduleRef.get(OneTrustService);
      expect(service).toBeInstanceOf(OneTrustService);
      await moduleRef.close();
    });

    it('exports the Site.ONETRUST = "onetrust" enum value', () => {
      expect(Site.ONETRUST).toBe('onetrust');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new OneTrustService();
      const result = await service.scrape({
        siteType: [Site.ONETRUST],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'onetrust-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ONETRUST);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('OneTrust');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/onetrust/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/onetrust/jobs/');
      ...
      expect(job0?.description).toContain('OneTrust');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/onetrust/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('OneTrust');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'onetrust-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
