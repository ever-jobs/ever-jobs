...
import { HarmattanAIModule, HarmattanAIService } from '../src';

const COMPANY_NAME_EXPECT = 'Harmattan AI';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'harmattanai-jobs.json'), 'utf8'),
);
...
describe('HarmattanAIService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through HarmattanAIModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [HarmattanAIModule],
      }).compile();
      const service = moduleRef.get(HarmattanAIService);
      expect(service).toBeInstanceOf(HarmattanAIService);
      await moduleRef.close();
    });

    it('exports the Site.HARMATTAN_AI = "harmattanai" enum value', () => {
      expect(Site.HARMATTAN_AI).toBe('harmattanai');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new HarmattanAIService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.HARMATTAN_AI],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'harmattanai-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.HARMATTAN_AI);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('harmattanai-' + first.id);
      ...
      expect(calledUrls[0]).toContain('harmattan-ai');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.HARMATTAN_AI);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('harmattanai-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new HarmattanAIService(registry);
      const result = await service.scrape({
        siteType: [Site.HARMATTAN_AI],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('harmattan-ai');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('harmattanai-x1');
      expect(result.jobs[0].site).toBe(Site.HARMATTAN_AI);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('harmattanai-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new HarmattanAIService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.HARMATTAN_AI],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new HarmattanAIService();
      const result = await service.scrape({
        siteType: [Site.HARMATTAN_AI],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
