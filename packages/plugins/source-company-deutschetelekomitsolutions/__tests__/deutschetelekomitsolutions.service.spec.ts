...
import { DeutscheTelekomITSolutionsModule, DeutscheTelekomITSolutionsService } from '../src';

const COMPANY_NAME_EXPECT = 'Deutsche Telekom IT Solutions';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'deutschetelekomitsolutions-jobs.json'), 'utf8'),
);
...
describe('DeutscheTelekomITSolutionsService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through DeutscheTelekomITSolutionsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [DeutscheTelekomITSolutionsModule],
      }).compile();
      const service = moduleRef.get(DeutscheTelekomITSolutionsService);
      expect(service).toBeInstanceOf(DeutscheTelekomITSolutionsService);
      await moduleRef.close();
    });

    it('exports the Site.DEUTSCHE_TELEKOM_IT_SOLUTIONS = "deutschetelekomitsolutions" enum value', () => {
      expect(Site.DEUTSCHE_TELEKOM_IT_SOLUTIONS).toBe('deutschetelekomitsolutions');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new DeutscheTelekomITSolutionsService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.DEUTSCHE_TELEKOM_IT_SOLUTIONS],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'deutschetelekomitsolutions-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.DEUTSCHE_TELEKOM_IT_SOLUTIONS);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('deutschetelekomitsolutions-' + first.id);
      ...
      expect(calledUrls[0]).toContain('deutschetelekomitsolutions');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.DEUTSCHE_TELEKOM_IT_SOLUTIONS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('deutschetelekomitsolutions-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new DeutscheTelekomITSolutionsService(registry);
      const result = await service.scrape({
        siteType: [Site.DEUTSCHE_TELEKOM_IT_SOLUTIONS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('deutschetelekomitsolutions');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('deutschetelekomitsolutions-x1');
      expect(result.jobs[0].site).toBe(Site.DEUTSCHE_TELEKOM_IT_SOLUTIONS);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('deutschetelekomitsolutions-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new DeutscheTelekomITSolutionsService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.DEUTSCHE_TELEKOM_IT_SOLUTIONS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new DeutscheTelekomITSolutionsService();
      const result = await service.scrape({
        siteType: [Site.DEUTSCHE_TELEKOM_IT_SOLUTIONS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
