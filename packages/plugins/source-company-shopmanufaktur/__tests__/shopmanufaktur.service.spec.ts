...
import { ShopManufakturModule, ShopManufakturService } from '../src';

const COMPANY_NAME_EXPECT = 'Shop Manufaktur';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'shopmanufaktur-jobs.json'), 'utf8'),
);
...
describe('ShopManufakturService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ShopManufakturModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ShopManufakturModule],
      }).compile();
      const service = moduleRef.get(ShopManufakturService);
      expect(service).toBeInstanceOf(ShopManufakturService);
      await moduleRef.close();
    });

    it('exports the Site.SHOP_MANUFAKTUR = "shopmanufaktur" enum value', () => {
      expect(Site.SHOP_MANUFAKTUR).toBe('shopmanufaktur');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new ShopManufakturService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.SHOP_MANUFAKTUR],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'shopmanufaktur-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Recruitee's defaults
      expect(job0?.site).toBe(Site.SHOP_MANUFAKTUR);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('shopmanufaktur-' + first.id);
      ...
      expect(calledUrls[0]).toContain('shopmanufaktur.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SHOP_MANUFAKTUR);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('shopmanufaktur-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new ShopManufakturService(registry);
      const result = await service.scrape({
        siteType: [Site.SHOP_MANUFAKTUR],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('shopmanufaktur');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('shopmanufaktur-x1');
      expect(result.jobs[0].site).toBe(Site.SHOP_MANUFAKTUR);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('shopmanufaktur-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new ShopManufakturService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.SHOP_MANUFAKTUR],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new ShopManufakturService();
      const result = await service.scrape({
        siteType: [Site.SHOP_MANUFAKTUR],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
