...
import { Version1Module, Version1Service } from '../src';

const COMPANY_NAME_EXPECT = 'Version 1';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'version1-jobs.json'), 'utf8'),
);
...
describe('Version1Service — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through Version1Module via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [Version1Module],
      }).compile();
      const service = moduleRef.get(Version1Service);
      expect(service).toBeInstanceOf(Version1Service);
      await moduleRef.close();
    });

    it('exports the Site.VERSION_1 = "version1" enum value', () => {
      expect(Site.VERSION_1).toBe('version1');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new Version1Service(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.VERSION_1],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('Version1');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.VERSION_1);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('version1-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new Version1Service(registry);
      const result = await service.scrape({
        siteType: [Site.VERSION_1],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('Version1');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('version1-x1');
      expect(result.jobs[0].site).toBe(Site.VERSION_1);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('version1-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new Version1Service(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.VERSION_1],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new Version1Service();
      const result = await service.scrape({
        siteType: [Site.VERSION_1],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
