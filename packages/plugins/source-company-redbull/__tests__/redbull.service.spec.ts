...
import { RedBullModule, RedBullService } from '../src';

const COMPANY_NAME_EXPECT = 'Red Bull';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'redbull-jobs.json'), 'utf8'),
);
...
describe('RedBullService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through RedBullModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [RedBullModule],
      }).compile();
      const service = moduleRef.get(RedBullService);
      expect(service).toBeInstanceOf(RedBullService);
      await moduleRef.close();
    });

    it('exports the Site.RED_BULL = "redbull" enum value', () => {
      expect(Site.RED_BULL).toBe('redbull');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new RedBullService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.RED_BULL],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('RedBull');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.RED_BULL);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('redbull-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new RedBullService(registry);
      const result = await service.scrape({
        siteType: [Site.RED_BULL],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('RedBull');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('redbull-x1');
      expect(result.jobs[0].site).toBe(Site.RED_BULL);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('redbull-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new RedBullService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.RED_BULL],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new RedBullService();
      const result = await service.scrape({
        siteType: [Site.RED_BULL],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
