...
import { PalmettoCleanTechnologyModule, PalmettoCleanTechnologyService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'palmettocleantech-jobs.json'), 'utf8'),
);
...
/**
 * Spec 907 / T04 — `PalmettoCleanTechnologyService` unit tests (≥ 9 cases).
 */
describe('PalmettoCleanTechnologyService — Spec 907 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through PalmettoCleanTechnologyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [PalmettoCleanTechnologyModule],
      }).compile();
      const service = moduleRef.get(PalmettoCleanTechnologyService);
      expect(service).toBeInstanceOf(PalmettoCleanTechnologyService);
      await moduleRef.close();
    });

    it('exports the Site.PALMETTO_CLEAN_TECHNOLOGY = "palmettocleantech" enum value', () => {
      expect(Site.PALMETTO_CLEAN_TECHNOLOGY).toBe('palmettocleantech');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new PalmettoCleanTechnologyService();
      const result = await service.scrape({
        siteType: [Site.PALMETTO_CLEAN_TECHNOLOGY],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'palmettocleantech-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.PALMETTO_CLEAN_TECHNOLOGY);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Palmetto Clean Technology');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/palmettocleantech/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/palmettocleantech/jobs/');
      ...
      expect(job0?.description).toContain('Palmetto Clean Technology');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/palmettocleantech/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Palmetto Clean Technology');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'palmettocleantech-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
