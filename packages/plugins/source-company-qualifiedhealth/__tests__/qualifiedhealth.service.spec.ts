...
import { QualifiedHealthModule, QualifiedHealthService } from '../src';

const COMPANY_NAME_EXPECT = 'Qualified Health';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'qualifiedhealth-jobs.json'), 'utf8'),
);
...
describe('QualifiedHealthService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through QualifiedHealthModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [QualifiedHealthModule],
      }).compile();
      const service = moduleRef.get(QualifiedHealthService);
      expect(service).toBeInstanceOf(QualifiedHealthService);
      await moduleRef.close();
    });

    it('exports the Site.QUALIFIED_HEALTH = "qualifiedhealth" enum value', () => {
      expect(Site.QUALIFIED_HEALTH).toBe('qualifiedhealth');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new QualifiedHealthService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.QUALIFIED_HEALTH],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'qualifiedhealth-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.QUALIFIED_HEALTH);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('qualifiedhealth-' + first.id);
      ...
      expect(calledUrls[0]).toContain('qualified-health-pbc');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.QUALIFIED_HEALTH);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('qualifiedhealth-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new QualifiedHealthService(registry);
      const result = await service.scrape({
        siteType: [Site.QUALIFIED_HEALTH],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('qualified-health-pbc');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('qualifiedhealth-x1');
      expect(result.jobs[0].site).toBe(Site.QUALIFIED_HEALTH);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('qualifiedhealth-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new QualifiedHealthService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.QUALIFIED_HEALTH],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new QualifiedHealthService();
      const result = await service.scrape({
        siteType: [Site.QUALIFIED_HEALTH],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
