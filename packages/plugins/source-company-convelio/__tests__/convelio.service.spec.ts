...
import { ConvelioModule, ConvelioService } from '../src';

const COMPANY_NAME_EXPECT = 'Convelio';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'convelio-jobs.json'), 'utf8'),
);
...
describe('ConvelioService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ConvelioModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ConvelioModule],
      }).compile();
      const service = moduleRef.get(ConvelioService);
      expect(service).toBeInstanceOf(ConvelioService);
      await moduleRef.close();
    });

    it('exports the Site.CONVELIO = "convelio" enum value', () => {
      expect(Site.CONVELIO).toBe('convelio');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new ConvelioService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.CONVELIO],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('convelio');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.CONVELIO);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('convelio-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new ConvelioService(registry);
      const result = await service.scrape({
        siteType: [Site.CONVELIO],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('convelio');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('convelio-x1');
      expect(result.jobs[0].site).toBe(Site.CONVELIO);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('convelio-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new ConvelioService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.CONVELIO],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new ConvelioService();
      const result = await service.scrape({
        siteType: [Site.CONVELIO],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
