...
import { TesseraTherapeuticsModule, TesseraTherapeuticsService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'tesseratherapeutics-jobs.json'), 'utf8'),
);
...
/**
 * Spec 952 / T04 — `TesseraTherapeuticsService` unit tests (≥ 9 cases).
 */
describe('TesseraTherapeuticsService — Spec 952 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through TesseraTherapeuticsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [TesseraTherapeuticsModule],
      }).compile();
      const service = moduleRef.get(TesseraTherapeuticsService);
      expect(service).toBeInstanceOf(TesseraTherapeuticsService);
      await moduleRef.close();
    });

    it('exports the Site.TESSERA_THERAPEUTICS = "tesseratherapeutics" enum value', () => {
      expect(Site.TESSERA_THERAPEUTICS).toBe('tesseratherapeutics');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new TesseraTherapeuticsService();
      const result = await service.scrape({
        siteType: [Site.TESSERA_THERAPEUTICS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'tesseratherapeutics-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.TESSERA_THERAPEUTICS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Tessera Therapeutics');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/tesseratherapeutics/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/tesseratherapeutics/jobs/');
      ...
      expect(job0?.description).toContain('Tessera Therapeutics');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/tesseratherapeutics/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Tessera Therapeutics');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'tesseratherapeutics-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
