...
import { RapidMicroBiosystemsModule, RapidMicroBiosystemsService } from '../src';

const COMPANY_NAME_EXPECT = 'Rapid Micro Biosystems';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'rapidmicrobio-jobs.json'), 'utf8'),
);
...
describe('RapidMicroBiosystemsService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through RapidMicroBiosystemsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [RapidMicroBiosystemsModule],
      }).compile();
      const service = moduleRef.get(RapidMicroBiosystemsService);
      expect(service).toBeInstanceOf(RapidMicroBiosystemsService);
      await moduleRef.close();
    });

    it('exports the Site.RAPID_MICRO_BIOSYSTEMS = "rapidmicrobio" enum value', () => {
      expect(Site.RAPID_MICRO_BIOSYSTEMS).toBe('rapidmicrobio');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new RapidMicroBiosystemsService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.RAPID_MICRO_BIOSYSTEMS],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'rapidmicrobio-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.RAPID_MICRO_BIOSYSTEMS);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('rapidmicrobio-' + first.id);
      ...
      expect(calledUrls[0]).toContain('rapidmicrobio');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.RAPID_MICRO_BIOSYSTEMS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('rapidmicrobio-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new RapidMicroBiosystemsService(registry);
      const result = await service.scrape({
        siteType: [Site.RAPID_MICRO_BIOSYSTEMS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('rapidmicrobio');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('rapidmicrobio-x1');
      expect(result.jobs[0].site).toBe(Site.RAPID_MICRO_BIOSYSTEMS);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('rapidmicrobio-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new RapidMicroBiosystemsService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.RAPID_MICRO_BIOSYSTEMS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new RapidMicroBiosystemsService();
      const result = await service.scrape({
        siteType: [Site.RAPID_MICRO_BIOSYSTEMS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
