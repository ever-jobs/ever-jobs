...
import { StaffingMedicalUSAModule, StaffingMedicalUSAService } from '../src';

const COMPANY_NAME_EXPECT = 'Staffing Medical USA';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'staffingmedicalusa-jobs.json'), 'utf8'),
);
...
describe('StaffingMedicalUSAService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through StaffingMedicalUSAModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [StaffingMedicalUSAModule],
      }).compile();
      const service = moduleRef.get(StaffingMedicalUSAService);
      expect(service).toBeInstanceOf(StaffingMedicalUSAService);
      await moduleRef.close();
    });

    it('exports the Site.STAFFING_MEDICAL_USA = "staffingmedicalusa" enum value', () => {
      expect(Site.STAFFING_MEDICAL_USA).toBe('staffingmedicalusa');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new StaffingMedicalUSAService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.STAFFING_MEDICAL_USA],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'staffingmedicalusa-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.STAFFING_MEDICAL_USA);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('staffingmedicalusa-' + first.id);
      ...
      expect(calledUrls[0]).toContain('StaffingMedicalUSA');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.STAFFING_MEDICAL_USA);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('staffingmedicalusa-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new StaffingMedicalUSAService(registry);
      const result = await service.scrape({
        siteType: [Site.STAFFING_MEDICAL_USA],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('StaffingMedicalUSA');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('staffingmedicalusa-x1');
      expect(result.jobs[0].site).toBe(Site.STAFFING_MEDICAL_USA);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('staffingmedicalusa-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new StaffingMedicalUSAService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.STAFFING_MEDICAL_USA],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new StaffingMedicalUSAService();
      const result = await service.scrape({
        siteType: [Site.STAFFING_MEDICAL_USA],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
