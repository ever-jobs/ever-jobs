...
import { Addepar1Module, Addepar1Service } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'addepar1-jobs.json'), 'utf8'),
);
...
/**
 * Spec 190 / T04 — `Addepar1Service` unit tests (≥ 9 cases).
 */
describe('Addepar1Service — Spec 190 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through Addepar1Module via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [Addepar1Module],
      }).compile();
      const service = moduleRef.get(Addepar1Service);
      expect(service).toBeInstanceOf(Addepar1Service);
      await moduleRef.close();
    });

    it('exports the Site.ADDEPAR1 = "addepar1" enum value', () => {
      expect(Site.ADDEPAR1).toBe('addepar1');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new Addepar1Service();
      const result = await service.scrape({
        siteType: [Site.ADDEPAR1],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'addepar1-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ADDEPAR1);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Addepar');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/addepar1/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/addepar1/jobs/');
      ...
      expect(job0?.description).toContain('Addepar');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/addepar1/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Addepar');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'addepar1-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
