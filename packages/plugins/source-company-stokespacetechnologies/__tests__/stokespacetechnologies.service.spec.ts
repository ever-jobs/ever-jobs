...
import { StokeSpaceModule, StokeSpaceService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'stokespacetechnologies-jobs.json'), 'utf8'),
);
...
/**
 * Spec 944 / T04 — `StokeSpaceService` unit tests (≥ 9 cases).
 */
describe('StokeSpaceService — Spec 944 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through StokeSpaceModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [StokeSpaceModule],
      }).compile();
      const service = moduleRef.get(StokeSpaceService);
      expect(service).toBeInstanceOf(StokeSpaceService);
      await moduleRef.close();
    });

    it('exports the Site.STOKE_SPACE = "stokespacetechnologies" enum value', () => {
      expect(Site.STOKE_SPACE).toBe('stokespacetechnologies');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new StokeSpaceService();
      const result = await service.scrape({
        siteType: [Site.STOKE_SPACE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'stokespacetechnologies-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.STOKE_SPACE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Stoke Space');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/stokespacetechnologies/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/stokespacetechnologies/jobs/');
      ...
      expect(job0?.description).toContain('Stoke Space');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/stokespacetechnologies/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Stoke Space');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'stokespacetechnologies-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
