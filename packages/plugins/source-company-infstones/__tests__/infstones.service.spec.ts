...
import { InfStonesModule, InfStonesService } from '../src';

const COMPANY_NAME_EXPECT = 'InfStones';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'infstones-jobs.json'), 'utf8'),
);
...
describe('InfStonesService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through InfStonesModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [InfStonesModule],
      }).compile();
      const service = moduleRef.get(InfStonesService);
      expect(service).toBeInstanceOf(InfStonesService);
      await moduleRef.close();
    });

    it('exports the Site.INFSTONES = "infstones" enum value', () => {
      expect(Site.INFSTONES).toBe('infstones');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new InfStonesService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.INFSTONES],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('infstones');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.INFSTONES);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('infstones-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new InfStonesService(registry);
      const result = await service.scrape({
        siteType: [Site.INFSTONES],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('infstones');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('infstones-x1');
      expect(result.jobs[0].site).toBe(Site.INFSTONES);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('infstones-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new InfStonesService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.INFSTONES],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new InfStonesService();
      const result = await service.scrape({
        siteType: [Site.INFSTONES],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
