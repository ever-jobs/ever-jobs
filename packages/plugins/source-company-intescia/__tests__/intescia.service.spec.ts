...
import { IntesciaModule, IntesciaService } from '../src';

const COMPANY_NAME_EXPECT = 'Intescia';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'intescia-jobs.json'), 'utf8'),
);
...
describe('IntesciaService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through IntesciaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [IntesciaModule],
      }).compile();
      const service = moduleRef.get(IntesciaService);
      expect(service).toBeInstanceOf(IntesciaService);
      await moduleRef.close();
    });

    it('exports the Site.INTESCIA = "intescia" enum value', () => {
      expect(Site.INTESCIA).toBe('intescia');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new IntesciaService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.INTESCIA],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('intescia.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.INTESCIA);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('intescia-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new IntesciaService(registry);
      const result = await service.scrape({
        siteType: [Site.INTESCIA],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('intescia');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('intescia-x1');
      expect(result.jobs[0].site).toBe(Site.INTESCIA);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('intescia-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new IntesciaService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.INTESCIA],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new IntesciaService();
      const result = await service.scrape({
        siteType: [Site.INTESCIA],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
