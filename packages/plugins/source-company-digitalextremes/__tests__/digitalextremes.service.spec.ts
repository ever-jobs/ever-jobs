...
import { DigitalExtremesModule, DigitalExtremesService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'digitalextremes-jobs.json'), 'utf8'),
);
...
/**
 * Spec 836 / T04 — `DigitalExtremesService` unit tests (≥ 9 cases).
 */
describe('DigitalExtremesService — Spec 836 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through DigitalExtremesModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [DigitalExtremesModule],
      }).compile();
      const service = moduleRef.get(DigitalExtremesService);
      expect(service).toBeInstanceOf(DigitalExtremesService);
      await moduleRef.close();
    });

    it('exports the Site.DIGITAL_EXTREMES = "digitalextremes" enum value', () => {
      expect(Site.DIGITAL_EXTREMES).toBe('digitalextremes');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new DigitalExtremesService();
      const result = await service.scrape({
        siteType: [Site.DIGITAL_EXTREMES],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'digitalextremes-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.DIGITAL_EXTREMES);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Digital Extremes');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/digitalextremes/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/digitalextremes/jobs/');
      ...
      expect(job0?.description).toContain('Digital Extremes');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/digitalextremes/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Digital Extremes');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'digitalextremes-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
