...
import { MercariModule, MercariService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'mercari-jobs.json'), 'utf8'),
);
...
/**
 * Spec 485 / T04 — `MercariService` unit tests (≥ 9 cases).
 */
describe('MercariService — Spec 485 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through MercariModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [MercariModule],
      }).compile();
      const service = moduleRef.get(MercariService);
      expect(service).toBeInstanceOf(MercariService);
      await moduleRef.close();
    });

    it('exports the Site.MERCARI = "mercari" enum value', () => {
      expect(Site.MERCARI).toBe('mercari');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new MercariService();
      const result = await service.scrape({
        siteType: [Site.MERCARI],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'mercari-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.MERCARI);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Mercari');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/mercari/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/mercari/jobs/');
      ...
      expect(job0?.description).toContain('Mercari');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/mercari/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Mercari');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'mercari-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
