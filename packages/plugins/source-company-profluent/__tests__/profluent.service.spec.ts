...
import { ProfluentModule, ProfluentService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'profluent-jobs.json'), 'utf8'),
);
...
/**
 * Spec 691 / T04 — `ProfluentService` unit tests (≥ 9 cases).
 */
describe('ProfluentService — Spec 691 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ProfluentModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ProfluentModule],
      }).compile();
      const service = moduleRef.get(ProfluentService);
      expect(service).toBeInstanceOf(ProfluentService);
      await moduleRef.close();
    });

    it('exports the Site.PROFLUENT = "profluent" enum value', () => {
      expect(Site.PROFLUENT).toBe('profluent');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ProfluentService();
      const result = await service.scrape({
        siteType: [Site.PROFLUENT],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'profluent-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.PROFLUENT);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Profluent');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/profluent/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/profluent/jobs/');
      ...
      expect(job0?.description).toContain('Profluent');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/profluent/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Profluent');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'profluent-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
