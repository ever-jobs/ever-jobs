...
import { CentreonModule, CentreonService } from '../src';

const COMPANY_NAME_EXPECT = 'Centreon';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'centreon-jobs.json'), 'utf8'),
);
...
describe('CentreonService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CentreonModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CentreonModule],
      }).compile();
      const service = moduleRef.get(CentreonService);
      expect(service).toBeInstanceOf(CentreonService);
      await moduleRef.close();
    });

    it('exports the Site.CENTREON = "centreon" enum value', () => {
      expect(Site.CENTREON).toBe('centreon');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new CentreonService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.CENTREON],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('centreon.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.CENTREON);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('centreon-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new CentreonService(registry);
      const result = await service.scrape({
        siteType: [Site.CENTREON],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('centreon');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('centreon-x1');
      expect(result.jobs[0].site).toBe(Site.CENTREON);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('centreon-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new CentreonService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.CENTREON],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new CentreonService();
      const result = await service.scrape({
        siteType: [Site.CENTREON],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
