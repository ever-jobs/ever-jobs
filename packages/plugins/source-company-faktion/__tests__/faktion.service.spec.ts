...
import { FaktionModule, FaktionService } from '../src';

const COMPANY_NAME_EXPECT = 'Faktion';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'faktion-jobs.json'), 'utf8'),
);
...
describe('FaktionService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through FaktionModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [FaktionModule],
      }).compile();
      const service = moduleRef.get(FaktionService);
      expect(service).toBeInstanceOf(FaktionService);
      await moduleRef.close();
    });

    it('exports the Site.FAKTION = "faktion" enum value', () => {
      expect(Site.FAKTION).toBe('faktion');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new FaktionService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.FAKTION],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('faktionbv1.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.FAKTION);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('faktion-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new FaktionService(registry);
      const result = await service.scrape({
        siteType: [Site.FAKTION],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('faktionbv1');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('faktion-x1');
      expect(result.jobs[0].site).toBe(Site.FAKTION);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('faktion-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new FaktionService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.FAKTION],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new FaktionService();
      const result = await service.scrape({
        siteType: [Site.FAKTION],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
