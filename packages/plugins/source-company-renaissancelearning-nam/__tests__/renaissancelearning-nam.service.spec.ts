...
import { RenaissanceLearningModule, RenaissanceLearningService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'renaissancelearning-nam-jobs.json'), 'utf8'),
);
...
/**
 * Spec 919 / T04 — `RenaissanceLearningService` unit tests (≥ 9 cases).
 */
describe('RenaissanceLearningService — Spec 919 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through RenaissanceLearningModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [RenaissanceLearningModule],
      }).compile();
      const service = moduleRef.get(RenaissanceLearningService);
      expect(service).toBeInstanceOf(RenaissanceLearningService);
      await moduleRef.close();
    });

    it('exports the Site.RENAISSANCE_LEARNING = "renaissancelearning-nam" enum value', () => {
      expect(Site.RENAISSANCE_LEARNING).toBe('renaissancelearning-nam');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new RenaissanceLearningService();
      const result = await service.scrape({
        siteType: [Site.RENAISSANCE_LEARNING],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'renaissancelearning-nam-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.RENAISSANCE_LEARNING);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Renaissance Learning');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/renaissancelearning-nam/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/renaissancelearning-nam/jobs/');
      ...
      expect(job0?.description).toContain('Renaissance Learning');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/renaissancelearning-nam/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Renaissance Learning');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'renaissancelearning-nam-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
