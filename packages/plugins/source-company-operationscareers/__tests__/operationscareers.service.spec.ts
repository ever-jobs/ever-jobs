...
import { VeoModule, VeoService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'operationscareers-jobs.json'), 'utf8'),
);
...
/**
 * Spec 902 / T04 — `VeoService` unit tests (≥ 9 cases).
 */
describe('VeoService — Spec 902 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through VeoModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [VeoModule],
      }).compile();
      const service = moduleRef.get(VeoService);
      expect(service).toBeInstanceOf(VeoService);
      await moduleRef.close();
    });

    it('exports the Site.VEO = "operationscareers" enum value', () => {
      expect(Site.VEO).toBe('operationscareers');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const job0 = dto.jobs.find((j) => j.id === 'operationscareers-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.VEO);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Veo');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/operationscareers/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/operationscareers/jobs/');
      ...
      expect(job0?.description).toContain('Veo');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/operationscareers/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Veo');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'operationscareers-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
