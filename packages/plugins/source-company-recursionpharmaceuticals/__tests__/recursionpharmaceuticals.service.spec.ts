...
import { RecursionModule, RecursionService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'recursionpharmaceuticals-jobs.json'), 'utf8'),
);
...
/**
 * Spec 584 / T04 — `RecursionService` unit tests (≥ 9 cases).
 */
describe('RecursionService — Spec 584 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through RecursionModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [RecursionModule],
      }).compile();
      const service = moduleRef.get(RecursionService);
      expect(service).toBeInstanceOf(RecursionService);
      await moduleRef.close();
    });

    it('exports the Site.RECURSION = "recursionpharmaceuticals" enum value', () => {
      expect(Site.RECURSION).toBe('recursionpharmaceuticals');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new RecursionService();
      const result = await service.scrape({
        siteType: [Site.RECURSION],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'recursionpharmaceuticals-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.RECURSION);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Recursion');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/recursionpharmaceuticals/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/recursionpharmaceuticals/jobs/');
      ...
      expect(job0?.description).toContain('Recursion');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/recursionpharmaceuticals/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Recursion');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'recursionpharmaceuticals-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
