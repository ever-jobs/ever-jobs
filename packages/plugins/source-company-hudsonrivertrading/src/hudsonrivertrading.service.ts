...
/**
 * Hudson River Trading — Quantitative trading (HQ: New York, NY, USA).
 *
 * Source (Spec 1737): Greenhouse board, scraped in this order:
 *   - `wehrtyou` — https://job-boards.greenhouse.io/wehrtyou
 *     verified live 2026-09-24: 85 open postings.
 *
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'wehrtyou', atsIdPrefix: 'gh-' },
];

@SourcePlugin({
  site: Site.HUDSON_RIVER_TRADING,
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['hudsonrivertrading.com'],
  description: 'Hudson River Trading careers via Greenhouse. Tags: segment=quant-trading; industry=quantitative-trading.',
})
@Injectable()
export class HudsonRiverTradingService implements IScraper {
  private readonly logger = new Logger(HudsonRiverTradingService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Greenhouse source plugin is not registered; cannot scrape Hudson River Trading');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`Hudson River Trading: delegating to Greenhouse (${board.companySlug})`);
      ...
      for (const job of result.jobs ?? []) {
        job.site = Site.HUDSON_RIVER_TRADING;
    ...
    this.logger.log(`Hudson River Trading: scraped ${jobs.length} jobs`);
