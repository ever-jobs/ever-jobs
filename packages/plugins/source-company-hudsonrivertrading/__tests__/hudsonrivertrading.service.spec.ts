...
import { HudsonRiverTradingModule, HudsonRiverTradingService } from '../src';
...
const FIXTURE: Fixture = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'fixtures', 'hudsonrivertrading-boards.json'), 'utf8'),
);
const COMPANY_NAME = 'Hudson River Trading';
const ID_PREFIX = 'hudsonrivertrading-';
...
describe('HudsonRiverTradingService — Greenhouse delegation (Spec 1737)', () => {
  beforeEach(() => {
    mockGet.mockReset();
    mockPost.mockReset();
  ...
  describe('registration', () => {
    it('resolves through HudsonRiverTradingModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({ imports: [HudsonRiverTradingModule] }).compile();
      expect(moduleRef.get(HudsonRiverTradingService)).toBeInstanceOf(HudsonRiverTradingService);
      await moduleRef.close();
    });

    it('exports Site.HUDSON_RIVER_TRADING = "hudsonrivertrading"', () => {
      expect(Site.HUDSON_RIVER_TRADING).toBe('hudsonrivertrading');
    });

    it('declares a tagged company plugin with its domains', () => {
      const meta = Reflect.getMetadata(SOURCE_PLUGIN_METADATA, HudsonRiverTradingService);
      expect(meta.site).toBe(Site.HUDSON_RIVER_TRADING);
      expect(meta.category).toBe('company');
      expect(meta.isAts).toBeFalsy();
      expect(meta.companyDomains).toEqual(['hudsonrivertrading.com']);
  ...
  describe('recorded board (real Greenhouse adapter, mocked HTTP)', () => {
    it('maps every recorded posting and re-stamps the company identity', async () => {
      const service = new HudsonRiverTradingService(registryWith());
      const result = await service.scrape({ siteType: [Site.HUDSON_RIVER_TRADING], resultsWanted: 100 } as ScraperInputDto);
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.HUDSON_RIVER_TRADING);
    ...
    it('only requests the recorded board URLs', async () => {
      const service = new HudsonRiverTradingService(registryWith());
      await service.scrape({ siteType: [Site.HUDSON_RIVER_TRADING] } as ScraperInputDto);
    ...
    it('honours resultsWanted=1', async () => {
      const service = new HudsonRiverTradingService(registryWith());
      const result = await service.scrape({ siteType: [Site.HUDSON_RIVER_TRADING], resultsWanted: 1 } as ScraperInputDto);
    ...
    it('resolves with an empty result when the board is gone (HTTP 404)', async () => {
      ...
      const service = new HudsonRiverTradingService(registryWith());
      const result = await service.scrape({ siteType: [Site.HUDSON_RIVER_TRADING] } as ScraperInputDto);
  ...
  describe('delegation contract', () => {
    it('forwards the board and the caller input untouched', async () => {
      ...
      const service = new HudsonRiverTradingService(
        registryWith(
          fakeBackend(
      ...
      const result = await service.scrape({
        siteType: [Site.HUDSON_RIVER_TRADING],
      ...
      expect(result.jobs[0].site).toBe(Site.HUDSON_RIVER_TRADING);
      expect(result.jobs[0].companyName).toBe(COMPANY_NAME);
    });

    it('passes an absent resultsWanted through as absent', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new HudsonRiverTradingService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      await service.scrape({ siteType: [Site.HUDSON_RIVER_TRADING] } as ScraperInputDto);
    ...
    it('makes no request when resultsWanted is 0', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new HudsonRiverTradingService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      const result = await service.scrape({ siteType: [Site.HUDSON_RIVER_TRADING], resultsWanted: 0 } as ScraperInputDto);
  ...
  describe('resilience', () => {
    it('reports not_registered when no registry is injected', async () => {
      const result = await new HudsonRiverTradingService().scrape({ siteType: [Site.HUDSON_RIVER_TRADING] } as ScraperInputDto);
      expect(result.jobs).toEqual([]);
    ...
    it('reports not_registered when Greenhouse is missing from the registry', async () => {
      const result = await new HudsonRiverTradingService(new PluginRegistry()).scrape({
        siteType: [Site.HUDSON_RIVER_TRADING],
      } as ScraperInputDto);
      expect(result.diagnostics?.reason).toBe('not_registered');
    });
