...
import { SodexoModule, SodexoService } from '../src';

const COMPANY_NAME_EXPECT = 'Sodexo';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'sodexo-jobs.json'), 'utf8'),
);
...
describe('SodexoService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SodexoModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SodexoModule],
      }).compile();
      const service = moduleRef.get(SodexoService);
      expect(service).toBeInstanceOf(SodexoService);
      await moduleRef.close();
    });

    it('exports the Site.SODEXO = "sodexo" enum value', () => {
      expect(Site.SODEXO).toBe('sodexo');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new SodexoService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.SODEXO],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('Sodexo');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SODEXO);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('sodexo-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new SodexoService(registry);
      const result = await service.scrape({
        siteType: [Site.SODEXO],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('Sodexo');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('sodexo-x1');
      expect(result.jobs[0].site).toBe(Site.SODEXO);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('sodexo-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new SodexoService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.SODEXO],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new SodexoService();
      const result = await service.scrape({
        siteType: [Site.SODEXO],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
