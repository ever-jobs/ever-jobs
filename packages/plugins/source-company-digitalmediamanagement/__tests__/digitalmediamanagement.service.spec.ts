...
import { DigitalMediaManagementModule, DigitalMediaManagementService } from '../src';

const COMPANY_NAME_EXPECT = 'Digital Media Management';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'digitalmediamanagement-jobs.json'), 'utf8'),
);
...
describe('DigitalMediaManagementService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through DigitalMediaManagementModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [DigitalMediaManagementModule],
      }).compile();
      const service = moduleRef.get(DigitalMediaManagementService);
      expect(service).toBeInstanceOf(DigitalMediaManagementService);
      await moduleRef.close();
    });

    it('exports the Site.DIGITAL_MEDIA_MANAGEMENT = "digitalmediamanagement" enum value', () => {
      expect(Site.DIGITAL_MEDIA_MANAGEMENT).toBe('digitalmediamanagement');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new DigitalMediaManagementService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.DIGITAL_MEDIA_MANAGEMENT],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'digitalmediamanagement-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.DIGITAL_MEDIA_MANAGEMENT);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('digitalmediamanagement-' + first.id);
      ...
      expect(calledUrls[0]).toContain('digitalmediamanagement');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.DIGITAL_MEDIA_MANAGEMENT);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('digitalmediamanagement-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new DigitalMediaManagementService(registry);
      const result = await service.scrape({
        siteType: [Site.DIGITAL_MEDIA_MANAGEMENT],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('digitalmediamanagement');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('digitalmediamanagement-x1');
      expect(result.jobs[0].site).toBe(Site.DIGITAL_MEDIA_MANAGEMENT);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('digitalmediamanagement-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new DigitalMediaManagementService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.DIGITAL_MEDIA_MANAGEMENT],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new DigitalMediaManagementService();
      const result = await service.scrape({
        siteType: [Site.DIGITAL_MEDIA_MANAGEMENT],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
