...
import { TheBotCompanyModule, TheBotCompanyService } from '../src';

const COMPANY_NAME_EXPECT = 'The Bot Company';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'thebotcompany-jobs.json'), 'utf8'),
);
...
describe('TheBotCompanyService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through TheBotCompanyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [TheBotCompanyModule],
      }).compile();
      const service = moduleRef.get(TheBotCompanyService);
      expect(service).toBeInstanceOf(TheBotCompanyService);
      await moduleRef.close();
    });

    it('exports the Site.THE_BOT_COMPANY = "thebotcompany" enum value', () => {
      expect(Site.THE_BOT_COMPANY).toBe('thebotcompany');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new TheBotCompanyService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.THE_BOT_COMPANY],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'thebotcompany-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.THE_BOT_COMPANY);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('thebotcompany-' + first.id);
      ...
      expect(calledUrls[0]).toContain('thebotcompany');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.THE_BOT_COMPANY);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('thebotcompany-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new TheBotCompanyService(registry);
      const result = await service.scrape({
        siteType: [Site.THE_BOT_COMPANY],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('thebotcompany');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('thebotcompany-x1');
      expect(result.jobs[0].site).toBe(Site.THE_BOT_COMPANY);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('thebotcompany-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new TheBotCompanyService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.THE_BOT_COMPANY],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new TheBotCompanyService();
      const result = await service.scrape({
        siteType: [Site.THE_BOT_COMPANY],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
