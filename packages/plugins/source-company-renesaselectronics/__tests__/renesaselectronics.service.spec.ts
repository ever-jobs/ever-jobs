...
import { RenesasElectronicsModule, RenesasElectronicsService } from '../src';

const COMPANY_NAME_EXPECT = 'Renesas Electronics';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'renesaselectronics-jobs.json'), 'utf8'),
);
...
describe('RenesasElectronicsService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through RenesasElectronicsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [RenesasElectronicsModule],
      }).compile();
      const service = moduleRef.get(RenesasElectronicsService);
      expect(service).toBeInstanceOf(RenesasElectronicsService);
      await moduleRef.close();
    });

    it('exports the Site.RENESAS_ELECTRONICS = "renesaselectronics" enum value', () => {
      expect(Site.RENESAS_ELECTRONICS).toBe('renesaselectronics');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new RenesasElectronicsService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.RENESAS_ELECTRONICS],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'renesaselectronics-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.RENESAS_ELECTRONICS);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('renesaselectronics-' + first.id);
      ...
      expect(calledUrls[0]).toContain('RenesasElectronics');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.RENESAS_ELECTRONICS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('renesaselectronics-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new RenesasElectronicsService(registry);
      const result = await service.scrape({
        siteType: [Site.RENESAS_ELECTRONICS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('RenesasElectronics');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('renesaselectronics-x1');
      expect(result.jobs[0].site).toBe(Site.RENESAS_ELECTRONICS);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('renesaselectronics-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new RenesasElectronicsService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.RENESAS_ELECTRONICS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new RenesasElectronicsService();
      const result = await service.scrape({
        siteType: [Site.RENESAS_ELECTRONICS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
