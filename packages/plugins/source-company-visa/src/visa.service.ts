...
/**
 * Visa — Payments (HQ: San Francisco, CA, USA).
 *
 * Source (Spec 1736): Workday boards, scraped in this order:
 *   - `visa:5:Visa_Early_Careers` (early careers) — https://visa.wd5.myworkdayjobs.com/Visa_Early_Careers
 *     verified live 2026-09-24: 44 open postings.
 *   - `visa:5:Visa` — https://visa.wd5.myworkdayjobs.com/Visa
 *     verified live 2026-09-24: 794 open postings.
 *
...
@Injectable()
export class VisaService implements IScraper {
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Workday source plugin is not registered; cannot scrape Visa');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`Visa: delegating to Workday (${board.companySlug})`);
    ...
    this.logger.log(`Visa: scraped ${jobs.length} jobs`);
