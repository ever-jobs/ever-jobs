...
import { OpenlyModule, OpenlyService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'openly-jobs.json'), 'utf8'),
);
...
/**
 * Spec 629 / T04 — `OpenlyService` unit tests (≥ 9 cases).
 */
describe('OpenlyService — Spec 629 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through OpenlyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [OpenlyModule],
      }).compile();
      const service = moduleRef.get(OpenlyService);
      expect(service).toBeInstanceOf(OpenlyService);
      await moduleRef.close();
    });

    it('exports the Site.OPENLY = "openly" enum value', () => {
      expect(Site.OPENLY).toBe('openly');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new OpenlyService();
      const result = await service.scrape({
        siteType: [Site.OPENLY],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'openly-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.OPENLY);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Openly');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/openly/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/openly/jobs/');
      ...
      expect(job0?.description).toContain('Openly');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/openly/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Openly');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'openly-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
