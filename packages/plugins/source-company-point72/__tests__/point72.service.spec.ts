...
import { Point72Module, Point72Service } from '../src';
...
const FIXTURE: Fixture = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'fixtures', 'point72-boards.json'), 'utf8'),
);
const COMPANY_NAME = 'Point72';
const ID_PREFIX = 'point72-';
...
describe('Point72Service — Greenhouse delegation (Spec 1737)', () => {
  beforeEach(() => {
    mockGet.mockReset();
    mockPost.mockReset();
  ...
  describe('registration', () => {
    it('resolves through Point72Module via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({ imports: [Point72Module] }).compile();
      expect(moduleRef.get(Point72Service)).toBeInstanceOf(Point72Service);
      await moduleRef.close();
    });

    it('exports Site.POINT72 = "point72"', () => {
      expect(Site.POINT72).toBe('point72');
    });

    it('declares a tagged company plugin with its domains', () => {
      const meta = Reflect.getMetadata(SOURCE_PLUGIN_METADATA, Point72Service);
      expect(meta.site).toBe(Site.POINT72);
      expect(meta.category).toBe('company');
      expect(meta.isAts).toBeFalsy();
      expect(meta.companyDomains).toEqual(['point72.com']);
  ...
  describe('recorded board (real Greenhouse adapter, mocked HTTP)', () => {
    it('maps every recorded posting and re-stamps the company identity', async () => {
      const service = new Point72Service(registryWith());
      const result = await service.scrape({ siteType: [Site.POINT72], resultsWanted: 100 } as ScraperInputDto);
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.POINT72);
    ...
    it('only requests the recorded board URLs', async () => {
      const service = new Point72Service(registryWith());
      await service.scrape({ siteType: [Site.POINT72] } as ScraperInputDto);
    ...
    it('honours resultsWanted=1', async () => {
      const service = new Point72Service(registryWith());
      const result = await service.scrape({ siteType: [Site.POINT72], resultsWanted: 1 } as ScraperInputDto);
    ...
    it('resolves with an empty result when the board is gone (HTTP 404)', async () => {
      ...
      const service = new Point72Service(registryWith());
      const result = await service.scrape({ siteType: [Site.POINT72] } as ScraperInputDto);
  ...
  describe('delegation contract', () => {
    it('forwards the board and the caller input untouched', async () => {
      ...
      const service = new Point72Service(
        registryWith(
          fakeBackend(
      ...
      expect(result.jobs[0].site).toBe(Site.POINT72);
      expect(result.jobs[0].companyName).toBe(COMPANY_NAME);
    });

    it('passes an absent resultsWanted through as absent', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new Point72Service(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      await service.scrape({ siteType: [Site.POINT72] } as ScraperInputDto);
    ...
    it('makes no request when resultsWanted is 0', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new Point72Service(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      const result = await service.scrape({ siteType: [Site.POINT72], resultsWanted: 0 } as ScraperInputDto);
  ...
  describe('resilience', () => {
    it('reports not_registered when no registry is injected', async () => {
      const result = await new Point72Service().scrape({ siteType: [Site.POINT72] } as ScraperInputDto);
      expect(result.jobs).toEqual([]);
    ...
    it('reports not_registered when Greenhouse is missing from the registry', async () => {
      const result = await new Point72Service(new PluginRegistry()).scrape({
        siteType: [Site.POINT72],
      } as ScraperInputDto);
      expect(result.diagnostics?.reason).toBe('not_registered');
    });
