...
/**
 * Point72 — Hedge fund (HQ: Stamford, CT, USA).
 *
 * Source (Spec 1737): Greenhouse board, scraped in this order:
 *   - `point72` — https://job-boards.greenhouse.io/point72
 *     verified live 2026-09-24: 220 open postings.
 *
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'point72', atsIdPrefix: 'gh-' },
];
...
@SourcePlugin({
  ...
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['point72.com'],
  description: 'Point72 careers via Greenhouse. Tags: segment=quant-trading; industry=hedge-fund.',
})
@Injectable()
export class Point72Service implements IScraper {
  private readonly logger = new Logger(Point72Service.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Greenhouse source plugin is not registered; cannot scrape Point72');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`Point72: delegating to Greenhouse (${board.companySlug})`);
    ...
    this.logger.log(`Point72: scraped ${jobs.length} jobs`);
