...
import { GenomicsPlcModule, GenomicsPlcService } from '../src';

const COMPANY_NAME_EXPECT = 'Genomics plc';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'genomicsplc-jobs.json'), 'utf8'),
);
...
describe('GenomicsPlcService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through GenomicsPlcModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [GenomicsPlcModule],
      }).compile();
      const service = moduleRef.get(GenomicsPlcService);
      expect(service).toBeInstanceOf(GenomicsPlcService);
      await moduleRef.close();
    });

    it('exports the Site.GENOMICS_PLC = "genomicsplc" enum value', () => {
      expect(Site.GENOMICS_PLC).toBe('genomicsplc');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new GenomicsPlcService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.GENOMICS_PLC],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'genomicsplc-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.GENOMICS_PLC);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('genomicsplc-' + first.id);
      ...
      expect(calledUrls[0]).toContain('genomics');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.GENOMICS_PLC);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('genomicsplc-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new GenomicsPlcService(registry);
      const result = await service.scrape({
        siteType: [Site.GENOMICS_PLC],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('genomics');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('genomicsplc-x1');
      expect(result.jobs[0].site).toBe(Site.GENOMICS_PLC);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('genomicsplc-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new GenomicsPlcService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.GENOMICS_PLC],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new GenomicsPlcService();
      const result = await service.scrape({
        siteType: [Site.GENOMICS_PLC],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
