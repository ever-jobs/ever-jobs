...
import { NorthStarStaffingSolutionsModule, NorthStarStaffingSolutionsService } from '../src';

const COMPANY_NAME_EXPECT = 'North Star Staffing Solutions';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'northstarstaffingsolutions-jobs.json'), 'utf8'),
);
...
describe('NorthStarStaffingSolutionsService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through NorthStarStaffingSolutionsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [NorthStarStaffingSolutionsModule],
      }).compile();
      const service = moduleRef.get(NorthStarStaffingSolutionsService);
      expect(service).toBeInstanceOf(NorthStarStaffingSolutionsService);
      await moduleRef.close();
    });

    it('exports the Site.NORTH_STAR_STAFFING_SOLUTIONS = "northstarstaffingsolutions" enum value', () => {
      expect(Site.NORTH_STAR_STAFFING_SOLUTIONS).toBe('northstarstaffingsolutions');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new NorthStarStaffingSolutionsService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.NORTH_STAR_STAFFING_SOLUTIONS],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'northstarstaffingsolutions-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.NORTH_STAR_STAFFING_SOLUTIONS);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('northstarstaffingsolutions-' + first.id);
      ...
      expect(calledUrls[0]).toContain('NorthStarStaffingSolutions1');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.NORTH_STAR_STAFFING_SOLUTIONS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('northstarstaffingsolutions-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new NorthStarStaffingSolutionsService(registry);
      const result = await service.scrape({
        siteType: [Site.NORTH_STAR_STAFFING_SOLUTIONS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('NorthStarStaffingSolutions1');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('northstarstaffingsolutions-x1');
      expect(result.jobs[0].site).toBe(Site.NORTH_STAR_STAFFING_SOLUTIONS);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('northstarstaffingsolutions-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new NorthStarStaffingSolutionsService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.NORTH_STAR_STAFFING_SOLUTIONS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new NorthStarStaffingSolutionsService();
      const result = await service.scrape({
        siteType: [Site.NORTH_STAR_STAFFING_SOLUTIONS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
