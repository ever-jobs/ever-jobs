...
import { North40OutfittersModule, North40OutfittersService } from '../src';

const COMPANY_NAME_EXPECT = 'North 40 Outfitters';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'north40outfitters-jobs.json'), 'utf8'),
);
...
describe('North40OutfittersService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through North40OutfittersModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [North40OutfittersModule],
      }).compile();
      const service = moduleRef.get(North40OutfittersService);
      expect(service).toBeInstanceOf(North40OutfittersService);
      await moduleRef.close();
    });

    it('exports the Site.NORTH_40_OUTFITTERS = "north40outfitters" enum value', () => {
      expect(Site.NORTH_40_OUTFITTERS).toBe('north40outfitters');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new North40OutfittersService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.NORTH_40_OUTFITTERS],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'north40outfitters-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.NORTH_40_OUTFITTERS);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('north40outfitters-' + first.id);
      ...
      expect(calledUrls[0]).toContain('North40Outfitters');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.NORTH_40_OUTFITTERS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('north40outfitters-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new North40OutfittersService(registry);
      const result = await service.scrape({
        siteType: [Site.NORTH_40_OUTFITTERS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('North40Outfitters');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('north40outfitters-x1');
      expect(result.jobs[0].site).toBe(Site.NORTH_40_OUTFITTERS);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('north40outfitters-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new North40OutfittersService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.NORTH_40_OUTFITTERS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new North40OutfittersService();
      const result = await service.scrape({
        siteType: [Site.NORTH_40_OUTFITTERS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
