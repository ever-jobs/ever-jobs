...
import { NebiusModule, NebiusService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'nebius-jobs.json'), 'utf8'),
);
...
/**
 * Spec 716 / T04 — `NebiusService` unit tests (≥ 9 cases).
 */
describe('NebiusService — Spec 716 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through NebiusModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [NebiusModule],
      }).compile();
      const service = moduleRef.get(NebiusService);
      expect(service).toBeInstanceOf(NebiusService);
      await moduleRef.close();
    });

    it('exports the Site.NEBIUS = "nebius" enum value', () => {
      expect(Site.NEBIUS).toBe('nebius');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new NebiusService();
      const result = await service.scrape({
        siteType: [Site.NEBIUS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'nebius-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.NEBIUS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Nebius');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/nebius/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/nebius/jobs/');
      ...
      expect(job0?.description).toContain('Nebius');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/nebius/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Nebius');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'nebius-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
