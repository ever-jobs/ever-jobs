...
import { BioAgilytixModule, BioAgilytixService } from '../src';

const COMPANY_NAME_EXPECT = 'BioAgilytix';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'bioagilytix-jobs.json'), 'utf8'),
);
...
describe('BioAgilytixService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through BioAgilytixModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [BioAgilytixModule],
      }).compile();
      const service = moduleRef.get(BioAgilytixService);
      expect(service).toBeInstanceOf(BioAgilytixService);
      await moduleRef.close();
    });

    it('exports the Site.BIOAGILYTIX = "bioagilytix" enum value', () => {
      expect(Site.BIOAGILYTIX).toBe('bioagilytix');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new BioAgilytixService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.BIOAGILYTIX],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'bioagilytix-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.BIOAGILYTIX);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('bioagilytix-' + first.id);
      ...
      expect(calledUrls[0]).toContain('bioagilytix');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.BIOAGILYTIX);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('bioagilytix-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new BioAgilytixService(registry);
      const result = await service.scrape({
        siteType: [Site.BIOAGILYTIX],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('bioagilytix');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('bioagilytix-x1');
      expect(result.jobs[0].site).toBe(Site.BIOAGILYTIX);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('bioagilytix-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new BioAgilytixService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.BIOAGILYTIX],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new BioAgilytixService();
      const result = await service.scrape({
        siteType: [Site.BIOAGILYTIX],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
