...
import { KomodoHealthModule, KomodoHealthService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'komodohealth-jobs.json'), 'utf8'),
);
...
/**
 * Spec 765 / T04 — `KomodoHealthService` unit tests (≥ 9 cases).
 */
describe('KomodoHealthService — Spec 765 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through KomodoHealthModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [KomodoHealthModule],
      }).compile();
      const service = moduleRef.get(KomodoHealthService);
      expect(service).toBeInstanceOf(KomodoHealthService);
      await moduleRef.close();
    });

    it('exports the Site.KOMODO_HEALTH = "komodohealth" enum value', () => {
      expect(Site.KOMODO_HEALTH).toBe('komodohealth');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new KomodoHealthService();
      const result = await service.scrape({
        siteType: [Site.KOMODO_HEALTH],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'komodohealth-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.KOMODO_HEALTH);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Komodo Health');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/komodohealth/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/komodohealth/jobs/');
      ...
      expect(job0?.description).toContain('Komodo Health');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/komodohealth/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Komodo Health');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'komodohealth-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
