...
import { OmadahealthModule, OmadahealthService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'omadahealth-jobs.json'), 'utf8'),
);
...
/**
 * Spec 436 / T04 — `OmadahealthService` unit tests (≥ 9 cases).
 */
describe('OmadahealthService — Spec 436 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through OmadahealthModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [OmadahealthModule],
      }).compile();
      const service = moduleRef.get(OmadahealthService);
      expect(service).toBeInstanceOf(OmadahealthService);
      await moduleRef.close();
    });

    it('exports the Site.OMADAHEALTH = "omadahealth" enum value', () => {
      expect(Site.OMADAHEALTH).toBe('omadahealth');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new OmadahealthService();
      const result = await service.scrape({
        siteType: [Site.OMADAHEALTH],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'omadahealth-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.OMADAHEALTH);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Omada Health');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/omadahealth/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/omadahealth/jobs/');
      ...
      expect(job0?.description).toContain('Omada Health');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/omadahealth/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Omada Health');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'omadahealth-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
