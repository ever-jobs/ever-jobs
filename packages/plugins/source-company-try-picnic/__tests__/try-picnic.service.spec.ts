...
import { PicnicDeliveryModule, PicnicDeliveryService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'try-picnic-jobs.json'), 'utf8'),
);
...
/**
 * Spec 961 / T04 — `PicnicDeliveryService` unit tests (≥ 9 cases).
 */
describe('PicnicDeliveryService — Spec 961 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through PicnicDeliveryModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [PicnicDeliveryModule],
      }).compile();
      const service = moduleRef.get(PicnicDeliveryService);
      expect(service).toBeInstanceOf(PicnicDeliveryService);
      await moduleRef.close();
    });

    it('exports the Site.PICNIC_DELIVERY = "try-picnic" enum value', () => {
      expect(Site.PICNIC_DELIVERY).toBe('try-picnic');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new PicnicDeliveryService();
      const result = await service.scrape({
        siteType: [Site.PICNIC_DELIVERY],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'try-picnic-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.PICNIC_DELIVERY);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Picnic Delivery');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/try-picnic/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/try-picnic/jobs/');
      ...
      expect(job0?.description).toContain('Picnic Delivery');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/try-picnic/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Picnic Delivery');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'try-picnic-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
