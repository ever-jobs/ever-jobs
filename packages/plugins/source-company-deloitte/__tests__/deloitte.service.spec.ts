...
import { DeloitteModule, DeloitteService } from '../src';

const COMPANY_NAME_EXPECT = 'Deloitte';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'deloitte-jobs.json'), 'utf8'),
);
...
describe('DeloitteService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through DeloitteModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [DeloitteModule],
      }).compile();
      const service = moduleRef.get(DeloitteService);
      expect(service).toBeInstanceOf(DeloitteService);
      await moduleRef.close();
    });

    it('exports the Site.DELOITTE = "deloitte" enum value', () => {
      expect(Site.DELOITTE).toBe('deloitte');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new DeloitteService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.DELOITTE],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('Deloitte6');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.DELOITTE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('deloitte-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new DeloitteService(registry);
      const result = await service.scrape({
        siteType: [Site.DELOITTE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('Deloitte6');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('deloitte-x1');
      expect(result.jobs[0].site).toBe(Site.DELOITTE);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('deloitte-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new DeloitteService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.DELOITTE],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new DeloitteService();
      const result = await service.scrape({
        siteType: [Site.DELOITTE],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
