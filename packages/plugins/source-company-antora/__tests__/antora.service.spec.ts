...
import { AntoraModule, AntoraService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'antora-jobs.json'), 'utf8'),
);
...
/**
 * Spec 275 / T04 — `AntoraService` unit tests (≥ 9 cases).
 */
describe('AntoraService — Spec 275 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AntoraModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AntoraModule],
      }).compile();
      const service = moduleRef.get(AntoraService);
      expect(service).toBeInstanceOf(AntoraService);
      await moduleRef.close();
    });

    it('exports the Site.ANTORA = "antora" enum value', () => {
      expect(Site.ANTORA).toBe('antora');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AntoraService();
      const result = await service.scrape({
        siteType: [Site.ANTORA],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'antora-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ANTORA);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Antora Energy');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/antora/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/antora/jobs/');
      ...
      expect(job0?.description).toContain('Antora Energy');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/antora/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Antora Energy');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'antora-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
