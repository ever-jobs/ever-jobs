...
import { StudyPointModule, StudyPointService } from '../src';

const COMPANY_NAME_EXPECT = 'StudyPoint';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'studypoint-jobs.json'), 'utf8'),
);
...
describe('StudyPointService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through StudyPointModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [StudyPointModule],
      }).compile();
      const service = moduleRef.get(StudyPointService);
      expect(service).toBeInstanceOf(StudyPointService);
      await moduleRef.close();
    });

    it('exports the Site.STUDYPOINT = "studypoint" enum value', () => {
      expect(Site.STUDYPOINT).toBe('studypoint');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new StudyPointService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.STUDYPOINT],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('studypoint');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.STUDYPOINT);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('studypoint-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new StudyPointService(registry);
      const result = await service.scrape({
        siteType: [Site.STUDYPOINT],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('studypoint');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('studypoint-x1');
      expect(result.jobs[0].site).toBe(Site.STUDYPOINT);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('studypoint-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new StudyPointService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.STUDYPOINT],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new StudyPointService();
      const result = await service.scrape({
        siteType: [Site.STUDYPOINT],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
