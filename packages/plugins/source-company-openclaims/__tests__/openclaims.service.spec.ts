...
import { OpenclaimsModule, OpenclaimsService } from '../src';

const COMPANY_NAME_EXPECT = 'Openclaims';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'openclaims-jobs.json'), 'utf8'),
);
...
describe('OpenclaimsService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through OpenclaimsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [OpenclaimsModule],
      }).compile();
      const service = moduleRef.get(OpenclaimsService);
      expect(service).toBeInstanceOf(OpenclaimsService);
      await moduleRef.close();
    });

    it('exports the Site.OPENCLAIMS = "openclaims" enum value', () => {
      expect(Site.OPENCLAIMS).toBe('openclaims');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new OpenclaimsService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.OPENCLAIMS],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('openclaims.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.OPENCLAIMS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('openclaims-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new OpenclaimsService(registry);
      const result = await service.scrape({
        siteType: [Site.OPENCLAIMS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('openclaims');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('openclaims-x1');
      expect(result.jobs[0].site).toBe(Site.OPENCLAIMS);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('openclaims-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new OpenclaimsService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.OPENCLAIMS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new OpenclaimsService();
      const result = await service.scrape({
        siteType: [Site.OPENCLAIMS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
