...
import { SocureModule, SocureService } from '../src';

const COMPANY_NAME_EXPECT = 'Socure';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'socure-jobs.json'), 'utf8'),
);
...
describe('SocureService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SocureModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SocureModule],
      }).compile();
      const service = moduleRef.get(SocureService);
      expect(service).toBeInstanceOf(SocureService);
      await moduleRef.close();
    });

    it('exports the Site.SOCURE = "socure" enum value', () => {
      expect(Site.SOCURE).toBe('socure');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new SocureService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.SOCURE],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('socure');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SOCURE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('socure-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new SocureService(registry);
      const result = await service.scrape({
        siteType: [Site.SOCURE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('socure');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('socure-x1');
      expect(result.jobs[0].site).toBe(Site.SOCURE);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('socure-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new SocureService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.SOCURE],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new SocureService();
      const result = await service.scrape({
        siteType: [Site.SOCURE],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
