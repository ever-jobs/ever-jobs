...
import { CROSSMARKModule, CROSSMARKService } from '../src';

const COMPANY_NAME_EXPECT = 'CROSSMARK';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'crossmark-jobs.json'), 'utf8'),
);
...
describe('CROSSMARKService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CROSSMARKModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CROSSMARKModule],
      }).compile();
      const service = moduleRef.get(CROSSMARKService);
      expect(service).toBeInstanceOf(CROSSMARKService);
      await moduleRef.close();
    });

    it('exports the Site.CROSSMARK = "crossmark" enum value', () => {
      expect(Site.CROSSMARK).toBe('crossmark');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new CROSSMARKService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.CROSSMARK],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('CROSSMARK1');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.CROSSMARK);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('crossmark-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new CROSSMARKService(registry);
      const result = await service.scrape({
        siteType: [Site.CROSSMARK],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('CROSSMARK1');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('crossmark-x1');
      expect(result.jobs[0].site).toBe(Site.CROSSMARK);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('crossmark-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new CROSSMARKService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.CROSSMARK],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new CROSSMARKService();
      const result = await service.scrape({
        siteType: [Site.CROSSMARK],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
