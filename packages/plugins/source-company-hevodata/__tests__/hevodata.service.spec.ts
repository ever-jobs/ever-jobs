...
import { HevoDataModule, HevoDataService } from '../src';

const COMPANY_NAME_EXPECT = 'Hevo Data';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'hevodata-jobs.json'), 'utf8'),
);
...
describe('HevoDataService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through HevoDataModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [HevoDataModule],
      }).compile();
      const service = moduleRef.get(HevoDataService);
      expect(service).toBeInstanceOf(HevoDataService);
      await moduleRef.close();
    });

    it('exports the Site.HEVO_DATA = "hevodata" enum value', () => {
      expect(Site.HEVO_DATA).toBe('hevodata');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new HevoDataService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.HEVO_DATA],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('hevodata');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.HEVO_DATA);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('hevodata-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new HevoDataService(registry);
      const result = await service.scrape({
        siteType: [Site.HEVO_DATA],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('hevodata');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('hevodata-x1');
      expect(result.jobs[0].site).toBe(Site.HEVO_DATA);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('hevodata-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new HevoDataService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.HEVO_DATA],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new HevoDataService();
      const result = await service.scrape({
        siteType: [Site.HEVO_DATA],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
