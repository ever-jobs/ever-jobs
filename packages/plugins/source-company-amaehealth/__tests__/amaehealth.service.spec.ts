...
import { AmaehealthModule, AmaehealthService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'amaehealth-jobs.json'), 'utf8'),
);
...
/**
 * Spec 255 / T04 — `AmaehealthService` unit tests (≥ 9 cases).
 */
describe('AmaehealthService — Spec 255 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AmaehealthModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AmaehealthModule],
      }).compile();
      const service = moduleRef.get(AmaehealthService);
      expect(service).toBeInstanceOf(AmaehealthService);
      await moduleRef.close();
    });

    it('exports the Site.AMAEHEALTH = "amaehealth" enum value', () => {
      expect(Site.AMAEHEALTH).toBe('amaehealth');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AmaehealthService();
      const result = await service.scrape({
        siteType: [Site.AMAEHEALTH],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'amaehealth-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.AMAEHEALTH);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Amae Health');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/amaehealth/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/amaehealth/jobs/');
      ...
      expect(job0?.description).toContain('Amae Health');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/amaehealth/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Amae Health');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'amaehealth-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
