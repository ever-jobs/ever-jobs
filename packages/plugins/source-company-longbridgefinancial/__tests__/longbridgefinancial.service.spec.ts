...
import { LongbridgeFinancialModule, LongbridgeFinancialService } from '../src';

const COMPANY_NAME_EXPECT = 'Longbridge Financial';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'longbridgefinancial-jobs.json'), 'utf8'),
);
...
describe('LongbridgeFinancialService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through LongbridgeFinancialModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [LongbridgeFinancialModule],
      }).compile();
      const service = moduleRef.get(LongbridgeFinancialService);
      expect(service).toBeInstanceOf(LongbridgeFinancialService);
      await moduleRef.close();
    });

    it('exports the Site.LONGBRIDGE_FINANCIAL = "longbridgefinancial" enum value', () => {
      expect(Site.LONGBRIDGE_FINANCIAL).toBe('longbridgefinancial');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new LongbridgeFinancialService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.LONGBRIDGE_FINANCIAL],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'longbridgefinancial-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.LONGBRIDGE_FINANCIAL);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('longbridgefinancial-' + first.id);
      ...
      expect(calledUrls[0]).toContain('LongbridgeFinancial');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.LONGBRIDGE_FINANCIAL);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('longbridgefinancial-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new LongbridgeFinancialService(registry);
      const result = await service.scrape({
        siteType: [Site.LONGBRIDGE_FINANCIAL],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('LongbridgeFinancial');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('longbridgefinancial-x1');
      expect(result.jobs[0].site).toBe(Site.LONGBRIDGE_FINANCIAL);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('longbridgefinancial-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new LongbridgeFinancialService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.LONGBRIDGE_FINANCIAL],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new LongbridgeFinancialService();
      const result = await service.scrape({
        siteType: [Site.LONGBRIDGE_FINANCIAL],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
