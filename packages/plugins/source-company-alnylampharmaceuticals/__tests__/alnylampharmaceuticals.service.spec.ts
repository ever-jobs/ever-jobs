...
import { AlnylamPharmaceuticalsModule, AlnylamPharmaceuticalsService } from '../src';

const COMPANY_NAME_EXPECT = 'Alnylam Pharmaceuticals';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'alnylampharmaceuticals-jobs.json'), 'utf8'),
);
...
describe('AlnylamPharmaceuticalsService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AlnylamPharmaceuticalsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AlnylamPharmaceuticalsModule],
      }).compile();
      const service = moduleRef.get(AlnylamPharmaceuticalsService);
      expect(service).toBeInstanceOf(AlnylamPharmaceuticalsService);
      await moduleRef.close();
    });

    it('exports the Site.ALNYLAM_PHARMACEUTICALS = "alnylampharmaceuticals" enum value', () => {
      expect(Site.ALNYLAM_PHARMACEUTICALS).toBe('alnylampharmaceuticals');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new AlnylamPharmaceuticalsService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.ALNYLAM_PHARMACEUTICALS],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'alnylampharmaceuticals-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.ALNYLAM_PHARMACEUTICALS);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('alnylampharmaceuticals-' + first.id);
      ...
      expect(calledUrls[0]).toContain('Alnylam');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ALNYLAM_PHARMACEUTICALS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('alnylampharmaceuticals-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new AlnylamPharmaceuticalsService(registry);
      const result = await service.scrape({
        siteType: [Site.ALNYLAM_PHARMACEUTICALS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('Alnylam');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('alnylampharmaceuticals-x1');
      expect(result.jobs[0].site).toBe(Site.ALNYLAM_PHARMACEUTICALS);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('alnylampharmaceuticals-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new AlnylamPharmaceuticalsService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.ALNYLAM_PHARMACEUTICALS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new AlnylamPharmaceuticalsService();
      const result = await service.scrape({
        siteType: [Site.ALNYLAM_PHARMACEUTICALS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
