...
import { FieldBuddyModule, FieldBuddyService } from '../src';

const COMPANY_NAME_EXPECT = 'FieldBuddy';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'fieldbuddy-jobs.json'), 'utf8'),
);
...
describe('FieldBuddyService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through FieldBuddyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [FieldBuddyModule],
      }).compile();
      const service = moduleRef.get(FieldBuddyService);
      expect(service).toBeInstanceOf(FieldBuddyService);
      await moduleRef.close();
    });

    it('exports the Site.FIELDBUDDY = "fieldbuddy" enum value', () => {
      expect(Site.FIELDBUDDY).toBe('fieldbuddy');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new FieldBuddyService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.FIELDBUDDY],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('fieldbuddy.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.FIELDBUDDY);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('fieldbuddy-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new FieldBuddyService(registry);
      const result = await service.scrape({
        siteType: [Site.FIELDBUDDY],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('fieldbuddy');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('fieldbuddy-x1');
      expect(result.jobs[0].site).toBe(Site.FIELDBUDDY);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('fieldbuddy-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new FieldBuddyService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.FIELDBUDDY],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new FieldBuddyService();
      const result = await service.scrape({
        siteType: [Site.FIELDBUDDY],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
