...
import { SalesforceModule, SalesforceService } from '../src';
...
const FIXTURE: Fixture = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'fixtures', 'salesforce-boards.json'), 'utf8'),
);
const COMPANY_NAME = 'Salesforce';
const ID_PREFIX = 'salesforce-';
...
describe('SalesforceService — Workday delegation (Spec 1736)', () => {
  beforeEach(() => {
    mockGet.mockReset();
    mockPost.mockReset();
  ...
  describe('registration', () => {
    it('resolves through SalesforceModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({ imports: [SalesforceModule] }).compile();
      expect(moduleRef.get(SalesforceService)).toBeInstanceOf(SalesforceService);
      await moduleRef.close();
    });

    it('exports Site.SALESFORCE = "salesforce"', () => {
      expect(Site.SALESFORCE).toBe('salesforce');
    });

    it('declares a tagged company plugin with its domains', () => {
      const meta = Reflect.getMetadata(SOURCE_PLUGIN_METADATA, SalesforceService);
      expect(meta.site).toBe(Site.SALESFORCE);
      expect(meta.category).toBe('company');
      expect(meta.isAts).toBeFalsy();
      expect(meta.companyDomains).toEqual(['salesforce.com']);
  ...
  describe('recorded board (real Workday adapter, mocked HTTP)', () => {
    it('maps every recorded posting and re-stamps the company identity', async () => {
      const service = new SalesforceService(registryWith());
      const result = await service.scrape({ siteType: [Site.SALESFORCE], resultsWanted: 100 } as ScraperInputDto);
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SALESFORCE);
    ...
    it('only requests the recorded board URLs', async () => {
      const service = new SalesforceService(registryWith());
      await service.scrape({ siteType: [Site.SALESFORCE] } as ScraperInputDto);
    ...
    it('honours resultsWanted=1', async () => {
      const service = new SalesforceService(registryWith());
      const result = await service.scrape({ siteType: [Site.SALESFORCE], resultsWanted: 1 } as ScraperInputDto);
    ...
    it('resolves with an empty result when the board is gone (HTTP 404)', async () => {
      ...
      const service = new SalesforceService(registryWith());
      const result = await service.scrape({ siteType: [Site.SALESFORCE] } as ScraperInputDto);
  ...
  describe('delegation contract', () => {
    it('forwards the board and the caller input untouched', async () => {
      ...
      expect(result.jobs[0].site).toBe(Site.SALESFORCE);
    ...
    it('never forwards the caller\'s credentials to the board (Spec 1735 §4.5)', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new SalesforceService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
    ...
    it('makes no request when resultsWanted is 0', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new SalesforceService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      const result = await service.scrape({ siteType: [Site.SALESFORCE], resultsWanted: 0 } as ScraperInputDto);
  ...
  describe('resilience', () => {
    it('reports not_registered when no registry is injected', async () => {
      const result = await new SalesforceService().scrape({ siteType: [Site.SALESFORCE] } as ScraperInputDto);
      expect(result.jobs).toEqual([]);
    ...
    it('reports not_registered when Workday is missing from the registry', async () => {
      const result = await new SalesforceService(new PluginRegistry()).scrape({
        siteType: [Site.SALESFORCE],
      } as ScraperInputDto);
      expect(result.diagnostics?.reason).toBe('not_registered');
    });
