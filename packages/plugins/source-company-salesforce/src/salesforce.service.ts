...
/**
 * Salesforce — Enterprise software (HQ: San Francisco, CA, USA).
 *
 * Source (Spec 1736): Workday board, scraped in this order:
 *   - `salesforce:12:External_Career_Site` — https://salesforce.wd12.myworkdayjobs.com/External_Career_Site
 *     verified live 2026-09-24: 1,529 open postings.
 *
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'salesforce:12:External_Career_Site', atsIdPrefix: 'wd-salesforce-' },
];

@SourcePlugin({
  site: Site.SALESFORCE,
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['salesforce.com'],
  description: 'Salesforce careers via Workday. Tags: segment=workday-enterprise; industry=enterprise-software.',
})
@Injectable()
export class SalesforceService implements IScraper {
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Workday source plugin is not registered; cannot scrape Salesforce');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`Salesforce: delegating to Workday (${board.companySlug})`);
    ...
    this.logger.log(`Salesforce: scraped ${jobs.length} jobs`);
