...
import { FieldNationModule, FieldNationService } from '../src';

const COMPANY_NAME_EXPECT = 'Field Nation';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'fieldnation-jobs.json'), 'utf8'),
);
...
describe('FieldNationService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through FieldNationModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [FieldNationModule],
      }).compile();
      const service = moduleRef.get(FieldNationService);
      expect(service).toBeInstanceOf(FieldNationService);
      await moduleRef.close();
    });

    it('exports the Site.FIELD_NATION = "fieldnation" enum value', () => {
      expect(Site.FIELD_NATION).toBe('fieldnation');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new FieldNationService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.FIELD_NATION],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'fieldnation-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.FIELD_NATION);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('fieldnation-' + first.id);
      ...
      expect(calledUrls[0]).toContain('fieldnation');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.FIELD_NATION);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('fieldnation-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new FieldNationService(registry);
      const result = await service.scrape({
        siteType: [Site.FIELD_NATION],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('fieldnation');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('fieldnation-x1');
      expect(result.jobs[0].site).toBe(Site.FIELD_NATION);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('fieldnation-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new FieldNationService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.FIELD_NATION],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new FieldNationService();
      const result = await service.scrape({
        siteType: [Site.FIELD_NATION],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
