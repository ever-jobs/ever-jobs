...
import { GhostStoryGamesModule, GhostStoryGamesService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'gsgcareers-jobs.json'), 'utf8'),
);
...
/**
 * Spec 861 / T04 — `GhostStoryGamesService` unit tests (≥ 9 cases).
 */
describe('GhostStoryGamesService — Spec 861 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through GhostStoryGamesModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [GhostStoryGamesModule],
      }).compile();
      const service = moduleRef.get(GhostStoryGamesService);
      expect(service).toBeInstanceOf(GhostStoryGamesService);
      await moduleRef.close();
    });

    it('exports the Site.GHOST_STORY_GAMES = "gsgcareers" enum value', () => {
      expect(Site.GHOST_STORY_GAMES).toBe('gsgcareers');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new GhostStoryGamesService();
      const result = await service.scrape({
        siteType: [Site.GHOST_STORY_GAMES],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'gsgcareers-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.GHOST_STORY_GAMES);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Ghost Story Games');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/gsgcareers/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/gsgcareers/jobs/');
      ...
      expect(job0?.description).toContain('Ghost Story Games');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/gsgcareers/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Ghost Story Games');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'gsgcareers-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
