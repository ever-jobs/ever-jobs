...
import { ZolaModule, ZolaService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'zola-jobs.json'), 'utf8'),
);
...
/**
 * Spec 727 / T04 — `ZolaService` unit tests (≥ 9 cases).
 */
describe('ZolaService — Spec 727 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ZolaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ZolaModule],
      }).compile();
      const service = moduleRef.get(ZolaService);
      expect(service).toBeInstanceOf(ZolaService);
      await moduleRef.close();
    });

    it('exports the Site.ZOLA = "zola" enum value', () => {
      expect(Site.ZOLA).toBe('zola');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const job0 = dto.jobs.find((j) => j.id === 'zola-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ZOLA);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Zola');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/zola/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/zola/jobs/');
      ...
      expect(job0?.description).toContain('Zola');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/zola/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Zola');
      }
    });
  });
