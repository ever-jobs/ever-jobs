...
import { MillModule, MillService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'mill-jobs.json'), 'utf8'),
);
...
/**
 * Spec 554 / T04 — `MillService` unit tests (≥ 9 cases).
 */
describe('MillService — Spec 554 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through MillModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [MillModule],
      }).compile();
      const service = moduleRef.get(MillService);
      expect(service).toBeInstanceOf(MillService);
      await moduleRef.close();
    });

    it('exports the Site.MILL = "mill" enum value', () => {
      expect(Site.MILL).toBe('mill');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const job0 = dto.jobs.find((j) => j.id === 'mill-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.MILL);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Mill');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/mill/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/mill/jobs/');
      ...
      expect(job0?.description).toContain('Mill');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/mill/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Mill');
      }
    });
  });
