...
import { AbnormalsecurityModule, AbnormalsecurityService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'abnormalsecurity-jobs.json'), 'utf8'),
);
...
/**
 * Spec 429 / T04 — `AbnormalsecurityService` unit tests (≥ 9 cases).
 */
describe('AbnormalsecurityService — Spec 429 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AbnormalsecurityModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AbnormalsecurityModule],
      }).compile();
      const service = moduleRef.get(AbnormalsecurityService);
      expect(service).toBeInstanceOf(AbnormalsecurityService);
      await moduleRef.close();
    });

    it('exports the Site.ABNORMALSECURITY = "abnormalsecurity" enum value', () => {
      expect(Site.ABNORMALSECURITY).toBe('abnormalsecurity');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AbnormalsecurityService();
      const result = await service.scrape({
        siteType: [Site.ABNORMALSECURITY],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'abnormalsecurity-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ABNORMALSECURITY);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Abnormal Security');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/abnormalsecurity/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/abnormalsecurity/jobs/');
      ...
      expect(job0?.description).toContain('Abnormal Security');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/abnormalsecurity/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Abnormal Security');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'abnormalsecurity-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
