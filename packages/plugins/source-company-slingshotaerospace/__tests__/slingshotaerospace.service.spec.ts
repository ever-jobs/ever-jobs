...
import { SlingshotAerospaceModule, SlingshotAerospaceService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'slingshotaerospace-jobs.json'), 'utf8'),
);
...
/**
 * Spec 785 / T04 — `SlingshotAerospaceService` unit tests (≥ 9 cases).
 */
describe('SlingshotAerospaceService — Spec 785 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SlingshotAerospaceModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SlingshotAerospaceModule],
      }).compile();
      const service = moduleRef.get(SlingshotAerospaceService);
      expect(service).toBeInstanceOf(SlingshotAerospaceService);
      await moduleRef.close();
    });

    it('exports the Site.SLINGSHOT_AEROSPACE = "slingshotaerospace" enum value', () => {
      expect(Site.SLINGSHOT_AEROSPACE).toBe('slingshotaerospace');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new SlingshotAerospaceService();
      const result = await service.scrape({
        siteType: [Site.SLINGSHOT_AEROSPACE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'slingshotaerospace-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.SLINGSHOT_AEROSPACE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Slingshot Aerospace');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/slingshotaerospace/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/slingshotaerospace/jobs/');
      ...
      expect(job0?.description).toContain('Slingshot Aerospace');
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Slingshot Aerospace');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'slingshotaerospace-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
