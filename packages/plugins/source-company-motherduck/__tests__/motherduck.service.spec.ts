...
import { MotherDuckModule, MotherDuckService } from '../src';

const COMPANY_NAME_EXPECT = 'MotherDuck';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'motherduck-jobs.json'), 'utf8'),
);
...
describe('MotherDuckService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through MotherDuckModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [MotherDuckModule],
      }).compile();
      const service = moduleRef.get(MotherDuckService);
      expect(service).toBeInstanceOf(MotherDuckService);
      await moduleRef.close();
    });

    it('exports the Site.MOTHERDUCK = "motherduck" enum value', () => {
      expect(Site.MOTHERDUCK).toBe('motherduck');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new MotherDuckService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.MOTHERDUCK],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('motherduck');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.MOTHERDUCK);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('motherduck-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new MotherDuckService(registry);
      const result = await service.scrape({
        siteType: [Site.MOTHERDUCK],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('motherduck');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('motherduck-x1');
      expect(result.jobs[0].site).toBe(Site.MOTHERDUCK);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('motherduck-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new MotherDuckService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.MOTHERDUCK],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new MotherDuckService();
      const result = await service.scrape({
        siteType: [Site.MOTHERDUCK],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
