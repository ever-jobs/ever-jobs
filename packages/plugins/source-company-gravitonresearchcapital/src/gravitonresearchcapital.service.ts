...
/**
 * Graviton Research Capital — Quantitative trading (HQ: Gurugram, India).
 *
 * Source (Spec 1737): Greenhouse board, scraped in this order:
 *   - `gravitonresearchcapital` — https://job-boards.greenhouse.io/gravitonresearchcapital
 *     verified live 2026-09-24: 19 open postings.
 *
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'gravitonresearchcapital', atsIdPrefix: 'gh-' },
];
...
@SourcePlugin({
  ...
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['gravitontrading.com', 'gravitonresearch.com'],
  description: 'Graviton Research Capital careers via Greenhouse. Tags: segment=quant-trading; industry=quantitative-trading.',
})
@Injectable()
export class GravitonResearchCapitalService implements IScraper {
  private readonly logger = new Logger(GravitonResearchCapitalService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Greenhouse source plugin is not registered; cannot scrape Graviton Research Capital');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`Graviton Research Capital: delegating to Greenhouse (${board.companySlug})`);
    ...
    this.logger.log(`Graviton Research Capital: scraped ${jobs.length} jobs`);
