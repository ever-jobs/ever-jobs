...
import { GravitonResearchCapitalModule, GravitonResearchCapitalService } from '../src';
...
const FIXTURE: Fixture = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'fixtures', 'gravitonresearchcapital-boards.json'), 'utf8'),
);
const COMPANY_NAME = 'Graviton Research Capital';
const ID_PREFIX = 'gravitonresearchcapital-';
...
describe('GravitonResearchCapitalService — Greenhouse delegation (Spec 1737)', () => {
  beforeEach(() => {
    mockGet.mockReset();
    mockPost.mockReset();
  ...
  describe('registration', () => {
    it('resolves through GravitonResearchCapitalModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({ imports: [GravitonResearchCapitalModule] }).compile();
      expect(moduleRef.get(GravitonResearchCapitalService)).toBeInstanceOf(GravitonResearchCapitalService);
      await moduleRef.close();
    });

    it('exports Site.GRAVITON = "gravitonresearchcapital"', () => {
      expect(Site.GRAVITON).toBe('gravitonresearchcapital');
    });

    it('declares a tagged company plugin with its domains', () => {
      const meta = Reflect.getMetadata(SOURCE_PLUGIN_METADATA, GravitonResearchCapitalService);
      expect(meta.site).toBe(Site.GRAVITON);
      expect(meta.category).toBe('company');
      expect(meta.isAts).toBeFalsy();
      expect(meta.companyDomains).toEqual(['gravitontrading.com','gravitonresearch.com']);
  ...
  describe('recorded board (real Greenhouse adapter, mocked HTTP)', () => {
    it('maps every recorded posting and re-stamps the company identity', async () => {
      const service = new GravitonResearchCapitalService(registryWith());
      const result = await service.scrape({ siteType: [Site.GRAVITON], resultsWanted: 100 } as ScraperInputDto);
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.GRAVITON);
    ...
    it('only requests the recorded board URLs', async () => {
      const service = new GravitonResearchCapitalService(registryWith());
      await service.scrape({ siteType: [Site.GRAVITON] } as ScraperInputDto);
    ...
    it('honours resultsWanted=1', async () => {
      const service = new GravitonResearchCapitalService(registryWith());
      const result = await service.scrape({ siteType: [Site.GRAVITON], resultsWanted: 1 } as ScraperInputDto);
    ...
    it('resolves with an empty result when the board is gone (HTTP 404)', async () => {
      ...
      const service = new GravitonResearchCapitalService(registryWith());
      const result = await service.scrape({ siteType: [Site.GRAVITON] } as ScraperInputDto);
  ...
  describe('delegation contract', () => {
    it('forwards the board and the caller input untouched', async () => {
      ...
      const service = new GravitonResearchCapitalService(
        registryWith(
          fakeBackend(
      ...
      expect(result.jobs[0].site).toBe(Site.GRAVITON);
    ...
    it('never forwards the caller\'s credentials to the board (Spec 1735 §4.5)', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new GravitonResearchCapitalService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
    ...
    it('makes no request when resultsWanted is 0', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new GravitonResearchCapitalService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      const result = await service.scrape({ siteType: [Site.GRAVITON], resultsWanted: 0 } as ScraperInputDto);
  ...
  describe('resilience', () => {
    it('reports not_registered when no registry is injected', async () => {
      const result = await new GravitonResearchCapitalService().scrape({ siteType: [Site.GRAVITON] } as ScraperInputDto);
      expect(result.jobs).toEqual([]);
    ...
    it('reports not_registered when Greenhouse is missing from the registry', async () => {
      const result = await new GravitonResearchCapitalService(new PluginRegistry()).scrape({
        siteType: [Site.GRAVITON],
      } as ScraperInputDto);
      expect(result.diagnostics?.reason).toBe('not_registered');
    });
