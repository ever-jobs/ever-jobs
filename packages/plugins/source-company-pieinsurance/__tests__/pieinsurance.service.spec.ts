...
import { PieInsuranceModule, PieInsuranceService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'pieinsurance-jobs.json'), 'utf8'),
);
...
/**
 * Spec 658 / T04 — `PieInsuranceService` unit tests (≥ 9 cases).
 */
describe('PieInsuranceService — Spec 658 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through PieInsuranceModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [PieInsuranceModule],
      }).compile();
      const service = moduleRef.get(PieInsuranceService);
      expect(service).toBeInstanceOf(PieInsuranceService);
      await moduleRef.close();
    });

    it('exports the Site.PIE_INSURANCE = "pieinsurance" enum value', () => {
      expect(Site.PIE_INSURANCE).toBe('pieinsurance');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new PieInsuranceService();
      const result = await service.scrape({
        siteType: [Site.PIE_INSURANCE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'pieinsurance-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.PIE_INSURANCE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Pie Insurance');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/pieinsurance/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/pieinsurance/jobs/');
      ...
      expect(job0?.description).toContain('Pie Insurance');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/pieinsurance/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Pie Insurance');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'pieinsurance-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
