...
import { BrueggerSBagelsModule, BrueggerSBagelsService } from '../src';

const COMPANY_NAME_EXPECT = 'Bruegger\'s Bagels';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'brueggersbagels-jobs.json'), 'utf8'),
);
...
describe('BrueggerSBagelsService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through BrueggerSBagelsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [BrueggerSBagelsModule],
      }).compile();
      const service = moduleRef.get(BrueggerSBagelsService);
      expect(service).toBeInstanceOf(BrueggerSBagelsService);
      await moduleRef.close();
    });

    it('exports the Site.BRUEGGER_S_BAGELS = "brueggersbagels" enum value', () => {
      expect(Site.BRUEGGER_S_BAGELS).toBe('brueggersbagels');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new BrueggerSBagelsService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.BRUEGGER_S_BAGELS],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'brueggersbagels-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.BRUEGGER_S_BAGELS);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('brueggersbagels-' + first.id);
      ...
      expect(calledUrls[0]).toContain('BrueggersBagels');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.BRUEGGER_S_BAGELS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('brueggersbagels-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new BrueggerSBagelsService(registry);
      const result = await service.scrape({
        siteType: [Site.BRUEGGER_S_BAGELS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('BrueggersBagels');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('brueggersbagels-x1');
      expect(result.jobs[0].site).toBe(Site.BRUEGGER_S_BAGELS);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('brueggersbagels-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new BrueggerSBagelsService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.BRUEGGER_S_BAGELS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new BrueggerSBagelsService();
      const result = await service.scrape({
        siteType: [Site.BRUEGGER_S_BAGELS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
