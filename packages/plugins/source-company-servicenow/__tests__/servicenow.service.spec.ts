...
import { ServiceNowModule, ServiceNowService } from '../src';

const COMPANY_NAME_EXPECT = 'ServiceNow';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'servicenow-jobs.json'), 'utf8'),
);
...
describe('ServiceNowService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ServiceNowModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ServiceNowModule],
      }).compile();
      const service = moduleRef.get(ServiceNowService);
      expect(service).toBeInstanceOf(ServiceNowService);
      await moduleRef.close();
    });

    it('exports the Site.SERVICENOW = "servicenow" enum value', () => {
      expect(Site.SERVICENOW).toBe('servicenow');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new ServiceNowService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.SERVICENOW],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('ServiceNow');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SERVICENOW);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('servicenow-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new ServiceNowService(registry);
      const result = await service.scrape({
        siteType: [Site.SERVICENOW],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('ServiceNow');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('servicenow-x1');
      expect(result.jobs[0].site).toBe(Site.SERVICENOW);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('servicenow-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new ServiceNowService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.SERVICENOW],
      } as ScraperInputDto);
