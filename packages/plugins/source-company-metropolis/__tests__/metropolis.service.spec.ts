...
import { MetropolisTechnologiesModule, MetropolisTechnologiesService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'metropolis-jobs.json'), 'utf8'),
);
...
/**
 * Spec 884 / T04 — `MetropolisTechnologiesService` unit tests (≥ 9 cases).
 */
describe('MetropolisTechnologiesService — Spec 884 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through MetropolisTechnologiesModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [MetropolisTechnologiesModule],
      }).compile();
      const service = moduleRef.get(MetropolisTechnologiesService);
      expect(service).toBeInstanceOf(MetropolisTechnologiesService);
      await moduleRef.close();
    });

    it('exports the Site.METROPOLIS_TECHNOLOGIES = "metropolis" enum value', () => {
      expect(Site.METROPOLIS_TECHNOLOGIES).toBe('metropolis');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new MetropolisTechnologiesService();
      const result = await service.scrape({
        siteType: [Site.METROPOLIS_TECHNOLOGIES],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'metropolis-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.METROPOLIS_TECHNOLOGIES);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Metropolis Technologies');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/metropolis/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/metropolis/jobs/');
      ...
      expect(job0?.description).toContain('Metropolis Technologies');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/metropolis/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Metropolis Technologies');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'metropolis-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
