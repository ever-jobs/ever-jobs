...
import { MethodFinancialModule, MethodFinancialService } from '../src';

const COMPANY_NAME_EXPECT = 'Method Financial';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'methodfinancial-jobs.json'), 'utf8'),
);
...
describe('MethodFinancialService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through MethodFinancialModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [MethodFinancialModule],
      }).compile();
      const service = moduleRef.get(MethodFinancialService);
      expect(service).toBeInstanceOf(MethodFinancialService);
      await moduleRef.close();
    });

    it('exports the Site.METHOD_FINANCIAL = "methodfinancial" enum value', () => {
      expect(Site.METHOD_FINANCIAL).toBe('methodfinancial');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new MethodFinancialService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.METHOD_FINANCIAL],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'methodfinancial-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.METHOD_FINANCIAL);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('methodfinancial-' + first.id);
      ...
      expect(calledUrls[0]).toContain('method');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.METHOD_FINANCIAL);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('methodfinancial-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new MethodFinancialService(registry);
      const result = await service.scrape({
        siteType: [Site.METHOD_FINANCIAL],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('method');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('methodfinancial-x1');
      expect(result.jobs[0].site).toBe(Site.METHOD_FINANCIAL);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('methodfinancial-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new MethodFinancialService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.METHOD_FINANCIAL],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new MethodFinancialService();
      const result = await service.scrape({
        siteType: [Site.METHOD_FINANCIAL],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
