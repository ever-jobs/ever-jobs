...
import { DashlaneModule, DashlaneService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'dashlane-jobs.json'), 'utf8'),
);
...
/**
 * Spec 473 / T04 — `DashlaneService` unit tests (≥ 9 cases).
 */
describe('DashlaneService — Spec 473 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through DashlaneModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [DashlaneModule],
      }).compile();
      const service = moduleRef.get(DashlaneService);
      expect(service).toBeInstanceOf(DashlaneService);
      await moduleRef.close();
    });

    it('exports the Site.DASHLANE = "dashlane" enum value', () => {
      expect(Site.DASHLANE).toBe('dashlane');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new DashlaneService();
      const result = await service.scrape({
        siteType: [Site.DASHLANE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'dashlane-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.DASHLANE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Dashlane');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/dashlane/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/dashlane/jobs/');
      ...
      expect(job0?.description).toContain('Dashlane');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/dashlane/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Dashlane');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'dashlane-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
