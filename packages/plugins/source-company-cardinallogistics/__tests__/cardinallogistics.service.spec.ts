...
import { CardinalLogisticsModule, CardinalLogisticsService } from '../src';

const COMPANY_NAME_EXPECT = 'Cardinal Logistics';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'cardinallogistics-jobs.json'), 'utf8'),
);
...
describe('CardinalLogisticsService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CardinalLogisticsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CardinalLogisticsModule],
      }).compile();
      const service = moduleRef.get(CardinalLogisticsService);
      expect(service).toBeInstanceOf(CardinalLogisticsService);
      await moduleRef.close();
    });

    it('exports the Site.CARDINAL_LOGISTICS = "cardinallogistics" enum value', () => {
      expect(Site.CARDINAL_LOGISTICS).toBe('cardinallogistics');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new CardinalLogisticsService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.CARDINAL_LOGISTICS],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'cardinallogistics-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.CARDINAL_LOGISTICS);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('cardinallogistics-' + first.id);
      ...
      expect(calledUrls[0]).toContain('CardinalLogistics1');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.CARDINAL_LOGISTICS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('cardinallogistics-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new CardinalLogisticsService(registry);
      const result = await service.scrape({
        siteType: [Site.CARDINAL_LOGISTICS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('CardinalLogistics1');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('cardinallogistics-x1');
      expect(result.jobs[0].site).toBe(Site.CARDINAL_LOGISTICS);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('cardinallogistics-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new CardinalLogisticsService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.CARDINAL_LOGISTICS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new CardinalLogisticsService();
      const result = await service.scrape({
        siteType: [Site.CARDINAL_LOGISTICS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
