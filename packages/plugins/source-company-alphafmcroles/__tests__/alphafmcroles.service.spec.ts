...
import { AlphafmcrolesModule, AlphafmcrolesService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'alphafmcroles-jobs.json'), 'utf8'),
);
...
/**
 * Spec 241 / T04 — `AlphafmcrolesService` unit tests (≥ 9 cases).
 */
describe('AlphafmcrolesService — Spec 241 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AlphafmcrolesModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AlphafmcrolesModule],
      }).compile();
      const service = moduleRef.get(AlphafmcrolesService);
      expect(service).toBeInstanceOf(AlphafmcrolesService);
      await moduleRef.close();
    });

    it('exports the Site.ALPHAFMCROLES = "alphafmcroles" enum value', () => {
      expect(Site.ALPHAFMCROLES).toBe('alphafmcroles');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AlphafmcrolesService();
      const result = await service.scrape({
        siteType: [Site.ALPHAFMCROLES],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'alphafmcroles-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ALPHAFMCROLES);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Alpha Financial Markets Consulting');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/alphafmcroles/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/alphafmcroles/jobs/');
      ...
      expect(job0?.description).toContain('Alpha Financial Markets Consulting');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/alphafmcroles/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Alpha Financial Markets Consulting');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'alphafmcroles-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
