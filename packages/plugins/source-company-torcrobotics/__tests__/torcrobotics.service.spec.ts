...
import { TorcRoboticsModule, TorcRoboticsService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'torcrobotics-jobs.json'), 'utf8'),
);
...
/**
 * Spec 800 / T04 — `TorcRoboticsService` unit tests (≥ 9 cases).
 */
describe('TorcRoboticsService — Spec 800 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through TorcRoboticsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [TorcRoboticsModule],
      }).compile();
      const service = moduleRef.get(TorcRoboticsService);
      expect(service).toBeInstanceOf(TorcRoboticsService);
      await moduleRef.close();
    });

    it('exports the Site.TORC_ROBOTICS = "torcrobotics" enum value', () => {
      expect(Site.TORC_ROBOTICS).toBe('torcrobotics');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new TorcRoboticsService();
      const result = await service.scrape({
        siteType: [Site.TORC_ROBOTICS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'torcrobotics-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.TORC_ROBOTICS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Torc Robotics');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/torcrobotics/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/torcrobotics/jobs/');
      ...
      expect(job0?.description).toContain('Torc Robotics');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/torcrobotics/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Torc Robotics');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'torcrobotics-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
