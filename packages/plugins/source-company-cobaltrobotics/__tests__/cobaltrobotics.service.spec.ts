...
import { CobaltRoboticsModule, CobaltRoboticsService } from '../src';

const COMPANY_NAME_EXPECT = 'Cobalt Robotics';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'cobaltrobotics-jobs.json'), 'utf8'),
);
...
describe('CobaltRoboticsService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CobaltRoboticsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CobaltRoboticsModule],
      }).compile();
      const service = moduleRef.get(CobaltRoboticsService);
      expect(service).toBeInstanceOf(CobaltRoboticsService);
      await moduleRef.close();
    });

    it('exports the Site.COBALT_ROBOTICS = "cobaltrobotics" enum value', () => {
      expect(Site.COBALT_ROBOTICS).toBe('cobaltrobotics');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new CobaltRoboticsService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.COBALT_ROBOTICS],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'cobaltrobotics-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.COBALT_ROBOTICS);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('cobaltrobotics-' + first.id);
      ...
      expect(calledUrls[0]).toContain('cobaltrobotics');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.COBALT_ROBOTICS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('cobaltrobotics-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new CobaltRoboticsService(registry);
      const result = await service.scrape({
        siteType: [Site.COBALT_ROBOTICS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('cobaltrobotics');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('cobaltrobotics-x1');
      expect(result.jobs[0].site).toBe(Site.COBALT_ROBOTICS);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('cobaltrobotics-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new CobaltRoboticsService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.COBALT_ROBOTICS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new CobaltRoboticsService();
      const result = await service.scrape({
        siteType: [Site.COBALT_ROBOTICS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
