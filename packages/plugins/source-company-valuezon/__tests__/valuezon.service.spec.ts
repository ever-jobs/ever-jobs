...
import { VALUEZONModule, VALUEZONService } from '../src';

const COMPANY_NAME_EXPECT = 'VALUEZON';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'valuezon-jobs.json'), 'utf8'),
);
...
describe('VALUEZONService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through VALUEZONModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [VALUEZONModule],
      }).compile();
      const service = moduleRef.get(VALUEZONService);
      expect(service).toBeInstanceOf(VALUEZONService);
      await moduleRef.close();
    });

    it('exports the Site.VALUEZON = "valuezon" enum value', () => {
      expect(Site.VALUEZON).toBe('valuezon');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new VALUEZONService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.VALUEZON],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('valuezon.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.VALUEZON);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('valuezon-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new VALUEZONService(registry);
      const result = await service.scrape({
        siteType: [Site.VALUEZON],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('valuezon');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('valuezon-x1');
      expect(result.jobs[0].site).toBe(Site.VALUEZON);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('valuezon-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new VALUEZONService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.VALUEZON],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new VALUEZONService();
      const result = await service.scrape({
        siteType: [Site.VALUEZON],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
