...
/**
 * Copart — Online vehicle auctions (HQ: Dallas, TX, USA).
 *
 * Source (Spec 1736): Workday board, scraped in this order:
 *   - `copart:12:Copart` — https://copart.wd12.myworkdayjobs.com/Copart
 *     verified live 2026-09-24: 371 open postings.
 *
 ...
 * party board.
 *
 * Tags: segment=workday-enterprise; industry=online-vehicle-auctions.
 */
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'copart:12:Copart', atsIdPrefix: 'wd-copart-' },
];
...
@SourcePlugin({
  ...
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['copart.com'],
  description: 'Copart careers via Workday. Tags: segment=workday-enterprise; industry=online-vehicle-auctions.',
})
@Injectable()
export class CopartService implements IScraper {
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Workday source plugin is not registered; cannot scrape Copart');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`Copart: delegating to Workday (${board.companySlug})`);
    ...
    this.logger.log(`Copart: scraped ${jobs.length} jobs`);
