...
import { TogetheraiModule, TogetheraiService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'togetherai-jobs.json'), 'utf8'),
);
...
/**
 * Spec 449 / T04 — `TogetheraiService` unit tests (≥ 9 cases).
 */
describe('TogetheraiService — Spec 449 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through TogetheraiModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [TogetheraiModule],
      }).compile();
      const service = moduleRef.get(TogetheraiService);
      expect(service).toBeInstanceOf(TogetheraiService);
      await moduleRef.close();
    });

    it('exports the Site.TOGETHERAI = "togetherai" enum value', () => {
      expect(Site.TOGETHERAI).toBe('togetherai');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new TogetheraiService();
      const result = await service.scrape({
        siteType: [Site.TOGETHERAI],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'togetherai-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.TOGETHERAI);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Together AI');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/togetherai/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/togetherai/jobs/');
      ...
      expect(job0?.description).toContain('Together AI');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/togetherai/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Together AI');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'togetherai-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
