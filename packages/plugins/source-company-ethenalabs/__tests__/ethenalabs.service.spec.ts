...
import { EthenaLabsModule, EthenaLabsService } from '../src';

const COMPANY_NAME_EXPECT = 'Ethena Labs';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'ethenalabs-jobs.json'), 'utf8'),
);
...
describe('EthenaLabsService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through EthenaLabsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [EthenaLabsModule],
      }).compile();
      const service = moduleRef.get(EthenaLabsService);
      expect(service).toBeInstanceOf(EthenaLabsService);
      await moduleRef.close();
    });

    it('exports the Site.ETHENA_LABS = "ethenalabs" enum value', () => {
      expect(Site.ETHENA_LABS).toBe('ethenalabs');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new EthenaLabsService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.ETHENA_LABS],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('ethenalabs');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ETHENA_LABS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('ethenalabs-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new EthenaLabsService(registry);
      const result = await service.scrape({
        siteType: [Site.ETHENA_LABS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('ethenalabs');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('ethenalabs-x1');
      expect(result.jobs[0].site).toBe(Site.ETHENA_LABS);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('ethenalabs-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new EthenaLabsService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.ETHENA_LABS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new EthenaLabsService();
      const result = await service.scrape({
        siteType: [Site.ETHENA_LABS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
