...
import { AeroVectModule, AeroVectService } from '../src';

const COMPANY_NAME_EXPECT = 'AeroVect';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'aerovect-jobs.json'), 'utf8'),
);
...
describe('AeroVectService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AeroVectModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AeroVectModule],
      }).compile();
      const service = moduleRef.get(AeroVectService);
      expect(service).toBeInstanceOf(AeroVectService);
      await moduleRef.close();
    });

    it('exports the Site.AEROVECT = "aerovect" enum value', () => {
      expect(Site.AEROVECT).toBe('aerovect');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new AeroVectService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.AEROVECT],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('aerovect');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.AEROVECT);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('aerovect-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new AeroVectService(registry);
      const result = await service.scrape({
        siteType: [Site.AEROVECT],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('aerovect');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('aerovect-x1');
      expect(result.jobs[0].site).toBe(Site.AEROVECT);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('aerovect-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new AeroVectService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.AEROVECT],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new AeroVectService();
      const result = await service.scrape({
        siteType: [Site.AEROVECT],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
