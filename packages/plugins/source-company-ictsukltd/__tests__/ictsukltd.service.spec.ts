...
import { ICTSUKLtdModule, ICTSUKLtdService } from '../src';

const COMPANY_NAME_EXPECT = 'ICTS (UK) Ltd';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'ictsukltd-jobs.json'), 'utf8'),
);
...
describe('ICTSUKLtdService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ICTSUKLtdModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ICTSUKLtdModule],
      }).compile();
      const service = moduleRef.get(ICTSUKLtdService);
      expect(service).toBeInstanceOf(ICTSUKLtdService);
      await moduleRef.close();
    });

    it('exports the Site.ICTS_UK_LTD = "ictsukltd" enum value', () => {
      expect(Site.ICTS_UK_LTD).toBe('ictsukltd');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new ICTSUKLtdService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.ICTS_UK_LTD],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('ICTSUKLtd');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ICTS_UK_LTD);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('ictsukltd-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new ICTSUKLtdService(registry);
      const result = await service.scrape({
        siteType: [Site.ICTS_UK_LTD],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('ICTSUKLtd');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('ictsukltd-x1');
      expect(result.jobs[0].site).toBe(Site.ICTS_UK_LTD);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('ictsukltd-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new ICTSUKLtdService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.ICTS_UK_LTD],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new ICTSUKLtdService();
      const result = await service.scrape({
        siteType: [Site.ICTS_UK_LTD],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
