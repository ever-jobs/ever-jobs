...
import { HiveHealthModule, HiveHealthService } from '../src';

const COMPANY_NAME_EXPECT = 'Hive Health';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'hivehealth-jobs.json'), 'utf8'),
);
...
describe('HiveHealthService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through HiveHealthModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [HiveHealthModule],
      }).compile();
      const service = moduleRef.get(HiveHealthService);
      expect(service).toBeInstanceOf(HiveHealthService);
      await moduleRef.close();
    });

    it('exports the Site.HIVE_HEALTH = "hivehealth" enum value', () => {
      expect(Site.HIVE_HEALTH).toBe('hivehealth');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new HiveHealthService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.HIVE_HEALTH],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('hivehealth');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.HIVE_HEALTH);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('hivehealth-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new HiveHealthService(registry);
      const result = await service.scrape({
        siteType: [Site.HIVE_HEALTH],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('hivehealth');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('hivehealth-x1');
      expect(result.jobs[0].site).toBe(Site.HIVE_HEALTH);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('hivehealth-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new HiveHealthService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.HIVE_HEALTH],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new HiveHealthService();
      const result = await service.scrape({
        siteType: [Site.HIVE_HEALTH],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
