...
import { OxfamAmericaModule, OxfamAmericaService } from '../src';

const COMPANY_NAME_EXPECT = 'Oxfam America';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'oxfamamerica-jobs.json'), 'utf8'),
);
...
describe('OxfamAmericaService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through OxfamAmericaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [OxfamAmericaModule],
      }).compile();
      const service = moduleRef.get(OxfamAmericaService);
      expect(service).toBeInstanceOf(OxfamAmericaService);
      await moduleRef.close();
    });

    it('exports the Site.OXFAM_AMERICA = "oxfamamerica" enum value', () => {
      expect(Site.OXFAM_AMERICA).toBe('oxfamamerica');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new OxfamAmericaService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.OXFAM_AMERICA],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('oxfamamerica2');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.OXFAM_AMERICA);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('oxfamamerica-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new OxfamAmericaService(registry);
      const result = await service.scrape({
        siteType: [Site.OXFAM_AMERICA],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('oxfamamerica2');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('oxfamamerica-x1');
      expect(result.jobs[0].site).toBe(Site.OXFAM_AMERICA);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('oxfamamerica-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new OxfamAmericaService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.OXFAM_AMERICA],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new OxfamAmericaService();
      const result = await service.scrape({
        siteType: [Site.OXFAM_AMERICA],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
