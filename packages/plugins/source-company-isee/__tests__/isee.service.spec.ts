...
import { ISEEModule, ISEEService } from '../src';

const COMPANY_NAME_EXPECT = 'ISEE';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'isee-jobs.json'), 'utf8'),
);
...
describe('ISEEService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ISEEModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ISEEModule],
      }).compile();
      const service = moduleRef.get(ISEEService);
      expect(service).toBeInstanceOf(ISEEService);
      await moduleRef.close();
    });

    it('exports the Site.ISEE = "isee" enum value', () => {
      expect(Site.ISEE).toBe('isee');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new ISEEService(registryWithLever());
      ...
      expect(calledUrls[0]).toContain('isee');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ISEE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('isee-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new ISEEService(registry);
      const result = await service.scrape({
        siteType: [Site.ISEE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('isee');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('isee-x1');
      expect(result.jobs[0].site).toBe(Site.ISEE);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('isee-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new ISEEService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.ISEE],
      } as ScraperInputDto);
