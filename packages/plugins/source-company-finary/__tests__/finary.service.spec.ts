...
import { FinaryModule, FinaryService } from '../src';

const COMPANY_NAME_EXPECT = 'Finary';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'finary-jobs.json'), 'utf8'),
);
...
describe('FinaryService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through FinaryModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [FinaryModule],
      }).compile();
      const service = moduleRef.get(FinaryService);
      expect(service).toBeInstanceOf(FinaryService);
      await moduleRef.close();
    });

    it('exports the Site.FINARY = "finary" enum value', () => {
      expect(Site.FINARY).toBe('finary');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new FinaryService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.FINARY],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('finary');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.FINARY);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('finary-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new FinaryService(registry);
      const result = await service.scrape({
        siteType: [Site.FINARY],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('finary');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('finary-x1');
      expect(result.jobs[0].site).toBe(Site.FINARY);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('finary-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new FinaryService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.FINARY],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new FinaryService();
      const result = await service.scrape({
        siteType: [Site.FINARY],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
