...
import { SpeakeasyModule, SpeakeasyService } from '../src';

const COMPANY_NAME_EXPECT = 'Speakeasy';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'speakeasy-jobs.json'), 'utf8'),
);
...
describe('SpeakeasyService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SpeakeasyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SpeakeasyModule],
      }).compile();
      const service = moduleRef.get(SpeakeasyService);
      expect(service).toBeInstanceOf(SpeakeasyService);
      await moduleRef.close();
    });

    it('exports the Site.SPEAKEASY = "speakeasy" enum value', () => {
      expect(Site.SPEAKEASY).toBe('speakeasy');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new SpeakeasyService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.SPEAKEASY],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('speakeasy');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SPEAKEASY);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('speakeasy-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new SpeakeasyService(registry);
      const result = await service.scrape({
        siteType: [Site.SPEAKEASY],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('speakeasy');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('speakeasy-x1');
      expect(result.jobs[0].site).toBe(Site.SPEAKEASY);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('speakeasy-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new SpeakeasyService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.SPEAKEASY],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new SpeakeasyService();
      const result = await service.scrape({
        siteType: [Site.SPEAKEASY],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
