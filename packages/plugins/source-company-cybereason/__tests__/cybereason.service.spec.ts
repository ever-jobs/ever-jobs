...
import { CybereasonModule, CybereasonService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'cybereason-jobs.json'), 'utf8'),
);
...
/**
 * Spec 541 / T04 — `CybereasonService` unit tests (≥ 9 cases).
 */
describe('CybereasonService — Spec 541 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CybereasonModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CybereasonModule],
      }).compile();
      const service = moduleRef.get(CybereasonService);
      expect(service).toBeInstanceOf(CybereasonService);
      await moduleRef.close();
    });

    it('exports the Site.CYBEREASON = "cybereason" enum value', () => {
      expect(Site.CYBEREASON).toBe('cybereason');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new CybereasonService();
      const result = await service.scrape({
        siteType: [Site.CYBEREASON],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'cybereason-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.CYBEREASON);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Cybereason');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/cybereason/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/cybereason/jobs/');
      ...
      expect(job0?.description).toContain('Cybereason');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/cybereason/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Cybereason');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'cybereason-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
