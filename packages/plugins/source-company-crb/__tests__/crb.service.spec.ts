...
import { CRBModule, CRBService } from '../src';

const COMPANY_NAME_EXPECT = 'CRB';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'crb-jobs.json'), 'utf8'),
);
...
describe('CRBService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CRBModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CRBModule],
      }).compile();
      const service = moduleRef.get(CRBService);
      expect(service).toBeInstanceOf(CRBService);
      await moduleRef.close();
    });

    it('exports the Site.CRB = "crb" enum value', () => {
      expect(Site.CRB).toBe('crb');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new CRBService(registryWithSmartRecruiters());
      ...
      expect(job0?.site).toBe(Site.CRB);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('crb-' + first.id);
      ...
      expect(calledUrls[0]).toContain('CRB');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.CRB);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('crb-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new CRBService(registry);
      const result = await service.scrape({
        siteType: [Site.CRB],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('CRB');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('crb-x1');
      expect(result.jobs[0].site).toBe(Site.CRB);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('crb-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new CRBService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.CRB],
      } as ScraperInputDto);
