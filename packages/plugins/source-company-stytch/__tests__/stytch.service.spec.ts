...
import { StytchModule, StytchService } from '../src';

const COMPANY_NAME_EXPECT = 'Stytch';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'stytch-jobs.json'), 'utf8'),
);
...
describe('StytchService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through StytchModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [StytchModule],
      }).compile();
      const service = moduleRef.get(StytchService);
      expect(service).toBeInstanceOf(StytchService);
      await moduleRef.close();
    });

    it('exports the Site.STYTCH = "stytch" enum value', () => {
      expect(Site.STYTCH).toBe('stytch');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new StytchService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.STYTCH],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('stytch');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.STYTCH);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('stytch-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new StytchService(registry);
      const result = await service.scrape({
        siteType: [Site.STYTCH],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('stytch');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('stytch-x1');
      expect(result.jobs[0].site).toBe(Site.STYTCH);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('stytch-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new StytchService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.STYTCH],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new StytchService();
      const result = await service.scrape({
        siteType: [Site.STYTCH],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
