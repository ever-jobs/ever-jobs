...
import { ExplodingKittensModule, ExplodingKittensService } from '../src';

const COMPANY_NAME_EXPECT = 'Exploding Kittens';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'explodingkittens-jobs.json'), 'utf8'),
);
...
describe('ExplodingKittensService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ExplodingKittensModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ExplodingKittensModule],
      }).compile();
      const service = moduleRef.get(ExplodingKittensService);
      expect(service).toBeInstanceOf(ExplodingKittensService);
      await moduleRef.close();
    });

    it('exports the Site.EXPLODING_KITTENS = "explodingkittens" enum value', () => {
      expect(Site.EXPLODING_KITTENS).toBe('explodingkittens');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new ExplodingKittensService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.EXPLODING_KITTENS],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'explodingkittens-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.EXPLODING_KITTENS);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('explodingkittens-' + first.id);
      ...
      expect(calledUrls[0]).toContain('explodingkittens');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.EXPLODING_KITTENS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('explodingkittens-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new ExplodingKittensService(registry);
      const result = await service.scrape({
        siteType: [Site.EXPLODING_KITTENS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('explodingkittens');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('explodingkittens-x1');
      expect(result.jobs[0].site).toBe(Site.EXPLODING_KITTENS);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('explodingkittens-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new ExplodingKittensService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.EXPLODING_KITTENS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new ExplodingKittensService();
      const result = await service.scrape({
        siteType: [Site.EXPLODING_KITTENS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
