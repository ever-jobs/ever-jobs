...
import { TabapayModule, TabapayService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'tabapay-jobs.json'), 'utf8'),
);
...
/**
 * Spec 547 / T04 — `TabapayService` unit tests (≥ 9 cases).
 */
describe('TabapayService — Spec 547 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through TabapayModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [TabapayModule],
      }).compile();
      const service = moduleRef.get(TabapayService);
      expect(service).toBeInstanceOf(TabapayService);
      await moduleRef.close();
    });

    it('exports the Site.TABAPAY = "tabapay" enum value', () => {
      expect(Site.TABAPAY).toBe('tabapay');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new TabapayService();
      const result = await service.scrape({
        siteType: [Site.TABAPAY],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'tabapay-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.TABAPAY);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('TabaPay');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/tabapay/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/tabapay/jobs/');
      ...
      expect(job0?.description).toContain('TabaPay');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/tabapay/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('TabaPay');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'tabapay-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
