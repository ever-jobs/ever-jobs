...
import { WintermuteModule, WintermuteService } from '../src';

const COMPANY_NAME_EXPECT = 'Wintermute';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'wintermutetrading-jobs.json'), 'utf8'),
);
...
describe('WintermuteService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through WintermuteModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [WintermuteModule],
      }).compile();
      const service = moduleRef.get(WintermuteService);
      expect(service).toBeInstanceOf(WintermuteService);
      await moduleRef.close();
    });

    it('exports the Site.WINTERMUTE = "wintermutetrading" enum value', () => {
      expect(Site.WINTERMUTE).toBe('wintermutetrading');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new WintermuteService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.WINTERMUTE],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'wintermutetrading-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.WINTERMUTE);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('wintermutetrading-' + first.id);
      ...
      expect(calledUrls[0]).toContain('wintermute-trading');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.WINTERMUTE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('wintermutetrading-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new WintermuteService(registry);
      const result = await service.scrape({
        siteType: [Site.WINTERMUTE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('wintermute-trading');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('wintermutetrading-x1');
      expect(result.jobs[0].site).toBe(Site.WINTERMUTE);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('wintermutetrading-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new WintermuteService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.WINTERMUTE],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new WintermuteService();
      const result = await service.scrape({
        siteType: [Site.WINTERMUTE],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
