...
import { GeckoRoboticsModule, GeckoRoboticsService } from '../src';

const COMPANY_NAME_EXPECT = 'Gecko Robotics';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'geckorobotics-jobs.json'), 'utf8'),
);
...
describe('GeckoRoboticsService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through GeckoRoboticsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [GeckoRoboticsModule],
      }).compile();
      const service = moduleRef.get(GeckoRoboticsService);
      expect(service).toBeInstanceOf(GeckoRoboticsService);
      await moduleRef.close();
    });

    it('exports the Site.GECKO_ROBOTICS = "geckorobotics" enum value', () => {
      expect(Site.GECKO_ROBOTICS).toBe('geckorobotics');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new GeckoRoboticsService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.GECKO_ROBOTICS],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'geckorobotics-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.GECKO_ROBOTICS);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('geckorobotics-' + first.id);
      ...
      expect(calledUrls[0]).toContain('gecko-robotics');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.GECKO_ROBOTICS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('geckorobotics-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new GeckoRoboticsService(registry);
      const result = await service.scrape({
        siteType: [Site.GECKO_ROBOTICS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('gecko-robotics');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('geckorobotics-x1');
      expect(result.jobs[0].site).toBe(Site.GECKO_ROBOTICS);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('geckorobotics-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new GeckoRoboticsService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.GECKO_ROBOTICS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new GeckoRoboticsService();
      const result = await service.scrape({
        siteType: [Site.GECKO_ROBOTICS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
