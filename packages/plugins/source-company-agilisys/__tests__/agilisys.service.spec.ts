...
import { AgilisysModule, AgilisysService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'agilisys-jobs.json'), 'utf8'),
);
...
/**
 * Spec 208 / T04 — `AgilisysService` unit tests (≥ 9 cases).
 */
describe('AgilisysService — Spec 208 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AgilisysModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AgilisysModule],
      }).compile();
      const service = moduleRef.get(AgilisysService);
      expect(service).toBeInstanceOf(AgilisysService);
      await moduleRef.close();
    });

    it('exports the Site.AGILISYS = "agilisys" enum value', () => {
      expect(Site.AGILISYS).toBe('agilisys');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AgilisysService();
      const result = await service.scrape({
        siteType: [Site.AGILISYS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'agilisys-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.AGILISYS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Agilisys');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/agilisys/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/agilisys/jobs/');
      ...
      expect(job0?.description).toContain('Agilisys');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/agilisys/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Agilisys');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'agilisys-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
