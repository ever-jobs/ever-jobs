...
import { PerplexityModule, PerplexityService } from '../src';

const COMPANY_NAME_EXPECT = 'Perplexity';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'perplexity-jobs.json'), 'utf8'),
);
...
describe('PerplexityService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through PerplexityModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [PerplexityModule],
      }).compile();
      const service = moduleRef.get(PerplexityService);
      expect(service).toBeInstanceOf(PerplexityService);
      await moduleRef.close();
    });

    it('exports the Site.PERPLEXITY = "perplexity" enum value', () => {
      expect(Site.PERPLEXITY).toBe('perplexity');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new PerplexityService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.PERPLEXITY],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('perplexity');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.PERPLEXITY);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('perplexity-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new PerplexityService(registry);
      const result = await service.scrape({
        siteType: [Site.PERPLEXITY],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('perplexity');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('perplexity-x1');
      expect(result.jobs[0].site).toBe(Site.PERPLEXITY);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('perplexity-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new PerplexityService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.PERPLEXITY],
      } as ScraperInputDto);
