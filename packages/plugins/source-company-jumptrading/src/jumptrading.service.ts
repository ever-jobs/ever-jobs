...
/**
 * Jump Trading — Quantitative trading (HQ: Chicago, IL, USA).
 *
 * Source (Spec 1737): Greenhouse board, scraped in this order:
 *   - `jumptrading` — https://job-boards.greenhouse.io/jumptrading
 *     verified live 2026-09-24: 114 open postings.
 *
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'jumptrading', atsIdPrefix: 'gh-' },
];

@SourcePlugin({
  site: Site.JUMP_TRADING,
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['jumptrading.com'],
  description: 'Jump Trading careers via Greenhouse. Tags: segment=quant-trading; industry=quantitative-trading.',
})
@Injectable()
export class JumpTradingService implements IScraper {
  private readonly logger = new Logger(JumpTradingService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Greenhouse source plugin is not registered; cannot scrape Jump Trading');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`Jump Trading: delegating to Greenhouse (${board.companySlug})`);
    ...
    this.logger.log(`Jump Trading: scraped ${jobs.length} jobs`);
