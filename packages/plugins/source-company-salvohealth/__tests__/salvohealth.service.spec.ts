...
import { SalvoHealthModule, SalvoHealthService } from '../src';

const COMPANY_NAME_EXPECT = 'Salvo Health';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'salvohealth-jobs.json'), 'utf8'),
);
...
describe('SalvoHealthService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SalvoHealthModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SalvoHealthModule],
      }).compile();
      const service = moduleRef.get(SalvoHealthService);
      expect(service).toBeInstanceOf(SalvoHealthService);
      await moduleRef.close();
    });

    it('exports the Site.SALVO_HEALTH = "salvohealth" enum value', () => {
      expect(Site.SALVO_HEALTH).toBe('salvohealth');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new SalvoHealthService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.SALVO_HEALTH],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'salvohealth-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.SALVO_HEALTH);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('salvohealth-' + first.id);
      ...
      expect(calledUrls[0]).toContain('salvohealth');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SALVO_HEALTH);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('salvohealth-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new SalvoHealthService(registry);
      const result = await service.scrape({
        siteType: [Site.SALVO_HEALTH],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('salvohealth');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('salvohealth-x1');
      expect(result.jobs[0].site).toBe(Site.SALVO_HEALTH);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('salvohealth-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new SalvoHealthService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.SALVO_HEALTH],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new SalvoHealthService();
      const result = await service.scrape({
        siteType: [Site.SALVO_HEALTH],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
