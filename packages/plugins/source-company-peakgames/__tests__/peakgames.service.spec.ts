...
import { PeakModule, PeakService } from '../src';

const COMPANY_NAME_EXPECT = 'Peak';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'peakgames-jobs.json'), 'utf8'),
);
...
describe('PeakService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through PeakModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [PeakModule],
      }).compile();
      const service = moduleRef.get(PeakService);
      expect(service).toBeInstanceOf(PeakService);
      await moduleRef.close();
    });

    it('exports the Site.PEAK = "peakgames" enum value', () => {
      expect(Site.PEAK).toBe('peakgames');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new PeakService(registryWithLever());
      ...
      expect(calledUrls[0]).toContain('peakgames');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.PEAK);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('peakgames-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new PeakService(registry);
      const result = await service.scrape({
        siteType: [Site.PEAK],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('peakgames');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('peakgames-x1');
      expect(result.jobs[0].site).toBe(Site.PEAK);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('peakgames-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new PeakService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.PEAK],
      } as ScraperInputDto);
