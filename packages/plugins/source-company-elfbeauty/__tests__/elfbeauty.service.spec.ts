...
import { ELFBeautyModule, ELFBeautyService } from '../src';

const COMPANY_NAME_EXPECT = 'e.l.f. Beauty';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'elfbeauty-jobs.json'), 'utf8'),
);
...
describe('ELFBeautyService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ELFBeautyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ELFBeautyModule],
      }).compile();
      const service = moduleRef.get(ELFBeautyService);
      expect(service).toBeInstanceOf(ELFBeautyService);
      await moduleRef.close();
    });

    it('exports the Site.E_L_F_BEAUTY = "elfbeauty" enum value', () => {
      expect(Site.E_L_F_BEAUTY).toBe('elfbeauty');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new ELFBeautyService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.E_L_F_BEAUTY],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('elfbeauty');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.E_L_F_BEAUTY);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('elfbeauty-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new ELFBeautyService(registry);
      const result = await service.scrape({
        siteType: [Site.E_L_F_BEAUTY],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('elfbeauty');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('elfbeauty-x1');
      expect(result.jobs[0].site).toBe(Site.E_L_F_BEAUTY);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('elfbeauty-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new ELFBeautyService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.E_L_F_BEAUTY],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new ELFBeautyService();
      const result = await service.scrape({
        siteType: [Site.E_L_F_BEAUTY],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
