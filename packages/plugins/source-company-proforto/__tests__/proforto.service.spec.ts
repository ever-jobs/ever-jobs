...
import { ProfortoModule, ProfortoService } from '../src';

const COMPANY_NAME_EXPECT = 'Proforto';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'proforto-jobs.json'), 'utf8'),
);
...
describe('ProfortoService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ProfortoModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ProfortoModule],
      }).compile();
      const service = moduleRef.get(ProfortoService);
      expect(service).toBeInstanceOf(ProfortoService);
      await moduleRef.close();
    });

    it('exports the Site.PROFORTO = "proforto" enum value', () => {
      expect(Site.PROFORTO).toBe('proforto');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new ProfortoService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.PROFORTO],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('proforto.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.PROFORTO);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('proforto-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new ProfortoService(registry);
      const result = await service.scrape({
        siteType: [Site.PROFORTO],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('proforto');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('proforto-x1');
      expect(result.jobs[0].site).toBe(Site.PROFORTO);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('proforto-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new ProfortoService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.PROFORTO],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new ProfortoService();
      const result = await service.scrape({
        siteType: [Site.PROFORTO],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
