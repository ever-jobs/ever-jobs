...
import { MiroModule, MiroService } from '../src';

const COMPANY_NAME_EXPECT = 'Miro';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'miro-jobs.json'), 'utf8'),
);
...
describe('MiroService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through MiroModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [MiroModule],
      }).compile();
      const service = moduleRef.get(MiroService);
      expect(service).toBeInstanceOf(MiroService);
      await moduleRef.close();
    });

    it('exports the Site.MIRO = "miro" enum value', () => {
      expect(Site.MIRO).toBe('miro');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new MiroService(registryWithAshby());
      ...
      expect(calledUrls[0]).toContain('miro');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.MIRO);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('miro-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new MiroService(registry);
      const result = await service.scrape({
        siteType: [Site.MIRO],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('miro');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('miro-x1');
      expect(result.jobs[0].site).toBe(Site.MIRO);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('miro-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new MiroService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.MIRO],
      } as ScraperInputDto);
