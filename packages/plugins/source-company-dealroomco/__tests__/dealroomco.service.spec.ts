...
import { DealroomCoModule, DealroomCoService } from '../src';

const COMPANY_NAME_EXPECT = 'Dealroom.co';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'dealroomco-jobs.json'), 'utf8'),
);
...
describe('DealroomCoService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through DealroomCoModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [DealroomCoModule],
      }).compile();
      const service = moduleRef.get(DealroomCoService);
      expect(service).toBeInstanceOf(DealroomCoService);
      await moduleRef.close();
    });

    it('exports the Site.DEALROOM_CO = "dealroomco" enum value', () => {
      expect(Site.DEALROOM_CO).toBe('dealroomco');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new DealroomCoService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.DEALROOM_CO],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('dealroomco.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.DEALROOM_CO);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('dealroomco-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new DealroomCoService(registry);
      const result = await service.scrape({
        siteType: [Site.DEALROOM_CO],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('dealroomco');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('dealroomco-x1');
      expect(result.jobs[0].site).toBe(Site.DEALROOM_CO);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('dealroomco-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new DealroomCoService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.DEALROOM_CO],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new DealroomCoService();
      const result = await service.scrape({
        siteType: [Site.DEALROOM_CO],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
