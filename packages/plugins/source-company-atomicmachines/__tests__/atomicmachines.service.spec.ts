...
import { AtomicMachinesModule, AtomicMachinesService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'atomicmachines-jobs.json'), 'utf8'),
);
...
/**
 * Spec 810 / T04 — `AtomicMachinesService` unit tests (≥ 9 cases).
 */
describe('AtomicMachinesService — Spec 810 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AtomicMachinesModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AtomicMachinesModule],
      }).compile();
      const service = moduleRef.get(AtomicMachinesService);
      expect(service).toBeInstanceOf(AtomicMachinesService);
      await moduleRef.close();
    });

    it('exports the Site.ATOMIC_MACHINES = "atomicmachines" enum value', () => {
      expect(Site.ATOMIC_MACHINES).toBe('atomicmachines');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AtomicMachinesService();
      const result = await service.scrape({
        siteType: [Site.ATOMIC_MACHINES],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'atomicmachines-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ATOMIC_MACHINES);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Atomic Machines');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/atomicmachines/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/atomicmachines/jobs/');
      ...
      expect(job0?.description).toContain('Atomic Machines');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/atomicmachines/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Atomic Machines');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'atomicmachines-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
