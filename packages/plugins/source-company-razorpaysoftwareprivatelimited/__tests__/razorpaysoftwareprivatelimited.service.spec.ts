...
import { RazorpayModule, RazorpayService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'razorpaysoftwareprivatelimited-jobs.json'), 'utf8'),
);
...
/**
 * Spec 917 / T04 — `RazorpayService` unit tests (≥ 9 cases).
 */
describe('RazorpayService — Spec 917 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through RazorpayModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [RazorpayModule],
      }).compile();
      const service = moduleRef.get(RazorpayService);
      expect(service).toBeInstanceOf(RazorpayService);
      await moduleRef.close();
    });

    it('exports the Site.RAZORPAY = "razorpaysoftwareprivatelimited" enum value', () => {
      expect(Site.RAZORPAY).toBe('razorpaysoftwareprivatelimited');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const job0 = dto.jobs.find((j) => j.id === 'razorpaysoftwareprivatelimited-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.RAZORPAY);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Razorpay');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/razorpaysoftwareprivatelimited/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/razorpaysoftwareprivatelimited/jobs/');
      ...
      expect(job0?.description).toContain('Razorpay');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/razorpaysoftwareprivatelimited/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Razorpay');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'razorpaysoftwareprivatelimited-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
