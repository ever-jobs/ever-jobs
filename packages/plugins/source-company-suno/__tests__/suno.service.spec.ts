...
import { SunoModule, SunoService } from '../src';

const COMPANY_NAME_EXPECT = 'Suno';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'suno-jobs.json'), 'utf8'),
);
...
describe('SunoService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SunoModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SunoModule],
      }).compile();
      const service = moduleRef.get(SunoService);
      expect(service).toBeInstanceOf(SunoService);
      await moduleRef.close();
    });

    it('exports the Site.SUNO = "suno" enum value', () => {
      expect(Site.SUNO).toBe('suno');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new SunoService(registryWithAshby());
      ...
      expect(calledUrls[0]).toContain('suno');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SUNO);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('suno-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new SunoService(registry);
      const result = await service.scrape({
        siteType: [Site.SUNO],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('suno');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('suno-x1');
      expect(result.jobs[0].site).toBe(Site.SUNO);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('suno-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new SunoService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.SUNO],
      } as ScraperInputDto);
