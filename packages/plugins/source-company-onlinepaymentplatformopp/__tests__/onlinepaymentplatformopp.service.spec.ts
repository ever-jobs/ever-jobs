...
import { OnlinePaymentPlatformOPPModule, OnlinePaymentPlatformOPPService } from '../src';

const COMPANY_NAME_EXPECT = 'Online Payment Platform (OPP)';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'onlinepaymentplatformopp-jobs.json'), 'utf8'),
);
...
describe('OnlinePaymentPlatformOPPService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through OnlinePaymentPlatformOPPModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [OnlinePaymentPlatformOPPModule],
      }).compile();
      const service = moduleRef.get(OnlinePaymentPlatformOPPService);
      expect(service).toBeInstanceOf(OnlinePaymentPlatformOPPService);
      await moduleRef.close();
    });

    it('exports the Site.ONLINE_PAYMENT_PLATFORM_OPP = "onlinepaymentplatformopp" enum value', () => {
      expect(Site.ONLINE_PAYMENT_PLATFORM_OPP).toBe('onlinepaymentplatformopp');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new OnlinePaymentPlatformOPPService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.ONLINE_PAYMENT_PLATFORM_OPP],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'onlinepaymentplatformopp-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Recruitee's defaults
      expect(job0?.site).toBe(Site.ONLINE_PAYMENT_PLATFORM_OPP);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('onlinepaymentplatformopp-' + first.id);
      ...
      expect(calledUrls[0]).toContain('onlinepaymentplatform.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ONLINE_PAYMENT_PLATFORM_OPP);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('onlinepaymentplatformopp-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new OnlinePaymentPlatformOPPService(registry);
      const result = await service.scrape({
        siteType: [Site.ONLINE_PAYMENT_PLATFORM_OPP],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('onlinepaymentplatform');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('onlinepaymentplatformopp-x1');
      expect(result.jobs[0].site).toBe(Site.ONLINE_PAYMENT_PLATFORM_OPP);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('onlinepaymentplatformopp-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new OnlinePaymentPlatformOPPService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.ONLINE_PAYMENT_PLATFORM_OPP],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new OnlinePaymentPlatformOPPService();
      const result = await service.scrape({
        siteType: [Site.ONLINE_PAYMENT_PLATFORM_OPP],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
