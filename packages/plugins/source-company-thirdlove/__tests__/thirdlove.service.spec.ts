...
import { ThirdLoveModule, ThirdLoveService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'thirdlove-jobs.json'), 'utf8'),
);
...
/**
 * Spec 600 / T04 — `ThirdLoveService` unit tests (≥ 9 cases).
 */
describe('ThirdLoveService — Spec 600 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ThirdLoveModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ThirdLoveModule],
      }).compile();
      const service = moduleRef.get(ThirdLoveService);
      expect(service).toBeInstanceOf(ThirdLoveService);
      await moduleRef.close();
    });

    it('exports the Site.THIRDLOVE = "thirdlove" enum value', () => {
      expect(Site.THIRDLOVE).toBe('thirdlove');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ThirdLoveService();
      const result = await service.scrape({
        siteType: [Site.THIRDLOVE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'thirdlove-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.THIRDLOVE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('ThirdLove');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/thirdlove/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/thirdlove/jobs/');
      ...
      expect(job0?.description).toContain('ThirdLove');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/thirdlove/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('ThirdLove');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'thirdlove-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
