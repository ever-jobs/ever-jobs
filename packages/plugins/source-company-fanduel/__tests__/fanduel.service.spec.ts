...
import { FanDuelModule, FanDuelService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'fanduel-jobs.json'), 'utf8'),
);
...
/**
 * Spec 587 / T04 — `FanDuelService` unit tests (≥ 9 cases).
 */
describe('FanDuelService — Spec 587 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through FanDuelModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [FanDuelModule],
      }).compile();
      const service = moduleRef.get(FanDuelService);
      expect(service).toBeInstanceOf(FanDuelService);
      await moduleRef.close();
    });

    it('exports the Site.FANDUEL = "fanduel" enum value', () => {
      expect(Site.FANDUEL).toBe('fanduel');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new FanDuelService();
      const result = await service.scrape({
        siteType: [Site.FANDUEL],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'fanduel-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.FANDUEL);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('FanDuel');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/fanduel/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/fanduel/jobs/');
      ...
      expect(job0?.description).toContain('FanDuel');
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('FanDuel');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'fanduel-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
