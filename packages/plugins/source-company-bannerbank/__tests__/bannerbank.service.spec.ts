...
import { BannerBankModule, BannerBankService } from '../src';

const COMPANY_NAME_EXPECT = 'Banner Bank';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'bannerbank-jobs.json'), 'utf8'),
);
...
describe('BannerBankService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through BannerBankModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [BannerBankModule],
      }).compile();
      const service = moduleRef.get(BannerBankService);
      expect(service).toBeInstanceOf(BannerBankService);
      await moduleRef.close();
    });

    it('exports the Site.BANNER_BANK = "bannerbank" enum value', () => {
      expect(Site.BANNER_BANK).toBe('bannerbank');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new BannerBankService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.BANNER_BANK],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('bannerbank');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.BANNER_BANK);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('bannerbank-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new BannerBankService(registry);
      const result = await service.scrape({
        siteType: [Site.BANNER_BANK],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('bannerbank');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('bannerbank-x1');
      expect(result.jobs[0].site).toBe(Site.BANNER_BANK);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('bannerbank-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new BannerBankService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.BANNER_BANK],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new BannerBankService();
      const result = await service.scrape({
        siteType: [Site.BANNER_BANK],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
