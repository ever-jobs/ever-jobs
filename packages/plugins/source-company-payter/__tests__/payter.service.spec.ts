...
import { PayterModule, PayterService } from '../src';

const COMPANY_NAME_EXPECT = 'Payter';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'payter-jobs.json'), 'utf8'),
);
...
describe('PayterService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through PayterModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [PayterModule],
      }).compile();
      const service = moduleRef.get(PayterService);
      expect(service).toBeInstanceOf(PayterService);
      await moduleRef.close();
    });

    it('exports the Site.PAYTER = "payter" enum value', () => {
      expect(Site.PAYTER).toBe('payter');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new PayterService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.PAYTER],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('payter.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.PAYTER);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('payter-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new PayterService(registry);
      const result = await service.scrape({
        siteType: [Site.PAYTER],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('payter');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('payter-x1');
      expect(result.jobs[0].site).toBe(Site.PAYTER);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('payter-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new PayterService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.PAYTER],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new PayterService();
      const result = await service.scrape({
        siteType: [Site.PAYTER],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
