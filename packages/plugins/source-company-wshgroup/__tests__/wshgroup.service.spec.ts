...
import { WSHGroupModule, WSHGroupService } from '../src';

const COMPANY_NAME_EXPECT = 'WSH Group';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'wshgroup-jobs.json'), 'utf8'),
);
...
describe('WSHGroupService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through WSHGroupModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [WSHGroupModule],
      }).compile();
      const service = moduleRef.get(WSHGroupService);
      expect(service).toBeInstanceOf(WSHGroupService);
      await moduleRef.close();
    });

    it('exports the Site.WSH_GROUP = "wshgroup" enum value', () => {
      expect(Site.WSH_GROUP).toBe('wshgroup');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new WSHGroupService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.WSH_GROUP],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('WSHGroup');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.WSH_GROUP);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('wshgroup-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new WSHGroupService(registry);
      const result = await service.scrape({
        siteType: [Site.WSH_GROUP],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('WSHGroup');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('wshgroup-x1');
      expect(result.jobs[0].site).toBe(Site.WSH_GROUP);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('wshgroup-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new WSHGroupService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.WSH_GROUP],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new WSHGroupService();
      const result = await service.scrape({
        siteType: [Site.WSH_GROUP],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
