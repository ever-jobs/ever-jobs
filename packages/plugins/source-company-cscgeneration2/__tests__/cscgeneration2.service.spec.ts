...
import { CSCGenerationModule, CSCGenerationService } from '../src';

const COMPANY_NAME_EXPECT = 'CSC Generation';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'cscgeneration2-jobs.json'), 'utf8'),
);
...
describe('CSCGenerationService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CSCGenerationModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CSCGenerationModule],
      }).compile();
      const service = moduleRef.get(CSCGenerationService);
      expect(service).toBeInstanceOf(CSCGenerationService);
      await moduleRef.close();
    });

    it('exports the Site.CSC_GENERATION = "cscgeneration2" enum value', () => {
      expect(Site.CSC_GENERATION).toBe('cscgeneration2');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new CSCGenerationService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.CSC_GENERATION],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'cscgeneration2-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.CSC_GENERATION);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('cscgeneration2-' + first.id);
      ...
      expect(calledUrls[0]).toContain('cscgeneration-2');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.CSC_GENERATION);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('cscgeneration2-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new CSCGenerationService(registry);
      const result = await service.scrape({
        siteType: [Site.CSC_GENERATION],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('cscgeneration-2');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('cscgeneration2-x1');
      expect(result.jobs[0].site).toBe(Site.CSC_GENERATION);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('cscgeneration2-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new CSCGenerationService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.CSC_GENERATION],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new CSCGenerationService();
      const result = await service.scrape({
        siteType: [Site.CSC_GENERATION],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
