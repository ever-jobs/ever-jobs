...
import { SpreadGroupModule, SpreadGroupService } from '../src';

const COMPANY_NAME_EXPECT = 'Spread Group';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'spreadgroup-jobs.json'), 'utf8'),
);
...
describe('SpreadGroupService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SpreadGroupModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SpreadGroupModule],
      }).compile();
      const service = moduleRef.get(SpreadGroupService);
      expect(service).toBeInstanceOf(SpreadGroupService);
      await moduleRef.close();
    });

    it('exports the Site.SPREAD_GROUP = "spreadgroup" enum value', () => {
      expect(Site.SPREAD_GROUP).toBe('spreadgroup');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new SpreadGroupService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.SPREAD_GROUP],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('spreadgroup.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SPREAD_GROUP);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('spreadgroup-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new SpreadGroupService(registry);
      const result = await service.scrape({
        siteType: [Site.SPREAD_GROUP],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('spreadgroup');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('spreadgroup-x1');
      expect(result.jobs[0].site).toBe(Site.SPREAD_GROUP);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('spreadgroup-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new SpreadGroupService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.SPREAD_GROUP],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new SpreadGroupService();
      const result = await service.scrape({
        siteType: [Site.SPREAD_GROUP],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
