...
import { GreenFluxModule, GreenFluxService } from '../src';

const COMPANY_NAME_EXPECT = 'GreenFlux';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'greenflux-jobs.json'), 'utf8'),
);
...
describe('GreenFluxService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through GreenFluxModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [GreenFluxModule],
      }).compile();
      const service = moduleRef.get(GreenFluxService);
      expect(service).toBeInstanceOf(GreenFluxService);
      await moduleRef.close();
    });

    it('exports the Site.GREENFLUX = "greenflux" enum value', () => {
      expect(Site.GREENFLUX).toBe('greenflux');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new GreenFluxService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.GREENFLUX],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('greenflux.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.GREENFLUX);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('greenflux-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new GreenFluxService(registry);
      const result = await service.scrape({
        siteType: [Site.GREENFLUX],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('greenflux');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('greenflux-x1');
      expect(result.jobs[0].site).toBe(Site.GREENFLUX);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('greenflux-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new GreenFluxService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.GREENFLUX],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new GreenFluxService();
      const result = await service.scrape({
        siteType: [Site.GREENFLUX],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
