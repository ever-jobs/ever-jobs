...
import { GlossGeniusModule, GlossGeniusService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'glossgenius-jobs.json'), 'utf8'),
);
...
/**
 * Spec 857 / T04 — `GlossGeniusService` unit tests (≥ 9 cases).
 */
describe('GlossGeniusService — Spec 857 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through GlossGeniusModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [GlossGeniusModule],
      }).compile();
      const service = moduleRef.get(GlossGeniusService);
      expect(service).toBeInstanceOf(GlossGeniusService);
      await moduleRef.close();
    });

    it('exports the Site.GLOSSGENIUS = "glossgenius" enum value', () => {
      expect(Site.GLOSSGENIUS).toBe('glossgenius');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new GlossGeniusService();
      const result = await service.scrape({
        siteType: [Site.GLOSSGENIUS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'glossgenius-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.GLOSSGENIUS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('GlossGenius');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/glossgenius/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/glossgenius/jobs/');
      ...
      expect(job0?.description).toContain('GlossGenius');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/glossgenius/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('GlossGenius');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'glossgenius-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
