...
import { AccorModule, AccorService } from '../src';

const COMPANY_NAME_EXPECT = 'Accor';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'accor-jobs.json'), 'utf8'),
);
...
describe('AccorService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AccorModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AccorModule],
      }).compile();
      const service = moduleRef.get(AccorService);
      expect(service).toBeInstanceOf(AccorService);
      await moduleRef.close();
    });

    it('exports the Site.ACCOR = "accor" enum value', () => {
      expect(Site.ACCOR).toBe('accor');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new AccorService(registryWithSmartRecruiters());
      ...
      expect(calledUrls[0]).toContain('Accor');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ACCOR);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('accor-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new AccorService(registry);
      const result = await service.scrape({
        siteType: [Site.ACCOR],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('Accor');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('accor-x1');
      expect(result.jobs[0].site).toBe(Site.ACCOR);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('accor-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new AccorService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.ACCOR],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new AccorService();
      const result = await service.scrape({
        siteType: [Site.ACCOR],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
