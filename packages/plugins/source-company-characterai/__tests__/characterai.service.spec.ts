...
import { CharacterAIModule, CharacterAIService } from '../src';

const COMPANY_NAME_EXPECT = 'Character.AI';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'characterai-jobs.json'), 'utf8'),
);
...
describe('CharacterAIService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CharacterAIModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CharacterAIModule],
      }).compile();
      const service = moduleRef.get(CharacterAIService);
      expect(service).toBeInstanceOf(CharacterAIService);
      await moduleRef.close();
    });

    it('exports the Site.CHARACTER_AI = "characterai" enum value', () => {
      expect(Site.CHARACTER_AI).toBe('characterai');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new CharacterAIService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.CHARACTER_AI],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'characterai-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.CHARACTER_AI);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('characterai-' + first.id);
      ...
      expect(calledUrls[0]).toContain('character');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.CHARACTER_AI);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('characterai-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new CharacterAIService(registry);
      const result = await service.scrape({
        siteType: [Site.CHARACTER_AI],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('character');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('characterai-x1');
      expect(result.jobs[0].site).toBe(Site.CHARACTER_AI);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('characterai-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new CharacterAIService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.CHARACTER_AI],
      } as ScraperInputDto);
