...
import { VoxMediaModule, VoxMediaService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'voxmedia-jobs.json'), 'utf8'),
);
...
/**
 * Spec 968 / T04 — `VoxMediaService` unit tests (≥ 9 cases).
 */
describe('VoxMediaService — Spec 968 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through VoxMediaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [VoxMediaModule],
      }).compile();
      const service = moduleRef.get(VoxMediaService);
      expect(service).toBeInstanceOf(VoxMediaService);
      await moduleRef.close();
    });

    it('exports the Site.VOX_MEDIA = "voxmedia" enum value', () => {
      expect(Site.VOX_MEDIA).toBe('voxmedia');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new VoxMediaService();
      const result = await service.scrape({
        siteType: [Site.VOX_MEDIA],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'voxmedia-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.VOX_MEDIA);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Vox Media');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/voxmedia/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/voxmedia/jobs/');
      ...
      expect(job0?.description).toContain('Vox Media');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/voxmedia/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Vox Media');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'voxmedia-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
