...
import { EvidentModule, EvidentService } from '../src';

const COMPANY_NAME_EXPECT = 'Evident';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'evidentid-jobs.json'), 'utf8'),
);
...
describe('EvidentService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through EvidentModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [EvidentModule],
      }).compile();
      const service = moduleRef.get(EvidentService);
      expect(service).toBeInstanceOf(EvidentService);
      await moduleRef.close();
    });

    it('exports the Site.EVIDENT = "evidentid" enum value', () => {
      expect(Site.EVIDENT).toBe('evidentid');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new EvidentService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.EVIDENT],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('evidentid');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.EVIDENT);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('evidentid-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new EvidentService(registry);
      const result = await service.scrape({
        siteType: [Site.EVIDENT],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('evidentid');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('evidentid-x1');
      expect(result.jobs[0].site).toBe(Site.EVIDENT);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('evidentid-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new EvidentService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.EVIDENT],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new EvidentService();
      const result = await service.scrape({
        siteType: [Site.EVIDENT],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
