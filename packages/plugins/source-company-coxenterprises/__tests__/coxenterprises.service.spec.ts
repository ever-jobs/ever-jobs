...
import { CoxEnterprisesModule, CoxEnterprisesService } from '../src';
...
const FIXTURE: Fixture = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'fixtures', 'coxenterprises-boards.json'), 'utf8'),
);
...
describe('CoxEnterprisesService — Workday delegation (Spec 1736)', () => {
  beforeEach(() => {
    mockGet.mockReset();
    mockPost.mockReset();
  ...
  describe('registration', () => {
    it('resolves through CoxEnterprisesModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({ imports: [CoxEnterprisesModule] }).compile();
      expect(moduleRef.get(CoxEnterprisesService)).toBeInstanceOf(CoxEnterprisesService);
      await moduleRef.close();
    });

    it('exports Site.COX_ENTERPRISES = "coxenterprises"', () => {
      expect(Site.COX_ENTERPRISES).toBe('coxenterprises');
    });

    it('declares a tagged company plugin with its domains', () => {
      const meta = Reflect.getMetadata(SOURCE_PLUGIN_METADATA, CoxEnterprisesService);
      expect(meta.site).toBe(Site.COX_ENTERPRISES);
      expect(meta.category).toBe('company');
      expect(meta.isAts).toBeFalsy();
      expect(meta.companyDomains).toEqual(['coxenterprises.com']);
  ...
  describe('recorded board (real Workday adapter, mocked HTTP)', () => {
    it('maps every recorded posting and re-stamps the company identity', async () => {
      const service = new CoxEnterprisesService(registryWith());
      const result = await service.scrape({ siteType: [Site.COX_ENTERPRISES], resultsWanted: 100 } as ScraperInputDto);
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.COX_ENTERPRISES);
    ...
    it('only requests the recorded board URLs', async () => {
      const service = new CoxEnterprisesService(registryWith());
      await service.scrape({ siteType: [Site.COX_ENTERPRISES] } as ScraperInputDto);
    ...
    it('honours resultsWanted=1', async () => {
      const service = new CoxEnterprisesService(registryWith());
      const result = await service.scrape({ siteType: [Site.COX_ENTERPRISES], resultsWanted: 1 } as ScraperInputDto);
    ...
    it('resolves with an empty result when the board is gone (HTTP 404)', async () => {
      ...
      const service = new CoxEnterprisesService(registryWith());
      const result = await service.scrape({ siteType: [Site.COX_ENTERPRISES] } as ScraperInputDto);
  ...
  describe('delegation contract', () => {
    it('forwards the board and the caller input untouched', async () => {
      ...
      const service = new CoxEnterprisesService(
        registryWith(
          fakeBackend(
      ...
      expect(result.jobs[0].site).toBe(Site.COX_ENTERPRISES);
      expect(result.jobs[0].companyName).toBe(COMPANY_NAME);
    });

    it('passes an absent resultsWanted through as absent', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new CoxEnterprisesService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      await service.scrape({ siteType: [Site.COX_ENTERPRISES] } as ScraperInputDto);
    ...
    it('makes no request when resultsWanted is 0', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new CoxEnterprisesService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      const result = await service.scrape({ siteType: [Site.COX_ENTERPRISES], resultsWanted: 0 } as ScraperInputDto);
  ...
  describe('resilience', () => {
    it('reports not_registered when no registry is injected', async () => {
      const result = await new CoxEnterprisesService().scrape({ siteType: [Site.COX_ENTERPRISES] } as ScraperInputDto);
      expect(result.jobs).toEqual([]);
    ...
    it('reports not_registered when Workday is missing from the registry', async () => {
      const result = await new CoxEnterprisesService(new PluginRegistry()).scrape({
        siteType: [Site.COX_ENTERPRISES],
      } as ScraperInputDto);
      expect(result.diagnostics?.reason).toBe('not_registered');
    });
