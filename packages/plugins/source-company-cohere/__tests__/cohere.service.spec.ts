...
import { CohereModule, CohereService } from '../src';

const COMPANY_NAME_EXPECT = 'Cohere';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'cohere-jobs.json'), 'utf8'),
);
...
describe('CohereService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CohereModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CohereModule],
      }).compile();
      const service = moduleRef.get(CohereService);
      expect(service).toBeInstanceOf(CohereService);
      await moduleRef.close();
    });

    it('exports the Site.COHERE = "cohere" enum value', () => {
      expect(Site.COHERE).toBe('cohere');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new CohereService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.COHERE],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('cohere');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.COHERE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('cohere-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new CohereService(registry);
      const result = await service.scrape({
        siteType: [Site.COHERE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('cohere');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('cohere-x1');
      expect(result.jobs[0].site).toBe(Site.COHERE);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('cohere-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new CohereService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.COHERE],
      } as ScraperInputDto);
