...
import { PathRoboticsModule, PathRoboticsService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'pathrobotics-jobs.json'), 'utf8'),
);
...
/**
 * Spec 782 / T04 — `PathRoboticsService` unit tests (≥ 9 cases).
 */
describe('PathRoboticsService — Spec 782 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through PathRoboticsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [PathRoboticsModule],
      }).compile();
      const service = moduleRef.get(PathRoboticsService);
      expect(service).toBeInstanceOf(PathRoboticsService);
      await moduleRef.close();
    });

    it('exports the Site.PATH_ROBOTICS = "pathrobotics" enum value', () => {
      expect(Site.PATH_ROBOTICS).toBe('pathrobotics');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new PathRoboticsService();
      const result = await service.scrape({
        siteType: [Site.PATH_ROBOTICS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'pathrobotics-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.PATH_ROBOTICS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Path Robotics');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/pathrobotics/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/pathrobotics/jobs/');
      ...
      expect(job0?.description).toContain('Path Robotics');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/pathrobotics/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Path Robotics');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'pathrobotics-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
