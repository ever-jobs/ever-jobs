...
import { ProphetSecurityModule, ProphetSecurityService } from '../src';

const COMPANY_NAME_EXPECT = 'Prophet Security';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'prophetsecurity-jobs.json'), 'utf8'),
);
...
describe('ProphetSecurityService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ProphetSecurityModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ProphetSecurityModule],
      }).compile();
      const service = moduleRef.get(ProphetSecurityService);
      expect(service).toBeInstanceOf(ProphetSecurityService);
      await moduleRef.close();
    });

    it('exports the Site.PROPHET_SECURITY = "prophetsecurity" enum value', () => {
      expect(Site.PROPHET_SECURITY).toBe('prophetsecurity');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new ProphetSecurityService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.PROPHET_SECURITY],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'prophetsecurity-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.PROPHET_SECURITY);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('prophetsecurity-' + first.id);
      ...
      expect(calledUrls[0]).toContain('prophet-security');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.PROPHET_SECURITY);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('prophetsecurity-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new ProphetSecurityService(registry);
      const result = await service.scrape({
        siteType: [Site.PROPHET_SECURITY],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('prophet-security');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('prophetsecurity-x1');
      expect(result.jobs[0].site).toBe(Site.PROPHET_SECURITY);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('prophetsecurity-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new ProphetSecurityService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.PROPHET_SECURITY],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new ProphetSecurityService();
      const result = await service.scrape({
        siteType: [Site.PROPHET_SECURITY],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
