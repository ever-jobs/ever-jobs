...
import { CraftzingModule, CraftzingService } from '../src';

const COMPANY_NAME_EXPECT = 'Craftzing';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'craftzing-jobs.json'), 'utf8'),
);
...
describe('CraftzingService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CraftzingModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CraftzingModule],
      }).compile();
      const service = moduleRef.get(CraftzingService);
      expect(service).toBeInstanceOf(CraftzingService);
      await moduleRef.close();
    });

    it('exports the Site.CRAFTZING = "craftzing" enum value', () => {
      expect(Site.CRAFTZING).toBe('craftzing');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new CraftzingService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.CRAFTZING],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('craftzing.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.CRAFTZING);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('craftzing-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new CraftzingService(registry);
      const result = await service.scrape({
        siteType: [Site.CRAFTZING],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('craftzing');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('craftzing-x1');
      expect(result.jobs[0].site).toBe(Site.CRAFTZING);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('craftzing-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new CraftzingService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.CRAFTZING],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new CraftzingService();
      const result = await service.scrape({
        siteType: [Site.CRAFTZING],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
