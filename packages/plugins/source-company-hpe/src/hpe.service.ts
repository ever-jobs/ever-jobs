...
/**
 * Hewlett Packard Enterprise — Enterprise IT (HQ: Houston, TX, USA).
 *
 * Source (Spec 1736): Workday board, scraped in this order:
 *   - `hpe:5:Jobsathpe` — https://hpe.wd5.myworkdayjobs.com/Jobsathpe
 *     verified live 2026-09-24: 1,292 open postings.
 *
 ...
 * through untouched.
 *
 * Tags: segment=workday-enterprise; industry=enterprise-it.
 */
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'hpe:5:Jobsathpe', atsIdPrefix: 'wd-hpe-' },
];
...
@SourcePlugin({
  ...
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['hpe.com'],
  description: 'Hewlett Packard Enterprise careers via Workday. Tags: segment=workday-enterprise; industry=enterprise-it.',
})
@Injectable()
export class HpeService implements IScraper {
  private readonly logger = new Logger(HpeService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Workday source plugin is not registered; cannot scrape Hewlett Packard Enterprise');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`Hewlett Packard Enterprise: delegating to Workday (${board.companySlug})`);
    ...
    this.logger.log(`Hewlett Packard Enterprise: scraped ${jobs.length} jobs`);
