...
import { WaymoModule, WaymoService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'waymo-jobs.json'), 'utf8'),
);
...
/**
 * Spec 444 / T04 — `WaymoService` unit tests (≥ 9 cases).
 */
describe('WaymoService — Spec 444 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through WaymoModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [WaymoModule],
      }).compile();
      const service = moduleRef.get(WaymoService);
      expect(service).toBeInstanceOf(WaymoService);
      await moduleRef.close();
    });

    it('exports the Site.WAYMO = "waymo" enum value', () => {
      expect(Site.WAYMO).toBe('waymo');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new WaymoService();
      ...
      const job0 = dto.jobs.find((j) => j.id === 'waymo-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.WAYMO);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Waymo');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/waymo/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/waymo/jobs/');
      ...
      expect(job0?.description).toContain('Waymo');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/waymo/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Waymo');
      }
    });
  });
