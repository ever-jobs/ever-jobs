...
import { Ag1Module, Ag1Service } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'ag1-jobs.json'), 'utf8'),
);
...
/**
 * Spec 204 / T04 — `Ag1Service` unit tests (≥ 9 cases).
 */
describe('Ag1Service — Spec 204 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through Ag1Module via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [Ag1Module],
      }).compile();
      const service = moduleRef.get(Ag1Service);
      expect(service).toBeInstanceOf(Ag1Service);
      await moduleRef.close();
    });

    it('exports the Site.AG1 = "ag1" enum value', () => {
      expect(Site.AG1).toBe('ag1');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const job0 = dto.jobs.find((j) => j.id === 'ag1-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.AG1);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('AG1');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/ag1/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/ag1/jobs/');
      ...
      expect(job0?.description).toContain('AG1');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/ag1/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('AG1');
      }
    });
  });
