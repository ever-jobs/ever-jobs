...
import { NayyaModule, NayyaService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'nayya-jobs.json'), 'utf8'),
);
...
/**
 * Spec 505 / T04 — `NayyaService` unit tests (≥ 9 cases).
 */
describe('NayyaService — Spec 505 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through NayyaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [NayyaModule],
      }).compile();
      const service = moduleRef.get(NayyaService);
      expect(service).toBeInstanceOf(NayyaService);
      await moduleRef.close();
    });

    it('exports the Site.NAYYA = "nayya" enum value', () => {
      expect(Site.NAYYA).toBe('nayya');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new NayyaService();
      ...
      const job0 = dto.jobs.find((j) => j.id === 'nayya-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.NAYYA);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Nayya');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/nayya/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/nayya/jobs/');
      ...
      expect(job0?.description).toContain('Nayya');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/nayya/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Nayya');
      }
    });
  });
