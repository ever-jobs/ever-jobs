...
import { TemporalTechnologiesModule, TemporalTechnologiesService } from '../src';

const COMPANY_NAME_EXPECT = 'Temporal Technologies';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'temporaltechnologies-jobs.json'), 'utf8'),
);
...
describe('TemporalTechnologiesService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through TemporalTechnologiesModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [TemporalTechnologiesModule],
      }).compile();
      const service = moduleRef.get(TemporalTechnologiesService);
      expect(service).toBeInstanceOf(TemporalTechnologiesService);
      await moduleRef.close();
    });

    it('exports the Site.TEMPORAL_TECHNOLOGIES = "temporaltechnologies" enum value', () => {
      expect(Site.TEMPORAL_TECHNOLOGIES).toBe('temporaltechnologies');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new TemporalTechnologiesService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.TEMPORAL_TECHNOLOGIES],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'temporaltechnologies-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.TEMPORAL_TECHNOLOGIES);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('temporaltechnologies-' + first.id);
      ...
      expect(calledUrls[0]).toContain('temporal');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.TEMPORAL_TECHNOLOGIES);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('temporaltechnologies-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new TemporalTechnologiesService(registry);
      const result = await service.scrape({
        siteType: [Site.TEMPORAL_TECHNOLOGIES],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('temporal');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('temporaltechnologies-x1');
      expect(result.jobs[0].site).toBe(Site.TEMPORAL_TECHNOLOGIES);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('temporaltechnologies-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new TemporalTechnologiesService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.TEMPORAL_TECHNOLOGIES],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new TemporalTechnologiesService();
      const result = await service.scrape({
        siteType: [Site.TEMPORAL_TECHNOLOGIES],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
