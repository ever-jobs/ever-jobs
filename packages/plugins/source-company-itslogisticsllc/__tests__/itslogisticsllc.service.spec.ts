...
import { ITSLogisticsModule, ITSLogisticsService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'itslogisticsllc-jobs.json'), 'utf8'),
);
...
/**
 * Spec 873 / T04 — `ITSLogisticsService` unit tests (≥ 9 cases).
 */
describe('ITSLogisticsService — Spec 873 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ITSLogisticsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ITSLogisticsModule],
      }).compile();
      const service = moduleRef.get(ITSLogisticsService);
      expect(service).toBeInstanceOf(ITSLogisticsService);
      await moduleRef.close();
    });

    it('exports the Site.ITS_LOGISTICS = "itslogisticsllc" enum value', () => {
      expect(Site.ITS_LOGISTICS).toBe('itslogisticsllc');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ITSLogisticsService();
      const result = await service.scrape({
        siteType: [Site.ITS_LOGISTICS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'itslogisticsllc-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ITS_LOGISTICS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('ITS Logistics');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/itslogisticsllc/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/itslogisticsllc/jobs/');
      ...
      expect(job0?.description).toContain('ITS Logistics');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/itslogisticsllc/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('ITS Logistics');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'itslogisticsllc-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
