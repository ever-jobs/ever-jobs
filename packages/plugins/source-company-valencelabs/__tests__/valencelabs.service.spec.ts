...
import { ValenceLabsModule, ValenceLabsService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'valencelabs-jobs.json'), 'utf8'),
);
...
/**
 * Spec 694 / T04 — `ValenceLabsService` unit tests (≥ 9 cases).
 */
describe('ValenceLabsService — Spec 694 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ValenceLabsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ValenceLabsModule],
      }).compile();
      const service = moduleRef.get(ValenceLabsService);
      expect(service).toBeInstanceOf(ValenceLabsService);
      await moduleRef.close();
    });

    it('exports the Site.VALENCE_LABS = "valencelabs" enum value', () => {
      expect(Site.VALENCE_LABS).toBe('valencelabs');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ValenceLabsService();
      const result = await service.scrape({
        siteType: [Site.VALENCE_LABS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'valencelabs-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.VALENCE_LABS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Valence Labs');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/valencelabs/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/valencelabs/jobs/');
      ...
      expect(job0?.description).toContain('Valence Labs');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/valencelabs/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Valence Labs');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'valencelabs-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
