...
import { AnalogDevicesModule, AnalogDevicesService } from '../src';
...
const FIXTURE: Fixture = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'fixtures', 'analogdevices-boards.json'), 'utf8'),
);
...
describe('AnalogDevicesService — Workday delegation (Spec 1736)', () => {
  beforeEach(() => {
    mockGet.mockReset();
    mockPost.mockReset();
  ...
  describe('registration', () => {
    it('resolves through AnalogDevicesModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({ imports: [AnalogDevicesModule] }).compile();
      expect(moduleRef.get(AnalogDevicesService)).toBeInstanceOf(AnalogDevicesService);
      await moduleRef.close();
    });

    it('exports Site.ANALOG_DEVICES = "analogdevices"', () => {
      expect(Site.ANALOG_DEVICES).toBe('analogdevices');
    });

    it('declares a tagged company plugin with its domains', () => {
      const meta = Reflect.getMetadata(SOURCE_PLUGIN_METADATA, AnalogDevicesService);
      expect(meta.site).toBe(Site.ANALOG_DEVICES);
      expect(meta.category).toBe('company');
      expect(meta.isAts).toBeFalsy();
      expect(meta.companyDomains).toEqual(['analog.com']);
  ...
  describe('recorded board (real Workday adapter, mocked HTTP)', () => {
    it('maps every recorded posting and re-stamps the company identity', async () => {
      const service = new AnalogDevicesService(registryWith());
      const result = await service.scrape({ siteType: [Site.ANALOG_DEVICES], resultsWanted: 100 } as ScraperInputDto);
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ANALOG_DEVICES);
    ...
    it('only requests the recorded board URLs', async () => {
      const service = new AnalogDevicesService(registryWith());
      await service.scrape({ siteType: [Site.ANALOG_DEVICES] } as ScraperInputDto);
    ...
    it('honours resultsWanted=1', async () => {
      const service = new AnalogDevicesService(registryWith());
      const result = await service.scrape({ siteType: [Site.ANALOG_DEVICES], resultsWanted: 1 } as ScraperInputDto);
    ...
    it('resolves with an empty result when the board is gone (HTTP 404)', async () => {
      ...
      const service = new AnalogDevicesService(registryWith());
      const result = await service.scrape({ siteType: [Site.ANALOG_DEVICES] } as ScraperInputDto);
  ...
  describe('delegation contract', () => {
    it('forwards the board and the caller input untouched', async () => {
      ...
      const service = new AnalogDevicesService(
        registryWith(
          fakeBackend(
      ...
      expect(result.jobs[0].site).toBe(Site.ANALOG_DEVICES);
    ...
    it('never forwards the caller\'s credentials to the board (Spec 1735 §4.5)', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new AnalogDevicesService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
    ...
    it('makes no request when resultsWanted is 0', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new AnalogDevicesService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      const result = await service.scrape({ siteType: [Site.ANALOG_DEVICES], resultsWanted: 0 } as ScraperInputDto);
  ...
  describe('resilience', () => {
    it('reports not_registered when no registry is injected', async () => {
      const result = await new AnalogDevicesService().scrape({ siteType: [Site.ANALOG_DEVICES] } as ScraperInputDto);
      expect(result.jobs).toEqual([]);
    ...
    it('reports not_registered when Workday is missing from the registry', async () => {
      const result = await new AnalogDevicesService(new PluginRegistry()).scrape({
        siteType: [Site.ANALOG_DEVICES],
      } as ScraperInputDto);
      expect(result.diagnostics?.reason).toBe('not_registered');
    });
