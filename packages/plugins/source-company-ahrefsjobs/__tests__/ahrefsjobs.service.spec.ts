...
import { AhrefsjobsModule, AhrefsjobsService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'ahrefsjobs-jobs.json'), 'utf8'),
);
...
/**
 * Spec 212 / T04 — `AhrefsjobsService` unit tests (≥ 9 cases).
 */
describe('AhrefsjobsService — Spec 212 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AhrefsjobsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AhrefsjobsModule],
      }).compile();
      const service = moduleRef.get(AhrefsjobsService);
      expect(service).toBeInstanceOf(AhrefsjobsService);
      await moduleRef.close();
    });

    it('exports the Site.AHREFSJOBS = "ahrefsjobs" enum value', () => {
      expect(Site.AHREFSJOBS).toBe('ahrefsjobs');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AhrefsjobsService();
      const result = await service.scrape({
        siteType: [Site.AHREFSJOBS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'ahrefsjobs-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.AHREFSJOBS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Ahrefs');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/ahrefsjobs/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/ahrefsjobs/jobs/');
      ...
      expect(job0?.description).toContain('Ahrefs');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/ahrefsjobs/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Ahrefs');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'ahrefsjobs-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
