...
import { HighLevelModule, HighLevelService } from '../src';

const COMPANY_NAME_EXPECT = 'HighLevel';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'gohighlevel-jobs.json'), 'utf8'),
);
...
describe('HighLevelService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through HighLevelModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [HighLevelModule],
      }).compile();
      const service = moduleRef.get(HighLevelService);
      expect(service).toBeInstanceOf(HighLevelService);
      await moduleRef.close();
    });

    it('exports the Site.HIGHLEVEL = "gohighlevel" enum value', () => {
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new HighLevelService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.HIGHLEVEL],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('gohighlevel');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.HIGHLEVEL);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('gohighlevel-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new HighLevelService(registry);
      const result = await service.scrape({
        siteType: [Site.HIGHLEVEL],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('gohighlevel');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('gohighlevel-x1');
      expect(result.jobs[0].site).toBe(Site.HIGHLEVEL);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('gohighlevel-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new HighLevelService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.HIGHLEVEL],
      } as ScraperInputDto);
