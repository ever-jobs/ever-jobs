...
import { SnapModule, SnapService } from '../src';
...
const FIXTURE: Fixture = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'fixtures', 'snap-boards.json'), 'utf8'),
);
...
describe('SnapService — Workday delegation (Spec 1736)', () => {
  beforeEach(() => {
    mockGet.mockReset();
    mockPost.mockReset();
  ...
  describe('registration', () => {
    it('resolves through SnapModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({ imports: [SnapModule] }).compile();
      expect(moduleRef.get(SnapService)).toBeInstanceOf(SnapService);
      await moduleRef.close();
    });

    it('exports Site.SNAP = "snap"', () => {
      expect(Site.SNAP).toBe('snap');
    });

    it('declares a tagged company plugin with its domains', () => {
      const meta = Reflect.getMetadata(SOURCE_PLUGIN_METADATA, SnapService);
      expect(meta.site).toBe(Site.SNAP);
      expect(meta.category).toBe('company');
      expect(meta.isAts).toBeFalsy();
      expect(meta.companyDomains).toEqual(['snap.com']);
  ...
  describe('recorded board (real Workday adapter, mocked HTTP)', () => {
    it('maps every recorded posting and re-stamps the company identity', async () => {
      const service = new SnapService(registryWith());
      const result = await service.scrape({ siteType: [Site.SNAP], resultsWanted: 100 } as ScraperInputDto);
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SNAP);
    ...
    it('only requests the recorded board URLs', async () => {
      const service = new SnapService(registryWith());
      await service.scrape({ siteType: [Site.SNAP] } as ScraperInputDto);
    ...
    it('honours resultsWanted=1', async () => {
      const service = new SnapService(registryWith());
      const result = await service.scrape({ siteType: [Site.SNAP], resultsWanted: 1 } as ScraperInputDto);
    ...
    it('resolves with an empty result when the board is gone (HTTP 404)', async () => {
      ...
      const service = new SnapService(registryWith());
      const result = await service.scrape({ siteType: [Site.SNAP] } as ScraperInputDto);
  ...
  describe('delegation contract', () => {
    it('forwards the board and the caller input untouched', async () => {
      ...
      expect(result.jobs[0].site).toBe(Site.SNAP);
    ...
    it('never forwards the caller\'s credentials to the board (Spec 1735 §4.5)', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new SnapService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
    ...
    it('makes no request when resultsWanted is 0', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new SnapService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      const result = await service.scrape({ siteType: [Site.SNAP], resultsWanted: 0 } as ScraperInputDto);
  ...
  describe('resilience', () => {
    it('reports not_registered when no registry is injected', async () => {
      const result = await new SnapService().scrape({ siteType: [Site.SNAP] } as ScraperInputDto);
      expect(result.jobs).toEqual([]);
    ...
    it('reports not_registered when Workday is missing from the registry', async () => {
      const result = await new SnapService(new PluginRegistry()).scrape({
        siteType: [Site.SNAP],
      } as ScraperInputDto);
      expect(result.diagnostics?.reason).toBe('not_registered');
    });
