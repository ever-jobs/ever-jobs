...
import { SymmetryFinancialGroupModule, SymmetryFinancialGroupService } from '../src';

const COMPANY_NAME_EXPECT = 'Symmetry Financial Group';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'symmetryfinancialgroup-jobs.json'), 'utf8'),
);
...
describe('SymmetryFinancialGroupService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SymmetryFinancialGroupModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SymmetryFinancialGroupModule],
      }).compile();
      const service = moduleRef.get(SymmetryFinancialGroupService);
      expect(service).toBeInstanceOf(SymmetryFinancialGroupService);
      await moduleRef.close();
    });

    it('exports the Site.SYMMETRY_FINANCIAL_GROUP = "symmetryfinancialgroup" enum value', () => {
      expect(Site.SYMMETRY_FINANCIAL_GROUP).toBe('symmetryfinancialgroup');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new SymmetryFinancialGroupService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.SYMMETRY_FINANCIAL_GROUP],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'symmetryfinancialgroup-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.SYMMETRY_FINANCIAL_GROUP);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('symmetryfinancialgroup-' + first.id);
      ...
      expect(calledUrls[0]).toContain('SymmetryFinancialGroup7');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SYMMETRY_FINANCIAL_GROUP);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('symmetryfinancialgroup-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new SymmetryFinancialGroupService(registry);
      const result = await service.scrape({
        siteType: [Site.SYMMETRY_FINANCIAL_GROUP],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('SymmetryFinancialGroup7');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('symmetryfinancialgroup-x1');
      expect(result.jobs[0].site).toBe(Site.SYMMETRY_FINANCIAL_GROUP);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('symmetryfinancialgroup-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new SymmetryFinancialGroupService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.SYMMETRY_FINANCIAL_GROUP],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new SymmetryFinancialGroupService();
      const result = await service.scrape({
        siteType: [Site.SYMMETRY_FINANCIAL_GROUP],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
