...
import { MonMarchFrModule, MonMarchFrService } from '../src';

const COMPANY_NAME_EXPECT = 'Mon-marché.fr';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'monmarchfr-jobs.json'), 'utf8'),
);
...
describe('MonMarchFrService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through MonMarchFrModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [MonMarchFrModule],
      }).compile();
      const service = moduleRef.get(MonMarchFrService);
      expect(service).toBeInstanceOf(MonMarchFrService);
      await moduleRef.close();
    });

    it('exports the Site.MON_MARCH_FR = "monmarchfr" enum value', () => {
      expect(Site.MON_MARCH_FR).toBe('monmarchfr');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new MonMarchFrService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.MON_MARCH_FR],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('monmarchefr.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.MON_MARCH_FR);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('monmarchfr-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new MonMarchFrService(registry);
      const result = await service.scrape({
        siteType: [Site.MON_MARCH_FR],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('monmarchefr');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('monmarchfr-x1');
      expect(result.jobs[0].site).toBe(Site.MON_MARCH_FR);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('monmarchfr-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new MonMarchFrService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.MON_MARCH_FR],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new MonMarchFrService();
      const result = await service.scrape({
        siteType: [Site.MON_MARCH_FR],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
