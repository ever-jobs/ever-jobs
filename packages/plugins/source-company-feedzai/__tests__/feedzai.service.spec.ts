...
import { FeedzaiModule, FeedzaiService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'feedzai-jobs.json'), 'utf8'),
);
...
/**
 * Spec 847 / T04 — `FeedzaiService` unit tests (≥ 9 cases).
 */
describe('FeedzaiService — Spec 847 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through FeedzaiModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [FeedzaiModule],
      }).compile();
      const service = moduleRef.get(FeedzaiService);
      expect(service).toBeInstanceOf(FeedzaiService);
      await moduleRef.close();
    });

    it('exports the Site.FEEDZAI = "feedzai" enum value', () => {
      expect(Site.FEEDZAI).toBe('feedzai');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new FeedzaiService();
      const result = await service.scrape({
        siteType: [Site.FEEDZAI],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'feedzai-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.FEEDZAI);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Feedzai');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/feedzai/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/feedzai/jobs/');
      ...
      expect(job0?.description).toContain('Feedzai');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/feedzai/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Feedzai');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'feedzai-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
