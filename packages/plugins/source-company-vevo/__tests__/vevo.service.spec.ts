...
import { VevoModule, VevoService } from '../src';

const COMPANY_NAME_EXPECT = 'Vevo';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'vevo-jobs.json'), 'utf8'),
);
...
describe('VevoService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through VevoModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [VevoModule],
      }).compile();
      const service = moduleRef.get(VevoService);
      expect(service).toBeInstanceOf(VevoService);
      await moduleRef.close();
    });

    it('exports the Site.VEVO = "vevo" enum value', () => {
      expect(Site.VEVO).toBe('vevo');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new VevoService(registryWithLever());
      ...
      expect(calledUrls[0]).toContain('vevo');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.VEVO);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('vevo-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new VevoService(registry);
      const result = await service.scrape({
        siteType: [Site.VEVO],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('vevo');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('vevo-x1');
      expect(result.jobs[0].site).toBe(Site.VEVO);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('vevo-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new VevoService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.VEVO],
      } as ScraperInputDto);
