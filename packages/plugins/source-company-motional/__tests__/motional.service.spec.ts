...
import { MotionalModule, MotionalService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'motional-jobs.json'), 'utf8'),
);
...
/**
 * Spec 748 / T04 — `MotionalService` unit tests (≥ 9 cases).
 */
describe('MotionalService — Spec 748 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through MotionalModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [MotionalModule],
      }).compile();
      const service = moduleRef.get(MotionalService);
      expect(service).toBeInstanceOf(MotionalService);
      await moduleRef.close();
    });

    it('exports the Site.MOTIONAL = "motional" enum value', () => {
      expect(Site.MOTIONAL).toBe('motional');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new MotionalService();
      const result = await service.scrape({
        siteType: [Site.MOTIONAL],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'motional-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.MOTIONAL);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Motional');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/motional/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/motional/jobs/');
      ...
      expect(job0?.description).toContain('Motional');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/motional/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Motional');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'motional-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
