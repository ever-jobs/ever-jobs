...
import { RemotecomModule, RemotecomService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'remotecom-jobs.json'), 'utf8'),
);
...
/**
 * Spec 445 / T04 — `RemotecomService` unit tests (≥ 9 cases).
 */
describe('RemotecomService — Spec 445 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through RemotecomModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [RemotecomModule],
      }).compile();
      const service = moduleRef.get(RemotecomService);
      expect(service).toBeInstanceOf(RemotecomService);
      await moduleRef.close();
    });

    it('exports the Site.REMOTECOM = "remotecom" enum value', () => {
      expect(Site.REMOTECOM).toBe('remotecom');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new RemotecomService();
      const result = await service.scrape({
        siteType: [Site.REMOTECOM],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'remotecom-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.REMOTECOM);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Remote');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/remotecom/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/remotecom/jobs/');
      ...
      expect(job0?.description).toContain('Remote');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/remotecom/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Remote');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'remotecom-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
