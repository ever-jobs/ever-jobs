...
import { ThunesModule, ThunesService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'thunes-jobs.json'), 'utf8'),
);
...
/**
 * Spec 491 / T04 — `ThunesService` unit tests (≥ 9 cases).
 */
describe('ThunesService — Spec 491 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ThunesModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ThunesModule],
      }).compile();
      const service = moduleRef.get(ThunesService);
      expect(service).toBeInstanceOf(ThunesService);
      await moduleRef.close();
    });

    it('exports the Site.THUNES = "thunes" enum value', () => {
      expect(Site.THUNES).toBe('thunes');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ThunesService();
      const result = await service.scrape({
        siteType: [Site.THUNES],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'thunes-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.THUNES);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Thunes');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/thunes/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/thunes/jobs/');
      ...
      expect(job0?.description).toContain('Thunes');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/thunes/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Thunes');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'thunes-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
