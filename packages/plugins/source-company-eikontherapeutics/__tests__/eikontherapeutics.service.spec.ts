...
import { EikonTherapeuticsModule, EikonTherapeuticsService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'eikontherapeutics-jobs.json'), 'utf8'),
);
...
/**
 * Spec 777 / T04 — `EikonTherapeuticsService` unit tests (≥ 9 cases).
 */
describe('EikonTherapeuticsService — Spec 777 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through EikonTherapeuticsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [EikonTherapeuticsModule],
      }).compile();
      const service = moduleRef.get(EikonTherapeuticsService);
      expect(service).toBeInstanceOf(EikonTherapeuticsService);
      await moduleRef.close();
    });

    it('exports the Site.EIKON_THERAPEUTICS = "eikontherapeutics" enum value', () => {
      expect(Site.EIKON_THERAPEUTICS).toBe('eikontherapeutics');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new EikonTherapeuticsService();
      const result = await service.scrape({
        siteType: [Site.EIKON_THERAPEUTICS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'eikontherapeutics-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.EIKON_THERAPEUTICS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Eikon Therapeutics');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/eikontherapeutics/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/eikontherapeutics/jobs/');
      ...
      expect(job0?.description).toContain('Eikon Therapeutics');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/eikontherapeutics/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Eikon Therapeutics');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'eikontherapeutics-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
