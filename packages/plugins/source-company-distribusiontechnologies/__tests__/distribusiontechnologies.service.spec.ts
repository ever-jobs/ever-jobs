...
import { DistribusionTechnologiesModule, DistribusionTechnologiesService } from '../src';

const COMPANY_NAME_EXPECT = 'Distribusion Technologies';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'distribusiontechnologies-jobs.json'), 'utf8'),
);
...
describe('DistribusionTechnologiesService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through DistribusionTechnologiesModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [DistribusionTechnologiesModule],
      }).compile();
      const service = moduleRef.get(DistribusionTechnologiesService);
      expect(service).toBeInstanceOf(DistribusionTechnologiesService);
      await moduleRef.close();
    });

    it('exports the Site.DISTRIBUSION_TECHNOLOGIES = "distribusiontechnologies" enum value', () => {
      expect(Site.DISTRIBUSION_TECHNOLOGIES).toBe('distribusiontechnologies');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new DistribusionTechnologiesService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.DISTRIBUSION_TECHNOLOGIES],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'distribusiontechnologies-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Recruitee's defaults
      expect(job0?.site).toBe(Site.DISTRIBUSION_TECHNOLOGIES);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('distribusiontechnologies-' + first.id);
      ...
      expect(calledUrls[0]).toContain('distribusion.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.DISTRIBUSION_TECHNOLOGIES);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('distribusiontechnologies-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new DistribusionTechnologiesService(registry);
      const result = await service.scrape({
        siteType: [Site.DISTRIBUSION_TECHNOLOGIES],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('distribusion');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('distribusiontechnologies-x1');
      expect(result.jobs[0].site).toBe(Site.DISTRIBUSION_TECHNOLOGIES);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('distribusiontechnologies-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new DistribusionTechnologiesService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.DISTRIBUSION_TECHNOLOGIES],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new DistribusionTechnologiesService();
      const result = await service.scrape({
        siteType: [Site.DISTRIBUSION_TECHNOLOGIES],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
