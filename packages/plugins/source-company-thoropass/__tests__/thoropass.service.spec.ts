...
import { ThoropassModule, ThoropassService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'thoropass-jobs.json'), 'utf8'),
);
...
/**
 * Spec 539 / T04 — `ThoropassService` unit tests (≥ 9 cases).
 */
describe('ThoropassService — Spec 539 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ThoropassModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ThoropassModule],
      }).compile();
      const service = moduleRef.get(ThoropassService);
      expect(service).toBeInstanceOf(ThoropassService);
      await moduleRef.close();
    });

    it('exports the Site.THOROPASS = "thoropass" enum value', () => {
      expect(Site.THOROPASS).toBe('thoropass');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ThoropassService();
      const result = await service.scrape({
        siteType: [Site.THOROPASS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'thoropass-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.THOROPASS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Thoropass');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/thoropass/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/thoropass/jobs/');
      ...
      expect(job0?.description).toContain('Thoropass');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/thoropass/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Thoropass');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'thoropass-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
