...
import { BombasModule, BombasService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'bombas-jobs.json'), 'utf8'),
);
...
/**
 * Spec 614 / T04 — `BombasService` unit tests (≥ 9 cases).
 */
describe('BombasService — Spec 614 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through BombasModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [BombasModule],
      }).compile();
      const service = moduleRef.get(BombasService);
      expect(service).toBeInstanceOf(BombasService);
      await moduleRef.close();
    });

    it('exports the Site.BOMBAS = "bombas" enum value', () => {
      expect(Site.BOMBAS).toBe('bombas');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new BombasService();
      const result = await service.scrape({
        siteType: [Site.BOMBAS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'bombas-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.BOMBAS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Bombas');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/bombas/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/bombas/jobs/');
      ...
      expect(job0?.description).toContain('Bombas');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/bombas/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Bombas');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'bombas-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
