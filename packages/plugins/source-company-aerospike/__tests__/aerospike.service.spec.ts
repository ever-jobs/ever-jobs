...
import { AerospikeModule, AerospikeService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'aerospike-jobs.json'), 'utf8'),
);
...
/**
 * Spec 198 / T04 — `AerospikeService` unit tests (≥ 9 cases).
 */
describe('AerospikeService — Spec 198 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AerospikeModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AerospikeModule],
      }).compile();
      const service = moduleRef.get(AerospikeService);
      expect(service).toBeInstanceOf(AerospikeService);
      await moduleRef.close();
    });

    it('exports the Site.AEROSPIKE = "aerospike" enum value', () => {
      expect(Site.AEROSPIKE).toBe('aerospike');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new AerospikeService();
      const result = await service.scrape({
        siteType: [Site.AEROSPIKE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'aerospike-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.AEROSPIKE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Aerospike');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/aerospike/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/aerospike/jobs/');
      ...
      expect(job0?.description).toContain('Aerospike');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/aerospike/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Aerospike');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'aerospike-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
