...
@SourcePlugin({
  site: Site.ICRUNCHDATA,
  name: 'iCrunchData',
  category: 'niche',
})
@Injectable()
export class IcrunchdataService implements IScraper {
  private readonly logger = new Logger(IcrunchdataService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    client.setHeaders(ICRUNCHDATA_HEADERS);

    this.logger.log(
      `Fetching iCrunchData RSS feed (resultsWanted=${resultsWanted})`,
    );

    try {
      const response = await client.get(ICRUNCHDATA_RSS_URL);
      const xml = response.data;

      if (!xml || typeof xml !== 'string') {
        this.logger.warn('Empty or invalid RSS response from iCrunchData');
        return new JobResponseDto([]);
      }

      const items = this.parseRssItems(xml);
      this.logger.log(`Parsed ${items.length} items from iCrunchData RSS feed`);
      ...
      for (const item of items) {
        ...
        try {
          ...
          this.logger.warn(
            `Error mapping iCrunchData job ${item.link}: ${err.message}`,
          );
        }
      }

      this.logger.log(`iCrunchData returned ${jobs.length} jobs`);
      return new JobResponseDto(jobs);
    } catch (err: any) {
      this.logger.error(`iCrunchData scrape error: ${err.message}`);
      return new JobResponseDto([], classifyScrapeError(err));
    }
  }

  /**
   * Parse RSS XML into an array of IcrunchdataRssItem using regex (no XML library).
   */
  private parseRssItems(xml: string): IcrunchdataRssItem[] {
    const items: IcrunchdataRssItem[] = [];
   ...
   */
  private matchesSearch(item: IcrunchdataRssItem, searchTerm: string): boolean {
   ...
   */
  private mapJob(
    item: IcrunchdataRssItem,
    descriptionFormat?: DescriptionFormat,
  ): JobPostDto | null {
    if (!item.title || !item.link) return null;

    // Process description (iCrunchData RSS may return HTML)
    ...
    // Company from dc:creator
    const companyName = item.dcCreator ?? undefined;

    // Generate ID from GUID or URL
    const jobId = this.extractIdFromUrl(item.guid ?? item.link);

    return new JobPostDto({
      id: `icrunchdata-${jobId}`,
      title: item.title,
      jobUrl: item.link,
      description,
      companyName,
