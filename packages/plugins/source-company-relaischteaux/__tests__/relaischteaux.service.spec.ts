...
import { RelaisChTeauxModule, RelaisChTeauxService } from '../src';

const COMPANY_NAME_EXPECT = 'Relais & Châteaux';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'relaischteaux-jobs.json'), 'utf8'),
);
...
describe('RelaisChTeauxService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through RelaisChTeauxModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [RelaisChTeauxModule],
      }).compile();
      const service = moduleRef.get(RelaisChTeauxService);
      expect(service).toBeInstanceOf(RelaisChTeauxService);
      await moduleRef.close();
    });

    it('exports the Site.RELAIS_CH_TEAUX = "relaischteaux" enum value', () => {
      expect(Site.RELAIS_CH_TEAUX).toBe('relaischteaux');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new RelaisChTeauxService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.RELAIS_CH_TEAUX],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'relaischteaux-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.RELAIS_CH_TEAUX);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('relaischteaux-' + first.id);
      ...
      expect(calledUrls[0]).toContain('RelaisChateaux');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.RELAIS_CH_TEAUX);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('relaischteaux-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new RelaisChTeauxService(registry);
      const result = await service.scrape({
        siteType: [Site.RELAIS_CH_TEAUX],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('RelaisChateaux');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('relaischteaux-x1');
      expect(result.jobs[0].site).toBe(Site.RELAIS_CH_TEAUX);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('relaischteaux-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new RelaisChTeauxService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.RELAIS_CH_TEAUX],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new RelaisChTeauxService();
      const result = await service.scrape({
        siteType: [Site.RELAIS_CH_TEAUX],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
