...
import { GalvanizeClimateSolutionsModule, GalvanizeClimateSolutionsService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'galvanizeclimatesolutions-jobs.json'), 'utf8'),
);
...
/**
 * Spec 854 / T04 — `GalvanizeClimateSolutionsService` unit tests (≥ 9 cases).
 */
describe('GalvanizeClimateSolutionsService — Spec 854 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through GalvanizeClimateSolutionsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [GalvanizeClimateSolutionsModule],
      }).compile();
      const service = moduleRef.get(GalvanizeClimateSolutionsService);
      expect(service).toBeInstanceOf(GalvanizeClimateSolutionsService);
      await moduleRef.close();
    });

    it('exports the Site.GALVANIZE_CLIMATE_SOLUTIONS = "galvanizeclimatesolutions" enum value', () => {
      expect(Site.GALVANIZE_CLIMATE_SOLUTIONS).toBe('galvanizeclimatesolutions');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new GalvanizeClimateSolutionsService();
      const result = await service.scrape({
        siteType: [Site.GALVANIZE_CLIMATE_SOLUTIONS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'galvanizeclimatesolutions-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.GALVANIZE_CLIMATE_SOLUTIONS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Galvanize Climate Solutions');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/galvanizeclimatesolutions/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/galvanizeclimatesolutions/jobs/');
      ...
      expect(job0?.description).toContain('Galvanize Climate Solutions');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/galvanizeclimatesolutions/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Galvanize Climate Solutions');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'galvanizeclimatesolutions-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
