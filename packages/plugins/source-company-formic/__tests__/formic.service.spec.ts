...
import { FormicModule, FormicService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'formic-jobs.json'), 'utf8'),
);
...
/**
 * Spec 779 / T04 — `FormicService` unit tests (≥ 9 cases).
 */
describe('FormicService — Spec 779 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through FormicModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [FormicModule],
      }).compile();
      const service = moduleRef.get(FormicService);
      expect(service).toBeInstanceOf(FormicService);
      await moduleRef.close();
    });

    it('exports the Site.FORMIC = "formic" enum value', () => {
      expect(Site.FORMIC).toBe('formic');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new FormicService();
      const result = await service.scrape({
        siteType: [Site.FORMIC],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'formic-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.FORMIC);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Formic');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/formic/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/formic/jobs/');
      ...
      expect(job0?.description).toContain('Formic');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/formic/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Formic');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'formic-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
