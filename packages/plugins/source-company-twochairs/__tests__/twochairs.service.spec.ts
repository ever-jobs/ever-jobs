...
import { TwoChairsModule, TwoChairsService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'twochairs-jobs.json'), 'utf8'),
);
...
/**
 * Spec 665 / T04 — `TwoChairsService` unit tests (≥ 9 cases).
 */
describe('TwoChairsService — Spec 665 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through TwoChairsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [TwoChairsModule],
      }).compile();
      const service = moduleRef.get(TwoChairsService);
      expect(service).toBeInstanceOf(TwoChairsService);
      await moduleRef.close();
    });

    it('exports the Site.TWO_CHAIRS = "twochairs" enum value', () => {
      expect(Site.TWO_CHAIRS).toBe('twochairs');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new TwoChairsService();
      const result = await service.scrape({
        siteType: [Site.TWO_CHAIRS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'twochairs-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.TWO_CHAIRS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Two Chairs');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/twochairs/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/twochairs/jobs/');
      ...
      expect(job0?.description).toContain('Two Chairs');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/twochairs/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Two Chairs');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'twochairs-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
