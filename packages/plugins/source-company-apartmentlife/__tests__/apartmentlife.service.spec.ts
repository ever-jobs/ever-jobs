...
import { ApartmentlifeModule, ApartmentlifeService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'apartmentlife-jobs.json'), 'utf8'),
);
...
/**
 * Spec 278 / T04 — `ApartmentlifeService` unit tests (≥ 9 cases).
 */
describe('ApartmentlifeService — Spec 278 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ApartmentlifeModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ApartmentlifeModule],
      }).compile();
      const service = moduleRef.get(ApartmentlifeService);
      expect(service).toBeInstanceOf(ApartmentlifeService);
      await moduleRef.close();
    });

    it('exports the Site.APARTMENTLIFE = "apartmentlife" enum value', () => {
      expect(Site.APARTMENTLIFE).toBe('apartmentlife');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ApartmentlifeService();
      const result = await service.scrape({
        siteType: [Site.APARTMENTLIFE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'apartmentlife-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.APARTMENTLIFE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Apartment Life');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/apartmentlife/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/apartmentlife/jobs/');
      ...
      expect(job0?.description).toContain('Apartment Life');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/apartmentlife/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Apartment Life');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'apartmentlife-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
