...
import { KongIncModule, KongIncService } from '../src';

const COMPANY_NAME_EXPECT = 'Kong Inc.';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'konginc-jobs.json'), 'utf8'),
);
...
describe('KongIncService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through KongIncModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [KongIncModule],
      }).compile();
      const service = moduleRef.get(KongIncService);
      expect(service).toBeInstanceOf(KongIncService);
      await moduleRef.close();
    });

    it('exports the Site.KONG_INC = "konginc" enum value', () => {
      expect(Site.KONG_INC).toBe('konginc');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new KongIncService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.KONG_INC],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('kong');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.KONG_INC);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('konginc-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new KongIncService(registry);
      const result = await service.scrape({
        siteType: [Site.KONG_INC],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('kong');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('konginc-x1');
      expect(result.jobs[0].site).toBe(Site.KONG_INC);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('konginc-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new KongIncService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.KONG_INC],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new KongIncService();
      const result = await service.scrape({
        siteType: [Site.KONG_INC],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
