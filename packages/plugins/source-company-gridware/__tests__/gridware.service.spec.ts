...
import { GridwareModule, GridwareService } from '../src';

const COMPANY_NAME_EXPECT = 'Gridware';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'gridware-jobs.json'), 'utf8'),
);
...
describe('GridwareService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through GridwareModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [GridwareModule],
      }).compile();
      const service = moduleRef.get(GridwareService);
      expect(service).toBeInstanceOf(GridwareService);
      await moduleRef.close();
    });

    it('exports the Site.GRIDWARE = "gridware" enum value', () => {
      expect(Site.GRIDWARE).toBe('gridware');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new GridwareService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.GRIDWARE],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('gridware');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.GRIDWARE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('gridware-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new GridwareService(registry);
      const result = await service.scrape({
        siteType: [Site.GRIDWARE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('gridware');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('gridware-x1');
      expect(result.jobs[0].site).toBe(Site.GRIDWARE);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('gridware-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new GridwareService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.GRIDWARE],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new GridwareService();
      const result = await service.scrape({
        siteType: [Site.GRIDWARE],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
