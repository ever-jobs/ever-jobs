...
import { KodiakModule, KodiakService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'kodiak-jobs.json'), 'utf8'),
);
...
/**
 * Spec 607 / T04 — `KodiakService` unit tests (≥ 9 cases).
 */
describe('KodiakService — Spec 607 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through KodiakModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [KodiakModule],
      }).compile();
      const service = moduleRef.get(KodiakService);
      expect(service).toBeInstanceOf(KodiakService);
      await moduleRef.close();
    });

    it('exports the Site.KODIAK_ROBOTICS = "kodiak" enum value', () => {
      expect(Site.KODIAK_ROBOTICS).toBe('kodiak');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new KodiakService();
      const result = await service.scrape({
        siteType: [Site.KODIAK_ROBOTICS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'kodiak-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.KODIAK_ROBOTICS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Kodiak Robotics');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/kodiak/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/kodiak/jobs/');
      ...
      expect(job0?.description).toContain('Kodiak Robotics');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/kodiak/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Kodiak Robotics');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'kodiak-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
