...
import { GditModule, GditService } from '../src';
...
const FIXTURE: Fixture = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'fixtures', 'gdit-boards.json'), 'utf8'),
);
const COMPANY_NAME = 'General Dynamics Information Technology';
const ID_PREFIX = 'gdit-';
...
describe('GditService — Workday delegation (Spec 1736)', () => {
  beforeEach(() => {
    mockGet.mockReset();
    mockPost.mockReset();
  ...
  describe('registration', () => {
    it('resolves through GditModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({ imports: [GditModule] }).compile();
      expect(moduleRef.get(GditService)).toBeInstanceOf(GditService);
      await moduleRef.close();
    });

    it('exports Site.GDIT = "gdit"', () => {
      expect(Site.GDIT).toBe('gdit');
    });

    it('declares a tagged company plugin with its domains', () => {
      const meta = Reflect.getMetadata(SOURCE_PLUGIN_METADATA, GditService);
      expect(meta.site).toBe(Site.GDIT);
      expect(meta.category).toBe('company');
      expect(meta.isAts).toBeFalsy();
      expect(meta.companyDomains).toEqual(['gdit.com']);
  ...
  describe('recorded board (real Workday adapter, mocked HTTP)', () => {
    it('maps every recorded posting and re-stamps the company identity', async () => {
      const service = new GditService(registryWith());
      const result = await service.scrape({ siteType: [Site.GDIT], resultsWanted: 100 } as ScraperInputDto);
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.GDIT);
    ...
    it('only requests the recorded board URLs', async () => {
      const service = new GditService(registryWith());
      await service.scrape({ siteType: [Site.GDIT] } as ScraperInputDto);
    ...
    it('honours resultsWanted=1', async () => {
      const service = new GditService(registryWith());
      const result = await service.scrape({ siteType: [Site.GDIT], resultsWanted: 1 } as ScraperInputDto);
    ...
    it('resolves with an empty result when the board is gone (HTTP 404)', async () => {
      ...
      const service = new GditService(registryWith());
      const result = await service.scrape({ siteType: [Site.GDIT] } as ScraperInputDto);
  ...
  describe('delegation contract', () => {
    it('forwards the board and the caller input untouched', async () => {
      ...
      expect(result.jobs[0].site).toBe(Site.GDIT);
    ...
    it('never forwards the caller\'s credentials to the board (Spec 1735 §4.5)', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new GditService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
    ...
    it('makes no request when resultsWanted is 0', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new GditService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      const result = await service.scrape({ siteType: [Site.GDIT], resultsWanted: 0 } as ScraperInputDto);
  ...
  describe('resilience', () => {
    it('reports not_registered when no registry is injected', async () => {
      const result = await new GditService().scrape({ siteType: [Site.GDIT] } as ScraperInputDto);
      expect(result.jobs).toEqual([]);
    ...
    it('reports not_registered when Workday is missing from the registry', async () => {
      const result = await new GditService(new PluginRegistry()).scrape({
        siteType: [Site.GDIT],
      } as ScraperInputDto);
      expect(result.diagnostics?.reason).toBe('not_registered');
    });
