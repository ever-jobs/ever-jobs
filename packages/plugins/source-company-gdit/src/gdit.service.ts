...
/**
 * General Dynamics Information Technology — Government IT (HQ: Falls Church, VA, USA).
 *
 * Source (Spec 1736): Workday board, scraped in this order:
 *   - `gdit:5:External_Career_Site` — https://gdit.wd5.myworkdayjobs.com/External_Career_Site
 *     verified live 2026-09-24: 1,181 open postings.
 *
 ...
 * through untouched.
 *
 * Tags: segment=workday-enterprise; industry=government-it.
 */
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'gdit:5:External_Career_Site', atsIdPrefix: 'wd-gdit-' },
];
...
@SourcePlugin({
  ...
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['gdit.com'],
  description: 'General Dynamics Information Technology careers via Workday. Tags: segment=workday-enterprise; industry=government-it.',
})
@Injectable()
export class GditService implements IScraper {
  private readonly logger = new Logger(GditService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Workday source plugin is not registered; cannot scrape General Dynamics Information Technology');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`General Dynamics Information Technology: delegating to Workday (${board.companySlug})`);
    ...
    this.logger.log(`General Dynamics Information Technology: scraped ${jobs.length} jobs`);
