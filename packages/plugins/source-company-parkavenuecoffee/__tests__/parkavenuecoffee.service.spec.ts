...
import { ParkAvenueCoffeeModule, ParkAvenueCoffeeService } from '../src';

const COMPANY_NAME_EXPECT = 'Park Avenue Coffee';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'parkavenuecoffee-jobs.json'), 'utf8'),
);
...
describe('ParkAvenueCoffeeService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ParkAvenueCoffeeModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ParkAvenueCoffeeModule],
      }).compile();
      const service = moduleRef.get(ParkAvenueCoffeeService);
      expect(service).toBeInstanceOf(ParkAvenueCoffeeService);
      await moduleRef.close();
    });

    it('exports the Site.PARK_AVENUE_COFFEE = "parkavenuecoffee" enum value', () => {
      expect(Site.PARK_AVENUE_COFFEE).toBe('parkavenuecoffee');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new ParkAvenueCoffeeService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.PARK_AVENUE_COFFEE],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'parkavenuecoffee-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.PARK_AVENUE_COFFEE);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('parkavenuecoffee-' + first.id);
      ...
      expect(calledUrls[0]).toContain('ParkAvenueCoffee');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.PARK_AVENUE_COFFEE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('parkavenuecoffee-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new ParkAvenueCoffeeService(registry);
      const result = await service.scrape({
        siteType: [Site.PARK_AVENUE_COFFEE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('ParkAvenueCoffee');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('parkavenuecoffee-x1');
      expect(result.jobs[0].site).toBe(Site.PARK_AVENUE_COFFEE);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('parkavenuecoffee-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new ParkAvenueCoffeeService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.PARK_AVENUE_COFFEE],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new ParkAvenueCoffeeService();
      const result = await service.scrape({
        siteType: [Site.PARK_AVENUE_COFFEE],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
