...
import { CookunityModule, CookunityService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'cookunity-jobs.json'), 'utf8'),
);
...
/**
 * Spec 487 / T04 — `CookunityService` unit tests (≥ 9 cases).
 */
describe('CookunityService — Spec 487 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CookunityModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CookunityModule],
      }).compile();
      const service = moduleRef.get(CookunityService);
      expect(service).toBeInstanceOf(CookunityService);
      await moduleRef.close();
    });

    it('exports the Site.COOKUNITY = "cookunity" enum value', () => {
      expect(Site.COOKUNITY).toBe('cookunity');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new CookunityService();
      const result = await service.scrape({
        siteType: [Site.COOKUNITY],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'cookunity-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.COOKUNITY);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('CookUnity');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/cookunity/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/cookunity/jobs/');
      ...
      expect(job0?.description).toContain('CookUnity');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/cookunity/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('CookUnity');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'cookunity-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
