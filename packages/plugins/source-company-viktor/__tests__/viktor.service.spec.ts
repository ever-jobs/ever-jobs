...
import { VIKTORModule, VIKTORService } from '../src';

const COMPANY_NAME_EXPECT = 'VIKTOR';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'viktor-jobs.json'), 'utf8'),
);
...
describe('VIKTORService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through VIKTORModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [VIKTORModule],
      }).compile();
      const service = moduleRef.get(VIKTORService);
      expect(service).toBeInstanceOf(VIKTORService);
      await moduleRef.close();
    });

    it('exports the Site.VIKTOR = "viktor" enum value', () => {
      expect(Site.VIKTOR).toBe('viktor');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new VIKTORService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.VIKTOR],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('viktor.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.VIKTOR);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('viktor-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new VIKTORService(registry);
      const result = await service.scrape({
        siteType: [Site.VIKTOR],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('viktor');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('viktor-x1');
      expect(result.jobs[0].site).toBe(Site.VIKTOR);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('viktor-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new VIKTORService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.VIKTOR],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new VIKTORService();
      const result = await service.scrape({
        siteType: [Site.VIKTOR],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
