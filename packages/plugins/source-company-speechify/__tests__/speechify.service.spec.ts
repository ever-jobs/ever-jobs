...
import { SpeechifyModule, SpeechifyService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'speechify-jobs.json'), 'utf8'),
);
...
/**
 * Spec 938 / T04 — `SpeechifyService` unit tests (≥ 9 cases).
 */
describe('SpeechifyService — Spec 938 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SpeechifyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SpeechifyModule],
      }).compile();
      const service = moduleRef.get(SpeechifyService);
      expect(service).toBeInstanceOf(SpeechifyService);
      await moduleRef.close();
    });

    it('exports the Site.SPEECHIFY = "speechify" enum value', () => {
      expect(Site.SPEECHIFY).toBe('speechify');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const job0 = dto.jobs.find((j) => j.id === 'speechify-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.SPEECHIFY);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Speechify');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/speechify/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/speechify/jobs/');
      ...
      expect(job0?.description).toContain('Speechify');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/speechify/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Speechify');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'speechify-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
