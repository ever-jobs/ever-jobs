...
import { TEGNAModule, TEGNAService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'tegnainc-jobs.json'), 'utf8'),
);
...
/**
 * Spec 949 / T04 — `TEGNAService` unit tests (≥ 9 cases).
 */
describe('TEGNAService — Spec 949 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through TEGNAModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [TEGNAModule],
      }).compile();
      const service = moduleRef.get(TEGNAService);
      expect(service).toBeInstanceOf(TEGNAService);
      await moduleRef.close();
    });

    it('exports the Site.TEGNA = "tegnainc" enum value', () => {
      expect(Site.TEGNA).toBe('tegnainc');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new TEGNAService();
      ...
      const job0 = dto.jobs.find((j) => j.id === 'tegnainc-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.TEGNA);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('TEGNA');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/tegnainc/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/tegnainc/jobs/');
      ...
      expect(job0?.description).toContain('TEGNA');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/tegnainc/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('TEGNA');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'tegnainc-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
