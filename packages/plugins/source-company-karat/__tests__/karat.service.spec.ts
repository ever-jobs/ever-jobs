...
import { KaratModule, KaratService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'karat-jobs.json'), 'utf8'),
);
...
/**
 * Spec 604 / T04 — `KaratService` unit tests (≥ 9 cases).
 */
describe('KaratService — Spec 604 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through KaratModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [KaratModule],
      }).compile();
      const service = moduleRef.get(KaratService);
      expect(service).toBeInstanceOf(KaratService);
      await moduleRef.close();
    });

    it('exports the Site.KARAT_INTERVIEWS = "karat" enum value', () => {
      expect(Site.KARAT_INTERVIEWS).toBe('karat');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new KaratService();
      const result = await service.scrape({
        siteType: [Site.KARAT_INTERVIEWS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'karat-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.KARAT_INTERVIEWS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Karat');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/karat/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/karat/jobs/');
      ...
      expect(job0?.description).toContain('Karat');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/karat/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Karat');
      }
    });
  });
