...
import { DexterraGroupModule, DexterraGroupService } from '../src';

const COMPANY_NAME_EXPECT = 'Dexterra Group';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'dexterragroup-jobs.json'), 'utf8'),
);
...
describe('DexterraGroupService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through DexterraGroupModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [DexterraGroupModule],
      }).compile();
      const service = moduleRef.get(DexterraGroupService);
      expect(service).toBeInstanceOf(DexterraGroupService);
      await moduleRef.close();
    });

    it('exports the Site.DEXTERRA_GROUP = "dexterragroup" enum value', () => {
      expect(Site.DEXTERRA_GROUP).toBe('dexterragroup');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new DexterraGroupService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.DEXTERRA_GROUP],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'dexterragroup-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.DEXTERRA_GROUP);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('dexterragroup-' + first.id);
      ...
      expect(calledUrls[0]).toContain('Dexterra');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.DEXTERRA_GROUP);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('dexterragroup-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new DexterraGroupService(registry);
      const result = await service.scrape({
        siteType: [Site.DEXTERRA_GROUP],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('Dexterra');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('dexterragroup-x1');
      expect(result.jobs[0].site).toBe(Site.DEXTERRA_GROUP);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('dexterragroup-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new DexterraGroupService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.DEXTERRA_GROUP],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new DexterraGroupService();
      const result = await service.scrape({
        siteType: [Site.DEXTERRA_GROUP],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
