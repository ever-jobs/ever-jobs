...
import { AxleEnergyModule, AxleEnergyService } from '../src';

const COMPANY_NAME_EXPECT = 'Axle Energy';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'axleenergy-jobs.json'), 'utf8'),
);
...
describe('AxleEnergyService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AxleEnergyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AxleEnergyModule],
      }).compile();
      const service = moduleRef.get(AxleEnergyService);
      expect(service).toBeInstanceOf(AxleEnergyService);
      await moduleRef.close();
    });

    it('exports the Site.AXLE_ENERGY = "axleenergy" enum value', () => {
      expect(Site.AXLE_ENERGY).toBe('axleenergy');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new AxleEnergyService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.AXLE_ENERGY],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('axle-careers');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.AXLE_ENERGY);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('axleenergy-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new AxleEnergyService(registry);
      const result = await service.scrape({
        siteType: [Site.AXLE_ENERGY],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('axle-careers');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('axleenergy-x1');
      expect(result.jobs[0].site).toBe(Site.AXLE_ENERGY);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('axleenergy-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new AxleEnergyService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.AXLE_ENERGY],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new AxleEnergyService();
      const result = await service.scrape({
        siteType: [Site.AXLE_ENERGY],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
