...
import { CamusEnergyModule, CamusEnergyService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'camusenergy-jobs.json'), 'utf8'),
);
...
/**
 * Spec 682 / T04 — `CamusEnergyService` unit tests (≥ 9 cases).
 */
describe('CamusEnergyService — Spec 682 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through CamusEnergyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [CamusEnergyModule],
      }).compile();
      const service = moduleRef.get(CamusEnergyService);
      expect(service).toBeInstanceOf(CamusEnergyService);
      await moduleRef.close();
    });

    it('exports the Site.CAMUS_ENERGY = "camusenergy" enum value', () => {
      expect(Site.CAMUS_ENERGY).toBe('camusenergy');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new CamusEnergyService();
      const result = await service.scrape({
        siteType: [Site.CAMUS_ENERGY],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'camusenergy-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.CAMUS_ENERGY);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Camus Energy');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/camusenergy/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/camusenergy/jobs/');
      ...
      expect(job0?.description).toContain('Camus Energy');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/camusenergy/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Camus Energy');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'camusenergy-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
