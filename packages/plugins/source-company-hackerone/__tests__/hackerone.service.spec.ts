...
import { HackerOneModule, HackerOneService } from '../src';

const COMPANY_NAME_EXPECT = 'HackerOne';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'hackerone-jobs.json'), 'utf8'),
);
...
describe('HackerOneService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through HackerOneModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [HackerOneModule],
      }).compile();
      const service = moduleRef.get(HackerOneService);
      expect(service).toBeInstanceOf(HackerOneService);
      await moduleRef.close();
    });

    it('exports the Site.HACKERONE = "hackerone" enum value', () => {
      expect(Site.HACKERONE).toBe('hackerone');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new HackerOneService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.HACKERONE],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('hackerone');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.HACKERONE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('hackerone-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new HackerOneService(registry);
      const result = await service.scrape({
        siteType: [Site.HACKERONE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('hackerone');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('hackerone-x1');
      expect(result.jobs[0].site).toBe(Site.HACKERONE);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('hackerone-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new HackerOneService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.HACKERONE],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new HackerOneService();
      const result = await service.scrape({
        siteType: [Site.HACKERONE],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
