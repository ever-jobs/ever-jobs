...
import { UncommonSchoolsModule, UncommonSchoolsService } from '../src';

const COMPANY_NAME_EXPECT = 'Uncommon Schools';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'uncommonschools-jobs.json'), 'utf8'),
);
...
describe('UncommonSchoolsService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through UncommonSchoolsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [UncommonSchoolsModule],
      }).compile();
      const service = moduleRef.get(UncommonSchoolsService);
      expect(service).toBeInstanceOf(UncommonSchoolsService);
      await moduleRef.close();
    });

    it('exports the Site.UNCOMMON_SCHOOLS = "uncommonschools" enum value', () => {
      expect(Site.UNCOMMON_SCHOOLS).toBe('uncommonschools');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new UncommonSchoolsService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.UNCOMMON_SCHOOLS],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'uncommonschools-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.UNCOMMON_SCHOOLS);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('uncommonschools-' + first.id);
      ...
      expect(calledUrls[0]).toContain('UncommonSchools');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.UNCOMMON_SCHOOLS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('uncommonschools-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new UncommonSchoolsService(registry);
      const result = await service.scrape({
        siteType: [Site.UNCOMMON_SCHOOLS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('UncommonSchools');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('uncommonschools-x1');
      expect(result.jobs[0].site).toBe(Site.UNCOMMON_SCHOOLS);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('uncommonschools-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new UncommonSchoolsService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.UNCOMMON_SCHOOLS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new UncommonSchoolsService();
      const result = await service.scrape({
        siteType: [Site.UNCOMMON_SCHOOLS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
