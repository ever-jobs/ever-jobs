...
import { VuoriModule, VuoriService } from '../src';

const COMPANY_NAME_EXPECT = 'Vuori';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'vuori-jobs.json'), 'utf8'),
);
...
describe('VuoriService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through VuoriModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [VuoriModule],
      }).compile();
      const service = moduleRef.get(VuoriService);
      expect(service).toBeInstanceOf(VuoriService);
      await moduleRef.close();
    });

    it('exports the Site.VUORI = "vuori" enum value', () => {
      expect(Site.VUORI).toBe('vuori');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new VuoriService(registryWithSmartRecruiters());
      ...
      expect(calledUrls[0]).toContain('VuoriInc');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.VUORI);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('vuori-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new VuoriService(registry);
      const result = await service.scrape({
        siteType: [Site.VUORI],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('VuoriInc');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('vuori-x1');
      expect(result.jobs[0].site).toBe(Site.VUORI);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('vuori-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new VuoriService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.VUORI],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new VuoriService();
      const result = await service.scrape({
        siteType: [Site.VUORI],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
