...
import { EnergageModule, EnergageService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'energage-jobs.json'), 'utf8'),
);
...
/**
 * Spec 537 / T04 — `EnergageService` unit tests (≥ 9 cases).
 */
describe('EnergageService — Spec 537 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through EnergageModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [EnergageModule],
      }).compile();
      const service = moduleRef.get(EnergageService);
      expect(service).toBeInstanceOf(EnergageService);
      await moduleRef.close();
    });

    it('exports the Site.ENERGAGE = "energage" enum value', () => {
      expect(Site.ENERGAGE).toBe('energage');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new EnergageService();
      const result = await service.scrape({
        siteType: [Site.ENERGAGE],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'energage-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ENERGAGE);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Energage');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/energage/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/energage/jobs/');
      ...
      expect(job0?.description).toContain('Energage');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/energage/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Energage');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'energage-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
