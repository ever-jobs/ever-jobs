...
import { WorldQuantModule, WorldQuantService } from '../src';
...
const FIXTURE: Fixture = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'fixtures', 'worldquant-boards.json'), 'utf8'),
);
const COMPANY_NAME = 'WorldQuant';
const ID_PREFIX = 'worldquant-';
...
describe('WorldQuantService — Greenhouse delegation (Spec 1737)', () => {
  beforeEach(() => {
    mockGet.mockReset();
    mockPost.mockReset();
  ...
  describe('registration', () => {
    it('resolves through WorldQuantModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({ imports: [WorldQuantModule] }).compile();
      expect(moduleRef.get(WorldQuantService)).toBeInstanceOf(WorldQuantService);
      await moduleRef.close();
    });

    it('exports Site.WORLDQUANT = "worldquant"', () => {
      expect(Site.WORLDQUANT).toBe('worldquant');
    });

    it('declares a tagged company plugin with its domains', () => {
      const meta = Reflect.getMetadata(SOURCE_PLUGIN_METADATA, WorldQuantService);
      expect(meta.site).toBe(Site.WORLDQUANT);
      expect(meta.category).toBe('company');
      expect(meta.isAts).toBeFalsy();
      expect(meta.companyDomains).toEqual(['worldquant.com']);
  ...
  describe('recorded board (real Greenhouse adapter, mocked HTTP)', () => {
    it('maps every recorded posting and re-stamps the company identity', async () => {
      const service = new WorldQuantService(registryWith());
      const result = await service.scrape({ siteType: [Site.WORLDQUANT], resultsWanted: 100 } as ScraperInputDto);
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.WORLDQUANT);
    ...
    it('only requests the recorded board URLs', async () => {
      const service = new WorldQuantService(registryWith());
      await service.scrape({ siteType: [Site.WORLDQUANT] } as ScraperInputDto);
    ...
    it('honours resultsWanted=1', async () => {
      const service = new WorldQuantService(registryWith());
      const result = await service.scrape({ siteType: [Site.WORLDQUANT], resultsWanted: 1 } as ScraperInputDto);
    ...
    it('resolves with an empty result when the board is gone (HTTP 404)', async () => {
      ...
      const service = new WorldQuantService(registryWith());
      const result = await service.scrape({ siteType: [Site.WORLDQUANT] } as ScraperInputDto);
  ...
  describe('delegation contract', () => {
    it('forwards the board and the caller input untouched', async () => {
      ...
      const service = new WorldQuantService(
        registryWith(
          fakeBackend(
      ...
      expect(result.jobs[0].site).toBe(Site.WORLDQUANT);
      expect(result.jobs[0].companyName).toBe(COMPANY_NAME);
    });

    it('passes an absent resultsWanted through as absent', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new WorldQuantService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      await service.scrape({ siteType: [Site.WORLDQUANT] } as ScraperInputDto);
    ...
    it('makes no request when resultsWanted is 0', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new WorldQuantService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      const result = await service.scrape({ siteType: [Site.WORLDQUANT], resultsWanted: 0 } as ScraperInputDto);
  ...
  describe('resilience', () => {
    it('reports not_registered when no registry is injected', async () => {
      const result = await new WorldQuantService().scrape({ siteType: [Site.WORLDQUANT] } as ScraperInputDto);
      expect(result.jobs).toEqual([]);
    ...
    it('reports not_registered when Greenhouse is missing from the registry', async () => {
      const result = await new WorldQuantService(new PluginRegistry()).scrape({
        siteType: [Site.WORLDQUANT],
      } as ScraperInputDto);
      expect(result.diagnostics?.reason).toBe('not_registered');
    });
