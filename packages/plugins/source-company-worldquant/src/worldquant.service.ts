...
/**
 * WorldQuant — Quantitative investment management (HQ: Old Greenwich, CT, USA).
 *
 * Source (Spec 1737): Greenhouse board, scraped in this order:
 *   - `worldquant` — https://job-boards.greenhouse.io/worldquant
 *     verified live 2026-09-24: 102 open postings.
 *
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'worldquant', atsIdPrefix: 'gh-' },
];

@SourcePlugin({
  site: Site.WORLDQUANT,
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['worldquant.com'],
  description: 'WorldQuant careers via Greenhouse. Tags: segment=quant-trading; industry=quantitative-investment-management.',
})
@Injectable()
export class WorldQuantService implements IScraper {
  private readonly logger = new Logger(WorldQuantService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Greenhouse source plugin is not registered; cannot scrape WorldQuant');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`WorldQuant: delegating to Greenhouse (${board.companySlug})`);
    ...
    this.logger.log(`WorldQuant: scraped ${jobs.length} jobs`);
