...
import { FactorialEnergyModule, FactorialEnergyService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'factorialenergy-jobs.json'), 'utf8'),
);
...
/**
 * Spec 671 / T04 — `FactorialEnergyService` unit tests (≥ 9 cases).
 */
describe('FactorialEnergyService — Spec 671 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through FactorialEnergyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [FactorialEnergyModule],
      }).compile();
      const service = moduleRef.get(FactorialEnergyService);
      expect(service).toBeInstanceOf(FactorialEnergyService);
      await moduleRef.close();
    });

    it('exports the Site.FACTORIAL_ENERGY = "factorialenergy" enum value', () => {
      expect(Site.FACTORIAL_ENERGY).toBe('factorialenergy');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new FactorialEnergyService();
      const result = await service.scrape({
        siteType: [Site.FACTORIAL_ENERGY],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'factorialenergy-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.FACTORIAL_ENERGY);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Factorial Energy');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/factorialenergy/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/factorialenergy/jobs/');
      ...
      expect(job0?.description).toContain('Factorial Energy');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/factorialenergy/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Factorial Energy');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'factorialenergy-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
