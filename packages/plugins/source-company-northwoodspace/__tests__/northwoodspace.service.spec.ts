...
import { NorthwoodSpaceModule, NorthwoodSpaceService } from '../src';

const COMPANY_NAME_EXPECT = 'Northwood Space';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'northwoodspace-jobs.json'), 'utf8'),
);
...
describe('NorthwoodSpaceService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through NorthwoodSpaceModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [NorthwoodSpaceModule],
      }).compile();
      const service = moduleRef.get(NorthwoodSpaceService);
      expect(service).toBeInstanceOf(NorthwoodSpaceService);
      await moduleRef.close();
    });

    it('exports the Site.NORTHWOOD_SPACE = "northwoodspace" enum value', () => {
      expect(Site.NORTHWOOD_SPACE).toBe('northwoodspace');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new NorthwoodSpaceService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.NORTHWOOD_SPACE],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'northwoodspace-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.NORTHWOOD_SPACE);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('northwoodspace-' + first.id);
      ...
      expect(calledUrls[0]).toContain('northwoodspace');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.NORTHWOOD_SPACE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('northwoodspace-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new NorthwoodSpaceService(registry);
      const result = await service.scrape({
        siteType: [Site.NORTHWOOD_SPACE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('northwoodspace');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('northwoodspace-x1');
      expect(result.jobs[0].site).toBe(Site.NORTHWOOD_SPACE);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('northwoodspace-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new NorthwoodSpaceService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.NORTHWOOD_SPACE],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new NorthwoodSpaceService();
      const result = await service.scrape({
        siteType: [Site.NORTHWOOD_SPACE],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
