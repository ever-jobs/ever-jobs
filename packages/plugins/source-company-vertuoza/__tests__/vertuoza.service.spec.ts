...
import { VertuozaModule, VertuozaService } from '../src';

const COMPANY_NAME_EXPECT = 'Vertuoza';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Recruitee's public careers API returns an { offers: [...] } envelope.
const JOBS_ENVELOPE_RAW: { offers: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'vertuoza-jobs.json'), 'utf8'),
);
...
describe('VertuozaService — Recruitee delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through VertuozaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [VertuozaModule],
      }).compile();
      const service = moduleRef.get(VertuozaService);
      expect(service).toBeInstanceOf(VertuozaService);
      await moduleRef.close();
    });

    it('exports the Site.VERTUOZA = "vertuoza" enum value', () => {
      expect(Site.VERTUOZA).toBe('vertuoza');
    });
  });
  ...
  describe('happy path (delegates to the registered Recruitee plugin)', () => {
    it('maps all fixture offers to JobPostDto with the company identity', async () => {
      ...
      const service = new VertuozaService(registryWithRecruitee());
      const result = (await service.scrape({
        siteType: [Site.VERTUOZA],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('vertuoza.recruitee.com');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.VERTUOZA);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('vertuoza-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Recruitee scraper', async () => {
      ...
      const service = new VertuozaService(registry);
      const result = await service.scrape({
        siteType: [Site.VERTUOZA],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('vertuoza');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('vertuoza-x1');
      expect(result.jobs[0].site).toBe(Site.VERTUOZA);
    });
    ...
    it('only rewrites a leading recruitee- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('vertuoza-recruitee-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Recruitee plugin is registered', async () => {
      const service = new VertuozaService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.VERTUOZA],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new VertuozaService();
      const result = await service.scrape({
        siteType: [Site.VERTUOZA],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
