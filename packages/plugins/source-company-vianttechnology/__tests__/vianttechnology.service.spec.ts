...
import { ViantTechnologyModule, ViantTechnologyService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'vianttechnology-jobs.json'), 'utf8'),
);
...
/**
 * Spec 966 / T04 — `ViantTechnologyService` unit tests (≥ 9 cases).
 */
describe('ViantTechnologyService — Spec 966 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ViantTechnologyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ViantTechnologyModule],
      }).compile();
      const service = moduleRef.get(ViantTechnologyService);
      expect(service).toBeInstanceOf(ViantTechnologyService);
      await moduleRef.close();
    });

    it('exports the Site.VIANT_TECHNOLOGY = "vianttechnology" enum value', () => {
      expect(Site.VIANT_TECHNOLOGY).toBe('vianttechnology');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ViantTechnologyService();
      const result = await service.scrape({
        siteType: [Site.VIANT_TECHNOLOGY],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'vianttechnology-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.VIANT_TECHNOLOGY);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Viant Technology');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/vianttechnology/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/vianttechnology/jobs/');
      ...
      expect(job0?.description).toContain('Viant Technology');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/vianttechnology/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Viant Technology');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'vianttechnology-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
