...
/**
 * Optiver — Market making (HQ: Amsterdam, Netherlands).
 *
 * Source (Spec 1737): Greenhouse board, scraped in this order:
 *   - `optiverus` — https://job-boards.greenhouse.io/optiverus
 *     verified live 2026-09-24: 165 open postings.
 *
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'optiverus', atsIdPrefix: 'gh-' },
];
...
@SourcePlugin({
  ...
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['optiver.com'],
  description: 'Optiver careers via Greenhouse. Tags: segment=quant-trading; industry=market-making.',
})
@Injectable()
export class OptiverService implements IScraper {
  private readonly logger = new Logger(OptiverService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Greenhouse source plugin is not registered; cannot scrape Optiver');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`Optiver: delegating to Greenhouse (${board.companySlug})`);
    ...
    this.logger.log(`Optiver: scraped ${jobs.length} jobs`);
