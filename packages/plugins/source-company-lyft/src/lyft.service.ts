...
/**
 * Lyft publishes its careers board through Greenhouse at the bare
 * `lyft` slug (no asymmetry; see Spec 029 § 10 D-05).
 */
const API_URL = 'https://api.greenhouse.io/v1/boards/lyft/jobs';
...
export class LyftService implements IScraper {
  private readonly logger = new Logger(LyftService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    try {
      ...
      this.logger.log(`Lyft: fetching ${url}`);
        ...
        jobs.push(
          new JobPostDto({
            ...
            jobUrl:
              ...
              `https://www.lyft.com/careers/${listing.id}`,
            location,
      ...
      this.logger.log(`Lyft: scraped ${jobs.length} jobs`);
    } catch (err: any) {
      this.logger.error(`Lyft scrape failed: ${err.message}`);
