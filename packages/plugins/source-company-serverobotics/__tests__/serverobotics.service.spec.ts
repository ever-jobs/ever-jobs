...
import { ServeRoboticsModule, ServeRoboticsService } from '../src';

const COMPANY_NAME_EXPECT = 'Serve Robotics';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'serverobotics-jobs.json'), 'utf8'),
);
...
describe('ServeRoboticsService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ServeRoboticsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ServeRoboticsModule],
      }).compile();
      const service = moduleRef.get(ServeRoboticsService);
      expect(service).toBeInstanceOf(ServeRoboticsService);
      await moduleRef.close();
    });

    it('exports the Site.SERVE_ROBOTICS = "serverobotics" enum value', () => {
      expect(Site.SERVE_ROBOTICS).toBe('serverobotics');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new ServeRoboticsService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.SERVE_ROBOTICS],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'serverobotics-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.SERVE_ROBOTICS);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('serverobotics-' + first.id);
      ...
      expect(calledUrls[0]).toContain('serverobotics');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SERVE_ROBOTICS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('serverobotics-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new ServeRoboticsService(registry);
      const result = await service.scrape({
        siteType: [Site.SERVE_ROBOTICS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('serverobotics');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('serverobotics-x1');
      expect(result.jobs[0].site).toBe(Site.SERVE_ROBOTICS);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('serverobotics-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new ServeRoboticsService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.SERVE_ROBOTICS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new ServeRoboticsService();
      const result = await service.scrape({
        siteType: [Site.SERVE_ROBOTICS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
