...
import { ZyngaModule, ZyngaService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'zyngacareers-jobs.json'), 'utf8'),
);
...
/**
 * Spec 974 / T04 — `ZyngaService` unit tests (≥ 9 cases).
 */
describe('ZyngaService — Spec 974 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ZyngaModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ZyngaModule],
      }).compile();
      const service = moduleRef.get(ZyngaService);
      expect(service).toBeInstanceOf(ZyngaService);
      await moduleRef.close();
    });

    it('exports the Site.ZYNGA = "zyngacareers" enum value', () => {
      expect(Site.ZYNGA).toBe('zyngacareers');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ZyngaService();
      ...
      const job0 = dto.jobs.find((j) => j.id === 'zyngacareers-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ZYNGA);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Zynga');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/zyngacareers/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/zyngacareers/jobs/');
      ...
      expect(job0?.description).toContain('Zynga');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/zyngacareers/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Zynga');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'zyngacareers-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
