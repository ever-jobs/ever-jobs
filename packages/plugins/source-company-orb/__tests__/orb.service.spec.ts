...
import { OrbModule, OrbService } from '../src';

const COMPANY_NAME_EXPECT = 'Orb';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'orb-jobs.json'), 'utf8'),
);
...
describe('OrbService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through OrbModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [OrbModule],
      }).compile();
      const service = moduleRef.get(OrbService);
      expect(service).toBeInstanceOf(OrbService);
      await moduleRef.close();
    });

    it('exports the Site.ORB = "orb" enum value', () => {
      expect(Site.ORB).toBe('orb');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new OrbService(registryWithAshby());
      ...
      expect(job0?.site).toBe(Site.ORB);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('orb-' + first.id);
      ...
      expect(calledUrls[0]).toContain('orb');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ORB);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('orb-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new OrbService(registry);
      const result = await service.scrape({
        siteType: [Site.ORB],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('orb');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('orb-x1');
      expect(result.jobs[0].site).toBe(Site.ORB);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('orb-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new OrbService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.ORB],
      } as ScraperInputDto);
