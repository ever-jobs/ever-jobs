...
import { PilotCompanyModule, PilotCompanyService } from '../src';

const COMPANY_NAME_EXPECT = 'Pilot Company';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'pilotcompany-jobs.json'), 'utf8'),
);
...
describe('PilotCompanyService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through PilotCompanyModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [PilotCompanyModule],
      }).compile();
      const service = moduleRef.get(PilotCompanyService);
      expect(service).toBeInstanceOf(PilotCompanyService);
      await moduleRef.close();
    });

    it('exports the Site.PILOT_COMPANY = "pilotcompany" enum value', () => {
      expect(Site.PILOT_COMPANY).toBe('pilotcompany');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new PilotCompanyService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.PILOT_COMPANY],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('PilotCompany');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.PILOT_COMPANY);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('pilotcompany-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new PilotCompanyService(registry);
      const result = await service.scrape({
        siteType: [Site.PILOT_COMPANY],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('PilotCompany');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('pilotcompany-x1');
      expect(result.jobs[0].site).toBe(Site.PILOT_COMPANY);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('pilotcompany-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new PilotCompanyService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.PILOT_COMPANY],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new PilotCompanyService();
      const result = await service.scrape({
        siteType: [Site.PILOT_COMPANY],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
