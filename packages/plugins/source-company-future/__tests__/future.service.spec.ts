...
import { FutureFitnessModule, FutureFitnessService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'future-jobs.json'), 'utf8'),
);
...
/**
 * Spec 589 / T04 — `FutureFitnessService` unit tests (≥ 9 cases).
 */
describe('FutureFitnessService — Spec 589 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through FutureFitnessModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [FutureFitnessModule],
      }).compile();
      const service = moduleRef.get(FutureFitnessService);
      expect(service).toBeInstanceOf(FutureFitnessService);
      await moduleRef.close();
    });

    it('exports the Site.FUTURE_FITNESS = "future" enum value', () => {
      expect(Site.FUTURE_FITNESS).toBe('future');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new FutureFitnessService();
      const result = await service.scrape({
        siteType: [Site.FUTURE_FITNESS],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'future-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.FUTURE_FITNESS);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Future');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/future/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/future/jobs/');
      ...
      expect(job0?.description).toContain('Future');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/future/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Future');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'future-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
