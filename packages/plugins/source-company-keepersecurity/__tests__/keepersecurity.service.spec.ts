...
import { KeeperSecurityModule, KeeperSecurityService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'keepersecurity-jobs.json'), 'utf8'),
);
...
/**
 * Spec 697 / T04 — `KeeperSecurityService` unit tests (≥ 9 cases).
 */
describe('KeeperSecurityService — Spec 697 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through KeeperSecurityModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [KeeperSecurityModule],
      }).compile();
      const service = moduleRef.get(KeeperSecurityService);
      expect(service).toBeInstanceOf(KeeperSecurityService);
      await moduleRef.close();
    });

    it('exports the Site.KEEPER_SECURITY = "keepersecurity" enum value', () => {
      expect(Site.KEEPER_SECURITY).toBe('keepersecurity');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new KeeperSecurityService();
      const result = await service.scrape({
        siteType: [Site.KEEPER_SECURITY],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'keepersecurity-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.KEEPER_SECURITY);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Keeper Security');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/keepersecurity/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/keepersecurity/jobs/');
      ...
      expect(job0?.description).toContain('Keeper Security');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/keepersecurity/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Keeper Security');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'keepersecurity-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
