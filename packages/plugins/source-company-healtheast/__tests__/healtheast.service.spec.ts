...
import { HealthEastModule, HealthEastService } from '../src';

const COMPANY_NAME_EXPECT = 'HealthEast';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'healtheast-jobs.json'), 'utf8'),
);
...
describe('HealthEastService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through HealthEastModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [HealthEastModule],
      }).compile();
      const service = moduleRef.get(HealthEastService);
      expect(service).toBeInstanceOf(HealthEastService);
      await moduleRef.close();
    });

    it('exports the Site.HEALTHEAST = "healtheast" enum value', () => {
      expect(Site.HEALTHEAST).toBe('healtheast');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new HealthEastService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.HEALTHEAST],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('HealthEastCareSystem');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.HEALTHEAST);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('healtheast-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new HealthEastService(registry);
      const result = await service.scrape({
        siteType: [Site.HEALTHEAST],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('HealthEastCareSystem');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('healtheast-x1');
      expect(result.jobs[0].site).toBe(Site.HEALTHEAST);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('healtheast-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new HealthEastService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.HEALTHEAST],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new HealthEastService();
      const result = await service.scrape({
        siteType: [Site.HEALTHEAST],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
