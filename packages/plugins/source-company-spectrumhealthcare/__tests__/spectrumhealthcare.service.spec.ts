...
import { SpectrumHealthCareModule, SpectrumHealthCareService } from '../src';

const COMPANY_NAME_EXPECT = 'Spectrum Health Care';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'spectrumhealthcare-jobs.json'), 'utf8'),
);
...
describe('SpectrumHealthCareService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SpectrumHealthCareModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SpectrumHealthCareModule],
      }).compile();
      const service = moduleRef.get(SpectrumHealthCareService);
      expect(service).toBeInstanceOf(SpectrumHealthCareService);
      await moduleRef.close();
    });

    it('exports the Site.SPECTRUM_HEALTH_CARE = "spectrumhealthcare" enum value', () => {
      expect(Site.SPECTRUM_HEALTH_CARE).toBe('spectrumhealthcare');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new SpectrumHealthCareService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.SPECTRUM_HEALTH_CARE],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'spectrumhealthcare-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over SmartRecruiters' defaults
      expect(job0?.site).toBe(Site.SPECTRUM_HEALTH_CARE);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('spectrumhealthcare-' + first.id);
      ...
      expect(calledUrls[0]).toContain('SpectrumHealthCare');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SPECTRUM_HEALTH_CARE);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('spectrumhealthcare-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new SpectrumHealthCareService(registry);
      const result = await service.scrape({
        siteType: [Site.SPECTRUM_HEALTH_CARE],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('SpectrumHealthCare');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('spectrumhealthcare-x1');
      expect(result.jobs[0].site).toBe(Site.SPECTRUM_HEALTH_CARE);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('spectrumhealthcare-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new SpectrumHealthCareService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.SPECTRUM_HEALTH_CARE],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new SpectrumHealthCareService();
      const result = await service.scrape({
        siteType: [Site.SPECTRUM_HEALTH_CARE],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
