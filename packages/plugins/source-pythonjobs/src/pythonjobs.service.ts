...
import { PYTHONJOBS_RSS_URL, PYTHONJOBS_HEADERS } from './pythonjobs.constants';
import { PythonJobsRssItem } from './pythonjobs.types';

@SourcePlugin({
  site: Site.PYTHONJOBS,
  name: 'PythonJobs',
  category: 'niche',
})
@Injectable()
export class PythonJobsService implements IScraper {
  private readonly logger = new Logger(PythonJobsService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    client.setHeaders(PYTHONJOBS_HEADERS);

    this.logger.log(`Fetching Python Jobs RSS feed (resultsWanted=${resultsWanted})`);

    try {
      const response = await client.get(PYTHONJOBS_RSS_URL);
      const xml = response.data;

      if (!xml || typeof xml !== 'string') {
        this.logger.warn('Empty or invalid RSS response from Python Jobs');
        return new JobResponseDto([]);
      }

      const items = this.parseRssItems(xml);
      this.logger.log(`Parsed ${items.length} items from Python Jobs RSS feed`);
      ...
      for (const item of items) {
        ...
        try {
          ...
          this.logger.warn(`Error mapping Python Jobs job ${item.link}: ${err.message}`);
        }
      }

      this.logger.log(`Python Jobs returned ${jobs.length} jobs`);
      return new JobResponseDto(jobs);
    } catch (err: any) {
      this.logger.error(`Python Jobs scrape error: ${err.message}`);
      return new JobResponseDto([], classifyScrapeError(err));
    }
  }

  /**
   * Parse RSS XML into an array of PythonJobsRssItem using regex (no XML library).
   */
  private parseRssItems(xml: string): PythonJobsRssItem[] {
    const items: PythonJobsRssItem[] = [];
   ...
   */
  private matchesSearch(item: PythonJobsRssItem, searchTerm: string): boolean {
