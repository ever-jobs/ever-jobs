...
import { JnjModule, JnjService } from '../src';
...
const FIXTURE: Fixture = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'fixtures', 'jnj-boards.json'), 'utf8'),
);
const COMPANY_NAME = 'Johnson & Johnson';
const ID_PREFIX = 'jnj-';
...
describe('JnjService — Workday delegation (Spec 1736)', () => {
  beforeEach(() => {
    mockGet.mockReset();
    mockPost.mockReset();
  ...
  describe('registration', () => {
    it('resolves through JnjModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({ imports: [JnjModule] }).compile();
      expect(moduleRef.get(JnjService)).toBeInstanceOf(JnjService);
      await moduleRef.close();
    });

    it('exports Site.JNJ = "jnj"', () => {
      expect(Site.JNJ).toBe('jnj');
    });

    it('declares a tagged company plugin with its domains', () => {
      const meta = Reflect.getMetadata(SOURCE_PLUGIN_METADATA, JnjService);
      expect(meta.site).toBe(Site.JNJ);
      expect(meta.category).toBe('company');
      expect(meta.isAts).toBeFalsy();
      expect(meta.companyDomains).toEqual(['jnj.com']);
  ...
  describe('recorded board (real Workday adapter, mocked HTTP)', () => {
    it('maps every recorded posting and re-stamps the company identity', async () => {
      const service = new JnjService(registryWith());
      const result = await service.scrape({ siteType: [Site.JNJ], resultsWanted: 100 } as ScraperInputDto);
    ...
    it('only requests the recorded board URLs', async () => {
      const service = new JnjService(registryWith());
      await service.scrape({ siteType: [Site.JNJ] } as ScraperInputDto);
    ...
    it('honours resultsWanted=1', async () => {
      const service = new JnjService(registryWith());
      const result = await service.scrape({ siteType: [Site.JNJ], resultsWanted: 1 } as ScraperInputDto);
    ...
    it('resolves with an empty result when the board is gone (HTTP 404)', async () => {
      ...
      const service = new JnjService(registryWith());
      const result = await service.scrape({ siteType: [Site.JNJ] } as ScraperInputDto);
  ...
  describe('delegation contract', () => {
    it('forwards the board and the caller input untouched', async () => {
      ...
      expect(result.jobs[0].site).toBe(Site.JNJ);
    ...
    it('never forwards the caller\'s credentials to the board (Spec 1735 §4.5)', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new JnjService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
    ...
    it('makes no request when resultsWanted is 0', async () => {
      const captured: ScraperInputDto[] = [];
      const service = new JnjService(registryWith(fakeBackend(() => new JobResponseDto([]), captured)));
      const result = await service.scrape({ siteType: [Site.JNJ], resultsWanted: 0 } as ScraperInputDto);
  ...
  describe('resilience', () => {
    it('reports not_registered when no registry is injected', async () => {
      const result = await new JnjService().scrape({ siteType: [Site.JNJ] } as ScraperInputDto);
      expect(result.jobs).toEqual([]);
    ...
    it('reports not_registered when Workday is missing from the registry', async () => {
      const result = await new JnjService(new PluginRegistry()).scrape({
        siteType: [Site.JNJ],
      } as ScraperInputDto);
      expect(result.diagnostics?.reason).toBe('not_registered');
    });
