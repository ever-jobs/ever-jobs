...
/**
 * Johnson & Johnson — Healthcare (HQ: New Brunswick, NJ, USA).
 *
 * Source (Spec 1736): Workday board, scraped in this order:
 *   - `jj:5:JJ` — https://jj.wd5.myworkdayjobs.com/JJ
 *     verified live 2026-09-24: 2,069 open postings.
 *
...
const BOARDS: ReadonlyArray<{ readonly companySlug: string; readonly atsIdPrefix: string }> = [
  { companySlug: 'jj:5:JJ', atsIdPrefix: 'wd-jj-' },
];
...
@SourcePlugin({
  ...
  name: COMPANY_NAME,
  category: 'company',
  companyDomains: ['jnj.com'],
  description: 'Johnson & Johnson careers via Workday. Tags: segment=workday-enterprise; industry=healthcare.',
})
@Injectable()
export class JnjService implements IScraper {
  private readonly logger = new Logger(JnjService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!backend) {
      this.logger.error('Workday source plugin is not registered; cannot scrape Johnson & Johnson');
    ...
    for (const board of BOARDS) {
      ...
      this.logger.log(`Johnson & Johnson: delegating to Workday (${board.companySlug})`);
    ...
    this.logger.log(`Johnson & Johnson: scraped ${jobs.length} jobs`);
