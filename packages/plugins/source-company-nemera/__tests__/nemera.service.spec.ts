...
import { NemeraModule, NemeraService } from '../src';

const COMPANY_NAME_EXPECT = 'Nemera';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'nemera-jobs.json'), 'utf8'),
);
...
describe('NemeraService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through NemeraModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [NemeraModule],
      }).compile();
      const service = moduleRef.get(NemeraService);
      expect(service).toBeInstanceOf(NemeraService);
      await moduleRef.close();
    });

    it('exports the Site.NEMERA = "nemera" enum value', () => {
      expect(Site.NEMERA).toBe('nemera');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new NemeraService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.NEMERA],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('Nemera');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.NEMERA);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('nemera-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new NemeraService(registry);
      const result = await service.scrape({
        siteType: [Site.NEMERA],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('Nemera');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('nemera-x1');
      expect(result.jobs[0].site).toBe(Site.NEMERA);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('nemera-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new NemeraService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.NEMERA],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new NemeraService();
      const result = await service.scrape({
        siteType: [Site.NEMERA],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
