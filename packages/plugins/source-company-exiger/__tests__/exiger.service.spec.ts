...
import { ExigerModule, ExigerService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'exiger-jobs.json'), 'utf8'),
);
...
/**
 * Spec 844 / T04 — `ExigerService` unit tests (≥ 9 cases).
 */
describe('ExigerService — Spec 844 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ExigerModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ExigerModule],
      }).compile();
      const service = moduleRef.get(ExigerService);
      expect(service).toBeInstanceOf(ExigerService);
      await moduleRef.close();
    });

    it('exports the Site.EXIGER = "exiger" enum value', () => {
      expect(Site.EXIGER).toBe('exiger');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ExigerService();
      const result = await service.scrape({
        siteType: [Site.EXIGER],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'exiger-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.EXIGER);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Exiger');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/exiger/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/exiger/jobs/');
      ...
      expect(job0?.description).toContain('Exiger');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/exiger/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Exiger');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'exiger-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
