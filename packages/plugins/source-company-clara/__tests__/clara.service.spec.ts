...
import { ClaraModule, ClaraService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'clara-jobs.json'), 'utf8'),
);
...
/**
 * Spec 649 / T04 — `ClaraService` unit tests (≥ 9 cases).
 */
describe('ClaraService — Spec 649 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ClaraModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ClaraModule],
      }).compile();
      const service = moduleRef.get(ClaraService);
      expect(service).toBeInstanceOf(ClaraService);
      await moduleRef.close();
    });

    it('exports the Site.CLARA = "clara" enum value', () => {
      expect(Site.CLARA).toBe('clara');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ClaraService();
      ...
      const job0 = dto.jobs.find((j) => j.id === 'clara-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.CLARA);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Clara');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/clara/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/clara/jobs/');
      ...
      expect(job0?.description).toContain('Clara');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/clara/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Clara');
      }
    });
  });
