...
import { StemXpertModule, StemXpertService } from '../src';

const COMPANY_NAME_EXPECT = 'Stem Xpert';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'stemxpert-jobs.json'), 'utf8'),
);
...
describe('StemXpertService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through StemXpertModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [StemXpertModule],
      }).compile();
      const service = moduleRef.get(StemXpertService);
      expect(service).toBeInstanceOf(StemXpertService);
      await moduleRef.close();
    });

    it('exports the Site.STEM_XPERT = "stemxpert" enum value', () => {
      expect(Site.STEM_XPERT).toBe('stemxpert');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new StemXpertService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.STEM_XPERT],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('StemXpert1');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.STEM_XPERT);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('stemxpert-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new StemXpertService(registry);
      const result = await service.scrape({
        siteType: [Site.STEM_XPERT],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('StemXpert1');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('stemxpert-x1');
      expect(result.jobs[0].site).toBe(Site.STEM_XPERT);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('stemxpert-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new StemXpertService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.STEM_XPERT],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new StemXpertService();
      const result = await service.scrape({
        siteType: [Site.STEM_XPERT],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
