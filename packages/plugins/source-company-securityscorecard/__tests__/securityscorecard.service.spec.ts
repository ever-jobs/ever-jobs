...
import { SecurityScorecardModule, SecurityScorecardService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'securityscorecard-jobs.json'), 'utf8'),
);
...
/**
 * Spec 927 / T04 — `SecurityScorecardService` unit tests (≥ 9 cases).
 */
describe('SecurityScorecardService — Spec 927 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SecurityScorecardModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SecurityScorecardModule],
      }).compile();
      const service = moduleRef.get(SecurityScorecardService);
      expect(service).toBeInstanceOf(SecurityScorecardService);
      await moduleRef.close();
    });

    it('exports the Site.SECURITYSCORECARD = "securityscorecard" enum value', () => {
      expect(Site.SECURITYSCORECARD).toBe('securityscorecard');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new SecurityScorecardService();
      const result = await service.scrape({
        siteType: [Site.SECURITYSCORECARD],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'securityscorecard-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.SECURITYSCORECARD);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('SecurityScorecard');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/securityscorecard/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/securityscorecard/jobs/');
      ...
      expect(job0?.description).toContain('SecurityScorecard');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/securityscorecard/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('SecurityScorecard');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'securityscorecard-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
