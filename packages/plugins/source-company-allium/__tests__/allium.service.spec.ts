...
import { AlliumModule, AlliumService } from '../src';

const COMPANY_NAME_EXPECT = 'Allium';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'allium-jobs.json'), 'utf8'),
);
...
describe('AlliumService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through AlliumModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [AlliumModule],
      }).compile();
      const service = moduleRef.get(AlliumService);
      expect(service).toBeInstanceOf(AlliumService);
      await moduleRef.close();
    });

    it('exports the Site.ALLIUM = "allium" enum value', () => {
      expect(Site.ALLIUM).toBe('allium');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new AlliumService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.ALLIUM],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('allium');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.ALLIUM);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('allium-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new AlliumService(registry);
      const result = await service.scrape({
        siteType: [Site.ALLIUM],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('allium');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('allium-x1');
      expect(result.jobs[0].site).toBe(Site.ALLIUM);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('allium-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new AlliumService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.ALLIUM],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new AlliumService();
      const result = await service.scrape({
        siteType: [Site.ALLIUM],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
