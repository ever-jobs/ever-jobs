...
import { HawkEye360Module, HawkEye360Service } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'hawkeye360-jobs.json'), 'utf8'),
);
...
/**
 * Spec 763 / T04 — `HawkEye360Service` unit tests (≥ 9 cases).
 */
describe('HawkEye360Service — Spec 763 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through HawkEye360Module via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [HawkEye360Module],
      }).compile();
      const service = moduleRef.get(HawkEye360Service);
      expect(service).toBeInstanceOf(HawkEye360Service);
      await moduleRef.close();
    });

    it('exports the Site.HAWKEYE_360 = "hawkeye360" enum value', () => {
      expect(Site.HAWKEYE_360).toBe('hawkeye360');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new HawkEye360Service();
      const result = await service.scrape({
        siteType: [Site.HAWKEYE_360],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'hawkeye360-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.HAWKEYE_360);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('HawkEye 360');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/hawkeye360/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/hawkeye360/jobs/');
      ...
      expect(job0?.description).toContain('HawkEye 360');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/hawkeye360/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('HawkEye 360');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'hawkeye360-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
