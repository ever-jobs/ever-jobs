...
import { ApiiroModule, ApiiroService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'apiiro-jobs.json'), 'utf8'),
);
...
/**
 * Spec 283 / T04 — `ApiiroService` unit tests (≥ 9 cases).
 */
describe('ApiiroService — Spec 283 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ApiiroModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ApiiroModule],
      }).compile();
      const service = moduleRef.get(ApiiroService);
      expect(service).toBeInstanceOf(ApiiroService);
      await moduleRef.close();
    });

    it('exports the Site.APIIRO = "apiiro" enum value', () => {
      expect(Site.APIIRO).toBe('apiiro');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new ApiiroService();
      const result = await service.scrape({
        siteType: [Site.APIIRO],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'apiiro-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.APIIRO);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Apiiro');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/apiiro/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/apiiro/jobs/');
      ...
      expect(job0?.description).toContain('Apiiro');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/apiiro/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Apiiro');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'apiiro-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
