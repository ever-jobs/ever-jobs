...
import { ColumnModule, ColumnService } from '../src';

const COMPANY_NAME_EXPECT = 'Column';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'column-jobs.json'), 'utf8'),
);
...
describe('ColumnService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through ColumnModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [ColumnModule],
      }).compile();
      const service = moduleRef.get(ColumnService);
      expect(service).toBeInstanceOf(ColumnService);
      await moduleRef.close();
    });

    it('exports the Site.COLUMN = "column" enum value', () => {
      expect(Site.COLUMN).toBe('column');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new ColumnService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.COLUMN],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('column');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.COLUMN);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('column-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new ColumnService(registry);
      const result = await service.scrape({
        siteType: [Site.COLUMN],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('column');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('column-x1');
      expect(result.jobs[0].site).toBe(Site.COLUMN);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('column-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new ColumnService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.COLUMN],
      } as ScraperInputDto);
