...
import { SentiLinkModule, SentiLinkService } from '../src';

const COMPANY_NAME_EXPECT = 'SentiLink';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'sentilink-jobs.json'), 'utf8'),
);
...
describe('SentiLinkService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SentiLinkModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SentiLinkModule],
      }).compile();
      const service = moduleRef.get(SentiLinkService);
      expect(service).toBeInstanceOf(SentiLinkService);
      await moduleRef.close();
    });

    it('exports the Site.SENTILINK = "sentilink" enum value', () => {
      expect(Site.SENTILINK).toBe('sentilink');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new SentiLinkService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.SENTILINK],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('sentilink');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.SENTILINK);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('sentilink-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new SentiLinkService(registry);
      const result = await service.scrape({
        siteType: [Site.SENTILINK],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('sentilink');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('sentilink-x1');
      expect(result.jobs[0].site).toBe(Site.SENTILINK);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('sentilink-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new SentiLinkService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.SENTILINK],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new SentiLinkService();
      const result = await service.scrape({
        siteType: [Site.SENTILINK],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
