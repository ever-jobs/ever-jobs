...
import { BlockstreamModule, BlockstreamService } from '../src';

const COMPANY_NAME_EXPECT = 'Blockstream';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'blockstream-jobs.json'), 'utf8'),
);
...
describe('BlockstreamService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through BlockstreamModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [BlockstreamModule],
      }).compile();
      const service = moduleRef.get(BlockstreamService);
      expect(service).toBeInstanceOf(BlockstreamService);
      await moduleRef.close();
    });

    it('exports the Site.BLOCKSTREAM = "blockstream" enum value', () => {
      expect(Site.BLOCKSTREAM).toBe('blockstream');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new BlockstreamService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.BLOCKSTREAM],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'blockstream-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Ashby's defaults
      expect(job0?.site).toBe(Site.BLOCKSTREAM);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('blockstream-' + first.id);
      ...
      expect(calledUrls[0]).toContain('blockstream');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.BLOCKSTREAM);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('blockstream-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new BlockstreamService(registry);
      const result = await service.scrape({
        siteType: [Site.BLOCKSTREAM],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('blockstream');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('blockstream-x1');
      expect(result.jobs[0].site).toBe(Site.BLOCKSTREAM);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('blockstream-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new BlockstreamService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.BLOCKSTREAM],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new BlockstreamService();
      const result = await service.scrape({
        siteType: [Site.BLOCKSTREAM],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
