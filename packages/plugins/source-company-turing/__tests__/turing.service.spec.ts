...
import { TuringModule, TuringService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'turing-jobs.json'), 'utf8'),
);
...
/**
 * Spec 466 / T04 — `TuringService` unit tests (≥ 9 cases).
 */
describe('TuringService — Spec 466 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through TuringModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [TuringModule],
      }).compile();
      const service = moduleRef.get(TuringService);
      expect(service).toBeInstanceOf(TuringService);
      await moduleRef.close();
    });

    it('exports the Site.TURING = "turing" enum value', () => {
      expect(Site.TURING).toBe('turing');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new TuringService();
      const result = await service.scrape({
        siteType: [Site.TURING],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'turing-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.TURING);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Turing');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/turing/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/turing/jobs/');
      ...
      expect(job0?.description).toContain('Turing');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/turing/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Turing');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'turing-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
