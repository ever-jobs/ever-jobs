...
import { OrcaSecurityModule, OrcaSecurityService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'orcasecurity-jobs.json'), 'utf8'),
);
...
/**
 * Spec 903 / T04 — `OrcaSecurityService` unit tests (≥ 9 cases).
 */
describe('OrcaSecurityService — Spec 903 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through OrcaSecurityModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [OrcaSecurityModule],
      }).compile();
      const service = moduleRef.get(OrcaSecurityService);
      expect(service).toBeInstanceOf(OrcaSecurityService);
      await moduleRef.close();
    });

    it('exports the Site.ORCA_SECURITY = "orcasecurity" enum value', () => {
      expect(Site.ORCA_SECURITY).toBe('orcasecurity');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new OrcaSecurityService();
      const result = await service.scrape({
        siteType: [Site.ORCA_SECURITY],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'orcasecurity-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.ORCA_SECURITY);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Orca Security');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/orcasecurity/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/orcasecurity/jobs/');
      ...
      expect(job0?.description).toContain('Orca Security');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/orcasecurity/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Orca Security');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'orcasecurity-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
