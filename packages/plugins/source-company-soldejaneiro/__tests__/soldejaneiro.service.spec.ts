...
import { SolDeJaneiroModule, SolDeJaneiroService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'soldejaneiro-jobs.json'), 'utf8'),
);
...
/**
 * Spec 936 / T04 — `SolDeJaneiroService` unit tests (≥ 9 cases).
 */
describe('SolDeJaneiroService — Spec 936 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through SolDeJaneiroModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [SolDeJaneiroModule],
      }).compile();
      const service = moduleRef.get(SolDeJaneiroService);
      expect(service).toBeInstanceOf(SolDeJaneiroService);
      await moduleRef.close();
    });

    it('exports the Site.SOL_DE_JANEIRO = "soldejaneiro" enum value', () => {
      expect(Site.SOL_DE_JANEIRO).toBe('soldejaneiro');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new SolDeJaneiroService();
      const result = await service.scrape({
        siteType: [Site.SOL_DE_JANEIRO],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'soldejaneiro-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.SOL_DE_JANEIRO);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Sol de Janeiro');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/soldejaneiro/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/soldejaneiro/jobs/');
      ...
      expect(job0?.description).toContain('Sol de Janeiro');

      const calledUrls = mockGet.mock.calls.map((c) => c[0] as string);
      expect(calledUrls[0]).toBe(
        'https://api.greenhouse.io/v1/boards/soldejaneiro/jobs?content=true',
      );
    });
  });
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Sol de Janeiro');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'soldejaneiro-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
