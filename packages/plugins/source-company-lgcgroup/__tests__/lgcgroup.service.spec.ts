...
import { LGCGroupModule, LGCGroupService } from '../src';

const COMPANY_NAME_EXPECT = 'LGC Group';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// SmartRecruiters' public Posting API returns a { content: [...] } envelope.
const JOBS_ENVELOPE_RAW: { content: any[] } = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'lgcgroup-jobs.json'), 'utf8'),
);
...
describe('LGCGroupService — SmartRecruiters delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through LGCGroupModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [LGCGroupModule],
      }).compile();
      const service = moduleRef.get(LGCGroupService);
      expect(service).toBeInstanceOf(LGCGroupService);
      await moduleRef.close();
    });

    it('exports the Site.LGC_GROUP = "lgcgroup" enum value', () => {
      expect(Site.LGC_GROUP).toBe('lgcgroup');
    });
  });
  ...
  describe('happy path (delegates to the registered SmartRecruiters plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new LGCGroupService(registryWithSmartRecruiters());
      const result = (await service.scrape({
        siteType: [Site.LGC_GROUP],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('LGCGroup');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.LGC_GROUP);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('lgcgroup-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the SmartRecruiters scraper', async () => {
      ...
      const service = new LGCGroupService(registry);
      const result = await service.scrape({
        siteType: [Site.LGC_GROUP],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('LGCGroup');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('lgcgroup-x1');
      expect(result.jobs[0].site).toBe(Site.LGC_GROUP);
    });
    ...
    it('only rewrites a leading sr- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('lgcgroup-sr-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no SmartRecruiters plugin is registered', async () => {
      const service = new LGCGroupService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.LGC_GROUP],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new LGCGroupService();
      const result = await service.scrape({
        siteType: [Site.LGC_GROUP],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
