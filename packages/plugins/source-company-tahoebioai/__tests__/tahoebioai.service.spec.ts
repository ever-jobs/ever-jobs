...
import { TahoeTherapeuticsModule, TahoeTherapeuticsService } from '../src';

const COMPANY_NAME_EXPECT = 'Tahoe Therapeutics';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
// Lever's public Postings API returns a bare array of postings.
const JOBS_PAGE_RAW: any[] = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'tahoebioai-jobs.json'), 'utf8'),
);
...
describe('TahoeTherapeuticsService — Lever delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through TahoeTherapeuticsModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [TahoeTherapeuticsModule],
      }).compile();
      const service = moduleRef.get(TahoeTherapeuticsService);
      expect(service).toBeInstanceOf(TahoeTherapeuticsService);
      await moduleRef.close();
    });

    it('exports the Site.TAHOE_THERAPEUTICS = "tahoebioai" enum value', () => {
      expect(Site.TAHOE_THERAPEUTICS).toBe('tahoebioai');
    });
  });
  ...
  describe('happy path (delegates to the registered Lever plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new TahoeTherapeuticsService(registryWithLever());
      const result = (await service.scrape({
        siteType: [Site.TAHOE_THERAPEUTICS],
        resultsWanted: 100,
      ...
      const job0 = result.jobs.find(
        (j) => j.id === 'tahoebioai-' + first.id,
      );
      expect(job0).toBeDefined();
      // company identity is re-stamped over Lever's defaults
      expect(job0?.site).toBe(Site.TAHOE_THERAPEUTICS);
      expect(job0?.companyName).toBe(COMPANY_NAME_EXPECT);
      expect(job0?.id).toBe('tahoebioai-' + first.id);
      ...
      expect(calledUrls[0]).toContain('tahoebio-ai');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.TAHOE_THERAPEUTICS);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('tahoebioai-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Lever scraper', async () => {
      ...
      const service = new TahoeTherapeuticsService(registry);
      const result = await service.scrape({
        siteType: [Site.TAHOE_THERAPEUTICS],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('tahoebio-ai');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('tahoebioai-x1');
      expect(result.jobs[0].site).toBe(Site.TAHOE_THERAPEUTICS);
    });
    ...
    it('only rewrites a leading lever- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('tahoebioai-lever-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Lever plugin is registered', async () => {
      const service = new TahoeTherapeuticsService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.TAHOE_THERAPEUTICS],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new TahoeTherapeuticsService();
      const result = await service.scrape({
        siteType: [Site.TAHOE_THERAPEUTICS],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
