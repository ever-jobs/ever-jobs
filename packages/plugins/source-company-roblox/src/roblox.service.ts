...
/**
 * Roblox publishes its careers board through Greenhouse at the bare
 * `roblox` slug (no asymmetry; see Spec 041 § 10 D-05).
 */
const API_URL = 'https://api.greenhouse.io/v1/boards/roblox/jobs';
...
export class RobloxService implements IScraper {
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    try {
      ...
      this.logger.log(`Roblox: fetching ${url}`);
        ...
        jobs.push(
          new JobPostDto({
            ...
            jobUrl:
              ...
              `https://careers.roblox.com/jobs/${listing.id}?gh_jid=${listing.id}`,
            location,
      ...
      this.logger.log(`Roblox: scraped ${jobs.length} jobs`);
    } catch (err: any) {
      this.logger.error(`Roblox scrape failed: ${err.message}`);
