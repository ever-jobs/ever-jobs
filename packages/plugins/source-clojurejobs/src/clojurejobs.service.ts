...
@SourcePlugin({
  site: Site.CLOJUREJOBS,
  name: 'ClojureJobs',
  category: 'niche',
})
@Injectable()
export class ClojurejobsService implements IScraper {
  private readonly logger = new Logger(ClojurejobsService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    client.setHeaders(CLOJUREJOBS_HEADERS);

    this.logger.log(
      `Fetching ClojureJobs RSS feed (resultsWanted=${resultsWanted})`,
    );

    try {
      const response = await client.get(CLOJUREJOBS_RSS_URL);
      const xml = response.data;

      if (!xml || typeof xml !== 'string') {
        this.logger.warn('Empty or invalid RSS response from ClojureJobs');
        return new JobResponseDto([]);
      }

      const items = this.parseRssItems(xml);
      this.logger.log(`Parsed ${items.length} items from ClojureJobs RSS feed`);
      ...
      for (const item of items) {
        ...
        try {
          ...
          this.logger.warn(
            `Error mapping ClojureJobs job ${item.link}: ${err.message}`,
          );
        }
      }

      this.logger.log(`ClojureJobs returned ${jobs.length} jobs`);
      return new JobResponseDto(jobs);
    } catch (err: any) {
      this.logger.error(`ClojureJobs scrape error: ${err.message}`);
      return new JobResponseDto([]);
    }
  }

  /**
   * Parse RSS XML into an array of ClojurejobsRssItem using regex (no XML library).
   */
  private parseRssItems(xml: string): ClojurejobsRssItem[] {
    const items: ClojurejobsRssItem[] = [];
   ...
   */
  private matchesSearch(item: ClojurejobsRssItem, searchTerm: string): boolean {
   ...
   */
  private mapJob(
    item: ClojurejobsRssItem,
    descriptionFormat?: DescriptionFormat,
  ): JobPostDto | null {
    if (!item.title || !item.link) return null;

    // Process description (ClojureJobs RSS may return HTML)
