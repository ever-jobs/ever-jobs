...
/**
 * Starface World — Starface World is an employer in the DTC beauty/skincare brand sector, headquartered in New York, New York, USA..
 *
 ...
 *
 * Highlights:
 *   - Sector: DTC beauty/skincare brand
 ...
 *     Jobs schema
 *
 * Source profile (Spec 943):
 *   - D-04 — Greenhouse canonical hosted-board host (variant 2):
 *     `https://job-boards.greenhouse.io/starfaceworld/jobs/<id>`.
 *   - D-08 — entity-decode-then-tag-strip description pipeline.
 *   - D-09 — wire `company_name` pass-through (`'Starface World'`).
 ...
 */
const API_URL = 'https://api.greenhouse.io/v1/boards/starfaceworld/jobs';

@SourcePlugin({
  site: Site.STARFACE_WORLD,
  name: 'Starface World',
  category: 'company',
})
@Injectable()
export class StarfaceWorldService implements IScraper {
  private readonly logger = new Logger(StarfaceWorldService.name);
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    try {
      ...
      this.logger.log(`Starface World: fetching ${url}`);
      ...
      for (const listing of listings) {
        ...
        jobs.push(
          new JobPostDto({
            ...
            companyName: listing.company_name ?? 'Starface World',
            // D-04: wire `absolute_url` flows through (variant 2).
            jobUrl:
              listing.absolute_url ??
              `https://job-boards.greenhouse.io/starfaceworld/jobs/${listing.id}`,
            location,
      ...
      this.logger.log(`Starface World: scraped ${jobs.length} jobs`);
    } catch (err: any) {
      this.logger.error(`Starface World scrape failed: ${err.message}`);
