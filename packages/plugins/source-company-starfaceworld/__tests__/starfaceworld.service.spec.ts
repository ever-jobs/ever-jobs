...
import { StarfaceWorldModule, StarfaceWorldService } from '../src';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'starfaceworld-jobs.json'), 'utf8'),
);
...
/**
 * Spec 943 / T04 — `StarfaceWorldService` unit tests (≥ 9 cases).
 */
describe('StarfaceWorldService — Spec 943 / T04', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through StarfaceWorldModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [StarfaceWorldModule],
      }).compile();
      const service = moduleRef.get(StarfaceWorldService);
      expect(service).toBeInstanceOf(StarfaceWorldService);
      await moduleRef.close();
    });

    it('exports the Site.STARFACE_WORLD = "starfaceworld" enum value', () => {
      expect(Site.STARFACE_WORLD).toBe('starfaceworld');
    });
  });
  ...
  describe('happy path', () => {
    it('maps all fixture listings to JobPostDto', async () => {
      ...
      const service = new StarfaceWorldService();
      const result = await service.scrape({
        siteType: [Site.STARFACE_WORLD],
        resultsWanted: 100,
      } as ScraperInputDto);
      ...
      const job0 = dto.jobs.find((j) => j.id === 'starfaceworld-' + first.id);
      expect(job0).toBeDefined();
      expect(job0?.site).toBe(Site.STARFACE_WORLD);
      // D-09 wire company_name pass-through.
      expect(job0?.companyName).toBe('Starface World');
      // D-10 title-trim lock.
      expect(job0?.title).toBe(String(first.title).trim());
      expect(job0?.title).not.toMatch(/\s$/);
      // D-04 variant-2 URL.
      expect(job0?.jobUrl).toBe(
        'https://job-boards.greenhouse.io/starfaceworld/jobs/' + first.id,
      );
      expect(job0?.jobUrl).toContain('job-boards.greenhouse.io/starfaceworld/jobs/');
      ...
      expect(job0?.description).toContain('Starface World');
  ...
  describe('company_name pass-through', () => {
    it('emits the wire company_name for every job', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.companyName).toBe('Starface World');
      }
    });
  });
  ...
  describe('searchTerm filter', () => {
    it('filters by case-insensitive substring of title', async () => {
      ...
      expect(result.jobs.map((j) => j.id)).toContain(
        'starfaceworld-' + JOBS_PAGE_RAW.jobs[0].id,
      );
    });
