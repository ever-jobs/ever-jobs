...
import { OscilarModule, OscilarService } from '../src';

const COMPANY_NAME_EXPECT = 'Oscilar';

const FIXTURE_DIR = path.join(__dirname, 'fixtures');
const JOBS_PAGE_RAW = JSON.parse(
  fs.readFileSync(path.join(FIXTURE_DIR, 'oscilar-jobs.json'), 'utf8'),
);
...
describe('OscilarService — Ashby delegation', () => {
  beforeEach(() => {
    mockGet.mockReset();
  });

  describe('registration scaffolding', () => {
    it('resolves through OscilarModule via NestJS DI', async () => {
      const moduleRef = await Test.createTestingModule({
        imports: [OscilarModule],
      }).compile();
      const service = moduleRef.get(OscilarService);
      expect(service).toBeInstanceOf(OscilarService);
      await moduleRef.close();
    });

    it('exports the Site.OSCILAR = "oscilar" enum value', () => {
      expect(Site.OSCILAR).toBe('oscilar');
    });
  });
  ...
  describe('happy path (delegates to the registered Ashby plugin)', () => {
    it('maps all fixture listings to JobPostDto with the company identity', async () => {
      ...
      const service = new OscilarService(registryWithAshby());
      const result = (await service.scrape({
        siteType: [Site.OSCILAR],
        resultsWanted: 100,
      ...
      expect(calledUrls[0]).toContain('oscilar');
    ...
    it('every job carries the company site, companyName, and id prefix', async () => {
      ...
      for (const job of result.jobs) {
        expect(job.site).toBe(Site.OSCILAR);
        expect(job.companyName).toBe(COMPANY_NAME_EXPECT);
        expect(job.id?.startsWith('oscilar-')).toBe(true);
      }
    });
  });
  ...
  describe('input pass-through', () => {
    it('forwards the company slug and caller input to the Ashby scraper', async () => {
      ...
      const service = new OscilarService(registry);
      const result = await service.scrape({
        siteType: [Site.OSCILAR],
        resultsWanted: 7,
      } as ScraperInputDto);
      ...
      expect(captured[0].companySlug).toBe('oscilar');
      expect(captured[0].resultsWanted).toBe(7);
      expect(result.jobs[0].id).toBe('oscilar-x1');
      expect(result.jobs[0].site).toBe(Site.OSCILAR);
    });
    ...
    it('only rewrites a leading ashby- id prefix', async () => {
      ...
      expect(result.jobs[0].id).toBe('oscilar-ashby-7');
    });
  });

  describe('resilience', () => {
    it('returns an empty response when no Ashby plugin is registered', async () => {
      const service = new OscilarService(new PluginRegistry());
      const result = await service.scrape({
        siteType: [Site.OSCILAR],
      } as ScraperInputDto);
    ...
    it('returns an empty response when no registry is injected', async () => {
      const service = new OscilarService();
      const result = await service.scrape({
        siteType: [Site.OSCILAR],
      } as ScraperInputDto);
      expect(result.jobs).toHaveLength(0);
